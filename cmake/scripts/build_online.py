...
class BuildOnline:
    ...
    @staticmethod
    def build_calculator(build_job_num: int = 4):
        builder = BuildOnline()

        # 路径准备
        src = Path(Path(__file__).parent.resolve(), "calculator")
        build_dir = Path(Path(__file__).parent.resolve(), f"calculator/{builder.py_mod_torch_version}")
        if build_dir.exists():
        ...
        # CMake Configure
        cmd = f"{builder.cmake} -S {src} -B {build_dir}"
