...
def inverse_pto(attn: pypto.Tensor, eye: pypto.Tensor, size: int, zeros_16, zeros_32, zeros_64) -> pypto.Tensor:
    ...
    attn_1_1 = attn.view([min_length, min_length], [0, 0]) + 0.0
    attn_2_2 = attn.view([min_length, min_length], [min_length, min_length]) + 0.0
    attn_3_3 = attn.view([min_length, min_length], [min_length*2, min_length*2]) + 0.0
    attn_4_4 = attn.view([min_length, min_length], [min_length*3, min_length*3]) + 0.0
    attn_5_5 = attn.view([min_length, min_length], [min_length*4, min_length*4]) + 0.0
    attn_6_6 = attn.view([min_length, min_length], [min_length*5, min_length*5]) + 0.0
    attn_7_7 = attn.view([min_length, min_length], [min_length*6, min_length*6]) + 0.0
    attn_8_8 = attn.view([min_length, min_length], [min_length*7, min_length*7]) + 0.0
    # pypto.set_pass_options(sg_set_scope=1)
    attn_tmp_dim1 = pypto.concat([attn_1_1, attn_2_2, attn_3_3, attn_4_4, attn_5_5, attn_6_6, attn_7_7, attn_8_8], dim=1)
    attn_tmp_dim0 = pypto.concat([attn_1_1, attn_2_2, attn_3_3, attn_4_4, attn_5_5, attn_6_6, attn_7_7, attn_8_8], dim=0)

    attn_tmp_dim1_inv = inverse_pto_min_length(attn_tmp_dim0, attn_tmp_dim1, eye, min_length, min_length*8)
    attn_1_1_inv = attn_tmp_dim1_inv[:, :min_length] + 0.0
    attn_2_2_inv = attn_tmp_dim1_inv[:, min_length:min_length*2] + 0.0
    attn_3_3_inv = attn_tmp_dim1_inv[:, min_length*2:min_length*3] + 0.0
    attn_4_4_inv = attn_tmp_dim1_inv[:, min_length*3:min_length*4] + 0.0
    attn_5_5_inv = attn_tmp_dim1_inv[:, min_length*4:min_length*5] + 0.0
    attn_6_6_inv = attn_tmp_dim1_inv[:, min_length*5:min_length*6] + 0.0
    attn_7_7_inv = attn_tmp_dim1_inv[:, min_length*6:min_length*7] + 0.0
    attn_8_8_inv = attn_tmp_dim1_inv[:, min_length*7:min_length*8] + 0.0
    pypto.set_pass_options(sg_set_scope=-1)

    # pypto.set_pass_options(sg_set_scope=3)
    attn_1_inv = inverse_matmul(attn=attn, attn_1_1_inv=attn_1_1_inv, attn_2_2_inv=attn_2_2_inv, x_ofs=0, y_ofs=0,  m_len=min_length, zero_tensor=zeros_16)
    attn_2_inv = inverse_matmul(attn=attn, attn_1_1_inv=attn_3_3_inv, attn_2_2_inv=attn_4_4_inv, x_ofs=min_length*2, y_ofs=min_length*2,  m_len=min_length, zero_tensor=zeros_16)
    attn_3_inv = inverse_matmul(attn=attn, attn_1_1_inv=attn_5_5_inv, attn_2_2_inv=attn_6_6_inv, x_ofs=min_length*4, y_ofs=min_length*4,  m_len=min_length, zero_tensor=zeros_16)
    attn_4_inv = inverse_matmul(attn=attn, attn_1_1_inv=attn_7_7_inv, attn_2_2_inv=attn_8_8_inv, x_ofs=min_length*6, y_ofs=min_length*6,  m_len=min_length, zero_tensor=zeros_16)

    attn_1_inv = inverse_matmul(attn=attn, attn_1_1_inv=attn_1_inv, attn_2_2_inv=attn_2_inv, x_ofs=0, y_ofs=0,  m_len=min_length*2, zero_tensor=zeros_32)
    attn_2_inv = inverse_matmul(attn=attn, attn_1_1_inv=attn_3_inv, attn_2_2_inv=attn_4_inv, x_ofs=min_length*4, y_ofs=min_length*4,  m_len=min_length*2, zero_tensor=zeros_32)

    attn_inv = inverse_matmul(attn=attn, attn_1_1_inv=attn_1_inv, attn_2_2_inv=attn_2_inv, x_ofs=0, y_ofs=0,  m_len=min_length*4, zero_tensor=zeros_64)
    # pypto.set_pass_options(sg_set_scope=-1)
...
def chunk_gated_delta_rule(nqk, nv, d, l):
    ...
    def kernel(
        ...
        for b_idx in pypto.loop(b, name="LOOP_B_TND", idx_name="b_idx"):
            ...
            for nv_idx in pypto.loop(nv, name="LOOP_Nv_TND", idx_name="nv_idx"): # , parallel_for=True
                nqk_idx = nv_idx // group
                pypto.set_vec_tile_shapes(16, 16, 128, 128)
                last_state = states[b_idx, nv_idx]
 
                for s_idx in pypto.loop(0, s, l, name="LOOP_S_TND", idx_name="s_idx", submit_before_loop=True): #, unroll_list=[16, 1]
...
def pypto_chunk_gated_delta_rule(
    ...
    chunk_gated_delta_rule(Nqk, Nv, D, L)(*inputs, *outputs)
...
def test_chunk_gated_delta_rule():
    ...
    # query_data = torch.load('/home/r00949563/pypto_models/pypto-dev2/examples/models/qwen3next/dump6/query_ts20260122_204446_758500_rank0_pid288991.pt')[:T].to(device=device_id).float()
    # key_data = torch.load('/home/r00949563/pypto_models/pypto-dev2/examples/models/qwen3next/dump6/key_ts20260122_204446_929134_rank0_pid288991.pt')[:T].to(device=device_id).float()
    # value_data = torch.load('/home/r00949563/pypto_models/pypto-dev2/examples/models/qwen3next/dump6/value_ts20260122_204447_097421_rank0_pid288991.pt')[:T].to(device=device_id).float()
    # beta_data = torch.load('/home/r00949563/pypto_models/pypto-dev2/examples/models/qwen3next/dump6/beta_ts20260122_204447_451610_rank0_pid288991.pt')[:T].to(device=device_id).float()
    # gate_data  = torch.load('/home/r00949563/pypto_models/pypto-dev2/examples/models/qwen3next/dump6/g_ts20260122_204447_445845_rank0_pid288991.pt')[:T].to(device=device_id).float()
    # states_data = torch.load('/home/r00949563/pypto_models/pypto-dev2/examples/models/qwen3next/dump6/recurrent_state_ts20260122_204447_454583_rank0_pid288991.pt').to(device=device_id).float()
