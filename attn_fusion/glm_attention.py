...
def attention(
    ...
    index_tmp = torch.empty((b,), dtype=torch.int32, device=device)
    ...
    return index_tmp, residual_tmp, key_cache
...
def ifa_func(b, bs, n1, hidden_size, total_head_size, head_size, half_rotary_dim, 
    ...
    def kernel(
        ...
        index_tmp: pypto.Tensor((b, ), pypto.DT_INT32)
    ):
            # 3. 定义基本变量
        bs_tile = 8
        ...
        for bs_idx in pypto.loop(bs_loop, name="LOOP_ATT_PRE_L0", idx_name="bs_idx"):
            ...
            # for tmp_idx in pypto.loop(bs_tile, name="LOOP_TMP_IDX", idx_name="tmp_idx"):
                pypto.set_vec_tile_shapes(1, vec_tile_value)
            ...
            index_cast = pypto.cast(index_view, pypto.DT_INT32)
            pypto.assemble(index_cast, [b_ofs], index_tmp)
        ...
        # bs_tile = 1
        # bs_num = b_scalar * s1_scalar // bs_tile
        # q_2d_shape = (b_scalar * s1_scalar, n1, dn)
        # q_2d = pypto.reshape(q_tmp, q_2d_shape, inplace=True)
        # for bs_idx in pypto.loop(bs_num):
        #     q_2d_i = pypto.view(q_2d, [bs_tile, n1, dn], [bs_idx, 0, 0])
        #     pypto.assemble(q_2d_i, [bs_idx, 0, 0], atten_out)
        # q_2d = pypto.reshape(q_tmp, q_2d_shape, inplace=True)
        ...
        #                             pypto.reshape(qi, [1, g_tile, dn]), dtype)
...
def test_attention():
    ...
    for _ in range(loop_times):
        output, residual_tmp, key_cache = attention(
    ...
    print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&", output)
    attention_output, residual_g, key_golden = attn_golden.attention_golden(
        hidden_states=hidden_states,
        residual=residual,
        input_layernorm_weight=input_layernorm_weight,
        input_layernorm_bias=input_layernorm_bias,
    ...
    # compare(np.array(key_golden.flatten().tolist()), np.array(key_cache.flatten().tolist()),
    #         "key_cache", rtol=0.003, atol=0.003)
