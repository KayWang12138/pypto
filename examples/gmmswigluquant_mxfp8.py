...
def compute_gloden_result(x, weight, scaled_x_gloden, scaled_weight_gloden, a_trans, b_trans):
    # 转置处理
    if a_trans:
        x = np.swapaxes(x, -1, -2)
        scaled_x_gloden = np.swapaxes(scaled_x_gloden, -1, -2)
        if len(scaled_x_gloden) == 3:
            scaled_x_gloden = scaled_x_gloden.reshape(scaled_x_gloden[0] * scaled_x_gloden[1], scaled_x_gloden[2])
        scaled_x_gloden = np.swapaxes(scaled_x_gloden, -1, -2)
    else:
        if len(scaled_x_gloden) == 3:
            scaled_x_gloden = scaled_x_gloden.reshape(scaled_x_gloden[0] * scaled_x_gloden[1], scaled_x_gloden[2])
    if b_trans:
        ...
        if len(scaled_weight_gloden) == 3:
            scaled_weight_gloden = scaled_weight_gloden.reshape(scaled_weight_gloden[0] * scaled_weight_gloden[1], scaled_weight_gloden[2])
    ...
    for _ in range(x1_dims - 1):
        x1_pad += [0,0]
    x1_gloden = torch.nn.functional.pad(x1, x1_pad, mode='constant', value = 0)

    weight_pad = [0,0]
    weight_pad += [0, x2_pad_len]
    for _ in range(x2_dims - 1):
        weight_pad += [0,0]
    weight_gloden = torch.nn.functional.pad(weight, weight_pad, mode='constant', value = 0)
    ...
    weight_gloden = weight_fp32 * scaled_weight_gloden_broadcast_fp32

    gmm_out = torch.matmul(x1_gloden, weight_gloden)
...
def quant_pertoken(swiglu_out):
    ...
    round_data_clamped = troch.clamp(round_data, min=-127, max=127)
...
def gen_gloden(a, b, scaled_a, scaled_b, group_list, a_trans, b_trans):
    round = b.shape[0]
    ...
    for i in range(round):
        ...
        if a_trans:
            ...
            scaled_x_gloden = scaled_a[,egin:end,:,:]
        scaled_weight_gloden = scaled_b[i]

        golden_temp, golden_temp_quant = compute_gloden_result(
            x=x,
            weight=weight,
    ...
    gmmswigluquant_output = torch.cat(golden_temp_quant, dim=0)
...
def test_gmm_mxfp8(params):
    ...
    gloden, gloden_quant = gen_gloden(a, b, scaled_a, scaled_b, group_list, tile_config)
