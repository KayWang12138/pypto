...
from ffn_module_impl import (
    FFNConfig,
    ffn_static_gule_kernel,
    ffn_static_relu_kernel,
    ffn_static_swiglu_kernel,
    ffn_dynamic_gelu_kernel
)
...
def ffn(hidden_states_torch: torch.Tensor, gate_proj_weight_torch: torch.Tensor, up_proj_weight_torch: torch.Tensor, down_proj_weight_torch: torch.Tensor, config: FFNConfig, use_dynamic: bool) -> torch.Tensor:
    ...
    if use_dynamic == False:
        if config.activation == "gelu":
            ffn_static_gule_kernel(hidden_states, gate_proj_weight,  down_proj_weight, output, config)
        if config.activation == "swiglu":
            ffn_static_swiglu_kernel(hidden_states, gate_proj_weight, up_proj_weight, down_proj_weight, output, config)
        if config.activation == "relu":
            ffn_static_relu_kernel(hidden_states, gate_proj_weight, down_proj_weight, output, config)
    elif use_dynamic == True:
        ffn_dynamic_gelu_kernel(hidden_states, gate_proj_weight, down_proj_weight, output, config)
...
def test_ffn_static_gelu():
    ...
    hidden_states_torch = torch.randn(batch_size, hidden_size, dtype=dtype, device=f'npu:{device_id}') / math.sqrt(batch_size)
    gate_proj_weight_torch = torch.randn(hidden_size, intermediate_size, dtype=dtype, device=f'npu:{device_id}') / math.sqrt(batch_size)
    up_proj_weight_torch = torch.randn(hidden_size, intermediate_size, dtype=dtype, device=f'npu:{device_id}') / math.sqrt(batch_size)
    down_proj_weight_torch = torch.randn(intermediate_size, hidden_size, dtype=dtype, device=f'npu:{device_id}') / math.sqrt(batch_size)
    ...
    output = ffn(hidden_states_torch, gate_proj_weight_torch, up_proj_weight_torch, down_proj_weight_torch, config, False)
...
def test_ffn_dynamic_gelu():
    ...
    output = ffn(hidden_states_torch, gate_proj_weight_torch, up_proj_weight_torch, down_proj_weight_torch, config, True)
    assert_allclose(output.cpu().to(torch.float32), output_torch_ref.cpu().to(torch.float32), rtol=3e-3, atol=3e-3)
