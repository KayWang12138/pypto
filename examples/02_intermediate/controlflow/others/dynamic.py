...
def scaled_dot_product_attention(
    ...
    @pypto.frontend.jit()
    def scaled_dot_product_attention_kernel_npu(
    ...
    def scaled_dot_product_attention_kernel_sim(
        q: pypto.Tensor((B, NUM_HEADS, Q_LEN, HEAD_DIM), pypto.DT_FP32),
        k: pypto.Tensor((B, NUM_HEADS, KV_LEN, HEAD_DIM), pypto.DT_FP32),
        v: pypto.Tensor((B, NUM_HEADS, KV_LEN, HEAD_DIM), pypto.DT_FP32),
    ) -> pypto.Tensor((B, NUM_HEADS, Q_LEN, HEAD_DIM), pypto.DT_FP32):
        ...
        for bs_idx in pypto.loop(b_loop):
            b_offset = bs_idx * TILE_B
            b_offset_end = min(b_offset + TILE_B, B)
            q_view = q[b_offset:b_offset_end, :NUM_HEADS, :Q_LEN, :HEAD_DIM]
            k_view = k[b_offset:b_offset_end, :NUM_HEADS, :KV_LEN, :HEAD_DIM]
            v_view = v[b_offset:b_offset_end, :NUM_HEADS, :KV_LEN, :HEAD_DIM]
            pypto.set_vec_tile_shapes(1, 8, 16, 64)
            res = scaled_dot_product_attention_core(
                q_view, k_view, v_view, SCALE, config.dtype
            )
    ...
    if run_mode == "npu":
        return scaled_dot_product_attention_kernel_npu(q, k, v)
    else:
        return scaled_dot_product_attention_kernel_sim(q, k, v)


def test_dynamic_shape(
    device_id=None, run_mode: str = "npu", dynamic: bool = True
) -> None:
    ...
    for batch_size, seq_len_q, seq_len_kv in test_cases:
        dtype = torch.float32
        q_torch = torch.randn(
            batch_size, num_heads, seq_len_q, head_dim, dtype=dtype, device=device
        )
        k_torch = torch.randn(
            batch_size, num_heads, seq_len_kv, head_dim, dtype=dtype, device=device
        )
        v_torch = torch.randn(
            batch_size, num_heads, seq_len_kv, head_dim, dtype=dtype, device=device
        )
        ...
        out_torch = scaled_dot_product_attention(
            q_torch, k_torch, v_torch, config, run_mode, dynamic
        ).cpu()
