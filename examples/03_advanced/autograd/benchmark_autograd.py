...
def benchmark_linear(batch: int = 32, in_dim: int = 512, out_dim: int = 256,
    ...
    def linear_fwd(x: pypto.Tensor, w: pypto.Tensor, out: pypto.Tensor, loss: pypto.Tensor):
        ...
        r = pypto.matmul(x, w, pypto.DT_FP32)
        out[:] = r  # Note: r is moved to out
        # Use out instead of r for sum, since out now contains the matmul result
        l = pypto.sum(pypto.sum(out, dim=-1, keepdim=True), dim=0, keepdim=True)
        loss[:] = l
...
def benchmark_mlp(batch: int = 32, dims: List[int] = [512, 256, 128],
    ...
    def mlp_fwd(x: pypto.Tensor, w0: pypto.Tensor, w1: pypto.Tensor,
        ...
        pypto.set_cube_tile_shapes([32, 32], [64, 64], [64, 64])
        pypto.set_vec_tile_shapes(32, 32)
        h = pypto.matmul(pypto.matmul(x, w0, pypto.DT_FP32), w1, pypto.DT_FP32)
        out[:] = h  # Note: h is moved to out
        # Use out instead of h for sum, since out now contains the matmul result
        l = pypto.sum(pypto.sum(out, dim=-1, keepdim=True), dim=0, keepdim=True)
        loss[:] = l
