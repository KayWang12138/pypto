...
def matmul_all_reduce_addrmsnorm_kernel(bs, ne, h_num, eps, group_name) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def kernel(
        ...
        weight: pypto.Tensor((h_num, ne), pypto.DT_BF16),
    ) -> (
        pypto.Tensor((bs, h_num), pypto.DT_BF16),
        pypto.Tensor((bs, h_num), pypto.DT_BF16)
    ):
        ...
        for bs_idx in pypto.loop(bs_loop, name="LOOP_MM_ALLREDUCE_ADDRMSNORM", idx_name="bs_idx"):
            # 1. CreateShmemTensor
            shmem_data = pypto.tensor((WORLD_SIZE, 1, view_row_shape, h_num), pypto.DT_FP32)
            shmem_signal = pypto.tensor((WORLD_SIZE, WORLD_SIZE, 1, view_row_shape, h_num), pypto.DT_INT32)
            for _ in pypto.loop(1, name="CREATE_SHMEM_TENSOR", idx_name = "_"):
                pypto.distributed.create_shmem_data(group_name, WORLD_SIZE, pypto.DT_FP32, 
                                                        [1, view_row_shape, h_num], shmem_data, 0)
                pypto.distributed.create_shmem_signal(group_name, shmem_data, shmem_signal)

            for ar_idx in pypto.loop(1, name="LOOP_MM_AR_ARMS_L0", idx_name="ar_idx"):
                tile_in_tensor = pypto.view(x, (view_row_shape, ne), [bs_idx * view_row_shape, 0],
                                                valid_shape=[(bs - bs_idx * view_row_shape).min(view_row_shape), ne])
                # 2. Matmul
                ...
                local_rank_id = pypto.distributed.get_symbolic_scalar_rank_id(group_name)
                for dyn_idx in pypto.loop(WORLD_SIZE, name="LOOP_AR", idx_name="dyn_idx"):
                    shmem_data_tile = pypto.view(shmem_data, [1, 1, view_row_shape, h_num], [dyn_idx, 0, 0, 0])
                    shmem_signal_tile = pypto.view(shmem_signal, [1, 1, 1, view_row_shape, h_num], [dyn_idx, dyn_idx, 0, 0, 0])
                    dummy = pypto.distributed.shmem_put(matmul_result, shmem_data_tile, matmul_result, pypto.AtomicType.ADD)
                    pypto.distributed.shmem_signal(dummy, shmem_signal_tile, pypto.AtomicType.ADD)
                local_data_tile = pypto.view(shmem_data, [1, 1, view_row_shape, h_num], [local_rank_id, 0, 0, 0])
                local_signal_tile = pypto.view(shmem_signal, [1, 1, 1, view_row_shape, h_num], 
                                            [local_rank_id, local_rank_id, 0, 0, 0])
                local_dummy = pypto.distributed.wait_until(matmul_result, local_signal_tile, WORLD_SIZE, True)
                pypto.set_vec_tile_shapes(1, h_num)
                x_tile = pypto.distributed.shmem_get_gm2ub(local_dummy, local_data_tile, input_dtype, pypto.AtomicType.SET)
                ...
                for tmp_idx in pypto.loop(view_row_shape, name="LOOP_ARMS", idx_name="tmp_idx"):
...
def test_matmul_all_reduce_addrmsnorm(in_tensor, mm_weight, allreduce_golden, rank):
    ...
    mm_weight = mm_weight.to(f'npu:{rank}')
    ...
    inputs = [input_hidden_states, residual_tensor, input_norm_weight, bias, mm_weight]

    out_hidden_states, out_residual = matmul_all_reduce_addrmsnorm_kernel(batch_size, ne, h_num, eps, groups[0])(*inputs)
    ...
    assert_allclose(np.array(out_hidden_states.cpu().flatten().tolist()), 
                    np.array(golden_hidden_states.cpu().flatten().tolist()), 
                    rtol=8e-3,atol=8e-3)
    assert_allclose(np.array(out_residual.cpu().flatten().tolist()), 
                    np.array(golden_residual.cpu().flatten().tolist()), 
                    rtol=1e-5,atol=1e-5)

@allow_in_graph
def matmul_all_reduce_addrmsnorm(
    ...
    if isinstance(hidden_states, FakeTensor):
        return hidden_states_res, residual_res
    
    inputs = [hidden_states, residual, input_norm_weight, bias, mm_weight]

    bs, ne = hidden_states.shape
    h_num = residual.shape[1]
    hidden_states_res, residual_res = matmul_all_reduce_addrmsnorm_kernel(bs, ne, h_num, eps, group_name)(*inputs)
...
def main():
    ...
    for idx, rank in enumerate(RANK_LIST):
        p = mp.Process(target=test_matmul_all_reduce_addrmsnorm, 
                       args=(in_tensors[idx], weights[idx], out_tensor_golden, rank))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
