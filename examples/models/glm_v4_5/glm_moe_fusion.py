...
def test_moe_fusion():
    ...
    for bs in [32, 16, 1029]:
        ...
        shapes = [hidden_states.shape, mm_weight.shape, e_score_bias.shape, w13.shape, w13_scale.shape, w2.shape, w2_scale.shape, (bs, top_k), (bs, top_k), (bs, hidden_size)]
        if enable_graph:
            g = torch.npu.NPUGraph()
            with torch.npu.graph(g):
                topk_weights, topk_ids, ffn_res = moe_fusion_kernel(shapes, renormalize, topk_group, num_expert_group)(hidden_states, mm_weight, e_score_bias, w13, w13_scale, w2, w2_scale)
            g.replay()
        else:
            topk_weights, topk_ids, ffn_res = moe_fusion_kernel(shapes, renormalize, topk_group, num_expert_group)(hidden_states, mm_weight, e_score_bias, w13, w13_scale, w2, w2_scale)
...
def moe_fusion(
    ...
    shapes = [hidden_states.shape, gate_weight.shape, e_score_correction_bias.shape, w13.shape, w13_scale.shape, w2.shape, w2_scale.shape, topk_weights.shape, topk_ids.shape, ffn_res.shape]
    topk_weights, topk_ids, ffn_res = moe_fusion_kernel(shapes, renormalize, topk_group, num_expert_group)(hidden_states, gate_weight, e_score_correction_bias, w13, w13_scale, w2, w2_scale)
    return topk_weights, topk_ids, ffn_res
