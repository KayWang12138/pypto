...
def gen_share_experts_param(
    ...
    torch.manual_seed(0)
    hidden_states = torch.rand((bs, hidden_size), dtype=dtypes, device=f'npu:{device_id}') * 0.05
    weight_gate_upper_tensor = torch.rand((hidden_size, intermediate_size * 2),
                                          dtype=dtypes, device=f'npu:{device_id}') * 0.05
    w13_nz, w13_scale = gen_quan_per_channel_weight_nz(weight_gate_upper_tensor)
    w13_scale = w13_scale.reshape(-1).to(dtypes)
    weight_down_proj_tensor = torch.rand((intermediate_size, hidden_size),
                                         dtype=dtypes, device=f'npu:{device_id}') * 0.05
    w2_nz, w2_scale = gen_quan_per_channel_weight_nz(weight_down_proj_tensor)
    w2_scale = w2_scale.reshape(-1).to(dtypes)
    ffn_res = torch.empty((bs, hidden_size), dtype=dtypes, device=f'npu:{device_id}')
    return hidden_states, w13_nz, w13_scale, w2_nz, w2_scale, ffn_res
...
def moe_fusion_kernel(hidden_states, mm_weight, e_score_bias_input, w13, w13_scale, w2, w2_scale, weight_k,
    ...
    for bs_idx, tile_batch in pypto.loop_unroll(0, bs, 1, name="LOOP_MOE_FUSION_L0", idx_name="bs_idx",
        ...
        res = pypto.matmul(tile_hidden_states_fp32, mm_weight, tile_hidden_states_fp32.dtype, b_trans=True)
        # gate end
...
def test_moe_fusion():
    ...
    for bs in [32, 16, 1029]:
        # 3. 准备测试数据
        hidden_states, w13, w13_scale, w2, w2_scale, ffn_res = gen_share_experts_param(bs, hidden_size,
                                                                                       intermediate_size, x_dtype,
                                                                                       device_id)
        ...
        # 5. 与PyTorch参考实现对比
        result = torch.matmul(hidden_states.to(torch.float32), mm_weight.t())
