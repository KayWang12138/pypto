...
def sum_lstm_compute(
    ...
    for bs_offset, unrollLength in pypto.loop_unroll(
        ...
        pypto.set_semantic_label("gelu_activation_core 22222")
...
def prepare_test_data(device) -> Dict[str, Any]:
    ...
    # pto_inputs = [pypto.from_torch(t, f"IN_{i}") if (t is not None and t.numel() > 0) else t for i, t in enumerate(inputs_torch)]
    # pto_outputs = [pypto.from_torch(t, f"OUT_{i}") for i, t in enumerate(outputs_torch)]
...
def main():
    ...
    # print(f"Compiling Kernel (Mode: {args.run_mode})...")
