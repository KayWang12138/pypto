...
pyptolib.define("compressor_decode(Tensor x, Tensor kv_state, Tensor score_state, Tensor kv_block_table, \
...
def compressor_decode(x, kv_state, score_state, kv_block_table, score_block_table, sin, cos, wkv, wgate, ape, weight, hadamard, start_pos,
        ratio, rope_head_dim, rotate):
...
def compressor_decode(x, kv_state, score_state, kv_block_table, score_block_table, sin, cos, wkv, wgate, ape, weight, hadamard, start_pos,
        ratio, rope_head_dim, rotate):
    return compressor_graph(x, kv_state, score_state, kv_block_table, score_block_table, sin, cos, wkv, wgate, ape, weight, hadamard, start_pos,
        ratio, rope_head_dim, rotate)


def compressor_decode_graph(x, kv_state, score_state, kv_block_table, score_block_table, sin, cos, wkv, wgate, ape,
                            weight, hadamard, start_pos, ratio, rope_head_dim, rotate):
    return torch.ops.pypto.compressor_decode(
...
def compressor_graph(
    ...
    pto_inputs = [x, kv_state_in, score_state_in, kv_block_table, score_block_table, sin, cos, wkv, wgate, ape, weight]
    pto_outputs = [out_res]

    if rotate and ratio == 4:
        compressor_decode_ratio_4_rotate(
            *pto_inputs, hadamard, *pto_outputs, kv_state_out, score_state_out, ratio, start_pos, rope_head_dim)
    elif not rotate and ratio == 4:
        compressor_decode_ratio_4(
            *pto_inputs, *pto_outputs, kv_state_out, score_state_out, ratio, start_pos, rope_head_dim)
...
def check_args(
    ...
    assert x.dim() == 3 and (x.size(1) == 2 or x.size(1) == 1) and x.size(2) == 4096, (
        f"x dim num is {x.dim()}, x axis1 is {x.size(1)}, x axis2 is {x.size(2)}, \
        expected 3, 1 or 2, 4096"
    )

    assert (
        kv_state.dim() == 3
        and kv_state.size(0) == kv_block_table.max() + 1
        and kv_state.size(1) == 128
        and kv_state.size(2) == coff * d
    ), (
        f"kv_state dim num is {kv_state.dim()}, kv_state axis0 is {kv_state.size(0)}, \
        kv_state axis1 is {kv_state.size(1)}, kv_state axis2 is {kv_state.size(2)}, \
        expected 3, {kv_block_table.max() + 1}, 128, {coff * d}"
    )

    assert (
        score_state.dim() == 3
        and score_state.size(0) == score_block_table.max() + 1
        and score_state.size(1) == 128
        and score_state.size(2) == coff * d
    ), (
        f"score_state dim num is {score_state.dim()}, score_state axis0 is {score_state.size(0)}, \
        score_state axis1 is {score_state.size(1)}, score_state axis2 is {score_state.size(2)}, \
        expected 3, {score_block_table.max() + 1}, 128, {coff * d}"
    )

    assert (
        kv_block_table.dim() == 2
        and kv_block_table.size(0) == bsz
        and kv_block_table.size(1) >= ((start_pos.max() + 1 + 128 - 1) // 128)
    ), (
        f"kv_block_table dim num is {kv_block_table.dim()}, kv_block_table axis0 is {kv_block_table.size(0)}, \
        kv_block_table axis1 is {kv_block_table.size(1)}, \
        expected 2, {bsz}, {((start_pos.max() + 1 + 128 - 1) // 128)}"
    )

    assert (
        score_block_table.dim() == 2
        and score_block_table.size(0) == bsz
        and score_block_table.size(1) >= ((start_pos.max() + 1 + 128 - 1) // 128)
    ), (
        f"score_block_table dim num is {score_block_table.dim()}, \
        score_block_table axis0 is {score_block_table.size(0)}, \
        score_block_table axis1 is {score_block_table.size(1)}, \
        expected 2, {bsz}, {((start_pos.max() + 1 + 128 - 1) // 128)}"
    )

    assert sin.dim() == 2 and sin.size(0) == bsz and sin.size(1) == rope_head_dim, (
        f"sin dim num is {sin.dim()}, sin axis0 is {sin.size(0)}, sin axis1 is {sin.size(1)}, \
        expected 2, {bsz}, {rope_head_dim}"
    )

    assert cos.dim() == 2 and cos.size(0) == bsz and cos.size(1) == rope_head_dim, (
    ...
    assert start_pos.dim() == 1 and start_pos.size(0) == bsz and start_pos.min() >= 0, (
        f"start_pos dim num is {start_pos.dim()}, start_pos axis0 is {start_pos.size(0)}, \
        start_pos is {start_pos}, expected 1, {bsz}, greater than or equal to 0"
    )
...
def interleaved_rope_2d(x: pypto.Tensor, cos: pypto.Tensor, sin: pypto.Tensor, rope_2d_config: Rope2dTileConfig):
...
@pypto.jit(
    ...
    debug_options=dict(compile_debug_mode=1, runtime_debug_mode=0)
)
def compressor_decode_ratio_4(
    x, kv_state_total, score_state_total, kv_block_table, score_block_table, sin, cos, wkv, wgate, ape, weight,
    out, kv_state_out, score_state_out, ratio, start_pos_dy, rope_head_dim
):
    ...
    for b_idx in pypto.loop(b_loop, name="LOOP_COMP_1", idx_name="b_idx"):
        ...
        kv_t = pypto.reshape(kv_t, [b, s1, coff*d]) + 0.0 ## b,s,2d
        score_t = pypto.matmul(x_view, wgate, pypto.DT_FP32, b_trans=True)
        score_t = pypto.reshape(score_t, [b, s1, coff*d]) + 0.0 ## b,s,2d
        ...
        for c_idx in pypto.loop(b_valid, name="LOOP_COMP_2", idx_name="c_idx"):
            ...
            if pypto.cond(start_pos % ratio + s1 < ratio):
                for i in range(s1):
                    ...
                    kv_block_idx = kv_block_table[b_idx*b+c_idx, (start_pos + i) // block_size]
                    score_block_idx = score_block_table[b_idx*b+c_idx, (start_pos + i) // block_size]
            ...
            ## 存在压缩
                ...
                if pypto.cond(start_pos < ratio):
                    ...
                    pre_score_state = pypto.view(score_state_total, [1, ratio, d], [pre_score_block_idx, pre_start, 0])
...
def compressor_decode_ratio_4_rotate(
    x, kv_state_total, score_state_total, kv_block_table, score_block_table, sin,
    cos, wkv,wgate, ape, weight, hadamard, out, kv_state_out, score_state_out, ratio, start_pos_dy, rope_head_dim
):
...
def compressor_decode_ratio_128(
    ...
    for b_idx in pypto.loop(b_loop, name="LOOP_COMP_1", idx_name="b_idx"):
        ...
        kv_t = pypto.reshape(kv_t, [b, s1, d]) + 0.0 ## b,s,d
        score_t = pypto.matmul(x_view, wgate, pypto.DT_FP32, b_trans=True)
        score_t = pypto.reshape(score_t, [b, s1, d]) + 0.0 ## b,s,d
