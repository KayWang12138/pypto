...
Example:
    See deepseekv4_lightning_indexer_quant.py for usage examples.
"""
import sys
from pypto.operation import op_wrapper
import pypto
from pypto import pypto_impl
from deepseekv4_lightning_indexer_quant import LightningIndexerConfigs
...
def lightning_indexer_decode_compute(
    ...
    Args:
        ...
        selected_count: Required parameter. It represents the number of topk selections, with a default value of 512.
    """
    ...
    length_512 = selected_count
    length_merge = selected_count * 4
    length_2k = 1024 * 2
    length_8k = 1024 * 8
    length_32k = 1024 * 32
    ...
    for b_idx in pypto.loop(0, b, 1, name="LI_LOOP_BATCH", idx_name="b_idx"):
        ...
        for s1_tile_idx in pypto.loop(0, s1_loop, 1, name="LI_LOOP_S1", idx_name="s1_loop"):
            ...
            for s1_idx in pypto.loop(0, s1, 1, name="LOOP_TOPK_S1", idx_name="s1_idx"):
                ...
                length_is_le512 = eff_seq <= length_512
                length_is_gt512 = eff_seq > length_512
                for _ in pypto.loop(length_is_le512, name="512_TOPK", idx_name="un_used"):
                    pad_x512 = pypto.tensor([1, length_512], xdtype, "pad_x512")
                    pypto.set_pass_options(pg_skip_partition=True)
                    pypto.set_vec_tile_shapes(1, length_512)
                    eff_512 = pypto.view(max_tensor, [1, length_512], [src_offset, 0], valid_shape=[1, eff_seq])
                    ax = pypto.view(eff_512, [1, length_512], [0, 0], valid_shape=[1, eff_seq])
                    bx = pypto.full([1, length_512], pad_value, pypto.DT_FP32, valid_shape=[1, length_512 - eff_seq])
                    pypto.assemble(pypto.clone(ax), [0, 0], pad_x512)
                    pypto.assemble(bx, [0, eff_seq], pad_x512)
                    pypto.set_pass_options(pg_skip_partition=False)
                    res, _ = topk_sort(pad_x512, 0)
                ...
                for _ in pypto.loop(length_is_gt512 * length_is_le8k, name="8K_TOPK", idx_name="unused"):
                    pypto.set_pass_options(pg_skip_partition=True)
                    pypto.set_vec_tile_shapes(1, length_8k)
                ...
                total_size_y2 = MAX_LI_S2 // length_8k * selected_count // length_8k * length_2k
                ...
                for idx2 in pypto.loop(num_of_32k * length_is_gt8k, name="32K_TO_8K_MERGE", idx_name="idx2"):
                    res = topk_merge(pypto.view(local_y1, [1, length_merge * 2],
                                                [0, idx2 * length_merge * 2]), selected_count)
                    pypto.assemble(pypto.clone(pypto.view(res, [1, selected_count * 2], [0, 0])),
                                [0, idx2 * selected_count * 2], local_y2)

                pypto.set_vec_tile_shapes(1, topk_tile)
                for _ in pypto.loop(1 * length_is_gt8k, name="8K_TO_2K_MERGE", idx_name="unused"):
                    res = topk_merge(pypto.view(local_y2, [1, length_merge * 2], [0, 0]), selected_count)
                    res_idx = topk_extract(res, selected_count, True)
                    pypto.set_vec_tile_shapes(1, 1, topk_tile)
                    topk_3d = pypto.reshape(res_idx, [1, 1, selected_count])
                    pypto.assemble(pypto.clone(topk_3d), [dst_offset, 0, 0], topk_res)
