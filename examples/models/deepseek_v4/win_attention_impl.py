...
def win_atten_main_tnd_prefill_mask(q_tnd, block_table, kv_cache, seqused_kv_list, atten_sink, actual_seq_list_q, mask2, atten_out, win):
    pypto.experimental.set_operation_config(combine_axis=True)
    t = q_tnd.shape[0]
    n_q = q_tnd.shape[1]
    d_q = q_tnd.shape[2]
    ...
    for b_idx in pypto.loop(b, name="LOOP_B", idx_name="B_idx"):
        ...
        # 假设b = 1, s1 = 512
        cur_s_q = actual_seq_list_q[b_idx + 1] - actual_seq_list_q[b_idx] # 512
        # 每个batch的第一个t索引位置
        t_start_idx = actual_seq_list_q[b_idx] # 0
        # 4个s1分为一组
        groups_num = pypto.ceil(cur_s_q, 4) # 128

        for g_idx in pypto.loop(groups_num, name="LOOP_G", idx_name="G_idx"):

            # 当前组内 全局的t索引
            group_start_t_idx = t_start_idx + g_idx * 4 # 组内第一个t索引 ：0, 4, 8...127 * 4
            group_end_t_idx = pypto.min(t_start_idx + g_idx * 4 + 3, t_start_idx + cur_s_q - 1) # 组内最后一个t索引：3, 7, 11 ... 127 * 4 + 3
            valid_group_len = group_end_t_idx - group_start_t_idx + 1 # 组内t的个数 4
 
            cur_offset = group_start_t_idx * n_q # 0, 256, 512...127 * 4 * 64
            actual_seq = seqused_kv_list[b_idx] # s2  512
            pypto.set_vec_tile_shapes(128, 512)
            q_tensor_cur = pypto.view(q_2d, [4 * n_q, d_q], [cur_offset, 0], valid_shape=[valid_group_len * n_q, d_q]) # 取出与组内t对应的q：[256, 512]

            # 组内s1的b内索引
            group_start_s1_idx = g_idx * 4 # 当前组内的第一个s1索引：0, 4, 8...127 * 4
            group_end_s1_idx = g_idx * 4 + valid_group_len - 1 # 当前组的最后一个s1索引：3, 7, 11...127 * 4 + 3

            # s2 = 8k
            # g_idx = 31时，是最后一组 s1索引124~127
            cur_end_s2_pos = actual_seq - (cur_s_q - 1) + group_end_s1_idx - 1 # 算末尾位置坐标 s2-1-s1+s1_idx  一组：512 - 1 - 511 + 0 = 0
            cur_start_s2_pos = pypto.max(0, cur_end_s2_pos - win - (valid_group_len - 1) + 1) # 算开头坐标  一组：0-131+1 = -130 => 0 
            ...
            if pypto.cond(start_block + 1 == end_block):
                ...
                pypto.set_cube_tile_shapes([256, 256], [128, 128], [128, 128], True, False)
                acc_s_0 = pypto.matmul(q_tensor_cur, kv_block_0, pypto.DT_FP32, b_trans=True) # [valid_group_len * n_q, 128]
                acc_s_1 = pypto.matmul(q_tensor_cur, kv_block_1, pypto.DT_FP32, b_trans=True) # [valid_group_len * n_q, 128]

                pypto.set_vec_tile_shapes(128, 128)
                acc_s = pypto.concat([acc_s_0, acc_s_1], dim=-1) # [valid_group_len * n_q, 256]

                pypto.set_vec_tile_shapes(64, 256)
                mask_block = pypto.view(mask2, [4 * n_q, block_size * 2], [0, 128 - start_block_offset], valid_shape=[valid_group_len * n_q, block_size * 2])
                ...
                start_block_id = block_table[b_idx, start_block]
                kv_block = pypto.view(kv_2d, [block_size, d_kv], [start_block_id * block_size, 0]) # [128, 512]

                pypto.set_cube_tile_shapes([256, 256], [128, 128], [128, 128], True, False)
                acc_s = pypto.matmul(q_tensor_cur, kv_block, pypto.DT_FP32, b_trans=True) # [valid_group_len * n_q, 128]

                pypto.set_vec_tile_shapes(64, 256)
                mask_block = pypto.view(mask2, [4 * n_q, block_size], [0, 258 - end_block_offset], valid_shape=[valid_group_len * n_q, block_size])
                acc_s = pypto.where(mask_block, acc_s, float("-inf")) # [valid_group_len * n_q, 256]
