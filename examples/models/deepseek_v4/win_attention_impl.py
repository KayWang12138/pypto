...
def win_atten_main_mtp(q, block_table, kv_cache, actual_seq_list, atten_sink, atten_out, win):
    pypto.experimental.set_operation_config(combine_axis=True)
    b = q.shape[0]
    s_q = q.shape[1]
    n_q = q.shape[2]
...
def deepseekv4_win_atten(q: torch.Tensor,
    ...
    if is_mtp:
        print("Using mtp kernel")
        win_atten_main_mtp(*pto_inputs, *pto_outputs, win_size)
    elif is_c128:
        print("Using C128A kernel")
