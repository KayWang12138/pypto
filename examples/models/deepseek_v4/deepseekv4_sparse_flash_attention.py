...
def compute_attention_no_flash(input_data, params, s2_tile):
    ...
    no flash 版本
    """
    q, kn, kr, topk_indices, block_table, actual_seq, atten_sink = input_data
    block_size, scalar, topk, d_v = params
    ...
    for b_idx in range(b):
        ...
        for s1_idx in range(s1):
            ...
            for s2_idx in range(bn_per_batch):
                ...
                tilda_lij = tilda_lij + atten_sink_2d
...
def gen_gather_select_attention_golden(dtype, bn1n2s1, actual_seq):
    block_size = 128
    ...
    kn = gen_uniform_data(shape_kn, -1, 1, dtype)
    ...
    # 3. 计算attention
    params = [block_size, scalar, topk, kv_lora_rank]
    input_data = [q_bsnd, kn, kr, topk_indices, block_table, actual_seq, atten_sink]
    ...
    input_data_map = [q_nope, q_rope, kn, kr, topk_indices, block_table, atten_sink]
...
def get_case_config(case_name: str):
    ...
    test_case_config = {
        "sfa_bf16_b4_s2_seq64K_total_int8_d": (
            (4, 128, 1, 2), 0, [65536, 16381, 666, 15]
        ),
        "sfa_bf16_b4_s2_seq64K_per_int8_d": (
            (4, 128, 1, 2), 0, [65536] * 4
        ),
        "sfa_bf16_b4_s4_seq64K_per_int8_d": (
            (4, 128, 1, 4), 0, [65536] * 4
        ),
...
def test_sfa_bf16_b4_s2_seq64k_total_d():
    '''
    sfa decode测试函数
    '''
...
def test_sfa_bf16_b4_s2_seq64k_per_d():
    '''
    sfa decode测试函数
    '''
...
# @pytest.mark.skip(reason="perf")
def test_sfa_bf16_b4_s4_seq64k_per_d():
    '''
    sfa decode测试函数
    '''
...
if __name__ == "__main__":
    ...
    test_sfa_bf16_b4_s2_seq64k_total_d()
    test_sfa_bf16_b4_s2_seq64k_per_d()
