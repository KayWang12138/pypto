...
class Conv1d():
    ...
    def __call__(self, x:pypto.tensor, weights:pypto.tensor, out_tensor: pypto.tensor, bias:pypto.tensor = None) -> pypto.tensor:
        ...
        for i in pypto.loop(0, self.padding, step, name="CONV1D_2", idx_name="CONV1D_2"):
            def f():
                padded_x[0:, 0:, self.padding+input_length+i:self.padding+input_length+i+step] = pypto.full([batch_size, conv_dim, step], 0., pypto.DT_FP32)
            f()
...
def compute_conv1d(mixed_qkv, weight, out):
    ...
    conv1d(mixed_qkv, weight, conv_result) # B CD S+K-1

    sStep = 1
    for sIdx in pypto.loop(0, seq_len, sStep, name="SILU_1", idx_name="SILU_1"):
        def f():
            pypto.set_vec_tile_shapes(8, 8, 8)
            conv_view = conv_result[:, :, sIdx:sIdx+sStep]
            sigm_view = pypto.sigmoid(conv_view)
            out_view = pypto.mul(conv_view, sigm_view)
            out[:, :, sIdx:sIdx+sStep] = out_view
        f()
    ...
    for sIdx in pypto.loop(0, S, sStep, name="LOOP06", idx_name="LOOP06IDX"):
        for hIdx in pypto.loop(0, vH, 1, name="LOOP07", idx_name="LOOP07IDX"):
            def f():
                ...
                query_reshaped = pypto.reshape(query_t, [B, sStep, 1, d])
                key_reshaped = pypto.reshape(key_t, [B, sStep, 1, d])
                pypto.set_vec_tile_shapes(8, 8, 8, 8)

                query[:, sIdx:, hIdx:, :] = query_reshaped
                key[:, sIdx:, hIdx:, :] = key_reshaped
            f()
    ...
    for sIdx in pypto.loop(0, S, sStep, name="LOOP08", idx_name="LOOP08IDX"):
        for hIdx in pypto.loop(0, vH*d, d, name="LOOP09", idx_name="LOOP09IDX"):
            def f():
                ...
                value_reshaped = pypto.reshape(value_t, [B, sStep, 1, d])
                pypto.set_vec_tile_shapes(8, 8, 8, 8)

                value[:, sIdx:, hIdx//d:, :] = value_reshaped
            f()
...
def cast_and_reshape(inputs, outputs):
    ...
    for b_idx in pypto.loop(0, b, view_b, name="LOOP_B", idx_name="b_idx"):
        for n_idx in pypto.loop(0, n, view_n, name="LOOP_N", idx_name="n_idx"):
            ...
            query_result_block = pypto.cast(query_block.transpose(1, 2).reshape([view_b*view_n, c, l, d]), pypto.DT_FP32)
            ...
            value_result_block = pypto.cast(value_block.transpose(1, 2).reshape([view_b*view_n, c, l, d]), pypto.DT_FP32)
...
def pypto_l2norm(tensor: pypto.Tensor, result_tensor: pypto.Tensor, eps=1e-6):
    ...
    for bn_idx in pypto.loop(0, bn, view_bn, name="LOOP_BN", idx_name="bn_idx"):
        for c_idx in pypto.loop(0, c, view_c, name="LOOP_C", idx_name="c_idx"):
            ...
            result_tensor[bn_idx:bn_idx+view_bn, c_idx:c_idx+view_c, :, :] = x / pypto.sqrt((x * x).sum(-1, keepdim=True).reshape([view_bn, view_c, l, 1]) + eps)
...
def cal_cumsum(g, triu, cumsum_g):
    ...
    for bn_idx in pypto.loop(0, bn, view_bn, name="BN_LOOP", idx_name="bn_idx"):
        ...
        cumsum_g[bn_idx:, :, :] = pypto.matmul(g_block, triu_block, pypto.DT_FP32) # 1 vC L
...
def cal_decay_mask(g, tril, decay_mask):
    ...
    for bn_idx in pypto.loop(0, bn, view_bn, name="BN_LOOP", idx_name="bn_idx"):
        ...
        decay_mask_block = ((pypto.expand_clone(g_block.reshape([view_bn, c, l, 1]), [view_bn, c, l, l]) - \
                             pypto.expand_clone(g_block.reshape([view_bn, c, 1, l]), [view_bn, c, l, l])) * tril_block).exp() * tril_block
...
def cal_pre_attn(k_beta, key, decay_mask, mask, attn):
    ...
    for bn_idx in pypto.loop(0, bn, view_bn, name="BN_LOOP", dx_name="bn_idx"):
        ...
        attn[bn_idx:bn_idx+view_bn, :, :, :] = pypto.matmul(k_beta_block, key_block.transpose(2, 3), pypto.DT_FP32) * decay_mask_block * mask_block
...
def cal_inverse(attn, eye, tril, output):
    ...
    for foo in pypto.loop(0,1,1, name="foo", idx_name="foo"):
...
def cal_value_and_kcumdecay(inputs, outputs):
    ...
    for bn_idx in pypto.loop(0, bn, view_bn, name="BN_LOOP", idx_name="bn_idx"):
        for c_idx in pypto.loop(0, c, view_c, name="C_LOOP", idx_name="c_idx"):
            ...
            value[bn_idx:bn_idx+view_bn, c_idx:c_idx+view_c, :, :] = pypto.matmul(attn_block, v_beta_block, pypto.DT_FP32) # view_bn, view_c, l, d
...
def recurrent_loop(inputs, outputs):
    ...
    for foo in pypto.loop(0, 1, 1, name="", idx_name=""):
    ...
    for i in pypto.loop(0, c, 1, name="LOOP_RECURRENT", idx_name="r_idx", submit_before_loop=True):
        for bn_idx in pypto.loop(0, bn, view_bn, name="LOOP_BN", idx_name="bn_idx"):
            # view data
            _qi = middle_query[bn_idx:(bn_idx+view_bn), i:i+1, :, :] # [view_bn, 1, L, D]
            _ki = middle_key[bn_idx:(bn_idx+view_bn), i:i+1, :, :] # [view_bn, 1, L, D]
            _vi = middle_value[bn_idx:(bn_idx+view_bn), i:i+1, :, :] # [view_bn, 1, L, D]
            _decay_mask = decay_mask[bn_idx:(bn_idx+view_bn), i:i+1, :, :] # [view_bn, 1, L, L]
            _tril = tril_mask[bn_idx:(bn_idx+view_bn), i:i+1, :, :] # [view_bn, 1, L, L]
            _k_cumdecay = middle_k_cumdecay[bn_idx:(bn_idx+view_bn), i:i+1, :, :] # [view_bn, 1, L, D]
            _state = recurrent_state[bn_idx:(bn_idx+view_bn), :, :] # [view_bn, D, D]
            _gate = gate[bn_idx:(bn_idx+view_bn), i:i+1, :] # [view_bn, 1, L]

            # # compute
            pypto.set_vec_tile_shapes(1, 16, 16, 16)
            attn = pypto.matmul(_qi, _ki.transpose(2, 3), pypto.DT_FP32) * _decay_mask * _tril # [view_bn, 1, L, L]
            v_prime = pypto.matmul(_k_cumdecay, _state.reshape([view_bn, 1, d, d]), pypto.DT_FP32) # [view_bn, 1, L, D]
            v_new = _vi - v_prime # [view_bn, 1, L, D]
            attn_inter = pypto.matmul(_qi * _gate.reshape([view_bn, 1, l, 1]).exp(), _state.reshape([view_bn, 1, d, d]), pypto.DT_FP32)
            _core_attn_out = attn_inter + pypto.matmul(attn, v_new, pypto.DT_FP32) # [view_bn, 1, L, D]
            pypto.set_vec_tile_shapes(16, 16, 16)
            _last_gate = pypto.expand_clone(_gate.reshape([view_bn, l])[:, l-1:l], [view_bn, l]).reshape([view_bn, 1, l]) # [view_bn, 1, l]
            pypto.set_vec_tile_shapes(1, 16, 16, 16)
            pypto.set_cube_tile_shapes([128,128], [128,128], [128,128])
            # comment
            _tmp_matmul_result = pypto.matmul((_ki * (_last_gate - _gate).reshape([view_bn, 1, l, 1]).exp()).transpose(2, 3), v_new, pypto.DT_FP32) # [view_bn, 1, D, D]
            _last_gate_2 = pypto.expand_clone(_gate.reshape([view_bn, l])[:, l-1:l], [view_bn, d]).reshape([view_bn, d, 1]) # [view_bn, 1, l]
            _updated_state = _state * _last_gate_2.exp() + _tmp_matmul_result.reshape([view_bn, d, d]) # [view_bn, D, D]

            # # assemble
            tmp_core_attn_out[bn_idx:, i:i+1, :, :] = _core_attn_out
            recurrent_state[bn_idx:bn_idx+view_bn, :, :] = _updated_state

    for foo in pypto.loop(0,1,1, name="foo", idx_name="foo", submit_before_loop=True):
...
def gated_delta_rule_process(inputs, outputs):
    ...
    for foo in pypto.loop(0, 1, 1, name="", idx_name="", submit_before_loop=True):
        pass
    # comment
    recurrent_loop([scaled_query, l2norm_key, value_matmul, decay_mask, output_k_cumdecay, g, states, tril_mask], [core_attn_out, final_state])
            ...
            def b_loop_func(b_idx):
                for s_idx in pypto.loop(s_loop, name="LOOP_RMS_NORM_L1", idx_name="s_idx"):
                    def s_loop_func(s_idx):
                        ...
                        # hanzi
                        tile_x = pypto.view(x, view_shape, [b_idx*view_shape[0], s_idx*view_shape[1], 0, 0], valid_shape = [(b-b_idx*view_shape[0]).min(view_shape[0]), (s-s_idx*view_shape[1]).min(view_shape[1]), n, d])
...
def compute_all(
    hidden_states, qkvz_weight, ba_weight, A_log, dt_bias, conv_weight, init_state, mask1, tril_mask, triu_mask, eye,# inputs
    hidden_state_out, final_state # outputs
):
    ####
    #20251122
    # B, S, N, D = core_attn_out.shape
    ...
    mixed_qkv = pypto.tensor([B, D*(2*kH+vH), S], pypto.DT_FP32, "MIXED_QKV_WORKSPACE")
    b = pypto.tensor([B, S, vH], pypto.DT_FP32, "B_WORKSPACE")
    a = pypto.tensor([B, S, vH], pypto.DT_FP32, "A_WORKSPACE")
    
    query = pypto.tensor([B, S, vH, D], pypto.DT_FP32, "query_workspace")
    key = pypto.tensor([B, S, vH, D], pypto.DT_FP32, "key_workspace")
    value = pypto.tensor([B, S, vH, D], pypto.DT_FP32, "value_workspace")
    g = pypto.tensor([B, S, vH], pypto.DT_FP32, "g_workspace")
    beta = pypto.tensor([B, S, vH], pypto.DT_FP32, "beta_workspace")

    ####
    #20251122
    z = pypto.tensor([B, S, N*D], pypto.DT_FP32, "z_workspace")
    core_attn_out = pypto.tensor([B, S, N, D], pypto.DT_FP32, "core_attn_out_workspace")
    core_attn_out_2nd = pypto.tensor([B, S, N, D], pypto.DT_FP32, "core_attn_out_workspace")
    ####
    ...
    mixed_qkv_after_conv = pypto.tensor([B, D*(2*kH+vH), S], pypto.DT_FP32, "MIXED_QKV_AFTER_CONV_WORKSPACE")
    compute_conv1d(mixed_qkv, conv_weight, mixed_qkv_after_conv)

    compute_interleave(mixed_qkv_after_conv, query, key, value, kH, vH, D)

    gated_delta_rule_process([query, key, value, beta, g, init_state, mask1, tril_mask, triu_mask, eye], [core_attn_out, final_state])
...
def test_all():
    ...
    hidden_states = torch.rand([B, S, D]).float()
    qkvz_weight = torch.rand([1, d*(kH + kH + vH + vH), D]).float()
    ba_weight = torch.rand([1, 2*vH, D]).float()
    A_log = torch.rand([1, 1, vH]).float()
    dt_bias = torch.rand([1, 1, vH]).float()
    conv_weight = torch.rand([CD, CD//CD, K]).float()
    init_state = torch.zeros([B, vH, D, D]).float()

    # helper data
    mask1 = torch.tril(-torch.ones([B*vH, C, L, L], dtype=torch.float), diagonal=-1)
    tril_mask = torch.ones([B*vH, C, L, L]).float().tril() # lower triangular
    triu_mask = torch.ones([L, L]).float().triu().reshape([1, L, L]) # Upper triangular
    eye = torch.eye(L).unsqueeze(0).unsqueeze(0).expand([B*vH, C, L, L]).contiguous().float()

    inputsCPU = [hidden_states, qkvz_weight, ba_weight, A_log, dt_bias, conv_weight, init_state]
    inputsNPU = [tsr.to(device) for tsr in inputsCPU] + [tsr.to(device) for tsr in [mask1, tril_mask, triu_mask, eye]]

    hidden_state_out = torch.zeros([B, S, D]).float()
    final_state = torch.zeros([B, vH, D, D]).float()
    outputsCPU = [hidden_state_out, final_state]
    ...
    torch_all(inputsCPU, outputsCPU)
    pypto_gated_delta_net(pto_inputs, pto_outputs)

    assert torch.allclose(outputsCPU[0], outputsNPU[0].cpu(), 0.001, 0.001)
    assert torch.allclose(outputsCPU[1], outputsNPU[1].cpu(), 0.001, 0.001)
    print("PASSED")
