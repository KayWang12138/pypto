...
def gated_attention_prefill_func(inputs, outputs):
    ...
    for b_idx in pypto.loop(b_scalar, name="LOOP_b", idx_name="b_idx", submit_before_loop=True):
        ...
        for s1_idx in pypto.loop(s1_scalar, name="LOOP_s1", idx_name="s1_idx", submit_before_loop=True):
            s2_loop = (s1_scalar + s2_tile - 1) // s2_tile
            bs_ofs = b_ofs + s1_idx
            ...
            for n2_idx in pypto.loop(n2_sym, name="LOOP_n2", idx_name="n2_idx"):
                for g_idx in pypto.loop(g_loop, name="LOOP_g", idx_name="g_idx"):
                    ...
                    for s2_idx in pypto.loop(s2_loop, name="LOOP_s2", idx_name="s2_idx"):
                        ...
                        actual_s2_tile = (s1_scalar - s2_idx * s2_tile).min(s2_tile)
...
def gated_attention_prefill(
    ...
    inputs = [q_torch, k_torch, v_torch, act_seq_torch, gate_torch, weight_torch]
    outputs = [out_torch]
...
def test_gated_attention_prefill():
    ...
    for i in range(b):
        s1 = get_s1size(actual_seq, i)
        b_ofs = get_bsoffset(actual_seq, i)
        for j in range(s1):
            bs_ofs = b_ofs + j
            ...
            for n2_idx in range(nkv):
                ...
                k_bs = k_torch[b_ofs: b_ofs + s1, n2_idx]
                v_bs = v_torch[b_ofs: b_ofs + s1, n2_idx]
