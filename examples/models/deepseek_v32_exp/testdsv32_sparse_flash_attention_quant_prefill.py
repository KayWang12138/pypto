...
import logging
from sparse_flash_attention_quant import sparse_flash_attention_quant_p_compute, SaTileShapeConfig
...
def gen_block_table(act_seq, block_size, s1, need_indices=False):
    ...
    if need_indices:
        ...
        for b_idx in range(b):
            cur_act = act_seq[b_idx]
            for s1_idx in range(s1):
                pos = cur_act - s1 + s1_idx
                block_idx_in_seq = pos // block_size
                global_block_id = block_table[b_idx, block_idx_in_seq]

                offset_in_block = pos % block_size
                global_index = global_block_id * block_size + offset_in_block
                cache_index[b_idx, s1_idx] = global_index
    else:
        cache_index = None
...
def get_case_config(case_name: str):
    ...
    test_case_config = {
        ...
        "DynamicGatherSlcFlashAttnDSASTest.dsa_gather_slc_attn_bf16_b1_s256_seq2047": (
            (1, 128, 1, 256), 0, [2047]
        ),
        "DynamicGatherSlcFlashAttnDSASTest.dsa_gather_slc_attn_bf16_b1_s256_seq2047_int8": (
            (1, 128, 1, 256), 1, [2047]
        ),
        "DynamicGatherSlcFlashAttnDSASTest.dsa_gather_slc_attn_bf16_b1_s512_seq2047": (
            (1, 128, 1, 512), 0, [2047]
        ),
        "DynamicGatherSlcFlashAttnDSASTest.dsa_gather_slc_attn_bf16_b1_s512_seq2047_int8": (
            (1, 128, 1, 512), 1, [2047]
        ),
...
@pytest.mark.skip(reason='perf')
def test_QSFA_p_bf16_b8_s4_seqtest2_int8():
