...
def mla_prolog_quant_d_compute(input_tensors, output_tensors, epsilon_cq, epsilon_ckv, cache_mode, tile_config):
    ...
    x_2d = pypto.tensor([b * s, h], token_x.dtype)
    cos_2d = pypto.tensor([b * s, qk_rope_head_dim], cos.dtype)
    sin_2d = pypto.tensor([b * s, qk_rope_head_dim], sin.dtype)
    k_cache_index_2d = pypto.tensor([b * s, 1], cache_index.dtype)

    for _ in pypto.loop(0, 1, 1, name="MLA_IN_RESHAPE_LOOP", idx_name="batch_id"):
        x_2d[:] = pypto.reshape(token_x, [b * s, h], inplace=True)
        cos_2d[:] = pypto.reshape(cos, [b * s, qk_rope_head_dim], inplace=True)
        sin_2d[:] = pypto.reshape(sin, [b * s, qk_rope_head_dim], inplace=True)
        k_cache_index_2d[:] = pypto.reshape(cache_index, [b * s, 1], inplace=True)
