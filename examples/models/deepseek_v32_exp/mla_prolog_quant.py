...
def k_nope_quant(x):
    ...
    max_value = pypto.amax(abs_res, keepdim=True)
...
def quant(input_tensor, is_symmetry=False, has_smooth_factor=False, smooth_factor=None):
    ...
    if is_symmetry:
        ...
        max_value = pypto.amax(abs_res, keepdim=True)
        temp127 = pypto.full(max_value.shape, 127.0, pypto.DataType.DT_FP32)
        ...
        max_value = pypto.amax(input_fp32, keepdim=True)
        min_value = pypto.amin(input_fp32, keepdim=True)
...
def rope_v2(x, cos, sin, tile_config):
    ...
    x_view = pypto.reshape(cast_x, [1, seq_size, d_r // 2, 2])
    pypto.set_vec_tile_shapes(*tile_config.four_dim)
    ...
    if not (x.shape[0] == cos.shape[0] and x.shape[1] == cos.shape[1]):
        cast_cos = pypto.reshape(cast_cos, x.shape)
        cast_sin = pypto.reshape(cast_sin, x.shape)
...
def rope_3d_v2(x, cos, sin, tile_config):
    ...
    pypto.set_vec_tile_shapes(1, 32, 128, 128)
    x_trans = pypto.transpose(x_view, 2, 3)
...
def mla_prolog_compute_p(input_tensors, output_tensors, epsilon_cq, epsilon_ckv, cache_mode, tile_config):
    ...
    for bs_idx in pypto.loop(0, bs_loop, 1, name="MLA_BS_LOOP", idx_name="bs_idx"):
        ...
        def mla_prolog_inner(params):
            for _ in pypto.loop(0, 1, 1, name="MLA_PREPARE_RES", idx_name="unused_idx"):

                def prepare_mla(prepare_params):
                    token_x, w_dq, w_uq_qr, dequant_scale, w_uk, w_dkv_kr, gamma_cq, gamma_ckv, cos, \
                    sin, k_cache_index_2d, kv_cache, kr_cache, k_scale_cache, q_norm_out, q_norm_scale_out, \
                    query_nope_out, query_rope_out, kv_cache_out, kr_cache_out, k_scale_cache_out, \
                    k_nope_2d, k_rope_2d, k_scale_2d, dtype, h, kv_lora_rank, qk_rope_head_dim, \
                    q_head_dim, block_num, block_size, n2, rope_cfg, t = prepare_params
            ...
            for _ in pypto.loop(0, 1, 1, name="MLA_UPDATE_CACHE", idx_name="unused_idx"):
                ...
                def update_cache(cache_params):
                    kr_cache_out2d, kv_cache_out2d, k_scale_cache_out2d, k_cache_index_2d, \
                        qk_rope_head_dim, kr_cache_2d, kv_cache_2d, k_scale_cache_2d = cache_params
        ...
        params = [token_x, w_dq, w_uq_qr, dequant_scale, w_uk, w_dkv_kr, gamma_cq, gamma_ckv, cos, \
                    ...
                    query_nope_out, query_rope_out, kv_cache_out, kr_cache_out, k_scale_cache_out, \
                    k_nope_2d, k_rope_2d, k_scale_2d, dtype, h, kv_lora_rank, qk_rope_head_dim, \
                    q_head_dim, block_num, block_size, n2, rope_cfg, t]
        mla_prolog_inner(params)
