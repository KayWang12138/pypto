...
def test_add_scalar_loop_dynamic_axis_static_cond(device_id=-1):
    if (device_id == -1):
    ...
    assert_allclose(np.array(npu_data1), np.array(torch_data), rtol=3e-3, atol=3e-3)
    print("test add1_flag False is OK!")


    ## test add1_flag is True
    val = 1
    output_data2 = torch.zeros(shape, dtype=torch.float, device=f'npu:{device_id}')
    add_add1flag_true(input_data0, input_data1, output_data2, val, True)

    torch_add1 = torch.add(input_data0, input_data1) + val
    npu_data2 = output_data2.cpu()
    torch_data1 = torch_add1.cpu()

    assert_allclose(np.array(npu_data2), np.array(torch_data1), rtol=3e-3, atol=3e-3)
    print("test add1_flag True is OK!")

if __name__ == "__main__":
    device_id = 5
    test_add_scalar_loop_dynamic_axis_static_cond(device_id)
