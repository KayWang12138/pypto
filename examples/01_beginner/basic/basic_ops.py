...
def element_wise_ops(a: torch.Tensor, b: torch.Tensor, dynamic: bool = False) -> torch.Tensor:
...
def test_element_wise_operations():
    ...
    c_torch = element_wise_ops(a_torch, b_torch)
...
@pypto.jit
def matrix_multiply_kernel(A: pypto.Tensor, B: pypto.Tensor, C: pypto.Tensor) -> None:
    pypto.set_cube_tile_shapes([32, 32], [64, 64], [64, 64])

    C[:] = pypto.matmul(A, B, out_dtype=pypto.DT_BF16)


def matrix_multiply(A: torch.Tensor, B: torch.Tensor, dynamic: bool = False) -> torch.Tensor:
    ...
    matrix_multiply_kernel(A_pto, B_pto, C_pto)
...
def test_matrix_multiplication():
    ...
    C_torch = matrix_multiply(A_torch, B_torch)
...
@pypto.jit
def apply_activations_kernel(x: pypto.Tensor, result: pypto.Tensor) -> None:
    pypto.set_vec_tile_shapes(32, 64)

    result[:] = pypto.sigmoid(x)


def apply_activations(x: torch.Tensor, dynamic: bool = False) -> torch.Tensor:
    ...
    apply_activations_kernel(x_pto, result_pto)
...
def test_activation_functions():
    ...
    output_torch = apply_activations(input_torch)
...
@pypto.jit
def tiled_operation_kernel(input_tensor: pypto.Tensor, output_tensor: pypto.Tensor) -> None:
    # Get shape
...
def tiled_operation(input_tensor: torch.Tensor, dynamic: bool = False) -> torch.Tensor:
    ...
    tiled_operation_kernel(input_pto, output_pto)
...
def test_view_operations():
    ...
    output_torch = tiled_operation(input_torch)
...
@pypto.jit
def linear_layer_with_activation_kernel(x: pypto.Tensor, W: pypto.Tensor, b: pypto.Tensor, y: pypto.Tensor) -> None:
    pypto.set_vec_tile_shapes(32, 64)
    pypto.set_cube_tile_shapes([32, 32], [64, 64], [64, 64])
...
def linear_layer_with_activation(x: torch.Tensor, W: torch.Tensor, b: torch.Tensor, dynamic: bool = False) -> torch.Tensor:
    ...
    linear_layer_with_activation_kernel(x_pto, W_pto, b_pto, y_pto)
...
def test_combined_operations():
    ...
    y_torch = linear_layer_with_activation(x_torch, W_torch, b_torch)
