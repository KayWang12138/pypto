...
def test_symbolicscalar_in_loop(device_id: int = None)->None:
    """SymbolicScalar as loop index inside kernel"""
    device = f'npu:{device_id}' if (global_run_mode == pypto.RunMode.NPU and device_id is not None) else 'cpu'
    x = torch.tensor(
        [1, 2, 3],
...
def test_init_symbolic_scalar_value_arg(device_id: int = None)->None:
...
def test_symbolic_scalar_prop(device_id: int = None)->None:
...
def test_symbolic_scalar_complex_expr(device_id: int = None)->None:
