...
def test_set_cube_tile_shapes_basic():
    ...
    out = compute_with_cube_tile_shapes_op(a, b, [[32, 32], [64, 64], [64, 64]])
    assert_allclose(out.cpu().numpy(), expected.cpu().numpy(), rtol=1e-3, atol=1e-3)
    print(f"Output: {out}")
    print(f"Expected: {expected}")
...
def compute_with_different_tile_shapes_op(a: torch.Tensor, b: torch.Tensor, dynamic: bool = False) -> tuple:
    ...
    compute_with_different_tile_shapes_kernel(a_pto, b_pto, out1_pto, out2_pto, out3_pto)
...
def test_set_different_tile_shapes_result():
    ...
    dtype = torch.float32
    # shape:(2, 2, 2)
    a = torch.tensor([[[1, 2], [3, 4]], [[5, 6], [7, 8]]], dtype=dtype, device=f'npu:{device_id}')
    b = torch.tensor([[[5, 6], [7, 8]], [[1, 2], [3, 4]]], dtype=dtype, device=f'npu:{device_id}')
    expected = torch.tensor([[[19, 22], [43, 50]], [[23, 34], [31, 46]]], dtype=dtype, device=f'npu:{device_id}')

    out1, out2, out3 = compute_with_different_tile_shapes_op(a, b)
...
@pypto.jit
def compute_with_specific_tile_shapes_kernel(a: pypto.Tensor, b: pypto.Tensor, out: pypto.Tensor) -> None:
    pypto.set_cube_tile_shapes([32, 32], [32, 32], [32, 32])
    out[:] = pypto.matmul(a, b, a.dtype, b_trans=True)


@pypto.jit
def compute_with_another_tile_shapes_kernel(a: pypto.Tensor, b: pypto.Tensor, out: pypto.Tensor) -> None:
    pypto.set_cube_tile_shapes([64, 64], [128, 128], [128, 128])
    out[:] = pypto.matmul(a, b, a.dtype, b_trans=True)


def compute_with_specific_tile_shapes_op(a: torch.Tensor, b: torch.Tensor, dynamic: bool = False) -> torch.Tensor:
    out = torch.zeros(a.shape[0], b.shape[1], dtype=a.dtype, device=a.device)
    ...
    compute_with_specific_tile_shapes_kernel(a_pto, b_pto, out_pto)

    return out


def compute_with_another_tile_shapes_op(a: torch.Tensor, b: torch.Tensor, dynamic: bool = False) -> torch.Tensor:
    out = torch.zeros(a.shape[0], b.shape[1], dtype=a.dtype, device=a.device)
    ...
    compute_with_another_tile_shapes_kernel(a_pto, b_pto, out_pto)
...
def test_set_different_tile_shapes_runtime():
    ...
    for _ in range(TEST_TIME):
        out1 = compute_with_specific_tile_shapes_op(a, b)
    runtime_1 = time.perf_counter() - start
    start = time.perf_counter()
    for _ in range(TEST_TIME):
        out2 = compute_with_another_tile_shapes_op(a, b)
...
def test_set_vec_tile_shapes_basic():
    ...
    b = torch.tensor([[[[7, 8, 9],
                        [10, 11, 12]]]], dtype=dtype, device=f'npu:{device_id}')
    expected = torch.tensor([[[[8, 10, 12],
                            [14, 16, 18]]]], dtype=dtype, device=f'npu:{device_id}')

    out = compute_with_vec_tile_shapes_op(a, b, (1, 1, 4, 8))
    assert_allclose(out.cpu().numpy(), expected.cpu().numpy(), rtol=1e-3, atol=1e-3)
    print(f"Output: {out}")
    print(f"Expected: {expected}")
...
@pypto.jit
def compute_with_vec_different_tile_shapes_kernel(a: pypto.Tensor, b: pypto.Tensor, out1: pypto.Tensor, out2: pypto.Tensor, out3: pypto.Tensor) -> None:
...
def compute_with_vec_different_tile_shapes_op(a: torch.Tensor, b: torch.Tensor, dynamic: bool = False) -> tuple:
    out1 = torch.zeros_like(a)
    out2 = torch.zeros_like(a)
    out3 = torch.zeros_like(a)
    ...
    compute_with_vec_different_tile_shapes_kernel(a_pto, b_pto, out1_pto, out2_pto, out3_pto)
...
def test_set_vec_different_tile_shapes_result():
    ...
    out1, out2, out3 = compute_with_vec_different_tile_shapes_op(a, b)
    assert_allclose(out1.cpu().numpy(), expected.cpu().numpy(), rtol=1e-3, atol=1e-3)
    assert_allclose(out2.cpu().numpy(), expected.cpu().numpy(), rtol=1e-3, atol=1e-3)
    assert_allclose(out3.cpu().numpy(), expected.cpu().numpy(), rtol=1e-3, atol=1e-3)
    print(f"out1 == out2: {torch.equal(out1, out2)}")
