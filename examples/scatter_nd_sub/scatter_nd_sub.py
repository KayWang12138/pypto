...
def create_scatter_nd_sub_kernel(target_shape: tuple, indices_shape: tuple, updates_shape: tuple, strides_shape: tuple, run_mode: str = "npu", dynamic: bool = True):
    if dynamic:
    ...
    @pypto.frontend.jit(runtime_options={"run_mode": mode, "stitch_function_num_initial": 128, "stitch_function_outcast_memory": 1024, "stitch_function_inner_memory": 1024})
    def scatter_nd_sub_kernel(
        ...
        for bs_idx, tile_batch in pypto.loop_unroll(0, indices_first, 1, name="LOOP_SCATTER_ND_SUB_L0", idx_name="bs_idx",
                                                    unroll_list=powers_of_2(512)):
            b_offset = bs_idx
...
def test_scatter_nd_sub(device_id: int = None, run_mode: str = "npu", dynamic: bool = False) -> None:
    ...
    pypto_output = create_scatter_nd_sub_kernel(target_shape, indices_shape, updates_shape, strides_shape, run_mode)(target, indices, updates, strides)
