...
def conv_dynamic_kernel(
    ...
    if is_conv2d:
        ...
        # TileShape 配置（tileWout 设为较大固定值，实际切分通过前端循环）
        tile_wout_front = 512  # 前端循环的 wout 切分步长
        tile_win_need = (tile_wout_front - 1) * stride_w + (kw - 1) * dilation_w + 1
        
        tile_l1_config = pypto.pypto_impl.TileL1Info(
            tileHin=1,
            tileHout=1,
            tileWin=tile_win_need,  # 根据前端 wout tile 计算所需 win
            tileWout=win,           # 设为完整宽度，前端循环控制实际切分
            tileCinFmap=16,
            tileCinWeight=16,
            tileN=16,
            tileBatch=1  # 必须为 1
        ...
        tile_l0_config = pypto.pypto_impl.TileL0Info(
            tileH=1,
            tileW=tile_win_need,    # 与 tileWin 匹配
            tileK=16,
            tileN=16
        )
        vec_tile_config = (16)
...
def test_conv2d(device_id: int = None):
    ...
    print(f"Tile config:")
    print(f"  - TileShape.tileBatch=1 (固定约束)")
    print(f"  - batch_tile=2 (前端循环)")
    print(f"  - cout_tile=32 (TileShape)")
    print(f"  - hout_tile=3 (TileShape)")
    print(f"  - wout_tile=16 (前端循环)")
    print(f"Loop counts:")
    print(f"  - batch_loop=2 (前端)")
    print(f"  - cout_loop=2")
    print(f"  - hout_loop=2")
    print(f"  - wout_loop=4 (前端)")
    print("✓ Conv2D test completed successfully")
...
def test_conv1d(device_id: int = None):
    ...
    print(f"  - TileShape.tileBatch=1 (固定约束)")
    print(f"  - batch_tile=2 (前端循环)")
    print(f"  - cout_tile=16 (TileShape)")
    print(f"  - wout_tile=512 (前端循环)")
    print(f"Loop counts:")
    print(f"  - batch_loop=1 (前端)")
    print(f"  - cout_loop=1")
    print(f"  - wout_loop=4 (前端)")
    print("注意: Conv1D 与 Conv2D 保持一致的动态切分模式")
    print("      - batch/wout 通过前端循环切分")
    print("      - cout 通过 TileShape 切分")
