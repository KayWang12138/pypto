...
def test_ir_file(ir_file: Path, converter: str = "numpy", output_dir: Path = None) -> tuple[bool, str]:
                ...
                if test_func_name:
                    ...
                    if converter == "pypto":
                        ...
                        run_result = subprocess.run(
                            [sys.executable, str(abs_output_file), "--run_mode=sim"],
                            capture_output=True,
                            text=True,
                            timeout=60,
...
def main():
    ...
    for ir_file in ir_files:
        ...
        for converter in ["numpy", "pypto"]:
            print(f"\n  [{converter.upper()}] Converting {ir_file.name}...")
            success, message = test_ir_file(ir_file, converter, output_dir)
    ...
    all_passed = all(
        success for converter in results.values() 
        for _, success, _ in converter
    )
