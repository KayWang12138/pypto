...
- Intelligent Script Analysis (via AST parsing):
    ...
    - Verifies --run_mode sim parameter support for simulation mode filtering
...
- Dual Execution Modes:
    ...
    - sim: Filters and runs only scripts that explicitly support --run_mode sim parameter 
...
def _supports_run_mode_sim(file_path: Path) -> bool:
    """Check if the file supports the '--run_mode sim' parameter using AST parsing"""
    try:
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()
...
def main() -> None:
    ...
    if target.is_file():
        ...
        py_files = [f for f in target.rglob("*.py") if not f.is_symlink()]
