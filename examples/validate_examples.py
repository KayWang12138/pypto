    ...
    def create_and_register_process(
        ...
        with self._lock:
            ...
            try:
                ...
                # Process creation failed - propagate exception
                raise
...
def _collect_result(
    ...
    if proc.returncode == 0:
        ...
        return ExecutionResult.success(rel_path, device_id)
    else:
        safe_print(f"❌ Failure: {rel_path}")
        return ExecutionResult.failure(
            rel_path, f"Non-zero exit code ({proc.returncode})",
            device_id=device_id, output_snippet=snippet
        )
...
def run_script(ctx: ExecutionContext, full_path: Path, rel_path: str) -> ExecutionResult:
    ...
    try:
        ...
        return _collect_result(proc, stdout, stderr, rel_path, device_id, ctx.safe_print)
