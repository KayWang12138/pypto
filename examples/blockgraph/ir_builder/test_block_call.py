...
def test_block_function(input_args, output_args, indices):
    builder = ir.IrBuilder()
    ctx = ir.IrBuilderContext()
    block = BlockBuilderHelper(builder, ctx)
    tile_shape = [128, 128]
    print("in test_block_function")
    sig = irFunctionSignature(input_args, output_args)
...
@pypto.jit(runtime_options = {"run_mode": pypto.RunMod.SIM})
def test_block_call(x, y, o):
    loop_count = 4
    for idx in pypto.loop(0, loop_count, 1, name="LOOP_L0", idx_name="idx"):
        o = BlockCall.call(test_block_function, [x, y], [o], [idx])[0]
