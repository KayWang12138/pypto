...
def get_ifa_tile_cfg():
    ...
    tile_cfg = AttentionTileConfig(
        g_tile=12,
        s2_tile=s2_tile,
        c1_tile_shape=[[m_tile, m_tile], [k_tile, k_tile], [n_tile, n_tile]],
        v1_tile_shape=[m_tile, s2_tile],
        c2_tile_shape=[[m_tile, m_tile], [k_tile, k_tile], [n_tile, n_tile]],
        v2_tile_shape=[m_tile, m_tile]
    )
    return tile_cfg
                    ...
                    for group_idx in pypto.loop(group_loop, name="LOOP_group_idx", idx_name="group_idx"):
                        ...
                        for s2_idx in pypto.loop(s2_loop, name="LOOP_s2", idx_name="s2_idx", unroll_list=[8, 4, 2, 1]):
                            ...
                            if pypto.is_loop_begin(s2_idx):
                                ...
                                vj_assemble = pypto.view(vj_assemble, [s2_tile, d],
                                                         [0, 0], valid_shape=[actual_s2_tile, d])
                            ...
                            if pypto.is_loop_end(s2_idx):
                                ...
                                oi_final_3d = pypto.cast(
                                    pypto.reshape(oi_final, [1, g_tile, d]),
                                    dtype)
                                # 7. 将结果搬运到输出tensor上
