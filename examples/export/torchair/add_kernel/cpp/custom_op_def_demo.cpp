...
// (produced by load_and_compile.py against the add .air that
// ../export_demo.py writes out via torchair).
//
...
// load_and_compile.py is model-format-agnostic (pypto.export.load_model
// dispatches on .onnx vs .air), so the .so registers AddPyptoCustomOp the
// same way a .onnx-driven build would.
