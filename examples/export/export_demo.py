...
def export_demo(onnx_path: Optional[str] = None, air_path: Optional[str] = None, force_cpu: bool = False, force_sim: bool = False):
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--onnx-path", type=str, help="onnx model path")
    parser.add_argument("-a", "--air-path", type=str, help="torch air model path")
    parser.add_argument("--force-cpu", action="store_true")
    parser.add_argument("--force-sim", action="store_true")

    args = parser.parse_args()
    if args.onnx_path is None and args.air_path is None:
        raise ValueError("Must specify either --onnx-path or --air-path")

    export_demo(
        onnx_path=args.onnx_path,
        air_path=args.air_path,
