...
The codegen targets the real Ascend ``sinkable_op_execution_context.h`` /
``custom_op.h`` API. ``--mock-include-dir`` is kept for back-compat with
older CANN installs that pre-date these headers; drop it once the local
install ships the real headers.
"""
import argparse
import shutil
import subprocess
...
def main():
    ...
    parser.add_argument(
        "--op-type",
        type=str,
        default="AddPyptoCustomOp",
        help="Custom op type to extract (default: AddPyptoCustomOp)",
    )
    parser.add_argument(
        "--domain",
        type=str,
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Directory whose headers shadow Ascend headers during build "
             "(repeatable; prepended in listed order).",
    )
    parser.add_argument(
        "--clean",
        action="store_true",
    ...
    mock_include_dirs = [Path(p).resolve() for p in args.mock_include_dir]
    for p in mock_include_dirs:
        if not p.is_dir():
            raise FileNotFoundError(f"Mock include dir does not exist: {p}")
        print(f"[INFO] Mock include dir: {p}")
    ...
    pypto.export.cpp.generate_bindings_cpp(
        ...
        module_name,
        op_def_cpp=cpp_paths.get("op_custom_def"),
    )
    pypto.export.cpp.generate_cmakelists(
        ...
        mock_include_dirs=mock_include_dirs,
    )
