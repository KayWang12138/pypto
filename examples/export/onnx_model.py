...
@torch.library.custom_op("pypto::add_pypto_npu", mutates_args=())
def add_pypto_npu(x0: torch.Tensor, x1: torch.Tensor) -> torch.Tensor:
    ...
    print("Goes through pypto.from_torch")
...
@torch.library.register_fake("pypto::add_pypto_npu")
def add_pypto_npu_fake(x0, x1):
    print("Goes through fake kernel")
    assert x0.shape == x1.shape
    return torch.empty_like(x0)

def add_pypto_onnx_symbolic(g, x0, x1):
    meta = getattr(add_kernel_py, "__pypto_meta__", {})
    meta_json = json.dumps(meta, sort_keys=True)

    # 这些将是onnx内部的属性
    return g.op("ai.onnx.contrib::AddPyptoCustomOp", x0, x1,
...
class CustomModel(nn.Module):
    def forward(self, x0, x1):
        return torch.ops.pypto.add_pypto_npu(x0, x1)
...
def example_export(models_dir: str, force_cpu: bool = False, force_sim: bool = False):
    ...
    y = model(input_data0, input_data1)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Please specify the models path")
        sys.exit(1)
