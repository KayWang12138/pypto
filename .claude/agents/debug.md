...
4. `./logs/<op>.log` — the actual failure log Verification pulled back
5. `custom/<op>/eval/evaluation_report.json` (sanitized) — `status`, `first_failure.case_id`, `first_failure.failing_module_boundary`, `first_failure.failure_category`, `first_failure.summary`, `stdout`. This is your **primary narrowing signal**: the `failing_module_boundary` field tells you the smallest k for which prefix-eval broke, isolating the fix domain to one module or one module-boundary contract. **You must NOT try to read `<op>_golden_modular.py` or any golden tensor values from this report** — the `_sanitize` step strips them; respect the information barrier.
...
| `infra` (SSH/rsync failure) | No skill — report environment issue to Lead for user attention |
...
2. Re-read the Verification failure log from `custom/plan/<op>.md` → Per-module verification log + Development & debug log, plus the raw log under `./logs/<op>.log`.
3. Run diagnostic tools as needed (all NPU-side runs go through `Run <file> on npu:<N>` or `./scripts/npu_run.sh`):
   - `diagnose_error(error_log=..., kernel_code=...)` for known pattern match (MCP, local)
   - `extract_pypto_calls.py` when an op-by-op protocol is called for (local)
   - Sub-skill-specific bisection (e.g. `pass_verify_save` checkpointing for precision) — written locally, executed via `Run <scratch_file> on npu:<N>`
...
- **Never** execute kernels locally. All re-runs go through `Run <file> on npu:<N>` or `./scripts/npu_*.sh`.
