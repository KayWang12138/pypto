...
- [ ] C++ headers (`framework/include/` for pypto, `framework/include/block/` for pypto_block)
- [ ] Python bindings (`python/src/` for pypto, `python/src/bindings/block/` for pypto_block) — snake_case method names
- [ ] Type stubs (`python/pypto/pypto_core/` for pypto, `python/pypto_block/pypto_core/` for pypto_block) — signatures match, `@overload` where needed
