...
## Architecture
- Look at `docs/` to get a global picture of the project. For details look at docs
  in `docs/api`, `docs/invocation`, `docs/tutorials`.
- This project is about to build a Python frontend for the MLIR of PTOAS.
  For details of the MLIR of PTOAS, you can search in `~/PTOAS/include/PTO/IR`.
- We design a new pypto-ir above the MLIR of PTOAS. The project transforms
  ops defined in `python/pypto_block/language/` to the MLIR of PTOAS via the frontend.
- We majorly use manual operations which are not SSA and all operands are in
  the arguments. Manual ops should be put in `python/pypto_block/language/op/manual/op/`.
- `framework/src/interface/block/` - C++ source code of pypto-ir and its transforms.
  Its Python bindings are located in `python/src/bindings/block/` and `python/pypto_block/ir`.
- `python/pypto_block/frontend` - The frontend kernel and JIT decorators, responsible
  for triggering the transformation from pypto-ir to PTOAS MLIR.
- `framework/src/interface/block/` and `framework/include/block/` contain the
  C++ implementation for lowering pypto-ir to PTOAS MLIR (emitted as string).
- The PTOAS MLIR string can be compiled by the `ptoas` tool, which will be 
  available in `PATH` after executing `source ~/setup.sh`.
- The lower pto-isa definition is located at ~/pto-isa/docs/isa/.
## Project Rules
## Build Commands
...
### mlir python path
~/llvm-project/build-shared/tools/mlir/python_packages/mlir_core/mlir
