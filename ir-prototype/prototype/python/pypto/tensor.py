...
from . import utils
from .scalar import Scalar, SymInt


class TensorSignature:
    """Lightweight tensor signature that only contains type and shape information.
    
    This is used in function signatures to avoid creating full Tensor objects
    until they are actually needed. It's particularly useful for output parameters
    where we only need type information during signature extraction.
    
    Attributes:
        shape: The shape of the tensor
        dtype: The data type of the tensor
        name: Optional name for the tensor
        format: The tensor format (default: TILEOP_ND)
    """
    
    def __init__(self, shape, dtype: Union[DataType, None] = None,
                 name: str = "", format: pypto_impl.TileOpFormat = pypto_impl.TileOpFormat.TILEOP_ND):
        """Initialize a TensorSignature.
        
        Args:
            shape: The shape of the tensor (list of int or Scalar)
            dtype: The data type of the tensor
            name: Optional name for the tensor
            format: The tensor format (default: TILEOP_ND)
        """
    ...
    def to_tensor(self) -> 'Tensor':
        """Create a full Tensor object from this signature.
        
        Returns:
            A Tensor object with the same shape, dtype, name, and format.
        """
        return Tensor(shape=self.shape, dtype=self.dtype, name=self.name, format=self.format)
...
class Tensor:
    ...
    def __init__(self, shape=None, dtype: Union[DataType, None] = None,
        ...
        if shape is None or dtype is None:
            ...
            # Create directly using C++ Tensor, without adding to scope
            if all([isinstance(s, int) for s in shape]):
                nshape = typing.cast(List[int], shape)
                self._base = pypto_impl.Tensor(dtype, nshape, name, format)
                self.ori_shape = ori_shape
            else:
                sym_shape = utils.to_syms(shape)
                assert isinstance(
                    sym_shape, list), "shape must be a list of int or Scalar"
    ...
    def __setitem__(self, key, value):
        ...
        Args:
            key (Union[int, Scalar, slice]): Index or slice to set.
        ...
        if self._is_empty_slice(key):
            # For SSA semantics, use assemble instead of move
            # This ensures we return a new tensor rather than modifying in-place
            if isinstance(value, Tensor):
                # For full slice assignment like c[:] = value, use assemble with zero offsets
                offsets = [0] * len(self.shape)
                return pypto.assemble(value, offsets, self)
            else:
                # Fallback to move for non-tensor values
                self.move(value)
                return
        ...
        if all(isinstance(k, (int, Scalar)) for k in key):
            pypto_impl.SetTensorData(utils.to_sym(value), utils.to_syms(key), self._base)
            return
        ...
        if all(isinstance(k, (slice, int, Scalar)) for k in key):
        ...
        raise ValueError("tuple key must be int, Scalar or slice")

    def __getitem__(self, key, *, valid_shape: Optional[List[Union[int, Scalar]]] = None):
        """
        Get tensor data by index, supporting integer indices, slices, ellipsis,
        and their combinations to retrieve sub-tensors.

        Args:
            key (Union[int, Scalar, slice, ellipsis]): Index or slice to get.
        ...
        if all(isinstance(k, (int, Scalar)) for k in key):
            return pypto_impl.GetTensorData(self._base, utils.to_syms(key))
    ...
    def shape(self) -> List[SymInt]:
        out = []
        # GetShape() now returns std::vector<Scalar>
        shape_list = self._base.GetShape()
        for n in shape_list:
            # n is a pypto_impl.Scalar, wrap it in Python Scalar
            out.append(Scalar.from_base(n))
        return out
    ...
    def _get_assemble_offset(key, shape):
        ...
        for axis, k in enumerate(key):
            ...
            if start is None and stop is None:
                offsets.append(0)
            elif isinstance(start, (int, Scalar)):
                offsets.append(start)
            elif isinstance(stop, (int, Scalar)):
    ...
    def assemble(self, input: 'Tensor', offsets: List[Union[int, Scalar]]) -> None:
        """
        ...
        Args:
            ...
            offsets (Union[List[int], List[Scalar]]): Offset for placing the input tensor.
    ...
    def reshape(self, shape: List[int], *, valid_shape: Optional[List[Union[int, Scalar]]] = None,
    ...
    def view(self, shape: List[int], offsets: List[Union[int, Scalar]],
             *, valid_shape: Optional[List[Union[int, Scalar]]] = None) -> 'Tensor':
    ...
    def _is_empty_slice(self, key):
        ...
        elif isinstance(key, (int, Scalar)):
            return False
    ...
    def _normalize_key(self, key):
        ...
        if isinstance(key, (int, Scalar, slice)) or key is Ellipsis:
            key = (key,)
