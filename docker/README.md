...
RUN pip install --no-cache-dir torch-npu==2.6.0
   #update cann
#step3 [Optional] Install the CANN package provided by pypto.
ARG CANN_VERSION
RUN  ARCH=$(case "${TARGETPLATFORM}" in \
        "linux/amd64") echo "x86_64" ;; \
        "linux/arm64") echo "aarch64" ;; \
        *) echo "Unsupported TARGETPLATFORM: ${TARGETPLATFORM}" 1>&2 && exit 1 ;; \
    esac) && \
    echo "ARCH=${ARCH}" && \
    PLATFORM_PATH=$(case "${TARGETPLATFORM}" in \
        "linux/amd64") echo "x86" ;; \
        "linux/arm64") echo "aarch64" ;; \
        *) echo "Unsupported TARGETPLATFORM: ${TARGETPLATFORM}" 1>&2 && exit 1 ;; \
    esac) && \
    echo "PLATFORM_PATH=${PLATFORM_PATH}" && \
    if echo "${CANN_VERSION}" | grep -iq "910b"; then \
        DEVICE_TYPE="910b"; \
    ...
    # create URL
    CANN_TOOLKIT_URL="https://ascend-cann.obs.cn-north-4.myhuaweicloud.com/pypto/cann/8.5.0/${PLATFORM_PATH}/Ascend-cann-toolkit_8.5.0_linux-${ARCH}.run" && \
    CANN_OPS_URL="https://ascend-cann.obs.cn-north-4.myhuaweicloud.com/pypto/cann/8.5.0/${PLATFORM_PATH}/Ascend-cann-${DEVICE_TYPE}-ops_8.5.0_linux-${ARCH}.run" && \
    echo "CANN_TOOLKIT_URL=${CANN_TOOLKIT_URL}" && \
    echo "CANN_OPS_URL=${CANN_OPS_URL}" && \
    # download (use proxy)
    if [ -n "${PROXY}" ]; then \
    ...
    wget --quiet --no-check-certificate --header="Referer: https://www.hiascend.com/" "${CANN_TOOLKIT_URL}" -O ~/Ascend-cann-toolkit.run && \
    wget --quiet --no-check-certificate --header="Referer: https://www.hiascend.com/" "${CANN_OPS_URL}" -O ~/Ascend-cann-ops.run && \
    echo "Download completed successfully"
# Note: Install CANN Toolkit Development Kit Package
RUN chmod +x ~/Ascend-cann-toolkit.run && \
#      ~/Ascend-cann-toolkit.run --quiet --check && \
#      ~/Ascend-cann-toolkit.run --quiet --upgrade && \
# Note: install the CANN package to a Custom directory, use the following command
     ~/Ascend-cann-toolkit.run --quiet --install --force --install-path=/usr/local/Ascend/CANN_pypto && \
    rm -f ~/Ascend-cann-toolkit.run

# Note: Install CANN kernel Package
RUN chmod +x ~/Ascend-cann-ops.run && \
#    ~/Ascend-cann-ops.run --quiet --install --force && \
# Note: install the CANN package to a Custom directory, use the following command
     ~/Ascend-cann-ops.run --quiet --install --force --install-path=/usr/local/Ascend/CANN_pypto && \
...
构建编包：<br>
python build.py -f=python3<br>
pip install build_out/pypto*whl --force-reinstall --no-deps<br>
