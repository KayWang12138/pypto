...
#
# 用法:
#   bash benchmark/scripts/local/test-integration.sh             # FULL
#   FULL=0 bash benchmark/scripts/local/test-integration.sh      # cheap
...
if [ "${FULL}" = "0" ]; then
  section "test-integration: cases=${CASES} concurrency=${CONCURRENCY} (cheap, --skip-pypto-gen, verifier=${VERIFIER_MODE})"
else
  section "test-integration: cases=${CASES} concurrency=${CONCURRENCY} (FULL, pypto 7-stage + verifier=${VERIFIER_MODE})"
  echo "  WARN: 单 case 约 50 min (含 Stage 7 性能调优 + LLM 调用)" >&2
fi

export CASES FULL VERIFIER_MODE SKILL_TIMEOUT SKILL_RETRY SKILL_RETRY_INTERVAL PYPTO_TIMEOUT MODE OPENCODE_MODEL
...
MONITOR_CMD="BENCHMARK_MONITOR_DIR=${BENCHMARK_MONITOR_DIR} python3 -m benchmark.monitor monitor"

echo "  command:"
printf '    %s\n' python3 -m benchmark.monitor
echo
echo "============================================================"
...
# 真跑
python3 -m benchmark.monitor 2>&1 | tee "${BENCHMARK_LOG_DIR}/batch.log"
