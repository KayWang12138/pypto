...
def main():
    ...
    print("="*60)
    dq_out_cpu = pto_dq.cpu().float()
    golden_dq_cpu = dq.cpu().float()
    for idx in range(total_iters):
        cp_pto = cp_dq_raw_c[idx].cpu().float()
        cp_golden = dq_raw_c_all[idx].cpu().float()
        diff_pto = (dq_out_cpu[:, 0, :] - cp_pto).abs().max().item()
        match_pto = torch.allclose(dq_out_cpu[:, 0, :], cp_pto, rtol=1e-3, atol=1e-3)
        diff_golden = (golden_dq_cpu[:, 0, :] - cp_golden).abs().max().item()
        match_golden = torch.allclose(golden_dq_cpu[:, 0, :], cp_golden, rtol=1e-3, atol=1e-3)
        print(f"  dq_out vs cp_dq_raw_c[idx={idx}]: {'MATCH' if match_pto else 'NO MATCH'}, max_diff={diff_pto:.6e}")
        print(f"  golden_dq vs dq_raw_c_all[idx={idx}]: {'MATCH' if match_golden else 'NO MATCH'}, max_diff={diff_golden:.6e}")
    print("="*60)
