...
def accumulate_kernel(
    ...
    for i_idx in pypto.loop(n_i, name="LOOP_i", idx_name="i_idx"):
        ...
        # for j_idx in pypto.loop(n_j, name="LOOP_j", idx_name="j_idx", submit_before_loop=True):
            j_off = j_idx * TILE
            pypto.set_vec_tile_shapes(TILE, TILE)
...
def run_test(n_i, n_j, device):
    ...
    # logger.info(
    #     "tensor addr: src=%#x acc=%#x n_outer=%#x n_inner=%#x",
    #     src.data_ptr(), acc.data_ptr(), ni_t.data_ptr(), nj_t.data_ptr()
    # )
    # logger.info(
    #     "kernel args addr: acc_in=%#x acc_out=%#x same_buffer=%s",
    #     acc.data_ptr(), acc.data_ptr(), acc.data_ptr() == acc.data_ptr()
    # )
