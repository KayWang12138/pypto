...
def TaskID(taskId):
    return int(taskId) & TASKID_TASK_MASK
...
class TraceCopy:
    ...
    def overlap(src: 'TraceCopy', dst: 'TraceCopy') -> bool:
        ...
        if dst_range.end <= src_range.begin:
            return False
        assert src_range == dst_range, ""
        assert len(src.offset) == len(dst.offset), ""
...
ltask_loader_dict: Dict[str, Callable[[TraceLeafTask, SchemaNode], None]] = {
    # "coa": lambda ltask, coa_node: setattr(
    #     ltask, 'coa_list', load_trace_coa_list(coa_node.at(0))
    # ),

    # "succ": lambda ltask, succ_node: (
    #     [ltask.add_succ(
    #         TraceLeafTaskUid(
    #             device_task_index=ltask.uid._device_task_index,
    #             dup_index=ltask.uid._dup_index,
    #             root_index=ltask.uid._root_index,
    #             operation_index=op_index,
    #             leaf_index=ltask.uid._leaf_index
    #         )
    #     ) for op_index in load_trace_succ_list(succ_node.at(0))]
    # ),
...
class TraceExecution:
    ...
    def build_task_successor_dict(self, file_path):
        ...
        uid_map = {}          # {唯一uid_str: TraceLeafTaskUid}
        succ_map = {}         # {唯一uid_str: 后继taskid列表}
        task_info = []        # [(seqNo, raw_taskid, uid_str, op_index)] 新增op_index便于匹配
        # 按seqNo分组存储有效op_index（seqNo -> {op_index1, op_index2...}）
        valid_uid_by_seq = {}  
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            lines = [l.strip() for l in f if l.strip()][1:]  # 跳过标题行
            for line in lines:
                parts = line.split(',')
                ...
                rootIndex = int(parts[2])
                leafIndex = int(parts[5])
                
                dup_index = raw_taskid >> TASKID_TASK_BITS
                op_index = TaskID(raw_taskid)
                
                uid_str = f"{seqNo}_{dup_index}_{rootIndex}_{op_index}_{leafIndex}"
                task_uid = TraceLeafTaskUid(seqNo, dup_index, rootIndex, op_index, leafIndex)
                uid_map[uid_str] = task_uid
                task_info.append((seqNo, raw_taskid, uid_str, task_uid))  # 新增op_index
        ...
        def _get_real_succ(uid_str, visited):
            ...
            for succ_tid in succ_map[uid_str]:
                for (s_no, t_id, u_str, task_uid) in task_info:
                    if t_id == succ_tid and u_str not in visited:
                        if s_no == current_seq and s_no in valid_uid_by_seq and task_uid in valid_uid_by_seq[s_no]:
                            real_succ.append(task_uid)
                        else:
                            real_succ.extend(_get_real_succ(u_str, visited))
        ...
        for uid_str, uid in valid_uid_str.items():
            result[uid] = _get_real_succ(uid_str, set())
