...
class TraceCopy:
    ...
    def overlap(self, src: 'TraceCopy', dst: 'TraceCopy') -> bool:
...
class TraceDeviceTask:
    ...
    def _check_all_copy_races(self, src_leaf_task, dst_leaf_task, race_list):
        ...
        self._check_write_write_race(
            src_leaf_task=src_leaf_task,
            dst_leaf_task=dst_leaf_task,
            race_list=race_list
        )

    def _check_copy_overlap_race(self, src_leaf_task, dst_leaf_task,
                           src_copy_attr, dst_copy_attr,
                           src_is_write, dst_is_write,
                           default_race_kind, race_list):
        src_copy_list = getattr(src_leaf_task, src_copy_attr)
        dst_copy_list = getattr(dst_leaf_task, dst_copy_attr)
        for i, src_copy in enumerate(src_copy_list):
            for j, dst_copy in enumerate(dst_copy_list):
                if TraceCopy.overlap(src_copy, dst_copy):
                    self._create_race_object(
                        src_leaf_task=src_leaf_task,
                        dst_leaf_task=dst_leaf_task,
                        src_idx=i,
                        dst_idx=j,
                        src_is_write=src_is_write,
                        dst_is_write=dst_is_write,
                        race_kind=default_race_kind,
                        race_list=race_list
                    )

    def _check_write_write_race(self, src_leaf_task, dst_leaf_task, race_list):
    ...
    def _create_race_object(self, src_leaf_task, dst_leaf_task,
                        src_idx, dst_idx, src_is_write, dst_is_write,
                        race_kind, race_list):
        race_part_src = TraceRacePart(src_leaf_task, src_is_write, src_idx)
        race_part_dst = TraceRacePart(dst_leaf_task, dst_is_write, dst_idx)
...
class TraceExecution:
    ...
    def _load_task_data(self, task, node_dict, loader_dict):
        for key, loader in loader_dict.items():  # 深度 2
            if key in node_dict:  # 深度 3
                node = node_dict[key][0]
                loader(task, node)
    
    def _build_valid_uid_by_seq(self, valid_uid_list):
        valid_uid_by_seq = {}
    ...
    def _parse_successor_file(self, file_path):
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for line in lines:
                ...
                uid_str = self._generate_uid_str(seqNo, dup_index, root_index, op_index, leaf_index)
                task_uid = TraceLeafTaskUid(seqNo, dup_index, root_index, op_index, leaf_index)
                
                if seqNo not in seq_taskid_to_uidstr:
                    seq_taskid_to_uidstr[seqNo] = {}
                seq_taskid_to_uidstr[seqNo][raw_taskid] = uid_str
                
                uid_str_to_info[uid_str] = (seqNo, task_uid)
    ...
    def _generate_uid_str(self, seqNo, dup_index, root_index, op_index, leaf_index):
        return f"{seqNo}_{dup_index}_{root_index}_{op_index}_{leaf_index}"

    def _parse_succ_list(self, parts):
        succ_list = []
    ...
    def _build_valid_uid_str_map(self, valid_uid_list):
        ...
        for uid in valid_uid_list:
            uid_str = self._generate_uid_str(
                seqNo=uid._device_task_index,
                dup_index=uid._dup_index,
                root_index=uid._root_index,
                op_index=uid._operation_index,
                leaf_index=uid._leaf_index
            )
            valid_uid_str[uid_str] = uid
        return valid_uid_str

    def _get_real_succ(self, uid_str, visited, succ_map, uid_str_to_info, seq_taskid_to_uidstr, valid_uid_by_seq):
        if uid_str in visited or uid_str not in succ_map:
            return []
        visited.add(uid_str)
        real_succ = []
        
        if uid_str not in uid_str_to_info:
            return []
        current_seq, _ = uid_str_to_info[uid_str]

        if current_seq not in seq_taskid_to_uidstr:
            return real_succ
        
        seq_taskid_map = seq_taskid_to_uidstr[current_seq]
        for succ_tid in succ_map[uid_str]:
            if succ_tid not in seq_taskid_map:
                continue
            u_str = seq_taskid_map[succ_tid]
            if u_str in visited or u_str not in uid_str_to_info:
                continue
            s_no, task_uid = uid_str_to_info[u_str]
            if task_uid in valid_uid_by_seq.get(s_no, set()):
        ...
        if current_token.kind() == ord('['):
            ...
            while True:
                ...
                if self.current().kind() == ord(','):
                    ...
                    raise ValueError(f"Invalid format at pos {self.pos}: expected , or ], got {self.current()}")
        else:
            if current_token.kind() != self.Token.ID:
                raise ValueError(f"Expected ID token at pos {self.pos}, got {current_token}")
            ...
            if self.current().kind() == ord('{'):
                ...
                raise ValueError(f"Invalid format after ID at pos {self.pos}: got {self.current()}")
        return curr_node
    ...
    def parse(self) -> List[SchemaNode]:
        ...
        while self.accessible():
            ...
            if not self.accessible():
                break
            if self.current().kind() != ord('#'):
                raise AssertionError(f"Expected # at pos {self.pos}, got {self.current()}")
            self.move_next()
...
class LoadTraceLog:
    ...
    def read_single_file(self, file_path: str, encoding: str, strip_whitespace: bool) -> None:
        try: 
            with open(file_path, 'r', encoding=encoding, errors='ignore') as f:  # 深度 3（try→with）
    ...
    def _clean_trace_content(self, line: str, trace_pos: int, strip_whitespace: bool) -> str:
        trace_content = (
            line[trace_pos:]
            .replace('"', '')
            .replace('#trace:  ', '#trace:')
