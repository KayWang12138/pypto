...
class PassComparator:
    ...
    def is_contain(a: Dict[str, Any], b: Dict[str, Any], key: str) -> bool:
        ...
        if a[":opcode"] == "ASSEMBLE" or a[":opcode"] == "COPY_OUT":
    ...
    def add_comparison_record(self, 
        ...
        recored = {}
        recored["NO."] = self.row_num
        recored["PATH_FUNC:func_magicname "] = a["PATH_FUNC:func_magicname"]
        recored["PATH_FUNC:funcmagic"] = a["PATH_FUNC:funcmagic"]
        recored["PATH_FUNC:hash"] = a["PATH_FUNC:hash"]
        recored["LOOP_INFO"] = a["LOOP_INFO"]
        recored[":symbol"] = a[":symbol"]
        recored[":validshape"] = a[":validshape"]
        recored[":datatype"] = a[":datatype"]
        recored["OP_ATTR_SYM_OFFSET"] = a["OP_ATTR_SYM_OFFSET"]
        recored["OP_IO_FLAG"] = a["OP_IO_FLAG"]
        recored["A>PHASE_NAME"] = self.golden_pass
        recored["B>PHASE_NAME"] = a["PHASE_NAME"]
        recored["A>TIMESTAMP"] = b["TIMESTAMP"] if b else None
        recored["B>TIMESTAMP"] = a["TIMESTAMP"]
        recored["A>FILENAME"] = b["FILENAME"] if b else None
        recored["B>FILENAME"] = a["FILENAME"]
        recored["A>FUNC:hash"] = b["FUNC:hash"] if b else None
        recored["B>FUNC:hash"] = a["FUNC:hash"]
        recored["A>ROOT_CALL:opmagic"] = b["ROOT_CALL:opmagic"] if b else None
        recored["B>ROOT_CALL:opmagic"] = a["ROOT_CALL:opmagic"]
        recored["A>ROOT_CALL:rawmagic"] = b["ROOT_CALL:rawmagic"] if b else None
        recored["B>ROOT_CALL:rawmagic"] = a["ROOT_CALL:rawmagic"]
        recored["A>:opmagic"] = b[":opmagic"] if b else None
        recored["B>:opmagic"] = a[":opmagic"]
        recored["A>:opcode"] = b[":opcode"] if b else None
        recored["B>:opcode"] = a[":opcode"]
        recored["A>:rawmagic"] = b[":rawmagic"] if b else None
        recored["A>:rawshape"] = b[":rawshape"] if b else None
        recored["A>:format"] = b[":format"] if b else None
        recored["B>:rawmagic"] = a[":rawmagic"]
        recored["B>:rawshape"] = a[":rawshape"]
        recored["B>:format"] = a[":format"]
        recored["A>:shape"] = b[":shape"] if b else None
        recored["B>:shape"] = a[":shape"]
        recored["A>EVAL:dynvalidshape"] = b["EVAL:dynvalidshape"] if b else None
        recored["B>EVAL:dynvalidshape"] = a["EVAL:dynvalidshape"]
        recored["AB>RESULT"] = result_is_close
        recored["result_reason"] = result_reason
        recored["AB>rtol/atol"] = str(self.rtol) + "/" + str(self.atol)
        if diff_conf is not None:
            recored["AB>fail_cnt/warn_cnt/tol_cnt"] = str(diff_conf[4]) + "/" + str(diff_conf[3]) + "/" + str(diff_conf[2])
            recored["AB>total_cnt/zero_cnt/infnan_cnt"] = str(diff_conf[0]) + "/" + str(diff_conf[1]) + "/" + str(diff_conf[5])
            recored["AB>mae"] = str(diff_conf[8])
            recored["AB>mae_top8"] = str(diff_conf[9])
            recored["AB>mae_top1permil"] = str(diff_conf[10])
            recored["AB>mre"] = str(diff_conf[11])
            recored["AB>mre_top8"] = str(diff_conf[12])
            recored["AB>mre_top1permil"] = str(diff_conf[13])
            recored["A>max"] = str(diff_conf[19])
            recored["A>min"] = str(diff_conf[20])
            recored["A>avg"] = str(diff_conf[21])
            recored["A>aavg"] = str(diff_conf[22])
            recored["A>zero"] = str(diff_conf[23])
            recored["A>infnan"] = str(diff_conf[7])
            recored["B>max"] = str(diff_conf[14])
            recored["B>min"] = str(diff_conf[15])
            recored["B>avg"] = str(diff_conf[16])
            recored["B>aavg"] = str(diff_conf[17])
            recored["B>zero"] = str(diff_conf[18])
            recored["B>infnan"] = str(diff_conf[6])
        self.row_num += 1
        self.comparison_records.append(recored)
