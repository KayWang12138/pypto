...
from tensor_diff import IsCloseConfig
...
class PassComparator:
    ...
    def _log_comparison_info(a: Dict, b: Dict):
        ...
        logging.info(f'line : {a["No."]}, 数据a的shape: {a["outputValidShape"]}, '
                    f'offset: {a["tensorOffset"]}, dtype: {a["outputDtype"]}')
        logging.info(f'line : {b["No."]}, 数据b的shape: {b["outputValidShape"]}, '
    ...
    def loop_compare(self, pass_a: str, pass_b: str, df_loop, 
        ...
        参数:
            ...
            raw_tensor_list: 指定要比较的raw tensor列表
...
def main():
    ...
    parser.add_argument("--func", nargs='*', type=str, default=[],
                       help="要比较的function的name，用空格分隔。留空则比较所有function")
    ...
    parser.add_argument("--verify_path", nargs='*', type=str, default=[],
                       help="验证文件目录。如果提供2个值，分别代表两个pass的路径")
