...
class TensorComparator:
    ...
    def save_info_to_csv(d_detail, csv_path, topk):
        ...
        print(f"数据已保存到: {csv_path}")  # 可选：保留一个简单的控制台提示
    ...
    def check_isclose(a, b, config: IsCloseConfig = IsCloseConfig()):
        ...
        if not is_detail:
            ...
            ab_rd_max = ab_rd[~torch.isnan(ab_rd)].max().item() if ab_rd[~torch.isnan(ab_rd)].numel() != 0 else float('nan') 
            data_diff_info_list = (ab_sub_abs.max().item(), ab_rd_max, ab_sub_abs.mean().item(), ab_rd.nanmean().item(),
                                cnt_out_warn, cnt_out_warn / ab_sub_abs.numel(), cnt_out_bothzero, cnt_out_bothzero / ab_sub_abs.numel())
        ...
        result_info = (diff_cnt, diff_conf, _diff_extra, diff_detail_warn, diff_detail_fail, diff_detail_infnan, data_diff_info_list)
