...
class TensorComparator:
    ...
    def check_isclose(a, b, config: IsCloseConfig = IsCloseConfig()):
        ...
        if not is_detail:
            ...
            data_diff_info_list = (cnt_all, cnt_out_bothzero, tol_cnt, cnt_out_warn, cnt_fail, cnt_infnan,
                                    a_infnan_cnt, b_infnan_cnt, ab_sub_abs.mean().item(),
                                    torch.topk(ab_sub_abs, min(8, ab_sub_abs.numel())).values.mean().item(),
                                    torch.topk(ab_sub_abs, min(100, ab_sub_abs.numel())).values.mean().item(),
                                    ...
                                    a.max().item(), a.min().item(), a.mean().item(), a_abs.mean().item(),
                                    torch.sum(a == 0).item(),b.max().item(), b.min().item(), b.mean().item(),
                                    b_abs.mean().item(), torch.sum(b == 0).item())
