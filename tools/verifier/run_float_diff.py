...
class DataDiffAnalyzer:
    ...
    def usage_exit(err_info='', err_no=-1):
        if err_info:
            logging.error(f'Error: {err_info}')
        logging.error(f'Usage: {sys.argv[0]} <file_a> <file_b> <fp16|fp32|npy|pt>-<fp16|fp32|npy|pt>')
        return err_info
    ...
    def gen_max_min_avg(self, data, mode='all'):  # mode:all|avg|avg2|pos|neg|pos2|neg2|pos4|neg4
        if mode in ['pos', 'pos2', 'pos4']:
            ...
            self.usage_exit('Error: bad mode ' + mode)
    ...
    def prepare_data(self):
        if len(sys.argv) >= 5:
            for arg in sys.argv[4:]:
                if arg in ['sort']:
                    ...
                    self.is_out_no_fig = True
                elif arg in ['nofile']:
                    self.is_out_no_fig = True
                elif arg in ['noinc']:
                    self.is_calc_inc = False
                elif arg in ['norel']:
                    self.is_calc_rel = False
                elif arg in ['png', 'jpg', 'jpeg', 'svg', 'pdf']:
                    self.conf_fig_format = arg
                elif arg.startswith('offset=') or arg.startswith('off='):
                    self.conf_seg_off = int(arg.lstrip('offset='))
                elif arg.startswith('length=') or arg.startswith('len='):
                    self.conf_seg_len = int(arg.lstrip('length='))
                else:
                    self.usage_exit('unknown parameter "' + arg + '"')
        ...
        if f_a == f_b:
            my_is_same_ab = True
        f_out_fig = f_b + '--sort-diff.' + self.conf_fig_format if self.is_sort \
                    else f_b + '--diff.' + self.conf_fig_format
        _f_type = sys.argv[3]
        ...
        f_a_info = 'A_' + a_t + '--v' + str(self.ver) + ': ' + f_a
        f_b_info = 'B_' + b_t + '--v' + str(self.ver) + ': ' + f_b
        logging.info(f_a_info)
        logging.info(f_b_info)
        if not self.is_out_no_fig:
            logging.info('fig   : ' + f_out_fig)
    ...
    def fix_input_and_compute(self, a, b, f_type, sort):
        ...
        if self.conf_seg_off is not None and self.conf_seg_off > 0:
            if self.conf_seg_off < st_pick_data_total:
                _seg_begin = self.conf_seg_off
            my_is_data_seg = True
        if self.conf_seg_len is not None and self.conf_seg_len > 0:
            _seg_end = _seg_begin + self.conf_seg_len
        ...
        _st_a_pick_info = ''
        _st_a_seg_info = str(st_a_seg_data_total) + '@' + str(_seg_begin) + '/' + \
                         str(_seg_end) + '#' if my_is_data_seg else ''
        st_a_info = 't=' + _st_a_seg_info + _st_a_pick_info + str(st_a_total_raw) + ' s=' + str(st_a_shape_raw) + \
                    ' inf=' + str(_st_a_inf) + ' nan=' + str(_st_a_nan) + ' z=' + str(_st_a_zero) +  \
                    ' p=' + str(_st_a_pos) + ' n=' + str(_st_a_neg) + _st_zamb_info + _st_pamb_info +  \
                    _st_namb_info + _st_zambd_info + _st_zdp_info + _st_pdp_info + _st_ndp_info
        logging.info('info_a: ' + st_a_info)
        _st_b_pick_info = ''
        _st_b_seg_info = str(st_b_seg_data_total) + '@' + str(_seg_begin) + '/' + \
                         str(_seg_end) + '#' if my_is_data_seg else ''
        st_b_info = 't=' + _st_b_seg_info + _st_b_pick_info + str(st_b_total_raw) + ' s=' + str(st_b_shape_raw) + \
                    ' inf=' + str(_st_b_inf) + ' nan=' + str(_st_b_nan) + ' z=' + str(_st_b_zero) + ' p=' + \
                    str(_st_b_pos) + ' n=' + str(_st_b_neg) + _st_zbma_info + _st_pbma_info + _st_nbma_info + \
                    _st_zambd_info + _st_zdp_info + _st_pdp_info + _st_ndp_info
        logging.info('info_b: ' + st_b_info)
        ...
        g_data_a_info = 'pmax=' + str(g_aa_pos_max_s) + ' nmin=' + str(g_aa_neg_min_s) + ' avg=' + \
                        str(g_aa_avg_s) + ' avgnz=' + str(g_aa_avg_nz_s) + ' pavg=' + str(g_aa_pos_avg_s) + \
                        ' navg=' + str(g_aa_neg_avg_s) + ' pmin=' + str(g_aa_pos_min_s) + ' nmax=' + str(g_aa_neg_max_s)
        g_data_a_info = g_data_a_info + ' pmaxa=' + str(g_aa_pos_max_avg_s) + ' pmina=' + str(g_aa_pos_min_avg_s) + \
                        ' nmaxa=' + str(g_aa_neg_max_avg_s) + ' nmina=' + str(g_aa_neg_min_avg_s)
        ...
        g_data_b_info = 'pmax=' + str(g_bb_pos_max_s) + ' nmin=' + str(g_bb_neg_min_s) + ' avg=' + str(g_bb_avg_s) + \
                        ' avgnz=' + str(g_bb_avg_nz_s) + ' pavg=' + str(g_bb_pos_avg_s) + ' navg=' + \
                        str(g_bb_neg_avg_s) + ' pmin=' + str(g_bb_pos_min_s) + ' nmax=' + str(g_bb_neg_max_s)
        g_data_b_info = g_data_b_info + ' pmaxa=' + str(g_bb_pos_max_avg_s) + ' pmina=' + str(g_bb_pos_min_avg_s) + \
                        ' nmaxa=' + str(g_bb_neg_max_avg_s) + ' nmina=' + str(g_bb_neg_min_avg_s)
        ...
        if self.is_calc_inc:
            ...
            g_diff_info = 'diff_num=' + str(diff_inc_diff_num) + ' pmax=' + str(g_diff_inc_pos_max_s) + \
                          ' nmin=' + str(g_diff_inc_neg_min_s) + ' pavg=' + str(g_diff_inc_pos_avg_s) + \
                          ' navg=' + str(g_diff_inc_neg_avg_s) + ' pmin=' + str(g_diff_inc_pos_min_s) + \
                          ' nmax=' + str(g_diff_inc_neg_max_s)
        ...
        if self.is_calc_rel:
            ...
            diff_rel_info = 'avg=' + str(g_diff_rel_avg_s) + ' max=' + str(g_diff_rel_max_s) + ' min=' + \
                            str(g_diff_rel_min_s) + ' pavg=' + str(g_diff_rel_pos_avg_s) + ' pmax=' + \
                            str(g_diff_rel_pos_max_s) + ' pmin=' + str(g_diff_rel_pos_min_s)
        ...
        if not self.is_out_no_fig:
            ...
            _subg_mark = [1, self.is_calc_inc, self.is_calc_rel]
...
if __name__ == '__main__':
    if len(sys.argv) < 4:
        ...
        analyzer = DataDiffAnalyzer()
        a, b, f_type = analyzer.prepare_data()
        sort = analyzer.is_sort
        analyzer.fix_input_and_compute(a, b, f_type, sort)