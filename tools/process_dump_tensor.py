########################################
# Invoke this script in ascendcpp/ dir #
########################################
...
def parse_tile_fwk_aicpu_ctrl(filename, inputs, outputs) -> List[RawTensorDesc]:
    ...
    for idx, input in enumerate(inputs):
        if input not in io_map:
            io_map[input] = 'i' + str(idx)
        else:
            io_map[input] += '-i' + str(idx)
    for idx, output in enumerate(outputs):
        if output not in io_map:
            io_map[output] = 'o' + str(idx)
        else:
            io_map[output] += 'o' + str(idx)
 
    rawTensors: List[RawTensorDesc] = []
    pattern = r'DumpTensorAddrInfo\s+"([^"]+)"'
    tensor_cache = {}
    with open(filename, 'r') as f:
        ...
        for line in f:
            ...
            if stripped_line.startswith("[DumpTensor] >>>"):
                startRecording = True
                continue
    ...
    def query(self, addr_start, addr_end):
        ...
        for base, size, data in self.blocks:
            ...
            if addr_end <= base or addr_start >= block_end:
                continue
            # 计算交集范围
            start_in_block = max(addr_start, base)
            end_in_block = min(addr_end, block_end)
            offset_in_block = start_in_block - base
            length = end_in_block - start_in_block
            result.extend(data[offset_in_block:offset_in_block + length])
        return bytes(result)
...
def main():
    ...
    def dump_raw_tensor(rt: RawTensorDesc):
        ...
        if rt.symlink_src is not None:
            assert rt.symlink_src != rt.name() # No self-symlink
    ...
    for i in range(0, len(raw_tensors), task_block):
        j = min(i + task_block, len(raw_tensors))
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            executor.map(dump_raw_tensor, raw_tensors[i:j])
        time_curr = time.perf_counter()
        print(f"{j}/{len(raw_tensors)} tasks have done, cost {time_curr - time_start:.1f}s")
