...
class IRToPyPTOConverter:
    ...
    def convert_ir_text(self, ir_text: str) -> str:
        ...
        lines.append("    ")
        lines.append("    # Setup device for NPU mode")
        lines.append("    if run_mode == \"npu\":")
        lines.append("        import torch_npu")
        lines.append("        device_id = int(os.environ.get('TILE_FWK_DEVICE_ID', '0'))")
        lines.append("        torch.npu.set_device(device_id)")
        lines.append("        device = f'npu:{device_id}'")
        lines.append("    else:")
        lines.append("        device = 'cpu'")
        lines.append("    ")

        # Generate test inputs based on parameters
        # Note: frontend.jit functions return values, so output parameters are not needed in test function
        test_inputs = []
        for param_name, param_type, direction in param_list:
            if direction == "out":
                # Output parameters are handled by return values in frontend.jit, skip them
                continue
            else:
                ...
                if "tensor" in param_type.lower() or "tile" in param_type.lower():
                    ...
                    lines.append(f"    torch.manual_seed(42)  # For reproducible results")
                    lines.append(f"    {param_name} = torch.randn({shape}, dtype=torch.float32, device=device)")
                else:
        ...
        lines.append("    ")
        lines.append("    # Create numpy arrays for golden data generation (same data as torch tensors)")
        lines.append("    import numpy as np")
        lines.append("    # Convert torch tensors to numpy for golden data comparison")
        for param_name, param_type, direction in param_list:
            if direction == "out":
                continue
            if "tensor" in param_type.lower() or "tile" in param_type.lower():
                lines.append(f"    {param_name}_numpy = {param_name}.cpu().numpy()")
        ...
        if return_vars or return_tensors:
            if (return_vars and len(return_vars) == 1) or (not return_vars and len(return_tensors) == 1):
                ...
                lines.append("        ")
                lines.append("        # Compare with numpy golden data")
                lines.append("        result_numpy = result_torch.cpu().numpy()")
                lines.append("        print(f\"Result (numpy): {result_numpy}\")")
                lines.append("        print(f\"Result shape: {result_numpy.shape}\")")
            else:
                ...
                if return_vars:
                    for i, var in enumerate(return_vars):
                        ...
                        lines.append(f"        result_torch = results[{i}].to_torch()")
                        lines.append(f"        print(f\"Result {i} ({var}): {result_torch}\")")
                        lines.append(f"        result_numpy = result_torch.cpu().numpy()")
                        lines.append(f"        print(f\"Result {i} (numpy): {result_numpy}\")")
        else:
