...
CANN_BASE_URL="https://ascend.devcloud.huaweicloud.com/artifactory/cann-run-mirror/software/master/"

CANN_TOOLKIT_URL_X86="https://ascend.devcloud.huaweicloud.com/artifactory/cann-run-mirror/software/master/Ascend-cann-toolkit_9.0.0_linux-x86_64.run"
CANN_TOOLKIT_URL_ARM="https://ascend.devcloud.huaweicloud.com/artifactory/cann-run-mirror/software/master/Ascend-cann-toolkit_9.0.0_linux-aarch64.run"
...
CANN_OPS_URL_X86_910b="https://ascend.devcloud.huaweicloud.com/artifactory/cann-run-mirror/software/master/Ascend-cann-910b-ops_9.0.0_linux-x86_64.run"
CANN_OPS_URL_ARM_910b="https://ascend.devcloud.huaweicloud.com/artifactory/cann-run-mirror/software/master/Ascend-cann-910b-ops_9.0.0_linux-aarch64.run"
CANN_OPS_URL_X86_910c="https://ascend.devcloud.huaweicloud.com/artifactory/cann-run-mirror/software/master/Ascend-cann-A3-ops_9.0.0_linux-x86_64.run"
CANN_OPS_URL_ARM_910c="https://ascend.devcloud.huaweicloud.com/artifactory/cann-run-mirror/software/master/Ascend-cann-A3-ops_9.0.0_linux-aarch64.run"
...
update_url() {
    ...
    if ! curl -sS -f -o "$tmpfile" "$CANN_BASE_URL" 2>/dev/null; then
        rm -f "$tmpfile"
        echo ""
        return 1
    fi
    ...
    local latest_dir
    latest_dir=$(grep -oP 'href="\K[0-9]+(?=/")' "$tmpfile" | sort -n | tail -1)
    rm -f "$tmpfile"
    ...
    local latest_url="${CANN_BASE_URL}${latest_dir}/"
    local index_html
    index_html=$(mktemp)

    if ! curl -sS -f -o "$index_html" "$latest_url" 2>/dev/null; then
        rm -f "$index_html"
        echo ""
        return 1
    fi
    ...
    local cann_version
    cann_version=$(grep -oP 'Ascend-cann-toolkit_\K[0-9]+\.[0-9]+\.[0-9]+' "$index_html" | head -1)
    rm -f "$index_html"
    ...
    # 更新cann version
    CANN_VERSION_LATEST="${cann_version}"

    # 更新URL
    CANN_TOOLKIT_URL_X86="${latest_url}Ascend-cann-toolkit_${cann_version}_linux-x86_64.run"
    CANN_TOOLKIT_URL_ARM="${latest_url}Ascend-cann-toolkit_${cann_version}_linux-aarch64.run"

    CANN_OPS_URL_X86_910b="${latest_url}Ascend-cann-910b-ops_${cann_version}_linux-x86_64.run"
    CANN_OPS_URL_ARM_910b="${latest_url}Ascend-cann-910b-ops_${cann_version}_linux-aarch64.run"

    CANN_OPS_URL_X86_910c="${latest_url}Ascend-cann-A3-ops_${cann_version}_linux-x86_64.run"
    CANN_OPS_URL_ARM_910c="${latest_url}Ascend-cann-A3-ops_${cann_version}_linux-aarch64.run"
}
...
download_cann_packages() {
    ...
    log_print "info" "Update latest CANN version"
    update_url
