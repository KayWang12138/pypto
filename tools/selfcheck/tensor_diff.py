...
import os
import torch
def print_tensor_stats_info(stats, prefix='tensor'):
    ...
    print(f'{"data "+prefix:>8s} : {dtype_str:>10s}{sep}{shape}{sep}{cnt_all}{sep}{cnt_z}{sep}{cnt_infnan}{sep}{st_max:g}{sep}{st_min:g}{sep}{st_avg:g}{sep}{st_aavg:g}\t(dtype/shape/all/zero/infnan/max/min/avg/aavg)')
    pass

def calc_tensor_stats(T, calc_dtype=torch.float64):
    assert calc_dtype in [torch.float64, torch.float32], f'not support calculating dtype: {calc_dtype}'
    t = T.flatten().to(calc_dtype)

    dtype_str = str(T.dtype).lstrip('torch.')
    shape = list(T.shape)
    cnt_all = t.numel()
    cnt_z = (t == 0).sum().item()
...
def print_isclose_info(result_is_close, result_reason_str, result_info, topk=16):
    ...
    sep = ', '
    print(f'     cnt : {cnt_all}{sep}{cnt_picked}{sep}{cnt_out_bothzero}{sep}{cnt_out_pass}{sep}{cnt_out_warn}{sep}{cnt_fail}{sep}{cnt_infnan}\t(all/picked/zero/pass/warn/fail/infnan)')
    print(f'    conf : {rtol:g}{sep}{atol:g}{sep}{_fail_factor}{sep}{tol_cnt}\t(rtol/atol/_fail_factor/tol_cnt)')

    if sum(_d_extra) != _d_extra[2]:
        print(f'  _extra : {cnt_warn_ww}{sep}{cnt_warn_w}{sep}{cnt_out_warn}{sep}{cnt_warn_s}{sep}{cnt_warn_ss}\t(ww/w/warn/s/ss)')
    print(f'is_close : {result_is_close}\t({result_reason_str})')

    def print_info(d_detail, d_str=''):
        num = len(d_detail[0])
        if num > 0:
            print(f'-{d_str}{"("+str(num)+")":-<{10}s} {"off":-^{10-3}s} {"a":-^14s} {"b":-^14s} {"ad":-^14s} {"rd":-^14s} {"ad_tol":-^14s} {"off_raw":-^{20}s}')
            ...
            for i in range(num_picked):
                ...
                info_a = d_detail[2][i].item()
                info_b = d_detail[3][i].item()
                info_ad = d_detail[4][i].item()
                info_rd = d_detail[5][i].item()
                info_tol = d_detail[6][i].item()
                print(f'{i+1:12g}){info_off:10d} {info_a:>14g} {info_b:>14g} {info_ad:>14g} {info_rd:<14g} {info_tol:<14g} {info_off_raw:20s}')
            if num_picked < num and num_picked > 0:
                print(f' ...... {topk+1}~{num} ({num-num_picked}) ......')

    print_info(d_detail_infnan, 'bad ')
    print_info(d_detail_fail, 'fail')
    print_info(d_detail_warn, 'warn')
...
def check_isclose(A, B, rtol=1.0e-2, atol=1.0e-2, calc_dtype=torch.float64, shape=None,
    ...
    cnt_adj = int(cnt_picked**0.5) // 2
    if tol_cnt_raw == 0:
        tol_cnt = min(16, cnt_adj)    # tol_cnt is zero, adjust to small value
    if tol_cnt_raw == 0:
        if cnt_adj > 0:
            tol_cnt = min(8, cnt_adj)    # tol_cnt is zero, adjust to small value
    elif tol_cnt_raw < cnt_adj:
        if cnt_adj < 16:
            tol_cnt = cnt_adj    # tol_cnt is too small, adjust to "cnt_adj"
...
if __name__ == '__main__':
    ...
    def usage_exit():
        print(f'Usage: {sys.argv[0]} <file_a> <file_b> <DTYPE_a>-<DTYPE_b>', file=sys.stderr)
        print(f'  <DTYPE>:   {"|".join(get_valid_file_type_list())}', file=sys.stderr)
    ...
    if False:  # XXX debug
        A = torch.tensor([8, 0, 2, float('inf'), 0, 0, 5, 7, 0], dtype=torch.float32)
        B = torch.tensor([4, 6, 0, 2, float('inf'), float('nan'), 0, 9, 0], dtype=torch.float32)

    #print(f'A: {A.dtype}')
    #print(f'B: {B.dtype}')

    stats_a = calc_tensor_stats(A, calc_dtype=calc_dtype)
    stats_b = calc_tensor_stats(B, calc_dtype=calc_dtype)
    print_tensor_stats_info(stats_a, prefix='a')
    print_tensor_stats_info(stats_b, prefix='b')

    result_is_close, result_reason_str, result_info = check_isclose(A, B, rtol=1.0e-3, atol=1.0e-3, calc_dtype=torch.float32, is_ignore_bothzero=True, is_detail=True)
    print_isclose_info(result_is_close, result_reason_str, result_info, topk=None)
