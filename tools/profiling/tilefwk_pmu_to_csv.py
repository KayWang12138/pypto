...
def main():
    ...
    parser.add_argument('--arch', default='auto', choices=['auto', 'dav_2201', 'dav_3510'],
                        help="架构类型，默认auto根据pmu计数器数量判断")
    ...
    if not table_pmu_header and pmu_cnt_num > 0:
        ...
        elif len(table_pmu_header) < pmu_cnt_num:
            start = len(table_pmu_header)
            table_pmu_header += [f"pmu_cnt{i}" for i in range(start, pmu_cnt_num)]
