...
def analyze_wait_time(
    ...
    detailed: bool,
    tilefwk_exec_cycles: Dict[Tuple[str, int], float],
) -> None:
    ...
    if detailed:
        base_idx_by_kind: Dict[str, int] = {}
        for row in per_core_rows:
            kind = "AIV" if "-AIV" in row["core_type"] else "AIC"
            if kind not in base_idx_by_kind:
                base_idx_by_kind[kind] = row["block_idx"]
            else:
                base_idx_by_kind[kind] = min(base_idx_by_kind[kind], row["block_idx"])
        ...
        for row in sorted(per_core_rows, key=lambda x: x["block_idx"]):
            ...
            kind = "AIV" if "-AIV" in core_type else "AIC"
            local_block = block_idx - base_idx_by_kind.get(kind, 0)
            exec_cyc = tilefwk_exec_cycles.get((kind, local_block))
            exec_str = "-" if exec_cyc is None else f"{exec_cyc:.0f}"

            util_str = "-"
            if exec_cyc is not None and sched_idx is not None and sched_idx in sched_duration and sched_duration[sched_idx] > 0:
                util = safe_div(exec_cyc * 100.0, sched_duration[sched_idx])
                util_str = f"{util:.2f}"
        ...
        print_table(
            ["blockIdx", "coreType", "wait_ctrl(cyc/us)", "wait_sched(cyc/us)", "exec(cyc)", "util(%)"],
            rows,
        )
...
def main() -> None:
    ...
    tilefwk_exec_cycles, per_type = collect_tilefwk_exec_cycles(output_dir)
    analyze_wait_time(aicpu_dev_pref, args.detailed, tilefwk_exec_cycles)
