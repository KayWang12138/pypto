...
*save_best_k* - allows saving folder with launch informaiton only for best combinations
...
To speed up the search, we can use the XGBoostRegressor model, which is capable of predicting preformance on tiling combination.
...
* `true` if result was achived by running on device
