...
class RandomParameterMutator():
    '''
    Takes the current combination of parameters -> selects one parameter -> get next value in candidates list or previous.
    '''
...
def make_flat(json_confg):
    ...
    for file_conf in json_confg["files"]:
        ...
        keys = [
            ...
            if not key.startswith("string") and not key.startswith("line")
        ]
