...
def analyze_aicore(aicore_exec_rows: List[Dict[str, Any]]) -> None:
    ...
    for round_id in sorted(rows_by_round.keys(), key=lambda x: -1 if x is None else x):
        ...
        metric_rows.append(["AICore", format_round(round_id), e2e_time, total_runtime_max])

    print_table(["Compute Units", "Round", "End-to-End time", "Total run time (max)"], metric_rows)
