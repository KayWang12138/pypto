...
def analyze_stage1_sched(aicpu_dev_pref: List[Dict[str, Any]]) -> None:
    ...
    for s in scheds:
        ...
        total_dur = None
        durations = [alloc_dur, init_dur, handshake_dur, dev_task_rcv]
...
def analyze_stage1_aicore(aicore_wait_rows: List[Dict[str, Any]]) -> None:
    ...
    print_table(
        ["Stats", "count", "min(us)", "avg(us)", "max(us)"],
        [["AICore wait for dev task", stat[0], stat[1], stat[2], stat[3]]]
    )
...
def analyze_stage3(aicore_wait_rows: List[Dict[str, Any]]) -> None:
    ...
    print_table(
        ["Stats", "count", "min(us)", "avg(us)", "max(us)"],
        [["AICore exit wait", stat[0], stat[1], stat[2], stat[3]]]
    )
