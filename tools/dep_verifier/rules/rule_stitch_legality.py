...
# -----------------------------------------------------------------------------------------------------------
"""Read/write linkage legality.

Every runtime read/write linkage between two kernel ops must use a tensor
that is actually declared as the producer's output and the consumer's input.
A violation here means the runtime is reading from / writing to a tensor
that the kernel API does not actually expose, which always points to an
encoder bug or a corrupted dependency record.
"""
