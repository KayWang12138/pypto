...
def load_static_topo(path: str) -> Dict[int, StaticFunction]:
    ...
    with open(path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            fn.ops[op.op_idx] = op
    logger.info("  静态 function=%d, op=%d", len(functions),
...
def load_dyn_topo(path: str) -> List[DynTask]:
    ...
    logger.info("  动态 task 实例=%d", len(tasks))
    return tasks
...
def load_dyn_stitch_edges(path: str) -> List[StitchEdge]:
    ...
    logger.info("  stitch 边=%d", len(edges))
    return edges


def load_slot_mapping(path: str) -> Tuple[Dict[int, int], Dict[int, str]]:
    """slot_mapping.csv → (frontend→runtime, runtime→role)。

    role ∈ {INPUT, OUTPUT, INOUT, INTERNAL}。缺列视为 INTERNAL。
    """
    logger.info("加载 slot_mapping: %s", path)
    f2r: Dict[int, int] = {}
    r2role: Dict[int, str] = {}
    with open(path, "r", encoding="utf-8") as f:
        reader = csv.reader(f)
        ...
        for row in reader:
            ...
            if role not in ("INPUT", "OUTPUT", "INOUT", "INTERNAL"):
                role = "INTERNAL"
            f2r[front] = rt
            r2role[rt] = role  # C++ 侧已按 runtime slot 判定，同一 rt 多行 role 一致
    logger.info("  slot 映射=%d", len(f2r))
    return f2r, r2role
...
def load_slot_cell_table(path: str) -> Dict[int, List[CellTableDesc]]:
    ...
    logger.info("  cell 表 slot=%d", len(out))
    return dict(out)
...
def load_slot_access(path: str) -> List[SlotAccessEvent]:
    ...
    logger.info("  slot 访问事件=%d", len(events))
    return events


def infer_edge_seq_no(edges: List[StitchEdge], tasks: List[DynTask]) -> None:
    """为每条 stitch 边推断唯一 seqNo，存入 edge.inferred_seq_no（失败为 None）。"""
