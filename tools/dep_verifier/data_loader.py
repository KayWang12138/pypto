...
# -----------------------------------------------------------------------------------------------------------
"""Parsers for dump files used by verifier rules."""
import csv
import logging
...
def _parse_slot_list(cell: str) -> List[int]:
    """Parse slot list fields like "[7;8]", "[]", or ""."""
    if not cell:
        return []
    s = cell.strip()
    if not s or s == "[]":
...
def _parse_bracket_int_list(cell: str) -> List[int]:
    """Parse integer list fields like "[0,1,2]" or "[]"."""
    if not cell:
        return []
    s = cell.strip().strip('"')
    if not s or s == "[]":
...
def load_static_topo(path: str) -> Dict[int, StaticFunction]:
    """static_topo.csv → { funcKey: StaticFunction }"""
    logger.info("load static_topo: %s", path)
    ...
    with open(path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            op = StaticOp(
                ...
                incast_slots=_parse_slot_list(row[col["incastSlots"]]),
                outcast_slots=_parse_slot_list(row[col["outcastSlots"]]),
            ...
            fn.ops[op.op_idx] = op
    logger.info("  static function=%d, op=%d", len(functions),
                sum(len(fn.ops) for fn in functions.values()))
    return functions


def load_dyn_topo(path: str) -> List[DynTask]:
    """dyn_topo.txt → [DynTask]"""
    logger.info("load dyn_topo: %s", path)
    tasks: List[DynTask] = []
    with open(path, "r", encoding="utf-8") as f:
        reader = csv.reader(f)
        col = _header_idx(path, reader)
    ...
    logger.info("  dynamic task instances=%d", len(tasks))
    return tasks


def load_dyn_stitch_edges(path: str) -> List[StitchEdge]:
    """dyn_stitch_edges.csv → [StitchEdge]"""
    logger.info("load dyn_stitch_edges: %s", path)
    ...
    logger.info("  stitch edges=%d", len(edges))
    return edges
...
def load_slot_mapping(
    ...
    """slot_mapping.csv -> (frontend->runtime, runtime->role, runtime->tensor, runtime->func)."""
    logger.info("load slot_mapping: %s", path)
    f2r: Dict[int, int] = {}
    r2role: Dict[int, str] = {}
    ...
    logger.info("  slot mapping=%d", len(f2r))
    return f2r, r2role, r2tensor, r2func


def _merge_unique(existing: str, incoming: str) -> str:
    """Merge two ';'-separated value strings and deduplicate while preserving order."""
    if not existing:
        return incoming
...
def load_slot_cell_table(path: str) -> Dict[int, List[CellTableDesc]]:
    """slot_cell_table.csv → { slotIdx: [CellTableDesc] }"""
    logger.info("load slot_cell_table: %s", path)
    ...
    logger.info("  slots with cell table=%d", len(out))
    return dict(out)


def load_slot_access(path: str) -> List[SlotAccessEvent]:
    """dyn_slot_access.csv → [SlotAccessEvent]"""
    logger.info("load dyn_slot_access: %s", path)
    ...
    logger.info("  slot access events=%d", len(events))
    return events


def infer_edge_seq_no(edges: List[StitchEdge], tasks: List[DynTask]) -> None:
    """Infer unique seqNo for each stitch edge and write to edge.inferred_seq_no."""
