...
from neg_golden import neg_golden
from neg_impl import neg_wrapper
...
def test_neg_level0(device_id=None, run_mode="npu"):
    """2D 基础功能验证。"""
    print("=" * 60)
    print("Test: neg Level 0 (2D)")
    print("=" * 60)
    device = f"npu:{device_id}" if (run_mode == "npu" and device_id is not None) else "cpu"
    torch.manual_seed(0)
    x = torch.randn(1024, 1024, dtype=torch.float32, device=device)
    result = neg_wrapper(x)
    golden = neg_golden(x)
...
def test_neg_level1(device_id=None, run_mode="npu"):
    """3D 验证。"""
    print("=" * 60)
    print("Test: neg Level 1 (3D)")
    print("=" * 60)
    device = f"npu:{device_id}" if (run_mode == "npu" and device_id is not None) else "cpu"
...
def test_neg_level2(device_id=None, run_mode="npu"):
    """4D 验证。"""
    print("=" * 60)
    print("Test: neg Level 2 (4D)")
    print("=" * 60)
    device = f"npu:{device_id}" if (run_mode == "npu" and device_id is not None) else "cpu"
...
def test_neg_fp16(device_id=None, run_mode="npu"):
    """FP16 dtype 支持。"""
    print("=" * 60)
    print("Test: neg FP16")
    print("=" * 60)
...
def test_neg_large(device_id=None, run_mode="npu"):
    """大规模验证。"""
    print("=" * 60)
    print("Test: neg Large")
    print("=" * 60)
    device = f"npu:{device_id}" if (run_mode == "npu" and device_id is not None) else "cpu"
...
EXAMPLES = {
    "neg::test_neg_level0": {"name": "neg Level 0", "function": test_neg_level0},
    "neg::test_neg_level1": {"name": "neg Level 1", "function": test_neg_level1},
    "neg::test_neg_level2": {"name": "neg Level 2", "function": test_neg_level2},
    "neg::test_neg_fp16": {"name": "neg FP16", "function": test_neg_fp16},
    "neg::test_neg_large": {"name": "neg Large", "function": test_neg_large},
}


def main():
    parser = argparse.ArgumentParser(description="PyPTO neg operator test")
