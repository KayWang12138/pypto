...
# -----------------------------------------------------------------------------------------------------------
"""
PyPTO Mul kernel implementation.
...
def mul_wrapper(x1: torch.Tensor, x2: torch.Tensor) -> torch.Tensor:
    """Mul 算子 wrapper。
    ...
    if ndim == 1:
        ...
        mul_kernel_1d(x1_broadcast, x2_broadcast, output)
    elif ndim == 2:
        output = torch.empty(output_shape, dtype=torch.float32, device=x1.device)
        mul_kernel_2d(x1_broadcast, x2_broadcast, output)
    else:
        ...
        mul_kernel_2d(x1_2d, x2_2d, output_2d)
