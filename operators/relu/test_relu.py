...
# -----------------------------------------------------------------------------------------------------------
"""
ReLU PyPTO Operator Test

测试入口:
- 从 relu_golden 导入 relu_golden 函数
- 从 relu_impl 导入 relu_wrapper 函数
- 精度对比使用 numpy.testing.assert_allclose
- 测试用例基于 spec.md 典型配置

运行方式:
1. 设置 TILE_FWK_DEVICE_ID 环境变量
2. 运行 test: python test_relu.py [case_id]
3. 支持参数:
    --run_mode: npu 或 sim
"""
...
# ─────────────────────────────────────────────
# 2. 测试函数（基于 spec.md 兺型配置)
# ─────────────────────────────────────────────

def test_relu_perf_p0(device_id=None, run_mode="npu"):
    """性能_P0: 核心性能场景 [1024, 1024]"""
    print("=" * 60)
    print("Test: ReLU 性能_P0 (1024, 1024)")
    print("=" * 60)
    ...
    for dtype in dtypes:
        ...
        result = relu_wrapper(x)
        golden = relu_golden(x)
...
def test_relu_func_p0(device_id=None, run_mode="npu"):
    """功能_P0: 核心功能验证 [32, 64]"""
    print("=" * 60)
    print("Test: ReLU 功能_P0 (32, 64)")
    print("=" * 60)
...
def test_relu_func_p1(device_id=None, run_mode="npu"):
    """功能_P1: 3维输入验证 [2, 128, 256]"""
    print("=" * 60)
    print("Test: ReLU 功能_P1 (2, 128, 256)")
    print("=" * 60)
...
def test_relu_func_p2(device_id=None, run_mode="npu"):
    """功能_P2: 4维输入验证 [1, 1, 64, 64]"""
    print("=" * 60)
    print("Test: ReLU 功能_P2 (1, 1, 64, 64)")
    print("=" * 60)
...
EXAMPLES = {
    "relu::test_relu_perf_p0": {
        "name": "ReLU 性能_P0",
        "description": "核心性能场景 [1024, 1024]",
        "function": test_relu_perf_p0,
    },
    "relu::test_relu_func_p0": {
        "name": "ReLU 功能_P0",
        "description": "核心功能验证 [32, 64]",
        "function": test_relu_func_p0,
    },
    "relu::test_relu_func_p1": {
        "name": "ReLU 功能_P1",
        "description": "3维输入验证 [2, 128, 256]",
        "function": test_relu_func_p1,
    },
    "relu::test_relu_func_p2": {
        "name": "ReLU 功能_P2",
        "description": "4维输入验证 [1, 1, 64, 64]",
        "function": test_relu_func_p2,
    },
}
...
def main():
    parser = argparse.ArgumentParser(
        description="PyPTO ReLU operator test",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s relu::test_relu_perf_p0    Run 性能 P0
  %(prog)s relu::test_relu_func_p0    Run 功能 P0
  %(prog)s relu::test_relu_func_p1    Run 功能 P1
  %(prog)s relu::test_relu_func_p2    Run 功能 P2
