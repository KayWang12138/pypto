...
- **算子名称**: max_reduction
- **算子分类**: reduction
- **数学公式**: $y = \text{max}(x, \text{dim}) = \max_{i \in \text{dim}} x_i$
- **功能描述**: 沿指定轴计算张量最大值的归约操作，类似于 PyTorch 的 `torch.max(input, dim, keepdim)` 或 `tensor.max(dim)`。支持动态轴（batch、seq 等维度）和 keepdim 参数。
...
| 变量 | Shape | Dtype | 动态轴 | 说明 |
|------|-------|-------|--------|------|
| y | 去掉 dim 维度的 shape (keepdim=False) 或保持维度 (keepdim=True) | float32 | b (若 dim != 0) | 归约后的最大值张量 |
...
### 11. 参考信息
- **参考实现**: PyTorch `torch.max(input, dim, keepdim=False)` - https://pytorch.org/docs/stable/generated/torch.max.html
- **论文**: N/A
- **类似算子**: sum_reduction, mean_reduction, min_reduction

### 12. 应用场景
- **目标模型**: 通用 Transformer 模型
- **使用位置**: 最大池化、特征提取、注意力机制中的 max pooling、RoI Pooling 等
