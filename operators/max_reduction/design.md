# max_reduction 算子设计文档

> **算子名称**: max_reduction
> **算子分类**: reduction
> **生成时间**: 2026-03-29T15:15:00Z
...
### 1.1 功能描述
沿指定轴计算张量最大值的归约操作，类似于 PyTorch 的 `torch.max()` 或 `tensor.max()`。支持动态轴（batch、seq 等维度)和 keepdim 参数。
### 1.2 数学公式
$$y = \text{max}(x, \text{dim}) = \max_{i \in \text{dim\_axis} x_i$$
### 1.3 算法描述
...
公式: y = max(x, dim) = max(x_i for i in dim_axis)  (沿指定轴dim归约)
动态轴: b (batch), s (seq)
```
---
...
| 步骤 | 数学表达 | 说明 |
|------|----------|------|
| 1 | max(x, dim, keepdim) | 沿 dim 轴求最大值 |
### 2.2 PyPTO API 映射表
| 步骤 | 数学表达 | PyPTO API | 参数 | 文档路径 |
|------|----------|-----------|------|----------|
| 1 | max(x, dim, keepdim) | `pypto.max(input, dim, keepdim)` | input: x, dim: int, keepdim: bool | `docs/api/operation/pypto-max.md` |
**说明**: PyPTO 揚供直接的 `pypto.max` API，无需 substitute 方案。
### 2.3 计算步骤序列
```python
# 伪代码展示计算流程
# 步骤 1: 沿 dim 轴求最大值
...
```
### 2.4 设计依据
- 来源：api_report.md + docs/api/operation/pypto-max.md
- 说明： PyPTO 揚供直接的 max API，语义与 torch.max 完全一致，支持动态轴和 keepdim 参数。
---
...
```python
@dataclass
class MaxReductionOutput:
    y: Tensor      # 归约后的最大值张量, shape: 去掉 dim 维度的 shape 或保持维度 (keepdim=True), dtype: DT_FP32
```
### 3.3 中间 Tensor 定义
| 名称 | Shape | Dtype | 说明 |
|------|-------|-------|------|
| 无 | - | - | 直接使用 pypto.max 输出，无需中间 tensor |
### 3.4 数据格式选择
| Tensor | 格式 | 说明 |
|--------|------|------|
...
2. **次尾轴限制**: max API 要求次尾轴 ≤ 255，设置 tile_s = 8 满足要求
...
### 5.1 Loop 判断结论
- **结论**: 不需要 pypto.loop
- **原因**: 虽然存在动态轴（b, s），但 PyPTO 的 max API 内部会自动处理动态轴的数据切分和迭代，编译器会生成相应的运行时代码
- **适用条件**: 单轴归约，使用 PyPTO 内置 max API
- **限制**: 多轴归约需要显式循环或多次调用
### 5.2 齒态轴 vs 动态轴处理
| 轴 | 类型 | 处理方式 |
|----|------|----------|
| b (dim=0 时) | 动态 | PyPTO max API 内部自动处理 |
| s (dim=1 时) | 动态 | PyPTO max API 内部自动处理 |
| n, d | 静态 | TileShape 直接覆盖 |
### 5.3 数据依赖处理
无复杂数据依赖。 max 为单步操作,无跨迭代依赖。
### 5.4 尾块处理策略
...
### 6.1 Golden 函数设计
```python
def max_reduction_golden(x: torch.Tensor, dim: int, keepdim: bool = False) -> torch.Tensor:
    """max_reduction 参考实现"""
...
| 场景 | 参数 | 说明 |
|------|------|------|
| 零值输入 | x = zeros | 验证 max of zeros = 0 |
| 常量输入 | x = full(5.0) | 验证 max of constant = constant |
...
## 8. 风险点与注意事项
### 8.1 已知约束
- pypto.max 要求输入 shape 为 2-4 维, Shape Size ≤ INT32_MAX
- pypto.max 要求 TileShape ≤ 64KB, 尾轴 32 bytes 对齐, 次尾轴 ≤ 255
- 输入张量必须 contiguous（is_contiguous() == True）
### 8.2 常见错误规避
| 风险 / 错误 | 触发场景 | 影响 / 原因 | 规避方法 |
|-------------|----------|-------------|----------|
| TileShape 未设置 | 调用 max 前未调用 set_vec_tile_shapes | 编译失败 | 确保在 max 调用前设置 TileShape |
...
| 封装 wrapper 函数 | 提供 max_reduction_wrapper 函数封装 from_torch、kernel 调用、输出转换 |
| 复用 golden 验证 | 直接使用 max_reduction_golden.py 中的验证逻辑 |
---

## 9. 交付件清单
### 9.1 目录结构
```
operators/max_reduction/
├── spec.md                          # 需求规范（已有）
├── api_report.md                    # API 探索报告（已有）
├── design.md                        # 设计文档（本文件）
├── max_reduction_golden.py         # Golden 参考实现（已有）
├── max_reduction_impl.py           # 算子实现代码
├── test_max_reduction.py           # 测试代码
...
| max_reduction_golden.py | 代码 | Golden 参考实现 | pypto-golden-generator |
| max_reduction_impl.py | 代码 | 算子核心实现 | pypto-op-develop |
| test_max_reduction.py | 代码 | 测试用例 | pypto-op-develop |
### 9.3 命名规范
| 项目 | 规范 | 示例 |
|------|------|------|
| 算子名称 | 小写字母 + 下划线 | `max_reduction` |
| 目录名 | 与算子名称一致 | `operators/max_reduction/` |
| Golden 文件 | `{op}_golden.py` | `max_reduction_golden.py` |
| 实现文件 | `{op}_impl.py` | `max_reduction_impl.py` |
| 测试文件 | `test_{op}.py` | `test_max_reduction.py` |
### 9.4 生成顺序
```
spec.md → api_report.md → design.md → max_reduction_golden.py → max_reduction_impl.py → test_max_reduction.py
```
