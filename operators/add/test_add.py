...
def test_add_broadcast(device=None):
    """测试广播."""
    print("\n[Test Broadcast]")
    x1 = torch.randn(2, 4, 128, 1024, dtype=torch.float32, device=device)
...
def test_add_scalar(device=None):
    """测试标量加法."""
    print("\n[Test Scalar]")
    x1 = torch.randn(128, 1024, dtype=torch.float32, device=device)
    x2 = torch.tensor(3.14, dtype=torch.float32, device=device)  # 标量
...
def test_add_large(device=None):
    """测试大规模输入."""
    print("\n[Test Large]")
...
def run_all_tests(device=None):
    ...
    try:
        ...
        test_add_3d(device)
        test_add_4d(device)
        test_add_broadcast(device)
        test_add_scalar(device)
        test_add_fp16(device)
        test_add_large(device)
...
def main():
    parser = argparse.ArgumentParser(description="Add operator test")
