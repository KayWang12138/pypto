# sum_reduction 算子设计文档

> **算子名称**: sum_reduction
> **算子分类**: reduction
> **生成时间**: 2026-03-29T08:50:00Z
> **基于**: spec.md
...
$$y = \text{sum}(x, \text{dim}) = \sum_{i} x_i$$
...
公式: y = sum(x, dim) = Σᵢ xᵢ  (沿指定轴dim归约)
动态轴: b (batch), s (seq)
```
...
| 步骤 | 数学表达 | 说明 |
|------|----------|------|
| 1 | y = sum(x, dim, keepdim) | 沿 dim 轴求和 |
...
**说明**: PyPTO 提供直接的 `pypto.sum` API，无需 substitute 方案。
...
### 2.4 设计依据
- 来源: spec.md + docs/api/operation/pypto-sum.md
- 说明: PyPTO 提供直接的 `pypto.sum` API，支持任意单轴归约和 keepdim 参数，功能完全匹配需求。
...
```python
@dataclass
class SumReductionOutput:
    y: Tensor      # 归约后的求和张量, shape: 去掉 dim 维度的 shape 或保持维度 (keepdim=True), dtype: DT_FP32
```
### 3.3 中间 Tensor 定义
无中间 Tensor。
### 3.4 数据格式选择
...
### 5.1 Loop 判断结论
- **结论**: 不需要 pypto.loop
- **原因**: 虽然存在动态轴 (b, s), 但 PyPTO 的 sum API 内部会自动处理动态轴的数据切分和迭代, 编译器会生成相应的运行时代码
- **适用条件**: 单轴归约, 使用 PyPTO 内置 sum API
- **限制**: 多轴归约需要显式循环或多次调用
### 5.2 静态轴 vs 动态轴处理
| 轴 | 类型 | 处理方式 |
|----|------|----------|
...
### 6.1 Golden 函数设计
```python
def sum_reduction_golden(x: torch.Tensor, dim: int, keepdim: bool = False) -> torch.Tensor:
    """sum_reduction 参考实现"""
...
| 场景 | 参数 | 说明 |
|------|------|------|
| 零值输入 | x = zeros | 验证 sum of zeros = 0 |
| 常量输入 | x = full(5.0) | 验证 sum of constant = constant * N |
...
| 封装 wrapper 函数 | 提供 sum_reduction_wrapper 函数封装 from_torch、kernel 调用、输出转换 |
| 复用 golden 验证 | 直接使用 sum_reduction_golden.py 中的验证逻辑 |
...
| sum_reduction_golden.py | 代码 | Golden 参考实现 | pypto-golden-generator |
| sum_reduction_impl.py | 代码 | 算子核心实现 | pypto-op-develop |
| test_sum_reduction.py | 代码 | 测试用例 | pypto-op-develop |
### 9.3 命名规范
| 项目 | 规范 | 示例 |
|------|------|------|
| 算子名称 | 小写字母 + 下划线 | `sum_reduction` |
| 目录名 | 与算子名称一致 | `operators/sum_reduction/` |
| Golden 文件 | `{op}_golden.py` | `sum_reduction_golden.py` |
| 实现文件 | `{op}_impl.py` | `sum_reduction_impl.py` |
| 测试文件 | `test_{op}.py` | `test_sum_reduction.py` |
### 9.4 生成顺序
```
spec.md → design.md → sum_reduction_golden.py → sum_reduction_impl.py → test_sum_reduction.py
```
