...
# -----------------------------------------------------------------------------------------------------------
"""PyPTO sum_reduction kernel implementation.

实现说明:
  - 本文件实现 sum_reduction 算子的 PyPTO kernel。
  - 导出函数 sum_reduction_wrapper() 供 test_sum_reduction.py 调用。
  - 使用 pypto.sum 直接 API 宲沿指定轴归约求和。
  - 支持动态轴 (b, s) 和 keepdim 参数。

设计依据:
  - 来源: design.md
  - API 映射: pypto.sum(input, dim, keepdim)
...
def sum_reduction_kernel(
    ...
    """PyPTO jit kernel for sum_reduction.

    实现 sum(x, dim) = sum(x, dim, keepdim)
...
def sum_reduction_wrapper(
    x: torch.Tensor,
    dim: int,
    keepdim: bool = False,
) -> torch.Tensor:
    """sum_reduction wrapper，供 test_sum_reduction.py 调用。
    ...
    sum_reduction_kernel(x, output, dim, keepdim)
