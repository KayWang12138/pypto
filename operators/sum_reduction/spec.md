...
- **算子名称**: sum_reduction
- **算子分类**: reduction
- **数学公式**: $y = \text{sum}(x, \text{dim}) = \sum_{i} x_i$
- **功能描述**: 沿指定轴计算张量元素求和的归约操作，类似于 PyTorch 的 `torch.sum()` 或 `tensor.sum()`。支持动态轴（batch、seq 等维度）和 keepdim 参数。
...
| 变量 | Shape | Dtype | 动态轴 | 说明 |
|------|-------|-------|--------|------|
| y | 去掉 dim 维度的 shape (keepdim=False) 或保持维度 (keepdim=True) | float32 | b (若 dim != 0) | 归约后的求和张量 |
...
### 11. 参考信息
- **参考实现**: PyTorch `torch.sum(input, dim, keepdim=False)` - https://pytorch.org/docs/stable/generated/torch.sum.html
- **论文**: N/A
- **类似算子**: mean_reduction, max_reduction, min_reduction

### 12. 应用场景
- **目标模型**: 通用 Transformer 模型
- **使用位置**: 损失函数计算、特征聚合、统计计算等
