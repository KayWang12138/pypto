...
# -----------------------------------------------------------------------------------------------------------
"""
PyPTO Eq kernel implementation.
...
def eq_kernel_2d(
    ...
    Args:
        ...
        out: 输出 tensor，bool 类型
    """
    # 使用小 tile shapes 兼容 BOOL 输出
...
def eq_wrapper(x1: torch.Tensor, x2: torch.Tensor) -> torch.Tensor:
    """Eq 算子 wrapper。
    ...
    # 根据维度选择 kernel
    if ndim == 1:
        output = torch.empty(output_shape, dtype=torch.bool, device=x1.device)
        eq_kernel_1d(x1_broadcast, x2_broadcast, output)
    elif ndim == 2:
        output = torch.empty(output_shape, dtype=torch.bool, device=x1.device)
        eq_kernel_2d(x1_broadcast, x2_broadcast, output)
    else:
        ...
        output_2d = torch.empty(x1_2d.shape, dtype=torch.bool, device=x1.device)
        eq_kernel_2d(x1_2d, x2_2d, output_2d)
