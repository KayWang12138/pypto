...
# -----------------------------------------------------------------------------------------------------------
"""PyPTO mean_reduction operator test.

测试说明:
  - 本文件测试 mean_reduction 算子的精度和功能。
  - golden 实现来自 mean_reduction_golden.py。
  - kernel 实现来自 mean_reduction_impl.py。
...
from mean_reduction_golden import mean_reduction_golden
...
def test_mean_reduction_level0(device_id=None, run_mode="npu"):
    """Level 0: 小数据量基础功能验证（8-16 元素）。"""
    print("=" * 60)
    print("Test: mean_reduction Level 0 (basic)")
    print("=" * 60)
    ...
    golden = mean_reduction_golden(x, dim=-1, keepdim=False)
    ...
    result = mean_reduction_wrapper(x, dim=0, keepdim=True)
    golden = mean_reduction_golden(x, dim=0, keepdim=True)
...
def test_mean_reduction_level1(device_id=None, run_mode="npu"):
    """Level 1: 典型场景验证（1K 元素）。"""
    print("=" * 60)
    print("Test: mean_reduction Level 1 (typical)")
    print("=" * 60)
    ...
    result = mean_reduction_wrapper(x, dim=1, keepdim=True)
    golden = mean_reduction_golden(x, dim=1, keepdim=True)
...
def test_mean_reduction_level2(device_id=None, run_mode="npu"):
    """Level 2: 边界和动态 shape 测试。"""
    print("=" * 60)
    print("Test: mean_reduction Level 2 (boundary)")
    print("=" * 60)
    ...
    result = mean_reduction_wrapper(x, dim=2, keepdim=False)
    golden = mean_reduction_golden(x, dim=2, keepdim=False)
    print(f"  Input shape : {x.shape}")
    print(f"  Output shape: {result.shape}")
    max_diff = np.abs(result.cpu().numpy() - golden.cpu().numpy()).max()
    print(f"  Max diff    : {max_diff:.6e}")
...
def test_mean_reduction_dynamic(device_id=None, run_mode="npu"):
    """动态 shape 测试（覆盖不同 batch/seq 组合）。"""
    print("=" * 60)
    print("Test: mean_reduction Dynamic Shape")
    print("=" * 60)
...
# 用例注册表
EXAMPLES = {
    "mean_reduction::test_mean_reduction_level0": {
        "name": "mean_reduction Level 0",
        "description": "小数据量基础功能验证",
        "function": test_mean_reduction_level0,
    },
    "mean_reduction::test_mean_reduction_level1": {
        "name": "mean_reduction Level 1",
        "description": "典型场景验证",
        "function": test_mean_reduction_level1,
    },
    "mean_reduction::test_mean_reduction_level2": {
        "name": "mean_reduction Level 2",
        "description": "边界和动态 shape 测试",
        "function": test_mean_reduction_level2,
    },
    "mean_reduction::test_mean_reduction_dynamic": {
        "name": "mean_reduction Dynamic",
        "description": "动态 shape 测试",
        "function": test_mean_reduction_dynamic,
    },
}
...
def main():
    parser = argparse.ArgumentParser(
        description="PyPTO mean_reduction operator test",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s mean_reduction::test_mean_reduction_level0    Run Level 0
