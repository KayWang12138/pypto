...
```
正在调用 gitcode-mcp-install skill...
```
...
### 关键约束（来自 pypto-pr-creator）

| 规则 | 说明 |
|------|------|
| **Commit Tag** | 必须是 feat/fix/docs/style/refactor/perf/test（⚠️ chore 不允许） |
| **格式** | `tag(scope): Summary`，冒号后有空格，Summary 首字母大写 |
| **语言** | 必须使用英文 |
| **长度** | Summary 10-200 字符，整个 commit message 不超过 10 行 |
| **PR 标题** | 与 Commit 首行一致 |
| **PR Body** | 禁止为空 |
...
| 场景 | Question 次数 | 说明 |
|------|---------------|------|
| 有 staged 文件，采用默认方案，GITCODE_TOKEN 已配置 | **1 次** | 仅在阶段3确认执行方案 |
| GITCODE_TOKEN 未配置（需调用 skill 配置） | 1+ 次 | 阶段0调用 skill 配置（skill 内部可能有交互）+ 阶段3确认执行 |
| 无 staged 文件 | 2 次 | 阶段1选择提交范围 + 阶段3确认执行 |
| 自定义 Commit | 2 次 | 阶段3选择自定义 + 输入自定义 commit |
| 自定义目标分支 | 2 次 | 阶段3选择自定义 + 选择目标分支 |