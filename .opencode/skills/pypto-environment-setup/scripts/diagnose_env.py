    ...
    *,
    ...
    if not fast:
        for name, min_ver in [
            ...
            if not info.get('found'):
                ...
                issues.append({'component': f'build:{name}', 'severity': 'error',
                               'message': (
                                   f"{name} {info.get('version')} version too low "
                                   f"(requires >= {min_ver})"
                               ),
                               'fix_hint': 'cd $PYPTO_REPO && bash tools/prepare_env.sh --quiet --type=deps'})
        ...
        for pkg in python_deps.get('packages', []):
            if pkg['status'] == 'missing':
                issues.append({'component': f"python_dep:{pkg['name']}", 'severity': 'warning',
                               'message': (
                                   f"Python package {pkg['name']} not installed"
                                   f"{(' (requires ' + pkg['required'] + ')') if pkg.get('required') else ''}"
                               ),
                               'fix_hint': 'pip3 install -r $PYPTO_REPO/python/requirements.txt'})
            elif pkg['status'] == 'outdated':
                issues.append({'component': f"python_dep:{pkg['name']}", 'severity': 'warning',
                               'message': (
                                   f"Python package {pkg['name']} "
                                   f"{pkg.get('installed_version', '')} version too low "
                                   f"(requires {pkg['required']})"
                               ),
                               'fix_hint': 'pip3 install -r $PYPTO_REPO/python/requirements.txt'})
    ...
    if not pto_path:
        issues.append({'component': 'pto-isa', 'severity': 'warning',
                       ...
                       'fix_hint': (
                           '优先：export PTO_TILE_LIB_CODE_PATH="$ASCEND_HOME_PATH/aarch64-linux"；'
                           '备用：export PTO_TILE_LIB_CODE_PATH=$PWD/pto-isa && '
                           'git clone https://gitcode.com/cann/pto-isa.git $PTO_TILE_LIB_CODE_PATH'
                       )})
        ...
        issues.append({'component': 'pto-isa', 'severity': 'warning',
                       'message': (
                           f'PTO_TILE_LIB_CODE_PATH={pto_path} 下缺少 '
                           'include/pto/comm/pto_comm_inst.hpp（源码 pto-isa 可能不包含 comm）'
                       ),
                       'fix_hint': (
                           '设置 PTO_TILE_LIB_CODE_PATH="$ASCEND_HOME_PATH/aarch64-linux"'
                           '（确保已安装 pto-isa .run 包），或见 '
                           'troubleshooting.md § "prepare_env.sh 使用 --quiet 仍卡住"'
                       )})
    ...
    if not fast:
        for name, min_ver in [
            ...
            detail = (
                f"{ver} ({info.get('path', '')})"
                if found
                else f"未安装 {('（需 ' + min_ver + '）') if min_ver else ''}"
            )
...
def main() -> int:
    ...
    if pto_isa_path:
        ...
        pto_isa['include_comm_exists'] = os.path.isfile(
            os.path.join(pto_isa_path, 'include', 'pto', 'comm', 'pto_comm_inst.hpp')
        )
