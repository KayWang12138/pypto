...
description: >-
  基于 Anthropic 官方最佳实践，对 PyPTO 项目 skill 进行深度定性审查并生成结构化改进报告。
...
基于 Anthropic 官方最佳实践，对单个 PyPTO skill 进行深度定性审查。
输出带有严重度排序的结构化报告和可操作的修复建议。

> **定位**: 单 skill 深度审查。生态级定量审计（冗余检测、断链、命名冲突）请使用 `skill-auditor`。
...
- Skill 路径：`$UNIFIED_SKILLS_ROOT/library/shared/pypto-performance-analyzer/`
- "所有 pypto skill" → 批量模式，逐个审查

解析 skill 根目录：
```bash
SKILL_ROOT="${UNIFIED_SKILLS_ROOT:-/workspace/code/skills}/library/shared/<skill-name>"
```
...
```
→ 读取: $SKILL_DIR/references/best-practices-checklist.md
```
...
1. 列出：`ls $UNIFIED_SKILLS_ROOT/library/shared/pypto-*`
...
- **硬编码绝对路径** → 使用 `$SKILL_DIR`、`$UNIFIED_SKILLS_ROOT` 等环境变量
...
| `references/official-best-practices-summary.md` | Anthropic 官方最佳实践精华摘要（4 份文档提炼） |

## 配套 Skill

- **skill-auditor**: 生态级定量审计（冗余检测、断链引用、命名冲突）
- **skill-creator**: 按最佳实践创建新 skill
- **unified-skill-manager**: 管理 skill 部署和存放位置
