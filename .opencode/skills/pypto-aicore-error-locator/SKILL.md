...
3. 对比两者的Uid，找出在 LActStart 中存在而在 LActFinish 中缺失所有的 Uid，找到Uid，从而提取出`<value2>`（括号中的最后一个值）
**重要**: 理论上，所有缺失的Uid对应的是同一个`<value2>`。如果所有缺失的Uid对应的不是同一个`<value2>`，输出多个`<value2>`对应的文件
...
**查找 CCE 文件**:
在 `kernel_aicore` 目录中查找包含 `_<value2>_` 后缀为cpp的文件。
