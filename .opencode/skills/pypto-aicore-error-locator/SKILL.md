...
3. 对比两者的Uid，找出在 LActStart 中存在而在 LActFinish 中缺失所有的 Uid，从Uid对应的LEvent提取出`<value2>`（括号中的最后一个值）
**重要**: 理论上，所有缺失的Uid对应的是同一个`<value2>`。如果所有缺失的Uid对应的不是同一个`<value2>`，可以输出多个`<value2>`对应的文件（如果存在）

**解析 LEvent 格式**:
LEvent 格式为 `#LEvent{LUid{0,0,0,<value1>,<value2>},LActStart{<uid>}}`

**查找 CCE 文件**:
在 `kernel_aicore` 目录中查找包含 `_<value2>_` 后缀为cpp的文件，可能存在多个`_<value2>_`，一定不要遗漏。
