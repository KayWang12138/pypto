...
def compare_with_golden(jit_data, golden_data, name, rtol=1e-3, atol=1e-3, verbose=True):
    ...
    if verbose:
        print(f"  对比范围: 前 {min_size} 个元素 (jit={jit_data.shape[0]}, golden={golden_data.shape[0]})")
    ...
    if verbose and not match:
        ...
        for i in range(min(10, min_size)):
            ...
            print(f"    [{i}] jit={jit_val:.6f}, golden={golden_val:.6f}, diff={diff_val:.6f}")
...
def analyze_results(results):
    ...
    if first_fail_idx == -1:
        print("✓ 所有检查点都匹配")
        print("→ 问题可能在：检查点之后的操作")
        return
    
    fail_name = results[first_fail_idx][0]
    
    if first_fail_idx == 0:
        print(f"✗ 第一个检查点 ({fail_name}) 就不匹配")
        print(f"→ 问题可能在：输入数据或第一个计算步骤")
        print(f"→ 建议：检查输入数据是否正确，或在更早的位置插入检查点")
    else:
        prev_name = results[first_fail_idx - 1][0]
        print(f"✗ 检查点 {prev_name} 匹配，但 {fail_name} 不匹配")
        print(f"→ 问题位置：{prev_name} 和 {fail_name} 之间的操作")
        print(f"→ 建议：在这两个检查点之间插入新的检查点，进一步定位问题")
...
def main():
    ...
    if not checkpoints:
        print(f"✗ 未在 {tensor_dir} 找到检查点文件")
        sys.exit(1)
    
    print(f"✓ 找到 output 目录: {latest_dir_full}")
    print(f"✓ 找到 {len(checkpoints)} 个检查点: {checkpoints}")
    ...
    for checkpoint_name in checkpoints:
        ...
        print(f"\n使用文件:")
        print(f"  jit: {os.path.basename(jit_file)}")
        print(f"  golden: {os.path.basename(golden_file)}")
