...
def find_latest_jit_files():
    ...
    if not os.path.exists(tensor_dir):
        print(f"tensor 目录不存在: {tensor_dir}")
        return {}
...
def main():
    print("=" * 80)
    print("PyPTO 算子二分查找调试 - 中间结果对比")
    print("=" * 80)

    # 读取 golden 数据
    print("\n" + "=" * 80)
    print("步骤 1：读取 Golden 数据")
    print("=" * 80)
    ...
    for key, filename in golden_files.items():
        if os.path.exists(filename):
            ...
            print(f"✓ 读取 {filename}: shape={golden_data[key].shape}")
        else:
            print(f"✗ 文件不存在: {filename}")

    # 读取 jit 数据
    print("\n" + "=" * 80)
    print("步骤 2：读取 JIT 数据")
    print("=" * 80)
    ...
    for key, filename in golden_files.items():
        if key in golden_data:
            print(f"  {key}: {filename} - shape={golden_data[key].shape}")
    ...
    if len(jit_file_list) >= 4 and len(golden_data) >= 4:
        ...
        print("数据文件数量不足，无法自动对比")
        print(f"  JIT 文件数量: {len(jit_file_list)}")
        print(f"  Golden 文件数量: {len(golden_data)}")

    print("\n" + "=" * 80)
    print("调试建议")
    print("=" * 80)
    print("1. 查看每个检查点的对比结果")
    print("2. 找到第一个结果不匹配的检查点")
    print("3. 问题位于该检查点之前或此处")
    print("4. 在该区间继续二分查找，定位具体问题 op")
    print("=" * 80)
