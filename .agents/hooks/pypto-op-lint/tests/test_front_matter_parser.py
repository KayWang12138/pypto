...
# SPEC
"""
    meta, body = mod._parse_front_matter(text)
...
def test_validate_doc_schema_requires_fields_by_doc_type():
    mod = load_lint_module()
    spec_errors = mod._validate_doc_schema("SPEC", {"op_name": "x"})
    design_errors = mod._validate_doc_schema("DESIGN", {"op_name": "x"})
    api_errors = mod._validate_doc_schema("API_REPORT", {})
