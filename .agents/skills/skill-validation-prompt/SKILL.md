...
1. **SKILL.md** — 工作流、约束、输出格式
2. **references/** — 参考文档、模板、规范
3. **scripts/** — 自动化工具逻辑
4. **templates/** — 输出模板
5. **AGENTS.md** — 触发词、职责边界
...
3. **远程比对**（Step 3）：≥5 个远程案例 + 交叉验证取众数 + 双向排查
...
保存至：`/workspace/agent_optimization/skills/{skill-name}-validation-prompt-YYYYMMDDHHMMSS.md`
...
| Skill 类型 | 校验侧重 | 本地执行方式 | 远程比对方式 |
|-----------|---------|-------------|-------------|
| 流程编排型 | 流程完整性、规范符合度、远程交互正确性 | 构造输入走完本地流程阶段（不触发远程写操作） | ≥5 个远程真实 PR/Issue |
| 代码生成型 | 代码质量、编译通过、测试覆盖 | 构造规格输入，生成代码文件 | 项目中同类文件 |
| 文档创建型 | 格式规范、内容完整性、引用准确性 | 构造主题输入，生成文档 | 项目文档风格 |
| 修复工具型 | 修复完整性、副作用范围 | 构造已知问题，执行修复流程 | 已修复的同类 issue |
| 分析诊断型 | 分析准确性、建议可落地性 | 构造分析目标，执行分析并收集建议 | 历史分析结果 |
| 无远程交互型 | 制品质量、格式一致性 | 构造输入，执行全流程 | 项目中已有同类文件 |
