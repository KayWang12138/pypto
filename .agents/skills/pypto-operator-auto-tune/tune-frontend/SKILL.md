...
2. **最内层循环次数是否足够大？**
   平衡切块与loop_unroll
   - ✅ 解决方案：
   - 内层循环次数较大时，考虑开启loop_unroll
   - 内层循环次数过小时，分析切块是否合理。如果当前切块已经可以用满算力，则继续开启loop_unroll；如果切块较大，则调整切块大小，同时开启loop_unroll
...
### 2. 优化循环结构的思路
当最内层循环次数<8时，**不要强行使用loop_unroll**，而应该：
1. **对某些层轴切块** - 减少外层循环次数，增加内层循环次数
2. **调整循环顺序** - 将迭代次数多的循环放在最内层
3. **合并循环** - 减少嵌套层级，增加单层迭代次数
...
##### 1.2.2 如果内层的动态轴范围很大，使用loop_unroll展开，增加并行度
   ```python
   ...
   **参数说明：**
   - `unroll_list=[8, 4, 2, 1]`: 数值代表并行块大小，优先尝试并行8个块
   - **⚠️ 重要**： loop_unroll 必须放在最内层循环
   - **如果次数<8**， 考虑对外层轴切块

   **参数说明：**
   - `loop_count`: 循环迭代次数
   - `unroll_list`: 展开因子列表，按优先级从高到低排列。数值代表并行块的大小
   - `[8, 4, 2, 1]`: 常用配置，适应性强
   - `[16, 8, 4, 2, 1]`: 适用于更大循环
   - `[4, 2, 1]`: 适用于较小循环
   - `name`: 循环名称（用于调试）
    ...
    **⚠️ 重要原则：**
    ...
    - **如果最内层循环次数<8，考虑对更外层轴切块**
    
    **💡 关键启发：unroll_list数值的含义**
    - `unroll_list=[8, 4, 2, 1]` 表示优先尝试并行处理8个块
...
**优化优先级**：
1. ⭐⭐⭐ **任务粒度优化**（切块、合并) - **最重要**
2. ⭐⭐ **loop_unroll 配置**（最内层)
3. ⭐ **TileShape 优化**（Cube/Vector 推荐配置)
