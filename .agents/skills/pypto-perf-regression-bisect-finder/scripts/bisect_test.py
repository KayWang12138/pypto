...
def add_debug_options(test_case_path):
    ...
    result = run_command(
        [PYTHON_EXEC, add_debug_script, "add", test_case_path],
        check=False
    )
...
def restore_test_case(test_case_path):
    ...
    result = run_command(
        [PYTHON_EXEC, add_debug_script, "restore", test_case_path],
        check=False
    )
...
def compile_pypto(work_dir):
    ...
    # 编译
    result = run_command(
        [PYTHON_EXEC, "-m", "pip", "install", ".", "--force-reinstall", "--no-deps", "--quiet"],
        check=False,
        cwd=work_dir
    )
