...
检查测试用例是否已配置泳道图开关：
- 检查 `@pypto.frontend.jit()` 装饰器内是否包含 `debug_options={"runtime_debug_mode": 1}`
- 如果没有此配置，需要在装饰器的 debug_options 中添加 `runtime_debug_mode: 1`
- 如果装饰器不存在，需要添加 `@pypto.frontend.jit(debug_options={"runtime_debug_mode": 1})` 装饰器
- 运行测试用例生成泳道图
...
使用 git bisect 自动查找导致性能退化的 commit。测试用例的 `@pypto.frontend.jit()` 装饰器内需要包含 `debug_options={"runtime_debug_mode": 1}` 配置，以便每次运行时生成泳道图文件。
...
`scripts/bisect_test.sh` 脚本需要：
1. 切换到当前测试版本
2. 重新编译安装 PyPTO
3. 运行测试用例生成泳道图
4. 运行性能分析脚本
5. 根据性能指标返回退出码（0=GOOD, 1=BAD, 2=UNKNOWN）
