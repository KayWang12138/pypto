...
**⚠️ 重要：根据用户描述的问题现象，参考"问题匹配指南"快速定位相似案例。**
...
| 用户问题关键词 | 可能的问题类型 | 相关 Issue | 推荐排查方法 |
|--------------|--------------|-----------|-------------|
| reshape 后精度异常 | valid_shape 推导错误 | #498, #292, #813 | 检查 reshape 前后 valid_shape |
| view + reshape 组合 | inplace 元数据问题 | #343 | 避免 inplace=True |
| unroll 后精度失败 | RegisterCopy pass 错误 | #223, #341, #653 | 设置 unroll_list=[1] 测试 |
| 非整除场景精度失败 | valid_shape 推导 | #498, #341 | 检查整除/非整除差异 |
| loop 外 tensor 问题 | tensor 作用域问题 | #340, #626 | 在 loop 内创建 tensor |
| 动态轴结果不一致 | JIT 缓存问题 | #273, #529, #533 | 检查动态轴缓存 |
| 嵌套循环精度异常 | 并行执行内存覆盖 | #4 (案例4) | 添加 submit_before_loop=True |
| 特定 shape 精度失败 | buffer 限制/对齐问题 | #787, #724, #355 | 检查 buffer 大小限制 |
| mix 场景精度失败 | UB2L1 脏数据 | #799 | 检查 viewShape/validShape |
| 多 kernel 配置干扰 | 配置作用域泄漏 | #712 | 检查配置栈清理 |
| 精度工具通过但上板失败 | 仿真不覆盖 | #539 | 二分定位问题 op |
| assemble 相关问题 | Memtype 约束缺失 | #260 | 检查 Assemble Memtype |
| bf16 精度失败 | 舍入/对齐问题 | #355, #354 | 检查尾轴对齐 |
| 多核切 K 精度不确定 | GM 原子累加 | #795 | 检查多核累加逻辑 |
| inplace tensor 问题 | 内存处理错误 | #481, #468 | 检查 inplace 内存管理 |
| 精度工具粗检失败 | GC 内存回收 | #680 | 保存 tensor 引用 |
