...
| 7 | workspace 不足 | 扩大 workspace | 修改源码扩大 workspace 大小 | 避免 workspace 越界导致的内存问题，参考 `docs/trouble_shooting/machine.md` |
| 8 | 内存重叠 | 调整分配顺序 | 避免原地修改 | 防止并行执行时的数据竞争和内存覆盖 |
...
根据 [docs/trouble_shooting/machine.md](../../../docs/trouble_shooting/machine.md)，可通过以下方式验证是否为 workspace 问题：

1. **扩大 workspace 大小**（验证性修改）：

   修改 `python/pypto/frontend/parser/entry.py`，将 workspace 扩大 10 倍：
   ```python
   # 修改前
   ...
   ```
   如果问题不复现，则是 workspace 计算问题。

2. **切换 workspace 管理方式**（验证性修改）：

   修改 `framework/src/machine/runtime/device_launcher.cpp`：
   ```cpp
   // 修改前：使用 torch 管理的 workspace
   if (config.workspaceAddr) {
       kArgs.workspace = (int64_t *)config.workspaceAddr;
   }

   // 修改后：使用内部自管理的 workspace
   if (0) {  // 禁用 torch 管理
       kArgs.workspace = (int64_t *)config.workspaceAddr;
   }
   ```
   如果问题不复现，则是 workspace 使用问题（可能存在内存踩踏）。

**判断**：修改后问题是否解决，确认是否为 workspace 相关问题
...
如果上述方法无法定位问题，使用 `pypto-binary-search-verify` skill查找定位具体问题 op。
...
| 文档 | 路径 | 说明 |
|------|------|------|
| Machine 错误排查 | [docs/trouble_shooting/machine.md](../../../docs/trouble_shooting/machine.md) | 内存相关错误排查指南 |
...
| 脚本 | 路径 | 用途 |
|------|------|------|
| 内存重叠检测 | [tools/schema/schema_memory_check.py](../../../tools/schema/schema_memory_check.py) | 检测 device task 内存重叠 |
| CCE 变量解析 | [tools/scripts/auto_parse_cce_var.py](../../../tools/scripts/auto_parse_cce_var.py) | 解析动态 CCE 中的 validShape |
| Tensor 解析 | [tools/verifier/parse_dump_tensors.py](../../../tools/verifier/parse_dump_tensors.py) | 解析 dump 的 tensor 信息 |
