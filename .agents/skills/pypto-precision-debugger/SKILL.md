...
| 优先级 | 问题现象 | 规避方法 | 代码示例 |
|-------|---------|---------|---------|
| 1 | 使用旧前端写法 | 切换到 `pypto.frontend.jit` | `@pypto.frontend.jit` |
| 2 | view + reshape 精度异常 | 避免 `inplace=True` | `pypto.reshape(tensor, shape, inplace=False)` |
| 3 | 循环展开后精度异常 | `unroll_list=[1]` | `pypto.loop(range(n), unroll_list=[1])` |
| 4 | 嵌套循环精度异常 | `submit_before_loop=True` | `pypto.loop(range(m), submit_before_loop=True)` |
| 5 | 特定 shape 精度异常 | 调整 shape | 避免尾轴为 1，避免非整除 |
| 6 | 编译器优化异常 | `+0.0` 技巧 | `result = compute(...) + 0.0` |
| 7 | workspace 不足 | 扩大 workspace | `pypto.set_workspace_size(larger_size)` |
| 8 | 内存重叠 | 调整分配顺序 | 避免原地修改 |
