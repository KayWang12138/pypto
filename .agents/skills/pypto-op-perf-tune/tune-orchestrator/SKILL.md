...
|-----------|---------|---------|--------|
| PHASE_FRONTEND | 加载 tune-frontend 子技能 + ⛔ 生成调优点清单 | 达标 或 (调优点清单已全部尝试 且 连续5轮无提升) | ⛔ 生成摘要 → ⛔ 通过 Task 工具启动新 subagent 会话压缩上下文 → PHASE_SWIMLANE |
| PHASE_SWIMLANE | 加载 tune-swimlane 子技能 + ⛔ 生成调优点清单 | 达标 或 (调优点清单已全部尝试 且 连续8轮无提升) | ⛔ 生成摘要 → ⛔ 通过 Task 工具启动新 subagent 会话压缩上下文 → PHASE_INCORE |
| PHASE_INCORE | 加载 tune-incore 子技能 + ⛔ 生成调优点清单 | 达标 或 (调优点清单已全部尝试 且 达到理论上限) | ⛔ 生成摘要 → S5_REPORT |
...
□ Q4: 强制动作是否已完成？
       → S1→S2: Todo 已创建？
       → PHASE 间: 交接摘要已生成 **且已通过 Task 工具启动新会话完成上下文压缩**？
...
| 调优点清单已全部尝试 且 连续5轮无提升 | 调优点清单中所有项已标记为 ✅已尝试 或 ❌已失败，且最近5轮无提升 | → 生成摘要 → PHASE_SWIMLANE |
...
| 调优点清单已全部尝试 且 连续8轮无提升 | 调优点清单中所有项已标记，且最近8轮无提升 | → 生成摘要 → PHASE_INCORE |
| 用户要求停止 | 用户明确说停止 | → 生成摘要 → S5_REPORT |
...
| 调优点清单已全部尝试 且 达到理论性能上限 | 清单全部尝试，核心利用率 > 80% 且 气泡率 < 10% | → 生成摘要 → S5_REPORT |
| 用户要求停止 | 用户明确说停止 | → 生成摘要 → S5_REPORT |
