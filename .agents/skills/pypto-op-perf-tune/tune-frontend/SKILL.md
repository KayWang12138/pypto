...
**优化优先级**：
1. ⭐⭐⭐ **P0 - 任务粒度** → 详见 [F-1][F-2][F-3][F-4]
2. ⭐⭐ **P1 - Loop 写法** → 详见 [F-5][F-6][F-7][F-8]
3. ⭐⭐ **P2 - TileShape 设置** → 详见 [F-9][F-10]
4. ⭐ **P3 - 常量配置** → 详见 [F-11]
5. ⭐ **P4 - 数据操作** → 详见 [F-12][F-13][F-14][F-15]

**🔥 P0 - 任务粒度 [F-1~F-4]**：
- [ ] [F-1] Matmul 的 M/N/K 轴是否充分利用硬件？（M 轴 < 8 是常见问题）
- [ ] [F-2] 循环体一次计算量是否太小？
- [ ] [F-3] 任务总数是否过多？（> 1000 可能调度开销大）
- [ ] [F-4] 合轴优化：shape 的维度是否超过 2 维？（超过两维，搬运及计算的开销较大）

**P1 - Loop 写法 [F-5~F-8]**：
- [ ] [F-5] 静态轴是否使用 Python for（而非 pypto.loop）
- [ ] [F-6] 是否可以合并独立 loop
- [ ] [F-7] 外层动态轴是否需要切块（范围较大时）
- [ ] [F-8] 内层动态轴是否需要 unroll（范围较大时）

**P2 - TileShape 设置 [F-9~F-10]**：
- [ ] [F-9] Cube 计算：每个 matmul 是否独立设置推荐配置
- [ ] [F-10] Vector 计算：是否使用推荐配置
- [ ] 归约轴是否避免切分

**P3 - 常量配置 [F-11]**：
- [ ] [F-11] BLOCK_SIZE 等常量是否合理（16/32/64）

**P4 - 数据操作 [F-12~F-15]**：
- [ ] [F-12] 输入矩阵格式是否优化（NZ 格式）
- [ ] [F-13] transpose 配置是否合理
- [ ] [F-14] reshape 操作是否可以消除或使用 inplace
- [ ] [F-15] 是否存在冗余搬运（concat → assemble）
