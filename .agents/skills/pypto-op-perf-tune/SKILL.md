...
4. **制品持久化**：优化清单、阶段交接摘要、调优报告必须保存为文件（如 `tuning_progress.md`），禁止仅存在于 LLM 对话上下文中。每个阶段结束后，将优化记录追加到 `tuning_progress.md`
...
根据用户输入计算：提升N倍 → 目标 = 原始时间 / N；或直接使用用户指定的目标时间。

### 0.3 设置调优终止条件

**自动终止**：①达到性能目标 ②核心利用率>80%且气泡率<10% ③调优时间限制（默认12小时）④连续5轮优化单轮提升<2%
**手动终止**：用户明确要求停止
...
1. **开箱性能调优** — 加载 tune-frontend 子技能，根据性能基准优化代码写法、TileShape、BLOCK_SIZE，不需要查看性能报告详细分析。结束后生成阶段交接摘要。
2. **深度性能调优** — 基于交接摘要启动，加载 tune-swimlane 子技能，分析泳道图，优化 Stitch 调度、合图、L1Reuse。结束后生成阶段交接摘要。
3. **核内性能调优** — 基于交接摘要启动，加载 tune-incore 子技能，分析核内瓶颈，指令级优化、核内流水优化、特殊 Shape 处理。
...
每次循环执行以下步骤：
1. **选择优化点** — 根据决策树或子技能指南选择
2. **修改代码** — 每次只修改一个参数
3. **验证精度 ⭐** — 运行测试用例；失败则尝试解决，不行则回退修改
4. **测试性能** — 采集性能数据，对比基准，计算提升百分比
 5. **记录结果 & 更新优化清单 ⭐** — 提升：保留修改；下降：回退修改；更新优化记录
6. **检查终止条件** — 达到目标或无法提升则停止
7. **状态看板** — 每 5 轮输出当前进度和性能趋势
...
**状态看板**（每 5 轮输出当前性能、累计提升%、成功率、当前瓶颈）：
...
| # | 节名称 | 必填内容 |
|---|--------|---------|
| 1 | 算子当前性能状态 | 算子名称/文件、基准与当前执行时间、累计提升%、核心利用率、气泡率、负载均衡度、目标达成状态 |
| 2 | 已采纳优化（代码中已生效） | 表格：优化项、具体修改、性能收益%、代码位置 |
| 3 | 已失败优化（避免重试） | 表格：尝试项、失败原因（精度失败/OOM/性能回退）、是否已回退、备注 |
| 4 | 约束与发现 | 后续阶段必须遵守的约束（如参数上限、内存限制）和关键发现（如瓶颈转移） |
| 5 | 当前关键代码配置 | jit 装饰器 runtime_options/pass_options/debug_options、BLOCK_SIZE、TileShape 等关键常量 |
| 6 | 下一阶段建议 | 当前主要瓶颈、建议优化方向、需关注的性能数据文件路径 |
| 7 | 性能趋势摘要 | 起始→结束性能及本阶段提升%、总迭代轮次/成功/失败数、最佳优化项、性能曲线趋势 |

#### 4.5.2 上下文压缩规则

摘要生成后，后续工作应基于摘要内容：
- **✅ 完整保留**：已采纳优化、已失败优化、约束与发现、当前代码配置、性能趋势
- **❌ 丢弃**：中间调试日志、已回退代码完整内容、冗余性能对比细节
- **❌ 不重新加载**：已完成阶段的子技能完整内容

#### 4.5.3 阶段交接流程

1. 当前阶段退出条件满足
2. ⛔ 强制生成阶段交接摘要（4.5.1 模板，含 7 个必填节）
3. ✅ 校验摘要完整性（已采纳/已失败优化、约束与发现、代码配置是否齐全）
4. 📋 输出摘要给用户确认
5. 🔄 进入下一阶段：以摘要为上下文基础 → 加载新子技能 SKILL.md → 基于摘要中"下一阶段建议"确定首个优化方向 → 开始新一轮迭代调优

**⚠️ 摘要校验清单（生成后必须逐项确认）**：
- 性能数据是否与实际一致？已采纳/已失败优化是否遗漏？约束与发现是否遗漏？代码配置是否最新？
...
4. 保存调优报告，报告格式参考 [性能调优报告模板](./perf-analyzer/templates/performance_report_template.md)

## 常见错误

常见错误及规避方法详见各子技能 SKILL.md（如 [tune-frontend/SKILL.md](tune-frontend/SKILL.md)）。
