...
- ❌ 禁止跳过编排器的自检流程
- ❌ 禁止卸载编排器后继续调优
- ❌ 禁止阶段交接时不通过编排器的 PHASE_SUMMARY 流程压缩上下文
...
# 1.3 阶段退出后 → 编排器触发 PHASE_SUMMARY（摘要 + 上下文压缩）
...
# 2.2 执行迭代调优
# 2.3 阶段退出后 → 编排器触发 PHASE_SUMMARY（摘要 + 上下文压缩）
...
# 3.2 执行迭代调优
# 3.3 阶段退出后 → 编排器触发 PHASE_SUMMARY → 进入报告生成
```
...
**阶段间衔接**：每个阶段完成后，编排器自动执行 PHASE_SUMMARY（摘要生成 + Task subagent 压缩上下文），再进入下一阶段。
