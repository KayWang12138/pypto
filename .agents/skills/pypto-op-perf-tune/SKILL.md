...
When used by the Optimization Agent (see `skills/lead-orchestrator/references/agents.md`), stages must be
...
|-------|-------------------|------------|-----------|---------------------------------------|
| 1. Frontend | `skills/tune-frontend/SKILL.md` | GATE 4 passed (correctness frozen); baseline perf measured | No further frontend-level gain, or target reached | ✅ yes |
| 2. Swimlane | `skills/tune-swimlane/SKILL.md` | Stage 1 exited; scheduling / graph-fusion potential remains | No further swimlane-level gain, or target reached | ✅ yes |
| 3. Incore | `skills/tune-incore/SKILL.md` | Stage 2 exited; single-task incore pipeline is the remaining bottleneck | Target reached, or no further incore gain | ✅ yes |
| Automation | `skills/pypto-operator-auto-tuner/SKILL.md` | Swimlane extraction / AIV dependency automation needed in any stage | Automation task finished | ✅ yes — unload back to the stage sub-skill |
...
- Every tuning change must pass the regression loop with the Verification
  Agent (see `skills/lead-orchestrator/references/agents.md` §8): tensor compare + layout check + perf delta,
