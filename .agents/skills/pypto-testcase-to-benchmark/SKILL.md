...
description: >
  ...
  pypto/benchmark 测试框架中以便统一验证 correctness + performance 的场景。
...
### Step 1: 读取 ADD_NEW_CASE.md 确认规则

**转换开始前，必须先读取 `pypto/benchmark/ADD_NEW_CASE.md`**，确认当前目标 case 文件的生成规则。该文档可能随框架版本更新而变化。
...
| 确认项                   | 来源                  | 示例                                                                                                |
| --------------------- | ------------------- | ------------------------------------------------------------------------------------------------- |
| case 放置路径             | ADD_NEW_CASE.md 第1节 | `benchmark/testdata/KernelBench/<level>/` 或 `.cache/KernelBench/KernelBench/<level>/` |
| 命名格式                  | ADD_NEW_CASE.md 第1节 | `<N>_<CaseName>.py`                                                                               |
| 序号规则                  | ADD_NEW_CASE.md 第1节 | 从当前 level 未占用编号继续递增                                                                               |
| **FORMULA（数学公式）**     | ADD_NEW_CASE.md 第3节 | 文件顶层添加 `FORMULA = "out[m,n] = x[m,k] @ w[k,n]"`                                                   |
| **DYNAMIC_AXIS（动态轴）** | ADD_NEW_CASE.md 第3节 | 文件顶层添加 `DYNAMIC_AXIS = ["M", "B"]`                                                                |


**注意区分两种路径**：

- **上游数据集路径**：`.cache/KernelBench/KernelBench/<level>/`（download_kernelbench.sh 下载产物）
- **仓内自维护路径**：`benchmark/testdata/KernelBench/<level>/`（ADD_NEW_CASE.md 推荐）

当前转换应使用哪个路径，以 `ADD_NEW_CASE.md` 文档为准。
...
- 如 ADD_NEW_CASE.md 要求，顶层添加 `FORMULA` 和 `DYNAMIC_AXIS`
...
根据 ADD_NEW_CASE.md 第3节要求，新增 case **应尽可能提供**这两个大写全局变量：
...
- `FORMULA` 已添加（如 ADD_NEW_CASE.md 要求）
- `DYNAMIC_AXIS` 已添加（如 ADD_NEW_CASE.md 要求）
...
| 陷阱                      | 现象                                                  | 解决                                                          |
| ----------------------- | --------------------------------------------------- | ----------------------------------------------------------- |
| 未读 ADD_NEW_CASE.md      | case 放错路径、缺少 FORMULA/DYNAMIC_AXIS                   | 转换前必须先读文档确认规则                                               |
...
- **新增 Case 规则**：`pypto/benchmark/ADD_NEW_CASE.md`（转换前必读）
