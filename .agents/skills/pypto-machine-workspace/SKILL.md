...
读取 `<pypto_path>/framework/src/machine/utils/device_switch.h`，将 `ENABLE_COMPILE_VERBOSE_LOG` 设置为 `1`：
...
| `stitch_function_inner_memory` | rootInner | 当未设置 stitch_function_max_num 时的 fallback |
| `stitch_function_outcast_memory` | devTaskInnerOutCasts | 当未设置 stitch_function_max_num 时的 fallback |
...
5. **路径规范**：所有路径必须使用绝对路径
6. **配置还原**：诊断完成后，提醒用户还原步骤 2 中修改的 `ENABLE_COMPILE_VERBOSE_LOG` 和 `dump_graph` 配置
