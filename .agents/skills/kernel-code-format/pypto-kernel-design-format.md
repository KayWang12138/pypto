...
**Canonical Python skeleton (complex-kernel workflow):** `.agents/skills/kernel-code-format/pypto_kernel_template.py` — agents **must** use this file as the starting layout for **each** staged module file and the **full** kernel under `custom/<op>/` (see **`skills/lead-orchestrator/references/rules.md`** rule **17** and **`skills/lead-orchestrator/references/rules.md`** rule **17**). This document (layers A–L) and that template stay aligned.
