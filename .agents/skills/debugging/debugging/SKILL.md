...
```bash
python3 .agents/pypto-kernel-custom-skills/skills/ci-and-pr/ci-and-layout-check/scripts/extract_pypto_calls.py custom/<operator_name>/<kernel_or_impl>.py
```
...
- Prefer module-at-a-time stubs (see `skills/orchestration/lead-orchestrator/references/rules.md` → Module-at-a-time enforcement): shrink the live region, then re-run Step 0 on the smaller file.
...
Verification Agent (see `skills/orchestration/lead-orchestrator/references/agents.md`), load **exactly one** sub-skill per
...
1. **Precision / accuracy mismatch**
   *Signal:* `detailed_tensor_compare` returns `all_close: false`; build and
   ...
   → Load `skills/debugging/pypto-precision-debug/SKILL.md`
...
2. **Precision bisection needed**
   *Signal:* precision still fails after (1), or the diverging checkpoint is
   ...
   → Load `skills/debugging/pypto-precision-compare/SKILL.md`
...
3. **aicore error / CCE file reported**
   *Signal:* log contains `aicore error`, `ERROR_CODE: EE...`, or a path to
   a CCE file.
   → Load `skills/debugging/pypto-aicore-error-locator/SKILL.md`
...
4. **Host-side crash / backtrace**
   *Signal:* segfault, Python/C++ stack trace, process killed before kernel
   launch finishes.
   → Load `skills/debugging/pypto-host-stacktrace-analyzer/SKILL.md`
...
5. **Workspace overlap suspected**
   *Signal:* non-deterministic precision failures that move with tensor
   ...
   → Load `skills/debugging/pypto-memory-overlap-detector/SKILL.md`.

6. **OOM / workspace size anomaly**
   *Signal:* `rtMalloc failed`, OOM error, or workspace size far exceeds
   expectation.
   → Load `skills/debugging/pypto-machine-workspace/SKILL.md`.
...
- `skills/orchestration/lead-orchestrator/references/rules.md` takes precedence over any sub-skill guidance on conflict.
