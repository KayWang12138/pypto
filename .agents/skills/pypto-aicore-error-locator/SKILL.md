...
1. **PyPTO 算子测试中出现 aicore error**
   - 测试运行时出现 "aicore error" 相关错误信息
   - 错误发生在 NPU 设备端执行阶段
   - 需要定位具体的 CCE 文件和代码行
...
2. **环境要求**
   ...
   - 有可用的 NPU 设备或模拟器环境
   - 有权限修改 PyPTO 源码和重新编译
   - 测试命令可稳定复现 aicore error

3. **可获取的工件**
   - device log 可正常落盘
   - program.json 文件可正常生成
   - CCE 文件可正常编译和测试

### 不适用的情况

以下情况不适用此技能，请使用其他调试方法：

1. **编译错误**
   - 如果错误发生在编译阶段（如语法错误、类型错误）
   - **降级方案**：检查编译日志，使用 pypto-pass-error-fixer 技能

2. **精度错误**
   - 如果错误是精度不匹配或数值偏差
   - **降级方案**：使用 pypto-precision-debugger 或 pypto-precision-verify 技能

3. **Python 运行时错误**
   - 如果错误是 Python 异常（如 ImportError, ValueError, RuntimeError）
   - **降级方案**：检查 Python 堆栈，使用常规 Python 调试方法

4. **环境配置问题**
   - 如果错误与 NPU 驱动、CANN 版本或环境变量相关
   - **降级方案**：使用 pypto-environment-setup 技能

5. **无法稳定复现**
   - 如果 aicore error 无法稳定复现
   - **降级方案**：检查测试用例的随机性，固定随机种子，简化测试场景

6. **没有 device log 或 program.json**
    - 如果无法获取 device log 或 program.json 文件
    - **降级方案**：检查日志配置，使用其他调试工具如 npu-smi

7. **链接错误（ld.lld: error: undefined）**
    - 如果错误是链接阶段的 undefined symbol 错误
    - **降级方案**：
      - 检查 `tile_fwk_config.json` 中的 `parallel_compile` 设置
      - 尝试将 `parallel_compile` 设置为 `1` 并重新编译
      - 检查分布式算子的特殊配置需求
      - 查看链接错误信息中的符号引用位置，手动检查相关代码

8. **段错误（segment fault）**
    - 如果错误是运行时段错误
    - **降级方案**：
      - 检查是否为空指针访问、数组越界等常见原因
...
1. **环境检查**
   - [ ] 检查 NPU 设备是否可用：`npu-smi info`
   - [ ] 检查 PyPTO 是否已安装：`python -c "import pypto"`
   - [ ] 检查测试命令是否可执行

2. **错误类型检查**
   - [ ] 确认错误是 aicore error（非编译错误、非精度错误、非 Python 异常）
   - [ ] 确认错误发生在设备端执行阶段
   - [ ] 检查是否为链接错误（ld.lld: error: undefined）
   - [ ] 检查是否为段错误（segment fault）

3. **工件检查**
   - [ ] 确认 device log 路径可写
   - [ ] 确认 program.json 可正常生成
   - [ ] 确认 CCE 文件路径可访问
...
1. **环境检查失败**
   - 输出：当前环境状态和缺失的组件
   - 建议：使用 pypto-environment-setup 技能修复环境问题
...
3. **工件检查失败**
   - 输出：缺失的工件和可能的原因
   - 建议：检查配置文件，确认路径权限，使用其他调试方法

4. **权限检查失败**
   - 输出：缺少的权限和受影响的操作
   - 建议：联系管理员获取权限，或在有权限的环境中执行
...
2. **出现链接错误（ld.lld: error: undefined）**
   - 输出：检测到链接错误，不是 aicore error
...
3. **出现段错误（segment fault）**
   - 输出：检测到段错误，不是 aicore error
- 输出：段错误发生的位置和上下文（如果有）
- 建议：
    - 使用调试工具（如 gdb、core dump）分析段错误
    - 检查空指针访问、数组越界等常见原因
...
4. **出现其他错误**
   - 输出：检测到其他类型的错误，不是 aicore error
- 输出：错误类型和错误信息摘要
- 建议：
    - 根据错误类型使用对应的调试技能
    - 检查编译日志或运行时日志
- 降级方案：
    - 使用 pypto-pass-error-fixer 技能（编译错误）
    - 使用 pypto-precision-debugger 技能（精度错误）
    - 使用常规 Python 调试方法（Python 异常）
- 停止执行后续步骤
