...
description: 定位测试案例中出现 aicore error error 时的问题 CCE 文件和问题代码行。当用户说"aicore error"、"定位aicore error的原因"、"帮我定位aicore error报错"时使用此技能。
license: 完整条款见 LICENSE.txt
---

# AICore Error 定位器

此技能用于定位 PyPTO 测试中出现的 aicore error error，通过系统化的排查流程，找出导致错误的 CCE 文件和具体代码行。
...
**⚠️ 重要提示**: 观察运行输出日志中是否有 aicore error，若无 error 则说明可能是 machine 调度框架的问题
...
- **头文件**: 修改 `aicore_entry.h`
  - 取消注释 `#if ENABLE_AICORE_PRINT`（它的下一行是`CoreFuncParam`）行及相邻的后6行
...
**⚠️ 判断逻辑**结果：
- **有 error**: 说明是 machine 调度框架的问题，已找到问题原因，**停止执行后续步骤！**
- **无 error**: 不适用于该 SKILL，**停止执行后续步骤！**
