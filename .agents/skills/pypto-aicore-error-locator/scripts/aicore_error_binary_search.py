...
def get_commentable_lines(lines):
    ...
    for i, line in enumerate(lines, 1):
        stripped = line.strip()
        # 跳过空行、注释行和包含大括号的行
        # 跳过 set_flag 和 wait_flag 行
        if not stripped or stripped.startswith('//') or '{' in stripped or '}' in stripped or '#' in stripped or 'set_flag' in stripped or 'wait_flag' in stripped:
            continue
        else:
...
def binary_search(cce_file, test_cmd, run_dir):
    ...
    # 注释所有 set_flag 和 wait_flag 行
    for i, line in enumerate(cce_lines):
        if 'set_flag' in line or 'wait_flag' in line:
