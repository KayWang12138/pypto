#!/usr/bin/env python3
"""
Script to comment/uncomment CallSubFuncTask in aicore_entry.h
"""
...
def comment_lines(lines, start_idx, end_idx):
    for i in range(start_idx, end_idx + 1):
        if not lines[i].strip().startswith('//'):
            lines[i] = '// ' + lines[i]


def uncomment_lines(lines, start_idx, end_idx):
    for i in range(start_idx, end_idx + 1):
        if lines[i].strip().startswith('// '):
            lines[i] = lines[i][3:]  # 移除 '// '
        elif lines[i].strip().startswith('//'):
            lines[i] = lines[i][2:]  # 移除 '//'
...
def comment_callsubfunctask(file_path):
    ...
    comment_lines(lines, start_idx, end_idx)
...
def uncomment_callsubfunctask(file_path):
    ...
    uncomment_lines(lines, start_idx, end_idx)
    write_file(file_path, lines)
