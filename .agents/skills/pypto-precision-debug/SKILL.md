...
| 2 | view + reshape 精度异常 | view 后避免 `inplace=True`（单独 reshape 可安全使用 `inplace=True`） | `pypto.reshape(tensor_view, shape, inplace=False)` | view 后 inplace=True 会错误修改内存地址；动态 shape 场景单独使用 reshape 推荐用 inplace=True |
