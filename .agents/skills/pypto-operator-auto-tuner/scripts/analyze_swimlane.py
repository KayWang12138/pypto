...
def main():
    ...
    with open(f"{BASE}/merged_swimlane.json", "r") as f:
        data = json.load(f)
    ...
    leaf_info = {}
    with open(f"{BASE}/dyn_topo.txt", "r") as f:
        reader = csv.reader(f)
        next(reader)
        for row in reader:
            if len(row) < 9:
    ...
    with open(f"{BASE}/program.json", "r") as f:
        prog = json.load(f)
    ...
    print(f"{'#':<3} {'leafHash':<22} {'cnt':>4} {'t/iter':>6} "
    ...
    for i, (lh, st) in enumerate(sorted_items, 1):
        ...
        print(f"{i:<3} {lh:<22} {st['count']:>4} {t_per_iter:>6.1f} "
    ...
    print(f"\n{'=' * 80}")
    print("Merge Tuning Guide (psgId = hashorder)")
    print(f"{'=' * 80}")
