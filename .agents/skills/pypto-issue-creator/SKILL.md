...
若 GitCode MCP 不可用（工具调用失败、token 过期、网络异常等），按以下降级流程处理：

1. **提示用户手动搜索**：引导用户在 GitCode 搜索框中输入以下关键词进行去重检查：
   - `{关键词}`
   - `[Bug-Report|缺陷反馈] {关键词}`
   - `{中文关键词}` 和 `{english keywords}`
2. **记录已执行的搜索关键词和结果**：将用户反馈的搜索结果记录到上下文中
3. **继续后续流程**：去重检查完成后，不阻塞 Issue 生成
