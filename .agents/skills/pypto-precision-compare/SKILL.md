...
### ⚠️ 关键说明：pass_verify_save 保存的是 CPU 模拟结果

`pypto.pass_verify_save()` 保存的中间结果来自 **CPU 模拟（sim）模式**的计算结果，**不是 NPU 实际执行的结果**。这意味着：
- 保存的数据反映的是 PyPTO 编译器对算子的模拟执行结果
- 如果 NPU 实际执行结果与 CPU 模拟结果不同，`pass_verify_save` 无法捕获此类差异
- 当需要对比 NPU 实际执行结果时，应使用二分对比方法（在内存中直接对比）
