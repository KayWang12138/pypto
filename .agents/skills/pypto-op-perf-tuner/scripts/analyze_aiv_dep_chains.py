...
用法:
    python3 tools/analyze_aiv_dep_chains.py <output_dir>
    python3 tools/analyze_aiv_dep_chains.py <output_dir> --json result.json
...
def build_aiv_graph(rows):
    ...
    for k in has_cube_succ:
        lh_has_cube.add(keyed[k]["leafHash"])
...
def load_leaf_info(path):
    ...
    for func in prog.get("functions", []):
        h = func.get("hash")
        if h is not None:
            ops = [op.get("opcode") for op in func.get("operations", [])]
            key_ops = [o for o in ops if o not in (
...
def extract_core_ops(ops):
    core = [o for o in ops if o not in (
        "PHASE1", "PHASE2", "SYNC_SRC", "SYNC_DST",
        "COPY_IN", "COPY_OUT", "BAR.V", "BAR.M", "EXPAND",
    ) and not o.startswith("L1_TO_L0")]
    return core
    ...
    for lvl in lvls:
        for n in lvl:
            if n["leafHash"] not in seen_h:
                ...
                print(f"  {n['leafHash']}: op={n['opmagic']}, psg={n['psgId']}, {lb}")
...
def main():
    ...
    if not topo.exists():
        print(f"错误: {topo} 不存在", file=sys.stderr)
        sys.exit(1)
    ...
    # === Part 1: 原始链路 ===
    print("=" * 80)
    print("AIV 依赖链分析（sg_set_scope 合图优化）")
    print("=" * 80)
    print(f"\n数据: {topo}")
    print(f"总任务: {len(rows)}, AIV: {aiv_count}, 起点: {len(starts)}")
    if lh_has_cube:
        print(f"后继含 cube 的 leafHash: {sorted(lh_has_cube)}")
    ...
    for sig, items in groups.items():
        ...
        if len(lvls) == 1 and lvls[0][0]["n_children"] == 0:
            n = lvls[0][0]
            print(f"\n链路{label}（{count}次）")
            print(f"{n['leafHash']}  (孤立，无依赖)")
            if n["leafHash"] in leaf_info:
                li = leaf_info[n["leafHash"]]
                print(f"  opmagic={n['opmagic']}, psgId={n['psgId']}, {infer_label(li['ops'])}")
        else:
            print(f"\n链路{label}（{count}次）")
            print(render_chain(lvls))
    ...
    print(f"\n{'=' * 80}")
    print("sg_set_scope 优化建议")
    print(f"{'=' * 80}")
    print("\n规则: 遇到后继含 cube 的 AIV 节点时保留该节点，但不继续展开")
    print("      截断后 >=2 节点且 psgId 有变化的链段建议 sg_set_scope 合并\n")

    if not lh_has_cube:
        print("  所有 AIV 节点均无 cube 后继，无需截断")
        print("  建议对完整链路中 psgId 有变化的段进行 sg_set_scope 合并")
    else:
        cut_chains = []
        for k in starts:
        ...
        sugg_idx = 0
        for sig, items in cut_groups.items():
            lvls = items[0][1]
            lh_row_count = count_start_rows([k for k, _ in items], keyed, rows)
            start_lh = keyed[items[0][0]]["leafHash"]
            count = lh_row_count.get(start_lh, len(items))
            ...
            sugg_idx += 1
            print(f"  建议 {sugg_idx}: 截断后 {len(all_nodes)} 个节点, {count} 次, psgId 变化: {' → '.join(str(n['psgId']) for n in all_nodes)}")

            if len(lvls) == 1 and lvls[0][0]["n_children"] == 0:
                print(f"    {lvls[0][0]['leafHash']}")
            else:
            ...
            for n in all_nodes:
                ...
                print(f"    {n['leafHash']}: psg={n['psgId']}, {lb}{cube_mark}")
            ...
            if can_merge:
                ...
                print(f"    → 建议: 用 sg_set_scope 包裹 psgId {' → '.join(str(p) for p in psg_path)} 的 vector 操作")
            else:
                print(f"    → 单节点或 psgId 无变化, 无需 sg_set_scope")

            if cut_points:
                print(f"    ✂ 截断点 (后继含 cube): {cut_points}")
