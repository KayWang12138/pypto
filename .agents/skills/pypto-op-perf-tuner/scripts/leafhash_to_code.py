...
从 merged_swimlane.json 和 program.json 中提取每个 leafHash 对应的前端代码文件和行号，
...
输入:
    output_dir/merged_swimlane.json — 泳道图数据（含 leafHash 和 callOpMagic）
...
def main():
    ...
    if not prog_path.exists():
        print(f"错误: {prog_path} 不存在", file=sys.stderr)
        sys.exit(1)
    ...
    print("=" * 80)
    print("leafHash → 前端代码行映射")
    print("=" * 80)
    print(f"\n数据: {d}")
    print(f"program.json 函数数: {len(prog_info)}")
    if topo_info:
        print(f"dyn_topo.txt leafHash 数: {len(topo_info)}")
    ...
    for h, info in sorted(prog_info.items()):
        ...
        print(f"\n--- {h} ---")
        print(f"  类型: {ct_name}{' (cube)' if is_cube else ''}")
        print(f"  函数: {info['name']}")
        if task_cnt:
            print(f"  执行次数: {task_cnt}, rootIndex: {root_idx}, psgId: {psg_ids}")
        print(f"  计算指令: {' + '.join(compute_ops) if compute_ops else '(无)'}")
        print(f"  代码位置: {loc_str}")

        if info["subfunc_symbols"]:
            print(f"  子函数 tensor: {info['subfunc_symbols']}")
