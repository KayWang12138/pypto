...
| 规则 | 说明 | 证据 |
|------|------|------|
| Vector TileShape | 每维 > 0，维度数按文档要求 | `docs/api/config/pypto-set_vec_tile_shapes.md` |
...
| TileShape 维度数 ≤ 输入维度数 | `set_vec_tile_shapes` 的维度数按文档要求 | 不匹配会触发编译错误 |
| 尾轴尽量取满 | 在 L0 容量允许范围内，尾轴取到对齐后最大值 | 尾轴越大，向量化效率越高，减少循环次数 |
| L1/L0 容量约束 | 所有同时驻留的 Tile 总大小不超过对应 buffer 容量 | 超容量触发溢出，严重劣化性能 |
