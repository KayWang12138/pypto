...
**§3.5 动态轴定义和 §5.2 静态/动态轴处理** 必须输出。处理规则：
- spec §8 声明了动态轴 → 按声明填写 §3.5 和 §5.2
- spec §8 明确写"无动态轴" → §3.5 写"无动态轴"，§5.2 全部标注为静态
- spec 未包含 §8 或未提及动态轴 → 默认第一轴（batch/token_num）DYNAMIC，§3.5 填写对应轴定义，§5.2 标注为动态
