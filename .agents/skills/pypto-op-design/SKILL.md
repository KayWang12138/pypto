...
│  □ 风险点是否具体                                                 │
│  □ 是否存在空话或占位符                                           │
│  □ 代码片段与章节描述一致性（§2.3 伪代码的 cast 操作是否在 §2.2  │
│    映射表和 §8 风险点中均有对应说明？）                           │
│  □ API dtype 交叉验证（§2.2 每个映射 API 是否在 docs/ 中验证了   │
│    对应 dtype 的支持情况？）                                      │
│                                                                   │
│  输出：通过 / 不通过 + 修复建议                                   │
└───────────────────────────────────────────────────────────────────┘
...
生成 DESIGN.md 草稿后，必须按以下 7 项检查表逐项检查：
...
知识库文件是预整理的经验总结，使用时需要到 docs/ 中验证其准确性。

### 冲突仲裁原则

当不同来源的信息产生矛盾时，按以下优先级仲裁：

| 优先级 | 来源 | 说明 |
|--------|------|------|
| 1（最高） | `models/`（生产代码） | 实际可运行的代码，反映真实行为 |
| 2 | `examples/`（教学示例） | 经过验证的示例，可靠性较高 |
| 3 | `docs/`（官方文档） | 文档可能滞后于代码更新 |

**仲裁流程**：
1. 发现矛盾时，先记录矛盾点和各来源的说法
2. 以高优先级来源为准
3. 在 DESIGN.md §8 风险点中标注矛盾和采用的仲裁结果
4. 若矛盾涉及核心 API 行为，建议向用户确认
