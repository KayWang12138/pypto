    ...
    for i, checkpoint in enumerate(checkpoints):
        ...
        for j in range(start_line, min(start_line + 20, len(lines))):
            if 'Tolerance: rtol=' in lines[j]:
                tolerance_match = re.search(r'Tolerance: rtol=([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?), atol=([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)', lines[j])
                if tolerance_match:
...
def main():
    ...
    for i, (ckpt, tol_rtol, tol_atol, act_rtol, act_atol) in enumerate(results):
        logger.info(f'{i+1}. {ckpt}: Tolerance(rt={tol_rtol:.6f}, at={tol_atol:.6f}) | Actual(rt={act_rtol:.6f}, at={act_atol:.6f})')
