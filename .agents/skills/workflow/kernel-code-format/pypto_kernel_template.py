...
# -----------------------------
#   `.agents/pypto-kernel-custom-skills/skills/workflow/kernel-code-format/pypto_kernel_template.py`
#
...
#
# **`pypto_function` (Layer K) vs `pypto.loop` (Layers I–J)** — enforced by **skills/orchestration/lead-orchestrator/references/rules.md** rule **18**,
# **skills/orchestration/lead-orchestrator/references/rules.md** rule **22**, and **`skills/ci-and-pr/ci-and-layout-check/scripts/validate_custom_kernel_layout.py`** (AST scan of
# `custom/<op>/**/*.py`):
...
#
# See also: **skills/orchestration/lead-orchestrator/references/rules.md** (template rule, rule 18), **pypto-kernel-design-format.md**, **skills/orchestration/lead-orchestrator/references/rules.md** (rule 22),
...
#
# ---------------------------------------------------------------------------
# Complex-kernel bundle (repo): `.agents/pypto-kernel-custom-skills/`
# ---------------------------------------------------------------------------
...
#   - **skills/orchestration/lead-orchestrator/references/rules.md** — zero tolerance, staged `*_module*.py` chain, plan updates,
#     `skills/ci-and-pr/ci-and-layout-check/skills/ci-and-pr/ci-and-layout-check/run_validate_layout.sh`, `set_vec_tile_shapes` (≥4 args), `view` ≤4D, **rule 18** (`pypto_function`
...
#   - **skills/debugging/debugging/DEBUG.md** — §1–§8: opaque `Errcode` / `FFFFF` / `UNKNOWN`: do not stop without playbook.
#     **§9**: agent-learned dev patterns — consult BEFORE writing PyPTO code (RULES 7b).
#   - **skills/ci-and-pr/ci-and-layout-check/CI.md** — layout validator command (no NPU).
# Tools (see README.md “Available Tools”):
#   - `python3 .agents/pypto-kernel-custom-skills/skills/ci-and-pr/ci-and-layout-check/skills/ci-and-pr/ci-and-layout-check/scripts/extract_pypto_calls.py <file.py>`
#   - `bash .agents/pypto-kernel-custom-skills/skills/ci-and-pr/ci-and-layout-check/skills/ci-and-pr/ci-and-layout-check/run_validate_layout.sh` (repo root) — also runs an AST
...
#   - Validation runner: `PYTHONPATH=.agents/pypto-kernel-custom-skills/skills/workflow/validation-and-deliverables python custom/<op>/test_<op>.py`
...
#   import from `.agents/pypto-kernel-custom-skills/skills/workflow/validation-and-deliverables/detailed_tensor_compare.py` when using
#   `custom/<op>/test_<op>.py` (see **skills/workflow/validation-and-deliverables/SKILL.md**).
...
# - Optional here: staged work often keeps a shared golden in **`custom/<op>/`** per **skills/workflow/plan-template/plan.template.md**.
# =============================================================================
...
#
# --- pypto-kernel-custom-skills (bundle) — reminders for this layer ---
# - Map each stage label **(A)(B)(C)** to a **semantic module** in the plan (**skills/workflow/phase2-phase3-construction/SKILL.md**:
...
#   current module passes `detailed_tensor_compare` (**skills/orchestration/lead-orchestrator/references/rules.md**).
...
# - **Zero skipping:** do not omit reading **skills/orchestration/lead-orchestrator/references/rules.md** / sub-skill sections that apply (skills/orchestration/lead-orchestrator/references/rules.md).
...
#
# --- pypto-kernel-custom-skills (bundle) — reminders for this layer ---
# - Layout rules for **`custom/<op>/`** are checked by **`skills/ci-and-pr/ci-and-layout-check/skills/ci-and-pr/ci-and-layout-check/run_validate_layout.sh`** (skills/ci-and-pr/ci-and-layout-check/CI.md) — run after
...
# - If stuck: **skills/debugging/debugging/DEBUG.md** → `extract_pypto_calls.py` → op-by-op protocol in **skills/debugging/debugging/SKILL.md** (RULES 7, 13).
# - Before writing PyPTO code: **skills/debugging/debugging/DEBUG.md §9** (RULES 7b) — JIT §9.1, view §9.4, matmul §9.19, etc.
# =============================================================================
...
#   golden boundary tensors until the active module matches golden (**skills/orchestration/lead-orchestrator/references/rules.md**).
...
# - Opaque errors (`Errcode`, `FFFFF`, …): follow **skills/debugging/debugging/DEBUG.md** — do not abandon without playbook (RULES 13).
# =============================================================================
...
# - After edits under **`custom/`**, run **`bash .agents/pypto-kernel-custom-skills/skills/ci-and-pr/ci-and-layout-check/skills/ci-and-pr/ci-and-layout-check/run_validate_layout.sh`**
#   (skills/ci-and-pr/ci-and-layout-check/CI.md; RULES 7, 15) — includes **`pypto_function` + `for ... in range`** detection (RULES 18).
...
# **`skills/workflow/plan-template/plan.template.md`**: `active_module`, `current_staged_file`, **Staged module files** table,
