...
| **CWD** | Repository root |
| **Command** | `PYTHONPATH=.agents/pypto-kernel-custom-skills/skills/workflow/validation-and-deliverables python custom/<operator_name>/test_<operator_name>.py` |
...
Intermediate module-boundary checks must use the same bundled `detailed_tensor_compare`. Record each run in `custom/plan/<operator_name>.md` → Per-module verification log (see `skills/workflow/plan-template/plan.template.md`).
...
```bash
PYTHONPATH=.agents/pypto-kernel-custom-skills/skills/workflow/validation-and-deliverables python custom/<operator_name>/test_<operator_name>.py
```
...
4. **Validation runner script** — `custom/<operator_name>/test_<operator_name>.py` run from repo root with `PYTHONPATH=.agents/pypto-kernel-custom-skills`; must compare all outputs. Do not use `pytest` as the default.
