...
**Agent:** Do **not** skip **`skills/orchestration/lead-orchestrator/references/rules.md`** or sub-skill obligations (staged files, all-output compare, plan logs). See **`.agents/pypto-kernel-custom-skills/skills/orchestration/lead-orchestrator/references/rules.md`** — *Zero tolerance*. **Code layout:** every staged file and the full kernel **must** follow **`.agents/pypto-kernel-custom-skills/skills/workflow/kernel-code-format/pypto_kernel_template.py`** (layers A–L); document exceptions here. After **`custom/`** changes, run **`bash .agents/pypto-kernel-custom-skills/skills/ci-and-pr/ci-and-layout-check/run_validate_layout.sh`** (see **`skills/ci-and-pr/ci-and-layout-check/CI.md`**).
...
- **Command (repo root):** `PYTHONPATH=.agents/pypto-kernel-custom-skills/skills/workflow/validation-and-deliverables python custom/<operator_name>/test_<operator_name>.py`
- **Comparison:** `from detailed_tensor_compare import detailed_tensor_compare` (bundled: `.agents/pypto-kernel-custom-skills/skills/workflow/validation-and-deliverables/detailed_tensor_compare.py`). **Do not** use `pytest` as the default for this golden vs PyPTO check unless documented under **blockers** as an exception.
...
`PYTHONPATH=.agents/pypto-kernel-custom-skills/skills/workflow/validation-and-deliverables python custom/<operator_name>/<operator_name>_module12.py`
...
- **`pypto.set_vec_tile_shapes`:** use tile dimensions as required by **`docs/api/config/pypto-set_vec_tile_shapes.md`** — see **`skills/workflow/phase4-phase5-integration/SKILL.md`** **5.4b**.
...
- **Template:** `.agents/pypto-kernel-custom-skills/skills/workflow/kernel-code-format/pypto_kernel_template.py` — **mandatory** skeleton for each `*_module*.py` and the integrated kernel (same layers; trim unused sections only with justification below).
- Layers A–L from `docs/pypto-kernel-design-format.md` (copy in this bundle: `.agents/pypto-kernel-custom-skills/skills/workflow/kernel-code-format/pypto-kernel-design-format.md`): which apply, which omitted, why.

## PyPTO call-site checklist (when debugging)

Paste output of:

`python3 .agents/pypto-kernel-custom-skills/skills/ci-and-pr/ci-and-layout-check/scripts/extract_pypto_calls.py custom/<operator_name>/<operator_name>_module1234.py` *(or current staged / final kernel file)*

| # | Line | Call | doc OK? | notes |

## `skills/debugging/debugging/DEBUG.md` §9 — pre-write checklist

Before writing each module's PyPTO code, review the matching subsections from **`.agents/pypto-kernel-custom-skills/skills/debugging/debugging/DEBUG.md` §9**. Check off after reading. See full lookup table in **`skills/workflow/phase2-phase3-construction/SKILL.md`** → **Phase 3 → Before writing PyPTO code**.
...
**Do not** abandon the run on these alone. Follow **`.agents/pypto-kernel-custom-skills/skills/debugging/debugging/DEBUG.md`** §1–§8, capture full logs, and iterate (token budget is not a limit). When debugging, also consult **§9.11** (common error → cause → solution quick table). Log each attempt below.
