...
def setup_logger(log_dir: str = "./perf_logs") -> logging.Logger:
    ...
    log_filename = os.path.join(
        log_dir, 
        f"pass_perf_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    )
...
class PassPerfAnalyzer:
    ...
    def print_report(self):
        ...
        lg.info("=" * 140)
        lg.info(f"Total Operations: {self.total_ops}")
        lg.info(f"Time Threshold: {self.time_threshold_s}s ({int(self.time_threshold_s * 1_000_000)} us)")
        lg.info("")
        
        has_comparison = any(d.durations_before for d in sorted_data)
        
        if has_comparison:
            lg.info(f"{'Pass Name':<30} | {'Count':<5} | {'Avg(s)':<8} | {'Target(s)':<9} | {'Total(s)':<9} | {'Avg Ops':<10} | {'Time%':<6} | {'Improve%':<8} | {'Status'}")
            lg.info("-" * 160)
        else:
            lg.info(f"{'Pass Name':<30} | {'Count':<5} | {'Avg(s)':<8} | {'Target(s)':<9} | {'Total(s)':<9} | {'Avg Ops':<10} | {'Time%':<6} | {'Status'}")
            lg.info("-" * 130)
        ...
        for data in sorted_data:
            ...
            if has_comparison and data.durations_before:
                ...
                lg.info(f"{data.name:<30} | {data.count:<5} | {data.avg_duration_s:<8.3f} | {dynamic_threshold:<9.2f} | {data.duration_s:<9.3f} | {avg_ops:<10} | {time_percent:<6} | {improve:<8} | {status}")
            else:
                lg.info(f"{data.name:<30} | {data.count:<5} | {data.avg_duration_s:<8.3f} | {dynamic_threshold:<9.2f} | {data.duration_s:<9.3f} | {avg_ops:<10} | {time_percent:<6} | {status}")
        ...
        if exceeding:
            ...
            for i, (data, threshold) in enumerate(exceeding, 1):
                lg.info(f"  {i}. {data.name}: avg {data.avg_duration_s:.1f}s > target {threshold:.1f}s (ops: {data.ops}, count: {data.count}, time%: {data.time_percent:.1f}%)")
            lg.info("=" * 140)
