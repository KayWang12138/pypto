...
def find_kernel_for_range_violations(source: str, rel_path: Path) -> list[str]:
    ...
    Reinforces skills/orchestration/lead-orchestrator/references/rules.md Prohibition B: use `pypto.loop` for kernel iteration, not Python `range` loops.
    """
    errors: list[str] = []
    try:
    ...
    def _append_for_range_error(lineno: int | str) -> None:
        ...
        errors.append(
            ...
            f"(see skills/orchestration/lead-orchestrator/references/rules.md rule 18). "
...
def find_pypto_view_reshape_misuse(source: str, rel_path: Path) -> list[str]:
    ...
    `docs/api/operation/pypto-view.md` and `skills/debugging/debugging/DEBUG.md` §9.4.
    """
    errors: list[str] = []
    try:
        tree = ast.parse(source)
    ...
    for node in ast.walk(tree):
        ...
        if (
            ...
            errors.append(
                ...
                f"`skills/debugging/debugging/DEBUG.md` §9.4."
            )
...
def find_missing_pypto_loop(source: str, rel_path: Path) -> list[str]:
    ...
    if found_pypto_function and not found_loop:
        warnings.append(
            ...
            f"See `skills/debugging/debugging/DEBUG.md` §9.15 and "
