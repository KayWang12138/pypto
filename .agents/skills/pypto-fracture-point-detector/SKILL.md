...
对每个候选断裂点，**必须实际读取相关文档**后执行检查。具体检查项和判定规则见 `references/detection-rules.md` 中的"二次校验规则"章节。

检查结果处理：
- 根因归属框架/文档 → 计入断裂点总数，输出到报告正文
- 根因归属模型自身 → 标记为"模型问题"，在报告末尾单独列出，不计入断裂点总数
...
- **二次校验**：必须执行步骤 4.5 的两项检查，验证根因是否确实归属于框架或文档，避免将模型自身问题误报为框架/文档断裂点