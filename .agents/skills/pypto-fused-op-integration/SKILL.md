...
| 模式 | 适用场景 | tensor 信息来源 | 核心步骤 |
|------|---------|---------------|---------|
| **整网集成模式** | 有完整部署环境（vllm + 模型权重 + 部署仓库） | 从整网运行采集 | 步骤 1-11 全部可执行 |
| **纯算子开发模式** | 仅算子开发仓库，无整网环境 | 从 README.md / 算子规格 / DESIGN.md 提取 | 跳过整网依赖步骤 |

**判断依据**：检查当前仓库是否包含 vllm 等部署框架和模型权重文件。若无，自动进入纯算子开发模式。
...
> **📌 纯算子开发模式**：本步骤为**集成阶段专属**。纯算子开发模式下，从 README.md / SPEC.md / DESIGN.md 中提取 tensor shape、dtype 等信息替代整网采集。
