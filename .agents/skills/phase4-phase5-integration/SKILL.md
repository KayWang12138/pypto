...
Instead (mirror in staged files — see `skills/lead-orchestrator/references/rules.md` rule 14):
...
**Cross-reference:** `skills/debugging/DEBUG.md §9` expands on many of these rules with concrete error messages, root causes, and fix patterns. In particular: §9.2 (dynamic shapes / `pypto.loop`), §9.4 (`pypto.view` dimensions), §9.15 (tile shape config), §9.19 (matmul API, reduction alignment, assemble shape).
...
When integration tests fail in Phase 4 or structural checks fail in Phase 5, and the standard debugging workflow (`skills/debugging/SKILL.md` + `DEBUG.md §9`) does not resolve the issue, escalate to these subskills:

- **Precision workarounds**: Read `skills/pypto-precision-debug/SKILL.md`
- **Precision bisection**: Read `skills/pypto-precision-compare/SKILL.md`
- **Memory overlap**: Read `skills/pypto-memory-overlap-detector/SKILL.md`
- **AICore error**: Read `skills/pypto-aicore-error-locator/SKILL.md`
- **Host crash**: Read `skills/pypto-host-stacktrace-analyzer/SKILL.md`
- **MACHINE workspace**: Read `skills/pypto-machine-workspace/SKILL.md`

Refer to `skills/phase2-phase3-construction/SKILL.md` → "Subskill delegation: debugging escalation" for the full prioritized order.
