...
> **推荐 agent team 入口**：完整端到端开发请优先使用 `pypto-op-orchestrator`（自带 7 阶段状态机、Stage 5 模块分解、Stage 6 三者循环和工件门禁）。本 Skill 仅适用于无状态手动串联场景，其 Stage 5/6 描述为简化版 legacy 流程（单一实现，无模块分解），实际多模块分解 + coder/verifier/debugger 三者循环以 `pypto-op-orchestrator` 描述为准。
