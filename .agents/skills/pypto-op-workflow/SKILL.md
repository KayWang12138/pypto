...
**关键约束**：
- 优先使用 PyTorch 内置 API，允许使用 numpy/math 等标准科学计算库辅助，禁止引入 pypto/torch_npu
...
| 无标记 + exit ≠ 0 | 运行失败（编译/import/runtime） | → Stage 5 内排查重试（**最多 3 次**） |

**Stage 5 重试限制**：
- 运行失败时在 Stage 5 内排查重试，**最多 3 次**
- 3 次重试后仍失败 → 向用户报告失败原因和已尝试的排查路径，等待用户决策
- 每次重试前必须记录失败原因和修改内容
...
**修复完成后路由**：精度修复通过后，**自动进入 Stage 7（性能调优）**，无需用户手动触发。
...
- **无可优化退出**：连续 2 轮优化无性能提升（<5% 改善），终止调优并报告当前最优结果
