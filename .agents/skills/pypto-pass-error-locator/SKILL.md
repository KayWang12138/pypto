...
2. **工具链依赖**
   - Python 3.8+
   - 可执行 `build_ci.py` 命令
   - 可访问 `$ASCEND_PROCESS_LOG_PATH/` 或 `$ASCEND_WORK_PATH/` 或 `$HOME/ascend/log/` 目录

3. **依赖技能**
   - `pypto-pass-workflow-analyzer`：用于分析业务流程
...
4. **图约束校验**
   - 打开precheck/postcheck可以校验图是否符合约束
   - 帮助定位错误位置

5. **反向定位用户代码**
   - 要了解是用户代码的哪一行异常
   - 反向推导：错误日志中op的magic -> 计算图 -> 前端代码行

6. **环境状态检查**
   - 环境问题可以通过 `pypto-environment-setup` 技能来检查环境状态

7. **日志级别调整**
   - 日志上下文信息不足
   - 可通过调整日志级别打印更多辅助信息
   - 日志相关配置可参考 `references/pass-error-locating-faq.md` 中的Q7、Q8

8. **对比分析**
   - 与正常状态的相对比分析变化点识别错误原因

9. **版本兼容性**
   - 检查PyPTO版本是否支持当前功能
   - 查看版本更新日志中的Breaking Changes
