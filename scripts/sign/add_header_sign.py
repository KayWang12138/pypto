...
def build_inifile(ini_args):
    ...
    返回：-1:失败，0：成功
    '''
    item_size_set, sign_file_dir, bios_tool_path, sign_tmp_path, product_delivery_path, add_sign = ini_args
    cms_flag = False
    if add_sign != "true"
        return 0
...
def build_sign(sign_args):
    ...
    返回：-1:失败，0：成功
    '''
    item_size_set, sign_file_dir, sign_tool_path, sign_tmp_path, root_dir, product_delivery_path = sign_args
    sign_dict = {}
    sign_dict["cms"] = []
    for (infile, conf_item) in list(item_size_set.items()):
    ...
    for file in sign_dict["cms"]:
        ...
        if ret.returncode != 0:
            ...
            return -1
        COMM_LOG.cilog_info(THIS_FILE_NAME, "%s", ret.stdout)
...
def convert_der_file(crl_file: str, der_file: str) -> int:
    ...
    try:
        ...
        if not os.path.isfile(crl_file):
            COMM_LOG.cilog_error(THIS_FILE_NAME, "Input CRL file[%s] not found.", crl_file)
            return 1
        # 调用 openssl 转换
        cmd = f"openssl crl -in {crl_file} -outform DER -out {der_file}"
        result = subprocess.run(shlex.split(cmd), capture_output=True, text=True)
        if result.returncode != 0:
            COMM_LOG.cilog_error(THIS_FILE_NAME, "OpenSSL conversion failed: %s", result.stderr)
            return 1
        return 0
    except Exception as e:
        COMM_LOG.cilog_error(THIS_FILE_NAME, "Unexpected error: %s", e)
        return 1


def assemble_pack_cmd(pack_args):
    bios_tool_path, add_sign, der_file, sign_path, input_file, input_item, conf_item = pack_args
    ...
    sign_flag = add_sign == "true" and conf_item.type != ""
    if sign_flag:
        ...
        for sign in conf_item.type.split('/'):
            ...
            # 临时目录下的ini文件
            ini_file = os.path.join(sign_path, os.path.basename(input_item))
            # certtype 1 表示社区前面
            cmd = cmd + " -cms %s.ini.p7s -ini %s.ini -crl %s -certtype 1 --addcms" % (ini_file, ini_file, der_file)
...
def pack_image_files(pack_args):
    item_size_set, sign_file_dir, bios_tool_path, add_sign, sign_tmp_path, product_delivery_path, der_file = pack_args
    for (input_item, conf_item) in list(item_size_set.items()):
        input_file = os.path.join(sign_file_dir, input_item)
        ...
        cmd = assemble_pack_cmd((bios_tool_path, add_sign, der_file, sign_path, input_file, input_item, conf_item))
        COMM_LOG.cilog_info(THIS_FILE_NAME, "------------------------------------")
        COMM_LOG.cilog_info(THIS_FILE_NAME, "execute:%s", cmd)
        ret = subprocess.run(shlex.split(cmd), capture_output=True, text=True)
        if ret.returncode != 0:
...
def add_bios_header(bios_args):
    ...
    item_size_set, sign_file_dir, bios_tool_path, sign_tool_path, root_dir, add_sign = bios_args
    ...
    ret_code = pack_image_files((item_size_set, sign_file_dir, bios_tool_path, add_sign, sign_tmp_path,
                                 product_delivery_path, der_file))
    if ret_code != 0:
        return ret_code
