...
def build_inifile(item_size_set, sign_file_dir, bios_tool_path,
    ...
    if add_sign == "true" and cms_flag:
        ...
        ret = subprocess.getstatusoutput(shlex.split(cmd))
        if ret[0] != 0:
...
def convert_der_file(crl_file: str, der_file: str) -> int:
    ...
    try:
        ...
        result = subprocess.getstatusoutput(shlex.split(cmd))
        if result[0] != 0:
            COMM_LOG.cilog_error(THIS_FILE_NAME, "OpenSSL conversion failed: %s", result[1])
            return 1
        return 0
