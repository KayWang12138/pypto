...
def cilog_get_logheader(filename, line_no):
    """
    功能描述：输出日志头信息
    参数：包含时间，文件名和行号
    返回值：日志头信息
    ...
    return "[%s][%s][%d] " % (log_timestamp, filename, line_no)
...
def cilog_error(filename, log_msg, *log_paras):
    ...
    log_whole_msg = cilog_get_logheader(filename, line_no) + log_msg % log_paras
    logging.error(log_whole_msg)
    return
