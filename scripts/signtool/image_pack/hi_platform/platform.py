...
import os
import binascii
from tools import to_bytes, cal_bin_hash


def __construct_header(header_args):
    n_buf, e_buf, hash_buf, code_len, suffix, head_type, version, nvcnt, tag, certtype,
                       before_header, large_packet, enc, pss, bcm, gcm, gm = header_args
    ...
    img_sign_obj_len = 0 if before_header and large_packet else code_len
    ...
    h2c_enable = 0x41544941 if bcm else 0xA5A55555
    h2c_cert_len = 0x800 if bcm else 0
    h2c_cert_offset = 0x1000 if bcm else 0
    root_pubkeyinfo = 0
    head_magic = 0x33CC33CC
    cms_flag = int(0).to_bytes(16, 'big')
    code_nvcnt = int(0).to_bytes(8, 'big')
    code_tag = int(0).to_bytes(16, 'big') if tag is None else bytes(str(tag), 'ascii')
    ...
    pack_list = (preamble, rev0, head_len, user_len, user_define_data, hash_buf, sub_key_cert_offset, sub_cert_len,
                 uw_rootkey_alg, img_sign_algo, root_pubkey_len, n_buf, e_buf, img_offset,
                 img_sign_obj_len, sign_offset, sign_len, code_encrypt_flag, code_encrypt_algo,
                 zero_bytes_32, km_ireation_cnt, code_encrypt_iv, code_encrypt_tag, code_encrypt_add, rsv1, h2c_enable,
                 h2c_cert_len, h2c_cert_offset, root_pubkeyinfo, rsv2, head_magic, zero_bytes_32, cms_flag, code_nvcnt,
...
def __write_header(header_args):
    out, header, suffix, head_type, code_len, before_header = header_args
    header_base = 0x1000 if head_type else 0
...
def __write_header_hash(header_args):
    out, suffix, head_type, sm, code_len, before_header = header_args
    header_base = 0x1000 if head_type else 0
...
def __write_single_header(args, out, hash_buf, code_len, head_type=0):
    ...
    header = __construct_header((n_buf, e_buf, hash_buf, code_len, args.S, head_type, args.ver, args.nvcnt, args.tag,
                                 args.certtype, before_header, large_packet, False, False, False, False, False))
...
def write_image(args, out):
    ...
    __write_header_hash((out, args.S, 0, False, 0, False))
...
def __add_magic_number_and_file_size(magic_args):
    args, out, cms_flag, suffix, code_len, before_header, large_packet = magic_args
...
def write_extern(args, out, code_list):
    before_header = True if (args.position == 'before_header') else False
    code_len = code_list[0] if before_header else 0
    if before_header:
        __write_header_hash((out, args.S, 0, args.sm, code_len, before_header))
    __add_magic_number_and_file_size((args, out, args.addcms, False, code_len, before_header, False))
    return
