...
def get_args():
    ...
    parser.add_argument('-hash_list_img', required=False, dest='hash_list_img_path',
            ...
            for elem in tree.iter(tag='image'):
                if elem.attrib['tag'] == args.new_image_name:
                    ...
                    with open(hash_list_path, 'a+') as f:
                        ...
                        line = '{};'.format(','.join(line_elem))
                        f.write(line)
                        logging.info("Add hash value[%s] of image[%s] to [%s].", hash_val, args.new_image_name, hash_list_path)
        else:
