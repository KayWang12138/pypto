...
def get_args():
    ...
    parser.add_argument('-position', required=False, choices=['before_header', 'after_header'],
                        help='INPUT: The relative position of raw_img and head')
    parser.add_argument('-pkt_type', required=False, choices=['normal_pkt', 'large_pkt'], nargs='?', const='large_pkt',
                        default='large_pkt', help='INPUT: The large_pkt support larger than 4GB packet')
    parser.add_argument('-partition_size', required=False, nargs='?', default='2048',
                        help='INPUT: The rootfs/app.img total size(M)')

    # nvcnt
    parser.add_argument('-nvcnt', required=False, dest='nvcnt', nargs='?', const=None,
    ...
    parser.add_argument('-certtype', required=False, dest='certtype', choices=[0x1, 0x2, 0xFFFFFFFF],
...
if __name__ == '__main__':
    ...
    if check_image_headered(args.raw) == True:
        logging.error("Detected 8K header magic number, No need to add head again")
        sys.exit()  # 退出程序
