...
@pypto.frontend.jit(
    ...
    verify_options = {
        ...
        # "pass_verify_pass_filter": ["InferParamIndex"],
        "pass_verify_pass_filter": ["all"]
    }
)
def chunk_gated_delta_rule(
        query: pypto.Tensor([pypto.DYNAMIC, ...], pypto.DT_FP32), 
        key: pypto.Tensor([pypto.DYNAMIC, ...], pypto.DT_FP32), 
        value: pypto.Tensor([pypto.DYNAMIC, ...], pypto.DT_FP32), 
        beta: pypto.Tensor([pypto.DYNAMIC, ...], pypto.DT_FP32), 
        gate: pypto.Tensor([pypto.DYNAMIC, ...], pypto.DT_FP32), 
        states: pypto.Tensor([pypto.DYNAMIC, ...], pypto.DT_FP32), 
        mask: pypto.Tensor([], pypto.DT_FP32), 
        tril_mask: pypto.Tensor([], pypto.DT_FP32), 
        eye: pypto.Tensor([], pypto.DT_FP32), 
        act_seq_len: pypto.Tensor([pypto.DYNAMIC, ...], pypto.DT_INT32), 
        zeros_16: pypto.Tensor([], pypto.DT_FP32), 
        zeros_32: pypto.Tensor([], pypto.DT_FP32), 
        zeros_64: pypto.Tensor([], pypto.DT_FP32), 
        core_attn_out: pypto.Tensor([pypto.DYNAMIC, ...], pypto.DT_FP32), 
        last_state_data: pypto.Tensor([pypto.DYNAMIC, ...], pypto.DT_FP32),
    ):
        ...
        for nv_idx in pypto.loop(nv, name="LOOP_Nv_TND", idx_name="nv_idx"):
            ...
            for s_idx in pypto.loop(0, s, l, name="LOOP_S_TND", idx_name="s_idx", unroll_list=[16, 1]): # 2
                ...
                if pypto.is_loop_end(s_idx):
                    ...
                    value_out, key_cum_out = cal_value_and_key_cumdecay(A_block_inverse, pad_v, pad_b, key_beta, gate_cum)
...
def pypto_chunk_gated_delta_rule(
    ...
    zeros_16 = torch.zeros([16, 16], dtype=torch.float32, device=query_data.device)
    zeros_32 = torch.zeros([32, 32], dtype=torch.float32, device=query_data.device)
    zeros_64 = torch.zeros([64, 64], dtype=torch.float32, device=query_data.device)


    inputs = [query_data, key_data, value_data, beta_data, gate_data, state_data, mask_data,
                    tril_mask_data, eye_data, act_seq_len, zeros_16, zeros_32, zeros_64]
    
    outputs = [core_attn_out, last_state_data]

    # torch.npu.empty_cache()
    chunk_gated_delta_rule(*inputs, *outputs)
