...
def engram_backward_kernel(
    ...
    d_embeddings_tmp = pypto.tensor([B, L, H, D], d_embeddings.dtype, name="d_embeddings")
    pypto.set_vec_tile_shapes(128, 128)
    ...
    for h_idx in pypto.loop(H, name="h_loop"):
        pypto.set_vec_tile_shapes(128)
        d_key_gamma_tmp = pypto.full(size=[D], fill_value=0.0, dtype=d_key_gamma.dtype)
        d_query_gamma_tmp = pypto.full(size=[D], fill_value=0.0, dtype=d_query_gamma.dtype)
        ...
        for b_idx in pypto.loop(0, B, 1, name="b_loop"):
            for l_idx, unroll_length in pypto.loop_unroll(0, L, 1, name="l_loop", unroll_list=unroll_list):
                tile = unroll_length
                # ! 对于H的冗余操作
                pypto.set_vec_tile_shapes(1, 128, 128)
                value_lineared_view = value_lineared[b_idx:b_idx+1, l_idx:l_idx+tile] # [1, tile, D]
                embeddings_view = embeddings[b_idx:b_idx+1, l_idx:l_idx+tile] # [1, tile, D]
                value_lineared_view_2d = value_lineared_view.reshape([tile, D])

                embeddings_view_2d = embeddings_view.reshape([tile, D])

                pypto.set_vec_tile_shapes(1, 128, 1, 128)
                grad_out_view = grad_out[b_idx, l_idx:l_idx+tile, h_idx] # [1, tile, 1, D]
                gate_view = gate[b_idx, l_idx:l_idx+tile, h_idx] # [1, tile, 1, 1]
                score_view = score[b_idx, l_idx:l_idx+tile, h_idx] # [1, tile, 1, 1]
                key_lineared_view = key_lineared[b_idx, l_idx:l_idx+tile, h_idx] # [1, tile, 1, D]
                query_view = hidden_states[b_idx, l_idx:l_idx+tile, h_idx] # [1, tile, 1, D]
                ...
                d_query_view, d_query_gamma_view = rms_norm_backward_module(
                    d_query_normed, query_view, query_gamma_view) # [tile, D], [D] # TODO remove
                ...
                pypto.set_vec_tile_shapes(128, 128, 1, 128)
                d_hidden[b_idx, l_idx:l_idx+tile, h_idx] = d_query_view # TODO change to d_query_normed

                pypto.set_vec_tile_shapes(128, 128)
                d_key_gamma_tmp[:] = d_key_gamma_tmp + d_key_gamma_view # 每个tile累加一次
                d_query_gamma_tmp[:] = d_query_gamma_tmp + d_query_gamma_view # 每个tile累加一次 # TODO remove
                ...
                # ! 累加线性梯度
                pypto.set_vec_tile_shapes(128, 128)
                d_key_w_tmp[:] = d_key_w_tmp + d_key_w_view
                pypto.set_vec_tile_shapes(128)
                ...
                pypto.set_vec_tile_shapes(128, 128)
                d_k_v_view = d_key_view + d_value_view
        ...
        d_query_gamma[h_idx] = d_query_gamma_tmp # TODO remove
    ...
    # ! issue 1
    pypto.set_vec_tile_shapes(1, 128, 1, 128)
    d_embeddings_summed = pypto.sum(d_embeddings_tmp, 2, keepdim=True)
    # d_embeddings[:] = d_embeddings_summed
    # pypto.assemble(d_embeddings_summed, [0, 0, 0, 0], d_embeddings)
...
def engram_backward_pto(
    ...
    d_embeddings = torch.zeros([B, L, 1, D], dtype=torch.float32, device=device) # TODO 待框架优化，改为[B, L, D]
    ...
    d_query_gamma = torch.zeros_like(query_gamma)
    ...
    d_embeddings = d_embeddings[:, :, 0, :]
    return d_hidden, d_embeddings, d_key_w, d_key_b, d_value_w, d_value_b, d_key_gamma, d_query_gamma


def test_engram_backward():
    B, L, H, D = 1, 4, 1, 5
    device_id = 3
    torch.npu.set_device(device_id)
    ...
    pto_outputs = [
        ...
        d_val_b_pto,         # [D]
        d_key_gamma_pto,     # [H, D]
        d_query_gamma_pto,   # [H, D]
    ] = engram_backward_pto(*inputs)


if __name__ == '__main__':
    test_engram_backward()
