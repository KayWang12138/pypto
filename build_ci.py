...
class BuildCtrl(CMakeParam):
    ...
    def _main(cls):
        ...
        if ctrl.feature.frontend_type_python3:
            ...
            ctrl.cmake_build()
        # 计算总耗时
        duration = ctrl.duration(ts=ts, remain=False)
        logging.info("Build[CI] Finish, %s", duration)
    ...
    def duration(self, ts: datetime, remain: bool = True) -> str:
        duration = int((datetime.now(tz=timezone.utc) - ts).seconds)
        duration_str = f"Duration {duration} secs"
        if self.timeout:
            self.timeout = max(self.timeout - duration, 0)
            duration_str += f" Remain {self.timeout} secs" if remain else ""
        return duration_str
