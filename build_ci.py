...
class BuildCtrl(CMakeParam):
    ...
    def py_run_examples(self, dist: Optional[Path], tests: TestsFilterParam, def_filter: str,
        ...
        if self.feature.backend_type == "npu":
            # NPU 模式
            cmd = f"{sys.executable} examples/validate_examples.py -t {filter_str} -d {dev_ext_comma}"
        ...
        elif self.feature.backend_type == "cost_model":
            # SIM 模式
            n_workers_val = int(n_workers) if n_workers != "auto" else 16
            cmd = f"{sys.executable} examples/validate_examples.py -t {filter_str} --run_mode sim -w {n_workers_val}"
