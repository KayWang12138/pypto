...
class BuildCtrl(CMakeParam):
    ...
    def __init__(self, args):
        ...
        self.timeout: Optional[int] = args.timeout if args.timeout and args.timeout > 0 else None  # 超时时长
    ...
    def _main(cls):
        ...
        if ctrl.feature.frontend_type_python3:
            ...
            ctrl.cmake_build()
        # 计算总耗时
        duration = ctrl.duration(ts=ts)
        logging.info("Build[CI] Finish, duration %s", duration)
    ...
    def run_build_cmd(self, cmd: str, update_env: Optional[Dict[str, str]] = None,
        ...
        with subprocess.Popen(shlex.split(cmd), env=env, text=True, encoding='utf-8',
            ...
            try:
                ...
                _stop_pg(_msg=f"Timeout({self.timeout})", _p=process)
                raise e
            except KeyboardInterrupt as e:
        ...
        # 超时时长更新
        duration = self.duration(ts=ts)
        return subprocess.CompletedProcess(process.args, ret_code, stdout, stderr), duration

    def duration(self, ts: datetime) -> str:
        duration = int((datetime.now(tz=timezone.utc) - ts).seconds)
        duration_str = f"{duration}/{self.timeout} secs" if self.timeout else f"{duration} secs"
        self.timeout = max(self.timeout - duration, 0) if self.timeout else self.timeout
        return duration_str
    ...
    def pip_install(self, whl: Path, dest: Optional[Path] = None, opt: str = "",
        ...
        logging.info("Install %s, Cmd: %s, Timeout: %s", whl, cmd, self.timeout)
        _, duration = self.run_build_cmd(cmd=cmd, update_env=update_env, pg_desc="pip")
        logging.info("Install %s%s success, Duration %s sec", whl, f" to {dest}" if dest else "", duration)
    ...
    def pip_uninstall(self, name: str, path: Optional[Path] = None):
        ...
        if path:
            ...
            logging.info("Uninstall %s package, Cmd: %s, Timeout: %s", name, cmd, self.timeout)
    ...
    def cmake_configure(self):
        ...
        logging.info("CMake Configure, Cmd: %s, Timeout: %s", cmd, self.timeout)
        _, duration = self.run_build_cmd(cmd=cmd, update_env=update_env)
        logging.info("CMake Configure success, Duration %s", duration)
    ...
    def cmake_build(self):
        ...
        for i, c in enumerate(cmd_list, start=1):
            ...
            logging.info("CMake Build(%s/%s), Cmd: %s, Timeout: %s", i, len(cmd_list), c, self.timeout)
            try:
                ...
                raise e
            logging.info("CMake Build(%s/%s) success, Duration %s", i, len(cmd_list), duration)
    ...
    def py_build(self):
        ...
        if self._use_pip_install_mode() or self.feature.whl_editable:
            ...
            logging.info("Build whl, Cmd: %s, Timeout: %s", cmd, self.timeout)
            _, duration = self.run_build_cmd(cmd=cmd, update_env=update_env, pg_desc="build")
            logging.info("Build whl success, Duration %s", duration)
    ...
    def _py_tests_run_pytest(self, dist: Optional[Path], filter_str: str, ext: str = ""):
        ...
        if case_timeout and case_timeout > 0:
            pytest_timeout_installed = self.check_pip_dependencies(deps={"pytest-timeout": ">=2.0.0"},
                                                                   raise_err=False, log_err=False)
            if pytest_timeout_installed:
                cmd += f" --timeout={case_timeout}"
            else:
                logging.warning("pytest-timeout not installed - configured test case timeout is ineffective.")
        # cmd 执行
        ...
        logging.info("pytest run, Cmd: %s, Timeout: %s", cmd, self.timeout)
        _, duration = self.run_build_cmd(cmd=cmd, update_env=update_env, pg_desc="pytest")
        logging.info("pytest run success, Duration: %s", duration)
