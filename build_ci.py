...
class BuildCtrl(CMakeParam):
    ...
    def py_tests(self):
        ...
        # 执行用例(STest, Example)
        # 为获取最大的并行度, 把这两个目录放在一次 pytest 内执行
        ...
        # filter 处理及用例执行
        fs_1: str = self.tests.example.get_filter_str(def_filter=str(Path(self.src_root, "examples")))
        fs_2: str = self.tests.stest.get_filter_str(def_filter=str(Path(self.src_root, "python/tests/st")))
        if fs_1 or fs_2:
            filter_str: str = f"{fs_1} {fs_2}"
            self._py_tests_run_pytest(dist=dist, filter_str=filter_str, ext=ext_str)
