...
class BuildCtrl(CMakeParam):
    ...
    def py_tests(self):
        ...
        if self.build.job_num is not None and self.build.job_num > 0:
            ...
            n_workers = "auto"
        self.py_tests_run_pytest(dist=dist, params=[(self.tests.utest, str(Path(self.src_root, "python/tests/ut")))],
        ...
        self.py_tests_run_pytest(dist=dist, params=[(self.tests.stest, str(Path(self.src_root, "python/tests/st"))),
                                                    (self.tests.example, str(Path(self.src_root, "examples")))],
                                 ext=ext_str)
    ...
    def py_tests_run_pytest(self, dist: Optional[Path], params: List[Tuple[TestsFilterParam, str]], ext: str = ""):
        ...
        if not filter_str:
            return
        # 执行 pytest
        self._py_tests_run_pytest(dist=dist, filter_str=cur_filter_str, ext=ext)
