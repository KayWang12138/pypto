...
class BuildCtrl(CMakeParam):
    ...
    def py_run_examples(self, dist: Optional[Path], tests: TestsFilterParam, def_filter: str, 
                        dev_ext_comma: str = "", n_workers: str = "auto"):
        if not tests.enable:
            return
        # filter 处理
        filter_str = tests.get_filter_str(def_filter=def_filter)
        # 执行 validate_examples脚本
        n_workers = int(n_workers) if n_workers != "auto" else 16
        self._py_run_examples(dist=dist, filter_str=filter_str, 
                              dev_ext_comma=dev_ext_comma, n_workers=n_workers)
    ...
    def _py_run_examples(self, dist: Optional[Path], filter_str: str, 
                         dev_ext_comma: str = "", n_workers: int = 16):
        if not self.tests.exec.auto_execute:
            return
        # filter 处理
        filter_str = filter_str.replace(',', ' ')
        ...
        cmd_sim = f"{sys.executable} examples/validate_examples.py -t {filter_str} \
            --run_mode sim -w {n_workers}"
        # cmd 执行
        origin_env = os.environ.copy()
        update_env = {}
        if dist:
        ...
        ret, duration = self.run_build_cmd(cmd=cmd_npu, check=True, update_env=update_env)
        ret.check_returncode()
        logging.info("examples --run_mode npu, Cmd: %s, Duration %s sec", cmd_npu, duration)
        logging.info("examples --run_mode sim, Cmd: %s", cmd_sim)
        ret, duration = self.run_build_cmd(cmd=cmd_sim, check=True, update_env=update_env)
        ret.check_returncode()
