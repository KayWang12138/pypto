...
class BuildCtrl(CMakeParam):
    ...
    def _py_tests_get_xsan_env(self) -> Dict[str, str]:
        ...
        try:
            ...
            logging.warning("Failed to read XSAN config file: %s", e)
        return update_env

    def _py_tests_mdf_xan_env(self, update_env: Dict[str, str]):
        # 特殊处理
        ...
        asan_option_list = [op for op in asan_options.split(",") \
                            if not op.startswith("detect_leaks") and not op.startswith("suppressions")]
        if enable_detect_leaks:
            ...
            asan_option_list.append(f"suppressions={suppressions_file}")
        else:
