...
OOO_PRESCHEDULE_METHOD_DEFAULT = "ooo_preschedule_method_default"
...
def mla_pre(**kwargs) -> List[pto.tensor]:
    ...
    if split_k:
        def inside_if_split_k():
            nonlocal q_mm_res
            tmp_c = pto.tensor(pto.DataType.DT_FP32, [bs, q_lora_rank], "tmp_q")
            ...
            for ki in range(k_split):
                ...
                tmp = pto.matmul(pto.DataType.DT_FP32, input_mk, input_kn)  # [b*s,h/2] * [h/2,q_lora_rank]
    ...
    if split_k:
        def inside_if_split():
            ...
            tmp_c_kv = pto.tensor(pto.DataType.DT_FP32, [bs, kv_n], "tmp_kv")
            ...
            for ki in range(k_split_kv):
                ...
                tmp = pto.matmul(pto.DataType.DT_FP32, input_mk, input_kn)  # [b*s,h/2] * [h/2,kv_n] = [b*s,kv_n]
                        ...
                        with pto.loop_function("LOOP_L1_nIdx", "n_idx", pto.loop_range(0, n_loop, 1)) as n_idx_loop:
                            for n_idx in n_idx_loop:
                                def inside_n_idx_loop(b_idx, n_idx, bn_per_batch):
                                    ...
                                    cur_n_tile = n_tile
                                    oi_update = pto.tensor(pto.DataType.DT_FP32, [n_tile, d_n], "oiUpdate")
                                    li_update = pto.tensor(pto.DataType.DT_FP32, [n_tile, 1], "liUpdate")
                                    mi_update = pto.tensor(pto.DataType.DT_FP32, [n_tile, 1], "miUpdate")
                                    ...
                                    with pto.loop_function("LOOP_L2_bn", "bn", pto.loop_range(0, bn_per_batch, 1),
                                        ...
                                        for bn in bn_loop:
                                            def inside_bn_loop(**kwargs):
                                                ...
                                                ## -> (curNTile, curS2Tile)
                                                sij_scale = pto.mul_s(
                                                    sij, pto.element(pto.DataType.DT_FP32, float(softmax_scale)))
            ...
            with pto.loop_function("PaPost", "b_idx", pto.loop_range(b_loop), set(), True) as b_idx_loop:
                for b_idx in b_idx_loop:
                    def inside_b_idx_loop_post(**kwargs):
                        ...
                        cast1 = pto.cast(r1_res, pto.DataType.DT_FP16)
                        ...
                        res = pto.matmul(pto.DataType.DT_INT32, quantized_a, weight_o)

                        pto.set_vec_tile_shapes(min(NUM_32, tile_b * s), min(NUM_32, h))  # raw (tileB*1, 7168)
                        res.move(pto.cast(res, pto.DataType.DT_FP32))
                        ...
                        bmm5_res = pto.cast(res, pto.DataType.DT_FP16, pto.cast_mode.CAST_RINT)
...
def test_dynamic_attention(params, pa_tile_config, is_quant=False, cache_mode="BNSD", use_pre_fetch=False):
    ...
    d_type_quant_in = pto.DataType.DT_INT8 if is_quant else d_type
    ...
    kv_len = pto.tensor(pto.DataType.DT_INT64, kv_len_shape, "kv_len")
    ...
    # pa
    block_table = pto.tensor(pto.DataType.DT_INT32, [b, max_block_num_per_batch], "blockTable")
    act_seqs = pto.tensor(pto.DataType.DT_INT32, [b], "actSeqs")
    #out mla
    pa_out = pto.tensor(pto.DataType.DT_FP32, [b * n * s, kv_lora_rank], "paOut")
    #post
    weight_uv = pto.tensor(d_type, [n, kv_lora_rank, v_head_dim], "weightUV")
    weight_o = pto.tensor(pto.DataType.DT_INT8, [n * v_head_dim, h], "weightO")
    weight_o_scale_w = pto.tensor(pto.DataType.DT_FP32, [1, h], "weightOScaleW")
    # output
