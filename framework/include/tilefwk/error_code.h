...
// -----------------------------------------------------------------------------
// 枚举底层值（统一定义；common.h 通过包含本头使用，避免重复定义）
// -----------------------------------------------------------------------------
template <typename T, typename = std::enable_if_t<std::is_enum_v<T>>>
inline constexpr std::underlying_type_t<T> ToUnderlying(T value) noexcept
{
...
enum class DevCommonErr : uint32_t {
    ...
    CMD_ERROR = ToUnderlying(MachineError::DEV_COMMON) + 0x08U,
