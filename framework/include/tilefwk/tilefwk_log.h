...
inline PyptoLogFuncInfo logFuncInfo;
}

#define INNER_PYPTO_LOG(level, module, fmt, ...)                                                                                                   \
    do {                                                                                                                                           \
        if (tile::fwk::logFuncInfo.checkLevel != nullptr && tile::fwk::logFuncInfo.record != nullptr) {                                            \
            if (tile::fwk::logFuncInfo.checkLevel(PYPTO, level)) {                                                                                 \
                tile::fwk::logFuncInfo.record(PYPTO, level, "[%s][%s:%d][%s]:" fmt, module, __FILE_NAME__, __LINE__, __FUNCTION__, ##__VA_ARGS__); \
            }                                                                                                                                      \
        }                                                                                                                                          \
    } while (0)
