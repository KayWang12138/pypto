...
{
    return std::getenv("ASCEND_HOME_PATH") == nullptr ? false : true;
}
namespace tile::fwk {
bool TileFwkCheckLogLevel(int32_t log_level);
void TileFwkLogRecord(int32_t log_level, const char *fmt, ...);
}
#define PYPTO_RECORD_TILEFWKLOG(level, fmt, ....)                                                                                  \
    do {                                                                                                                           \
        if (tile::fwk::TileFwkCheckLogLevel(level)) {                                                                              \
            tile::fwk::TileFwkLogRecord(level, "[%s:%d][%lu][%s]" fmt, __FILE__, __LINE__, GetTid(), __FUNCTION__, ##__VA_ARGS__); \
        }                                                                                                                          \
    } while (0)
...
#define PYPTO_RECORD_SLOG(level, fmt, ....)                                                                              \
    do {                                                                                                                 \
        if (CheckLogLevel(AICPU, level) == 1) {                                                                          \
            DlogRecord(AICPU, level, "[%s:%d][%lu][%s]" fmt, __FILE__, __LINE__, GetTid(), __FUNCTION__, ##__VA_ARGS__); \
        }                                                                                                                \
    } while (0)
#endif

#if defined(__DEVICE__)
#define PYPTO_LOG_INNER(level, fmt, ....) PYPTO_RECORD_SLOG(level, fmt, ...)
#elif !defined(BUILD_WITH_CANN)
#define PYPTO_LOG_INNER(level, fmt, ....) PYPTO_RECORD_TILEFWKLOG(level, fmt, ...)
#else
#define PYPTO_LOG_INNER(level, fmt, ....)
    do {
        if (IsSlogEnable) {
            PYPTO_RECORD_SLOG(level, fmt, ...);
        } else {
            PYPTO_RECORD_TILEFWKLOG(level, fmt, ...);
        }
    } while (0)
#endif
