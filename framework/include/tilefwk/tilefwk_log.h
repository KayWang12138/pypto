...
#ifdef __DEVICE__
#include "dlog_pub.h"
const int32_t PYPTO_MOD_ID = AICPU;
#else
const int32_t PYPTO_MOD_ID = 36;
...
#define INNFER_PYPTO_LOG(level, fmt, ...)                                                                             \
    do {                                                                                                              \
        if (CheckLogLevel(PYPTO_MOD_ID, level) == 1) {                                                                \
            DlogRecord(PYPTO_MOD_ID, level, "[%s:%d][%s]:" fmt, __FILENAME__, __LINE__, __FUNCTION__, ##__VA_ARGS__); \
        }                                                                                                             \
    } while (0)
    ...
    do {                                                                                                                          \
        if (logFuncInst.checkLevel != nullptr && logFuncInst.record != nullptr) {                                                 \
            if (logFuncInst.checkLevel(PYPTO_MOD_ID, level)) {                                                                    \
                logFuncInst.record(PYPTO_MOD_ID, level, "[%s:%d][%s]:" fmt, __FILENAME__, __LINE__, __FUNCTION__, ##__VA_ARGS__); \
            }                                                                                                                     \
        }                                                                                                                         \
    } while (0)
