...
public:
     ...
     * \param dependConst : Whether the tensor depends on const. The default value is false.
    ...
    Tensor(DataType dataType, const Shape &shape, uint8_t *dataPtr, std::string name,
        TileOpFormat format = TileOpFormat::TILEOP_ND, bool dependConst = false)
        : Tensor(dataType, shape, name, format, dependConst) {
        SetData(dataPtr);
    }
    ...
    Tensor(DataType t, std::initializer_list<SymbolicScalar> shape, std::string name = "",
        TileOpFormat format = TileOpFormat::TILEOP_ND, bool dependConst = false)
        : Tensor(t, std::vector<SymbolicScalar>(shape), name, format, dependConst) {}
    ...
    */
     ...
     *
     * \param dependConst : Whether the tensor depends on const.
     */
    void SetDependConst(bool dependConst);
