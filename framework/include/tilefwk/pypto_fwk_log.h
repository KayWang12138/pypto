...
public:
    ...
    void(*record)(int32_t, int32_t, const char *, ...) __attribute__((format(printf, 2, 3)));
    void(*pyptoRecord)(int32_t, int32_t, const char *, ...) __attribute__((format(printf, 2, 3)));
    void(*setAttr)(bool);
private:
    LogFuncInfo();
    ~LogFuncInfo();
...
#define PYPTO_SIM_LOG(level, module, fmt, ...)                                                                                                        \
    do {                                                                                                                                              \
        ...
        if (npu::tile_fwk::LogFuncInfo::Instance().checkLevel != nullptr && npu::tile_fwk::LogFuncInfo::Instance().pyptoRecord != nullptr) {          \
            if (npu::tile_fwk::LogFuncInfo::Instance().checkLevel(PYPTO, level)) {                                                                    \
                npu::tile_fwk::LogFuncInfo::Instance().pyptoRecord(PYPTO, level, "[%s:%d][%s]:" fmt, __FILE_NAME__, __LINE__, module, ##__VA_ARGS__); \
            }                                                                                                                                         \
        }                                                                                                                                             \
    } while (0)
