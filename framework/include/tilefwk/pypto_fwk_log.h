        ...
        if (len < MAX_LOG_LENGTH) {                                                                                                          \
            ...
            while (msgBegin < msgEnd) {                                                                                                      \
                npu::tile_fwk::LogFuncInfo::Instance().record(PYPTO, level, "[%s:%d][%s]:%.880s", __FILE_NAME__, __LINE__,                   \
                                                              npu::tile_fwk::GetLogModuleName(module).c_str(), msgBegin);                    \
...
#define CODEGEN_LOGI_FULL(...) PYPTO_HOST_SPLIT_LOG(DLOG_INFO, npu::tile_fwk::LogModule::CODEGEN, __VA_ARGS__)
...
#define VERIFY_LOGE_FULL(...) PYPTO_HOST_SPLIT_LOG(DLOG_ERROR, npu::tile_fwk::LogModule::VERIFY, __VA_ARGS__)
