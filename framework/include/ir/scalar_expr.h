...
inline DataType PromoteSameCategoryDtype(
    const DataType &left_dtype, const DataType &right_dtype, const std::string &opName) {
    if (IsBoolDtype(left_dtype) || IsBoolDtype(right_dtype)) {
        throw TypeError("Operator '" + opName + "' does not accept bool dtype");
    }
    auto left_category = GetNumericCategory(left_dtype, opName);
    auto right_category = GetNumericCategory(right_dtype, opName);
    if (left_category != right_category) {
        throw TypeError("Operator '" + opName + "' requires same numeric dtype category, got " + left_dtype.ToString() +
...
inline BinaryOperands PromoteBinaryOperands(
    ...
    DataType promoted_dtype = PromoteSameCategoryDtype(left_dtype, right_dtype, opName);
...
inline BinaryOperands PromoteIntBinaryOperands(
    ...
    if (!left_dtype.IsInt() || !right_dtype.IsInt()) {
        throw TypeError("Operator '" + opName + "' requires integer dtype, got " + left_dtype.ToString() + " and " +
