...
/**
 * \brief Get a keyword argument value from kwargs vector
 *
 * Returns std::nullopt if the key is not found, so the caller can decide
 * how to handle the missing key (e.g., use a default or report an error
 * via CHECK). This avoids throwing exceptions for normal control flow.
 *
 * \tparam T The expected value type
 * \param kwargs The keyword arguments vector
 * \param key The key to look up
 * \param span Source location for error reporting on type mismatch
 * \return The value if found, or std::nullopt if not found
 * \throws TypeError if the key exists but its value has the wrong type
 */
template <typename T>
std::optional<T> GetKwarg(const std::vector<std::pair<std::string, std::any>> &kwargs, const std::string &key,
    const Span &span = Span::Unknown()) {
    for (const auto &entry : kwargs) {
        if (entry.first == key) {
            return AnyCast<T>(entry.second, "kwarg key: " + key + " at " + span.ToString());
        }
    }
    return std::nullopt;
}
...
/**
 * \brief Get a keyword argument value from kwargs vector with a default
 *
 * Returns defaultValue if the key is not found. Never throws for a missing key.
 *
 * \tparam T The expected value type
 * \param kwargs The keyword arguments vector
 * \param key The key to look up
...
T GetKwarg(const std::vector<std::pair<std::string, std::any>> &kwargs, const std::string &key,
    const T &defaultValue, const Span &span = Span::Unknown()) {
    for (const auto &entry : kwargs) {
        if (entry.first == key) {
            return AnyCast<T>(entry.second, "kwarg key: " + key + " at " + span.ToString());
        }
