...
void WinAttentionCompute(const Tensor &qNope, Tensor &vNopeCache, const Tensor &qRope, Tensor &kRopeCache, int nQ, int nKv,
    ...
    ASSERT(ERROR_CODE_UNDEFINED, nKv != 0) << "nKv cant't be zero!";
    auto gGroup = nQ / nKv;
    int gTile = tileConfig.gTile; // 128
    ASSERT(ERROR_CODE_UNDEFINED, blockSize != 0) << "blockSize can't be zero!";
    ...
    ASSERT(ERROR_CODE_UNDEFINED, bTile != 0) << "bTile can't be zero!";
    ASSERT(ERROR_CODE_UNDEFINED, nQ != 0) << "nQ can't be zero!";
    SymbolicScalar bLoop = bSize / bTile;
    SymbolicScalar s1Size = qNope.GetShape()[0] / bSize / nQ; // [B_s1_N1, D]
    SymbolicScalar s1Tile = 1;
    ASSERT(ERROR_CODE_UNDEFINED, s1Tile != 0) << "s1Tile can't be zero!";
    SymbolicScalar s1Loop = s1Size / s1Tile;
    SymbolicScalar n2Tile = 1;
    ASSERT(ERROR_CODE_UNDEFINED, n2Tile != 0) << "n2Tile can't be zero!";
    SymbolicScalar n2Loop = nKv / n2Tile;
    ASSERT(ERROR_CODE_UNDEFINED, gTile != 0) << "gTile can't be zero!";
