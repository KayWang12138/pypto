        ...
        LOOP("LOOP_L1_s1_SA", FunctionType::DYNAMIC_LOOP, s1Idx, LoopRange(0, s1Sym, 1)) {
            LOOP("LOOP_L2_n2_SA", FunctionType::DYNAMIC_LOOP, n2Idx, LoopRange(0, n2Sym, 1)) {      // GQA场景
                LOOP("LOOP_L3_g_SA", FunctionType::DYNAMIC_LOOP, gIdx, LoopRange(0, gLoopSym, 1)) { // slc_attn
                    ...
                    LOOP("LOOP_L4_s2_SA", FunctionType::DYNAMIC_LOOP, s2Idx, LoopRange(0, 1, 1),
                        PowersOf2(1)) { // 非Flash
                        ...
                        for (int topKIdx = 0; topKIdx < topk; topKIdx++) {
                            ...
                            assembeItems.emplace_back(AssembleItem{
                                kvSlcBlock_fp16, std::vector<SymbolicScalar>{slcOutSOffset, 0}
                            });
                            assembeItems.emplace_back(AssembleItem{
                                krSlcBlock_fp16, std::vector<SymbolicScalar>{slcOutSOffset, dN}
                            });
                        }
...
void SelectedAttention(Tensor &topKIndcies, Tensor &kvNopeCache, Tensor &kRopeCache, Tensor &kvActSeqs,
    ...
    FUNCTION("SA_MAIN", {topKIndcies, kvNopeCache, kRopeCache, kvActSeqs, blockTable, qNope, qRope}, {attentionOut}) {
        SelectedAttentionCompute(topKIndcies, kvNopeCache, kRopeCache, kvActSeqs, blockTable, qNope, qRope,
            attentionOut, nQ, nKv, softmaxScale, front, near, topk, blockSize, cmpBlockSize, slcBlockSize,
            saTileConfig);
    }
}
