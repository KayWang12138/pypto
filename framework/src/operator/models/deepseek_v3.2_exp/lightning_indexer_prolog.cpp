...
Tensor LayerNorm(const Tensor &x, const Tensor &weight, const Tensor &bias, const int dim) {
    ASSERT(ERROR_CODE_UNKNOWN, dim == (int)(x.GetShape().size() - 1) || dim == -1) << "We only support LayerNorm for the last dimension";
    ASSERT(ERROR_CODE_UNKNOWN, x.GetStorage()->Datatype() == DT_FP32);
...
Tensor RotateHalfValidShape(const Tensor &input) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, shapeSize >= 1) << "rope rotate_half input dim less than 1";
    ASSERT(ERROR_CODE_UNKNOWN, shape[shapeSize - 1] % NUM2 == 0) << "rope rotate_half last dim shape is even.";
...
Tensor Rope3D(const Tensor &x, const Tensor &cos, const Tensor &sin, const RopeTileShapeConfig &tileConfig) {
    (void)tileConfig;
    ASSERT(ERROR_CODE_UNKNOWN, x.GetShape().size() == SHAPE_DIM3 && cos.GetShape().size() == SHAPE_DIM2 &&
...
Tensor Rope(const Tensor &x, const Tensor &cos, const Tensor &sin, const RopeTileShapeConfig &tileConfig) {
    (void)tileConfig;
    ASSERT(ERROR_CODE_UNKNOWN, x.GetShape().size() == SHAPE_DIM2 && cos.GetShape().size() == SHAPE_DIM2 &&
           sin.GetShape().size() == SHAPE_DIM2);
