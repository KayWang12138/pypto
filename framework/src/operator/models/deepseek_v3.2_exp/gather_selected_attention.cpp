...
void SelectedAttentionComputeV2(const Tensor &qNope, const Tensor &qRope, const Tensor &kNope2D, const Tensor &kRope2D,
    const Tensor &kNopeScales, const Tensor &topKIndcies, const Tensor &blockTable, const Tensor &kvSlcActSeqs,
    const int nQ, const int nKv, const float softmaxScale, const int topk, const int blockSize,
    const int maxBlockNumPerBatch, Tensor &attentionOut, SaTileShapeConfig tileConfig) {
        ...
        LOOP("LOOP_L1_s1_SA", FunctionType::DYNAMIC_LOOP, s1Idx, LoopRange(0, s1Sym, 1)) {
            ...
            LOOP("LOOP_L2_n2_SA", FunctionType::DYNAMIC_LOOP, n2Idx, LoopRange(0, n2Sym, 1)) { // GQA场景
                LOOP("LOOP_L3_g_SA", FunctionType::DYNAMIC_LOOP, gIdx, LoopRange(0, gLoopSym, 1)) {
                    ...
                    LOOP(
                        "LOOP_L4_s2_SA", FunctionType::DYNAMIC_LOOP, s2Idx, LoopRange(0, bnPerBatch, 1), PowersOf2(1)) {
                        ...
                        auto curTopKIndcies = View(topKIndcies, {1, curS2Tile},
                            {1, std::min(curSeq - s2Idx * curS2Tile, curS2Tile)}, {bS1Idx, s2Idx * curS2Tile});
