...
void WriteHealthReport(const json& report, const std::string &reportPath, const std::string& filename) {
    if (!CreateMultiLevelDir(reportPath)) {
        APASS_LOG_ERROR_F(Elements::Config, "Failed to create directory for health report");
    }
    std::ofstream out(reportPath + "/" + filename);
    if (!out.is_open()) {
        APASS_LOG_ERROR_F(Elements::Config, "Failed to open health report file for writing");
        return;
    }
...
Status CheckTileShapeAIC(Operation *op, std::vector<int> &res)  {
    ...
    if (op->GetIOperands().size() != CUDE_IOPERAND_NUM2 && op->GetIOperands().size() != CUDE_IOPERAND_NUM3) {
        APASS_LOG_WARN_F(Elements::Operation, "Cube operation %d %s ioperands size is %d, should be 2 or 3, CheckTileShapeAIC failed!", op->GetOpMagic(), op->GetOpcodeStr().c_str(), op->GetIOperands().size());
        return FAILED;
    }
