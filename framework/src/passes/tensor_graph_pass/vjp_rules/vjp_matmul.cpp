...
LogicalTensorPtr CreateMatmulOp(VJPContext& ctx, LogicalTensorPtr a, LogicalTensorPtr b,
    ...
    // Keep matmul opcodes consistent with the forward path: use A_MUL_B/A_MUL_BT
    // and encode transpose flags via attributes to avoid unsupported opcodes in codegen.
    Opcode opcode = transB ? Opcode::OP_A_MUL_BT : Opcode::OP_A_MUL_B;
    Operation &op = ctx.function->AddRawOperation(opcode, {a, b}, {output});
    op.SetAttr(OP_ATTR_PREFIX + "trans_a", transA);
    op.SetAttr(OP_ATTR_PREFIX + "trans_b", transB);
    return output;
}
