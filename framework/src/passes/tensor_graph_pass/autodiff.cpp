...
Status AutodiffPass::RunOnFunction(Function& function) {
    ...
    for (const auto& kv : gradInfos) {
        ...
        // Note: We do NOT add gradient tensors to outCasts_ because the slot
        // allocation has already been done before AutodiffPass runs. Adding
        // new outcasts would cause outcastSlot array out-of-bounds access.
        // Instead, gradients are tracked via tensor attributes (gradient_magic)
        // and can be retrieved through the Python API.

        // Explicitly insert gradient tensor into TensorMap for O(1) lookup.
        // The AddRawOperation may not have inserted it if updateTensorMap was false.
