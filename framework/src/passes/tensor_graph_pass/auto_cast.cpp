...
Status AutoCast::ShortenChain(Function& function, const std::vector<Operation*>& castChain, Operation* tailOp)
{
    ...
    bool isTgtOut = (FunctionUtils::GetNodeType(tgtTensor) == NodeType::OUTCAST);
    ...
    for (int i = static_cast<int>(castChain.size()) - 1; i >= 0; i--) {
        ...
        bool isSrcIn = (FunctionUtils::GetNodeType(srcTensor) == NodeType::INCAST);
        bool isSrcOut = (FunctionUtils::GetNodeType(srcTensor) == NodeType::OUTCAST);
