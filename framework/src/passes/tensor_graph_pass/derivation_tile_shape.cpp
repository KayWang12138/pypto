...
static bool CanNotAlignShape(const std::vector<int64_t> &inShape, const std::vector<int64_t> &outShape, int64_t inSize, int64_t outSize) {
    if ((inSize == 0) || (outSize == 0)) {
        APASS_LOG_ERROR_F("DerivationTileShape", "Operation",
...
bool CheckMemoryCondition(const std::vector<size_t> &indexes, std::vector<ShapeStatus> &shapeStatus) {
...
static Status HandleSplitTileShape(std::vector<ShapeStatus> &inStatus, std::vector<size_t> alignedIndexes, 
    ...
    for (auto i = alignedIndexes.rbegin(); i != alignedIndexes.rend(); ++i) {
        ...
        if (tileShape % alignedShape != 0 && alignedShape % tileShape != 0) {
            APASS_LOG_ERROR_F("DerivationTileShape", "Tensor", "Non-compliance: Tensor Shape:%s, TileShape:%s, AlignedShape:%s",
                GetShapeStr(inStatus).c_str(), GetTileStr(inStatus).c_str(), GetShapeStr(alignedStatus).c_str());
            return FAILED;
        }
        if (tileShape % alignedShape == 0) {
            alignStatus.tileSize = alignedShape;
            tileShape = tileShape / (alignStatus.tileSize);
            continue;
        } else if (alignedShape % tileShape == 0) {
...
static Status DerivationAlignShapeTile(std::vector<ShapeStatus> &inStatus, std::vector<ShapeStatus> &alignedStatus) {
    if (inStatus.size() == 0 || alignedStatus.size() == 0) {
        APASS_LOG_ERROR_F("DerivationTileShape", "Tensor", "The shape status is empty.");
        return FAILED;
    }
    ...
    for (auto &shapeStatus : inStatus) {
        ...
        if (shapeStatus.axisType != AXIS_SPLIT) {
            APASS_LOG_ERROR_F("DerivationTileShape", "Tensor", "The axisType property of inStatus is invalid.");
            return FAILED;
        }
        if (shapeStatus.size % tileShape != 0) {
            APASS_LOG_ERROR_F("DerivationTileShape", "Tensor", "Tensor Shape:%s is not divisible by the tile shape:%s",
                GetShapeStr(inStatus).c_str(), GetTileStr(inStatus).c_str());
            return FAILED;
        }

        /* 拆轴的集合对应的alignedshape状态处理 */
        if (HandleSplitTileShape(inStatus, alignedIndexes, tileShape, alignedStatus) != SUCCESS) {
            return FAILED;
        }

        if (!CheckMemoryCondition(alignedIndexes, alignedStatus)) {
            APASS_LOG_ERROR_F("DerivationTileShape", "Tensor",
...
static Status DerivationOutShapeTileWithAlign(std::vector<ShapeStatus> &alignedStatus,
    ...
    for (auto &alignStatus : alignedStatus) {
        ...
        if (alignStatus.axisType != AXIS_KEEP && alignStatus.axisType != AXIS_MERGE) {
            APASS_LOG_ERROR_F("DerivationTileShape", "Tensor", "The axisType property of alignedStatus is invalid.");
            return FAILED;
        }
...
void TiledReshape(const int dimIdx, const std::vector<int64_t> &inShape, const std::vector<int64_t> &tileShape,
    ...
    std::vector<std::vector<int64_t>> &allActTileShape, std::vector<std::vector<int64_t>> &allActOffset)
{
    if (static_cast<size_t>(dimIdx) == inShape.size()) {
        tileCnt++;
        APASS_LOG_DEBUG_F("DerivationTileShape", "Tensor", "tileCnt:%lld, actOffset%s, actTileShape%s",
...
int64_t TiledReshape(const std::vector<int64_t> &inShape, const std::vector<int64_t> &tileShape,
    std::vector<std::vector<int64_t>> &allActTileShape, std::vector<std::vector<int64_t>> &allActOffset)
{
...
static int64_t calcTileSubDistance(int64_t currSub, const std::vector<int64_t> &currTile, const Stride currStride,
    ...
    APASS_LOG_DEBUG_F("DerivationTileShape", "Tensor", "currSub:%lld, currTileSub%s in tile",
        currSub, GetStr(currTileSub).c_str());
    /* 将tile块上的坐标，转移到整个shape上的坐标，进行坐标的偏移 */
    for (k = 0; k < currTile.size(); k++) {
        currTileSub[k] = currTileSub[k] + actOffset[k];
    }
    APASS_LOG_DEBUG_F("DerivationTileShape", "Tensor", "currSub:%lld, currTileSub%s in tensor",
        currSub, GetStr(currTileSub).c_str());
...
static std::vector<int64_t> GetTileCntShape(const std::vector<int64_t> &shape, const std::vector<int64_t> &tileShape) {
    ...
    for (size_t k = 0; k < shape.size(); ++k) {
        tileCntShape[k] = shape[k] / tileShape[k]; // shape一定大于tileshape
    }

    APASS_LOG_DEBUG_F("DerivationTileShape", "Tensor", "shape:%s, tileShape:%s, tileCntShape:%s",
...
static Status CheckTileShape(const std::vector<int64_t> &inShape, const std::vector<int64_t> &outShape,
    ...
    if (inTileCnt != ouTileCnt || inTileCnt <= 0 || ouTileCnt <= 0) {
        /* 检查tile展开后的个数是否相等 */
        APASS_LOG_WARN_F("DerivationTileShape", "Operation", "inTileCnt:%lld is not same as ouTileCnt:%lld",
            inTileCnt, ouTileCnt);
        return FAILED;
    }
    ...
    APASS_LOG_DEBUG_F("DerivationTileShape", "Operation", "inStride:%s, outStride:%s, inTileCntShape:%s, outTileCntShape:%s",
    ...
    for (int64_t i = 0; i < inTileCnt; ++i) {
        if (!IsVertex(inTileCntShape, i) && !IsVertex(outTileCntShape, i)) {
            APASS_LOG_DEBUG_F("DerivationTileShape", "Operation", "inTileCntShape:%s, outTileCntShape:%s, i=%lld",
        ...
        if (inTileSize != outTileSize) {
            APASS_LOG_WARN_F("DerivationTileShape", "Tensor", "tileCnt:%lld, inTileSize:%lld is not equal to outTileSize:%lld",
        ...
        for (int64_t j = 0; j < inTileSize; j++) {
            ...
            if (inDistance != outDistance) {
                APASS_LOG_WARN_F("DerivationTileShape", "Tensor", "Cnt:%lld %lld, inDistance:%lld is not same as outDistance:%lld",
...
Status DerivationTileShape::DerivationReshapeTileShape(Operation *op, const Shape &inShape, const Shape &outShape,
    ...
    if (DerivationAlignShape(inShape, outShape, inStatus, alignedShape, alignedStatus) != SUCCESS) {
        APASS_LOG_ERROR_F("DerivationTileShape", "Operation", "Op: %d derivation alignedShape failed",
            op->GetOpMagic());
        return FAILED;
    }
    ...
    if (DerivationAlignShapeTile(inStatus, alignedStatus) != SUCCESS) {
        APASS_LOG_ERROR_F("DerivationTileShape", "Operation", "Op: %d derivation aligned tileshape failed",
            op->GetOpMagic());
        return FAILED;
    }
    ...
    if (DerivationOutShapeTileWithAlign(alignedStatus, outShape, newTileShape) != SUCCESS) {
        APASS_LOG_ERROR_F("DerivationTileShape", "Operation", "Op: %d derivation out tileshape with align failed",
            op->GetOpMagic());
        return FAILED;
    }
    APASS_LOG_EVENT_F("DerivationTileShape", "Operation", "Op: %d, inShape%s, alignShape%s, outShape%s, inTile%s, alignTile%s, outTile%s",
    ...
    if (CheckTileShape(inShape, outShape, inTileShape, newTileShape) != SUCCESS) {
        APASS_LOG_ERROR_F("DerivationTileShape", "Operation", "Op: %d check tileshape failed", op->GetOpMagic());
        return FAILED;
    }
