...
static bool CanNotAlignShape(
    ...
    if (ret) {
        APASS_LOG_WARN_F(Elements::Operation,
            "Non-segmentable axis, cannot calculate the align of the input%s and the output%s", GetStr(inShape).c_str(),
            GetStr(outShape).c_str());
    }
    return ret;
}
...
static Status HandleInputProductCase(const std::vector<int64_t> &inShape, const std::vector<int64_t> &outShape,
    AlignContext &ctx, std::vector<ShapeStatus> &inStatus, std::vector<int64_t> &alignedShape,
...
Status DerivationTileShape::DerivationReshapeTileShape(Operation *op, const Shape &inShape, const Shape &outShape,
    ...
    if (DerivationAlignShapeTile(inStatus, alignedStatus) != SUCCESS) {
        APASS_LOG_WARN_F(Elements::Operation,
            "Op: %d derivation aligned tileshape failed, inShape%s, outShape%s, inTile%s", op->GetOpMagic(),
    ...
    APASS_LOG_INFO_F(Elements::Operation,
        ...
        GetStr(inShape).c_str(), GetStr(alignedShape).c_str(), GetStr(outShape).c_str(), GetStr(inTileShape).c_str(),
        GetTileStr(alignedStatus).c_str(), GetStr(newTileShape).c_str());
