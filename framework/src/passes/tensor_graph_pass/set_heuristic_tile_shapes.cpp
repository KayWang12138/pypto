    ...
    if(graph_tiles.is_open()){
        for(auto &op: function.Operations()){
            if(op.GetSemanticLabel()){
                std::cout<<"HELLO FROM PASS!"<<std::endl;
                ...
                if(op.GetCoreTypeStr() == "AIC"){
                    ...
                    semanticJson[sem_label]["enableSplitK"] = cubeShape.GetCubeTile().enableSplitK;
                    semanticJson[sem_label]["enableMultiDataLoad"] = cubeShape.GetCubeTile().enableMultiDataLoad;
