...
void SetReduceTiles(std::vector<Operation *> reduceOrderedOperations) {
    ...
    for (auto op : reduceOrderedOperations) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, inputDims == outputDims) << "Input dims should be equal output dims";
        ASSERT(ERROR_CODE_UNDEFINED, outputsNum == 1) << "ReduceOp must have 1 output";
        ...
        for (size_t dim = 0; dim < inputDims; dim++) {
            if ((maxInputShape[dim] != op->GetOOperands()[0]->shape[dim]) && (op->GetOOperands()[0]->shape[dim] == 1) && (reducedDim == -1)) {
                ...
                ASSERT(ERROR_CODE_UNDEFINED, !((maxInputShape[dim] != op->GetOOperands()[0]->shape[dim]) && (op->GetOOperands()[0]->shape[dim] == 1) && (reducedDim != -1))) << "Several reduced dims \n";
            }
        }
        ASSERT(ERROR_CODE_UNDEFINED, reducedDim >= 0) << "Not found reduced dims";
...
void SetHeuristicTileShapes::SetHeuristicTileShapesFunc(Function &function) const {
    ...
    for (auto &op : function.Operations()) {
        ASSERT(ERROR_CODE_UNDEFINED, op.GetTileShape().GetVecTile()[0] != -1) << "Not all tiles was set";
    }
    #endif
