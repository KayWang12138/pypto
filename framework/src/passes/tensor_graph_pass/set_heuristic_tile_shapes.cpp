...
std::tuple<std::array<int64_t, MAX_MDIM>, std::array<int64_t, MAX_KDIM>, std::array<int64_t, MAX_NDIM>> FindAndSetCubeTileShapes(
...
void UnstableDimsCalculation(std::queue<size_t> &unstableDims, Shape opBaseInputShape, Shape opBaseOutputShape)
{
...
void TileThroughGather(Operation *producerOp, Operation *op, std::vector<int64_t>& vectorTilesOld) 
{
    ...
    if (gatherVectorOps.find(op->GetOpcode()) != gatherVectorOps.end()) {
        ...
        if (op->GetIOperands()[0]->magic == producerOp->GetOOperands()[0]->magic) {
            vectorTilesOld[0] = static_cast<int64_t>(std::pow(2, static_cast<int64_t>(std::log2(vectorTilesOldTmp[0]))));
            vectorTilesOld[1] = static_cast<int64_t>(std::pow(2, static_cast<int64_t>(std::log2(vectorTilesOldTmp[1]))));
        }
        
        if ((op->GetIOperands()[1]->magic == producerOp->GetOOperands()[0]->magic) || (op->GetIOperands()[2]->magic == producerOp->GetOOperands()[0]->magic)) { 
            vectorTilesOld[0] = static_cast<int64_t>(std::pow(2, static_cast<int64_t>(std::log2(producerOp->GetOOperands()[0]->shape[0]))));
...
void TransposeTileSetting(Operation *op, const std::vector<int64_t>& vectorTilesOld, std::vector<int64_t>& vectorTilesNew) 
{
    ...
    DataType inputType = op->GetIOperands()[0]->tensor->GetDataType();
    DataType outputType = op->GetOOperands()[0]->tensor->GetDataType();
    ASSERT(inputType == outputType) << "Input & output data types shpuld be the same for Transpose";
...
void ViewTileSetting(Operation *op, const std::vector<int64_t>& vectorTilesOld, std::vector<int64_t>& vectorTilesNew) 
{
    ...
    for (auto sp : changedDims) {
        ...
        ASSERT((size_t)sp < vectorTilesNew.size()) << "Split dim > vectorTiles.size(): " << sp << " vs " << vectorTilesNew.size();
...
void GatherTilesForFirstInput(std::vector<int64_t> &vectorTilesNew, std::vector<int64_t> inputShape, size_t outputDims) 
{
    if (outputDims == 2) {
        vectorTilesNew[1] = static_cast<int64_t>(std::pow(2, static_cast<int64_t>(std::log2(inputShape[1]))));
    } else if (outputDims == 3) {
        vectorTilesNew[2] = static_cast<int64_t>(std::pow(2, static_cast<int64_t>(std::log2(inputShape[1]))));
    } else {
        APASS_LOG_ERROR_F(Elements::Operation, "outputDims != 2 or 3, need to check this case");
    }
}

void GatherTilesForSecondInput(std::vector<int64_t> &vectorTilesNew, std::vector<int64_t> inputShape, size_t outputDims) 
{
    if (outputDims == 2) {
        vectorTilesNew[0] = static_cast<int64_t>(std::pow(2, static_cast<int64_t>(std::log2(inputShape[0]))));
    } else if (outputDims == 3) {
        vectorTilesNew[0] = static_cast<int64_t>(std::pow(2, static_cast<int64_t>(std::log2(inputShape[0]))));
        vectorTilesNew[1] = static_cast<int64_t>(std::pow(2, static_cast<int64_t>(std::log2(inputShape[1]))));
    } else {
...
void GatherFromTiles(Operation *op, std::vector<int64_t> & vectorTilesNew, std::vector<int64_t> inpTile, 
                     size_t outputDims, size_t inp, bool &firstInputHasTile, bool &secondInputHasTile)
{
    if (gatherVectorOps.find(op->GetOpcode()) != gatherVectorOps.end()) {
...
void GatherFromShapes(Operation *op, std::vector<int64_t> & vectorTilesNew, size_t outputDims, bool firstInputHasTile, bool secondInputHasTile)
{
    if (gatherVectorOps.find(op->GetOpcode()) != gatherVectorOps.end()) {
...
void GatherTileSetting(Operation *op, std::vector<int64_t>& vectorTilesNew) 
{
    ...
    uint32_t argsCount = inputsNum + outputsNum + 1;
    ...
    for (size_t inp = 0; inp < op->GetIOperands().size(); inp++) {
        ...
        // Evaluate new tile according to all existing tiles
        GatherFromTiles(op, vectorTilesNew, inpTile, outputDims, inp, firstInputHasTile, secondInputHasTile);
    }
    GatherFromShapes(op, vectorTilesNew, outputDims, firstInputHasTile, secondInputHasTile);
...
void TopkTileSetting(Operation *op, std::vector<int64_t>& vectorTilesNew) 
{
    ...
    int64_t maxTypeSize = std::max(inputTypeSize, outputTypeSize);
    const uint64_t UB_MAX_SIZE = Platform::Instance().GetDie().GetMemoryLimit(MemoryType::MEM_UB);
    uint32_t argsCount = 3 + 4; // input + 2 outputs + temporal buffer (4 inputs)
    ASSERT(inputDims == outputDims) << "Reduce have different shape size for input and output";
    ASSERT(maxTypeSize != 0);
    ASSERT(argsCount != 0);
...
void BackPropagate(Operation *producerOp, Operation *op) 
{
    ...
    if (gatherVectorOps.find(op->GetOpcode()) == gatherVectorOps.end() && gatherMoveOps.find(op->GetOpcode()) == gatherMoveOps.end()) {
    ...
    if (!(gatherVectorOps.find(producerOp->GetOpcode()) == gatherVectorOps.end() && gatherMoveOps.find(producerOp->GetOpcode()) == gatherMoveOps.end())) {
        DataType inputType = producerOp->GetIOperands()[0]->tensor->GetDataType();
        DataType outputType = producerOp->GetOOperands()[0]->tensor->GetDataType();            
        int64_t inputTypeSize = BytesOf(inputType);  
        int64_t outputTypeSize = BytesOf(outputType); 
        int64_t maxTypeSize = std::max(inputTypeSize, outputTypeSize);  
        size_t outputsNum = producerOp->GetOOperands().size();  
        uint32_t argsCount = inputsNum + outputsNum + 1; 
        AdjustTileToUB(argsCount, maxTypeSize, inputTypeSize, vectorTilesNew);        
    }
...
void SetHeuristicCubeTiles(Function& function, std::unordered_set<Operation*> cubeOperations)
{
    ...
    int64_t cubeL1Reuse = (function.paramConfigs_.cubeL1ReuseSetting.size() == 1 &&
                           ...
                           function.paramConfigs_.cubeL1ReuseSetting.begin()->second : 1;
    int64_t cubeNBuffer = (function.paramConfigs_.cubeNBufferSetting.size() == 1 &&
                           ...
                           function.paramConfigs_.cubeNBufferSetting.begin()->second : 1;
    ...
    for (auto& op : function.Operations()) {
        if (op.ConsumerOps().size() == 0) {
            ...
            if (op.GetTileShape().GetVecTile()[0] == -1) {
                ...
                curTile = (curTile == 0) ? 2048 : curTile;
                ASSERT(curTile != 0);
...
void SetHeuristicVectorTiles(Function &function, std::unordered_set<Operation *> cubeOperations) 
{
    ...
    for (auto pOps: priorOps) {
        ...
        APASS_LOG_ERROR_F(Elements::Operation, "              backward of pass is %ld us.", duration.count());
        ...
        APASS_LOG_ERROR_F(Elements::Operation, "              forward of pass is %ld us.", duration.count());
    }
    std::vector<int64_t> vectorTilesNew;
...
void PrintTiles(Function& function, std::string fileName) 
{
    ...
    if (file.is_open()) {
        ...
        for (auto &op : function.Operations()) {
            if (op.GetCoreTypeStr() == "AIC") {
                file << "!Cube Operation " << opIdx << " : " << op.GetOpcodeStr() << " magic : " << op.GetOpMagic() << std::endl;
                file << op.GetTileShape().ToString(TileType::CUBE) << std::endl;
            } else if (op.GetCoreTypeStr() == "AIV") {
                file << "!Vector Operation " << opIdx << " : " << op.GetOpcodeStr() << " magic : " << op.GetOpMagic() << std::endl;
                file << op.GetTileShape().ToString(TileType::VEC) << std::endl;
            } else {
                file << "!Other Operation " << opIdx << " : " << op.GetOpcodeStr() << " magic : " << op.GetOpMagic() << std::endl;
                file << op.GetTileShape().ToString(TileType::VEC) << std::endl;
            }
            for (size_t it = 0; it < op.GetIOperands().size(); it++) {
                file << "Input " << it << " Magic = " << op.GetIOperands()[it]->magic << " DataType = " << BytesOf(op.GetIOperands()[it]->tensor->GetDataType()) << " Shape = ["; 
                auto InputShape = op.GetIOperands()[it]->shape;
                for (size_t j = 0; j < InputShape.size(); j++) {
                    file << InputShape[j] << " ";
                }
            ...
            for (size_t it = 0; it < op.GetOOperands().size(); it++) {
                file << "Output " << it << " Magic = " << op.GetOOperands()[it]->magic << " DataType = " << BytesOf(op.GetOOperands()[it]->tensor->GetDataType()) << " Shape = ["; 
                auto OutputShape = op.GetOOperands()[it]->shape;
                for (size_t j = 0; j < OutputShape.size(); j++) {
                    file << OutputShape[j] << " ";
                }
...
{
    (void)function;
    std::ofstream coutFile("set_heuristic_output.txt", std::ofstream::app);
