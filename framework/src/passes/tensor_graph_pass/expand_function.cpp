...
Status ExpandFunction::ExpandOperation(Function &function, Operation &op) const{
    ...
    if (info.scopeId >= 0) { // scopeIdx < 0 means no need to merge
        ...
        if (!GraphUtils::IsCVMixPlatform() && scopeMap_[info.scopeId].find(CoreType::AIC) != scopeMap_[info.scopeId].end() && scopeMap_[info.scopeId].find(CoreType::AIV) != scopeMap_[info.scopeId].end()) {
            APASS_LOG_ERROR_F(Elements::Function, "Cannot mix cube and vector op on a CV seperate platform in function: %s, please check your setting: sg_set_scope=%d", function.GetRawName().c_str(), info.scopeId);
            return FAILED;
        }
    }
    std::vector<int> scopeVec = {
        static_cast<int>(info.scopeId),
        static_cast<int>(info.allowParallelMerge),
        static_cast<int>(info.allowCrossScopeMerge),
        static_cast<int>(info.mixId)
    };
    ...
    config::SetPassOption(SG_SET_SCOPE, std::vector<int>{-1, 0, 0, -1});
    return SUCCESS;
}
