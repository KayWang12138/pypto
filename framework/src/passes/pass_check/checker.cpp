...
Status Checker::CheckCompleteness(Function &function) {
    ...
    for (const auto &incast : function.GetIncast()) {
        ...
        if (incast->GetConsumers().empty()) {
            APASS_LOG_WARN_F(Elements::Tensor, "The incast[%d] has no consumer.", incast->GetMagic());
            continue;
        }
    }
    ...
    for (const auto &outcast : function.GetOutcast()) {
        ...
        if (outcast->GetProducers().empty()) {
            APASS_LOG_WARN_F(Elements::Tensor, "The outcast[%d] has no producer.", outcast->GetMagic());
            continue;
        }
    }
    return SUCCESS;
...
Status Checker::CheckGraphLoop(Function &function) {
    if (function.GetTotalSubGraphCount() == 0 && !function.OperationLoopCheck()) {
        APASS_LOG_ERROR_F(Elements::Function, "OperationLoopCheck failed, there is a loop in function[%d].", function.GetFuncMagic());
        return FAILED;
    }
