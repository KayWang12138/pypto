...
Status RemoveRedundantOpChecker::ProcessPreCheck(Function &function, const Operation &op) {
    if (op.GetOpcode() == Opcode::OP_ASSEMBLE) {
        ...
        if (PreCheckAssemble(op, assemble_in) != SUCCESS) {
            APASS_LOG_ERROR_F(Elements::Operation, "PreCheck for assemble op[%d] failed.", op.GetOpMagic());
            return FAILED;
        }
        return SUCCESS;
    }
    if (op.GetOpcode() == Opcode::OP_VIEW) {
        ...
        if (PreCheckView(function, op, view_in) != SUCCESS) {
            APASS_LOG_ERROR_F(Elements::Operation, "PreCheck for view op[%d] failed.", op.GetOpMagic());
            return FAILED;
        }  
    }
    return SUCCESS;
...
Status RemoveRedundantOpChecker::PostCheckView(const Operation &op) {
    ...
    if (childOp->GetOpcode() == Opcode::OP_COMM_WAIT_FLAG) {
        APASS_LOG_ERROR_F(Elements::Operation, "View op[%d] has only one commit wait child; Please check view op[%d].", 
        op.GetOpMagic(), op.GetOpMagic());
        return FAILED;
    }
    return SUCCESS;
...
Status RemoveRedundantOpChecker::ProcessPostCheck(const Operation &op) {
    if (op.GetOpcode() == Opcode::OP_ASSEMBLE) {
        if (PostCheckAssemble(op) != SUCCESS) {
            APASS_LOG_ERROR_F(Elements::Operation, "PostCheck for Assemble failed.");
            return FAILED;
        }
        return SUCCESS;
    }
    if (op.GetOpcode() == Opcode::OP_VIEW) {
        if (PostCheckView(op) != SUCCESS) {
            APASS_LOG_ERROR_F(Elements::Operation, "PostCheck for View failed.");
            return FAILED;
        }
        return SUCCESS;
    }
    if (op.GetOpcode() == Opcode::OP_REGISTER_COPY) {
        if (PostCheckRegCopy(op) != SUCCESS) {
            APASS_LOG_ERROR_F(Elements::Operation, "PostCheck for RegCopy failed.");
            return FAILED;
        }
        return SUCCESS;
    }
    if (op.GetOpcode() == Opcode::OP_COPY_IN) {
        if (PostCheckCopyIn(op) != SUCCESS) {
            APASS_LOG_ERROR_F(Elements::Operation, "PostCheck for CopyIn failed.");
            return FAILED;
        }
        return SUCCESS;
    }
    if (op.GetOpcode() == Opcode::OP_EXPAND) {
        if (PostCheckExpand(op) != SUCCESS) {
            APASS_LOG_ERROR_F(Elements::Operation, "PostCheck for Expand failed.");
            return FAILED;
        }
    }
    return SUCCESS;
...
Status RemoveRedundantOpChecker::DoPreCheck(Function &function) {
    ...
    if (CheckValidOp(function) != SUCCESS) {
        APASS_LOG_ERROR_F(Elements::Operation, "Found invalid op in the function.");
        return FAILED;
    }
    if (CheckOpIOValid(function) != SUCCESS) {
        APASS_LOG_ERROR_F(Elements::Operation, "Found invalid input/output from the function.");
        return FAILED;
    }
    for (auto &op : function.Operations()) {
        if (ProcessPreCheck(function, op) != SUCCESS) {
            APASS_LOG_ERROR_F(Elements::Operation, "PreCheck for RemoveRedundantOp failed.");
            return FAILED;
        }
    }
    return SUCCESS;
...
Status RemoveRedundantOpChecker::DoPostCheck(Function &function) {
    ...
    if (CheckOpIOValid(function) != SUCCESS) {
        APASS_LOG_ERROR_F(Elements::Operation, "Found invalid input/output in the function.");
        return FAILED;
    }
    for (auto &op : function.Operations()) {
        if (ProcessPostCheck(op) != SUCCESS) {
            APASS_LOG_ERROR_F(Elements::Operation, "PostCheck for RemoveRedundantOp failed.");
            return FAILED;
        }
    }
    return SUCCESS;
