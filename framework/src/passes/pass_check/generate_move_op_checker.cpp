    ...
    for (auto &operation : operations) {
        ...
        if(op == Opcode::OP_ASSEMBLE || op == Opcode::OP_VIEW) {
            ...
            if(inputMemType != outputMemType) {
                APASS_LOG_ERROR_F(Elements::Operation, "Operation validation failed: Operation %s[%d] has dismatched memory type. Input memory type:%s. Output memory type:%s",
                    operation.GetOpcode(),operation.GetOpMagic(),
