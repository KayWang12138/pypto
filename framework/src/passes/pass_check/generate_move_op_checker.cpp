...
Status GenerateMoveOpChecker::DoPostCheck(Function &function) {
    ...
    for (auto &operation : operations) {
        ...
        if(op == Opcode::OP_DUPLICATE || op == Opcode::OP_CONVERT) {
            APASS_LOG_ERROR_F(Elements::Operation, "Operation validation failed: Operation %s[%d] is invalid here.",operation.GetOpcode(),operation.GetOpMagic());
            return FAILED;
        }
        if(op == Opcode::OP_ASSEMBLE || op == Opcode::OP_VIEW) {
            if(operation.GetIOperands().size() != 1) {
                APASS_LOG_ERROR_F(Elements::Operation, "Operation validation failed: Operation %s[%d] has more than one input.",operation.GetOpcode(),operation.GetOpMagic());
                return FAILED;
            }
            if(operation.GetOOperands().size() != 1) {
                APASS_LOG_ERROR_F(Elements::Operation, "Operation validation failed: Operation %s[%d] has more than one output.",operation.GetOpcode(),operation.GetOpMagic());
                return FAILED;
            }
            ...
            if(inputMemType != outputMemType) {
                APASS_LOG_ERROR_F(Elements::Operation, "Operation validation failed: Operation %s[%d] has dismatched memory type. Input memory type:%s. Output memory type:%s",operation.GetOpcode(),operation.GetOpMagic(),
