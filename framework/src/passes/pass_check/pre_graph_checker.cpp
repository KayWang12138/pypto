...
Status PreGraphProcessChecker::DoPreCheck(Function& function)
{
    ...
    for (auto& op : function.Operations()) {
        ...
        if (op.GetSubgraphID() == NOT_IN_SUBGRAPH) {
            APASS_LOG_ERROR_C(
                GraphErr::GRAPH_SUBGRAPH_ID_INVALID, Elements::Graph, "%s[%d] is not partitioned. %s",
        ...
        if ((op.GetIOperands().size() != 1) || (op.GetOOperands().size() != 1)) {
            ...
            APASS_LOG_ERROR_C(
                ...
                "Invalid %s[%d], input num: %zu, output num: %zu .%s", op.GetOpcodeStr().c_str(), op.opmagic,
                op.GetIOperands().size(), op.GetOOperands().size(), GetFormatBacktrace(op).c_str());
            return FAILED;
        }
