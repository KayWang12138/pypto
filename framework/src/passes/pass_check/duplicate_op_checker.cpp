    ...
    for (const auto &oOperand : op.GetOOperands()) {
        ...
        for (auto &consumer : consumers) {
            ...
            if (consumer->GetOpcode() == Opcode::OP_GATHER_IN_L1) {
                APASS_LOG_ERROR_F(Elements::Tensor, "OP_GATHER_IN_L1[%d]'s consumer cannot be OP_GATHER_IN_L1.",
                    oOperand->GetMagic());
                return FAILED;
            }
        }
...
Status DuplicateOpChecker::PostCheckView(const Operation &op) {
    ...
    for (const auto &oOperand : op.oOperand) {
        ...
        if (consumerNum == kNumTwo) {
            APASS_LOG_ERROR_F(Elements::Operation,
                "There can not be more than one non-view node among its consumers for op[%d].%s", op.opmagic,
                GetFormatBacktrace(op).c_str());
            return FAILED;
        }
    }
