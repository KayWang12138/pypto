...
Status checkAssemble(const std::unordered_map<LogicalTensorPtr, int64_t> &tensorMap,
    ...
    for (auto [logicTensor, rawId] : tensorMap) {
        ...
        } else if (rawIdToRawOffset[rawId] != rawOffset) {
            APASS_LOG_ERROR_F(Elements::Tensor, "RawTensor(%d) is overlap.", rawId);
            return FAILED;
        }
    }
    for (auto &[id, shape] : rawTensorSize) {
        if (shape != 0) {
            APASS_LOG_ERROR_F(Elements::Tensor, "RawTensor(%d) is not fully covered.", id);
            return FAILED;
        }
    }
    return SUCCESS;
    ...
    for (const auto &tMap : tensorMap) {
        for (const auto &logicalTensor : tMap.second) {
            if (checkTensor(logicalTensor) != SUCCESS) {
                APASS_LOG_ERROR_F(Elements::Function, "checkTensor Failed.");
                return FAILED;
            } 
        }
    }
