...
Status IntraSubgraphAdapterChecker::PostSubgraphCheck(const std::vector<std::vector<Operation *>> &subgraphs) {
    for (const auto &subgraph : subgraphs) {
        ...
        if (aicCount > 0 && aivCount > 0) {
            APASS_LOG_ERROR_F(Elements::Operation, "Subgraph %d has both AIV(%d) and AIC(%d) operation.", subgraphId,
                aivCount, aicCount);
            return FAILED;
        }
        if (aicMemoryCount > 0 && aivMemoryCount > 0) {
            APASS_LOG_ERROR_F(Elements::Operation, "Subgraph %d has both ub(%d) and l0/l1(%d) memory type tensor.",
