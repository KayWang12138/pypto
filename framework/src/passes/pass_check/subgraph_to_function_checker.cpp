...
Status SubGraphToFuncChecker::NOPCheck(const Operation &op) const {
    ...
    if (op.GetIOperands().size() > 0) {
        APASS_LOG_ERROR_F(Elements::Tensor, "NOP[%d] has IOperands size %zu. %s", op.GetOpMagic(),
            op.GetIOperands().size(), GetFormatBacktrace(op).c_str());
        return FAILED;
    }
    if (op.GetInCtrlOperations().size() > 0) {
        APASS_LOG_ERROR_F(Elements::Operation, "NOP[%d] has InCtrlOperations size %zu. %s", op.GetOpMagic(),
            op.GetInCtrlOperations().size(), GetFormatBacktrace(op).c_str());
        return FAILED;
    }
    if (op.GetOOperands().size() > 0) {
        APASS_LOG_ERROR_F(Elements::Tensor, "NOP[%d] has OOperands size %zu. %s", op.GetOpMagic(),
            op.GetOOperands().size(), GetFormatBacktrace(op).c_str());
        return FAILED;
    }
    if (op.GetOutCtrlOperations().size() > 0) {
        APASS_LOG_ERROR_F(Elements::Operation, "NOP[%d] has OutCtrlOperations size %zu. %s", op.GetOpMagic(),
...
Status SubGraphToFuncChecker::CheckSubGraphTopo(Function &function) const {
    ...
    for (size_t i = 0; i < operations.size(); i++) {
        ...
        if (subGraphId < 0 && NOPCheck(op) != SUCCESS) {
            APASS_LOG_ERROR_F(Elements::Operation, "operation %zu has negative subGraphID %d and failed NOP check. %s",
                i, subGraphId, GetFormatBacktrace(op).c_str());
            return FAILED;
        }
...
Status SubGraphToFuncChecker::BuildOutGraph(Function &function) {
    ...
    for (size_t i = 0; i < operationViewer.size(); i++) {
        ...
        for (const auto &outControlOp : operationViewer[i].GetOutCtrlOperations()) {
            ...
            if (EdgeIndexCheck(found, childSeqNo, inGraph_.size()) != SUCCESS) {
                APASS_LOG_ERROR_F(Elements::Operation, "Error inserting op magic %d in function %d %s to outGraph_. %s",
    ...
    for (size_t i = 0; i < inEdgeGraph.size(); i++) {
        for (size_t j = 0; j < inEdgeGraph[i].size(); j++) {
            ...
            if (nodeColIdx[parentSeqNo] >= outEdgeGraph[parentSeqNo].size()) {
                APASS_LOG_ERROR_F(Elements::Graph, "node %zu, %zu th parentSeqNo %zu exceeds outgraph[%zu] size %zu", i,
                    j, parentSeqNo, parentSeqNo, outEdgeGraph[parentSeqNo].size());
                return FAILED;
            }
            // inEdgeGraph和outEdgeGraph都是按顺序排列的
            if (static_cast<size_t>(outEdgeGraph[parentSeqNo][nodeColIdx[parentSeqNo]++]) != i) {
                APASS_LOG_ERROR_F(Elements::Graph, "node %zu, %zu th parentSeqNo %zu is not found in outgraph[%zu]", i,
                    j, parentSeqNo, parentSeqNo);
                return FAILED;
            }
        }
    }
...
Status SubGraphToFuncChecker::VerifyRedundantEdge(const int srcNode, const int dstNode) const {
    ...
    APASS_LOG_ERROR_F(Elements::Graph,
        "source node %d and destination node %d are not related in colorOutGraph_ within three jumps", srcNode,
        dstNode);
    return FAILED;
}
