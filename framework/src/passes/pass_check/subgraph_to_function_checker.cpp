...
Status SubGraphToFuncChecker::CheckSubGraphBoundary(Function& function)
{
    ...
    for (size_t i = 0; i < operations.size(); i++) {
        ...
        for (size_t k = 0; k < op.iOperand.size(); k++) {
            ...
            if (iOperand->GetMemoryTypeOriginal() == MemoryType::MEM_DEVICE_DDR && !IsSubGraphBoundary(iOperand) &&
            ...
            if (subGraphId != iOperand->subGraphID && !IsSubGraphBoundary(iOperand)) {
                APASS_LOG_ERROR_C(
                    TensorErr::TENSOR_SUBGRAPH_BOUNDARY, Elements::Tensor,
            ...
            if (IsCopyIn(op.GetOpcode()) && !IsSubGraphBoundary(iOperand)) {
                APASS_LOG_ERROR_C(
                    TensorErr::TENSOR_SUBGRAPH_BOUNDARY, Elements::Tensor,
        ...
        for (size_t k = 0; k < op.oOperand.size(); k++) {
            ...
            bool hasOnlyViewProducers = HasOnlyViewProducers(producers);
            // Rule 1: Operands from DDR memory must be marked as subgraph boundary
            if (oOperand->GetMemoryTypeOriginal() == MemoryType::MEM_DEVICE_DDR && !IsSubGraphBoundary(oOperand) &&
                !hasOnlyViewProducers) {
                APASS_LOG_ERROR_C(
                    TensorErr::TENSOR_SUBGRAPH_BOUNDARY, Elements::Tensor,
            ...
            if (subGraphId != oOperand->subGraphID && !IsSubGraphBoundary(oOperand)) {
                APASS_LOG_ERROR_C(
                    TensorErr::TENSOR_SUBGRAPH_BOUNDARY, Elements::Tensor,
            ...
            if (IsCopyOut(op.GetOpcode()) && !IsSubGraphBoundary(oOperand)) {
                APASS_LOG_ERROR_C(
                    TensorErr::TENSOR_SUBGRAPH_BOUNDARY, Elements::Tensor,
