...
void ParallelTool::parallel(int start, int end, int step, std::function<void(int, int, int)> body) {
    parallel(start,end,step,numThread,body);
}

void ParallelTool::parallel(int start, int end, int step, int numWork, std::function<void(int, int, int)> body) { 
    if (numThread <= 1){
        body(start, end, 0);
        return;
    }
