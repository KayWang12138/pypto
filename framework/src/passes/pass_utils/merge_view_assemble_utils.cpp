...
Status MergeViewAssembleUtils::Process(Function &function) {
    ...
    if (status != SUCCESS) {   
        ALOG_ERROR_F("MergeViewAssembleUtils initialization failed.");
        return status;
    }
    status = ProcessOperations(function);
    if (status != SUCCESS) {   
        ALOG_ERROR_F("Processing operations failed.");
        return status;
    }
...
Status MergeViewAssembleUtils::ProcessOperations(Function &function) {
    for (auto &op : function.Operations()) {
        ...
        if (processStatus != SUCCESS) {
            ALOG_ERROR_F("ProcessOperations failed.");
            return processStatus;
        }
    }
    Status status = AppendMergedViewOperations(function);
    if (status != SUCCESS) {
        ALOG_ERROR_F("AppendMergedViewOperations phase failed.");
        return status;
    }
    status = AppendMergedAssembleOperations(function);
    if (status != SUCCESS) {
        ALOG_ERROR_F("AppendMergedAssembleOperations phase failed.");
        return FAILED;
    }
    return status;
}
...
Status MergeViewAssembleUtils::AppendMergedViewOperations(Function &function) {
    ...
    for (auto &viewOp : viewOpToAppend_) {
        ...
        if (!attr) {
            ALOG_ERROR_F("Failed to create ViewOpAttribute.");
            return FAILED;
        }
    ...
    for (auto &op : consumers) {
        ...
        if (op->GetOpcode() == Opcode::OP_VIEW) {
            ...
            if (viewOpAttribute == nullptr) {
                ALOG_ERROR_F("View operation has null viewOpAttribute.");
                return FAILED;
            }
    ...
    std::vector<Operation *> &chain)
{
    ...
    if (startOp->iOperand.empty()) {
        ALOG_ERROR_F("First operation in chain has no input operands.");
        return FAILED;
    }
    if (endOp->oOperand.empty()) {
        ALOG_ERROR_F("Last operation in chain has no output operands.");
        return FAILED;
    }
    auto &startTensor = startOp->iOperand.front();
    auto &endTensor = endOp->oOperand.front();
    if (!startTensor) {
        ALOG_ERROR_F("Null input tensor found for first operation in chain.");
        return FAILED;
    }
    if (!endTensor) {
        ALOG_ERROR_F("Null output tensor found for last operation in chain.");
        return FAILED;
    }
...
Status MergeViewAssembleUtils::CalculateMergedOffsets(const std::vector<Operation *> &chain, std::vector<int64_t> &newOffset,
    ...
    for (size_t i = 0; i < chain.size(); ++i) {
        ...
        if (!view) {
            ALOG_ERROR_F("Null view operation in chain.");
            return FAILED;
        }
        auto viewOpAttribute = std::dynamic_pointer_cast<ViewOpAttribute>(view->GetOpAttribute());
        if (!viewOpAttribute) {
            ALOG_ERROR_F("Failed to get ViewOpAttribute.");
            return FAILED;
        }
        if (i == 0) {
    ...
    bool &chainEnd, bool &hasAssembleConsumer)
{
    ...
    for (auto &op : consumers) {
        if (!op) {
            ALOG_ERROR_F("Null consumer operation found.");
            return FAILED;
        }
        if (op->GetOpcode() == Opcode::OP_ASSEMBLE) {
            ...
            if (status != SUCCESS) { 
                ALOG_ERROR_F("Run MergeAssembleChain failed.");
                return status;
            }
            continue;
        }
...
Status MergeViewAssembleUtils::EraseRedundantAssemble(Function &function) const {
    ...
    for (auto &op : function.Operations(false)) {
        ...
        if (op.iOperand.empty()) {
            ALOG_ERROR_F("Assemble operation with no input operands.");
            return FAILED;
        }
