...
{
    // 检查是否为 INCAST
    auto in_it = std::find_if(function.inCasts_.begin(), function.inCasts_.end(), [&tensor](const std::shared_ptr& t) {
    ...
    auto out_it = std::find_if(
        function.outCasts_.begin(), function.outCasts_.end(),
        [&tensor](const std::shared_ptr<LogicalTensor>& t) { return t != nullptr && t.get() == &tensor; });
