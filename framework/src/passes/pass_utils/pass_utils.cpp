...
void SubfuncInvokeInfoTy::ConstructActualInvokeParam(int esgId) {
    if (!isFinalized_) {
        APASS_LOG_ERROR_F(Elements::Config, "Error: does not finalized before constructing InvokeParam");
        return;
    }

    int paramLoc = 0;
    for (auto &tensorArg : tensorArgs_) {
        APASS_LOG_DEBUG_F(Elements::Config, "######## Construct TA for %d ########", esgId);
...
void SubfuncInvokeInfoTy::PrintInvokeInfo(const std::string &extraInfo) const {
    ...
    ss << "])\n";
    APASS_LOG_DEBUG_F(Elements::Tensor, "%s", ss.str().c_str());
}
...
void SubfuncInvokeInfoTy::LoadIncastFromJson(const Json& incastJson, Function* belongTo) {
    ...
    if (tensorPtr == nullptr) {
        APASS_LOG_ERROR_F(Elements::Tensor, "Tile FWK for incast %d op %d is nullptr, function type %d name %s",
...
void SubfuncInvokeInfoTy::LoadOutcastFromJson(const Json& outcastJson, Function* belongTo) {
    ...
    if (tensorPtr == nullptr) {
        APASS_LOG_ERROR_F(Elements::Tensor, "Tile FWK for outcast %d op %d is nullptr function type %d name %s",
...
void SubfuncInvokeInfoTy::LoadTensorFromJson(const Json& tensorJson, Function* belongTo) {
    ...
    if (tensorPtr == nullptr) {
        APASS_LOG_ERROR_F(Elements::Tensor, "Tile FWK for tensor %d op %d is nullptr, function type %d name %s",
