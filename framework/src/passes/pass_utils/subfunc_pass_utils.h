...
public:
    inline void RecordTensorArg(const int operandIdx, const int realDDRId,
        ...
        tensorArgs_.emplace_back(
            operandIdx, realDDRId, offset, shape, rawShape, dtype, isOutputToGM, tensor, opMagic);
    }
    ...
    inline void RecordConnection(const int srcESgId, const int dstESgId, const int operandIndex,
        ...
        const std::vector<int64_t> &rawShape, const DataType dtype, const LogicalTensorPtr &tensor,
        const int opMagic) {
...
public:
    void AppendIncastParam(const int operandIdx, const int symDDRId, const std::vector<int64_t> &shape,
        ...
        inCastArgs_.emplace_back(InCastParamTy(operandIdx, symDDRId, shape, offset, symName, paramLoc, symbol, dataType));
    }
    ...
    void AppendTensorParam(const int operandIdx, const int symDDRId, const std::vector<int64_t> &shape,
        const std::vector<int64_t> &offset, const std::string &symName, const int paramLoc,
        const std::string &symbol, const DataType dataType) {
        tensorsArgs_.emplace_back(TensorParamTy(operandIdx, symDDRId, shape, offset, symName, paramLoc, symbol, dataType));
    }
