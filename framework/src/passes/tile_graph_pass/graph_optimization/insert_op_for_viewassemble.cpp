    ...
    for (auto &assOp : assembleOut->GetProducers()) {
        ...
        if (assembleAttr->GetToDynOffset().size() != viewAttr->GetFromDynOffset().size()) {
            APASS_LOG_INFO_F(Elements::Operation,
                "assOp[%d] GetToDynOffset size is not equal to viewOp[%d] GetFromDynOffset size.", assOp->GetOpMagic(),
                prodOp->GetOpMagic());
            isNeedInsert = true;
            continue;
        }
        ...
        for (size_t i = 0; i < inShape.size(); i++) {
            if (inShape[i] != outShape[i]) {
                APASS_LOG_INFO_F(Elements::Operation,
                    "assOp[%d] GetShape value is not equal to viewOp[%d] GetShape value.", assOp->GetOpMagic(),
                    prodOp->GetOpMagic());
                isNeedInsert = true;
                break;
            }
