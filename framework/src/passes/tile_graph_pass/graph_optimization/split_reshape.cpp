...
Status SplitReshape::RunOnFunction(Function &function) {
    APASS_LOG_INFO_F("SplitReshape", "Operation", "===> Start SplitReshapeOp.");
    if (Init() != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Init failed!");
        return FAILED;
    }
    if (CollectCopyOut(function) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "CollectCopyOut failed!");
        return FAILED;
    }
    if (CheckCopyIn(function) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "CheckCopyIn failed!");
        return FAILED;
    }
    if (AddOperation(function) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "AddOperation failed!");
        return FAILED;
    }
    if (EraseReshape(function) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "EraseReshape failed!");
        return FAILED;
    }
    if (EliminateDeadOperation(function) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "EliminateDeadOperation failed!");
        return FAILED;
    }
    if (SetMemoryType(function) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "SetMemoryType failed!");
        return FAILED;
    }
    APASS_LOG_INFO_F("SplitReshape", "Operation", "===> End SplitReshapeOp.");
    return SUCCESS;
}
    ...
    for (size_t i = 0; i < input.size(); ++i) {
        ...
        while (curVal > 1) {
            if (alignedShape[alignIdx] == 0) {
                APASS_LOG_ERROR_F("SplitReshape", "Tensor", "Incorrect alignedShape, alignedShape[%d] is 0; Please check whether the alignshape generated by the ShapeAlign function is corrent.", alignIdx);
                return FAILED;
            }
...
Status SplitReshape::CheckDynStatus(std::vector<int64_t> alignedShape, std::vector<int64_t> input, std::vector<int64_t> output, std::vector<SymbolicScalar> dynOutput) {
    ...
    if (output.size() != dynOutput.size()) {
        APASS_LOG_ERROR_F("SplitReshape", "Tensor", "The dim of output %zu does not equal to dynOutput %zu; Make sure the dim of output equal to dynOutput.", output.size(), dynOutput.size());
        return FAILED;
    }
    int64_t curVal;
    size_t alignIdx = 0;
    ...
    if (ObtainChangingAxis(alignedShape, input, ChangingAxis) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "ObtainChangingAxis failed.");
        return FAILED;
    }
    for (size_t i = 0; i < output.size(); ++i) {
        if (alignedShape[alignIdx] == output[i]) {
            if (ChangingAxis[alignIdx] && !dynOutput[i].IsImmediate()) {
                APASS_LOG_WARN_F("SplitReshape", "Tensor", "Found undetermined axis from dynOutput[%d], which is also a merged/split axis.", i);
                return WARNING;
            }
            alignIdx++;
            continue;
...
Status SplitReshape::UpdateDynShape(const std::shared_ptr<ReshapeOp> &reshapeOp, const std::vector<int64_t> &offset, const std::vector<SymbolicScalar> &dynShape) {
    ...
    if (curOffset.size() != dynShape.size()) {
        APASS_LOG_ERROR_F("SplitReshape", "Tensor", "The dim of curOffset %s does not equal to dynShape %s.", GetStr(curOffset).c_str(), GetStr(dynShape).c_str());
        return FAILED;
    }
...
Status SplitReshape::GroupReshapeOffset(const std::shared_ptr<ReshapeOp> &isAddReshapeop, const std::vector<int64_t> &offset) {
    ...
    if (curStartIdx.size() != upperleftIdx.size()) {
        APASS_LOG_ERROR_F("SplitReshape", "Tensor", "The dim of curStartIdx (%s, offset %s) does not equal to upperleftIdx; Please check the dim of the curStartIdx and upperleftIdx.", GetStr(curStartIdx).c_str(), GetStr(offset).c_str());
        return FAILED;
    }
    ...
    for (auto &op : function.Operations()) {
        if (op.GetOpcode() == Opcode::OP_RESHAPE) {
            ...
            if (input == nullptr || output == nullptr || output->GetRawTensor() == nullptr) {
                APASS_LOG_ERROR_F("SplitReshape", "Operation", "Invalid reshape op [%d], at least one of input, output or raw tensor of output is nullptr; Please check the input and output of op.", op.opmagic);
                return FAILED;
            }
    ...
    while (i1 < shape1.size() || i2 < shape2.size()) {
        if (prod1 != 1) {
            if (prod1 % shape2[i2] != 0 && shape2[i2] % prod1 != 0) {
                APASS_LOG_WARN_F("SplitReshape", "Tensor", "Non-segmentable axis, cannot apply splitting for current tiling case.");
                return WARNING;
            }
            if (shape2[i2] > prod1) {
        ...
        if ((i1 >= shape1.size() || i2 >= shape2.size())) {
            APASS_LOG_ERROR_F("SplitReshape", "Tensor", "Shape1 index i1(%d) exceeds shape1 size(%d) or shape2 index i2(%d) exceeds shape2 size(%d); Please check the process of compute alignShape.", i1, shape1.size(), i2, shape2.size());
            return FAILED;
        }
        if (shape2[i2] % shape1[i1] != 0 && shape1[i1] % shape2[i2] != 0) {
            APASS_LOG_WARN_F("SplitReshape", "Tensor", "Non-segmentable axis, cannot calculate the align of the input[%s] and the output[%s].", GetStr(shape1).c_str(), GetStr(shape2).c_str());
            return WARNING;
        }
...
Status SplitReshape::UpdateShapeOffset(UpdatePara &para, bool &flag, int &currentShape, int &currentOffset) {
    ...
    if (flag && currentShape <= stride) {
        ...
        if (currentOffset + currentShape > stride) {
            APASS_LOG_WARN_F("SplitReshape", "Tensor", "Found tail cond, currentOffset is %d, currentShape is %d, stride is %d.", currentOffset, currentShape, stride);
            return WARNING;
        }
        return SUCCESS;
    }
    if (currentOffset % stride != 0) {
        APASS_LOG_WARN_F("SplitReshape", "Tensor", "Found tail cond, currentOffset is %d, stride is %d.", currentOffset, stride);
        return WARNING;
    }
    if (currentShape % stride != 0) {
        APASS_LOG_WARN_F("SplitReshape", "Tensor", "Found tail cond, currentShape is %d, stride is %d.", currentShape, stride);
        return WARNING;
    }
    para.OffsetVal = currentOffset / stride;
...
Status SplitReshape::ConstructShapeOffset(const ReshapeTilePara &shapePara, size_t &i, size_t j,
    ...
    while (stride > 1) {
        if (stride % alignedShape[i] != 0) {
            APASS_LOG_WARN_F("SplitReshape", "Tensor", "Cannot cut shape, stride is %d, alignedShape[%zu] is %d.", stride, i, alignedShape[i]);
            return WARNING;
        }
        stride /= alignedShape[i];
        updatePara.stride = stride;
        if (UpdateShapeOffset(updatePara, flag, currentShape, currentOffset) == WARNING) {
            APASS_LOG_WARN_F("SplitReshape", "Tensor", "Found tail condition, cannot align the shape and the offset for current tiling case.");
            return WARNING;
        }
    ...
    for (size_t j = 0UL; j < shape.size(); j++) {
        if (shape[j] == 1) {
            if (tileOffset[j] != 0 || tileShape[j] != 1) {
                APASS_LOG_ERROR_F("SplitReshape", "Tensor", "Shape[%zu] is 1, but tileOffset[%zu] is %d (should be 0) and tileShape[%zu] is %d (should be 1); The tileshape is incorrect.", j, j, tileOffset[j], j, tileShape[j]);
                return FAILED;
            }
            continue;
        }
        if (ConstructShapeOffset(shapePara, i, j, newOffset, newShape) != SUCCESS) {
            APASS_LOG_WARN_F("SplitReshape", "Tensor", "ConstructShapeOffset failed, cannot apply splitting for current tiling case.");
            return WARNING;
        }
    }
    if (i != alignedShape.size()) {
        APASS_LOG_WARN_F("SplitReshape", "Tensor", "Shape index calculated by construct shape offset (%zu) is different from aligned shape size (%zu), cannot apply splitting for current tiling case.", i, alignedShape.size());
        return WARNING;
    }
    return SUCCESS;
}
...
Status SplitReshape::AlignToRaw(
    ...
    for (size_t j = 0UL; j < newRawshape.size(); j++) {
        ...
        while (stride > 1) {
            if (rawShape[i] == 0) {
                APASS_LOG_ERROR_F("SplitReshape", "Tensor", "Incorrect rawShape, rawShape[%d] is 0; Please set the correct rawShape.", i);
                return FAILED;
            }
            if (stride % rawShape[i] != 0) {
                APASS_LOG_ERROR_F("SplitReshape", "Tensor", "Incorrect alignment, rawShape[i] %d should divides %d stride; Please set the correct alignment.", rawShape[i], stride);
                return FAILED;
            }
            stride /= rawShape[i];
...
Status SplitReshape::ObtainCopyOutTile(Function &function, const copyOutTilePara &copyOutTile, LogicalTensors &overlaps, LogicalTensors &newOverlaps) {
    ...
    for (auto &copyOutSource : AssembleOutToInput[reshapeSource->GetRawTensor()->GetRawMagic()]) {
        ...
        if (ret == WARNING) {
            APASS_LOG_WARN_F("SplitReshape", "Operation", "Found RawToAlign warning case, copyOutInfo is %s.", GetStr(CopyOutInfo).c_str());
            return WARNING;
        }
        if (ret == FAILED) {
            APASS_LOG_ERROR_F("SplitReshape", "Operation", "Run RawToAlign failed, copyOutInfo is %s.", GetStr(CopyOutInfo).c_str());
            return FAILED;
        }
...
Status SplitReshape::ObtainReshapeSource(Function &function, const OpPara &para, LogicalTensorPtr &newReshapeSource) {
    ...
    if (newReshapeSource == nullptr) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Failed to make a shared ptr for newReshapeSource.");
        return FAILED;
    }
...
Status SplitReshape::ProcessPerfectlyMatch(Function &function, Operation &op, const PerfectlyMatchPara &para) {
    ...
    if (ObtainReshapeSource(function, opPara, newReshapeSource) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "ObtainReshapeSource failed.");
        return FAILED;
    }
    ...
    if (isAddReshapeOp == nullptr || viewOpAttribute == nullptr) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Failed to make a shared ptr for isAddReshapeOp or found null view attr for op[%d].", op.opmagic);
        return FAILED;
    }
...
Status SplitReshape::ProcessOnetoOne(Function &function, Operation &op, const CalcOverlapPara &para) {
    ...
    if (AlignToRaw(reshapeInfo, reshapeTileOffset, reshapeTileShape) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "AlignToRaw failed, reshape info is %s", GetStr(reshapeInfo).c_str());
        return FAILED;
    }
    if (reshapeTileShape != output->shape) {
        APASS_LOG_ERROR_F("SplitReshape", "Tensor", "Shape of output (%s) is different from reshapeTileShape (%s).", GetStr(output->shape), GetStr(reshapeTileShape));
        return FAILED;
    }
    ...
    if (ProcessPerfectlyMatch(function, op, perfectlyMatchPara) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "ProcessPerfectlyMatch failed.");
        return FAILED;
    }
    return SUCCESS;
}
...
Status SplitReshape::ProcessBeCovered(Function &function, Operation &op, const BeCoveredPara &para) {
    ...
    if (viewOpAttribute == nullptr) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Failed to obtain a shared ptr for viewOpAttribute.");
        return FAILED;
    }
    LogicalTensorPtr newReshapeSource;
...
Status SplitReshape::CalcTileInfo(const CalcOverlapPara &para, std::vector<int64_t> &newShape,
    ...
    if (AlignToRaw(newInfo, newOffset, newShape) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Failed to compute the tile of the raw input, newInfo is %s.", GetStr(newInfo).c_str());
        return FAILED;
    }
    if (newShape != output->shape) {
        APASS_LOG_ERROR_F("SplitReshape", "Tensor", "The new input shape of view[%s] does not equal to output[%s].", GetStr(newShape).c_str(), GetStr(output->shape).c_str());
        return FAILED;
    }
    ...
    if (AlignToRaw(reshapeTileInfo, reshapeTileOffset, reshapeTileShape) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Tensor", "Failed to compute the tile of the raw input of inputView, reshapeTileInfo is %s.", GetStr(reshapeTileInfo).c_str());
        return FAILED;
    }
    return SUCCESS;
}
...
Status SplitReshape::ProcessOnetoMulti(Function &function, Operation &op, const CalcOverlapPara &para) {
    ...
    if (CalcTileInfo(para, newShape, newOffset, reshapeTileShape, reshapeTileOffset)) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Process CalcTileInfo failed for view op[%d].", op.GetOpMagic());
        return FAILED;
    }
    if (reshapeTileOffset.size() == 0 && reshapeTileShape.size() == 0) {
        APASS_LOG_WARN_F("SplitReshape", "Tensor", "One new assemble overlap tile block cannot cover the input of view op[%d].", op.GetOpMagic());
    ...
    if (ProcessBeCovered(function, op, beCoveredPara) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Process ProcessBeCovered failed.");
        return FAILED;
    }
    return SUCCESS;
}
...
Status SplitReshape::ProcessPerfectlyMatchWithAll(Function &function, Operation &op, const PerfectlyMatchWithAllPara &para) {
    ...
    auto isAddReshapeOp = std::make_shared<ReshapeOp>(newReshapeSource, reshapeOutput);
    if (isAddReshapeOp == nullptr || viewOpAttribute == nullptr) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Failed to create isAddReshapeOp or viewOpAttribute; Please make sure newReshapeSource, reshapeOutput are valid and op has attribute.");
        return FAILED;
    }
    auto existOp = ReshapeOperationExist(isAddReshapeOp);
    if (existOp != nullptr) {
...
Status SplitReshape::UpdateForPerfectlyMatchWithAll(Function &function, Operation &op, const CalcOverlapPara &para, const ReshapeSourcePara &sourcePara) {
    ...
    if (newReshapeSource == nullptr) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Failed to make a newReshapeSource ptr.");
        return FAILED;
    }
    newReshapeSource->SetMemoryTypeBoth(reshapeSource->GetMemoryTypeOriginal());
    ...
    if (reshapeOutput == nullptr) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Failed to make a reshapeOutput ptr.");
        return FAILED;
    }
    ...
    if (ProcessPerfectlyMatchWithAll(function, op, perfectlyMatchwithAllPara) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Process ProcessPerfectlyMatchWithAll failed.");
        return FAILED;
    }
    return SUCCESS;
}
...
Status SplitReshape::ProcessMultitoOne(Function &function, Operation &op, const CalcOverlapPara &para) {
    ...
    if (AlignToRaw(newInfo, newReshapeSourceTileOffset, newReshapeSourceTileShape) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Failed to compute the tile of the raw input of reshapeSource, newInfo is %s.", GetStr(newInfo).c_str());
        return FAILED;
    }
    ...
    if (UpdateForPerfectlyMatchWithAll(function, op, para, sourcePara) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Process ProcessMultitoOne failed.");
        return FAILED;
    }
    return SUCCESS;
}
...
Status SplitReshape::UpdateReshapeOp(Function &function, Operation &op, const OverlapStatus &status, const CalcOverlapPara &calcpara) {
    if (status == OverlapStatus::PERFECTLY_MATCH) {
        if (ProcessOnetoOne(function, op, calcpara) != SUCCESS) {
            APASS_LOG_ERROR_F("SplitReshape", "Operation", "Process ProcessOnetoOne of view[%d] failed.", op.GetOpMagic());
            return FAILED;
        }
        return SUCCESS;
    }
    if (status == OverlapStatus::BE_COVERED) {
        if (ProcessOnetoMulti(function, op, calcpara) != SUCCESS) {
            APASS_LOG_ERROR_F("SplitReshape", "Operation", "Process ProcessOnetoMulti of view[%d] failed.", op.GetOpMagic());
            return FAILED;
        }
        return SUCCESS;
    }
    if (status == OverlapStatus::PERFECTLY_MATCH_WITH_ALL) {
        if (ProcessMultitoOne(function, op, calcpara) != SUCCESS) {
            APASS_LOG_ERROR_F("SplitReshape", "Operation", "Process ProcessMultitoOne of view[%d] failed.", op.GetOpMagic());
            return FAILED;
        }
        return SUCCESS;
    }
    APASS_LOG_WARN_F("SplitReshape", "Tensor", "The new input of view[%d] intersects the input of assemble, skip splitreshape.", op.GetOpMagic());
    return SUCCESS;
}
...
Status SplitReshape::CheckValidOp(const CheckParam &para, CheckOutputParam &checkOutputParam) {
    ...
    if (input->shape == output->shape) {
        APASS_LOG_WARN_F("SplitReshape", "Tensor", "The input and output of view op have the same shape, no need to split reshape (if exist) for each view op.");
        return WARNING;
    }
    if (reshapeSources.find(input->GetRawTensor()->GetRawMagic()) == reshapeSources.end()) {
        APASS_LOG_WARN_F("SplitReshape", "Tensor", "View op has no preceding reshape op.");
        return WARNING;
    }
    ...
    if (!CheckSameRawInput(checkOutputParam.reshapeSource)) {
        APASS_LOG_WARN_F("SplitReshape", "Operation", "Found assembling block from different raw tensor.");
        return WARNING;
    }
    ...
    if (dynStatus == WARNING) {
        APASS_LOG_WARN_F("SplitReshape", "Operation", "Undetermined variable in the changing axis, input is %s, output is %s, dynOutput is %s.", GetStr(checkOutputParam.reshapeSource->GetRawTensor()->GetRawShape()).c_str(), GetStr(input->GetRawTensor()->GetRawShape()).c_str(), GetStr(reshapeDynOutput[input->GetRawTensor()->GetRawMagic()]).c_str());
        return WARNING;
    }
    if (dynStatus == FAILED) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Illegal dynamic info for the reshape op, input is %s, output is %s, dynOutput is %s", GetStr(checkOutputParam.reshapeSource->GetRawTensor()->GetRawShape()).c_str(), GetStr(input->GetRawTensor()->GetRawShape()).c_str(), GetStr(reshapeDynOutput[input->GetRawTensor()->GetRawMagic()]).c_str());
        return FAILED;
    }
    ...
    if (alignRet == WARNING) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Cannot process the cond, reshapeInfo is %s.", GetStr(reshapeInfo).c_str());
        return WARNING;
    }
    if (alignRet == FAILED) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Process RawToAlign failed, reshapeInfo is %s.", GetStr(reshapeInfo).c_str());
        return FAILED;
    }
    return SUCCESS;
}
...
Status SplitReshape::CheckOp(Function &function, Operation &op) {
    ...
    if (input == nullptr || output == nullptr || input->GetRawTensor() == nullptr) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Invalid op [%d], at least one of input, output or raw tensor of input is nullptr; Please check the input and output of op.", op.GetOpMagic());
        return FAILED;
    }
    auto viewOpAttribute = dynamic_cast<ViewOpAttribute *>(op.GetOpAttribute().get());
    if (viewOpAttribute == nullptr) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Failed to obtain a view attr for op[%d].", op.opmagic);
        return FAILED;
    }
    ...
    if (checkRet == WARNING) {
        APASS_LOG_WARN_F("SplitReshape", "Operation", "Skip splitreshape for op[%d].", op.GetOpMagic());
        return SUCCESS;
    }
    if (checkRet == FAILED) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Failed to CheckValidOp for op[%d].", op.GetOpMagic());
        return FAILED;
    }
    ...
    if (ret == WARNING) {
        APASS_LOG_WARN_F("SplitReshape", "Operation", "Obtain CopyOutTile failed, skip splitreshape for [%d].", op.GetOpMagic());
        return SUCCESS;
    }
    if (ret == FAILED) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Process ObtainCopyOutTile failed.");
        return FAILED;
    }
    ...
    if (UpdateReshapeOp(function, op, status, calcpara) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Process UpdateReshapeOp failed.");
        return FAILED;
    }
    return SUCCESS;
}
...
Status SplitReshape::CheckCopyIn(Function &function) {
    for (auto &op : function.Operations()) {
        ...
        if (CheckOp(function, op) == FAILED) {
            APASS_LOG_ERROR_F("SplitReshape", "Operation", "Run CheckOp failed.");
            return FAILED;
        }
    }
    return SUCCESS;
...
Status SplitReshape::GetAssembleDynShape(const LogicalTensorPtr &input, const LogicalTensorPtr &output, const std::vector<int64_t> &toOffset, std::vector<SymbolicScalar> &dynValidShape) {
    ...
    if (iter == reshapeOffset.end()) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Cannot find output from reshapeOffset.");
        return FAILED;
    }
...
Status SplitReshape::AddAssembleOp(const MemoryType &memoryType, const std::vector<int64_t> &outputOffset, const LogicalTensorPtr &input, const LogicalTensorPtr &output) {
    ...
    if (outputOffset.empty() || outputOffset.size() != curReshapeOffset.size()) {
        APASS_LOG_ERROR_F("SplitReshape", "Tensor", "Incorrect axis, toOffset is %s, curReshapeOffset is %s.", GetStr(outputOffset).c_str(), GetStr(curReshapeOffset).c_str());
        return FAILED;
    }
...
Status SplitReshape::GetReshapeDynShape(const std::shared_ptr<ReshapeOp> &op, std::vector<SymbolicScalar> &dynValidShape) {
    ...
    if (iter == viewOffset.end()) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Cannot find reshapeop with sharedptr.");
        return FAILED;
    }
    auto upperleftIdx = iter->second;
    for (const auto &validShape : op->dynValidShapes) {
        ...
        if (dynValidShape.size() != validShape.size() || upperleftIdx.size() != validShape.size()) {
            APASS_LOG_ERROR_F("SplitReshape", "Tensor", "Incorrect size, dynValidShape is %s, validShape is %s, upperleftIdx is %s.", GetStr(dynValidShape).c_str(), GetStr(validShape).c_str(), GetStr(upperleftIdx).c_str());
            return FAILED;
        }
        for (size_t i = 0; i < validShape.size(); ++i) {
...
Status SplitReshape::AddOperation(Function &function) {
    ...
    for (auto &a : assembles) {
        ...
        if (GetAssembleDynShape(a.input, a.output, a.toOffset, dynValidShape) != SUCCESS) {
            APASS_LOG_ERROR_F("SplitReshape", "Tensor", "Get assemble dynamic shape for AddOperation failed.");
            return FAILED;
        }
        auto &newCopyOut = GraphUtils::AddAssembleOperation(function, a, {dynValidShape});
        APASS_LOG_INFO_F("SplitReshape", "Operation", "ADD OP_ASSEMBLE, magic %d, IOperand tensor magic %d OOperand tensor magic %d, dynValidShape %s.", newCopyOut.opmagic,
    ...
    for (auto &b : reshapes) {
        ...
        if (GetReshapeDynShape(b.second, dynValidShape) != SUCCESS) {
            APASS_LOG_ERROR_F("SplitReshape", "Tensor", "Get reshape dynamic shape for AddOperation failed.");
            return FAILED;
        }
        auto &newReshape = GraphUtils::AddReshapeOperation(function, b.second->input, b.second->output, dynValidShape);
        APASS_LOG_INFO_F("SplitReshape", "Operation", "ADD OP_RESHAPE, magic %d, IOperand tensor magic %d OOperand tensor magic %d, dynValidShape %s.", newReshape.opmagic,
...
Status SplitReshape::EraseReshape(Function &function) {
    ...
    for (auto &opView : redundantViewops) {
        if (opView == nullptr) {
            APASS_LOG_ERROR_F("SplitReshape", "Operation", "Found null ptr for opView.");
            return FAILED;
        }
        APASS_LOG_INFO_F("SplitReshape", "Operation", "Remove OP_VIEW, magic %d.", opView->opmagic);
        opView->SetAsDeleted();
    }
    ...
    for (auto &op : function.Operations(false)) {
        ...
        if (output == nullptr) {
            APASS_LOG_ERROR_F("SplitReshape", "Operation", "Found null oOperand ptr for op[%d].", op.opmagic);
            return FAILED;
        }
