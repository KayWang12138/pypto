    ...
    if (auto it = assembleOutToInput_.find(reshapeInputRawMagic); it != assembleOutToInput_.end()) {
        ...
        if (!copySources.empty()) {
            ...
            result = std::all_of(copySources.begin(), copySources.end(),
                [&](const auto &src) { return src->GetRawTensor()->GetRawMagic() == firstAssInputRawMagic; });
        }
    }
...
std::vector<int64_t> SplitReshape::ObtainMapOffset(
    const LogicalTensorPtr &input, const LogicalTensorPtr &output) const {
...
Status SplitReshape::UpdateShapeOffset(UpdatePara &para, bool &flag, int &currentShape, int &currentOffset) {
    ...
    if (flag && currentShape <= stride) {
        ...
        if (currentOffset + currentShape > stride) {
            APASS_LOG_WARN_F(Elements::Tensor,
                "Found tail cond, currentOffset is %ld, currentShape is %ld, stride is %ld.",
    ...
    for (const auto &copyOutSource : assembleOutToInput_[copyOutTile.reshapeSource->GetRawTensor()->GetRawMagic()]) {
        ...
        if (it != rawToAlignCache_.end()) {
            ...
            if (result.st == SUCCESS) {
                result.newCopyOutSource =
                    std::make_shared<LogicalTensor>(function, copyOutTile.reshapeSource->GetRawTensor(),
...
Status SplitReshape::ProcessOnetoOne(Function &function, Operation &op, const CalcOverlapPara &para) {
    ...
    PerfectlyMatchPara perfectlyMatchPara = {
        input, output, overlap, reshapeSource, reshapeOutput, para.oriViewDynShape};
...
Status SplitReshape::UpdateForPerfectlyMatchWithAll(
    ...
    auto newReshapeSource =
        std::make_shared<LogicalTensor>(function, reshapeRawInputs_[overlaps.front()->GetRawTensor()->GetRawMagic()],
    ...
    PerfectlyMatchWithAllPara perfectlyMatchwithAllPara = {
        input, output, overlaps.front(), reshapeOutput, newReshapeSource, para.oriViewDynShape};
...
Status SplitReshape::ProcessMultitoOne(Function &function, Operation &op, const CalcOverlapPara &para) {
    ...
    if (AlignToRaw(newInfo, newReshapeSourceTileOffset, newReshapeSourceTileShape) != SUCCESS) {
        APASS_LOG_ERROR_F(Elements::Tensor,
            "Failed to compute the tile of the raw input of reshapeSource, newInfo is %s.", GetStr(newInfo).c_str());
        return FAILED;
    }
...
Status SplitReshape::CheckValidOp(const CheckParam &para, CheckOutputParam &checkOutputParam) {
    ...
    if (input->shape == output->shape) {
        APASS_LOG_WARN_F(Elements::Tensor, "The input and output of view op have the same shape, no need to split "
                                           "reshape (if exist) for each view op.");
        return WARNING;
    }
    ...
    Status dynStatus =
        CheckDynStatus(checkOutputParam.alignedShape, checkOutputParam.reshapeSource->GetRawTensor()->GetRawShape(),
            input->GetRawTensor()->GetRawShape(), reshapeDynOutput_[input->GetRawTensor()->GetRawMagic()]);
    if (dynStatus == WARNING) {
        APASS_LOG_WARN_F(Elements::Tensor,
            "Undetermined variable in the changing axis, input is %s, output is %s, dynOutput is %s.",
    ...
    if (dynStatus == FAILED) {
        APASS_LOG_ERROR_F(Elements::Tensor,
            "Illegal dynamic info for the reshape op, input is %s, output is %s, dynOutput is %s.",
            GetStr(checkOutputParam.reshapeSource->GetRawTensor()->GetRawShape()).c_str(),
            GetStr(input->GetRawTensor()->GetRawShape()).c_str(),
            GetStr(reshapeDynOutput_[input->GetRawTensor()->GetRawMagic()]).c_str());
        return FAILED;
...
Status SplitReshape::CheckOp(Function &function, Operation &op) {
    ...
    auto newInputView = std::make_shared<LogicalTensor>(function, inputView->GetRawTensor(),
...
Status SplitReshape::EraseReshape(Function &function) {
    ...
    for (auto &op : function.Operations(false)) {
        ...
        if (output == nullptr) {
            APASS_LOG_ERROR_F(Elements::Operation, "Found null oOperand ptr for op[%d]. %s", op.opmagic,
                GetFormatBacktrace(op).c_str());
            return FAILED;
        }
