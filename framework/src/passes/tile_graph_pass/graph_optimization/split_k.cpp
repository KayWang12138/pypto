...
Status SplitK::PreCheck(Function &function) {
    APASS_LOG_INFO_F(Elements::Operation, "PreCheck for SplitK.");
    if (!function.LoopCheck().empty()) {
        APASS_LOG_ERROR_F(Elements::Operation, "Loopcheck failed before PreGraph; Please check if there is a Loop.");
        return FAILED;
    }
    for (auto &op : function.Operations()) {
        if (op.GetOpcode() == Opcode::OP_A_MUL_B && op.GetOpcode() == Opcode::OP_A_MULACC_B) {
            ...
            if (op.GetOOperands().size() != 1) {
                APASS_LOG_ERROR_F(Elements::Operation, "Invalid op: [%d] has output num not equal to one; Please check if the output num is one.", op.GetOpMagic());
                return FAILED;
            }
            auto output = op.GetOOperands().front();
            if ((output->GetMemoryTypeOriginal() != MemoryType::MEM_L0C) || (output->GetConsumers().size() != 1) 
                || (*output->GetConsumers().begin() == nullptr)) {
                APASS_LOG_ERROR_F(Elements::Operation, "Op[%d] has invalid output tenosr[%d]; Please check if the output tensor is vaild.", op.GetOpMagic(), output->magic);
                return FAILED;
            }
        }
        if (op.GetOpcode() == Opcode::OP_REDUCE_ACC) {
            ...
            if (op.GetIOperands().size() < 1) {
                APASS_LOG_ERROR_F(Elements::Operation, "Op[%d] has input num less than 1; Please check the input num.", op.GetOpMagic());
                return FAILED;
            }
            // 输出数量必须等于1
            if (op.GetOOperands().size() != 1) {
                APASS_LOG_ERROR_F(Elements::Operation, "Op[%d] has output num not equal to one; Please check if the output num for is one.", op.GetOpMagic());
                return FAILED;
            }
            // Reduce Acc 的输入和输出必须都是DDR类型
            for (auto &in : op.GetIOperands()) {
                if (in->GetMemoryTypeOriginal() != MemoryType::MEM_DEVICE_DDR) {
                    APASS_LOG_ERROR_F(Elements::Operation, "Op[%d] has non-DDR input tenosr[%d]; Please check the memory type of the input tensor.", op.GetOpMagic(), in->magic);
                    return FAILED;
                }
            }
            for (auto &out : op.GetOOperands()) {
                if (out->GetMemoryTypeOriginal() != MemoryType::MEM_DEVICE_DDR) {
                    APASS_LOG_ERROR_F(Elements::Operation, "Op[%d] has non-DDR output tenosr[%d]; Please check the memory type of the output tensor.", op.GetOpMagic(), out->magic);
                    return FAILED;
                }
            }
        }
...
Status SplitK::RunOnFunction(Function &function) {
    APASS_LOG_INFO_F(Elements::Operation, "===> Start SplitK.");
    if (EliminateReduceAcc(function) != SUCCESS) {
        APASS_LOG_ERROR_F(Elements::Operation, "Eliminate ReduceAcc failed.");
        return FAILED;
    }
    ...
    APASS_LOG_INFO_F(Elements::Operation, "===> End SplitK.");
    return SUCCESS;
}
