    ...
    for (auto &op : function.Operations()) {
        ...
        for (const auto &consumer : consumers) {
            ...
            if (startTensor->shape == endTensor->shape && startTensor->offset == endTensor->offset ) {
                ...
                if ((funtion.IsFromInCast(startTensor) && CommonUtils::ContainsNegatvieOne(startTensor->GetShape())) ||
                    (function.IsFromOutCast(endTensor) && CommonUtils::ContainsNegatvieOne(endTensor->GetShape()))) {
                        continue;
                    }
