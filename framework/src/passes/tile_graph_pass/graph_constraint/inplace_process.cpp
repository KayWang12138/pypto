...
LogicalTensorPtr FindInplaceSource(Function &function, Operation &op, std::unordered_map<Operation *, LogicalTensorPtr> &visited) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, inplaceIdx >= 0 && inplaceIdx < static_cast<int>(op.GetIOperands().size())) << "Invalid inplaceIdx " << inplaceIdx << " for operation " << op.GetOpMagic();
    ...
    for (const auto producer : inplaceIOperand->GetProducers()) {
        ...
        if (res == nullptr) {
            ...
            ASSERT(ERROR_CODE_UNDEFINED, res == tmp) << "Inconsistent inplace source for operation " << op.GetOpMagic(); // inplace路径应总是交汇于同一起点
        }
    }
    if (res == nullptr) {
