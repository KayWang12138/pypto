...
bool PadLocalBuffer::IsInputB8(const Operation &op, const LogicalTensorPtr &in) const {
    return IsInputDataType(op, in, b8DataSupport); // b8DataSupport 作为默认的b8类型集合
}

bool PadLocalBuffer::IsInputB4(const Operation &op, const LogicalTensorPtr &in) const {
    return IsInputDataType(op, in, b4DataSupport); // b4DataSupport 作为默认的B4类型集合
}
...
void PadLocalBuffer::PadMatmul(Operation &op, LogicalTensorPtr &in) {
    ...
    const bool IsInputB8 = IsInputB8(op, in);
    const bool IsInputB4 = IsInputB4(op, in);
    /*
