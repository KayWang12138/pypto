...
bool CalculateNewRawShape(const std::vector<int64_t> &oriShape, const std::vector<int64_t> &newShape,
    ...
    for (size_t i = 0; i < oriSize; i++) {
        ...
        if ((i != 0) && (oriScale[i] != 1)) {
            ...
            APASS_LOG_ERROR_F(Elements::Function, "CalculateNewRawShape failed.");
            return false;
        }
    }
...
Status ProcessView(Function &function) {
    for (auto &op : function.Operations()) {
        ...
        for (auto reshapeConsumer : reshapeOutput->GetConsumers()) {
        ...
        if (!CalculateNewRawShape(
                reshapeInput->shape, reshapeOutput->shape, viewInput->tensor->rawshape, newRawShape, true)) {
            return FAILED;
        }
        GetDynOffsetBeforeReshape(offset, reshapeInput->shape, newRawShape, newDynOffset);
        for (auto copyIn : reshapeOutput->GetConsumers()) {
