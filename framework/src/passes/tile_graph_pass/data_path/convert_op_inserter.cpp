...
const std::unordered_set<DataType> kA5SupportedDtypes = {
    DT_INT4, DT_INT8, DT_UINT8, DT_FP16, DT_BF16, DT_HF8, DT_FP8, DT_FP32};
...
void ConvertInserter::UpdateTensorTobeMap(const LogicalTensorPtr &tensor, Operation &operation, MemoryType t) {
    ...
    if (tensorTobeMap.count(tensor) == 0) {
        ...
        APASS_LOG_DEBUG_F(Elements::Tensor, "Tensor %d first set toBeMap(%s[%d]) as %s", tensor->GetMagic(),
...
void ConvertInserter::PrintTensorTobeMap(LogicalTensorPtr &tensor) const {
    ...
    for (const auto &item : tensorTobeMap.at(tensor)) {
        APASS_LOG_INFO_F(Elements::Tensor, "\t|--- TensorTobeMap: %s --> %s[%d].",
            BriefMemoryTypeToString(item.second).c_str(), item.first->GetOpcodeStr().c_str(), item.first->GetOpMagic());
    }
}
...
std::map<MemoryType, std::set<Operation *>> ConvertInserter::ReformMap(
    std::map<Operation *, MemoryType> &oriMap) const {
    std::map<MemoryType, std::set<Operation *>> result;
...
void ConvertInserter::InsertConvertOpForEachConsumer(Function &function, const Operation &op,
    const std::shared_ptr<LogicalTensor> &oOperand, std::set<Operation *> &consumers, std::vector<MemoryType> &paths) {
...
void ConvertInserter::ProcessSpecialProducersOrConsumers(const Operation &op,
    const std::shared_ptr<LogicalTensor> &oOperand, std::set<Operation *> &consumers, MemoryType &requiredMemoryType) {
    ...
    bool crossCore = std::all_of(items.begin(), items.end(),
        [this, &oOperand](const auto &item) { return CrossCore(oOperand->GetMemoryTypeOriginal(), item.second); });
...
void ConvertInserter::GraphReconnect(const std::shared_ptr<LogicalTensor> &oOperand,
    std::shared_ptr<LogicalTensor> output, const std::set<Operation *> &consumers, Function &function) const {
    for (const auto &consumer : consumers) {
        if (consumer->BelongTo() == &function) {
            UpdateConsumerAndReconnect(oOperand, output, consumer);
        }
...
void ConvertInserter::CreateMoveOpForConvert(Operation &op) {
    ...
    if (from == MemoryType::MEM_DEVICE_DDR) {
        ...
        op.SetOpAttribute(std::make_shared<ViewOpAttribute>(op.iOperand.front()->GetOffset(), to,
            op.iOperand.front()->GetDynOffset(), op.iOperand.front()->GetDynValidShape()));
    ...
    if (to == MemoryType::MEM_DEVICE_DDR) {
        ...
        op.SetOpAttribute(std::make_shared<AssembleOpAttribute>(from, op.oOperand.front()->GetOffset(),
            op.oOperand.front()->GetDynOffset(), op.iOperand.front()->GetDynValidShape()));
