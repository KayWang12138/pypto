...
private:
    ...
    Status SetNumLR(std::map<int, int>& numLRMapResult);
    Status SetNumDB(std::map<int, int>& numDBMapResult);
    ...
    static int globalL1ReuseHashOrder_;      // L1复用阶段全局计数器
    static int globalCubeMergeHashOrder_;    // Cube合并阶段全局计数器
};
