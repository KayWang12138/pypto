...
inline void HashUpdate(std::unordered_map<uint64_t, std::vector<int>> &hashMap,
    std::unordered_map<uint64_t, int> &hashOrder, int color, std::vector<uint64_t> hashColor) {
    // 更新子图哈希
    ...
    for (auto &entry : hashMap) {
        APASS_LOG_INFO_F(Elements::Operation, "Subgraph hash: %lu, Subgraph ID: %s.", entry.first,
...
Status L1CopyInReuseRunner::L1MergeProcess(OperationsViewer &opOriList, std::vector<std::vector<int>> &colorNode,
    std::vector<uint64_t> &hashColor, std::vector<int> &colorCopyIn, std::map<std::vector<uint64_t>, int> &l1InputList,
    int &tmpColor, std::vector<int> &mergedNum, int &i) {
    for (auto opIdx : colorNode[i]) {
        ...
        if (vec.size() == 0) {
            APASS_LOG_ERROR_F(Elements::Operation, "L1MergeProcess: op %d %s GetGMInputFeature failed. %s",
                opOriList[i].GetOpMagic(), opOriList[i].GetOpcodeStr().c_str(),
                GetFormatBacktrace(opOriList[i]).c_str());
            return FAILED;
        }
...
bool L1CopyInReuseRunner::GetMergedL1(int maxInColor, std::vector<int> &mergedNum, int maxMergeNum, int &tmpColor,
    int i, std::map<std::vector<uint64_t>, int> &l1InputList, std::vector<uint64_t> &vec, std::vector<int> &colorCopyIn,
...
Status L1CopyInReuseRunner::Phase1(Function &func, int color, std::vector<std::vector<int>> &colorNode,
    ...
    for (int ii = 0; ii < color; ii++) {
        ...
        while (colorCopyIn[i] <= mgCopyInUpperBound_ && j < colorNode[i].size()) {
            ...
            if (GetMergedL1(maxInColor, mergedNum, numLRList[hashOrder_[hashColor[i]]], tmpColor, i, l1InputList, vec,
        ...
        if (L1MergeProcess(opOriList, colorNode, hashColor, colorCopyIn, l1InputList, tmpColor, mergedNum, i) ==
            FAILED) {
            APASS_LOG_ERROR_F(Elements::Operation, "L1MergeProcess failed; Please check the L1MergeProcess method. %s",
                GetFormatBacktrace(opOriList[i]).c_str());
            return FAILED;
        }
    }
