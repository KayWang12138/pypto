...
Status L1CopyInReuseRunner::SetNumLR(std::vector<int>& numLRList)
{
    ...
    for (auto& entry : numLRMap_) {
        ...
        if (hashOrderKey >= static_cast<int>(hashMap_.size())) {
            APASS_LOG_WARN_F(
                Elements::Config, "Invalid hash order ID: %d in cubeL1ReuseSetting, ignored.", hashOrderKey);
            continue;
        }

        if (entry.second < 0) {
            APASS_LOG_ERROR_F(
                Elements::Config, "Invalid merge count for hash order %d: merge count=%ld, please check.", hashOrderKey,
...
Status L1CopyInReuseRunner::ApplySemanticLabelSettingsCubeNBuffer(
    const OperationsViewer& opOriList, std::vector<int>& numDBList, const std::vector<uint64_t>& hashColor, int color)
{
    ...
    for (const auto& [order, val] : labelOverrides) {
        if (order < static_cast<int>(numDBList.size())) {
            numDBList[order] = val;
            APASS_LOG_INFO_F(
                Elements::Config, "Applied cube nbuffer semantic label override: hash_order=%d, merge_num=%d", order,
                val);
        }
    }
