...
Status L1CopyInReuseRunner::SetNumLR(std::map<int, int>& numLRMapResult)
{
    ...
    if (numLR != numLRMap_.end()) {
        ...
        for (auto& [hashcolor, order] : hashOrder_) {
            numLRMapResult[order] = numLR->second;
        }
    } else {
        for (auto& [hashcolor, order] : hashOrder_) {
            numLRMapResult[order] = -1;
        }
    }
    ...
    for (auto& entry : numLRMap_) {
        ...
        if (!found) {
            APASS_LOG_WARN_F(Elements::Config, "Invalid hashOrder: %d in cubeL1ReuseSetting, ignored.", hashOrderKey);
            continue;
        }
        if (entry.second < 0) {
            APASS_LOG_ERROR_F(
        ...
        numLRMapResult[hashOrderKey] = entry.second;
    }
    return SUCCESS;
}
...
Status L1CopyInReuseRunner::SetNumDB(std::map<int, int>& numDBMapResult)
{
    ...
    if (numDB != numDBMap_.end()) {
        ...
        for (auto& [hashcolor, order] : hashOrder_) {
            numDBMapResult[order] = numDB->second;
        }
    } else {
        for (auto& [hashcolor, order] : hashOrder_) {
            numDBMapResult[order] = -1;
        }
    }
    ...
    for (auto& entry : numDBMap_) {
        ...
        if (!found) {
            APASS_LOG_WARN_F(Elements::Config, "Invalid hashOrder: %d in cubeNBufferSetting, ignored.", hashOrderKey);
            continue;
        }
        if (entry.second < 1) {
            APASS_LOG_ERROR_F(
        ...
        numDBMapResult[hashOrderKey] = entry.second;
    }
    return SUCCESS;
}
...
Status L1CopyInReuseRunner::Run(Function& func, int color, std::vector<std::vector<int>>& colorNode)
{
    ...
    // NBuffer参数设置
    if (SetNumDB(hashMergeNumMap) == FAILED) {
