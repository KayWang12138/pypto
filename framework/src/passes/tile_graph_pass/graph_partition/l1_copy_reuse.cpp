...
static Status AnyOpSpecialProcess(Function &function) {
    std::vector<bool> isAllAnyOp(function.GetTotalSubGraphCount(), true);
    ...
    std::unordered_set<Opcode> anyOp{Opcode::OP_VIEW, Opcode::OP_RESHAPE, Opcode::OP_ASSEMBLE};
    for (auto &op : function.Operations()) {
        int currSubgraphID = op.GetSubgraphID();
        if (OpcodeManager::Inst().GetCoreType(op.GetOpcode()) != OpCoreType::ANY || anyOp.count(op.GetOpcode()) == 0) {
            isAllAnyOp[currSubgraphID] = false;
        }
