...
Function* SubgraphToFunction::CreateRootFunc(npu::tile_fwk::Function &function) {
    ...
    // 下面这一段复制了subgraph_to_function中的流程
    ...
    printf("Done EndFunction of root name is %s\n", rootName.c_str());

    auto resultFunc = std::get<0>(rootEndResult);
    if (resultFunc != rootFunc) { 
        ALOG_ERROR_F("Root function mismatch after finalization."); 
        return nullptr;
    }
...
Status SubgraphToFunction::HandleBlockCall(Function &function) {
    printf("Enter HandleBlockCall by function %s\n", function.GetMagicName().c_str());
    ...
    for (auto &oriCallOp : function.Operations(false)) {
        ...
        auto opAttribute = std::make_shared<CallOpAttribute>(oriCallOp.GetCalleeHash(), args.argList, 
