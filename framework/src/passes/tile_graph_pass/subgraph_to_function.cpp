...
static std::unordered_map<int, GetTensorDataOutcastDesc> GetTensorDataBuildOutcastDescDict(Function &function) {
    ...
    for (auto &[index, desc] : getTensorDataOutcastDescDict) {
        (void)index;
        ASSERT(ERROR_CODE_UNKNOWN, desc.opListDict[Opcode::OP_ADDS].size() == 1) << "Expect the size is 1 for opListDict, but we get " << desc.opListDict[Opcode::OP_ADDS].size() << "OP_ADDS";
        ...
        ASSERT(ERROR_CODE_UNKNOWN, copyout->GetOpcode() == Opcode::OP_COPY_OUT) << "Expect Opcode OP_COPY_OUT, but we get " << copyout->GetOpcodeStr() << " at operation[" << copyout->GetOpMagic() << "].";;
...
static std::vector<GetTensorDataUsageDesc> GetTensorDataBuildUsageDesc(Function &function) {
    ...
    for (size_t i = 0; i < operationViewer.size(); i++) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, subgraphTensor != nullptr) << "Expect operation[" << refOp.GetOpMagic() << "] has valid IOperand/OOperand, but we get nullptr. Please check the operation.";
