    ...
    Function& rootFunc, std::vector<Function*>& mergedFuncList1, size_t i) const
{
    ...
    for (auto& incast : rootFunc.GetIncast()) {
        symbolToPythonIdx[incast->GetRawTensor()->GetSymbol()] =
            static_cast<int>(symbolToPythonIdx.size());
    }
    for (auto& outcast : rootFunc.GetOutcast()) {
        symbolToPythonIdx[outcast->GetRawTensor()->GetSymbol()] =
            static_cast<int>(symbolToPythonIdx.size());
    }
    ...
    std::stable_sort(indices.begin(), indices.end(), [&](size_t a, size_t b) {
        int idxA = symbolToPythonIdx.count(tensorArgs[a].symbol)
                       ? symbolToPythonIdx[tensorArgs[a].symbol]
                       : fallbackIdx;
        int idxB = symbolToPythonIdx.count(tensorArgs[b].symbol)
                       ? symbolToPythonIdx[tensorArgs[b].symbol]
                       : fallbackIdx;
        return idxA < idxB;
    });
