...
const std::string &Pass::LogFolder(const std::string &topFolder, size_t i) const {
    if (CreateLogFolder(topFolder, i) == FAILED) {
        APASS_LOG_WARN_F(Elements::Config, "Create log folder failed.");
...
Status Pass::CreateLogFolder(const std::string &topFolder, size_t i) const {
    ...
    if (res == false) {
        APASS_LOG_ERROR_F(Elements::Config, "Failed to create directory: [%s].",
        passFolder_.c_str());
        return FAILED;
    }
    return SUCCESS;
...
Status Pass::Run(Function &function, const std::string &strategy,
    ...
    if (passDfxconfigs_.disablePass) {
        APASS_LOG_WARN_F(Elements::Manager, "Pass [%s] is skipped.", identifier_.c_str());
        return SUCCESS;
    }
    if (PreRun(function) == FAILED) {
        APASS_LOG_ERROR_F(Elements::Manager, "PreRun pass [%s] failed.", identifier_.c_str());
        return FAILED;
    } 
    if (RunOnFunction(function) == FAILED) {
        APASS_LOG_ERROR_F(Elements::Manager, "Run pass [%s] failed.", identifier_.c_str());
        return FAILED;
    }
    if (PostRun(function) == FAILED) {
        APASS_LOG_ERROR_F(Elements::Manager, "PostRun pass [%s] failed.", identifier_.c_str());
        return FAILED;
    }
    identifier_.clear();
    strategy_.clear();
...
Status Pass::CreateGraphFolder(Function &function) {
    if (passDfxconfigs_.dumpGraph) {
        ...
        if (res == false) {
            APASS_LOG_ERROR_F(Elements::Config, "Failed to create directory: [%s].", graphFolder_.c_str());
            return FAILED;
        }
    }
    return SUCCESS;
...
Status Pass::PreRun(Function &function) {
    ...
    if (CreateGraphFolder(function) != SUCCESS) {
        APASS_LOG_WARN_F(Elements::Graph, "Create graph directory failed.");
    }
    if (passDfxconfigs_.dumpGraph) {
        if (name_ == "ExpandFunction") {
            fileName = graphFolder_ + "/End_TensorGraph";
            if (DumpGraphJson(function, fileName) != SUCCESS) {
                APASS_LOG_WARN_F(Elements::Graph, "Dump End TensorGraph json failed.");
            }
        }
        if (name_ == "RemoveRedundantReshape") {
            fileName = graphFolder_ + "/Begin_TensorGraph";
            if (DumpGraphJson(function, fileName) != SUCCESS) {
                APASS_LOG_WARN_F(Elements::Graph, "Dump Begin TensorGraph json failed.");
            }
        }
    }
    if (passDfxconfigs_.dumpGraph) {
        if (name_ == "SubgraphToFunction") {
            ...
            if (DumpGraphJson(function, fileName) != SUCCESS) {
                APASS_LOG_WARN_F(Elements::Graph, "Dump End TileGraph json failed.");
            }
        }
    }
    if (passDfxconfigs_.dumpGraph) {
    ...
    if (passDfxconfigs_.preCheck) {
        if (PreCheck(function) != SUCCESS) {
            APASS_LOG_ERROR_F(Elements::Manager, "Precheck of pass [%s] failed.", identifier_.c_str());
            return FAILED;
        }
    }
...
Status Pass::PostRun(Function &function) {
    ...
    if (passDfxconfigs_.dumpGraph && name_ == "ExpandFunction") {
        if (name_ == "ExpandFunction") {
            ...
            if (DumpGraphJson(function, fileName) != SUCCESS) {
                APASS_LOG_WARN_F(Elements::Graph, "Dump Begin TileGraph json failed.");
            }
        }
    }
    if (passDfxconfigs_.dumpGraph) {
        if (name_ == "SubgraphToFunction") {
            ...
            if (DumpGraphJson(function, fileName) != SUCCESS) {
                APASS_LOG_WARN_F(Elements::Graph, "Dump Begin BlockGraph json failed.");
            }
        }
        if (name_ == "CodegenPreproc") {
            fileName = graphFolder_ + "/End_BlockGraph";
            if (DumpGraphJson(function, fileName) != SUCCESS) {
                APASS_LOG_WARN_F(Elements::Graph, "Dump End BlockGraph json failed.");
            }
        }
    }
    if (passDfxconfigs_.dumpGraph) {
    ...
    if (passDfxconfigs_.postCheck) {
        if (PostCheck(function) != SUCCESS) {
            APASS_LOG_ERROR_F(Elements::Manager, "Postcheck of pass [%s] failed.", identifier_.c_str());
            return FAILED;
        }
    }
    if (passDfxconfigs_.healthCheck) {
