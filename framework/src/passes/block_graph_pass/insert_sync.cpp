...
std::string PipeSync::DumpMergeCVInfo(PipePair targetPp, int maxOverlapIdx,
                                      std::unordered_map<PipePair, DataDepInfo, PipePairHash> cvDepInfoMap) {
    std::stringstream ss;
    ss << "Merge PipePair: " << GetPipeTypeDict().Find(targetPp.first.pipe) << " ";
...
std::string PipeSync::DataDepInfo::DumpDataDepInfo(const std::vector<IndexOp>& syncedOpLog) {
    ...
    for (auto [setidx, waitidx] : opDepList) {
        ss << setidx << " " << oriOpList_[setidx]->GetOpMagic() << " " << oriOpList_[setidx]->GetOpcodeStr();
        ss << "  " << waitidx << " " << oriOpList_[waitidx]->GetOpMagic() << " " << oriOpList_[waitidx]->GetOpcodeStr() << "\n";
    }
    return ss.str();
}
...
Status PipeSync::RelaxCvEventId(std::vector<IndexOp>& syncedOpLog) {
    for (const auto& corePair : cvCorePair) {
        ...
        APASS_LOG_DEBUG_F(Elements::Operation, "CoreType: %s AIVCore: %d -> CoreType: %s AIVCore: %d has no eventid to use, start relax cv eventid.",
                          GetCoreTypeDict().Find(corePair.first.first), static_cast<int>(corePair.first.second),
                          GetCoreTypeDict().Find(corePair.second.first), static_cast<int>(corePair.second.second));
