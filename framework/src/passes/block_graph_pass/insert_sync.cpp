    ...
    for (size_t i = 0; i < opLogPtr.size(); i++) {
        if (opLogPtr[i]->GetOpcodeStr().find("ALLOC") != std::string::npos) {
            APASS_LOG_ERROR_F(Elements::Operation,
                "%d ALLOC op should not appear in InsertSync, PipeDispatch failed.%s", opLogPtr[i]->GetOpMagic(),
                GetFormatBacktrace(opLogPtr[i]).c_str());
            return FAILED;
        }
...
Status PipeSync::IssueOpPipeSeq(Function &function, std::vector<Operation *> opLogPtr,
...
Status PipeSync::IssueSyncOp(Function &function, std::vector<Operation *> opLogPtr, std::vector<IndexOp> &syncedOpLog,
...
bool PipeSync::BufOverlap(const TileRange &range1, int magic1, const TileRange &range2, int magic2) const {
    APASS_LOG_DEBUG_F(Elements::Tensor, "Range 1 [%zu ~ %zu], range 2 [%zu ~ %zu].", range1.start, range1.end,
    ...
    for (size_t setIdx = 0; setIdx < opSet.GetOOperands().size(); setIdx++) {
        for (size_t waitIdx = 0; waitIdx < opWait.GetOOperands().size(); waitIdx++) {
            ...
            if (BufOverlap(range1, magic1, range2, magic2)) {
                APASS_LOG_DEBUG_F(Elements::Operation, "%d %zu %s and %d %zu %s has WAW data dependency",
                    opSet.GetOpMagic(), k, opSet.GetOpcodeStr().c_str(), opWait.GetOpMagic(), idx,
                    opWait.GetOpcodeStr().c_str());
                return true;
            }
        }
    }
    ...
    for (size_t outIdx = 0; outIdx < opSet.GetOOperands().size(); outIdx++) {
        for (size_t inIdx = 0; inIdx < opWait.GetIOperands().size(); inIdx++) {
            ...
            if (overlap || ddrTensorSame) {
                APASS_LOG_DEBUG_F(Elements::Operation, "%d %zu %s and %d %zu %s has RAW data dependency",
                    opSet.GetOpMagic(), k, opSet.GetOpcodeStr().c_str(), opWait.GetOpMagic(), idx,
                    opWait.GetOpcodeStr().c_str());
                return true;
            }
...
bool PipeSync::CheckWarDependency(const Operation &opSet, const Operation &opWait, size_t k, size_t idx) {
    for (size_t outIdx = 0; outIdx < opWait.GetOOperands().size(); outIdx++) {
        for (size_t inIdx = 0; inIdx < opSet.GetIOperands().size(); inIdx++) {
            ...
            auto ddrTensorSame = memType == MemoryType::MEM_DEVICE_DDR && range1.memId == range2.memId;
            if (overlap || ddrTensorSame) {
                APASS_LOG_DEBUG_F(Elements::Operation, "%d %zu %s and %d %zu %s has WAR data dependency",
                    opSet.GetOpMagic(), k, opSet.GetOpcodeStr().c_str(), opWait.GetOpMagic(), idx,
                    opWait.GetOpcodeStr().c_str());
                return true;
            }
...
void PipeSync::FindDep(
    ...
    for (auto it = dataDependencySet.rbegin(); it != dataDependencySet.rend(); it++) {
        ...
        APASS_LOG_DEBUG_F(Elements::Operation, "Current process ops: %d %zu %s and %d %zu %s", prevAOp->GetOpMagic(), k,
...
void PipeSync::PhaseKernelProcess(
    Function &function, std::vector<Operation *> srcLog, std::vector<Operation *> &dstLog) {
    size_t prerun = 0;
    size_t i = 0;
...
Status PipeSync::ReorderViewAssemble(std::vector<Operation *> &opLog, std::vector<Operation *> &opListNew,
...
Status InsertSync::CheckNewOpListSeq(
    const std::vector<Operation *> &oriOpList, const std::vector<Operation *> &opListNew) {
    if (oriOpList.size() <= 1) {
        return SUCCESS;
    }
    size_t i = 0;
