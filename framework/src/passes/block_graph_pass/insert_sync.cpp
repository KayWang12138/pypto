...
Status PipeSync::InjectWaitFlag(Function &function, size_t idx, std::vector<IndexOp> &syncedOpList) {
    ...
    for (const auto &ele : depInfoList_[idx].waitPipe) {
        ...
        APASS_LOG_DEBUG_F(Elements::Operation, "Insert wait flag for prevOpIdx: %d, syncOp[%d] %s, setpipe: %s, waitpipe: %s, eventid: %d",
...
Status PipeSync::InjectSetFlag(Function &function, size_t idx, std::vector<IndexOp> &syncedOpList) {
    ...
    for (const auto &ele : depInfoList_[idx].setPipe) {
        ...
        if (res) {
            ...
            APASS_LOG_DEBUG_F(Elements::Operation, "Insert set flag for successor opIdx: %d, syncOp[%d] %s, setpipe: %s, waitpipe: %s, eventid: %d",
                ele, syncOp.GetOpMagic(), syncOp.GetOpcodeStr().c_str(), GetPipeTypeDict().Find(syncOp.syncQueue_.pipeId_).c_str(),
                GetPipeTypeDict().Find(syncOp.syncQueue_.trigPipeId_).c_str(), syncOp.syncQueue_.eventId_);
...
void PipeSync::UpdateDep(DepOp &currOp, DepOp &prevOp) {
    ...
    if (currSetPipeIter == currPipeDep.setPipes.end() || currSetPipeIter->second < prevOp.idx) {
        ...
        APASS_LOG_DEBUG_F(Elements::Operation, "Save dependency relationships in their respective pipe, prevOpIdx: %d, prevPipeEnd: %s, currOpIdx: %d, currPipeStart: %s",
