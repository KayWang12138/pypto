...
void CodegenPreproc::MarkForScope(Function &function) {
    int64_t lastGroupId = INVALID_LOOP_GROUPID;
    int64_t curGroupId = INVALID_LOOP_GROUPID;
    std::shared_ptr<Operation> lastOp = nullptr;
    for (auto &subProgram : function.rootFunc_->programs_) {
        for (auto &op : subProgram.second->Operations(false)) {
            ...
            APASS_LOG_INFO_F(Elements::Operation, "op %s[mgaic:%d] curGroupId is %d", op.GetOpcodeStr().c_str(),
                op.GetOpMagic(), curGroupId);
            if (curGroupId == INVALID_LOOP_GROUPID) {
                if (lastGroupId != INVALID_LOOP_GROUPID && lastOp != nullptr) {
                    APASS_LOG_INFO_F(Elements::Operation, "op set loopGroupEnd: %s[mgaic:%d]",
                        op.GetOpcodeStr().c_str(), op.GetOpMagic());
                    lastOp->SetAttribute(OpAttributeKey::loopGroupEnd, true);
            ...
            if (lastGroupId != curGroupId) {
                ...
                if (lastGroupId != INVALID_LOOP_GROUPID && lastOp != nullptr) {
                    APAAPASS_LOG_INFO_F(Elements::Operation, "op set loopGroupEnd: %s[mgaic:%d]",
                        op.GetOpcodeStr().c_str(), op.GetOpMagic());
                    lastOp->SetAttribute(OpAttributeKey::loopGroupEnd, true);
                }

                lastGroupId = curGroupId;
                lastOp = op.shared_from_this();
                continue;
            }
        }
        // last op
        if (lastGroupId != INVALID_LOOP_GROUPID && lastOp != nullptr) {
            subProgram.second->Operations(false).back().SetAttribute(OpAttributeKey::loopGroupEnd, true);
        }
        lastGroupId = INVALID_LOOP_GROUPID;
        lastOp = nullptr;
    }
