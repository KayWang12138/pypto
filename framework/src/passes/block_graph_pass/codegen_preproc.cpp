...
Status CodegenPreproc::SaveGmTensorParamIdxToOp(Function& func) const
{
    ...
    for (auto& subProgram : func.rootFunc_->programs_) {
        ...
        for (auto& op : subProgram.second->Operations(false)) {
            int gmTensorParamIdx{0};
            if (op.HasAttribute(OpAttributeKey::gmTensorParamIdxInCallFunc)) {
                gmTensorParamIdx = op.GetAttr<int>(OpAttributeKey::gmTensorParamIdxInCallFunc);
            }
            int attrOffset{0};
            for (size_t i = 0; i < op->GetIOperands().size(); ++i) {
                ...
                if (tensor->GetMemoryTypeToBe() == MEM_DEVICE_DDR) {
                    SetTensorParamAddr(
                        *tensor, gmTensorParamIdx, SymbolicScalar(op->GetIOpAttrOffset(attrOffset++)),
                        op->GetOpMagic());
                }
            }
            attrOffset = 0;
            for (size_t i = 0; i < op->GetOOperands().size(); ++i) {
                ...
                if (tensor->GetMemoryTypeToBe() == MEM_DEVICE_DDR) {
                    SetTensorParamAddr(
                        *tensor, gmTensorParamIdx, SymbolicScalar(op->GetOOpAttrOffset(attrOffset++)),
                        op->GetOpMagic());
                }
            }
        }
