...
void OoOScheduler::UpdateOpAttr(Operation &op, int opLatency, LogicalTensorPtr spillTensor, std::vector<int64_t> offset,
    ...
    for (auto &succId : spillIssue->successors) {
        ...
        if (!succ->isRetired && (std::count(succ->reqMemIds.begin(), succ->reqMemIds.end(), spillMemId) > 0)) {
            ...
            if (succ->predecessors.erase(spillIssue->id) == 0) {
                APASS_LOG_ERROR_F(Elements::Operation, "Erase issueEntry %s failed. %s",
                    spillIssue->GetOpInfo().c_str(), GetFormatBacktrace(spillIssue->tileOp).c_str());
                return FAILED;
            }
            succ->predecessors.insert(reloadCopyin->id);
            if (reloadCopyin->tileOp.GetOutputOperand(0) == nullptr) {
                APASS_LOG_ERROR_F(Elements::Operation, "%s cannot find oOperand[0]. %s",
                    reloadCopyin->GetOpInfo().c_str(), GetFormatBacktrace(reloadCopyin->tileOp).c_str());
                return FAILED;
            }
...
Status OoOScheduler::UpdateReloadIssueInfo(IssueEntryPtr reloadAlloc, IssueEntryPtr reloadCopyin,
    ...
    if (UpdateReloadIssueDepend(reloadCopyin, spillIssue, spillMemId) != SUCCESS) {
        APASS_LOG_ERROR_F(Elements::Operation, "UpdateReloadIssueDepend failed. %s",
...
Status OoOScheduler::CreateSpillReloadIssue(LogicalTensorPtr spillOutTensor, LogicalTensorPtr spillTensor,
...
Status OoOScheduler::SpillReshapeParticalBuffer(
    ...
    auto spillReshapeIssue = UpdateIssueAttr(reshapeOp,
        {reshapeTensor->memoryrange.memId, reshapeTensor->memoryrange.memId}, allocIssue, bufNextUseOrder, isGenSpill);
...
Status OoOScheduler::CreateSpillCopyout(IssueEntryPtr spillIssue, LogicalTensorPtr spillTensor, int spillMemId,
    ...
    std::shared_ptr<RawTensor> ddrRawTensor =
        (spillInfo.spillIssue_ != spillIssue && spillTensor->GetMemoryTypeOriginal() == MemoryType::MEM_L0C) ?
            ...
            std::make_shared<RawTensor>(spillTensor->Datatype(), spillTensor->tensor->rawshape, TileOpFormat::TILEOP_ND,
    ...
    if (spillInfo.spillIssue_ != spillIssue && spillTensor->GetMemoryTypeOriginal() == MemoryType::MEM_L0C) {
        ...
        if (spillInfo.spillIssue_->tileOp.GetAttr(OpAttributeKey::scaleValue, scaleValue)) {
            ...
            APASS_LOG_DEBUG_F(Elements::Operation, "Transfer scaleValue %s from L0C_COPY_L1 to L0C_COPY_OUT",
...
Status OoOScheduler::UpdateCopyOutMode(Operation &copyOutOp) {
    ...
    if (!input) {
        APASS_LOG_ERROR_F(Elements::Operation, "CopyOutOp %s[%d] does not have inputOperand",
            copyOutOp.GetOpcodeStr().c_str(), copyOutOp.GetOpMagic());
        return FAILED;
    }
...
Status OoOScheduler::UpdateCopyInMode(Operation &copyInOp) {
    ...
    if (!output) {
        APASS_LOG_ERROR_F(Elements::Operation, "CopyInOp %s[%d] does not have outputOperand",
...
Status OoOScheduler::CreateSpecialL1Copyout(
    ...
    if (spillIssue->tileOp.GetOpcode() != Opcode::OP_RESHAPE &&
        ...
        APASS_LOG_ERROR_F(Elements::Operation,
            "spillIssue %s is not COPY_IN/UB_COPY_L1/UB_COPY_L1/RESHAPE in A5 L1 spill",
    ...
    if (spillIssue->tileOp.GetOpcode() == Opcode::OP_RESHAPE) {
        ...
        if (actualSpillIssue->tileOp.GetInputOperand(0)->GetMemoryTypeOriginal() != MemoryType::MEM_UB &&
            ...
            APASS_LOG_ERROR_F(Elements::Operation, "SpillIssue is Reshape, preop: %s, ioperand of L1: %s",
    ...
    if (CreateSpillCopyout(actualSpillIssue, actualSpillTensor, actualSpillTensor->memoryrange.memId, spillCopyout,
            spillInfo) != SUCCESS) {
        APASS_LOG_ERROR_F(Elements::Operation, "CreateSpillCopyout failed for specialL1 spill! %s",
...
Status OoOScheduler::SpillOutBuffer(SpillInfo &spillInfo, IssueEntryPtr issue, size_t &pcIdx, bool isGenSpill) {
    ...
    if (spillInfo.isSpecialL1_) {
        ...
        if (CreateSpillCopyout(spillInfo.spillIssue_, spillInfo.spillTensor_, spillInfo.spillMemId_, spillCopyout,
                spillInfo) != SUCCESS) {
            APASS_LOG_ERROR_F(Elements::Operation, "CreateSpillCopyout failed! %s",
...
Status OoOScheduler::GetSpillTensor(IssueEntryPtr spillIssue, int spillMemId, LogicalTensorPtr &spillTensor) {
    ...
    if (spillTensor == nullptr) {
        APASS_LOG_ERROR_F(Elements::Operation, "Op cannot find oOperand[%d]. %s", spillTensorIdx,
            GetFormatBacktrace(spillIssue->tileOp).c_str());
        return FAILED;
    }
    return SUCCESS;
...
Status OoOScheduler::SpillBuffer(
    ...
    if (oooCheck.doHealthCheck) {
        oooCheck.spillInfoVec.emplace_back(RecordSpillInfo(allocBuffer->memType, spillInfo.spillMemId_, allocBuffer,
            spillInfo.ddrTensor_, spillInfo.spillIssue_->tileOp.GetOpcodeStr().find("COPY_IN") == std::string::npos));
    }
...
Status OoOScheduler::FindAssembleWithSpillTensor(SpillInfo &spillInfo, std::vector<IssueEntryPtr> &assembleList) {
    for (auto producer : spillInfo.spillTensor_->GetProducers()) {
        if (producer->GetOpcode() != Opcode::OP_ASSEMBLE) {
            APASS_LOG_ERROR_F(Elements::Operation, "All producer of Tensor[%d] must be assemble, now has %s[%d].",
...
Status OoOScheduler::SpillParticalBuffer(SpillInfo &spillInfo, IssueEntryPtr allocIssue, IssueEntryPtr assemble,
    ...
    UpdateIssueAttr(assembleOp, {assembleTensor->memoryrange.memId, assembleTensor->memoryrange.memId}, allocIssue,
        bufNextUseOrder, isGenSpill);
    numTotalIssues += TWO_ISSUE;
    return SUCCESS;
}
...
Status OoOScheduler::GetSpillInfo(IssueEntryPtr allocIssue, int spillMemId, bool isGenSpill, SpillInfo &spillInfo) {
    ...
    if (GetSpillTensor(spillIssue, spillMemId, spillTensor) != SUCCESS) {
        APASS_LOG_ERROR_F(Elements::Operation, "%s GetSpillTensor failed! %s", spillIssue->GetOpInfo().c_str(),
            GetFormatBacktrace(spillIssue->tileOp).c_str());
        return FAILED;
    }
    APASS_LOG_DEBUG_F(
    ...
    for (auto &spillMemId : spillGroup) {
        ...
        if (spillInfo.spillIssue_->tileOp.GetOpcode() == Opcode::OP_ASSEMBLE) {
            ...
            if (SpillBuffer(spillInfo, allocIssue, pcIdx, allocBuffer, isGenSpill) != SUCCESS) {
                APASS_LOG_ERROR_F(Elements::Operation, "SpillBuffer[%d] failed. %s", spillMemId,
                    GetFormatBacktrace(spillInfo.spillIssue_->tileOp).c_str());
                return FAILED;
            }
        }
...
Status OoOScheduler::SpillAllBuffer(
    ...
    for (auto memId : memIds) {
        ...
        if (SpillBuffer(spillInfo, allocIssue, pcIdx, allocBuffer, isGenSpill) != SUCCESS) {
            APASS_LOG_ERROR_F(Elements::Operation, "SpillBuffer[%d] failed. %s", memId,
                GetFormatBacktrace(spillInfo.spillIssue_->tileOp).c_str());
            return FAILED;
        }
    }
    ...
    if (RearrangeBuffer(allocIssue, memType, corePair, isGenSpill) != SUCCESS) {
        APASS_LOG_WARN_F(Elements::Operation, "RearrangeBuffer failed at SpillAllBuffer. %s",
