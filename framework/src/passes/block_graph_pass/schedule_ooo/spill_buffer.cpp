...
{
    static const std::unordered_set<Opcode> kL0CConsumerOps = {
        Opcode::OP_L0C_COPY_UB, Opcode::OP_L0C_TO_L1,
        Opcode::OP_L0C_COPY_OUT, Opcode::OP_L0C_COPY_OUT_CONV};
    for (auto* c : spillTensor->GetConsumers()) {
        if (c == nullptr || opIsRetiredMap[c]) {
            continue;
        }
        if (kL0CConsumerOps.find(c->GetOpcode()) == kL0CConsumerOps.end()) {
            APASS_LOG_WARN_F(Elements::Operation,
                "L0C spill: unsupported consumer %s of L0C tensor[%d].",
...
Operation* OoOScheduler::PickReusableCopyOut(const std::vector<Operation*> &consumers, LogicalTensorPtr spillTensor)
{
    for (auto* c : consumers) {
        auto opc = c->GetOpcode();
        if (opc != Opcode::OP_L0C_COPY_OUT && opc != Opcode::OP_L0C_COPY_OUT_CONV) {
            continue;
        }
        if (opIsRetiredMap[c]) {
            continue;
