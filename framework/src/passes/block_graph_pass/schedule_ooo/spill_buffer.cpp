...
const std::string WORKSPACE_BASE_OFFSET = OP_ATTR_PREFIX + "workspace_base_offset";
...
void OoOScheduler::UpdateOpAttr(Operation &op, int opLatency, LogicalTensorPtr spillTensor, 
    ...
    if (op.GetOpcode() == Opcode::OP_COPY_OUT) {
        op.SetAttr(WORKSPACE_BASE_OFFSET, workspaceBaseOffset);
...
Status OoOScheduler::GetWorkspaceBaseOffset(LogicalTensorPtr ddrTensor, int64_t &base) {
    for (auto* producer : ddrTensor->GetProducers()) {
        if (producer->GetOpcode() == Opcode::OP_COPY_OUT) {
            if (producer->GetAttr(WORKSPACE_BASE_OFFSET, base)) {
                return SUCCESS;
            }
        }
    }
...
Status OoOScheduler::SpillParticalBuffer(SpillInfo &spillInfo, IssueEntryPtr allocIssue, IssueEntryPtr assemble, 
    ...
    spillCopyInOp.SetAttr(WORKSPACE_BASE_OFFSET, gmRelatOffset + base);
