...
// 找未被执行的 consumer
void OptimizeSort::GetConsumerGroup(
    std::unordered_set<Operation *> &consumers, std::vector<Operation *> &consumersGroup) {
...
Status OptimizeSort::AllocExecute(Operation *op, std::shared_ptr<std::vector<Operation *>> &curOpList,
    ...
    if (IsBufferFull(
        ...
        APASS_LOG_DEBUG_F(Elements::Operation, "backTraceOp_: %s, backTraceIndex: %zu, memType: %d",
            GetOpInfo(backTraceOp_).c_str(), backTraceOpList_[backTraceOp_].first,
