...
Status MixSubgraphSplit::CalculateSplit(Function &function, std::vector<MixSubgraphInfo> &mixSubgraphs,
    std::set<uint64_t> &mixSubgraphIDsToDelete, std::unordered_map<uint64_t, std::vector<uint64_t>> &mixSubgraphNewIDs,
    std::unordered_map<uint64_t, uint64_t> &programIDRemap) {
    // 计算最终的programID分配
    auto rootFunc = function.rootFunc_;
...
Status MixSubgraphSplit::ExecuteSplit(Function &function, std::vector<MixSubgraphInfo> &mixSubgraphs,
    std::vector<Operation *> callOpsToDelete, std::unordered_map<uint64_t, std::vector<uint64_t>> &mixSubgraphNewIDs,
    std::unordered_map<uint64_t, uint64_t> &programIDRemap) {
    // 执行实际的拆分
...
void MixSubgraphSplit::RecordSplitResult(Function *leafFunc, const std::vector<Function *> &newFunctions,
    const std::vector<uint64_t> &newProgramIDs, const std::vector<InternalComponentInfo> &components, uint64_t mixId,
    ...
    if (existingIt != globalSplitRecords_.end()) {
        ...
        if (existingIt->second.originalLeafFunc == leafFunc) {
            ...
            APASS_LOG_ERROR_F(Elements::Function,
                "Hash collision for function %s (hash: %lu) with existing function %s", leafFunc->GetRawName().c_str(),
                funcHash.GetHash(),
    ...
    for (size_t i = 0; i < components.size(); i++) {
        ...
        APASS_LOG_DEBUG_F(Elements::Function, "  Component[%zu]: internalID=%d, aivCore=%d, ops=%zu", i,
            component.internalSubgraphID, static_cast<int>(component.aivCore), component.operations.size());
    }
}

void MixSubgraphSplit::ApplyFinalDependencies(const std::vector<Function *> &newFunctions,
