...
static void SetTensorParamAddr(Operation &op, std::shared_ptr<LogicalTensor> &tensor,
    ...
    APASS_LOG_INFO_F(Elements::Operation, "BuildParamAddr: Set paramAddr for DDR tensor, "
        "op [%d][%s], isConst=%d, GmTensorParamIdxInCallFunc=%d, gmParamIdx=%d, rawMagic=0x%x.",
        op.GetOpMagic(), op.GetOpcodeStr().c_str(), isConst, GmTensorParamIdxInCallFunc, gmParamIdx, rawMagic);
}
