...
bool SrcDstBufferMergeImpl::IsL1ToL0Transfer(const Operation& op) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, op.GetIOperands().size() == 1 && op.GetOOperands().size() == 1) << "The L1-to-L0 copy op can have only one input and one output tensor";
    return true;
}
...
bool SrcDstBufferMergeImpl::IsL0CToL1Transfer(const Operation& op) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, op.GetIOperands().size() == 1 && op.GetOOperands().size() == 1) << "The L0C-to-L1 copy op can have only one input and one output tensor";
    return true;
}
