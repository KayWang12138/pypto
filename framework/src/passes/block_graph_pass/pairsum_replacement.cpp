...
Status PairSumReplacementPass::RunOnFunction(Function &function) {
    APASS_LOG_INFO_F(Elements::Function, "===> Start PairSumReplacementPass on function: %s", function.GetMagicName().c_str());
    std::cout << "Start PairSumReplacementPass on function: " << function.GetMagicName() << std::endl;
    int replacedCount = 0;
    int totalPairSumOps = 0;
    ...
    for (auto &op : function.Operations(false)) {
        ...
        totalPairSumOps++;
        APASS_LOG_DEBUG_F(Elements::Operation, "Found OP_PAIRSUM[%d] in function %s", 
            op.GetOpMagic(), function.GetMagicName().c_str());
    ...
    APASS_LOG_INFO_F(Elements::Function, "===> End PairSumReplacementPass on function: %s, found %d OP_PAIRSUM, replaced %d operations", 
        function.GetMagicName().c_str(), totalPairSumOps, replacedCount);
    return SUCCESS;
}

bool PairSumReplacementPass::ValidateOperationInputsOutputs(const Operation &op) const {
...
bool PairSumReplacementPass::CheckValidShapeCompatibility(const Operation &op,
...
bool PairSumReplacementPass::ShouldReplacePairSum(const Operation &op) const {
    ...
    if (!ValidateOperationInputsOutputs(op)) {
        return false;
    }
    ...
    // Check valid shape compatibility
    return CheckValidShapeCompatibility(op, input0, input1);
}
