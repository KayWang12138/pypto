...
Status MixCallOperationBuilder::CreateCallOps(Function &rootFunc, const std::vector<Operation *> &originalCallOps,
    Function *originalMixFunc, const std::vector<InternalComponentInfo> &components,
...
Status MixCallOperationBuilder::CreateCallOpInRootFunction(Function &rootFunc, Function &leafFunc,
    uint64_t newProgramID, uint64_t componentIndex, Operation *originalCallOp, Function *originalMixFunc,
    ...
    FindNewIOperandsAndOOperandsInPropagateInOutcast(originalIOperands, originalOOperands, originalIncasts,
        originalOutcasts, actualIncasts, actualOutcasts, newIOperands, newOOperands, processedIncasts,
        processedOutcasts);
...
void MixCallOperationBuilder::FindNewIOperandsInOriginalIncast(const std::vector<LogicalTensorPtr> &originalIOperands,
...
void MixCallOperationBuilder::FindNewOOperandsInOriginalOutcast(const std::vector<LogicalTensorPtr> &originalOOperands,
    ...
    for (const auto &outcast : actualOutcasts) {
        ...
        if (originalIndex >= 0 && originalIndex < static_cast<int>(originalOOperands.size())) {
            ...
            APASS_LOG_DEBUG_F(Elements::Tensor,
                "    Found: propagated outcast tensor magic=%d -> original oOperand[%d]", tensorMagic, originalIndex);
        }
    }
}
...
void MixCallOperationBuilder::FindIOpAttrOffsetAndOOpAttrOffset(Function &leafFunc,
    const SubfuncInvokeInfoTy &invokeInfo, std::vector<int> &iOffsets, std::vector<int> &oOffsets,
...
bool MixCallOperationBuilder::FindOOpAttrOffsetFromActualOutcasts(
    ...
    for (const auto &outcast : actualOutcasts) {
        ...
        APASS_LOG_DEBUG_F(Elements::Tensor,
            "Extracted propagated outcast: tensor rawmagic = %d -> original offset = %d", outcast->GetRawMagic(),
            offset);
    }
    return true;
}
...
void MixCallOperationBuilder::SetCallOpAttribute(Function &leafFunc, Operation &callOp, Operation *originalCallOp,
    ...
    if (callOpAttr != nullptr) {
        ...
        APASS_LOG_DEBUG_F(Elements::Operation,
            "Set wrapId=%lu to callOp attribute for programID=%lu (from original callOp %d)", info.wrapId, newProgramID,
