...
Status MixCallOperationBuilder::CreateCallOpInRootFunction(Function& rootFunc,
                                                           ...
                                                           CallOpCreationInfo& info)
{
    ALOG_DEBUG_F("Creating callOp in root function for leaf: %s, programID=%lu, component=%lu, wrapId=%lu",
    ...
    ALOG_INFO_F("Successfully created callOp %d in root function for programID=%lu, leaf=%s",
                                                 ...
                                                 CallOpCreationInfo& info)
{
    ...
    if (callOpAttr != nullptr) {
        ...
        ALOG_DEBUG_F("Set wrapId=%lu to callOp attribute for programID=%lu (from original callOp %d)",
    ...
    uint64_t wrapId) const
{
    ...
    for (const auto& info : infos) {
        if (info->createdCallOp) {
            ...
            ALOG_DEBUG_F("Wrap %lu: map component %lu -> call op %d",
