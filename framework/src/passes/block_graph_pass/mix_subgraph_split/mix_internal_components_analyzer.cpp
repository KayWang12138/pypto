...
Status MixInternalComponentsAnalyzer::ProcessInternalSubgraphIDs(
    ...
    if (!unassignedOps.empty()) {
        APASS_LOG_INFO_F(Elements::Operation,
            "Found %zu operations without internalSubgraphID, using heuristic analysis", unassignedOps.size());
...
void MixInternalComponentsAnalyzer::ProcessUnassignedOperations(std::vector<Operation *> &unassignedOps,
    std::map<int, std::vector<Operation *>> &componentsByInternalID, Function &mixSubgraphFunc) const {
    ...
    for (auto *syncOp : syncOps) {
        ...
        if (!merged) {
            ...
            APASS_LOG_DEBUG_F(Elements::Operation, "Sync operation %s %d not merged", syncOp->GetOpcodeStr().c_str(),
                syncOp->GetOpMagic());
        }
    }
    // 报告未分配的op
...
Status MixInternalComponentsAnalyzer::DetermineComponentType(
    ...
    for (auto *op : component.operations) {
        ...
        if (op->HasAttribute(OpAttributeKey::isCube)) {
            ...
            if (isCube) {
                APASS_LOG_DEBUG_F(Elements::Operation,
                    "Component %s determined as C_SCOPE (non-sync op %d has isCube=true)", component.suffix.c_str(),
                    op->GetOpMagic());
    ...
    // 如果所有操作都是同步操作
    APASS_LOG_DEBUG_F(Elements::Operation, "Component %s has only sync operations (%zu ops)", component.suffix.c_str(),
...
Status MixInternalComponentsAnalyzer::DetermineComponentAIVCore(
    ...
    switch (componentType) {
        case ComponentType::C_SCOPE: return ProcessCubeScope(operations, componentID);
        case ComponentType::V_SCOPE: return ProcessVecScope(operations, componentID, outAivCore);
        default:
            APASS_LOG_ERROR_F(Elements::Operation,
...
Status MixInternalComponentsAnalyzer::ProcessCubeScope(
    const std::vector<Operation *> &operations, int componentID) const {
...
Status MixInternalComponentsAnalyzer::CheckL0CCopyUBConsumerAIVCoreConsistency(
    Operation *copyOp, int componentID) const {
    if (copyOp == nullptr) {
