...
inline PInstrParam MteMovAlign(InstrName name, PipeId pipe, DataType type, const std::vector<std::string> params) {
    ...
    instr->name = name;
    instr->pipe = pipe;
    instr->param = {static_cast<uint32_t>(type) /* type */, GetParam(params[3]) /* nBurst */,
        GetParam(params[4]) /* lenBurst */, GetParam(params[5]) /* lpad */, GetParam(params[6]) /* rpad */,
...
inline PInstrParam Nd2Nz(DataType type, const vector<string> params) {
    ...
    instr->param = {static_cast<uint32_t>(type) /* type */, GetParam(params[3]) /* ndNum */,
        GetParam(params[4]) /* nValue */, GetParam(params[5]) /* dValue */, GetParam(params[6]) /* srcNdMatrixStride */,
        GetParam(params[7]) /* srcDValue */, GetParam(params[8]) /* dstNzC0Stride */,
        GetParam(params[9]) /* dstNzNStride */, GetParam(params[10]) /* dstNzMatrixStride */};
    return instr;
...
inline PInstrParam FixL0cOut(const vector<string> templates, const vector<string> params) {
    ...
    instr->pipe = PipeId::F;
    instr->param = {GetDataType(templates[0]) /* type */, GetParam(params[3]) /* n */, GetParam(params[4]) /* m */,
        GetParam(params[6]) /* src_std */, GetParam(params[5]) /* dst_std */, 1 /* nd_num */};
    return instr;
...
inline deque<PInstrParam> GetProgram(vector<string> program) {
    ...
    for (auto &funcStr : program) {
        ...
        if (funcName == "set_vector_mask") {
        ...
        } else if (funcName == "set_mask_count" || funcName == "set_mask_norm" || funcName == "set_va_reg_sb" ||
            ...
            SIMULATION_LOGE("ErrCode: F%u, %s not support.",
