...
namespace CostModel
{
    ...
    static bool GenerateCode(const std::string& buffer, std::string fileName)
    {
        ...
        if (!os.is_open()) {
            SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "can't open %s", fileName.c_str());
            return false;
        }
    ...
    std::vector<std::string> RunExeAndCaptureOutput(const std::string& exePath) {
        ...
        if (!pipe) {
            SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "run error");
            return outputLines;
        }
    ...
    uint64_t PipeSimulator<Simulator>::Simulate(const TileOpPtr& tileOp)
    {
        ...
        if (buf == "" || buf == "CODEGEN_ERROR") {
            SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "can't generate buf");
            return 0;
        }
        int notSize = 3;
        ...
        if (!CreateDir(simulatorDir)) {
            SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "can't create simulator dir");
            return 0;
        }
        ...
        if (!success) {
            SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "can't generate code, buf: %s", buf.c_str());
        ...
        if (program.empty()) {
            SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "can't run code, buf: %s", buf.c_str());
