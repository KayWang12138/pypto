...
void ParseInput::ParseJson(std::shared_ptr<CostModel::SimSys> sim, const std::string &jsonPath)
{
    ...
    if (!input.is_open()) {
        SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "Error: fail to open file: %s", jsonPath.c_str());
        return;
    }
    Json j;
    input >> j;
...
void ParseInput::BuildFunction(std::shared_ptr<CostModel::SimSys> sim, npu::tile_fwk::Function *parentFunc, FunctionPtr func)
{
    ...
    ASSERT(ERROR_CODE_UNDEFINED, hasCall || func->opSequenceAfterOOO_.size() == 0 || (func->tileOps.size() == func->opSequenceAfterOOO_.size()))
                                     ...
                                     std::vector<npu::tile_fwk::Function *> &inputFuncs, bool topoFromRootFunc)
{
    if (topoFromRootFunc) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, inputFuncs.size() == 1) << "[SIMULATION]: inputFuncs.size is not equals to 1."
...
void ParseInput::ParseJsonConfig(const std::string &path, std::vector<std::string> &cfg) const
{
    ...
    if (!file.is_open()) {
        SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "Error: fail to open file: %s", path.c_str());
        return;
    }
    Json j;
    file >> j;
...
void ParseInput::ParseConfig(const std::string &path, std::vector<std::string> &cfg) const
{
    ...
    while (std::getline(file, line)) {
        ...
        if (pos != std::string::npos) {
            ...
            SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "Parse Config File Error: %s", line.c_str());
        }
    }
    file.close();
}
        ...
        for (const auto &task : core["tasks"]) {
            ...
            if (task.contains("functionHash")) {
                ...
                if (sim->config.calendarMode == static_cast<uint64_t>(CalendarMode::GLOBAL_COUNTER)) {
                    ASSERT(ERROR_CODE_UNDEFINED, waitVector.size() == 1) << "[SIMULATION]: task has two wait in calendar global counter."
...
void ParseInput::ParseFixedLatencyTask(std::shared_ptr<CostModel::SimSys> sim, std::string const &path)
{
    ...
    for (const auto& item : fixedLatencyTask) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, entry.fixedLatencyVal > 0) << "[SIMULATION]: " << "entry.fixedLatencyVal=" << entry.fixedLatencyVal;
