...
void CostModelAgent::GetFunctionFromJson(const std::string &jsonPath)
{
    ...
    CHECK(ERROR_CODE_UNKNOWN, file.good()) << "[SIMULATION]: " << "Json file: " << jsonPath << " open failed!!!";
    Json jsonData;
    try {
        file >> jsonData;
    } catch (const std::exception &e) {
        CHECK(ERROR_CODE_UNKNOWN, false) << "[SIMULATION]: " << "Json file: " << jsonPath << " parsing error: " << e.what();
    }
