...
void TileOp::GetPipeType()
{
    ...
    if (coreTypeQuery == SCHED_CORE_PIPE_TYPE.end() && !IsCall() && opcode != "LOOP") {
        ASSERT(ERROR_CODE_UNKNOWN, false) << "[SIMULATION]: " << "No pipe type corresponding to opcode is found. opcode=" << opcode;
    }
    if (IsCall()) {
...
uint64_t TileOp::GetAddress()
{
    ...
    if (IsReadCache(pipeType)) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, false) << "[SIMULATION]: " << "PipeType Unrecognized." << Dump() << CorePipeName(pipeType);
    }
