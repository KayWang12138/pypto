...
void CoreMachine::SetCalendar()
{
    if ((GetSim()->config.calendarMode != static_cast<uint64_t>(CalendarMode::DEVICE)) && needSet) {
        ...
        SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "[Cycle: %lu][CoreMachine][SetCalendar] task id %lu SetCalendar pass!!!", static_cast<unsigned long>(GetSim()->GetCycles()), static_cast<unsigned long>(executingTaskId));
    }
}
...
void CoreMachine::StepQueue()
{
    ...
    while (!counterSetQueue.Empty()) {
        ...
        if ((number == calendarSecondSet) && (sim->calendarCounter[sim->taskSetMap[executingTaskId]] !=
            ...
            SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "[Cycle: %lu][CoreMachine][StepQueue] task id %lu after set counter %d calendar value is %d expect value is %d", 
            ...
            SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "[Cycle: %lu][CoreMachine][StepQueue] task id %lu after first set counter %d calendar value is %d expect value is %d", 
