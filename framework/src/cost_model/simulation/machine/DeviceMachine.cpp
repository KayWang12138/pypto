...
void DeviceMachine::RunPVModelDeviceTask()
{
    ...
    taskMap.clear();
    SIMULATION_LOGI("[Cycle: %lu][Device %lu] run pvmodel execute tasks %zu", static_cast<unsigned long>(GetSim()->GetCycles()), static_cast<unsigned long>(machineId), taskMap.size());
}
...
void DeviceMachine::SubmitDeviceTask()
{
    ...
    SIMULATION_LOGW("[Cycle: %lu][Device %lu] submit a new device task to AICPUs, size = %zu", static_cast<unsigned long>(GetSim()->GetCycles()), static_cast<unsigned long>(machineId), 
              taskMap.size());
...
void DeviceMachine::Xfer()
{
    ...
    if (currentHeartModulo < lastHeartModulo) {
        SIMULATION_LOGW("@CostModel Heart Cycle: %lu, submit tasks: %lu", static_cast<unsigned long>(GetSim()->GetCycles()), static_cast<unsigned long>(stats->totalSubmitNum));
    }
