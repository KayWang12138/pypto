...
void SimSys::CalendarDispatchTasksToCore(int key, std::shared_ptr<CoreMachine> coreMachine)
{
    ...
    for (const auto &task : corePacketMap[key]) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, packet.task.taskPtr != nullptr) << "[SIMULATION]: " << "task does not exist. taskId=" << packet.taskId;
...
void SimSys::BuildAICPU(DevicePtr device, uint64_t idInDevice)
{
    ...
    ASSERT(ERROR_CODE_UNKNOWN, config.coreMachineNumberPerAICPU == (aicNum + aivNum)) << "[SIMULATION]: " 
...
{
    deadlock = true;
    SIMULATION_LOGE(npu::tile_fwk::ERROR_CODE_UNDEFINED, "[ReportDeadlock] Machine %zu is deadlock at cycle %lu", machineId, static_cast<unsigned long>(globalCycles));
}
...
void SimSys::OutputLogForPipeSwimLane(std::string prefix)
{
    ...
    if (ret != 0) {
        SIMULATION_LOGE(npu::tile_fwk::ERROR_CODE_UNDEFINED, "cmd error: %s", cmd.c_str());
    }
}
...
void SimSys::UpdateNextCycles(uint64_t nextCycle) {
    ASSERT(ERROR_CODE_UNKNOWN, nextCycle > globalCycles) << "[SIMULATION]: " 
