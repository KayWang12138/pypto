...
public:
    ...
    virtual void OverrideDefaultConfig(std::vector<std::string> *cfgs) final
    {
        ...
        for (auto &c : *cfgs) {
            regex_match(c, sm, r);
            ASSERT(ERROR_CODE_UNDEFINED, sm.size() == parameterNum) << "[SIMULATION]: " << "the config regex size is 3. the format is error: " << c;
    ...
    void ParseConfig(std::string const &cfgName, std::string const &cfgValue)
    {
        if (cfgName.substr(0, prefix.size()) == prefix) {
            ASSERT(ERROR_CODE_UNDEFINED, cfgName[prefix.size()] == '.') << "[SIMULATION]: " << "cfgName format is error: " << cfgName;
            auto it = dispatcher.find(cfgName.substr(prefix.size() + 1));
            if (it != dispatcher.end()) {
                it->second(cfgValue);
            } else {
                SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "Invalid config name: %s", cfgName.c_str());
            }
        }
    }
