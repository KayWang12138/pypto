...
public:
    ...
    uint8_t* AllocHostAddr(uint64_t size) {
        if (size == 0) {
            SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "malloc size is 0!");
            return nullptr;
        }
...
private:
    ...
    void DumpTensorContents(const DeviceKernelArgs &kArgs,
        ...
        SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "[DumpTensor] dumpTensorWsPtr=%p, memory used=%lu\n", dumpTensorWsPtr, dumpTensorWsUsed);
        ...
        auto printIODevAddrs = [&](const std::vector<RawTensorDataPtr> &ptrs) {
            ...
            for (auto &ptr : ptrs) {
                ...
                SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "[DumpTensor] devPtr %d = %lu\n", idx++, devPtr);
                fout.write(reinterpret_cast<const char *>(&devPtr), sizeof(devPtr));
            }
        };

        // write input/output devAddr list
        SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "[DumpTensor] #inputs=%zu\n", inputs.size());
        printIODevAddrs(inputs);
        SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "[DumpTensor] #outputs=%zu\n", outputs.size());
    ...
    void RunPvModel(DeviceKernelArgs &kArgs, const std::vector<RawTensorDataPtr> &inputs, const std::vector<RawTensorDataPtr> &outputs)
    {
        ...
        try {
            ...
            SIMULATION_LOGE(ERROR_CODE_UNDEFINED, "pv init fail.");
            return;
        }
