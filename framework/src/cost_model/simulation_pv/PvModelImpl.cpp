...
void DynPvModelImpl<SystemConfig, CaseConfig>::SetUp(PvModelCceBin *cce, DynFuncData *funcdata, uint64_t opAttrOffset, std::string dir, DynFuncData *dupData)
{
    ...
    if (binName.find("aiv") != std::string::npos) {
        this->subcoreId = static_cast<uint64_t>(1);
    }
    pv_set_toml_((dir + "/spec.toml").c_str());
    pv_init_(0, 0, 1, (dir + std::string("/../pvlog/")).c_str(), 0);
    pv_launch_sub_core_(binAddr, (dir + "/" + binName).c_str(), subcoreId, 0);

    uint64_t hbm_para_start_addr = HBM_SATRT_ADDR;
    uint8_t value_1_ = 1;
    uint8_t *value_1_ptr = &value_1_;
    pv_reg_write_(static_cast<uint32_t>(1), PV_REG_PC, (uint8_t *)&binAddr, subcoreId, 0);
    pv_reg_write_(static_cast<uint32_t>(1), PV_REG_PARA_BASE, (uint8_t *)&hbm_para_start_addr, subcoreId, 0);
    pv_reg_write_(static_cast<uint32_t>(1), PV_REG_BLOCK_DIM, value_1_ptr, subcoreId, 0);
    pv_reg_write_(static_cast<uint32_t>(1), PV_REG_TASK_CFG, value_1_ptr, subcoreId, 0);
...
void DynPvModelImpl<SystemConfig, CaseConfig>::LoadPvConfig(DynFuncData *funcdata, uint64_t opAttrOffset, DynFuncData *dupData, uint64_t hbm_para_start_addr) {
    ...
    pv_mem_write_(0, addr, dup.size(), dup.data(), subcoreId, 0);
    ...
    pv_mem_write_(0, addr, offset.size(), offset.data(), subcoreId, 0);
    para_args.push_back(addr);

    // ref
    pv_mem_write_(0, refAddr, refSize, ref_data.data(), subcoreId, 0);
    ...
    for (size_t i = 0; i < data_.size(); i++) {
        ...
        pv_mem_write_(0, data_[i].devPtr, data_[i].size, tensorData.data(), subcoreId, 0);
    ...
    if (workspace_.size) {
        ...
        pv_mem_write_(0, workspace_.devPtr, workspace_.size, workspaceData.data(), subcoreId, 0);
        para_args.push_back(dupData->stackWorkSpaceAddr);
    }
...
void DynPvModelImpl<SystemConfig, CaseConfig>::RunModel()
{
    ...
    do {
        step_status = static_cast<step_status_t>(pv_step_(PV_STEP_PIPE_ID, subcoreId, 0, 0));
...
void DynPvModelImpl<SystemConfig, CaseConfig>::CopyToHost(uint64_t hostAddr, uint64_t devAddr, uint64_t size)
{
    ...
    for (uint64_t i = 0; i < size; i += MAX_READ_SIZE) {
        ...
        pv_mem_read_(0, devAddr + i, read_size, model_data.data(), subcoreId, 0);
