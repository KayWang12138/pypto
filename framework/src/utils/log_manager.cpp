...
int ParseStrToInt(const char *str) {
    try {
        if (int logLevel = std::stoi(str); logLevel >= 0 && logLevel < 4) {
...
int GetModLogLevel(const char *str) {
    const char *left = strstr(str, kModulePrefix);
    if (left == nullptr) {
        return -1;
    }
    left += strlen(kModulePrefix);
    if (strlen(left) == 0) {
        return -1;
    }
    const char *right = strchr(left, ':');
    if (right == nullptr) {
        return ParseStrToInt(left);
    } else {
        int size = right - left;
        if (size > 0) {
            char ret[10];
            strncpy(ret, left, size);
            ret[size] = '\0';
            return ParseStrToInt(ret);
        } else {
            return -1;
        }
    }
