...
LogManager::LogManager() {
    ...
    if (envGlobalLogLevel != nullptr) {
        std::string envGlobalLogLevelStr = envGlobalLogLevel;
        SetLogLevel(static_cast<LogLevel>(ParseStrToInt(envGlobalLogLevelStr)));
    }
    const char *envModuleLogLevel = getenv(kEnvModuleLogLevel);
    if (envModuleLogLevel != nullptr) {
        std::string envModuleLogLevelStr = envModuleLogLevel;
        SetLogLevel(static_cast<LogLevel>(GetModLogLevel(envModuleLogLevelStr)));
    }
