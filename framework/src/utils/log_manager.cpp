...
const size_t MAX_LOG_FILES_SIZE = 10 * 1024 * 1024;  // 10MB
...
LogManager::LogManager() {
    ...
    if (GetEnvStr(kEnvProcessLogPath, envProcessLogPath)) {
        ...
        if (CreateMultiLevelDirectory(fileDir_)) {
            enableStdOut_ = false;
        } else {
            std::cout << "Fail to create directory: " << fileDir_ << ", still using stdout." << std::endl;
        }
    }
}
...
void LogManager::CreateAndOpenNewLogFile() {
    ...
    oss << kLogFilePrefix << GetTid() << "-" << GetCurrentTimeStr() << kLogFileSuffix;
