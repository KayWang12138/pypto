...
const char *GetPidName() {
     return (__progname != nullptr) ? __progname : "None";
 }
...
void LogManager::ConstructMsgHeader(const LogLevel logLevel, LogMsg &logMsg) {
    int ret = snprintf_s(logMsg.msg, kMsgMaxLen, kMsgMaxLen - 1, "[%s] %s(%lu,%s):%s ",
        GetLogLevelStr(logLevel).c_str(), kModuleName, GetTid(), GetPidName(), GetCurrentTime().c_str());
    if (ret < 0) {
