...
namespace tile::fwk {
namespace {
const int32_t kPyptoModuleId = 76;
...
DLogHandler::DLogHandler() {
    ...
    checkLevelFunc_ = (int32_t(int32_t, int32_t))dlsym(handle_, "CheckLogLevel");
    if (checkLevelFunc_ == nullptr) {
        std::cerr << "Fail to dlsym CheckLogLevel function from " << dLogLibPath << std::endl;
        CloseHandle();
        return;
    }
    logRecordFunc_ = (void(int32_t, int32_t, const char *, ...))dlsym(handle_, "DlogRecord");
...
bool DLogHandler::CheckLogLevel(int32_t level) {
    if (checkLevelFunc_ == nullptr) {
        return false;
    }
    return checkLevelFunc_(kPyptoModuleId, level);
}
void DLogHandler::LogRecord(int32_t level, const char *fmt, va_list list) {
    if (logRecordFunc_ == nullptr) {
        return;
    }
    logRecordFunc_(kPyptoModuleId, level, list);
}
}
