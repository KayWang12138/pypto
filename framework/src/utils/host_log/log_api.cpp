...
PyptoLogFuncInfo::PyptoLogFuncInfo() {
    if (DLogHandler::Instance().IsAvailable()) {
        checkLevel = DLogHandler::Instance().checkLevelFunc_;
        record = DLogHandler::Instance().logRecordFunc_;
    } else {
        checkLevel = PyptoCheckLogLevel;
        record = PyptoLogRecord;
    }
}
