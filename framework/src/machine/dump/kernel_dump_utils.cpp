...
bool KernelDumpUtils::DumpBinFile(const DeviceAgentTask *deviceAgentTask, const std::string &kernelName,
    ...
    if (opBinData.empty()) {
        MACHINE_LOGI("OP binary data is empty, function type is [%s].", function->GetFunctionTypeStr().c_str());
        return true;
    }
...
void KernelDumpUtils::DumpJsonFile(const DeviceAgentTask *deviceAgentTask, const std::string &kernelName, const std::string &dumpDirPath) {
    ...
    MACHINE_LOGI("Work space size is [%lu].", workspaceSize);
    binJson["workspace"] = {
        {"num", 1},
        {"size", {workspaceSize}},
        {"type", {0}}
...
bool KernelDumpUtils::GetBufferFromBinFile(const std::string &binFilePath, std::vector<char> &buffer) {
    ...
    if (realpath(binFilePath.c_str(), resolvedPath) == nullptr) {
        MACHINE_LOGI("Bin file path[%s] is invalid.", binFilePath.c_str());
        return false;
    }
    std::ifstream ifStream(resolvedPath, std::ios::binary | std::ios::ate);
    if (!ifStream.is_open()) {
        MACHINE_LOGI("Fail to open bin file path[%s].", binFilePath.c_str());
        return false;
    }
    try {
        ...
        if (bufferSize <= 0) {
            ifStream.close();
            MACHINE_LOGI("Get stream failed");
            return false;
        }
        if (bufferSize > INT_MAX) {
            ifStream.close();
            MACHINE_LOGI("bufferSize is invalid.");
            return false;
        }
        ...
        ifStream.close();
        MACHINE_LOGE("Failed to read bin file[%s], reason is [%s]", binFilePath.c_str(), e.what());
        return false;
    }
    return true;
}
...
bool KernelDumpUtils::WriteBufferToFatbin(FatbinHeadInfo &fatbinHeadInfo, const std::string &path,
    ...
    if (!fatbinFile.is_open()) {
        MACHINE_LOGE("Failed to open file[%s].", path.c_str());
        return false;
    }
    ...
    if (!fatbinFile.good()) {
        MACHINE_LOGW("Error occurred during writing");
    }
    MACHINE_LOGI("headInfoSize is: %zu, fatbin size is: %ld.", offset, fatbinFile.tellp());
    fatbinFile.close();
    return true;
}
...
bool KernelDumpUtils::GetSubJsonInfo(const std::string &jsonPath, JsonInfo &kernelJsonInfo) {
    ...
    if (realpath(jsonPath.c_str(), resolvedPath) == nullptr) {
        MACHINE_LOGI("realpath failed, val is: %s.", jsonPath.c_str());
        return false;
    }
    Json jsonValue;
    ...
    try {
        ...
        MACHINE_LOGE("Fail to parse json, error:%s, json is %s.", e.what(), jsonValue.dump().c_str());
        return false;
    }
    return true;
}
...
void GetEnv(const char *envName, std::string &envValue) {
    ...
    if ((envTemp == nullptr) || (strnlen(envTemp, envValueMaxLen) >= envValueMaxLen)) {
        MACHINE_LOGI("Env[%s] is not found.", envName);
        return;
    }
    envValue = envTemp;
}
...
void* KernelDumpUtils::LoadTileFwkImplOpLib() {
    ...
    MACHINE_LOGI("Value of env[TILE_FWK_OP_IMPL_PATH] is [%s].", tileFwkLibPath.c_str());
    if (tileFwkLibPath.empty()) {
        MACHINE_LOGW("Value of env[TILE_FWK_OP_IMPL_PATH] is empty.");
        return nullptr;
    }
    void *opLibHandle = dlopen(tileFwkLibPath.c_str(), RTLD_NOW | RTLD_GLOBAL);
    if (opLibHandle == nullptr) {
        MACHINE_LOGI("Failed to dlopen %s, reason is %s.", tileFwkLibPath.c_str(), dlerror());
    }
    return opLibHandle;
}
