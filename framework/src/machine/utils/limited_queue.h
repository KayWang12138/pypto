...
class LimitedQueue
{
  ...
  */
  ...
  {
    memcpy(&_elements[_head], obj, n * sizeof(T));
    _head += n;
  }
   ...
   * \param[in] obj The object to push into the queue.
   */
  inline void push(const T* obj, const size_t n = 1)
  {
    memcpy(&_elements[_head], obj, n * sizeof(T));
    _head += n;
  }
   ...
   */
  inline std::pair<T*, size_t> pop(const size_t n = 1)
  {
    const size_t count = std::min(n, size());
    const size_t curTail = _tail;
    _tail += count;
    return { &_elements[curTail], count };
  }
