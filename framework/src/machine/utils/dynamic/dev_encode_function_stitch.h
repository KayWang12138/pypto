...
static void CellMatchHandle(const uint64_t offset[DEV_SHAPE_DIM_MAX], const uint64_t shape[DEV_SHAPE_DIM_MAX],
    ...
    for (int i = 0; i < cellMatchTableDesc.GetDimensionSize(); ++i) {
        ...
        if(cellMatchShapeDim != 0) {
            ...
            DEV_ERROR(ctrl.task.pre.stitch.cell_match, "dim=%d # cellMatchShapeDim is zero for dimension", i);
            DEV_ASSERT(0);
        }
    }
    ...
    case DEV_SHAPE_DIM_NUM_5:
        ...
        break;
    default:
        DEV_ERROR(ctrl.task.pre.stitch.cell_match, "dimCount=%d # Too many dimension", (int)cellMatchTableDesc.GetDimensionSize());
        break;
    }
}
