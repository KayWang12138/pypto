...
struct DevAscendFunctionDuppedStitch {
    ...
    uint32_t At(uint32_t idx) const {
        DEV_ASSERT_MSG(ProgEncodeErr::STITCH_HANDLE_INDEX_OUT_OF_RANGE, idx < size_, "Index %u exceeds stitch size %u.",
            idx, size_);
...
static bool GetTensorOffsetAndShape(const DevAscendFunction *devFunc, uint64_t offset[DEV_SHAPE_DIM_MAX],
    uint64_t shape[DEV_SHAPE_DIM_MAX], const uint64_t *runtimeExpressionList, int dims, int operationIndex,
    int operandIndex, bool isIOperand = true) {
    ...
    auto validateAndRefreshOffsetShape = [&devFunc, &runtimeExpressionList, &cellMatchTableDesc, &isIOperand](
        ...
        if (paramConcrete) {
            for (int j = 0; j < cellMatchTableDesc.GetDimensionSize(); j++) {
                DEV_VERBOSE_DEBUG("cell match fill, operation[%d] -> dimension[%d] = (offset:%lu ,shape:%lu, "
                                  "rawshape:%lu, cellshape:%d)",
