...
struct DevAscendTensorDataCreator {
    ...
    static void Init(DevTensorData *tensorData, uintdevptr_t tensorAddress, const T *dims, int n) {
        if (n > DEV_SHAPE_DIM_MAX) {
            DEV_ERROR(TensorMetaErrorScene::TENSOR_DIM_COUNT_EXCEEDED, "task.pre.tensor.init",
                      "dimCount=%d maxAllowed=%d # dimension count exceeds maximum allowed",
                      n, DEV_SHAPE_DIM_MAX);
        }
    ...
    static std::vector<int64_t> Encode(const std::vector<DevTensorData> &tensors) {
        ...
        if (ptr != data.data() + data.size()) {
            DEV_ERROR(TensorMetaErrorScene::TENSOR_ENCODE_PTR_MISMATCH, "task.pre.tensor.encode",
                      "ptr=0x%p expected=0x%p # pointer mismatch",
