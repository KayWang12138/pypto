...
public:
    ...
    WsAllocation Allocate() {
        DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, freeListHeader_ != nullptr,
    ...
    void Deallocate(uintdevptr_t ptr) {
        DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, workspaceAddr_ <= ptr && ptr < workspaceAddr_ + slotNum_ * slotStandardMemReq_,
            "Pointer to deallocate is out of range");
        DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, notInUseHeaders_ != nullptr,
