    ...
    StageAllocInfo PopStageAllocMem(bool keepTail, uint32_t memType) {
        ...
        for (uint32_t i = 0; i < SLAB_ALLOCATOR_MAX_CACHES; i++) {
            ...
            if (caches_[i].stageAllocHead == caches_[i].stageAllocTail) {
                ...
                if (temp == nullptr) {
                    DEV_ERROR("[mem_alloc] KeepStageAllocTail failed: stageAllocHead is null for cache index %u.", i);
                }
                ...
                if (temp == nullptr) {
                    DEV_ERROR("[mem_alloc] KeepStageAllocTail failed: stageAllocHead is null after loop for cache index %u, stageAllocTail: %p.", i, caches_[i].stageAllocTail);
                }
                DEV_ASSERT(temp != nullptr);
