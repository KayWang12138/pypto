        ...
        for (uint32_t i = 0; i < SLAB_ALLOCATOR_MAX_CACHES; i++) {
            ...
            if (caches_[i].stageAllocHead == caches_[i].stageAllocTail) {
                ...
                while (*static_cast<void**>(temp) != caches_[i].stageAllocTail) {
                    __PYPTO_TIMEOUT_CHECK(stageAllocTimeoutState, TIMEOUT_NS_10MIN, TIMEOUT_NS_10MIN, ,
                        "workspace.slab.stage: Stage alloc traversal timeout/warning 10min for cacheIndex=%u\n", i);
