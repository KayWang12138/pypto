...
public:
    void init(uint8_t *addr, int size) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, nrSlot_ > 0 && "size too small")<<"nrSlot_= "<<nrSlot_<<", expected >0";
    ...
    void sync(int slotId) {
        ASSERT(ERROR_CODE_UNDEFINED, slotId < nrSlot_)<<"slotId="<<slotId<<" >= nrSlot_="<<nrSlot_;
...
public:
    ...
    void Finish(int64_t slot) {
        ASSERT(ERROR_CODE_UNDEFINED, slot < nrSlot_)<<"slot="<<slot<<" >= nrSlot_="<<nrSlot_;
