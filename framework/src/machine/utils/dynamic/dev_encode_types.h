...
struct DevRelocVector {
    ...
    T &operator[](size_t idx) {
        if (idx >= size_) {
            DEV_ERROR(DataStructErr::DEV_RELOC_VECTOR_INDEX_OOB, "Index out of bounds: idx=%zu, size=%zu", idx, size_);
        }
        DEV_ASSERT(DataStructErr::DEV_RELOC_VECTOR_INDEX_OOB, idx < size_);
        return data_[idx];
    }
...
struct AddressDescriptor {
    ...
    static AddressDescriptor MakeFromRtOutcast(ItemPoolIter iter) {
        DEV_ASSERT_MSG(DataStructErr::DEV_RELOC_VECTOR_INDEX_OOB, (iter & (1ULL << 63)) == 0,
    ...
    uint64_t GetAddress() const {
        DEV_ASSERT_MSG(DataStructErr::DEV_RELOC_VECTOR_INDEX_OOB, IsAddress(),
    ...
    ItemPoolIter GetRtOutcastIter() const {
        DEV_ASSERT_MSG(DataStructErr::DEV_RELOC_VECTOR_INDEX_OOB, IsRtOutcast(),
