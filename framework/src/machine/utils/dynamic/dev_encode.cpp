...
void DevAscendFunction::PopulateOperationEncodedContent(
    ...
    int copyOutResolveSuccIdxSize = 0;
    for (size_t index = 0; index < callList.size(); index++) {
        FillSingleOperationEncodedContent(index, expressionTable, callList, tlist, rawList, callOpSuccDict,
            calleeHashIndexDict, outcastStitchIndexList, copyOutResolveSuccIndexListDict, dupData, operanSize,
            staticAttributeSize, sucSize, copyOutResolveSuccIdxSize);
    }
}

void DevAscendFunction::FillSingleOperationEncodedContent(
        size_t index,
        const SymbolicExpressionTable *expressionTable,
        const OrderedSet<Operation *> &callList,
        const OrderedSet<std::shared_ptr<LogicalTensor>> &tlist,
