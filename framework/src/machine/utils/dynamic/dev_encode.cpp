...
static int EstimatedStitchingCount() {
    uint16_t value = config::GetRuntimeOption<uint16_t>(STITCH_FUNCTION_NUM); 
    if (value > 0) {
        return value <= MAX_CACHED_FUNC_NUM ? value : MAX_CACHED_FUNC_NUM;
    }
...
void EncodeDevAscendProgram(Function *func, uint64_t &offset, DevAscendProgram *base) {
    ...
    if (base == nullptr) {
        ...
        uint16_t value = config::GetRuntimeOption<uint16_t>(STITCH_FUNCTION_NUM); 
        if (value > 0) {
            printf("===TEST=== use STITCH_FUNCTION_NUM:%u.\n", value);
