...
void DevAscendFunction::InitOperationDynamicField(
    ...
    ONFILLCONTENT {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, offset == totalDataSize) << "Offset mismatch:offset " << offset << " != totalDataSize " << totalDataSize;
        ...
        ASSERT(ERROR_CODE_UNDEFINED, dataEnd == dataEndAlloc) << "Pointer mismatch:dataEnd " << dataEnd << " != dataEndAlloc " << dataEndAlloc;
        for (uint64_t i = 0; i < operationSize; i++) {
            uint8_t *ptr = reinterpret_cast<uint8_t *>(&dupData->GetOperationCurrPredCount(i));
            ASSERT(ERROR_CODE_UNDEFINED, dataBegin <= ptr && ptr < incastBegin) << "OperationCurrPredCount out of range:  ptr "
        ...
        for (uint64_t i = 0; i < incastSize; i++) {
            ...
            ASSERT(ERROR_CODE_UNDEFINED, incastBegin <= ptr && ptr < outcastBegin) "Incast address out of range:  ptr "
        ...
        for (uint64_t i = 0; i < outcastSize; i++) {
            ...
            ASSERT(ERROR_CODE_UNDEFINED, outcastBegin <= ptr && ptr < expressionBegin) << "Outcast address out of range:  ptr "
        ...
        for (uint64_t i = 0; i < expressionSize; i++) {
            ...
            ASSERT(ERROR_CODE_UNDEFINED, expressionBegin <= ptr && ptr < stitchBegin) << "Expression address out of range:  ptr "
...
void DevAscendFunction::UpdateRawTensorDesc(const std::shared_ptr<RawTensor> &rawTensor, size_t i, size_t incastRawListSize,
    ...
    case DevIOProperty::NONE:
        ...
        break;
    default:
        ASSERT(ERROR_CODE_UNDEFINED, false) << "Unexpected ioProperty value: " << static_cast<int>(encoded.ioProperty);
        break;
    }
    ...
    ONFILLCONTENT {
        ...
        for (size_t i = 0; i < rawList.size(); i++) {
            ...
            if (rawTensor->actualRawmagic != -1 && rawTensor->actualRawmagic != rawTensor->rawmagic) {
                ...
                ASSERT(ERROR_CODE_UNDEFINED, it != rawMagicToRawTensor.end()) << "rawMagic is not found in rawMagicToRawTensor: " <<
                ...
                if (fromType != toType) {
                    ...
                    ASSERT(ERROR_CODE_UNDEFINED, inSize != 0 && outSize != 0) << "Detected zero byte size data type, fromType: "
                           << static_cast<int>(fromType) << ", toType: " << static_cast<int>(toType);
                    if (inSize > outSize) {
                        ASSERT(ERROR_CODE_UNDEFINED, (rawTensor->GetRawShapeSize() * (inSize / outSize)) == actualRaw->GetRawShapeSize())
                               << "Shape size mismatch: expected " << rawTensor->GetRawShapeSize() * (inSize / outSize)
                               << ", got: " << actualRaw->GetRawShapeSize();
                    } else {
                        ASSERT(ERROR_CODE_UNDEFINED, rawTensor->GetRawShapeSize() == (actualRaw->GetRawShapeSize() * (outSize / inSize)))
                               << "Shape size mismatch: expected " << actualRaw->GetRawShapeSize() * (outSize / inSize)
                               << ", got " << rawTensor->GetRawShapeSize();
                    }
                    ASSERT(ERROR_CODE_UNDEFINED, rawTensor->GetRawDataSize() == actualRaw->GetRawDataSize()) << "Data size mismatch:"
                           << rawTensor->GetRawDataSize() << "!=" << actualRaw->GetRawDataSize();
                    continue;
                }
                ASSERT(ERROR_CODE_UNDEFINED, rawTensor->GetRawShapeSize() == actualRaw->GetRawShapeSize()) << "Shape size mismatch:"
        ...
        for (size_t i = 0; i < rawList.size(); i++) {
            ...
            if (outIncastLinkMap.find(rawList[i]) != outIncastLinkMap.end()) {
                ASSERT(ERROR_CODE_UNDEFINED, outIncastLinkMap[rawList[i]]->actualRawmagic != rawList[i]->rawmagic) << "Unexpected rawmagic match: actualRawmagic "
...
void DevAscendFunction::InitOperation(
    ...
    ONFILLCONTENT {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, noPredOpList_.size() == noPredOpList.size()) << "Size mismatch: noPredOpList size: " <<
        ...
        for (size_t i = 0; i < noPredOpList.size(); i++) {
            ...
            ASSERT(ERROR_CODE_UNDEFINED, !callOpPredDict.count(op) || callOpPredDict.at(op) == 0) << "callOpPredDict for op: " <<
                   op << " is not zero: " << callOpPredDict.at(op);
        }
        ASSERT(ERROR_CODE_UNDEFINED, noSuccOpList_.size() == noSuccOpList.size()) << "Size mismatch: noSuccOpList size: " <<
               noSuccOpList_.size() << " != noSuccOpList size: " << noSuccOpList.size();
        for (size_t i = 0; i < noSuccOpList.size(); i++) {
            int opIdx = At(noSuccOpList_, i);
            auto *op = callList[opIdx];
            ASSERT(ERROR_CODE_UNDEFINED, !callOpSuccDict.count(op) || callOpSuccDict.at(op).empty()) << "callOpSuccDict for op: " <<
                   op << " is not empty";
        }
    }
    ...
    ONFILLCONTENT {
        ...
        for (size_t idx = 0; idx < callList.size(); idx++) {
            ...
            ASSERT(ERROR_CODE_UNDEFINED, callOpPredDict.count(op)) << "callOpPredDict does not contain op " << op;
            ASSERT(ERROR_CODE_UNDEFINED, At(operationList_, idx).depGraphPredCount == callOpPredDict.find(op)->second) << "depGraphPredCount mismatch: expected " <<
            ...
            ASSERT(ERROR_CODE_UNDEFINED, dupData->GetOperationCurrPredCount(idx) == callOpPredDict.find(op)->second) << "GetOperationCurrPredCount mismatch: expected " <<
        ...
        for (size_t index = 0; index < callList.size(); index++) {
            ...
            uint8_t *stitchBegin = &dupData->data_[dupData->operationList_.stitchBase];
            uint8_t *dataEndAlloc = &dupData->data_[duppedDataAllocSize_ - sizeof(DevAscendFunctionDuppedData)];
            ASSERT(ERROR_CODE_UNDEFINED, stitchBegin <= ptr && ptr < dataEndAlloc) << "Address out of range: ptr " << ptr << " not in [" <<
...
struct EncodeDevAscendFunctionInfo {
    ...
    void UpdateCellMatchStrideAndSize(int &cellMatchSize, DevCellMatchTableDesc &cellMatchTableDesc, const std::shared_ptr<LogicalTensor> &tensor, int dim) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, cellMatchStride[0] < MAX_CELLMATCHSSTRIDE) << " Assemble outcast " << tensor->magic << " raw " << tensor->GetRawMagic()
            ...
            for (size_t j = 0; j < callList.size(); j++) {
                ...
                for (size_t k = 0; k < op.GetIOperands().size(); ++k) {
                    ...
                    if (index->tensor->rawmagic == iOperand->tensor->rawmagic) {
                        ASSERT(ERROR_CODE_UNDEFINED, iOperand->GetShape().size() == dimSize) << "Shape size mismatch: expected: " << dimSize << ", got " <<
    ...
    Operation *MakeDummyCall() {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, GetCoreType(dummyOp.get()) == static_cast<int>(CoreType::HUB)) << "GetCoreType return unexpected value: " <<
    ...
    void AddDummyCallsAtBeginningAndEnding(std::vector<Operation *> &callopList) {
        ...
        if (zeroPreds.size() >= OPTIMIZATION_THRESHOLD) {
            ...
            for (auto *op : zeroPreds) {
                ASSERT(ERROR_CODE_UNDEFINED, callOpPredDict[op] == 0) << "callOpPredDict[op] is not zero:" << callOpPredDict[op] << ", expected 0";
                callOpSuccDict[dummyOp].Insert(op);
                callOpPredDict[op] = 1;
            }
            ASSERT(ERROR_CODE_UNDEFINED, callOpSuccDict[dummyOp].size() == zeroPreds.size()) << "callOpSuccDict[dummyOp] size mismatch: expected: " <<
        ...
        if (zeroSuccs.size() >= OPTIMIZATION_THRESHOLD) {
            ...
            for (auto *op : zeroSuccs) {
                ASSERT(ERROR_CODE_UNDEFINED, callOpSuccDict[op].empty()) << "callOpSuccDict[op] is not empty, expect empty";
    ...
    void EncodeZeroPredCount(std::vector<Operation *>& callopList) {
        ...
        for (auto &op : callopList) {
            ...
            ASSERT(ERROR_CODE_UNDEFINED, calleeHashIndexDict.count(calleeHash)) << "calleeHash 0x" << std::hex << calleeHash << " is not found in calleeHashIndexDict";
            int cceIndex = calleeHashIndexDict.find(calleeHash)->second;
            ASSERT(ERROR_CODE_UNDEFINED, cceIndex < static_cast<int>(cceCodeInfoList.size())) << "cceIndex " << cceIndex << " exceeds cceCodeInfoList size: " << cceCodeInfoList.size();

            uint32_t coreType = cceCodeInfoList[cceIndex].coreType;
            ASSERT(ERROR_CODE_UNDEFINED, coreType == static_cast<uint32_t>(CoreType::AIV) || coreType == static_cast<uint32_t>(CoreType::AIC) ||
        ...
        std::sort(callopList.begin(), callopList.end(), [&](Operation *lhs, Operation *rhs) {
            ...
            ASSERT(ERROR_CODE_UNDEFINED, callopCoreTypeDict.count(lhs)) << "lhs operation " << lhs << " is not found in callopCoreTypeDict";
            ASSERT(ERROR_CODE_UNDEFINED, callopCoreTypeDict.count(rhs)) << "rhs operation " << rhs << " is not found in callopCoreTypeDict";
        ...
        for (size_t index = totalZeroPred; index < callopList.size(); index++) {
            ASSERT(ERROR_CODE_UNDEFINED, callOpPredDict[callopList[index]] != 0) << "callOpPredDict[callopList[" << index << "]] is zero, callopList[" << index <<
                   "] = " << callopList[index];
        }
        ...
        for (uint32_t index = 0; index < totalZeroPred; index++) {
            if (callopCoreTypeDict[callopList[index]] == static_cast<uint32_t>(CoreType::AIV)) {
                ...
                ASSERT(ERROR_CODE_UNDEFINED, false) << "Invalid coreType for callopList[" << index << "], op : " << callopList[index];
            }
        }
    }
    ...
    void EncodeCopyOutReslove(std::unordered_map<Operation *, std::unordered_map<Operation *, int>>& producerConsumerOOperandIndexDict) {
        ...
        for (auto &[callop, succSet] : callOpSuccDict) {
            ...
            if (devLeafFunc == nullptr) {
                ASSERT(ERROR_CODE_UNDEFINED, GetCoreType(callop) == static_cast<int>(CoreType::HUB)) << "GetCoreType return unexpected value: " <<
            ...
            copyOutResolveSuccIndexList.push_back(copyOutResolveSuccList.size());
            ASSERT(ERROR_CODE_UNDEFINED, copyOutResolveSuccIndexList[0] == 0) << "copyOutResolveSuccIndexList[0] is " << copyOutResolveSuccIndexList[0] << ", expected 0";
            ...
            ASSERT(ERROR_CODE_UNDEFINED, std::set<Operation *>(succSet.begin(), succSet.end()) == std::set<Operation *>(copyOutResolveSuccList.begin(), copyOutResolveSuccList.end())) <<
            ...
            for (Operation *copyOutResolveSucc : copyOutResolveSuccList) {
                ...
                ASSERT(ERROR_CODE_UNDEFINED, succSet.Insert(copyOutResolveSucc)) << "Duplicate item " << copyOutResolveSucc << " found in copyOutResolveSuccList";
            }
    ...
    EncodeDevAscendFunctionInfo(
        ...
        (void)dyndev;
        ASSERT(ERROR_CODE_UNDEFINED, dyndev->GetDyndevAttribute()->rootTileDict.count(devRoot)) << "devRoot: " << devRoot << " not found in rootTileDict of dyndev";
    ...
    for (size_t d = 0; d < tensorShape.size(); d++) {
        ...
        for (size_t index = 0; index < outcastList.size(); index++) {
            if (index == 0) {
                ...
                ASSERT(ERROR_CODE_UNDEFINED, outcastList[index]->cellMatchTableDesc.GetCellShape(d) == -1) << "Invalid cell shape for outcastList[" << index << "], dimension: " <<
...
void DevAscendProgram::InitPartialUpdateSlot(
    ...
    for (size_t index = 0; index < tPartialUpdateSlotIndexList.size(); index++) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, slotRootOutcastDict.count(slotIndex)) << "slotIndex: " << slotIndex << " not found in slotRootOutcastDict";
        for (auto &[root, outcastIndex] : slotRootOutcastDict.find(slotIndex)->second) {
            ASSERT(ERROR_CODE_UNDEFINED, rootFuncKeyDict.count(root)) << "root: " << root << " not found in rootFuncKeyDict";
...
static int EstimatedStitchingCount() {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, value > 0) << "Invalid value for STITCH_FUNCTION_OUTCAST_MEMORY: " << value << ", must be greater than 0";
    return value;
}
...
static int WorkspaceRecyclePeriod() {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, value > 0) << "Invalid value for STITCH_FUNCTION_INNER_MEMORY: " << value << ", must be greater than 0";
    return value;
}
...
struct EncodeDevAscendProgramInfo {
    ...
    explicit EncodeDevAscendProgramInfo(Function *tfunc) : func(tfunc) {
        ASSERT(ERROR_CODE_UNDEFINED, func->GetDyndevAttribute() != nullptr) << "DyndevAttribute is null for function: " << func;
    ...
    void Init(DevAscendProgram *devProg, bool fillContent) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, reinterpret_cast<uint8_t *>(devProg->controlFlowCache.cacheData.end()) == reinterpret_cast<uint8_t *>(initOffset)) << "controlFlowCache.cacheData.end()"
        ...
        ASSERT(ERROR_CODE_UNDEFINED, devProg->GetSize() == sizeof(*devProg) + devProg->dataSize) << "devProg->GetSize() does not match expected size, expected: " <<
...
static uint64_t CalcUnrolledRootBudget(uint64_t budget, int unrollTimes, int configMultiplier) {
    ASSERT(ERROR_CODE_UNDEFINED, unrollTimes > 0) << "Invalid unrollTimes:  " << unrollTimes << ", must be greater than 0";
...
static SymbolicScalar GetDynRawTensorSize(Function *dynFunc, int funcKey, int idx) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, devRoot) << "func " << funcKey << " missing";

    auto rawTensor = devRoot->GetOutcast()[idx]->GetRawTensor();
    ASSERT(ERROR_CODE_UNDEFINED, !rawTensor->GetDynRawShape().empty()) << "Not dynamic shape tensor";
