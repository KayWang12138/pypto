...
struct EncodeDevAscendFunctionInfo {
    ...
    void InsertProducerConsmerOOperandIndexDict(std::shared_ptr<LeafFuncAttribute> leafAttr, Operation *op, Operation *consumer,
        ...
        for (auto &op : callopList) {
            ...
            for (auto &o : op->GetOOperands()) {
                for (auto &consumer : consumerDict[o]) {
                    ...
                    InsertProducerConsmerOOperandIndexDict(leafAttr, op, consumer);
                }
            }
        }
        for (size_t index = 0; index < callopList.size(); index++) {
    ...
    void BuildCallpList(std::vector<Operation *>& callopList) {
    ...
    EncodeDevAscendFunctionInfo(
        ...
        BuildCallpList(callOpList);
        BuildColorOutGraphAndProducerConsumerOOperandDict(callOpList, producerConsumerOOperandIndexDict);
