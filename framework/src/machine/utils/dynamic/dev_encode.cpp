...
=======
>>>>>>> aa406d74 (fix(machine): Fix some isssues of ctrlflow)
...
static uint32_t ExpectedMaxCachedNum() {
    ...
    if (numStep != 0) {
        return static_cast<uint32_t>(MAX_CACHED_FUNC_NUM);
    }
    int expectedMaxCachedNum = std::max(numInitial, std::max(innerMemAllowedNum, outcastMemAllowedNum));
    if (expectedMaxCachedNum <= 0) {
        return 1;
    }
    expectedMaxCachedNum = (expectedMaxCachedNum > MAX_WRAP_FUNC_NUM) ? expectedMaxCachedNum : MAX_WRAP_FUNC_NUM;
...
static uint64_t CalcUnrolledRootBudget(uint64_t budget, int unrollTimes, int configMultiplier) {
    ...
    int expectedConfigMultiplier = ExpectedMaxCachedNum() * MAX_UNROLL_TIMES;
    if (configMultiplier > expectedConfigMultiplier) {
...
void DevControlFlowCache::Init(void *dyndevAttrPtr,
    ...
    uint64_t slottedCount = dyndevAttr->inoutLink.totalSlot * (stitchMaxFunctionNum + SLOTS_NEED_ALLOC_SIZE);
