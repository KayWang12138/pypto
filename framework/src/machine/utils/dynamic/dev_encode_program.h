...
#define programLastField                              controlFlowCache.cacheData
    ...
    void RuntimeVerify(uintptr_t workspaceBegin, uintptr_t workspaceEnd) const {
        ...
        if ((uintptr_t)data != rangeList[0].begin) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "Assertion failed: data (0x%p) != rangeList[0].begin (0x%p)", data, (void*)rangeList[0].begin);
        }
        DEV_ASSERT(ERROR_CODE_UNDEFINED, (uintptr_t)data == rangeList[0].begin);
        if (rangeList[0].begin > rangeList[0].end) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "Assertion failed: rangeList[0].begin (0x%p) > rangeList[0].end (0x%p)",
                      (void*)rangeList[0].begin, (void*)rangeList[0].end);
        }
        DEV_ASSERT(ERROR_CODE_UNDEFINED, rangeList[0].begin <= rangeList[0].end);
        for (size_t k = 1; k < rangeList.size(); k++) {
            if (rangeList[k - 1].end > rangeList[k].begin) {
                DEV_ERROR(ERROR_CODE_UNDEFINED, "Ranges overlap: range[%d].end (0x%p) > range[%d].begin (0x%p)",
                      (int)(k - 1), (void*)rangeList[k - 1].end,
                      (int)k, (void*)rangeList[k].begin);
            }
            if (rangeList[k].begin > rangeList[k].end) {
                DEV_ERROR(ERROR_CODE_UNDEFINED, "Invalid range: range[%d].begin (0x%p) > range[%d].end (0x%p)",
                      (int)k, (void*)rangeList[k].begin,
                      (int)k, (void*)rangeList[k].end);
            }
            DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, rangeList[k - 1].end <= rangeList[k].begin, "range:%d->%d", (int)(k - 1), (int)(k));
            DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, rangeList[k].begin <= rangeList[k].end, "range:%d", (int)k);
        }
        uintptr_t lastEnd = rangeList.back().end;
        uintptr_t dataEnd = (uintptr_t)(&data[dataSize]);
        if (lastEnd != dataEnd) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "Last range end does not match data end: rangeList.back().end (0x%p) != dataEnd (0x%p)",
                      (void*)lastEnd, (void*)dataEnd);
        }
        DEV_ASSERT(ERROR_CODE_UNDEFINED, lastEnd == dataEnd);
    }
