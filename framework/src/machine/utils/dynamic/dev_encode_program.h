...
#define programLastField                              controlFlowCache.cacheData
    ...
    void RuntimeVerify(uintptr_t workspaceBegin, uintptr_t workspaceEnd) const {
        ...
        if ((uintptr_t)data != rangeList[0].begin) {
            DEV_ERROR(ctrl.program.verify, "data=0x%p rangeBegin=0x%p # Data pointer mismatch", data, (void*)rangeList[0].begin);
        }
        DEV_ASSERT((uintptr_t)data == rangeList[0].begin);
        if (rangeList[0].begin > rangeList[0].end) {
            DEV_ERROR(ctrl.program.verify, "begin=0x%p end=0x%p # Invalid range",
        ...
        for (size_t k = 1; k < rangeList.size(); k++) {
            if (rangeList[k - 1].end > rangeList[k].begin) {
                DEV_ERROR(ctrl.program.verify, "rangeIdx1=%d end=0x%p rangeIdx2=%d begin=0x%p # Ranges overlap",
                      (int)(k - 1), (void*)rangeList[k - 1].end,
                      (int)k, (void*)rangeList[k].begin);
            }
            if (rangeList[k].begin > rangeList[k].end) {
                DEV_ERROR(ctrl.program.verify, "rangeIdx=%d begin=0x%p end=0x%p # Invalid range",
                      (int)k, (void*)rangeList[k].begin,
        ...
        if (lastEnd != dataEnd) {
            DEV_ERROR(ctrl.program.verify, "lastRangeEnd=0x%p dataEnd=0x%p # Data size mismatch",
