...
#define programLastField                              controlFlowCache.cacheData
    ...
    void RuntimeVerify(uintptr_t workspaceBegin, uintptr_t workspaceEnd) const {
        ...
        if ((uintptr_t)data != rangeList[0].begin) {
            DEV_ERROR(ProgEncodeErr::PROGRAM_RANGE_VERIFY_FAILED, "Assertion failed: data (0x%p) != rangeList[0].begin (0x%p)", data, (void*)rangeList[0].begin);
        }
        ...
        if (rangeList[0].begin > rangeList[0].end) {
            DEV_ERROR(ProgEncodeErr::PROGRAM_RANGE_VERIFY_FAILED, "Assertion failed: rangeList[0].begin (0x%p) > rangeList[0].end (0x%p)",
        ...
        for (size_t k = 1; k < rangeList.size(); k++) {
            if (rangeList[k - 1].end > rangeList[k].begin) {
                DEV_ERROR(ProgEncodeErr::PROGRAM_RANGE_VERIFY_FAILED, "Ranges overlap: range[%d].end (0x%p) > range[%d].begin (0x%p)",
                      (int)(k - 1), (void*)rangeList[k - 1].end,
                      (int)k, (void*)rangeList[k].begin);
            }
            if (rangeList[k].begin > rangeList[k].end) {
                DEV_ERROR(ProgEncodeErr::PROGRAM_RANGE_VERIFY_FAILED, "Invalid range: range[%d].begin (0x%p) > range[%d].end (0x%p)",
                      (int)k, (void*)rangeList[k].begin,
        ...
        if (lastEnd != dataEnd) {
            DEV_ERROR(ProgEncodeErr::PROGRAM_RANGE_VERIFY_FAILED, "Last range end does not match data end: rangeList.back().end (0x%p) != dataEnd (0x%p)",
