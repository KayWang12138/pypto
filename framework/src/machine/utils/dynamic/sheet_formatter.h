...
public:
    ...
    void AddRow(Args &&... args) {
        DEV_ASSERT_MSG(DataStructErr::SHEET_COLUMN_MISMATCH, sizeof...(Args) == columnTitles_.size(),
    ...
    void AddRowSeparator(size_t fromColumn = 0, char c = '-') {
        DEV_ASSERT_MSG(DataStructErr::SHEET_COLUMN_INDEX_OUT_OF_RANGE, fromColumn < columnTitles_.size(),
