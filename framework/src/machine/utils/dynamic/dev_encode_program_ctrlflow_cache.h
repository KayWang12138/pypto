...
#define ADDRESS_CACHE_KIND_OUTPUT            3
...
#define CFGCACHE_ALIGN      8
    ...
    static void RelocDescToCache(
            ...
            std::unordered_map<uint64_t, AddressDescriptor> &cacheInputOutputDict, bool isCommKind = false) {
    ...
    void TaskAddrRestoreHcclContext(uint64_t* restoreHcclContextAddr) {
        for (size_t i = 0; i < deviceTaskCount; i++) {
            ...
            for (size_t dupIndex = 0; dupIndex < dynFuncDataList->Size(); ++dupIndex) {
                DynFuncData *dynData = &dynFuncDataList->At(dupIndex);
                (void)memcpy_s(dynData->hcclContext, sizeof(dynData->hcclContext), 
                                restoreHcclContextAddr, sizeof(dynData->hcclContext));
            }
        }
    }
