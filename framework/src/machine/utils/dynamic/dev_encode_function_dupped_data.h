    ...
    const DevAscendFunctionDuppedStitchList &GetOperationStitch(int operationIndex, bool maybeNull = true) const {
        ...
        DEV_IF_NONDEVICE {
            if (!maybeNull && outcastStitchIndex == 0) {
                DEV_ERROR("[func.encode.stitch.outcast] Operation %d has invalid outcast stitch index 0.", operationIndex);
            }
...
struct DevAscendFunctionDupped {
    ...
    void DumpAttr(const DevAscendFunction *func, const SymInt *attrs, const DevAscendOperationOperandInfo &info,
        ...
        if (rawIndex >= func->GetRawTensorSize()) {
            DEV_ERROR("[func.encode.data.valid] Invalid rawIndex: %lu exceeds raw tensor size %lu.", rawIndex, func->GetRawTensorSize());
        }
        if (dim != rawTensor->GetDim()) {
            DEV_ERROR("[func.encode.data.valid] Dimension mismatch: info.dim=%d, rawTensor.dim=%d.", dim, rawTensor->GetDim());
        }
        ...
        for (int d = 0; d < rawTensor->GetDim(); d++) {
            ...
            if (actualShape != shape) {
                DEV_ERROR("[func.encode.data.valid] Shape mismatch at dim %d: expected=%ld, actual=%ld.", d, shape, actualShape);
            }
            DEV_ASSERT(actualShape == shape);
        }
        if (dim != rawTensor->GetDim()) {
            DEV_ERROR("[func.encode.data.valid] Final dimension mismatch: info.dim=%d, rawTensor.dim=%d.", dim, rawTensor->GetDim());
        }
        DEV_ASSERT(dim == rawTensor->GetDim());
        for (int i = 0; i < dim * ARG_ATTR_TYPE; i++) {
    ...
    std::string DumpDyn(int funcIdx, const DevCceBinary *cceBinary) const {
        ...
        for (uint64_t i = 0; i < func->GetRawTensorDescSize(); i++) {
            ...
            if (GetRawTensorAddrEx(i) != GetRawTensorAddr(i)) {
                DEV_ERROR("[func.encode.data.valid] Tensor address mismatch at index %lu: addr=%lu, addrEx=%lu.", i, GetRawTensorAddr(i), GetRawTensorAddrEx(i));
            }
