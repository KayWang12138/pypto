    ...
    const DevAscendFunctionDuppedStitchList &GetOperationStitch(int operationIndex, bool maybeNull = true) const {
        ...
        DEV_IF_NONDEVICE {
            if (!maybeNull && outcastStitchIndex == 0) {
                DEV_ERROR(ctrl.task.pre.stitch.outcast, "operationIndex=%d # Operation has invalid outcast stitch index 0", operationIndex);
            }
...
struct DevAscendFunctionDupped {
    ...
    void DumpAttr(const DevAscendFunction *func, const SymInt *attrs, const DevAscendOperationOperandInfo &info,
        ...
        if (rawIndex >= func->GetRawTensorSize()) {
            DEV_ERROR(ctrl.task.pre.attr.dump, "rawIndex=%lu rawTensorSize=%lu # Invalid rawIndex", rawIndex, func->GetRawTensorSize());
        }
        if (dim != rawTensor->GetDim()) {
            DEV_ERROR(ctrl.task.pre.attr.dump, "infoDim=%d rawTensorDim=%d # Dimension mismatch", dim, rawTensor->GetDim());
        }
        ...
        for (int d = 0; d < rawTensor->GetDim(); d++) {
            ...
            if (actualShape != shape) {
                DEV_ERROR(ctrl.task.pre.attr.dump, "dim=%d expected=%ld actual=%ld # Shape mismatch at dim", d, shape, actualShape);
            }
            DEV_ASSERT(actualShape == shape);
        }
        if (dim != rawTensor->GetDim()) {
            DEV_ERROR(ctrl.task.pre.attr.dump, "infoDim=%d rawTensorDim=%d # Final dimension mismatch", dim, rawTensor->GetDim());
        }
        DEV_ASSERT(dim == rawTensor->GetDim());
        for (int i = 0; i < dim * ARG_ATTR_TYPE; i++) {
    ...
    std::string DumpDyn(int funcIdx, const DevCceBinary *cceBinary) const {
        ...
        for (uint64_t i = 0; i < func->GetRawTensorDescSize(); i++) {
            ...
            if (GetRawTensorAddrEx(i) != GetRawTensorAddr(i)) {
                DEV_ERROR(ctrl.task.pre.func.dump, "index=%lu addr=%lu addrEx=%lu # Tensor address mismatch at index", i, GetRawTensorAddr(i), GetRawTensorAddrEx(i));
            }
