...
struct DevAscendFunctionDupped {
    ...
    void DumpRawShape(DevAscendRawTensor *rawTensor, uint32_t dimSize, std::vector<std::string> &lines,
    ...
    void DumpAttr(const SymInt *attrs, const DevAscendOperationOperandInfo &info, std::stringstream &oss);

    void DumpFuncData(const DevAscendFunction *func, std::stringstream &oss);
