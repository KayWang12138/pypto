    ...
    const DevAscendFunctionDuppedStitchList &GetOperationStitch(int operationIndex, bool maybeNull = true) const {
        ...
        DEV_IF_NONDEVICE {
            if (!maybeNull && outcastStitchIndex == 0) {
                DEV_ERROR(ProgEncodeErr::STITCH_HANDLE_INDEX_OUT_OF_RANGE, "GetOperationStitch: operationIndex=%d has invalid outcast stitch index 0", operationIndex);
            }
...
struct DevAscendFunctionDupped {
    ...
    void DumpAttr(const DevAscendFunction *func, const SymInt *attrs, const DevAscendOperationOperandInfo &info,
        ...
        if (rawIndex >= func->GetRawTensorSize()) {
            DEV_ERROR(TensorMetaErr::RAW_TENSOR_INDEX_OUT_OF_RANGE, "Invalid rawIndex=%lu, exceeds raw tensor size=%lu", rawIndex, func->GetRawTensorSize());
        }
        if (dim != rawTensor->GetDim()) {
            DEV_ERROR(TensorMetaErr::SHAPE_VALUE_MISMATCH, "Dimension mismatch: info.dim=%d, rawTensor->dim=%d", dim, rawTensor->GetDim());
        }
        ...
        for (int d = 0; d < rawTensor->GetDim(); d++) {
            ...
            if (actualShape != shape) {
                DEV_ERROR(TensorMetaErr::SHAPE_VALUE_MISMATCH, "Shape mismatch at dim %d: expacted=%ld, got=%ld", d, shape, actualShape);
            }
            DEV_ASSERT(TensorMetaErr::SHAPE_VALUE_MISMATCH, actualShape == shape);
        }
        if (dim != rawTensor->GetDim()) {
            DEV_ERROR(TensorMetaErr::SHAPE_VALUE_MISMATCH, "Final dimension mismatch after shape validation: info.dim=%d, rawTensor->dim=%d", dim, rawTensor->GetDim());
        }
        DEV_ASSERT(TensorMetaErr::SHAPE_VALUE_MISMATCH, dim == rawTensor->GetDim());
        for (int i = 0; i < dim * ARG_ATTR_TYPE; i++) {
    ...
    std::string DumpDyn(int funcIdx, const DevCceBinary *cceBinary) const {
        ...
        for (uint64_t i = 0; i < func->GetRawTensorDescSize(); i++) {
            ...
            if (GetRawTensorAddrEx(i) != GetRawTensorAddr(i)) {
                DEV_ERROR(TensorMetaErr::TENSOR_ENCODE_PTR_MISMATCH, "Tensor address mismatch at index %lu: addr=%lu, addrEx=%lu.", i, GetRawTensorAddr(i), GetRawTensorAddrEx(i));
            }
