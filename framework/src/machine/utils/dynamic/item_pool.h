...
public:
    ...
    void Init(WsAllocator_T &allocator, size_t count, WsMemCategory category = WsMemCategory::UNCLASSIFIED_ITEMPOOL) {
        DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, !allocator_, "ItemPool has been initialized already");
    ...
    T *Create(Args &&...args) {
        DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, freeListHeadIndex_ != ITEM_POOL_INVALID_INDEX,
    ...
    void Destroy(T *item) {
        ...
        DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, block->freeListNextIndex == ITEM_POOL_NON_FREE_INDEX,
...
private:
    ...
    inline ItemBlock &ItemAt(ItemPoolIter index) {
        DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, index >= 0 && static_cast<size_t>(index) < count_,
