...
public:
    ...
    ~ItemPool() {
        if (allocation_) {
            ...
            DEV_ASSERT(DataStructErr::ITEM_POOL_UNINITIALIZED, allocator_);
    ...
    void Init(WsAllocator_T &allocator, size_t count, WsMemCategory category = WsMemCategory::UNCLASSIFIED_ITEMPOOL) {
        DEV_ASSERT_MSG(DataStructErr::ITEM_POOL_UNINITIALIZED, !allocator_, "ItemPool has been initialized already");
    ...
    T *Create(Args &&...args) {
        DEV_ASSERT_MSG(DataStructErr::ITEM_POOL_FREE_LIST_INVALID, freeListHeadIndex_ != ITEM_POOL_INVALID_INDEX,
    ...
    void Destroy(T *item) {
        ...
        DEV_ASSERT_MSG(DataStructErr::ITEM_POOL_FREE_LIST_INVALID, block->freeListNextIndex == ITEM_POOL_NON_FREE_INDEX,
...
private:
    ...
    inline ItemBlock &ItemAt(ItemPoolIter index) {
        DEV_ASSERT_MSG(DataStructErr::ITEM_POOL_INDEX_OUT_OF_RANGE, index >= 0 && static_cast<size_t>(index) < count_,
