...
private:
    struct MemoryInfo {
        ...
        void DumpError() const {
            ...
            DEV_ERROR(ctrl.task.pre.workspace.dump, "func=%2zu name=%16s rawTensor=%2zu address=0x%" PRIx64 " size=%zu bytes # Workspace dump",
                ...
                switch (VerifyTensorMemoryState(memInfo.ptr, memInfo.size)) {
                    case WsMemoryState::INSIDE:
                        if (!IsValidWsTensor(memInfo.ptr, memInfo.size)) {
                            DEV_ERROR(ctrl.task.pre.workspace.verify, "# Invalid workspace tensor: not completely inside any workspace segment.");
                            memInfo.DumpError();
                            verificationSuccess = false;
                        }
                        break;
                    case WsMemoryState::CROSS_BOUNDARY:
                        DEV_ERROR(ctrl.task.pre.workspace.verify, "# Memory crossing workspace boundary.");
                        memInfo.DumpError();
                        verificationSuccess = false;
                        break;
                    default:
                        if (!inoutAddr.count(memInfo.ptr)) {
                            DEV_ERROR(ctrl.task.pre.workspace.verify, "# Non input/output tensor outside of workspace.");
                            memInfo.DumpError();
                            verificationSuccess = false;
                        }
                        break;
...
public:
    ...
    void CalculateSlabCapacityPerType (uint32_t slabSize, uint32_t* slabCapacity, uint32_t slabTypeNum) {
        if (slabCapacity == nullptr) {
            DEV_ERROR(workspace.init.resource, "# slabCapacity is nullptr.");
            return;
        }
        constexpr uint32_t maxSlabTypes = ToUnderlying(WsAicpuSlabMemType::COHERENT_SLAB_MEM_TYPE_BUTT);
        if (slabTypeNum > maxSlabTypes) {
            DEV_ERROR(workspace.init.check, "slabTypeNum=%u maxAllowed=%u # slabTypeNum exceeds the allowed typenum", slabTypeNum, maxSlabTypes);
            return;
        }
    ...
    WsAllocation SlabAlloc(uint32_t objSize, WsAicpuSlabMemType type) {
        ...
        do {
            ...
            if (submmitTaskSlabMemQueue_.IsEmpty()) {
                ...
                DEV_ERROR(ctrl.task.pre.workspace.slab.alloc, "type=%u objSize=%u # no memory available", ToUnderlying(type), objSize);
...
private:
    ...
    void SlabTryDynAddCache(WsAicpuSlabMemType type, uint32_t objSize) {
        if (type < WsAicpuSlabMemType::COHERENT_SLAB_MEM_TYPE_BUTT) {
            ...
            DEV_ERROR(ctrl.task.pre.workspace.slab.type, "type=%u # Invalid slab memory type", (unsigned int)type);
            DEV_ASSERT(false);
        }
    }
public:
