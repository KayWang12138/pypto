...
private:
    struct MemoryInfo {
        ...
        void DumpError() const {
            ...
            DEV_ERROR("[ctrl.task.pre.workspace.dump]   Func (%2zu) %16s rawTensor[%2zu], @%" PRIx64 " [%zu bytes]%s.",
...
public:
    ...
    void CalculateSlabCapacityPerType (uint32_t slabSize, uint32_t* slabCapacity, uint32_t slabTypeNum) {
        ...
        if (slabTypeNum > maxSlabTypes) {
            DEV_ERROR("[workspace.init.check] slabTypeNum %u exceeds the allowed typenum %u.", slabTypeNum, maxSlabTypes);
            return;
        }
...
private:
    ...
    void SlabTryDynAddCache(WsAicpuSlabMemType type, uint32_t objSize) {
        if (type < WsAicpuSlabMemType::COHERENT_SLAB_MEM_TYPE_BUTT) {
            ...
            DEV_ERROR("[ctrl.task.pre.workspace.slab.type] Invalid slab memory type: %u.", (unsigned int)type);
            DEV_ASSERT(false);
        }
    }
public:
