...
private:
    struct MemoryInfo {
        ...
        void DumpError() const {
            ...
            DEV_ERROR(ERROR_CODE_UNDEFINED, "  Func (%2zu) %16s rawTensor[%2zu], @%" PRIx64 " [%zu bytes]%s.",
...
public:
    void VerifyStitchedListMemory(DevStartArgs &args, const DevAscendFunctionDupped *stitchedList, size_t size) {
                ...
                switch (VerifyTensorMemoryState(memInfo.ptr, memInfo.size)) {
                    case WsMemoryState::INSIDE:
                        if (!IsValidWsTensor(memInfo.ptr, memInfo.size)) {
                            DEV_ERROR(ERROR_CODE_UNDEFINED, "Invalid workspace tensor (not completely inside any workspace segment):");
                            memInfo.DumpError();
                            verificationSuccess = false;
                        }
                        break;
                    case WsMemoryState::CROSS_BOUNDARY:
                        DEV_ERROR(ERROR_CODE_UNDEFINED, "Memory crossing workspace boundary:");
                        memInfo.DumpError();
                        verificationSuccess = false;
                        break;
                    default:
                        if (!inoutAddr.count(memInfo.ptr)) {
                            DEV_ERROR(ERROR_CODE_UNDEFINED, "Non input/output tensor outside of workspace:");
                            memInfo.DumpError();
                            verificationSuccess = false;
                        }
                        break;
        ...
        DEV_ASSERT(ERROR_CODE_UNDEFINED, verificationSuccess);
    }
...
private:
    void AllocateFunctionInnerWorkspace(DevAscendFunctionDupped dup, uint64_t rootInnerMemReq,
        ...
        if (!tensorAllocators_.rootInner.CanAllocate(rootInnerMemReq)) {
            ...
            DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, tensorAllocators_.rootInner.CanAllocate(rootInnerMemReq),
    ...
    void AllocateOutcastWorkspaceForDup(DevAscendFunctionDupped devRootDup,
        ...
        if (outcastMemReq != 0) {
            DEV_ASSERT(ERROR_CODE_UNDEFINED, tensorAllocators_.devTaskInnerExclusiveOutcasts.CanAllocate(outcastMemReq));
    ...
    void AssignIncastAddresses(DevAscendFunctionDupped devRootDup, DeviceExecuteSlot *slotList) {
        ...
        for (size_t i = 0; i < devRootSrc->GetIncastSize(); ++i) {
            DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, devRootSrc->GetIncast(i).fromSlotList.size() > 0,
            ...
            DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, slotList[slotIndex].rtOutcastIter != ITEM_POOL_INVALID_INDEX,
        ...
        for (size_t i = 0; i < devRootSrc->GetOutcastSize(); ++i) {
            ...
            if (outputSlotIndex != -1) {
                ...
                if (slotList[assembleSlotIndex].isAssembleSlotNeedAlloc) {
                    ...
                    DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, slotList[assembleSlotIndex].rtOutcastIter != ITEM_POOL_INVALID_INDEX,
                ...
                DEV_ASSERT(ERROR_CODE_UNDEFINED, incastDesc.IsRtOutcast());
                DEV_ASSERT(ERROR_CODE_UNDEFINED, incastDesc.GetRtOutcastIter() != ITEM_POOL_INVALID_INDEX);
...
public:
    ...
    WsAllocation DebugDumpTensorAllocate(size_t memReq,
        ...
        DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, dumpTensorWsAllocator_.CanAllocate(memReq),
    ...
    RuntimeOutcastTensor &GetRuntimeOutcastTensor(ItemPoolIter iter) {
        DEV_ASSERT(ERROR_CODE_UNDEFINED, iter != ITEM_POOL_INVALID_INDEX);
    ...
    void RuntimeOutcastTensorDeref(ItemPoolIter iter) {
        ...
        DEV_ASSERT(ERROR_CODE_UNDEFINED, outcast.refCnt > 0);
        outcast.refCnt--;
        if (outcast.refCnt == 0) {
    ...
    void RuntimeOutcastTensorRef(ItemPoolIter iter) {
        DEV_ASSERT(ERROR_CODE_UNDEFINED, iter != ITEM_POOL_INVALID_INDEX);
        auto &outcast = runtimeOutcastTensorPool_.At(iter);
        DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, outcast.refCnt > 0, "Shouldn't ref a possibly destroyed tensor, iter=%" PRId64, iter);
        outcast.refCnt++;
    }
...
public:
    ...
    void InitMetadataSlabAllocator() {
        DEV_ASSERT(ERROR_CODE_UNDEFINED, metadataAllocators_.general.FreeMemorySize() > 0);
        ...
        for (size_t i = 0; i < ToUnderlying(WsAicpuSlabMemType::COHERENT_SLAB_MEM_TYPE_BUTT); i++) {
            if (slabMemObjSizeFunc[i] != nullptr) {
                ...
                DEV_ASSERT(ERROR_CODE_UNDEFINED, registCacheRes);
            }
        }
    }
    ...
    void CalculateSlabCapacityPerType (uint32_t slabSize, uint32_t* slabCapacity, uint32_t slabTypeNum) {
        if (slabCapacity == nullptr) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "slabCapacity is nullptr");
            return;
        }
        constexpr uint32_t maxSlabTypes = ToUnderlying(WsAicpuSlabMemType::COHERENT_SLAB_MEM_TYPE_BUTT);
        if (slabTypeNum > maxSlabTypes) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "slabTypeNum exceeds the allowed maxSlabTypes=%u", maxSlabTypes);
            return;
        }
    ...
    WsAllocation SlabAlloc(uint32_t objSize, WsAicpuSlabMemType type) {
        ...
        do {
            ...
            if (submmitTaskSlabMemQueue_.IsEmpty()) {
                ...
                DEV_ERROR(ERROR_CODE_UNDEFINED, "Slab alloc null,type=%u,objsize=%u.", ToUnderlying(type), objSize);
                DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, false, "Slab alloc null,type=%u,objsize=%u.", ToUnderlying(type), objSize);
            }
    ...
    WsAllocation Allocate(uint64_t count, WsMemCategory category) {
        DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, category == WsMemCategory::VECTOR_STITCHED_LIST,
...
private:
    ...
    void InitTensorAllocators(uintdevptr_t workspaceAddr,
        ...
        DEV_ASSERT(ERROR_CODE_UNDEFINED, workspaceAddr <= baseAddr && baseAddr <= workspaceAddr + tensorWorkspaceSize);
    }
    ...
    uint32_t CalcAicpuMetaSlabAlloctorSlabPageSize(uint32_t totalMemSize) {
        ...
        DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, leastSlabReqMem < totalMemSize,
    ...
    void InitAicpuStitchSlabAllocator(void* memBase, uint32_t totalSize) {
        DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, memBase != nullptr && totalSize > 0,
        ...
        for (size_t i = ToUnderlying(WsAicpuSlabMemType::COHERENT_SLAB_MEM_TYPE_BUTT) + 1;
            ...
            if (slabMemObjSizeFunc[i] != nullptr) {
                ...
                DEV_ASSERT_MSG(ERROR_CODE_UNDEFINED, slabSize > objSize, "slabSize=%u <= objSize=%u", slabSize, objSize);
                [[maybe_unused]] bool registCacheRes =
    ...
    void SlabTryDynAddCache(WsAicpuSlabMemType type, uint32_t objSize) {
        if (type < WsAicpuSlabMemType::COHERENT_SLAB_MEM_TYPE_BUTT) {
            ...
            DEV_ERROR(ERROR_CODE_UNDEFINED, "Invalid slab memory type: %u", (unsigned int)type);
            DEV_ASSERT(ERROR_CODE_UNDEFINED, false);
        }
    }
public:
