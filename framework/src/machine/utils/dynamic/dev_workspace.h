                ...
                switch (VerifyTensorMemoryState(memInfo.ptr, memInfo.size)) {
                    case WsMemoryState::INSIDE:
                        if (!IsValidWsTensor(memInfo.ptr, memInfo.size)) {
                            DEV_ERROR(WsErr::WS_TENSOR_ADDRESS_OUT_OF_RANGE, "Invalid workspace tensor (not completely inside any workspace segment):");
                            memInfo.DumpError();
                            verificationSuccess = false;
                        }
                        break;
                    case WsMemoryState::CROSS_BOUNDARY:
                        DEV_ERROR(WsErr::WS_TENSOR_ADDRESS_OUT_OF_RANGE, "Memory crossing workspace boundary:");
                        memInfo.DumpError();
                        verificationSuccess = false;
                        break;
                    default:
                        if (!inoutAddr.count(memInfo.ptr)) {
                            DEV_ERROR(WsErr::WS_TENSOR_ADDRESS_OUT_OF_RANGE, "Non input/output tensor outside of workspace:");
                            memInfo.DumpError();
                            verificationSuccess = false;
                        }
                        break;
...
public:
    ...
    void CalculateSlabCapacityPerType (uint32_t slabSize, uint32_t* slabCapacity, uint32_t slabTypeNum) {
        ...
        if (slabTypeNum > maxSlabTypes) {
            DEV_ERROR(WsErr::SLAB_TYPE_INVALID, "slabTypeNum exceeds the allowed maxSlabTypes=%u", maxSlabTypes);
            return;
        }
    ...
    WsAllocation SlabAlloc(uint32_t objSize, WsAicpuSlabMemType type) {
        ...
        do {
            ...
            if (submmitTaskSlabMemQueue_.IsEmpty()) {
                ...
                DEV_ERROR(WsErr::SLAB_ADD_CACHE_FAILED, "Slab alloc null,type=%u,objsize=%u.", ToUnderlying(type), objSize);
