...
private:
    struct MemoryInfo {
        ...
        void DumpError() const {
            ...
            DEV_ERROR("[workspace.dump]   Func (%2zu) %16s rawTensor[%2zu], @%" PRIx64 " [%zu bytes]%s.",
                ...
                switch (VerifyTensorMemoryState(memInfo.ptr, memInfo.size)) {
                    case WsMemoryState::INSIDE:
                        if (!IsValidWsTensor(memInfo.ptr, memInfo.size)) {
                            DEV_ERROR("[workspace.data.valid] Invalid workspace tensor: not completely inside any workspace segment.");
                            memInfo.DumpError();
                            verificationSuccess = false;
                        }
                        break;
                    case WsMemoryState::CROSS_BOUNDARY:
                        DEV_ERROR("[workspace.data.valid] Memory crossing workspace boundary.");
                        memInfo.DumpError();
                        verificationSuccess = false;
                        break;
                    default:
                        if (!inoutAddr.count(memInfo.ptr)) {
                            DEV_ERROR("[workspace.data.valid] Non input/output tensor outside of workspace.");
                            memInfo.DumpError();
                            verificationSuccess = false;
                        }
                        break;
    ...
    WsAllocation SlabAlloc(uint32_t objSize, WsAicpuSlabMemType type) {
        ...
        do {
            ...
            if (submmitTaskSlabMemQueue_.IsEmpty()) {
                ...
                DEV_ERROR("[workspace.alloc.slab] type=%u, objSize=%u, no memory available.", ToUnderlying(type), objSize);
