...
public:
    ...
    void InitMetadataSlabAllocator() {
        ...
        uint64_t CalcMetadataItemPoolMemSize() {
        constexpr size_t kRuntimeOutcastTensorSize = sizeof(RuntimeOutcastTensor);
        constexpr size_t kItemPoolIterSize = sizeof(ItemPoolIter);
        size_t kItemBlockSize = AlignUp(
        kRuntimeOutcastTensorSize + kItemPoolIterSize,
        alignof(ItemPool<RuntimeOutcastTensor>::ItemBlock));
        DEV_DEBUG("ItemBlockSize is: %zu, OutcastPoolSize is %u", 
                   kItemBlockSize, devProg_->runtimeOutcastPoolSize);
        uint64_t ItemPoolMemSize = kItemBlockSize * devProg_->runtimeOutcastPoolSize;
        return ItemPoolMemSize;
    }

    uint64_t CalcMetadataVectorMemSize() {
        // 1. symbolTable
        uint64_t symbolTableCapacity = CalculateVectorCapacity(devProg_->symbolTable.size());
        uint64_t symbolTableMemory = symbolTableCapacity * sizeof(int64_t);
        DEV_DEBUG("symbolTableMemory is %lu,", symbolTableMemory);
        // 2. slotList_
        uint64_t slotListCapacity = CalculateVectorCapacity(devProg_->slotSize);
        ...
        uint64_t slotInfosCapacity = CalculateVectorCapacity(devProg_->slotSize);
        ...
        uint64_t BoundaryOutcastToFreeListSize = CalculateVectorCapacity(devProg_->memBudget.tensor.devTaskBoundaryOutcastNum);
    ...
    uint64_t CalcMetadataSlotAllocatorMemSize() {
        size_t BlockHeaderSize = sizeof(WsSlotAllocator::BlockHeader);
        uint64_t slotNum_ = devProg_->memBudget.tensor.devTaskBoundaryOutcastNum;
        DEV_DEBUG("slotNum of boundary outcast is %lu", slotNum_);
        return slotNum_ * BlockHeaderSize;
    }
