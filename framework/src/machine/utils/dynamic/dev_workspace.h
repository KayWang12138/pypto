...
private:
    struct MemoryInfo {
        ...
        void DumpError() const {
            ...
            DEV_ERROR(WorkspaceErrorScene::SLAB_CAPACITY_CALC_INVALID, "ctrl.task.pre.workspace.dump",
                ...
                switch (VerifyTensorMemoryState(memInfo.ptr, memInfo.size)) {
                    case WsMemoryState::INSIDE:
                        if (!IsValidWsTensor(memInfo.ptr, memInfo.size)) {
                            DEV_ERROR(WorkspaceErrorScene::WS_TENSOR_ADDRESS_OUT_OF_RANGE, "ctrl.task.pre.workspace.verify",
...
public:
    ...
    void CalculateSlabCapacityPerType (uint32_t slabSize, uint32_t* slabCapacity, uint32_t slabTypeNum) {
        if (slabCapacity == nullptr) {
            DEV_ERROR(WorkspaceErrorScene::WORKSPACE_INIT_RESOURCE_ERROR, "workspace.init.resource",
        ...
        if (slabTypeNum > maxSlabTypes) {
            DEV_ERROR(WorkspaceErrorScene::WORKSPACE_INIT_PARAM_INVALID, "workspace.init.check",
    ...
    WsAllocation SlabAlloc(uint32_t objSize, WsAicpuSlabMemType type) {
        ...
        do {
            ...
            if (submmitTaskSlabMemQueue_.IsEmpty()) {
                ...
                DEV_ERROR(WorkspaceErrorScene::SLAB_ADD_CACHE_FAILED, "ctrl.task.pre.workspace.slab.alloc",
...
private:
    ...
    void SlabTryDynAddCache(WsAicpuSlabMemType type, uint32_t objSize) {
        if (type < WsAicpuSlabMemType::COHERENT_SLAB_MEM_TYPE_BUTT) {
            ...
            DEV_ERROR(WorkspaceErrorScene::SLAB_TYPE_INVALID, "ctrl.task.pre.workspace.slab.type",
