...
std::string DevAscendFunctionDupped::DumpDyn(int funcIdx, int operIdx, const DevCceBinary *cceBinary) {
...
void DevAscendFunctionDupped::DumpRawShape(DevAscendRawTensor *rawTensor, uint32_t dimSize,
                                           std::vector<std::string> &lines, std::stringstream &oss) {
    oss << "        rawShape=[";
    bool isFirstDim = true;
    for (uint32_t i = 0; i < dimSize; i++) {
        if (isFirstDim) {
...
// Return result lines
std::vector<std::string> DevAscendFunctionDupped::DumpLeafs(uint32_t seqNo, uint32_t funcIdx) {
...
void DevAscendFunctionDupped::DumpAttr(const DevAscendFunction *func, const SymInt *attrs,
                                       const DevAscendOperationOperandInfo &info, std::stringstream &oss) {
...
void DevAscendFunctionDupped::DumpFuncData(const DevAscendFunction *func, std::stringstream &oss) {
    oss << "#funcData: [\n" << std::dec;
...
std::string DevAscendFunctionDupped::DumpDyn(int funcIdx, const DevCceBinary *cceBinary) {
    std::stringstream oss;
    auto func = GetSource();
    for (size_t opIdx = 0; opIdx < DupData()->GetSource()->GetOperationSize(); opIdx++) {
