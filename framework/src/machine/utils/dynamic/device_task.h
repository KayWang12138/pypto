...
struct DynDeviceTask : DynDeviceTaskBase {
    ...
    void DumpLeafs() {
        for (size_t funcIdx = 0; funcIdx < stitchedList.size(); funcIdx++) {
            ...
            for (auto &&line : lines) {
                DEV_ERROR("[ctrl.task.dump.leafs] %s", line.c_str());
            }
        }
    }
    ...
    void DumpTensorAddrInfo(uintdevptr_t dumpTensorWsAddr, uint64_t dumpTensorWsSize) {
        ...
        DEV_ERROR("[ctrl.task.dump.tensor] seqNo,taskId,rawMagic,address,dtype,bytesOfDtype,(shapes,)");
        DEV_ERROR("[ctrl.task.dump.tensor] >>>");
        for (auto &info : infos) {
            DEV_ERROR("[ctrl.task.dump.tensor] %s", info.c_str());
        }
        DEV_ERROR("[ctrl.task.dump.tensor] <<<");
    }
#endif
