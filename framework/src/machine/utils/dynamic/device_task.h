...
struct DynDeviceTask : DynDeviceTaskBase {
    ...
    void DumpLeafs() {
        for (size_t funcIdx = 0; funcIdx < stitchedList.size(); funcIdx++) {
            ...
            for (auto &&line : lines) {
                DEV_ERROR(ctrl.task.pre.leafs.dump, "data=%s # Leafs dump", line.c_str());
            }
        }
    }
    ...
    void DumpTensorAddrInfo(uintdevptr_t dumpTensorWsAddr, uint64_t dumpTensorWsSize) {
        ...
        DEV_ERROR(ctrl.task.pre.tensor.dump, "# seqNo,taskId,rawMagic,address,dtype,bytesOfDtype,(shapes,)");
        DEV_ERROR(ctrl.task.pre.tensor.dump, "# >>>");
        for (auto &info : infos) {
            DEV_ERROR(ctrl.task.pre.tensor.dump, "data=%s # Tensor dump", info.c_str());
        }
        DEV_ERROR(ctrl.task.pre.tensor.dump, "# <<<");
    }
#endif
