...
struct DynDeviceTask : DynDeviceTaskBase {
    ...
    void DumpLeafs() {
        for (size_t funcIdx = 0; funcIdx < stitchedList.size(); funcIdx++) {
            ...
            for (auto &&line : lines) {
                DEV_ERROR(TensorMetaError::TENSOR_DUMP_INFO_INCONSISTENT, "ctrl.task.pre.leafs.dump",
    ...
    void DumpTensorAddrInfo(uintdevptr_t dumpTensorWsAddr, uint64_t dumpTensorWsSize) {
        ...
        DEV_ERROR(TensorMetaError::TENSOR_DUMP_INFO_INCONSISTENT, "ctrl.task.pre.tensor.dump",
