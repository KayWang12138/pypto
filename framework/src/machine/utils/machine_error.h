...
enum class CtrlErr : uint32_t {
    ...
    SYMBOL_HANDLER_INVALID    = 0x7300AU, // SymbolHandlerId 非法
    SYMBOL_HANDLER_NOT_FOUND  = 0x7300BU, // handler 未找到
    COMM_GROUP_INDEX_INVALID  = 0x7300CU, // 通信组索引非法
    MEM_TYPE_INVALID          = 0x7300DU, // 内存类型非法
    WIN_SIZE_EXCEEDED         = 0x7300EU, // window size 超限
...
enum class WsErr : uint32_t {
    ...
    WORKSPACE_ITER_INVALID      = 0x74008U, // workspace 迭代器非法
    WORKSPACE_REFCOUNT_INVALID  = 0x74009U, // 引用计数非法
    WORKSPACE_ALLOCATOR_REGIST_FAILED = 0x7400AU, // allocator 注册失败
    WORKSPACE_CATEGORY_INVALID  = 0x7400BU, // category 非法
    WORKSPACE_CAPACITY_INSUFFICIENT = 0x7400CU, // 容量不足
    WORKSPACE_BASE_ADDR_OUT_OF_RANGE = 0x7400DU, // 基址越界
...
enum class DumpDfxErr : uint32_t {
    ...
    DUMP_SYMBOL_MISSING          = 0x75008U, // dump 符号缺失
...
enum class TensorMetaErr : uint32_t {
    ...
    INCAST_ADDRESS_NULL         = 0x77006U, // incast 地址为空
    OUTCAST_ADDRESS_NULL        = 0x77007U, // outcast 地址为空
    RUNTIME_WORKSPACE_NULL      = 0x77008U, // runtime workspace 为空
...
enum class ThreadErr : uint32_t {
    ...
    THREAD_ALLOC_TIMEOUT    = 0x79004U, // 线程分配超时
    THREAD_CPU_ALLOC_FAILED = 0x79005U, // CPU 分配线程失败
    SERVER_INIT_FAILED      = 0x79006U, // server 初始化失败
...
enum class DataStructErr : uint32_t {
    ...
    VECTOR_UNINITIALIZED       = 0x7A003U, // vector 未初始化
    VECTOR_INDEX_OUT_OF_RANGE  = 0x7A004U, // vector 索引越界
    VECTOR_EMPTY_ACCESS        = 0x7A005U, // 空 vector 访问 front/back
    ITEM_POOL_UNINITIALIZED    = 0x7A006U, // item pool 未初始化
    ITEM_POOL_FREE_LIST_INVALID = 0x7A007U, // freelist 非法
    ITEM_POOL_INDEX_OUT_OF_RANGE = 0x7A008U, // item pool 索引越界
    SHEET_COLUMN_MISMATCH      = 0x7A009U, // 列数量不匹配
    SHEET_COLUMN_INDEX_OUT_OF_RANGE = 0x7A00AU, // 列索引越界
