...
enum class HostBackEndErr : uint32_t {
    ...
    LINK_FAILED              = ToUnderlying(MachineError::HOST_BACKEND) + 0x04U, // 链接 core machine 命令失败
    GEN_AICORE_FILE_FAILED   = ToUnderlying(MachineError::HOST_BACKEND) + 0x07U, // 生成 AICore 源码失败
    GEN_DYNAMIC_OP_FAILED    = ToUnderlying(MachineError::HOST_BACKEND) + 0x0BU, // 生成动态算子失败
    PRECOMPILE_FAILED        = ToUnderlying(MachineError::HOST_BACKEND) + 0x0CU, // 算子预编译失败
    FUNCTION_CACHE_HASH_MISS = ToUnderlying(MachineError::HOST_BACKEND) + 0x0EU, // 函数缓存 hash 未命中/不一致
    DUPLICATE_LEAF_FUNC_HASH = ToUnderlying(MachineError::HOST_BACKEND) + 0x11U, // leaf 函数 hash 重复
};

enum class HostLauncherErr : uint32_t {
    LAUNCH_AICPU_FAILED                 = ToUnderlying(MachineError::HOST_LAUNCHER) + 0x08U, // 启动 AICPU 任务失败
    LAUNCH_PREPARE_FAILED               = ToUnderlying(MachineError::HOST_LAUNCHER) + 0x09U, // launch 准备阶段失败
    LAUNCH_CUSTOM_AICPU_FAILED          = ToUnderlying(MachineError::HOST_LAUNCHER) + 0x0AU, // 启动 custom AICPU 失败
    LAUNCH_AICORE_FAILED                = ToUnderlying(MachineError::HOST_LAUNCHER) + 0x0BU, // 启动 AICore 失败
    LAUNCH_BUILTIN_OP_NULL_FAILED       = ToUnderlying(MachineError::HOST_LAUNCHER) + 0x0CU, // 内置算子句柄/函数为空
    REGISTER_KERNEL_FAILED              = ToUnderlying(MachineError::HOST_LAUNCHER) + 0x0EU, // 注册 kernel bin 失败
    PREPARE_ARGS_FAILED                 = ToUnderlying(MachineError::HOST_LAUNCHER) + 0x10U, // kernel 参数准备失败
    MAP_REG_ADDR_FAILED                 = ToUnderlying(MachineError::HOST_LAUNCHER) + 0x23U, // 寄存器地址映射失败
    MEM_POOL_CHECK_ALL_SENTINELS_FAILED = ToUnderlying(MachineError::HOST_LAUNCHER) + 0x2CU, // 哨兵校验失败
    TRIPLE_STREAM_ERROR                 = ToUnderlying(MachineError::HOST_LAUNCHER) + 0x3CU, // 三级流水报错
    SYNC_FAILED                         = ToUnderlying(MachineError::HOST_LAUNCHER) + 0x3DU, // 同步失败
};

enum class SchedErr : uint32_t {
    TASK_WAIT_TIMEOUT        = ToUnderlying(MachineError::SCHEDULE) + 0x02U, // AIC 任务等待超时
    HANDSHAKE_TIMEOUT        = ToUnderlying(MachineError::SCHEDULE) + 0x06U, // 线程握手超时
    READY_QUEUE_OVERFLOW     = ToUnderlying(MachineError::SCHEDULE) + 0x07U, // 就绪队列溢出
    CORE_TASK_EXEC_FAILED    = ToUnderlying(MachineError::SCHEDULE) + 0x08U, // core 任务执行返回错误
    CORE_TASK_PROCESS_FAILED = ToUnderlying(MachineError::SCHEDULE) + 0x10U, // core 任务处理失败
};
...
enum class CtrlErr : uint32_t {
    ...
    DEVICE_TASK_BUILD_FAILED = ToUnderlying(MachineError::CONTROL_FLOW) + 0x05U, // device task 构建失败
    TASK_STATS_ABNORMAL      = ToUnderlying(MachineError::CONTROL_FLOW) + 0x09U, // 任务统计异常
    CTRL_INIT_FAILED         = ToUnderlying(MachineError::CONTROL_FLOW) + 0x0FU, // 控制流初始化失败
    CTRL_SIM_FAILED          = ToUnderlying(MachineError::CONTROL_FLOW) + 0x10U, // 模拟控制流失败
};
...
enum class WsErr : uint32_t {
    ...
    WORKSPACE_ITER_INVALID            = ToUnderlying(MachineError::WORKSPACE) + 0x08U, // workspace 迭代器非法
    WORKSPACE_REFCOUNT_INVALID        = ToUnderlying(MachineError::WORKSPACE) + 0x09U, // 引用计数非法
    WORKSPACE_ALLOCATOR_REGIST_FAILED = ToUnderlying(MachineError::WORKSPACE) + 0x0AU, // allocator 注册失败
    WORKSPACE_CATEGORY_INVALID        = ToUnderlying(MachineError::WORKSPACE) + 0x0BU, // category 非法
    WORKSPACE_CAPACITY_INSUFFICIENT   = ToUnderlying(MachineError::WORKSPACE) + 0x0CU, // 容量不足
    WORKSPACE_BASE_ADDR_OUT_OF_RANGE  = ToUnderlying(MachineError::WORKSPACE) + 0x0DU, // 基址越界
};
...
enum class TensorMetaErr : uint32_t {
    ...
    INCAST_ADDRESS_NULL           = ToUnderlying(MachineError::TENSOR_META) + 0x06U, // incast 地址为空
    OUTCAST_ADDRESS_NULL          = ToUnderlying(MachineError::TENSOR_META) + 0x07U, // outcast 地址为空
    RUNTIME_WORKSPACE_NULL        = ToUnderlying(MachineError::TENSOR_META) + 0x08U, // runtime workspace 为空
};

enum class ServerKernelErr : uint32_t {
    KERNEL_EXEC_FAILED = ToUnderlying(MachineError::SERVER_KERNEL) + 0x03U, // kernel 执行函数失败
};

enum class ThreadErr : uint32_t {
    SIGNAL_HANDLER_ABNORMAL = ToUnderlying(MachineError::THREAD_MACHINE) + 0x02U, // 信号处理异常
    RESET_REG_ALL_TRIGGERED = ToUnderlying(MachineError::THREAD_MACHINE) + 0x03U, // 触发全量寄存器复位
    THREAD_CPU_ALLOC_FAILED = ToUnderlying(MachineError::THREAD_MACHINE) + 0x05U, // CPU 分配线程失败
};
