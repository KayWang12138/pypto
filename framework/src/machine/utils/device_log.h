...
#define D_DEV_LOG(level, file, line, function_name, fmt, ...)                                                        \
    do {                                                                                                             \
        DlogRecord(AICPU, level, "[%s:%d] %lu %s" #fmt, file, line, GET_TID(), function_name, ##__VA_ARGS__);        \
    } while (0)

constexpr const int MAX_LOG_SIZE = 8192;
[[maybe_unused]] static thread_local char g_logMsgBuf[MAX_LOG_SIZE] = {0};

static inline void LogChunkedMessage(int level, [[maybe_unused]] const std::string& mode_name,
                                     ...
                                     const char* msg, size_t msg_length)
    {
        ...
        if (msg_length < msgmaxlen) {
            D_DEV_LOG(level, file, line, function_name, "%s", msg);
            return;
        }
        // 处理长消息
        while (msgchunkbegin < msgend) {
            // 跳过开头的换行符
            if (*msgchunkbegin == '\n') {
                D_DEV_LOG(level, file, line, function_name, "");
                msgchunkbegin++;
                continue;
            }
            // 查找下一个换行符
            ...
            while (msgchunkend > msgchunkbegin) {
                ...
                D_DEV_LOG(level, file, line, function_name, "%s", msgchunk.c_str());
...
static inline void D_DEV_LOGF_Function(int level, const std::string& mode_name,
                                       const char* file, int line, const char* function_name, 
                                       const char* fmt, Args... args)
    {
        ...
        // 调用分块处理函数，传入函数名
        LogChunkedMessage(level, mode_name, file, line, function_name, msgbuf, msglength);
    }

#define D_DEV_LOGF(level, MODE_NAME, fmt, ...)                                                              \
    do {                                                                                                    \
        if (CheckLogLevel(AICPU, level) == 1){                                                              \
            D_DEV_LOGF_Function(level, MODE_NAME, __FILE__, __LINE__, __FUNCTION__, fmt, ##__VA_ARGS__);    \
        }                                                                                                   \
    } while (0)
