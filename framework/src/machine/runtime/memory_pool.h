...
struct MemoryBlock {
    ...
    void* Allocate(uint64_t alignSize) {
        if (!is_huge_1g) {
            if (used_size == 0 && block_size >= alignSize) {
                ...
                MACHINE_LOGE_E(DevCommonErr::ALLOC_FAILED,
    ...
    void Free(void* ptr, size_t size) {
        if (!is_huge_1g) {
            MACHINE_LOGE_E(DevCommonErr::FREE_FAILED,
...
public:
    ...
    bool AllocDevAddrInPool(uint8_t **devAddr, uint64_t size) {
        ...
        if (devAddr == nullptr) {
            MACHINE_LOGE_E(DevCommonErr::NULLPTR, "devAddr is nullptr");
            return false;
        }
        ...
        MACHINE_LOGE_E(DevCommonErr::ALLOC_FAILED, "Allocate failed: size=%lu", size);
        return false;
    }
    
    void FreeDevAddr(void* ptr) {
        if (ptr == nullptr) {
            MACHINE_LOGE_E(DevCommonErr::NULLPTR, "Freeing nullptr");
            return; 
        }
        ...
        if (it == addrToBlock_.end()) {
            MACHINE_LOGE_E(DevCommonErr::FREE_FAILED, "Freeing unknown pointer: %p", ptr);
            return;
        }
    ...
    bool CheckSentinel(uint8_t *baseAddr, bool remove = true) {
        ...
        if (iter == sentinelValMap_.end()) {
            MACHINE_LOGE_E(DevCommonErr::PARAM_CHECK_FAILED,
...
private:
    ...
    MemoryBlock* CreateNewBlock(uint64_t alignSize) {
        ...
        MACHINE_LOGE_E(DevCommonErr::ALLOC_FAILED, "All memory alloc strategies failed");
        return nullptr;
    }
