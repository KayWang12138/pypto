...
struct MemoryBlock {
    ...
    void* Allocate(uint64_t alignSize) {
        if (!is_huge_1g) {
            if (used_size == 0 && block_size >= alignSize) {
                ...
                MACHINE_LOGE(ERROR_CODE_UNDEFINED, "Logic Error: 2MB block allocation failed. (used_size: %zu, block_size: %zu, req: %lu)", used_size, block_size, alignSize);
                return nullptr;
            }
        }
    ...
    void Free(void* ptr, size_t size) {
        if (!is_huge_1g) {
            MACHINE_LOGE(ERROR_CODE_UNDEFINED, "Logic Error: 2MB block should not call Free()");
            return;
        }
...
public:
    ...
    bool AllocDevAddrInPool(uint8_t **devAddr, uint64_t size) {
        ...
        if (devAddr == nullptr) {
            MACHINE_LOGE(ERROR_CODE_UNDEFINED, "devAddr is nullptr");
            return false;
        }
        ...
        MACHINE_LOGE(ERROR_CODE_UNDEFINED, "Allocate failed size %lu", size);
        return false;
    }
    
    void FreeDevAddr(void* ptr) {
        if (ptr == nullptr) {
            MACHINE_LOGE(ERROR_CODE_UNDEFINED, "Freeing nullptr");
            return; 
        }
        ...
        if (it == addrToBlock_.end()) {
            MACHINE_LOGE(ERROR_CODE_UNDEFINED, "Freeing unknown pointer: %p", ptr);
            return;
        }
    ...
    bool CheckAllSentinels() {
        ...
        if (!allGood) {
            MACHINE_LOGE(ERROR_CODE_UNDEFINED, "CheckAllSentinels failed.");
        }
    ...
    void PrintSentinelVal(std::vector<uint64_t> &sentinelVal, uint8_t *sentinelAddr) {
        ...
        for (uint32_t i = 0; i < SENTINEL_MEM_SIZE; ++i) {
            ...
            if ((i + 1) % 64 == 0) {
                MACHINE_LOGE(ERROR_CODE_UNDEFINED, "Sentinel Addr:%p Val:[\n%s]", sentinelAddr + i, oss.str().c_str());
                oss.str("");
            }
        }
    }
    ...
    bool CheckSentinel(uint8_t *baseAddr, bool remove = true) {
        ...
        if (iter == sentinelValMap_.end()) {
            MACHINE_LOGE(ERROR_CODE_UNDEFINED, "Base addr %p not found in map, need check code.", baseAddr);
            return false;
        }
        ...
        if (!allGood) {
            MACHINE_LOGE(ERROR_CODE_UNDEFINED, "BaseAddr:%p check sentinel failed.", baseAddr);
        } else {
...
private:
    ...
    MemoryBlock* CreateNewBlock(uint64_t alignSize) {
        ...
        MACHINE_LOGE(ERROR_CODE_UNDEFINED, "All memory alloc strategies failed");
        return nullptr;
    }
