...
public:
    ...
    void PrintSentinelVal(std::vector<uint64_t> &sentinelVal, uint8_t *sentinelAddr) {
        ...
        oss << "Sentinel original val[" << std::hex << "0x" << SENTINEL_VALUE << "]" << std::endl;
        ALOG_ERROR_F("%s", oss.str().c_str());
        oss.str("");
        oss << "Print Sentinel val in hex." << std::endl;
        for (uint32_t i = 0; i < SENTINEL_MEM_SIZE; ++i) {
            oss << std::hex << std::setw(2) << std::setfill('0') << (int)byte_ptr[i];
            if ((i + 1) % 16 == 0) {
                oss << std::endl;
            } else if ((i + 1) % 64 == 0) {
                ALOG_ERROR_F("Sentinel Addr:%p Val: [%s]", sentinelAddr + i, oss.str().c_str());
                oss.str("");
            } else {
                oss << " ";
            }
