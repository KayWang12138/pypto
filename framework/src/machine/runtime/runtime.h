...
public:
    ...
    bool CheckAllSentinels() const {
        if (!needMemCheck_) {
            return true;
        }
        bool allGood = true;
        for (auto &iter : sentinelValMap_) {
            if (!CheckSentinel(iter.first)) {
                allGood = false;
            }
        }
        if (!allGood) {
            ALOG_ERROR_F("BaseAddr:%p check failed", baseAddr);
        }
        return allGood;
    }

    // Check sentinel values for memory corruption
    bool CheckSentinel(uint8_t *baseAddr) const {
        if (!needMemCheck_) {
            return true;
        }
        ...
        for (auto sentinelAddr : sentinelVec) {
            ALOG_ERROR_F("Check base:%p sentinelAddr:%p.", baseAddr, sentinelAddr);
            ...
            for (uint32_t i = 0; i < SENTINEL_NUM; i++) {
                if (sentinelVal[i] != SENTINEL_VALUE) {
                    ALOG_ERROR_F("SentinelAddr:%p check failed at address %p: expected 0x%lx, got 0x%lx", sentinelAddr, 
