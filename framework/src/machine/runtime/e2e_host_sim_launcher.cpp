...
private:
    ...
    void AicoreWorker(size_t logicalId)
    {
        ...
        bool isAic = (static_cast<int64_t>(logicalId) < profile_.blockdim);
        if (isAic) {
            MACHINE_LOGI("AIC (ID: %zu), working for 5us...", logicalId);
            HostSimClock::AdvanceNs(kCallSubFuncTaskNs);
        } else {
            MACHINE_LOGI("AIV (ID: %zu), working for 5us...", logicalId);
            HostSimClock::AdvanceNs(kCallSubFuncTaskNs);
        }
