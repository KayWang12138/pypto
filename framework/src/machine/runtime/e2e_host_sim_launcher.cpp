...
static void DumpE2EHostSimBindingPlan(int nrValidAic, int scheCpuNum, int totalCpuCores, bool enableBind)
{
    ...
    std::cout << "[E2E_HOST_SIM][bind_plan] blockdim=" << blockdim << " aicoreNum=" << aicoreNum
              << " scheCpuNum=" << scheCpuNum << " enableBind=" << enableBind << " cpuBase=" << cpuBase
              << " bindCoreNum=" << bindCoreNum << " aicPerSche=" << aicPerSche << " aivPerSche=" << aivPerSche
              << std::endl;
    MACHINE_LOGI(
    ...
    for (int s = 0; s < scheCpuNum; ++s) {
        ...
        std::cout << "[E2E_HOST_SIM][bind_plan] sche=" << s << " AIC=[" << aicStart << "," << aicEnd << "] AIV=["
                  << aivStart << "," << aivEnd << "]" << std::endl;
        MACHINE_LOGI(
    ...
    for (int s = 0; s < scheCpuNum; ++s) {
        std::cout << "[E2E_HOST_SIM][bind_plan] sche=" << s << " cnt(AIC)=" << aicCnt[static_cast<size_t>(s)]
                  << " cnt(AIV)=" << aivCnt[static_cast<size_t>(s)] << std::endl;
        MACHINE_LOGI(
    ...
    for (int i = 0; i < headN; ++i) {
        ...
        std::cout << "[E2E_HOST_SIM][bind_plan] map blockId=" << i << " -> sche=" << s << " -> cpu=" << cpu
                  << std::endl;
    }
    if (aicoreNum > headN) {
...
static void PrintE2EHostSimLine(const std::string& line) { std::cout << line << std::endl; }
...
static void DumpHostSimAicoreStates(const DevAscendProgram* devProg, int aicoreNum, const char* tag)
{
    if (devProg == nullptr || devProg->devArgs.sharedBuffer == 0 || aicoreNum <= 0) {
        ...
        PrintE2EHostSimLine(std::string("[E2E_HOST_SIM][") + tag + "] sharedBuffer is not ready");
        return;
    }
    ...
    for (int i = 0; i < dumpCoreNum; ++i) {
        ...
        const unsigned long long mainBase =
            static_cast<unsigned long long>(HostRegBus::Global().ReadMainBase(static_cast<size_t>(i)));
        const unsigned long long cond =
            static_cast<unsigned long long>(HostRegBus::Global().ReadCond(static_cast<size_t>(i)));
        MACHINE_LOGI(
            "[E2E_HOST_SIM][%s] core=%d hello=0x%llx stage=%llu mainBase=0x%llx cond=0x%llx", tag, i,
            static_cast<unsigned long long>(hello), static_cast<unsigned long long>(stage),
            static_cast<unsigned long long>(mainBase), static_cast<unsigned long long>(cond));
        std::cout << "[E2E_HOST_SIM][" << tag << "] core=" << i << " hello=0x" << std::hex << hello
                  << " stage=" << std::dec << stage << " mainBase=0x" << std::hex << mainBase << " cond=0x" << cond
                  << std::dec << std::endl;
    }
}
...
static void E2EAicoreWorker(DeviceKernelArgs kArgs, int blockId, int phyId, int bindCpu, int nrValidAic, int scheCpuNum)
{
    ...
    std::cout << "[E2E_HOST_SIM] AICore thread start: blockId=" << blockId << ", bindCpu=" << bindCpu
              << ", schedIdx=" << schedIdx << ", name=" << name << std::endl;
    HostCoreContext ctx;
    ctx.blockId = blockId;
    ctx.phyId = phyId;
...
static int E2EHostSimLaunchOnce(DeviceKernelArgs& kArgs)
{
    ...
    std::cout << "[E2E_HOST_SIM] cfgdata=" << kArgs.cfgdata << " sharedBuffer=0x" << std::hex
              << static_cast<unsigned long long>(devProg->devArgs.sharedBuffer) << " runtimeDataRingBufferAddr=0x"
              << static_cast<unsigned long long>(devProg->devArgs.runtimeDataRingBufferAddr) << " coreRegAddr=0x"
              << static_cast<unsigned long long>(devProg->devArgs.coreRegAddr) << " devDfxArgAddr=0x"
              << static_cast<unsigned long long>(devProg->devArgs.devDfxArgAddr) << std::dec
              << " nrAic=" << static_cast<unsigned int>(devProg->devArgs.nrAic)
              << " nrAiv=" << static_cast<unsigned int>(devProg->devArgs.nrAiv)
              << " nrAicpu=" << static_cast<unsigned int>(devProg->devArgs.nrAicpu)
              << " scheCpuNum=" << static_cast<unsigned int>(devProg->devArgs.scheCpuNum) << std::endl;
    ...
    std::cout << "[E2E_HOST_SIM] Binding config: totalCpuCores=" << totalCpuCores << ", nrValidAic=" << nrValidAic
              << ", scheCpuNum=" << devProg->devArgs.scheCpuNum << ", enableBind=" << enableBind << std::endl;
    ...
    DumpHostSimAicoreStates(devProg, aicoreNum, "after_stop_notify");
    std::this_thread::sleep_for(std::chrono::milliseconds(2));
    DumpHostSimAicoreStates(devProg, aicoreNum, "after_stop_notify_wait");
    ...
    DumpHostSimAicoreStates(devProg, aicoreNum, "after_aicore_join");
