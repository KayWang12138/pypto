...
namespace npu::tile_fwk::dynamic {
    static void PrintE2EHostSimLine(const std::string& line)
    {
        ...
        explicit ScopedHostSimClockTicker(std::chrono::microseconds sleepInterval) : sleepInterval_(sleepInterval)
        {
            worker_ = std::thread([this]() {
                ...
                while (!stop_.load(std::memory_order_relaxed)) {
                    ...
                    auto delta =
                        static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(now - last).count());
                    if (delta > 0) {
    ...
    static void DumpHostSimAicoreStates(const DevAscendProgram* devProg, int aicoreNum, const char* tag)
    {
        ...
        for (int i = 0; i < dumpCoreNum; ++i) {
            ...
            std::cout << "[E2E_HOST_SIM][" << tag << "] core=" << i << " hello=0x" << std::hex << hello
                      << " stage=" << std::dec << stage << " mainBase=0x" << std::hex << mainBase << " cond=0x"
                      << cond << std::dec << std::endl;
        }
    }
    ...
    static void NotifyAicoreStop(const DevAscendProgram* devProg, int aicoreNum)
    {
        ...
        auto* sharedBufferBase = reinterpret_cast<uint8_t*>(static_cast<uintptr_t>(devProg->devArgs.sharedBuffer));
        for (int i = 0; i < aicoreNum; ++i) {
            // Keep protocol semantics: send stop command through mainBase.
            HostRegBus::Global().WriteMainBase(
                static_cast<size_t>(i), static_cast<uint64_t>(AICORE_TASK_STOP) + 1ULL);

            // Wake up waiting core and ask it to exit loop.
            auto* args =
                reinterpret_cast<KernelArgs*>(sharedBufferBase + static_cast<size_t>(i) * SHARED_BUFFER_SIZE);
    ...
    static int E2EHostSimLaunchOnce(DeviceKernelArgs& kArgs) {
        ...
        MACHINE_LOGI(
            ...
            kArgs.cfgdata,
            static_cast<unsigned long long>(devProg->devArgs.sharedBuffer),
            ...
            static_cast<unsigned int>(devProg->devArgs.nrAic),
            static_cast<unsigned int>(devProg->devArgs.nrAiv),
            static_cast<unsigned int>(devProg->devArgs.nrAicpu),
            static_cast<unsigned int>(devProg->devArgs.scheCpuNum));
        std::cout << "[E2E_HOST_SIM] cfgdata=" << kArgs.cfgdata << " sharedBuffer=0x" << std::hex
                  ...
                  << static_cast<unsigned long long>(devProg->devArgs.runtimeDataRingBufferAddr)
                  << " coreRegAddr=0x" << static_cast<unsigned long long>(devProg->devArgs.coreRegAddr)
                  << " devDfxArgAddr=0x" << static_cast<unsigned long long>(devProg->devArgs.devDfxArgAddr)
                  << std::dec << " nrAic=" << static_cast<unsigned int>(devProg->devArgs.nrAic)
