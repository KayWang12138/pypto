...
struct DeviceState {
    ...
    DeviceArgs* GetDevArgsPtr() const { return reinterpret_cast<DeviceArgs*>(static_cast<uintptr_t>(devArgsAddr)); }

    void UpdateLaunchParams(int blockDimVal, int aicpuNumVal)
    {
        blockDim = blockDimVal;
        args.nrValidAic = blockDimVal;
        args.nrAicpu = aicpuNumVal;
        args.scheCpuNum = dynamic::CalcSchAicpuNumByBlockDim(
            static_cast<uint32_t>(blockDimVal), static_cast<uint32_t>(aicpuNumVal), args.archInfo);
    }
    ...
    void UpdateAicpuNumFromPlatform()
    {
        ...
        args.scheCpuNum = dynamic::CalcSchAicpuNumByBlockDim(
            static_cast<uint32_t>(blockDim), static_cast<uint32_t>(aicpuNum), args.archInfo);
    }
};
} // namespace npu::tile_fwk
