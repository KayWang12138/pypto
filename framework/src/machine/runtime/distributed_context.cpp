...
namespace {  
TileOp::CommContext g_hostAddr[DIST_COMM_GROUP_NUM];
...
class TilingStructV2 : public Mc2CommConfigMgrBase {
public:
    TilingStructV2() {}
    ~TilingStructV2() {}
    int32_t MakeMc2TilingStruct(const std::string& groupName) override
...
uint64_t DistributedContext::GetHcclContextV1(uint64_t ctxAddr)
{
    ...
    for (uint32_t i = 0; i < ctxHost->rankNum; i++) {
        ...
        ctxHost->winAddr[i + (1 * ctxHost->rankNum)] = hcclParamhost.windowsOut[i];
        ctxHost->winAddr[i + (2 * ctxHost->rankNum)] = hcclParamhost.windowsExp[i];
    }
...
uint64_t DistributedContext::GetHcclContextV2(uint64_t ctxAddr)
{
    ...
    for (uint64_t i = 0; i < hcclParamhost->rankSize; i++) {
        if (i == hcclParamhost->localUsrRankId) {
            ...
            ctxHost->winAddr[i + (1 * ctxHost->rankNum)] = hcclParamhost->localWindowsOut;
            ctxHost->winAddr[i + (2 * ctxHost->rankNum)] = hcclParamhost->localWindowsExp;
            continue;
        }
    ...
    aclrtMemcpy(ctxDevice, izeof(TileOp::CommContext) + sizeof(uint64_t) * hcclParamhost->rankSize * winTypeNum, 
...
std::vector<uint64_t> DistributedContext::GetCommContext(const std::vector<std::string> &groupNames)
{
    ...
    std::shared_ptr<TilingStruct> tilingStruct = (topoType == COMM_MESH)
        ? std::make_shared<TilingStructV1>()
        : std::make_shared<TilingStructV2>();
    ...
    for (size_t groupIndex = 0; groupIndex < groupNames.size(); ++groupIndex) {
        ...
        if (topoType == COMM_MESH) {
           CommContext[groupIndex] = GetHcclContextV1(hcclContext[groupIndex]);
        } else {
           CommContext[groupIndex] = GetHcclContextV2(hcclContext[groupIndex]);
        }
