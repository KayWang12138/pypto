...
{
#ifdef BUILD_WITH_CANN
    TilingStructBase commConfig = make_shared<TilingStruct>();
    ...
    for (size_t groupIndex = 0; groupIndex < groupNames.size(); ++groupIndex) {
        ...
        ASSERT(ret == 0);
        bool makeTilingSuccess = commConfig->MakeMc2TilingStruct(groupName);
        ASSERT(makeTilingSuccess);
        ret = HcclAllocComResourceByTiling(commHandle, machine::GetRA()->GetStream(), &(cfgMgr->Mc2CommConfig_),
