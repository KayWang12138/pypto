...
std::vector<uint64_t> DistributedContext::GetHcclContextToHost(const std::vector<std::string> &groupNames) {
    ...
    for (size_t i = 0; i < groupNames.size(); i++) {
        ...
        (void)aclrtMemcpy(&g_hostAddrV1[i], sizeof(g_hostAddrV1[i]), (uint8_t *)devAddrs[0], sizeof(g_hostAddrV1[i]), ACL_MEMCPY_DEVICE_TO_HOST);
        hosts.push_back((uint64_t)(&g_hostAddrV1[i]));
    }
    else {
        (void)aclrtMemcpy(&g_hostAddrV2[i], sizeof(g_hostAddrV2[i]), (uint8_t *)devAddrs[0], sizeof(g_hostAddrV2[i]), ACL_MEMCPY_DEVICE_TO_HOST);
