...
inline void DumpAicpuDevTask(const DeviceArgs &args, json &aicpuPrefArray) {
    ...
    auto ret = rtMemcpy(PtrToPtr<AicpuMetrPer, void>(&aicpuMetric), sizeof(AicpuMetrPer), aicpuPer,
                        sizeof(AicpuMetrPer), RT_MEMCPY_DEVICE_TO_HOST);
    if (ret != 0) {
    ...
    for (uint32_t i = 0; i < args.nrAicpu; i++) {
        ...
        } else if (i == MAX_SCHEDULE_AICPU_NUM) {
            coreType = "AICPU-CTRL";
        }
        aicpu["blockIdx"] = i;
...
void DumpAicpuPreInfo(DeviceArgs &args, const std::vector<void *> &perfData) {
    ...
    std::string cmd = "python3 " + scriptPath + " gen_perfetto " + aicpuPerfilePath + " "
                        + npu::tile_fwk::config::LogTopFolder() + "/machine_runtime_operater_trace.json";
    ...
    npu::tile_fwk::config::SetRunDataOption(KEY_AICPU_PERF_GRAPH_PATH,
            npu::tile_fwk::config::GetAbsoluteTopFolder() + "/pypto_aicpu_prefetto.json");
}
} // namespce
#endif
