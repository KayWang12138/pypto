...
void ContructTaskInfo(const uint32_t &index, json &rootTaskStats, const DeviceArgs &args,
    const std::vector<void *> &perfData, const std::string& coreType) {
...
void DumpAicoreTaskExectInfo(DeviceArgs &args, const std::vector<void *> &perfData) {
    ...
    for (uint32_t i = blockNum; i < Distributed::AICPU_NUM_OF_RUN_AICPU_TASKS; i++) {
