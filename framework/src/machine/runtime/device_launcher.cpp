...
int DeviceLauncher::GetStreamCaptureInfo(rtStream_t aicoreStream, aclmdlRI &rtModel, bool &isCapture)
{
    ...
    if (ret == ACL_ERROR_RT_FEATURE_NOT_SUPPORT) {
        ...
        MACHINE_LOGE_E(RtErr::RT_CAPTURE_FAILED,
...
int DeviceLauncher::SetCaptureStream(rtStream_t aicoreStream, rtStream_t aicpuStream, bool &isCapture)
{
    ...
    if (isCapture) {
        if (rtModel ==  nullptr) {
            MACHINE_LOGE_E(DevCommonErr::NULLPTR, "rtModel is null!");
            return -1;;
        }
        rtError_t ret = rtStreamAddToModel(aicpuStream, rtModel);
        if (ret != 0) {
            MACHINE_LOGE_E(RtErr::RT_LAUNCH_FAILED,
...
int DeviceLauncher::DeviceLaunchOnceWithDeviceTensorData(
    ...
    if (rc < 0) {
        MACHINE_LOGE_E(HostLauncherErr::REGISTER_KERNEL_FAILED, "Register kernel bin failed.");
        return rc;
    }
...
uint8_t *DeviceLauncher::CopyControlFlowCache(DevControlFlowCache *ctrlCache) {
    ...
    if (devCache == nullptr) {
        MACHINE_LOGE_E(RtErr::RT_MALLOC_FAILED, "control flow cache malloc failed");
        return nullptr;
    }
    ...
    for (int i = 0; i < bufNum; ++i) {
        ...
        if (ret != 0) {
            MACHINE_LOGE_E(RtErr::RT_MEMCPY_FAILED,
...
void DeviceLauncher::GetCaptureInfo(aclrtStream aicoreStream, aclmdlRI &rtModel) {
    ...
    if (ret == ACL_ERROR_RT_FEATURE_NOT_SUPPORT) {
        ...
        MACHINE_LOGE_E(RtErr::RT_CAPTURE_FAILED, "get capture info failed: %d", ret);
        return;
    }
...
void DeviceLauncher::UnregisterKernelBin(void *hdl) {
    ...
    if (ret != RT_ERROR_NONE) {
        MACHINE_LOGE_E(RtErr::RT_REGISTER_FAILED, "unregister kernel failed, ret: %d", ret);
    }
#else
    (void)hdl;
#endif
...
int DeviceLauncher::LaunchAicpuKernel(rtAicpuArgsEx_t &rtArgs, bool tripleStream,
    ...
    if (tripleStream) {
        ...
        if (scheCpuNum == 1) {
            ...
            MACHINE_LOGE_E(HostLauncherErr::TRIPLE_STREAM_ERROR,
...
int DeviceLauncher::LaunchAicoreKernel(
    ...
    if (debugEnable) {
        ...
        if (rc != 0) {
            MACHINE_LOGE_E(HostLauncherErr::SYNC_FAILED, "sync failed");
            return rc;
        }
