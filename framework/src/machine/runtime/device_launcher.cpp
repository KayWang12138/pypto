...
uint8_t* DeviceLauncher::CopyControlFlowCache(DevControlFlowCache* ctrlCache)
{
    ...
    for (int i = 0; i < bufNum; ++i) {
        ...
        if (ret != 0) {
            MACHINE_LOGE(RtErr::RtMemcpyKind::FAILED, "control flow cache memcpy failed, ret: %d", ret);
...
int DeviceLauncher::LaunchAicpuKernel(
    RtAicpuArgsEx& rtArgs, bool tripleStream, [[maybe_unused]] bool debugEnable, [[maybe_unused]] Function* function)
{
#ifdef BUILD_WITH_CANN
    auto ctrlStream = (AclRtStream)machine::GetRA()->GetCtrlStream();
    ...
    if (tripleStream) {
        ...
        ret = RuntimeAicpuKernelLaunchExWithArgs(
            RtKernelType::AICPU_KFC, "AST_DYN_AICPU", 1, &rtArgs, nullptr, ctrlStream,
        ...
        ret = RuntimeAicpuKernelLaunchExWithArgs(
            RtKernelType::AICPU_KFC, "AST_DYN_AICPU", nrAicpu, &rtArgs, nullptr, schedStream,
            RT_KERNEL_USE_SPECIAL_TIMEOUT);
