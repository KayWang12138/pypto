...
int DeviceLauncher::GetStreamCaptureInfo(rtStream_t aicoreStream, aclmdlRI &rtModel, bool &isCapture)
{
    ...
    if (ret == ACL_ERROR_RT_FEATURE_NOT_SUPPORT) {
        ...
        MACHINE_LOGE(ERROR_CODE_UNDEFINED, "aclmdlRICaptureGetInfo failed, return[%d]", ret);
        return -1;
    }
    ...
    if (it != captureStatusHandlers.end()) {
        ...
        MACHINE_LOGE(ERROR_CODE_UNDEFINED, "GetStreamCaptureInfo get unsupport capture status");
        return -1;
    }
...
int DeviceLauncher::SetCaptureStream(rtStream_t aicoreStream, rtStream_t aicpuStream, bool &isCapture)
{
    ...
    if (isCapture) {
        if (rtModel ==  nullptr) {
            MACHINE_LOGE(ERROR_CODE_UNDEFINED, "rtModel is null!");
            return -1;;
        }
        rtError_t ret = rtStreamAddToModel(aicpuStream, rtModel);
        if (ret != 0) {
            MACHINE_LOGE(ERROR_CODE_UNDEFINED, "rtStreamAddToModel failed, return[%d]", ret);
            return -1;
        }
    }
    return 0;
...
int DeviceLauncher::DeviceLaunchOnceWithDeviceTensorData(
    ...
    if (rc < 0) {
        MACHINE_LOGE(ERROR_CODE_UNDEFINED, "Register kernel bin failed.");
        return rc;
    }
    ...
    if (streamSynchronize) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, machine::GetRA()->CheckAllSentinels());
    }
...
void DataDumpInit() {
    if (IsPtoDataDumpEnabled()) {
        if (!AdxDataDumpServerInit) {
            MACHINE_LOGE(ERROR_CODE_UNDEFINED, "AdxDataDumpServerInit function not found.");
            return;
        }
        MACHINE_LOGD("DataDumpServerInit is called \n");
        int sf = AdxDataDumpServerInit();
        if (sf != 0) {
            MACHINE_LOGE(ERROR_CODE_UNDEFINED, "ERROR AdxDataDumpServerInit failed \n");
        }
    }
}
...
void DataDumpUnInit() {
    if (IsPtoDataDumpEnabled()) {
        if (!AdxDataDumpServerUnInit) {
            MACHINE_LOGE(ERROR_CODE_UNDEFINED, "AdxDataDumpServerUnInit function not found.");
            return;
        }
        MACHINE_LOGD("DataDumpServerUnInit is called \n");
        int sf = AdxDataDumpServerUnInit();
        if (sf != 0) {
            MACHINE_LOGE(ERROR_CODE_UNDEFINED, "AdxDataDumpServerUnInit is failed %d \n", sf);
        }
    }
}
...
uint8_t *DeviceLauncher::CopyControlFlowCache(DevControlFlowCache *ctrlCache) {
    ...
    if (devCache == nullptr) {
        MACHINE_LOGE(ERROR_CODE_UNDEFINED, "control flow cache malloc failed");
        return nullptr;
    }
    ...
    for (int i = 0; i < bufNum; ++i) {
        ...
        if (ret != 0) {
            MACHINE_LOGE(ERROR_CODE_UNDEFINED, "control flow cache memcpy failed, ret: %d", ret);
            rtFree(devCache);
            return nullptr;
        }
    }
...
void DeviceLauncher::GetCaptureInfo(aclrtStream aicoreStream, aclmdlRI &rtModel) {
    ...
    if (ret == ACL_ERROR_RT_FEATURE_NOT_SUPPORT) {
        ...
        MACHINE_LOGE(ERROR_CODE_UNDEFINED, "get capture info failed: %d", ret);
        return;
    }
...
void *DeviceLauncher::RegisterKernelBin(const std::vector<uint8_t> &kernelBinary) {
    ...
    if (ret != RT_ERROR_NONE) {
        MACHINE_LOGE(ERROR_CODE_UNDEFINED, "register kernel failed, ret: %d", ret);
    }
    return hdl;
#else
    (void)kernelBinary;
...
void DeviceLauncher::UnregisterKernelBin(void *hdl) {
    ...
    if (ret != RT_ERROR_NONE) {
        MACHINE_LOGE(ERROR_CODE_UNDEFINED, "unregister kernel failed, ret: %d", ret);
    }
#else
    (void)hdl;
#endif
...
int DeviceLauncher::LaunchAicpuKernel(rtAicpuArgsEx_t &rtArgs, bool tripleStream,
    ...
    if (tripleStream) {
        ...
        if (scheCpuNum == 1) {
            ...
            MACHINE_LOGE(ERROR_CODE_UNDEFINED, "sche num is 1, no need lauch more aicpu in tripleStream, nrAicpu changed to %u",
...
int DeviceLauncher::LaunchAicoreKernel(
    ...
    if (debugEnable) {
        ...
        if (rc != 0) {
            MACHINE_LOGE(ERROR_CODE_UNDEFINED, "sync failed");
            return rc;
        }
