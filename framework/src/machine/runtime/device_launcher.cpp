...
#ifdef BUILD_WITH_CANN
static const std::unordered_map<npu::tile_fwk::AclMdlRICaptureStatus, std::function<void(bool&)>> captureStatusHandlers = {
    ...
    RtAicpuArgsEx& rtArgs, bool tripleStream, [[maybe_unused]] bool debugEnable, [[maybe_unused]] Function* function)
{
    ...
    if (tripleStream) {
        ...
        ret = RuntimeAicpuKernelLaunchExWithArgs(
            static_cast<uint32_t>(npu::tile_fwk::RtKernelType::AICPU_KFC), "AST_DYN_AICPU", nrAicpu, &rtArgs, nullptr, schedStream,
            RT_KERNEL_USE_SPECIAL_TIMEOUT);
