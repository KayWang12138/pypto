...
int DeviceLauncher::DeviceLaunchOnceWithDeviceTensorData(
    ...
    if (!config.hcclContext.empty()) {
        PrepareHcclContext(config.hcclContext, function->GetDyndevAttribute()->devProgBinary);
    } else {
        DeviceInitDistributedContext(function->GetDyndevAttribute()->commGroupNames,
