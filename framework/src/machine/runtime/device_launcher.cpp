    ...
    DevControlFlowCache* inputDevCtrlCache, const DeviceLauncherConfig& config)
{
    ...
    DeviceLauncherConfigFillDeviceInfo(config);
 	DeviceMemoryUtils devMemoryUtils;
 	DeviceInitDistributedContext(devMemoryUtils, dynAttr->commGroupNames, kArgs);

    HOST_PERF_TRACE(TracePhase::RunDevEnvReady);
    DeviceInitTilingData(devMemoryUtils, kArgs, dynAttr->devProgBinary, inputDevCtrlCache, config, cachedOperator);
    HOST_PERF_TRACE(TracePhase::RunDevInitTiling);

    DeviceRunCacheKernelSet(function, (uint8_t*)kArgs.cfgdata);
    DeviceInitKernelInOuts(devMemoryUtils, kArgs, inputList, outputList, dynAttr->disableL2List);
    ...
    Function* function, DevControlFlowCache* hostCtrlCache, const DeviceLauncherConfig& config)
{
    ...
    CopyFromDev(devMemory, outputDataList);
    if (HasInplaceArgs(function) || outputDataList.size() == 0) {
        CopyFromDev(devMemory, inputDataList);
    }
