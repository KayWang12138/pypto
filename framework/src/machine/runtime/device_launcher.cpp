...
int GetCfgBlockdim() {
    ...
    // 通过GetCfgMaxBlockdim接口获取设置的最大核数，如果设置的最大核数大于硬件物理最大核数时，控核不生效
    // 如果未进行控核，GetCfgMaxBlockdim接口将通过aclrtGetResInCurrentThread函数返回硬件物理最大核数
    auto maxBlk = GetCfgMaxBlockdim();
    blk = maxBlk < blk ? maxBlk : blk;
