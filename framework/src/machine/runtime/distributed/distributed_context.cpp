...
void DistributedContext::FillCommCtxAttr<npu::tile_fwk::HcclCombinOpParamA5>(
    ...
    ASSERT(ERROR_CODE_UNKNOWN, hcclParamhost->winSize > WIN_EXP_SIZE) << "winSize: " << hcclParamhost->winSize <<
...
uint64_t AllocateAndSetupCommContext(void* paramHost, uint32_t rankNum, const std::string& groupName,
    ...
    ASSERT(ERROR_CODE_UNKNOWN, ctxHost != nullptr) << "ctxHost malloc failed";
    ...
    ASSERT(ERROR_CODE_UNKNOWN, ctxDevice != nullptr) << "ctxDevice malloc failed";

    auto ret = aclrtMemcpy(ctxDevice, commCtxSize, ctxHost, commCtxSize, ACL_MEMCPY_HOST_TO_DEVICE);
    ASSERT(ERROR_CODE_UNKNOWN, ret == 0) << "aclrtMemcpy failed, error: " << ret;
...
uint64_t DistributedContext::AllocCommContext<ResType::MESH_A5>(
    ...
    ASSERT(ERROR_CODE_UNKNOWN, hcclParamhost != nullptr) << "hcclParamhost malloc failed";
...
uint64_t DistributedContext::AllocCommContext<ResType::RING_A2>([[maybe_unused]] const uint64_t ctxAddr,
    ...
    ASSERT(ERROR_CODE_UNKNOWN, remoteResPtr != nullptr) << "remoteResPtr malloc failed";
...
std::vector<uint64_t> DistributedContext::GetCommContext([[maybe_unused]] const std::vector<std::string> &groupNames) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, HcomGetL0TopoTypeEx(group, &topoRet, COMM_IS_NOT_SET_DEVICE) == HCCL_SUCCESS) << "Get hcom topo failed";
    ...
    ASSERT(ERROR_CODE_UNKNOWN, groupNames.size() <= DIST_COMM_GROUP_NUM) <<"Commgroup size is not be supported";
    for (size_t groupIndex = 0; groupIndex < groupNames.size(); ++groupIndex) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, HcomGetCommHandleByGroup(groupName.c_str(), &commHandle) == 0) << "Get hcom handle failed";
        ...
        ASSERT(ERROR_CODE_UNKNOWN, (ret == 0) && (commContext[groupIndex] != 0UL)) << "Hccl alloc resource failed";
