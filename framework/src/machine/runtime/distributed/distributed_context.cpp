...
std::vector<uint64_t> DistributedContext::GetCommContext([[maybe_unused]] const std::vector<std::string>& groupNames)
{
    ...
    ASSERT(DistributedErrorCode::INVALID_HCCL_TOPO, HcomGetL0TopoTypeEx(group, &topoRet, COMM_IS_NOT_SET_DEVICE) == HCCL_SUCCESS) << "Get hcom topo failed";
    ...
    ASSERT(DistributedErrorCode::INVALID_WORLD_SIZE, groupNames.size() <= DIST_COMM_GROUP_NUM) << "Commgroup size is not be supported";
    for (size_t groupIndex = 0; groupIndex < groupNames.size(); ++groupIndex) {
        ...
        ASSERT(DistributedErrorCode::INVALID_GROUP_NAME, HcomGetCommHandleByGroup(groupName.c_str(), &commHandle) == 0) << "Get hcom handle failed";
        ...
        ASSERT(DistributedErrorCode::HCCL_ALLOC_RESOURCE_FAILED, (ret == 0) && (commContext[groupIndex] != 0UL)) << "Hccl alloc resource failed";
        DISTRIBUTED_LOGI(
