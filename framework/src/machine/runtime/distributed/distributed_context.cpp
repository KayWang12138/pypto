    ...
    [[maybe_unused]] const uint64_t ctxAddr, [[maybe_unused]] const std::string& groupName)
{
    ...
    ret = aclrtMemcpy(remoteResPtr, remoteResSize, &(hcclParam->remoteRes), remoteResSize, AclRtMemcpyKind::DEVICE_TO_HOST);
    ASSERT(ret == 0) << "aclrtMemcpy failed, error: " << ret;
    ...
    for (uint64_t i = 0; i < hcclParamhost->rankSize; i++) {
        ...
        ret = aclrtMemcpy(
            ...
            sizeof(npu::tile_fwk::HcclRankRelationResV2), AclRtMemcpyKind::DEVICE_TO_HOST);
        ASSERT(ret == 0) << "aclrtMemcpy failed, error: " << ret;
