...
template<>
void DistributedContext::FillCommCtxAttr<npu::tile_fwk::HcclCombinOpParamA5>(TileOp::CommContext *ctxHost, npu::tile_fwk::HcclCombinOpParam *hcclParamhost) {
...
template<>
void DistributedContext::FillCommCtxWinArr<npu::tile_fwk::HcclCombinOpParamA5>(int i, TileOp::CommContext *ctxHost, npu::tile_fwk::HcclCombinOpParam *hcclParamhost) {
...
uint64_t AllocateAndSetupCommContext(void* paramHost, uint32_t rankNum, const std::string& groupName,
    ...
    size_t commCtxSize = sizeof(TileOp::CommContext) +
                        sizeof(uint64_t) * rankNum * WIN_TYPE_NUM;

    TileOp::CommContext* ctxHost = static_cast<TileOp::CommContext*>(
        machine::GetRuntimeHostAgent()->AllocHostAddr(commCtxSize));
    ...
    aclrtMemcpy(ctxDevice, commCtxSize, ctxHost, commCtxSize,
                ACL_MEMCPY_HOST_TO_DEVICE);

    g_context[groupName] = std::make_pair(reinterpret_cast<uint64_t>(ctxDevice),
                                            reinterpret_cast<uint64_t>(ctxHost));
