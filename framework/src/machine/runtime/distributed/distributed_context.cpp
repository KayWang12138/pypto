...
void DistributedContext::FillCommCtxAttr<npu::tile_fwk::HcclCombinOpParamA5>(
    ...
    // 对应winAddr排布，startIndex:[0,rankNum-1], winStatusIndex:[rankNum, 2*rankNum-1], winDebugIndex:[2*rankNum, 3*rankNum-1]
...
template<>
uint64_t DistributedContext::AllocCommContext<ResType::MESH>(
    [[maybe_unused]] const uint64_t ctxAddr, [[maybe_unused]]const std::string &groupName) {
#ifdef BUILD_WITH_CANN
...
template<>
uint64_t DistributedContext::AllocCommContext<ResType::RING>([[maybe_unused]] const uint64_t ctxAddr,
    ...
    ret = aclrtMemcpy(remoteResPtr, remoteResSize,
    ...
    for (uint64_t i = 0; i < hcclParamhost->rankSize; i++) {
        ...
        ret = aclrtMemcpy(&remoteResDevicePtr, sizeof(uint64_t),
