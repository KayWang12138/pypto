...
public:
    ...
    int DynamicLaunch(rtStream_t aicpuStream, rtStream_t ctrlStream, rtStream_t aicoreStream, int64_t taskId, KernelArgs *kernelArgs, int blockdim, int launchAicpuNum);
    int DynamicLaunchSynchronize(rtStream_t aicpuStream, rtStream_t ctrlStream, rtStream_t aicoreStream);
    int DynamicRun(rtStream_t aicpuStream, rtStream_t ctrlStream, rtStream_t aicoreStream, int64_t taskId, KernelArgs *kernelArgs, int blockdim = 25, int launchAicpuNum = 5);
...
private:
    ...
    /**************DynamicFunction**************/
    int launchDynamicAiCore(rtStream_t aicoreStream, KernelArgs *kernelArgs);
    int launchDynamicAiCpu(rtStream_t aicpuStream, KernelArgs *kArgs);
    int RunPrepare();
    int RunPreSync(rtStream_t aicpuStream, rtStream_t aicoreStream);
    int RunPost(rtStream_t aicpuStream, rtStream_t aicoreStream);
    int launchDynamicAiCpuInit(rtStream_t aicpuStream, KernelArgs *kArgs);
    void InitAiCpuSoBin();
    ...
    int DynamicKernelLaunch(rtStream_t aicpuStream, rtStream_t aicoreStream, KernelArgs *kernelArgs, int blockdim);
    int DynamicSeparateLaunch(rtStream_t aicpuStream, rtStream_t ctrlStream, rtStream_t aicoreStream, KernelArgs *kernelArgs, int blockdim);
private:
    int devId_;
    int aicpuNum_{5};
    int blockDim_{24};
