...
{
    (void)useHugePage;
    return hostMemMgr_.Free(ptr) == 0 ? kRtOk : kRtErr;
}

int HostRtApi::Memcpy(void* dst, size_t dstSize, const void* src, size_t size, int kind)
{
    return hostMemMgr_.Memcpy(dst, dstSize, src, size, kind) == 0 ? kRtOk : kRtErr;
}

int HostRtApi::Memset(void* dst, size_t dstSize, int value, size_t size)
{
    return hostMemMgr_.Memset(dst, dstSize, value, size) == 0 ? kRtOk : kRtErr;
}
