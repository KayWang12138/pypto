...
std::unique_ptr<DevControlFlowCache> EmulationLauncher::CreateHostCtrlFlowCache(DevAscendProgram *devProg, Function *function) {
    ...
    hostCtrlFlowCache->Init(function->GetDyndevAttribute().get(),
        devProg->ctrlFlowCacheSize, devProg->runtimeOutcastPoolSize, initOffset);
    return std::unique_ptr<DevControlFlowCache>(hostCtrlFlowCache);
}
...
int EmulationLauncher::BuildControlFlowCacheWithEmulationTensorData(
    ...
    std::unique_ptr<DevControlFlowCache> hostCtrlFlowCache = CreateHostCtrlFlowCache(devProg, function);
