...
static int EmulationLaunchOnce(DeviceKernelArgs &kArgs) {
    ...
    auto shmSize = sizeof(RuntimeDataRingBufferHead) + DEVICE_SHM_SIZE + DEVICE_TASK_QUEUE_SIZE * threadNum;
    std::vector<int64_t> shmMem(shmSize / sizeof(int64_t), 0);
    devProg->devArgs.runtimeDataRingBufferAddr = (uint64_t)shmMem.data();
