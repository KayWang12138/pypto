...
int EmulationLauncher::BuildControlFlowCacheWithEmulationTensorData(
    Function* function, std::vector<DeviceTensorData>& inputList, const std::vector<DeviceTensorData>& outputList,
    ...
    const DeviceLauncherConfig& config)
{
    ...
    npu::tile_fwk::GetReadyOnHostTensorsSet(function, readyOnHostIndices);
    for (int idx : readyOnHostIndices) {
        if (idx < 0 || static_cast<size_t>(idx) >= inputList.size()) {
            continue;
        }
        DeviceTensorData& td = inputList[static_cast<size_t>(idx)];
        void* src = td.GetAddr();
        if (src == nullptr) {
            continue;
        }
    ...
    if (inputList.size() == 0 && outputList.size() == 0) {
        ...
        for (auto& input : inputList) {
            inputDeviceDataList.emplace_back(
                input.GetDataType(), input.GetAddr(), input.GetShape());
            index++;
        }
        for (auto& output : outputList) {
            outputDeviceDataList.emplace_back(
                output.GetDataType(), output.GetShape(), output.GetShape());
            index++;
        }
        return BuildControlFlowCacheWithEmulationTensorData(
            function, inputDeviceDataList, outputDeviceDataList, nullptr, outCtrlFlowCache, memUtils, config);
