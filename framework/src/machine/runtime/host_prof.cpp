...
void HostProf::HostProfReportBasicInfo(
    const uint64_t &endTime, const uint32_t blockDim, const uint16_t taskType) const {
...
void HostProf::HostProfReportTensorInfo(const uint64_t &endTime) const {
    ...
    MACHINE_LOGD("Op [%s] with inputs[%zu], outputs[%zu]", opName_.c_str(), profFunction_->inCasts_.size(),
