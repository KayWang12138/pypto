...
void HostProf::BuildCacheTensorInfo(CacheTaskInfo *taskInfo) const {
    ...
    for (uint32_t i = 0U; i < profFunction_->inCasts_.size(); ++i) {
        MsrofTensorData &tensorData = taskInfo->tensorData[i];
        BuildTensor(MSPROF_GE_TENSOR_TYPE_INPUT, profFunction_->inCasts_.get(i), taskInfo->tensorData[i]);
    }
    for (uint32_t i = 0U; i < profFunction_->inCasts_.size(); ++i) {
        BuildTensor(MSPROF_GE_TENSOR_TYPE_OUTPUT, profFunction_->outCasts_.get(i), taskInfo->tensorData[i + profFunction_->inCasts_.size()]);
    }
}

bool HostProf::IsCacheOpInfoEnable(const rtStream_t stream) {
    if (stream == nullptr) {
        return false;
    }
...
void HostProf::HostProfReportCacheTaskInfo(const rtStream_t stream, const uint32_t numBlocks, const uint32_t taskType) const {
    ...
    MACHINE_LOGI("Report op [%s] info cache, task type[%u], numBlocks[%u], attrId[%llu] size[%zu]",
        executor->opType, taskType, numBlocks, taskInfo->attrId, bufferSize);
    free(buffer);
}
