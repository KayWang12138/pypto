...
void HostProf::BuildTensor(const uint32_t tensorType, const RawTensorDataPtr &tensorInfo,
    ...
    tensorData.format = 0U;
    tensorData.dataType = static_cast<uint32_t>(tensorInfo->GetDataType());
...
void HostProf::BuildCacheTensorInfo(CacheTaskInfo *taskInfo) {
    ...
    for (size_t i = 0; i < input_tensors; ++i) {
    ...
    for (size_t i = 0; i < output_tensors; ++i) {
        BuildTensor(MSPROF_GE_TENSOR_TYPE_OUTPUT, output_tensors.at(i),
                    taskInfo->tensorData[i + profFunction_->inCasts_.size()]);
    }
}
...
void HostProf::HostProfReportCacheTaskInfo(const aclrtStream stream, const uint32_t numBlocks, const uint32_t taskType) const {
    ...
    uint64_t itemId = profFunction_->GetFunctionHash().GetHash();
