...
void HostProf::HostProfReportCacheTaskInfo(const aclrtStream stream, const uint32_t numBlocks,
    ...
    MACHINE_LOGD("Begin to report task cache [%s].", profFunction_->GetMagicName().c_str());
    ...
    if (aclrtCacheLastTaskOpInfo(buffer, bufferSize) != ACL_SUCCESS) {
        ...
        MACHINE_LOGI("Report task info cache, node id[%lu], task type[%u], numBlocks[%u], attrId[%lu] size[%zu]",
                     itemId, taskType, numBlocks, taskInfo->attrId, bufferSize);
    }
    free(buffer);
}
