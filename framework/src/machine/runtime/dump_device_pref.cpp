...
void ContructTaskInfo(const uint32_t &blockNum, json &rootTaskStats,
    ...
    for (uint32_t i = 0; i < blockNum; i++) {
        ...
        if (aicpuMetric->taskCount > MAX_DFX_TASK_NUM_PER_CORE) {aicpuMetric->taskCount = MAX_DFX_TASK_NUM_PER_CORE;} // Limit to the maximum value 
...
void DumpAicoreTaskExectInfo(DeviceArgs &args, const std::vector<void *> &perfData) {
    ...
    DumpAicpuPreInfo(args, perfData);
}
...
inline void DumpAicoreDevTask(DeviceArgs &args, json &aicpuPrefArray, const std::vector<void *> &perfData) {
    ...
    for(int i = 0; i < PART; i++) {
        SparateCore(args.nrValidAic, i, PART, 0, coreArray);
        SparateCore(args.nrValidAic * AIV_NUM_PER_AI_CORE, i, PART, args.nrValidAic, coreArray);
    }
...
inline void DumpAicpuDevTask(const DeviceArgs &args, json &aicpuPrefArray) {
    auto aicpuPer = (ValueToPtr(args.aicpuPerAddr));
    if (aicpuPer == nullptr) {
        ALOG_ERROR_F("Aicpu per ptr is null");
        return;
    }
    AicpuMetrPer aicpuMetric;
    auto ret = rtMemcpy(PtrToPtr<AicpuMetrPer, void>(&aicpuMetric), sizeof(AicpuMetrPer), aicpuPer, sizeof(AicpuMetrPer), RT_MEMCPY_DEVICE_TO_HOST);
    if (ret != 0) {
...
void DumpAicpuPreInfo(DeviceArgs &args, const std::vector<void *> &perfData) {
    ...
    std::string scriptPath = GetCurrentSharedLibPath() + "/../../../tools/scripts/machine_perf_trace.py";
    std::string cmd = "python3 " + scriptPath + " gen_perfetto " + aicpuPerfilePath + " " + npu::tile_fwk::config::LogTopFolder() + "/pypto_aicpu_prefetto.json";
