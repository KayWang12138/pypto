...
int RuntimeAgentMemory::GetAicoreRegInfo(std::vector<int64_t> &aic, std::vector<int64_t> &aiv, const int &addrType) {
    ...
    if (halFunc == nullptr) {
        MACHINE_LOGE_E(HostLauncherErr::HAL_FUNCTION_NOT_FOUND, "Hal function not found.");
        return -1;
    }
    ...
    if (ret != 0) {
        MACHINE_LOGE_E(HostLauncherErr::MAP_REG_ADDR_FAILED,
...
void *RuntimeAgentMemory::MapAiCoreReg() {
    ...
    if (devAddr == nullptr) {
        MACHINE_LOGE_E(HostLauncherErr::RT_MALLOC_FAILED, "rtMalloc failed. size: %zu", regAddrSize);
        return nullptr;
    }

    int rc = rtMemcpy(devAddr, regAddrSize, regAddr.data(), regAddrSize, RT_MEMCPY_HOST_TO_DEVICE);
    if (rc != 0) {
        MACHINE_LOGE_E(HostLauncherErr::RT_MEMCPY_FAILED, "rtMemcpy failed. size: %zu", regAddrSize);
