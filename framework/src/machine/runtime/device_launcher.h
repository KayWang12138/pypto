...
public:
    ...
    static void ResetPrefetchInfo() {
 	   l2prefetch_.prefetchNum = 0;
 	}
 	 
 	static void AddPrefetch(uint64_t addr, uint64_t size) {
    ...
    static void FillL2PrefetchInfo(DevAscendProgram *devProg, size_t idx, uint64_t addr, bool isTest, bool isInput) {
        if (!isInput || isTest) {
            return;
        }
    ...
    static void DeviceInitKernelInOuts(DeviceMemoryTy& devMem, DeviceKernelArgs &kArgs,
        ...
        auto buildInouts = [&](const std::vector<DeviceTensorData> &tensorDataList, DevTensorData* data,
            ...
            for (size_t k = 0; k < tensorDataList.size(); ++k) {
                ...
                if (func != nullptr) {
                    FillL2PrefetchInfo(devProg, k, reinterpret_cast<uint64_t>(addr), false, isInput);
                }
...
private:
    ...
    static L2TensorInfo l2prefetch_;
};
