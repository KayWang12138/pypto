...
public:
    ...
    static bool AddAicpuStream(
        aclrtStream aicoreStream, aclrtStream ctrlStream, aclrtStream schedtream, bool tripleStream);
    static int LaunchAicpuKernel(aclrtStream aicpuStream, rtAicpuArgsEx_t &rtArgs, uint32_t blockDim);
    static int LaunchAicoreKernel(aclrtStream aicoreStream, void *kernel, uint32_t blockDim,
        rtArgsEx_t &rtArgs, rtTaskCfgInfo_t &rtTaskCfg);
