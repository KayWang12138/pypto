...
void *ManualManMem(int size) {
    ...
    if (rtMalloc(&devPtr, alignSize, TWO_MB_HUGE_PAGE_FLAGS, 0) != 0) {
        rtFree(devPtr);
...
void DeviceRunner::InitDynamicArgs(DeviceArgs &args) {
    ...
    if (GetEnvVar("DUMP_DEVICE_PERF") == "true") {
        ...
        std::lock_guard<std::mutex> lock(dumpThreadMutex_);
    }
}
...
void DeviceRunner::StopDumpThread() {
    ...
    MACHINE_LOGI("Dump thread stopped");
    
    std::lock_guard<std::mutex> lock(dumpThreadMutex_);    
    for (auto ptr : perfData_) {
        if (ptr != nullptr) {
            rtFree(ptr);
        }
    }
    ...
    if (aicpuDevPtr_ != nullptr) {
        rtFree(aicpuDevPtr_);
        aicpuDevPtr_ = nullptr;
    }
}
...
void DeviceRunner::DumpThreadFunc() {
    while (!dumpThreadStopFlag_.load()) {
        ...
        {
            std::lock_guard<std::mutex> lock(dumpThreadMutex_);
            if (dumpThreadInitialized_ && perfData_.size() > 0) {
                MACHINE_LOGI("Dump thread iteration %d", count);
                npu::tile_fwk::dynamic::DumpAicpuPerfInfo(args_, perfData_, false);
            }
        }
    }
    if (dumpThreadInitialized_) {
        MACHINE_LOGI("Dump thread final dump");
        npu::tile_fwk::dynamic::DumpAicpuPerfInfo(args_, perfData_, true);
    }
}

DeviceRunner::~DeviceRunner() {
    MACHINE_LOGE("start to dump perfData");
