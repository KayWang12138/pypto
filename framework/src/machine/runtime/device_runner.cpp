...
rtError_t AicpuLaunchCallback(rtAicpuInfo_t &aicpuInfo) {
    return DeviceRunner::Get().AicpuLaunchCallback(aicpuInfo);
}

rtError_t DeviceRunner::AicpuLaunchCallback(rtAicpuInfo_t &aicpuInfo) {
    // not pypto
    aclrtEvent event = useEvent1_ ? event1_ : event2_;

    rc = aclrtRecordEvent(event, aicpuInfo.stm);
    if (rc < 0) {
