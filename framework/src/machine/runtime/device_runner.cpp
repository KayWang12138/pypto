...
int DeviceRunner::launchDynamicAiCpu(rtStream_t aicpuStream, DeviceKernelArgs *kArgs) {
...
#endif
    auto args = reinterpret_cast<struct dynamic::AiCpuArgs*>(kArgs->inputs);
    uint32_t argsSize = reinterpret_cast<uint32_t>(kArgs->outputs);
    ...
    rtArgs.kernelNameAddrOffset = offsetof(struct dynamic::AiCpuArgs, kernelName);
    rtArgs.soNameAddrOffset = offsetof(struct dynamic::AiCpuArgs, soName);
    rtArgs.hostInputInfoNum = 1;
    rtHostInputInfo_t hostInputInfo;
    hostInputInfo.addrOffset = reinterpret_cast<uint32_t>(args->kArgs.inputs) - reinterpret_cast<uint32_t>(args);
    hostInputInfo.dataOffset = sizeof(struct AiCpuArg);
