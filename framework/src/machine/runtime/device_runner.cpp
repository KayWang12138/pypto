...
void *DeviceRunner::DevAlloc(int size) {
    ...
    if (rc != 0) {
        ...
        MACHINE_LOGE_E(RtErr::RT_MEMSET_FAILED, "rtMemset failed size=%d rc=%d\n", size, rc);
        return nullptr;
    }
    return devPtr;
}
...
int DeviceRunner::InitDeviceArgsCore(DeviceArgs &args, const std::vector<int64_t> &regs, const std::vector<int64_t> &regsPmu) {
    ...
    if (args.devDfxArgAddr == 0) {
        MACHINE_LOGE_E(DevCommonErr::ALLOC_FAILED, "Alloc devDfx info failed");
        return -1;
    }
...
void DeviceRunner::InitAiCpuSoBin(DeviceArgs &devArgs) {
    ...
    if (!ReadBytesFromFile(fileName, buffer)) {
        MACHINE_LOGE_E(DevCommonErr::FILE_ERROR,
...
int DeviceRunner::InitAicpuServer() {
    ...
    if (ret != RT_ERROR_NONE) {
        MACHINE_LOGE_E(RtErr::RT_LAUNCH_FAILED, "Aicpu server init failed %d", ret);
        return ret;
    }
...
int DeviceRunner::RunPreSync(rtStream_t aicpuStream, rtStream_t aicoreStream) {
    ...
    if (rc < 0) {
        MACHINE_LOGE_E(RtErr::RT_EVENT_FAILED, "aclrtCreateEvent failed %d\n", rc);
        return rc;
    }

    rc = aclrtRecordEvent(event, aicoreStream);
    if (rc < 0) {
        MACHINE_LOGE_E(RtErr::RT_EVENT_FAILED, "aclrtRecordEvent failed %d\n", rc);
        return rc;
    }

    rc = aclrtStreamWaitEvent(aicpuStream, event);
    if (rc < 0) {
        MACHINE_LOGE_E(RtErr::RT_EVENT_FAILED, "aclrtStreamWaitEvent failed %d\n", rc);
        return rc;
    }
    return 0;
}
...
int DeviceRunner::DynamicLaunch(rtStream_t aicpuStream, rtStream_t ctrlStream, rtStream_t aicoreStream, [[maybe_unused]] int64_t taskId,
    ...
    int rc = RunPrepare();
    if (rc < 0) {
        MACHINE_LOGE_E(HostLauncherErr::LAUNCH_PREPARE_FAILED, "Prepare failed.");
        return rc;
    }
...
int DeviceRunner::RegisterKernelBin(void **hdl, std::vector<uint8_t> *funcBinBuf) {
    ...
    if (rc != 0) {
        MACHINE_LOGE_E(HostLauncherErr::REGISTER_KERNEL_FAILED, "RegisterKernelBin failed\n");
    }
    binHdl_ = *hdl;
...
int DeviceRunner::Init(void) {
    ...
    if (LoadAicpuOp::GetInstance().GetBuiltInOpBinHandle() != 0) {
        MACHINE_LOGE_E(DevCommonErr::GET_HANDLE_FAILED, "Get builtInOp Funchandle failed\n");
        return -1;
    }
