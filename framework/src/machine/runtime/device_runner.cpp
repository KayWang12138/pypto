...
void SyncStreams(RtStream aicpuStream, RtStream aicoreStream, bool useSyncFlag)
{
    ...
    if (useSyncFlag) {
        rc = AclRtCreateEventExWithFlag(&event, ACL_RT_EVENT_SYNC);
    } else {
    ...
    DeviceArgs& args, const std::vector<int64_t>& regs, const std::vector<int64_t>& regsPmu)
{
    ...
    RuntimeMemcpy(reinterpret_cast<void*>(args.corePmuRegAddr), size, regsPmu.data(), size, RtMemcpyKind::HOST_TO_DEVICE);
    size = pmuEvtType_.size() * sizeof(int64_t);
    RuntimeMemcpy(reinterpret_cast<void*>(args.pmuEventAddr), size, pmuEvtType_.data(), size, RtMemcpyKind::HOST_TO_DEVICE);
    MACHINE_LOGI(
...
void DeviceRunner::Dump()
{
    ...
    int rc = RuntimeMemcpy(buffer.data(), size, reinterpret_cast<void*>(args_.sharedBuffer), size, RtMemcpyKind::DEVICE_TO_HOST);
    if (rc != 0) {
...
int DeviceRunner::launchDynamicAiCore(RtStream aicoreStream, DeviceKernelArgs* kernelArgs)
{
    ...
    RtTaskCfgInfo cfg = {};
    cfg.schemMode = RtSchemModeType::BATCH;
...
int DeviceRunner::launchDynamicAiCpu(RtStream aicpuStream, DeviceKernelArgs* kArgs)
{
    ...
    return RuntimeAicpuKernelLaunchExWithArgs(
        RtKernelType::AICPU_KFC, "AST_DYN_AICPU", aicpuNum_, &rtArgs, nullptr, aicpuStream,
...
void DeviceRunner::InitAiCpuSoBin(DeviceArgs& devArgs)
{
    ...
    RuntimeMemcpy(
        dAicpuData, aicpuDataLength, reinterpret_cast<void*>(buffer.data()), aicpuDataLength, RtMemcpyKind::HOST_TO_DEVICE);
...
int DeviceRunner::InitAicpuServer()
{
    ...
    int ret = RuntimeAicpuKernelLaunchExWithArgs(
        RtKernelType::AICPU_KFC, "AST_DYN_AICPU", 1, &rtArgs, nullptr, aicpuStream, 0);
    if (ret != RT_SUCCESS) {
...
void DeviceRunner::SetDebugEnable()
{
    for (uint32_t i = 0; i < args_.nrAic + args_.nrAiv; i++) {
        ...
        RuntimeMemcpy(
            ...
            sizeof(uint64_t), reinterpret_cast<uint8_t*>(&perfData_[i]), sizeof(uint64_t), RtMemcpyKind::HOST_TO_DEVICE);
    }
...
int DeviceRunner::DynamicSeparateLaunch(
    RtStream aicpuStream, RtStream ctrlStream, RtStream aicoreStream, DeviceKernelArgs* kernelArgs, int blockdim)
{
    ...
    ReportHostProfInfo(ctrlStream, startTime, blockdim, MSPF_GE_TASK_TYPE_AI_CPU, true);
    ...
    RtStream schedStream, RtStream ctrlStream, RtStream aicoreStream, DeviceKernelArgs* kernelArgs, int blockdim)
{
    ...
    int rc = RuntimeAicpuKernelLaunchExWithArgs(
        RtKernelType::AICPU_KFC, "AST_DYN_AICPU", 1, &rtArgs, nullptr, (AclRtStream)ctrlStream, 0);
    if (rc < 0) {
    ...
    rc = RuntimeAicpuKernelLaunchExWithArgs(
        RtKernelType::AICPU_KFC, "AST_DYN_AICPU", aicpuNum_, &rtArgs, nullptr, (AclRtStream)schedStream,
        0);
    if (rc < 0) {
...
int DeviceRunner::Init(void)
{
    ...
    if (AclRtCreateEventExWithFlag(&event_, ACL_RT_EVENT_SYNC) < 0) {
