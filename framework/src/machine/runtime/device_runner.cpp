...
void DeviceRunner::InitAiCpuArgs(rtAicpuArgsEx_t &rtArgs, AiCpuArgs *args, DeviceKernelArgs *kArgs) {
...
int DeviceRunner::launchDynamicAiCpu(rtStream_t aicpuStream, DeviceKernelArgs *kArgs) {
    ...
    InitAiCpuArgs(rtArgs, args, kArgs);
...
int DeviceRunner::DynamicTripleStreamLaunch(rtStream_t schedStream, rtStream_t ctrlStream, rtStream_t aicoreStream,
    ...
    rtAicpuArgsEx_t rtArgs;
    InitAiCpuArgs(rtArgs, args, kernelArgs);
    ...
    if (rc < 0) {
        MACHINE_LOGE("launch ctrl aicpu failed %d\n", rc);
        return rc;
    }
    ...
    if (rc < 0) {
        MACHINE_LOGE("launch sche aicpu failed %d\n", rc);
        return rc;
    }
