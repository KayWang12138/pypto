...
int DeviceRunner::ConstrutDeviceArgs(DeviceArgs &args, const std::vector<int64_t> &regs, const std::vector<int64_t> &regsPmu) {
...
int DeviceRunner::InitDeviceArgs(DeviceArgs &args) {
    ...
    return ConstrutDeviceArgs(args, regs, regsPmu);
}
...
int DeviceRunner::RunPrepare() {
   for (uint32_t i = 0; i < args_.nrAic + args_.nrAiv; i++) {
    ...
    args_.aicpuPerAddr = npu::tile_fwk::dynamic::PtrToValue(DevAlloc(args_.nrAicpu * sizeof(AicpuMetrPer)));
    if (args_.aicpuPerAddr == 0) {
...
int DeviceRunner::DynamicRun(rtStream_t aicpuStream, rtStream_t ctrlStream, rtStream_t aicoreStream, int64_t taskId, DeviceKernelArgs *kernelArgs, int blockdim, int launchAicpuNum) {
    ...
    DynamicLaunchSynchronize(aicpuStream, ctrlStream, aicoreStream);
    ALOG_ERROR("Start to dump aicpu perf info");
    npu::tile_fwk::dynamic::DumpAicpuPreInfo(args_, perfData_);
    return 0;
}
