...
int DeviceRunner::DynamicTripleStreamLaunch(rtStream_t schedStream, rtStream_t ctrlStream, rtStream_t aicoreStream,
    ...
    if (rc < 0) {
        MACHINE_LOGE(HostLauncherErr::LAUNCH_AICPU_FAILED, "launch ctrl aicpu failed %d\n", rc);
        return rc;
    }
    ...
    if (rc < 0) {
        MACHINE_LOGE(HostLauncherErr::LAUNCH_AICPU_FAILED, "launch sche aicpu failed %d\n", rc);
        return rc;
    }
