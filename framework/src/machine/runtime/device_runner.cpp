...
void DeviceRunner::SetPmuEventType(int32_t &profPmuType) {
    // 根据平台类型动态选择PMU事件配置
    NPUArch npuArch = Platform::Instance().GetSoc().GetNPUArch();
    
    if (npuArch == NPUArch::DAV_3510) {
        // 新硬件（DAV_3510）的PMU事件类型配置
        switch (profPmuType) {
        ...
        ALOG_INFO_F("Using DAV_3510 PMU event type configuration.");
    } else {
        // 旧硬件（DAV_2201）的PMU事件类型配置
        switch (profPmuType) {
            case ARITHMETIC_UTILIZATION:
        ...
        ALOG_INFO_F("Using DAV_2201 PMU event type configuration.");
    }
}
...
int DeviceRunner::InitDeviceArgs(DeviceArgs &args) {
    ...
    pmuEvtType_.resize(PMU_EVENT_TYPE_MAX_DAV_2201, 0x0);
