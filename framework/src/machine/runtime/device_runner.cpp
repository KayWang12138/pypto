...
extern "C" void DumpDevicePerfData() {
    if (DeviceRunner::Get().GetEnableDumpDevPref()) {
        DeviceRunner::Get().DumpDevTaskPerfData();
    }
}
}
...
void DeviceRunner::InitDynamicArgs(DeviceArgs &args) {
    ...
    if (GetEnvVar("DUMP_DEVICE_PERF") == "true") {
        args_.aicpuPerfAddr = npu::tile_fwk::dynamic::PtrToValue(DevAlloc(MAX_PROF_NUM * sizeof(MetricPerf)));
...
void DeviceRunner::DumpDevTaskPerfData() {
    uint64_t freq = (args_.archInfo == ArchInfo::DAV_2201) ? npu::tile_fwk::dynamic::FREQ_DAV_2201 : npu::tile_fwk::dynamic::FREQ_DAV_3510;
    npu::tile_fwk::dynamic::DumpAicpuPerfInfo(args_, perfData_, freq);
}
...
void DeviceRunner::SetDebugEnable() {
    ...
    MACHINE_LOGE("Set debug enable aicore 0 devPtr: %p", perfData_[0]);
}
