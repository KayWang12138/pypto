...
void *DeviceRunner::DevAlloc(int size) {
    ...
    if (rc != 0) {
        ...
        MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "rtMemset failed size=%d rc=%d\n", size, rc);
        return nullptr;
    }
    return devPtr;
}
...
int DeviceRunner::InitDeviceArgsCore(DeviceArgs &args, const std::vector<int64_t> &regs, const std::vector<int64_t> &regsPmu) {
    ...
    if (args.devDfxArgAddr == 0) {
        MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "Alloc devDfx info failed");
        return -1;
    }
...
void DeviceRunner::InitAiCpuSoBin(DeviceArgs &devArgs) {
    ...
    if (!ReadBytesFromFile(fileName, buffer)) {
        MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "Read bin form tilefwk_backend_server.so failed, please check the so[%s]", fileName.c_str());
        return;
    }
...
int DeviceRunner::InitAicpuServer() {
    ...
    if (ret != RT_ERROR_NONE) {
        MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "Aicpu server init failed %d", ret);
        return ret;
    }
...
int DeviceRunner::RunPreSync(rtStream_t aicpuStream, rtStream_t aicoreStream) {
    ...
    if (rc < 0) {
        MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "aclrtCreateEvent failed %d\n", rc);
        return rc;
    }

    rc = aclrtRecordEvent(event, aicoreStream);
    if (rc < 0) {
        MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "aclrtRecordEvent failed %d\n", rc);
        return rc;
    }

    rc = aclrtStreamWaitEvent(aicpuStream, event);
    if (rc < 0) {
        MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "aclrtStreamWaitEvent failed %d\n", rc);
        return rc;
    }
    return 0;
}
...
int DeviceRunner::DynamicKernelLaunch(rtStream_t aicpuStream, rtStream_t aicoreStream, DeviceKernelArgs *kernelArgs, int blockdim) {
    ...
    if (rc < 0) {
        MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "launch aicpu failed %d\n", rc);
        return rc;
    }
...
int DeviceRunner::DynamicSeparateLaunch(rtStream_t aicpuStream, rtStream_t ctrlStream, rtStream_t aicoreStream,
    ...
    if (rc < 0) {
        MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "prepare failed %d\n", rc);
        return rc;
    }
    ...
    if (rc < 0) {
        MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "launch custom aicpu failed %d\n", rc);
        return rc;
    }
    ReportHostProfInfo(startTime, blockdim, MSPROF_GE_TASK_TYPE_AI_CPU, true);
    ...
    rc = launchDynamicAiCore(aicoreStream, kernelArgs);
    if (rc < 0) {
        MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "launch aicore failed %d\n", rc);
        return rc;
    }
    ReportHostProfInfo(startTime, blockdim, MSPROF_GE_TASK_TYPE_MIX_AIC, true);
...
int DeviceRunner::DynamicLaunch(rtStream_t aicpuStream, rtStream_t ctrlStream, rtStream_t aicoreStream, [[maybe_unused]] int64_t taskId,
    ...
    if (!g_IsNullLaunched) {
        ...
        if (ret != 0) {
            MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "launch built null failed");
            return ret;
        }
    ...
    int rc = RunPrepare();
    if (rc < 0) {
        MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "Prepare failed.");
        return rc;
    }
...
int DeviceRunner::RegisterKernelBin(void **hdl, std::vector<uint8_t> *funcBinBuf) {
    ...
    if (rc != 0) {
        MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "RegisterKernelBin failed\n");
    }
    binHdl_ = *hdl;
...
int DeviceRunner::Init(void) {
    ...
    if (LoadAicpuOp::GetInstance().GetBuiltInOpBinHandle() != 0) {
        MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "Get builtInOp Funchandle failed\n");
        return -1;
    }
    ...
    if (InitDeviceArgs(args_) != 0) {
        MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "prepareArgs failed\n");
        return -1;
    }
