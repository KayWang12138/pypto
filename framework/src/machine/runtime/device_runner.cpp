...
void DeviceRunner::SetPmuEventType_DAV2201(int32_t &profPmuType) {
...
void DeviceRunner::SetPmuEventType_DAV3510(int32_t &profPmuType) {
    // 按照环境变量设置的数值，获取pmu事件类型
    switch (profPmuType) {
        case  ARITHMETIC_UTILIZATION:
...
void DeviceRunner::GetPmuEventType(DeviceArgs &args) {
    ...
    if (args.archInfo == ArchInfo::DAV_2201) {
        SetPmuEventType_DAV2201(profPmuType);
    } else if (args.archInfo == ArchInfo::DAV_3510) {
        SetPmuEventType_DAV3510(profPmuType);
    } else {
...
int DeviceRunner::InitDeviceArgs(DeviceArgs &args) {
    ...
    size_t pmuEventTypeSize = 0;
    switch (args.archInfo) {
        case ArchInfo::DAV_2201:
            pmuEventTypeSize = PMU_EVENT_TYPE_MAX_DAV2201;
            break;
        case ArchInfo::DAV_3510:
            pmuEventTypeSize = PMU_EVENT_TYPE_MAX_DAV3510;
            break;
        default:
    ...
    pmuEvtType_.resize(pmuEventTypeSize, 0x0);
