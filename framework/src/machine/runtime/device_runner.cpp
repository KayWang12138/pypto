...
void DeviceRunner::InitDynamicArgs(DeviceArgs &args) {
    ...
    for (uint64_t i = 0; i < args.nrAic + args.nrAiv + args.nrAicpuRunNum; i++) {
...
void DeviceRunner::ResetPerData() {
    ...
    for (uint64_t i = 0; i < args_.nrAic + args_.nrAiv + args_.nrAicpuRunNum; i++) {
...
int DeviceRunner::InitDeviceArgsCore(DeviceArgs &args, const std::vector<int64_t> &regs, const std::vector<int64_t> &regsPmu) {
    ...
    int nrCore = regs.size() + args.nrAicpuRunNum;
...
void DeviceRunner::AllocDfxMetricMemory() {
    for (uint32_t i = 0; i < args_.nrAic + args_.nrAiv + args_.nrAicpuRunNum; i++) {
...
#else
    int size = (args_.nrAic + args_.nrAiv + args_.nrAicpuRunNum) * SHARED_BUFFER_SIZE;
...
void DeviceRunner::Dump() {
    ...
    int coreNum = args_.nrAic + args_.nrAiv + args_.nrAicpuRunNum;
...
void DeviceRunner::DumpAiCoreExecutionTimeData() {
    ...
    for (uint32_t i = args_.nrAic + args_.nrAiv; i < args_.nrAic + args_.nrAiv + args_.nrAicpuRunNum; i++) {
