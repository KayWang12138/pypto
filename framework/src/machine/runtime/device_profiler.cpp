...
void DeviceProfiler::SetDebugEnable(const DeviceArgs& args)
{
...
void DeviceProfiler::ResetMetrics(const uint32_t& coreId)
{
    if (perfData_.empty()) {
        return;
    }
...
void DeviceProfiler::ResetPerData(const DeviceArgs& args)
{
...
void DeviceProfiler::DumpAiCoreExecutionTimeData(DeviceArgs& args)
{
    ...
    npu::tile_fwk::dynamic::DumpAicoreTaskExectInfo(args, perfData_);
}

void DeviceProfiler::DumpAiCorePmuData() { MACHINE_LOGI("TODO: DumpAiCorePmuData"); }

void DeviceProfiler::SynchronizeDeviceToHostProfData(DeviceArgs& args)
{
    if (config::GetDebugOption<int64_t>(CFG_RUNTIME_DBEUG_MODE) == CFG_DEBUG_ALL) {
        DumpAiCoreExecutionTimeData(args);
    }
}

int DeviceProfiler::RunPrepare(const DeviceArgs& args)
{
    int ret = 0;
...
void DeviceProfiler::AllocPerfData(const DeviceArgs& args)
{
    for (uint64_t i = 0; i < args.nrAic + args.nrAiv + AICPU_NUM_OF_RUN_AICPU_TASKS; i++) {
...
void DeviceProfiler::AllocDfxMetricMemory(const DeviceArgs& args)
{
    for (uint32_t i = 0; i < args.nrAic + args.nrAiv + AICPU_NUM_OF_RUN_AICPU_TASKS; i++) {
...
void DeviceProfiler::Dump(const DeviceArgs& args)
{
...
void DeviceProfiler::StartMachinePerfTraceDumpThread(const DeviceArgs& args)
{
    ...
    dumpThread_ = std::thread(&DeviceProfiler::MachinePerfTraceDumpThread, this, std::cref(args));
...
void DeviceProfiler::MachinePerfTraceDumpThread(const DeviceArgs& args)
{
    ...
    while (!dumpThreadStopFlag_.load()) {
        usleep(10000);
        npu::tile_fwk::dynamic::DumpDevTaskPerfData(args, perfData_, false);
    }
    MACHINE_LOGD("Dump thread final dump");
    npu::tile_fwk::dynamic::DumpDevTaskPerfData(args, perfData_, true);
}
