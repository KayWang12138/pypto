...
#if defined(__MIX__) && defined(__AIV__)
#define blockIndex __v_blockIdx
#define GmWorkspace __v_GmWorkspace
#endif

[[block_local]] int blockIndex;
...
INLINE void ExecStaticCoreFunctionKernel(ExecuteContext *ctx, uint32_t taskId) {
    ...
    kernel((__gm__ int64_t *)functionInfo->invokeEntryAddr,
           coreFuncData->stackWorkSpaceAddr + blockIndex * coreFuncData->stackWorkSpaceSize,
...
#endif
    CallSubFuncTask(opAttrs[0], &param, funcData->stackWorkSpaceAddr + blockIndex * funcData->stackWorkSpaceSize,
...
extern "C" __global__ __aicore__ void KERNEL_ENTRY(__OPTYPE__, __TILINGKEY__)(int64_t ffts_addr, int64_t inputs,
    ...
    blockIndex = get_block_idx() * get_subblockdim() + get_subblockid() + get_block_num();
#else
    blockIndex = get_block_idx();
#endif
    auto devArgs = (DeviceArgs*)cfgdata;
    __gm__ KernelArgs *args = (__gm__ KernelArgs *)(devArgs->sharedBuffer + blockIndex * SHARED_BUFFER_SIZE);
