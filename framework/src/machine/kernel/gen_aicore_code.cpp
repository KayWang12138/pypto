...
#if defined(__MIX__) && defined(__AIV__)
#define genBlockIdx __v_blockIdx
#define GmWorkspace __v_GmWorkspace
#endif

[[block_local]] int genBlockIdx;
...
INLINE void HandshakeClient(volatile __gm__ int64_t *shakeBuf) {
    ...
    set_cond(((int64_t)genBlockIdx << 48) | ((int64_t)get_coreid() << 32) | AICORE_REG_SAY_HELLO);
#endif
...
#endif
    CallSubFuncTask(opAttrs[0], &param, funcData->stackWorkSpaceAddr + genBlockIdx * funcData->stackWorkSpaceSize,
...
extern "C" __global__ __aicore__ void KERNEL_ENTRY(__OPTYPE__, __TILINGKEY__)(int64_t ffts_addr, int64_t inputs,
    ...
    genBlockIdx = get_block_idx() * get_subblockdim() + get_subblockid() + get_block_num();
#else
    genBlockIdx = get_block_idx();
#endif
    auto devArgs = (DeviceArgs*)cfgdata;
    __gm__ KernelArgs *args = (__gm__ KernelArgs *)(devArgs->sharedBuffer + genBlockIdx * SHARED_BUFFER_SIZE);
