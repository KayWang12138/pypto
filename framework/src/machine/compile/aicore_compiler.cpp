...
static int CompileCoreMachine(const std::string &objFile, bool isCube, uint64_t tilingKey,
  ...
  if (ret < 0) {
    MACHINE_LOGE_E(HostBackEndErr::COMPILE_AICORE_FAILED, "Compile aicore construct cmd failed.");
    return ret;
  }
...
std::string GenSubFuncCall(std::map<uint64_t, Function *> &leafDict, CoreType coreType,
  ...
  if (fsrc == nullptr) {
    MACHINE_LOGE_E(DevCommonErr::FILE_ERROR, "Fail to open call.h.");
    return "";
  }
...
static int LinkObject(const std::string &src_objs, std::string &objPath, const std::string &ccePath, bool relocate,
  ...
  if (ret < 0) {
    MACHINE_LOGE_E(HostBackEndErr::LINK_FAILED, "LinkCoreMachine construct cmd failed.");
    return ret;
  }
  ...
  if (ret != 0) {
    MACHINE_LOGE_E(HostBackEndErr::LINK_FAILED, "Link kernel failed.");
  }
  return ret;
}
...
int CompileAICoreKernel(std::map<uint64_t, Function *> &leafDict, dynamic::EncodeDevAscendFunctionParam &param,
  ...
  if (ccePath.empty()) {
    MACHINE_LOGE_E(DevCommonErr::FILE_ERROR, "No cce path.");
    return -1;
  }
  ...
  if (!GenAicoreSrcFile(aicoreSrcFile, funcHash)) {
    MACHINE_LOGE_E(HostBackEndErr::GEN_AICORE_FILE_FAILED, "Fail to generate aicore src file.");
    return -1;
  }
