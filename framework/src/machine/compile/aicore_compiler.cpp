...
static int CompileCoreMachine(const std::string &objFile, bool isCube, uint64_t tilingKey,
  ...
  if (ret < 0) {
    MACHINE_LOGE(ERROR_CODE_UNDEFINED, "Compile aicore construct cmd failed.");
    return ret;
  }
  MACHINE_LOGD("Compile ccec command:[%s].", ccecCmd.c_str());
  ret = std::system(ccecCmd.c_str());
  if (ret != 0) {
    MACHINE_LOGE(ERROR_CODE_UNDEFINED, "Compile ccec failed.");
  }
  return ret;
}
...
std::string GenSubFuncCall(std::map<uint64_t, Function *> &leafDict, CoreType coreType,
  ...
  for (const auto &iter : leafDict) {
      ...
      ASSERT(ERROR_CODE_UNDEFINED, leafFuncAttr != nullptr) << "LeafFuncAttr is null for leaf: " << leaf;
  ...
  if (fsrc == nullptr) {
    MACHINE_LOGE(ERROR_CODE_UNDEFINED, "Fail to open call.h.");
    return "";
  }
...
static int LinkObject(const std::string &src_objs, std::string &objPath, const std::string &ccePath, bool relocate,
  ...
  if (ret < 0) {
    MACHINE_LOGE(ERROR_CODE_UNDEFINED, "LinkCoreMachine construct cmd failed.");
    return ret;
  }
  ...
  if (ret != 0) {
    MACHINE_LOGE(ERROR_CODE_UNDEFINED, "Link kernel failed.");
  }
  return ret;
}
...
int CompileAICoreKernel(std::map<uint64_t, Function *> &leafDict, dynamic::EncodeDevAscendFunctionParam &param,
  ...
  if (ccePath.empty()) {
    MACHINE_LOGE(ERROR_CODE_UNDEFINED, "No cce path.");
    return -1;
  }
  ...
  if (!GenAicoreSrcFile(aicoreSrcFile, funcHash)) {
    MACHINE_LOGE(ERROR_CODE_UNDEFINED, "Fail to generate aicore src file.");
    return -1;
  }
  ...
  std::function task = [&ccePath, &funcHash, &leafDict, &param, &aic_obj, &aicoreSrcFile, &tilingKey]() {
      ...
      ASSERT(ERROR_CODE_UNDEFINED, ret == 0) << "CompileCoreMachine failed with return code  " << ret;
      src_aic_obj << mid_aic_obj;
      ret = LinkObject(src_aic_obj.str(), aic_obj, ccePath, true, "aic");
      ASSERT(ERROR_CODE_UNDEFINED, ret == 0) << "LinkObject failed with return code  " << ret;
      return;
  };
