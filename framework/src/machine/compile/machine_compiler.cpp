...
void ProcTensorParamOffset(const Function *compiledFunction, const Function* function,
    const TensorParamPackTy &tensorParam,
    InvokeParaOffset &paraOffset, std::map<int, std::pair<uint64_t, uint64_t>> &rawTensorOffsetMap,
    uint64_t &totalSize) {
    paraOffset.funcitonMagic = compiledFunction->GetFuncMagic();
...
void CalcFunctionInvokeWorkespace(Function* cacheFunction, Function* function, MachineCompileInfo& compileInfo) {
    ...
    for (uint64_t i = 0; i < compiledFunction->rootFunc_->Operations().size(); ++i) {
        ...
        for (const TensorParamPackTy &elm : subfuncInvoke.GetTensorParamList()) {
            InvokeParaOffset paraOffset;
        ...
        int incastIndx = 0;
        for (const IncastParamPackTy &elm : subfuncInvoke.GetIncastTensorParamList()) {
            InvokeParaOffset paraOffset;
        ...
        int outcastIndx = 0;
        for (const OutcastParamPackTy &elm : subfuncInvoke.GetOutcastTensorParamList()) {
            InvokeParaOffset paraOffset;
