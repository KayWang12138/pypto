...
INLINE void RecordAicorePrefTrace(uint32_t devTaskId, __gm__ Metrics* metric,
                                  AicorePerfTrace type, uint64_t cycle = 0) {
...
INLINE void PerfTraceRecord(uint32_t devTaskId, __gm__ Metrics* metric, AicorePerfTrace type, __gm__ KernelArgs *args
    ...
    RecordAicorePrefTrace(devTaskId, metric, type, cycle);
#else
...
INLINE void InitCtx(ExecuteContext *ctx, __gm__ Metrics* metric, uint64_t coreFuncData) {
    ...
    PerfTraceRecord(ctx->seqNo, metric, PERF_TRACE_CORE_DEV_TASK_RCV_MODEL, args);
