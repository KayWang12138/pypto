...
public:
    ...
    void HeaderFileEnd(std::ostringstream &exprHeaderOss) const {
        ...
        if (!header.is_open()) {
            ASSERT(ERROR_CODE_UNDEFINED, false) << "File batch_expr.h open failed!";
            return;
        }
        exprHeaderOss << "\n} // namespace npu::tile_fwk\n";
    ...
    void GenerateBatchFile(std::ostringstream &controlFlowOss, std::ostringstream &exprHeaderOss, const std::string &expName,
        ...
        for (auto& batch : batches_) {
            ...
            if (!out.is_open()) {
                ASSERT(ERROR_CODE_UNDEFINED, false) << "File set_expr open failed!";
                return;
            }
            // Write file header
...
private:
    ...
    void GenerateLinkScript() const {
        ...
        if (!file.is_open()) {
            ASSERT(ERROR_CODE_UNDEFINED, false) << "File merge.link open failed!";
            return;
        }
        file << "SECTIONS\n{\n"
             << "    . = 0x10000;\n"  // align 4K
