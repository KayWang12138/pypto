...
public:
    ...
    static bool CheckAndInsertTargetExpression(std::ofstream &out, 
        ...
        if (!foundTargetExpr && std::regex_search(exprStr, targetPattern)) {
            ...
            out << std::setw((indent + 1) * TABSIZE) << ' ' << "static bool __once_flag__ = false;\n";
            out << std::setw((indent + 1) * TABSIZE) << ' ' << "if (!__once_flag__) {\n";
            out << std::setw((indent + 2) * TABSIZE) << ' ' << "WaitForSyncFlag(startArgs);\n";
            out << std::setw((indent + 2) * TABSIZE) << ' ' << "__once_flag__ = true;\n";
            out << std::setw((indent + 1) * TABSIZE) << ' ' << "}\n";
            out << std::setw(indent * TABSIZE) << ' ' << "}\n";
