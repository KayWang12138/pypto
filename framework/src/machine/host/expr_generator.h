...
public:
    ...
    void GenerateBatchFile(SymbolicExpressionTable *exprTable, std::ostringstream &controlFlowOss, std::ostringstream &exprHeaderOss,
        ...
        int indent, int devRootKey, std::unordered_map<std::string, bool> &tensorNameToDependConst) {
        ...
        for (auto& batch : batches_) {
            ...
            for (size_t idx = batch.startExprIndex; idx < batch.endExprIndex; idx++) {
                ...
                auto exprStr = exprTable->BuildExpression(expr, [&tensorNameToDependConst, &needSync](const std::string &tensorName) {
                    auto it = tensorNameToDependConst.find(tensorName);
                    ASSERT(it != tensorNameToDependConst.end())<<"Tensor "<<tensorName<<" not found in tensorNameToDependConst";
                    FUNCTION_LOGI("Value depend TensorName:%s dependCnnst:%d.", tensorName.c_str(), it->second);
                    if (!it->second) {
                        needSync = true;
                    }
                });
