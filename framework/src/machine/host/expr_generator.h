    ...
    void GenerateBatchFile(SymbolicExpressionTable *exprTable, std::ostringstream &controlFlowOss, std::ostringstream &exprHeaderOss,
        ...
        for (auto& batch : batches_) {
            ...
            for (size_t idx = batch.startExprIndex; idx < batch.endExprIndex; idx++) {
                ...
                auto exprStr = exprTable->BuildExpression(expr, [&tensorNameToDependCore, &needSync](const std::string &tensorName) {
                    auto it = tensorNameToDependCore.find(tensorName);
                    ASSERT(it != tensorNameToDependCore.end())<<"Tensor "<<tensorName<<" not found in tensorNameToDependCore";
