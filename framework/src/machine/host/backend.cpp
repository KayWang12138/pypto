...
static bool CheckShapeEquality(Shape &shape, std::vector<SymbolicScalar>&dynShape,
                         ...
                         std::unordered_set<std::string> &mainBlockStrSet)
{
    for (auto &op : func->Operations()) {
        for (auto &iop : op.GetIOperands()) {
            ...
            auto cond = CheckShapeEquality(iop->shape, iop->GetDynValidShapeOri(), mainBlockCondGroup, mainBlockStrSet);
            if (!cond) {
        ...
        for (auto &oop : op.GetOOperands()) {
            ...
            auto cond = CheckShapeEquality(oop->shape, oop->GetDynValidShapeOri(), mainBlockCondGroup, mainBlockStrSet);
            if (!cond) {
