...
static void CompileDyndevFunction(Function* function, FunctionCache& cache, [[maybe_unused]] const std::string& ccePath)
{
    ...
    }
    {
        ...
        if (slotMappingOf.is_open()) {
            slotMappingOf << "frontendSlotIdx,runtimeSlotIdx\n";
            for (auto& slot : slotIdxMapping) {
                slotMappingOf << slot.first << "," << slot.second << "\n";
            }
            slotMappingOf.close();
    ...
    std::string staticTopoPath = config::LogTopFolder() + "/static_topo.csv";
    std::ofstream staticTopoOf(staticTopoPath);
