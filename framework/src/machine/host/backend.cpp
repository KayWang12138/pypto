                         ...
                         std::unordered_set<std::string> &mainBlockStrSet, bool isCallOpFlag = false)
{
    for (auto &op : func->Operations()) {
        for (auto &iop : op.GetIOperands()) {
            ...
            const auto& dynShape = isCallOpFlag ? iop->GetDynValidShapeOri() : iop->GetDynValidShape();
        ...
        for (auto &oop : op.GetOOperands()) {
            ...
            const auto& dynShape = isCallOpFlag ? oop->GetDynValidShapeOri() : oop->GetDynValidShape();
