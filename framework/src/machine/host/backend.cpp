...
void GetReadyOnHostTensorsSet(std::unordered_set<int> &readyOnHostTensorsSet) {
    ...
    // Convert readyOnHostTensors (string list) to unordered_set<int>
    for (const auto &tensorStr : readyOnHostTensors) {
        try {
            int tensorId = std::stoi(tensorStr);
            FUNCTION_LOGE("tensorId %d", tensorId);
            readyOnHostTensorsSet.insert(tensorId);
        } catch (const std::exception &e) {
            FUNCTION_LOGE("Failed to convert tensor string '%s' to int: %s", tensorStr.c_str(), e.what());
        }
    }
}
...
static void BuildControlFlow(FunctionCache &cache, Linker &linker, const std::string &sectionName,
    ...
    if (funcType == FunctionType::DYNAMIC) {
        ...
        for (size_t idx = 0; idx < inputNameList.size(); idx++) {
            ...
            if (readyOnHostTensorsSet.find(idx) != readyOnHostTensorsSet.end()) {
