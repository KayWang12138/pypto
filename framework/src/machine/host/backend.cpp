    ...
    if (funcType == FunctionType::DYNAMIC) {
        ...
        for (size_t idx = 0; idx < inputNameList.size(); idx++) {
            ...
            if (tensor && tensor->GetStorage(false)) {
                tensorNameToDependCore[inputName] = tensor->GetStorage(false)->GetRawTensor()->GetReadyOnAicoreStart();
            } else {
                tensorNameToDependCore[inputName] = true; // 默认为 DEPEND_CONST，对应 true
            }
        }
        ...
        for (size_t idx = 0; idx < outputNameList.size(); idx++) {
            ...
            if (tensor && tensor->GetStorage(false)) {
                tensorNameToDependCore[outputName] = tensor->GetStorage(false)->GetRawTensor()->GetReadyOnAicoreStart();
            } else {
                tensorNameToDependCore[outputName] = true; // 默认为 DEPEND_CONST，对应 true
            }
        }
