...
static void InsertWaitCoreSync(SymbolicExpressionTable *exprTable, std::ostringstream &controlFlowOss,
    ...
    for (const auto &expr : primaryExprs) {
        if (!needSync && exprTable->CheckExprDependCore(expr, valDependTensorMeta.tensorNameToDependCore, valDependTensorMeta.valDependMap)) {
            needSync = true;
            break;
        }
    }
    if (needSync) {
        controlFlowOss << std::string(indent, ' ') << "WaitCoreSync();\n";
    }
}
...
static void BuildControlFlow(FunctionCache &cache, Linker &linker, const std::string &sectionName,
    ...
    if (funcType == FunctionType::DYNAMIC) {
        ...
        if (exprTable != nullptr) {
            InsertWaitCoreSync(exprTable, controlFlowOss, valDependTensorMeta);
