...
static bool CheckAndInsertTargetExpression(std::ostringstream &controlFlowOss, 
    ...
    if (!foundTargetExpr && std::regex_search(exprStr, targetPattern)) {
        ...
        controlFlowOss << std::setw((indent + 2) * TABSIZE) << ' ' << "DEV_IF_DEVICE {\n";
        controlFlowOss << std::setw((indent + 3) * TABSIZE) << ' ' << "uint64_t start = GetCycles();\n";
        controlFlowOss << std::setw((indent + 3) * TABSIZE) << ' ' << "while ((startArgs->devProg->devArgs.disableSync == 0) && startArgs->syncFlag != 1) {\n";
        controlFlowOss << std::setw((indent + 4) * TABSIZE) << ' ' << "if (GetCycles() - start > HAND_SHAKE_TIMEOUT) {\n";
        controlFlowOss << std::setw((indent + 5) * TABSIZE) << ' ' << "DEV_ERROR(\"Wait sync flag timeout.\");\n";
        controlFlowOss << std::setw((indent + 5) * TABSIZE) << ' ' << "break;\n";
        controlFlowOss << std::setw((indent + 4) * TABSIZE) << ' ' << "}\n";
