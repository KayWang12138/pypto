...
static constexpr size_t EXPR_PER_BATCH = 100000;
...
static void BuildControlFlow(FunctionCache &cache, Linker &linker, const std::string &sectionName,
    ...
    auto funcType = func->GetFunctionType();
        ...
        //   - 辅助函数将在 ControlFlowEntry 之后生成
        //
        // 用于存储辅助函数代码的缓冲区
        std::vector<std::string> subFuncCodeBuffer;
        // 用于存储辅助函数声明的缓冲区
        std::vector<std::string> subFuncDeclBuffer;
        ...
        //
        // 输出顺序：
        //   - 函数声明在 ControlFlowEntry 之前（subFuncDeclBuffer）
        //   - 函数实现在 ControlFlowEntry 之后（subFuncCodeBuffer）
        //
        ...
        for (auto &[devRootKey, exprTable] : exprTablesToSplit) {
            ...
            for (size_t batchIdx = 0; batchIdx < batchCount; batchIdx++) {
                ...
                std::ostringstream subFuncDeclOss;
                subFuncDeclOss << "__attribute__((section(\"" << sectionName
                    << "\")))\n"
                    << "static uint64_t SetExprSubFunc" << g_globalFuncIdx
                    << "(int64_t *symbolTable, uint64_t *exprList" << devRootKey << ");\n";
                subFuncDeclBuffer.push_back(subFuncDeclOss.str());

                std::ostringstream subFuncOss;
                subFuncOss << "__attribute__((section(\"" << sectionName
                    << "\")))\n"
                    << "static uint64_t SetExprSubFunc" << g_globalFuncIdx
                    << "(int64_t *symbolTable, uint64_t *exprList" << devRootKey << ") {\n";
                ...
                for (size_t exprIdx = startIdx; exprIdx < endIdx; exprIdx++) {
                    ...
                    //      index - 表达式在列表中的索引
                    //      exprStr - 表达式的字符串表示
                    subFuncOss << "  RUNTIME_SetExpr(exprList" << devRootKey << ", " << index << ", " << exprStr << ");\n";
                }
                ...
                // 将辅助函数代码保存到缓冲区，稍后输出
                subFuncCodeBuffer.push_back(subFuncOss.str());
        ...
        // ========================================================================
        // 第三阶段：输出辅助函数声明（在 ControlFlowEntry 之前）
        // ========================================================================
        // 目的：提供函数前向声明，确保 ControlFlowEntry 中调用辅助函数时编译通过
        for (const auto &decl : subFuncDeclBuffer) {
            controlFlowOss << decl;
        }

        // ========================================================================
        // 第四阶段：生成 ControlFlowEntry 主函数
        // ========================================================================
        controlFlowOss << BuildControlFlowCallee(func, 0)
            << "__attribute__((section(\"" << sectionName
            << "\")))\n"
        ...
        // ========================================================================
        // 第五阶段：输出 SetExprSubFunc 辅助函数实现（在 ControlFlowEntry 之后）
        // ========================================================================
        // 目的：确保辅助函数在主函数之后定义
        for (const auto &code : subFuncCodeBuffer) {
            controlFlowOss << code;
        }
                ...
                if (found) {
                    ...
                    for (size_t i = 0; i < batchCount; i++) {
                        controlFlowOss << std::setw(indent * TABSIZE) << ' ' 
                            ...
                            << "(symbolTable, exprList" << devRootKey << ");\n";
                    }
                } else {
