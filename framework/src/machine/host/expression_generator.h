...
struct ExprBatchInfo {
    ...
    std::set<std::string> dependArgs;
 public:
    std::string GetDependArgs() {
        std::string dependArgsStr;
        for (const auto& arg : dependArgs) {
            dependArgsStr += ", " + arg;
        }
        printf("batchIndex:%zu dependArgsStr: %s\n", batchIndex, dependArgsStr.c_str());
        return dependArgsStr;
    }
};
...
public:
    ...
    void GenerateHeaderFile() const {
        std::string headerPath = outputDir_ + "/batch_expr_" + std::to_string(devRootKey_) + ".h";
        ...
        header << "#pragma once\n"
               << "#include <cstdint>\n\n"
               << "namespace npu::tile_fwk {\n\n";
        for (const auto& headDec : headDecs_) {
            header << headDec << "\n";
        }
        header << "\n} // namespace npu::tile_fwk\n";
        header.close();
    ...
    std::string GenerateBatchFile(ExprBatchInfo& batch, const std::string &expName,
        ...
        for (const auto& expr : expressions) {
            ...
            if (currentIdx >= batch.startExprIndex && currentIdx < batch.endExprIndex) {
                ...
                exprOss << "    RUNTIME_SetExpr(exprList, " << currentIdx << ", " << exprStr << ");\n";
            }
        ...
        for (const auto& arg : batch.dependArgs) {
            header << ", int64_t " << arg;
        }
        out << header.str() << ") {\n";
        header << ");";
        headDecs_.emplace_back(header.str());
        out << exprOss.str();
        out << "}\n\n"
            << "} // namespace npu::tile_fwk\n";
        out.close();
...
private:
    ...
    std::vector<std::string> headDecs_;
};
