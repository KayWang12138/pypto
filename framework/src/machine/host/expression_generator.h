...
public:
    ...
    std::string GenerateBatchFile(ExprBatchInfo& batch, const std::string &expName,
        ...
        out << "void " << batch.functionName << "(void *ctx, int64_t *symbolTable, RuntimeCallEntryType runtimeCallList[], DevStartArgsBase *startArgs, uint64_t *exprList" <<
            << batch.GetDependArgs() << ") {\n";
        out << exprOss.str();
        out << "}\n\n"
            << "} // namespace npu::tile_fwk\n";
