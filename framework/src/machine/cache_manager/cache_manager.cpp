...
bool CacheManager::Initialize() {
    ...
    if (envPath == nullptr) {
        MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "Env[HOME] is not existed or empty.");
        return false;
    }
    ...
    if (RealPath(cacheDirPath_).empty() && !CreateMultiLevelDir(cacheDirPath_)) {
        MACHINE_LOGE_E(ERROR_CODE_UNDEFINED, "Failed to create cache dir[%s].", cacheDirPath_.c_str());
        return false;
    }
