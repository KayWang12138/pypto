...
bool CacheManager::Initialize() {
    ...
    if (envPath == nullptr) {
        MACHINE_LOGE_E(DevCommonErr::GET_ENV_FAILED, "Env[HOME] is not existed or empty.");
        return false;
    }
    ...
    if (RealPath(cacheDirPath_).empty() && !CreateMultiLevelDir(cacheDirPath_)) {
        MACHINE_LOGE_E(DevCommonErr::FILE_ERROR, "Failed to create cache dir[%s].", cacheDirPath_.c_str());
        return false;
    }
