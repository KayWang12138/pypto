...
public:
    ...
    inline int32_t PrepareTask(uint64_t taskId, const npu::tile_fwk::dynamic::DevRelocVector<int32_t>& aicpuCode)
    {
        ...
        DEV_DEBUG( 
            "PrepareTask baseAddr=0x%lx, actualAddr=0x%lx, ownerRank=%u, actual rawShape=[%lu, %d], actual offset=[%u, %d]," 
            "buffer maxTileNum=%lu, bufferStride=%u", 
        ...
        DEV_DEBUG(
            ...
            "logical offset=%s, ownerRank=%u, actual rawShape=[%lu, %d], actual offset=[%u, %d],"
            "buffer maxTileNum=%lu, bufferStride=%u",
            info.rawAddr, reinterpret_cast<uint64_t>(addr), FormatArray(&paramInfo_.rawShape[1], dataDim).c_str(),
            FormatArray(paramInfo_.tileShape, tileShapeDim).c_str(), FormatArray(&info.offset[1], dataDim).c_str(),
