...
public:
    ...
    inline int32_t PrepareTask(uint64_t taskId, const npu::tile_fwk::dynamic::DevRelocVector<int32_t> &aicpuCode) {
        ...
        DEV_ERROR(DistributedErrorCode::NULLPTR, "ShmemWaitUntilImpl::EnqueueOp logical rawShape=[%u, %u, %u],"
            ...
            paramInfo_.expandDimRawShape, paramInfo_.rawShapeRow, paramInfo_.rawShapeCol, paramInfo_.tileShapeRow, paramInfo_.tileShapeCol,
            info.offset[EXPAND_DIM_INDEX], info.offset[SHMEM_DIM_ROW], info.offset[SHMEM_DIM_COL], info.offset[OWNER_RANK_ID_INDEX],
            paramInfo_.rankNum, paramInfo_.expandDimRawShape, totalTileNum,
            info.offset[OWNER_RANK_ID_INDEX], info.offset[EXPAND_DIM_INDEX], tileIndex, paramInfo_.maxTileNum, paramInfo_.bufferStride);

        int32_t* addr = reinterpret_cast<int32_t*>(info.rawAddr) +
            CalcLinearOffset(paramInfo_.expandDimRawShape, totalTileNum,
            info.offset[OWNER_RANK_ID_INDEX], info.offset[EXPAND_DIM_INDEX], tileIndex) * paramInfo_.bufferStride;
