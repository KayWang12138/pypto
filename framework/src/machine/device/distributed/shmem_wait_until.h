...
public:
    ...
    inline int32_t PrepareTask(uint64_t taskId, const npu::tile_fwk::dynamic::DevRelocVector<int32_t> &aicpuCode) {
        ...
        DEV_DEBUG("ShmemWaitUntilImpl::EnqueueOp logical rawShape=[%u, %u],"
            "logical tile=[%u, %u], logical offset=[%u, %u], ownerRank=%u,"
            "actual rawShape=[%u, %u, %d], actual offset=[%u, %d], buffer maxTileNum=%u, bufferStride=%u",
            ...
            info.offset[OWNER_RANK_ID_INDEX], tileIndex, paramInfo_.maxTileNum, paramInfo_.bufferStride);

        int32_t* addr = reinterpret_cast<int32_t*>(info.rawAddr) +
            CalcLinearOffset(totalTileNum,
            info.offset[OWNER_RANK_ID_INDEX], tileIndex) * paramInfo_.bufferStride;
