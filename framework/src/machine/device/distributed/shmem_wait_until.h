...
public:
    ...
    SignalTileOp* CreateTaskData(uint32_t taskId, int32_t *addr, int32_t expectSum, bool resetSignal) {
        if (taskCount >= AICPU_TASK_ARRAY_SIZE) {
            DEV_ERROR(SchedError::SIGNAL_QUEUE_OVERFLOW, "ctrl.task.pre.task.create",
    ...
    int32_t InsertTask(uint32_t taskId, int32_t *addr, int32_t expectSum, bool resetSignal) {
        ...
        if (newTask == nullptr) {
            DEV_ERROR(SchedError::SIGNAL_QUEUE_OVERFLOW, "ctrl.task.pre.task.insert",
...
public:
    ...
    inline int32_t Enqueue(SignalTileOp* task) {
        ...
        if (rear_ == front_) {
            DEV_ERROR(SchedError::SIGNAL_QUEUE_OVERFLOW, "ctrl.task.pre.task.enqueue",
    ...
    inline int32_t Dequeue() {
        if (IsEmpty()) {
            DEV_ERROR(SchedError::QUEUE_DEQUEUE_WHEN_EMPTY, "sche.task.end.task.dequeue",
                      "# Queue is empty.");
            return dynamic::DEVICE_MACHINE_ERROR;
        }
