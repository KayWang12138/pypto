...
public:
    ...
    inline int32_t PrepareTask(uint64_t taskId, const npu::tile_fwk::dynamic::DevRelocVector<int32_t>& aicpuCode)
    {
        ...
        int32_t tileCols = (paramInfo_.rawShape[4] + paramInfo_.tileShapeCol - 1) / paramInfo_.tileShapeCol;
        int32_t tileRows = (paramInfo_.rawShape[3] + paramInfo_.tileShapeRow - 1) / paramInfo_.tileShapeRow;
        ...
        int32_t* addr = reinterpret_cast<int32_t*>(info.rawAddr) + 
            CalcLinearOffset(paramInfo_.rawShape[1], paramInfo_.rawShape[2], 
                totalTileNum, info.offset[0], info.offset[1], info.offset[2], tileIndex) * paramInfo_.bufferStride;
        ...
        DEV_DEBUG( 
            ...
            info.rawAddr, reinterpret_cast<uint64_t>(addr), paramInfo_.rawShape[3], paramInfo_.rawShape[4],
            paramInfo_.tileShapeRow, paramInfo_.tileShapeCol, info.offset[3], info.offset[4], 
            info.offset[MAX_TENSOR_DIM - info.dim], GetRankNum(hcclContextAddr_, info.vaddr), 
            totalTileNum, info.offset[MAX_TENSOR_DIM - info.dim], tileIndex, 
