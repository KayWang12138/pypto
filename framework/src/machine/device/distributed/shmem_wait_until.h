...
public:
    ...
    inline int32_t PrepareTask(uint64_t taskId, const npu::tile_fwk::dynamic::DevRelocVector<int32_t>& aicpuCode)
    {
        ...
        uint32_t tileShapeDim = paramInfo_.tileShapeDim;
        ...
        // DEV_DEBUG( 
        //     "PrepareTask baseAddr=0x%lx, actualAddr=0x%lx, logical rawShape=[%u,%u,%u,%u], logical tile=[%u,%u,%u,%u]," 
        //     "logical offset=[%u,%u,%u,%u], ownerRank=%u, actual rawShape=[%lu, %d], actual offset=[%u, %d]," 
        //     "buffer maxTileNum=%lu, bufferStride=%u", 
        //     info.rawAddr, reinterpret_cast<uint64_t>(addr), paramInfo_.rawShape[1], paramInfo_.rawShape[2], 
        //     paramInfo_.rawShape[3], paramInfo_.rawShape[4], paramInfo_.tileShape[0], paramInfo_.tileShape[1], 
        //     paramInfo_.tileShape[2], paramInfo_.tileShape[3], info.offset[1], info.offset[2], info.offset[3], 
        //     info.offset[4], info.offset[OWNER_RANK_ID_INDEX], GetRankNum(hcclContextAddr_, info.vaddr), totalTileNum,
        //     info.offset[OWNER_RANK_ID_INDEX], tileIndex, TileOp::Distributed::DecodeShmemAddrMaxTileNum(info.vaddr),
        //     paramInfo_.bufferStride);

        DEV_DEBUG( 
            "PrepareTask baseAddr=0x%lx, actualAddr=0x%lx, ownerRank=%u, actual rawShape=[%lu, %u], actual offset=[%u, %u]," 
            "buffer maxTileNum=%lu, bufferStride=%u", 
            info.rawAddr, reinterpret_cast<uint64_t>(addr), info.offset[OWNER_RANK_ID_INDEX], GetRankNum(hcclContextAddr_, info.vaddr), 
            totalTileNum, info.offset[OWNER_RANK_ID_INDEX], tileIndex, TileOp::Distributed::DecodeShmemAddrMaxTileNum(info.vaddr),
            paramInfo_.bufferStride);
        
        DEV_DEBUG(
            "PrepareTask baseAddr=0x%lx, actualAddr=0x%lx, logical rawShape=%s, logical tile=%s,"
            "logical offset=%s, ownerRank=%u, actual rawShape=[%lu, %u], actual offset=[%u, %u],"
            "buffer maxTileNum=%lu, bufferStride=%u",
            info.rawAddr, reinterpret_cast<uint64_t>(addr), FormatArray(&paramInfo_.rawShape[1], tileShapeDim).c_str(),
            FormatArray(paramInfo_.tileShape, tileShapeDim).c_str(), FormatArray(&info.offset[1], tileShapeDim).c_str(),
