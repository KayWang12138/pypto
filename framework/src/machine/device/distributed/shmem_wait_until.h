...
public:
    ...
    inline int32_t PrepareTask(uint64_t taskId, const npu::tile_fwk::dynamic::DevRelocVector<int32_t>& aicpuCode)
    {
        ...
        int32_t totalTileNum = tileRows * tileCols;
        DEV_DEBUG(
            "ShmemWaitUntilImpl::EnqueueOp logical rawShape=[%u, %u, %u, %u],"
            "logical tile=[%u, %u], logical offset=[%u, %u, %u, %u], ownerRank=%u,"
            "actual rawShape=[%u, %d], actual offset=[%u, %d], buffer maxTileNum=%u, bufferStride=%u",
            paramInfo_.rawShape[1], paramInfo_.rawShape[2], paramInfo_.rawShape[3], paramInfo_.rawShape[4], 
            paramInfo_.tileShapeRow, paramInfo_.tileShapeCol,
            info.offset[1], info.offset[2], info.offset[3], info.offset[4], info.offset[0],
