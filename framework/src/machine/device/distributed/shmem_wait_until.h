...
public:
    ...
    int32_t InsertTask(uint32_t taskId, int32_t *addr, int32_t expectSum, bool resetSignal) {
        ...
        if (newTask == nullptr) {
            DEV_ERROR("[memory.task.insert] task creation returned null for taskId=%u.", taskId);
...
public:
    ...
    inline int32_t Dequeue() {
        if (IsEmpty()) {
            DEV_ERROR("[memory.queue.dequeue] queue is empty.");
            return dynamic::DEVICE_MACHINE_ERROR;
        }
...
public:
    ...
    inline int32_t EnqueueOp(uint64_t taskId, TaskStat* taskStat) {
        ...
        if (task == nullptr) {
            DEV_ERROR("[memory.op.enqueue] taskId %lu not found in hashMap.", taskId);
            return dynamic::DEVICE_MACHINE_ERROR;
        }
        if (taskStat != nullptr) {
