...
public:
    ...
    SignalTileOp* CreateTaskData(uint32_t taskId, int32_t *addr, int32_t expectSum, bool resetSignal) {
        if (taskCount >= AICPU_TASK_ARRAY_SIZE) {
            DEV_ERROR(MachineDistributedErr::AICPU_TASK_NUM_EXCEED_LIMIT, "taskCount=%u >= AICPU_TASK_ARRAY_SIZE=%lu", taskCount, AICPU_TASK_ARRAY_SIZE);
            return nullptr;
        }
...
public:
    ...
    inline int32_t Enqueue(SignalTileOp* task) {
        ...
        if (rear_ == front_) {
            DEV_ERROR(MachineDistributedErr::AICPU_TASK_NUM_EXCEED_LIMIT, "SignalTileOp queue_ is full, front=%u, rear=%u", front_, rear_);
            return dynamic::DEVICE_MACHINE_ERROR;
        }
        return dynamic::DEVICE_MACHINE_OK;
    }
    ...
    inline int32_t Dequeue() {
        if (IsEmpty()) {
            DEV_ERROR(MachineDistributedErr::AICPU_TASK_QUEUE_EMPTY, "Queue is empty.");
            return dynamic::DEVICE_MACHINE_ERROR;
        }
...
public:
    ...
    inline int32_t EnqueueOp(uint64_t taskId, TaskStat* taskStat) {
        ...
        if (task == nullptr) {
            DEV_ERROR(MachineDistributedErr::AICPU_TASKID_NOT_IN_MAP, "taskId=%lu not found", taskId);
            return dynamic::DEVICE_MACHINE_ERROR;
        }
        if (taskStat != nullptr) {
