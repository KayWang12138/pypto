...
public:
    ...
    inline int32_t PrepareTask(uint64_t taskId, const npu::tile_fwk::dynamic::DevRelocVector<int32_t> &aicpuCode) {
        ...
        DEV_DEBUG("ShmemWaitUntil::PrepareTask aicpuCode.size=%u", static_cast<unsigned>(codeSize));
        for (size_t i = 0; i < codeSize; ++i) {
            DEV_DEBUG("ShmemWaitUntil::PrepareTask aicpuCode[%u]=%d", static_cast<unsigned>(i), aicpuCode[i]);
        }
        ...
        DEV_DEBUG("ShmemWaitUntil::EnqueueOp offset0=%u offset1=%u offset2=%u offset3=%u offset4=%u tileIndex=%d totalTileNum=%d",
