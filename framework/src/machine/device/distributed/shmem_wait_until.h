...
public:
    ...
    inline int32_t PrepareTask(uint64_t taskId, const npu::tile_fwk::dynamic::DevRelocVector<int32_t> &aicpuCode) {
        ...
        int32_t totalTileNum = (paramInfo_.rawShapeRow / paramInfo_.tileShapeRow + (paramInfo_.rawShapeRow % paramInfo_.tileShapeRow == 0 ? 0 : 1)) *
            (paramInfo_.rawShapeCol / paramInfo_.tileShapeCol + (paramInfo_.rawShapeCol % paramInfo_.tileShapeCol == 0 ? 0 : 1));
