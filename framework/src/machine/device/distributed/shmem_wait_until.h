...
public:
    ...
    SignalTileOp* CreateTaskData(uint32_t taskId, int32_t *addr, int32_t expectSum, bool resetSignal) {
        if (taskCount >= AICPU_TASK_ARRAY_SIZE) {
            DEV_ERROR("[ctrl.task.pre.task.create] Task array is full: taskCount=%u, maxSize=%lu.", taskCount, AICPU_TASK_ARRAY_SIZE);
            return nullptr;
        }
    ...
    int32_t InsertTask(uint32_t taskId, int32_t *addr, int32_t expectSum, bool resetSignal) {
        ...
        if (newTask == nullptr) {
            DEV_ERROR("[ctrl.task.pre.task.insert] New task creation returned null for taskId=%u.", taskId);
...
public:
    ...
    inline int32_t Enqueue(SignalTileOp* task) {
        ...
        if (rear_ == front_) {
            DEV_ERROR("[ctrl.task.pre.task.enqueue] SignalTileOp queue is full: front=%u, rear=%u, resize required.", front_, rear_);
            return dynamic::DEVICE_MACHINE_ERROR;
        }
        return dynamic::DEVICE_MACHINE_OK;
    }
...
public:
    ...
    inline int32_t EnqueueOp(uint64_t taskId, TaskStat* taskStat) {
        ...
        if (task == nullptr) {
            DEV_ERROR("[ctrl.task.pre.task.enqueue] taskId %lu not found in hashMap.", taskId);
            return dynamic::DEVICE_MACHINE_ERROR;
        }
        if (taskStat != nullptr) {
