...
public:
    ...
    SignalTileOp* CreateTaskData(uint32_t taskId, int32_t *addr, int32_t expectSum, bool resetSignal) {
        if (taskCount >= AICPU_TASK_ARRAY_SIZE) {
            DEV_ERROR("[memory.task.create] Task array is full: taskCount=%u, maxSize=%lu.", taskCount, AICPU_TASK_ARRAY_SIZE);
            return nullptr;
        }
    ...
    int32_t InsertTask(uint32_t taskId, int32_t *addr, int32_t expectSum, bool resetSignal) {
        ...
        if (newTask == nullptr) {
            DEV_ERROR("[memory.task.insert] InsertTask failed: task creation returned null for taskId=%u.", taskId);
...
public:
    ...
    inline int32_t Enqueue(SignalTileOp* task) {
        ...
        if (rear_ == front_) {
            DEV_ERROR("[memory.queue.enqueue] SignalTileOp queue is full: front=%u, rear=%u, resize required.", front_, rear_);
            return dynamic::DEVICE_MACHINE_ERROR;
        }
        return dynamic::DEVICE_MACHINE_OK;
    }
    ...
    inline int32_t Dequeue() {
        if (IsEmpty()) {
            DEV_ERROR("[memory.queue.dequeue] Dequeue failed: queue is empty.");
            return dynamic::DEVICE_MACHINE_ERROR;
        }
...
public:
    ...
    inline int32_t EnqueueOp(uint64_t taskId, TaskStat* taskStat) {
        ...
        if (task == nullptr) {
            DEV_ERROR("[memory.op.enqueue] EnqueueOp failed: taskId %lu not found in hashMap.", taskId);
            return dynamic::DEVICE_MACHINE_ERROR;
        }
        if (taskStat != nullptr) {
