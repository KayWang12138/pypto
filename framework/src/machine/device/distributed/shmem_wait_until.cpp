...
uint64_t ShmemWaitUntilImpl::GetRawAddr(const uint64_t addr, const uint64_t dstRankId)
{
    (void)dstRankId;
...
TensorInfo ShmemWaitUntilImpl::GetTensorInfo(uint64_t taskId, const npu::tile_fwk::dynamic::DevRelocVector<int32_t> &aicpuCode)
{
    ...
    info.rawAddr = ShmemWaitUntilImpl::GetRawAddr(funcData.rawTensorAddr[desc->offsetOrIndex], 0);
    return info;
}
} // namespace npu::tile_fwk::Distributed
