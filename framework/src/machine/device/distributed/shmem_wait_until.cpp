...
void ShmemWaitUntil::EnqueueOp(uint64_t taskId, const npu::tile_fwk::dynamic::DevRelocVector<int32_t> &aicpuCode)
{
    ...
    const uint32_t offset4 = info.offset[4]; // offset 4
    const uint32_t shape3 = info.shape[3]; // shape 3
    const uint32_t shape4 = info.shape[4]; // shape 4
    const uint32_t rawShape2 = info.rawShape[2]; // raw shape 2
    const uint32_t rawShape3 = info.rawShape[3]; // raw shape 3
    const uint32_t rawShape4 = info.rawShape[4]; // raw shape 4
    const int32_t expectedSum = info.expectedSum;
    const int32_t resetSignal = info.resetSignal;

    DEV_DEBUG("ShmemWaitUntil::EnqueueOp offset1=%u, offset2=%u, offset3=%u, offset4=%u, shape3=%u, shape4=%u, rawShape3=%u, rawShape4=%u",
        offset1, offset2, offset3, offset4, shape3, shape4, rawShape3, rawShape4);
    int32_t stride = aicpuCode[paramInfo_.attrIndex + 1];
    int32_t tileIndex = (offset3 / shape3) * ((rawShape4 + shape4 - 1) / shape4) + (offset4 / shape4);
    int32_t totalTileNum = (rawShape3 / shape3 + (rawShape3 % shape3 == 0 ? 0 : 1)) *
        (rawShape4 / shape4 + (rawShape4 % shape4 == 0 ? 0 : 1));
    int32_t* addr = reinterpret_cast<int32_t*>(info.rawAddr) + offset1 * rawShape2 * totalTileNum * stride +
        (offset2 * totalTileNum + tileIndex) * stride;
    ...
    signalTileOp_[signalTileOpCount_].Init(taskId, addr, 1, stride, expectedSum, resetSignal);
    ++signalTileOpCount_;
}
