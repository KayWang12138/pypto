    ...
    uint64_t taskId, const npu::tile_fwk::dynamic::DevRelocVector<int32_t>& aicpuCode)
{
    ...
    for (uint32_t i = 0; i < info.dim && i < 6; ++i) {
        info.offset[i + (6 - info.dim)] = GetCoa(index + i, opAttrs, expressionTable);
    }

    // info.offset = GetCoaVector(index, info.dim, opAttrs, expressionTable);

    inline std::vector<uint32_t> GetCoaVector(
        const uint32_t index, const uint32_t dim, uint64_t* opAttrs, uint64_t* expressionTable)
    {
        std::vector<uint32_t> vec(dim);
        for (uint32_t i = 0; i < dim; ++i) {
            vec[i] = GetCoa(index + i, opAttrs, expressionTable);
        }
        return vec;
    }
