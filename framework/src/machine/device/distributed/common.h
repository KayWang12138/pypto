...
inline AicpuParamInfo DecodeAicpuCode(const npu::tile_fwk::dynamic::DevRelocVector<int32_t>& aicpuCode)
{
    ...
    paramInfo.expandDimRawShape = aicpuCode[paramInfo.rawShapeIndex + 1]; // ShmemSignal RawShape[ranksize, ranksize, ranksize, row, col], 2表示rankShape的值
    ...
    paramInfo.shapeIndex = paramInfo.rawShapeIndex + aicpuCode[index] / 2; // 存储了signal_dim * 2个参数, tieShape往后偏移dim位
    paramInfo.shapeRow = aicpuCode[paramInfo.shapeIndex + 2]; // ShmemSignal Shape[ranksize, ranksize, ranksize, row, col], 3表示row的值
    paramInfo.shapeCol = aicpuCode[paramInfo.shapeIndex + 3]; // ShmemSignal Shape[ranksize, ranksize, ranksize, row, col], 4表示col的值
    index = index + aicpuCode[index] + 1;
