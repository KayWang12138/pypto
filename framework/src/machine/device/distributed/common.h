...
struct TensorInfo {
    ...
    uint32_t offset[MAX_TENSOR_DIM] = {0, 0, 0, 0, 0};
...
struct AicpuParamInfo {
    ...
    uint32_t rawShape[MAX_TENSOR_DIM] = {1, 1, 1, 1, 1};
    uint32_t shape[MAX_TENSOR_DIM] = {1, 1, 1, 1, 1};
    uint32_t dim{0};
...
inline AicpuParamInfo DecodeAicpuCode(const npu::tile_fwk::dynamic::DevRelocVector<int32_t>& aicpuCode)
{
    ...
    for (uint32_t i = 0; i < paramInfo.dim && i < MAX_TENSOR_DIM; ++i) {
        paramInfo.rawShape[i + (MAX_TENSOR_DIM - paramInfo.dim)] = aicpuCode[paramInfo.rawShapeIndex + i];
    }

    paramInfo.shapeIndex = paramInfo.rawShapeIndex + paramInfo.dim;

    for (uint32_t i = 0; i < paramInfo.dim && i < MAX_TENSOR_DIM; ++i) {
        paramInfo.shape[i + (MAX_TENSOR_DIM - paramInfo.dim)] = aicpuCode[paramInfo.shapeIndex + i];
    }
