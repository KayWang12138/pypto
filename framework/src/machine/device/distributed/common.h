...
struct AicpuParamInfo {
    ...
    uint32_t rawShape[5] = {1, 1, 1, 1, 1};
    uint32_t shape[5] = {1, 1, 1, 1, 1};
    uint32_t dim;
...
inline AicpuParamInfo DecodeAicpuCode(const npu::tile_fwk::dynamic::DevRelocVector<int32_t>& aicpuCode)
{
    ...
    for (uint32_t i = 0; i < paramInfo.dim && i < 5; ++i) {
        paramInfo.rawShape[i + (5 - paramInfo.dim)] = aicpuCode[paramInfo.rawShapeIndex + i];
    }

    paramInfo.shapeIndex = paramInfo.rawShapeIndex + paramInfo.dim;

    for (uint32_t i = 0; i < paramInfo.dim && i < 5; ++i) {
        paramInfo.shape[i + (5 - paramInfo.dim)] = aicpuCode[paramInfo.shapeIndex + i];
    }
