...
inline AicpuParamInfo DecodeAicpuCode(const npu::tile_fwk::dynamic::DevRelocVector<int32_t> &aicpuCode) {
    ...
    paramInfo.rawShapeRow = aicpuCode[paramInfo.rawShapeIndex + 1]; // ShmemSignal RawShape[ranksize, row, col], 3表示row的值
    paramInfo.rawShapeCol = aicpuCode[paramInfo.rawShapeIndex + 2]; // ShmemSignal RawShape[ranksize, row, col], 4表示col的值
