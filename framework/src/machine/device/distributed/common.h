...
inline AicpuParamInfo DecodeAicpuCode(const npu::tile_fwk::dynamic::DevRelocVector<int32_t> &aicpuCode) {
    ...
    paramInfo.rawRankShape = aicpuCode[paramInfo.rawShapeIndex + 2]; // ShmemSignal Shape[ranksize, ranksize, ranksize, row, col], 2表示rankShape的值
    paramInfo.rawShapeRow = aicpuCode[paramInfo.rawShapeIndex + 3]; // ShmemSignal Shape[ranksize, ranksize, ranksize, row, col], 3表示row的值
    paramInfo.rawShapeCol = aicpuCode[paramInfo.rawShapeIndex + 4]; // ShmemSignal Shape[ranksize, ranksize, ranksize, row, col], 4表示col的值
    paramInfo.tileShapeIndex = paramInfo.rawShapeIndex + aicpuCode[index] / 2; // 存储了signal_dim * 2个参数, tieShape往后偏移dim位
    paramInfo.tileShapeRow = aicpuCode[paramInfo.tileShapeIndex + 3]; // ShmemSignal Shape[ranksize, ranksize, ranksize, row, col], 3表示col的值
