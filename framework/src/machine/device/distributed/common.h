...
inline uint64_t MapVirtualSignalAddr(int64_t* hcclContextAddr, uint64_t addr)
{
    uint64_t groupIndex = DECODE_SHMEM_ADDR_GROUP_INDEX(addr);
    uint64_t offset = DECODE_SHMEM_ADDR_OFFSET(addr);
...
inline uint64_t GetRankNum(int64_t* hcclContextAddr, uint64_t addr)
{
    uint64_t groupIndex = DECODE_SHMEM_ADDR_GROUP_INDEX(addr);
    auto hcclOpParam = reinterpret_cast<TileOp::CommContext*>(hcclContextAddr[groupIndex]);
