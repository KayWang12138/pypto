...
__attribute__((visibility("default"))) uint32_t StaticPyptoKernelServer(void *args) {
    ...
    if (ret != 0) {
        DEV_ERROR(aicpu.static.server, "kernelFunc=%s # KernelFunc execution error", staticServerKernelkFun.c_str());
        return 1;
    }
    return 0;
}
...
__attribute__((visibility("default"))) uint32_t DynPyptoKernelServerNull(void *args) {
    ...
    if (!g_handleManager.SaveSoFile(data, devArgs->aicpuSoLen, devArgs->deviceId)) {
        DEV_ERROR(sche.task.pre.dyn.server, "deviceId=%lu # Cannot save so file for device", devArgs->deviceId);
        return 1;
    }
    g_handleManager.SetTileFwkKernelMap();
    return 0;
...
__attribute__((visibility("default"))) uint32_t DynPyptoKernelServer(void *args) {
    ...
    if (ret != 0) {
        DEV_ERROR(sche.task.run.dyn.server, "kernelFunc=%s # KernelFunc execution error", dynServerKernelFun.c_str());
        return 1;
    }
    return 0;
}
...
__attribute__((visibility("default"))) uint32_t DynPyptoKernelServerInit(void *args) {
    ...
    if (ret != 0) {
        DEV_ERROR(sche.task.pre.dyn.server.init, "kernelFunc=%s # KernelFunc execution error", dynServerKernelInitFun.c_str());
        return 1;
    }
    return 0;
}
