...
__attribute__((visibility("default"))) uint32_t StaticPyptoKernelServer(void *args) {
    ...
    if (args == nullptr) {
        DEV_ERROR(ServerKernelError::DYN_SERVER_ARGS_NULL, "aicpu.static.server",
                  "# Args is null.");
        return 1;
    }
    ...
    if (!g_handleManager.SaveSoFile(data, devArgs->aicpuSoLen)) {
        DEV_ERROR(ServerKernelError::DYN_SERVER_SAVE_SO_FAILED, "aicpu.static.server",
                  "# Cannot save so file.");
        return 1;
    }
    ...
    if (ret != 0) {
        DEV_ERROR(ServerKernelError::KERNEL_EXEC_FUNC_FAILED, "aicpu.static.server",
...
__attribute__((visibility("default"))) uint32_t DynPyptoKernelServerNull(void *args) {
...
#endif
    if (args == nullptr) {
        DEV_ERROR(ServerKernelError::DYN_SERVER_ARGS_NULL, "sche.task.pre.dyn.server",
                  "# Input args is null.");
        return 1;
    }
    ...
    if (!g_handleManager.SaveSoFile(data, devArgs->aicpuSoLen, devArgs->deviceId)) {
        DEV_ERROR(ServerKernelError::DYN_SERVER_SAVE_SO_FAILED, "sche.task.pre.dyn.server",
...
__attribute__((visibility("default"))) uint32_t DynPyptoKernelServer(void *args) {
    ...
    if (ret != 0) {
        DEV_ERROR(ServerKernelError::KERNEL_EXEC_FUNC_FAILED, "sche.task.run.dyn.server",
...
__attribute__((visibility("default"))) uint32_t DynPyptoKernelServerInit(void *args) {
    ...
    if (ret != 0) {
        DEV_ERROR(ServerKernelError::KERNEL_EXEC_FUNC_FAILED, "sche.task.pre.dyn.server.init",
