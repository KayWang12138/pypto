...
__attribute__((visibility("default"))) uint32_t DynPyptoKernelServerNull(void *args) {
...
#endif
    if (args == nullptr) {
        DEV_ERROR(ServerKernelErr::DYN_SERVER_ARGS_NULL, "Server init input args is null");
        return 1;
    }
    auto kargs = (DeviceKernelArgs *)args;
    if (kargs == nullptr) {
        DEV_ERROR(ServerKernelErr::DYN_SERVER_ARGS_NULL, "Server init DeviceKernelArgs is null");
        return 1;
    }
    ...
    if (!g_handleManager.SaveSoFile(data, devArgs->aicpuSoLen, devArgs->deviceId)) {
        DEV_ERROR(ServerKernelErr::DYN_SERVER_SAVE_SO_FAILED, "create so failed");
        return 1;
    }
...
__attribute__((visibility("default"))) uint32_t DynPyptoKernelServer(void *args) {
    ...
    if (ret != 0) {
        DEV_ERROR(ServerKernelErr::KERNEL_EXEC_FUNC_FAILED, "TileFwk kernelFunc [%s] exec not Success", dynServerKernelFun.c_str());
        return 1;
    }
    return 0;
}
...
__attribute__((visibility("default"))) uint32_t DynPyptoKernelServerInit(void *args) {
    ...
    if (ret != 0) {
        DEV_ERROR(ServerKernelErr::KERNEL_EXEC_FUNC_FAILED, "TileFwk kernelFunc [%s] exec not Success", dynServerKernelInitFun.c_str());
        return 1;
    }
    return 0;
}
