...
__attribute__((visibility("default"))) uint32_t StaticPyptoKernelServer(void *args) {
    ...
    if (args == nullptr) {
        DEV_ERROR("[ctrl.aicpu.init.params] Args is null.");
        return 1;
    }
    ...
    if (!g_handleManager.SaveSoFile(data, devArgs->aicpuSoLen)) {
        DEV_ERROR("[ctrl.aicpu.init.resource] Cannot save so file.");
        return 1;
    }
    ...
    if (ret != 0) {
        DEV_ERROR("[ctrl.aicpu.kernel.exec] KernelFunc [%s] execution error.", staticServerKernelkFun.c_str());
        return 1;
    }
    return 0;
}
...
__attribute__((visibility("default"))) uint32_t DynPyptoKernelServerNull(void *args) {
...
#endif
    if (args == nullptr) {
        DEV_ERROR("[ctrl.aicpu.init.params] Input args is null.");
        return 1;
    }
    auto kargs = (DeviceKernelArgs *)args;
    if (kargs == nullptr) {
        DEV_ERROR("[ctrl.aicpu.init.params] DeviceKernelArgs is null.");
        return 1;
    }
    ...
    if (!g_handleManager.SaveSoFile(data, devArgs->aicpuSoLen, devArgs->deviceId)) {
        DEV_ERROR("[ctrl.aicpu.init.resource] Cannot save so file for device %lu.", devArgs->deviceId);
        return 1;
    }
    g_handleManager.SetTileFwkKernelMap();
    return 0;
...
__attribute__((visibility("default"))) uint32_t DynPyptoKernelServer(void *args) {
    ...
    if (ret != 0) {
        DEV_ERROR("[ctrl.aicpu.kernel.exec] KernelFunc [%s] execution error.", dynServerKernelFun.c_str());
        return 1;
    }
    return 0;
}
...
__attribute__((visibility("default"))) uint32_t DynPyptoKernelServerInit(void *args) {
    ...
    if (ret != 0) {
        DEV_ERROR("[ctrl.aicpu.kernel.exec] KernelFunc [%s] execution error.", dynServerKernelInitFun.c_str());
        return 1;
    }
    return 0;
}
