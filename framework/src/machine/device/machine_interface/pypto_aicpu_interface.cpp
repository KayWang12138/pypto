...
__attribute__((visibility("default"))) uint32_t StaticPyptoKernelServer(void *args) {
    ...
    if (!g_handleManager.SaveSoFile(data, devArgs->aicpuSoLen)) {
        DEV_ERROR("[init_resource] StaticPyptoKernelServer failed: cannot save so file.");
        return 1;
    }
    ...
    if (ret != 0) {
        DEV_ERROR("[kernel_exec] StaticPyptoKernelServer failed: kernelFunc [%s] execution error.", staticServerKernelkFun.c_str());
        return 1;
    }
    return 0;
}
...
__attribute__((visibility("default"))) uint32_t DynPyptoKernelServerNull(void *args) {
...
#endif
    if (args == nullptr) {
        DEV_ERROR("[init_params] DynPyptoKernelServerNull failed: input args is null.");
        return 1;
    }
    auto kargs = (DeviceKernelArgs *)args;
    if (kargs == nullptr) {
        DEV_ERROR("[init_params] DynPyptoKernelServerNull failed: DeviceKernelArgs is null.");
        return 1;
    }
    ...
    if (!g_handleManager.SaveSoFile(data, devArgs->aicpuSoLen, devArgs->deviceId)) {
        DEV_ERROR("[init_resource] DynPyptoKernelServerNull failed: cannot save so file for device %lu.", devArgs->deviceId);
        return 1;
    }
    g_handleManager.SetTileFwkKernelMap();
    return 0;
...
__attribute__((visibility("default"))) uint32_t DynPyptoKernelServer(void *args) {
    ...
    if (ret != 0) {
        DEV_ERROR("[kernel_exec] DynPyptoKernelServer failed: kernelFunc [%s] execution error.", dynServerKernelFun.c_str());
        return 1;
    }
    return 0;
}
...
__attribute__((visibility("default"))) uint32_t DynPyptoKernelServerInit(void *args) {
    ...
    if (ret != 0) {
        DEV_ERROR("[kernel_exec] DynPyptoKernelServerInit failed: kernelFunc [%s] execution error.", dynServerKernelInitFun.c_str());
        return 1;
    }
    return 0;
}
