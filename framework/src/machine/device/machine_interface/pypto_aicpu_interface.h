...
public:
    bool SaveSoFile(char *data, const uint64_t &len, uint8_t deviceId = 0) {
        ...
        if (!file) {
            DEV_ERROR(ServerKernelError::DYN_SERVER_SAVE_SO_FAILED, "sche.task.pre.init.resource",
    ...
    inline int32_t ExecuteFunc(void *args, const uint64_t funcKey) {
        ...
        if (func == nullptr) {
            DEV_ERROR(ServerKernelError::KERNEL_SO_OR_FUNC_LOAD_FAILED, "sche.func.exec",
...
private:
    void LoadTileFwkKernelFunc(const std::string &kernelName) {
        ...
        if (!soHandle_) {
            DEV_ERROR(ServerKernelError::KERNEL_SO_OR_FUNC_LOAD_FAILED, "sche.task.pre.kernel.load",
    ...
    TileFwkKernelServelEnty GetTileFwkKernelFunc(const uint64_t funcKey) {
        ...
        DEV_ERROR(ServerKernelError::KERNEL_SO_OR_FUNC_LOAD_FAILED, "sche.kernel.check",
