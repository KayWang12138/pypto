...
public:
    bool SaveSoFile(char *data, const uint64_t &len, uint8_t deviceId = 0) {
        ...
        if (!file) {
            DEV_ERROR(sche.task.pre.init.resource, "file=%s # Cannot create file", pyptoServerSoName_.c_str());
            return false;
        }
        ...
        if (!file) {
            DEV_ERROR(sche.task.pre.init.resource, "file=%s # Cannot write to file", pyptoServerSoName_.c_str());
            return false;
        }
    ...
    inline int32_t ExecuteFunc(void *args, const uint64_t funcKey) {
        ...
        if (func == nullptr) {
            DEV_ERROR(sche.func.exec, "funcKey=%lu so=%s # Kernel func is invalid, cannot get from so", funcKey, pyptoServerSoName_.c_str()); 
            return -1;
        }
        return func(args);
    }
...
private:
    void LoadTileFwkKernelFunc(const std::string &kernelName) {
        ...
        if (!soHandle_) {
            DEV_ERROR(sche.task.pre.kernel.load, "so=%s # Cannot open so", pyptoServerSoName_.c_str());
            return;
        }
        ...
        if (tileFwkServrFuncEnty == nullptr) {
            DEV_ERROR(sche.task.pre.kernel.load, "kernelName=%s # kernelName is null in so", kernelName.c_str());
            (void)dlclose(soHandle_);
            return;
        }
    ...
    TileFwkKernelServelEnty GetTileFwkKernelFunc(const uint64_t funcKey) {
        ...
        DEV_ERROR(sche.kernel.check, "funcKey=%lu # function is null", funcKey);
        return nullptr;
    }
