...
public:
    bool SaveSoFile(char *data, const uint64_t &len, uint8_t deviceId = 0) {
        ...
        if (!file) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "Coundn't create file [%s]", pyptoServerSoName_.c_str());
            return false;
        }
        ...
        if (!file) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "Write to file [%s] not success", pyptoServerSoName_.c_str());
            return false;
        }
    ...
    inline int32_t ExecuteFunc(void *args, const uint64_t funcKey) {
        ...
        if (func == nullptr) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "kernel func[%lu] is invalid, cannot get from so %s", funcKey, pyptoServerSoName_.c_str()); 
            return -1;
        }
        return func(args);
    }
...
private:
    void LoadTileFwkKernelFunc(const std::string &kernelName) {
        ...
        if (!soHandle_) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "Cannot open so %s", pyptoServerSoName_.c_str());
            return;
        }
        uint64_t funcKey = 0;
        ...
        if (tileFwkServrFuncEnty == nullptr) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "Current KernelName [%s] is null", kernelName.c_str());
            (void)dlclose(soHandle_);
            return;
        }
    ...
    TileFwkKernelServelEnty GetTileFwkKernelFunc(const uint64_t funcKey) {
        ...
        DEV_ERROR(ERROR_CODE_UNDEFINED, "Function[%lu] is null.", funcKey);
        return nullptr;
    }
