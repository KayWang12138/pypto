...
public:
    bool SaveSoFile(char *data, const uint64_t &len, uint8_t deviceId = 0) {
        ...
        if (!file) {
            DEV_ERROR("[aicpu.init.resource] Cannot create file [%s].", pyptoServerSoName_.c_str());
            return false;
        }
        ...
        if (!file) {
            DEV_ERROR("[aicpu.init.resource] Cannot write to file [%s].", pyptoServerSoName_.c_str());
            return false;
        }
    ...
    inline int32_t ExecuteFunc(void *args, const uint64_t funcKey) {
        ...
        if (func == nullptr) {
            DEV_ERROR("[aicpu.kernel.exec] kernel func[%lu] is invalid, cannot get from so %s.", funcKey, pyptoServerSoName_.c_str()); 
            return -1;
        }
        return func(args);
    }
...
private:
    void LoadTileFwkKernelFunc(const std::string &kernelName) {
        ...
        if (!soHandle_) {
            DEV_ERROR("[aicpu.kernel.load] Cannot open so %s.", pyptoServerSoName_.c_str());
            return;
        }
        ...
        if (tileFwkServrFuncEnty == nullptr) {
            DEV_ERROR("[aicpu.kernel.exec] kernelName [%s] is null in so.", kernelName.c_str());
            (void)dlclose(soHandle_);
            return;
        }
    ...
    TileFwkKernelServelEnty GetTileFwkKernelFunc(const uint64_t funcKey) {
        ...
        DEV_ERROR("[aicpu.kernel.exec] function[%lu] is null.", funcKey);
        return nullptr;
    }
