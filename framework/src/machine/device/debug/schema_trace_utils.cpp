...
void SchemaDumpUtil::DumpSchemaOperationInfo(DynDeviceTask *dyntask, int coreIdx, uint64_t taskId, uint32_t deviceTaskId) {
#if ENABLE_DUMP_OPERATION
    auto dyntask = reinterpret_cast<DynDeviceTask*>(curDevTask_);
    uint32_t funcId = FuncID(taskId);
    auto duppedDaya = dyntask->dynFuncDataCacheList[funcId].duppedData;
