...
int DeviceTaskContext::FillDynFuncData(DynFuncData *dyndata, const DevAscendFunctionDupped &dupFunc) {
    dyndata->opAttrs = reinterpret_cast<uint64_t *>(const_cast<SymInt *>(dupFunc.GetSource()->GetSymoffset(0)));
    dyndata->opAtrrOffsets = const_cast<DevAscendFunctionDupped&>(dupFunc).GetSource()->GetOpAttrOffsetAddr();
    dyndata->exprNum = dupFunc.GetSource()->expressionList.size();
    dyndata->exprTbl = dupFunc.GetExpressionAddr();
    dyndata->rawTensorAddr = reinterpret_cast<uint64_t *>(
        &(const_cast<DevAscendFunctionDupped&>(dupFunc).GetIncastAddress(0)));
    dyndata->rawTensorDesc = const_cast<DevAscendFunctionDupped&>(dupFunc).GetSource()->GetRawTensorDesc(0);
    ...
    dyndata->opAttrSize = const_cast<DevAscendFunctionDupped&>(dupFunc).GetSource()->GetOpAttrSize();
...
int DeviceTaskContext::BuildDynFuncData(DynDeviceTask *dyntask, uint32_t taskId,
    ...
    for (size_t funcIdx = 0; funcIdx < stitchedSize; ++funcIdx) {
        ...
        if (unlikely(FillDynFuncData(dyndata, dupFunc) != DEVICE_MACHINE_OK)) {
            return DEVICE_MACHINE_ERROR;
        }
