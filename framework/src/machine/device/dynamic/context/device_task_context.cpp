...
void DeviceTaskContext::ProcessAivTasks(DynDeviceTask *dyntask, size_t funcIndex, bool isNeedWrap,
    uint32_t *wrapTasklistAddr, WrapInfoQueue *wrapQueue, ReadyCoreFunctionQueue *aivQueue,
    predcount_t *dupPredCountList, int &wrapTaskNum) {
    auto &predInfo = duppedData->GetSource()->GetPredInfo();
    size_t totalZeroPredAIVBatchEnd = isNeedWrap ? 0 : predInfo.totalZeroPredAIV & ~0x7;
    ...
    for (size_t opIndex = totalZeroPredAIVBatchEnd; opIndex < predInfo.totalZeroPredAIV; ++opIndex) {
        if (likely(dupPredCountList[opIndex] == 0)) { // likely预判：大概率前置依赖为0，走快速分支
...
int DeviceTaskContext::BuildReadyQueue(DynDeviceTask *dyntask, DevAscendProgram *devProg) {
    ...
    ReadyCoreFunctionQueue *queue[READY_QUEUE_SIZE];
    int ret = InitReadyQueues(dyntask, devProg, queue);
    if (ret != DEVICE_MACHINE_OK) { return ret;}
    ...
    for (size_t funcIndex = 0; funcIndex < funcSize; ++funcIndex) {
        ...
        ProcessAivTasks(dyntask, funcIndex, isNeedWrap, wrapTasklistAddr, wrapQueue, aivQueue, dupPredCountList, wrapTaskNum);
