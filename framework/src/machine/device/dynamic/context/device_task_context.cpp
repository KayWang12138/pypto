...
void DeviceTaskContext::ShowStats() {
    DEV_INFO("   Stitched function count: %10lu.", stitchedFuncNum);
    DEV_INFO("       Root function count: %10lu.", rootFuncNum);
    DEV_INFO("       Leaf function count: %10lu.", leafFuncNum);
    DEV_INFO("   Inital ready task count: %10lu.", readyTaskNum);
    DEV_INFO(" Static function data size: %10lu bytes.", dynFuncDataSize);
    DEV_INFO("   Leaf function data size: %10lu bytes.", leafFuncDataSize);
}
...
int DeviceTaskContext::InitReadyQueues(DynDeviceTask *dyntask, DevAscendProgram *devProg,
    ...
    if (dyntask->devTask.coreFunctionCnt > devProg->stitchFunctionsize) {
        DEV_ERROR(CtrlErr::DEVICE_TASK_BUILD_FAILED, "coreFunctionCnt (%lu) exceeds stitchFunctionsize (%u), cannot build ready queue.",
...
int DeviceTaskContext::BuildDynFuncData(DynDeviceTask *dyntask, uint32_t taskId, DevAscendFunctionDupped *stitchedList, 
    ...
    if (reinterpret_cast<uint64_t>(funcHeader->cceBinary) % CCE_BINARY_MOD != 0) {
        DEV_ERROR(ProgEncodeErr::DYNFUNC_DATA_ALIGNMENT_ERROR, "cceBinary address  is not aligned.");
        return DEVICE_MACHINE_ERROR;
    }
    ...
    for (size_t funcIdx = 0; funcIdx < stitchedSize; ++funcIdx) {
        ...
        if (reinterpret_cast<uint64_t>(dyndata->opAttrs) % OP_ATTRS_PRE_NUM != 0) {
            DEV_ERROR(ProgEncodeErr::DYNFUNC_DATA_ALIGNMENT_ERROR, "opAttrs address is not aligned.");
            return DEVICE_MACHINE_ERROR;
        }
        if (reinterpret_cast<uint64_t>(dyndata->opAtrrOffsets) % OP_ATTRS_OFFSET_PRE_NUM != 0) {
            DEV_ERROR(ProgEncodeErr::DYNFUNC_DATA_ALIGNMENT_ERROR, "opAtrrOffsets address is not aligned.");
            return DEVICE_MACHINE_ERROR;
        }
        if (reinterpret_cast<uint64_t>(dyndata->exprTbl) % EXPR_TABLE_PRE_NUM != 0) {
            DEV_ERROR(ProgEncodeErr::DYNFUNC_DATA_ALIGNMENT_ERROR, "exprTbl address is not aligned.");
            return DEVICE_MACHINE_ERROR;
        }
        if (reinterpret_cast<uint64_t>(dyndata->rawTensorAddr) % RAW_TENSOR_ADDR_MASK != 0) {
            DEV_ERROR(ProgEncodeErr::DYNFUNC_DATA_ALIGNMENT_ERROR, "rawTensorAddr address is not aligned.");
            return DEVICE_MACHINE_ERROR;
        }
...
void DeviceTaskContext::DumpReadyQueue(DynDeviceTask *dynTask, const char *prefix) {
    DEV_INFO("%s: coreFunctionCnt: %d", prefix, (int)dynTask->devTask.coreFunctionCnt);
    ...
    DEV_INFO("%s: ready queue aiv: %d-%d", prefix, (int)dynTask->readyQueue[aivIndex]->head, (int)dynTask->readyQueue[aivIndex]->tail);
    for (uint32_t i = dynTask->readyQueue[aivIndex]->head; i < dynTask->readyQueue[aivIndex]->tail; i++) {
        DEV_INFO("%s: ready queue aiv[%d]: %x", prefix, (int)i, dynTask->readyQueue[aivIndex]->elem[i]);
    }

    DEV_INFO("%s: ready queue aic: %d-%d", prefix, (int)dynTask->readyQueue[aicIndex]->head, (int)dynTask->readyQueue[aicIndex]->tail);
    for (uint32_t i = dynTask->readyQueue[aicIndex]->head; i < dynTask->readyQueue[aicIndex]->tail; i++) {
        DEV_INFO("%s: ready queue aic[%d]: %x", prefix, (int)i, dynTask->readyQueue[aicIndex]->elem[i]);
    }

    DEV_INFO("%s: ready queue aicpu: %d-%d", prefix, (int)dynTask->readyQueue[aicpuIndex]->head, (int)dynTask->readyQueue[aicpuIndex]->tail);
    for (uint32_t i = dynTask->readyQueue[aicpuIndex]->head; i < dynTask->readyQueue[aicpuIndex]->tail; i++) {
        DEV_INFO("%s: ready queue aicpu[%d]: %x", prefix, (int)i, dynTask->readyQueue[aicpuIndex]->elem[i]);
    }
}
void DeviceTaskContext::DumpDepend(DynDeviceTask *dyntask, DevAscendProgram *devProg, DevStartArgs *startArgs, const char *prefix) {
    ...
    DEV_INFO("%s: ready total:%d", prefix, total);
    for (size_t i = 0; i < READY_QUEUE_SIZE; i++) {
        ReadyCoreFunctionQueue *q = dyntask->readyQueue[i];
        for (uint32_t k = q->head; k < q->tail; k++) {
            ...
            DEV_INFO("%s: ready %d-%d:L(%d,%d,%d)\n",
                prefix,
                (int)i, (int)k,
                (int)dyntask->GetDynFuncDataList()->seqNo, (int)dupIndex, (int)opIndex);
        }
    }
    DEV_INFO("%s: workspace:%llx", prefix, (unsigned long long)startArgs->contextWorkspaceAddr);
    for (size_t i = 0; i < startArgs->inputTensorSize; i++) {
        DEV_INFO("%s: input-%d:%llx", prefix, (int)i, (unsigned long long)startArgs->GetInputTensor(i).address);
    }
    for (size_t i = 0; i < startArgs->outputTensorSize; i++) {
        DEV_INFO("%s: output-%d:%llx", prefix, (int)i, (unsigned long long)startArgs->GetOutputTensor(i).address);
    }
    ...
    for (size_t dupIndex = 0; dupIndex < dynFuncDataList->Size(); dupIndex++) {
        ...
        for (size_t stitchIndex = 1; stitchIndex < duppedData->GetStitchSize(); stitchIndex++) {
            ...
            stitchList.ForEach([&](int succTaskId){
                ...
                DEV_INFO("%s: R(%d,%d).succ-%d: L(%d,%d,%d)\n",
        ...
        for (size_t incastIndex = 0; incastIndex < duppedData->GetIncastSize(); incastIndex++) {
            ...
            DEV_INFO("%s: R(%d,%d).incast-%d: 0x%llx - 0x%llx\n",
