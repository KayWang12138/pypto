...
void DeviceTaskContext::ShowStats() {
    DEV_ERROR(ERROR_CODE_UNDEFINED, "   Stitched function count: %10lu.", stitchedFuncNum);
    DEV_ERROR(ERROR_CODE_UNDEFINED, "       Root function count: %10lu.", rootFuncNum);
    DEV_ERROR(ERROR_CODE_UNDEFINED, "       Leaf function count: %10lu.", leafFuncNum);
    DEV_ERROR(ERROR_CODE_UNDEFINED, "   Inital ready task count: %10lu.", readyTaskNum);
    DEV_ERROR(ERROR_CODE_UNDEFINED, " Static function data size: %10lu bytes.", dynFuncDataSize);
    DEV_ERROR(ERROR_CODE_UNDEFINED, "   Leaf function data size: %10lu bytes.", leafFuncDataSize);
}
...
int DeviceTaskContext::InitReadyQueues(DynDeviceTask *dyntask, DevAscendProgram *devProg,
    ...
    if (dyntask->devTask.coreFunctionCnt > devProg->stitchFunctionsize) {
        DEV_ERROR(ERROR_CODE_UNDEFINED, "coreFunctionCnt (%lu) exceeds stitchFunctionsize (%u), cannot build ready queue.",
        dyntask->devTask.coreFunctionCnt, devProg->stitchFunctionsize);
        return DEVICE_MACHINE_ERROR;
    }
    DEV_ASSERT(ERROR_CODE_UNDEFINED, dyntask->devTask.coreFunctionCnt <= devProg->stitchFunctionsize);
...
int DeviceTaskContext::BuildDynFuncData(DynDeviceTask *dyntask, uint32_t taskId, DevAscendFunctionDupped *stitchedList, 
    ...
    if (reinterpret_cast<uint64_t>(funcHeader->cceBinary) % CCE_BINARY_MOD != 0) {
        DEV_ERROR(ERROR_CODE_UNDEFINED, "cceBinary address  is not aligned.");
        return DEVICE_MACHINE_ERROR;
    }
    DEV_ASSERT(ERROR_CODE_UNDEFINED, reinterpret_cast<uint64_t>(funcHeader->cceBinary) % CCE_BINARY_MOD == 0);
    ...
    for (size_t funcIdx = 0; funcIdx < stitchedSize; ++funcIdx) {
        ...
        if (reinterpret_cast<uint64_t>(dyndata->opAttrs) % OP_ATTRS_PRE_NUM != 0) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "opAttrs address is not aligned.");
            return DEVICE_MACHINE_ERROR;
        }
        if (reinterpret_cast<uint64_t>(dyndata->opAtrrOffsets) % OP_ATTRS_OFFSET_PRE_NUM != 0) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "opAtrrOffsets address is not aligned.");
            return DEVICE_MACHINE_ERROR;
        }
        if (reinterpret_cast<uint64_t>(dyndata->exprTbl) % EXPR_TABLE_PRE_NUM != 0) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "exprTbl address is not aligned.");
            return DEVICE_MACHINE_ERROR;
        }
        if (reinterpret_cast<uint64_t>(dyndata->rawTensorAddr) % RAW_TENSOR_ADDR_MASK != 0) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "rawTensorAddr address is not aligned.");
            return DEVICE_MACHINE_ERROR;
        }
        DEV_ASSERT(ERROR_CODE_UNDEFINED, reinterpret_cast<uint64_t>(dyndata->opAttrs) % OP_ATTRS_PRE_NUM == 0);
        DEV_ASSERT(ERROR_CODE_UNDEFINED, reinterpret_cast<uint64_t>(dyndata->opAtrrOffsets) % OP_ATTRS_OFFSET_PRE_NUM == 0);
        DEV_ASSERT(ERROR_CODE_UNDEFINED, reinterpret_cast<uint64_t>(dyndata->exprTbl) % EXPR_TABLE_PRE_NUM == 0);
        DEV_ASSERT(ERROR_CODE_UNDEFINED, reinterpret_cast<uint64_t>(dyndata->rawTensorAddr) % RAW_TENSOR_ADDR_MASK == 0);
...
void DeviceTaskContext::DumpReadyQueue(DynDeviceTask *dynTask, const char *prefix) {
    DEV_ERROR(ERROR_CODE_UNDEFINED, "%s: coreFunctionCnt: %d", prefix, (int)dynTask->devTask.coreFunctionCnt);
    ...
    DEV_ERROR(ERROR_CODE_UNDEFINED, "%s: ready queue aiv: %d-%d", prefix, (int)dynTask->readyQueue[aivIndex]->head, (int)dynTask->readyQueue[aivIndex]->tail);
    for (uint32_t i = dynTask->readyQueue[aivIndex]->head; i < dynTask->readyQueue[aivIndex]->tail; i++) {
        DEV_ERROR(ERROR_CODE_UNDEFINED, "%s: ready queue aiv[%d]: %x", prefix, (int)i, dynTask->readyQueue[aivIndex]->elem[i]);
    }

    DEV_ERROR(ERROR_CODE_UNDEFINED, "%s: ready queue aic: %d-%d", prefix, (int)dynTask->readyQueue[aicIndex]->head, (int)dynTask->readyQueue[aicIndex]->tail);
    for (uint32_t i = dynTask->readyQueue[aicIndex]->head; i < dynTask->readyQueue[aicIndex]->tail; i++) {
        DEV_ERROR(ERROR_CODE_UNDEFINED, "%s: ready queue aic[%d]: %x", prefix, (int)i, dynTask->readyQueue[aicIndex]->elem[i]);
    }

    DEV_ERROR(ERROR_CODE_UNDEFINED, "%s: ready queue aicpu: %d-%d", prefix, (int)dynTask->readyQueue[aicpuIndex]->head, (int)dynTask->readyQueue[aicpuIndex]->tail);
    for (uint32_t i = dynTask->readyQueue[aicpuIndex]->head; i < dynTask->readyQueue[aicpuIndex]->tail; i++) {
        DEV_ERROR(ERROR_CODE_UNDEFINED, "%s: ready queue aicpu[%d]: %x", prefix, (int)i, dynTask->readyQueue[aicpuIndex]->elem[i]);
    }
}
void DeviceTaskContext::DumpDepend(DynDeviceTask *dyntask, DevAscendProgram *devProg, DevStartArgs *startArgs, const char *prefix) {
    ...
    DEV_ERROR(ERROR_CODE_UNDEFINED, "%s: ready total:%d", prefix, total);
    for (size_t i = 0; i < READY_QUEUE_SIZE; i++) {
        ReadyCoreFunctionQueue *q = dyntask->readyQueue[i];
        for (uint32_t k = q->head; k < q->tail; k++) {
            ...
            DEV_ERROR(ERROR_CODE_UNDEFINED, "%s: ready %d-%d:L(%d,%d,%d)\n",
                prefix,
                (int)i, (int)k,
                (int)dyntask->GetDynFuncDataList()->seqNo, (int)dupIndex, (int)opIndex);
        }
    }
    DEV_ERROR(ERROR_CODE_UNDEFINED, "%s: workspace:%llx", prefix, (unsigned long long)startArgs->contextWorkspaceAddr);
    for (size_t i = 0; i < startArgs->inputTensorSize; i++) {
        DEV_ERROR(ERROR_CODE_UNDEFINED, "%s: input-%d:%llx", prefix, (int)i, (unsigned long long)startArgs->GetInputTensor(i).address);
    }
    for (size_t i = 0; i < startArgs->outputTensorSize; i++) {
        DEV_ERROR(ERROR_CODE_UNDEFINED, "%s: output-%d:%llx", prefix, (int)i, (unsigned long long)startArgs->GetOutputTensor(i).address);
    }
    ...
    for (size_t dupIndex = 0; dupIndex < dynFuncDataList->Size(); dupIndex++) {
        ...
        for (size_t opIndex = 0; opIndex < duppedData->GetOperationSize(); opIndex++) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "%s: L(%d,%d,%d) pred:%d\n",
                prefix,
        ...
        for (size_t stitchIndex = 1; stitchIndex < duppedData->GetStitchSize(); stitchIndex++) {
            ...
            stitchList.ForEach([&](int succTaskId){
                ...
                DEV_ERROR(ERROR_CODE_UNDEFINED, "%s: R(%d,%d).succ-%d: L(%d,%d,%d)\n",
        ...
        for (size_t exprIndex = 0; exprIndex < duppedData->GetExpressionSize(); exprIndex++) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "%s: R(%d,%d).expr-%d: %lld\n",
        ...
        for (size_t incastIndex = 0; incastIndex < duppedData->GetIncastSize(); incastIndex++) {
            ...
            DEV_ERROR(ERROR_CODE_UNDEFINED, "%s: R(%d,%d).incast-%d: 0x%llx - 0x%llx\n",
