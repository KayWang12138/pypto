...
void *DeviceExecuteContext::SymbolHandlerIdToHandler(SymbolHandlerId id) {
    switch (id) {
        ...
        default:
            DEV_ERROR("[ctrl.task.pre.handler] Invalid SymbolHandlerId: %lu.", static_cast<uint64_t>(id));
            DEV_ASSERT(0);
            return nullptr;
    }
    return nullptr;
...
int DeviceExecuteContext::RunControlFlow(DevStartArgs *startArgs) {
    ...
    if (finalErrorState != originalErrorState && finalErrorState != DEVICE_MACHINE_OK) {
        DEV_ERROR("[ctrl.ctrlflow.leave] Control flow execution failed with error code: %d.", finalErrorState);
...
int DeviceExecuteContext::SubmitToAicoreAndRecycleMemory(bool withoutTail, bool isLastTask) {
    ...
    if (dynTask == nullptr) {
        DEV_ERROR("[ctrl.buildtask.leave] Returned null for taskId=%lu.", taskId);
        return DEVICE_MACHINE_ERROR;
    }
