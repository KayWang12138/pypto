...
int DeviceExecuteContext::SubmitToAicoreAndRecycleMemory(bool withoutTail, bool isLastTask) {
    ...
    uint64_t allocatedBefore = workspace.GetTensorAllocator().devTaskInnerExclusiveOutcasts.AllocatedSize();
    uint64_t freeBefore = workspace.GetTensorAllocator().devTaskInnerExclusiveOutcasts.FreeMemorySize();
    size_t stitchedCount = stitchContext.Size();
    DEV_ERROR("[InnerOutcast] SubmitToAicore Before: taskId=%lu, stitchedCount=%zu, withoutTail=%d, allocated=%lu, free=%lu",
    ...
    uint64_t allocatedAfter = workspace.GetTensorAllocator().devTaskInnerExclusiveOutcasts.AllocatedSize();
    uint64_t freeAfter = workspace.GetTensorAllocator().devTaskInnerExclusiveOutcasts.FreeMemorySize();
    DEV_ERROR("[InnerOutcast] SubmitToAicore After Recycle: taskId=%lu, allocated=%lu, free=%lu",
...
void *DeviceExecuteContext::CallRootFunctionStitch(uint64_t rootKey) {
    ...
    uint64_t allocatedBefore = workspace.GetTensorAllocator().devTaskInnerExclusiveOutcasts.AllocatedSize();
    uint64_t freeBefore = workspace.GetTensorAllocator().devTaskInnerExclusiveOutcasts.FreeMemorySize();
    uint64_t capacity = workspace.GetTensorAllocator().devTaskInnerExclusiveOutcasts.Capacity();
    uint32_t resetTimes = workspace.GetTensorAllocator().devTaskInnerExclusiveOutcasts.ResetTimes();
    size_t stitchedCount = stitchContext.Size();
    ...
    while (!workspace.TryAllocateFunctionMemory(currDevRootDup, slotContext.GetSlotList())) {
        uint64_t allocatedNow = workspace.GetTensorAllocator().devTaskInnerExclusiveOutcasts.AllocatedSize();
        uint64_t freeNow = workspace.GetTensorAllocator().devTaskInnerExclusiveOutcasts.FreeMemorySize();
        ...
        DEV_INFO("[Stitch Finish] Memory Limit Exceeded.");
         uint64_t allocatedAfter = workspace.GetTensorAllocator().devTaskInnerExclusiveOutcasts.AllocatedSize();
         uint64_t freeAfter = workspace.GetTensorAllocator().devTaskInnerExclusiveOutcasts.FreeMemorySize();
         uint32_t resetTimesAfter = workspace.GetTensorAllocator().devTaskInnerExclusiveOutcasts.ResetTimes();
