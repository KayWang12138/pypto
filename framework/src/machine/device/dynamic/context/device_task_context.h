...
private:
    ...
    void GetDieReadyQueues(DynDeviceTask *dyntask, ReadyCoreFunctionQueue *targetAivQueue,
    ...
    inline void doResolve(DynDeviceTask *dyntask, int coreType, size_t funcIdx, size_t succIdx, predcount_t *predList) {
        ...
        if (coreType == static_cast<int>(CoreType::HUB)) {
            ...
            } else if (IsMixArch(devProg_) && (funcIdx < dieIdList_.size()) && (dieIdList_[funcIdx] >= 0)) {
