...
void DeviceStitchContext::CheckStitch(DevAscendFunctionDupped *stitchedList, int size, DevAscendFunctionDupped *nextDup) {
    DEV_IF_NONDEVICE {
        ...
        if (dynPredCount != dynSuccCount) {
            DEV_ERROR("[ctrl.ctx.stitch.check] dynPredCount=%u does not match dynSuccCount=%u.", dynPredCount, dynSuccCount);
        }
...
int DeviceStitchContext::MoveTo(DynDeviceTask *dynTask) {
    ...
    if (dynTask->stitchedList.size() > MAX_CACHED_FUNC_NUM) {
        DEV_ERROR("[ctrl.ctx.stitch.move] Stitch list size %u exceeds maximum allowed cached function number %zu.", dynTask->stitchedList.size(), MAX_CACHED_FUNC_NUM);
...
void DeviceStitchContext::HandleOneStitch(
    ...
    DEV_IF_NONDEVICE {
        if (producerOperationIdx >= producerDup.GetSource()->GetOperationSize()) {
            DEV_ERROR("[ctrl.ctx.stitch.handle] producerOperationIdx %zu exceeds the size of GetOperation %zu.", producerOperationIdx, producerDup.GetSource()->GetOperationSize());
        }
        if (consumerOperationIdx >= consumerDup.GetSource()->GetOperationSize()) {
            DEV_ERROR("[ctrl.ctx.stitch.handle] consumerOperationIdx %zu exceeds the size of GetOperation %zu.", consumerOperationIdx, consumerDup.GetSource()->GetOperationSize());
        }
    ...
    for (size_t incastIdx = 0; incastIdx < nextSrc->GetIncastSize(); ++incastIdx) {
        ...
        for (size_t j = 0; j < incast.fromSlotList.size(); ++j) {
            ...
            if (slotIdx >= (int)slotSize) {
                DEV_ERROR("[ctrl.ctx.stitch.fast] slotIdx %d is larger than slotSize %zu.", slotIdx, slotSize);
                continue;
            }
