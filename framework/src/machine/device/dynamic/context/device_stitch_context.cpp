...
void DeviceStitchContext::CheckStitch(DevAscendFunctionDupped *stitchedList, int size, DevAscendFunctionDupped *nextDup) {
    DEV_IF_NONDEVICE {
        ...
        if (dynPredCount != dynSuccCount) {
            DEV_ERROR(ProgramEncodeErrorScene::STITCH_PRED_SUCC_MISMATCH, "ctrl.task.pre.stitch.check",
                      "dynPredCount=%u dynSuccCount=%u # dynPredCount does not match dynSuccCount",
                      dynPredCount, dynSuccCount);
        }
