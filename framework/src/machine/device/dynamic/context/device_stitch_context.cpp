...
void DeviceStitchContext::CheckStitch(DevAscendFunctionDupped *stitchedList, int size, DevAscendFunctionDupped *nextDup) {
    DEV_IF_NONDEVICE {
        ...
        if (dynPredCount != dynSuccCount) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "dynPredCount %u does not match dynSuccCount %u", dynPredCount, dynSuccCount);
        }
        DEV_ASSERT(ERROR_CODE_UNDEFINED, dynPredCount == dynSuccCount);
    }
}
...
int DeviceStitchContext::DecideIncastOutcast(uint64_t taskId) {
    ...
    for (size_t funcIndex = 0; funcIndex < stitchedList_.size(); ++funcIndex) {
        ...
        for (size_t i = 0; i < incastSize; ++i) {
            ...
            DEV_ASSERT(ERROR_CODE_UNDEFINED, desc.IsRtOutcast());
...
int DeviceStitchContext::MoveTo(DynDeviceTask *dynTask) {
    ...
    if (dynTask->stitchedList.size() > MAX_STITCH_FUNC_NUM) {
        DEV_ERROR(ERROR_CODE_UNDEFINED, "Stitch list size:%u exceeds maximum allowed cached function number:%zu.", dynTask->stitchedList.size(), MAX_STITCH_FUNC_NUM);
        return DEVICE_MACHINE_ERROR;
    }
    DEV_ASSERT(ERROR_CODE_UNDEFINED, dynTask->stitchedList.size() <= MAX_STITCH_FUNC_NUM);
...
void DeviceStitchContext::HandleOneStitch(
    ...
    DEV_IF_NONDEVICE {
        if (producerOperationIdx >= producerDup.GetSource()->GetOperationSize()) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "producerOperationIdx %zu exceeds the size of GetOperation %zu", producerOperationIdx, producerDup.GetSource()->GetOperationSize());
        }
        if (consumerOperationIdx >= consumerDup.GetSource()->GetOperationSize()) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "consumerOperationIdx %zu exceeds the size of GetOperation %zu", consumerOperationIdx, consumerDup.GetSource()->GetOperationSize());
        }
        DEV_ASSERT(ERROR_CODE_UNDEFINED, producerOperationIdx < producerDup.GetSource()->GetOperationSize());
        DEV_ASSERT(ERROR_CODE_UNDEFINED, consumerOperationIdx < consumerDup.GetSource()->GetOperationSize());
    ...
    for (size_t incastIdx = 0; incastIdx < nextSrc->GetIncastSize(); ++incastIdx) {
        ...
        for (size_t j = 0; j < incast.fromSlotList.size(); ++j) {
            ...
            if (slotIdx >= (int)slotSize) {
                DEV_ERROR(ERROR_CODE_UNDEFINED, "slotIdx %d is larger than slotSize %zu!.", slotIdx, slotSize);
                continue;
            }
