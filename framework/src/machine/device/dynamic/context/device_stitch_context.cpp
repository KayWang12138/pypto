...
int DeviceStitchContext::MoveTo(DynDeviceTask *dynTask) {
    ...
    if (dynTask->stitchedList.size() > MAX_CACHED_FUNC_NUM) {
        DEV_ERROR("[ctrl.stitch.toomany_root] Stitch list size %u exceeds maximum allowed cached function number %zu.", dynTask->stitchedList.size(), MAX_CACHED_FUNC_NUM);
    ...
    for (size_t incastIdx = 0; incastIdx < nextSrc->GetIncastSize(); ++incastIdx) {
        ...
        for (size_t j = 0; j < incast.fromSlotList.size(); ++j) {
            ...
            if (slotIdx >= (int)slotSize) {
                DEV_ERROR("[ctrl.stitch.invalid_slot] slotIdx %d is larger than slotSize %zu.", slotIdx, slotSize);
                continue;
            }
