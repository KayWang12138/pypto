...
public:
        ...
        for (int idx = coreStart; idx < coreEnd; idx++) {
            ...
            while (*finishRegQueues_[GetPhyIdByBlockId(idx)] != val) {
                if (GetCycles() - startCycle > TIMEOUT_CYCLES) {
                    DEV_ERROR(SchedError::ALL_AICORE_SYNC_TIMEOUT, "sche.aicore.wait_finish",
    ...
    Metrics* GetMetrics(int coreIdx) {
        ...
        if (metric == nullptr) {
            DEV_ERROR(DumpDfxError::METRIC_ALLOC_OR_WAIT_TIMEOUT, "sche.prof.aicore.getaddr",
        ...
        while (metric->isMetricStop != 1) {
            if (GetCycles() - cycles_start > PROF_DUMP_TIMEOUT_CYCLES) {
                DEV_ERROR(DumpDfxError::METRIC_ALLOC_OR_WAIT_TIMEOUT, "sche.prof.aicore.wait_finish",
