...
public:
    ...
    inline std::pair<int, int> GetSubCoreId(int coreIdx) {
        ...
        int subCoreIdx = (coreIdx - BLOCKDIM) % SUBBLOCKDIM_NUM + 1;
    ...
    inline void InitTaskData(int coreIdx, int64_t funcdata, int64_t buffer) {
        ...
        if constexpr (IsDeviceMode()) {
            ...
            // arg->shakeBufferCpuToCore[CPU_TO_CORE_SHAK_BUF_COREFUNC_DATA_INDEX] = funcdata;
            // arg->waveBufferCpuToCore[CPU_TO_CORE_SHAK_BUF_GOODBYE_INDEX] = 0;
            uint64_t eslShakeBufferAddr = reinterpret_cast<uint64_t>(&arg->shakeBufferCpuToCore[CPU_TO_CORE_SHAK_BUF_COREFUNC_DATA_INDEX]);
            busDirectWrite_(eslShakeBufferAddr, sizeof(funcdata), &funcdata, 0);
            // uint64_t val = 0;
            // uint64_t eslShakeBufferAddr = reinterpret_cast<uint64_t>(&arg->shakeBufferCpuToCore[CPU_TO_CORE_SHAK_BUF_COREFUNC_DATA_INDEX]);
            // busDirectWrite_(eslAddr, sizeof(funcdata), &funcdata);
#else 
            if (costModel_) {
...
#ifdef __ESL_SIMULATION__
        uint64_t valToSent = 0;
        busDirectWrite_(reinterpret_cast<uint64_t>(&args_[coreIdx]->shakeBuffer[0]), sizeof(uint64_t), &valToSent, 0);
        busDirectWrite_(reinterpret_cast<uint64_t>(&args_[coreIdx]->shakeBufferCpuToCore[CPU_TO_CORE_SHAK_BUF_COREFUNC_DATA_INDEX]), sizeof(uint64_t), &valToSent, 0);
        valToSent = AICORE_SAY_GOODBYE;
        busDirectWrite_(reinterpret_cast<uint64_t>(&args_[coreIdx]->waveBufferCpuToCore[CPU_TO_CORE_SHAK_BUF_GOODBYE_INDEX]), sizeof(uint64_t), &valToSent, 0);
#endif
        return;
    }
