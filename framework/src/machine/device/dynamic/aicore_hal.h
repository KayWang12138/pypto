...
public:
    ...
    void SetParallelDevTask(volatile ParallelDevTask* kernelParallDevTask, int parallelIdx, int64_t funcData)
    {
        DEV_IF_DEVICE
        {
            kernelParallDevTask->elements[parallelIdxidx % npu::tile_fwk::SCH_DEVTASK_MAX_PARALLELISM] = funcData;
        } else {
            if (enableEslModel_) {
                eslModel_.WriteEslMem(
                    reinterpret_cast<uint64_t>(
                        &kernelParallDevTask->elements[parallelIdxidx % npu::tile_fwk::SCH_DEVTASK_MAX_PARALLELISM]),
                        sizeof(funcdata), &funcdata);
            }
        }
    }

    void SetParallelDevTaskSize(volatile ParallelDevTask* kernelParallDevTask, uint32_t front, uint32_t c)
    {
        DEV_IF_DEVICE
        {
            kernelParallDevTask->front = front;
            kernelParallDevTask->rear = front;
        } else {
            if (enableEslModel_) {
                eslModel_.WriteEslMem(reinterpret_cast<uint64_t>(&kernelParallDevTask->front), sizeof(front), &front);
                eslModel_.WriteEslMem(reinterpret_cast<uint64_t>(&kernelParallDevTask->rear), sizeof(front), &rear);
            }
        }
    }
