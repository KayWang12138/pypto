...
public:
    ...
    inline void InitTaskData(int coreIdx, int64_t funcdata, int64_t buffer) {
        ...
        if constexpr (IsDeviceMode()) {
            ...
            if (enableEslModel_) {
                ...
                eslModel_.WriteEslMem(reinterpret_cast<uint64_t>(&arg->waveBufferCpuToCore[CPU_TO_CORE_SHAK_BUF_GOODBYE_INDEX]), sizeof(uint64_t), 0);
            } else if (costModel_) {
    ...
    void ResetShakeBuf(int coreIdx) {
        if constexpr (IsDeviceMode()) {
            ...
            if (enableEslModel_) {
                eslModel_.WriteEslMem(reinterpret_cast<uint64_t>(&args_[coreIdx]->shakeBuffer[0]), sizeof(uint64_t), 0);
                eslModel_.WriteEslMem(reinterpret_cast<uint64_t>(&args_[coreIdx]->waveBufferCpuToCore[CPU_TO_CORE_SHAK_BUF_GOODBYE_INDEX]), sizeof(uint64_t), AICORE_SAY_GOODBYE);
            }
