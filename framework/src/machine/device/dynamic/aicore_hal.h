...
public:
        ...
        if constexpr (IsDeviceMode()) {
            *readyRegQueues_[GetPhyIdByBlockId(coreIdx)] = value;
            ...
            if (enableEslModel_) {
                ...
                const auto hostSimPhyId = static_cast<size_t>(GetHostSimPhyId(coreIdx));
                if (value != 0 && HostRegBus::Global().ReadMainBase(hostSimPhyId) == value) {
                    HostRegBus::Global().WriteMainBase(hostSimPhyId, 0);
                }
                HostRegBus::Global().WriteMainBase(hostSimPhyId, value);
            } else {
    ...
    inline void InitKernelArgs(int coreIdx, int64_t buffer) {
        ...
        if constexpr (IsDeviceMode()) {
            ...
            } else if (IsHostSimMode() && buffer != 0) {
#if ENABLE_AICORE_PRINT 
                volatile KernelArgs* arg = args_[coreIdx];
                arg->shakeBuffer[SHAK_BUF_PRINT_BUFFER_INDEX] = buffer;
