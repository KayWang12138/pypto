...
struct PerfEvtMgr {
    ...
    static void RepeatPuts(char c, size_t count) {
        ...
        buf[count] = '\0';
        DEV_ERROR(ERROR_CODE_UNDEFINED, "%s.", buf);
    }
    ...
    void Dump() {
        ...
        DEV_ERROR(ERROR_CODE_UNDEFINED, "%40s | %10s | %10s | %10s.", "EventType", "Count", "Total(us)", "Avg(us)");
        RepeatPuts('-', SHEET_WIDTH);
        ...
        for (int i = 0; i < PERF_EVT_MAX; i++) {
            ...
            if (evt.count != 0) {
                ...
                DEV_ERROR(ERROR_CODE_UNDEFINED, "%-40s | %10ld | %10lu | %10.1f.", PerfEventName[i], evt.count, total, avg);
            }
        }
    ...
    void DumpPerfTrace(uint32_t scheCpuNum, std::string file = "") {
        ...
        while (startPos < totalLength) {
            ...
            DEV_ERROR(ERROR_CODE_UNDEFINED, "tile_fwk aicpu prof:%s", batch.c_str());
            startPos = endPos;
        }
