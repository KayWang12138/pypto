...
struct PerfEvtMgr {
    ...
    static void RepeatPuts(char c, size_t count) {
        ...
        buf[count] = '\0';
        DEV_ERROR(DumpDfxErrorScene::PERF_TRACE_FORMAT_ERROR, "sche.task.end.perf.format",
                  "data=%s # Perf format", buf);
    }
    ...
    void Dump() {
        ...
        DEV_ERROR(DumpDfxErrorScene::PERF_TRACE_DUMP_ERROR, "sche.task.end.perf.trace",
                  "EventType=%40s Count=%10s Total(us)=%10s Avg(us)=%10s # Perf trace header",
        ...
        for (int i = 0; i < PERF_EVT_MAX; i++) {
            ...
            if (evt.count != 0) {
                ...
                DEV_ERROR(DumpDfxErrorScene::PERF_TRACE_DUMP_ERROR, "sche.task.end.perf.trace",
                          "EventType=%-40s Count=%10ld Total(us)=%10lu Avg(us)=%10.1f # Perf trace event",
    ...
    void DumpPerfTrace(uint32_t scheCpuNum, std::string file = "") {
        ...
        while (startPos < totalLength) {
            ...
            DEV_ERROR(DumpDfxErrorScene::PERF_TRACE_DUMP_ERROR, "sche.task.end.perf.trace",
                      "data=%s # Aicpu profile data", batch.c_str());
            startPos = endPos;
        }
