...
TraceError DeviceTrace::BindHandleToEventHandle(TraHandle handle) {
    if (g_thread_idx % MAX_HANDLE_NUM == 0) {
        std::string eventTraceHandleName = "PYPTO_Event_Trace_" + std::to_string(g_thread_idx);
    ...
    auto status = TraceEventBindTrace(eventHandleArry_.git s(), handle);
    if (status < 0) {
        DEV_ERROR(
...
TraHandle DeviceTrace::CreateTraceHandle()
{
    ...
    auto status = BindHandleToEventHandle(pyptoHandle);
