...
{
    auto it = pyptoHandleArray_.find(g_thread_idx);
    if (it != pyptoHandleArray_.end()) {
        return it->second;
    }
    g_thread_idx++;
    std::string handleName = "PyPTO_Aicpu_" + std::to_string(g_thread_idx) + "_Trace";
    static thread_local TraHandle pyptoHandle = TraceCreate(TracerType::TRACER_TYPE_SCHEDULE, handleName.c_str());
    if (pyptoHandle < 0) {
        DEV_ERROR(DevCommonErr::GET_HANDLE_FAILED, "Create pypto trace failed");
        return -1;
    }
    pyptoHandleArray_[g_thread_idx] = pyptoHandle;
    DEV_INFO("Create pypto trace Handle successful");
    auto status = TraceEventBindTrace(eventHandle_, pyptoHandle);
    if (status < 0) {
        DEV_ERROR(
