        ...
        for (uint32_t taskIdx =0; taskIdx < maxTaskCnt && idx < aicReadyCnt; taskIdx++) {
            ...
            switch (wrapInfo->mixResourceType) {
                ...
                case static_cast<uint32_t>(MixResourceType::MIX_1C2V):
                    ...
                    break;
                default:
                    DEV_ERROR("illegal mixType %u", mixType);
                    break;
            }
            DEV_VERBOSE_DEBUG("add wrapInfo, aicCoreIdx = %u, aivCoreIdxZero = %u, aivCoreIdxOne = %u, mixResourceType = %u, schedIdx_=%d",
                aicoreIdxList[WRAP_IDX_AIC], aicoreIdxList[WRAP_IDX_AIV0], aicoreIdxList[WRAP_IDX_AIV1], mixType, schedIdx_);
        }
