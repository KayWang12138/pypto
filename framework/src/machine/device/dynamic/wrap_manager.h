...
public:
    ...
    inline ReadyCoreFunctionQueue* GetDieReadyQueue(CoreType type, DieId dieId, ReadyCoreFunctionQueue* readyQue) {
        ReadyCoreFunctionQueue* dieReadyQueue = readyQue;
        if (!GetIsmixarch() || dieId == DieId::DIE_MIX || dieId == DieId::DIE_UNKNOW) {
            return dieReadyQueue;
        }
