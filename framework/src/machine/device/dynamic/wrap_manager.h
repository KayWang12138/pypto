...
public:
    ...
    inline bool CheckCoreIdxInitStatus(uint32_t coreIdx) {
        DEV_IF_VERBOSE_DEBUG {
            if (pendingIds_[coreIdx] != AICORE_TASK_INIT || runningIds_[coreIdx] != AICORE_TASK_INIT) {
                DEV_ERROR("core[%u] is pendingId = %lx, runningId = %lx, is illegal!", coreIdx, pendingIds_[coreIdx], runningIds_[coreIdx]);
            }
        }
    }
