...
public:
    ...
    inline bool GetWrapCoreAvailable(int coreIdx) {
        ...
        for (uint32_t idx = wrapQueueForThread_.head; idx < wrapQueueForThread_.tail; idx++) {
            ...
            if (static_cast<uint32_t>(coreIdx) == wrapInfo->aicCoreIdx ||
                static_cast<uint32_t>(coreIdx) == wrapInfo->aivCoreIdxZero ||
                static_cast<uint32_t>(coreIdx) == wrapInfo->aivCoreIdxOne) {
                return false;
            }
        }
        return true;
