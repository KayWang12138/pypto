...
using RemoveReadyCoreIdxFunc = std::function<void(int coreIdx, int type)>;
...
public:
    ...
    inline uint32_t RemoveAndGetAvailableCoreIdx(MixResourceType mixType = MixResourceType::MIX_UNKNOWN) {
        ...
        RemoveReadyCoreIdx(coreIdx, static_cast<int>(CoreType::AIC));
        ...
        RemoveReadyCoreIdx(aivIdx0, static_cast<int>(CoreType::AIV));
        ...
        if (mixType != MixResourceType::MIX_1C1V) {
            ...
            if (RemoveReadyCoreIdx != nullptr) {
                RemoveReadyCoreIdx(aivIdx1, static_cast<int>(CoreType::AIV));
            }
