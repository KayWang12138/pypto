...
public:
    ...
    inline void CheckCoreIdxInitStatus(uint32_t coreIdx) {
        DEV_IF_VERBOSE_DEBUG {
            if (pendingIds_[coreIdx] != AICORE_TASK_INIT || runningIds_[coreIdx] != AICORE_TASK_INIT) {
                DEV_ERROR(ERROR_CODE_UNDEFINED, "core[%u] is pendingId = %x, runningId = %x, is illegal!", coreIdx, pendingIds_[coreIdx], runningIds_[coreIdx]);
            }
        }
    }
    ...
    inline void UpdateFinishIdForMixCore(uint32_t finishId) {
        ...
        if (wrapInfo == nullptr) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "cant find wrapInfo in wrapQueueForThread!");
            return;
        }
        wrapInfo->taskCnt--;
