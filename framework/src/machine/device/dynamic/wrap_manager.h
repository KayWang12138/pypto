...
public:
    ...
    inline DieId GetDieId(int scheCpuIdx) {
     ...
     inline void RemoveMixReadyCoreIdx(int coreIdx, int type) {
        ...
        context_->coreIdxPosition_[tail] = INVALID_COREIDX_POSITION;
     }
    ...
    inline uint32_t RemoveAndGetAvailableCoreIdx(MixResourceType mixType = MixResourceType::MIX_UNKNOWN) {
        ...
        for (uint32_t i = 0; i < coreRunReadyCnt_[CORE_IDX_AIC]; i++) {
            if (mixType == MixResourceType::MIX_1C1V) {
                uint32_t aivIdx = runReadyCoreIdx_[i] * AIV_NUM_PER_AI_CORE + aicValidNum_;
                if (coreIdxPosition_[aivIdx] != INVALID_COREIDX_POSITION) {
                    coreIdx = runReadyCoreIdx_[i];
                    break;
                }
            }
            if (mixType == MixResourceType::MIX_1C2V) {
                uint32_t aivIdx = runReadyCoreIdx_[i] * AIV_NUM_PER_AI_CORE + aicValidNum_;
