...
void DumpPrefTrace(DeviceArgs *devArgs) {
    ...
    DEV_ERROR("Begin dump machine perf trace: with device addr: %lu", devArgs->aicpuPerfAddr);
    PerfEvtMgr::Instance().DumpPerfTrace(devArgs->scheCpuNum,
                                            "/tmp/tile_fwk_aicpu_perftrace.json", devArgs->aicpuPerfAddr);
    DEV_IF_DEVICE {
...
extern "C" __attribute__((visibility("default"))) int DynTileFwkBackendKernelServerInit(void *targ) {
    ...
    PerfEvtMgr::Instance().SetIsOpenProf(isOpenProf);
...
extern "C" __attribute__((visibility("default"))) int DynTileFwkBackendKernelServer(void *targ) {
    ...
    if (rc == npu::tile_fwk::dynamic::DEVICE_MACHINE_FINISHED) {
        ...
        if (PerfEvtMgr::Instance().GetIsOpenProf() || ENABLE_PERF_TRACE == 1) {
