...
struct DynMachineManager {
    int allocThreadIdx(int nrAicpu, uint32_t scheCpuNum) {
        ...
        for (int index = 0; index < static_cast<int>(sizeof(uint64_t)); ++index) {
            ...
            if (__builtin_popcount(static_cast<uint32_t>(mask)) >= scheCpuNum) {
                clus_id = index;
                break;
            }
    ...
    int Run(DeviceKernelArgs *args) {
        ...
        if (devArgs->scheCpuNum > devArgs->nrAicpu - 1) {
            DEV_ERROR("Aicpu num[%u] less than sche num[%d].", devArgs->nrAicpu, devArgs->scheCpuNum);
        ...
        if ((threadIdx != -1) && threadIdx < devArgs->scheCpuNum) {
            ...
            if (devArgs->enableCtrl == 1 && threadIdx == devArgs->scheCpuNum) {
