...
struct SchDeviceTaskContext {
    ...
    void Dump()
    {
        DEV_ERROR(
            ...
            "Devtask:parallelidx=%u, taskid=%lu,ver=%u,forid=%u,iterid=%u,wsid=%u,allsent=%u,total=%u,stage=%u,"
            "%u, %u, %u, %u",
            ...
            curStage, readyAicCoreFunctionQue->head, readyAicCoreFunctionQue->tail,
...
struct ParallelSchDeviceTaskContext {
    ...
    void RecycleFreeContexts()
    {
        while (!Empty()) {
            ...
            if (FrontElement()->IsFree()) {
                PopFront();
                DEV_DEBUG("Recycling parallel context parallel_idx=%u, version=%u, devtaskid=%lu, leftCtxNum=%u", 
