    ...
    if (tCnt % 50 == 0) { // 50 is loop count to check whether timeout occurs
        ...
        DEV_IF_VERBOSE_DEBUG {
            if (tCur - tStart > DFX_TIME_OUT_THRESHOLD) {
                DEV_ERROR("[sync_timeout] DFX timeout for %s: aicpu forced exit, ttl=%lu.", opString.c_str(), tCnt);
                return DEVICE_MACHINE_ERROR;
            }
        } else {
            if (tCur - tStart > TIME_OUT_THRESHOLD) {
                DEV_ERROR("[sync_timeout] Timeout for %s: aicpu forced exit, ttl=%lu.", opString.c_str(), tCnt);
                return DEVICE_MACHINE_ERROR;
            }
        }
    }
