...
constexpr uint64_t TIMEOUT_NS_10SEC   = 10ULL * 1000ULL * 1000ULL * 1000ULL;  // 10 秒
constexpr uint64_t TIMEOUT_NS_1MIN     = 60ULL * 1000ULL * 1000ULL * 1000ULL;   // 1 分钟
constexpr uint64_t TIMEOUT_NS_2MIN     = 120ULL * 1000ULL * 1000ULL * 1000ULL;  // 2 分钟
...
#define __PYPTO_TIMEOUT_CHECK(state, timeout_ns, warn_interval_ns, action, fmt, ...) \
    do { \
        uint64_t elapsed_ns = state.ElapsedNs(); \
        if (elapsed_ns > timeout_ns) { \
            DEV_ERROR(DEVICE_MACHINE_ERROR, fmt, ##__VA_ARGS__); \
            action; \
        } \
