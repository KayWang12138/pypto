...
#define TIMEOUT_CHECK_EXIT(state, timeout_ns, err_code, fmt, ...) \
    do { \
        ...
        uint64_t timeout_threshold = timeout_ns; \
        if (elapsed_ns > timeout_threshold) { \
            DEV_ERROR(err_code, fmt, ##__VA_ARGS__); \
            return err_code; \
        } \
    } while (0)

// ========== 简化宏：超时退出（带自定义退出动作）==========
// 用于返回类型不是 int/err_code 的场景，如返回结构体
// action 可以是复合语句块，如: { WsAllocation empty; empty.ptr = 0; return empty; }
#define TIMEOUT_CHECK_EXIT_WITH_ACTION(state, timeout_ns, err_code, action, fmt, ...) \
    do { \
        uint64_t elapsed_ns = state.ElapsedNs(); \
        uint64_t timeout_threshold = timeout_ns; \
        if (elapsed_ns > timeout_threshold) { \
...
#define TIMEOUT_CHECK_EXIT_VOID(state, timeout_ns, err_code, fmt, ...) \
    TIMEOUT_CHECK_EXIT_WITH_ACTION(state, timeout_ns, err_code, return, fmt, ##__VA_ARGS__)
...
// ========== 简化宏：先警告后退出（void 函数版本）==========
#define TIMEOUT_CHECK_WARN_EXIT_VOID(state, timeout_ns, warn_ns, err_code, warn_fmt, err_fmt, ...) \
    do { \
        uint64_t elapsed_ns = state.ElapsedNs(); \
        uint64_t timeout_threshold = timeout_ns; \
        uint64_t warn_threshold = warn_ns; \
