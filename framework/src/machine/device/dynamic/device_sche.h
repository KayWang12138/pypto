...
public:
    ...
    int RunThread(int threadIdx, DevStartArgs *devStartArgs, DeviceArgs *args, int schedIdx) {
        ...
        if (args->nrAic == 0 || args->nrValidAic == 0 || args->nrAicpu < NEED_LAUNCH_AICPU_MINNUM) {
            DEV_ERROR("[sched.init.params] Invalid device args: aicNum=%u, validAicNum=%u, aicpuNum=%u (minimum required: %u).",
                args->nrAic, args->nrValidAic, args->nrAicpu, NEED_LAUNCH_AICPU_MINNUM);
    ...
    void ResetRegAll() {
      sleep(1);
      DEV_ERROR("[sched.except.reset] ResetRegAll: Starting register reset for all cores.");
      for (uint32_t i = 0; i < schAicpuNum_; ++i) {
        aicoreManager_[i]->ResetRegAll();
      }
      sleep(1);
      aicoreManager_[0]->CheckAndResetReg();
      DEV_ERROR("[sched.except.reset] Register reset completed.");
    }
    ...
    inline void DumpAicorePerfTrace(std::string file = "") {
        ...
        while (startPos < totalLength) {
            ...
            DEV_ERROR("[sched.perf.trace] AICore profile data: %s", batch.c_str());
            startPos = endPos;
        }
...
struct DynMachineManager {
    ...
    void RunPost(DevAscendProgram *devProg) {
        ...
        DEV_ERROR("[sched.perf.trace] Beginning machine performance trace dump.");
        ...
        DEV_ERROR("[sched.perf.trace] Machine performance trace dump completed.");
#endif
    }
    ...
    int RunUnifiedStream(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry) {
        ...
        if (devArgs->scheCpuNum > devArgs->nrAicpu - 1) {
            DEV_ERROR("[sched.init.params] Insufficient Aicpu: available=%u, required=%u.", devArgs->nrAicpu - 1, devArgs->scheCpuNum);
    ...
    void SigAct(int signum, siginfo_t* info, void* act) {
        (void)info;
        (void)act;
        DEV_ERROR("[sched.except.signal] Signal %d received, invoking handler.", signum);
        PrintBacktrace("signal " + std::to_string(signum));
        if (reset_.load()) {
            DEV_ERROR("[sched.except.reset] Already in reset state, skipping duplicate signal handling.");
            sleep(SIGNAL_DELAY_SECONDS);
            return;
        }
        reset_.store(true);
        if (!init_.load()) {
            DEV_ERROR("[sched.except.signal] System not initialized, calling original signal handler.");
            __sighandler_t handle = GetSigHandle(signum);
            if (handle == SIG_DFL) {
                DEV_ERROR("[sched.except.signal] Original handler is SIG_DFL.");
                signal(signum, SIG_DFL);
                raise(signum);
            } else if (handle == SIG_IGN) {
                DEV_ERROR("[sched.except.signal] Original handler is SIG_IGN.");
            } else if (handle != nullptr) {
                DEV_ERROR("[sched.except.signal] Calling original signal handler.");
                handle(signum);
            }
            return;
        }
    ...
    int EntryUnifiedStream(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry) {
        ...
        if (ret != DEVICE_MACHINE_OK) {
            DEV_ERROR("[sched.init.server] Control server initialization failed.");
            return ret;
        }
    ...
    int Entry(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry) {
        switch (kargs->parameter.runMode) {
            ...
            case RUN_SPLITTED_STREAM_SCHE:
                ...
                break;
            default:
                DEV_ERROR("[sched.init.params] Invalid run mode: %d.", (int)kargs->parameter.runMode);
                break;
        }
