...
public:
    ...
    int RunThread(int threadIdx, DevStartArgs *devStartArgs, DeviceArgs *args, int schedIdx) {
        ...
        if (args->nrAic == 0 || args->nrValidAic == 0 || args->nrAicpu < args->scheCpuNum) {
            DEV_ERROR(ThreadMachineError::DEVICE_ARGS_INVALID, "sche.thread.init",
    ...
    void ResetRegAll() {
      sleep(1);
      DEV_ERROR(ThreadMachineError::RESET_REG_ALL_TRIGGERED, "ctrl.signal.reset.enter",
      ...
      DEV_ERROR(ThreadMachineError::RESET_REG_ALL_TRIGGERED, "ctrl.signal.reset.leave",
    ...
    inline void DumpAicorePerfTrace(std::string file = "") {
        ...
        while (startPos < totalLength) {
            ...
            DEV_ERROR(DumpDfxError::PERF_TRACE_DUMP_ERROR, "sche.task.end.perf.trace",
...
struct DynMachineManager {
    ...
    int RunUnifiedStream(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry) {
        ...
        if (devArgs->scheCpuNum > devArgs->nrAicpu - 1) {
            DEV_ERROR(ThreadMachineError::DEVICE_ARGS_INVALID, "dev.unistream.init.no_cpu",
    ...
    void SigAct(int signum, siginfo_t* info, void* act) {
        (void)info;
        (void)act;
        DEV_ERROR(ThreadMachineError::SIGNAL_HANDLER_ABNORMAL, "sche.except.signal",
                  "signal=%d # Signal received, invoking handler", signum);
        PrintBacktrace("signal " + std::to_string(signum));
        if (reset_.load()) {
            DEV_ERROR(SchedError::EXCEPTION_SIGNAL_RECEIVED, "sche.except.reset",
    ...
    int EntryUnifiedStream(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry) {
        ...
        if (ret != DEVICE_MACHINE_OK) {
            DEV_ERROR(ServerKernelError::DYN_SERVER_INIT_FAILED, "dev.unistream.init.ctrl_init",
    ...
    int Entry(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry) {
        switch (kargs->parameter.runMode) {
            ...
            case RUN_SPLITTED_STREAM_SCHE:
                ...
                break;
            default:
                DEV_ERROR(ThreadMachineError::UNKNOWN, "dev.entry.invalid_mode",
