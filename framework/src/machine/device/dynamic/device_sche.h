...
public:
    ...
    int RunThread(int threadIdx, DevStartArgs *devStartArgs, DeviceArgs *args, int schedIdx) {
        ...
        if (args->nrAic == 0 || args->nrValidAic == 0 || args->nrAicpu < args->scheCpuNum) {
            DEV_ERROR(ThreadMachineErrorScene::DEVICE_ARGS_INVALID, "sche.thread.init",
                      "aicNum=%u validAicNum=%u aicpuNum=%u # Invalid device args",
    ...
    void ResetRegAll() {
      sleep(1);
      DEV_ERROR(ThreadMachineErrorScene::RESET_REG_ALL_TRIGGERED, "ctrl.signal.reset.enter",
                "# Starting register reset for all cores.");
      for (uint32_t i = 0; i < schAicpuNum_; ++i) {
        aicoreManager_[i]->ResetRegAll();
      }
      sleep(1);
      aicoreManager_[0]->CheckAndResetReg();
      DEV_ERROR(ThreadMachineErrorScene::RESET_REG_ALL_TRIGGERED, "ctrl.signal.reset.leave",
                "# Register reset completed.");
    }
    ...
    inline void DumpAicorePerfTrace(std::string file = "") {
        ...
        while (startPos < totalLength) {
            ...
            DEV_ERROR(DumpDfxErrorScene::PERF_TRACE_DUMP_ERROR, "sche.task.end.perf.trace",
                      "data=%s # AICore profile data", batch.c_str());
            startPos = endPos;
        }
...
struct DynMachineManager {
    ...
    void RunPost(DevAscendProgram *devProg) {
        ...
        DEV_ERROR(DumpDfxErrorScene::PERF_TRACE_DUMP_ERROR, "sche.task.end.perf.trace",
                  "# Beginning machine performance trace dump.");
        ...
        DEV_ERROR(DumpDfxErrorScene::PERF_TRACE_DUMP_ERROR, "sche.task.end.perf.trace",
                  "# Machine performance trace dump completed.");
#endif
    }
    ...
    int RunUnifiedStream(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry) {
        ...
        if (devArgs->scheCpuNum > devArgs->nrAicpu - 1) {
            DEV_ERROR(ThreadMachineErrorScene::DEVICE_ARGS_INVALID, "dev.unistream.init.no_cpu",
                      "available=%u required=%u # Insufficient Aicpu",
                      devArgs->nrAicpu - 1, devArgs->scheCpuNum);
    ...
    void SigAct(int signum, siginfo_t* info, void* act) {
        (void)info;
        (void)act;
        DEV_ERROR(ThreadMachineErrorScene::SIGNAL_HANDLER_ABNORMAL, "sche.except.signal",
                  "signal=%d # Signal received, invoking handler", signum);
        PrintBacktrace("signal " + std::to_string(signum));
        if (reset_.load()) {
            DEV_ERROR(SchedErrorScene::EXCEPTION_SIGNAL_RECEIVED, "sche.except.reset",
                      "# Already in reset state, skipping duplicate signal handling.");
        ...
        if (!init_.load()) {
            DEV_ERROR(ThreadMachineErrorScene::SIGNAL_HANDLER_ABNORMAL, "sche.except.signal",
                      "# System not initialized, calling original signal handler.");
            __sighandler_t handle = GetSigHandle(signum);
            if (handle == SIG_DFL) {
                DEV_ERROR(ThreadMachineErrorScene::SIGNAL_HANDLER_ABNORMAL, "sche.except.signal",
                          "# Original handler is SIG_DFL.");
                signal(signum, SIG_DFL);
                raise(signum);
            } else if (handle == SIG_IGN) {
                DEV_ERROR(ThreadMachineErrorScene::SIGNAL_HANDLER_ABNORMAL, "sche.except.signal",
                          "# Original handler is SIG_IGN.");
            } else if (handle != nullptr) {
                DEV_ERROR(ThreadMachineErrorScene::SIGNAL_HANDLER_ABNORMAL, "sche.except.signal",
                          "# Calling original signal handler.");
                handle(signum);
            }
            return;
        }
    ...
    int EntryUnifiedStream(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry) {
        ...
        if (ret != DEVICE_MACHINE_OK) {
            DEV_ERROR(ServerKernelErrorScene::DYN_SERVER_INIT_FAILED, "dev.unistream.init.ctrl_init",
                      "# Control server initialization failed.");
            return ret;
        }
    ...
    int Entry(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry) {
        switch (kargs->parameter.runMode) {
            ...
            case RUN_SPLITTED_STREAM_SCHE:
                ...
                break;
            default:
                DEV_ERROR(ThreadMachineErrorScene::UNKNOWN, "dev.entry.invalid_mode",
                          "runMode=%d # Invalid run mode", (int)kargs->parameter.runMode);
                break;
        }
