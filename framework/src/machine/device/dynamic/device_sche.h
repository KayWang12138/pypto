...
public:
    ...
    int RunThread(int threadIdx, DevStartArgs *devStartArgs, DeviceArgs *args, int schedIdx) {
        ...
        if (args->nrAic == 0 || args->nrValidAic == 0 || args->nrAicpu < args->scheCpuNum) {
            DEV_ERROR(ThreadErr::DEVICE_ARGS_INVALID, "Device machine run invalid args: aicNum=%u, blockdim=%u, launchAicpuNum=%u, launchScheAicpuNum=%u",
    ...
    void ResetRegAll() {
      sleep(1);
      DEV_ERROR(ThreadErr::RESET_REG_ALL_TRIGGERED, "ResetRegAll");
      for (uint32_t i = 0; i < schAicpuNum_; ++i) {
        aicoreManager_[i]->ResetRegAll();
      }
      sleep(1);
      aicoreManager_[0]->CheckAndResetReg();
      DEV_ERROR(ThreadErr::RESET_REG_ALL_TRIGGERED, "Exception reset reg finish.");
    }
...
struct DynMachineManager {
    ...
    int AllocThreadIdxForDav3510(DeviceArgs *devArgs, int cpu, int &curThreadIdx, std::atomic<int> &threadIdx) {
        ...
        while (__builtin_popcount(cpumask_.load(std::memory_order_acquire)) != static_cast<int>(devArgs->nrAicpu)) {
            if (GetCycles() - start > TIMEOUT_CYCLES) {
                DEV_ERROR(ThreadErr::THREAD_ALLOC_TIMEOUT, "Thread alloc timeout: threadIdx=%d, physicalCpu=%d.", curThreadIdx, cpu);
    ...
    int RunUnifiedStream(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry) {
        ...
        if (devArgs->scheCpuNum > devArgs->nrAicpu - 1) {
            DEV_ERROR(ThreadErr::DEVICE_ARGS_INVALID, "Aicpu num[%u] less than scheNum[%u].", devArgs->nrAicpu, devArgs->scheCpuNum);
            return npu::tile_fwk::dynamic::DEVICE_MACHINE_ERROR;
        }
        int threadIdx = -1;
        if (AllocThreadIdx(devArgs, threadIdx, threadIdx_) != npu::tile_fwk::dynamic::DEVICE_MACHINE_OK) {
            DEV_ERROR(ThreadErr::THREAD_CPU_ALLOC_FAILED, "Current cpu[%d] alloc thread failed.", sched_getcpu());
            return npu::tile_fwk::dynamic::DEVICE_MACHINE_ERROR;
        }
    ...
    void SigAct(int signum, siginfo_t* info, void* act) {
        (void)info;
        (void)act;
        DEV_ERROR(ThreadErr::SIGNAL_HANDLER_ABNORMAL, "Exception Signum[%d] Act.", signum);
        PrintBacktrace("signal " + std::to_string(signum));
        if (reset_.load()) {
            DEV_ERROR(ThreadErr::RESET_REG_ALL_TRIGGERED, "Exception Already reset.");
            sleep(SIGNAL_DELAY_SECONDS);
            return;
        }
        reset_.store(true);
        if (!init_.load()) {
            DEV_ERROR(ThreadErr::SIGNAL_HANDLER_ABNORMAL, "Exception call ori sigact.");
            __sighandler_t handle = GetSigHandle(signum);
            if (handle == SIG_DFL) {
                DEV_ERROR(ThreadErr::SIGNAL_HANDLER_ABNORMAL, "Ori sigact SIG_DFL.");
                signal(signum, SIG_DFL);
                raise(signum);
            } else if (handle == SIG_IGN) {
                DEV_ERROR(ThreadErr::SIGNAL_HANDLER_ABNORMAL, "Ori sigact SIG_IGN.");
            } else if (handle != nullptr) {
                DEV_ERROR(ThreadErr::SIGNAL_HANDLER_ABNORMAL, "Call Ori sigact.");
                handle(signum);
            }
            return;
        }
    ...
    int EntryUnifiedStream(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry) {
        ...
        if (ret != DEVICE_MACHINE_OK) {
            DEV_ERROR(ThreadErr::SERVER_INIT_FAILED, "Server init failed");
            return ret;
        }
    ...
    int Entry(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry) {
        switch (kargs->parameter.runMode) {
            ...
            case RUN_SPLITTED_STREAM_SCHE:
                ...
                break;
            default:
                DEV_ERROR(ThreadErr::DEVICE_ARGS_INVALID, "Invalid run mode: %d\n", (int)kargs->parameter.runMode);
                break;
        }
