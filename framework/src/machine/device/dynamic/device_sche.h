...
public:
    ...
    void SetStachSchduleContext(int scheIdx, SchduleContext* context) {
        aicoreManager_[scheIdx]->SetSchduleContext(context);
    }
...
struct DynMachineManager {
    ...
    int EntrySplittedStreamSche(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry) {
        ...
        int ret = DEVICE_MACHINE_OK;
        if (threadIdx != -1 && threadIdx < devArgs.scheCpuNum) {
