...
public:
    ...
    int RunThread(int threadIdx, DevStartArgs *devStartArgs, DeviceArgs *args, int schedIdx) {
        ...
        if (args->nrAic == 0 || args->nrValidAic == 0 || args->nrAicpu < args->scheCpuNum) {
            DEV_ERROR(sche.thread.init, "aicNum=%u validAicNum=%u aicpuNum=%u # Invalid device args",
    ...
    void ResetRegAll() {
      sleep(1);
      DEV_ERROR(ctrl.signal.reset.enter, "# Starting register reset for all cores.");
      for (uint32_t i = 0; i < schAicpuNum_; ++i) {
        aicoreManager_[i]->ResetRegAll();
      }
      sleep(1);
      aicoreManager_[0]->CheckAndResetReg();
      DEV_ERROR(ctrl.signal.reset.leave, "# Register reset completed.");
    }
    ...
    inline void DumpAicorePerfTrace(std::string file = "") {
        ...
        while (startPos < totalLength) {
            ...
            DEV_ERROR(sche.task.end.perf.trace, "data=%s # AICore profile data", batch.c_str());
            startPos = endPos;
        }
...
struct DynMachineManager {
    ...
    int RunUnifiedStream(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry) {
        ...
        if (devArgs->scheCpuNum > devArgs->nrAicpu - 1) {
            DEV_ERROR(dev.unistream.init.no_cpu, "available=%u required=%u # Insufficient Aicpu", devArgs->nrAicpu - 1, devArgs->scheCpuNum);
    ...
    void SigAct(int signum, siginfo_t* info, void* act) {
        (void)info;
        (void)act;
        DEV_ERROR(sche.except.signal, "signal=%d # Signal received, invoking handler", signum);
    ...
    int EntryUnifiedStream(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry) {
        ...
        if (ret != DEVICE_MACHINE_OK) {
            DEV_ERROR(dev.unistream.init.ctrl_init, "# Control server initialization failed.");
            return ret;
        }
    ...
    int Entry(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry) {
        switch (kargs->parameter.runMode) {
            ...
            case RUN_SPLITTED_STREAM_SCHE:
                ...
                break;
            default:
                DEV_ERROR(dev.entry.invalid_mode, "runMode=%d # Invalid run mode", (int)kargs->parameter.runMode);
                break;
        }
