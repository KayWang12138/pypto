...
struct DynMachineManager {
    ...
    int AllocThreadIdxForDav3510(DeviceArgs* devArgs, int cpu, int& curThreadIdx, std::atomic<int>& threadIdx)
    {
        ...
        while (__builtin_popcount(cpumask_.load(std::memory_order_acquire)) != static_cast<int>(devArgs->nrAicpu)) {
            if (state.ElapsedNs() > TIMEOUT_NS_20MIN) {
                DEV_ERROR(DEVICE_MACHINE_TIMEOUT_THREAD_ALLOC,
                          "#sche.thread.init: Thread alloc timeout 20min: threadIdx=%d, physicalCpu=%d.",
                          curThreadIdx, cpu);
                return DEVICE_MACHINE_TIMEOUT_THREAD_ALLOC;
            }
            sched_yield();
        }
    ...
    struct SplittedInfo {
        ...
        void ScheWait(DevAscendProgram* devProg)
        {
            ...
            while (unlikely(!devProg->runtimeDataRingBufferInited)) {
                ...
                uint64_t elapsed1 = state1.ElapsedNs();
                if (elapsed1 > TIMEOUT_NS_1MIN / 2 && !state1.warnPrinted) {
                    DEV_WARN("#sche.wait: RingBuffer init waiting 30s, may timeout soon.");
                    state1.warnPrinted = true;
                }
                if (elapsed1 > TIMEOUT_NS_1MIN) {
            ...
            while (unlikely(ringBufferHead->Empty())) {
                ...
                uint64_t elapsed2 = state2.ElapsedNs();
                if (elapsed2 > TIMEOUT_NS_1MIN / 2 && !state2.warnPrinted) {
                    DEV_WARN("#sche.wait: RingBuffer data waiting 30s, may timeout soon.");
                    state2.warnPrinted = true;
                }
                if (elapsed2 > TIMEOUT_NS_1MIN) {
                    DEV_ERROR(SchedErr::RINGBUFFER_WAIT_TIMEOUT,
