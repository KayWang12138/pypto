...
public:
    ...
    int RunThread(int threadIdx, DevStartArgs *devStartArgs, DeviceArgs *args) {
        int ret = 0;
...
#endif
        ret = aicoreManager_[threadIdx]->RunManager(threadIdx, devStartArgs, args);
...
struct DynMachineManager {
    ...
    int RunSche(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry, int threadIdx) {
        ...
        machine_.SetStachSchduleContext(threadIdx, &localContext);
        ...
        int ret = machine_.RunThread(threadIdx, devStartArgs, devArgs);
    ...
    int EntrySplittedStreamSche(DeviceKernelArgs *kargs, const KernelCtrlEntry &entry) {
        ...
        int threadIdx = allocThreadIdx(devProg->nrAicpu - 1, devProg->scheCpuNum, runtimeDataCurrent->devScheState.threadIdx);
