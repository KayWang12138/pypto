...
struct DynMachineManager {
    ...
    int BindThreadIdxAndCpuId(DeviceArgs *devArgs) {
        ...
        if (cpuId == 1) {
            threadIdx_ = 0; // ctrl cpu thread
        } else if (cpuId == maxCpuId) {
            threadIdx_ = maxCpuId - 1;  // signal reg cpu thread
        } else {    // schedule cpu thread
            if (cpuId <= die0MaxCpuid && die0ScheIdx_ < die0MaxCpuNum) {
                threadIdx_++;
                die0ScheIdx_++;
            } else if (cpuId > die0MaxCpuid && die1ScheIdx_ < die1MaxCpuNum) {
                threadIdx_++;
                die1ScheIdx_++;
            } else {
                threadIdx_++;
    ...
    int Run(DeviceKernelArgs *args, const KernelCtrlEntry &entry) {
        ...
        int threadIdx = BindThreadIdxAndCpuId(devArgs);
