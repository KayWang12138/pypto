...
class DeviceCtrlMachine {
 public:
    ...
    int ExecDyn(npu::tile_fwk::DeviceKernelArgs *args) {
        DEV_INFO("start control flow, inputs:%p size:%lu", args->inputs, (uint64_t)args->outputs);
        auto devProg = PtrToPtr<int64_t, DevAscendProgram>(args->cfgdata);
        auto devStartArgs = (DevStartArgs *)devProg->devArgs.startArgsAddr;
        uint64_t inputSize = *args->inputs;
        uint64_t outputSize = *(args->inputs + 1);
        auto inputPtr = PtrToPtr<int64_t, DevTensorData>(args->inputs + TENSOR_INFO_OFFSET);
