...
public:
    void InitTaskCtrl(int idx, int type, uint64_t taskId, DeviceTask *devTask, DeviceExecuteContext *ctx) {
        if (ctx == nullptr) {
            DEV_ERROR(ServerKernelErrorScene::DYN_SERVER_INIT_FAILED, "ctrl.push.init_dtask",
                      "# Init task control failed, DeviceExecuteContext is null.");
            return;
        }
    ...
    int EntryInit(DeviceKernelArgs *kargs) {
        ...
        if (kargs->inputs == nullptr || kargs->cfgdata == nullptr) {
            DEV_ERROR(ServerKernelErrorScene::DYN_SERVER_ARGS_NULL, "ctrl.init",
                      "inputs=%p workspace=%p cfgdata=%p # Null pointer detected",
