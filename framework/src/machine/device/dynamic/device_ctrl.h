...
public:
    void InitTaskCtrl(int idx, int type, uint64_t taskId, DeviceTask *devTask, DeviceExecuteContext *ctx) {
        if (ctx == nullptr) {
            DEV_ERROR("[ctrl.task.pre.init] Init task control failed, DeviceExecuteContext is null.");
            return;
        }
    ...
    int EntryInit(DeviceKernelArgs *kargs) {
        ...
        if (kargs->inputs == nullptr || kargs->cfgdata == nullptr) {
            DEV_ERROR("[ctrl.task.pre.init] Null pointer detected - inputs=%p, workspace=%p, cfgdata=%p.",
