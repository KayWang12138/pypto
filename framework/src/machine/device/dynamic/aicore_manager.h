...
public:
    ...
    int32_t ProcessCompletedAicpuTask(uint64_t taskId) {
        aicpuTaskManager_.aicpuTaskStat_->tasks[aicpuTaskManager_.aicpuTaskStat_->taskCount].execEnd = GetCycles();
...
private:
    ...
    inline int32_t ResolveDepForAicpuTask(uint64_t& taskCount, uint64_t sharedBuffer) {
        int32_t ret = aicpuTaskManager_.TaskProcess(taskCount, sharedBuffer);
        if (unlikely(ret != DEVICE_MACHINE_OK)) {
            return ret;
        }
