...
public:
    ...
    template <bool enableAicpuInit = false>
    inline int32_t RunCoreTask(SchDeviceTaskContext* devTaskCtx)
    {
        ...
        if (IsNeedProcAicpuTask()) {
            if constexpr (enableAicpuInit) {
                const bool profSwitch = aicoreProf_.ProfIsEnable();
                ret = aicpuTaskManager_.Init(devTaskCtx, profSwitch);
                if (unlikely(ret != DEVICE_MACHINE_OK)) {
                    return ret;
                }
            }
            ret = aicpuTaskManager_.DispatchAndPollTask(devTaskCtx, aicpuTaskSent, this);
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                return ret;
            }
        }
    ...
    inline int ProcessTaskLoop(SchDeviceTaskContext* deviceTaskCtx, bool& isFinish)
    {
        ...
        while (!deviceTaskCtx->IsCoreTaskSendFinish()) {
            int32_t ret = RunCoreTask(deviceTaskCtx);
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                return ret;
            }
    ...
    int32_t ProcessCompletedAicpuTask(uint64_t taskId, uint32_t parallelIdx)
    {
        SchDeviceTaskContext* devTaskCtx = context_->schParallelDevTaskCtx.Element(parallelIdx);
        int32_t ret = ResolveDepDyn(devTaskCtx, taskId);
        if (unlikely(ret != DEVICE_MACHINE_OK)) {
            return ret;
        }
        return BatchPushReadyQueue(devTaskCtx);
    }
...
private:
    ...
    {
        if (IsNeedProcAicpuTask()) {
            auto ret = aicpuTaskManager_.SyncAicpuTaskFinish(devTaskCtx, this);
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                return ret;
            }
        }
    ...
    inline uint64_t TryBatchSendTask(
    ...
    */
    ...
    inline int32_t ResolveDepForAllAiCore(
    ...
    inline void HandShakePostProc(SchDeviceTaskContext* schDeviceTaskCtx, bool needSendAic, bool needSendAiv)
    {
        ...
        if (schDeviceTaskCtx) {
            ...
            DEV_DEBUG(
                "hand shake presend task cnt : aic=%lu, aiv=%lu",
    ...
    inline bool IsNeedProcAicpuTask() { return true; }
