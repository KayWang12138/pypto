...
public:
    ...
    inline int RunManager(int threadIdx, DevStartArgs *devStartArgs, DeviceArgs *deviceArgs) {
        int ret = DEVICE_MACHINE_OK;
        DEV_DEBUG("schedule run threadIdx:%d", threadIdx);
        Init(threadIdx, devStartArgs, deviceArgs);
        PerfMtTrace(PERF_TRACE_INIT, threadIdx);
        DEV_DEBUG("Schedule run init succ");
        DeviceTaskCtrl *taskCtrl = nullptr;
        taskQueue_ = &(devStartArgs->deviceRuntimeDataDesc.taskQueueList[threadIdx]);
        if constexpr (IsDeviceMode()) {
            ret = HandShake();
...
private:
    ...
    inline void Init(int threadIdx, DevStartArgs *startArgs, DeviceArgs *deviceArgs) {
