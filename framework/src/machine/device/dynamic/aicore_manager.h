...
private:
    ...
    inline int32_t PendingUpdateAndResolveDep(bool& isWrapCoreAvailable, CoreType type, int coreIdx, 
    uint32_t& runningIdValue, int& runningResolveIndexBaseValue, uint32_t& pendingIdRef, int& pendingResolveIndexBaseRef) {
    ...
    inline int32_t ResolveByRegVal(CoreType type, int coreIdx) {
        ...
        if (likely(finTaskId == ContextPendAndRun.pendingIdRef && finTaskState == TASK_FIN_STATE)) {
            ...
            ret = HandleRunningCopyOut(ContextPendAndRun, coreIdx, aicpuCallCode);
        } else {
