...
private:
    ...
    inline int32_t DispatchAiCoreTask(CoreType type, ReadyCoreFunctionQueue* readyQue,
        ...
        if (wrapManager_.GetIsmixarch()) {
            ReadyCoreFunctionQueue* dieReadyQue = wrapManager_.GetDieReadyQueue(type, wrapManager_.GetDieId(schedIdx_, aicpuNum_), readyQue);
        ...
        if (context_->readyCount[aicIndex] > 0) {
            ...
            if (context_->readyCount[aicIndex] > 0) {
                ...
                if (wrapManager_.GetIsmixarch() && IsExistAicpuIdleOneDie(CoreType::AIC)) {
                    targetReadyQue = wrapManager_.GetDieReadyQueue(CoreType::AIC, wrapManager_.GetDieId(schedIdx_, aicpuNum_), readyAicCoreFunctionQue_);
                }
        ...
        if (context_->readyCount[aivIndex] > 0) {
            ...
            if (context_->readyCount[aivIndex] > 0) {
                ...
                if (wrapManager_.GetIsmixarch() && IsExistAicpuIdleOneDie(CoreType::AIV)) {
                    targetReadyQue = wrapManager_.GetDieReadyQueue(CoreType::AIV, wrapManager_.GetDieId(schedIdx_, aicpuNum_), readyAivCoreFunctionQue_);
                }
    ...
    inline int32_t PushReadyTask(int coreType, uint64_t taskId) {
        ...
        if (unlikely(context_->readyCount[coreType] == READY_ID_FIX_CACHE_NUM)) {
            ...
            if (wrapManager_.GetIsmixarch() && IsExistAicpuIdleOneDie(static_cast<CoreType>(coreType))) {
                readyQue = wrapManager_.GetDieReadyQueue(static_cast<CoreType>(coreType), wrapManager_.GetDieId(schedIdx_, aicpuNum_), readyQue);
            }
