...
private:
    ...
    inline int32_t ResolveDepForAllAiCore(CoreType type, int coreIdxStart, int coreIdxEnd) {
        ...
        TryBatchSendTask(type, (type == CoreType::AIC) ? readyAicCoreFunctionQue_ : readyAivCoreFunctionQue_, coreIdxStart, coreIdxEnd);
    ...
    inline int32_t ReleaseCoreByRegVal(CoreType type, int coreIdx, uint64_t* finishIds, size_t* resolveIndexBase, int* finishCoreIdx, uint32_t& finishCnt) {
        ...
        auto RecordResolveTask = [&finishIds, &resolveIndexBase, &finishCoreIdx, &finishCnt, &coreIdx] (uint32_t taskId, int indexBase) {
        ...
        if (likely(finTaskId == pendingIdRef && finTaskState == TASK_FIN_STATE)) {
            ...
            if (runningIdValue != AICORE_TASK_INIT) {
                RecordResolveTask(runningIdValue, runningResolveIndexBaseValue);
            }
            RecordResolveTask(pendingIdValue, pendingResolveIndexBaseValue);
            wrapManager_.UpdateFinishIdForMixCore(finTaskId);
            ...
            if (runningIdValueCopyout != AICORE_TASK_INIT) {
                RecordResolveTask(runningIdValueCopyout, runningResolveIndexBaseValueCopyout);
            }
            ...
            if (runningIdValueAck != AICORE_TASK_INIT) {
                RecordResolveTask(runningIdValueAck, runningResolveIndexBaseValueAck);
            }
