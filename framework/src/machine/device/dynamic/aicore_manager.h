...
private:
    ...
    inline int32_t UpdateTaskStateAndResolveDep(bool& isWrapCoreAvailable, AicoreManagerStackVars* m_contextPtr_,
    CoreType type, int coreIdx, uint32_t& runningIdValue, int& runningResolveIndexBaseValue, uint32_t& pendingIdRef,
    ...
    inline int32_t ResolveByRegVal(CoreType type, int coreIdx) {
        ...
        if (likely(finTaskId == pendingIdRef && finTaskState == TASK_FIN_STATE)) {
            ...
            ret = UpdateTaskStateAndResolveDep(isWrapCoreAvailable, m_contextPtr_, type, 
                coreIdx, runningIdValue, runningResolveIndexBaseValue, pendingIdRef, pendingResolveIndexBaseRef);
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                return ret;
            }
