...
public:
    ...
    inline int RunTask(DeviceTaskCtrl *taskCtrl) {
        ...
        wrapManager_.Deinit();
        if (unlikely(ret != DEVICE_MACHINE_OK)) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "Aicpu %d proc finish %lu %lu %lu, but timeout !.", aicpuIdx_,
    ...
    inline void PostRun(int ret, DeviceTaskCtrl *taskCtrl) {
        if (ret) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "task %lu execute error %d, skip rest tasks", taskCtrl->taskId, ret);
...
private:
    ...
    inline void DumpDfxWhenCoreNotStop(AicoreStatus *coreStatus) {
        for (int i = aicStart_; i < aicEnd_; i++) {
            if (coreStatus[i] != AicoreStatus::CORE_FINISH_STOP) {
                DEV_ERROR(ERROR_CODE_UNDEFINED, "left aic core %d not stop, status %d, pending:%u, rungning:%u, regfinishid: %lu,"
        ...
        for (int i = aivStart_; i < aivEnd_; i++) {
            if (coreStatus[i] != AicoreStatus::CORE_FINISH_STOP) {
                DEV_ERROR(ERROR_CODE_UNDEFINED, "left aiv core %d not stop, status %d, pending:%u, rungning:%u, regfinishid: %lu,"
                "core last status:%lu", i, ToUnderlying(coreStatus[i]), pendingIds_[i], runningIds_[i],
                aicoreHal_.GetFinishedTask(i), aicoreHal_.GetAicoreStatus(i));
            }
        }
    ...
    inline int HandShake() {
        ...
        if (rc != DEVICE_MACHINE_OK) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "Aicpu %d handshake failed end.", aicpuIdx_);
            return rc;
        }
