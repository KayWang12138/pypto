...
private:
    ...
    inline int32_t DispatchAiCoreTask(CoreType type, ReadyCoreFunctionQueue* readyQue,
        ...
        if (wrapManager_.GeIsMixArch()) {
            ReadyCoreFunctionQueue* dieReadyQue = wrapManager_.GetDieReadyQueue(type, GetDieId(schedIdx_, aicpuNum_));
    ...
    inline int32_t BatchPushReadyQueue() {
        ...
        if (context_->readyCount[aicIndex] > 0) {
            ...
            if (context_->readyCount[aicIndex] > 0) {
                ReadyCoreFunctionQueue* dieReadyQue = wrapManager_.GetDieReadyQueue(CoreType::AIC, GetDieId(schedIdx_, aicpuNum_));
        ...
        if (context_->readyCount[aivIndex] > 0) {
            ...
            if (context_->readyCount[aivIndex] > 0) {
                ReadyCoreFunctionQueue* dieReadyQue = wrapManager_.GetDieReadyQueue(CoreType::AIV, GetDieId(schedIdx_, aicpuNum_));
    ...
    inline int32_t PushReadyTask(int coreType, uint64_t taskId) {
        ...
        if (unlikely(context_->readyCount[coreType] == READY_ID_FIX_CACHE_NUM)) {
            ReadyCoreFunctionQueue* dieReadyQue = wrapManager_.GetDieReadyQueue(static_cast<CoreType>(coreType), GetDieId(schedIdx_, aicpuNum_));
