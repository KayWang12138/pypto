...
class AiCoreManager {
public:
    explicit AiCoreManager(AicpuTaskManager& aicpuTaskManager, SchThreadStatus& schThreadStatus)
        : aicpuTaskManager_(aicpuTaskManager), threadStatus(schThreadStatus), aicoreProf_(*this) {};
    ~AiCoreManager() {};
    ...
    {
            volatile ParallelDevTask* kernelParallDevTask = aicoreHal_.GetGetParallelDevTask(coreIdx);
            for (uint32_t idx = parallelCtx->front; idx < parallelCtx->rear; idx++) {
                ...
                kernelParallDevTask->elements[idx % npu::tile_fwk::SCH_DEVTASK_MAX_PARALLELISM] =
                    static_cast<int64_t>(PtrToValue(dyntask->GetDynFuncDataList()));
            }
            kernelParallDevTask->front = parallelCtx->front;
            kernelParallDevTask->rear = parallelCtx->rear;
    ...
    {
        int32_t ret = DEVICE_MACHINE_OK;
        DEV_VERBOSE_DEBUG("Run device task entry stage : %u", deviceTaskCtx->CurStage());
        while (true) {
            ...
            switch (deviceTaskCtx->CurStage()) {
                ...
                case DevTaskExecStage::SEND_CORE_TASK: {
                    ret = ProcessTaskLoop(deviceTaskCtx, isStageFinish); // 传入 DeviceTaskCtrl*
                    if (isStageFinish) {
                ...
                case DevTaskExecStage::WAIT_TAIL_TASK_FINISH: {
                    ...
                    ret = SyncTaskFinish(deviceTaskCtx, isStageFinish); // 调用无参版本，内部使用成员变量
                ...
                default:
                    DEV_ERROR(SchedErr::FSM_STATUS_ERROR, "Invalid stage %u.", deviceTaskCtx->CurStage());
            ...
            if (!isStageFinish && deviceTaskCtx->IsParallel()) {
                DEV_VERBOSE_DEBUG("Run device task leave stage : %u", deviceTaskCtx->curStage);
                return ret; // wait parallel scheduled next time
            }
        }

        DEV_DEBUG("aicpu %d proc finish devtask(%lu),aic: %lu, aiv: %lu, aicpu: %lu, stage:%u, ret: %d.",
            aicpuIdx_, deviceTaskCtx->TaskId(), procAicCoreFunctionCnt_,
            procAivCoreFunctionCnt_, procAicpuFunctionCnt_, deviceTaskCtx->CurStage(), ret);
        if (unlikely(ret != DEVICE_MACHINE_OK)) {
            DEV_ERROR(
                ret, "#sche.dtask.leave: Aicpu[%d] proc finish: finishedFunctionCnt=%lu, "
    ...
    {
        int32_t ret = DEVICE_MACHINE_OK;
        DEV_INFO("receive new task %lu.", deviceTaskCtx->TaskId());
    ...
    inline void DumpLastWord(int coreIdx)
    {
        ...
        if (runningIds_[coreIdx] != AICORE_TASK_INIT) {
            DEV_ERROR(
                SchedErr::ABNOMAL_LAST_WORD, "coreod=%d, status=%lu, running taskid=%x, parallelIdx=%u, coreVer=%u",
    ...
    inline void PostRun(int ret)
    {
        if (ret) {
            DEV_ERROR(
                SchedErr::CORE_TASK_EXEC_FAILED,
            ...
            for (uint32_t i = parallelDevTaskCtx.front; i != parallelDevTaskCtx.rear; ++i) {
                ...
                DEV_ERROR(
                    SchedErr::ABNOMAL_LAST_WORD, "Force finish parallel ctx  parallelidx:%u.", i % SCH_DEVTASK_MAX_PARALLELISM);
                taskCtx.Dump();
            }
...
private:
        ...
        SchDeviceTaskContext* devTaskCtx, int coreIdx, bool isLastDevTask, uint32_t& resloveParallelIdx)
    {
        ...
        if (likely(regLFinTaskState == TASK_FIN_STATE)) {
            if (pendingIdRef == regLFinTaskId) {
                ...
                if (devTaskCtx->wrapManager.GetWrapCoreAvailable(coreIdx)) {
                ...
                if (runningIdRef != AICORE_TASK_INIT) {
                    if (ParallelIndex(runningIdRef) == devTaskCtx->parallelIdx) {
                        ...
                        (void)ResolveDepWithDfx(s
                            tatic_cast<CoreType>(type), coreIdx, runningIdRef,
    ...
    {
        SchDeviceTaskContext tmpCtx; // dummy ctx
        tmpCtx.Init();
        SendStopToIdleCore(&tmpCtx, true);
    }
    ...
    inline bool CheckIsTailBatch(SchDeviceTaskContext* devTaskCtx, CoreType type, uint64_t& remaining)
    {
        ...
        remaining =
            devTaskCtx->DeviceTask()->coreFunctionCnt -
            devTaskCtx->DeviceTaskCtrl()->finishedFunctionCnt.load(std::memory_order_relaxed);
        ...
        int coreIdxStart, int coreIdxEnd)
    {
        ...
        if (devTaskCtx->waitTaskCnt[static_cast<int>(type)] > 0) {
            ...
            dwrapManager.DispatchMixCoreTask();
        }
        ...
        if (enableFairSch_ || wrapManager.GetIsMixarch()) { // for die-to-die scheduling
...
#define RAW_TENSOR_ADDR_MASK ((1UL << 63) - 1)
    ...
    uint64_t UpdateParallelCtxAndCalcModifyFlag(int coreIdx, uint32_t coreParallelVersion)
    {
        ...
        volatile ParallelDevTask* coreParallelDevTask = aicoreHal_.GetGetParallelDevTask(coreIdx);
        ...
        for (uint32_t i = parallelCtx.front; i < parallelCtx.rear; ++i) {
            ...
            if (devTaskCtx->bindParallelCtxVersion > coreParallelVersion) {
                modifyFlag |= (1ULL << idx);
                coreParallelDevTask->elements[idx] = funcData;
            }
        }
        return modifyFlag;
    }
    ...
    /*
     |--------16bit-------------|----16bit----|-----1bit------|------1bit-----|-----3bit--------|---10bit---|---16bit--|
     |-parallel ctx modifyflag--|--devtaskid--|--pingpongflag-|---dcci flag---|--prallel index--|--func id--|--opindex-|
    */
    ...
    inline void SendTaskToAiCore(SchDeviceTaskContext* devTaskCtx, CoreType type, int coreIdx, uint64_t newTask)
    {
        ...
        if (devTaskCtx->isFirstTaskSend) {
            ...
            devTaskCtx->isFirstTaskSend = false;
        }
    ...
    int32_t BatchPushReadyQueForParallel(uint32_t resloveParallelIdx)
    {
        ...
        while (mask) {
            ...
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                DEV_ERROR("Failed to batch push parallel devtask readyque index=%d.", idx);
                return ret;
            }
            mask &= (mask - 1);
        }
    ...
    inline int32_t BatchPushReadyQueue(SchDeviceTaskContext* devTaskCtx)
    {
        ...
        if (devTaskCtx->readyCount[aicIndex] > 0) {
            ...
            if (devTaskCtx->readyCount[aicIndex] > 0) {
                ...
                if (devTaskCtx->GetWrapManager().GetIsMixarch() &&
                    EnableDieSceduling(devTaskCtx, CoreType::AIC, devTaskCtx->readyIds[aicIndex][0])) {
        ...
        if (devTaskCtx->readyCount[aivIndex] > 0) {
            ...
            if (devTaskCtx->readyCount[aivIndex] > 0) {
                ...
                if (devTaskCtx->GetWrapManager().GetIsMixarch() &&
                    EnableDieSceduling(devTaskCtx, CoreType::AIV, devTaskCtx->readyIds[aivIndex][0])) {
    ...
    {
        int32_t ret = DEVICE_MACHINE_OK;
        if (enableL2CacheSch_ && TrySendTaskDirectly(devTaskCtx, coreType, taskId)) {
            return DEVICE_MACHINE_OK;
        }
        ...
        if (unlikely(devTaskCtx->readyCount[coreType] == READY_ID_FIX_CACHE_NUM)) {
            ...
            auto wrapManager = devTaskCtx->GetWrapManager();
            if (wrapManager.GetIsMixarch() &&
                EnableDieSceduling(devTaskCtx, static_cast<CoreType>(coreType), devTaskCtx->readyIds[coreType][0])) {
                readyQue =
                    coreType == static_cast<int>(CoreType::AIC) ?
    ...
    {
        uint32_t resolveIndexBase, uint32_t& resloveParallelIdx) {
        ...
        CoreType type, int coreIdx, uint64_t finishId, size_t resolveIndexBase, uint32_t& resloveParallelIdx)
    {
        ...
        DEV_VERBOSE_DEBUG(
            "[Call]: Core %d Dispatch Task: %lu, %u, %u", coreIdx, deviceTaskCtx->TaskId(), FuncID(finishId), TaskID(finishId));
        DfxProcAfterFinishTask(coreIdx, finishId);
        deviceTaskCtx->waitTaskCnt_[static_cast<int>(type)]--;
        return ret;
    }
    ...
    inline bool EnableDieSceduling(SchDeviceTaskContext* deviceTaskCtx, CoreType type, uint32_t taskId)
    {
        if (!enableFairSch_) {
            auto duppedData = GetDuppedData(deviceTaskCtx, taskId);
            auto loopDieId = duppedData->loopDieId_;
            if (loopDieId < 0 ||
                (loopDieId !=
                 static_cast<int8_t>(
                     deviceTaskCtx->GetWrapManager().GetDieId()))) { // prevent parallel_loop incorrectly, task depends on other die
                return false;
            }
            return true;
        } else {
    ...
    inline void DfxProcAfterFinishTask(SchDeviceTaskContext* deviceTaskCtx, int coreIdx, uint64_t taskId)
    {
        ...
        DEV_IF_VERBOSE_DEBUG { recvFinTask_[coreIdx].push_back(TaskInfo(coreIdx, taskId, devTaskCtx->TaskId())); }
        (void)stat;
    }
                ...
                if (unlikely(devTaskCtx->IsParallel())) {
                    ...
                    if (curTaskCtrl->ExistNextSameIterTask()) {
                        ...
                        if (nextTaskCtrl != nullptr) {
                            ...
                            DEV_DEBUG("Sch dev ctx bind next same parallel iter device task(%u), forid %u, iterid %u",
