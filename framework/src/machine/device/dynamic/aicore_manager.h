...
private:
    ...
    inline int32_t DispatchAiCoreTask(CoreType type, ReadyCoreFunctionQueue* readyQue,
        ...
        if (mixManager_.GeIsMixArch()) {
            ReadyCoreFunctionQueue* dieReadyQue = mixManager_.GetDieReadyQueue(type, GetDieId(schedIdx_, aicpuNum_));
            if (dieReadyQue != nullptr) {
    ...
    inline int32_t BatchPushReadyQueue() {
        ...
        if (context_->readyCount[aicIndex] > 0) {
            ...
            if (context_->readyCount[aicIndex] > 0) {
                ReadyCoreFunctionQueue* dieReadyQue = mixManager_.GetDieReadyQueue(CoreType::AIC, GetDieId(schedIdx_, aicpuNum_));
                if (dieReadyQue != nullptr && IsExistOtherAicpuIdleOneDie(CoreType::AIC)) {
        ...
        if (context_->readyCount[aivIndex] > 0) {
            ...
            if (context_->readyCount[aivIndex] > 0) {
                ReadyCoreFunctionQueue* dieReadyQue = mixManager_.GetDieReadyQueue(CoreType::AIV, GetDieId(schedIdx_, aicpuNum_));
                if (dieReadyQue != nullptr && IsExistOtherAicpuIdleOneDie(CoreType::AIV)) {
                    ret = PushReadyQue(dieReadyQue, context_->readyIds[aivIndex], context_->readyCount[aivIndex]);
                } else {
    ...
    inline int32_t PushReadyTask(int coreType, uint64_t taskId) {
        ...
        if (unlikely(context_->readyCount[coreType] == READY_ID_FIX_CACHE_NUM)) {
            ReadyCoreFunctionQueue* dieReadyQue = mixManager_.GetDieReadyQueue(static_cast<CoreType>(coreType), GetDieId(schedIdx_, aicpuNum_));
            if (dieReadyQue != nullptr && IsExistOtherAicpuIdleOneDie(static_cast<CoreType>(coreType))) {
    ...
    inline bool IsExistOtherAicpuIdleOneDie(CoreType type) {
        int idx = (schedIdx_ + 1) % aicpuNum_;
        while (idx != schedIdx_) {
            if (GetDieId(schedIdx_, aicpuNum_) == GetDieId(idx, aicpuNum_)) {
                if (curTaskCtrl_->isAicpuIdle[static_cast<int>(type)][idx].load(std::memory_order_relaxed) == true) {
                    return true;
                }
            }
