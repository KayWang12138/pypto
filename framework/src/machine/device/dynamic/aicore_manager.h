...
inline int ProcessTaskLoop(SchDeviceTaskContext* deviceTaskCtx, bool& isFinish)
    {
        ...
        while (!deviceTaskCtx->IsCoreTaskSendFinish()) {
            ...
            DEV_IF_DEVICE
            {
                __PYPTO_TIMEOUT_CHECK(timeoutState, TIMEOUT_NS_1MIN, TIMEOUT_NS_10SEC,
                    return DEVICE_MACHINE_ERROR,
                    "#sche.task.loop.timeout: ProcessTaskLoop timeout/warning.");
            }
        }
        deviceTaskCtx->SyncAllSchCoreTaskSent();
        isFinish = true;
        ...
        while (ret == 0) {
            ...
            DEV_IF_DEVICE {
                __PYPTO_TIMEOUT_CHECK(timeoutState, TIMEOUT_NS_1MIN, TIMEOUT_NS_10SEC,
                    ...
                    "#sche.parallel.devtask.timeout: Schedule parallel devtask timeout/warning, dequeueFinish=%d.",
                    taskCtrlDequeFinish);
            }
        }
        ...
        while (devTaskCtx->coreFinishedNum < mngCoreNum) {
            ...
            DEV_IF_DEVICE
            {
                __PYPTO_TIMEOUT_CHECK(timeoutState, TIMEOUT_NS_1MIN, TIMEOUT_NS_10SEC,
                    ...
                    "#sche.task.end.sync.timeout: SyncAicoreDevTaskFinish timeout/warning, notstopNum=%u.",
    ...
    inline int HandShakeByGmWithPreSendTask(DevStartArgs* devStartArgs)
    {
        ...
        while (handShakeNum < mngAicoreNum) {
            ...
            __PYPTO_TIMEOUT_CHECK(timeoutState, HAND_SHAKE_TIMEOUT_NS, TIMEOUT_NS_1MIN,
                ...
                "#sche.handshake.timeout: HandShakeByGmWithPreSendTask timeout/warning, notHandshakeNum=%d.",
