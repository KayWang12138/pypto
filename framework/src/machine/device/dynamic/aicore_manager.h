...
private:
    ...
    inline int32_t ResolveDepForAllAiCore(CoreType type, int coreIdxStart, int coreIdxEnd) {
        ...
        uint64_t finishIds[MAX_MANAGER_AIV_NUM] = {0};
        size_t resolveIndexBase[MAX_MANAGER_AIV_NUM] = {0};
        int finishCoreIdx[MAX_MANAGER_AIV_NUM] = {0};
        ...
        for (int i = coreIdxStart; i < coreIdxEnd; i++) {
            if ((runningIds_[i] != AICORE_TASK_INIT || pendingIds_[i] != AICORE_TASK_INIT)) {
                // release finish core
                ret = ReleaseCoreByRegVal(type, i, finishIds, resolveIndexBase, finishCoreIdx, finishCnt);
                if (unlikely(ret != DEVICE_MACHINE_OK)) {
                    return ret;
                }
                if (enableFairSch_) {
        ...
        for (uint32_t i = 0; i < finishCnt; i++) {
            ret = ResolveDepWithDfx(type, finishCoreIdx[i], finishIds[i], resolveIndexBase[i]);
        }
    ...
    inline void RecordResolveTask(uint64_t* finishIds, size_t* resolveIndexBase, int* finishCoreIdx, uint32_t& finishCnt, int coreIdx, uint32_t taskId, int indexBase) {
        finishIds[finishCnt] = taskId;
        resolveIndexBase[finishCnt] = indexBase;
        finishCoreIdx[finishCnt] = coreIdx;
        finishCnt++;
    }

    inline int32_t ReleaseCoreByRegVal(CoreType type, int coreIdx, uint64_t* finishIds, size_t* resolveIndexBase, int* finishCoreIdx, uint32_t& finishCnt) {
        int32_t ret = DEVICE_MACHINE_OK;
        ...
        if (likely(finTaskId == pendingIdRef && finTaskState == TASK_FIN_STATE)) {
            ...
            if (runningIdValue != AICORE_TASK_INIT) {
                RecordResolveTask(finishIds, resolveIndexBase, finishCoreIdx, finishCnt, coreIdx, runningIdValue, runningResolveIndexBaseValue);
            }
            RecordResolveTask(finishIds, resolveIndexBase, finishCoreIdx, finishCnt, coreIdx, pendingIdValue, pendingResolveIndexBaseValue);
            wrapManager_.UpdateFinishIdForMixCore(finTaskId);
            ...
            if (runningIdValueCopyout != AICORE_TASK_INIT) {
                RecordResolveTask(finishIds, resolveIndexBase, finishCoreIdx, finishCnt, coreIdx, runningIdValueCopyout, runningResolveIndexBaseValueCopyout);
            }
            ...
            if (runningIdValueAck != AICORE_TASK_INIT) {
                RecordResolveTask(finishIds, resolveIndexBase, finishCoreIdx, finishCnt, coreIdx, runningIdValueAck, runningResolveIndexBaseValueAck);
            }
