...
private:
    ...
    */
    ...
    inline void SendTaskToAiCore(SchDeviceTaskContext* devTaskCtx, CoreType type, int coreIdx, uint64_t newTask)
    {
        ...
        DEV_IF_NONDEVICE
        {
            if (aicoreHal_.IsHostSimMode() && (coreIdx == 13 || coreIdx == 63)) {
                uint32_t lowTaskId = static_cast<uint32_t>(encodeTaskId & 0xFFFFFFFFU);
                std::fprintf(
                    stderr,
                    "[HOST_SIM_SEND_REG] core=%d phy=%d newTask=0x%lx encode=0x%lx writeMainBase=0x%lx "
                    "lowTask=0x%x parallelIdx=%u dcci=%u devTaskId=%u\n",
                    coreIdx, aicoreHal_.GetHostSimPhyId(coreIdx), newTask, encodeTaskId, encodeTaskId + 1,
                    lowTaskId, ParallelIndex(lowTaskId), DevTaskDcciFlag(lowTaskId), DevTaskId(encodeTaskId >> 32));
            }
        }
    ...
    inline void UpdateAiCoreBlockIndexSection(ArchInfo archInfo)
    {
        ...
        DEV_IF_NONDEVICE
        {
            // Host-sim builds ready-core mapping during handshake (TryHandShakeByGm).
            // Pre-filling here would duplicate ready entries and break scheduler/core communication semantics.
            if (!aicoreHal_.IsHostSimMode()) {
