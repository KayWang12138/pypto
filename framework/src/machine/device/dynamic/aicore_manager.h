...
public:
    ...
    inline void InitDevTask(DeviceTaskCtrl *taskCtrl) {
        ...
        wrapManager_.Init(curDevTask_, context_->coreRunReadyCnt_, context_->runReadyCoreIdx_[CORE_IDX_AIV],
            ...
            runningIds_.data(), aicValidNum_, context_->coreIdxPosition_, context_->wrapCoreStatus_,
...
private:
    ...
    inline bool CheckStopTaskCanBeSent(int coreIdx) {
        ...
        if (bMatch) {
            ...
            context_->coreRunReadyAvail_[coreIdx] = 0;
            return true;
        }
    ...
    inline uint32_t BatchSendTask(CoreType type, uint32_t *newTask, uint32_t taskCount,
        ...
        while (context_->corePendReadyCnt_[static_cast<int>(type)] > 0 && sendCnt < taskCount) {
            if (pendingIds_[idx] == AICORE_TASK_INIT && context_->wrapCoreStatus_[idx] == 0) {
    ...
    inline void SendTaskToAiCore(CoreType type, int coreIdx, uint64_t newTask) {
        ...
        DEV_IF_VERBOSE_DEBUG {
            ...
            if (wrapManager_.IsBindedWrapId(newTask) && context_->wrapCoreStatus_[coreIdx] == 0) {
                DEV_WARN("newTask[%lu][%lx] is mix task, but core[%d] is available!", newTask, newTask, coreIdx);
            }
            if (!wrapManager_.IsBindedWrapId(newTask) && !context_->coreRunReadyAvail_[coreIdx]) {
    ...
    inline void AddReadyCoreIdx(int coreIdx, int type) {
        ...
        context_->coreIdxPosition_[coreIdx] = context_->coreRunReadyCnt_[type]; // not mix can skip
    }
