...
private:
    ...
    inline int32_t ResolveWhenSyncMode(CoreType type, uint32_t finTaskId, uint32_t finTaskState, int coreIdx)  {
        ...
        if (finTaskId == pendingIds_[coreIdx]) {
            uint64_t finTaskRegVal = aicoreHal_.GetFinishedTask(coreIdx);
            uint32_t taskState = REG_LOW_TASK_STATE(finTaskRegVal);
            whhile (taskState == TASK_ACK_STATE) {
                finTaskRegVal = aicoreHal_.GetFinishedTask(coreIdx);
                taskState = REG_LOW_TASK_STATE(finTaskRegVal);
            }
        }
