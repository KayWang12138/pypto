...
public:
    ...
    inline int RunTask(DeviceTaskCtrl *taskCtrl) {
        ...
        wrapManager_.Deinit();
        if (unlikely(ret != DEVICE_MACHINE_OK)) {
            DEV_ERROR(SchedErr::AICPU_TASK_SYNC_TIMEOUT, "Aicpu[%d] proc finish: finishedFunctionCnt=%lu, coreFunctionCnt=%lu, taskId=%lu, but timeout!.", aicpuIdx_,
    ...
    inline void PostRun(int ret, DeviceTaskCtrl *taskCtrl) {
        if (ret) {
            DEV_ERROR(SchedErr::CORE_TASK_PROCESS_FAILED, "taskId=%lu execute error=%d, skip rest tasks", taskCtrl->taskId, ret);
...
private:
    ...
    inline void DumpDfxWhenCoreNotStop(AicoreStatus *coreStatus) {
        for (int i = aicStart_; i < aicEnd_; i++) {
            if (coreStatus[i] != AicoreStatus::CORE_FINISH_STOP) {
                DEV_ERROR(SchedErr::AIC_TASK_WAIT_TIMEOUT, "left aic core %d not stop, status=%d, pendingNum=%u, runningNum=%u, regfinishid=%lu, "
        ...
        for (int i = aivStart_; i < aivEnd_; i++) {
            if (coreStatus[i] != AicoreStatus::CORE_FINISH_STOP) {
                DEV_ERROR(SchedErr::AIV_TASK_WAIT_TIMEOUT, "left aiv core %d not stop, status=%d, pendingNum=%u, runningNum=%u, regfinishid=%lu, "
                "core last status=%lu", i, ToUnderlying(coreStatus[i]), pendingIds_[i], runningIds_[i],
                aicoreHal_.GetFinishedTask(i), aicoreHal_.GetAicoreStatus(i));
            }
        }
        ...
        while (finishStopNum < mngCoreNum) {
            ...
            DEV_IF_DEVICE {
                if (GetCycles() - start_cycles > TIMEOUT_CYCLES) {
                    ...
                    DEV_ERROR(SchedErr::ALL_AICORE_SYNC_TIMEOUT, "SyncAicoreDevTaskFinish timeout notstopNum=%d.", mngCoreNum - finishStopNum);
    ...
    inline int32_t PushReadyQue(ReadyCoreFunctionQueue *readyQue, void *idList, uint32_t idCnt) const {
        ...
        DEV_IF_NONDEVICE {
            if (readyQue->tail > readyQue->capacity){
                DEV_ERROR(SchedErr::READY_QUEUE_OVERFLOW, "readyQue tail=%u > readyQue capacity=%u", readyQue->tail, readyQue->capacity);
    ...
    inline void DumpAicoreStatusWhenTimeout(bool *handFlag) {
        for (int i = aicStart_; i < aicEnd_; i++) {
            if (handFlag[i]) {
                ...
                DEV_ERROR(SchedErr::HANDSHAKE_TIMEOUT, "Aic core[%d] hand shake timeout, status=%lu.",  i, aicoreHal_.GetAicoreStatus(i));
            }
        }
        ...
        for (int i = aivStart_; i < aivEnd_; i++) {
            if (handFlag[i]) {
                ...
                DEV_ERROR(SchedErr::HANDSHAKE_TIMEOUT, "Aiv core[%d] hand shake timeout, status=%lu.", i, aicoreHal_.GetAicoreStatus(i));
            }
        }
    }
    ...
    inline int HandShakeByGmWithPreSendTask() {
        ...
        while (handShakeNum < mngAicoreNum) {
            ...
            if (GetCycles() - start_cycles > HAND_SHAKE_TIMEOUT) {
                ...
                DEV_ERROR(SchedErr::HANDSHAKE_TIMEOUT, "HandShakeByGmWithPreSendTask timeout notHandshakeNum=%d.", mngAicoreNum - handShakeNum);
                return DEVICE_MACHINE_ERROR;
            }
        }
    ...
    inline int HandShake() {
        ...
        if (rc != DEVICE_MACHINE_OK) {
            DEV_ERROR(SchedErr::HANDSHAKE_TIMEOUT, "Aicpu[%d] handshake failed.", aicpuIdx_);
            return rc;
        }
    ...
    inline int ForEachManageAicoreWithRet(std::function<int(int coreIdx)> func) const {
        ...
        for (int i = aicStart_; i < aicEnd_; ++i) {
            ret = func(i);
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                DEV_ERROR(SchedErr::CORE_TASK_PROCESS_FAILED, "proc aicore aic[%d] failed.", i);
                return ret;
            }
        }
        for (int i = aivStart_; i < aivEnd_; ++i) {
            ret = func(i);
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                DEV_ERROR(SchedErr::CORE_TASK_PROCESS_FAILED, "proc aicore aiv[%d] failed.", i);
                return ret;
            }
        }
        return ret;
