...
public:
    ...
    inline int RunTask(DeviceTaskCtrl *taskCtrl) {
        ...
        wrapManager_.Deinit();
        if (unlikely(ret != DEVICE_MACHINE_OK)) {
            DEV_ERROR("[sche.dtask.leave] Aicpu %d task sync timeout: finishedCnt=%lu, expectedCnt=%lu, taskId=%lu.", aicpuIdx_,
    ...
    inline void DumpLastWord(int coreIdx) {
        ...
        if (pendingIds_[coreIdx] != AICORE_TASK_INIT) {
            DEV_ERROR("[sche.lastword.invalid_pending] AICore pending task: coreIdx=%d, status=%lu, taskId=%s, funcData=%s.", coreIdx, status, std::to_string(pendingIds_[coreIdx]).c_str(),
            ((DynDeviceTask *)curDevTask_)->DumpTaskData(pendingIds_[coreIdx]).c_str());
        }
        if (runningIds_[coreIdx] != AICORE_TASK_INIT) {
            DEV_ERROR("[sche.lastword.invalid_pending] AICore running task: coreIdx=%d, status=%lu, taskId=%s, funcData=%s.", coreIdx, status, std::to_string(runningIds_[coreIdx]).c_str(),
    ...
    inline void PostRun(int ret, DeviceTaskCtrl *taskCtrl) {
        if (ret) {
            DEV_ERROR("[sche.dtask.leave.post] Task %lu execution failed with error %d, skipping remaining tasks.", taskCtrl->taskId, ret);
...
private:
    ...
    inline int32_t PushReadyQue(ReadyCoreFunctionQueue *readyQue, void *idList, uint32_t idCnt) const {
        ...
        DEV_IF_NONDEVICE {
            if (readyQue->tail > readyQue->capacity){
                DEV_ERROR("[sche.resolve.enqueue] Ready queue overflow: tail=%u exceeds capacity=%u.", readyQue->tail, readyQue->capacity);
    ...
    inline void DumpAicoreStatusWhenTimeout(bool *handFlag) {
        for (int i = aicStart_; i < aicEnd_; i++) {
            if (handFlag[i]) {
                DEV_ERROR("[sche.handshake.timeout] AIC core %d handshake success, phyId=%d.", i, aicoreHal_.GetPhyIdByBlockId(i));
            } else {
                DEV_ERROR("[sche.handshake.timeout] AIC core %d handshake timeout, status=%lu.", i, aicoreHal_.GetAicoreStatus(i));
            }
        }

        for (int i = aivStart_; i < aivEnd_; i++) {
            if (handFlag[i]) {
                DEV_ERROR("[sche.handshake.timeout] AIV core %d handshake success, phyId=%d.", i, aicoreHal_.GetPhyIdByBlockId(i));
            } else {
                DEV_ERROR("[sche.handshake.timeout] AIV core %d handshake timeout, status=%lu.", i, aicoreHal_.GetAicoreStatus(i));
            }
        }
    }
    ...
    inline int HandShakeByGmWithPreSendTask() {
        ...
        while (handShakeNum < mngAicoreNum) {
            ...
            if (GetCycles() - start_cycles > HAND_SHAKE_TIMEOUT) {
                ...
                DEV_ERROR("[sche.handshake.timeout] Handshake timeout: %d cores not handshaked.", mngAicoreNum - handShakeNum);
                return DEVICE_MACHINE_ERROR;
            }
        }
