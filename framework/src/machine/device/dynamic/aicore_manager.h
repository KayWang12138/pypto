...
public:
    ...
    inline void SetCommonMembers(AicoreManagerStackVars* stackVars) {
        m_contextPtr_ = stackVars;
    }
    ...
    inline void InitDevTask(DeviceTaskCtrl *taskCtrl) {
        ...
        m_contextPtr_->resolveHubCnt_ = 0;
        ...
        wrapManager_.Init(curDevTask_, m_contextPtr_->coreRunReadyCnt_, m_contextPtr_->runReadyCoreIdx_[CORE_IDX_AIV],
            m_contextPtr_->runReadyCoreIdx_[CORE_IDX_AIC], m_contextPtr_->corePendReadyCnt_, aicValidNum_,
    ...
    void CountSendTask(uint64_t &sentAic, uint64_t &sentAiv) {
        sentAic = m_contextPtr_->sendCnt_[static_cast<int>(CoreType::AIC)];
        sentAiv = m_contextPtr_->sendCnt_[static_cast<int>(CoreType::AIV)];
        m_contextPtr_->waitTaskCnt_[static_cast<int>(CoreType::AIC)] += sentAic;
        m_contextPtr_->waitTaskCnt_[static_cast<int>(CoreType::AIV)] += sentAiv;
        m_contextPtr_->sendCnt_[static_cast<int>(CoreType::AIC)] = 0;
        m_contextPtr_->sendCnt_[static_cast<int>(CoreType::AIV)] = 0;
    }
    ...
    inline int32_t RunCoreTask(DeviceTaskCtrl *taskCtrl, uint64_t& sent) {
        ...
        DEV_IF_VERBOSE_DEBUG {
            ...
            __sync_fetch_and_add(&(taskCtrl->finishedHubFunctionCnt), m_contextPtr_->resolveHubCnt_);
...
private:
    ...
    inline bool CheckStopTaskCanBeSent(int coreIdx) {
        ...
        if (likely(regLFinTaskState == TASK_FIN_STATE)) {
            if (pendingIds_[coreIdx] == regLFinTaskId) {
                bMatch = true;
                m_contextPtr_->runReadyCoreIdx_[type][m_contextPtr_->coreRunReadyCnt_[type]++] = coreIdx;
                m_contextPtr_->corePendReadyCnt_[type]++;
                if (runningIds_[coreIdx] != AICORE_TASK_INIT) {
    ...
    inline void PreSendStopToIdleCore(bool isLastDevTask, AicoreStatus *coreStatus, int &finishStopNum) {
        uint32_t aicIdleNum = m_contextPtr_->coreRunReadyCnt_[static_cast<int>(CoreType::AIC)];
        uint32_t aivIdleNum = m_contextPtr_->coreRunReadyCnt_[static_cast<int>(CoreType::AIV)];

        for (uint32_t i = 0; i < aicIdleNum; i++) {
            SendStopToCore(m_contextPtr_->runReadyCoreIdx_[static_cast<int>(CoreType::AIC)][i],
                isLastDevTask, coreStatus, finishStopNum);
        }

        for (uint32_t i = 0; i < aivIdleNum; i++) {
            SendStopToCore(m_contextPtr_->runReadyCoreIdx_[static_cast<int>(CoreType::AIV)][i],
                isLastDevTask, coreStatus, finishStopNum);
        }
    }
    ...
    inline int SyncTaskFinish() {
        ...
        if (!isLastDevTask) {
            ...
            aicAllStop = (m_contextPtr_->coreRunReadyCnt_[static_cast<int>(CoreType::AIC)] == static_cast<uint32_t>(aicNum));
            aivAllStop = (m_contextPtr_->coreRunReadyCnt_[static_cast<int>(CoreType::AIV)] == static_cast<uint32_t>(aivNum));
        }
    ...
    inline uint32_t GetReadyCoreNum(CoreType type) {
        if (enableFairSch_ && IsExistOtherAicpuIdle(type))  {
            return m_contextPtr_->coreRunReadyCnt_[static_cast<int>(type)];
        }
        return m_contextPtr_->corePendReadyCnt_[static_cast<int>(type)];
    }
    ...
    inline uint64_t TryBatchSendTask(CoreType type, ReadyCoreFunctionQueue* readyQue,
        ...
        DEV_VERBOSE_DEBUG("core ready cnt: %u", m_contextPtr_->corePendReadyCnt_[static_cast<int>(type)]);
    ...
    inline uint32_t BatchSendTask(CoreType type, uint32_t *newTask, uint32_t taskCount,
        ...
        uint32_t sendCnt = 0;
        uint32_t coreRunReadyCnt = m_contextPtr_->coreRunReadyCnt_[static_cast<int>(type)];
        DEV_VERBOSE_DEBUG("Begin Batch send %s task: corerunreadycnt:%u, pendreadyCnt:%u, taskCount:%u.",
            ...
            m_contextPtr_->corePendReadyCnt_[static_cast<int>(type)], taskCount);
        while (sendCnt < static_cast<uint64_t>(coreRunReadyCnt) && sendCnt < taskCount) {
            DEV_VERBOSE_DEBUG("  ## send task use runready core %u.",
                m_contextPtr_->runReadyCoreIdx_[static_cast<int>(type)][m_contextPtr_->coreRunReadyCnt_[static_cast<int>(type)] - 1]);
            SendTaskToAiCore(type,
                m_contextPtr_->runReadyCoreIdx_[static_cast<int>(type)][--m_contextPtr_->coreRunReadyCnt_[static_cast<int>(type)]],
                isLifo ? *newTask-- : *newTask++);
            sendCnt++;
        }
        m_contextPtr_->corePendReadyCnt_[static_cast<int>(type)] -= sendCnt;

        uint32_t idx = m_contextPtr_->lastPendReadyCoreIdx_[static_cast<int>(type)];
        ...
        DEV_VERBOSE_DEBUG("  ## send task left pend ready cnt %u , last core index:%u.",
            m_contextPtr_->corePendReadyCnt_[static_cast<int>(type)], idx);
        while (m_contextPtr_->corePendReadyCnt_[static_cast<int>(type)] > 0 && sendCnt < taskCount) {
            if (pendingIds_[idx] == AICORE_TASK_INIT) {
                ...
                sendCnt++;
                m_contextPtr_->corePendReadyCnt_[static_cast<int>(type)]--;
                lastProcCore = idx;
            }
            idx = coreIdxStart + (idx - coreIdxStart + 1) % coreNum;
        }

        if (lastProcCore != m_contextPtr_->lastPendReadyCoreIdx_[static_cast<int>(type)]) {
            m_contextPtr_->lastPendReadyCoreIdx_[static_cast<int>(type)] = coreIdxStart + (lastProcCore - coreIdxStart + 1) % coreNum;
        }
        DEV_VERBOSE_DEBUG("  ## finish send task left runreadycnt:%u pendreadycnt %u, last coreindex:%u.",
            m_contextPtr_->coreRunReadyCnt_[static_cast<int>(type)], m_contextPtr_->corePendReadyCnt_[static_cast<int>(type)], idx);
        return sendCnt;
    }
    ...
    inline int32_t DispatchAiCoreTask(CoreType type, ReadyCoreFunctionQueue* readyQue,
                                       int coreIdxStart, int coreIdxEnd) {
        int32_t ret = DEVICE_MACHINE_OK;
        if (m_contextPtr_->waitTaskCnt_[static_cast<int>(type)] > 0) {
        ...
        if (enableFairSch_) {
            if (m_contextPtr_->coreRunReadyCnt_[static_cast<int>(type)] > 0)  {
                AicpuIsIdle(type);
            } else {
                AicpuIsBusy(type);
            }
    ...
    inline void SendTaskToAiCore(CoreType type, int coreIdx, uint64_t newTask) {
        ...
        pendingResolveIndexList_[coreIdx] = 0;
        m_contextPtr_->sendCnt_[static_cast<int>(type)]++;
    ...
    inline int32_t BatchPushReadyQueue() {
        ...
        if (m_contextPtr_->readyCount[aicIndex] > 0) {
            uint32_t needSendCnt = std::min(GetReadyCoreNum(CoreType::AIC), m_contextPtr_->readyCount[aicIndex]);
            if (needSendCnt > 0) {
                m_contextPtr_->readyCount[aicIndex] -= BatchSendTask(CoreType::AIC, &m_contextPtr_->readyIds[aicIndex][m_contextPtr_->readyCount[aicIndex] - 1],
                    needSendCnt, aicStart_, aicEnd_, true);
            }
            DEV_VERBOSE_DEBUG("resolved new task, aic ready count: %u coretype:%u.", m_contextPtr_->readyCount[aicIndex], aicIndex);
            if (m_contextPtr_->readyCount[aicIndex] > 0) {
                ret = PushReadyQue(readyAicCoreFunctionQue_, m_contextPtr_->readyIds[aicIndex], m_contextPtr_->readyCount[aicIndex]);
                if (unlikely(ret != DEVICE_MACHINE_OK)) {
                    return ret;
                }
            }
            m_contextPtr_->readyCount[aicIndex] = 0;
        }

        if (m_contextPtr_->readyCount[aivIndex] > 0) {
            uint32_t needSendCnt = std::min(GetReadyCoreNum(CoreType::AIV), m_contextPtr_->readyCount[aivIndex]);
            if (needSendCnt > 0) {
                m_contextPtr_->readyCount[aivIndex] -= BatchSendTask(CoreType::AIV, &m_contextPtr_->readyIds[aivIndex][m_contextPtr_->readyCount[aivIndex] - 1],
                    needSendCnt, aivStart_, aivEnd_, true);
            }
            DEV_VERBOSE_DEBUG("resolved new task, aiv ready count: %u coretype: %u.", m_contextPtr_->readyCount[aivIndex], aivIndex);
            if (m_contextPtr_->readyCount[aivIndex] > 0) {
                ret = PushReadyQue(readyAivCoreFunctionQue_, m_contextPtr_->readyIds[aivIndex], m_contextPtr_->readyCount[aivIndex]);
                if (unlikely(ret != DEVICE_MACHINE_OK)) {
                    return ret;
                }
            }
            m_contextPtr_->readyCount[aivIndex] = 0;
        }
        return ret;
    }
    ...
    inline int32_t ResolveWhenSyncMode(CoreType type, uint32_t finTaskId, uint32_t finTaskState, int coreIdx)  {
        ...
        if (finTaskId == pendingIds_[coreIdx] && finTaskState == TASK_FIN_STATE) {
            ...
            pendingResolveIndexList_[coreIdx] = 0;
            m_contextPtr_->corePendReadyCnt_[static_cast<int>(type)]++;
            m_contextPtr_->runReadyCoreIdx_[static_cast<int>(type)][m_contextPtr_->coreRunReadyCnt_[static_cast<int>(type)]++] = coreIdx;
        }
        return ret;
    }
    ...
    inline int32_t PendingResetAndResolveDep(const TaskResolveContext& ContextPendAndRun, const CoreType& type, 
        const int& coreIdx, const uint32_t& runningIdValue, const int& runningResolveIndexBaseValue) {
        ContextPendAndRun.pendingIdRef = AICORE_TASK_INIT; // ResolveDepWithDfx depend this line
        ContextPendAndRun.pendingResolveIndexBaseRef = 0;
        if (ContextPendAndRun.isWrapCoreAvailable) {
            m_contextPtr_->corePendReadyCnt_[static_cast<int>(type)]++;
        }
        if (runningIdValue != AICORE_TASK_INIT) {
            return ResolveDepWithDfx(type, coreIdx, runningIdValue, runningResolveIndexBaseValue);
        }
        return DEVICE_MACHINE_OK;
    }

    inline int32_t HandlePendingFinished(const TaskResolveContext& ContextPendAndRun, const CoreType& type, const int& coreIdx) {
        int32_t ret = DEVICE_MACHINE_OK;
        // pending task is finished, resolve both running and pending task.
        DEV_VERBOSE_DEBUG("Pending Finished: core:%d pending:%x,%d running:%x,%d", coreIdx, ContextPendAndRun.pendingIdRef, 
            ContextPendAndRun.pendingResolveIndexBaseRef, ContextPendAndRun.runningIdRef, ContextPendAndRun.runningResolveIndexBaseRef);
        uint32_t runningIdValue = ContextPendAndRun.runningIdRef;
        int runningResolveIndexBaseValue = ContextPendAndRun.runningResolveIndexBaseRef;
        uint32_t pendingIdValue = ContextPendAndRun.pendingIdRef;
        int pendingResolveIndexBaseValue = ContextPendAndRun.pendingResolveIndexBaseRef;
        ContextPendAndRun.runningIdRef = AICORE_TASK_INIT;
        ContextPendAndRun.runningResolveIndexBaseRef = 0;
        ContextPendAndRun.pendingIdRef = AICORE_TASK_INIT; // ResolveDepWithDfx depend this line
        ContextPendAndRun.pendingResolveIndexBaseRef = 0;
        if (ContextPendAndRun.isWrapCoreAvailable) { // wrapcore doesnt support pending & running yet
            m_contextPtr_->runReadyCoreIdx_[static_cast<int>(type)][m_contextPtr_->coreRunReadyCnt_[static_cast<int>(type)]++] = coreIdx;
            m_contextPtr_->corePendReadyCnt_[static_cast<int>(type)]++;
        }
        if (runningIdValue != AICORE_TASK_INIT) {
    ...
    inline int32_t HandlePendingCopyOut(const TaskResolveContext& ContextPendAndRun, const CoreType& type, const int& coreIdx, const uint32_t& aicpuCallCode) {
        int32_t ret = DEVICE_MACHINE_OK;
        // pending task is copyout, reolve both running and pending task.
        DEV_VERBOSE_DEBUG("Pending Copyout: core:%d pending:%x,%d running:%x,%d", coreIdx, ContextPendAndRun.pendingIdRef, 
            ContextPendAndRun.pendingResolveIndexBaseRef, ContextPendAndRun.runningIdRef, ContextPendAndRun.runningResolveIndexBaseRef);
        ...
        uint32_t pendingIdValue = ContextPendAndRun.pendingIdRef;
        int pendingResolveIndexBaseValue = ContextPendAndRun.pendingResolveIndexBaseRef;
        ContextPendAndRun.runningIdRef = ContextPendAndRun.pendingIdRef;
        ContextPendAndRun.runningResolveIndexBaseRef = copyOutResolveCounter + 1;
        ret = PendingResetAndResolveDep(ContextPendAndRun, type, coreIdx, runningIdValue, runningResolveIndexBaseValue);
        if (unlikely(ret != DEVICE_MACHINE_OK)) {
            return ret;
        }
    ...
    inline int32_t HandlePendingAcknowledged(const TaskResolveContext& ContextPendAndRun, const CoreType& type, const int& coreIdx) {
        int32_t ret = DEVICE_MACHINE_OK;
        // pending task is acknowledged, resolve running task. And move pending to running
        uint32_t curFinTaskId = ContextPendAndRun.pendingIdRef;
        DEV_VERBOSE_DEBUG("Pending Acknowledged: core:%d pending:%x,%d running:%x,%d", coreIdx, ContextPendAndRun.pendingIdRef, 
            ContextPendAndRun.pendingResolveIndexBaseRef, ContextPendAndRun.runningIdRef, ContextPendAndRun.runningResolveIndexBaseRef);
        DEV_IF_VERBOSE_DEBUG {
            recvAckTask_[coreIdx].push_back(TaskInfo(coreIdx, curFinTaskId));
        }
        uint32_t runningIdValue = ContextPendAndRun.runningIdRef;
        int runningResolveIndexBaseValue = ContextPendAndRun.runningResolveIndexBaseRef;
        ContextPendAndRun.runningIdRef = curFinTaskId;
        ContextPendAndRun.runningResolveIndexBaseRef = ContextPendAndRun.pendingResolveIndexBaseRef;
        ret = PendingResetAndResolveDep(ContextPendAndRun, type, coreIdx, runningIdValue, runningResolveIndexBaseValue);
        return ret;
    }

    inline int32_t HandleRunningFinished(const TaskResolveContext& ContextPendAndRun, const CoreType& type, const int& coreIdx) {
        int32_t ret = DEVICE_MACHINE_OK;
        // running task is finished, resolve running task. Pending task is unmodified
        DEV_VERBOSE_DEBUG("Running finished: core:%d pending:%x,%d running:%x,%d", coreIdx, ContextPendAndRun.pendingIdRef, 
            ContextPendAndRun.pendingResolveIndexBaseRef, ContextPendAndRun.runningIdRef, ContextPendAndRun.runningResolveIndexBaseRef);
        uint32_t runningIdValue = ContextPendAndRun.runningIdRef;
        int runningResolveIndexBaseValue = ContextPendAndRun.runningResolveIndexBaseRef;
        ContextPendAndRun.runningIdRef = AICORE_TASK_INIT;
        ContextPendAndRun.runningResolveIndexBaseRef = 0;
        if (ContextPendAndRun.isWrapCoreAvailable && ContextPendAndRun.pendingIdRef == AICORE_TASK_INIT) {
            m_contextPtr_->runReadyCoreIdx_[static_cast<int>(type)][m_contextPtr_->coreRunReadyCnt_[static_cast<int>(type)]++] = coreIdx;
        }
        ret = ResolveDepWithDfx(type, coreIdx, runningIdValue, runningResolveIndexBaseValue);
        if (unlikely(ret != DEVICE_MACHINE_OK)) {
    ...
    inline int32_t HandleRunningCopyOut(const TaskResolveContext& ContextPendAndRun, [[maybe_unused]]const CoreType& type, const int& coreIdx, const uint32_t& aicpuCallCode) {
        int32_t ret = DEVICE_MACHINE_OK;
        // running task is copyout, resolve running task. Pending task is unmodified
        DEV_VERBOSE_DEBUG("Running copyout: core:%d pending:%x,%d running:%x,%d", coreIdx, ContextPendAndRun.pendingIdRef, 
            ContextPendAndRun.pendingResolveIndexBaseRef, ContextPendAndRun.runningIdRef, ContextPendAndRun.runningResolveIndexBaseRef);
        uint32_t copyOutResolveCounter = RuntimeCopyOutResolveCounterDecode(aicpuCallCode);
        uint32_t runningIdValue = ContextPendAndRun.runningIdRef;
        int runningResolveIndexBaseValue = ContextPendAndRun.runningResolveIndexBaseRef;
    ...
    inline int32_t ResolveByRegVal(CoreType type, int coreIdx) {
        ...
        TaskResolveContext ContextPendAndRun(pendingIds_[coreIdx], pendingResolveIndexList_[coreIdx],
            runningIds_[coreIdx], runningResolveIndexList_[coreIdx], wrapManager_.GetWrapCoreAvailable(coreIdx));
        if (likely(finTaskId == ContextPendAndRun.pendingIdRef && finTaskState == TASK_FIN_STATE)) {
            ret = HandlePendingFinished(ContextPendAndRun, type, coreIdx);
        } else if (unlikely(finTaskId == ContextPendAndRun.pendingIdRef && aicpuCallCode != 0)) {
            ret = HandlePendingCopyOut(ContextPendAndRun, type, coreIdx, aicpuCallCode);
        } else if (finTaskId == ContextPendAndRun.pendingIdRef && finTaskState == TASK_ACK_STATE) {
            ret = HandlePendingAcknowledged(ContextPendAndRun, type, coreIdx);
        } else if (finTaskId == ContextPendAndRun.runningIdRef && finTaskState == TASK_FIN_STATE) {
            ret = HandleRunningFinished(ContextPendAndRun, type, coreIdx);
        } else if (unlikely(finTaskId == ContextPendAndRun.runningIdRef && aicpuCallCode != 0)) {
            ret = HandleRunningCopyOut(ContextPendAndRun, type, coreIdx, aicpuCallCode);
        } else {
            DEV_VERBOSE_DEBUG("Warning, maybe inconsistent state. coreidx: %d,finTask: %lx,pending: %x,running: %x.", coreIdx, finTaskRegVal, 
                ContextPendAndRun.pendingIdRef, ContextPendAndRun.runningIdRef);
        }
        if (unlikely(ret != DEVICE_MACHINE_OK)) {
            return ret;
        }
    ...
    inline bool TrySendTaskDirectly(int coreType, uint32_t taskId) {
        if (m_contextPtr_->coreRunReadyCnt_[coreType] > 0) {
            m_contextPtr_->corePendReadyCnt_[coreType]--;
            DEV_VERBOSE_DEBUG("Direct send task when task ready %x.", taskId);
            SendTaskToAiCore(static_cast<CoreType>(coreType),
                m_contextPtr_->runReadyCoreIdx_[coreType][--m_contextPtr_->coreRunReadyCnt_[coreType]], taskId);
            return true;
        }

        if (m_contextPtr_->corePendReadyCnt_[coreType] == 0) {
            return false;
        }
        ...
        int startIdx;
        int coreNum;
        int idx = static_cast<int>(m_contextPtr_->lastPendReadyCoreIdx_[coreType]);
        ...
        m_contextPtr_->lastPendReadyCoreIdx_[coreType] = static_cast<uint32_t>(startIdx + (idx - startIdx + 1) % (coreNum));
        m_contextPtr_->corePendReadyCnt_[coreType]--;
        DEV_VERBOSE_DEBUG("Direct send task when task ready %x.", taskId);
    ...
    inline int32_t PushReadyTask(int coreType, uint64_t taskId) {
        ...
        if (unlikely(m_contextPtr_->readyCount[coreType] == READY_ID_FIX_CACHE_NUM)) {
            ReadyCoreFunctionQueue* readyQue =
                coreType == static_cast<int>(CoreType::AIC) ?  readyAicCoreFunctionQue_ : readyAivCoreFunctionQue_;
            ret = PushReadyQue(readyQue, m_contextPtr_->readyIds[coreType], m_contextPtr_->readyCount[coreType]);
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                return ret;
            }
            m_contextPtr_->readyCount[coreType] = 0;
        }
        m_contextPtr_->readyIds[coreType][m_contextPtr_->readyCount[coreType]++] = taskId;
        return ret;
    }
    ...
    inline int32_t ResolveDepWithDfx(CoreType type, int coreIdx, uint64_t finishId, size_t resolveIndexBase = 0) {
        ...
        m_contextPtr_->waitTaskCnt_[static_cast<int>(type)]--;
        return ret;
    }
    ...
    inline void ResetCnt() {
        m_contextPtr_->waitTaskCnt_[static_cast<int>(CoreType::AIC)] = 0;
        m_contextPtr_->waitTaskCnt_[static_cast<int>(CoreType::AIV)] = 0;
        m_contextPtr_->readyCount[static_cast<int>(CoreType::AIC)] = 0;
        m_contextPtr_->readyCount[static_cast<int>(CoreType::AIV)] = 0;
        m_contextPtr_->sendCnt_[static_cast<int>(CoreType::AIC)] = 0;
        m_contextPtr_->sendCnt_[static_cast<int>(CoreType::AIV)] = 0;
    }
        ...
        while (handShakeNum < mngAicoreNum) {
            ...
            for (int i = aicEnd_ - 1; (!aicAllSuccess) && i >= aicStart_; i--) {
                ...
                if (aicoreHal_.TryHandShakeByGm(i, dotStatus_)) {
                    ...
                    aicSucessCnt++;
                    handFlag[i] = true;
                    m_contextPtr_->corePendReadyCnt_[static_cast<int>(CoreType::AIC)]++;
                    m_contextPtr_->runReadyCoreIdx_[static_cast<int>(CoreType::AIC)][m_contextPtr_->coreRunReadyCnt_[static_cast<int>(CoreType::AIC)]++] = i;
                } else {
            ...
            for (int i = aivEnd_ - 1; (!aivAllSuccess) && i >= aivStart_; i--) {
                ...
                if (aicoreHal_.TryHandShakeByGm(i, dotStatus_)) {
                    ...
                    aivSucessCnt++;
                    handFlag[i] = true;
                    m_contextPtr_->corePendReadyCnt_[static_cast<int>(CoreType::AIV)]++;
                    m_contextPtr_->runReadyCoreIdx_[static_cast<int>(CoreType::AIV)][m_contextPtr_->coreRunReadyCnt_[static_cast<int>(CoreType::AIV)]++] = i;
                } else {
    ...
    inline void UpdateAiCoreBlockIndexSection() {
        ...
        m_contextPtr_->corePendReadyCnt_[static_cast<int>(CoreType::AIC)] = 0;
        m_contextPtr_->corePendReadyCnt_[static_cast<int>(CoreType::AIV)] = 0;
        m_contextPtr_->coreRunReadyCnt_[static_cast<int>(CoreType::AIC)] = 0;
        m_contextPtr_->coreRunReadyCnt_[static_cast<int>(CoreType::AIV)] = 0;

        DEV_IF_NONDEVICE {
            m_contextPtr_->corePendReadyCnt_[static_cast<int>(CoreType::AIC)] = aicEnd_ - aicStart_;
            m_contextPtr_->corePendReadyCnt_[static_cast<int>(CoreType::AIV)] = aivEnd_ - aivStart_;
            ForEachManageAicoreReverse(
                [this](int coreIdx) {
                int coreType = static_cast<int>(AicoreType(coreIdx));
                m_contextPtr_->runReadyCoreIdx_[coreType][m_contextPtr_->coreRunReadyCnt_[coreType]++] = coreIdx;
                });
        } 

        m_contextPtr_->lastPendReadyCoreIdx_[static_cast<int>(CoreType::AIV)] = static_cast<uint32_t>(aivStart_);
        m_contextPtr_->lastPendReadyCoreIdx_[static_cast<int>(CoreType::AIC)] = static_cast<uint32_t>(aicStart_);
...
private:
    ...
    AicoreManagerStackVars* m_contextPtr_{nullptr};
