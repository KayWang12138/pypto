...
public:
    ...
    inline int ProcessTaskLoop(DeviceTaskCtrl *taskCtrl) {
        ...
        while (allSentCnt < curDevTask_->coreFunctionCnt) {
            uint64_t curSent = 0;
            int32_t ret = RunCoreTask<true>(taskCtrl, curSent); // 调度
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                return ret;
            }
            if (likely(curSent == 0)) {
            ...
            if (GetCycles() - start > TIMEOUT_CYCLES) {
                ...
                uint32_t aicRunReady = context_->coreRunReadyCnt_[static_cast<int>(CoreType::AIC)];
                uint32_t aivRunReady = context_->coreRunReadyCnt_[static_cast<int>(CoreType::AIV)];
                uint32_t aicPendReady = context_->corePendReadyCnt_[static_cast<int>(CoreType::AIC)];
                uint32_t aivPendReady = context_->corePendReadyCnt_[static_cast<int>(CoreType::AIV)];
                ...
                DEV_ERROR("ProcessTaskLoop TIMEOUT: taskId=%lu allSentCnt=%u coreFunctionCnt=%lu lastSent=%u "
                          "elapsedCycles=%lu TIMEOUT_CYCLES=%lu aicRunReady=%u aivRunReady=%u "
                          "aicPendReady=%u aivPendReady=%u aicQueueSize=%u aivQueueSize=%u.",
                          taskCtrl->taskId, allSentCnt, curDevTask_->coreFunctionCnt, lastSent,
