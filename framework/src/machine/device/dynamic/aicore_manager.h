...
struct ResolveTaskContext {
    uint32_t finishIds[MAX_MANAGER_AIV_NUM]{0};
    uint32_t resolveIndexBase[MAX_MANAGER_AIV_NUM]{0};
...
private:
    ...
    inline int32_t ResolveDepForAllAiCore(CoreType type, int coreIdxStart, int coreIdxEnd) {
        ...
        // resolve dep
        for (uint32_t i = 0; i < resolveCtx.finishCnt; i++) {
            ret = ResolveDepWithDfx(type, resolveCtx.finishCoreIdx[i], resolveCtx.finishIds[i], resolveCtx.resolveIndexBase[i]);
        }
    ...
    inline void RecordResolveTask(ResolveTaskContext& ctx, int coreIdx, uint32_t taskId, int indexBase) {
        ctx.finishIds[ctx.finishCnt] = taskId;
        ctx.resolveIndexBase[ctx.finishCnt] = indexBase;
        ctx.finishCoreIdx[ctx.finishCnt] = coreIdx;
        ctx.finishCnt++;
    }

    inline int32_t ReleaseCoreByRegVal(CoreType type, int coreIdx, ResolveTaskContext& ctx) {
        int32_t ret = DEVICE_MACHINE_OK;
        ...
        if (likely(finTaskId == pendingIdRef && finTaskState == TASK_FIN_STATE)) {
            ...
            if (runningIdValue != AICORE_TASK_INIT) {
                RecordResolveTask(ctx, coreIdx, runningIdValue, runningResolveIndexBaseValue);
            }
            RecordResolveTask(ctx, coreIdx, pendingIdValue, pendingResolveIndexBaseValue);
            wrapManager_.UpdateFinishIdForMixCore(finTaskId);
            ...
            if (runningIdValueCopyout != AICORE_TASK_INIT) {
                RecordResolveTask(ctx, coreIdx, runningIdValueCopyout, runningResolveIndexBaseValueCopyout);
            }
            ...
            if (runningIdValueAck != AICORE_TASK_INIT) {
                RecordResolveTask(ctx, coreIdx, runningIdValueAck, runningResolveIndexBaseValueAck);
            }
