...
public:
    ...
    inline int RunTask(DeviceTaskCtrl *taskCtrl) {
        ...
        wrapManager_.Deinit();
        if (unlikely(ret != DEVICE_MACHINE_OK)) {
            DEV_ERROR("[aicpu.sync.timeout] Aicpu %d task sync timeout: finishedCnt=%lu, expectedCnt=%lu, taskId=%lu.", aicpuIdx_,
    ...
    inline void DumpLastWord(int coreIdx) {
        ...
        if (pendingIds_[coreIdx] != AICORE_TASK_INIT) {
            DEV_ERROR("[aicore.kernel.exec] AICore pending task: coreIdx=%d, status=%lu, taskId=%s, funcData=%s.", coreIdx, status, std::to_string(pendingIds_[coreIdx]).c_str(),
            ((DynDeviceTask *)curDevTask_)->DumpTaskData(pendingIds_[coreIdx]).c_str());
        }
        if (runningIds_[coreIdx] != AICORE_TASK_INIT) {
            DEV_ERROR("[aicore.kernel.exec] AICore running task: coreIdx=%d, status=%lu, taskId=%s, funcData=%s.", coreIdx, status, std::to_string(runningIds_[coreIdx]).c_str(),
    ...
    inline void PostRun(int ret, DeviceTaskCtrl *taskCtrl) {
        if (ret) {
            DEV_ERROR("[aicore.kernel.exec] Task %lu execution failed with error %d, skipping remaining tasks.", taskCtrl->taskId, ret);
    ...
    inline int RunManager(int threadIdx, DevStartArgs *devStartArgs, DeviceArgs *deviceArgs, int schedIdx) {
        ...
        if constexpr (IsDeviceMode()) {
            ...
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                DEV_ERROR("[aicore.init.handshake] AICore handshake timeout.");
                AbnormalStop();
                while ((taskCtrl = taskQueue_->Dequeue())) {
                    taskCtrl->PutTask(ret);
                }
...
private:
    ...
    inline void DumpDfxWhenCoreNotStop(AicoreStatus *coreStatus) {
        for (int i = aicStart_; i < aicEnd_; i++) {
            if (coreStatus[i] != AicoreStatus::CORE_FINISH_STOP) {
                DEV_ERROR("[aicore.sync.timeout] AIC core %d failed to stop: status=%d, pendingTask=%u, runningTask=%u, regFinishId=%lu, coreStatus=%lu.",
        ...
        for (int i = aivStart_; i < aivEnd_; i++) {
            if (coreStatus[i] != AicoreStatus::CORE_FINISH_STOP) {
                DEV_ERROR("[aicore.sync.timeout] AIV core %d failed to stop: status=%d, pendingTask=%u, runningTask=%u, regFinishId=%lu, coreStatus=%lu.",
                    i, ToUnderlying(coreStatus[i]), pendingIds_[i], runningIds_[i],
                    aicoreHal_.GetFinishedTask(i), aicoreHal_.GetAicoreStatus(i));
            }
        }
        ...
        while (finishStopNum < mngCoreNum) {
            ...
            DEV_IF_DEVICE {
                if (GetCycles() - start_cycles > TIMEOUT_CYCLES) {
                    ...
                    DEV_ERROR("[aicore.sync.timeout] SyncAicoreDevTaskFinish timeout: %d cores not stopped.", mngCoreNum - finishStopNum);
    ...
    inline int32_t PushReadyQue(ReadyCoreFunctionQueue *readyQue, void *idList, uint32_t idCnt) const {
        ...
        DEV_IF_NONDEVICE {
            if (readyQue->tail > readyQue->capacity){
                DEV_ERROR("[ctrl.task.dispatch] Ready queue overflow: tail=%u exceeds capacity=%u.", readyQue->tail, readyQue->capacity);
    ...
    inline void DumpAicoreStatusWhenTimeout(bool *handFlag) {
        for (int i = aicStart_; i < aicEnd_; i++) {
            if (handFlag[i]) {
                DEV_ERROR("[aicore.init.handshake] AIC core %d handshake success, phyId=%d.", i, aicoreHal_.GetPhyIdByBlockId(i));
            } else {
                DEV_ERROR("[aicore.init.handshake] AIC core %d handshake timeout, status=%lu.", i, aicoreHal_.GetAicoreStatus(i));
            }
        }

        for (int i = aivStart_; i < aivEnd_; i++) {
            if (handFlag[i]) {
                DEV_ERROR("[aicore.init.handshake] AIV core %d handshake success, phyId=%d.", i, aicoreHal_.GetPhyIdByBlockId(i));
            } else {
                DEV_ERROR("[aicore.init.handshake] AIV core %d handshake timeout, status=%lu.", i, aicoreHal_.GetAicoreStatus(i));
            }
        }
    }
    ...
    inline int HandShakeByGmWithPreSendTask() {
        ...
        while (handShakeNum < mngAicoreNum) {
            ...
            if (GetCycles() - start_cycles > HAND_SHAKE_TIMEOUT) {
                ...
                DEV_ERROR("[aicore.init.handshake] HandShakeByGmWithPreSendTask timeout: %d cores not handshaked.", mngAicoreNum - handShakeNum);
                return DEVICE_MACHINE_ERROR;
            }
        }
    ...
    inline int HandShake() {
        ...
        if (rc != DEVICE_MACHINE_OK) {
            DEV_ERROR("[aicore.init.handshake] Aicpu %d handshake failed.", aicpuIdx_);
            return rc;
        }
    ...
    inline int ForEachManageAicoreWithRet(std::function<int(int coreIdx)> func) const {
        ...
        for (int i = aicStart_; i < aicEnd_; ++i) {
            ret = func(i);
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                DEV_ERROR("[aicore.kernel.exec] Failed to process AIC core %d.", i);
                return ret;
            }
        }
        for (int i = aivStart_; i < aivEnd_; ++i) {
            ret = func(i);
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                DEV_ERROR("[aicore.kernel.exec] Failed to process AIV core %d.", i);
                return ret;
            }
        }
        return ret;
