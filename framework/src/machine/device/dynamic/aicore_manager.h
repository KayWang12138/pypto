...
public:
    ...
    inline int RunTask(DeviceTaskCtrl *taskCtrl) {
        ...
        wrapManager_.Deinit();
        if (unlikely(ret != DEVICE_MACHINE_OK)) {
            DEV_ERROR(sche.dtask.leave, "aicpuIdx=%d finishedCnt=%lu expectedCnt=%lu taskId=%lu # Aicpu task sync timeout", aicpuIdx_,
    ...
    inline void DumpLastWord(int coreIdx) {
        ...
        if (pendingIds_[coreIdx] != AICORE_TASK_INIT) {
            DEV_ERROR(sche.lastword.invalid_pending, "coreIdx=%d status=%lu taskId=%s funcData=%s # AICore pending task", coreIdx, status, std::to_string(pendingIds_[coreIdx]).c_str(),
            ((DynDeviceTask *)curDevTask_)->DumpTaskData(pendingIds_[coreIdx]).c_str());
        }
        if (runningIds_[coreIdx] != AICORE_TASK_INIT) {
            DEV_ERROR(sche.lastword.invalid_pending, "coreIdx=%d status=%lu taskId=%s funcData=%s # AICore running task", coreIdx, status, std::to_string(runningIds_[coreIdx]).c_str(),
    ...
    inline void PostRun(int ret, DeviceTaskCtrl *taskCtrl) {
        if (ret) {
            DEV_ERROR(sche.dtask.leave.post, "taskId=%lu errorCode=%d # Task execution failed", taskCtrl->taskId, ret);
    ...
    inline int RunManager(int threadIdx, DevStartArgs *devStartArgs, DeviceArgs *deviceArgs, int schedIdx) {
        ...
        if constexpr (IsDeviceMode()) {
            ...
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                DEV_ERROR(sche.handshake.error, "# Handshake timeout.");
                AbnormalStop();
                while ((taskCtrl = taskQueue_->Dequeue())) {
                    taskCtrl->PutTask(ret);
                }
...
private:
    ...
    inline void DumpDfxWhenCoreNotStop(AicoreStatus *coreStatus) {
        for (int i = aicStart_; i < aicEnd_; i++) {
            if (coreStatus[i] != AicoreStatus::CORE_FINISH_STOP) {
                DEV_ERROR(sche.task.end.sync.timeout, "coreIdx=%d status=%d pendingTask=%u runningTask=%u regFinishId=%lu coreStatus=%lu # AIC core failed to stop",
        ...
        for (int i = aivStart_; i < aivEnd_; i++) {
            if (coreStatus[i] != AicoreStatus::CORE_FINISH_STOP) {
                DEV_ERROR(sche.task.end.sync.timeout, "coreIdx=%d status=%d pendingTask=%u runningTask=%u regFinishId=%lu coreStatus=%lu # AIV core failed to stop",
                    i, ToUnderlying(coreStatus[i]), pendingIds_[i], runningIds_[i],
                    aicoreHal_.GetFinishedTask(i), aicoreHal_.GetAicoreStatus(i));
            }
        }
        ...
        while (finishStopNum < mngCoreNum) {
            ...
            DEV_IF_DEVICE {
                if (GetCycles() - start_cycles > TIMEOUT_CYCLES) {
                    ...
                    DEV_ERROR(sche.task.end.sync.timeout, "coresNotStopped=%d # SyncAicoreDevTaskFinish timeout", mngCoreNum - finishStopNum);
    ...
    inline int32_t PushReadyQue(ReadyCoreFunctionQueue *readyQue, void *idList, uint32_t idCnt) const {
        ...
        DEV_IF_NONDEVICE {
            if (readyQue->tail > readyQue->capacity){
                DEV_ERROR(sche.resolve.enqueue, "tail=%u capacity=%u # Ready queue overflow", readyQue->tail, readyQue->capacity);
    ...
    inline void DumpAicoreStatusWhenTimeout(bool *handFlag) {
        for (int i = aicStart_; i < aicEnd_; i++) {
            if (handFlag[i]) {
                DEV_ERROR(sche.handshake.timeout, "coreIdx=%d phyId=%d # AIC core handshake success", i, aicoreHal_.GetPhyIdByBlockId(i));
            } else {
                DEV_ERROR(sche.handshake.timeout, "coreIdx=%d status=%lu # AIC core handshake timeout", i, aicoreHal_.GetAicoreStatus(i));
            }
        }

        for (int i = aivStart_; i < aivEnd_; i++) {
            if (handFlag[i]) {
                DEV_ERROR(sche.handshake.timeout, "coreIdx=%d phyId=%d # AIV core handshake success", i, aicoreHal_.GetPhyIdByBlockId(i));
            } else {
                DEV_ERROR(sche.handshake.timeout, "coreIdx=%d status=%lu # AIV core handshake timeout", i, aicoreHal_.GetAicoreStatus(i));
            }
        }
    }
    ...
    inline int HandShakeByGmWithPreSendTask() {
        ...
        while (handShakeNum < mngAicoreNum) {
            ...
            if (GetCycles() - start_cycles > HAND_SHAKE_TIMEOUT) {
                ...
                DEV_ERROR(sche.handshake.timeout, "coresNotHandshaked=%d # Handshake timeout", mngAicoreNum - handShakeNum);
                return DEVICE_MACHINE_ERROR;
            }
        }
    ...
    inline int HandShake() {
        ...
        if (rc != DEVICE_MACHINE_OK) {
            DEV_ERROR(sche.handshake.presend, "aicpuIdx=%d # Aicpu handshake failed", aicpuIdx_);
            return rc;
        }
    ...
    inline int ForEachManageAicoreWithRet(std::function<int(int coreIdx)> func) const {
        ...
        for (int i = aicStart_; i < aicEnd_; ++i) {
            ret = func(i);
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                DEV_ERROR(sche.task.end.prof.dump, "coreIdx=%d # Failed to process AIC core", i);
                return ret;
            }
        }
        for (int i = aivStart_; i < aivEnd_; ++i) {
            ret = func(i);
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                DEV_ERROR(sche.task.end.prof.dump, "coreIdx=%d # Failed to process AIV core", i);
                return ret;
            }
        }
        return ret;
