...
public:
    ...
    inline void PostRun(int ret, DeviceTaskCtrl *taskCtrl) {
        if (ret) {
            DEV_ERROR(SchedErr::CORE_TASK_EXEC_FAILED,
                "#sche.dtask.leave.post: taskId=%lu execute error=%d, skip rest tasks", taskCtrl->taskId, ret);
        ...
        DEV_IF_NONDEVICE {
            if (readyQue->tail > readyQue->capacity) {
                DEV_ERROR(SchedErr::READY_QUEUE_OVERFLOW,
                    "#sche.resolve.enqueue: readyQue tail=%u > readyQue capacity=%u", readyQue->tail,
                    readyQue->capacity);
