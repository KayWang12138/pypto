...
private:
    ...
    inline DynFuncData* GetDynFuncData(uint64_t taskId) {
        ...
        auto funDataList = (DynFuncData *)(header + 1);
    ...
    inline uint64_t GetCoa(DynFuncData *dynFuncData, const SymInt *attrs, int idx) {
        return attrs[idx].IsExpression() ? dynFuncData->exprTbl[attrs[idx].Value()] : atts[idx].Value();
    }
    ...
    inline schema::offset SchemaGetOffset(DynFuncData *dynFuncData, const SymInt *attrs, const DevAscendOperationOperandInfo &info) {
        ...
        return schema::offset(schema::offsetList(actualOffset));
    }
    ...
    inline void DumpSchemaOperationInfo(int coreIdx, uint64_t newTask) {
        ...
        auto dynFuncData = GetDynFuncData(newTask);
        ...
        for (size_t i = 0; i < iOperandSize; i++) {
            ...
            auto onInfo = duppedData->GetSource()->GetOperationIOperandInfo(opIdx, i);
            DEV_TRACE_DEBUG(LEvent(LUid(deviceTaskId, funcId, rootIndex, taskId, leafIndex),
                LActIncast(SchemaGetIOperandShape(opIdx, i), SchemaGetIOperandOffset(opIdx, i), Range(base, base + size))));
        }
        ...
        for (size_t i = 0; i < oOperandSize; i++) {
            ...
            auto onInfo = duppedData->GetSource()->GetOperationOOperandInfo(opIdx, i);
            DEV_TRACE_DEBUG(LEvent(LUid(deviceTaskId, funcId, rootIndex, taskId, leafIndex),
                LActOutcast(SchemaGetIOperandShape(opIdx, i), SchemaGetIOperandOffset(opIdx, i),, Range(base, base + size))));
        }  
    }
