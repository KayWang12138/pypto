...
private:
    ...
    inline int32_t UpdateTaskStateAndResolveDep(bool& isWrapCoreAvailable, CoreType type, int coreIdx, 
    ...
    inline int32_t ResolveByRegVal(CoreType type, int coreIdx) {
        ...
        if (likely(finTaskId == pendingIdRef && finTaskState == TASK_FIN_STATE)) {
            ...
            ret = UpdateTaskStateAndResolveDep(isWrapCoreAvailable, type, coreIdx, runningIdValue, 
                runningResolveIndexBaseValue, pendingIdRef, pendingResolveIndexBaseRef);
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                return ret;
            }
