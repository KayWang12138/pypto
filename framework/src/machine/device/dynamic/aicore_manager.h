...
public:
    ...
    inline int RunTask(DeviceTaskCtrl *taskCtrl) {
        ...
        wrapManager_.Deinit();
        if (unlikely(ret != DEVICE_MACHINE_OK)) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "Aicpu[%d] proc finish: finishedFunctionCnt=%lu, coreFunctionCnt=%lu, taskId=%lu, but timeout!.", aicpuIdx_,
    ...
    inline void DumpLastWord(int coreIdx) {
        ...
        if (pendingIds_[coreIdx] != AICORE_TASK_INIT) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "status=%lu, pending taskId=%s, funcdata=%s", status, std::to_string(pendingIds_[coreIdx]).c_str(),
            ((DynDeviceTask *)curDevTask_)->DumpTaskData(pendingIds_[coreIdx]).c_str());
        }
        if (runningIds_[coreIdx] != AICORE_TASK_INIT) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "status=%lu, running taskId=%s, funcdata=%s", status, std::to_string(runningIds_[coreIdx]).c_str(),
    ...
    inline void PostRun(int ret, DeviceTaskCtrl *taskCtrl) {
        if (ret) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "taskId=%lu execute error=%d, skip rest tasks", taskCtrl->taskId, ret);
    ...
    inline int RunManager(int threadIdx, DevStartArgs *devStartArgs, DeviceArgs *deviceArgs, int schedIdx) {
        ...
        if constexpr (IsDeviceMode()) {
            ...
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                DEV_ERROR(ERROR_CODE_UNDEFINED, "hand shake timeout.");
                AbnormalStop();
                while ((taskCtrl = taskQueue_->Dequeue())) {
                    taskCtrl->PutTask(ret);
                }
...
private:
    ...
    inline void DumpDfxWhenCoreNotStop(AicoreStatus *coreStatus) {
        for (int i = aicStart_; i < aicEnd_; i++) {
            if (coreStatus[i] != AicoreStatus::CORE_FINISH_STOP) {
                DEV_ERROR(ERROR_CODE_UNDEFINED, "left aic core %d not stop, status=%d, pendingNum=%u, runningNum=%u, regfinishid=%lu, "
        ...
        for (int i = aivStart_; i < aivEnd_; i++) {
            if (coreStatus[i] != AicoreStatus::CORE_FINISH_STOP) {
                DEV_ERROR(ERROR_CODE_UNDEFINED, "left aiv core %d not stop, status=%d, pendingNum=%u, runningNum=%u, regfinishid=%lu, "
                "core last status=%lu", i, ToUnderlying(coreStatus[i]), pendingIds_[i], runningIds_[i],
                aicoreHal_.GetFinishedTask(i), aicoreHal_.GetAicoreStatus(i));
            }
        }
        ...
        while (finishStopNum < mngCoreNum) {
            ...
            DEV_IF_DEVICE {
                if (GetCycles() - start_cycles > TIMEOUT_CYCLES) {
                    ...
                    DEV_ERROR(ERROR_CODE_UNDEFINED, "SyncAicoreDevTaskFinish timeout notstopNum=%d.", mngCoreNum - finishStopNum);
    ...
    inline int32_t PushReadyQue(ReadyCoreFunctionQueue *readyQue, void *idList, uint32_t idCnt) const {
        ...
        DEV_IF_NONDEVICE {
            if (readyQue->tail > readyQue->capacity){
                DEV_ERROR(ERROR_CODE_UNDEFINED, "readyQue tail=%u > readyQue capacity=%u", readyQue->tail, readyQue->capacity);
                return DEVICE_MACHINE_ERROR;
            }
            DEV_ASSERT(ERROR_CODE_UNDEFINED, readyQue->tail <= readyQue->capacity);
        }
        ReadyQueueUnLock(readyQue);
        return DEVICE_MACHINE_OK;
    }
    ...
    inline void DumpAicoreStatusWhenTimeout(bool *handFlag) {
        for (int i = aicStart_; i < aicEnd_; i++) {
            if (handFlag[i]) {
                DEV_ERROR(ERROR_CODE_UNDEFINED, "Aic core[%d] hand shake success, phyid=%d.", i, aicoreHal_.GetPhyIdByBlockId(i));
            } else {
                DEV_ERROR(ERROR_CODE_UNDEFINED, "Aic core[%d] hand shake timeout, status=%lu.",  i, aicoreHal_.GetAicoreStatus(i));
            }
        }

        for (int i = aivStart_; i < aivEnd_; i++) {
            if (handFlag[i]) {
                DEV_ERROR(ERROR_CODE_UNDEFINED, "Aiv core[%d] hand shake success, phyid=%d.", i,  aicoreHal_.GetPhyIdByBlockId(i));
            } else {
                DEV_ERROR(ERROR_CODE_UNDEFINED, "Aiv core[%d] hand shake timeout, status=%lu.", i, aicoreHal_.GetAicoreStatus(i));
            }
        }
    }
    ...
    inline int HandShakeByGmWithPreSendTask() {
        ...
        while (handShakeNum < mngAicoreNum) {
            ...
            if (GetCycles() - start_cycles > HAND_SHAKE_TIMEOUT) {
                ...
                DEV_ERROR(ERROR_CODE_UNDEFINED, "HandShakeByGmWithPreSendTask timeout notHandshakeNum=%d.", mngAicoreNum - handShakeNum);
                return DEVICE_MACHINE_ERROR;
            }
        }
    ...
    inline int HandShake() {
        ...
        if (rc != DEVICE_MACHINE_OK) {
            DEV_ERROR(ERROR_CODE_UNDEFINED, "Aicpu[%d] handshake failed.", aicpuIdx_);
            return rc;
        }
    ...
    inline int ForEachManageAicoreWithRet(std::function<int(int coreIdx)> func) const {
        ...
        for (int i = aicStart_; i < aicEnd_; ++i) {
            ret = func(i);
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                DEV_ERROR(ERROR_CODE_UNDEFINED, "proc aicore aic[%d] failed.", i);
                return ret;
            }
        }
        for (int i = aivStart_; i < aivEnd_; ++i) {
            ret = func(i);
            if (unlikely(ret != DEVICE_MACHINE_OK)) {
                DEV_ERROR(ERROR_CODE_UNDEFINED, "proc aicore aiv[%d] failed.", i);
                return ret;
            }
        }
        return ret;
