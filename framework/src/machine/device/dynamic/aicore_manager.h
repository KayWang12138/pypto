...
private:
    ...
    inline int32_t BatchPushReadyQueue() {
        ...
        if (context_->readyCount[aicIndex] > 0) {
            ...
            if (context_->readyCount[aicIndex] > 0) {
                ...
                if (wrapManager_.GetIsMixarch() && EnableDieSceduling(CoreType::AIC)) {
        ...
        if (context_->readyCount[aivIndex] > 0) {
            ...
            if (context_->readyCount[aivIndex] > 0) {
                ...
                if (wrapManager_.GetIsMixarch() && EnableDieSceduling(CoreType::AIV)) {
    ...
    inline int32_t PushReadyTask(int coreType, uint64_t taskId) {
        ...
        if (unlikely(context_->readyCount[coreType] == READY_ID_FIX_CACHE_NUM)) {
            ...
            if (wrapManager_.GetIsMixarch() && EnableDieSceduling(static_cast<CoreType>(coreType))) {
    ...
    inline bool EnableDieSceduling(CoreType type, uint64_t taskId) {
        if (!enableFairSch_) {
            auto duppedData = GetDuppedData(taskId);
