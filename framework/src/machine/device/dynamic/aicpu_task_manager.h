...
public:
    ...
    __attribute__((always_inline)) inline int32_t DispatchAndPollTask(uint64_t& taskCount, AiCoreManager* aiCoreManager)
    {
        if (likely(IsReady())) {
...
private:
    ...
    {
        npu::tile_fwk::Distributed::WaitUntilTaskCache::Instance().Reset();
    ...
    __attribute__((always_inline)) inline bool IsReady()
    {
        return static_cast<AicpuTaskPrepareState>(curDevTask_->aicpuTaskPrepareState.load(std::memory_order_relaxed)) ==
               AicpuTaskPrepareState::COMPLETED;
    }
};
} // namespace npu::tile_fwk::dynamic
