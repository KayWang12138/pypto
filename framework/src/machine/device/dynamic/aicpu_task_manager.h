...
public:
    ...
    inline int32_t SyncAicpuTaskFinish(AiCoreManager* aiCoreManager)
    {
        ...
        while (!Finished()) {
            ...
            TIMEOUT_CHECK_EXIT_WITH_ACTION(timeoutState, TIMEOUT_NS_1MIN,
                DistributedErrorCode::AICPU_TASK_TIMEOUT,
                return static_cast<int32_t>(DistributedErrorCode::AICPU_TASK_TIMEOUT),
                "#sche.task.end.sync.timeout: SyncAicpuTaskFinish timeout after 1 minute.");
        }
        return DEVICE_MACHINE_OK;
    }
