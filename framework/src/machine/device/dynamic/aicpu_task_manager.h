...
private:
    ...
    __attribute__((always_inline)) inline bool CheckAndUpdateCacheState()
    {
        auto state =
            static_cast<AicpuTaskCacheInitState>(curDevTask_->aicpuTaskCacheInitState.load(std::memory_order_relaxed));
        if (state == AicpuTaskCacheInitState::COMPLETED) {
            cacheReadyCached_ = true;
            return true;
        }
        return false;
