...
__attribute__((weak)) int32_t MsprofReportAdditionalInfo(uint32_t agingFlag, const void *data, uint32_t length);
__attribute__((weak)) int32_t MsprofCheckFeatureIsOn(uint64_t feature);
};

typedef int32_t (*ProfReportAdditionalInfoFunc)(uint32_t agingFlag, const void *data, uint32_t length);
