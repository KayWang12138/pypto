    ...
    inline int ForEachManageAicoreWithRet(std::function<int(int coreIdx)> func) const {
        ...
        for (int i = aicStart_; i < aicEnd_; ++i) {
            ...
            if (ret != npu::tile_fwk::dynamic::DEVICE_MACHINE_OK) {
                DEV_ERROR("[kernel_exec] ForEachManageAicoreWithRet failed: AIC core %d processing error.", i);
                return ret;
            }
        }
        for (int i = aivStart_; i < aivEnd_; ++i) {
            ret = func(i);
            if (ret != npu::tile_fwk::dynamic::DEVICE_MACHINE_OK) {
                DEV_ERROR("[kernel_exec] ForEachManageAicoreWithRet failed: AIV core %d processing error.", i);
                return ret;
            }
        }
        return ret;
