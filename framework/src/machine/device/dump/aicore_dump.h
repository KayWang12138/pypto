...
struct DumpTensorData {
    ...
    void TraverseAllAhapeIndexCombinations(const uint64_t shape[], const uint64_t stride[], const uint64_t offset[],
        ...
        if (idx != dims - 1) {
            ...
            if (ret != 0) {
                DEV_ERROR(DumpDfxError::DUMP_MEMCPY_FAILED, "sche.dump.prep",
...
public:
    ...
    void Dump(const IDE_SESSION &ideSession, DumpTensorInfo dumpTensorInfo, std::string &fileName,
        ...
        if (!ret) {
            DEV_ERROR(DumpDfxError::DUMP_TENSOR_INFO_FAILED, "sche.dump.info",
        ...
        if (!ret) {
            DEV_ERROR(DumpDfxError::DUMP_TENSOR_DATA_FAILED, "sche.dump.data",
