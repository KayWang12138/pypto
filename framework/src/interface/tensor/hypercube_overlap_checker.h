...
// 判断两个hypercube间是否有重叠
template <typename T>
bool HypercubeOverlapCheckerBlock<T>::NoOverlap(
    const std::vector<int> &hypercube1, const std::vector<int> &hypercube2) {
...
// 计算两个hypercube间重叠的大小
template <typename T>
int HypercubeOverlapCheckerBlock<T>::CalOverlap(
    const std::vector<int> &hypercube1, const std::vector<int> &hypercube2) {
    int dim = hypercube1.size() / elementOfDim;
    int res{1};
    for (int i = 0; i < dim; i++) {
