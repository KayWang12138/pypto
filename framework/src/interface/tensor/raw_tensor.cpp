...
std::shared_ptr<RawTensor> RawTensor::LoadJson(const Json &rawTensorDump) {
    ASSERT(ERROR_CODE_UNDEFINED, rawTensorDump[T_FIELD_KIND].get<int>() == static_cast<int>(Kind::T_KIND_RAW_TENSOR))
...
void RawTensor::AddRefCount(int value) {
    ASSERT(ERROR_CODE_UNDEFINED, value == 1 || value == -1) << "value: " << value;
    refCount_ += value;
    if (refCount_ < 0) {
