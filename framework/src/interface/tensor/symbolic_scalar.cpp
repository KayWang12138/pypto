...
std::string CompileSourceCode(const std::string& sourceFilePath, const std::string& gcc, const std::string& extraCflag)
{
    ...
    std::string cmdGccLog;
    for (const auto& arg : argsGcc) {
        cmdGccLog += arg + " ";
    }
    FUNCTION_LOGI("[RunCmd] %s", cmdGccLog.c_str());
    ...
    std::string cmdAsLog;
    for (const auto& arg : argsAs) {
        cmdAsLog += arg + " ";
    }
    FUNCTION_LOGI("[RunCmd] %s", cmdAsLog.c_str());
...
namespace {
void DumpSourceIfNeeded(const std::string& code, const std::string& sourceFilePath, bool boolNeedDump)
{
    if (!boolNeedDump) {
        return;
    }
...
std::string CompileAndLink(
    const std::string& sourceFilePath, const std::vector<std::string>& exprSrcFiles, const std::string& aicpuPath,
    const std::string& gcc, const std::string& ld, const std::string& extraCflag)
{
    ...
    std::string cmdLdLog;
    for (const auto& arg : argsLd) {
        cmdLdLog += arg + " ";
    }
    FUNCTION_LOGI("[RunCmd] %s", cmdLdLog.c_str());
    FUNCTION_ASSERT(SafeExecCommand(argsLd) == 0);
    return objectFilePath;
}

std::string ExtractBinarySection(
    const std::string& objectFilePath, const std::string& sectionName, const std::string& objcopy)
{
    std::string binaryFilePath = objectFilePath.substr(0, objectFilePath.rfind('.')) + ".bin";
    std::vector<std::string> argsObjcopy = {
        objcopy, "--dump-section", sectionName + "=" + binaryFilePath, objectFilePath};

    std::string cmdObjcopyLog;
    for (const auto& arg : argsObjcopy) {
        cmdObjcopyLog += arg + " ";
    }
    FUNCTION_LOGI("[RunCmd] %s", cmdObjcopyLog.c_str());
    FUNCTION_ASSERT(SafeExecCommand(argsObjcopy) == 0);
    return binaryFilePath;
}

std::vector<uint8_t> ReadBinaryFile(const std::string& binaryFilePath)
{
...
{
    DumpSourceIfNeeded(code, sourceFilePath, needDump);
    std::string objectFilePath = CompileAndLink(sourceFilePath, exprSrcFiles, aicpuPath, gcc, ld, extraCflag);
    std::string binaryFilePath = ExtractBinarySection(objectFilePath, sectionName, objcopy);
    return ReadBinaryFile(binaryFilePath);
}
