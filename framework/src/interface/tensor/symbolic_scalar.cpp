...
std::string CompileSourceCode(const std::string& sourceFilePath, const std::string& gcc, const std::string& extraCflag)
{
    ...
    ASSERT(FError::EINTERNAL, system(cmdGcc.c_str()) == 0);
    ...
    ASSERT(FError::EINTERNAL, system(cmdAs.c_str()) == 0);
    ...
    const std::vector<std::string>& sourceFiles, const std::string& gcc, const std::string& extraCflag)
{
    ...
    ASSERT(FError::EINTERNAL, numThreads > 0);
    ...
    const std::string& sectionName, bool needDump, const std::string& extraCflag)
{
    ...
    ASSERT(FError::EINTERNAL, system(cmdAs.str().c_str()) == 0);
    ...
    ASSERT(FError::EINTERNAL, system(cmdObjcopy.c_str()) == 0);
...
void SymbolicExpressionTable::SetElementKeyOnce(const std::string& key)
{
    if (elementKey_.size() == 0) {
        ...
        ASSERT(FError::INVALID_VAL, elementKey_ == key) << "elementKey_: " << elementKey_ << ", key: " << key;
    }
}
...
void SymbolicExpressionTable::SetTitleOnce(const std::string& title)
{
    if (title_.size() == 0) {
        ...
        ASSERT(FError::INVALID_VAL, title_ == title) << "title_: " << title_ << ", title: " << title;
    }
}
    ...
    std::ostringstream& oss)
{
    ...
    ASSERT(FError::INVALID_VAL, operands.size() >= MIN_EXTREMA_OPERANDS)
        ...
        case SymbolicScalarKind::T_SCALAR_SYMBOLIC_EXPRESSION: {
            ...
            if (expr->Opcode() == SymbolicOpcode::T_MOP_CALL) {
                ...
                if (CallIsGetInputData(callee)) {
                    ...
                    ASSERT(FError::NOT_EXIST, it != tensorNameToDependCore.end())
...
{
    ASSERT(FError::INVALID_TYPE, IsImmediate())
...
{
    ASSERT(FError::INVALID_TYPE, IsSymbol()) << "Mismatch symbol type: " << SymbolicScalarKind2Name(Kind());
    auto symbol = static_cast<const RawSymbolicSymbol*>(this);
    return symbol->Name();
}
SymbolicOpcode RawSymbolicScalar::GetExpressionOpcode() const
{
    ASSERT(FError::INVALID_TYPE, IsExpression())
...
{
    ASSERT(FError::INVALID_VAL, operandList_.size() >= MIN_EXTREMA_OPERANDS)
