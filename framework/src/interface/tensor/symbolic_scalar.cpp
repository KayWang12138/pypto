...
bool SymbolicExpressionTable::CheckDependAicore(const std::string &expr, const std::function<bool(const std::string&)> &getDependConstFunc) {
    size_t pos = 0;
    ...
    while (pos < expr.size()) {
        ...
        if (next < expr.size() && expr[next] == '(') {
            ...
            bool dependConst = getDependConstFunc(tensorId);
            if (!dependConst) { // dependConst 为 false 表示 DEPEND_AICORE
                return true;
            }
            return false;
        }
