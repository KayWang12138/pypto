...
std::vector<uint8_t> CompileAndLoadSection(const std::string &code, const std::string &sourceFilePath, std::string aicpuPth,
    ...
    if (exprSrcFiles.empty()) {
        ...
        cmdAs += " -O2 " + objectFilePath + " -T " + aicpuPth + "/merge.link";
        FUNCTION_LOGI("[RunCmd] %s", cmdAs.c_str());
        ASSERT(system(cmdAs.c_str()) == 0);
    }
    std::string LD_PRELOAD = "LD_PRELOAD= ";
