...
std::string SymbolicExpressionTable::BuildExpression(const SymbolicScalar &ss, std::set<std::string> *dependArgs) {
    return BuildExpression(ss.Raw(), dependArgs);
}

std::string SymbolicExpressionTable::BuildExpression(const RawSymbolicScalarPtr &ss, std::set<std::string> *dependArgs) {
...
std::string SymbolicExpressionTable::BuildExpressionByRaw(const RawSymbolicScalarPtr &raw, const std::unordered_map<RawSymbolicScalarPtr, std::string> &exprDict,
    ...
    switch (raw->Kind()) {
        ...
        case SymbolicScalarKind::T_SCALAR_SYMBOLIC_EXPRESSION: {
            ...
            result = BuildExpressionCode(expr, exprDict, dependArgs);
        } break;
