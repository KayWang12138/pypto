...
inline std::string SymbolicScalarKind2Name(SymbolicScalarKind kind) {
    ...
    case SymbolicScalarKind::T_SCALAR_SYMBOLIC_EXPRESSION:
        ...
        break;
    default:
        ASSERT(ERROR_CODE_UNKNOWN, false) << " undefined kind.";
        break;
    }
    return name;
}
    ...
    RawSymbolicExpression(SymbolicOpcode opcode, const std::vector<RawSymbolicScalarPtr> &operandList)
        ...
        for (const auto &operand : operandList) {
            if (operand->IsExpression()) {
                ...
                ASSERT(ERROR_CODE_UNKNOWN, !expression->IsLoopBeginCall());
                ASSERT(ERROR_CODE_UNKNOWN, !expression->IsLoopEndCall());
            }
        }
    }
...
#undef  RAW_SYMBOLIC_EXPRESSION_CALC_DEFINE_BOPF
    ...
    static ScalarImmediateType CalcMopMin(const std::vector<ScalarImmediateType>& immediateList) {
        ASSERT(ERROR_CODE_UNKNOWN, !immediateList.empty());
    ...
    static SymbolicCalcMop GetSymbolicCalcMultiple(SymbolicOpcode opcode) {
        switch (opcode) {
            ...
            default:
                ASSERT(ERROR_CODE_UNKNOWN, false) << "Not a MOP extrema opcode: " << static_cast<size_t>(opcode);
                return nullptr;
        }
    }
    ...
    static inline ScalarImmediateType CalcMopCall(const std::vector<ScalarImmediateType> &immediateList) {
        ...
        switch (immediateList.size()) {
            ...
            case 4:
                ...
                break;
            default: ASSERT(ERROR_CODE_UNKNOWN, false) << "immediateList.size(): " << immediateList.size(); break;
        }
        return result;
    }
    ...
    static void Handle2NonzeroOperand(RawSymbolicScalarPtr& raw, SymbolicOpcode opcode, std::vector<RawSymbolicScalarPtr>& nonzeroOperandList) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, nonzeroOperandList.size() == size2)
        ...
        if (nonzeroOperandList[0]->IsImmediate()) {
            ...
            if (!nonzeroOperandList[1]->IsImmediate() || !nonzeroOperandList[0]->IsExpression()) {
                ...
                if (expression->Opcode() == SymbolicOpcode::T_BOP_ADD) {
                    ASSERT(ERROR_CODE_UNKNOWN, expression->OperandList().size() == size2)
    ...
    static bool AllImmediate(const std::vector<RawSymbolicScalarPtr>& ops) {
        ASSERT(ERROR_CODE_UNKNOWN, !ops.empty());
    ...
    static ScalarImmediateType FoldAllImmediate(SymbolicOpcode opcode,
        ...
        if (SymbolicOpcode::T_UOP_BEGIN <= opcode && opcode < SymbolicOpcode::T_UOP_END) {
            ASSERT(ERROR_CODE_UNKNOWN, immediateList.size() == 1) << "immediateList.size():  " << immediateList.size();
        ...
        ASSERT(ERROR_CODE_UNKNOWN, false) << "undefined behavior.";
        return 0;
    }
    ...
    ScalarImmediateType Evaluate(const RawSymbolicScalarPtr &raw) const {
        ...
        switch (raw->Kind()) {
            ...
            case SymbolicScalarKind::T_SCALAR_SYMBOLIC_SYMBOL: {
                ...
                ASSERT(ERROR_CODE_UNKNOWN, symbolValueDict->count(symbol->Name())) << symbol->Name() << " has not been found in symbolValueDict";
            ...
            default: ASSERT(ERROR_CODE_UNKNOWN, false) << " undefined behavior."; break;
        }
        return result;
    }
};
...
struct SymbolicExpressionTableX {
    ...
    int LookupExpressionIndex(const SymbolicScalar &ss) const {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, expressionIndexTable.count(str)) << str << " has not been found in expressionIndexTable.";
...
struct SymbolicSymbolTable {
    ...
    void AddSymbol(const RawSymbolicScalarPtr &raw) {
        ASSERT(ERROR_CODE_UNKNOWN, raw->Kind() == SymbolicScalarKind::T_SCALAR_SYMBOLIC_SYMBOL)
...
private:
    void AddAllSymbol(const RawSymbolicScalarPtr &raw) {
        switch (raw->Kind()) {
            ...
            default: ASSERT(ERROR_CODE_UNKNOWN, false) << SymbolicScalarKind2Name(raw->Kind()) << " undefined behavior"; break;
        }
    }
};
...
struct SymbolicExpressionTable {
    ...
    void NormalizeForSymbolTable(const SymbolicSymbolTable &symbolTable) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, symTable.size() == symExprTable.size())
    ...
    int LookupPrimaryExpressionIndex(const SymbolicScalar &ss) const {
        std::string str = BuildExpressionByRaw(ss.Raw(), {});
        ASSERT(ERROR_CODE_UNKNOWN, primaryExpressionDict_.count(str))  << str << " has not been found in primaryExpressionDict_.";
        auto raw = primaryExpressionDict_.find(str)->second;
        ASSERT(ERROR_CODE_UNKNOWN, primaryExpressionSet.count(raw))  << raw << " has not been found in primaryExpressionSet.";
