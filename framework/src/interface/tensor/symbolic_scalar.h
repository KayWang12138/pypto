...
inline std::string SymbolicScalarKind2Name(SymbolicScalarKind kind)
{
    ...
    switch (kind) {
        ...
        case SymbolicScalarKind::T_SCALAR_SYMBOLIC_EXPRESSION:
            ...
            break;
        default:
            ASSERT(FError::EINTERNAL, false) << " undefined kind.";
            break;
    }
    return name;
}
          ...
          operandList_(operandList)
    {
        ...
        for (const auto& operand : operandList) {
            if (operand->IsExpression()) {
                ...
                ASSERT(FError::EINTERNAL, !expression->IsLoopBeginCall());
                ASSERT(FError::EINTERNAL, !expression->IsLoopEndCall());
            }
        }
    }
...
#undef RAW_SYMBOLIC_EXPRESSION_CALC_DEFINE_BOPF
    ...
    {
        ASSERT(FError::EINTERNAL, !immediateList.empty());
    ...
    static SymbolicCalcMop GetSymbolicCalcMultiple(SymbolicOpcode opcode)
    {
        switch (opcode) {
            ...
            default:
                ASSERT(FError::EINTERNAL, false) << "Not a MOP extrema opcode: " << static_cast<size_t>(opcode);
                return nullptr;
        }
    }
        ...
        RawSymbolicScalarPtr& raw, SymbolicOpcode opcode, std::vector<RawSymbolicScalarPtr>& nonzeroOperandList)
    {
        ...
        ASSERT(FError::EINTERNAL, nonzeroOperandList.size() == size2)
        ...
        if (nonzeroOperandList[0]->IsImmediate()) {
            ...
            if (!nonzeroOperandList[1]->IsImmediate() || !nonzeroOperandList[0]->IsExpression()) {
                ...
                if (expression->Opcode() == SymbolicOpcode::T_BOP_ADD) {
                    ASSERT(FError::EINTERNAL, expression->OperandList().size() == size2)
    ...
    {
        ASSERT(FError::EINTERNAL, !ops.empty());
        ...
        SymbolicOpcode opcode, const std::vector<ScalarImmediateType>& immediateList)
    {
        if (SymbolicOpcode::T_UOP_BEGIN <= opcode && opcode < SymbolicOpcode::T_UOP_END) {
            ASSERT(FError::EINTERNAL, immediateList.size() == 1) << "immediateList.size():  " << immediateList.size();
        ...
        ASSERT(FError::EINTERNAL, false) << "undefined behavior.";
        return 0;
    }
    ...
    ScalarImmediateType Evaluate(const RawSymbolicScalarPtr& raw) const
    {
        ...
        switch (raw->Kind()) {
            ...
            case SymbolicScalarKind::T_SCALAR_SYMBOLIC_SYMBOL: {
                ...
                ASSERT(FError::EINTERNAL, symbolValueDict->count(symbol->Name()))
...
struct SymbolicExpressionTableX {
    ...
    int LookupExpressionIndex(const SymbolicScalar& ss) const
    {
        ...
        ASSERT(FError::EINTERNAL, expressionIndexTable.count(str)) << str << " has not been found in expressionIndexTable.";
...
struct SymbolicSymbolTable {
    ...
    {
        ASSERT(FError::EINTERNAL, raw->Kind() == SymbolicScalarKind::T_SCALAR_SYMBOLIC_SYMBOL)
...
private:
    void AddAllSymbol(const RawSymbolicScalarPtr& raw)
    {
        switch (raw->Kind()) {
            ...
            default:
                ASSERT(FError::EINTERNAL, false) << SymbolicScalarKind2Name(raw->Kind()) << " undefined behavior";
                break;
        }
    }
};
...
struct SymbolicExpressionTable {
    ...
    void NormalizeForSymbolTable(const SymbolicSymbolTable& symbolTable)
    {
        ...
        ASSERT(FError::EINTERNAL, symTable.size() == symExprTable.size())
    ...
    {
        std::string str = BuildExpressionByRaw(ss.Raw(), {});
        ASSERT(FError::EINTERNAL, primaryExpressionDict_.count(str)) << str << " has not been found in primaryExpressionDict_.";
        auto raw = primaryExpressionDict_.find(str)->second;
        ASSERT(FError::EINTERNAL, primaryExpressionSet.count(raw)) << raw << " has not been found in primaryExpressionSet.";
