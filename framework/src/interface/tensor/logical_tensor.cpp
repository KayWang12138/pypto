      ...
      function_(&function)
{
    ...
    isSubGraphBoundary = false;
    ASSERT(FError::INVALID_VAL, shape.size() == offset.size())
...
{
    ASSERT(FError::EINTERNAL, tensorDump[T_FIELD_KIND].get<int>() == static_cast<int>(Kind::T_KIND_TENSOR))
    ...
    if (tensorDump[T_FIELD_RAWTENSOR].is_number()) {
        ...
        ASSERT(FError::NOT_EXIST, rawTensorDict.count(rawTensorMagic))
...
{
    ASSERT(FError::INVALID_VAL, shape.size() == newShape.size())
        << "Tensor.view, shape must be the same dimension";
    ASSERT(FError::INVALID_VAL, offset.size() == newOffset.size())
    ...
    for (size_t i = 0; i < shape.size(); i++) {
        ASSERT(FError::OUT_OF_RANGE, shape[i] >= (newShape[i] + newOffset[i]))
    ...
    const std::vector<SymbolicScalar>& viewDynOffset, const Shape& viewShape)
{
    ...
    ASSERT(FError::INVALID_VAL, validShape.size() == viewShape.size())
...
{
    ASSERT(FError::INVALID_VAL, src.GetShape().size() == offset.size())
...
{
    ASSERT(FError::INVALID_VAL, src.GetShape() == Shape(src.GetShape().size(), 1))
        << "src.GetShape(): " << src.GetShape()
        << ", Shape(src.GetShape().size(), 1): " << Shape(src.GetShape().size(), 1);
    ASSERT(FError::INVALID_VAL, src.GetShape().size() == dst.GetShape().size())
...
{
    ASSERT(FError::EINTERNAL, currOutcastIdx != newOutcastIdx)
    ...
    while (filledFound) {
        ...
        for (auto [index, callList] : GetTensorDataDict(curr)) {
            ...
            for (auto& call : callList) {
                ...
                ASSERT(FError::EINTERNAL, currIOType->IsImmediate())
                    << "its' kind: " << SymbolicScalarKind2Name(currIOType->kind);
                ASSERT(FError::EINTERNAL, currIOTypeIndex->IsImmediate())
