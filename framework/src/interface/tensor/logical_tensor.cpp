...
LogicalTensor::LogicalTensor(
    Function &function, DataType t, Shape tshape, TileOpFormat format, std::string tname, NodeType tnodetype, bool readyOnAicoreStart)
    : isSubGraphBoundary(false),
      subGraphID(NOT_IN_SUBGRAPH),
      tensor(std::make_shared<RawTensor>(t, tshape, format, std::move(tname), -1, readyOnAicoreStart)),
...
LogicalTensor::LogicalTensor(Function &function, DataType t, Shape tshape, std::vector<SymbolicScalar> tValidShape,
    TileOpFormat format, std::string tname, NodeType tnodetype, bool readyOnAicoreStart)
    : isSubGraphBoundary(false),
      subGraphID(NOT_IN_SUBGRAPH),
      tensor(std::make_shared<RawTensor>(t, tshape, format, std::move(tname), -1, readyOnAicoreStart)),
      offset(Offset(tshape.size(), 0)),
