...
LogicalTensor::LogicalTensor(Function &function, std::shared_ptr<RawTensor> rawTensor, Offset toffset, Shape tshape,
    ...
    isSubGraphBoundary = false;
    ASSERT(ERROR_CODE_UNKNOWN, shape.size() == offset.size()) << "shape.size(): " << shape.size() << ", offset.size(): " << offset.size();
}
...
std::shared_ptr<LogicalTensor> LogicalTensor::LoadJson(Function &function,
    ...
    ASSERT(ERROR_CODE_UNKNOWN, tensorDump[T_FIELD_KIND].get<int>() == static_cast<int>(Kind::T_KIND_TENSOR))
    ...
    if (tensorDump[T_FIELD_RAWTENSOR].is_number()) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, rawTensorDict.count(rawTensorMagic)) << "rawTensorDict doesn't have magic " << rawTensorMagic;
...
std::shared_ptr<LogicalTensor> LogicalTensor::View(
    ...
    ASSERT(ERROR_CODE_UNKNOWN, shape.size() == newShape.size()) << "Tensor.view, shape must be the same dimension";
    ASSERT(ERROR_CODE_UNKNOWN, offset.size() == newOffset.size()) << "Tensor.view, offset must be the same dimension";

    auto view = std::make_shared<LogicalTensor>(
        function, this->tensor, this->offset, this->shape, this->nodetype);
    for (size_t i = 0; i < shape.size(); i++) {
        ASSERT(ERROR_CODE_UNKNOWN, shape[i] >= (newShape[i] + newOffset[i]))
            << "i: " << i << ", shape[i]: " << shape[i]
...
std::vector<SymbolicScalar> npu::tile_fwk::GetViewValidShape(const std::vector<SymbolicScalar> &validShape,
    ...
    ASSERT(ERROR_CODE_UNKNOWN, validShape.size() == viewShape.size())
...
Tensor TensorExtract(const Tensor &src, const std::vector<SymbolicScalar> &offset) {
    ASSERT(ERROR_CODE_UNKNOWN, src.GetShape().size() == offset.size())
...
void TensorInsert(const Tensor &src, const std::vector<SymbolicScalar> &offset, Tensor &dst) {
    ASSERT(ERROR_CODE_UNKNOWN, src.GetShape() == Shape(src.GetShape().size(), 1))
        << "src.GetShape(): " << src.GetShape() << ", Shape(src.GetShape().size(), 1): " << Shape(src.GetShape().size(), 1);
    ASSERT(ERROR_CODE_UNKNOWN, src.GetShape().size() == dst.GetShape().size())
...
SymbolicScalar UpdateGetTensorDataIOIndex(size_t currOutcastIdx, size_t newOutcastIdx, const SymbolicScalar &scalar) {
    ASSERT(ERROR_CODE_UNKNOWN, currOutcastIdx != newOutcastIdx) <<
    ...
    while (filledFound) {
        ...
        for (auto [index, callList] : GetTensorDataDict(curr)) {
            ...
            for (auto &call : callList) {
                ...
                ASSERT(ERROR_CODE_UNKNOWN, currIOType->IsImmediate()) << "its' kind: " << SymbolicScalarKind2Name(currIOType->kind);
                ASSERT(ERROR_CODE_UNKNOWN, currIOTypeIndex->IsImmediate()) << "its' kind: " << SymbolicScalarKind2Name(currIOTypeIndex->kind);
