...
public:
    ...
    [[nodiscard]] bool Match_(const RawPtr& value) const
    {
        ...
        bool opcodeMatch = (op == Opcode) || (Opcode == SymbolicOpcode::T_BOP_MIN && op == SymbolicOpcode::T_MOP_MIN) ||
                           (Opcode == SymbolicOpcode::T_BOP_MAX && op == SymbolicOpcode::T_MOP_MAX);
...
SYM_PATTERN_BINARY_NAMED(sym_min, SymbolicOpcode::T_BOP_MIN)
SYM_PATTERN_BINARY_NAMED(sym_max, SymbolicOpcode::T_BOP_MAX)
...
private:
    ...
    RawPtr VisitExpression(const RawPtr& node)
    {
        ...
        if (opcode == SymbolicOpcode::T_BOP_MIN) {
            return VisitMin(newOps[0], newOps[1]);
        }
        if (opcode == SymbolicOpcode::T_BOP_MAX) {
    ...
    {
        RawPtr ret = Expr::Create(SymbolicOpcode::T_BOP_MIN, {a, b});
        PVarRaw x, y, z;
        PVarImm c1, c2;
    ...
    {
        RawPtr ret = Expr::Create(SymbolicOpcode::T_BOP_MAX, {a, b});
        PVarRaw x, y, z;
        PVarImm c1, c2;
