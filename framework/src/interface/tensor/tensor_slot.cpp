...
void TensorSlotManager::MarkInput(const Tensor &tensor) {
    ...
    FUNCTION_LOGD(
        "MarkInput push input name[%s].", inputName.c_str());
...
void TensorSlotManager::MarkOutput(const Tensor &tensor) {
    ...
    FUNCTION_LOGD(
        "MarkInput push output name[%s].", outputName.c_str());
...
void TensorSlotManager::MarkInplace(const Tensor &out, const Tensor &in) {
    ...
    FUNCTION_LOGD(
        "Slot already inplace [%s, %s].", inSlot.GetSymbolName().c_str(), outSlot.GetSymbolName().c_str());
}
