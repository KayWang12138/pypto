...
DEFINE_NORMALIZER(OP_ADD, builder, values) {
    ...
    for (auto &v : values) {
        ...
        if (v.IsInputValue() || v.IsResultofValue() || v.IsInsertValue() || v.IsExtractValue()) {
            ...
            ASSERT(v.IsOperationValue()) << "Its' valueKind doesn't match OPERATION.";
...
DEFINE_EVALUATOR(OP_VIEW, builder, values) {
    ...
    ASSERT(operation.GetIOperands().size() == 1) << "iOperands.size(): " << operation.GetIOperands().size();
    ASSERT(operation.GetOOperands().size() == 1) << "oOperands.size(): " << operation.GetOOperands().size();
...
DEFINE_EVALUATOR(OP_CONVERT, builder, values) {
    ...
    ASSERT(CheckSameShape(operation.GetIOperands()[0], operation.GetOOperands()[0]))
        << "operation.GetIOperands()[0] or operation.GetOOperands()[0] is nullptr.";
    return values[0];
}

ExpectedValue ExpectedValueBuilder::CreateValue(const Operation &operation, const std::vector<ExpectedValue> &values) {
    ASSERT(operation.GetOpcode() != Opcode::OP_CALL) << "Forbid that Operation's code is OP_CALL";
...
{
    ASSERT(incast->nodetype == NodeType::INCAST) << "incast nodetype: " << NodeType2String(incast->nodetype);
    ASSERT(incast->tensor->GetRawShape() == incast->GetShape())
        << "incast->tensor->GetRawShape():" << incast->tensor->GetRawShape() << ", incast->GetShape(): " << incast->GetShape();
...
std::vector<ExpectedValue> ExpectedValueBuilder::CreateOperationInsertOOperands(const Operation &op, const std::vector<ExpectedValue> &ioperandExpectedValueList, const std::vector<ExpectedValue> &ooperandExpectedValueList)
{
    ASSERT(op.GetOpcode() == Opcode::OP_ASSEMBLE || op.GetOpcode() == Opcode::OP_COPY_OUT)
        << "Opcode != OP_ASSEMBLE && Opcode != OP_COPY_OUT";
    ASSERT(op.GetIOperands().size() == 1) << "iOperands.size(): " << op.GetIOperands().size();
    ASSERT(op.GetOOperands().size() == 1) << "oOperands.size(): " << op.GetOOperands().size();
    ASSERT(ioperandExpectedValueList.size() == 1) << "iOperandExpectedValueList.size(): " << ioperandExpectedValueList.size();
    ASSERT(ooperandExpectedValueList.size() == 1) << "oOperandExpectedValueList.size(): " << ooperandExpectedValueList.size();
    ...
    if (!ooperandExpectedValueList[0].IsNull()) {
        ASSERT(ooperandExpectedValueList[0].IsInsertValue()) << "ooperandExpectedValueList[0].IsInsertValue() = false";
...
std::shared_ptr<CallExpectedValue> ExpectedValueBuilder::CreateCall(Function *func, const std::vector<ExpectedValue> &incastExpectedValueList, const std::string &debugTracePrefix) {
    ...
    ASSERT(incastExpectedValueList.size() == static_cast<size_t>(0) ||
           ...
           << "incastExpectedValueList.size(): " << incastExpectedValueList.size()
           << ", func->GetIncast().size(): " << func->GetIncast().size();
