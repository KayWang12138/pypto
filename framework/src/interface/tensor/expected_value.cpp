...
DEFINE_NORMALIZER(OP_ADD, builder, values) {
    ...
    for (auto &v : values) {
        ...
        if (v.IsInputValue() || v.IsResultofValue() || v.IsInsertValue() || v.IsExtractValue()) {
            ...
            ASSERT(ERROR_CODE_UNKNOWN, v.IsOperationValue());
...
DEFINE_EVALUATOR(OP_VIEW, builder, values) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, operation.GetIOperands().size() == 1);
    ASSERT(ERROR_CODE_UNKNOWN, operation.GetOOperands().size() == 1);
...
DEFINE_EVALUATOR(OP_CONVERT, builder, values) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, operation.GetIOperands().size() == 1);
    ASSERT(ERROR_CODE_UNKNOWN, operation.GetOOperands().size() == 1);
    ASSERT(ERROR_CODE_UNKNOWN, CheckSameShape(operation.GetIOperands()[0], operation.GetOOperands()[0]));
    return values[0];
}

ExpectedValue ExpectedValueBuilder::CreateValue(const Operation &operation, const std::vector<ExpectedValue> &values) {
    ASSERT(ERROR_CODE_UNKNOWN, operation.GetOpcode() != Opcode::OP_CALL);
...
{
    ASSERT(ERROR_CODE_UNKNOWN, incast->nodetype == NodeType::INCAST);
    ASSERT(ERROR_CODE_UNKNOWN, incast->tensor->GetRawShape() == incast->GetShape());
...
{
    ASSERT(ERROR_CODE_UNKNOWN, op.GetOpcode() == Opcode::OP_ASSEMBLE || op.GetOpcode() == Opcode::OP_COPY_OUT);
    ASSERT(ERROR_CODE_UNKNOWN, op.GetIOperands().size() == 1);
    ASSERT(ERROR_CODE_UNKNOWN, op.GetOOperands().size() == 1);
    ASSERT(ERROR_CODE_UNKNOWN, ioperandExpectedValueList.size() == 1);
    ASSERT(ERROR_CODE_UNKNOWN, ooperandExpectedValueList.size() == 1);
    ...
    if (!ooperandExpectedValueList[0].IsNull()) {
        ASSERT(ERROR_CODE_UNKNOWN, ooperandExpectedValueList[0].IsInsertValue());
...
std::shared_ptr<CallExpectedValue> ExpectedValueBuilder::CreateCall(Function *func, const std::vector<ExpectedValue> &incastExpectedValueList, const std::string &debugTracePrefix) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, incastExpectedValueList.size() == static_cast<size_t>(0) ||
