...
DEFINE_NORMALIZER(OP_ADD, builder, values) {
    ...
    for (auto &v : values) {
        ...
        if (v.IsInputValue() || v.IsResultofValue() || v.IsInsertValue() || v.IsExtractValue()) {
            ...
            ASSERT(ERROR_CODE_UNDEFINED, v.IsOperationValue());
...
DEFINE_EVALUATOR(OP_VIEW, builder, values) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, operation.GetIOperands().size() == 1);
    ASSERT(ERROR_CODE_UNDEFINED, operation.GetOOperands().size() == 1);
...
DEFINE_EVALUATOR(OP_CONVERT, builder, values) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, operation.GetIOperands().size() == 1);
    ASSERT(ERROR_CODE_UNDEFINED, operation.GetOOperands().size() == 1);
    ASSERT(ERROR_CODE_UNDEFINED, CheckSameShape(operation.GetIOperands()[0], operation.GetOOperands()[0]));
    return values[0];
}

ExpectedValue ExpectedValueBuilder::CreateValue(const Operation &operation, const std::vector<ExpectedValue> &values) {
    ASSERT(ERROR_CODE_UNDEFINED, operation.GetOpcode() != Opcode::OP_CALL);
...
{
    ASSERT(ERROR_CODE_UNDEFINED, incast->nodetype == NodeType::INCAST);
    ASSERT(ERROR_CODE_UNDEFINED, incast->tensor->GetRawShape() == incast->GetShape());
...
{
    ASSERT(ERROR_CODE_UNDEFINED, op.GetOpcode() == Opcode::OP_ASSEMBLE || op.GetOpcode() == Opcode::OP_COPY_OUT);
    ASSERT(ERROR_CODE_UNDEFINED, op.GetIOperands().size() == 1);
    ASSERT(ERROR_CODE_UNDEFINED, op.GetOOperands().size() == 1);
    ASSERT(ERROR_CODE_UNDEFINED, ioperandExpectedValueList.size() == 1);
    ASSERT(ERROR_CODE_UNDEFINED, ooperandExpectedValueList.size() == 1);
    ...
    if (!ooperandExpectedValueList[0].IsNull()) {
        ASSERT(ERROR_CODE_UNDEFINED, ooperandExpectedValueList[0].IsInsertValue());
...
std::shared_ptr<CallExpectedValue> ExpectedValueBuilder::CreateCall(Function *func, const std::vector<ExpectedValue> &incastExpectedValueList, const std::string &debugTracePrefix) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, incastExpectedValueList.size() == static_cast<size_t>(0) ||
