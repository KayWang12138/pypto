...
Tensor::~Tensor() {
    ...
    ASSERT(TensorErr::STORAGE_NULL, storage_->tensor != nullptr);
...
Tensor::Tensor(std::shared_ptr<LogicalTensor> s)
    ...
    ASSERT(TensorErr::STORAGE_NULL, storage_ != nullptr);
    ASSERT(TensorErr::STORAGE_NULL, storage_->tensor != nullptr);
    Program::GetInstance().InsertAliveTensor(this);
    storage_->tensor->AddRefCount(1);
...
static std::vector<SymbolicScalar> ToDynShape(const std::string &tname, const Shape &shape) {
    ...
    for (size_t dim = 0; dim < shape.size(); dim++) {
        CHECK(TensorErr::INVALID_SHAPE, shape[dim] >= -1) << "Invalid shape " << shape[dim];
        if (shape[dim] == -1) {
...
void AssignTensorData(Tensor &lhs, const Tensor &rhs) {
    if (lhs.GetData() != nullptr) {
        if (rhs.GetData() != nullptr) {
            CHECK(TensorErr::SELF_ASSIGNMENT, lhs.GetData() == rhs.GetData()) << "Prohibit self-assignment.";
        }
    } else {
...
int32_t Tensor::GetShape(int axis) const {
    ...
    ASSERT(TensorErr::NO_DIMENSIONS, dimCount > 0) << "Tensor has no dimensions! disCount: " << dimCount;
    if (axis < 0) {
        axis += static_cast<int>(dimCount);
    }
    ASSERT(TensorErr::AXIS_OUT_OF_RANGE, axis >= 0 && static_cast<size_t>(axis) < dimCount)
...
static SymbolicScalar GetInputDataInt32Dim1(const Tensor &t, SymbolicScalar off0) {
    ...
    ASSERT(TensorErr::NOT_IN_INPUT_LIST,
...
SymbolicScalar GetInputData(const Tensor &t, const std::vector<SymbolicScalar> &offset) {
    ASSERT(TensorErr::DIMS_INCONSISTENT, t.Dim() == offset.size())
...
static SymbolicScalar DoGetTensorDataInt32(
    ...
    ASSERT(TensorErr::DIMS_INCONSISTENT, t.GetShape().size() == offset.size())
    ...
    ASSERT(TensorErr::NOT_UNDER_DYNAMIC_FUNCTION, currDynFunc != nullptr) << "Not under dynamic function!\n";
...
SymbolicScalar GetTensorData(const Tensor &t, const std::vector<SymbolicScalar> &offset) {
    CHECK(TensorErr::DATATYPE_MISMATCH, t.GetDataType() == DT_INT32) << "Tensor dtype must be DT_INT32.";
...
static void DoSetTensorDataInt32(const SymbolicScalar &v, const std::vector<SymbolicScalar> &off, Tensor &t) {
    CHECK(TensorErr::DIMS_INCONSISTENT, t.GetShape().size() == off.size())
...
void SetTensorData(const SymbolicScalar &v, const std::vector<SymbolicScalar> &off, Tensor &dst) {
    CHECK(TensorErr::DATATYPE_MISMATCH, dst.GetDataType() == DT_INT32) << "Tensor dtype must be DT_INT32.";
