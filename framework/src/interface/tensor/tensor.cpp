...
Tensor::~Tensor()
{
    ...
    ASSERT(FError::INVALID_PTR, storage_->tensor != nullptr);
...
{
    ASSERT(FError::INVALID_PTR, storage_ != nullptr);
    ASSERT(FError::INVALID_PTR, storage_->tensor != nullptr);
    Program::GetInstance().InsertAliveTensor(this);
    storage_->tensor->AddRefCount(1);
...
int32_t Tensor::GetShape(int axis) const
{
    ...
    ASSERT(FError::INVALID_VAL, dimCount > 0) << "Tensor has no dimensions! disCount: " << dimCount;
    if (axis < 0) {
        axis += static_cast<int>(dimCount);
    }
    ASSERT(FError::OUT_OF_RANGE, axis >= 0 && static_cast<size_t>(axis) < dimCount)
...
{
    ASSERT(FError::INVALID_VAL, t.Dim() == offset.size())
        << "t.Dim(): " << t.Dim() << "!= offset.size(): " << offset.size();
    ASSERT(FError::INVALID_VAL, t.Dim() > 0 && t.Dim() <= 0x4)
        << "t.Dim(): " << t.Dim() << ", limit: [1, 4]";
    ASSERT(FError::INVALID_VAL, IsInteger(t.GetDataType()))
...
{
    ASSERT(FError::INVALID_VAL, t.GetShape().size() == offset.size())
    ...
    ASSERT(FError::INVALID_OPERATION, currDynFunc != nullptr) << "Not under dynamic function!\n";
