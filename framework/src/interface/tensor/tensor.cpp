...
Tensor::~Tensor() {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, storage_->tensor != nullptr);
...
Tensor::Tensor(std::shared_ptr<LogicalTensor> s) : storage_(std::move(s)), index_(IdGen<IdType::TENSOR_INDEX>::Inst().NewId()) {
    ASSERT(ERROR_CODE_UNDEFINED, storage_ != nullptr);
    ASSERT(ERROR_CODE_UNDEFINED, storage_->tensor != nullptr);
    Program::GetInstance().InsertAliveTensor(this);
    storage_->tensor->AddRefCount(1);
...
static std::vector<SymbolicScalar> ToDynShape(const std::string &tname, const Shape &shape) {
    ...
    for (size_t dim = 0; dim < shape.size(); dim++) {
        CHECK(ERROR_CODE_UNDEFINED, shape[dim] >= -1) << "Invalid shape " << shape[dim];
        if (shape[dim] == -1) {
...
void AssignTensorData(Tensor &lhs, const Tensor &rhs) {
    if (lhs.GetData() != nullptr) {
        if (rhs.GetData() != nullptr) {
            CHECK(ERROR_CODE_UNDEFINED, lhs.GetData() == rhs.GetData()) << "Prohibit self-assignment.";
        }
    } else {
...
int32_t Tensor::GetShape(int axis) const {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, dimCount > 0) << "Tensor has no dimensions! disCount: " << dimCount;
    if (axis < 0) {
        axis += static_cast<int>(dimCount);
    }
    ASSERT(ERROR_CODE_UNDEFINED, axis >= 0 && static_cast<size_t>(axis) < dimCount)
...
SymbolicScalar GetInputDataInt32Dim1(const Tensor &t, SymbolicScalar off0) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, inputIndex >= 0 && static_cast<size_t>(inputIndex) < slotManager->GetInputNameList().size()) <<
...
SymbolicScalar GetInputData(const Tensor &t, const std::vector<SymbolicScalar> &offset) {
    ASSERT(ERROR_CODE_UNDEFINED, t.Dim() == offset.size()) << "t.Dim(): " << t.Dim() << "!= offset.size(): " << offset.size();
    ASSERT(ERROR_CODE_UNDEFINED, t.Dim() >0 && t.Dim() <= 0x4) << "t.Dim(): " << t.Dim() << ", limit: [1, 4]";
    if (t.Dim() == 0x1) {
...
SymbolicScalar DoGetTensorDataInt32(SymbolHandlerId handlerId, const Tensor &t, const std::vector<SymbolicScalar> &offset) {
    ASSERT(ERROR_CODE_UNDEFINED, t.GetShape().size() == offset.size()) << "Mismatch dimension: " << t.GetShape().size() << " vs " << offset.size() << "\n";
    Program::GetInstance().GetTensorSlotManager()->TensorRead(t);

    auto currDynFunc = Program::GetInstance().GetCurrentDynamicFunction();
    ASSERT(ERROR_CODE_UNDEFINED, currDynFunc != nullptr) << "Not under dynamic function!\n";
...
SymbolicScalar GetTensorData(const Tensor &t, const std::vector<SymbolicScalar> &offset) {
    CHECK(ERROR_CODE_UNDEFINED, t.GetDataType() == DT_INT32) << "Tensor dtype must be DT_INT32.";
    ...
    CHECK(ERROR_CODE_UNDEFINED, offset.size() <= MAX_GET_TENSOR_DATA_DIM) << "Offset.size() must be less than " << MAX_GET_TENSOR_DATA_DIM;
...
void DoSetTensorDataInt32(const SymbolicScalar &v, const std::vector<SymbolicScalar> &off, Tensor &t) {
    CHECK(ERROR_CODE_UNDEFINED, t.GetShape().size() == off.size()) << "Mismatch dimen:" << t.GetShape().size() << " vs " << off.size() << "\n";
...
void SetTensorData(const SymbolicScalar &v, const std::vector<SymbolicScalar> &off, Tensor &dst) {
    CHECK(ERROR_CODE_UNDEFINED, dst.GetDataType() == DT_INT32) << "Tensor dtype must be DT_INT32.";
