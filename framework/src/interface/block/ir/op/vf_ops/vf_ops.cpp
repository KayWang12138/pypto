...
TypePtr DeduceVFUnknownType(const std::vector<ExprPtr>& args,
                            const std::vector<std::pair<std::string, std::any>>& kwargs) {
  return GetUnknownType();
}

TypePtr DeduceVFScalarType(const std::vector<ExprPtr>& args,
                           const std::vector<std::pair<std::string, std::any>>& kwargs) {
  return std::make_shared<ScalarType>(DataType::FP32);
}

TypePtr DeduceVFFromFirstArg(const std::vector<ExprPtr>& args,
                             const std::vector<std::pair<std::string, std::any>>& kwargs) {
  if (!args.empty()) {
...
TypePtr DeduceVFFromDstArg(const std::vector<ExprPtr>& args,
                           const std::vector<std::pair<std::string, std::any>>& kwargs) {
  if (args.size() >= 3) {
...
REGISTER_OP("vf.Muls")
    ...
    .f_deduce_type([](const std::vector<ExprPtr>& args,
      ...
      if (args.size() >= 2) return args[1]->GetType();
      return std::make_shared<ScalarType>(DataType::FP32);
    });
