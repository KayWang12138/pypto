...
// If scalar has 'index' type, cast indexâ†’i64 first, then cast i64 to dst element type if needed.
...
static std::string MakeManualLoadCodegenPTO(const CallPtr& op, codegen::CodegenBase& codegen_base) {
  ...
  if (eff.is_dynamic) {
    // Dynamic tile: get partition_view sizes from tileâ†’valid_shape mapping
...
static std::string MakeManualStoreCodegenPTO(const CallPtr& op, codegen::CodegenBase& codegen_base) {
  ...
  // Build optional attributes string
  std::string attrs;
...
// ============================================================================
static std::string MakeManualMoveFpCodegenPTO(const ir::CallPtr& op, codegen::CodegenBase& codegen_base) {
  auto& codegen = dynamic_cast<codegen::PTOCodegen&>(codegen_base);
  CHECK(op->args_.size() == 3) << "manual.move_fp: expected 3 args, got " << op->args_.size();
  ...
  std::string fp_tile_buf_type = codegen.GetExprTypeAnnotation(op->args_[1]);
  ...
  if (!src_type.empty()) oss << " : " << src_type;
  oss << ", " << fp_tile;
  if (!fp_tile_buf_type.empty()) oss << " : " << fp_tile_buf_type;
  oss << ") outs(" << dst;
  if (!dst_type.empty()) oss << " : " << dst_type;
  oss << ")";
  if (!attrs.empty()) oss << " {" << attrs << "}";
...
REGISTER_BACKEND_OP(Backend910B_PTO, "manual.move_fp")
    .set_pipe(ir::PipeType::FIX)
    .f_codegen([](const ir::CallPtr& op, codegen::CodegenBase& codegen) {
      return MakeManualMoveFpCodegenPTO(op, codegen);
    });
...
REGISTER_BACKEND_OP(Backend910B_PTO, "manual.set_validshape")
    ...
    .f_codegen([](const ir::CallPtr& op, codegen::CodegenBase& codegen) {
      ...
      // Update tileâ†’valid_shape mapping so load/store can retrieve it
