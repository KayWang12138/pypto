...
TILEOP __gm__ T* MapVirtualAddr(__gm__ int64_t *hcclContext, __gm__ T* vAddr, uint32_t dstRankId)
{
    ...
    if (memType == 0) {
        return (__gm__ T*)(((__gm__ TileOp::CommContext *)hcclContext[groupIndex])->winAddr[dstRankId] + offset);
    } else {
        return (__gm__ T*)(((__gm__ TileOp::CommContext *)hcclContext[groupIndex])->winAddr[dstRankId + 
                            2 * hcclOpParam->rankNum] + offset);
    }
}
