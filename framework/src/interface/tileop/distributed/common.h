...
{
    return (val >> TileOp::VirtualAddr::TILE_NUM_SHIFT) & ((1UL << TileOp::VirtualAddr::TILE_NUM_BITS) - 1UL);
}
...
TILEOP __gm__ T* MapVirtualAddr(__gm__ int64_t* hcclContext, __gm__ T* vAddr, uint32_t dstRankId)
{
    ...
    uint64_t groupIndex = GetVirtualAddrGroupIndex(addrVal);
    uint64_t offset = GetVirtualAddrOffset(addrVal);
