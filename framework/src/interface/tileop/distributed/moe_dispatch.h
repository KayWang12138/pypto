...
template <typename HcclContextType, typename T, int32_t axisH, int32_t tRowOffset, int32_t tColOffset,
    int32_t tRowShape, int32_t tColShape, int32_t groupIndex>
    ...
    for (int32_t row = tRowOffset; row < tRowOffset + tRowShape; ++row) {
        for (int32_t col = tColOffset; col < tColOffset + tColShape; ++col) {
            ...
            __gm__ T* remoteShmemBaseAddr = 
                    MapVirtualAddr<HcclContextType, T>(hcclContext, shmemDataBaseAddr, static_cast<uint32_t>(remoteRankId));
    ...
    __gm__ int64_t *hcclContext)
{
    ...
    for (int32_t rankId = shmemFlagOffset0; rankId < shmemFlagOffset0 + rankShape; ++rankId) {
        ...
        for (int32_t dstExpertId = shmemFlagOffset1; dstExpertId < shmemFlagOffset1 + expertShape; ++dstExpertId) {
            __gm__ int32_t* remoteFlagBaseAddr =
            MapVirtualAddr<HcclContextType, T>(hcclContext, shmemFlagBaseAddr, rankId);
            int32_t remoteExpertId = dstExpertId + rankId * static_cast<int32_t>(shmemFlagRawShape1);
            __gm__ int32_t* shmemFlagWriteAddr =remoteFlagBaseAddr +
    ...
    __gm__ int64_t *hcclContext, uint32_t processRankSize, __gm__ int32_t *shmemFlagBaseAddr, DispatchInfo &dispatchInfo)
{
    ...
    ClearFlagV2<HcclContextType>(reinterpret_cast<__ubuf__ int32_t *>(src0), offset, cnt, hcclContext, dispatchInfo,
...
template<typename HcclContextType, typename T, uint32_t tileIndex, uint32_t groupIndex, uint32_t shareRankCnt,
    uint32_t totalTileNum, int32_t rankShape, int32_t expertNum>
    ...
    uint32_t flagShmemShape0, uint32_t flagShmemShape1, uint32_t flagShmemShape2, uint32_t flagShmemShape3, __gm__ int64_t *hcclContext)
{
    ...
    MoeRankWaitFlag<HcclContextType, T>(out, src0, src1, dst, hcclContext, moeOpProcessRankSize, shmemFlagBaseAddr,
        dispatchInfo);
}
    ...
    uint32_t rankSize, uint32_t bs, uint32_t shmemLength)
{
    ...
    for (int32_t rankId = dispatchInfo.rankOffset; rankId < dispatchInfo.rankOffset + dispatchInfo.rankShape; rankId++) {
        ...
        ReadFlagV2<HcclContextType, int32_t>(flag, thisRankFlagOffset, 1, hcclContext, shmemFlagBaseAddr,
...
template<typename HcclContextType, typename T, uint32_t tileIndex, uint32_t groupIndex, uint32_t shareRankCnt,
    uint32_t totalTileNum, uint32_t rankShape, uint32_t axisH, uint32_t bs, uint32_t expandXRow>
    ...
    uint32_t shmemDataShape1, uint32_t shmemDataShape2, uint32_t shmemDataShape3, __gm__ int64_t *hcclContext)
{
    ...
    MoeRankCopyOut<HcclContextType, int32_t, T>(combineInfo, nullptr, reinterpret_cast<__ubuf__ uint8_t *>(buffer),
