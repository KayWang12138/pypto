...
template <typename HcclContextType, typename T, int32_t axisH, int32_t tRowOffset, int32_t tColOffset, int32_t tRowShape, int32_t tColShape, int32_t groupIndex>
    ...
    __gm__ int64_t *hcclContext)
{
    ...
    auto localUsrRankId = GetLocalUsrRankId<HcclContextType>(hcclContext, groupIndex);
    ...
    for (int32_t row = tRowOffset; row < tRowOffset + tRowShape; ++row) {
        for (int32_t col = tColOffset; col < tColOffset + tColShape; ++col) {
            ...
            __gm__ T* remoteShmemBaseAddr = MapVirtualAddr<HcclContextType, T>(hcclContext, shmemDataBaseAddr, static_cast<uint32_t>(remoteRankId));
...
template <typename HcclContextType, typename T, int32_t bs, int32_t axisH, int32_t tileRowShape, int32_t groupIndex>
TILEOP void SendToSharedExpert(__gm__ int32_t *syncTensor, __ubuf__ T *tokenBuffer, __gm__ T *token, __gm__ T *shmemDataBaseAddr,
    ...
    __gm__ int64_t *hcclContext)
{
    int32_t localUsrRankId = GetLocalUsrRankId<HcclContextType>(hcclContext, groupIndex);
    int32_t rankSize = GetRankNum<HcclContextType>(hcclContext, groupIndex);
    ...
    for (int32_t row = tokenOffset0; row < tokenOffset0 + tileRowShape; ++row) {
        ...
        GM_ADDR remoteShmemBaseAddr = (GM_ADDR)MapVirtualAddr<HcclContextType, T>(hcclContext, shmemDataBaseAddr, remoteRankId);
...
template <typename HcclContextType, typename T, int32_t bs, int32_t axisH, int32_t tileRowShape>
...
template <typename HcclContextType, typename T, int32_t bs, int32_t topK, int32_t groupIndex, int32_t expertShape, int32_t rankShape>
    ...
    __gm__ int64_t *hcclContext)
{
    ...
    for (int32_t rankId = shmemFlagOffset0; rankId < shmemFlagOffset0 + rankShape; ++rankId) {
        ...
        for (int32_t dstExpertId = shmemFlagOffset1; dstExpertId < shmemFlagOffset1 + expertShape; ++dstExpertId) {
            __gm__ int32_t* remoteFlagBaseAddr = MapVirtualAddr<HcclContextType, T>(hcclContext, shmemFlagBaseAddr, rankId);
...
template <typename HcclContextType, typename T>
    ...
    __gm__ int64_t *hcclContext, uint32_t processRankSize, __gm__ int32_t *shmemFlagBaseAddr, DispatchInfo &dispatchInfo)
{
    ...
    while (flagSum != cnt) {
        ReadFlagV2<HcclContextType, T>(src0, offset, cnt, hcclContext, shmemFlagBaseAddr, dispatchInfo);
    ...
    ClearFlagV2<HcclContextType>(reinterpret_cast<__ubuf__ int32_t *>(src0), offset, cnt, hcclContext, dispatchInfo, shmemFlagBaseAddr); // 暂时放在读完之后就清 flag
}
...
template<typename HcclContextType, typename T, uint32_t tileIndex, uint32_t groupIndex, uint32_t shareRankCnt, uint32_t totalTileNum, int32_t rankShape, int32_t expertNum>
...
{
    auto rankNum = GetRankNum<HcclContextType>(hcclContext, groupIndex);
    DispatchInfo dispatchInfo = {tileIndex, groupIndex, 0, 0, rankShape, 
        ...
        static_cast<int32_t>(rankNum), static_cast<int32_t>(flagShmemOffset1)};
    ...
    MoeRankWaitFlag<HcclContextType, T>(out, src0, src1, dst, hcclContext, moeOpProcessRankSize, shmemFlagBaseAddr, dispatchInfo);
}
...
template<typename HcclContextType, typename T, int32_t expertShape>
    ...
    uint32_t shmemFlagRawShape0, uint32_t shmemFlagRawShape1, uint32_t shmemFlagRawShape2, uint32_t shmemFlagRawShape3, __gm__ int64_t *hcclContext)
{
    ...
    __gm__ int32_t *winFlagBaseAddr = MapVirtualAddr<HcclContextType, int32_t>(hcclContext, shmemFlagBaseAddr, localUsrRankId);
...
{
    uint32_t localUsrRankId = GetLocalUsrRankId<HcclContextType>(hcclContext, dispatchInfo.groupIndex);
    GM_ADDR localShmemBaseAddr = (GM_ADDR)MapVirtualAddr<HcclContextType, T>(hcclContext, shmemDataBaseAddr, localUsrRankId);
    ...
    for (int32_t rankId = dispatchInfo.rankOffset; rankId < dispatchInfo.rankOffset + dispatchInfo.rankShape; rankId++) {
        ...
        ReadFlagV2<HcclContextType, int32_t>(flag, thisRankFlagOffset, 1, hcclContext, shmemFlagBaseAddr, dispatchInfo); // 每次读取一张卡的 flag, 512B, 存到 UB 是 32B
        pipe_barrier(PIPE_ALL);
...
template<typename HcclContextType, typename T1, typename T2>
    ...
    __gm__ int32_t *shmemFlagBaseAddr, uint32_t rankSize, uint32_t bs, uint32_t shmemLength, bool copyOutData)
{
    ...
    if (copyOutData) {
        ...
        MoeRankWinCopyOut<HcclContextType, T2>(expandX + expandXOffset, validCnt, buffer, dispatchInfo, hcclContext, shmemDataBaseAddr,
        ...
        CombineInfoCopyOut<HcclContextType, T2>(combineInfo + infoOffset, buffer, dispatchInfo, hcclContext, shmemDataBaseAddr,
        shmemFlagBaseAddr, rankSize, bs, shmemLength);
    }
}
    ...
    uint32_t shmemDataRawShape2, uint32_t shmemDataRawShape3)
{
    ...
    uint32_t localUsrRankId = GetLocalUsrRankId<HcclContextType>(hcclContext, dispatchInfo.groupIndex);;
    GM_ADDR winTokenAddr = (GM_ADDR)MapVirtualAddr<HcclContextType, T>(hcclContext, shmemDataAddr, localUsrRankId);
...
{
    auto rankNum = GetRankNum<HcclContextType>(hcclContext, dispatchInfo.groupIndex);
    ...
    // 每个共享专家处理的 moe 卡数
    uint32_t processMoeRankCnt = (rankNum - dispatchInfo.shareRankCnt) / dispatchInfo.shareRankCnt;
    ...
    ShareRankWinCopyOut<HcclContextType, T>(expandX + outOffset, buffer, processTokenCnt, recvTokenOffset, dispatchInfo, hcclContext,
        processMoeRankCnt, shmemDataBaseAddr, shmemDataRawShape2, shmemDataRawShape3);
}

template<typename HcclContextType, typename T, uint32_t tileIndex, uint32_t groupIndex, uint32_t shareRankCnt, uint32_t totalTileNum, uint32_t rankShape, uint32_t axisH, uint32_t bs, uint32_t expandXRow>
    ...
    uint32_t shmemDataShape0, uint32_t shmemDataShape1, uint32_t shmemDataShape2, uint32_t shmemDataShape3, __gm__ int64_t *hcclContext)
{
    ...
    MoeRankCopyOut<HcclContextType, T, T>(expandX, reinterpret_cast<__gm__ uint32_t *>(validCnt),
    ...
    uint32_t shmemDataShape1, uint32_t shmemDataShape2, uint32_t shmemDataShape3, __gm__ int64_t *hcclContext)
{
    ...
    MoeRankCopyOut<HcclContextType, int32_t, T>(combineInfo, nullptr, reinterpret_cast<__ubuf__ uint8_t *>(buffer), reinterpret_cast<__gm__ uint32_t *>(gmRecvTokenCnt),
