...
/*!
 * \file moe_distributed_dispatch.h
 * \brief
*/
...
TILEOP void DispatchSendToRoutingExperts(__gm__ int32_t *syncTensor, __ubuf__ T *xBuffer, __ubuf__ int32_t *expertIdsUb,
    __ubuf__ int32_t *expertBuffer, __gm__ T *x, __gm__ T *shmemDataBaseAddr, __gm__ int32_t *expertIds,
    uint32_t expertIdsOffset0, uint32_t expertIdsOffset1, uint32_t expertIdsRawShape0, uint32_t expertIdsRawShape1,
...
{
    int32_t topK = static_cast<int32_t>(expertIdsRawShape1);
    int32_t expertTblSize = static_cast<int32_t>(expertIdsRawShape0) * static_cast<int32_t>(expertIdsRawShape1);
    ...
    const int32_t xQuantAlign32 = AlignUp<int32_t>(hOutSize , 32) / sizeof(int32_t);
    __ubuf__ int32_t *tmpTokenBuffer = reinterpret_cast<__ubuf__ int32_t *>(xBuffer);
    int32_t assistInfoForCombineOffset = 32;
    for (int32_t row = tRowOffset; row < tRowOffset + tRowShape; ++row) {
        copy_gm_to_ubuf(xBuffer, x + row * axisH, 0, 1, hOutSize / 32, 0, 0);
        set_flag(PIPE_MTE2, PIPE_S, EVENT_ID0);
        wait_flag(PIPE_MTE2, PIPE_S, EVENT_ID0);
        for (int32_t col = tColOffset; col < tColOffset + tColShape; ++col) {
            tmpTokenBuffer[xQuantAlign32 + assistInfoForCombineOffset] = static_cast<int32_t>(localUsrRankId);
            tmpTokenBuffer[xQuantAlign32 + (assistInfoForCombineOffset + 1)] = row;
            tmpTokenBuffer[xQuantAlign32 + (assistInfoForCombineOffset + 2)] = col;
            int32_t expertIdsIndex = row * topK + col;
            int32_t remoteExpertId = expertIdsUb[expertIdsIndex];
            ...
            int32_t xOffset = CalcOccurrencesVector(expertIdsUb, remoteExpertId, expertIdsIndex, expertBuffer);
            ...
            __gm__ T* remoteShmemDataAddr = remoteShmemBaseAddr + static_cast<uint64_t>(localUsrRankId *
                    ...
                    static_cast<uint64_t>(xOffset) * static_cast<uint64_t>(shmemDataLength));
            set_flag(PIPE_S, PIPE_MTE3, EVENT_ID0);
            wait_flag(PIPE_S, PIPE_MTE3, EVENT_ID0);
            copy_ubuf_to_gm(remoteShmemDataAddr, xBuffer, 0 , 1, shmemDataLength * sizeof(T) / 32, 0, 0);
...
TILEOP void SendToSharedExpert(__gm__ int32_t *syncTensor, __ubuf__ T *xBuffer, __gm__ T *x, __gm__ T *shmemDataBaseAddr,
    uint32_t xOffset0, uint32_t xOffset1, uint32_t xShape0, uint32_t xShape1,
    uint32_t shmemDataOffset0, uint32_t shmemDataOffset1, uint32_t shmemDataOffset2, uint32_t shmemDataOffset3,
    uint32_t shmemDataRawShape0, uint32_t shmemDataRawShape1, uint32_t shmemDataRawShape2, uint32_t shmemDataRawShape3,
    __gm__ int64_t *hcclContext)
{
    ...
    constexpr int32_t xQuantAlign32 = AlignUp<int32_t>(hOutSize, 32) / sizeof(int32_t);
    ...
    __ubuf__ int32_t *tmpTokenBuffer = reinterpret_cast<__ubuf__ int32_t *>(xBuffer); // 类型转换使用
    for (int32_t row = xOffset0; row < xOffset0 + tileRowShape; ++row) {
        copy_gm_to_ubuf(xBuffer, x + row * axisH, 0, 1, hOutSize / 32, 0, 0);
        set_flag(PIPE_MTE2, PIPE_S, EVENT_ID0);
        wait_flag(PIPE_MTE2, PIPE_S, EVENT_ID0);
        tmpTokenBuffer[xQuantAlign32] = localUsrRankId; // 暂时先不考虑 A3 数组，前端大小可以暂时用 x[1],暂不修改
        tmpTokenBuffer[xQuantAlign32 + 1] = row;
        tmpTokenBuffer[xQuantAlign32 + 2] = 1;
        ...
        set_flag(PIPE_S, PIPE_MTE3, EVENT_ID0);
        wait_flag(PIPE_S, PIPE_MTE3, EVENT_ID0);
        copy_ubuf_to_gm(remoteShmemDataAddr, xBuffer, 0, 1, shmemDataRawShape3 * sizeof(T) / 32, 0, 0);
        set_flag(PIPE_MTE3, PIPE_MTE2, EVENT_ID0);
        wait_flag(PIPE_MTE3, PIPE_MTE2, EVENT_ID0);
    }
}
...
TILEOP void CopyToLocalExpert(__gm__ T *expandX, __gm__ int32_t *syncTensor, __ubuf__ T *xBuffer, __gm__ T *x,
    uint32_t xOffset0, uint32_t xOffset1, uint32_t xShape0, uint32_t xShape1, __gm__ int64_t *hcclContext)
{
    constexpr int32_t hOutSize = axisH * sizeof(T);
    ...
    for (int32_t row = xOffset0; row < xOffset0 + tileRowShape; ++row) {
        ...
        set_flag(PIPE_MTE2, PIPE_MTE3, EVENT_ID0);
        wait_flag(PIPE_MTE2, PIPE_MTE3, EVENT_ID0);
        copy_ubuf_to_gm(expandX + row * axisH, xBuffer, 0, 1, hOutSize / 32, 0, 0);
        set_flag(PIPE_MTE3, PIPE_S, EVENT_ID0);
        wait_flag(PIPE_MTE3, PIPE_S, EVENT_ID0);
    }
}
...
TILEOP void DispatchSetRecvSignals(__gm__ int32_t *syncDummy, __ubuf__ int32_t *statusTensor, __ubuf__ int32_t *expertIdsUb,
...
TILEOP void DispatchWaitRecvFlags(__gm__ T *out, __ubuf__ int32_t *buffer, __gm__ int32_t *dummy, __gm__ int32_t *shmemFlagBaseAddr,
...
TILEOP void DispatchBuildExpertTokenNum(__gm__ int32_t *expertTokenNums, __ubuf__ int32_t *buffer, __gm__ int32_t *gmRecvTokenCnt, __gm__ int32_t *shmemFlagBaseAddr,
    uint32_t shmemFlagOffset0, uint32_t shmemFlagOffset1, uint32_t shmemFlagOffset2, uint32_t shmemFlagOffset3,
    ...
    uint32_t rankSize, uint32_t bs, uint32_t shmemLength)
{
    ...
    uint32_t xCnt = 0; // 本 op 处理的 cnt 总数
    int32_t assistInfoForCombineOffset = 32;
    ...
    for (int32_t rankId = dispatchInfo.rankOffset; rankId < dispatchInfo.rankOffset + dispatchInfo.rankShape; rankId++) {
        ...
        __gm__ int32_t *thisExpertCombineAddr = assistInfoForCombine + xCnt * MOE_COMBINE_INFO_NUM;
...
{
    uint32_t offset = 0;
    __ubuf__ T *x = reinterpret_cast<__ubuf__ T *>(buffer + offset);
    uint32_t xSize = dispatchInfo.colShape * sizeof(T); // 这里需要为真实的 colShape
    offset = offset + xSize;
    ...
    for (int32_t rankId = dispatchInfo.rankOffset; rankId < dispatchInfo.rankOffset + dispatchInfo.rankShape; rankId++) {
        GM_ADDR thisRankExpertAddrBase = localShmemBaseAddr + rankId * dispatchInfo.expertNumPerRank * bs * shmemLength * sizeof(T);
        GM_ADDR thisRankOutAddr = reinterpret_cast<GM_ADDR>(expandX) + xCnt * dispatchInfo.colShape * sizeof(T);
        GM_ADDR thisRankExpertTokenAddr = thisRankExpertAddrBase + dispatchInfo.expertIndex * bs * shmemLength * sizeof(T);
        ...
        for (int i = 0; i < thisRankSendTokenCnt; i++) {
            ...
            thisRankExpertTokenAddr + i * shmemLength * sizeof(T), x, gmToUbParams, ubToGmParams);
        }
    }
}

// 对于 MOE 专家卡，搬出需要确定当前 TileOp 收到了多少个 x，获取偏移
    ...
    uint32_t shmemDataRawShape2, uint32_t shmemDataRawShape3)
{
    ...
    // 每张卡在 win 区都占据完整大小，row，col，共享专家处理从 startIdx 开始的连续 64 个 x（处理 scale + 三元组）
    ...
    uint32_t offset = 0;
    __ubuf__ T *x = reinterpret_cast<__ubuf__ T *>(buffer + offset);
    uint32_t xSize = dispatchInfo.colShape * sizeof(T);
    offset = offset + xSize;
    ...
    gmToUbParams.srcStride = 0; // 前一个尾巴和下一个的开头，gap，这里根据 x 结构需要跳跃 7168+512
    gmToUbParams.dstStride = 0; // 前一个尾巴和下一个开头，gap
 
    DataCopyParams ubToGmParams;
    ubToGmParams.sid = 0;
    ubToGmParams.nBurst = 1; // 搬运次数，
    ubToGmParams.lenBurst = dispatchInfo.colShape * sizeof(T) / 32; // col 就是 x 实际列宽
    ...
    __gm__ int64_t *hcclContext, __gm__ T*shmemDataBaseAddr, uint32_t shmemDataRawShape2, uint32_t shmemDataRawShape3)
{
    ...
    // 理论上不需要切分 TileOp, 但是为了和 moe 专家切分保持一致，按照 x cnt 切分
    ...
    // bs * processMoeRankCnt, bs 典型值 8，从某张卡开始连续的 64 个 x
    uint32_t xCntRecvFromMoeRank = processMoeRankCnt * dispatchInfo.rowShape;
    uint32_t tailProcessTokenCnt = xCntRecvFromMoeRank / vectorCnt; // 尾块处理 x 个数
    uint32_t tileCnt = xCntRecvFromMoeRank % vectorCnt; // 整块个数
    uint32_t tileProcessTokenCnt = tailProcessTokenCnt + 1; // 整块处理 x 个数
 
    uint32_t recvTokenOffset = 0; // 本 TileOp 处理的 x 的偏移
    ...
    uint32_t outOffset = dispatchInfo.rowShape * dispatchInfo.colShape; // 共享专家本身的 8 个 x 放在最开头
...
TILEOP void DispatchAssembleExpandX(__gm__ T *expandX, __gm__ int32_t *recvCounts, __ubuf__ int32_t *buffer,
...
template<typename T, uint32_t tileIndex, uint32_t groupIndex, uint32_t shareRankCnt, uint32_t totalTileNum, uint32_t rankShape, uint32_t axisH, uint32_t bs, uint32_t expandXRow>
TILEOP void DispatchAssembleCombineInfo(__gm__ int32_t *assistInfoForCombine, __ubuf__ int32_t *buffer, __gm__ T *shmemDataBaseAddr, 
