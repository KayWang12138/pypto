    ...
    for (LoopVar index0 = 0; index0 < srcValidShape0; ++index0) {
        for (LoopVar index1 = 0; index1 < srcValidShape1; ++index1) {
            for (LoopVar index2 = 0; index2 < srcValidShape2; ++index2) {
                ...
                __gm__ dstDtype* dstAddr =
                    MapVirtualAddr<dstDtype>(hcclContext, dst.GetAddr(), ownerRank) + dstOffset + dstOffset3d;
...
template <
    typename Type, uint32_t tileRowShape, uint32_t tileColShape, AtomicType atomicType, typename T1, typename T2,
    typename C1, typename C2>
    ...
    __gm__ int64_t* hcclContext)
{
    ...
    uint64_t baseOffset = 
        static_cast<uint64_t>(CalcLinearOffset(srcShape1, srcShape2, totalTileNum, c0, c1, c2, tileIndex) * stride);
