    ...
    CoreFuncParam* param, __ubuf__ int32_t* buffer, T1 src, C1 srcCoordinate, uint32_t ownerRank, __gm__ int64_t* hcclContext)
{
    ...
    uint64_t baseOffset = static_cast<uint64_t>(CalcLinearOffset(srcShape1, srcShape2, totalTileNum, c0, c1, c2, tileIndex) * stride);
            ...
            for (LoopVar index2 = 0; index2 < dstValidShape2; ++index2) {
                ...
                if constexpr (std::is_same_v<NonShmemType, ShmemType>) {
                    ...
                    CopyGmToGmByLaodStore<
                        ...
                        dstStride, atomicType>(dstAddr, buffer, srcAddr, dstValidShape3, dstValidShape4);
                }
            }
        }
    }
