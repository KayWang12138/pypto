...
template <typename T0>
__aicore__ inline constexpr bool IsValidShapeDimensionMX() {
    if constexpr (T0::IsStaticLayout()) {
        constexpr auto shapeSize = Std::tuple_size<typename T0::Shape>::value;
        if constexpr (shapeSize == DIM_2ND || shapeSize == DIM_3RD || shapeSize == DIM_4TH) {
            return true;
        }
        return false;
    }
...
__aicore__ constexpr bool IsConstContinousMX() {
    return IsValidShapeDimensionMX<T0>();
}

template <typename T0, typename T1, typename... Args>
__aicore__ constexpr bool IsConstContinousMX() {
    if constexpr (!IsValidShapeDimensionMX<T0>()) {
        return false;
    }
