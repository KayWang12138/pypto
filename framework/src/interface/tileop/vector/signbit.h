...
TILEOP void SignbitImpl32Bit(DstTile dstTile, SrcTile srcTile, TmpTile tmpTile)
{
    constexpr auto shiftAmount = sizeof(T) * TileOp::BLOCK_NELEM_B32 - 1;
    pto::TSHRS(srcTile, srcTile, static_cast<T>(shiftAmount));
#ifdef __DAV_V220
    pipe_barrier(PIPE_V);
#endif
...
TILEOP void SignbitImpl16Bit(DstTile dstTile, SrcTile srcTile)
{
    constexpr auto shiftAmount = sizeof(T) * TileOp::BLOCK_NELEM_B32 - 1;
    pto::TSHRS(srcTile, srcTile, static_cast<T>(shiftAmount));
#ifdef __DAV_V220
...
template <typename DstTile, typename SrcTile8Bit, typename TempTile>
TILEOP void SignbitImpl8Bit(DstTile dstTile, SrcTile8Bit srcTile8Bit, TempTile tempTile)
{
    constexpr auto shiftAmount = sizeof(int8_t) * TileOp::BLOCK_NELEM_B32 - 1;
    pto::TCVT(tempTile, srcTile8Bit, pto::RoundMode::CAST_NONE);
#ifdef __DAV_V220
    pipe_barrier(PIPE_V);
#endif
...
template <typename T, typename DstTile, typename SrcTile, typename TmpTile, typename SrcTile8Bit, typename TempTile>
TILEOP void SignbitImpl(DstTile dstTile, SrcTile srcTile, TmpTile tmpTile, SrcTile8Bit srcTile8Bit, TempTile tempTile)
{
    if constexpr (std::is_same<T, float>::value || std::is_same<T, int32_t>::value) {
        SignbitImpl32Bit<T, DstTile, SrcTile, TmpTile>(dstTile, srcTile, tmpTile);
    } else if constexpr (std::is_same<T, half>::value || std::is_same<T, bfloat16_t>::value || std::is_same<T, int16_t>::value) {
        SignbitImpl16Bit<T, DstTile, SrcTile>(dstTile, srcTile);
    } else if constexpr (std::is_same<T, int8_t>::value || std::is_same<T, bool>::value) {
        SignbitImpl8Bit<DstTile, SrcTile8Bit, TempTile>(dstTile, srcTile8Bit, tempTile);
    }
    return;
}
...
TILEOP void TSignbit(T0 dst, T1 src, T3 tmp) {
    ...
    SrcTile8Bit srcTile8Bit(srcShape3, srcShape4);
    TempTile tempTile(srcShape3, srcShape4);
    pto::TASSIGN(tmpTile, (uint64_t)(tmp.GetAddr()));
    pto::TASSIGN(tempTile, (uint64_t)(tmp.GetAddr() + srcTileH * tmpTileW * sizeof(half)));
    ...
    for (LoopVar n0Index = 0; n0Index < dstShape0; ++n0Index) {
        for (LoopVar n1Index = 0; n1Index < dstShape1; ++n1Index) {
            for (LoopVar n2Index = 0; n2Index < dstShape2; ++n2Index) {
                ...
                pto::TASSIGN(srcTile8Bit, (uint64_t)(src.GetAddr() + srcOffset * srcTypeSize));
                SignbitImpl<typename T1::Type, DstTile, SrcTile, TmpTile, SrcTile8Bit, TempTile>(dstTile, srcTile, tmpTile, srcTile8Bit, tempTile);
            }
        }
    }
}
