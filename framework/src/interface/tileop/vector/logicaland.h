...
TILEOP void LogicalAndImpl(T &dst, U1 &src1, U2 &src2, L &tmp1, L &tmp2, L &ones, L &zeros, Res &res) {
...
TILEOP void TLogicalAnd(T0 dst, T1 src1, T2 src2, T3 tmp) {
    ...
    auto resTile = PtoTile<ResTileTensor>();
    ...
    constexpr auto tmpOffset = sizeof(DType) * COUNT_MAX;

    auto zeroAddr = GenTmpTileAddr(tmp.GetAddr(), vcmpBitSize);
    zeroTile.Assign((uint64_t)zeroAddr);

    auto oneAddr = GenTmpTileAddr(zeroAddr, COUNT_MAX);
    oneTile.Assign((uint64_t)oneAddr);

    auto tmp1Addr = GenTmpTileAddr(oneAddr, tmpOffset);
    tmp1Tile.Assign((uint64_t)tmp1Addr);

    auto tmp2Addr = GenTmpTileAddr(tmp1Addr, tmpOffset);
            ...
            for (size_t n2Index = 0; n2Index < dstShape2; ++n2Index) {
                for (size_t n3Index = 0; n3Index < dstShape3; ++n3Index) {
                    ...
                    for (int j = 0; j < numLoop; j++) {
                        dstTile.Assign(dst.GetAddr(), dstOffset + j * COUNT_MAX);
                        src1Tile.Assign(src1.GetAddr(), src1Offset + j * COUNT_MAX);
                        src2Tile.Assign(src2.GetAddr(), src2Offset + j * COUNT_MAX);
                    ...
                    if (remainAfterLoop > 0) {
                        if (numLoop > 0) {
                            ...
                            dstTile.Assign(dst.GetAddr(), dstOffset + numLoop * COUNT_MAX);
                            src1Tile.Assign(src1.GetAddr(), src1Offset + numLoop * COUNT_MAX);
                            src2Tile.Assign(src2.GetAddr(), src2Offset + numLoop * COUNT_MAX);
                            zeroTile.Assign((uint64_t)zeroAddr);
                            oneTile.Assign((uint64_t)oneAddr);
                            tmp1Tile.Assign((uint64_t)tmp1Addr);
                            tmp2Tile.Assign((uint64_t)tmp2Addr);
