...
TILEOP void TLogicalAnd(T0 dst, T1 src1, T2 src2, T3 tmp) {
    ...
    using CmpBitsTileTensor = TileTensor<uint8_t, LocalLayout2Dim<1, CMP_TILE>, Hardware::UB>;
    ...
    auto cmp_valid_shape = (w + BITS_PER_BYTE - 1) / BITS_PER_BYTE;
    auto cmp1Tile = PtoTile<CmpBitsTileTensor>(1, cmp_valid_shape);
    auto cmp2Tile = PtoTile<CmpBitsTileTensor>(1, cmp_valid_shape);
                ...
                for (size_t n3Index = 0; n3Index < dstShape3; ++n3Index) {
                    ...
                    if (remainAfterLoop > 0) {
                        if (numLoop > 0) {
                            ...
                            cmp_valid_shape = (w + BITS_PER_BYTE - 1) / BITS_PER_BYTE;
                            cmp1Tile = PtoTile<CmpBitsTileTensor>(1, cmp_valid_shape);
                            cmp2Tile = PtoTile<CmpBitsTileTensor>(1, cmp_valid_shape);
