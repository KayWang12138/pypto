...
TILEOP void CalculateSrcLogicalTile(T &dst, U &src, T &zeros, T &ones, R &res) {
    auto tile = StandardizedSrcTile(dst, src);
...
TILEOP void TLogicalAnd(T0 dst, T1 src1, T2 src2, T3 tmp) {
    ...
    auto has_loop = numLoop > 0;
    auto loopDstTile = has_loop ? DstTile(1, COUNT_MAX) : nullptr;
    auto loopSrc1Tile = has_loop ? Src1Tile(1, COUNT_MAX) : nullptr;
    auto loopSrc2Tile = has_loop ? Src2Tile(1, COUNT_MAX) : nullptr;
    auto loopTmp1Tile = has_loop ? TmpTile(1, COUNT_MAX, tmp1Addr) : nullptr;
    auto loopTmp2Tile = has_loop ? TmpTile(1, COUNT_MAX, tmp2Addr) : nullptr;
    auto loopOneTile = has_loop ? TmpTile(1, COUNT_MAX, oneAddr) : nullptr;
    auto loopZeroTile = has_loop ? TmpTile(1, COUNT_MAX, zeroAddr) : nullptr;
    auto loopCvtTile = has_loop ? CvtTile(1, COUNT_MAX, cvtAddr) : nullptr;

    auto has_remain = remainAfterLoop > 0;
    auto remainDstTile = has_remain ? DstTile(1, remainAfterLoop : nullptr);
    auto remainSrc1Tile = has_remain ? Src1Tile(1, remainAfterLoop) : nullptr;
    auto remainSrc2Tile = has_remain ? Src2Tile(1, remainAfterLoop) : nullptr;
    auto remainOneTile = has_remain ? TmpTile(1, remainAfterLoop, oneAddr) : nullptr;
    auto remainZeroTile = has_remain ? TmpTile(1, remainAfterLoop, zeroAddr) : nullptr;
    auto remainTmp1Tile = has_remain ? TmpTile(1, remainAfterLoop, tmp1Addr) : nullptr;
    auto remainTmp2Tile = has_remain ? TmpTile(1, remainAfterLoop, tmp2Addr) : nullptr;
    auto remainCvtTile = has_remain ? CvtTile(1, remainAfterLoop, cvtAddr) : nullptr;
            ...
            for (size_t n2Index = 0; n2Index < dstShape2; ++n2Index) {
                for (size_t n3Index = 0; n3Index < dstShape3; ++n3Index) {
                    ...
                    if (remainAfterLoop > 0) {
                        remaindDstTile.Assign(dst.GetAddr(), dstOffset + numLoop * COUNT_MAX);
