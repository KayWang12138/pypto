...
template <BinaryOp op, BroadcastOperand operand, typename T0, typename T1, typename T2>
TILEOP void BinaryComputeImpl(T0 dst, T1 src0, T2 src1) {
    if constexpr (op == BinaryOp::ADD) {
        if constexpr (operand == BroadcastOperand::NONE) {
...
template <BroadcastOperand operand = BroadcastOperand::NONE, typename T0, typename T1, typename T2>
