...
TILEOP void BinaryComputeImpl(T0 dst, T1 src0, T2 src1) {
    ...
    if constexpr (op == BinaryOp::EXPANDEXPDIF) {
        auto col_src0 = T1::Cols == -1 ? src0.GetValidCol() : T1::Cols;
        auto col_src1 = T2::Cols == -1 ? src1.GetValidCol() : T2::Cols;
        auto row_src0 = T1::Rows == -1 ? src0.GetValidRow() : T1::Rows;
        auto row_src1 = T2::Rows == -1 ? src1.GetValidRow() : T2::Rows;
        if (col_src0 == col_src1 && row_src0 != row_src1) {
...
TILEOP void TExpandExpDif(T0 dst, T1 src0, T2 src1) {
    BinaryCompute<BinaryOp::EXPANDEXPDIF, operand>(dst, src0, src1);
}
