...
TILEOP void CalcPow(T0 dst, T1 src0, T2 src1, T3 tmp0, T3 tmp1, T3 tmp2, T4 cmp0) {
    ...
    pto::TABS(tmp2, tmp2);
        ...
        pipe_barrier(PIPE_V);
    #endif
    pto::TCMPS(cmp0, tmp2, NUM1, pto::CmpMode::EQ);
    #ifdef __DAV_V220
        pipe_barrier(PIPE_V);
    #endif
    pto::TSEL(tmp0, cmp0, tmp1, tmp0);
    pto::TFMODS(tmp2, src1, NUM1);
    #ifdef __DAV_V220
        pipe_barrier(PIPE_V);
    #endif
    pto::TCMPS(cmp0, tmp2, NUM0, pto::CmpMode::EQ);
    #ifdef __DAV_V220
        pipe_barrier(PIPE_V);
    #endif
...
TILEOP void FloatPow(T0 dst, T1 src0, T2 src1, T3 tmp) {
    ...
    constexpr auto cmpTileW = (tileW + TileOp::BLOCK_SIZE - 1) / TileOp::BLOCK_SIZE * TileOp::BLOCK_SIZE;
    ...
    auto tmpTile2 = TmpTile(shape3, shape4);
    auto tmpTile3 = CmpTile(shape3, shape4);
    for (LoopVar n0Index = 0; n0Index < shape0; ++n0Index) {
        for (LoopVar n1Index = 0; n1Index < shape1; ++n1Index) {
            for (LoopVar n2Index = 0; n2Index < shape2; ++n2Index) {
                auto tileOffsets = TileOffset(n0Index, n1Index, n2Index);
                ...
                pto::TASSIGN(tmpTile3, (uint64_t)(tmp.GetAddr() + tileH * 3 * tileW * sizeof(float)));
                CalcPow(dstTile.Data(), src0Tile.Data(), src1Tile.Data(), tmpTile0, tmpTile1, tmpTile2, tmpTile3);
            }
        }
    }
}
