...
template <BinaryOp op, TileOp::BroadcastOperand WBrcSide, TileOp::PenuBroadcastOperand HBrcSide, typename LastUse,
...
TILEOP void BinaryCompute(T0 dst, T1 src0, T2 src1) {
    ...
    if constexpr (TileOp::IsConstContinous<T0, T1, T2>() == true) {
        ...
        using Src0PtoTile =
            typename std::conditional<(Src0TileInfo::tileW == 1 && WBrcSide == TileOp::BroadcastOperand::LEFT_OPERAND),
                PtoTile<T1, pto::BLayout::ColMajor, true>, PtoTile<T1, pto::BLayout::RowMajor, true>>::type;
        using Src1PtoTile =
            typename std::conditional<(Src1TileInfo::tileW == 1 && WBrcSide == TileOp::BroadcastOperand::RIGHT_OPERAND),
    ...
    for (LoopVar n0Index = 0; n0Index < info.shape0; ++n0Index) {
        for (LoopVar n1Index = 0; n1Index < info.shape1; ++n1Index) {
            for (LoopVar n2Index = 0; n2Index < info.shape2; ++n2Index) {
                ...
                auto src0tileOffsets = TileOffset(Src0TileInfo::tile0 == 1 ? 0 : n0Index,
                    Src0TileInfo::tile1 == 1 ? 0 : n1Index, Src0TileInfo::tile2 == 1 ? 0 : n2Index);
                auto src1tileOffsets = TileOffset(Src1TileInfo::tile0 == 1 ? 0 : n0Index,
                    Src1TileInfo::tile1 == 1 ? 0 : n1Index, Src1TileInfo::tile2 == 1 ? 0 : n2Index);
...
template <typename LastUse = LastUse3Dim<0, 0, 0>, TileOp::BroadcastOperand WBrcSide = TileOp::BroadcastOperand::NONE,
...
template <BinaryOp op, TileOp::BroadcastOperand WBrcSide, TileOp::PenuBroadcastOperand HBrcSide, typename T0,
