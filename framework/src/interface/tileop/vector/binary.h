...
TILEOP void TFmod(T0 dst, T1 src0, T2 src1, T3 tmp) {
        ...
        for (size_t n1Index = 0; n1Index < dstShape1; ++n1Index) {
            for (size_t n2Index = 0; n2Index < dstShape2; ++n2Index) {
                ...
                if constexpr (std::is_same_v<DstType, float>) {
                    ...
                    pto::TASSIGN(castTile, reinterpret_cast<uint64_t>(castBufAddr + dstTileH * dstTileW * 4));
                    ...
                    pipe_barrier(PIPE_V);
                    pto::TCVT(dstTileTmp, dstTileTmp, pto::RoundMode::CAST_TRUNC);
                    pipe_barrier(PIPE_V);
                    pto::TMUL(dstTileTmp, dstTileTmp, src1TileTmp);
                    pipe_barrier(PIPE_V);
