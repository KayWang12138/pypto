...
template <
    typename LastUse = LastUse2Dim<0, 0>, TileOp::BroadcastOperand WBrcSide = TileOp::BroadcastOperand::NONE,
    ...
    for (LoopVar n0Index = 0; n0Index < shape0; ++n0Index) {
        for (LoopVar n1Index = 0; n1Index < shape1; ++n1Index) {
            for (LoopVar n2Index = 0; n2Index < shape2; ++n2Index) {
                auto dsttileOffsets = TileOffset(n0Index, n1Index, n2Index);
                auto src0tileOffsets = TileOffset(
                    SrcTileInfo::tile0 == 1 ? 0 : n0Index,
                    SrcTileInfo::tile1 == 1 ? 0 : n1Index,
                    SrcTileInfo::tile2 == 1 ? 0 : n2Index);
                dstTile.Assign(dst, dsttileOffsets);
                src0Tile.Assign(src0, src0tileOffsets);
                PTO_WITH_LAST_USE(
                    pto::TAXPY(dstTile.Data(), src0Tile.Data(), static_cast<typename T1::Type>(alpha)), n1, n2);
            }
        }
    }
}
