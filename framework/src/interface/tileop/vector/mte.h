...
__aicore__ inline void IndexPutCopyOut(size_t copyShapes[], size_t copyStrides[], size_t valuesStride, uint64_t ubAddr, ADDR *dstAddr) {
    using DstData = pto::GlobalTensor<ADDR, pto::Shape<-1, -1, -1, -1, -1>, pto::Stride<-1, -1, -1, -1, -1>>;
    constexpr size_t MAX_N_BURST = 4095;
    size_t repeat = copyShapes[1] / MAX_N_BURST;
    size_t lastNBurst = copyShapes[1] - repeat * MAX_N_BURST;
    set_flag(PIPE_S, PIPE_MTE3, EVENT_ID7);
    wait_flag(PIPE_S, PIPE_MTE3, EVENT_ID7);
    for (LoopVar i = 0; i < repeat; ++i) {
        ValuesTileDefine valuesData(MAX_N_BURST, copyShapes[2]);
        pto::TASSIGN(valuesData, ubAddr);
        ubAddr += copyShapes[0] * MAX_N_BURST * valuesStride * sizeof(ValuesDtype);
        DstData dstData(dstAddr, pto::Shape(1, 1, copyShapes[0], MAX_N_BURST, copyShapes[2]), pto::Stride(0, 0, copyStrides[0], copyStrides[1], 0));
        dstAddr += MAX_N_BURST * copyStrides[1];
        pto::TSTORE<ValuesTileDefine, DstData, atomicType>(dstData, valuesData);
    }
    if (lastNBurst != 0) {
        ValuesTileDefine valuesData(lastNBurst, copyShapes[2]);
        pto::TASSIGN(valuesData, ubAddr);
        DstData dstData(dstAddr, pto::Shape(1, 1, copyShapes[0], lastNBurst, copyShapes[2]), pto::Stride(0, 0, copyStrides[0], copyStrides[1], 0));
        pto::TSTORE<ValuesTileDefine, DstData, atomicType>(dstData, valuesData);
    }
...
__aicore__ inline void DoIndexPut(size_t indicesShape, size_t dstShapes[], size_t valuesStrides[], size_t valuesShapes[],
    uint64_t valuesAddr, ADDR *dstAddr, IDX indices0, IDX indices1, IDX indices2, IDX indices3) {
    ...
    for (LoopVar i = 0; i < indicesShape; ++i) {
        ...
        IndexPutCopyOut<atomicType, ValuesDtype, ValuesTileDefine>(copyShapes, copyStrides, valuesStrides[2], valuesAddr, dstAddr + dstOffset);
        valuesAddr += valuesStrides[4 - valuesSize] * sizeof(ValuesDtype);
    }
}
...
__aicore__ inline void TIndexPut(DST dst, C coordinate, VAL values, IDX indices0, IDX indices1, IDX indices2, IDX indices3) {
    ...
    set_flag(PIPE_MTE3, PIPE_S, EVENT_ID7);
    wait_flag(PIPE_MTE3, PIPE_S, EVENT_ID7);
    DoIndexPut<atomicType, dstShapeSize, valuesSize, VAL>(indicesShape, dstShapes, valuesStrides, valuesShapes,
