...
template <pto::AtomicType atomicType, size_t dstShapeSize, size_t valuesSize, typename VAL, typename DST, typename IDX>
__aicore__ inline void DoIndexPut(size_t indicesLength, size_t dstShapes[], size_t valuesStride, size_t valuesShapes[],
    uint64_t valuesAddr, DST *dstAddr, IDX indices0, IDX indices1, IDX indices2, IDX indices3) {
    ...
    using DstData = pto::GlobalTensor<DST, pto::Shape<-1, -1, -1, -1, -1>, pto::Stride<-1, -1, -1, -1, -1>>;
    ...
    for (LoopVar i = 0; i < indicesLength; ++i) {
        ...
        DstData dstData(dstAddr + dstOffset, pto::Shape(1, 1, copyShapes[0], copyShapes[1], copyShapes[2]), pto::Stride(0, 0, copyStrides[0], copyStrides[1], 0));
