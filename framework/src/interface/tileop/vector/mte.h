...
__aicore__ inline void IndexPutCopyOut(int nBurst, int lenBurst, size_t valuesStrides[], uint64_t ubAddr, ADDR *dstAddr) {
    ...
    for (size_t i = 0; i < nBurst; ++i) {
        ...
        pto::TASSIGN(valuesData, ubAddr + i * valuesStrides[2] * sizeof(ValuesDtype));
        using DstData = pto::GlobalTensor<ADDR, pto::Shape<-1, -1, -1, -1, -1>, pto::Stride<-1, -1, -1, -1, -1>>;
        DstData dstData(dstAddr + i * lenBurst, pto::Shape(1, 1, 1, 1, lenBurst), pto::Stride(0, 0, 0, 0, 0));
...
__aicore__ inline void DoIndexPut(size_t indicesShape, size_t dstShapes[], size_t valuesStrides[], uint64_t valuesAddr,
    ...
    for (size_t i = 0; i < indicesShape; ++i) {
        ...
        if constexpr (indicesSize >= 1) {
            dstOffset += ((__ubuf__ IndicesDtype *) indices0.GetAddr())[i] * IndexPutGetStride<4 - dstShapes>(dstShapes);
        }
        if constexpr (indicesSize >= 2) {
            dstOffset += ((__ubuf__ IndicesDtype *) indices1.GetAddr())[i] * IndexPutGetStride<5 - dstShapes>(dstShapes);
        }
        if constexpr (indicesSize >= 3) {
            dstOffset += ((__ubuf__ IndicesDtype *) indices2.GetAddr())[i] * IndexPutGetStride<6 - dstShapes>(dstShapes);
        }
        if constexpr (indicesSize >= 4) {
            dstOffset += ((__ubuf__ IndicesDtype *) indices3.GetAddr())[i] * IndexPutGetStride<7 - dstShapes>(dstShapes);
        }
        size_t nBurst = 1;
...
__aicore__ inline void TIndexPut(DST dst, VAL values, IDX indices0, IDX indices1, IDX indices2, IDX indices3) {
    constexpr auto atomicType = static_cast<pto::AtomicType>(accumulate);
