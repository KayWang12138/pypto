...
TILEOP HypotTmpBuffers<T> InitHypotTmpBuffers(TTmp tmpbuf) {
    constexpr uint64_t countNum = 4096 / sizeof(float);
    ...
    buffers.fp32Buf0 = reinterpret_cast<__ubuf__ float*>(currentPtr);
    buffers.buf0 = reinterpret_cast<__ubuf__ T*>(currentPtr);

    currentPtr += countNum * sizeof(float);
    currentPtr = reinterpret_cast<__ubuf__ uint8_t*>(
        (reinterpret_cast<uintptr_t>(currentPtr) + ALIGNMENT - 1) & ~(ALIGNMENT - 1));

    buffers.fp32Buf1 = reinterpret_cast<__ubuf__ float*>(currentPtr);
    buffers.buf1 = reinterpret_cast<__ubuf__ T*>(currentPtr);
...
TILEOP void THypot(TDst dst, T src0, T src1, TTmp tmpbuf) {
    auto buffers = InitHypotTmpBuffers<T>(tmpbuf);
    ...
    using DataTile = pto::Tile<pto::TileType::Vec, typename T::Type, 1, COUNT_MAX, pto::BLayout::RowMajor, -1, -1>;
    using Fp32Tile = pto::Tile<pto::TileType::Vec, float, 1, COUNT_MAX, pto::BLayout::RowMajor, -1, -1>;
        ...
        for (LoopVar n1Index = 0; n1Index < info.shape1; ++n1Index) {
            for (LoopVar n2Index = 0; n2Index < info.shape2; ++n2Index) {
                for (LoopVar n3Index = 0; n3Index < info.shape3; ++n3Index) {
                    ...
                    if constexpr (std::is_same_v<typename T::Type, half>) {
