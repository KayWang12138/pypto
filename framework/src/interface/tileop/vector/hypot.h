...
TILEOP HypotTmpBuffers<T> InitHypotTmpBuffers(TTmp tmpbuf) {
    ...
    uint64_t totalBytes = tmpbuf.GetSize(); 
    uint64_t offset = totalBytes / 2;

    HypotTmpBuffers<T> buffers;
    buffers.fp32Buf0 = reinterpret_cast<__ubuf__ float*>(basePtr);
    buffers.buf0 = reinterpret_cast<__ubuf__ T*>(basePtr);
    __ubuf__ uint8_t* ptrBuf1 = basePtr + offset;

    buffers.fp32Buf1 = reinterpret_cast<__ubuf__ float*>(ptrBuf1);
    buffers.buf1 = reinterpret_cast<__ubuf__ T*>(ptrBuf1);
