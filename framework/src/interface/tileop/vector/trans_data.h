    ...
    DST dst, TYPEC coordinate, TMPDST tmpDstTensor, TMP tmpTensor, INPUT input, int n, int c, int h, int w)
{
    ...
    constexpr auto C1 = Std::tuple_element<DIM_2ND, typename TMPDST::TileShape>::type::value;
    ...
    constexpr int elementSize = tmpDstTensorN * C1 * tmpDstTensorH * tmpDstTensorW * C0;
    constexpr int bufferSize = elementSize * sizeof(typename TMPDST::Type);

    using inputTileData = pto::ConvTile<
        pto::TileType::Vec, typename INPUT::Type, bufferSize, pto::Layout::NCHW, pto::ConvTileShape<N, C, H, W>>;
    ...
    constexpr auto dstStride0 = C1 * H * W * C0;
        ...
        for (LoopVar j = 0; j < inputC1; j++) {
            for (LoopVar k = 0; k < inputH; k++) {
                ...
                GlobalData globalData(
                    DstAddr + gmOffset + DstStride, pto::Shape(1, 1, 1, inputW, C0), pto::Stride(0, 0, 0, 0, 0));
