...
TILEOP void BinaryRowExpandComputeImpl(T0 dst, T1 src0, T2 src1)
{
    ...
    if constexpr (op == BinaryOp::ADD) {
        ...
        if constexpr (PrecisionType == DivAlgorithm::HIGH_PRECISION) {
            PTO_WITH_LAST_USE(pto::TROWEXPANDDIV<pto::DivAlgorithm::HIGH_PRECISION>(dst, src0, src1), n1, n2, n3);
        } else {
            PTO_WITH_LAST_USE(pto::TROWEXPANDDIV<pto::DivAlgorithm::DEFAULT>(dst, src0, src1), n1, n2, n3);
        }
        return;
    } else if constexpr (op == BinaryOp::MAX) {
...
TILEOP void BinaryColExpandComputeImpl(T0 dst, T1 src0, T2 src1)
{
    ...
    if constexpr (op == BinaryOp::ADD) {
        ...
        if constexpr (PrecisionType == DivAlgorithm::HIGH_PRECISION) {
            PTO_WITH_LAST_USE(pto::TCOLEXPANDDIV<pto::DivAlgorithm::HIGH_PRECISION>(dst, src0, src1), n1, n2, n3);
        } else {
            PTO_WITH_LAST_USE(pto::TCOLEXPANDDIV<pto::DivAlgorithm::DEFAULT>(dst, src0, src1), n1, n2, n3);
        }
        return;
    } else if constexpr (op == BinaryOp::MAX) {
