 ...
 * @param offset  零点偏移, 形状 [..., H, 1] (对称量化时传全0)
 *
...
TILEOP void TDequantInt8(T0 dst, T1 src, T2 scale, T3 offset) {
    ...
    constexpr int paddedCol_dst = PtoCeil(dstTileW, static_cast<int>(TILE_ALIGNMENT_BYTES / sizeof(float)));
    ...
    constexpr int paddedCol_src = PtoCeil(srcTileW, static_cast<int>(TILE_ALIGNMENT_BYTES / sizeof(int8_t)));
    ...
    using OffsetTileDefine = pto::Tile<pto::TileType::Vec, OffsetDtype, offsetTileH, offsetTileW,
                                       pto::BLayout::ColMajor, -1, -1>;
    ...
    for (LoopVar n0Index = 0; n0Index < dstShape0; ++n0Index) {
        for (LoopVar n1Index = 0; n1Index < dstShape1; ++n1Index) {
            for (LoopVar n2Index = 0; n2Index < dstShape2; ++n2Index) {
                ...
                pto::TDEQUANT<pto::DequantType::INT8>(dstTile, srcTile, scaleTile, offsetTile);
            }
        }
    }
}
...
TILEOP void TDequantInt16(T0 dst, T1 src, T2 scale, T3 offset) {
    ...
    constexpr auto srcTileH = TileOp::GetTensorTileShapeDim<T1, 3, expectSize>();
    constexpr auto srcTileW = TileOp::GetTensorTileShapeDim<T1, 4, expectSize>();
    constexpr int paddedCol_src = PtoCeil(srcTileW, static_cast<int>(TILE_ALIGNMENT_BYTES / sizeof(int16_t)));
    ...
    for (LoopVar n0Index = 0; n0Index < dstShape0; ++n0Index) {
        for (LoopVar n1Index = 0; n1Index < dstShape1; ++n1Index) {
            for (LoopVar n2Index = 0; n2Index < dstShape2; ++n2Index) {
                ...
                pto::TDEQUANT<pto::DequantType::INT16>(dstTile, srcTile, scaleTile, offsetTile);
            }
        }
    }
}
