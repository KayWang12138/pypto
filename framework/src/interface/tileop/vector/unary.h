...
template<typename T, typename IntB16Type, typename HalfTileDefineSrc, typename FloatTileDefineSrc, typename TileDefineDst, typename B16TileDefineSrc>
TILEOP void IsFiniteComputeImpl(TileDefineDst dst, B16TileDefineSrc src, BufferTileDefine buffer) {
    ...
    if constexpr (std::is_same_v<T, float>) {
        // FP32
        FloatTileDefineSrc bufferFP32(src.GetValidRow(), src.GetValidCol() / (sizeof(float) / sizeof(IntB16Type)));
        pto::TASSIGN(bufferFP32, reinterpret_cast<std::uintptr_t>(buffer.data()));

        IntB16Type mask = 0x7F80;                         // 0111 1111 1000 0000
        IntB16Type FLOAT_SHL_NUM = 0x4000;                         // 0100 0000 0000 0000
        ...
        pto::TMAXS(bufferB16, bufferB16, (IntB16Type) -1);
        pto::TMULS(bufferB16, bufferB16, (IntB16Type) -1);

        // srcStride
        pto::TMULS(bufferB16, bufferB16, FLOAT_SHL_NUM);
        pto::TCVT(bufferFP16, bufferFP32, pto::RoundMode::CAST_NONE);
        pto::TMINS(bufferB16, bufferB16, (IntB16Type) 1);
...
TILEOP void TIsFinite(DstTileTensor dst, SrcTileTensor src, BufferTileTensor buffer) {
    ...
    if constexpr (TileOp::IsConstContinous<DstTileTensor, SrcTileTensor>() == true) {
        ...
        using FloatTileDefineSrc = pto::Tile<pto::TileType::Vec, float, tileH, tileW, pto::BLayout::RowMajor, validH, validW>;
        ...
        IsFiniteComputeImpl<typename SrcTileTensor::Type, IntB16Type, HalfTileDefineSrc, FloatTileDefineSrc>(dstTile, srcTile, bufferTile);
        return;
    }
    ...
    using FloatTileDefineSrc = pto::Tile<pto::TileType::Vec, float, tileH, tileW, pto::BLayout::RowMajor, -1, -1>;

    HalfTileDefineSrc bufferTile(validH, validW);
    pto::ASSIGN(bufferTile, buffer.GetAddr());
