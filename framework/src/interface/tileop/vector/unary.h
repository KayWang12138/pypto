...
TILEOP void UnaryComputeImpl(T0 dst, T1 src)
{
    ...
    if constexpr (op == UnaryOp::EXP) {
        // 检查 PrecisionType 是否是 ExpAlgorithm 类型或 ExpAlgorithm 枚举值
        if constexpr (std::is_same_v<PrecisionType, pto::ExpAlgorithm> || std::is_enum_v<PrecisionType>) {
    ...
    if constexpr (op == UnaryOp::SQRT) {
        // 检查 PrecisionType 是否是 SqrtAlgorithm 类型或 SqrtAlgorithm 枚举值
        if constexpr (std::is_same_v<PrecisionType, pto::SqrtAlgorithm> || std::is_enum_v<PrecisionType>) {
    ...
    if constexpr (op == UnaryOp::RECIPROCAL) {
        // 检查 PrecisionType 是否是 RecipAlgorithm 类型或 RecipAlgorithm 枚举值
        if constexpr (std::is_same_v<PrecisionType, pto::RecipAlgorithm> || std::is_enum_v<PrecisionType>) {
            PTO_WITH_LAST_USE(pto::TRECIP<PrecisionType>(dst, src), n1, n2);
        } else {
            PTO_WITH_LAST_USE(pto::TRECIP<pto::RecipAlgorithm::DEFAULT>(dst, src), n1, n2);
        }
        return;
    }
    ...
    if constexpr (op == UnaryOp::LN) {
        // 检查 PrecisionType 是否是 LogAlgorithm 类型或 LogAlgorithm 枚举值
        if constexpr (std::is_same_v<PrecisionType, pto::LogAlgorithm> || std::is_enum_v<PrecisionType>) {
