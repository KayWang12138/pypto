...
template <UnaryOp op, typename PrecisionType = void, typename LastUse, typename T0, typename T1>
TILEOP void UnaryComputeImpl(T0 dst, T1 src)
{
    constexpr auto n1 = Std::tuple_element<DIM_1ST, LastUse>::type::value;
    constexpr auto n2 = Std::tuple_element<DIM_2ND, LastUse>::type::value;
    if constexpr (op == UnaryOp::EXP) {
        if constexpr (std::is_same_v<PrecisionType, pto::ExpAlgorithm>) {
            PTO_WITH_LAST_USE(pto::TEXP<PrecisionType>(dst, src), n1, n2);
        } else {
            PTO_WITH_LAST_USE(pto::TEXP<pto::ExpAlgorithm::DEFAULT>(dst, src), n1, n2);
        }
        return;
    }
    ...
    if constexpr (op == UnaryOp::SQRT) {
        if constexpr (std::is_same_v<PrecisionType, pto::SqrtAlgorithm>) {
            PTO_WITH_LAST_USE(pto::TSQRT<PrecisionType>(dst, src), n1, n2);
        } else {
            PTO_WITH_LAST_USE(pto::TSQRT<pto::SqrtAlgorithm::DEFAULT>(dst, src), n1, n2);
        }
        return;
    }
    ...
    if constexpr (op == UnaryOp::RECIPROCAL) {
        if constexpr (std::is_same_v<PrecisionType, pto::ReciprocalAlgorithm>) {
            PTO_WITH_LAST_USE(pto::TRECIPROCAL<PrecisionType>(dst, src), n1, n2);
        } else {
            PTO_WITH_LAST_USE(pto::TRECIPROCAL<pto::ReciprocalAlgorithm::DEFAULT>(dst, src), n1, n2);
        }
        return;
    }
    ...
    if constexpr (op == UnaryOp::LN) {
        if constexpr (std::is_same_v<PrecisionType, pto::LogAlgorithm>) {
            PTO_WITH_LAST_USE(pto::TLOG<PrecisionType>(dst, src), n1, n2);
        } else {
            PTO_WITH_LAST_USE(pto::TLOG<pto::LogAlgorithm::DEFAULT>(dst, src), n1, n2);
        }
        return;
    }
}
    ...
    for (LoopVar n0Index = 0; n0Index < shape0; ++n0Index) {
        for (LoopVar n1Index = 0; n1Index < shape1; ++n1Index) {
            for (LoopVar n2Index = 0; n2Index < shape2; ++n2Index) {
                ...
                UnaryComputeImpl<op, PrecisionType, LastUse)>(
                    dstTile.Data(), srcTile.Data());
            }
        }
    }
}
    ...
    for (LoopVar n0Index = 0; n0Index < shape0; ++n0Index) {
        for (LoopVar n1Index = 0; n1Index < shape1; ++n1Index) {
            for (LoopVar n2Index = 0; n2Index < shape2; ++n2Index) {
                ...
                UnaryComputeImpl<UnaryOp::BRCB, void, LastUse>(dstTile, srcTile);
            }
        }
    }
}
...
#define OP_TILE_OP_EXP TExp
template <typename PrecisionType = void, typename LastUse = LastUse2Dim<0, 0>, typename T0, typename T1>
...
{
    UnaryCompute<UnaryOp::RSQRT, void, LastUse>(dst, src);
}
...
{
    UnaryCompute<UnaryOp::ABS, void, LastUse>(dst, src);
}
...
{
    UnaryCompute<UnaryOp::BITWISENOT, void, LastUse>(dst, src);
}
...
{
    UnaryCompute<UnaryOp::LN, PrecisionType, LastUse>(dst, src);
}
...
template <typename PrecisionType = pto::ReciprocalAlgorithm::DEFAULT, typename LastUse = LastUse2Dim<0, 0>, typename T0, typename T1>
TILEOP void TReciprocal(T0 dst, T1 src)
{
    UnaryCompute<UnaryOp::RECIPROCAL, PrecisionType, LastUse, T0, T1>(dst, src);
}

template <typename LastUse = LastUse2Dim<0, 0>, typename T0, typename T1>
TILEOP void TReciprocal(T0 dst, T1 src)
{
    UnaryCompute<UnaryOp::RECIPROCAL, pto::ReciprocalAlgorithm::DEFAULT, LastUse, T0, T1>(dst, src);
}
...
{
    UnaryCompute<UnaryOp::RELU, void, LastUse>(dst, src);
}
#endif
