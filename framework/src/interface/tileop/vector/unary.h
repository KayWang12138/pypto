...
TILEOP void TIsFinite(DstTileTensor dst, SrcTileTensor src, BufferTileTensor buffer) {
    ...
    if constexpr (TileOp::IsConstContinous<DstTileTensor, SrcTileTensor>() == true) {
        // ** we must use `DstTileTensor` to get tileH and tileW, because its bit width is smaller. **
        constexpr size_t tileH = GetMergedAxisIfNeed<DstTileTensor, true>();
        constexpr size_t tileW = TileOp::GetTensorTileShapeDim<DstTileTensor, DIM_5TH, MAX_DIMS>();
        constexpr int validH = GetValidHeight<DstTileTensor, true>();
        constexpr int validW = GetValidWidth<DstTileTensor>();
        ...
        using B16TileDefineSrc = pto::Tile<pto::TileType::Vec, int16_t, tileH, tileW, pto::BLayout::RowMajor, validH, validW>;
    ...
    // ** we must use `DstTileTensor` to get tileH and tileW, because its bit width is smaller. **
    constexpr size_t tileH = GetMergedAxisIfNeed<DstTileTensor, false>();
    constexpr size_t tileW = TileOp::GetTensorTileShapeDim<DstTileTensor, DIM_5TH, MAX_DIMS>();
    ...
    using B16TileDefineSrc = pto::Tile<pto::TileType::Vec, int16_t, tileH, tileW, pto::BLayout::RowMajor, -1, -1>;
