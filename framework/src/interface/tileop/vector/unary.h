...
template<typename T, typename IntB16Type, typename TileDefineDst, typename B16TileDefineSrc>
...
TILEOP void TIsFinite(DstTileTensor dst, SrcTileTensor src) {
    ...
    if constexpr (TileOp::IsConstContinous<DstTileTensor, SrcTileTensor>() == true) {
        ...
        using TileDefineDst = pto::Tile<pto::TileType::Vec, typename DstTileTensor::Type, tileH, tileW, pto::BLayout::RowMajor, validH, validW>;
        ...
        pto::ASSIGN(dstTile, 0);
        pto::ASSIGN(srcTile, 0);
        IsFiniteComputeImpl<typename SrcTileTensor::Type, IntB16Type>(dstTile, srcTile);
        return;
    }
    ...
    constexpr int validH = GetValidHeight<SrcTileTensor, false>();
    constexpr int validW = GetValidWidth<SrcTileTensor>();
    using TileDefineDst = pto::Tile<pto::TileType::Vec, typename DstTileTensor::Type, tileH, tileW, pto::BLayout::RowMajor, -1, -1>;
    ...
    for (LoopVar n0Index = 0; n0Index < shape0; ++n0Index) {
        for (LoopVar n1Index = 0; n1Index < shape1; ++n1Index) {
            for (LoopVar n2Index = 0; n2Index < shape2; ++n2Index) {
                auto tileOffsets = TileOffset(n0Index, n1Index, n2Index);
                pto::TASSIGN(dstTile, GenTileOffset(dst, tileOffsets) * sizeof(typename DstTileTensor::Type));
                pto::TASSIGN(srcTile, GenTileOffset(src, tileOffsets) * sizeof(typename SrcTileTensor::Type));
                IsFiniteComputeImpl<typename SrcTileTensor::Type, IntB16Type>(dstTile, srcTile);
            }
        }
    }
