...
template <float powDecimals, typename T0, typename T1>
TILEOP void RoundComputeImpl(T0 dst, T1 src) {
...
template <float powDecimals, typename T0, typename T1>
TILEOP void TRound(T0 dst, T1 src) {
    ...
    if constexpr (TileOp::IsConstContinous<T0, T1>() == true) {
        ...
        pto::TASSIGN(dstTile, (uint64_t)dst.GetAddr());
        pto::TASSIGN(srcTile, (uint64_t)src.GetAddr());
        RoundComputeImpl<powDecimals>(dstTile, srcTile);
        return;
    }
