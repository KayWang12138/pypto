...
TILEOP void IsFiniteComputeImpl(TileDefineDst dst, B16TileDefineSrc src) {
    if constexpr (!(std::is_same_v<T, float> || std::is_same_v<T, bfloat16_t> || std::is_same_v<T, bfloat16_t>)) {
        pto::TEXPANDS(dst, (TileDefineDst::DType) 1);
        return;
    }
...
TILEOP void TIsFinite(DstTileTensor dst, SrcTileTensor src) {
    ...
    for (LoopVar n0Index = 0; n0Index < shape0; ++n0Index) {
        for (LoopVar n1Index = 0; n1Index < shape1; ++n1Index) {
            for (LoopVar n2Index = 0; n2Index < shape2; ++n2Index) {
                auto tileOffsets = TileOffset(n0Index, n1Index, n2Index);
                pto::ASSIGN(dstTile, GenTileOffset(dstTile, tileOffsets) * sizeof(typename DstTileTensor::Type));
                pto::ASSIGN(srcTile, GenTileOffset(srcTile, tileOffsets) * sizeof(typename B16TileDefineSrc::Type));
                IsFiniteComputeImpl<typename SrcTileTensor::Type, IntB16Type>(dstTile, srcTile);
            }
        }
    }
