...
template<typename T, typename IntB16Type, typename HalfTileDefineSrc, typename TileDefineDst, typename B16TileDefineSrc>
TILEOP void IsFiniteComputeImpl(TileDefineDst dst, B16TileDefineSrc src) {
    ...
    if constexpr (std::is_same_v<T, float>) {
        ...
        pto::TCMPS(cmpMask, srcFP16, *reinterpret_cast<half*>(&mask), pto::CmpMode::EQ);
        // SELS need dst/src0/src1 to be same type, and it support 16-bit operation => IntB16Type
        pto::TSELS(buffer, srcFP16, cmpMask, (half) 0, 1);  // selectmode == 1 means we can deliver a scalar to calculate?
        pto::TCVT(dst, buffer, pto::RoundMode::CAST_NONE);
