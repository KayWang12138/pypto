...
template <typename T0, typename T1>
TILEOP void RoundComputeImpl(T0 dst, T1 src, float powDecimals) {
    ...
    pto::MULS(srcfloatTile, srcfloatTile, powDecimals);
#ifdef __DAV_V220
    pipe_barrier(PIPE_V);
#endif
    ...
    if constexpr (IsSameType<T1, float>::value) {
        pto::DIVS(dst, srcfloatTile, powDecimals);
    } else {
        pto::DIVS(srcfloatTile, srcfloatTile, powDecimals);
#ifdef __DAV_V220
        pipe_barrier(PIPE_V);
#endif
...
template <typename T0, typename T1>
TILEOP void TRound(T0 dst, T1 src, int decimals = 0) {
    float powDecimals = pow(10.0f, static_cast<float>(decimals));
    if constexpr (decimals > 38) {
    ...
    if constexpr (TileOp::IsConstContinous<T0, T1>() == true) {
        ...
        pto::TASSIGN(dstTile, (uint64_t)dst.GetAddr());
        pto::TASSIGN(srcTile, (uint64_t)src.GetAddr());
        RoundComputeImpl(dstTile, srcTile, powDecimals);
        return;
    }
