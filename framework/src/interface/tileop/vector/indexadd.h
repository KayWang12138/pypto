...
TILEOP void IndexAddUBNotLastAxisCompute(dstTileDefine dstTile, tmpTileDefine tmpTile, src1TileDefine src1Tile,
    ...
    if constexpr (Std::is_same_v<Scalar, bfloat16_t>) {
        ...
        if (Std::is_same_v<typename T3::Type, int32_t> || abs(static_cast<float>(alpha) - 1) > TileOp::EPSILON) {
            ...
            pipe_barrier(PIPE_V);
#endif
            pto::TCVT(tmpTile, dstTile, pto::RoundMode::CAST_RINT); // fp32->bf16
#ifdef __DAV_V220
            pipe_barrier(PIPE_V);
#endif
            pto::TCVT(dstTile, tmpTile, pto::RoundMode::CAST_NONE); // bf16->fp32
        }
    } else {
        set_flag(PIPE_S, PIPE_V, EVENT_ID7);
        wait_flag(PIPE_S, PIPE_V, EVENT_ID7);
...
TILEOP void TIndexAddUB(T0 dst, T1 src0, T2 src1, T3 src2, T4 tmpTensor, Scalar alpha) { // T0: tileTensor
    ...
    auto dstAddr = (__ubuf__ typename T0::Type *)((uint64_t)(dst.GetAddr()));
    auto tmpAddr = (__ubuf__ typename T4::Type *)((uint64_t)(tmpTensor.GetAddr()));
    auto src1Addr = (__ubuf__ typename T2::Type *)((uint64_t)(src1.GetAddr()));
    auto idxAddr = (__ubuf__ typename T3::Type *)((uint64_t)(src2.GetAddr()));
    ...
    if constexpr (axis == 4) { // 尾轴
        ...
        using tmpTileDefine = pto::Tile<pto::TileType::Vec, typename T4::Type, 1, tmpTileW, pto::BLayout::RowMajor>;
...
TILEOP void IndexAddNotLastAxisCompute(dstTileDefine dstGlobal, tmpTileDefine tmpTile, src1TileDefine src1Tile,
    ...
    pto::TSTORE<src1TileDefine, dstTileDefine, AtomicType::AtomicAdd>(dstGlobal, src1Tile);
}
...
TILEOP void IndexAddLastAxisCompute(T0 dst, T2 src1, T3 src2, Scalar alpha, size_t src1Shape0, size_t src1Shape1,
    size_t src1Shape2, size_t src1Shape3, size_t src1Shape4, size_t dstStride0, size_t dstStride1, size_t dstStride2,
    size_t dstStride3, size_t src1Stride0, size_t src1Stride1, size_t src1Stride2, size_t src1Stride3) {
    auto dstAddr = (__gm__ typename T0::Type *)((uint64_t)(dst.GetAddr()));
    auto src1Addr = (__ubuf__ typename T2::Type *)((uint64_t)(src1.GetAddr()));
    auto idxAddr = (__ubuf__ typename T3::Type *)((uint64_t)(src2.GetAddr()));
    set_flag(PIPE_V, PIPE_S, EVENT_ID7);
    wait_flag(PIPE_V, PIPE_S, EVENT_ID7);
                ...
                for (LoopVar l = 0; l < src1Shape3; ++l) {
                    for (LoopVar idx = 0; idx < src1Shape4; ++idx) {
                        ...
                        if constexpr (Std::is_same_v<Scalar, half>) {
                            if (abs(static_cast<float>(alpha) - 1) < TileOp::EPSILON &&
                                Std::is_same_v<typename T3::Type, int64_t>) {
                                dstAddr[dstOffset] += src1Addr[src1Offset]; // 不需要转换
                            } else {
                                float addResult =
                                    static_cast<float>(dstAddr[dstOffset]) + static_cast<float>(src1Addr[src1Offset]);
                                dstAddr[dstOffset] = static_cast<half>(addResult);
...
TILEOP size_t GetDstOffset(vector<size_t> dstStrides, vector<size_t> idx, __ubuf__ typename T3::Type *idxAddr) {
    size_t dstOffset = 0;
    if constexpr (axis == 0) {
...
axis是泛化成5维后的值，实际值为 axis + shapeSize - 5
*/
template <int axis, typename T0, typename T1, typename T2, typename T3, typename T4, typename C, Scalar>
