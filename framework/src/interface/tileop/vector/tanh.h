...
TILEOP void TanhImpl(DstTile dstTile, SrcTile srcTile, TmpTile tmpTile)
{
    constexpr auto n1 = Std::tuple_element<DIM_1ST, LastUse>::type::value;
    constexpr auto n2 = Std::tuple_element<DIM_2ND, LastUse>::type::value;
    pto::TMULS(tmpTile, srcTile, static_cast<T>(2.0f));
#ifdef __DAV_V220
    pipe_barrier(PIPE_V);
#endif
    pto::TEXP(tmpTile, tmpTile);
#ifdef __DAV_V220
    pipe_barrier(PIPE_V);
#endif
    pto::TADDS(dstTile, tmpTile, static_cast<T>(1.0f));
#ifdef __DAV_V220
    pipe_barrier(PIPE_V);
#endif
    pto::TSUBS(tmpTile, tmpTile, static_cast<T>(1.0f));
#ifdef __DAV_V220
    pipe_barrier(PIPE_V);
#endif
...
TILEOP void Ttanh(T0 dst, T1 src, T3 tmp) {
    ...
    using TmpTile =
        pto::Tile<pto::TileType::Vec, typename T1::Type, srcTileH, tmpTileW, pto::BLayout::RowMajor, -1, -1>;
    ...
    for (LoopVar n0Index = 0; n0Index < dstShape0; ++n0Index) {
        for (LoopVar n1Index = 0; n1Index < dstShape1; ++n1Index) {
            for (LoopVar n2Index = 0; n2Index < dstShape2; ++n2Index) {
                ...
                TanhImpl<LastUse, typename T0::Type, DstTile, SrcTile, TmpTile>(dstTile, srcTile, tmpTile);
            }
        }
    }
}
