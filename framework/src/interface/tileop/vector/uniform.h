...
TILEOP void TUniform(TDst dst, TTmp tmpbuf, uint64_t key, uint64_t counter0, uint64_t counter1, uint16_t rounds) {
    ...
    if constexpr (isFloat) {
        ...
        constexpr int64_t uint32BufferLowBytes = ((Size * sizeof(uint32_t) + ALIGN_SIZE - 1) / ALIGN_SIZE) * ALIGN_SIZE;
        __ubuf__ uint16_t* uint16Buffer = reinterpret_cast<__ubuf__ uint16_t*>(tmpbufAddr + uint32BufferBytes + uint32BufferLowBytes);
