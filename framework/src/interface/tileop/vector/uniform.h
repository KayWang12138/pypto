...
TILEOP void TUniform(TDst dst, TTmp tmpbuf, uint64_t key, uint64_t counter0, uint64_t counter1, uint16_t rounds) {
    ...
    __ubuf__ uint16_t* uint16Buffer = reinterpret_cast<__ubuf__ uint16_t*>(tmpbufAddr + Size * sizeof(uint32_t));
    __ubuf__ float* floatBuffer = reinterpret_cast<__ubuf__ float*>(tmpbufAddr + Size * sizeof(uint32_t) + Size * sizeof(uint16_t));
    ...
    if constexpr (isFloat) {
        ...
        pto::TCVT(uint16Tile, uint32Tile, pto::RoundMode::CAST_NONE);
        
        pto::TASSIGN(dstUint16Tile, (uint64_t)uint16Buffer);
        ...
        using TileUint16Dst = pto::Tile<pto::TileType::Vec, uint16_t, 1, tileW, pto::BLayout::RowMajor, -1, -1>;
        TileUint16Dst uint16DstTile(1, Size);
        pto::TASSIGN(uint16DstTile, (uint64_t)uint16Buffer);
        
        if constexpr (isHalf) {
            pto::TSUBS(dstTile, uint16DstTile, static_cast<half>(1.0));
        } else {
            pto::TSUBS(dstTile, uint16DstTile, static_cast<bfloat16_t>(1.0));
        }
    }
}
