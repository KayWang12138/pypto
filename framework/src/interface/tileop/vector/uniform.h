...
TILEOP void TUniform(__ubuf__ T* dst, uint64_t key, uint64_t counter0, uint64_t counter1, uint16_t rounds, int64_t tileOffset) {
    ...
    pto::TUniformKey uniformKey = {static_cast<uint32_t>(key & 0xFFFFFFFF),
                                   static_cast<uint32_t>(key >> 32)};
    pto::TUniformCounter uniformCounter = {static_cast<uint32_t>(tileCounter[0] & 0xFFFFFFFF),
                                           static_cast<uint32_t>(tileCounter[0] >> 32),
                                           static_cast<uint32_t>(tileCounter[1] & 0xFFFFFFFF),
                                           static_cast<uint32_t>(tileCounter[1] >> 32)};

    if (rounds == 7) {
        pto::TUniform<7, TileDst>(dstTile.data(), uniformKey, uniformCounter, 1, Size);
    } else {
        pto::TUniform<10, TileDst>(dstTile.data(), uniformKey, uniformCounter, 1, Size);
    }
}
