...
template <typename LastUse, typename DstTile, typename SrcTile, typename TmpTile>
TILEOP void TanImpl(DstTile dstTile, SrcTile srcTile, TmpTile tmpTile)
{
    ...
    pipe_barrier(PIPE_V);
#endif
    pto::TROUND(tmpTile, tmpTile, pto::RoundMode::CAST_RINT);
#ifdef __DAV_V220
    pipe_barrier(PIPE_V);
#endif
...
TILEOP void TanHalfImpl(DstTile dstTile, SrcTile srcTile, TmpTile tmpTile, TmpFp32Tile tmpFp32Tile)
{
    ...
    pipe_barrier(PIPE_V);
#endif
    TanImpl<LastUse, TmpFp32Tile, TmpFp32Tile, TmpFp32Tile>(tmpFp32Tile, tmpFp32Tile, tmpFp32Tile);
#ifdef __DAV_V220
    pipe_barrier(PIPE_V);
#endif
        ...
        for (LoopVar n1Index = 0; n1Index < dstShape1; ++n1Index) {
            for (LoopVar n2Index = 0; n2Index < dstShape2; ++n2Index) {
                ...
                if constexpr (std::is_same<typename T1::Type, float>::value) {
                    TanImpl<LastUse, DstTile, SrcTile, TmpFp32Tile>(dstTile, srcTile, tmpFp32Tile);
                } else if constexpr (std::is_same<typename T1::Type, half>::value ||
                                     std::is_same<typename T1::Type, bfloat16_t>::value) {
                    TanHalfImpl<LastUse, DstTile, SrcTile, TmpFp32Tile, TmpFp32Tile>(dstTile, srcTile, tmpFp32Tile, tmpFp32Tile);
                }
            }
        }
    }
