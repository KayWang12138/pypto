...
TILEOP void TExpandExpDif(T0 dst, T1 src0, T2 src1)
{
    if constexpr (GetBrcOperandAt<DIM_5TH, BrcOperands...>() == BRC_NONE) {
        ...
        if ((src0Shape3 == src1Shape3) && (src0Shape4 == src1Shape4)) {
            BinaryCompute<BinaryOp::SUB, pto::DivAlgorithm::DEFAULT, LastUse3Dim<0, 0, 0>, BrcOperands...>(
                dst, src0, src1);
#ifdef __DAV_V220
            pipe_barrier(PIPE_V);
#endif
    ...
    BinaryCompute<BinaryOp::EXPANDEXPDIF, pto::DivAlgorithm::DEFAULT, LastUse3Dim<0, 0, 0>, BrcOperands...>(
        dst, src0, src1);
}
