...
TILEOP void TPhiloxRandomOp(T0 dst, T1 key, T2 counter, uint16_t rounds) {
    ...
    uint32_t *keyPtr = reinterpret_cast<uint32_t *>(key.GetAddr());
    uint32_t *counterPtr = reinterpret_cast<uint32_t *>(counter.GetAddr());
    ...
    for (LoopVar n0Index = 0; n0Index < dstShape0; ++n0Index) {
        for (LoopVar n1Index = 0; n1Index < dstShape1; ++n1Index) {
            for (LoopVar n2Index = 0; n2Index < dstShape2; ++n2Index) {
                ...
                PhiloxRandomImpl<LastUse, DstTile>(dstTile, keyPtr, counterPtr, rounds);
            }
        }
    }
}
