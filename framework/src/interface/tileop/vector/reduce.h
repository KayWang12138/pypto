...
TILEOP void TRowProdSingle(T0 dst, T1 src, T2 tmp) {
    ReduceLastAxisCompute<ReduceOp::PROD>(dst, src, tmp);
}
...
template <ReduceOp op, int axis, typename DstTileDefine, typename SrcTileDefine, typename TmpTileDefine, typename T0, typename T1, typename T2>
TILEOP void TRowSumProdLineDynamic(T0 dst, T1 src, T2 tmp) {
    constexpr size_t expectSize = 5;
    constexpr auto typeSize = sizeof(typename T1::Type);
    const auto dstLayout = dst.GetLayout();
    const auto srcLayout = src.GetLayout();
...
TILEOP void TRowSumLine(T0 dst, T1 src, T2 tmp) {
    ...
    TRowSumProdLineDynamic<ReduceOp::SUM, axis, DstTileDefine, SrcTileDefine, TmpTileDefine>(dst, src, tmp);
}
#endif