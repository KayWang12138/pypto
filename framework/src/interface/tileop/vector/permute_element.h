 ...
 *
 * Key difference from Tpermute:
 *   Tpermute always gets 5D padded tile (via transpose decomposition), so it can use
 *   GetGmOffset and GetStrideDim<0..4, 5>() safely.
 *
 *   TpermuteElement gets N-dimensional tile (3D/4D/5D) directly without decomposition.
 *   For dimCount<5, the Layout tuple has only N elements, so:
 *   - Cannot use GetGmOffset (it internally accesses DIM_1ST..DIM_5TH = index 0..4)
 *   - Must use GetStrideDim<dim, N>() where dim < N
 *   - Must manually compute srcBase from coordinates
 *   - sc[] array uses N elements (not 5), indexed by axis0..axisN directly
 */
