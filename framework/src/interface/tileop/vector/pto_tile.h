...
public:
    ...
    __aicore__ inline void Assign(uint64_t addr, uint64_t offset) {
        pto::TASSIGN(data_, addr + offset * sizeof(typename T::Type));
    }
    ...
    __aicore__ inline void Assign(T &tensor, const TileOffset &offsets) {
        pto::TASSIGN(data_, (uint64_t)(tensor.GetAddr() + GenTileOffset(tensor, offsets)));
    }
