...
template <typename T, typename Shape, typename Stride, bool is_store = false>
class PtoGlobal {
private:
...
public:
    ...
    __aicore__ inline PtoGlobal(const Shape &shape, const Stride &stride)
        : data_((__gm__ Dtype *)(0),
              pto::Shape((is_store ? 1 : GetTupleElement<Shape, DIM_1ST, expect_size, 1>(shape)),
                  (is_store ? 1 : GetTupleElement<Shape, DIM_2ND, expect_size, 1>(shape)),
                  (is_store ? 1 : GetTupleElement<Shape, DIM_3RD, expect_size, 1>(shape)),
                  GetTupleElement<Shape, DIM_4TH, expect_size, 1>(shape),
                  GetTupleElement<Shape, DIM_5TH, expect_size, 1>(shape)),
              pto::Stride((is_store ? 0 : GetTupleElement<Stride, DIM_1ST, expect_size, 0>(stride)),
                  (is_store ? 0 : GetTupleElement<Stride, DIM_2ND, expect_size, 0>(stride)),
                  (is_store ? 0 : GetTupleElement<Stride, DIM_3RD, expect_size, 0>(stride)),
                  GetTupleElement<Stride, DIM_4TH, expect_size, 0>(stride),
                  GetTupleElement<Stride, DIM_5TH, expect_size, 0>(stride))) {}
