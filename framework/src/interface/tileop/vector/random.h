...
TILEOP void TRandomOp(T0 dst, uint64_t key, uint64_t *counter, uint16_t rounds, int64_t *originalShape, int64_t originalShapeSize) {
    ...
    uint64_t initialCounter[2] = {counter[0], counter[1]};
    ...
    for (LoopVar n0Index = 0; n0Index < dstShape0; ++n0Index) {
        for (LoopVar n1Index = 0; n1Index < dstShape1; ++n1Index) {
            for (LoopVar n2Index = 0; n2Index < dstShape2; ++n2Index) {
                ...
                uint64_t tileCounter[2] = {initialCounter[0], initialCounter[1]};
