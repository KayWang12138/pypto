...
template <
    size_t srcTypeSize,
    typename VAL, typename IDX, typename RES_VAL, typename RES_IDX>
TILEOP void RadixSelectExtractSortResult(VAL val, IDX idx, RES_VAL resultVal, RES_IDX resultIdx)
{
    constexpr auto valPattern = srcTypeSize == 4 ? pto::MaskPattern::P0101 : pto::MaskPattern::P0001;
    pto::TGATHER<VAL, RES_VAL, valPattern>(val, resultVal);
    pto::TGATHER<IDX, RES_IDX, pto::MaskPattern::P1010>(idx, resultIdx);
}
...
TILEOP void RadixSelectTwiddle(TWI twi, SRC src, TMP tmp, CMP cmp, USELESS useless)
{
    ...
    if constexpr (srcTypeSize == 4) {
        pto::TXORS(tmp, src, static_cast<int32_t>(0x80000000), useless);
        ...
        for (LoopVar n1Index = 0; n1Index < srcShape1; ++n1Index) {
            for (LoopVar n2Index = 0; n2Index < srcShape2; ++n2Index) {
                for (LoopVar n3Index = 0; n3Index < srcShape3; ++n3Index) {
                    ...
                    pto::TSORT32(sortResultTile, valTile, idxUInt32Tile, sortTempTile);
                    RadixSelectExtractSortResult<srcTypeSize>(valIntTile, idxTile, sortResultIntTile, sortResultInt32Tile);
                    if constexpr (!isLargest) {
                        pto::TMULS(valTile, valTile, -1.0);
                    }
                }
