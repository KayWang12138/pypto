...
template <typename T0, typename T1, unsigned tileW, unsigned dstTypeSize, int isSum>
...
template <int isSum, typename T0, typename T1>
TILEOP void TCumOperation(T0 dst, T1 src, int diagonal) {
    ...
    if constexpr (axis == 0) {
        ...
        CumOperationTool<T0, T1, tileW, dstTypeSize, isSum>(dst, src, tileH, tmpStride);
        return;
