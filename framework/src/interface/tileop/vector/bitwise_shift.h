...
template <BitwiseShiftScalarOp op, typename T0, typename Scalar, typename T1, typename T2>
    ...
    for (size_t n0Index = 0; n0Index < shape0; ++n0Index) {
        for (size_t n1Index = 0; n1Index < shape1; ++n1Index) {
            for (size_t n2Index = 0; n2Index < shape2; ++n2Index) {
                ...
                src0Tile.Assign(src0, tileOffsets);
                tmpTile.Assign(tmp, tileOffsets);
                ScalarBitwiseShiftImpl<op, MAX_SHIFT_NUM>(dstTile.Data(), src0Tile.Data(), tmpTile.Data(), src1);
            }
        }
    }
}
...
TILEOP void TSBitrshift(T0 dst, Scalar src0, T1 src1, T2 tmp) {
    ScalarBitwiseShiftCompute<BitwiseShiftScalarOp::BITWISERIGHTSHIFT>(dst, src0, src1, tmp);
}

#define OP_TILE_OP_SBITWISELEFTSHIFT TSBitlshift
template <typename Scalar, typename T0, typename T1, typename T2>
TILEOP void TSBitlshift(T0 dst, Scalar src0, T1 src1, T2 tmp) {
    ScalarBitwiseShiftCompute<BitwiseShiftScalarOp::BITWISELEFTSHIFT>(dst, src0, src1, tmp);
}
#endif