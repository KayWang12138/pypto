...
TILEOP void TPRelu(T0 dst, T1 src, T2 weight, T3 tmp) {
    ...
    if constexpr (axis == 4) {
        ...
        WeightTileDefine weightTile(1, srcShape4);
        ...
        constexpr auto weightTileW = TileOp::GetTensorTileShapeDim<T2, 4, expectSize>();
        using DstTileDefine =
            pto::Tile<pto::TileType::Vec, typename T0::Type, 1, dstTileW, pto::BLayout::RowMajor, -1, -1>;
        using SrcTileDefine =
            pto::Tile<pto::TileType::Vec, typename T1::Type, 1, srcTileW, pto::BLayout::RowMajor, -1, -1>;
        using WeightTileDefine =
            pto::Tile<pto::TileType::Vec, typename T2::Type, 1, weightTileW, pto::BLayout::RowMajor, -1, -1>;
        DstTileDefine dstTile(1, dstShape4);
        SrcTileDefine srcTile(1, srcShape4);
        WeightTileDefine weightTile(1, srcShape3);
        for (LoopVar n2Index = 0; n2Index < dstShape2; ++n2Index) {
            for (LoopVar n3Index = 0; n3Index < dstShape3; ++n3Index) {
                auto dstOffset = n2Index * dstStride2 + n3Index * dstStride3;
                auto srcOffset = n2Index * srcStride2 + n3Index * srcStride3;
                auto negative_slope = weightTile.GetValue(n3Index);
        ...
        WeightTileDefine weightTile(1, srcShape2);
        ...
        for (LoopVar n1Index = 0; n1Index < dstShape1; ++n1Index) {
            for (LoopVar n2Index = 0; n2Index < dstShape2; ++n2Index) {
                ...
                auto negative_slope = weightTile.GetValue(n2Index);
