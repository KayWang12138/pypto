...
TILEOP void SignIntCast(DstTile dstTile, SrcTile srcTile, TmpTile tmpTile)
{
    constexpr auto n1 = Std::tuple_element<DIM_1ST, LastUse>::type::value;
    constexpr auto n2 = Std::tuple_element<DIM_2ND, LastUse>::type::value;
    pto::TCVT(tmpTile, srcTile);
#ifdef __DAV_V220
    pipe_barrier(PIPE_V);
#endif
    SignHalf<LastUse, T, DstTile, SrcTile>(tmpTile, tmpTile);
#ifdef __DAV_V220
    pipe_barrier(PIPE_V);
#endif
    pto::TCVT(dstTile, tmpTile, static_cast<T>(-1));
}
...
TILEOP void SignImpl(DstTile dstTile, SrcTile srcTile, TmpTile tmpTile)
{
    if constexpr (std::is_same<T, int32_t>::value || std::is_same<T, int16_t>::value) {
        ...
        SignInt<LastUse, T, DstTile, SrcTile, TmpTile>(dstTile, srcTile, tmpTile);
    }
    return;
}
...
TILEOP void TSign(T0 dst, T1 src, T3 tmp) {
    ...
    using TmpTile =
        pto::Tile<pto::TileType::Vec, half, srcTileH, srcTileW, pto::BLayout::RowMajor, -1, -1>;
