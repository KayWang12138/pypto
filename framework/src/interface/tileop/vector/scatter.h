...
TILEOP void Tscatter(T0 dst, T1 src1, T2 src2, T3 tmp, C coordinate) {
    ...
    // using SrcTileType = pto::Tile<pto::TileType::Vec, typename T2::Type, 1, srcTileW, pto::BLayout::RowMajor -1, -1>;
        ...
        for (LoopVar i = 0; i < idxShape0; ++i) {
            for (LoopVar j = 0; j < idxShape1; ++j) {
                for (LoopVar k = 0; k < idxShape2; ++k) {
                    ...
                    using IdxTileType = pto::Tile<pto::TileType::Vec, uint32_t, srcTileH, srcTileW, pto::BLayout::RowMajor>; 
                    ...
                    GlobalDstType globalDst(dstGmBase, pto::Shape(1, 1, 1, d3Shape, d4Shape), pto::Stride(1, 1, 1, dstStride3, 1));
