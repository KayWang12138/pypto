...
TILEOP void Tscatter(T0 dst, T1 src1, T2 src2, T3 temp, C coordinate) {
    ...
    if constexpr (useScalarPath) {
        ...
        using DstTileType = pto::Tile<pto::TileType::Vec, DstType, 1, 1, pto::BLayout::RowMajor>;
        using SrcTileType = pto::Tile<pto::TileType::Vec, SrcType, 1, 1, pto::BLayout::RowMajor>;
        ...
        if constexpr (useMScatter) {
            ...
            for (LoopVar i = 0; i < i0Shape; ++i) {
                for (LoopVar j = 0; j < i1Shape; ++j) {
                    for (LoopVar k = 0; k < i2Shape; ++k) {
                        for (LoopVar l = 0; l < i3Shape; ++l) {
                            // Fill offset buffer for current (i,j,k,l) slice
                            for (LoopVar m = 0; m < i4Shape; ++m) {
                            ...
                            uint64_t srcTileAddr = reinterpret_cast<uint64_t>(srcBaseAddr + i * s0Stride + j * s1Stride + k * s2Stride);
                            ...
                            if constexpr (scatterMode == 0) {
                                pto::MSCATTER<pto::ScatterAtomicOp::None, pto::ScatterOOB::Skip>(dstGlobal, srcTile, offsetTile);
                            } else if constexpr (scatterMode == 1) {
                                pto::MSCATTER<pto::ScatterAtomicOp::Add, pto::ScatterOOB::Skip>(dstGlobal, srcTile, offsetTile);
                            }
                        }
                    }
                }
