                ...
                for (int l = 0; l < n3IdxShape; ++l) {
                    for (int m = 0; m < n4IdxShape; ++m) {
                        ...
                        if constexpr (scatterMode == 0) {
                            ...
                            dstAddr[dstOffset] = src2 + dstAddr[dstOffset];
                        } else {
                            dstAddr[dstOffset] = src2 * dstAddr[dstOffset];
                        }
                    }
...
template <int axis, int scatterMode, typename T0, typename T1, typename T2>
TILEOP void Tscatter(T0 dst, T1 src1, T2 src2) {
    static_assert(scatterMode < SCATTER_MODE_MAX, "Unsupport scatterMode");
    constexpr auto shapeSize = Std::tuple_size<typename T0::Shape>::value;
    constexpr size_t expectSize = 5;
    const auto dstLayout = dst.GetLayout();
    ...
    constexpr auto idxTileH = Std::tuple_element<shapeSize - 2, typename T1::TileShape>::type::value;
    constexpr auto idxTileW = Std::tuple_element<shapeSize - 1, typename T1::TileShape>::type::value;
    constexpr auto srcTileH = Std::tuple_element<shapeSize - 2, typename T2::TileShape>::type::value;
    constexpr auto srcTileW = Std::tuple_element<shapeSize - 1, typename T2::TileShape>::type::value;
    ...
    using idxTileDefine = pto::Tile<pto::TileType::Vec, typename T1::Type, idxTileH, idxTileW, pto::BLayout::RowMajor, -1, -1>;
    using srcTileDefine = pto::Tile<pto::TileType::Vec, typename T0::Type, srcTileH, srcTileW, pto::BLayout::RowMajor>;
        ...
        for (int j = 0; j < n1IdxShape; ++j) {
            for (int k = 0; k < n2IdxShape; ++k) {
                ...
                if constexpr (scalarFlag == false) {
                    ...
                    auto srcOffset = i * n0SrcStride + j * n1SrcStride + k * n2SrcStride;
