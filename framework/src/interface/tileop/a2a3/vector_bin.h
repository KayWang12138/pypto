...
// dim2 & dim1 (T0 = 1 for dim1)
template <typename T, unsigned T0, unsigned S0T1, unsigned S1T1, unsigned DS,
          unsigned S0S0, unsigned S0S1, unsigned S1S0, unsigned S1S1>
...
TILEOP void T_BIN(__ubuf__ T *dst, __ubuf__ T *src0, __ubuf__ T *src1) {
    ...
    for (int i = 0; i < T0; i++) {
        ...
        __ubuf__ T *src1_ = src1;
        for (int j = 0; j < T1; j++) {
            T_BIN<T, T2, S0T3, S1T3, DS3, S0S2, S0S3, S1S2, S1S3>(dst_, src0_, src1_);
            dst_ += DS2 * DS3;
