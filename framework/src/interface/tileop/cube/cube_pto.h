...
TILEOP void TExtractL0CToL1(
    l1Data &dstL1, l0cData &srcL0C, V &fixbuf, uint16_t locOffset0, uint16_t locOffset1, uint64_t scaleValue = 0) {
    if constexpr (std::is_same<typename l0cData::DType, int32_t>::value &&
        ...
        if (scaleValue != 0) {
            ...
            pto::TEXTRACT<l1Data, l0cData, relu_mode>(dstL1, srcL0C, scaleValue, locOffset0, locOffset1);
        } else {
            ...
            pto::TEXTRACT_FP<l1Data, l0cData, decltype(scaleData),
                config::kReluMode == 0 ? pto::ReluPreMode::NoRelu : pto::ReluPreMode::NormalRelu>(
                dstL1, srcL0C, scaleData, locOffset0, locOffset1);
        }
    } else {
        pto::TEXTRACT<l1Data, l0cData,
            config::kReluMode == 0 ? pto::ReluPreMode::NoRelu : pto::ReluPreMode::NormalRelu>(
            dstL1, srcL0C, locOffset0, locOffset1);
    }
}
...
TILEOP void TExtract(T &dst, U &src, V &fixbuf, const Coord &l1Coord, const Coord &locCoord, uint64_t scaleValue = 0) {
    ...
    uint16_t locOffset0 = locCoord.GetValue();
    uint16_t locOffset1 = static_cast<const Std::tuple<size_t> &>(locCoord).GetValue();
    ...
    if (dstShape0 < srcShape0 || dstShape1 < srcShape1) {
        TExtractL0CToL1<config, l1TileData, l0cTileData, V>(dstL1, srcL0C, fixbuf, locOffset0, locOffset1, scaleValue);
    } else {
