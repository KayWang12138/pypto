...
INLINE void TLoadND2NZ(T &dst, U &src, const int64_t &offset0, const int64_t &offset1) {
    ...
    if constexpr (TileOp::IsConstContinous<T>()) {
        ...
        using tileData = pto::Tile<pto::TileType::Mat, typename T::Type, staticL1H, staticL1W, pto::BLayout::ColMajor,
            staticDstShape0, staticDstShape1, pto::SLayout::RowMajor>;
        ...
        pto::TASSIGN(dstL1, (uint64_t)dst.GetAddr());
        pto::TLOAD(dstL1, src0Global);
        pto::TFILLPAD<tileData, PadValue::Zero, CompactMode::Normal>(dstL1, dstL1);
    }
    return;
}
...
INLINE void TLoadND2ND(T &dst, U &src, const int64_t &offset0, const int64_t &offset1) {
    ...
    if constexpr (TileOp::IsConstContinous<T>()) {
        ...
        constexpr int64_t staticDstShape1 = TileOp::GetTensorShapeDim<T, shapeSize - 1>();
        globalData src0Global((__gm__ typename U::Type *)(src.GetAddr() + gmOffset),
            shapeDim2(staticDstShape0, staticDstShape1), strideDim2(srcStride0, srcStride1));
        using tileData = pto::Tile<pto::TileType::Mat, typename T::Type, staticL1H, staticL1W, pto::BLayout::RowMajor,
            staticDstShape0, staticDstShape1>;
        tileData dstL1;
        pto::TASSIGN(dstL1, (uint64_t)dst.GetAddr());
        pto::TLOAD(dstL1, src0Global);
    } else {
...
INLINE void TLoadAMXStatic(T &dst, U &src, int64_t offset0, int64_t offset1, int64_t srcShape0, int64_t srcShape1) {
    constexpr auto shapeSize = Std::tuple_size<typename T::Shape>::value;
    ...
    constexpr auto staticL1W = Std::tuple_element<shapeSize - SHAPE_DIM2, typename T::TileShape>::type::value * SHAPE_DIM2;

    using shapeDim2 = pto::Shape<1, 1, -1, -1, SHAPE_DIM2>;
    using strideDim3 = pto::Stride<-1, -1, -1, SHAPE_DIM2, 1>;
    using tileData = pto::Tile<pto::TileType::Mat, typename T::Type, staticL1H, staticL1W, pto::BLayout::RowMajor,
        staticDstShape0, staticDstShape1 * SHAPE_DIM2, pto::SLayout::RowMajor, pto::TileConfig::alignedSize>;
    ...
    if constexpr (mode == CopyInMode::ND2NZ) {
        ...
        globalData src0Global((__gm__ typename U::Type *)(src.GetAddr() + gmOffset),
            shapeDim2(staticDstShape0, staticDstShape1),
...
template <CopyInMode mode, typename Coord, typename T, typename U>
INLINE void TLoadAMXDynamic(T &dst, U &src, int64_t offset0, int64_t offset1, int64_t srcShape0, int64_t srcShape1) {
    constexpr auto shapeSize = Std::tuple_size<typename T::Shape>::value;
    int64_t dstShape0 = GetShape<0>(dst);
    int64_t dstShape1 = GetShape<1>(dst);
    constexpr auto staticL1H = Std::tuple_element<shapeSize - SHAPE_DIM3, typename T::TileShape>::type::value;
...
template <CopyInMode mode, typename Coord, typename T, typename U>
INLINE void TLoadBMXStatic(T &dst, U &src, int64_t offset0, int64_t offset1, int64_t srcShape0, int64_t srcShape1) {
    constexpr auto shapeSize = Std::tuple_size<typename T::Shape>::value;
    constexpr int64_t staticDstShape0 = TileOp::GetTensorShapeDim<T, shapeSize - SHAPE_DIM3>();
    constexpr int64_t staticDstShape1 = TileOp::GetTensorShapeDim<T, shapeSize - SHAPE_DIM2>();
    constexpr auto staticL1H = Std::tuple_element<shapeSize - SHAPE_DIM3, typename T::TileShape>::type::value * SHAPE_DIM2;
    constexpr auto staticL1W = Std::tuple_element<shapeSize - SHAPE_DIM2, typename T::TileShape>::type::value;

    using shapeDim2 = pto::Shape<1, 1, -1, -1, SHAPE_DIM2>;
    using strideDim3 = pto::Stride<-1, -1, -1, SHAPE_DIM2, 1>;
    using tileData = pto::Tile<pto::TileType::Mat, typename T::Type, staticL1H, staticL1W, pto::BLayout::ColMajor,
        staticDstShape0 * SHAPE_DIM2, staticDstShape1, pto::SLayout::ColMajor, pto::TileConfig::alignedSize>;
...
template <CopyInMode mode, typename Coord, typename T, typename U>
INLINE void TLoadBMXDynamic(T &dst, U &src, int64_t offset0, int64_t offset1, int64_t srcShape0, int64_t srcShape1) {
    constexpr auto shapeSize = Std::tuple_size<typename T::Shape>::value;
    int64_t dstShape0 = GetShape<0>(dst);
    int64_t dstShape1 = GetShape<1>(dst);
    constexpr auto staticL1H = Std::tuple_element<shapeSize - SHAPE_DIM3, typename T::TileShape>::type::value * SHAPE_DIM2;
...
TILEOP void TLoadAMX(T &dst, U &src, const Coord &coord) {
    ...
    int64_t srcShape0 = GetShape<0>(src);
    int64_t srcShape1 = GetShape<1>(src);
    if constexpr (TileOp::IsConstContinousMX<T>()) {
        TLoadAMXStatic<mode, Coord, T, U>(dst, src, offset0, offset1, srcShape0, srcShape1);
    } else {
        TLoadAMXDynamic<mode, Coord, T, U>(dst, src, offset0, offset1, srcShape0, srcShape1);
    }
}
...
TILEOP void TLoadBMX(T &dst, U &src, const Coord &coord) {
    ...
    int64_t srcShape1 = GetShape<1>(src);
    if constexpr (TileOp::IsConstContinousMX<T>()) {
        TLoadBMXStatic<mode, Coord, T, U>(dst, src, offset0, offset1, srcShape0, srcShape1);
    } else {
        TLoadBMXDynamic<mode, Coord, T, U>(dst, src, offset0, offset1, srcShape0, srcShape1);
    }
}
...
INLINE void TExtractL0CToL1Static(T &dst, U &src, V &fixbuf, uint16_t l1Offset0, uint16_t l1Offset1,
    uint16_t l0cOffset0, uint16_t l0cOffset1, uint64_t scaleValue) {
    constexpr int64_t shapeSize = Std::tuple_size<typename T::Shape>::value;
    constexpr int64_t staticDstShape0 = TileOp::GetTensorShapeDim<T, shapeSize - SHAPE_DIM2>();
    constexpr int64_t staticDstShape1 = TileOp::GetTensorShapeDim<T, shapeSize - 1>();
    ...
    using L1Tile = pto::Tile<pto::TileType::Mat, typename T::Type, tileL1H, tileL1W,
    ...
    using L0CTile = pto::Tile<pto::TileType::Acc, typename U::Type, tileL0CH, tileL0CW, pto::BLayout::ColMajor,
        staticSrcShape0, staticSrcShape1, pto::SLayout::RowMajor>;
...
template <typename config, typename T, typename U, typename V>
INLINE void TExtractL0CToL1Dynamic(T &dst, U &src, V &fixbuf, uint16_t l1Offset0, uint16_t l1Offset1,
    uint16_t l0cOffset0, uint16_t l0cOffset1, uint64_t scaleValue) {
    constexpr int64_t shapeSize = Std::tuple_size<typename T::Shape>::value;
    constexpr int64_t tileL1H = Std::tuple_element<shapeSize - SHAPE_DIM2, typename T::TileShape>::type::value;
    constexpr int64_t tileL1W = Std::tuple_element<shapeSize - 1, typename T::TileShape>::type::value;
    ...
    using L0CTile = pto::Tile<pto::TileType::Acc, typename U::Type, tileL0CH, tileL0CW, pto::BLayout::ColMajor, -1, -1,
...
INLINE void TExtractL1ToL0Static(T &dst, U &src, const int64_t &offset0, const int64_t &offset1) {
    ...
    constexpr int64_t staticSrcShape0 = TileOp::GetTensorShapeDim<U, shapeSize - SHAPE_DIM2>();
    constexpr int64_t staticSrcShape1 = TileOp::GetTensorShapeDim<U, shapeSize - 1>();
    constexpr auto srcValidH = isTrans ? staticSrcShape1 : staticSrcShape0;
    constexpr auto srcValidW = isTrans ? staticSrcShape0 : staticSrcShape1;
    using tileL1Tensor = pto::Tile<pto::TileType::Mat, typename U::Type, isTrans ? staticL1W : staticL1H,
        isTrans ? staticL1H : staticL1W, isTrans ? pto::BLayout::RowMajor : pto::BLayout::ColMajor, srcValidH,
        srcValidW, isTrans ? pto::SLayout::ColMajor : pto::SLayout::RowMajor>;
...
INLINE void TExtractL1ToBTOrFB(T &dst, U &src) {
    ...
    if constexpr (TileOp::IsConstContinous<T, U>()) {
        ...
        using tileBiasOrFbTensor = pto::Tile<T::FORMAT == Hardware::BIAS ? pto::TileType::Bias : pto::TileType::Scaling,
            typename T::Type, 1, staticL0BW, pto::BLayout::RowMajor, 1, staticNL0>;
...
INLINE void TExtractMXStatic(T &dst, U &src, const Coord &coord) {
    ...
    using tileL1Tensor = std::conditional_t<T::FORMAT == Hardware::L0A_MX,
        pto::Tile<pto::TileType::Mat, typename U::Type, staticL1H, staticL1W * SHAPE_DIM2, pto::BLayout::RowMajor,
            staticSrcShape0, staticSrcShape1 * SHAPE_DIM2, pto::SLayout::RowMajor, pto::TileConfig::alignedSize>,
        pto::Tile<pto::TileType::Mat, typename U::Type, staticL1H * SHAPE_DIM2, staticL1W, pto::BLayout::ColMajor,
    ...
    using tileL0MXTensor = std::conditional_t<T::FORMAT == Hardware::L0A_MX,
        pto::TileLeftScaleCompact<typename T::Type, staticL0H, staticL0W * SHAPE_DIM2, staticDstShape0,
            staticDstShape1 * SHAPE_DIM2>,
        pto::TileRightScaleCompact<typename T::Type, staticL0H * SHAPE_DIM2, staticL0W,
            staticDstShape0 * SHAPE_DIM2, staticDstShape1>>;
...
TILEOP void TExtractMX(T &dst, U &src, const Coord &coord) {
    ...
    if constexpr (TileOp::IsConstContinousMX<T, U>()) {
...
TILEOP void TExtractL0CToUBStatic(T &dst, U &src, const Coord &coord, int16_t subblockId) {
    constexpr auto shapeSize = Std::tuple_size<typename T::Shape>::value;
...
template <CopyOutMode mode, typename Coord, typename T, typename U>
TILEOP void TExtractL0CToUBDynamic(T &dst, U &src, const Coord &coord, int16_t subblockId) {
    constexpr auto shapeSize = Std::tuple_size<typename T::Shape>::value;
    constexpr int64_t c0Size = BLOCK_ALIGN_BYTE / sizeof(typename U::Type);
    int64_t offset0 = coord.GetValue();
    int64_t offset1 = static_cast<const Std::tuple<size_t> &>(coord).GetValue();
...
template <bool isZeroC, TransMode transMode, bool hasBias, typename tileL0ATensor, typename tileL0BTensor,
    typename tileL0CTensor, typename T0, typename T1, typename T2, typename T3 = void *,
    typename tileBiasTensor = void *>
INLINE void ExecuteMatmulPipeline(tileL0ATensor &l0a, tileL0BTensor &l0b, tileL0CTensor &l0c, T0 &c, T1 &a, T2 &b,
    T3 bias = nullptr, tileBiasTensor biasT = nullptr) {
...
TILEOP void TMatmul(T &c, U &a, V &b) {
    ...
    if constexpr (TileOp::IsConstContinous<T, U, V>()) {
        ...
        tileL0BTensor l0b;
        tileL0CTensor l0c;
        ExecuteMatmulPipeline<isZeroC, transMode, false>(l0a, l0b, l0c, c, a, b);
    } else {
...
TILEOP void TMatmul(T0 &c, T1 &a, T2 &b, T3 &bias) {
    ...
    if constexpr (TileOp::IsConstContinous<T0, T1, T2, T3>()) {
        ...
        constexpr int64_t staticValidN = TileOp::GetTensorShapeDim<T2, shapeSizeB - 1>();
        if constexpr (staticValidM == 0 || staticValidK == 0 || staticValidN == 0) {
            return;
        }
        using tileL0ATensor = pto::TileLeft<typename T1::Type, staticL0AH, staticL0AW, staticValidM, staticValidK>;
        using tileL0BTensor = pto::TileRight<typename T2::Type, staticL0BH, staticL0BW, staticValidK, staticValidN>;
        ...
        ExecuteMatmulPipeline<false, transMode, true>(l0a, l0b, l0c, c, a, b, bias, biasT);
    } else {
...
#if defined PTO_NPU_ARCH_A5
template <bool isZeroC, bool hasBias, typename tileL0ATensor, typename tileL0AScaleTensor, typename tileL0BTensor,
...
INLINE void ExecuteMatmulMXPipeline(tileL0ATensor &l0a, tileL0AScaleTensor &l0aScale, tileL0BTensor &l0b,
    tileL0BScaleTensor &l0bScale, tileL0CTensor &l0c, T0 &c, T1 &a, T2 &aScale, T3 &b, T4 &bScale, T5 bias = nullptr,
...
template <bool isZeroC, bool hasBias, typename T0, typename T1, typename T2, typename T3, typename T4,
    typename T5 = void *>
TILEOP void MatmulMXStatic(T0 &c, T1 &a, T2 &aScale, T3 &b, T4 &bScale, T5 bias = nullptr) {
    constexpr auto shapeSizeC = Std::tuple_size<typename T0::Shape>::value;
    constexpr auto shapeSizeA = Std::tuple_size<typename T1::Shape>::value;
    ...
    constexpr int64_t staticValidScaleK = TileOp::GetTensorShapeDim<T2, shapeSizeAScale - SHAPE_DIM2>() * SHAPE_DIM2;
    if (staticValidM == 0 || staticValidK == 0 || staticValidN == 0 || staticValidScaleK == 0) {
        return;
    }
    using tileL0CTensor = pto::TileAcc<typename T0::Type, staticL0CH, staticL0CW, staticValidM, staticValidN>;
    using tileL0ATensor = pto::TileLeft<typename T1::Type, staticL0AH, staticL0AW, staticValidM, staticValidK>;
    using tileL0AScaleTensor = pto::TileLeftScale<typename T1::Type, staticL0AH, staticL0AScaleW * SHAPE_DIM2,
        staticValidM, staticValidScaleK>;
    using tileL0BTensor = pto::TileRight<typename T3::Type, staticL0BH, staticL0BW, staticValidK, staticValidN>;
    using tileL0BScaleTensor = pto::TileRightScale<typename T3::Type, staticL0BScaleH * SHAPE_DIM2, staticL0BW,
        staticValidScaleK, staticValidN>;
    tileL0ATensor l0a;
    ...
    if constexpr (hasBias) {
        using tileBiasTensor =
            pto::Tile<pto::TileType::Bias, typename T5::Type, 1, staticL0BW, pto::BLayout::RowMajor, 1, staticValidM>;
        tileBiasTensor biasT;
        ExecuteMatmulMXPipeline<isZeroC, hasBias>(
            l0a, l0aScale, l0b, l0bScale, l0c, c, a, aScale, b, bScale, bias, biasT);
    } else {
        ExecuteMatmulMXPipeline<isZeroC, hasBias>(l0a, l0aScale, l0b, l0bScale, l0c, c, a, aScale, b, bScale);
    }
}

template <bool isZeroC, bool hasBias, typename T0, typename T1, typename T2, typename T3, typename T4,
    typename T5 = void *>
TILEOP void MatmulMXDynamic(T0 &c, T1 &a, T2 &aScale, T3 &b, T4 &bScale, T5 bias = nullptr) {
    constexpr auto shapeSizeC = Std::tuple_size<typename T0::Shape>::value;
    constexpr auto shapeSizeA = Std::tuple_size<typename T1::Shape>::value;
    constexpr auto shapeSizeAScale = Std::tuple_size<typename T2::Shape>::value;
    constexpr auto shapeSizeB = Std::tuple_size<typename T3::Shape>::value;
...
INLINE void TStoreNZ2ND(
    ...
    if constexpr (TileOp::IsConstContinous<U>()) {
        ...
        using tileData = pto::Tile<pto::TileType::Acc, typename U::Type, tileH, tileW, pto::BLayout::ColMajor,
            staticSrcShape0, staticSrcShape1, pto::SLayout::RowMajor, pto::TileConfig::fractalCSize,
...
TILEOP void TStore(T &dst, U &src, const Coord &coord) {
    ...
    if constexpr (config::kMode == CopyOutMode::ND2ND) {
        if constexpr (TileOp::IsConstContinous<U>()) {
            ...
            constexpr int64_t staticSrcShape1 = TileOp::GetTensorShapeDim<U, shapeSize - 1>();
            using globalData = pto::GlobalTensor<typename T::Type, shapeDim2, strideDim2, pto::Layout::ND>;
            using tileData = pto::Tile<pto::TileType::Mat, typename U::Type, staticL1H, staticL1W,
                pto::BLayout::RowMajor, staticSrcShape0, staticSrcShape1>;
            globalData dstGlobal((__gm__ typename T::Type *)(dst.GetAddr()),
                shapeDim2(staticSrcShape0, staticSrcShape1), strideDim2(dstStride0, dstStride1));
            tileData srcL1;
            ...
            globalData dstGlobal((__gm__ typename T::Type *)(dst.GetAddr()), shapeDim2(srcShape0, srcShape1),
...
template <int64_t blockSize, typename globalData, typename tileData, typename DstT, typename SrcT, typename BlockT,
...
INLINE void TGatherInL1Static(DstT dst, SrcT src, BlockT block, OffsetT offset, int64_t srcShape1, int64_t srcStride0,
    int64_t srcStride1, uint64_t srcColumnStartOffset, uint64_t offsetsStartOffset, uint64_t GMBlockTableOffset) {
    ...
    if constexpr (staticDstShape1 % c0Size > 0) {
        using tileData = pto::Tile<pto::TileType::Mat, typename DstT::Type, staticL1H, staticL1W,
            pto::BLayout::ColMajor, staticDstShape0, staticDstShape1, pto::SLayout::RowMajor>;
        GatherExecute<blockSize, globalData, tileData, DstT, SrcT, BlockT, OffsetT, true>(dst, src, block, offset,
            offsetsStartOffset, srcColumnStartOffset, GMBlockTableOffset, 1, staticL1W, srcStride0, srcStride1,
            staticDstShape0, staticDstShape1, srcShape1, staticDstShape0, c0Size);
    } else {
        using tileData = pto::Tile<pto::TileType::Mat, typename DstT::Type, staticL1W / c0Size, staticL1H * c0Size,
            pto::BLayout::RowMajor, staticDstShape1 / c0Size, c0Size>;
        ...
        GatherExecute<blockSize, globalData, tileData, DstT, SrcT, BlockT, OffsetT, true>(dst, src, block, offset,
            offsetsStartOffset, srcColumnStartOffset, GMBlockTableOffset, staticDstShape1 / c0Size, c0Size, c0Size, 1,
            staticDstShape1 / c0Size, c0Size, srcShape1, staticDstShape0, c0Size);
    }
}

template <int64_t blockSize, typename DstT, typename SrcT, typename BlockT, typename OffsetT, typename globalData>
INLINE void TGatherInL1Dynamic(DstT dst, SrcT src, BlockT block, OffsetT offset, int64_t srcShape1, int64_t srcStride0,
    int64_t srcStride1, uint64_t srcColumnStartOffset, uint64_t offsetsStartOffset, uint64_t GMBlockTableOffset) {
    constexpr auto shapeSize = Std::tuple_size<typename DstT::Shape>::value;
    constexpr int64_t c0Size = BLOCK_ALIGN_BYTE / sizeof(typename DstT::Type);
    constexpr auto staticL1H = Std::tuple_element<shapeSize - SHAPE_DIM2, typename DstT::TileShape>::type::value;
    ...
    if (dstShape1 % c0Size > 0) {
        ...
        GatherExecute<blockSize, globalData, tileData, DstT, SrcT, BlockT, OffsetT, false>(dst, src, block, offset,
            offsetsStartOffset, srcColumnStartOffset, GMBlockTableOffset, 1, staticL1W, srcStride0, srcStride1,
            dstShape0, dstShape1, srcShape1, dstShape0, c0Size);
    } else {
        using tileData = pto::Tile<pto::TileType::Mat, typename DstT::Type, staticL1W / c0Size, staticL1H * c0Size,
        ...
        // / c0Size, staticL1H * c0Size)的ND格式。 那么L1上有效数据就是(dstShape1 / c0Size, c0Size)。
        GatherExecute<blockSize, globalData, tileData, DstT, SrcT, BlockT, OffsetT, false>(dst, src, block, offset,
            offsetsStartOffset, srcColumnStartOffset, GMBlockTableOffset, dstShape1 / c0Size, c0Size, c0Size, 1,
            dstShape1 / c0Size, c0Size, srcShape1, dstShape0, c0Size);
    }
}
...
TILEOP void TGatherInL1(DstT dst, SrcT src, BlockT block, OffsetT offset, SrcCoord srcCoord, OffsetCoord offsetCoord,
    ...
    if constexpr (TileOp::IsConstContinous<DstT>()) {
        TGatherInL1Static<blockSize, DstT, SrcT, BlockT, OffsetT, globalData>(dst, src, block, offset, srcShape1,
            srcStride0, srcStride1, srcColumnStartOffset, offsetsStartOffset, GMBlockTableOffset);
    } else {
        TGatherInL1Dynamic<blockSize, DstT, SrcT, BlockT, OffsetT, globalData>(dst, src, block, offset, srcShape1,
            srcStride0, srcStride1, srcColumnStartOffset, offsetsStartOffset, GMBlockTableOffset);
    }
}
