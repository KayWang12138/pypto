...
template <typename l1Data, typename l0cData>
constexpr bool NeedsScaling() {
    return std::is_same<typename l0cData::DType, int32_t>::value && std::is_same<typename l1Data::DType, half>::value;
}

template <typename l1Data, typename l0cData, typename V>
TILEOP void TExtractL0CToL1(
    l1Data &dstL1, l0cData &srcL0C, V &fixbuf, int64_t locOffset0, int64_t locOffset1, uint64_t scaleValue = 0) {
    if constexpr (NeedsScaling<l1Data, l0cData>()) {
        if (scaleValue != 0) {
            pto::TEXTRACT(dstL1, srcL0C, scaleValue, locOffset0, locOffset1);
        } else {
            auto scaleData = CreateScaleData(fixbuf);
...
TILEOP void TInsertL0CToL1(
    l1Data &dstL1, l0cData &srcL0C, V &fixbuf, int64_t l1Offset0, int64_t l1Offset1, uint64_t scaleValue = 0) {
    if constexpr (NeedsScaling<l1Data, l0cData>()) {
        if (scaleValue != 0) {
...
TILEOP void TExtract(T &dst, U &src, V &fixbuf, const Coord &l1Coord, const Coord &locCoord, uint64_t scaleValue = 0) {
    constexpr int64_t shapeSize = Std::tuple_size<typename T::Shape>::value;
    static_assert(shapeSize == SHAPE_DIM2 && Std::tuple_size<Coord>::value == SHAPE_DIM2, "Shape Size should be 2 Dim");
    static_assert(U::FORMAT == Hardware::L0C && T::FORMAT == Hardware::L1);

    int64_t l1Offset0 = l1Coord.GetValue();
    int64_t l1Offset1 = static_cast<const Std::tuple<size_t> &>(l1Coord).GetValue();
    int64_t locOffset0 = locCoord.GetValue();
    int64_t locOffset1 = static_cast<const Std::tuple<size_t> &>(locCoord).GetValue();
    ...
    // 把首地址赋值给srcL0C和dstL1数据
    pto::TASSIGN(srcL0C, (uint64_t)src.GetAddr());
    pto::TASSIGN(dstL1, (uint64_t)dst.GetAddr());

    if (dstShape0 < srcShape0 || dstShape1 < srcShape1) {
        TExtractL0CToL1<l1TileData, l0cTileData, V>(dstL1, srcL0C, fixbuf, locOffset0, locOffset1, scaleValue);
    } else {
        TInsertL0CToL1<l1TileData, l0cTileData, V>(dstL1, srcL0C, fixbuf, l1Offset0, l1Offset1, scaleValue);
    }
    return;
}
