...
// ==================== 公共执行辅助（严格按原始顺序：先isExtract，再subblockId） ====================
template <typename TileUB, typename TileL0C>
INLINE void TExtract_Execute(TileUB &ubTile, TileL0C &l0cTile,
    int64_t dstOffset0, int64_t dstOffset1, int64_t srcOffset0, int64_t srcOffset1,
    bool isExtract, int16_t subblockId) {
    if (isExtract) {
        if (subblockId == 0) {
            pto::TEXTRACT<TileUB, TileL0C, pto::AccToVecMode::SingleModeVec0>(
                ubTile, l0cTile, srcOffset0, srcOffset1);
        } else {
            pto::TEXTRACT<TileUB, TileL0C, pto::AccToVecMode::SingleModeVec1>(
                ubTile, l0cTile, srcOffset0, srcOffset1);
        }
    } else {
        if (subblockId == 0) {
            pto::TINSERT<TileUB, TileL0C, pto::AccToVecMode::SingleModeVec0>(
                ubTile, l0cTile, dstOffset0, dstOffset1);
        } else {
            pto::TINSERT<TileUB, TileL0C, pto::AccToVecMode::SingleModeVec1>(
                ubTile, l0cTile, dstOffset0, dstOffset1);
        }
    }
}
...
INLINE void TExtractL0C2UBStatic(T &dst, U &src, const Coord &dstCoord, const Coord &srcCoord, int16_t subblockId) {
    constexpr auto shapeSize = Std::tuple_size<typename T::Shape>::value;
    ...
    bool isExtract = staticDstShape0 < staticSrcShape0 || staticDstShape1 < staticSrcShape1;
    TExtract_Execute(ubTile, l0cTile, dstOffset0, dstOffset1, srcOffset0, srcOffset1, isExtract, subblockId);
}

template <CopyOutMode mode, typename Coord, typename T, typename U>
INLINE void TExtractL0C2UBDynamic(T &dst, U &src, const Coord &dstCoord, const Coord &srcCoord, int16_t subblockId) {
    constexpr auto shapeSize = Std::tuple_size<typename T::Shape>::value;
    constexpr size_t expect_size = Std::tuple_size<Coord>::value;
    int64_t dstOffset0 = TileOp::GetTupleElement<Coord, 0, expect_size, 0>(dstCoord);
    int64_t dstOffset1 = TileOp::GetTupleElement<Coord, 1, expect_size, 0>(dstCoord);
    ...
    pto::TASSIGN(ubTile, (uint64_t)dst.GetAddr());
    pto::TASSIGN(l0cTile, (uint64_t)src.GetAddr());
    bool isExtract = dstShape0 < srcShape0 || dstShape1 < srcShape1;
    TExtract_Execute(ubTile, l0cTile, dstOffset0, dstOffset1, srcOffset0, srcOffset1, isExtract, subblockId);
}
...
TILEOP void TExtract(T &dst, U &src, const Coord &dstCoord, const Coord &srcCoord, int16_t subblockId) {
    ...
    if constexpr (T::FORMAT == Hardware::UB && U::FORMAT == Hardware::L0C) {
        if constexpr (TileOp::IsConstContinous<T, U>()) {
            TExtractL0C2UBStatic<mode, Coord, T, U>(dst, src, dstCoord, srcCoord, subblockId);
        } else {
            TExtractL0C2UBDynamic<mode, Coord, T, U>(dst, src, dstCoord, srcCoord, subblockId);
        }
    }
}
