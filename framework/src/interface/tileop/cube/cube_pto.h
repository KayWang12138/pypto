...
TILEOP void TExtract(T &dst, U &src, const Coord &dstCoord, const Coord &srcCoord, int16_t subblockId) {
    ...
    if constexpr (T::FORMAT == Hardware::UB && U::FORMAT == Hardware::L0C) {
        int64_t dstOffset0 = GetTupleElement<Coord, 0, expect_size, 0>(dstCoord);
        int64_t dstOffset1 = GetTupleElement<Coord, 1, expect_size, 0>(dstCoord);
        int64_t srcOffset0 = GetTupleElement<Coord, 0, expect_size, 0>(srcCoord);
        int64_t srcOffset1 = GetTupleElement<Coord, 1, expect_size, 0>(srcCoord);
