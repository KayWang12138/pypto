...
template <int64_t blockSize, int64_t srcColumnStartOffset, typename DstT, typename SrcT, typename OffsetT, typename BlockT, typename OffsetCoord, typename BlockCoord>
TILEOP void TGatherInL1(DstT dst, SrcT src, OffsetT offset, BlockT block, OffsetCoord offsetCoord, BlockCoord blockCoord)
{
    ...
    for (int64_t i = 0; i < dstShape0; i++) {
        ...
        globalData src0Global((__gm__ typename SrcT::Type *)(src.GetAddr() + gatherOffset * srcShape1 + srcColumnStartOffset),
            pto::Shape<1, 1, 1, -1, -1>(1, staticL1W), pto::Stride<1, 1, 1, -1, -1>(srcStride0, srcStride1));
