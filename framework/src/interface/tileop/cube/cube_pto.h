...
TILEOP void TMoveND2NZ(T &dst, U &src) {
    ...
    using tileNZTensor = pto::Tile<pto::TileType::Vec, typename T::Type, staticVecH, staticVecW, pto::BLayout::ColMajor, -1,
        -1, pto::SLayout::RowMajor>;
...
INLINE void TExtractL1ToL0(T &dst, U &src, const int64_t &offset0, const int64_t &offset1) {
    ...
    using tileL0Tensor =
        std::conditional_t<T::FORMAT == Hardware::L0A, pto::TileLeftCompact<typename T::Type, staticL0H, staticL0W, -1, -1>,
...
INLINE void TStoreExecute(globalData dstGlobal, tileData srcL0C, uint64_t scaleValue) {
    if constexpr (std::is_same<typename tileData::DType, int32_t>::value &&
        ...
        pto::TSTORE<tileData, globalData, config::kIsAcc ? pto::AtomicType::AtomicAdd : pto::AtomicType::AtomicNone>(dstGlobal, srcL0C);
    }
}
