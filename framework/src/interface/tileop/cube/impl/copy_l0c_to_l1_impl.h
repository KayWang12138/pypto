...
#ifndef TILEOP_TILE_OPERATOR_COPY_L0C_TO_L1__H
#define TILEOP_TILE_OPERATOR_COPY_L0C_TO_L1__H
...
template <typename config, typename DstTileData, typename SrcTileData, typename FpTileData>
INLINE void TExtractL0CToL1(
    DstTileData& dstL1, SrcTileData& srcL0C, FpTileData& fixbuf, uint16_t l0cOffset0, uint16_t l0cOffset1,
    uint64_t scaleValue = 0)
{
    constexpr bool isQuant = std::is_same<typename SrcTileData::DType, int32_t>::value &&
                             std::is_same<typename DstTileData::DType, half>::value;
    constexpr bool isNonQuant = std::is_same<typename SrcTileData::DType, typename DstTileData::DType>::value;
    static_assert(isQuant || isNonQuant, "Only int32 to fp16 quantized path is supported.");
    if constexpr (isQuant) {
        // 量化场景，执行L0C->L1随路cast模式
        if (scaleValue != 0) {
            constexpr pto::ReluPreMode relu_mode =
                (config::kReluMode == 0) ? pto::ReluPreMode::NoRelu : pto::ReluPreMode::NormalRelu;
            pto::TEXTRACT<DstTileData, SrcTileData, relu_mode>(dstL1, srcL0C, scaleValue, l0cOffset0, l0cOffset1);
        } else {
            auto scaleData = CreateScaleTileData(fixbuf);
            pto::TASSIGN(scaleData, static_cast<uint64_t>(fixbuf.GetAddr()));
            pto::TEXTRACT_FP<
                DstTileData, SrcTileData, decltype(scaleData),
                config::kReluMode == 0 ? pto::ReluPreMode::NoRelu : pto::ReluPreMode::NormalRelu>(
        ...
        // 量化场景，执行L0C->L1基本模式
        pto::TEXTRACT<
            DstTileData, SrcTileData, config::kReluMode == 0 ? pto::ReluPreMode::NoRelu : pto::ReluPreMode::NormalRelu>(
...
INLINE void TInsertL0CToL1(
    DstTileData& dstL1, SrcTileData& srcL0C, FpTileData& fixbuf, uint16_t l1Offset0, uint16_t l1Offset1,
    uint64_t scaleValue = 0)
{
    constexpr bool isQuant = std::is_same<typename SrcTileData::DType, int32_t>::value &&
                             std::is_same<typename DstTileData::DType, half>::value;
    ...
    if constexpr (isQuant) {
        if (scaleValue != 0) {
            constexpr pto::ReluPreMode relu_mode =
                (config::kReluMode == 0) ? pto::ReluPreMode::NoRelu : pto::ReluPreMode::NormalRelu;
            pto::TINSERT<DstTileData, SrcTileData, relu_mode>(dstL1, srcL0C, scaleValue, l1Offset0, l1Offset1);
        } else {
            auto scaleData = CreateScaleTileData(fixbuf);
            pto::TASSIGN(scaleData, static_cast<uint64_t>(fixbuf.GetAddr()));
            pto::TINSERT_FP<
...
#endif // TILEOP_TILE_OPERATOR_COPY_L0C_TO_L1__H