...
template <typename config, typename l1Data, typename l0cData, typename SrcTileData, typename FpTileData>
...
{
    constexpr bool isQuant = !std::is_same_v<typename SrcTileData::DType, typename FpTileData::DType>;
    ...
    uint64_t scaleValue = 0)
{
    ...
    if (dstShape0 < srcShape0 || dstShape1 < srcShape1) {
        TExtractL0CToL1<config, l1TileData, l0cTileData, SrcTileData, FpTileData>(
            dstL1, srcL0C, fixbuf, l0cOffset0, l0cOffset1, scaleValue);
    } else {
        TInsertL0CToL1<config, l1TileData, l0cTileData, SrcTileData, FpTileData>(
