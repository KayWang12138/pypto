...
{
    constexpr bool isQuant = std::is_same<typename TileData::DType, int32_t>::value &&
                             std::is_same<typename GlobalData::DType, __gm__ half>::value;
    constexpr bool isNonQuant = std::is_same<typename TileData::DType, typename GlobalData::DType>::value;
    static_assert(isQuant || isNonQuant, "Only int32 to fp16 quantized path is supported.");
    if constexpr (isQuant) {
        // 量化场景，执行L0C->GM随路cast模式
        if (scaleValue != 0) {
            pto::TSTORE<
