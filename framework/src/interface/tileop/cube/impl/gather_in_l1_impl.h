...
template <
    int64_t blockSize, typename TileData, typename GlobalData, typename TileData, typename SrcT, typename BlockT,
    typename OffsetT>
INLINE void GatherExecute(
    TileData dst, GlobalData src, BlockT block, OffsetT offset, uint64_t offsetsStartOffset,
    ...
    int64_t c0Size)
{
    for (int64_t i = 0; i < loop; i++) {
        ...
        pto::TASSIGN(dstL1, static_cast<uint64_t>((__cbuf__ typename TileData::Type*)dst.GetAddr() + i * c0Size));
...
INLINE void TGatherInL1Impl(
    TileData dst, SrcT src, BlockT block, OffsetT offset, SrcCoord srcCoord, OffsetCoord offsetCoord,
    BlockCoord blockCoord)
{
    ...
    if (dstShape1 % c0Size > 0) {
        ...
        GatherExecute<blockSize, TileData, GlobalData>(
        ...
        using TileData = pto::Tile<
            pto::TileType::Mat, typename TileData::Type, staticL1W / c0Size, staticL1H * c0Size, pto::BLayout::RowMajor,
            -1, -1>;
