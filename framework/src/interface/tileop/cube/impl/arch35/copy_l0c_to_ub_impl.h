...
template <typename ubTileData, typename l0cTileData, typename FbTileTensor>
TILEOP void TExtractL0CToUB(
    ubTileData& ubTile, l0cTileData& l0cTile, FbTileTensor& fixbuf, int64_t l0cOffset0, int64_t l0cOffset1,
...
TILEOP void TInsertL0CToUB(
    ubTileData& ubTile, l0cTileData& l0cTile, FbTileTensor& fixbuf, int64_t ubOffset0, int64_t ubOffset1,
    int16_t subblockId, uint64_t scaleValue)
{
    constexpr bool supportedQuantMode =
        std::is_same<typename l0cData::DType, int32_t>::value && std::is_same<typename l1Data::DType, half>::value;
...
INLINE void TExtractL0C2UBImpl(DstTileTensor& dst, SrcTileTensor& src, FbTileTensor& fixbuf, const Coord& coord, int16_t subblockId, uint64_t scaleValue = 0)
{
    ...
    constexpr auto staticUBH = Std::tuple_element<shapeSize - SHAPE_DIM2, typename DstTileTensor::TileShape>::type::value;
    ...
    using l0cTileData =
        pto::Tile<pto::TileType::Acc, typename SrcTileTensor::Type, staticL0CH, staticL0CW, pto::BLayout::ColMajor, -1,
    ...
    if (dstShape0 < srcShape0 || dstShape1 < srcShape1) {
        TExtractL0CToUB<config, ubTileData, l0cTileData, FbTileTensor>(ubTile, l0cTile, fixbuf, srcOffset0, srcOffset1, subblockId, scaleValue);
    } else {
        TInsertL0CToUB<config, ubTileData, l0cTileData, FbTileTensor>(ubTile, l0cTile, fixbuf, dstOffset0, dstOffset1, subblockId, scaleValue);
    }
}
