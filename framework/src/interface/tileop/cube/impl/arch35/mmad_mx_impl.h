    ...
    tileL0CTensor& l0c, T0& c, T1& a, T2& aScale, T3& b, T4& bScale, T5 bias, tileBiasTensor biasT)
{
    ...
    if constexpr (hasBias) {
        static_assert(!std::is_same_v<T5, std::nullptr_t>);
...
template <bool isZeroC, bool hasBias, typename T0, typename T1, typename T2, typename T3, typename T4, typename T5 = void*>
INLINE void MatmulMXStatic(T0& c, T1& a, T2& aScale, T3& b, T4& bScale, T5 bias = nullptr)
{
    ...
    static_assert(
        ...
        "[MatmulMX ERROR]: Shape dim size should be 2");
