    ...
    for (const auto &rawOutcast : rawOutcasts) {
        ...
        for (size_t i = 0; i < iOperand.size(); i++) {
            ...
            if (partitalAssemble) {
                ...
                DEFINE_SOURCE_LOCATION();
                FUNCTION_ASSERT(FError::INVALID_OPERATION, !HasNegativeNum(oOperand[i]->oriShape))
                    << "The ioperand of outcast can't be dynamic shape! It's shape: " << oOperand[i]->oriShape;
