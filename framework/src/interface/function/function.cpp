...
{
    ASSERT(FError::INVALID_TYPE, graphType_ == GraphType::BLOCK_GRAPH)
...
{
    ASSERT(FError::INVALID_VAL, explicitArgAddrs_.size() > static_cast<uint64_t>(index))
...
GetTensorDataIODescDict Function::GetTensorDataForTensorGraph()
{
    ...
    for (auto& op : Operations(false)) {
        ...
        ASSERT(FError::INVALID_VAL, getTensorDataIndex != -1)
            << "Failed to get tensor data index for operation";
        ASSERT(FError::NOT_EXIST, currDynAttr->getTensorDataUsageDict.count(this))
        ...
        ASSERT(FError::NOT_EXIST, importDict.count(getTensorDataIndex))
        ...
        if (outcastIndex != INVALID_IOINDEX) {
            ...
            ASSERT(FError::INVALID_VAL, incastIndex != INVALID_IOINDEX)
...
bool HasCalleeConsumer(Function& func, Function& calleeFunc, size_t outcastIdx)
{
    ...
    for (auto otherCallee : func.GetCalleeFunctionList()) {
        ASSERT(FError::INVALID_PTR, otherCallee != nullptr) << func.GetRawName() << "has nullptr callee";
...
void Function::EraseCallOpOpnd(const FunctionHash& calleeHash, size_t index)
{
    for (auto callop : GetCallopList()) {
        ...
        ASSERT(FError::INVALID_PTR, callopAttr != nullptr) << "Processing CallOp:" << callop->Dump();
        if (callopAttr->GetCalleeHash() != calleeHash) {
            continue;
        }
        ASSERT(FError::INVALID_VAL, index < callop->oOperand.size())
            << "Index " << index << " out of bounds for oOperand size " << callop->oOperand.size();
        ASSERT(FError::EINTERNAL, callop->oOpAttrOffset.empty())
            << "oOpAttrOffset is not empty for CallOp:" << callop->Dump();
        ASSERT(FError::EINTERNAL, callopAttr->GetArgList().empty())
            << "ArgList is not empty for CallOp:" << callop->Dump();
        ASSERT(FError::EINTERNAL, callopAttr->GetOutCastIndexToExpr().empty())
    ...
    Function* func, std::map<size_t, size_t>& outcasts)
{
    for (auto calleeFunc : func->GetCalleeFunctionList()) {
        ASSERT(FError::INVALID_PTR, calleeFunc != nullptr) << func->GetMagicName() << "has nullptr calleeFunc";
        ...
        for (auto& [outCastIdx, val] : outcastIdx2parent) {
            (void)val;
            ASSERT(FError::INVALID_VAL, calleeOutCasts[outCastIdx].get() != nullptr)
            ...
            if (calleeOutCasts[outCastIdx]->IsGetTensorDataOutcast()) {
                ...
                ASSERT(FError::INVALID_VAL, outcastIdx2parent.count(outCastIdx) > 0)
...
void Function::CleanRedundantOutCast()
{
    ...
    for (auto& [outCastIdx, val] : outputMap) {
        ...
        if (getTensorDataRecord[this].count(outCastIdx) > 0) {
            ASSERT(FError::NOT_EXIST, outputMap.count(outCastIdx) > 0)
    ...
    for (auto& op : operationList) {
        ...
        for (auto& oOperand : op->oOperand) {
            ...
            if (shouldAddOutcast && outcasts.Insert(oOperand)) {
                ...
                ASSERT(FError::EINTERNAL, incasts.count(oOperand) == 0)
...
FunctionCallArgs Function::EndFunction(const std::shared_ptr<TensorSlotScope>& scope)
{
    ...
    if (IsGraphType(GraphType::TENSOR_GRAPH) ||
        ...
        ASSERT(FError::INVALID_TYPE, false) << "Not support connecting other type of function currently";
    }
...
void Function::OperationLoopCheck(const std::string& errorMsg)
{
    ...
    for (auto&& op : operations_) {
        ...
        ASSERT(FError::EINTERNAL, !cycleDetection(op.get(), cycleDetection)) << errorMsg;
    }
}
    ...
    for (const auto& opPtr : operations) {
        ...
        for (auto&& operand : op.GetIOperands()) {
            ...
            for (auto& consumer : operand->GetConsumers()) {
                ...
                ASSERT(FError::NOT_EXIST, opToSubgraph.find(magic) != opToSubgraph.end())
...
auto Function::AnnotateOperation()
{
    ...
    for (auto&& op : Operations()) {
        ...
        ASSERT(FError::IS_EXIST, opToSubgraph.find(op.GetOpMagic()) == opToSubgraph.end())
            << "Same op magic shall only appear once."
            << "\n"
...
std::vector<std::shared_ptr<Operation>> Function::GetSortedOperations() const
{
    ...
    for (size_t idx = 0; idx < operations_.size(); idx++) {
        ...
        ASSERT(FError::IS_EXIST, opToIndex.count(op) == 0) << "Duplicate operation found: " << op->Dump();
    ...
    auto addProd = [&](auto operation, auto ioperand) {
        for (const auto& prod : ioperand->GetProducers()) {
            ...
            ASSERT(FError::NOT_EXIST, opToIndex.count(prod) != 0)
    ...
    for (auto& op : operations_) {
        ASSERT(FError::EINTERNAL, outDegree[opToIndex[op.get()]] == 0) << "cycle detected: " << op->Dump();
    }
    ASSERT(FError::EINTERNAL, operations_.size() == sortedOperations.size())
...
void Function::ScheduleBy(const std::vector<Operation*>& newList, bool needRefresh)
{
    ...
    ASSERT(FError::EINTERNAL, newList.size() == operations_.size())
    ...
    for (auto op : newList) {
        ASSERT(FError::NOT_EXIST, opPosition_.count(op) > 0)
...
void Function::AddOperationGroup(std::vector<Operation*> operationGroup)
{
    ...
    for (const auto& operation : operationGroup) {
        ASSERT(FError::IS_EXIST, operation->GroupID() == NON_GROUP)
...
void Function::CheckGroupValid() const
{
    ...
    for (size_t i = 0; i < operationGroups_.size(); i++) {
        for (auto& operation : operationGroups_[i]) {
            ASSERT(FError::EINTERNAL, operation->GroupID() == i)
                << "Operation GroupID mismatch:\n"
                << "Expected: " << i << ", Actual: " << operation->GroupID() << "\n"
                << "Operation:" << operation->Dump();
            ASSERT(FError::IS_EXIST, inGroupOp.count(operation) == 0)
                << "Duplicate operation in group:" << operation->Dump();
            inGroupOp.emplace(operation);
        }
    }
    for (const auto& operation : operations_) {
        ASSERT(FError::IS_EXIST, inGroupOp.count(operation.get()) == (operation->GroupID() != NON_GROUP))
...
void Function::RefreshOpPosition()
{
    opPosition_.clear();
    for (size_t idx = 0; idx < operations_.size(); ++idx) {
        ASSERT(FError::NOT_EXIST, opPosition_.count(operations_[idx].get()) == 0)
...
void Function::EraseOperations(bool eraseRelatedTensor, bool sorted)
{
    ...
    for (auto& op : operations_) {
        ...
        ASSERT(FError::EINTERNAL, op->IsDeleted()) << "Operation not marked as deleted:" << op->Dump();
    ...
    const Opcode opCode, LogicalTensors iOperands, const LogicalTensors& oOperands, const bool updateTensorMap)
{
    ...
    for (auto& iOperand : iOperands) {
        ASSERT(FError::INVALID_VAL, iOperand->shape.size() != 0) << "tensor shape size invalid";
...
void Function::UpdateTensorDataUsage(Operation& op)
{
    ...
    for (auto& [index, callList] : dict) {
        (void)callList;
        ASSERT(FError::INVALID_VAL, descDict.count(index)) << "Invalid index" << op.Dump();
...
{
    ASSERT(FError::INVALID_TYPE, operand->Datatype() == dst->Datatype()) << "Check Dtype failed!";
...
std::vector<Function*> Function::GetCalleeFunctionList() const
{
    ...
    for (auto callop : callopList) {
        ...
        ASSERT(FError::NOT_EXIST, calleeFunc) << callopAttr->GetCalleeMagicName() << " is not in functionmap!";
    ...
    for (auto& operation : operations_) {
        ...
        for (size_t i = 0; i < cur.GetInputOperandSize(); i++) {
            ...
            if (replaced.count(inputTensor) == 0) {
                ASSERT(FError::EINTERNAL, inputTensor->HasConsumer(cur))
    ...
    for (auto& operation : operations_) {
        ...
        for (size_t i = 0; i < cur.GetOOperands().size(); i++) {
            if (cur.GetOOperands()[i] == oldTensor) {
                ASSERT(FError::INVALID_VAL, cur.GetOOperands()[i]->shape == newTensor->shape)
                    << "Shape mismatch:\n"
                    << "Old Tensor Shape: " << StringUtils::ToString(cur.GetOOperands()[i]->shape) << "\n"
                    << "New Tensor Shape: " << StringUtils::ToString(newTensor->shape) << "\n"
                    << "Operation: " << cur.Dump();
                ASSERT(FError::EINTERNAL, cur.GetOOperands()[i]->HasProducer(cur))
...
void Function::RemoveOriginIncastConsumer(const std::shared_ptr<LogicalTensor>& originIncast) const
{
    ...
    for (const auto& producer : originIncast->GetProducers()) {
        ...
        ASSERT(FError::INVALID_PTR, targetFunc != nullptr)
    ...
    const std::shared_ptr<LogicalTensor>& newIncast, const std::shared_ptr<LogicalTensor>& originIncast)
{
    ...
    ASSERT(FError::NOT_EXIST, it != originInCasts_.end()) << "OriginIncast not found in originInCasts_:\n"
...
LogicalTensors Function::MakeIncasts(const std::shared_ptr<TensorSlotScope>& scope)
{
    ...
    ASSERT(FError::EINTERNAL, HasParent()) << "Function does not have a parent";
    ...
    for (auto& rawIncast : rawIncasts) {
        ...
        for (auto& originIncast : sameRawIncasts) {
            ...
            if (newincastMap.count(viewKey) != 0) {
                ...
                ASSERT(FError::EINTERNAL, originIncast->conflicterTensors.empty())
            ...
            for (const auto& producer : inArgument->GetProducers()) {
                ASSERT(FError::EINTERNAL, producer->BelongTo() != this)
            ...
            for (const auto& consumer : inArgument->GetConsumers()) {
                ASSERT(FError::EINTERNAL, consumer->BelongTo() != this)
...
LogicalTensors Function::MakeOutcasts(const std::shared_ptr<TensorSlotScope>& scope)
{
        ...
        for (size_t i = 0; i < iOperand.size(); i++) {
            ...
            if (partitalAssemble) {
                for (auto producer : producerSet) {
                    ...
                    ASSERT(FError::INVALID_PTR, producerAttr)
    ...
    for (const auto& tensor : outArgumentList) {
        ASSERT(FError::NOT_EXIST, tensor->GetProducers().empty()) << "Tensor has producers:" << tensor->Dump();
    }
...
{
    ASSERT(FError::INVALID_VAL, funcJson[T_FIELD_KIND].get<int>() == static_cast<int>(Kind::T_KIND_FUNCTION))
...
TensorGraphInfo Function::GetGraphInfo()
{
    ...
    for (auto& op : operations_) {
        ...
        ASSERT(FError::INVALID_VAL, op->GetOpcode() == Opcode::OP_CALL)
...
{
    ASSERT(FError::INVALID_PTR, slotScope_ != nullptr) << "slotScope_ is null";
...
std::vector<int> Function::GetInCastSlot(const std::shared_ptr<LogicalTensor>& incast)
{
    ...
    for (size_t idx = 0; idx < inCasts_.size(); ++idx) {
        if (inCasts_[idx] == incast) {
            ...
            ASSERT(FError::INVALID_PTR, scope != nullptr) << "SlotScope is null";
...
std::vector<OriArgInfo> Function::GetOpOriginArgsInfo()
{
    ...
    for (const auto& incast : inCasts_) {
        ...
        if (args.count(subscript) > 0) {
            ASSERT(FError::EINTERNAL, args.at(subscript) == info)
...
void Function::OpValidCheck(Operation& op) const
{
    ...
    if (SPECIAL_OPCODE_SET.count(op.GetOpcode()) != 0) {
        if (op.GetOpcode() == Opcode::OP_VIEW) {
            ASSERT(FError::OUT_OF_RANGE, op.GetIOperands().size() == 1)
                << "OP_VIEW expects 1 input operand, but got " << op.GetIOperands().size();
            ASSERT(FError::OUT_OF_RANGE, op.GetOOperands().size() <= 1)
            ...
            ASSERT(FError::INVALID_PTR, opAttr != nullptr)
        ...
        ASSERT(FError::INVALID_PTR, op.GetOpAttribute() == nullptr)
            << "Non-special operation should not have an operation attribute";
    }

    ASSERT(FError::OUT_OF_RANGE, op.GetOpMagic() >= 0 && op.GetOpMagic() < opSeed_)
    ...
    for (auto& oOperand : op.GetOOperands()) {
        ASSERT(FError::INVALID_VAL, oOperand->GetShape().size() == oOperand->GetOffset().size())
            << "Output operand shape size does not match offset size";
        ASSERT(FError::EINTERNAL, &oOperand->BelongFunction() == this)
            << "Output operand does not belong to current function";
        auto tmp = GetTensorMap().GetTensorByMagic(oOperand->magic);
        ASSERT(FError::NOT_EXIST, tmp == oOperand) << "Tensor map does not match output operand";
        ASSERT(FError::EINTERNAL, oOperand->HasProducer(op))
            << "Output operand does not have current operation as a producer, opmagic: " << op.GetOpMagic()
            << ", operand magic: " << oOperand->magic;
    }
    for (auto& iOperand : op.GetIOperands()) {
        ASSERT(FError::INVALID_VAL, iOperand->GetShape().size() == iOperand->GetOffset().size())
            << "Input operand shape size does not match offset size";
        ASSERT(FError::EINTERNAL, &iOperand->BelongFunction() == this)
        ...
        if (!iOperand->GetProducers().empty() || incasts.count(iOperand) != 0) {
            ...
            ASSERT(FError::NOT_EXIST, tmp == iOperand) << "Tensor map does not match input operand";
        }

        ASSERT(FError::EINTERNAL, iOperand->HasConsumer(op))
            << "Input operand does not have current operation as a consumer";
        for (const auto& producer : iOperand->GetProducers()) {
            ASSERT(FError::EINTERNAL, producer->BelongTo() == this)
                << "Producer does not belong to current function";
            ASSERT(FError::OUT_OF_RANGE, producer->GetOpMagic() >= 0 && producer->GetOpMagic() < opSeed_)
            ...
            ASSERT(FError::NOT_EXIST, opMap.find(producer) != opMap.end())
                << "Producer not found in operation map";
        }
    }

    ASSERT(FError::IS_EXIST, opMap.count(&op) == 0) << "Operation is already in the operation map";
    opMap.emplace(&op);
}
...
void Function::ValidCheck() const
{
    ...
    ASSERT(FError::OUT_OF_RANGE, opMagic + 1 <= opSeed_)
        ...
        for (const auto& operand : op.GetOOperands()) {
            if (used.count(operand) > 0) {
                for (auto innerOp : used.at(operand)) {
                    ASSERT(FError::IS_EXIST, innerOp->ComputeHash() != op.ComputeHash())
...
std::shared_ptr<LogicalTensor> Function::ConnectWithOverlap(std::shared_ptr<LogicalTensor> iOperand)
{
    ...
    ASSERT(FError::EINTERNAL, !matches.empty()) << "Matches should not be empty";
    ...
    switch (overlapStatus) {
        case OverlapStatus::PERFECTLY_MATCH_WITH_ALL: {
            ...
            ASSERT(FError::NOT_EXIST, assembleResult->GetProducers().empty())
        ...
        default:
            ASSERT(FError::EINTERNAL, false) << "unexpected behavior";
    }
...
void Function::DoMergeFunctionDupIncast()
{
    ...
    for (auto& pair : sameSlotSetIndex) {
        ...
        ASSERT(FError::INVALID_VAL, slotSetIndex.size() > 1)
        ...
        for (auto incastIdx : slotSetIndex) {
            ASSERT(FError::NOT_EXIST, inCasts_[incastIdx]->GetConsumers().size() > 0)
...
void Function::DoMergeFunctionDupOutcast()
{
    ...
    for (auto& pair : sameSlotSetIndex) {
        ...
        for (auto incastIdx : slotSetIndex) {
            ASSERT(FError::NOT_EXIST, outCasts_[incastIdx]->GetProducers().size() > 0)
