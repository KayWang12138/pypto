...
std::vector<OperationPtr> &Function::GetProgramOp() {
    ASSERT(ERROR_CODE_UNKNOWN, graphType_ == GraphType::BLOCK_GRAPH)
...
const SubfuncInvokeInfoTy &Function::GetSubFuncInvokeInfo(const size_t i) const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, callAttr != nullptr)
...
void *Function::GetParamAddress(int index) {
    ASSERT(ERROR_CODE_UNKNOWN, explicitArgAddrs_.size() > static_cast<uint64_t>(index)) << "The param address is not stored.";
...
GetTensorDataIODescDict Function::GetTensorDataForTensorGraph() {
    ...
    for (auto &op : Operations(false)) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, getTensorDataIndex != -1)
            << "Failed to get tensor data index for operation";
        ASSERT(ERROR_CODE_UNKNOWN, currDynAttr->getTensorDataUsageDict.count(this))
        ...
        ASSERT(ERROR_CODE_UNKNOWN, importDict.count(getTensorDataIndex))
        ...
        if (outcastIndex != INVALID_IOINDEX) {
            ...
            ASSERT(ERROR_CODE_UNKNOWN, incastIndex != INVALID_IOINDEX) << "Both outcast and incast indices are invalid";
...
bool HasCalleeConsumer(Function &func, Function &calleeFunc, size_t outcastIdx) {
    ...
    for (auto otherCallee : func.GetCalleeFunctionList()) {
        ASSERT(ERROR_CODE_UNKNOWN, otherCallee != nullptr) << func.GetRawName() << "has nullptr callee";
...
void Function::EraseCallOpOpnd(const FunctionHash &calleeHash, size_t index) {
    for (auto callop : GetCallopList()) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, callopAttr != nullptr) << "Processing CallOp:" << callop->Dump();
        if (callopAttr->GetCalleeHash() != calleeHash) {
            continue;
        }
        ASSERT(ERROR_CODE_UNKNOWN, index < callop->oOperand.size())
            << "Index " << index
            << " out of bounds for oOperand size " << callop->oOperand.size();
        ASSERT(ERROR_CODE_UNKNOWN, callop->oOpAttrOffset.empty())
            << "oOpAttrOffset is not empty for CallOp:"
            << callop->Dump();
        ASSERT(ERROR_CODE_UNKNOWN, callopAttr->GetArgList().empty())
            << "ArgList is not empty for CallOp:"
            << callop->Dump();
        ASSERT(ERROR_CODE_UNKNOWN, callopAttr->GetOutCastIndexToExpr().empty())
...
void RedundantOutCastCheck(std::map<Function *, std::set<size_t>> &removeRecord,
    ...
    for (auto calleeFunc : func->GetCalleeFunctionList()) {
        ASSERT(ERROR_CODE_UNKNOWN, calleeFunc != nullptr) << func->GetMagicName() << "has nullptr calleeFunc";
        ...
        for (auto &[outCastIdx, val] : outcastIdx2parent) {
            (void)val;
            ASSERT(ERROR_CODE_UNKNOWN, calleeOutCasts[outCastIdx].get() != nullptr)
            ...
            if (calleeOutCasts[outCastIdx]->IsGetTensorDataOutcast()) {
                ...
                ASSERT(ERROR_CODE_UNKNOWN, outcastIdx2parent.count(outCastIdx) > 0)
...
void Function::CleanRedundantOutCast() {
    ...
    for (auto &[outCastIdx, val] : outputMap) {
        ...
        if (getTensorDataRecord[this].count(outCastIdx) > 0) {
            ASSERT(ERROR_CODE_UNKNOWN, outputMap.count(outCastIdx) > 0)
    ...
    for (auto &op : operationList) {
        ...
        for (auto &oOperand : op->oOperand) {
            ...
            if (shouldAddOutcast && outcasts.Insert(oOperand)) {
                ...
                ASSERT(ERROR_CODE_UNKNOWN, incasts.count(oOperand) == 0)
...
FunctionCallArgs Function::EndFunction(const std::shared_ptr<TensorSlotScope> &scope) {
    ...
    if (IsGraphType(GraphType::TENSOR_GRAPH) ||
        ...
        ASSERT(ERROR_CODE_UNKNOWN, false) << "Not support connecting other type of function currently";
    }
...
void Function::OperationLoopCheck(const std::string &errorMsg) {
    ...
    for (auto &&op : operations_) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, !cycleDetection(op.get(), cycleDetection)) << errorMsg;
    }
}
    ...
    for (const auto &opPtr : operations) {
        ...
        for (auto &&operand : op.GetIOperands()) {
            ...
            for (auto &consumer : operand->GetConsumers()) {
                ...
                ASSERT(ERROR_CODE_UNKNOWN, opToSubgraph.find(magic) != opToSubgraph.end())
...
auto Function::AnnotateOperation() {
    ...
    for (auto &&op : Operations()) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, opToSubgraph.find(op.GetOpMagic()) == opToSubgraph.end())
                        << "Same op magic shall only appear once." << "\n"
...
std::vector<std::shared_ptr<Operation>> Function::GetSortedOperations() const {
    ...
    for (size_t idx = 0; idx < operations_.size(); idx++) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, opToIndex.count(op) == 0)
    ...
    auto addProd = [&] (auto operation, auto ioperand) {
        for (const auto &prod : ioperand->GetProducers()) {
            ...
            ASSERT(ERROR_CODE_UNKNOWN, opToIndex.count(prod) != 0)
    ...
    for (auto &op : operations_) {
        ASSERT(ERROR_CODE_UNKNOWN, outDegree[opToIndex[op.get()]] == 0)
            << "cycle detected: " << op->Dump();
    }
    ASSERT(ERROR_CODE_UNKNOWN, operations_.size() == sortedOperations.size())
...
void Function::ScheduleBy(const std::vector<Operation *> &newList, bool needRefresh) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, newList.size() == operations_.size())
    ...
    for (auto op : newList) {
        ASSERT(ERROR_CODE_UNKNOWN, opPosition_.count(op) > 0)
...
void Function::AddOperationGroup(std::vector<Operation *> operationGroup) {
    ...
    for (const auto &operation : operationGroup) {
        ASSERT(ERROR_CODE_UNKNOWN, operation->GroupID() == NON_GROUP)
...
void Function::CheckGroupValid() const {
    ...
    for (size_t i = 0; i < operationGroups_.size(); i++) {
        for (auto &operation : operationGroups_[i]) {
            ASSERT(ERROR_CODE_UNKNOWN, operation->GroupID() == i)
                << "Operation GroupID mismatch:\n" << "Expected: " << i << ", Actual: " << operation->GroupID() << "\n"
                << "Operation:" << operation->Dump();
            ASSERT(ERROR_CODE_UNKNOWN, inGroupOp.count(operation) == 0) << "Duplicate operation in group:" << operation->Dump();
            inGroupOp.emplace(operation);
        }
    }
    for (const auto &operation : operations_) {
        ASSERT(ERROR_CODE_UNKNOWN, inGroupOp.count(operation.get()) == (operation->GroupID() != NON_GROUP))
...
void Function::RefreshOpPosition() {
    opPosition_.clear();
    for (size_t idx = 0; idx < operations_.size(); ++idx) {
        ASSERT(ERROR_CODE_UNKNOWN, opPosition_.count(operations_[idx].get()) == 0)
...
void Function::EraseOperations(bool eraseRelatedTensor, bool sorted) {
    ...
    for (auto &op : operations_) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, op->IsDeleted())
...
Operation &Function::AddOperation(const Opcode opCode, LogicalTensors iOperands,
    ...
    for (auto &iOperand : iOperands) {
        ASSERT(ERROR_CODE_UNKNOWN, iOperand->shape.size() != 0) << "tensor shape size invalid";
...
void Function::UpdateTensorDataUsage(Operation &op) {
    ...
    for (auto &[index, callList] : dict) {
        (void)callList;
        ASSERT(ERROR_CODE_UNKNOWN, descDict.count(index)) << "Invalid index" << op.Dump();
...
void Function::SetSameMemId(const LogicalTensorPtr &operand, LogicalTensorPtr &dst) {
    ASSERT(ERROR_CODE_UNKNOWN, operand->Datatype() == dst->Datatype()) << "Check Dtype failed!";
...
std::vector<Function *> Function::GetCalleeFunctionList() const {
    ...
    for (auto callop : callopList) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, calleeFunc) << callopAttr->GetCalleeMagicName() << " is not in functionmap!";
    ...
    for (auto &operation : operations_) {
        ...
        for (size_t i = 0; i < cur.GetInputOperandSize(); i++) {
            ...
            if (replaced.count(inputTensor) == 0) {
                ASSERT(ERROR_CODE_UNKNOWN, inputTensor->HasConsumer(cur))
    ...
    for (auto &operation : operations_) {
        ...
        for (size_t i = 0; i < cur.GetOOperands().size(); i++) {
            if (cur.GetOOperands()[i] == oldTensor) {
                ASSERT(ERROR_CODE_UNKNOWN, cur.GetOOperands()[i]->shape == newTensor->shape)
                    << "Shape mismatch:\n" << "Old Tensor Shape: "
                    << StringUtils::ToString(cur.GetOOperands()[i]->shape) << "\n"
                    << "New Tensor Shape: "
                    << StringUtils::ToString(newTensor->shape) << "\n"
                    << "Operation: " << cur.Dump();
                ASSERT(ERROR_CODE_UNKNOWN, cur.GetOOperands()[i]->HasProducer(cur))
...
void Function::RemoveOriginIncastConsumer(const std::shared_ptr<LogicalTensor> &originIncast) const {
    ...
    for (const auto &producer : originIncast->GetProducers()) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, targetFunc != nullptr)
...
void Function::ReplaceMaybeParams(const std::shared_ptr<LogicalTensor> &newIncast,
    ...
    ASSERT(ERROR_CODE_UNKNOWN, it != originInCasts_.end())
...
LogicalTensors Function::MakeIncasts(const std::shared_ptr<TensorSlotScope> &scope) {
    ASSERT(ERROR_CODE_UNKNOWN, IsGraphType(GraphType::TENSOR_GRAPH) || IsFunctionTypeAndGraphType(FunctionType::STATIC, GraphType::TILE_GRAPH))
        << "Invalid function type or graph type";
    ASSERT(ERROR_CODE_UNKNOWN, HasParent())
    ...
    for (auto &rawIncast : rawIncasts) {
        ...
        for (auto &originIncast : sameRawIncasts) {
            ...
            if (newincastMap.count(viewKey) != 0) {
                ...
                ASSERT(ERROR_CODE_UNKNOWN, originIncast->conflicterTensors.empty())
            ...
            for (const auto &producer : inArgument->GetProducers()) {
                ASSERT(ERROR_CODE_UNKNOWN, producer->BelongTo() != this)
            ...
            for (const auto &consumer : inArgument->GetConsumers()) {
                ASSERT(ERROR_CODE_UNKNOWN, consumer->BelongTo() != this)
...
LogicalTensors Function::MakeOutcasts(const std::shared_ptr<TensorSlotScope> &scope) {
        ...
        for (size_t i = 0; i < iOperand.size(); i++) {
            ...
            if (partitalAssemble) {
                for (auto producer : producerSet) {
                    ...
                    ASSERT(ERROR_CODE_UNKNOWN, producerAttr) << "mix assemble and common operation for same output \n" << producer->Dump();
    ...
    for (const auto &tensor : outArgumentList) {
        ASSERT(ERROR_CODE_UNKNOWN, tensor->GetProducers().empty())
...
void Function::DumpJsonFile(std::string fileName) {
    ...
    CHECK(ERROR_CODE_UNKNOWN, file.is_open()) << "Failed to open file: " << filePath;
    Json progDump;
...
std::shared_ptr<Function> Function::LoadJson(Program &belongTo, const Json &funcJson) {
    ASSERT(ERROR_CODE_UNKNOWN, funcJson[T_FIELD_KIND].get<int>() == static_cast<int>(Kind::T_KIND_FUNCTION))
...
void Function::DumpFile(const std::string &filePath) const {
    ...
    CHECK(ERROR_CODE_UNKNOWN, fout.is_open()) << "Failed to open file: " << filePath;
    fout << Dump();
    fout.close();
}
...
TensorGraphInfo Function::GetGraphInfo() {
    ...
    for (auto &op : operations_) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, op->GetOpcode() == Opcode::OP_CALL)
...
void Function::UpdateOriIocastSlot(const std::shared_ptr<TensorSlotScope> scope) {
    ASSERT(ERROR_CODE_UNKNOWN, slotScope_ != nullptr) << "slotScope_ is null";
...
std::vector<int> Function::GetInCastSlot(const std::shared_ptr<LogicalTensor> &incast) {
    ...
    for (size_t idx = 0; idx < inCasts_.size(); ++idx) {
        if (inCasts_[idx] == incast) {
            ...
            ASSERT(ERROR_CODE_UNKNOWN, scope != nullptr) << "SlotScope is null";
...
std::vector<OriArgInfo> Function::GetOpOriginArgsInfo() {
    ...
    for (const auto &incast : inCasts_) {
        ...
        if (args.count(subscript) > 0) {
            ASSERT(ERROR_CODE_UNKNOWN, args.at(subscript) == info)
...
void Function::OpValidCheck(Operation &op) const {
    ...
    if (SPECIAL_OPCODE_SET.count(op.GetOpcode()) != 0) {
        if (op.GetOpcode() == Opcode::OP_VIEW) {
            ASSERT(ERROR_CODE_UNKNOWN, op.GetIOperands().size() == 1) << "OP_VIEW expects 1 input operand, but got " << op.GetIOperands().size();
            ASSERT(ERROR_CODE_UNKNOWN, op.GetOOperands().size() <= 1) << "OP_VIEW expects at most 1 output operand, but got " << op.GetOOperands().size();
            auto opAttr = std::dynamic_pointer_cast<ViewOpAttribute>(op.GetOpAttribute());
            ASSERT(ERROR_CODE_UNKNOWN, opAttr != nullptr)
                << "OP_VIEW should have a ViewOpAttribute, but it is null";
            ASSERT(ERROR_CODE_UNKNOWN, op.GetIOperands()[0]->GetOffset().size() == opAttr->GetFromOffset().size())
                << "OP_VIEW input operand offset size does not match attribute from offset size";
            if (!op.GetOOperands().empty()) {
                ASSERT(ERROR_CODE_UNKNOWN, op.GetOOperands()[0]->GetOffset().size() == opAttr->GetFromOffset().size())
                    << "OP_VIEW output operand offset size does not match attribute from offset size";
            }
        }
        if (op.GetOpcode() == Opcode::OP_ASSEMBLE) {
            ASSERT(ERROR_CODE_UNKNOWN, op.GetIOperands().size() == 1)
                << "OP_ASSEMBLE should have exactly 1 input operand, but has " << op.GetIOperands().size();
            ASSERT(ERROR_CODE_UNKNOWN, op.GetOOperands().size() <= 1)
            ...
            if (!op.GetIOperands().empty()) {
                ASSERT(ERROR_CODE_UNKNOWN, op.GetIOperands()[0]->GetOffset().size() == opAttr->GetToOffset().size())
                    << "OP_ASSEMBLE input operand offset size does not match attribute to offset size";
            }
            ASSERT(ERROR_CODE_UNKNOWN, op.GetOOperands()[0]->GetOffset().size() == opAttr->GetToOffset().size())
                << "OP_ASSEMBLE output operand offset size does not match attribute to offset size";
        }
    } else {
        ASSERT(ERROR_CODE_UNKNOWN, op.GetOpAttribute() == nullptr)
            << "Non-special operation should not have an operation attribute";
    }

    ASSERT(ERROR_CODE_UNKNOWN, op.GetOpMagic() >= 0 && op.GetOpMagic() < opSeed_)
    ...
    for (auto &iOperand : op.GetIOperands()) {
        ...
        if (!iOperand->GetProducers().empty() || incasts.count(iOperand) != 0) {
            ...
            ASSERT(ERROR_CODE_UNKNOWN, tmp == iOperand)
                << "Tensor map does not match input operand";
        }

        ASSERT(ERROR_CODE_UNKNOWN, iOperand->HasConsumer(op))
            << "Input operand does not have the current operation as a consumer";
        for (const auto &producer : iOperand->GetProducers()) {
            ASSERT(ERROR_CODE_UNKNOWN, producer->BelongTo() == this)
                << "Producer does not belong to the current function";
            ASSERT(ERROR_CODE_UNKNOWN, producer->GetOpMagic() >= 0 && producer->GetOpMagic() < opSeed_)
            ...
            ASSERT(ERROR_CODE_UNKNOWN, opMap.find(producer) != opMap.end())
                << "Producer not found in operation map";
        }
    }

    ASSERT(ERROR_CODE_UNKNOWN, opMap.count(&op) == 0)
...
void Function::ValidCheck() const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, opMagic + 1 <= opSeed_)
        ...
        for (const auto &operand : op.GetOOperands()) {
            if (used.count(operand) > 0) {
                for (auto innerOp : used.at(operand)) {
                    ASSERT(ERROR_CODE_UNKNOWN, innerOp->ComputeHash() != op.ComputeHash()) << "Duplicate operation detected with the same hash: " << op.ComputeHash();
                }
            }
...
std::shared_ptr<LogicalTensor> Function::ConnectWithOverlap(std::shared_ptr<LogicalTensor> iOperand) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, !matches.empty())
    ...
    switch (overlapStatus) {
        case OverlapStatus::PERFECTLY_MATCH_WITH_ALL: {
            auto assembleResult = std::make_shared<LogicalTensor>(*this, iOperand->Datatype(), iOperand->shape,
                ...
                iOperand->nodetype);
            ASSERT(ERROR_CODE_UNKNOWN, assembleResult->GetProducers().empty()) << "Assemble result should have no producers";
        ...
        default: ASSERT(ERROR_CODE_UNKNOWN, false) << "unexpected behavior";
    }

    ASSERT(ERROR_CODE_UNKNOWN, false) << "unexpected behavior";
    return nullptr;
}
...
void Function::DoMergeFunctionDupIncast() {
    ...
    for (auto& pair : sameSlotSetIndex) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, slotSetIndex.size() > 1)
        ...
        for (auto incastIdx : slotSetIndex) {
            ASSERT(ERROR_CODE_UNKNOWN, inCasts_[incastIdx]->GetConsumers().size() > 0)
...
void Function::DoMergeFunctionDupOutcast() {
    ...
    for (auto& pair : sameSlotSetIndex) {
        ...
        for (auto incastIdx : slotSetIndex) {
            ASSERT(ERROR_CODE_UNKNOWN, outCasts_[incastIdx]->GetProducers().size() > 0)
