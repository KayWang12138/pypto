...
public:
    ...
    public:
        ...
        void operator++()
        {
            F_ASSERT(cur_ <= operations_.size())
...
public:
    ...
    [[nodiscard]] int GetOpPosition(const Operation& op) const
    {
        ...
        if (it == opPosition_.end()) {
            F_ASSERT(FError::NOT_EXIST, false) << "Magic[" << op.opmagic << "] Op has not been found in opPosition.";
            return 0;
        }
        return it->second;
    }
    ...
    [[nodiscard]] std::pair<int, bool> FindOpPosition(const Operation& op) const
    {
        ...
        F_ASSERT(operations_[it->second].get() == &op)
...
public:
    ...
    {
        F_ASSERT(groupID < operationGroups_.size())
