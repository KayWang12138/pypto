...
public:
    ...
    public:
        ...
        void operator++() {
            ASSERT(ERROR_CODE_UNDEFINED, cur_ <= operations_.size())
...
public:
    ...
    [[nodiscard]]int GetOpPosition(const Operation &op) const {
        ...
        if (it == opPosition_.end()) {
            ASSERT(ERROR_CODE_UNDEFINED, false) << "Magic[" << op.opmagic << "] Op has not been found in opPosition.";
            return 0;
        }
        return it->second;
    }
    ...
    [[nodiscard]] std::pair<int, bool> FindOpPosition(const Operation &op) const {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, operations_[it->second].get() == &op)
...
public:
    ...
    const auto &GetGroupByID(const size_t groupID) const {
        ASSERT(ERROR_CODE_UNDEFINED, groupID < operationGroups_.size())
