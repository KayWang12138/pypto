...
#define RuntimeGetInputDataInt32Dim3(input, off0, off1, off2)                               \
    (((int32_t *)(input)->address)[(off0) * (input)->shape.dim[1] * (input)->shape.dim[2] + \
                                   (off1) * (input)->shape.dim[2] + (off2)])
#define RuntimeGetInputDataInt32Dim4(input, off0, off1, off2, off3)                                               \
    (((int32_t *)(input)->address)[(((off0) * (input)->shape.dim[1] + (off1)) * (input)->shape.dim[2] + (off2)) * \
                                       (input)->shape.dim[3] +                                                    \
                                   (off3)])
