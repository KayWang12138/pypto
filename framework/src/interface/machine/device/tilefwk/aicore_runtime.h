...
#define RT_DECL_TYPE_TILE(name, primType, space, dim, ...) using name = TileTensor<RT_##primType, LocalLayout##dim##Dim<__VA_ARGS__>, mem>;
...
#define RT_INIT_ADDR(name, primType, space, addr, size) RT_##primType *name = (RT_##primType *)get_imm(addr); \
                                                RT_##primType RT_##space *name = (RT_##primType RT_##space *)get_imm(addr);
...
#define RT_INIT_VALUE_TENSOR(name, defType, addr, dim, ...) name = defType(addr, RT_INIT_VALUE_TENSOR_##dim(__VA_ARGS__));
