...
INLINE uint32_t GetNextLeafTask(ExecuteContext *ctx, uint32_t lastTaskIdx)
{
    ...
    int64_t ffts_addr, int64_t inputs, int64_t outputs, int64_t workspace, int64_t tilingdata, int64_t cfgdata)
{
    ...
    while (true) {
        ++loop_count;
        if ((loop_count % 1000 == 0) && (get_sys_cnt() - t0 > AICORE_LEAF_TASK_RUN_TIMEOUT)) {
            break;
        }

        curTaskIdx = GetNextLeafTask(&ctx, lastTaskIdx);
