...
INLINE ValT SafeExtractValue(const ElemT& dataElem)
{
    ElemT tmp = dataElem;
    return SafeBitCast<ValT>(tmp);
}

template <typename ElemT, NodeTy NodeType, typename ValT>
INLINE void EncodeIndexedElement(AicoreLogger* logger, int64_t i, const ElemT& dataElem)
{
    ValT v = SafeExtractValue<ElemT, ValT>(dataElem);
    logger->EncodeIndexed(NodeType, i, reinterpret_cast<uint8_t*>(&v), sizeof(v));
}
                                         ...
                                         int64_t begin, __gm__ const char* name)
{
    ...
    for (int64_t i = begin; i < end; ++i) {
        if constexpr (std::is_floating_point_v<ElemT>) {
            EncodeIndexedElement<ElemT, INDEXED_FP32, float>(logger, i, data[i]);
        } else if constexpr (std::is_integral_v<ElemT>) {
            EncodeIndexedElement<ElemT, INDEXED_INT64, int64_t>(logger, i, data[i]);
        } else if constexpr (std::is_pointer_v<ElemT>) {
            EncodeIndexedElement<ElemT, INDEXED_INT64, int64_t>(logger, i, data[i]);
#if IS_AICORE
        } else if constexpr (std::is_same_v<ElemT, bfloat16_t>) {
            EncodeIndexedElement<ElemT, INDEXED_BF16, uint16_t>(logger, i, data[i]);
        } else if constexpr (std::is_same_v<ElemT, half>) {
            EncodeIndexedElement<ElemT, INDEXED_FP16, uint16_t>(logger, i, data[i]);
#if SUPPORT_FP8_HF8_PRINT
        } else if constexpr (std::is_same_v<ElemT, float8_e4m3_t>) {
            EncodeIndexedElement<ElemT, INDEXED_FP8E4M3, uint8_t>(logger, i, data[i]);
        } else if constexpr (std::is_same_v<ElemT, float8_e5m2_t>) {
            EncodeIndexedElement<ElemT, INDEXED_FP8E5M2, uint8_t>(logger, i, data[i]);
        } else if constexpr (std::is_same_v<ElemT, float8_e8m0_t>) {
            EncodeIndexedElement<ElemT, INDEXED_FP8E8M0, uint8_t>(logger, i, data[i]);
        } else if constexpr (std::is_same_v<ElemT, hifloat8_t>) {
            EncodeIndexedElement<ElemT, INDEXED_HF8, uint8_t>(logger, i, data[i]);
#endif
#endif
        }
        logger->EncodeEnd();
