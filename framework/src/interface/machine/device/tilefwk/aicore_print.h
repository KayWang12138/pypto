...
INLINE float DecodeFp8E4M3(uint8_t bits) {
    const uint8_t exp = (bits >> Fp8E4M3Const::MANT_BITS) & 0xF;
    if (exp == 15) {  // E4M3 special: exp=15 → 240.0 (no Inf/NaN)
        const uint8_t sign = (bits >> Fp8Const::SIGN_SHIFT) & 0x1;
        return sign ? -240.0f : 240.0f;
    }
