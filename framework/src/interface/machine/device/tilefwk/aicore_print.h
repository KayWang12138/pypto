...
// FP8/HF8 打印支持（仅在支持 FP8 类型的平台可用）
// 条件需与 CANN __clang_cce_types.h 中 FP8 类型定义条件一致
...
namespace AicorePrintConst {
    ...
    constexpr int16_t SHORT_MAX_VALUE = 32767;    ///< short类型最大值（用于nameLen校验）
}
...
/**
 * \brief FP8 (8-bit Floating Point) 格式说明
 * 
 * 本模块支持四种FP8格式，均为华为昇腾AI处理器自定义或行业标准格式：
 * 
 * 1. **FP8 E4M3** (IEEE-like, 无Inf):
 *    - 1位符号位 + 4位指数 + 3位尾数
 *    - Exponent bias: 7
 *    - 表示范围: 0 ~ 448 (无无穷大，NaN通过特殊值表示)
 *    - 典型应用: 权重存储、量化计算
 * 
 * 2. **FP8 E5M2** (IEEE 754-like, 有Inf/NaN):
 *    - 1位符号位 + 5位指数 + 2位尾数
 *    - Exponent bias: 15
 *    - 表示范围: 与FP16相近，支持Inf/NaN
 *    - 典型应用: 激活值存储、梯度计算
 * 
 * 3. **FP8 E8M0** (纯指数格式):
 *    - 1位符号位 + 7位指数 + 0位尾数
 *    - Exponent bias: 127
 *    - 仅表示2的幂次: ±2^(e-127)
 *    - 典型应用: Scale factor、缩放因子存储
 * 
 * 4. **HiFloat8 (HF8)** - 华为自定义格式:
 *    - 动态指数范围设计，优化小值精度和大值范围
 *    - 算法详见: Huawei HiFloat8 Specification v1.0
 *    - 位模式解析规则见 DecodeHf8 函数注释
 * 
 * \note 仅在 __DAV_V310 平台可用，其他平台编译时 SUPPORT_FP8_HF8_PRINT=0
 */
...
/**
 * \brief Decode Hifloat8 (HF8) - 华为自定义8位浮点格式解码
 * 
 * HF8格式采用动态指数编码，根据值的范围自动调整指数宽度：
 * 
 * \par 位模式解析规则 (Huawei HiFloat8 Specification v1.0):
 * - bit[7]: 符号位 (sign)
 * - bit[6:0]: 动态编码区，根据top bits确定编码模式：
 * 
 * \par 编码模式详解:
 * | Top bits | Mode | Exponent range | Mantissa bits |
 * |----------|------|----------------|---------------|
 * | top4=0   | Tiny | mv决定exp(-23~-1) | 3 bits        |
 * | top4=1   | Small| exp=0, mv=mant   | 3 bits        |
 * | top3=1   | Medium| eb=±1, mv=mant  | 3 bits        |
 * | top2=1   | Large| eb=±(2~3), mv=mant| 3 bits      |
 * | top2=2   | Huge | eb=±(4~7), mv=mant| 2 bits      |
 * | default  | Max  | eb=±(8~15), mv=mant| 1 bit       |
 * 
 * \par 解码算法:
 * 1. 提取sign bit (bit[7])
 * 2. 提取lower 7 bits作为编码值
 * 3. 根据top bits判断编码模式
 * 4. 计算exponent和mantissa
 * 5. 组合为FP32格式输出
 * 
 * \param bits 8位HF8编码值
 * \return 解码后的FP32浮点值
 * 
 * \note 此算法由华为定义，具体规格见 HiFloat8 Specification v1.0
 */
