...
INLINE float DecodeBf16(uint16_t bits) {
    using namespace Bf16Const;
    uint32_t u = static_cast<uint32_t>(bits) << TO_FP32_SHIFT;
...
public:
    ...
    __aicore__ void Sync() {
        ...
        using namespace AicorePrintConst;
...
private:
    ...
    __aicore__ void EncodeTyped(AicorePrint::DataType ty, const uint8_t* val, short valLen, 
    ...
    __aicore__ void SkipToEndMarker() {
        while (ReadByte(tail_) != static_cast<uint8_t>(AicorePrint::DataType::End)) {
    ...
    int DecodeLegacyRecord(AicorePrint::DataType type, char* buf, size_t maxSize) {
        ...
        auto valOff = tail_ + NAMELEN_FIELD_SIZE;
        tail_ += ReadValue<short>(tail_) + NAMELEN_FIELD_SIZE;
        auto fmtOff = tail_ + NAMELEN_FIELD_SIZE;
