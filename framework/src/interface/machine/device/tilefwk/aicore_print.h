...
namespace EncodeSizes {
    ...
    // OverflowWarning encoding size: type(1) + bufferSize(8) + recommendedSize(8) + tailBefore(8) + tailAfter(8) + End(1) = 34 bytes
    constexpr int64_t OverflowWarningSize = 1 + 8 + 8 + 8 + 8 + 1;  ///< 34 bytes
...
public:
    ...
    __aicore__ void EncodeOverflowWarning(int64_t bufferSize, int64_t recommendedSize,
                                          int64_t tailBefore, int64_t tailAfter) {
        EncodeByteUnsafe(static_cast<uint8_t>(AicorePrint::DataType::OverflowWarning));
        EncodeValueUnsafe<int64_t>(bufferSize);
        EncodeValueUnsafe<int64_t>(recommendedSize);
        EncodeValueUnsafe<int64_t>(tailBefore);
        EncodeValueUnsafe<int64_t>(tailAfter);
    }
...
private:
    __aicore__ void EncodeByte(uint8_t val) {
        ...
        // Overflow condition: buffer is full (written data >= buffer size)
        // Use (head_ - tail_) >= size_ instead of head_ == tail_ + size_
        // Because head_ and tail_ are int64_t that can grow infinitely and wrap around
    ...
    __aicore__ void EncodeByteUnsafe(uint8_t val) {
        volatile __gm__ uint8_t* p = &data_[head_++ % size_];
        *p = val;
    }
    
    template<typename T>
    __aicore__ void EncodeValueUnsafe(T value) {
        auto* bytes = reinterpret_cast<uint8_t*>(&value);
        for (size_t i = 0; i < sizeof(T); ++i) {
            EncodeByteUnsafe(bytes[i]);
        }
    }
    
    __aicore__ void TriggerOverflowWarning() {
        overflowTailBefore_ = tail_;
        
        // OverflowWarningSize 已包含 End marker
        // OverflowWarningSize = type(1) + bufferSize(8) + recommendedSize(8) + tailBefore(8) + tailAfter(8) + End(1) = 34 bytes
        int64_t spaceNeeded = EncodeSizes::OverflowWarningSize;
        
        // 释放完整记录直到有足够空间
        // 每次释放一条完整记录（包含 End marker），不能只释放部分数据
        SkipToFreeSpace(spaceNeeded);
        
        // 再次检查是否真的有足够空间
        // buffer 大小必须至少能容纳一条 OverflowWarning + End
        int64_t freeSpace = size_ - (head_ - tail_);
        if (freeSpace < spaceNeeded) {
            // 即使跳完了所有记录，仍然没有足够空间（buffer 太小）
            return;
        }
        
        int64_t tailAfter = tail_;
        
        // 写入 OverflowWarning（使用 Unsafe 方法，因为已确认有足够空间）
        EncodeByteUnsafe(static_cast<uint8_t>(AicorePrint::DataType::OverflowWarning));
        
        int64_t bufferSize = size_;
        EncodeValueUnsafe<int64_t>(bufferSize);
        
        int64_t recommendedSize = size_ * 2;
        EncodeValueUnsafe<int64_t>(recommendedSize);
        
        EncodeValueUnsafe<int64_t>(overflowTailBefore_);
        EncodeValueUnsafe<int64_t>(tailAfter);
        
        // 写入 End marker（完整记录结束）
        EncodeByteUnsafe(static_cast<uint8_t>(AicorePrint::DataType::End));
    ...
    __aicore__ void SkipToFreeSpace(int64_t spaceNeeded) {
        // Free space = size_ - (head_ - tail_)
        // Target: head_ - tail_ <= size_ - spaceNeeded
        // Note: TensorHeader records are preserved to keep name info for subsequent prints
        
        while (head_ - tail_ > size_ - spaceNeeded) {
           AicorePrint::DataType segType = static_cast<AicorePrint::DataType>(ReadByte(tail_));
           
           // Skip TensorHeader: preserve it to keep name info, but still skip its End marker
           if (segType == AicorePrint::DataType::TensorHeader) {
               tail_++;  // skip type marker
               short nameLen = ReadValue<short>(tail_);
               tail_ += sizeof(short) + nameLen + sizeof(int64_t) * 2;
               // After TensorHeader, there's always an End marker from PrintTensorImpl
               // We need to skip that End marker too
               if (ReadByte(tail_) == static_cast<uint8_t>(AicorePrint::DataType::End)) {
                   tail_++;
               }
               continue;  // preserve TensorHeader, don't remove name info
           }
           
           SkipRecord(segType);     
           tail_++;  // skip End marker after the record
        }
    }
    
    __aicore__ void SkipRecord(AicorePrint::DataType type) {
        switch (type) {
		...
		short nameLen = ReadValue<short>(tail_);
            ...
            case AicorePrint::DataType::IndexedHf8:
                tail_ += EncodeSizes::IndexedFp8;
                break;
#endif
            default:
		tail_++;
                tail_ += ReadValue<short>(tail_) + sizeof(short);
                tail_ += ReadValue<short>(tail_) + sizeof(short);
                break;
        }
    }
    ...
    int DecodeOverflowWarning(char* buf, size_t maxSize) {
        ...
        int64_t recommendedSize = ReadValue<int64_t>(tail_);
        tail_ += AicorePrintConst::TENSOR_RANGE_SIZE;
        
        int64_t tailBefore = ReadValue<int64_t>(tail_);
        tail_ += AicorePrintConst::TENSOR_RANGE_SIZE;
        
        int64_t tailAfter = ReadValue<int64_t>(tail_);
        tail_ += AicorePrintConst::TENSOR_RANGE_SIZE;
        
        int64_t fullBufferSize = bufferSize + sizeof(RemoteHeader);
        int64_t fullRecommended = recommendedSize + sizeof(RemoteHeader);
        
        // 确保开头换行，让 warning 与前面的输出分开
        buf[0] = '\n';
        int written = snprintf_s(buf + 1, maxSize - 1, maxSize - 2,
            "[WARNING] Ring buffer overflow detected! "
            "Buffer size is %ld bytes (%ld KB), data area is %ld bytes. "
            "Tail advanced from %ld to %ld (delta=%ld bytes), indicating data loss. "
            "All subsequent prints are blocked to preserve this warning. "
            "To fix, set PRINT_BUFFER_SIZE >= %ld (%ld KB) "
            "(recommend doubling current size: 2 × %ld KB) "
            "in framework/src/interface/machine/device/tilefwk/aicpu_common.h, "
            "then rebuild and reinstall.\n",
            fullBufferSize, fullBufferSize / 1024,
            bufferSize,
            tailBefore, tailAfter, (tailAfter - tailBefore),
            fullRecommended, fullRecommended / 1024,
            bufferSize / 1024);
        return written + 1;  // +1 for the leading '\n'
    }
