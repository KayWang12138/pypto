...
struct AicoreLogger {
    ...
    __aicore__ void PrintFloat(__gm__ const char **fmt, float val) {
        ...
        switch (curFmt[idx++]) {
            case 'f': {
                Encode(FP32, reinterpret_cast<uint8_t *>(&val), sizeof(val), *fmt, idx);
                break;
            }
            default: Encode(NORMAL, static_cast<uint8_t *>(nullptr), 0, *fmt, idx); break;
        }
    ...
    __aicore__ void PrintF16(__gm__ const char **fmt, uint16_t rawBits) {
        ...
        switch (curFmt[idx++]) {
            case 'f': {
                Encode(FP16, reinterpret_cast<uint8_t *>(&rawBits), sizeof(rawBits), *fmt, idx);
                break;
            }
            default: Encode(NORMAL, static_cast<uint8_t *>(nullptr), 0, *fmt, idx); break;
        }
...
INLINE void __AiCorePrintTensorImpl(LogContext *ctx, PtrT data, int64_t end, int64_t begin = 0)
{
    ...
    for (int64_t i = begin; i < end; ++i) {
        if constexpr (std::is_integral_v<ElemT>) {
            ...
            AiCoreLogF(ctx, "%p\n", data[i]);
#if IS_AICORE
        } else if constexpr (IS_AICORE && std::is_same_v<ElemT, bfloat16_t>) {
            AiCoreLogF(ctx, "%f\n", data[i]);
