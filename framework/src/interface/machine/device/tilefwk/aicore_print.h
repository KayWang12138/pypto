...
// FP8 类型特化：从 FP8 类型转换为 uint8_t
INLINE uint8_t SafeBitCast(const float8_e4m3_t& src)
{
    uint8_t dst;
    const unsigned char* srcBytes = reinterpret_cast<const unsigned char*>(&src);
    unsigned char* dstBytes = reinterpret_cast<unsigned char*>(&dst);
...
INLINE uint8_t SafeBitCast(const float8_e5m2_t& src)
{
    uint8_t dst;
    const unsigned char* srcBytes = reinterpret_cast<const unsigned char*>(&src);
    unsigned char* dstBytes = reinterpret_cast<unsigned char*>(&dst);
...
INLINE uint8_t SafeBitCast(const float8_e8m0_t& src)
{
    uint8_t dst;
    const unsigned char* srcBytes = reinterpret_cast<const unsigned char*>(&src);
    unsigned char* dstBytes = reinterpret_cast<unsigned char*>(&dst);
                                         ...
                                         int64_t begin, __gm__ const char* name)
{
    ...
    for (int64_t i = begin; i < end; ++i) {
        ...
        } else if constexpr (std::isatermplate_v<ElemT, float8_e8m0_t>) {
            uint8_t v = SafeBitCast<uint8_t>(data[i]);
