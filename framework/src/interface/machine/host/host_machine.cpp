...
void HostMachine::SubTask(Function *function) {
    ...
    // printf("=== Stashed function idx:%d begin compile, function name: %s .\n", function_done_idx, function->GetRawName().c_str());
    MonitorManager::Instance().SetCurrentFunctionName(function->GetRawName());
...
void HostMachine::StashTask(Function* function) {
    ...
    COMPILER_LOGI("Stashed function queue size:%lu, push function: %s .", stashedFuncQueue_.Size(),
        function->GetRawName().c_str());
    // printf("=== Stashed function queue size:%lu, push function: %s .\n", stashedFuncQueue_.Size(), function->GetRawName().c_str());
}
...
void HostMachine::SubAllStashedTask() {
    ...
    if (totalStashed > 0) {
        ...
        // printf("=== Compiler monitor set function total count: %d.\n\n\n", static_cast<int>(totalStashed));
    }
...
void HostMachine::PushFinishQueue(std::unique_ptr<MachineTask> task) {
    ...
    // printf("=== Stashed function idx:%d finish compile. \n\n", task->GetFunctionIndex());
