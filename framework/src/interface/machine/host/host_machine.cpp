...
void HostMachine::CompileFunction(Function* func) const
{
    ...
    if (!func->HasCallOperation() && backend.runPass) {
        ...
        F_ASSERT(backend.runPass(Program::GetInstance(), *func, config::GetPassStrategy())) << "Run pass failed.";
    }
...
MachineTask* HostMachine::Compile(MachineTask* task) const
{
    ...
    if (existResumeFile) {
        ...
        F_ASSERT(FError::BAD_FD, file.good()) << "Json file: " << jsonPath << " open failed!!!";
        Json jsonData;
        try {
            file >> jsonData;
        } catch (const std::exception& e) {
            F_ASSERT(FError::INVALID_FILE, false) << "Json file: " << jsonPath << " parsing error: " << e.what();
        }
