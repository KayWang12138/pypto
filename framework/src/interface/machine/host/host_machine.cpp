...
void HostMachine::CompileFunction(Function* func) const {
    ...
    if (!func->HasCallOperation() && backend.runPass) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, backend.runPass(Program::GetInstance(), *func, config::GetPassStrategy())) << "Run pass failed.";
    }
...
MachineTask *HostMachine::Compile(MachineTask *task) const {
    ...
    if (existResumeFile) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, file.good()) << "Json file: " << jsonPath << " open failed!!!";
        Json jsonData;
        try {
            file >> jsonData;
        } catch (const std::exception &e) {
            ASSERT(ERROR_CODE_UNKNOWN, false) << "Json file: " << jsonPath << " parsing error: " << e.what();
        }
