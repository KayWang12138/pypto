...
void FunctionCache::UpdateTopoCache(const Function &func, CacheValue &value) {
    ...
    *reinterpret_cast<uint64_t*>(topoCachePtr) = totalSize;
    ...
    for (uint32_t i = 0; i < topoNum; i++) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, (tempPtr->coreType == static_cast<uint64_t>(CoreType::AIV)) ||
    ...
    ASSERT(ERROR_CODE_UNDEFINED, topoNum != 0)<<"Invalid topoNum: "<<topoNum;
...
void FunctionCache::UpdateBinCache(const Function &func, CacheValue &value) {
    ...
    for (auto &ele : func.programs_) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, leafFuncAttr != nullptr)<<"Leaf function attr not found";
...
void FunctionCache::UpdateReadyFunction(const Function &func, CacheValue &value) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, value.header.readyCoreFunctionNum != 0)<<"readyCoreFunctionNum is 0, value.header.readyCoreFunctionNum="<<value.header.readyCoreFunctionNum;
}
