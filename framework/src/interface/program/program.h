...
public: // public api for torch
    ...
    void InsertFuncToFunctionMap(const std::string &magicName, std::shared_ptr<Function> func) {
        ASSERT(ERROR_CODE_UNKNOWN, functionmap_.count(magicName) == 0) << magicName << " already exists in functionmap.";
    ...
    void SetCurrentDynamicFunction(Function *dynFunc) {
        ASSERT(ERROR_CODE_UNKNOWN, currentDynamicFunctionPtr_ != dynFunc) << "Under: " << currentDynamicFunctionPtr_->GetRawName() << " " << dynFunc->GetRawName();
