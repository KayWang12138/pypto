...
bool RecordFunc::Iterator::operator!=(const IteratorEnd& rhs)
{
    ...
    ASSERT(FError::EINTERNAL, rhs.wrappedEnd.has_value()) << "Input param rhs has no value";
    ...
    if (parallel_) {
        for (auto& rlf : Program::GetInstance().GetLoopStack()) {
            if (rlf.get().Getparallel()) {
                ASSERT(FError::INVALID_OPERATION, !rlf.get().Getparallel()) << "The parallel attribute value does not allow nesting";
            }
        }
    }
...
bool RecordLoopFunc::MatchUnrollTimes(int unrollTimes)
{
    ...
    ASSERT(FError::EINTERNAL, curRlf.StillHaveUnrollTimes()) << "unrollTimes_ is empty.";
...
RecordLoopFunc::Iterator RecordLoopFunc::Iterator::operator++()
{
    ...
    if (!rlf_.CustomUnrollTimesMatched()) {
        ...
        ASSERT(FError::EINTERNAL, cur_ == 0) << "The cur_ = " << cur_;
...
bool RecordLoopFunc::Iterator::operator!=(const IteratorEnd& rhs)
{
    ...
    ASSERT(FError::EINTERNAL, rlf_.StillHaveUnrollTimes()) << "unrollTimes_ is empty.";
    ...
    ASSERT(FError::EINTERNAL, cur_ == rlf_.CurUnrollTimes())
...
{
    ASSERT(FError::EINTERNAL, customUnrollTimes_.empty() || customUnrollTimes_.count(1) > 0)
...
{
    ASSERT(FError::IS_EXIST, visited_.count(unrollTimes) == 0)
        << "unrollTimes[" << unrollTimes << "] already exists in visited.";
    ASSERT(FError::IS_EXIST, unrollTimes_.count(unrollTimes) == 0)
...
{
    ASSERT(FError::EINTERNAL, StillHaveUnrollTimes()) << "unrollTimes_ is empty.";
