...
void RecordFunc::RecordDynFuncInner(const std::vector<std::reference_wrapper<const Tensor>> &startArgsInputTensorList,
    ...
    const std::vector<std::pair<std::reference_wrapper<const Tensor>, std::reference_wrapper<const Tensor>>> &inplaceArgs) {
        CHECK(ERROR_CODE_UNDEFINED, config::GetFunctionType() == FunctionType::DYNAMIC)
...
bool RecordFunc::Iterator::operator!=(const IteratorEnd &rhs) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, rhs.wrappedEnd.has_value()) << "Input param rhs has no value";
...
RecordLoopFunc::RecordLoopFunc(const std::string &name, FunctionType funcType, const std::string &iterName,
    ...
    : name_(FUNCTION_PREFIX + name),
        ...
        funcType_(funcType) {
    CHECK(ERROR_CODE_UNDEFINED, funcType == FunctionType::STATIC || funcType == FunctionType::DYNAMIC_LOOP)
...
void RecordLoopFunc::BeginLoopFunction() {
    ...
    CHECK(ERROR_CODE_UNDEFINED, currentLoopFunc_->InsertLoopIdxNameList(iterName_)) << "Forbid duplicate name of loop idx. It names " << iterName_;
...
bool RecordLoopFunc::MatchUnrollTimes(int unrollTimes) {
    CHECK(ERROR_CODE_UNDEFINED, unrollTimes > 0) << "unrollTimes[" << unrollTimes << "] must larger than zero!";
    ...
    ASSERT(ERROR_CODE_UNDEFINED, curRlf.StillHaveUnrollTimes()) << "unrollTimes_ is empty.";
...
RecordLoopFunc::Iterator RecordLoopFunc::Iterator::operator++() {
    ...
    if (!rlf_.CustomUnrollTimesMatched()) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, cur_ == 0) << "The cur_ = " << cur_;
...
bool RecordLoopFunc::Iterator::operator!=(const IteratorEnd &rhs) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, rlf_.StillHaveUnrollTimes()) << "unrollTimes_ is empty.";
    ...
    ASSERT(ERROR_CODE_UNDEFINED, cur_ == rlf_.CurUnrollTimes())
...
void RecordLoopFunc::IterationNext() {
    ASSERT(ERROR_CODE_UNDEFINED, customUnrollTimes_.empty() || customUnrollTimes_.count(1) > 0) << "Must have unroll 1 if user defined custom unroll times.";
}
...
void RecordLoopFunc::VisitUnroll(int unrollTimes) {
    ASSERT(ERROR_CODE_UNDEFINED, visited_.count(unrollTimes) == 0) << "unrollTimes[" << unrollTimes << "] already exists in visited.";
    ASSERT(ERROR_CODE_UNDEFINED, unrollTimes_.count(unrollTimes) == 0) << "unrollTimes[" << unrollTimes << "] already exists..";
    visited_.emplace(unrollTimes);
    unrollTimes_.emplace(unrollTimes);
}

int RecordLoopFunc::CurUnrollTimes() const {
    ASSERT(ERROR_CODE_UNDEFINED, StillHaveUnrollTimes()) << "unrollTimes_ is empty.";
