...
void RecordFunc::RecordDynFuncInner(const std::vector<std::reference_wrapper<const Tensor>> &startArgsInputTensorList,
    ...
    const std::vector<std::pair<std::reference_wrapper<const Tensor>, std::reference_wrapper<const Tensor>>>
        &inplaceArgs) {
    CHECK(FuncErr::GRAPH_TYPE_MISMATCH, config::GetFunctionType() == FunctionType::DYNAMIC)
...
RecordLoopFunc::RecordLoopFunc(const std::string &name, FunctionType funcType, const std::string &iterName,
    ...
    : name_(FUNCTION_PREFIX + name),
      ...
      funcType_(funcType) {
    CHECK(FuncErr::FUNCTION_TYPE_MISMATCH, funcType == FunctionType::STATIC || funcType == FunctionType::DYNAMIC_LOOP)
...
void RecordLoopFunc::BeginLoopFunction() {
    ...
    CHECK(ProgErr::LOOP_INDEX_NAME_DUPLICATE, currentLoopFunc_->InsertLoopIdxNameList(iterName_))
...
bool RecordLoopFunc::MatchUnrollTimes(int unrollTimes) {
    CHECK(ProgErr::LOOP_UNROLL_TIMES_INVALID, unrollTimes > 0)
    ...
    ASSERT(ProgErr::LOOP_UNROLL_TIMES_EMPTY, curRlf.StillHaveUnrollTimes()) << "unrollTimes_ is empty.";
...
bool RecordLoopFunc::Iterator::operator!=(const IteratorEnd &rhs) {
    ...
    ASSERT(ProgErr::LOOP_UNROLL_TIMES_EMPTY, rlf_.StillHaveUnrollTimes()) << "unrollTimes_ is empty.";
...
void RecordLoopFunc::IterationNext() {
    ASSERT(ProgErr::MUST_HAVE_UNROLL_ONE, customUnrollTimes_.empty() || customUnrollTimes_.count(1) > 0)
...
void RecordLoopFunc::VisitUnroll(int unrollTimes) {
    ASSERT(ProgErr::LOOP_UNROLL_TIMES_EXISTS, visited_.count(unrollTimes) == 0)
        << "unrollTimes[" << unrollTimes << "] already exists in visited.";
    ASSERT(ProgErr::LOOP_UNROLL_TIMES_EXISTS, unrollTimes_.count(unrollTimes) == 0)
...
int RecordLoopFunc::CurUnrollTimes() const {
    ASSERT(ProgErr::LOOP_UNROLL_TIMES_EMPTY, StillHaveUnrollTimes()) << "unrollTimes_ is empty.";
