    ...
    const std::vector<std::reference_wrapper<const Tensor>>& explicitOpArgs, bool isHiddenFunction)
{
    ...
    if (functionmap_.find(funcMagicName) == functionmap_.end()) { // new function
        ...
        currentFunctionPtr_ = newFunc.get();
        ASSERT(FError::IS_EXIST, functionmap_.count(funcMagicName) == 0)
...
{
    ASSERT(FError::EINTERNAL, functionMagicNameStack_.size() != 0)
    ...
    ASSERT(FError::NOT_EXIST, currentFunctionPtr_->GetMagicName() == funcMagicName)
    ...
    ASSERT(FError::EINTERNAL, currentFunctionPtr_->HasParent())
    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand)
{
    ...
    ASSERT(FError::EINTERNAL, currentFunctionMagicName_ != PROGRAM_ENTRY_FUNCTION_NAME) << "No active function to add operation.";
        ...
        if (dyndevFunc != nullptr) {
            ...
            if (!rootFuncs.empty()) {
                ...
                if (!tileGraphFuncs.empty()) {
                    ...
                    ASSERT(FError::NOT_EXIST, false) << "cannot find current function magic";
                }
...
void Program::LoadJson(Json& programJson)
{
    ...
    ASSERT(FError::NOT_EXIST, currentFunctionPtr_ != nullptr) << "loss of pointer.";
}
...
void Program::DumpJsonFile(const std::string& fileName, Function* mainFunc)
{
    ...
    ASSERT(FError::BAD_FD, file.is_open()) << "Failed to open file: " << filePath;
...
bool Program::QueryAndUpdateCurrentFunction()
{
    ...
    if (cacheValue == std::nullopt) {
        ...
        ASSERT(FError::INVALID_TYPE, currentFunctionPtr_->IsGraphType(GraphType::BLOCK_GRAPH))
