...
void Program::CreateCallerCalleeLink(Function *caller, Function *callee) {
    ASSERT(ERROR_CODE_UNKNOWN, caller->IsGraphType(GraphType::TENSOR_GRAPH) &&
...
bool Program::BeginFunction(const std::string &funcName,
    ...
    if (currentFunctionPtr_->IsFlattening() && (funcType == FunctionType::STATIC && (graphType == GraphType::TENSOR_GRAPH || graphType == GraphType::TILE_GRAPH))) {
        ...
        CHECK(ERROR_CODE_UNKNOWN, funcName != currentFunctionPtr_->GetRawName())
    ...
    if (functionmap_.find(funcMagicName) == functionmap_.end()) { // new function
        ...
        currentFunctionPtr_ = newFunc.get();
        ASSERT(ERROR_CODE_UNKNOWN, functionmap_.count(funcMagicName) == 0) << funcMagicName << " already exists in funcmap.";
...
Operation *Program::FinishCurrentFunction(const std::shared_ptr<TensorSlotScope> &scope, bool generateCall) {
    ASSERT(ERROR_CODE_UNKNOWN, functionMagicNameStack_.size() != 0) << "The stack of functionMagicName is null.";
    auto funcMagicName = currentFunctionPtr_->GetRawName() + "_" + std::to_string(currentFunctionPtr_->GetFuncMagic());
    ASSERT(ERROR_CODE_UNKNOWN, currentFunctionPtr_->GetMagicName() == funcMagicName)
    ...
    ASSERT(ERROR_CODE_UNKNOWN, currentFunctionPtr_->HasParent()) << "CurrentFunction doesn't have a parent function.";
    if (scope) {
...
Operation &Program::AddOperation(const Opcode opCode,
    ...
    if (currentFunctionMagicName_ == PROGRAM_ENTRY_FUNCTION_NAME) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, false) << "No active function to add operation.";
    }
        ...
        if (dyndevFunc != nullptr) {
            ...
            if (!rootFuncs.empty()) {
                ...
                if (!tileGraphFuncs.empty()) {
                    ...
                    ASSERT(ERROR_CODE_UNKNOWN, false) << "cannot find current function magic";
                }
...
void Program::LoadJson(Json &programJson) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, currentFunctionPtr_ != nullptr) << "loss of pointer.";
}
...
void Program::DumpJsonFile(const std::string &fileName, Function *mainFunc) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, file.is_open()) << "Failed to open file: " << filePath;
...
bool Program::QueryAndUpdateCurrentFunction() {
    ...
    if (cacheValue == std::nullopt) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, currentFunctionPtr_->IsGraphType(GraphType::BLOCK_GRAPH))
