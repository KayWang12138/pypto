...
void Program::CreateCallerCalleeLink(Function *caller, Function *callee) {
    ASSERT(ERROR_CODE_UNDEFINED, caller->IsGraphType(GraphType::TENSOR_GRAPH) &&
...
bool Program::BeginFunction(const std::string &funcName,
    ...
    if (currentFunctionPtr_->IsFlattening() && (funcType == FunctionType::STATIC && (graphType == GraphType::TENSOR_GRAPH || graphType == GraphType::TILE_GRAPH))) {
        ...
        CHECK(ERROR_CODE_UNDEFINED, funcName != currentFunctionPtr_->GetRawName())
    ...
    if (functionmap_.find(funcMagicName) == functionmap_.end()) { // new function
        ...
        currentFunctionPtr_ = newFunc.get();
        ASSERT(ERROR_CODE_UNDEFINED, functionmap_.count(funcMagicName) == 0) << funcMagicName << " already exists in funcmap.";
        functionmap_.emplace(funcMagicName, std::move(newFunc));
        currentFunctionMagicName_ = funcMagicName;
    } else {
        FUNCTION_LOGE(ERROR_CODE_UNDEFINED, "funcMagicName[%s] is already in the function map", funcMagicName.c_str());
        currentFunctionMagicName_ = funcMagicName;
...
Operation *Program::FinishCurrentFunction(const std::shared_ptr<TensorSlotScope> &scope, bool generateCall) {
    ASSERT(ERROR_CODE_UNDEFINED, functionMagicNameStack_.size() != 0) << "The stack of functionMagicName is null.";
    auto funcMagicName = currentFunctionPtr_->GetRawName() + "_" + std::to_string(currentFunctionPtr_->GetFuncMagic());
    ASSERT(ERROR_CODE_UNDEFINED, currentFunctionPtr_->GetMagicName() == funcMagicName)
    ...
    ASSERT(ERROR_CODE_UNDEFINED, currentFunctionPtr_->HasParent()) << "CurrentFunction doesn't have a parent function.";
    if (scope) {
...
std::tuple<Function*, Operation *, bool> Program::EndFunction(const std::string &funcName,
    ...
    if (currentFunctionPtr_->IsStatic() && funcName != currentFunctionPtr_->GetRawName()) {
        FUNCTION_LOGE(ERROR_CODE_UNDEFINED, 
...
Operation &Program::AddOperation(const Opcode opCode,
    ...
    if (currentFunctionMagicName_ == PROGRAM_ENTRY_FUNCTION_NAME) {
        FUNCTION_LOGE(ERROR_CODE_UNDEFINED, "Error: No active function to add operation.");
        ASSERT(ERROR_CODE_UNDEFINED, false) << "No active function to add operation.";
    }
        ...
        if (dyndevFunc != nullptr) {
            ...
            if (!rootFuncs.empty()) {
                ...
                if (!tileGraphFuncs.empty()) {
                    ...
                    ASSERT(ERROR_CODE_UNDEFINED, false) << "cannot find current function magic";
                }
                ...
                FUNCTION_LOGE(ERROR_CODE_UNDEFINED, "Failed to find main function.");
            }
        }
    } else {
...
std::shared_ptr<Function> Program::GetFunctionByMagic(int funcMagic)
{
    ...
    FUNCTION_LOGE(ERROR_CODE_UNDEFINED, "Cannot find function iter by magic %d", funcMagic);
    return nullptr;
}
...
void Program::LoadJson(Json &programJson) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, currentFunctionPtr_ != nullptr) << "loss of pointer.";
}
...
void Program::DumpJsonFile(const std::string &fileName, Function *mainFunc) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, file.is_open()) << "Failed to open file: " << filePath;
...
bool Program::QueryAndUpdateCurrentFunction() {
    ...
    if (cacheValue == std::nullopt) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, currentFunctionPtr_->IsGraphType(GraphType::BLOCK_GRAPH))
