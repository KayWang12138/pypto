...
bool Program::BeginFunction(const std::string &funcName,
    ...
    if (currentFunctionPtr_->IsFlattening() && (funcType == FunctionType::STATIC && (graphType == GraphType::TENSOR_GRAPH || graphType == GraphType::TILE_GRAPH))) {
        ...
        CHECK(ProgErr::FUNCTION_NAME_DUPLICATE, funcName != currentFunctionPtr_->GetRawName())
    ...
    if (functionmap_.find(funcMagicName) == functionmap_.end()) { // new function
        ...
        currentFunctionPtr_ = newFunc.get();
        ASSERT(ProgErr::FUNCTION_ALREADY_EXISTS, functionmap_.count(funcMagicName) == 0)
        ...
        FUNCTION_LOGE_E(ProgErr::FUNCTION_ALREADY_EXISTS,
...
Operation *Program::FinishCurrentFunction(const std::shared_ptr<TensorSlotScope> &scope, bool generateCall) {
    ASSERT(ProgErr::FUNCTION_STACK_NULL, functionMagicNameStack_.size() != 0)
    ...
    ASSERT(ProgErr::CURRENT_MAGIC_NOT_FOUND, currentFunctionPtr_->GetMagicName() == funcMagicName)
    ...
    ASSERT(ProgErr::FUNCTION_NO_PARENT, currentFunctionPtr_->HasParent())
...
std::tuple<Function*, Operation *, bool> Program::EndFunction(const std::string &funcName,
    ...
    if (currentFunctionPtr_->IsStatic() && funcName != currentFunctionPtr_->GetRawName()) {
        FUNCTION_LOGE_E(ProgErr::CURRENT_MAGIC_NOT_FOUND, "Function name not match current: %s != %s",
...
Operation &Program::AddOperation(const Opcode opCode,
    ...
    if (currentFunctionMagicName_ == PROGRAM_ENTRY_FUNCTION_NAME) {
        FUNCTION_LOGE_E(ProgErr::NO_ACTIVE_FUNCTION, "Error: No active function to add operation.");
        ...
        if (dyndevFunc != nullptr) {
            ...
            if (!rootFuncs.empty()) {
                ...
                if (!tileGraphFuncs.empty()) {
                    ...
                    ASSERT(ProgErr::CURRENT_MAGIC_NOT_FOUND, false) << "cannot find current function magic";
                }
                ...
                FUNCTION_LOGE_E(ProgErr::PROGRAM_ENTRY_NOT_FOUND, "Failed to find main function.");
            }
        }
    } else {
...
std::shared_ptr<Function> Program::GetFunctionByMagic(int funcMagic)
{
    ...
    FUNCTION_LOGE_E(ProgErr::FUNCTION_NOT_FOUND, "Cannot find function iter by magic %d", funcMagic);
    return nullptr;
}
...
void Program::LoadJson(Json &programJson) {
    ...
    ASSERT(ProgErr::CURRENT_FUNCTION_NOT_FOUND, currentFunctionPtr_ != nullptr) << "loss of pointer.";
}
...
void Program::DumpJsonFile(const std::string &fileName, Function *mainFunc) {
    ...
    ASSERT(ProgErr::DUMP_FILE_OPEN_FAILED, file.is_open()) << "Failed to open file: " << filePath;
...
bool Program::QueryAndUpdateCurrentFunction() {
    ...
    if (cacheValue == std::nullopt) {
        ...
        ASSERT(FuncErr::GRAPH_TYPE_MISMATCH, currentFunctionPtr_->IsGraphType(GraphType::BLOCK_GRAPH))
