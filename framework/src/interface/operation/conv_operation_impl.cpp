...
{
    std::vector<int64_t> dstBiasL1Shape = std::vector<int64_t>{convTileInfo.nL0};
    ...
    std::vector<int64_t> dstBiasBtShape = std::vector<int64_t>{convTileInfo.nL0};
...
LogicalTensorPtr constructFmapTile(Function &function, const ConvGraphNodes &tensorGraphNodes,
                                   const ConvTileInfo &convTileInfo, LogicalTensorPtr &dstAL1TensorPtr)
{
    if (kl0Idx % convTileInfo.kAL1 == 0) {
        aL1UpadateFlag = true;
    }
    // L1层级 Fmap 展开
    if (aL1UpadateFlag) {
        ...
        dstAL1TensorPtr =
            std::make_shared<LogicalTensor>(function, operandVec[0]->Datatype(), dstAL1Shape,
    ...
    LogicalTensorPtr dstAL0TensorPtr =
        std::make_shared<LogicalTensor>(function, operandVec[0]->Datatype(), dstAL0Shape,
...
LogicalTensorPtr constructWeightTile(Function &function, const ConvGraphNodes &tensorGraphNodes,
                                     const ConvTileInfo &convTileInfo, LogicalTensorPtr &dstBL1TensorPtr)
{
    if (kl0Idx % convTileInfo.kBL1 == 0 || nl0Idx % convTileInfo.nBL1) {
        bL1UpadateFlag = true;
    }
    // L1层级 Weight 展开
    if (bL1UpadateFlag) {
...
void DoMmad(Function &function, const ConvAttrParam &convAttrParam, const ConvGraphNodes &tensorGraphNodes,
            const ConvGraphNodes &tileGraphNodes, const ConvTileInfo &convTileInfo)
{
    OP_CHECK(true, {
    ...
    if (kl0Idx + 16 >= convTileInfo.orgK) {
        ...
        // }
        tileGraphNodes.cL0PartialSumPtr->UpdateDynValidShape({64, 16});
                        ...
                        const LogicalTensorPtr &cTensorPtr, const Operation &op)
{
    ...
    for (int64_t groupIdx = 0; groupIdx < convAttrParam.groups; groupIdx += 1) {
        for (int64_t batchIdx = 0; batchIdx < convTileInfo.orgBatch; batchIdx += 1) {
            for (int64_t nl0Idx = 0; nl0Idx < (convTileInfo.orgCout / convAttrParam.groups); nl0Idx += ConvTileInfo.nL0) {
                // bias 载入
                if (convAttrParam.hasBias) {
                    // get bias in bt tile for mmad
                    tileGraphNodes.biasTensorPtr = constructBiasTile(function, tensorGraphNodes, convTileInfo);
                }
                for (int64_t ml0Idx = 0; ml0Idx < convTileInfo.orgHout * convTileInfo.orgWout; ml0Idx += 64) {
                    LogicalTensorPtr fmapL1TensorPtr = nullptr;
                    LogicalTensorPtr weightL1TensorPtr = nullptr;
                    for (int64_t kl0Idx = 0; kl0Idx < (convTileInfo.orgK / convAttrParam.groups); kl0Idx += 16) {
                        // fmap and weight link
                        tileGraphNodes.fmapTensorPtr =
                            constructFmapTile(function, tensorGraphNodes, convTileInfo, fmapL1TensorPtr);
                        tileGraphNodes.weightTensorPtr =
                            constructWeightTile(function, tensorGraphNodes, convTileInfo, weightL1TensorPtr);
                        // add mmad node
                        DoMmad(function, convAttrParam, tensorGraphNodes, tileGraphNodes, convTileInfo);
                    }
                }
            }
        }
