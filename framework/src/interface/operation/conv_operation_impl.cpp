...
void CheckValueRange(
    int64_t value, const std::string &name, int64_t min, int64_t max, const std::string &formula = "") {
...
LogicalTensorPtr ConstructBiasTile(
    ...
    LogicalTensorPtr dstBiasl1TensorPtr = std::make_shared<LogicalTensor>(function,
        tensorGraphNodes.biasTensorPtr->Datatype(), dstBiasL1Shape, SymbolicScalar::FromConcrete(dstBiasL1Shape),
    ...
    LogicalTensorPtr dstBiasBtTensorPtr = std::make_shared<LogicalTensor>(function, DataType::DT_FP32, dstBiasBtShape,
        SymbolicScalar::FromConcrete(dstBiasBtShape), tensorGraphNodes.biasTensorPtr->Format(), "biasBtTensor",
        NodeType::LOCAL);
...
void SetCopyInAL1Op(Operation &copyInOpAl1, const ConvTileInfo &convTileInfo, ConvIterInfo &iterInfo,
    ...
    auto copyAttr = std::make_shared<CopyOpAttribute>(OpImmediate::Specified(srcFmapGmOffset), MemoryType::MEM_L1,
        OpImmediate::Specified(srcGmValidShape), OpImmediate::Specified(dstAL1Shape),
...
LogicalTensorPtr ConstructFmapTile(Function &function, const ConvGraphNodes &tensorGraphNodes,
    const ConvTileInfo &convTileInfo, ConvIterInfo &iterInfo, LogicalTensorPtr &dstAL1TensorPtr,
...
void SetCopyInBL1Op(Operation &copyInOpBl1, const ConvTileInfo &convTileInfo, ConvIterInfo &iterInfo,
    ...
    auto copyAttr = std::make_shared<CopyOpAttribute>(OpImmediate::Specified(srcWeightGmOffset), MemoryType::MEM_L1,
        OpImmediate::Specified(srcGmValidShape), OpImmediate::Specified(dstBL1Shape),
...
LogicalTensorPtr ConstructWeightTile(Function &function, const ConvGraphNodes &tensorGraphNodes,
    const ConvTileInfo &convTileInfo, ConvIterInfo &iterInfo, LogicalTensorPtr &dstBL1TensorPtr,
    const ConvAttrParam &convAttrParam) {
    ...
    if (iterInfo.bL1UpadateFlag) {
        ...
        dstBL1TensorPtr = std::make_shared<LogicalTensor>(function, tensorGraphNodes.weightTensorPtr->Datatype(),
            dstBL1Shape, SymbolicScalar::FromConcrete(dstBL1Shape), tensorGraphNodes.weightTensorPtr->Format(),
            "bL1Tensor", NodeType::LOCAL);
...
LogicalTensorPtr DoMmad(Function &function, const ConvAttrParam &convAttrParam, const ConvGraphNodes &tensorGraphNodes,
    ...
    if (iterInfo.isLastK) {
        ...
        tileGraphNodes.cL0PartialSumPtr = std::make_shared<LogicalTensor>(function, DataType::DT_FP32,
            cL0PartialSumShape, SymbolicScalar::FromConcrete({iterInfo.mL0Size, iterInfo.nL0Size}),
...
void ConstrucCopyOutTile(Function &function, const ConvAttrParam &convAttrParam, const ConvGraphNodes &tensorGraphNodes,
    ...
    auto copyAttr = std::make_shared<CopyOpAttribute>(MemoryType::MEM_L1, OpImmediate::Specified(dstResGmOffset),
...
void IterL0ExpandFunc(Function &function, ConvIterInfo &iterInfo, ConvTileInfo &convTileInfo,
    const ConvAttrParam &convAttrParam, const ConvGraphNodes &tensorGraphNodes, ConvGraphNodes &tileGraphNodes) {
        ...
        for (iterInfo.hL0Offset = 0; iterInfo.hL0Offset < iterInfo.houtL1Size; iterInfo.hL0Offset += convTileInfo.hL0) {
            for (iterInfo.wL0Offset = 0; iterInfo.wL0Offset < iterInfo.woutL1Size;
                ...
                std::vector<int64_t> dstCL0Shape = std::vector<int64_t>{
                    ConvAlignB(iterInfo.mL0Size, MKN_M_VALUE), ConvAlignB(iterInfo.nL0Size, MKN_N_VALUE)};
                tileGraphNodes.resTensorPtr = std::make_shared<LogicalTensor>(function,
                    tensorGraphNodes.fmapTensorPtr->Datatype(), dstCL0Shape, SymbolicScalar::FromConcrete(dstCL0Shape),
