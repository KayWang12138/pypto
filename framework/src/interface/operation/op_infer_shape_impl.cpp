...
{
    ASSERT(VectorErrorCode::ERR_RUNTIME_NULLPTR, op != nullptr) << "op should not be nullptr";
    if (op->HasAttr(Matrix::L1_TO_L0_OFFSET) && op->HasAttr(Matrix::L1_TO_L0_TILE)) {
        ...
        ASSERT(VectorErrorCode::ERR_CONFIG_TILE, offset.size() == SHAPE_DIM2) << "offset.size() should be SHAPE_DIM2";
        ASSERT(VectorErrorCode::ERR_CONFIG_TILE, tile.size() == SHAPE_DIM2) << "tile.size() should be SHAPE_DIM2";
        ASSERT(VectorErrorCode::ERR_PARAM_INVALID,
               !op->GetIOperands().empty() && op->GetIOperands()[0] != nullptr &&
        ...
        ASSERT(VectorErrorCode::ERR_CONFIG_TILE, dstValidShape.size() == SHAPE_DIM2)
    ...
    std::vector<std::vector<SymbolicScalar>> inputValidShapes;
    for (auto inputTensor : op->GetIOperands()) {
        ASSERT(VectorErrorCode::ERR_RUNTIME_NULLPTR, inputTensor != nullptr) << "inputTensor should not be nullptr";
        inputValidShapes.push_back(inputTensor->GetDynValidShape());
    }
...
void LoadL0MXInferFunc(Operation* op, std::vector<std::vector<SymbolicScalar>>& outValidShapes)
{
    ...
    op->GetAttr(Matrix::L1_TO_L0_OFFSET, offset);
    op->GetAttr(Matrix::L1_TO_L0_TILE, tile);
    ASSERT(VectorErrorCode::ERR_CONFIG_TILE, offset.size() == SHAPE_DIM3) << "offset.size() should be SHAPE_DIM3";
    ASSERT(VectorErrorCode::ERR_CONFIG_TILE, tile.size() == SHAPE_DIM3) << "tile.size() should be SHAPE_DIM3";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID,
           !op->GetIOperands().empty() && op->GetIOperands()[0] != nullptr &&
               op->GetIOperands()[0]->GetDynValidShape().size() == SHAPE_DIM3)
    ...
    std::vector<SymbolicScalar> dstValidShape = GetViewValidShape(
        srcValidShape, SymbolicScalar::Concrete(offset, 0), offset, SymbolicScalar::Concrete(tile, 0));
    ASSERT(VectorErrorCode::ERR_CONFIG_TILE, dstValidShape.size() == SHAPE_DIM3)
...
void L1CopyInConvInferFunc(Operation* op, std::vector<std::vector<SymbolicScalar>>& outValidShapes)
{
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, op->HasAttr(L1_TILE_SHAPE)) << "op should have L1_TILE_SHAPE attr";
    std::vector<SymbolicScalar> tile;
    op->GetAttr(L1_TILE_SHAPE, tile);
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, op->HasAttr(IS_FMAP_FLAG)) << "op should have IS_FMAP_FLAG attr";
    bool isFmap = false;
    op->GetAttr(IS_FMAP_FLAG, isFmap);
    if (isFmap) {
        ASSERT(VectorErrorCode::ERR_CONFIG_TILE, tile.size() == SHAPE_DIM5 || tile.size() == SHAPE_DIM6)
            << "tile.size() should be SHAPE_DIM5 or SHAPE_DIM6";
    } else {
        ASSERT(VectorErrorCode::ERR_CONFIG_TILE, tile.size() == SHAPE_DIM4) << "tile.size() should be SHAPE_DIM4";
    }
    std::vector<SymbolicScalar> outShape;
...
void L1ToL0ConvInferFunc(Operation* op, std::vector<std::vector<SymbolicScalar>>& outValidShapes)
{
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, op->HasAttr(L0_TILE_SHAPE)) << "op should have L0_TILE_SHAPE attr";
    std::vector<SymbolicScalar> tile;
...
void L0CCopyOutConvInferFunc(Operation* op, std::vector<std::vector<SymbolicScalar>>& outValidShapes)
{
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, op->HasAttr(RES_TILE_SHAPE)) << "op should have RES_TILE_SHAPE attr";
    std::vector<SymbolicScalar> tile;
