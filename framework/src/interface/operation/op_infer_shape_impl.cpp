...
void Load2L1MXScaleInferFunc(Operation *op, std::vector<std::vector<SymbolicScalar>> &outValidShapes)
{
    ...
    int64_t copyInMod = 1;
    op->GetAttr(A_MUL_B_COPY_IN_MODE, copyInMod);
    for (auto output : op->GetOOperands()) {
        if (copyInMod == 3) {
            outValidShapes.push_back({srcValidShape[1], srcValidShape[0]});
        } else {
            outValidShapes.push_back({srcValidShape[0], srcValidShape[1]});
        }
    }
}
...
void LoadMXL0InferFunc(Operation *op, std::vector<std::vector<SymbolicScalar>> &outValidShapes)
{
    ASSERT(op != nullptr);
    const std::string L1_TO_L0_OFFSET = OP_ATTR_PREFIX + "l1_to_l0_offset";
    const std::string L1_TO_L0_TILE = OP_ATTR_PREFIX + "l1_to_l0_tile";
...
REGISTER_INFER_SHAPE_FUNC(OP_L1_TO_L0A_SCALE, Opcode::OP_L1_TO_L0A_SCALE, LoadMXL0InferFunc);
REGISTER_INFER_SHAPE_FUNC(OP_L1_TO_L0B_SCALE, Opcode::OP_L1_TO_L0B_SCALE, LoadMXL0InferFunc);
