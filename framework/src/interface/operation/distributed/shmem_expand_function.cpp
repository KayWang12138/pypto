    ...
    Function& function)
{
    ...
    CHECK(ERROR_CODE_UNKNOWN, tileRowNum > 0 && tileColNum > 0) << "TileRowNum and tileColNum cannot be zero";
...
{
    ASSERT(ERROR_CODE_UNKNOWN, iOperand.size() == 3UL) << "TiledShmemPut iOperand size is not equal to 3";
    ASSERT(ERROR_CODE_UNKNOWN, oOperand.size() == 1UL) << "TiledShmemPut oOperand size is not equal to 1";
...
void TiledShmemSignal(Function& function, const TileShape& tileShape,
    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ASSERT(ERROR_CODE_UNKNOWN, iOperand.size() == 2UL) << "TiledShmemSignal iOperand size is not equal to 2";
    ASSERT(ERROR_CODE_UNKNOWN, oOperand.size() == 1UL) << "TiledShmemSignal oOperand size is not equal to 1";
    auto predToken = iOperand[0];
...
void TiledShmemWaitUntil(Function& function, const TileShape& tileShape,
    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ASSERT(ERROR_CODE_UNKNOWN, iOperand.size() == 2UL) << "TiledShmemWaitUntil iOperand size is not equal to 2";
    ASSERT(ERROR_CODE_UNKNOWN, oOperand.size() == 1UL) << "TiledShmemWaitUntil oOperand size is not equal to 1";
    auto predToken = iOperand[0];
    auto shmemSignal = iOperand[1];
    auto out = oOperand[0];
    ...
    DfsTiling(tileShape.GetVecTile(), shmemSignal, [&](uint32_t tileIndex, Input& input) {
        ...
        tileNumOfWaitUntil++;
        ASSERT(ERROR_CODE_UNKNOWN, tileNumOfWaitUntil <= MAX_TILE_NUM) <<
...
void TiledShmemGet(Function& function, const TileShape& tileShape,
    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ASSERT(ERROR_CODE_UNKNOWN, iOperand.size() == 2UL) << "TiledShmemGet iOperand size is not equal to 2";
    ASSERT(ERROR_CODE_UNKNOWN, oOperand.size() == 1UL) << "TiledShmemGet oOperand size is not equal to 1";
    auto predToken = iOperand[0];
    auto shmemData = iOperand[1];
    auto out = oOperand[0];
    DummyTileFunc predTokenTileFunc = GetDummyTileFunc(predToken, shmemData, tileShape.GetVecTile(), function);
...
void TiledShmemGetGM2UB(Function& function, const TileShape& tileShape,
    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ASSERT(ERROR_CODE_UNKNOWN, iOperand.size() == 2UL) << "TiledShmemGetGM2UB iOperand size is not equal to 2";
    ASSERT(ERROR_CODE_UNKNOWN, oOperand.size() == 1UL) << "TiledShmemGetGM2UB oOperand size is not equal to 1";
    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ...
    ASSERT(ERROR_CODE_UNKNOWN, iOperand.size() == 2UL) << "TiledShmemSet iOperand size is not equal to 2";
    ASSERT(ERROR_CODE_UNKNOWN, oOperand.size() == 1UL) << "TiledShmemSet oOperand size is not equal to 1";
    auto predToken = iOperand[0];
    auto shmemTensor = iOperand[1];
    auto out = oOperand[0];

    ASSERT(ERROR_CODE_UNKNOWN, UB_BUFFER_BYTE_SIZE % REPEAT_BYTE == 0) << "UB_BUFFER_BYTE_SIZE must be a multiple of 256, but got "
        << UB_BUFFER_BYTE_SIZE;
...
void TiledShmemReduce(Function& function, const TileShape& tileShape,
    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ASSERT(ERROR_CODE_UNKNOWN, iOperand.size() == 3UL) << "TiledShmemReduce iOperand size is not equal to 3";
    ASSERT(ERROR_CODE_UNKNOWN, oOperand.size() == 1UL) << "TiledShmemReduce oOperand size is not equal to 1";
    auto in = iOperand[0];
    auto shmemData = iOperand[1];
