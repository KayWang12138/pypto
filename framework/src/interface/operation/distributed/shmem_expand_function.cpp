...
LogicalTensorPtr View2DTile(LogicalTensorPtr& dummy,
    int32_t tileIndex, const TileShape& tileShape, const Shape& dataShape, Function& function)
{
    Shape dummyShape = dummy->shape;
    auto& vecTile = tileShape.GetVecTile();
    int32_t tileRows = dataShape[0] / vecTile[0] + (dataShape[0] % vecTile[0] == 0 ? 0 : 1);
    int32_t tileCols = dataShape[1] / vecTile[1] + (dataShape[1] % vecTile[1] == 0 ? 0 : 1);
    int32_t rowIndex = tileIndex / tileCols;
    int32_t colIndex = tileIndex % tileCols;

    int32_t baseRow = dummyShape[0] / tileRows;
    int32_t remRow = dummyShape[0] % tileRows;
    ...
    int32_t baseCol = dummyShape[1] / tileCols;
    int32_t remCol = dummyShape[1] % tileCols;
    ...
    return dummy->View(function, {tileRowSize, tileColSize}, {tileRowOffset, tileColOffset});
}
    ...
    int32_t tileIndex, const TileShape& tileShape, const Shape& dataShape, Function& function)
{
    ...
    if (tileRows <= dummyShape[0] && tileCols <= dummyShape[1]) {
        return View2DTile(dummy, tileIndex, tileShape, dataShape, function);
    }
