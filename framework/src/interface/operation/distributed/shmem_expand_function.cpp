    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ...
    DfsTiling(tileShape, curDim, startInput, startInDim, startTileIndex, [&](uint32_t tileIndex, Input& input) {
        ...
        auto shmemDataTile = shmemData->View(function, shmemShape, shmemOffset);

        auto dummyTile = dummy->View(function, {1, 1}, {tileIndex, 0});
        auto copyBufferShape = GetCopyBufferShape(inTile->Datatype(), shmemDataTile->Datatype(), input.tileInfo.shape);
        auto ubTensor = CreateAdaptiveUbTensor(function, copyBufferShape, inTile->Datatype(), shmemDataTile->Datatype());
        auto& tileOp = function.AddOperation(Opcode::OP_SHMEM_PUT, {inTile, shmemDataTile, predToken},
            {dummyTile, ubTensor});
    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ...
    DfsTiling(tileShape, curDim, startInput, startDim, startTileIndex, [&](uint32_t tileIndex, Input& input) {
        ...
        auto copyBufferShape = GetCopyBufferShape(out->Datatype(), shmemDataTile->Datatype(), input.tileInfo.shape);
