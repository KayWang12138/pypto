    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ...
    DummyTileFunc shmemPutOutTileFunc = GetDummyTileFunc(shmemPutOut, shmemData, tileShape.GetVecTile(), function);
    DfsTiling(tileShape.GetVecTile(), shmemData, [&](uint32_t tileIndex, Input& input) {
        ...
        auto shmemPutOutTile = shmemPutOutTileFunc(tileIndex);
    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ...
    DummyTileFunc shmemSignalOutTileFunc = GetDummyTileFunc(shmemSignalOut, shmemSignal, tileShape.GetVecTile(), function);
    DfsTiling(tileShape.GetVecTile(), shmemSignal, [&](uint32_t tileIndex, Input& input){
        ...
        auto shmemSignalOutTile = shmemSignalOutTileFunc(tileIndex);
    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ...
    DummyTileFunc predTokenTileFunc = GetDummyTileFunc(predToken, shmemSignal, tileShape.GetVecTile(), function);
    DummyTileFunc waitUntilOutTileFunc = GetDummyTileFunc(waitUntilOut, shmemSignal, tileShape.GetVecTile(), function);
    DfsTiling(tileShape.GetVecTile(), shmemSignal, [&](uint32_t tileIndex, Input& input) {
        ...
        std::vector<int64_t>& shmemSignalTileOffset = input.tileInfo.offset;
        auto shmemSignalTile = shmemSignal->View(function, shmemSignalTileShape, shmemSignalTileOffset);
        auto waitUntilOutTile = waitUntilOutTileFunc(tileIndex);

        auto& tileOp = function.AddOperation(Opcode::OP_SHMEM_WAIT_UNTIL, {predTokenTile, shmemSignalTile}, {waitUntilOutTile});
