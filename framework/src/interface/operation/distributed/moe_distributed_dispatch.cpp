    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ...
    for (int expertIndex = 0; expertIndex < expertNumPerRank; ++expertIndex) {
        for (int rankIndex = 0; rankIndex < tileRankCnt; ++rankIndex) {
            ...
            auto &opr = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_ASSEMBLE_COMBINEINFO, {shmemDataTile, shmemFlag, recvTokenCntOut}, 
    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ...
    const auto &tileRank = tileShape.GetDistTileRank();
    const int32_t tileNumPerExpert = GetTotalTileNum(tileRank);
    ...
    for (int expertIndex = 0; expertIndex < expertNumPerRank; ++expertIndex) {
        for (int rankIndex = 0; rankIndex < tileRankCnt; ++rankIndex) {
            ...
            auto shmemDataTile = shmemData->View(function, {1, rankShape, 1, shmemDataLength}, 
                {0, rankOffset, expertIndex, 0});
            auto &opr = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_ASSEMBLE_EXPANDX, {shmemDataTile, shmemFlag, recvTokenCntOut}, 
    ...
    int32_t expandXRow, int32_t ffnTileNum, uint32_t expertNumPerRank)
{
    ...
    auto &oper = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_ASSEMBLE_COMBINEINFO, {recvTokenCntOut.GetStorage(), shmemData.GetStorage(),
    ...
    int32_t expandXRow, int32_t ffnTileNum, uint32_t expertNumPerRank)
{
    ...
    auto &oper = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_ASSEMBLE_EXPANDX, {recvTokenCntOut.GetStorage(), shmemData.GetStorage(),
