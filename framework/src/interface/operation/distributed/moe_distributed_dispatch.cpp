    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ...
    for (int expertIndex = 0; expertIndex < expertNumPerRank; ++expertIndex) {
        for (int rankIndex = 0; rankIndex < tileRankCnt; ++rankIndex) {
            ...
            auto &opr = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_WAIT_SIGNAL, {syncTensor, shmemFlagTile}, 
    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ...
    for (int expertIndex = 0; expertIndex < expertNumPerRank; ++expertIndex) {
        for (int rankIndex = 0; rankIndex < tileRankCnt; ++rankIndex) {
            ...
            auto &opr = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_ASSEMBLE_ASSIST_INFO_FOR_COMBINE, {shmemDataTile, shmemFlag, recvTokenCntOut}, 
    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ...
    op.GetAttr("dispatchBufferSize", ubShape);
    op.GetAttr("hiddenSize", axisH);
    op.GetAttr("tokenBatchSize", batchSize);

    auto &tileRank = tileShape.GetDistTileRank();
    int32_t tileNumPerExpert = GetTotalTileNum(tileRank);
    int32_t totalTileNum = tileNumPerExpert * static_cast<int32_t>(expertNumPerRank);
    int32_t tileRankShape = tileRank[DIST_HEAD_SHAPE];
    int32_t tileRankCnt = tileRank[DIST_HEAD_COUNT] + (tileRank[DIST_TAIL_SHAPE] == 0 ? 0 : 1);
    int32_t tailRankShape = tileRank[DIST_TAIL_SHAPE];
    ...
    for (int expertIndex = 0; expertIndex < expertNumPerRank; ++expertIndex) {
        for (int rankIndex = 0; rankIndex < tileRankCnt; ++rankIndex) {
            int32_t rankShape = ((tileRank[2] != 0) && (rankIndex == tileRankCnt - 1) ? tailRankShape :tileRankShape);
            int32_t rankOffset = rankIndex * tileRankShape;
            auto bufferTensor = std::make_shared<LogicalTensor>(function, DataType::DT_INT32, ubShape);
            auto shmemDataTile = shmemData->View(function, {1, rankShape, 1, shmemDataLength}, 
                {0, rankOffset, expertIndex, 0});
            auto &opr = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_ASSEMBLE_EXPAND_X, {shmemDataTile, shmemFlag, recvTokenCntOut}, 
    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ...
    for (int32_t expertIndex = 0; expertIndex < expertCount; ++expertIndex) {
        ...
        auto shmemFlagTile = shmemFlag->View(function, {1, expertShape, rankSize, flagColSize},
            {0, expertOffset, 0, 0});
        auto &tileop = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_EXPERT_TOKEN_NUM, {recvTokenCntOut, shmemFlagTile},
...
Tensor DispatchBuildExpertTokenNum(const Tensor& recvTokenCntOut, const Tensor& shmemFlag, uint32_t expertNumPerRank)
{
    ...
    auto &oper = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_EXPERT_TOKEN_NUM, {recvTokenCntOut.GetStorage(), shmemFlag.GetStorage()}, {expertTokenNumsPtr});
    (void)oper;
    ...
    int32_t expandXRow, int32_t ffnTileNum, uint32_t expertNumPerRank)
{
    ...
    auto &oper = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_ASSEMBLE_ASSIST_INFO_FOR_COMBINE, {recvTokenCntOut.GetStorage(), shmemData.GetStorage(),
    ...
    int32_t expandXRow, int32_t ffnTileNum, uint32_t expertNumPerRank)
{
    ...
    auto &oper = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_ASSEMBLE_EXPAND_X, {recvTokenCntOut.GetStorage(), shmemData.GetStorage(),
...
Tensor DispatchWaitRecvFlags(const char *group, const Tensor& flagDummy, Tensor& shmemFlag, uint32_t expertNumPerRank, int32_t ffnTileNum)
{
    ...
    auto &oper = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_WAIT_SIGNAL, {flagDummy.GetStorage(), shmemFlag.GetStorage()},
        {recvTokenCntOutPtr});
    ...
    CreateTileOp(tileShape,
        [&](int32_t tileIndex, int32_t rowOffset, int32_t colOffset, int32_t rowShape, int32_t colShape) {
            ...
            auto tokenBuffer = std::make_shared<LogicalTensor>(function, x->Datatype(), 
                GetCommBufferSize(x));
            auto &tileop = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_SEND_ROUTED, {x, shmemData, 
    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ...
    CreateTileOp(tileShape,
        [&](int32_t tileIndex, int32_t rowOffset, int32_t colOffset, int32_t rowShape, int32_t colShape) {
            ...
            auto tokenBuffer = std::make_shared<LogicalTensor>(function, x->Datatype(), 
                GetCommBufferSize(x));
            auto &tileop = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_SEND_SHARED, {xTile, shmemData},
    ...
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ...
    CreateTileOp(tileShape,
        [&](int32_t tileIndex, int32_t rowOffset, int32_t colOffset, int32_t rowShape, int32_t colShape) {
            ...
            auto tokenBuffer = std::make_shared<LogicalTensor>(function, x->Datatype(), 
                GetCommBufferSize(x));
            auto &tileop = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_LOCAL_COPY_OUT, {xTile},
...
void TiledDispatchSetRecvSignals(Function& function, const TileShape& tileShape,
    const std::vector<std::shared_ptr<LogicalTensor>>& iOperand,
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
    ...
    for (int32_t rankIndex = 0; rankIndex < rankCount; ++rankIndex) {
        ...
        for (int32_t expertIndex = 0; expertIndex < expertCount; ++expertIndex) {
            ...
            auto &tileop = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_SET_SIGNAL, {expertIds, shmemFlagTile, 
    ...
    const Tensor& expertIds, const char *group, uint32_t expertNumPerRank)
{
    ...
    auto &oper = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_SEND_ROUTED, {shmemData.GetStorage(),
...
{
    auto &function = *Program::GetInstance().GetCurrentFunction();
    auto &oper = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_SEND_SHARED, {shmemData.GetStorage(), 
        x.GetStorage()},
...
Tensor DispatchSetRecvSignals(Tensor& shmemFlag, const Tensor& expertIds, const Tensor& syncTensor,
    const char *group, uint32_t expertNumPerRank)
{
    Shape shape = {1, 1};
    auto &function = *Program::GetInstance().GetCurrentFunction();
    auto syncDummy = std::make_shared<LogicalTensor>(function, DataType::DT_INT32, shape);
    auto &oper = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_SET_SIGNAL, {shmemFlag.GetStorage(), syncTensor.GetStorage(), 
...
Tensor CopyToLocalExpert(const Tensor& x, const Tensor& syncTensor, uint32_t routedExpertNum)
{
    ...
    auto expandXPtr = std::make_shared<LogicalTensor>(function, x.GetDataType(), expandXShape);
    auto &oper = function.AddOperation(Opcode::OP_MOE_DISTRIBUTED_DISPATCH_LOCAL_COPY_OUT, {x.GetStorage()}, 
...
std::tuple<int32_t, int32_t, int32_t> GetRecvTileParam(uint32_t epWorldSize)
{
...
void MoeDispatchValidate(const Tensor& x, const Tensor& expertIds, const char *group, 
...
void DispatchCreateShmemLoop(Tensor& shmemData, Tensor& shmemFlag, const Tensor& x, int32_t hcclGroupIndex,
    int32_t epWorldSize, int32_t expertNumPerRank, int32_t batchSize, int32_t shmemDataLength)
{
    LOOP("CreateShmemTensor", FunctionType::DYNAMIC_LOOP, index, LoopRange(1)) {
        (void) index;
        int32_t shmemFlagCol = 128;
        int32_t shmemDataCol = shmemDataLength * batchSize;
        Shape shmemDataShape = {epWorldSize, expertNumPerRank, shmemDataCol};
        Shape shmemFlagShape = {expertNumPerRank, epWorldSize, shmemFlagCol};
...
{
    MoeDispatchValidate(x, expertIds, group, epWorldSize, moeExpertNum, sharedExpertNum,
        sharedExpertRankNum, expandX, expertTokenNums, assistInfoForCombine, recvCounts);
    ASSERT(epWorldSize > 0) << "MoeDispatch epWorldSize must be > 0, but got " << epWorldSize;
    int32_t routedExpertNum = moeExpertNum - sharedExpertNum;
    int32_t expertNumPerRank = routedExpertNum / epWorldSize;
    ...
    Tensor shmemData;
    Tensor shmemFlag;
    DispatchCreateShmemLoop(shmemData, shmemFlag, x, hcclGroupIndex, epWorldSize,
        expertNumPerRank, batchSize, shmemDataLength);
    LOOP("L0", FunctionType::DYNAMIC_LOOP, index, LoopRange(1)) {
        (void) index;
        TileShape::Current().SetDistTile({1, batchSize, 0}, {topK, 1, 0}, {static_cast<int32_t>(epWorldSize), 1, 0});
        Tensor syncTensor = DispatchSendToRoutingExperts(shmemData, x, expertIds, group, expertNumPerRank);
        TileShape::Current().SetDistTile({flagRow, 1, 0}, {static_cast<int32_t>(epWorldSize), 1, 0}, {1, expertNumPerRank, 0});
        auto localShmemFlag = View(shmemFlag, {epWorldSize, expertNumPerRank, 1, flagCol}, {0, 0, thisRank, 0});
        Tensor flagDummy = DispatchSetRecvSignals(localShmemFlag, expertIds, syncTensor, group, expertNumPerRank);
        auto [recvTileCnt, recvTileNum, recvTailNum] = GetRecvTileParam(epWorldSize);
        TileShape::Current().SetDistTile({batchSize, 1, 0}, {hiddenSize, 1, 0}, {recvTileCnt, recvTileNum, recvTailNum});
        auto shmemFlagSched = View(shmemFlag, {1, expertNumPerRank, epWorldSize, flagCol}, {thisRank, 0, 0, 0});
        auto recvTokenCntOut = DispatchWaitRecvFlags(group, flagDummy, shmemFlagSched, expertNumPerRank, recvTileNum);
        auto shmemDataBatching = View(shmemData, {1, epWorldSize, expertNumPerRank, shmemDataLength}, {thisRank, 0 ,0 ,0});
        auto [expandXPtr, recvCountsPtr] = DispatchAssembleExpandX(group, x, recvTokenCntOut, shmemDataBatching,
            localShmemFlag, expandX.GetShape(0), recvTileNum + recvTailNum, expertNumPerRank);
