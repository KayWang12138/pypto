...
void TiledMoeDistributedCombineSend(
    ...
    const Operation& op)
{
    CHECK(iOperand.size() == 5UL) << "TiledMoeDistributedCombineSend iOperand size is not equal to 5";
    CHECK(oOperand.size() == 1UL) << "TiledMoeDistributedCombineSend oOperand size is not equal to 1";
    ...
    CHECK(dataByteSize != 0) << "Input expandX dType nullptr";
    ...
    const Operation& op)
{
    ...
    CHECK(iOperand.size() == 4UL) << "TiledMoeDistributedCombineReceive iOperand size is not equal to 4";
    CHECK(oOperand.size() == 1UL) << "TiledMoeDistributedCombineReceive oOperand size is not equal to 1";
    ...
    CHECK(dataByteSize != 0) << "Output out dType nullptr";
    int64_t paddedColShape = AlignUp(dataByteSize * hiddenSize, COPY_BLOCK_BYTE_SIZE) / dataByteSize;
    int64_t floatByteSize = BytesOf(DataType::DT_FP32);
    CHECK(floatByteSize != 0) << "Divisor cannot be zero";
    ...
    uint32_t sharedExpertNum, uint32_t sharedExpertRankNum, Tensor& out)
{
    ...
    LOOP("MoeDistributedCombineReceive", FunctionType::DYNAMIC_LOOP, tokenId, LoopRange(batchSize)) {
        ...
        CHECK((BytesOf(DT_FP32) != 0) && (kTileShape != 0)) << "Divisor cannot be zero";
