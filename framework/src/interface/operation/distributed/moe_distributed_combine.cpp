    ...
    int32_t moeExpertNum)
{
    ...
    CHECK(ERROR_CODE_UNKNOWN, expandX.GetShape().size() == supportedDim) << "The dim of \"expandX\" only supports " << supportedDim
    ...
    CHECK(ERROR_CODE_UNKNOWN, expandXRow == expectedRow) << "The first axis of \"expandX\" must be the smaller value between topK (the "
    ...
    CHECK(ERROR_CODE_UNKNOWN, expandXCol == supportedHiddenSize) << "The second axis of \"expandX\" only supports " << supportedHiddenSize
        << ", but got " << expandXCol;

    CHECK(ERROR_CODE_UNKNOWN, expandX.GetDataType() == DT_BF16) << "The data type of \"expandX\" only supports DT_BF16, but got "
        << DataType2String(expandX.GetDataType());

    CHECK(ERROR_CODE_UNKNOWN, expandX.Format() == npu::tile_fwk::TileOpFormat::TILEOP_ND) << "The format of \"expandX\" only supports ND, "
        << "but got NZ";
}
...
{
    uint32_t supportedDim = 2;
    CHECK(ERROR_CODE_UNKNOWN, assistInfoForCombine.GetShape().size() == supportedDim) << "The dim of \"assistInfoForCombine\" only "
    ...
    int32_t expandXRow = expandX.GetShape(0);
    CHECK(ERROR_CODE_UNKNOWN, assistInfoForCombineRow == expandXRow) << "The first axis of \"assistInfoForCombine\" must be consistent "
    ...
    CHECK(ERROR_CODE_UNKNOWN, assistInfoForCombineCol == supportedAssistInfoForCombineCol) << "The second axis of "
        << "\"assistInfoForCombine\" must be " << supportedAssistInfoForCombineCol << ", but got "
        << assistInfoForCombineCol;

    CHECK(ERROR_CODE_UNKNOWN, assistInfoForCombine.GetDataType() == DT_INT32) << "The data type of \"assistInfoForCombine\" only supports "
        << "DT_INT32, but got " << DataType2String(assistInfoForCombine.GetDataType());

    CHECK(ERROR_CODE_UNKNOWN, assistInfoForCombine.Format() == npu::tile_fwk::TileOpFormat::TILEOP_ND) << "The format of "
        << "\"assistInfoForCombine\" only supports ND, but got NZ";
}

void MoeDistributedCombineValidateRecvCounts(const Tensor& recvCounts)
{
    CHECK(ERROR_CODE_UNKNOWN, recvCounts.GetShape().size() == 1) << "The dim of \"recvCounts\" only supports 1, but got "
        << recvCounts.GetShape().size();

    int32_t recvCountsSize = recvCounts.GetShape(0);
    CHECK(ERROR_CODE_UNKNOWN, recvCountsSize == 1) << "The size of \"recvCounts\" must be 1, but recvCountsSize=" << recvCountsSize;

    CHECK(ERROR_CODE_UNKNOWN, recvCounts.GetDataType() == DT_INT32) << "The data type of \"recvCounts\" only supports DT_INT32, but got "
        << DataType2String(recvCounts.GetDataType());

    CHECK(ERROR_CODE_UNKNOWN, recvCounts.Format() == npu::tile_fwk::TileOpFormat::TILEOP_ND) << "The format of \"recvCounts\" only "
...
{
    uint32_t supportedDim = 2;
    CHECK(ERROR_CODE_UNKNOWN, expertScales.GetShape().size() == supportedDim) << "The dim of \"expertScales\" only supports "
    ...
    CHECK(ERROR_CODE_UNKNOWN, (expertScalesRow == supportedExpertScalesRow1) || (expertScalesRow == supportedExpertScalesRow2)) << "The "
    ...
    CHECK(ERROR_CODE_UNKNOWN, expertScalesCol == supportedExpertScalesCol) << "The second axis of \"expertScales\" only supports "
        << supportedExpertScalesCol << ", but got " << expertScalesCol;

    CHECK(ERROR_CODE_UNKNOWN, expertScales.GetDataType() == DT_FP32) << "The data type of \"expertScales\" only supports DT_FP32, but got "
        << DataType2String(expertScales.GetDataType());

    CHECK(ERROR_CODE_UNKNOWN, expertScales.Format() == npu::tile_fwk::TileOpFormat::TILEOP_ND) << "The format of \"expertScales\" only "
        << "supports ND, but got NZ";
}
...
{
    uint32_t supportedDim = 2;
    CHECK(ERROR_CODE_UNKNOWN, out.GetShape().size() == supportedDim) << "The dim of \"out\" only supports " << supportedDim << ", but got "
        << out.GetShape().size();

    int32_t outRow = out.GetShape(0);
    int32_t expertScalesRow = expertScales.GetShape(0);
    CHECK(ERROR_CODE_UNKNOWN, outRow == expertScalesRow) << "The first axis of \"out\" must be consistent with that of \"expertScales\", "
        << "but expertScalesRow=" << expertScalesRow << ", outRow=" << outRow;

    int32_t outCol = out.GetShape(1);
    int32_t expandXCol = expandX.GetShape(1);
    CHECK(ERROR_CODE_UNKNOWN, outCol == expandXCol) << "The second axis of \"out\" must be consistent with that of \"expandX\", but "
        << "expandXCol=" << expandXCol << ", outCol=" << outCol;

    CHECK(ERROR_CODE_UNKNOWN, out.GetDataType() == expandX.GetDataType()) << "The data type of \"out\" must be consistent with that of "
    ...
    CHECK(ERROR_CODE_UNKNOWN, out.Format() == npu::tile_fwk::TileOpFormat::TILEOP_ND) << "The format of \"out\" only supports ND, but got "
        << "NZ";
}

void MoeDistributedCombineValidateGroup(const char* group)
{
    CHECK(ERROR_CODE_UNKNOWN, group != nullptr) << "\"group\" cannot be nullptr";
    int32_t groupLen = std::strlen(group);
    int32_t maxGroupLen = 128;
    CHECK(ERROR_CODE_UNKNOWN, (groupLen >= 1) && (groupLen < maxGroupLen)) << "The length of \"group\" only supports [1, " << maxGroupLen
        << "), but got " << groupLen;
}
...
void MoeDistributedCombineValidateMoeEpWorldSize(int32_t epWorldSize)
{
    ...
    CHECK(ERROR_CODE_UNKNOWN, (epWorldSize == supportedEpWorldSize1) || (epWorldSize == supportedEpWorldSize2)) << "epWorldSize only "
...
void MoeDistributedCombineValidateMoeExpertNum(int32_t moeExpertNum)
{
    ...
    CHECK(ERROR_CODE_UNKNOWN, moeExpertNum == supportedMoeExpertNum) << "moeExpertNum only supports " << supportedMoeExpertNum << ", but "
        << "got " << moeExpertNum;
}
...
void TiledMoeDistributedCombineSend(
    ...
    const Operation& op)
{
    ASSERT(ERROR_CODE_UNKNOWN, iOperand.size() == 5UL) << "TiledMoeDistributedCombineSend iOperand size is not equal to 5";
    ASSERT(ERROR_CODE_UNKNOWN, oOperand.size() == 1UL) << "TiledMoeDistributedCombineSend oOperand size is not equal to 1";
    ...
    ASSERT(ERROR_CODE_UNKNOWN, dataByteSize != 0) << "iOperand expandX dType size cannot be zero";
    ...
    const Operation& op)
{
    ...
    ASSERT(ERROR_CODE_UNKNOWN, iOperand.size() == 4UL) << "TiledMoeDistributedCombineReceive iOperand size is not equal to 4";
    ASSERT(ERROR_CODE_UNKNOWN, oOperand.size() == 1UL) << "TiledMoeDistributedCombineReceive oOperand size is not equal to 1";
    ...
    ASSERT(ERROR_CODE_UNKNOWN, dataByteSize != 0) << "oOperand out dType size cannot be zero";
    int64_t paddedColShape = AlignUp(dataByteSize * hiddenSize, COPY_BLOCK_BYTE_SIZE) / dataByteSize;
    int64_t floatByteSize = BytesOf(DataType::DT_FP32);
    ASSERT(ERROR_CODE_UNKNOWN, floatByteSize != 0) << "floatByteSize cannot be zero";
    ...
    uint32_t sharedExpertNum, uint32_t sharedExpertRankNum, Tensor& out)
{
    ...
    LOOP("MoeDistributedCombineReceive", FunctionType::DYNAMIC_LOOP, tokenId, LoopRange(batchSize)) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, (BytesOf(DT_FP32) != 0) && (kTileShape != 0)) << "Divisor kTileShape cannot be zero";
