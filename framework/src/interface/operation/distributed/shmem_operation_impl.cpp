...
Tensor ShmemPutUb2Gm(const Tensor& predToken, const Tensor& in, const Tensor& shmemData,
...
Tensor ShmemGetGm2Ub(const Tensor& predToken, const Tensor& shmemData, DataType nonShmemDataType, AtomicType atomicType)
{
    if (nonShmemDataType == DT_BOTTOM) {
        nonShmemDataType = shmemData.GetDataType();
    }
    ...
    auto &op = function.AddOperation(Opcode::OP_SHMEM_GET_GM2UB, {predToken.GetStorage(), shmemData.GetStorage()},
        {out});
    DistOpAttr distOpAttr;
    distOpAttr.atomicType = atomicType;
    op.SetAttr(OpAttributeKey::distOpAttr, distOpAttr);
