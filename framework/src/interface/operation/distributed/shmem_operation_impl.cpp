...
void ValidateGroup(const char* group)
{
    ...
    CHECK(ERROR_CODE_UNKNOWN, (groupLen >= 1) && (groupLen < 128)) << "The length of \"group\" only supports [1, 128), but got "
        << groupLen;
}

void ValidateTypeAndShape(const Tensor& tensor, const DataType expectedType, const Shape expectedShape)
{
    CHECK(ERROR_CODE_UNKNOWN, tensor.GetDataType() == expectedType) << "Tensor dtype not supported";
    CHECK(ERROR_CODE_UNKNOWN, tensor.GetShape() == expectedShape) << "Tensor shape not supported";
}

void ValidateTilingSize(const Opcode& opCode, const VecTile& vecTile, const int32_t supportDim)
{
    CHECK(ERROR_CODE_UNKNOWN, vecTile.valid()) << OpcodeManager::Inst().GetOpcodeStr(opCode) << ": vecTile must contains exactly " <<
        supportDim << " elements and both must be non-zero";
    CHECK(ERROR_CODE_UNKNOWN, supportDim == static_cast<int32_t>(vecTile.size())) << OpcodeManager::Inst().GetOpcodeStr(opCode) <<
...
void ValidateDataType(const Tensor& tensor, const std::string& tensorDesc, const std::unordered_set<DataType>& allowedTypes)
{
    ...
    oss << "]";
    CHECK(ERROR_CODE_UNKNOWN, allowedTypes.count(dataType)) << "Invalid data type: " << tensorDesc << " data type only support "
...
void ValidateShape(const Tensor& tensor, const std::string& tensorDesc, uint32_t supportedDim) {
    ...
    CHECK(ERROR_CODE_UNKNOWN, shape.size() == supportedDim) << "Invalid dimensional: " << tensorDesc << " dimensional must be "
        << supportedDim << ", but got dimensional=" << shape.size();
    for (size_t i = 0; i < shape.size(); ++i) {
        CHECK(ERROR_CODE_UNKNOWN, shape[i] > 0) << "Invaild dimemsion value: " << tensorDesc << " dimension " << i
            << " must be greater than 0, but got " << shape[i];
    }
}

void ValidateFormat(const Tensor& tensor, const std::string& tensorDesc) {
    CHECK(ERROR_CODE_UNKNOWN, tensor.Format() == TileOpFormat::TILEOP_ND) << "Invalid format: " << tensorDesc
...
void ValidateWorldSize(const char* group, int64_t worldSize) {
    ...
    if (groupWorldSize== groupWorldSizeMap.end()) {
        CHECK(ERROR_CODE_UNKNOWN, worldSize > 0) << "Invalid world size for group " << group << ": world size must be greather than 0"
            << ", but got " << worldSize;
        groupWorldSizeMap.emplace(group, worldSize);
    } else {
        CHECK(ERROR_CODE_UNKNOWN, worldSize == groupWorldSize->second) << "WorldSize mismatch for group " << group
    ...
    const std::unordered_set<DataType>& allowedTypes = {})
{
    ...
    CHECK(ERROR_CODE_UNKNOWN, predRow > 0 && predCol > 0) << "PredToken parameter error - the 'row' and 'col' dimensional of the input tensor must be greater than 0, "
        << "but got row=" << predRow << ", col=" << predCol;
    ValidateShape(in, "Input tensor", 2);
    ValidateShape(out, "Output tensor", 2);
    CHECK(ERROR_CODE_UNKNOWN, out.GetDataType() == in.GetDataType()) << "The data type of \"out\" must be consistent with that of \"in\", "
    ...
    if (checkShapeMatch) {
        CHECK(ERROR_CODE_UNKNOWN, (inRow == outRow) && (inCol == outCol)) <<
    ...
    CHECK(ERROR_CODE_UNKNOWN, shmemSize < winSize) << "Exceeds winSize limit. Maximum allowed: " << winSize << ", got: " << shmemSize;
}
...
Tensor ShmemDataSet(const Tensor& predToken, const Tensor& shmemData)
{
    ...
    CHECK(ERROR_CODE_UNKNOWN, shmemData.GetShape().size() == supportedDim) << "The dim of \"shmemData\" only supports " << supportedDim
...
Tensor ShmemSignalSet(const Tensor& predToken, const Tensor& shmemSignal)
{
    ...
    CHECK(ERROR_CODE_UNKNOWN, shmemSignal.GetShape().size() == supportedDim) << "The dim of \"shmemSignal\" only supports " << supportedDim
    ...
    Tensor& out)
{
    ...
    CHECK(ERROR_CODE_UNKNOWN, worldSize > 0) << "worldSize should be more than 0.";
    ...
    DistReduceType reduceType, Tensor& out)
{
    ...
    uint32_t worldSize = shmemData.GetShape()[0];
    CHECK(ERROR_CODE_UNKNOWN, worldSize > 0) << "worldSize should be more than 0.";
    CHECK(ERROR_CODE_UNKNOWN, (row % worldSize) == 0) << "ReduceScatter constraint: row must be divisible by worldSize, but row: " << row
        << ", worldSize: " << worldSize;
    ...
    Tensor& shmemSignal, Tensor& out)
{
    ...
    int32_t col = in.GetShape(1);
    uint32_t worldSize = shmemData.GetShape()[0];
    CHECK(ERROR_CODE_UNKNOWN, worldSize > 0) << "AllReduce worldSize should be more than 0.";
    CHECK(ERROR_CODE_UNKNOWN, row % worldSize == 0) << "Two_Shot_AllReduce constraint: row must be divisible by worldSize but row: " << row
        << ", worldSize: " << worldSize;
