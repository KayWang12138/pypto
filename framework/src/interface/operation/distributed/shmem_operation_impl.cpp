    ...
    AtomicType putOp, const Tensor& pred, bool isUb2Gm)
{
    ...
    if (src.GetStorage()->GetDynValidShape().size() == 0) {
    ...
    op.SetOpAttribute(std::make_shared<CopyOpAttribute>(
        ...
        OpImmediate::Specified({src.GetShape()}),
        OpImmediate::Specified(src.GetStorage()->GetDynValidShape())));
...
Tensor ShmemGet(const ShmemTensor& src, const SymbolicScalar& srcRank, const Tensor& pred, DataType targetDataType)
{
    ...
    auto &op = function.AddOperation(Opcode::OP_SHMEM_GET, {pred.GetStorage(), src.data.GetStorage()},
        {out});
    if (src.data.GetStorage()->GetDynValidShape().size() == 0) {
    ...
    op.SetOpAttribute(std::make_shared<CopyOpAttribute>(
        ...
        OpImmediate::Specified(std::vector<SymbolicScalar>{
            src.data.GetStorage()->GetDynValidShape()[1], src.data.GetStorage()->GetDynValidShape()[2]})
    ));
    function.UpdateTensorDataUsage(op);
