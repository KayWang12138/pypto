    ...
    AtomicType putOp, const Tensor& pred, bool isUb2Gm)
{
    ...
    auto copyOpAttribute = std::make_shared<CopyOpAttribute>(
        fromType,
        OpImmediate::Specified({0, 0}),
        OpImmediate::Specified({src.GetShape()}),
        OpImmediate::Specified({src.GetShape()}),
        OpImmediate::Specified(src.GetValidShape()));
    auto viewOp = *dst.data.GetStorage()->GetProducers().begin();
    std::shared_ptr<ViewOpAttribute> viewOpAttr = std::dynamic_pointer_cast<ViewOpAttribute>(viewOp->GetOpAttribute());
    copyOpAttribute->SetFromOffset(OpImmediate::Specified(viewOpAttr->GetFromDynOffset()));
    op.SetOpAttribute(copyOpAttribute);
