...
void ValidateShmemTensor(const ShmemTensor& t) {
    ...
    ASSERT(DistributedErrorCode::INVALID_TENSOR_SHAPE, (t.data.GetStorage() != nullptr) || (t.signal.GetStorage() != nullptr)) <<
        "shmem tensor's data and signal should not be empty, data shape: " << ToString(t.data.GetShape()) <<
        ", signal shape: " << ToString(t.signal.GetShape());
    if (t.data.GetStorage() != nullptr) {
    ...
    if (t.signal.GetStorage() != nullptr) {
...
{
    ValidateShmemTensor(shmemTensor);
