...
Tensor ShmemPut(const Tensor &predToken, const Tensor &in, const Tensor &shmemDataTile, AtomicType atomicType)
{
...
Tensor ShmemGet(const Tensor &predToken, const Tensor &shmemDataTile, DataType nonShmemDataType, AtomicType atomicType)
{
...
Tensor WaitUntil(const Tensor &predToken, const Tensor &shmemSignalTile, int32_t expectedSum, bool resetSignal)
{
    auto &function = *Program::GetInstance().GetCurrentFunction();
    auto out = std::make_shared<LogicalTensor>(function, DT_INT32, predToken.GetShape());
    ...
    DistReduceType reduceType, Tensor& out)
{
    ...
    for (uint32_t dynRankId = 0; dynRankId < worldSize; ++dynRankId) {
        ...
        shmemsignalOut = ShmemSignal(shmemPutOut, shmemSignalTile, AtomicType::ADD);
    }
