...
void ValidateTilingSize(const VecTile& vecTile, const Tensor& in, int32_t worldSize)
{
    ...
    ASSERT(tileRowNum * tileColNum <= MAX_TILE_NUM / worldSize) <<
        "TotalTileNum is invalid, totalTileNum shoule be less than " << MAX_TILE_NUM / worldSize << ", but got " <<
        tileRowNum * tileColNum;
}
...
void ValidateDataType(DataType dataType, const std::unordered_set<DataType>& allowedTypes)
{
    ...
    oss << "]";
    ASSERT(allowedTypes.count(dataType)) << "Invalid data type for input tensor. Expected: " << oss.str() <<
...
{
    ASSERT(predToken.GetShape().size() == 2UL) <<
        "Invalid dimensional: PredToken dimensional must be 2, but got dimensional=" << predToken.GetShape().size(); 
    int32_t predRow = predToken.GetShape(0);
    int32_t predCol = predToken.GetShape(1);
    ASSERT(predRow > 0 && predCol > 0) <<
        "PredToken parameter error - the 'row' and 'col' dimensional of the input tensor must be greater than 0, " <<
        "but got row=" << predRow << ", col=" << predCol;
    ASSERT(in.GetShape().size() == 2UL) << "Invalid dimensional: Input dimensional must be 2, but got dimensional=" <<
        in.GetShape().size();
    ASSERT(out.GetShape().size() == 2UL) << "Invalid dimensional: Output dimensional must be 2, but got dimensional=" <<
        out.GetShape().size();
    ...
    ASSERT((in.Format() != TileOpFormat::TILEOP_NZ) && (out.Format() != TileOpFormat::TILEOP_NZ)) <<
        "NZ not supported.";
    ASSERT(inRow > 0 && inCol > 0) <<
        "Input parameter error - the 'row' and 'col' dimensional of the input tensor must be greater than 0, "
        << "but got row=" << inRow << ", col=" << inCol;
    if (checkShapeMatch) {
        ASSERT((inRow == outRow) && (inCol == outCol)) <<
