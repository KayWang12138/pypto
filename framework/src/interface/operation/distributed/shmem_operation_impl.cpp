...
void ValidateDataType(const Tensor& tensor, std::string tensorName, const std::unordered_set<DataType>& allowedTypes)
{
    ...
    oss << "]";
    CHECK(allowedTypes.count(dataType)) << "Invalid data type: " << tensorName << " data type only support "
        << oss.str() << ", but got:" << DataType2CCEStr(dataType);
}

void ValidateShape(const Tensor& tensor, std::string tensorName, uint32_t supportedDim) {
    const auto& shape = tensor.GetShape();
    CHECK(shape.size() == supportedDim) << "Invalid dimensional: " << tensorName << " dimensional must be "
        << supportedDim << ", but got dimensional=" << shape.size();
    for (size_t i = 0; i < shape.size(); ++i) {
        CHECK(shape[i] > 0) << "Invaild dimemsion value: " << tensorName << " dimension " << i
            << " must be greater than 0, but got " << shape[i];
    }
}

void ValidateFormat(const Tensor& tensor, std::string tensorName) {
    CHECK(tensor.Format() == TileOpFormat::TILEOP_ND) << "Invalid format: " << tensorName
        << " only support ND format, but got format: " << std::to_string(tensor.Format());
}

void ValidateWorldSize(const char* group, uint64_t worldSize) {
    auto hcclContext = npu::tile_fwk::dynamic::DistributedContext::GetCommContextToHost(std::vector<std::string>{group})[0];
    auto hcclOpParam = reinterpret_cast<TileOp::CommContext*>(hcclContext);
    CHECK(worldSize == hcclOpParam->rankNum) << "Mismatched world size: Current worldSize (" << worldSize
        << ") does not match the worldSize (" << hcclOpParam->rankNum
        << ") initialized for the HCCL communication domain.";
}
