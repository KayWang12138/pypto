...
void ValidateTensor(const Tensor &tensor, const std::string &tensorDesc,
    const std::unordered_set<size_t> &allowedDims = {}, const std::unordered_set<DataType> &allowedTypes = {},
...
void ValidateShmemTensor(const ShmemTensor &t, bool hasData = false, bool hasSignal = false) {
    ...
    if (groupWorldSize == groupWorldSizeMap.end()) {
        ASSERT(DistributedErrorCode::INVALID_WORLD_SIZE, t.worldSize > 0)
            << "Invalid world size for group " << t.group << ": world size must be greather than 0" << ", but got "
            << t.worldSize;
...
static Tensor ShmemPutImpl(const Tensor &src, const ShmemTensor &dst, const SymbolicScalar &dstRank, AtomicType putOp,
    const Tensor &pred, bool isUb2Gm) {
    ...
    auto &op = isUb2Gm ? function.AddOperation(Opcode::OP_SHMEM_PUT_UB2GM,
