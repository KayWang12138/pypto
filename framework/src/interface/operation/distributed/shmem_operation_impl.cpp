...
void ValidateTiling(const Opcode& opCode, const Tensor& target, const std::string& desc)
{
    ...
    ASSERT(DistributedErrorCode::INVALID_TILE_SHAPE, vecTile.valid())
        << ToString(opCode) << ": vecTile should not empty, and all value should > 0, but got:" << ToString(vecTile.tile);
...
void ValidateDim(const Tensor& tensor, const std::string& desc, const std::unordered_set<size_t>& allowedDims)
{
    ...
    ASSERT(DistributedErrorCode::INVALID_TENSOR_DIM, allowedDims.empty() || allowedDims.count(shape.size())) << "Invalid dimensional: " << desc <<
...
void ValidateShape(const Tensor& tensor, const std::string& desc, const Shape& expectShape)
{
    ...
    ASSERT(DistributedErrorCode::INVALID_TENSOR_SHAPE,
        ...
        desc << ", all value must be greater than 0, but got " << ToString(shape);
...
void ValidateTensor(const Tensor& tensor, const std::string& desc,
    ...
    const std::vector<SymbolicScalar>& validShapes = {})
{
    ...
    ASSERT(DistributedErrorCode::INVALID_SHMEM_VIEW_PARAM, signalShape.size() > shapes.size()) <<
        ...
        shapes.size() << ", signal dim:" << signalShape.size() <<
        ", maybe you should check if local tensor shape dim is valid";
    ASSERT(DistributedErrorCode::INVALID_SHMEM_VIEW_PARAM, signalShape.size() > offsets.size()) <<
        "input offsets dim should smaller than signal dim, input offsets dim:" <<
        offsets.size() << ", signal dim:" << signalShape.size();
    ...
    bool isUb2Gm)
{
    ...
    ValidateTensor(dst.data, "data of shmem tensor", {3}, {shmemDataType}, {TileOpFormat::TILEOP_ND}, {1, src.GetShape(0), src.GetShape(1)});
...
Tensor ShmemGet(const ShmemTensor& src, const SymbolicScalar& srcRank, const Tensor& pred, DataType targetDataType)
{
    ...
    ValidateTensor(src.data, "data of shmem tensor", {3}, {DT_INT32, DT_FP32, DT_FP16, DT_BF16}, {TileOpFormat::TILEOP_ND},
...
{
    ValidateShmemTensor(src, true);
    ValidateTensor(src.data, "data of shmem data", {3}, {DT_INT32, DT_FP32, DT_FP16, DT_BF16}, {TileOpFormat::TILEOP_ND},
        {1, src.data.GetShape(1), src.data.GetShape(2)});
    ValidateTensor(pred, "pred tensor", {2});
...
void AllGather(const Tensor& predToken, const Tensor& in, ShmemTensor& shmemTensor, Tensor& out)
{
    ...
    ValidateTensor(shmemTensor.data, "data of shmem tensor", {}, {in.GetDataType()}, {in.Format()}, {worldSize, row, col});
