...
static Tensor ShmemSignalImpl(const ShmemTensor& dst, const SymbolicScalar &dstRank, const SymbolicScalar &notifyRank,
...
{
    return ShmemSignalImpl(dst, dstRank, notifyRank, signal, sigOp, pred);
}

Tensor ShmemSignal(const ShmemTensor& dst, const SymbolicScalar &dstRank, int32_t signal, AtomicType sigOp, const Tensor& pred)
{
    return ShmemSignalImpl(dst, dstRank, dstRank, signal, sigOp, pred);
}
...
{
    return ShmemSignalImpl(dst, dstRank, 0, signal, sigOp, pred, true);
}
...
static Tensor ShmemClearImpl(const ShmemTensor& src, Tensor &pred, bool clearData)
{
    ...
    auto& op = function.AddOperation(Opcode::OP_SHMEM_SET, {pred.GetStorage(), clearData ? src.data.GetStorage() : src.signal.GetStorage()}, {out});
