    ...
    DistReduceType reduceType, Tensor& out)
{
    ...
    uint32_t worldSize = shmemData.GetShape()[0];
    ASSERT(worldSize > 0) << "worldSize should be more than 0.";
    ASSERT((row % worldSize) == 0) << "ReduceScatter constraint: row must be divisible by worldSize row:" << row
        << " worldSize : " << worldSize; 
    ...
    Tensor& shmemSignal, Tensor& out)
{
    ...
    ASSERT(row % worldSize == 0) << "Two_Shot_AllReduce constraint: row must be divisible by worldSize row:" << row
        << " worldSize : " << worldSize; 
