...
std::pair<int32_t, int32_t> GetworldSizeAndTileCount()
{
    const TileShape& tileShape = TileShape::Current();

    auto rankShape = tileShape.GetDistTileRank();
    int32_t worldSize = rankShape[0] * rankShape[1] + rankShape[2];
...
void ShmemBarrier(const Tensor& predToken, Tensor& shmemSignal, const char* group, uint32_t worldSize, Tensor& out)
{
    ...
    auto shmemSignalTile = View(shmemSignal, {worldSize, 1, 1, shmemSignal.GetShape(0), shmemSignal.GetShape(1)}, 
        std::vector<SymbolicScalar>{0, 0, 0, 0, 0});
    auto shmemSignalOut = ShmemSignal(predToken, shmemSignalTile, AtomicType::ADD);
    auto shmemSignalLocal = View(shmemSignal, {1, 1, 1, shmemSignal.GetShape(0), shmemSignal.GetShape(1)}, 
...
void OneShotAllReduceImpl(const Tensor& predToken, const Tensor& in, const Tensor& shmemData,
...
void TwoShotAllReduceImpl(const Tensor& predToken, const Tensor& in, const Tensor& shmemData,
    const Tensor& shmemSignal, const char* group, uint32_t worldSize, Tensor& out)
{
    int32_t row = in.GetShape(0);
    int32_t col = in.GetShape(1);
...
void CreateShmemTensorAndAllReduce(const Tensor& predToken, const Tensor& in, const char* group, uint32_t worldSize, Tensor& out)
{
    ...
    if constexpr (allReduceType == AllReduceType::ONE_SHOT) {
        OneShotAllReduceImpl(predToken, in, shmemData, shmemSignal, group, worldSize, out);
    } else {
        TwoShotAllReduceImpl(predToken, in, shmemData, shmemSignal, group, worldSize, out);
    }
...
void ShmemMoeCombine(const Tensor& in, const Tensor& combineInfo, const Tensor& scale, const char* group,
    int32_t worldSize, int32_t totalExpertNum, Tensor& out)
{
    ...
    int32_t expectedRow = std::min(topK * batchSize * worldSize, batchSize * totalExpertNum);

    ASSERT(inRow == expectedRow) << "The first axis of \"in\" must be the smaller value between topK (the second axis "
        << "of \"scale\") * batchSize (the first axis of \"scale\") * worldSize and batchSize * totalExpertNum, topK="
        << topK << ", batchSize=" << batchSize << ", worldSize=" << worldSize << ", totalExpertNum=" << totalExpertNum
    ...
    ASSERT((worldSize == 4) || (worldSize == 8)) << "worldSize only supports 4 or 8, but got " << worldSize;
    ...
    LOOP("CreateShmemTensor", FunctionType::DYNAMIC_LOOP, index, LoopRange(1)) {
        (void)index;
        CreateShmemData(group, worldSize, in.GetDataType(), shmemDataShape, shmemData);
        CreateShmemData(group, worldSize, DT_INT32, shmemSignalShape, shmemData);
    }
