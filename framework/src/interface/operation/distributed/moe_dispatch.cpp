    ...
    Tensor &validCnt, Tensor &combineInfo, const char *group, const MoeConfig &moeConfig)
{
    ...
    CHECK(batchSize > 0) << "MoeDispatch constraint violated: batchSize must be > 0, but got " << batchSize;
    CHECK(hiddenSize > 0) << "MoeDispatch constraint violated: hiddenSize must be > 0, but got " << hiddenSize;
    ...
    int32_t expandXRow = std::min(static_cast<int32_t>(batchSize) * static_cast<int32_t>(topK) * moeConfig.rankNum,
    ...
    Tensor &validCnt, Tensor &combineInfo, const char *group, const MoeConfig &moeConfig)
{
    ...
    LOOP("L0", FunctionType::DYNAMIC_LOOP, index, LoopRange(1)) {
        ...
        auto combineInfoPtr = DispatchFFNCombineInfo(group, tokenTensor, recvTokenCntOut, shmemDataBatching,
            localShmemFlag, expandX.GetShape(0), ffnTileNum + ffnTailNum, moeConfig);
        int32_t validCntTileNum = std::min<int32_t>(10, moeConfig.expertNumPerRank);
        int32_t validCntTileShape = moeConfig.expertNumPerRank / validCntTileNum;
        int32_t validCntTileTail = moeConfig.expertNumPerRank % validCntTileNum;
        if (validCntTileShape == 0) {
            validCntTileShape = 1;
            validCntTileNum = moeConfig.expertNumPerRank;
            validCntTileTail = 0;
        }
    ...
    Tensor& expertTokenNums, Tensor& assistInfoForCombine, Tensor& recvCounts)
{
    ...
    CHECK(moeExpertNum > 0) << "MoeDispatch constraint violated: moeExpertNum must be > 0, but got " << moeExpertNum;
    ...
    CHECK(routedExpertNum > 0) << "MoeDispatch constraint violated: routedExpertNum must be > 0, but got "
        << routedExpertNum;
    CHECK((routedExpertNum % static_cast<int32_t>(epWorldSize)) == 0) << "MoeDispatch constraint violated: "
        << "routedExpertNum must be divisible by epWorldSize, but got routedExpertNum=" << routedExpertNum
        << ", epWorldSize=" << epWorldSize;
    ...
    CHECK(hiddenSize > 0) << "MoeDispatch constraint violated: hiddenSize must be > 0, but got " << hiddenSize;
    CHECK(topK > 0) << "MoeDispatch constraint violated: topK must be > 0, but got " << topK;
    CHECK(checkValidInput(x, 2, DataType::DT_BF16, batchSize, hiddenSize, assertResult)) << assertResult;
    CHECK(checkValidInput(expertIds, 2, DataType::DT_INT32, batchSize, topK, assertResult)) << assertResult;
    ...
    CHECK(checkValidInput(expandX, 2, DataType::DT_BF16, expandXRow, hiddenSize, assertResult)) << assertResult;
    CHECK(checkValidInput(assistInfoForCombine, 2, DataType::DT_INT32, expandXRow, 64, assertResult)) << assertResult;
