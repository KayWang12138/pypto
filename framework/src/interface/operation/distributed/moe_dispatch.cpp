    ...
    Tensor& assistInfoForCombine, Tensor& expertTokenNums, Tensor& recvCounts)
{
    ...
    LOOP("MoeDistributedDispatchSendData", FunctionType::DYNAMIC_LOOP, index, LoopRange(topK * batchSize)) {
        ...
        auto shmemDataTile = ShmemView(shmemData, {1, 1, hiddenSize}, std::vector<SymbolicScalar>{(remoteExpertOffset * epWorldSize + thisRank) * batchSize + tokenOffset, 0});
        ...
        auto shmemInfoTile = ShmemView(shmemInfo, {1, 1, infoSize}, std::vector<SymbolicScalar>{(remoteExpertOffset * epWorldSize + thisRank) * batchSize + tokenOffset, 0});
    ...
    LOOP("MoeDistributedDispatchReceive", FunctionType::DYNAMIC_LOOP, i, LoopRange(1)) {
        ...
        for (uint32_t index = 0; index < expertNumPerRank * epWorldSize; ++index) {
            ...
            auto curShmemDataTile = ShmemView(shmemData, {batchSize, hiddenSize},
                std::vector<SymbolicScalar>{batchSize + curCount, hiddenSize}, {index * batchSize, 0});
            ...
            auto curShmemInfoTile = ShmemView(shmemInfo, {batchSize, assistInfoForCombine.GetShape(1)},
                std::vector<SymbolicScalar>{batchSize + curCount, assistInfoForCombine.GetShape(1)}, {index * batchSize, 0});
