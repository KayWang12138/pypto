...
void TiledDispatchDispatchAssembleCombineInfo(Function& function, const TileShape& tileShape,
    const std::vector<std::shared_ptr<LogicalTensor>>& iOperand,
    const std::vector<std::shared_ptr<LogicalTensor>>& oOperand, const Operation& op)
{
...
Tensor DispatchDispatchAssembleCombineInfo(const char *group, const Tensor &x,
...
std::tuple<Tensor, Tensor> DispatchFFNBatching(const char *group, const Tensor &x, 
    const Tensor &recvTokenCntOut, const Tensor &shmemData, const Tensor &shmemFlag, 
    int32_t expandXRow, int32_t ffnTileNum, uint32_t expertNumPerRank)
{
    auto &function = *Program::GetInstance().GetCurrentFunction();
...
Tensor DispatchDispatchWaitRecvFlags(const char *group, const Tensor &flagDummy, Tensor &shmemFlag, uint32_t expertNumPerRank, int32_t ffnTileNum)
{
    auto &function = *Program::GetInstance().GetCurrentFunction();
...
Tensor DispatchSendTokensToExperts(const Tensor &shmemData, const Tensor &x,
    ...
    Tensor &expertTokenNums, Tensor &assistInfoForCombine, Tensor& recvCounts)
{
    ...
    LOOP("L0", FunctionType::DYNAMIC_LOOP, index, LoopRange(1)) {
        ...
        Tensor syncTensor = DispatchSendTokensToExperts(shmemData, x, expertIds, group, expertNumPerRank);
        TileShape::Current().SetDistTile(
            {flagRow, 1, 0},
            {static_cast<int32_t>(epWorldSize), 1, 0},
        ...
        auto shmemFlagSched = View(shmemFlag, {1, expertNumPerRank, epWorldSize, flagCol},
            {thisRank, 0, 0, 0});
        auto recvTokenCntOut = DispatchDispatchWaitRecvFlags(group, flagDummy, shmemFlagSched, expertNumPerRank, ffnTileNum);
        auto shmemDataBatching = View(shmemData, {1, epWorldSize, expertNumPerRank, shmemDataLength},
            {thisRank, 0 ,0 ,0});
        auto [expandXPtr, recvCountsPtr] = DispatchFFNBatching(group, x, recvTokenCntOut, shmemDataBatching,
            localShmemFlag, expandX.GetShape(0), ffnTileNum + ffnTailNum, expertNumPerRank);
        auto assistInfoForCombinePtr = DispatchDispatchAssembleCombineInfo(group, x, recvTokenCntOut, shmemDataBatching,
            localShmemFlag, expandX.GetShape(0), ffnTileNum + ffnTailNum, expertNumPerRank);
