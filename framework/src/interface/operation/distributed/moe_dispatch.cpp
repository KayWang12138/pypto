    ...
    Tensor &validCnt, Tensor &combineInfo, const char *group, const MoeConfig &moeConfig)
{
    ...
    CHECK(ERROR_CODE_UNKNOWN, checkValidConfig(moeConfig, assertResult)) << assertResult;
    CHECK(ERROR_CODE_UNKNOWN, group != nullptr) << "MoeDispatch constraint violated: group name can't be nullptr.";
    CHECK(ERROR_CODE_UNKNOWN, group[0] != '\0') << "MoeDispatch constraint violated: group name is not valid.";
    CHECK(ERROR_CODE_UNKNOWN, strnlen(group, 128) < 128) << "MoeDispatch constraint violated: group name max size must be 128.";
    CHECK(ERROR_CODE_UNKNOWN, checkValidInput(tokenTensor, 2, DataType::DT_BF16, 8, 5120, assertResult)) << assertResult; // 当前仅支持shape:8,5120
    ...
    CHECK(ERROR_CODE_UNKNOWN, checkValidInput(tokenExpertTable, 2, DataType::DT_INT32, 8, 8, assertResult)) << assertResult; // 当前仅支持shape:8,8
    CHECK(ERROR_CODE_UNKNOWN, checkValidInput(validCnt, 1, DataType::DT_INT32, moeConfig.expertNumPerRank, 1, assertResult)) << assertResult;
    CHECK(ERROR_CODE_UNKNOWN, checkValidInput(expandX, 2, DataType::DT_BF16, expandXRow, 5120, assertResult)) << assertResult; // 当前仅支持hiddenSize:5120
    CHECK(ERROR_CODE_UNKNOWN, checkValidInput(combineInfo, 2, DataType::DT_INT32, expandXRow, 3, assertResult)) << assertResult; // comBineInfo固定hiddenSize:3
}
    ...
    Tensor& expertTokenNums, Tensor& assistInfoForCombine, Tensor& recvCounts)
{
    std::string assertResult;
    CHECK(ERROR_CODE_UNKNOWN, group != nullptr) << "MoeDispatch constraint violated: group name can't be nullptr.";
    CHECK(ERROR_CODE_UNKNOWN, group[0] != '\0') << "MoeDispatch constraint violated: group name must be valid, but got '\0'";
    CHECK(ERROR_CODE_UNKNOWN, strnlen(group, 128) < 128) << "MoeDispatch constraint violated: group name max size must be 128, but got " << strnlen(group, 128);
    CHECK(ERROR_CODE_UNKNOWN, epWorldSize > 0) << "MoeDispatch constraint violated: epWorldSize must be > 0, but got " << epWorldSize;
    CHECK(ERROR_CODE_UNKNOWN, moeExpertNum == 160) << "MoeDispatch constraint violated: moeExpertNum must 160, but got " << moeExpertNum;
    CHECK(ERROR_CODE_UNKNOWN, sharedExpertNum == 0) << "MoeDispatch constraint violated: sharedExpertNum must 0, but got " << sharedExpertNum;
    CHECK(ERROR_CODE_UNKNOWN, sharedExpertRankNum == 0) << "MoeDispatch constraint violated: sharedExpertRankNum must 0, but got " << sharedExpertRankNum;
    int32_t routedExpertNum =  moeExpertNum - sharedExpertNum;
    int32_t expertNumPerRank = routedExpertNum / epWorldSize;
    CHECK(ERROR_CODE_UNKNOWN, checkValidInput(x, 2, DataType::DT_BF16, 8, 5120, assertResult)) << assertResult;
    CHECK(ERROR_CODE_UNKNOWN, checkValidInput(expertIds, 2, DataType::DT_INT32, 8, 8, assertResult)) << assertResult;
    CHECK(ERROR_CODE_UNKNOWN, checkValidInput(expertTokenNums, 1, DataType::DT_INT32, expertNumPerRank, 1, assertResult)) << assertResult;
    CHECK(ERROR_CODE_UNKNOWN, checkValidInput(recvCounts, 1, DataType::DT_INT32, 1, 0, assertResult)) << assertResult;
    ...
    CHECK(ERROR_CODE_UNKNOWN, checkValidInput(expandX, 2, DataType::DT_BF16, expandXRow, 5120, assertResult)) << assertResult;
    CHECK(ERROR_CODE_UNKNOWN, checkValidInput(assistInfoForCombine, 2, DataType::DT_INT32, expandXRow, 3, assertResult)) << assertResult;
    ...
    CHECK(ERROR_CODE_UNKNOWN, shmemSize < winSize) << "Exceeds winSize limit. Masxmum allowed: " << winSize << ", got: " << shmemSize;
}
    ...
    Tensor& assistInfoForCombine, Tensor& expertTokenNums, Tensor& recvCounts)
{
    ...
    CHECK(ERROR_CODE_UNKNOWN, topK > 0) << "MoeDispatch constraint violated: topK must be > 0, but got " << topK;
    CHECK(ERROR_CODE_UNKNOWN, expertNumPerRank > 0) << "MoeDispatch constraint violated: expertNumPerRank must be > 0, but got " << expertNumPerRank;
    Tensor shmemData;
    Tensor shmemInfo;
    Tensor shmemCount;
