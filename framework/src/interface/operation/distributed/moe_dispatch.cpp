...
Tensor DispatchFFNSched(const char *group, const Tensor &flagDummy, Tensor &shmemFlag, const MoeConfig &moeConfig, int32_t ffnTileNum)
{
    auto &function = *Program::GetInstance().GetCurrentFunction();
    int32_t totalTileNum = moeConfig.expertNumPerRank * ffnTileNum;
    Shape shape = {totalTileNum * totalTileNum, 128}; // 每个flag_count预留512个int存储
    ...
    Tensor &validCnt, Tensor &combineInfo, const char *group, const MoeConfig &moeConfig)
{
    ...
    LOOP("CreateShmemTensor", FunctionType::DYNAMIC_LOOP, index, LoopRange(1)) {
        ...
        CreateShmem(shmemData, moeConfig.rankNum, hcclGroupIndex, tokenTensor.GetDataType(), shmemDataShape);
        CreateShmem(shmemFlag, moeConfig.rankNum, hcclGroupIndex, DT_INT32, shmemFlagShape);
    }
    LOOP("L0", FunctionType::DYNAMIC_LOOP, index, LoopRange(1)) {
        ...
        auto shmemFlagSched = View(shmemFlag, {1, moeConfig.expertNumPerRank, moeConfig.rankNum, flagCol},
            {thisRank, 0, 0, 0});
        auto recvTokenCntOut = DispatchFFNSched(group, flagDummy, shmemFlagSched, moeConfig, ffnTileNum);
