...
void CreateShmemDispatchLoop(Tensor& shmemData, Tensor& shmemFlag, const char *group,
    const MoeConfig &moeConfig, int32_t shmemDataCol, int32_t flagCol)
{
    LOOP("CreateShmemTensor", FunctionType::DYNAMIC_LOOP, index, LoopRange(1)) {
        ...
        CreateShmemData(group, moeConfig.rankNum, tokenTensor.GetDataType(), shmemDataShape, shmemData, 0);
    ...
    Tensor &validCnt, Tensor &combineInfo, const char *group, const MoeConfig &moeConfig)
{
    ...
    Tensor shmemData;
    Tensor shmemFlag;
    CreateShmemDispatchLoop(shmemData, shmemFlag, group, moeConfig, shmemDataCol, flagCol);
...
void CreateShmemDispatchV2Loop(Tensor& shmemData, Tensor& shmemInfo, Tensor& shmemCount, 
    Tensor& shmemCountSignal, Tensor& shmemDataSignal, const Tensor& x, const Tensor& assistInfoForCombine,
    const char* group, int32_t expertNumPerRank, int32_t epWorldSize, int32_t routedExpertNum)
{
    int32_t countSize = 8;
    ...
    LOOP("CreateShmemTensor", FunctionType::DYNAMIC_LOOP, index, LoopRange(1)) {
        (void) index;
        Shape shmemDataShape = {expertNumPerRank * epWorldSize, x.GetShape(0), x.GetShape(1)};
        Shape shmemInfoShape = {expertNumPerRank * epWorldSize, x.GetShape(0), assistInfoForCombine.GetShape(1)};
        Shape shmemCountShape = {1, cumSumRowShape, countSize};
        Shape shmemCountSignalShape = {1, routedExpertNum, 1, signalCol};
...
void MoeDistributedDispatchPrepare(Tensor& offsetTable, Tensor& expertIdsVec, const Tensor& expertIds)
{
    int32_t expertIdsSize = expertIdsVec.GetShape(1);
    ...
    LOOP("MoeDistributedDispatchPrepare", FunctionType::DYNAMIC_LOOP, i, LoopRange(1)) {
        (void) i;
        for (int index = 0; index < expertIdsSize; ++index) {
            int32_t rowIndex = index / topK;
            int32_t colIndex = index % topK;
            TileShape::Current().SetVecTile({1, sizeof(int32_t)});
...
void MoeDistributedDispatchSendData(const Tensor& x, const Tensor& expertIds, const Tensor& assistInfoForCombine, 
    const Tensor& shmemData, const Tensor& shmemInfo, const Tensor& shmemDataSignal, const Tensor& offsetTable,
    int32_t epWorldSize, int32_t expertNumPerRank, SymbolicScalar thisRank)
{
    int32_t batchSize = x.GetShape(0);
    int32_t hiddenSize = x.GetShape(1);
    int32_t topK = expertIds.GetShape(1);
    int32_t infoSize = shmemInfo.GetShape(3);
    int32_t signalCol = shmemDataSignal.GetShape(4);
    ASSERT(topK > 0) << "MoeDispatch constraint violated: topK must be > 0, but got " << topK;
...
void MoeDistributedDispatchSendCount(const Tensor& shmemCount, const Tensor& shmemCountSignal, Tensor& expertIdsVec,
    int32_t expertIdsSize, int32_t moeExpertNum, int32_t expertNumPerRank, int32_t epWorldSize, SymbolicScalar thisRank)
{
    int32_t signalCol = shmemCountSignal.GetShape(4);
    ASSERT(expertNumPerRank > 0) << "MoeDispatch constraint violated: expertNumPerRank must be > 0, but got " << expertNumPerRank;
    LOOP("MoeDistributedDispatchSendCount", FunctionType::DYNAMIC_LOOP, expertId, LoopRange(moeExpertNum)) {
        Tensor expertOffset = DispatchCalcOccurrences(expertIdsVec, expertId, expertIdsSize);
        ...
        Tensor totalOffsetTile = View(expertOffset, {1, 1}, {0, expertIdsSize - 1});
        ...
        ShmemSignal(shmemPutOut, shmemCountSignalTile, AtomicType::ADD);
    }
}

void MoeDistributedDispatchCumSum(Tensor& recvCounts, Tensor& expertTokenNums, const Tensor& shmemDataSignal,
    const Tensor& shmemCountSignal, const Tensor& shmemCount, Tensor& cumSumResult, Tensor& localExpertRecvCount, 
    int32_t moeExpertNum, int32_t expertNumPerRank, int32_t expertIdsSize, SymbolicScalar thisRank)
{
    int32_t countSize = shmemCount.GetShape(3);
    int32_t cumSumRowShape = shmemCount.GetShape(2);
    int32_t signalCol = shmemCountSignal.GetShape(4);
    int32_t epWorldSize = shmemCountSignal.GetShape(0);
    LOOP("MoeDistributedDispatchCumSum", FunctionType::DYNAMIC_LOOP, i, LoopRange(1)) {
        ...
        Tensor waitUntilOut1 = WaitUntil(cumSumResult, shmemDataSignalLocalTile, expertIdsSize * epWorldSize, true);
        TileShape::Current().SetVecTile({1, signalCol});
        ...
        Tensor shmemCountTile = View(shmemCount, {1, 1, cumSumRowShape, countSize}, std::vector<SymbolicScalar>{1, 1, expertNumPerRank * epWorldSize + 1, countSize}, {thisRank, 0, 0, 0});
...
void MoeDistributedDispatchReceive(const Tensor& shmemData, const Tensor& shmemInfo,
    const Tensor& cumSumResult, const Tensor& localExpertRecvCount, Tensor& expandX, Tensor& assistInfoForCombine,
    int32_t expertNumPerRank, SymbolicScalar thisRank)
{
    int32_t epWorldSize = shmemData.GetShape(0);
...
void ShmemMoeDistributedDispatch(const Tensor& x, const Tensor& expertIds, const char* group,
    int32_t epWorldSize, int32_t moeExpertNum, int32_t sharedExpertNum, int32_t sharedExpertRankNum, Tensor& expandX,
    Tensor& assistInfoForCombine, Tensor& expertTokenNums, Tensor& recvCounts)
{
    ...
    ASSERT(epWorldSize > 0) << "MoeDispatch constraint violated: epWorldSize must be > 0, but got " << epWorldSize;
    int32_t expertNumPerRank = moeExpertNum / epWorldSize;
    int32_t batchSize = x.GetShape(0);
    int32_t hiddenSize = x.GetShape(1);
    int32_t topK = expertIds.GetShape(1);
    int32_t expertIdsSize = batchSize * topK;
    Tensor shmemData;
    Tensor shmemInfo;
    Tensor shmemCount;
    Tensor shmemCountSignal;
    Tensor shmemDataSignal;
    CreateShmemDispatchV2Loop(shmemData, shmemInfo, shmemCount, shmemCountSignal,
        shmemDataSignal, x, assistInfoForCombine, group, expertNumPerRank, epWorldSize, moeExpertNum);
    int32_t countSize = shmemCount.GetShape(3);
    int32_t cumSumRowShape = shmemCount.GetShape(2);
    SymbolicScalar thisRank = GetHcclRankId(group);
    Tensor expertIdsVec = Reshape(expertIds, {1, expertIdsSize});
    Tensor offsetTable(DataType::DT_INT32, {batchSize, topK}, "offsetTable");
    MoeDistributedDispatchPrepare(offsetTable, expertIdsVec, expertIds);
    MoeDistributedDispatchSendData(x, expertIds, assistInfoForCombine, shmemData, shmemInfo, 
        shmemDataSignal, offsetTable, epWorldSize, expertNumPerRank, thisRank);
    MoeDistributedDispatchSendCount(shmemCount, shmemCountSignal, expertIdsVec, expertIdsSize, 
        moeExpertNum, expertNumPerRank, epWorldSize, thisRank);
    ...
    MoeDistributedDispatchCumSum(recvCounts, expertTokenNums, shmemDataSignal, shmemCountSignal, shmemCount, 
        cumSumResult, localExpertRecvCount, moeExpertNum, expertNumPerRank, expertIdsSize, thisRank);
    MoeDistributedDispatchReceive(shmemData, shmemInfo, cumSumResult, localExpertRecvCount, expandX, assistInfoForCombine,
        expertNumPerRank, thisRank);
}
}
}