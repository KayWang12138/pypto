...
void CreateShmemDispatchV2(Tensor& shmemData, Tensor& shmemInfo, Tensor& shmemCount, 
    ...
    int32_t expertNumPerRank, SymbolicScalar thisRank)
{
    ...
    LOOP("MoeDistributedDispatchReceive", FunctionType::DYNAMIC_LOOP, i, LoopRange(1)) {
        (void) i;
        for (int32_t index = 0; index < expertNumPerRank * epWorldSize; ++index) {
    ...
    Tensor& assistInfoForCombine, Tensor& expertTokenNums, Tensor& recvCounts)
{
    ...
    CreateShmemDispatchV2(shmemData, shmemInfo, shmemCount, shmemCountSignal,
