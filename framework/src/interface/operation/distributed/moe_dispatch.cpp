    ...
    const Tensor& input, uint64_t dim, DataType dType, int32_t row, int32_t col)
{
    ...
    ASSERT(DistributedErrorCode::INVALID_TENSOR_SHAPE, input.Dim() != 1 && input.GetShape(1) == col)
    ...
    Tensor& expertTokenNums, Tensor& recvCounts)
{
    ...
    ASSERT(DistributedErrorCode::INVALID_EXPERT_NUM_PER_RANK, expertNumPerRank > 0) << "MoeDispatch constraint violated: expertNumPerRank must be > 0, but got "
