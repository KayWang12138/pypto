...
void OpcodeManager::RegisterInfo(Opcode opcode, OpCoreType coreType, std::string str,
    ...
    ASSERT(ERROR_CODE_UNDEFINED, opcode < Opcode::OP_UNKNOWN) << "opcode: " << static_cast<int64_t>(opcode);
    ASSERT(ERROR_CODE_UNDEFINED, strToEnum_.count(str) == 0) << str << " doesn't exist.";
    ASSERT(ERROR_CODE_UNDEFINED, registered_.count(opcode) == 0) << "opcode: " << static_cast<int64_t>(opcode) << " not registered.";
...
OpcodeManager::OpcodeManager() {
    ...
    RegisterDistribute();
    RegisterCommon();
    ASSERT(ERROR_CODE_UNDEFINED, strToEnum_.size() == static_cast<size_t>(Opcode::OP_UNKNOWN));
}
