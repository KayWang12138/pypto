...
bool IsSFANonDB(
    ...
    constexpr int64_t dimKScene1 = 576;
    constexpr int64_t dimNScene1 = 2048;
    constexpr int64_t dimKScene2 = 2048;
    constexpr int64_t dimNScene2 = 512;
    ...
    const bool aMatrixValid1 = (aMatrixM == dimM && aMatrixK == dimKScene1);
    const bool bMatrixValid1 = (bMatrixK == dimKScene1 && bMatrixN == dimNScene1);
    const bool scene1Valid = aMatrixValid1 && bMatrixValid1;

    const bool aMatrixValid2 = (aMatrixM == dimM && aMatrixK == dimKScene2);
    const bool bMatrixValid2 = (bMatrixK == dimKScene2 && bMatrixN == dimNScene2);
    const bool scene2Valid = aMatrixValid2 && bMatrixValid2;

    const bool dataTypeValid = (operandVec[0]->Datatype() == DataType::DT_BF16);

    return (tileValid && (scene1Valid || scene2Valid) && dataTypeValid);
}
