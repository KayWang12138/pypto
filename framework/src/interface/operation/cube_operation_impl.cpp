...
void CheckOperandDtype(DataType outType, const Tensor &operand1, const Tensor &operand2) {
    ...
    if (operand1.GetDataType() == DataType::DT_FP8E5M2 || operand1.GetDataType() == DataType::DT_FP8E4M3) {
        OP_CHECK(true, {
            ASSERT(Platform::Instance().GetSoc().GetNPUArch() == NPUArch::DAV_3510)
                << "Float8 is only supported on 950PR and 950DT platform." << std::endl;
        });
        OP_CHECK(true, {
            ASSERT(operand2.GetDataType() == DataType::DT_FP8E5M2 || operand2.GetDataType() == DataType::DT_FP8E4M3)
        ...
        if (operand1.GetDataType() == DataType::DT_FP8E5M2) {
            OP_CHECK(true, {
                ASSERT(operand1.Format() == TileOpFormat::TILEOP_ND)
                    << "When operand1 data type is DT_FP8E5M2, format must be ND.";
            });
        }
        if (operand2.GetDataType() == DataType::DT_FP8E5M2) {
            OP_CHECK(true, {
                ASSERT(operand2.Format() == TileOpFormat::TILEOP_ND)
