...
const int64_t UBSIZE_ASCEND910B1 = 196608;
const int64_t UBSIZE_ASCEND950PR_9579 = 253952;
...
// 根据SoC版本获取UB大小
static int GetUbSizeBySocVersion() {
    std::string socVersion;
    OP_CHECK(true, {
        ASSERT(CannHostRuntime::Instance().GetSocVersion(socVersion))
            << "Can not get platform info when construct tensorGraph" << std::endl;
    });
    return (socVersion == "Ascend910B1") ? UBSIZE_ASCEND910B1 : UBSIZE_ASCEND950PR_9579;
}

// 根据UB大小设置VecTile
static void SetVecTileBasedOnUbSize(DataType outType, const CubeTile &cubeTile, int ubSize) {
    auto &cubeTile = TileShape::Current().GetCubeTile();
...
static Tensor GetGmAtomicAccumulationTensor(DataType outType, std::vector<Tensor> gmPartialSums,
...
static Tensor AssembleGmAccumulationTensor(DataType outType, const Tensor gmAccumulationTensor,
    ...
    OP_CHECK(true,
        { ASSERT(gmAccumulationTensor.GetStorage() != nullptr) << "Can not get gmAccumulationTensor's storage" << std::endl; });
...
static Tensor ConstructGmAccumulationTensorGraph(
    ...
    int64_t ubSize = GetUbSizeBySocVersion();

    SetVecTileBasedOnUbSize(outType, cubeTile, ubSize);
    ...
    for (int64_t kIdx = 0; kIdx < kLoop; ++kIdx) {
        ...
        if (outType == DT_INT8) {
            tensorGraphNodes.gmAccumulationTensorPtr = gmAccumulationTensor;
        }
        Tensor gmPartialSum = ConstructTensorGraph(outType, tensorGraphNodes, attrParam);
        gmPartialSums.emplace_back(gmPartialSum);
    }
    return (outType == DT_INT8) ? GetGmAtomicAccumulationTensor(
                                      outType, {mSize, nSize}, {mValidShape, nValidShape}, attrParam.isCMatrixNZ) :
