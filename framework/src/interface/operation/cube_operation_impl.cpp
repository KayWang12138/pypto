...
Status CheckOperandShape(const Tensor &operand1, const Tensor &operand2) {
    ...
    // 检查 shape 维度一致性
    MATMUL_CHECK(ERR_PARAM_MISMATCH, operand1.GetShape().size() == operand2.GetShape().size(),
        "Shape dimension mismatch: operand1=%zu, operand2=%zu", operand1.GetShape().size(), operand2.GetShape().size());

    // 检查 shape 与 offset 的一致性
    MATMUL_CHECK(ERR_PARAM_MISMATCH, operand1.GetShape().size() == operand1.GetStorage()->offset.size(),
        "operand1 shape size(%zu) != offset size(%zu)", operand1.GetShape().size(),
        operand1.GetStorage()->offset.size());

    MATMUL_CHECK(ERR_PARAM_MISMATCH, operand2.GetShape().size() == operand2.GetStorage()->offset.size(),
        "operand2 shape size(%zu) != offset size(%zu)", operand2.GetShape().size(),
        operand2.GetStorage()->offset.size());

    // 检查最小维度
    MATMUL_CHECK(ERR_PARAM_INVALID, operand1.GetShape().size() >= SHAPE_DIM2, "operand1 dimension(%zu) must be >= 2",
        operand1.GetShape().size());

    MATMUL_CHECK(ERR_PARAM_INVALID, operand2.GetShape().size() >= SHAPE_DIM2, "operand2 dimension(%zu) must be >= 2",
        operand2.GetShape().size());

    // 检查每个维度的值
    for (size_t i = 0; i < operand1.GetShape().size(); ++i) {
        MATMUL_CHECK(ERR_PARAM_INVALID, operand1.GetShape()[i] > 0, "operand1 dim[%zu] = %ld, must be > 0", i,
            operand1.GetShape()[i]);
    }

    for (size_t i = 0; i < operand2.GetShape().size(); ++i) {
        MATMUL_CHECK(ERR_PARAM_INVALID, operand2.GetShape()[i] > 0, "operand2 dim[%zu] = %ld, must be > 0", i,
            operand2.GetShape()[i]);
    }
...
Status CheckL1L0Tile(
    ...
    MATMUL_CHECK(ERR_CONFIG_TILE, L0Tile != 0, "%s cannot be zero, got %ld", L0TileName.c_str(), L0Tile);

    MATMUL_CHECK(ERR_CONFIG_TILE, L0Tile <= L1Tile && L1Tile % L0Tile == 0,
...
void CheckNZFormatAligned(const Tensor &operand1, const Tensor &operand2, const MatmulAttrParam &attrParam) {
    ...
    if (opFormatB == TileOpFormat::TILEOP_NZ) {
        if (attrParam.transB) {
            ...
            MATMUL_ASSERT(MatmulErrorCode::ERR_CONFIG_ALIGNMENT,
                          nL0 * BytesOf(operand1.GetDataType()) % ALIGN_SIZE_32 == 0,
...
Tensor MatmulMX(DataType outType, const Tensor &aMatrix, const Tensor &aScale, const Tensor &bMatrix,
    ...
    MATMUL_ASSERT(ERR_RUNTIME_LOGIC, checkMXStatus == SUCCESS, "MXMatmul operands check failed");
    MatmulGraphNodes tensorGraphNodes(
...
Tensor ABatchMulB3D(
    ...
    MATMUL_ASSERT(ERR_RUNTIME_LOGIC, checkStatus == SUCCESS, "BatchMatmul 3D operands check failed");
...
Tensor ABatchMulB4D(
    ...
    MATMUL_ASSERT(ERR_RUNTIME_LOGIC, checkStatus == SUCCESS, "BatchMatmul 4D operands check failed");
    const int64_t batchSizeA1 = operand1.GetShape()[0];
    const int64_t batchSizeA2 = operand1.GetShape()[1];
    const int64_t batchSizeB1 = operand2.GetShape()[0];
    const int64_t batchSizeB2 = operand2.GetShape()[1];
