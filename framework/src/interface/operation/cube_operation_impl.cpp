...
void SetAMulBAttr(const MatmulGraphNodes &tensorGraphNodes, const MatmulAttrParam &attrParam, Operation &op) {
    MATMUL_ASSERT(MatmulErrorCode::ERR_RUNTIME_NULLPTR,
        ...
        "Expected aTensorPtr, bTensorPtr, and outTensorPtr to be non-nullptr.");
...
Status CheckOperandShape(const Tensor &operand1, const Tensor &operand2) {
    MATMUL_LOGD("CheckOperandShape: operand1 dim=%zu, operand2 dim=%zu", operand1.GetShape().size(),
        operand2.GetShape().size());

    // 检查 shape 维度一致性
    MATMUL_CHECK(MatmulErrorCode::ERR_PARAM_MISMATCH, operand1.GetShape().size() == operand2.GetShape().size(),
        "Shape dimension mismatch: operand1=%zu, operand2=%zu", operand1.GetShape().size(), operand2.GetShape().size());

    // 检查 shape 与 offset 的一致性
    MATMUL_CHECK(MatmulErrorCode::ERR_PARAM_MISMATCH,
    ...
    // 检查最小维度
    MATMUL_CHECK(MatmulErrorCode::ERR_PARAM_INVALID, operand1.GetShape().size() >= SHAPE_DIM2,
        "operand1 dimension(%zu) must be >= 2", operand1.GetShape().size());

    MATMUL_CHECK(MatmulErrorCode::ERR_PARAM_INVALID, operand2.GetShape().size() >= SHAPE_DIM2,
        "operand2 dimension(%zu) must be >= 2", operand2.GetShape().size());

    // 检查每个维度的值
    for (size_t i = 0; i < operand1.GetShape().size(); ++i) {
        MATMUL_CHECK(MatmulErrorCode::ERR_PARAM_INVALID, operand1.GetShape()[i] > 0,
            "operand1 dim[%zu] = %ld, must be > 0", i, operand1.GetShape()[i]);
    }

    for (size_t i = 0; i < operand2.GetShape().size(); ++i) {
        MATMUL_CHECK(MatmulErrorCode::ERR_PARAM_INVALID, operand2.GetShape()[i] > 0,
...
Status CheckL1L0Tile(
    ...
    MATMUL_CHECK(MatmulErrorCode::ERR_CONFIG_TILE, L0Tile != 0, "%s cannot be zero, got %ld", L0TileName.c_str(), L0Tile);

    MATMUL_CHECK(MatmulErrorCode::ERR_CONFIG_TILE, L0Tile <= L1Tile && L1Tile % L0Tile == 0,
...
Status CheckCubeTiling(const Tensor &operand1, const Tensor &operand2, const MatmulAttrParam &attrParam) {
    ...
    MATMUL_LOGD("CheckCubeTiling: kL0=%ld, kL1a=%ld, kL1b=%ld, mL0=%ld, mL1=%ld, nL0=%ld, nL1=%ld", kL0, kL1a, kL1b,
        mL0, mL1, nL0, nL1);

    // 检查所有 tile 值为正
    MATMUL_CHECK(MatmulErrorCode::ERR_CONFIG_TILE,
    ...
    // 检查对齐要求
    MATMUL_CHECK(MatmulErrorCode::ERR_CONFIG_ALIGNMENT, kL0 % ALIGN_SIZE_16 == 0 && nL0 % ALIGN_SIZE_16 == 0,
    ...
    // 检查字节对齐
    MATMUL_CHECK(MatmulErrorCode::ERR_CONFIG_ALIGNMENT, kL0 * BytesOf(operand1.GetDataType()) % ALIGN_SIZE_32 == 0,
        "kL0 * sizeof(dtype) = %ld bytes, must be 32-byte aligned", kL0 * BytesOf(operand1.GetDataType()));

    MATMUL_CHECK(MatmulErrorCode::ERR_CONFIG_ALIGNMENT, nL0 * BytesOf(operand2.GetDataType()) % ALIGN_SIZE_32 == 0,
        "nL0 * sizeof(dtype) = %ld bytes, must be 32-byte aligned", nL0 * BytesOf(operand2.GetDataType()));

    if (operand1.Format() == TileOpFormat::TILEOP_ND) {
        if (attrParam.transA) { // For ND A transpose, mL0 must be 32B aligned
            MATMUL_CHECK(MatmulErrorCode::ERR_CONFIG_ALIGNMENT,
                mL0 * BytesOf(operand1.GetDataType()) % ALIGN_SIZE_32 == 0,
                "mL0 memory not aligned when transA=true: %ld bytes, must be 32-byte aligned",
...
void CheckNZFormatAligned(const Tensor &operand1, const Tensor &operand2, const MatmulAttrParam &attrParam) {
    ...
    if (opFormatA == TileOpFormat::TILEOP_NZ) {
        if (attrParam.transA) {
            ...
            MATMUL_ASSERT(MatmulErrorCode::ERR_CONFIG_ALIGNMENT,
                          kL0 % ALIGN_SIZE_16 == 0,
                          "Current length of kL0: %ld elements, the length must be aligned to 16 elements",
                          kL0);
        } else {
            MATMUL_ASSERT(MatmulErrorCode::ERR_CONFIG_ALIGNMENT,
            ...
            MATMUL_ASSERT(MatmulErrorCode::ERR_CONFIG_ALIGNMENT,
                          mL0 % ALIGN_SIZE_16 == 0,
                          "Current length of mL0: %ld elements, the length must be aligned to 16 elements",
                          mL0);
        }
    }
    if (opFormatB == TileOpFormat::TILEOP_NZ) {
        if (attrParam.transB) {
            ...
            MATMUL_ASSERT(MatmulErrorCode::ERR_CONFIG_ALIGNMENT,
                          nL0 % ALIGN_SIZE_16 == 0,
                          "Current length of nL0: %ld elements, the length must be aligned to 16 elements",
                          nL0);
        } else {
            MATMUL_ASSERT(MatmulErrorCode::ERR_CONFIG_ALIGNMENT,
...
void CheckCMatrixNZFormatAligned(const DataType &outType, const Tensor &operand, const MatmulAttrParam &attrParam) {
    ...
    if (attrParam.isCMatrixNZ) {
        ...
        if (outType == DataType::DT_INT32) {
            MATMUL_ASSERT(MatmulErrorCode::ERR_CONFIG_ALIGNMENT,
                          nView % ALIGN_SIZE_16 == 0,
                          "Current nView: %ld elements, nView must be aligned to 16 elements when CMatrix is NZ and outType is int32",
                          nView);
            MATMUL_ASSERT(MatmulErrorCode::ERR_CONFIG_ALIGNMENT,
                          nL0 % ALIGN_SIZE_16 == 0,
            ...
            MATMUL_ASSERT(MatmulErrorCode::ERR_CONFIG_ALIGNMENT,
                          nView * BytesOf(outType) % ALIGN_SIZE_32 == 0,
                          "Current nView: %ld bytes, nView must be aligned to 32 bytes when CMatrix is NZ",
                          nView * BytesOf(outType));
            MATMUL_ASSERT(MatmulErrorCode::ERR_CONFIG_ALIGNMENT,
                          nL0 * BytesOf(outType) % ALIGN_SIZE_32 == 0,
                          "Current nL0: %ld bytes, nL0 must be aligned to 32 bytes when CMatrix is NZ",
                          nL0 * BytesOf(outType));
        }
    }
}
...
Status CheckMXMatmulOperands(const Tensor &aTensor, const Tensor &aScaleTensor, const Tensor &bTensor,
    ...
    MATMUL_ASSERT(MatmulErrorCode::ERR_PARAM_UNSUPPORTED,
        inDType == DataType::DT_FP8E4M3 || inDType == DataType::DT_FP8E5M2,
...
Tensor ConstructTensorGraph(DataType dataType, MatmulGraphNodes &tensorGraphNodes, const MatmulAttrParam &attrParam,
    ...
    if (attrParam.isCMatrixNZ) {
        MATMUL_ASSERT(MatmulErrorCode::ERR_RUNTIME_LOGIC, BytesOf(dataType) > 0,
            "BytesOf(dataType): %d. Must be positive.", BytesOf(dataType));
