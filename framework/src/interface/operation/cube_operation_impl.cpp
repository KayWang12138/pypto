...
Status CheckL1L0Tile(
    ...
    MATMUL_ASSERT(MatmulErrorCode::ERR_CONFIG_TILE, L0Tile != 0, "%s cannot be zero, got %ld", L0TileName.c_str(), L0Tile);
...
void CheckNZFormatAligned(const Tensor &operand1, const Tensor &operand2, const MatmulAttrParam &attrParam) {
    ...
    if (opFormatA == TileOpFormat::TILEOP_NZ) {
        if (attrParam.transA) {
            MATMUL_ASSERT(MatmulErrorCode::ERR_CONFIG_ALIGNMENT,
                          mL0 * BytesOf(operand1.GetDataType()) % ALIGN_SIZE_32 == 0,
                          "Current length of mL0: %zu bytes, the length must be aligned to 32 bytes",
                          mL0 * BytesOf(operand1.GetDataType()));
            MATMUL_ASSERT(MatmulErrorCode::ERR_CONFIG_ALIGNMENT,
                          kL0 % ALIGN_SIZE_16 == 0,
            ...
            MATMUL_ASSERT(MatmulErrorCode::ERR_CONFIG_ALIGNMENT,
                          ...
                          "Current length of kL0: %zu bytes, the length must be aligned to 32 bytes",
    ...
    if (opFormatB == TileOpFormat::TILEOP_NZ) {
        if (attrParam.transB) {
            ...
            MATMUL_ASSERT(MatmulErrorCode::ERR_CONFIG_ALIGNMENT,
                          ...
                          "Current length of nL0: %zu bytes, the length must be aligned to 32 bytes",
    ...
    if (attrParam.isCMatrixNZ) {
        ...
        if (outType == DataType::DT_INT32) {
            ...
            MATMUL_ASSERT(MatmulErrorCode::ERR_CONFIG_ALIGNMENT,
                          ...
                          "Current nL0: %zu bytes, nL0 must be aligned to 32 bytes when CMatrix is NZ",
                          nL0 * BytesOf(outType));
        }
    }
}
