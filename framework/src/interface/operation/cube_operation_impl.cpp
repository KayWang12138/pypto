...
void CheckABatchMulB(const Tensor &operand1, const Tensor &operand2) {
    MATMUL_ASSERT(MatmulErrorCode::ERR_RUNTIME_NULLPTR, operand1.GetStorage() != nullptr, "A Tensor cannot be null");
    MATMUL_ASSERT(MatmulErrorCode::ERR_RUNTIME_NULLPTR, operand2.GetStorage() != nullptr, "B Tensor cannot be null");
    MATMUL_ASSERT(MatmulErrorCode::ERR_PARAM_INVALID,
    ...
    for (uint64_t bIdx = 0; bIdx < operand1.GetShape().size(); bIdx++) {
        const int64_t batchSizeA = operand1.GetShape()[bIdx];
        const int64_t batchSizeB = operand2.GetShape()[bIdx];
        MATMUL_ASSERT(MatmulErrorCode::ERR_PARAM_INVALID,
            batchSizeA == batchSizeB || batchSizeB == 1 || batchSizeA == 1,
            "batchSize invalid: A%zu=B%zu or 1 allowed. A%zu: %ld, B%zu: %ld", bIdx, bIdx, bIdx, batchSizeA, bIdx,
            batchSizeB);
    }
}
