...
Tensor TransposedBatchMatmul(DataType dataType, const Tensor &aMatrix, const Tensor &bMatrix) {
    ...
    OP_CHECK(kaSize != kbSize, {
        ASSERT(false) << "kSize invalid, expect kaSize = kbSize, given kaSize: " << kaSize
                      << ", kbSize: " << kbSize << std::endl;
    });
    ...
    for (int64_t bIdx = 0; bIdx < batchSizeA; ++bIdx) {
        // auto aTensor = aMatrixFused.GetStorage()->View(curFunc, {mSize, kaSize}, {0, bIdx * kaSize});
        Tensor aTensor = View(aMatrix, {mSize, kaSize}, {0, bIdx * kaSize});
        Tensor bTensor = Reshape(View(bMatrix, {1, kbSize, nSize}, {bIdx, 0, 0}), {kbSize, nSize});
        Tensor cTensor(dataType, {mSize, nSize}, "TensorC");
        AddAMulBNode<false, false>(
            aTensor.GetStorage(), bTensor.GetStorage(), cTensor.GetStorage(), nullptr);
