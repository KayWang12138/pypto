...
void CheckOperandDtype(DataType outType, const Tensor &operand1, const Tensor &operand2) {
    ...
    ASSERT(MatmulErrorCode::ERR_PARAM_INVALID,
        (operand1Dtype != DataType::DT_FP8E5M2 && operand1Dtype != DataType::DT_FP4_E2M1X2) ||
        operand1.Format() == TileOpFormat::TILEOP_ND)
        << "When operand1 data type is DT_FP8E5M2 or DT_FP4_E2M1X2, format must be ND.";

    ASSERT(MatmulErrorCode::ERR_PARAM_INVALID,
        (operand2Dtype != DataType::DT_FP8E5M2 && operand2Dtype != DataType::DT_FP4_E2M1X2) ||
        operand2.Format() == TileOpFormat::TILEOP_ND)
        << "When operand2 data type is DT_FP8E5M2 or DT_FP4_E2M1X2, format must be ND.";
