...
public:
    ...
    void SetOpAttribute(const std::shared_ptr<OpAttribute> &attr) {
        ...
        if (copyOpAttrOpTypes.count(opcode_) > 0) {
            ASSERT(ERROR_CODE_UNDEFINED, std::dynamic_pointer_cast<CopyOpAttribute>(opAttribute_) != nullptr);
            return;
        }

        switch (opcode_) {
            case Opcode::OP_VIEW: {
                ASSERT(ERROR_CODE_UNDEFINED, std::dynamic_pointer_cast<ViewOpAttribute>(opAttribute_) != nullptr);
                break;
            }
            case Opcode::OP_ASSEMBLE: {
                ASSERT(ERROR_CODE_UNDEFINED, std::dynamic_pointer_cast<AssembleOpAttribute>(opAttribute_) != nullptr);
                break;
            }
            ...
            case Opcode::OP_CALL: {
                ASSERT(ERROR_CODE_UNDEFINED, std::dynamic_pointer_cast<CallOpAttribute>(opAttribute_) != nullptr);
                break;
            }
            case Opcode::OP_CONVERT: {
                ASSERT(ERROR_CODE_UNDEFINED, std::dynamic_pointer_cast<ConvertOpAttribute>(opAttribute_) != nullptr);
                break;
            }
            default: ASSERT(ERROR_CODE_UNDEFINED, opAttribute_ == nullptr);
        }
    }

    void SetAssembleOpAttribute(
        const std::vector<int64_t> &toOffset, const std::vector<SymbolicScalar> &toDynOffset = {}) {
        ASSERT(ERROR_CODE_UNDEFINED, opcode_ == Opcode::OP_ASSEMBLE || opcode_ == Opcode::OP_ASSEMBLE_SSA);
    ...
    std::string GetCalleeMagicName() const {
        ASSERT(ERROR_CODE_UNDEFINED, IsCall());
    ...
    const FunctionHash &GetCalleeHash() const {
        ASSERT(ERROR_CODE_UNDEFINED, IsCall() || opcode_ == Opcode::OP_BLOCK_CALL);
