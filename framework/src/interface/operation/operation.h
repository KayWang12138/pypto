...
public:
    ...
    void SetOpAttribute(const std::shared_ptr<OpAttribute>& attr)
    {
        ...
        if (copyOpAttrOpTypes.count(opcode_) > 0) {
            F_ASSERT(std::dynamic_pointer_cast<CopyOpAttribute>(opAttribute_) != nullptr);
            return;
        }

        switch (opcode_) {
            case Opcode::OP_VIEW: {
                F_ASSERT(std::dynamic_pointer_cast<ViewOpAttribute>(opAttribute_) != nullptr);
                break;
            }
            case Opcode::OP_ASSEMBLE: {
                F_ASSERT(std::dynamic_pointer_cast<AssembleOpAttribute>(opAttribute_) != nullptr);
                break;
            }
            ...
            case Opcode::OP_CALL: {
                F_ASSERT(std::dynamic_pointer_cast<CallOpAttribute>(opAttribute_) != nullptr);
                break;
            }
            case Opcode::OP_CONVERT: {
                F_ASSERT(std::dynamic_pointer_cast<ConvertOpAttribute>(opAttribute_) != nullptr);
                break;
            }
            default:
                F_ASSERT(opAttribute_ == nullptr);
        }
    }
    ...
    {
        F_ASSERT(opcode_ == Opcode::OP_ASSEMBLE || opcode_ == Opcode::OP_ASSEMBLE_SSA);
    ...
    {
        F_ASSERT(IsCall() || opcode_ == Opcode::OP_BLOCK_CALL);
