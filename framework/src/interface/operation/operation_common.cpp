...
inline const std::vector<size_t> &GetShapeLenLimit(const Opcode op) {
  ...
  static std::unordered_map<const Opcode, const std::vector<size_t>>
      op_shape_len_limit = {{Opcode::OP_ADD, {1, 4}},
                            {Opcode::OP_CAST, {1, 4}},
...
using Constraint =
    std::tuple<const InputsDTypeSet, const std::vector<std::vector<int>>>;
inline const Constraint &GetSupportDTypeSet(const Opcode op) {
  ...
  static const dtype_binary = {DT_FP32, DT_FP16, DT_BF16, DT_INT32, DT_INT16};
  static const dtype_32_16 = {DT_FP32,  DT_FP16,   DT_BF16,  DT_INT32,
                              DT_INT16, DT_UINT32, DT_UINT16};
  static const dtype_i64_32_16 = {DT_INT64, DT_INT32, DT_INT16};
  static const dtype_i = {DT_INT64, DT_INT32, DT_INT16, DT_INT8};
  static const dtype_logical = {DT_FP32,  DT_FP16, DT_BF16,
                                DT_UINT8, DT_INT8, DT_BOOL};
  static const dtype_base_i32 = {DT_FP32, DT_FP16, DT_BF16, DT_INT32};
  static const dtype_b_i3_1 = {DT_FP32, DT_FP16, DT_BF16, DT_INT32, DT_INT16};
  static const dtype_b_i3_1_8 = {DT_FP32,  DT_FP16,  DT_BF16,
                                 DT_INT32, DT_INT16, DT_INT8};
  static const dtype_base_i_u = {DT_FP32,   DT_FP16,   DT_BF16,
                                 DT_INT32,  DT_INT16,  DT_INT8,
                                 DT_UINT32, DT_UINT16, DT_UINT8};
  static const dtype_base_i_u_b = {DT_FP32,  DT_FP16, DT_BF16,   DT_INT32,
  ...
  // consistent with first input
    static const std::unordered_map<const Opcode, const Constraint> op_2_dtype = {
        ...
        // {Opcode::OP_VEC_DUP, Constraint({dtype_base, scalar_dtype}, false, {dtype_base}, true)},
    ...
    if (op_2_dtype.find(op) == op_2_dtype.end()) {
      ASSERT(false) << "Operation " << OpcodeManager::Inst().GetOpcodeStr(op)
                    << " has not set dtype checker yet.";
    }
    return op_2_dtype.at(op);
}

inline const std::vector<DataType> &GetInputSupportDtype(const Opcode op,
                                                         size_t index = 0) {
...
void CheckTensorDType(const LogicalTensorPtr &tensor,
...
void CheckInputDTypConstraint(const std::vector<LogicalTensorPtr> &inputs,
  ...
  std::for_each(cons.begin() + 1, cons.end(), [&inputs, dtype](auto &con) {
    ...
    if (dtype != idtype) {
      ASSERT(false) << "The dtype of input[" << con << "] should be "
                    << DataType2String(dtype) << " but got "
                    << DataType2String(idtype);
    }
  });
}

void CheckOperationDType(const std::vector<LogicalTensorPtr> &inputs,
                         const Opcode op) {
  for (size_t index = 0; index < inputs.size(); ++index) {
    CheckTensorDType(inputs[index], GetInputSupportDtype(op, index));
  }
  auto consReq = std::get<1>(GetSupportDTypeSet(op));
  std::for_each(consReq.begin(), consReq.end(), [&inputs](const auto &cons) {
    CheckInputDTypConstraint(inputs, cons);
  });
}

std::vector<int> GetBroadCastShape(LogicalTensorPtr &operand1,
