    ...
    const LogicalTensorPtr& tensor, const std::unordered_set<DataType>& supportedTypes, const std::string& opName)
{
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_DTYPE_UNSUPPORTED, isSupported)
        << "Tensor data type " << DataType2String(dtype) << " is not in supported types for op: " << opName;
}
...
void CheckTensorsDataTypeConsistency(
    const LogicalTensorPtr& tensor, const Element& element, const std::string& opName)
{
...
void CheckTensorsDataTypeConsistency(
    const std::vector<LogicalTensorPtr>& tensors, const std::string& opName)
{
    if (tensors.empty()) {
        return;
    }
    ...
    for (size_t i = 1; i < tensors.size(); ++i) {
        ASSERT(VectorErrorCode::ERR_PARAM_DTYPE_UNSUPPORTED, tensors[i]->Datatype() == firstDtype)
            << "Tensor data type inconsistent, tensor0 dtype: " << DataType2String(firstDtype)
            << ", tensor" << i << " dtype: " << DataType2String(tensors[i]->Datatype()) << " for op: " << opName;
    }
}
    ...
    const std::vector<LogicalTensorPtr>& tensors, const std::string& opName)
{
    ...
    for (size_t i = 1; i < tensors.size(); ++i) {
        ASSERT(VectorErrorCode::ERR_PARAM_INVALID, tensors[i]->Format() == firstFormat)
            << "Tensor format inconsistent, tensor0 format: " << std::to_string(firstFormat)
            << ", tensor" << i << " format: " << std::to_string(tensors[i]->Format()) << " for op: " << opName;
    }
}
