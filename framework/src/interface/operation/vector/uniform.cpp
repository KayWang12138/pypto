...
LogicalTensorPtr TensorUniform(Function &function, LogicalTensorPtr &result, const Element &key,
    ...
    std::cout << "[Uniform Trace]   dtype: " << static_cast<int>(dtype) << std::endl;
...
static void TiledUniformBuildIn(Function &function, const TileShape &tileShape, const LogicalTensorPtr &result, 
    ...
    int64_t tempByteSize = uint32BufferSize * BytesOf(DT_UINT32);
    
    if (dtype == DT_FP32) {
        tempByteSize += uint32BufferSize * BytesOf(DT_FP32);
    } else if (dtype == DT_FP16 || dtype == DT_BF16) {
        tempByteSize += uint32BufferSize * BytesOf(DT_UINT16);
    }
    
    tempByteSize = ((tempByteSize + ALIGN_SIZE - 1) / ALIGN_SIZE) * ALIGN_SIZE;
    ...
    for (int64_t i = 0; i < result->shape[0]; i += vecTile[0]) {
        ...
        SkipCounter(tileCounter0, tileCounter1, static_cast<uint64_t>(i / 4));
...
void UniformOperationTileFunc(Function &function, const TileShape &tileShape,
    ...
    if (op.HasAttr(OP_ATTR_PREFIX + "COUNTER0")) {
        counter0 = op.GetElementAttribute(OP_ATTR_PREFIX + "COUNTER0").Cast<uint64_t>();
    }
    ...
    TiledUniformBuildIn(function, tileShape, oOperand[0], resultTileInfo, key, counter0, counter1, rounds, shape, dtype);
}
...
Tensor Uniform(const Element &key, const Element &counter0, const Element &counter1,
    ...
    return RealUniform(key.Cast<uint64_t>(), counter0.Cast<uint64_t>(), counter1.Cast<uint64_t>(), shape, roundsVal, dtype);
}
