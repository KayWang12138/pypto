...
static void TiledUniformBuildIn(Function &function, const TileShape &tileShape, const LogicalTensorPtr &result, 
    ...
    // Calculate tempbuff size
    const size_t ALIGN_SIZE = 32;
    int64_t uint32BufferSize = vecTile[0]; // 需要存储的uint32元素数量
    int64_t tempByteSize = uint32BufferSize * BytesOf(DT_UINT32) + uint32BufferSize * BytesOf(DT_FP32);
    tempByteSize = ((tempByteSize + ALIGN_SIZE - 1) / ALIGN_SIZE) * ALIGN_SIZE; // 对齐到32字节
...
static Tensor RealUniform(uint64_t key, uint64_t counter0, uint64_t counter1,
    ...
    auto resTensor = Tensor(DT_FP32, shape);
