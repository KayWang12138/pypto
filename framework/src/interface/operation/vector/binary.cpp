...
void PReLUOperationOperandCheck(
    ...
    ASSERT(input->GetDataType() == weight->GetDataType()) 
...
LogicalTensorPtr TensorPReLUOperation(Function &function, const Tensor &input, const Tensor &weight) {
    auto inputTensor = input.GetStorage();
    auto weightTensor = weight.GetStorage();
    
    auto result = std::make_shared<LogicalTensor>(function, inputTensor->Datatype(), inputTensor->shape, inputTensor->GetDynValidShape());
    function.AddOperation(Opcode::OP_PRELU, {inputTensor, weightTensor}, {result});
    return result;
}
...
Tensor PReLU(const Tensor &input, const Tensor &weight) {
    ...
    RETURN_CALL(PReLUOperation, *Program::GetInstance().GetCurrentFunction(), input, weight);
}
