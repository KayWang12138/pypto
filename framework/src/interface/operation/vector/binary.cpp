...
void TiledPReLUOperation(
    ...
    ASSERT(input->shape.size() == input->offset.size()) << "[VectorErrorCode::ERR_PARAM_INVALID] The shape size of input and offset must be equal";
    ASSERT(weight->shape.size() == weight->offset.size()) << "[VectorErrorCode::ERR_PARAM_INVALID] The shape size of weight and offset must be equal";
...
void PReLUOperationOperandCheck(
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, inputSize <= INT32_MAX) << "The input shape size should not exceed INT32_MAX";
    
    int64_t weightSize = weight->shape[0];
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, weightSize <= INT32_MAX) << "The weight shape size should not exceed INT32_MAX";
}
...
Tensor Gcd(const Tensor &self, const Tensor &other) {
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, dataType == other.GetDataType()) << "Inputs must have the same dataType.";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, SHAPE_DIM1 <= shapeSize && shapeSize <= SHAPE_DIM5) << "This operation's input only support 1-5 dims";
    std::unordered_set<DataType> GCD_SUPPORT_DATATYPES = {DataType::DT_INT32, DataType::DT_INT16, DataType::DT_INT8, DataType::DT_UINT8};
    ASSERT(VectorErrorCode::ERR_PARAM_DTYPE_UNSUPPORTED, GCD_SUPPORT_DATATYPES.count(dataType)) << "This datatype is not supported";
...
Tensor Maximum(const Tensor &operand1, const Element &operand2) {
    DECLARE_TRACER();
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, operand1.GetDataType() == operand2.GetDataType()) << "The datatype of the two input must be equal";
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_DTYPE_UNSUPPORTED, std::find(MAXS_SUPPORT_DATATYPES.begin(), MAXS_SUPPORT_DATATYPES.end(), operand1.GetDataType()) !=
...
Tensor Minimum(const Tensor &operand1, const Element &operand2) {
    ...
    std::vector<DataType> MINS_SUPPORT_DATATYPES = {
        DataType::DT_FP32, DataType::DT_FP16, DataType::DT_INT32, DataType::DT_INT16, DataType::DT_BF16};
    ASSERT(VectorErrorCode::ERR_PARAM_DTYPE_UNSUPPORTED, std::find(MINS_SUPPORT_DATATYPES.begin(), MINS_SUPPORT_DATATYPES.end(), operand1.GetDataType()) !=
