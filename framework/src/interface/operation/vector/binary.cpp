    ...
    const Element& alpha, const LogicalTensorPtr& result, TileInfo& resultTileInfo)
{
    ...
    if (cur == shapeSize) {
        ...
        std::vector<int64_t> brcOperand(shapeSize, 0);
        for (size_t i = 0; i < shapeSize; i++) {
            int result = BrcAxisBinaryOp(inputSelf.tensor, inputOther.tensor, i);
            brcOperand[i] = (result == 1) ? -1 : result;
        }
        if (brcOperand[shapeSize - 1] != -1) {
            op.SetAttribute(OpAttributeKey::excludeBufferReuse, true);
    ...
    for (int i = 0; i < result->shape[cur]; i += vecTile[cur]) {
        ...
        inputSelf.tileInfo.shape[cur] = std::min(inputSelf.tensor->GetShape()[cur] - inputSelf.tileInfo.offset[cur], vecTile[cur]);
        inputOther.tileInfo.offset[cur] = i % inputOther.tensor->GetShape()[cur];
        inputOther.tileInfo.shape[cur] = std::min(inputOther.tensor->GetShape()[cur] - inputOther.tileInfo.offset[cur], vecTile[cur]);
        TiledAxpyOperation(function, tileShape, cur + 1, inputSelf, inputOther, alpha, result, resultTileInfo);
    }
}

void TiledAxpyOperation(
    Function& function, const TileShape& tileShape, LogicalTensorPtr self, LogicalTensorPtr other,
    const Element& alpha, const LogicalTensorPtr& result)
{
    CheckBinOpOperandsValid(self, other);
    BroadcastOperandTensor(self, other, result, function, tileShape);
    BroadcastOperandTensor(self, other, result, function, tileShape);
...
LogicalTensorPtr TensorAxpyOperation(Function& function, const Tensor& self, const Tensor& other, const Element& alpha)
{
    auto selfTensor = self.GetStorage();
...
Tensor Axpy(const Tensor& self, const Tensor& other, const Element& alpha)
{
    DECLARE_TRACER();
