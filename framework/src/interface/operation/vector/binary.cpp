...
void CheckOperandsValid(const LogicalTensorPtr &operand1, const LogicalTensorPtr &operand2) {
    ASSERT(ERROR_CODE_UNKNOWN, operand1->shape.size() == operand2->shape.size()) << "The shape size of the two input tensors must be equal";
}
...
void CheckBinOpOperandsValid(const LogicalTensorPtr &operand1, const LogicalTensorPtr &operand2) {
    ...
    for (size_t i = 0; i < operand1->shape.size(); ++i) {
        if (operand1->shape[i] != operand2->shape[i] && (operand1->shape[i] != 1 && operand2->shape[i] != 1)) {
            ASSERT(ERROR_CODE_UNKNOWN, false && "shape not support binary operation");
        }
    }
}
...
void CheckBinaryInputTensors(const LogicalTensorPtr &tensor1, const LogicalTensorPtr &tensor2, std::string &op) {
    ...
    if (tensor1->Datatype() != tensor2->Datatype()) {
        ASSERT(ERROR_CODE_UNKNOWN, false && "The dtype of input tensors are not same.");
    }
    if (tensor1->Format() != tensor2->Format()) {
        ASSERT(ERROR_CODE_UNKNOWN, false && "The format of input tensors are not same.");
    }
}
...
void BinaryOperationOperandCheck(
    ...
    ASSERT(ERROR_CODE_UNKNOWN, iOperand.size() == inOpSize && "iOperand size should be 2");
    ASSERT(ERROR_CODE_UNKNOWN, oOperand.size() == outOpSize && "oOperand size should be 1");
}

// [m,n] + [m, 1]
bool CallBrcBinOp(LogicalTensorPtr operand1, LogicalTensorPtr operand2) {
    ASSERT(ERROR_CODE_UNKNOWN, operand1->shape.size() == operand2->shape.size() && "Dims not match");
...
void TiledPReLUOperation(
    ...
    ASSERT(ERROR_CODE_UNKNOWN, input->shape.size() == input->offset.size()) << "The shape size of input and offset must be equal";
    ASSERT(ERROR_CODE_UNKNOWN, weight->shape.size() == weight->offset.size()) << "The shape size of weight and offset must be equal";
...
void PReLUOperationOperandCheck(
    const std::vector<LogicalTensorPtr> &iOperand, const std::vector<LogicalTensorPtr> &oOperand) {
    ASSERT(ERROR_CODE_UNKNOWN, iOperand.size() == 2) << "The input operand size should be 2";
    ASSERT(ERROR_CODE_UNKNOWN, oOperand.size() == 1) << "The output operand size should be 1";
    
    auto input = iOperand[0];
    auto weight = iOperand[1];
    
    ASSERT(ERROR_CODE_UNKNOWN, input->Datatype() == weight->Datatype()) 
        << "The input and weight should have the same data type";
    
    ASSERT(ERROR_CODE_UNKNOWN, input->shape.size() >= 2 && input->shape.size() <= 4) 
        << "The input shape dimension should be in range [2, 4]";
    
    ASSERT(ERROR_CODE_UNKNOWN, weight->shape.size() == 1) 
        << "The weight should be 1-dimensional";
    
    ASSERT(ERROR_CODE_UNKNOWN, weight->shape[0] == input->shape[1]) 
    ...
    ASSERT(ERROR_CODE_UNKNOWN, inputSize <= INT32_MAX) << "The input shape size should not exceed INT32_MAX";
    
    int64_t weightSize = weight->shape[0];
    ASSERT(ERROR_CODE_UNKNOWN, weightSize <= INT32_MAX) << "The weight shape size should not exceed INT32_MAX";
}
...
Tensor Gcd(const Tensor &self, const Tensor &other) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, dataType == other.GetDataType()) << "Inputs must have the same dataType.";
    ASSERT(ERROR_CODE_UNKNOWN, SHAPE_DIM1 <= shapeSize && shapeSize <= SHAPE_DIM5) << "This operation's input only support 1-5 dims";
    std::unordered_set<DataType> GCD_SUPPORT_DATATYPES = {DataType::DT_INT32, DataType::DT_INT16, DataType::DT_INT8, DataType::DT_UINT8};
    ASSERT(ERROR_CODE_UNKNOWN, GCD_SUPPORT_DATATYPES.count(dataType))<< "This datatype is not supported";
...
Tensor Maximum(const Tensor &operand1, const Element &operand2) {
    DECLARE_TRACER();
    ASSERT(ERROR_CODE_UNKNOWN, operand1.GetDataType() == operand2.GetDataType()) << "The datatype of the two input must be equal";
    ...
    ASSERT(ERROR_CODE_UNKNOWN, std::find(MAXS_SUPPORT_DATATYPES.begin(), MAXS_SUPPORT_DATATYPES.end(), operand1.GetDataType()) !=
...
Tensor Minimum(const Tensor &operand1, const Element &operand2) {
    ...
    std::vector<DataType> MINS_SUPPORT_DATATYPES = {
        DataType::DT_FP32, DataType::DT_FP16, DataType::DT_INT32, DataType::DT_INT16, DataType::DT_BF16};
    ASSERT(ERROR_CODE_UNKNOWN, std::find(MINS_SUPPORT_DATATYPES.begin(), MINS_SUPPORT_DATATYPES.end(), operand1.GetDataType()) !=
