...
Tensor ColExpandExpDif(const Tensor &self, const Tensor &other) {
    ...
    RETURN_CALL(BinaryOperation<BinaryOpType::COLEXPANDEXPDIF>, *Program::GetInstance().GetCurrentFunction(), self, other);
}

Tensor RowExpandExpDif(const Tensor &self, const Tensor &other) {
    DECLARE_TRACER();

    RETURN_CALL(BinaryOperation<BinaryOpType::ROWEXPANDEXPDIF>, *Program::GetInstance().GetCurrentFunction(), self, other);
}
