...
Tensor Remainder(const Tensor &self, const Tensor &other) {
    ...
    if (selfDtype == DT_INT16) {
        ...
        LogicalTensorPtr result = CALL(BinaryOperation<BinaryOpType::REM>,
            *Program::GetInstance().GetCurrentFunction(), castSelf.GetStorage(), castOther.GetStorage());
        Tensor castedResult = Cast(result.GetStorage(), selfDtype, CastMode::CAST_TRUNC, SaturationMode::OFF);
        return castedResult;
    }
...
Tensor Remainder(const Tensor &self, const Element &other) {
    ...
    if (selfDtype == DT_INT16) {
        ...
        LogicalTensorPtr result = CALL(BinaryOperationScalar<BinaryOpType::REM>,
...
Tensor Remainder(const Element &self, const Tensor &other) {
    ...
    if (otherDtype == DT_INT16) {
        ...
        LogicalTensorPtr result = CALL(BinaryOperationAllScalar<BinaryOpType::REMR>,
            *Program::GetInstance().GetCurrentFunction(), castOther.GetStorage(), self_, true);
        Tensor castedResult = Cast(result.GetStorage(), otherDtype, CastMode::CAST_TRUNC, SaturationMode::OFF);
        return castedResult;
    }
