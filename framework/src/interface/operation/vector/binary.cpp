    ...
    if (tensor1->Datatype() != tensor2->Datatype()) {
        if (op == "AXPY") {
            ASSERT(
                ...
                tensor1->Datatype() == DataType::DT_FP32 && tensor2->Datatype() == DataType::DT_FP16)
        ...
        if (op == "AXPY") {
            ASSERT(
                VectorErrorCode::ERR_PARAM_INVALID, tensor1->Datatype() == DataType::DT_FP32 ||
                                                        tensor1->Datatype() == DataType::DT_FP16 ||
                                                        tensor1->Datatype() == DataType::DT_BF16)
                << "AXPY: same dtype only support fp32, fp16 or bf16.";
        }
    }
