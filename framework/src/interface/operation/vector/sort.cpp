...
std::tuple<Tensor, Tensor> sort(const Tensor &self, int axis = -1, bool descending = false) {
    ...
    ASSERT(self.GetDataType() == DataType::DT_FP32 || self.GetDataType() == DataType::DT_FP16) << "Only Support FP32 and FP16.\n";
    ...
    if (checkIsExceedUB(vecTileShape.tile, self.GetShape(), axis, 32)) {
        int64_t tileNum = 1;
        for (int i = 0; i < (int)vecTileShape.size(); i ++ ) {
            tileNum *= (self.GetShape()[i] + vecTileShape[i] - 1) / vecTileShape[i];
        }

        ASSERT(tileNum < 128) << "For Large Shape in GM, tile number must less 128. Got " << tileNum << ".\n";
    }
