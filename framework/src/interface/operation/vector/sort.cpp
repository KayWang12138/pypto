...
bool checkIsExceedUB(const std::vector<int64_t> &tileShape, const std::vector<int64_t> &shape, int axis, int blockSize = 32) {
    int64_t UBSize = static_cast<int64_t>(Platform::Instance().GetDie().GetMemoryLimit(MemoryType::MEM_UB));
...
void TiledSort(Function &function, const TileShape &tileShape, size_t cur, Input &input,
    ...
    if (static_cast<int>(cur) == axis) {
        ...
        auto tileOutputValidShape = source->GetDynValidShape();
        ...
        for (int64_t i = 0; i < input.tensor.GetShape()[axis]; i += vecTileAlign[axis]) {
            ...
            auto mrgSortTile = sortOutputTensor->View(function, tileOutputShape, tileOutputOffset);
            tileOutputValidShape[axis] = mrgSortTile->GetDynValidShape()[axis];
            auto tmp = std::make_shared<LogicalTensor>(function, source->Datatype(), tileOutputShape);
            auto &mrgSortOp = function.AddOperation(Opcode::OP_MRGSORT_TO_GM, {bitSortTile}, {mrgSortTile, tmp});
            ...
            mrgSortTile->UpdateDynValidShape(mrgSortDynValidShape);
        ...
        for (int64_t round = 1; round <= roundNum; round ++ ) {
            ...
            for (int64_t i = 0; i < sortOutputShape[axis];) {
                ...
                auto outputTile = roundOutputTensor->View(function, tileOutputShape, tileOutputOffset);
                auto &tileMrgSortInGMOp = function.AddOperation(Opcode::OP_TILEMRGSORT_IN_GM, {src}, {outputTile, outputInUB});
                tileMrgSortInGMOp.SetAttribute(SORT_FIRSTSHAPE, static_cast<int>(firstShape));
                tileMrgSortInGMOp.SetAttribute(OP_ATTR_PREFIX + "round", static_cast<int>(round));
                tileMrgSortInGMOp.SetAttribute(OP_ATTR_PREFIX + "offset", static_cast<int>(tileOutputOffset[axis]));
                std::vector<SymbolicScalar> tileMrgSortDynValidShape(src->GetDynValidShape());
                outputTile->UpdateDynValidShape(tileMrgSortDynValidShape);
        ...
        for (int i = 0; i < sortOutputShape[axis]; i += vecTileAlign[axis]) {
            ...
            auto valueTile = valueResult->View(function, resultTileInfo.shape, resultTileInfo.offset);
            auto &valueOp = function.AddOperation(Opcode::OP_EXTRACT, {src}, {valueTile});
            valueOp.SetAttribute(SORT_ORDER, descending);
            valueOp.SetAttribute(EXTRACT_MASKMODE, 0);
            valueOp.SetAttribute(SORT_KVALUE, static_cast<int>(resultTileInfo.shape[axis]));
            ...
            auto indexTile = indexResult->View(function, resultTileInfo.shape, resultTileInfo.offset);
            auto &indexOp = function.AddOperation(Opcode::OP_EXTRACT, {src}, {indexTile});
            indexOp.SetAttribute(SORT_ORDER, descending);
            indexOp.SetAttribute(EXTRACT_MASKMODE, 1);
            indexOp.SetAttribute(SORT_KVALUE, static_cast<int>(resultTileInfo.shape[axis]));
...
void TensorSort(Function &function, const LogicalTensorPtr &self, LogicalTensorPtr &valueResult, 
    ...
    auto &op = GraphUtils::AddDynOperation(function, Opcode::OP_SORT, {self}, {valueResult, indexResult}, {validShape, validShape});
...
REGISTER_OPERATION_TILED_FUNC(OP_SORT, Opcode::OP_SORT, SortOperationTileFunc);
