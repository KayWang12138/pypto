...
void TiledTopK(Function &function, const TileShape &tileShape, size_t cur, Input &input,
    ...
    int isLargest) {
    auto &vecTile = tileShape.GetVecTile();
    ASSERT(ERROR_CODE_UNDEFINED, k <= vecTile[axis]) << "The k should less than or equal to" << vecTile[axis];
    if (static_cast<int>(cur) == axis) {
...
std::tuple<Tensor, Tensor> TopK(const Tensor &self, int k, int axis, bool isLargest) {
    DECLARE_TRACER();
    const auto len = static_cast<int>(self.GetShape().size());
    ASSERT(ERROR_CODE_UNDEFINED, axis == (len - 1) || axis == -1) << "TopK only support last axis";
...
bool checkIsExceedUB(const std::vector<int64_t> &tileShape, const std::vector<int64_t> &shape, int axis, int blockSize = 32) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, tileShape[axis] > 0) << "tileShape in axis must greater than 0.";
    ASSERT(ERROR_CODE_UNDEFINED, blockSize > 0) << "blockSize must greater than 0.";
...
std::tuple<Tensor, Tensor> sort(const Tensor &self, int axis = -1, bool descending = false) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, len >= 1 && len <= 4) << "Only support 1 dim to 4 dim.\n";

    axis = axis >= 0 ? axis : axis + len;
    ASSERT(ERROR_CODE_UNDEFINED, axis >= 0 && axis < len) << "Invalid axis value: " << axis << ". Expected range: [-" << len << "," << len - 1 << "]\n";

    ASSERT(ERROR_CODE_UNDEFINED, len != 4 || axis != 0) << "Sort not support the 0th axis of 4D input.\n";

    auto validShape = self.GetStorage()->GetDynValidShape();
    auto vecTileShape = TileShape::Current().GetVecTile();
    ASSERT(ERROR_CODE_UNDEFINED, vecTileShape[axis] % 32 == 0) << "The size of the tile shape along axis " << axis << " must be a multiple of 32. Got " << vecTileShape[axis] << ".\n";
    ...
    if (checkIsExceedUB(vecTileShape.tile, self.GetShape(), axis, 32)) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, tileNum < 128) << "For Large Shape in GM, the number of tile on sort axis must less than 128. Got " << tileNum << ".\n";
    }
