...
void TiledBitSort(Function &function, const TileShape &tileShape, size_t cur, Input &input,
    ...
    if (cur == input.tensor.GetShape().size()) {
        ...
        std::vector<int64_t> tmpShape = {1, resultTile->shape[resultTile->shape.size() - 1]};
        auto tempTensor =
...
void TiledMrgSort(Function &function, const TileShape &tileShape, size_t cur, Input &input,
    ...
    if (cur == input.tensor.GetShape().size()) {
        auto inputTile = input.tensor.GetStorage()->View(function, input.tileInfo.shape, input.tileInfo.offset);
        auto resultTile = result->View(function, resultTileInfo.shape, resultTileInfo.offset);
        std::vector<int64_t> tmpShape = {1, inputTile->shape[inputTile->shape.size() - 1]};
        auto tempTensor =
                std::make_shared<LogicalTensor>(function, inputTile->Datatype(), tmpShape);
...
Tensor Sort(const Tensor &self, int axis) {
    DECLARE_TRACER();
    const auto len = static_cast<int>(self.GetShape().size());
    ASSERT(axis == (len - 1) || axis == -1) << "Sort only support last axis";
    axis = axis >= 0 ? axis : (axis + len);

    auto bitsortOutShape = self.GetShape();
    bitsortOutShape[axis] = (bitsortOutShape[axis] + NUM_VALUE_31) / NUM_VALUE_32 * NUM_VALUE_32;
    bitsortOutShape[axis] *= NUM_VALUE_2;
    auto bitsortResult = Tensor(self.GetStorage()->tensor->datatype, bitsortOutShape);
    CALL(BitsortOperation, *Program::GetInstance().GetCurrentFunction(), self.GetStorage(), bitsortResult.GetStorage(),
        0, len - 1, 1);
    
    const auto k = static_cast<int>(self.GetShape()[len - 1]);
    auto mrgsortOutShape = bitsortOutShape;
    mrgsortOutShape[len - 1] = (k * sizeof(float) + kBlockSize - 1) / kBlockSize * kBlockSize / sizeof(float) * NUM_VALUE_2;
    auto mrgsortResult = Tensor(self.GetStorage()->tensor->datatype, mrgsortOutShape);
    CALL(MrgSortOperation, *Program::GetInstance().GetCurrentFunction(), bitsortResult.GetStorage(), mrgsortResult.GetStorage(),
        NUM_VALUE_32, len - 1, k);

    auto resultOutShape = self.GetShape();
    auto result = Tensor(self.GetStorage()->tensor->datatype, resultOutShape);
    CALL(ExtractOperation, *Program::GetInstance().GetCurrentFunction(), mrgsortResult.GetStorage(), result.GetStorage(), 0, k, true);
    return result;
}
...
void TiledTopK(Function &function, const TileShape &tileShape, size_t cur, Input &input,
    ...
    if (static_cast<int>(cur) == axis) {
        ...
        for (int i = 0; i < input.tensor.GetShape()[axis]; i += vecTileAlign[axis]) {
            ...
            std::vector<int64_t> tmpShape = {1, bitsortTile->shape[bitsortTile->shape.size() - 1]};
...
void TiledSort(Function &function, const TileShape &tileShape, size_t cur, Input &input,
    ...
    if (static_cast<int>(cur) == axis) {
        ...
        if (!isInGM) {
            ...
            std::vector<int64_t> tmpShape = {1, bitSortOutputShape[axis]};
