...
LogicalTensorPtr TensorPadOperation(
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, mode == "constant")
    ...
    if (ndim == 1) {
        ...
        ASSERT(VectorErrorCode::ERR_PARAM_INVALID, padding[0] == 0)
            << "Pad: 1D tensor only support right pad.";
        padRight = padding[1];
        outputShape[0] += padRight;
    } else {
        ASSERT(VectorErrorCode::ERR_PARAM_INVALID, padding.size() == 4)
            << "Pad: only support last 2 axis pad.";
...
LogicalTensorPtr TensorFillPadOperation(Function &function, const Tensor &self, const std::string &mode, float value) {
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, mode == "constant")
        << "FillPad: only 'constant' mode is supported.";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, std::isinf(value) || (std::abs(value) < 1e-6))
