...
LogicalTensorPtr TensorPadOperation(
    ...
    ASSERT(ERROR_CODE_UNKNOWN, mode == "constant") << "Pad: only 'constant' mode is supported.";
    ASSERT(ERROR_CODE_UNKNOWN, std::isinf(value) || (std::abs(value) < 1e-6)) << "Pad: pad value must be -inf, inf, or 0.";
    ...
    if (ndim == 1) {
        ASSERT(ERROR_CODE_UNKNOWN, padding.size() == 2) << "Pad: 1D tensor only support 2 padding values.";
        ASSERT(ERROR_CODE_UNKNOWN, padding[0] == 0) << "Pad: 1D tensor only support right pad.";
        padRight = padding[1];
        outputShape[0] += padRight;
    } else {
        ASSERT(ERROR_CODE_UNKNOWN, padding.size() == 4) << "Pad: only support last 2 axis pad.";
        ASSERT(ERROR_CODE_UNKNOWN, padding[0] == 0 && padding[2] == 0) << "Pad: only support bottom and right pad.";
        padRight = padding[1];
        padBottom = padding[3];
