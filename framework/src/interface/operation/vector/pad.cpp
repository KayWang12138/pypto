...
void PadOperationTileFunc(Function &function, const TileShape &tileShape, const std::vector<LogicalTensorPtr> &iOperand,
    ...
    int64_t padRight = op.HasAttribute(OP_ATTR_PREFIX + "pad_right") ? op.GetIntAttribute(OP_ATTR_PREFIX + "pad_right") : 0;
    int64_t padBottom = op.HasAttribute(OP_ATTR_PREFIX + "pad_bottom") ? op.GetIntAttribute(OP_ATTR_PREFIX + "pad_bottom") : 0;
    Element padValue = op.HasAttribute(OpAttributeKey::scalar) ? op.GetElementAttribute(OpAttributeKey::scalar) : Element(DT_FP32, 0.0f);
