...
void TiledCompareOperationImpl(Function &function, const TileShape &tileShape, size_t cur, Input &input1, Input &input2,
    ...
    if (cur == result->shape.size()) {
        ...
        ASSERT(element_size != 0) << "[VectorErrorCode::ERR_RUNTIME_LOGIC] Element size cannot be zero.";
...
LogicalTensorPtr TensorCompareOperation(
    ...
    if (mode == OutType::BIT) {
        resultType = DT_UINT8;
        ASSERT(resultShape.empty() || resultShape.back() % NUM_VALUE_8 == 0)
            << "[VectorErrorCode::ERR_CONFIG_ALIGNMENT] Last dimension must be divisible by 8 in BIT mode";
