...
void TiledWhereOperation(Function &function, const TileShape &tileShape, const LogicalTensorPtr &condition,
    ...
    if (condition->Datatype() == DT_UINT8) {
        int bitsNumOfByte = 8;
        ASSERT(ERROR_CODE_UNDEFINED, tileShape.GetVecTile().tile.back() % bitsNumOfByte == 0)
...
LogicalTensorPtr TensorWhereOperation(
    ...
    ASSERT(ERROR_CODE_UNDEFINED, condition.GetShape().size() == condition.GetStorage()->offset.size())
        << "The shape size of condition and offset must be equal";
    ASSERT(ERROR_CODE_UNDEFINED, input.GetShape().size() == input.GetStorage()->offset.size())
        << "The shape size of input and offset must be equal";
    ASSERT(ERROR_CODE_UNDEFINED, other.GetShape().size() == other.GetStorage()->offset.size())
