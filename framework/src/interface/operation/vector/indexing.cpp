...
void CheckIndexAddParamsInvalid(
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, axis < static_cast<int>(self.GetShape().size()) && axis >= -static_cast<int>(self.GetShape().size()))
        << "axis out of range of shape size";
    int axis_ = axis < 0 ? self.GetShape().size() + axis : axis;
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, self.GetShape().size() == src.GetShape().size()) << "shape size of self and src should be equal";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, src.GetShape()[axis_] == indices.GetShape()[0]) << "src shape[axis] and indices[0] must equal";
    for (size_t i = 0; i < self.GetShape().size(); ++i) {
        if (static_cast<int>(i) == axis_) {
            continue;
        }
        ASSERT(VectorErrorCode::ERR_PARAM_INVALID, src.GetShape()[i] == self.GetShape()[i]) << "src shape and self shape should be equal";
    }

    const std::unordered_set<DataType> SRC_SUPPORT_DATATYPES = {DT_FP32, DT_FP16, DT_BF16, DT_INT32, DT_INT16, DT_INT8};
    ASSERT(VectorErrorCode::ERR_PARAM_DTYPE_UNSUPPORTED, SRC_SUPPORT_DATATYPES.count(self.GetDataType()) > 0) << "The datatype is not supported";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, self.GetDataType() == src.GetDataType()) << "Datatype of src and self should be equal";
...
std::vector<int64_t> GatherOperationResultShape(LogicalTensorPtr params, LogicalTensorPtr indices, int axis) {
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, params->shape.size() == params->offset.size()) << "The size of params shape and offset should be equal";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, indices->shape.size() == indices->offset.size()) << "The size of indices shape and offset should be equal";
...
void TiledGatherOperation(Function &function, const TileShape &tileShape, const LogicalTensorPtr &params,
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, result->shape.size() == result->offset.size()) << "The size of result shape and offset should be equal";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, params->shape.size() == params->offset.size()) << "The size of params shape and offset should be equal";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, indices->shape.size() == indices->offset.size()) << "The size of indices shape and offset should be equal";

    ASSERT(VectorErrorCode::ERR_CONFIG_UNSUPPORTED, result->shape.size() <= NUM_VALUE_5) << "Not support shape size of result greater than 5";
    ASSERT(VectorErrorCode::ERR_CONFIG_UNSUPPORTED, indices->shape.size() <= NUM_VALUE_2) << "Not support shape size of indices greater than 2";
    if (axis < 0) {
...
LogicalTensorPtr TensorGatherOperation(
    ...
    if (axis < 0) {
        axis += paramsRank;
        ASSERT(VectorErrorCode::ERR_PARAM_INVALID, axis >= 0 && axis < paramsRank) << "The configuration of the axis is incorrect";
    }
...
Tensor GatherElements(const Tensor &params, const Tensor &indices, int axis) {
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, axis < static_cast<int>(params.GetShape().size()) && axis >= -static_cast<int>(params.GetShape().size()))
    ...
    for (size_t i = 0; i < params.GetShape().size(); ++i) {
        ...
        ASSERT(VectorErrorCode::ERR_PARAM_INVALID, indices.GetShape()[i] <= params.GetShape()[i]) << "The shape of params and indices should be equal";
    }
...
void InnerTiledScatterElementS(size_t cur, Function &function, const TileShape &tileShape,
    ...
    // 按照dstShape进行切分
    auto &vecTile = tileShape.GetVecTile();
    ASSERT(VectorErrorCode::ERR_CONFIG_TILE, vecTile[axis] >= dstTensor->shape[axis]) << "The axis is not supported for tile splitting";
    ASSERT(VectorErrorCode::ERR_CONFIG_TILE, vecTile[axis] >= idxInput->shape[axis]) << "The axis is not supported for tile splitting";
    int64_t tmpTile = vecTile[cur];
...
void TiledScatterElementS(Function &function, const TileShape &tileShape, const ScatterElementSPara &scatterPara) {
    // Check Operands Valid
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, scatterPara.srcInput->shape.size() == scatterPara.srcInput->offset.size())
        << "The size of srcInput shape and offset should be equal";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, scatterPara.idxInput->shape.size() == scatterPara.idxInput->offset.size())
        << "The size of idxInput shape and offset should be equal";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, scatterPara.dstTensor->shape.size() == scatterPara.dstTensor->offset.size())
...
static void CheckScatterElementSParamsInvalid(
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, self.GetShape().size() == indices.GetShape().size()) << "The shape size of self and indices should be equal";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, axis < static_cast<int>(self.GetShape().size())) << "The axis should be less than size of self shape";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, reduce <= ScatterMode::UNKNOWN) << "The ScatterMode of reduce should be less than UNKNOWN";
    for (size_t i = 0; i < self.GetShape().size(); i++) {
        if (static_cast<int>(i) == axis) {
            continue;
        }
        ASSERT(VectorErrorCode::ERR_PARAM_INVALID, indices.GetShape()[i] <= self.GetShape()[i]) << "The shape of indices and self should be equal";
    }
}
...
void TiledScatter(Function &function, const TileShape &tileShape, const ScatterPara &scatterPara) {
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, scatterPara.selfInput->shape.size() == scatterPara.selfInput->offset.size())
...
static void CheckScatterParamsInvalid(
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, idx_dtype == DataType::DT_INT32 || idx_dtype == DataType::DT_INT64)
        << "Scatter: 'indices' must be of integer type (int32 or int64)";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, self.GetShape().size() == indices.GetShape().size()) << "The shape size of self and indices should be equal";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, src.GetShape().size() == indices.GetShape().size()) << "The shape size of src and indices should be equal";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, axis < static_cast<int>(self.GetShape().size())) << "The axis should be less than size of self shape";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, reduce <= ScatterMode::UNKNOWN) << "The ScatterMode of reduce should be less than UNKNOWN";
    for (size_t i = 0; i < self.GetShape().size(); i++) {
        ASSERT(VectorErrorCode::ERR_PARAM_INVALID, indices.GetShape()[i] <= src.GetShape()[i]) << "The shape size of src and indices should be equal";
        if (static_cast<int>(i) == axis) {
            continue;
        }
        ASSERT(VectorErrorCode::ERR_PARAM_INVALID, indices.GetShape()[i] <= self.GetShape()[i]) << "The shape size of src and indices should be equal";
    }
}

Tensor Scatter(const Tensor &self, const Tensor &indices, const Tensor &src, int axis, ScatterMode reduce) {
    DECLARE_TRACER();
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, self.GetDataType() == src.GetDataType()) << "Datatype of self and src should be equal";
...
void TiledScatterUpdateFor2Dims(Function &function, const TileShape &tileShape, const LogicalTensorPtr &result,
    ...
    ASSERT(VectorErrorCode::ERR_CONFIG_TILE, tileD == src->shape[NUM_VALUE_1]) << "The tileD and src shape[0] should be equal";
...
void TiledScatterUpdate(Function &function, const TileShape &tileShape, const LogicalTensorPtr &result,
    ...
    if (cacheMode == "PA_BSND") {
        ASSERT(VectorErrorCode::ERR_PARAM_INVALID, src->shape.size() == NUM_VALUE_2 || src->shape.size() == NUM_VALUE_4) << "shape must be 2 or 4";
    ...
    // Check Operands Valid
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, result->shape.size() == result->offset.size()) << "The shape of result and offset should be equal";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, src->shape.size() == src->offset.size()) << "The shape of src and offset should be equal";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, index->shape.size() == index->offset.size()) << "The shape of index and offset should be equal";
...
static void CheckScatterUpdateInput(const Tensor &input) {
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, (input.GetShape().size() == NUM_VALUE_2 &&
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, input.GetShape().size() == NUM_VALUE_2 || input.GetShape().size() == NUM_VALUE_4)
        << "The shape size of input is invaild";
}

static void CheckScatterUpdateIndex(const Tensor &index) {
    ASSERT(VectorErrorCode::ERR_PARAM_DTYPE_UNSUPPORTED, index.GetDataType() == DT_INT64 || index.GetDataType() == DT_INT32 || index.GetDataType() == DT_INT16)
        << "The datatype of input is not supported";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, index.GetShape().size() == NUM_VALUE_2 && index.GetShape(NUM_VALUE_0) != NUM_VALUE_0 &&
...
static void CheckScatterUpdateInvalid(const Tensor &dst, const Tensor &index, const Tensor &src) {
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, src.GetShape().size() == dst.GetShape().size()) << "The shape size of src and dst should be equal";
...
Tensor ScatterUpdate(
    ...
    if (cacheMode == "PA_NZ") {
        axis = 1;
        ASSERT(VectorErrorCode::ERR_PARAM_INVALID, src.GetShape().size() == NUM_VALUE_2) << "Only support 2 dim"; // only support 2 dim
...
void TensorIndexPut(Function &function, const LogicalTensorPtr &self, const LogicalTensors &indices, const LogicalTensorPtr &values,
    ...
    for (size_t i = 0; i < indicesSize; i++) {
        ASSERT(VectorErrorCode::ERR_PARAM_INVALID, indices[i]->GetShape().size() == 1) << "Tensors in indices should be 1D";
        ASSERT(VectorErrorCode::ERR_PARAM_INVALID, indices[i]->GetShape()[0] == indicesShape) << "Tensors in indices should have the same shape";
    }
    constexpr size_t num1 = 1;
    constexpr size_t num4 = 4;
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, indicesSize >= num1 && indicesSize <= num4) << "indicesSize is out of range [1, 4]";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, dimSelf >= num1 && dimSelf <= num4) << "input dimSelf is out of range [2, 4]";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, dimValues >= num1 && dimValues <= num4) << "input sizeIndices is out of range [1, 4]";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, dimValues +  indicesSize == dimSelf + num1) << "unsupport the inputs shape combination: dimValues +  indicesSize != dimSelf + 1";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, valuesFirstDim == indicesShape) << "valuesFirstDim should equal to indicesSize";
    for (size_t i = 1; i < dimValues; i++) {
        ASSERT(VectorErrorCode::ERR_PARAM_INVALID, selfShape[dimSelf - i] == valuesShape[dimValues - i]) << "valuesShape should match selfShape";
    }
...
Tensor RealRange(Element &start, Element &end, Element &step) {
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, resultSize > 0) << "The positivity or negativity of the step should be aligned with the end-start";
...
Tensor GatherMask(const Tensor &self, const uint8_t patternMode) {
    ...
    if (patternMode == 1 || patternMode == 2) {
        ...
        ASSERT(VectorErrorCode::ERR_PARAM_INVALID, patternMode == 7) << "Just support patternMode is 1, 2, 3, 4, 5, 6, 7";
    }
