...
Tensor GatherElements(const Tensor& params, const Tensor& indices, int axis)
{
    ...
    CheckTensorDimRange(params.GetStorage(), 2, 4, "GATHERELEMENTS");
    ...
    const Tensor& self, const Tensor& indices, int axis, const ScatterMode reduce)
{
    ...
    CheckTensorDimRange(self.GetStorage(), 2, 4, "SCATTER");
...
{
    std::unordered_set<DataType> supportedTypes = {DT_FP32, DT_FP16, DT_BF16, DT_INT32, DT_INT16};
    CheckTensorDataType(input.GetStorage(), supportedTypes, "SCATTERUPDATE");
    ASSERT(
        VectorErrorCode::ERR_PARAM_INVALID,
...
static void CheckScatterUpdateInvalid(const Tensor& dst, const Tensor& index, const Tensor& src)
{
    ...
    CheckTensorsFormatConsistency(dst.GetStorage(), src.GetStorage(), "SCATTERUPDATE");
    CheckTensorsFormatConsistency(dst.GetStorage(), index.GetStorage(), "SCATTERUPDATE");
    CheckTensorsFormatConsistency(src.GetStorage(), index.GetStorage(), "SCATTERUPDATE");
}
...
{
    std::unordered_set<DataType> supportedTypes = {DT_INT8, DT_UINT8, DT_INT16, DT_UINT16, DT_INT32, DT_UINT32, DT_INT64, DT_UINT64, DT_BF16, DT_FP16, DT_FP32};
    ...
    std::unordered_set<DataType> indexSupportedTypes = {DT_INT8, DT_UINT8, DT_INT16, DT_UINT16, DT_INT32, DT_UINT32, DT_INT64, DT_UINT64};
    int indicesShape = -1;
    for (size_t i = 0; i < indices.size(); i++) {
    ...
    for (size_t i = 1; i < values.GetShape().size(); i++) {
        ASSERT(VectorErrorCode::ERR_PARAM_INVALID, self.GetShape()[self.GetShape().size() - i] == values.GetShape()[values.GetShape().size() - i])
