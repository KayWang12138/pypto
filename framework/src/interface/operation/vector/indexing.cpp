...
void TiledGatherOperation(Function &function, const TileShape &tileShape, const LogicalTensorPtr &params,
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, result->shape.size() <= NUM_VALUE_5) << "Not support shape size of result greater than 5";
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, indices->shape.size() <= NUM_VALUE_2) << "Not support shape size of indices greater than 2";
    if (axis < 0) {
