...
void TiledIndexAdd(Function &function, const TileShape &tileShape, const IndexAddPara indexaddPara) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, indexaddPara.selfInput->GetShape().size() == indexaddPara.selfInput->GetOffset().size())
        << "The size of indexaddPara selfinput shape and selfinput offset should be equal";
    ASSERT(ERROR_CODE_UNDEFINED, indexaddPara.srcInput->GetShape().size() == indexaddPara.srcInput->GetOffset().size())
        << "The size of indexaddPara srcInput shape and srcInput offset should be equal";
    ASSERT(ERROR_CODE_UNDEFINED, indexaddPara.indicesInput->GetShape().size() == indexaddPara.indicesInput->GetOffset().size())
...
void CheckIndexAddParamsInvalid(
    ...
    ASSERT(ERROR_CODE_UNDEFINED, axis < static_cast<int>(self.GetShape().size()) && axis >= -static_cast<int>(self.GetShape().size()))
        << "axis out of range of shape size";
    int axis_ = axis < 0 ? self.GetShape().size() + axis : axis;
    ASSERT(ERROR_CODE_UNDEFINED, self.GetShape().size() == src.GetShape().size()) << "shape size of self and src should be equal";
    ASSERT(ERROR_CODE_UNDEFINED, src.GetShape()[axis_] == indices.GetShape()[0]) << "src shape[axis] and indices[0] must equal";
    for (size_t i = 0; i < self.GetShape().size(); ++i) {
        if (static_cast<int>(i) == axis_) {
            continue;
        }
        ASSERT(ERROR_CODE_UNDEFINED, src.GetShape()[i] == self.GetShape()[i]) << "src shape and self shape should be equal";
    }

    const std::unordered_set<DataType> SRC_SUPPORT_DATATYPES = {DT_FP32, DT_FP16, DT_BF16, DT_INT32, DT_INT16, DT_INT8};
    ASSERT(ERROR_CODE_UNDEFINED, SRC_SUPPORT_DATATYPES.count(self.GetDataType()) > 0) << "The datatype is not supported";
    ASSERT(ERROR_CODE_UNDEFINED, self.GetDataType() == src.GetDataType()) << "Datatype of src and self should be equal";
    ASSERT(ERROR_CODE_UNDEFINED, indices.GetDataType() == DT_INT32 || indices.GetDataType() == DT_INT64)
    ...
    if (CheckAlphaOverflow(alpha, self.GetDataType())) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, false) << errorMessage;
    }
}
...
std::vector<int64_t> GatherOperationResultShape(LogicalTensorPtr params, LogicalTensorPtr indices, int axis) {
    ASSERT(ERROR_CODE_UNDEFINED, params->shape.size() == params->offset.size()) << "The size of params shape and offset should be equal";
    ASSERT(ERROR_CODE_UNDEFINED, indices->shape.size() == indices->offset.size()) << "The size of indices shape and offset should be equal";
...
void TiledGatherOperation(Function &function, const TileShape &tileShape, const LogicalTensorPtr &params,
    ...
    ASSERT(ERROR_CODE_UNDEFINED, result->shape.size() == expectedShape.size())
        << "The size of result shape and expectedShape should be equal";
    ASSERT(ERROR_CODE_UNDEFINED, result->shape.size() == result->offset.size()) << "The size of result shape and offset should be equal";
    ASSERT(ERROR_CODE_UNDEFINED, params->shape.size() == params->offset.size()) << "The size of params shape and offset should be equal";
    ASSERT(ERROR_CODE_UNDEFINED, indices->shape.size() == indices->offset.size()) << "The size of indices shape and offset should be equal";

    ASSERT(ERROR_CODE_UNDEFINED, result->shape.size() <= NUM_VALUE_5) << "Not support shape size of result greater than 5";
    ASSERT(ERROR_CODE_UNDEFINED, indices->shape.size() <= NUM_VALUE_2) << "Not support shape size of indices greater than 2";
    if (axis < 0) {
        axis += params->shape.size();
    }
    ASSERT(ERROR_CODE_UNDEFINED, axis >= 0 && axis < static_cast<int>(params->shape.size()))
...
LogicalTensorPtr TensorGatherOperation(
    ...
    if (axis < 0) {
        axis += paramsRank;
        ASSERT(ERROR_CODE_UNDEFINED, axis >= 0 && axis < paramsRank) << "The configuration of the axis is incorrect";
    }
...
Tensor GatherElements(const Tensor &params, const Tensor &indices, int axis) {
    DECLARE_TRACER();
    ASSERT(ERROR_CODE_UNDEFINED, params.GetShape().size() == indices.GetShape().size())
        << "The shape size of params and indices should be equal";
    ASSERT(ERROR_CODE_UNDEFINED, axis < static_cast<int>(params.GetShape().size()) && axis >= -static_cast<int>(params.GetShape().size()))
    ...
    for (size_t i = 0; i < params.GetShape().size(); ++i) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, indices.GetShape()[i] <= params.GetShape()[i]) << "The shape of params and indices should be equal";
    }
...
void InnerTiledScatterElementS(size_t cur, Function &function, const TileShape &tileShape,
    ...
    ASSERT(ERROR_CODE_UNDEFINED, vecTile[axis] >= dstTensor->shape[axis]) << "The axis is not supported for tile splitting";
    ASSERT(ERROR_CODE_UNDEFINED, vecTile[axis] >= idxInput->shape[axis]) << "The axis is not supported for tile splitting";
    int64_t tmpTile = vecTile[cur];
...
void TiledScatterElementS(Function &function, const TileShape &tileShape, const ScatterElementSPara &scatterPara) {
    // Check Operands Valid
    ASSERT(ERROR_CODE_UNDEFINED, scatterPara.srcInput->shape.size() == scatterPara.srcInput->offset.size())
        << "The size of srcInput shape and offset should be equal";
    ASSERT(ERROR_CODE_UNDEFINED, scatterPara.idxInput->shape.size() == scatterPara.idxInput->offset.size())
        << "The size of idxInput shape and offset should be equal";
    ASSERT(ERROR_CODE_UNDEFINED, scatterPara.dstTensor->shape.size() == scatterPara.dstTensor->offset.size())
...
static void CheckScatterElementSParamsInvalid(
    ...
    ASSERT(ERROR_CODE_UNDEFINED, self.GetShape().size() == indices.GetShape().size()) << "The shape size of self and indices should be equal";
    ASSERT(ERROR_CODE_UNDEFINED, axis < static_cast<int>(self.GetShape().size())) << "The axis should be less than size of self shape";
    ASSERT(ERROR_CODE_UNDEFINED, reduce <= ScatterMode::UNKNOWN) << "The ScatterMode of reduce should be less than UNKNOWN";
    for (size_t i = 0; i < self.GetShape().size(); i++) {
        if (static_cast<int>(i) == axis) {
            continue;
        }
        ASSERT(ERROR_CODE_UNDEFINED, indices.GetShape()[i] <= self.GetShape()[i]) << "The shape of indices and self should be equal";
    }
}
...
void TiledScatter(Function &function, const TileShape &tileShape, const ScatterPara &scatterPara) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, scatterPara.selfInput->shape.size() == scatterPara.selfInput->offset.size())
...
static void CheckScatterParamsInvalid(
    ...
    ASSERT(ERROR_CODE_UNDEFINED, self.GetShape().size() == indices.GetShape().size()) << "The shape size of self and indices should be equal";
    ASSERT(ERROR_CODE_UNDEFINED, src.GetShape().size() == indices.GetShape().size()) << "The shape size of src and indices should be equal";
    ASSERT(ERROR_CODE_UNDEFINED, axis < static_cast<int>(self.GetShape().size())) << "The axis should be less than size of self shape";
    ASSERT(ERROR_CODE_UNDEFINED, reduce <= ScatterMode::UNKNOWN) << "The ScatterMode of reduce should be less than UNKNOWN";
    for (size_t i = 0; i < self.GetShape().size(); i++) {
        ASSERT(ERROR_CODE_UNDEFINED, indices.GetShape()[i] <= src.GetShape()[i]) << "The shape size of src and indices should be equal";
        if (static_cast<int>(i) == axis) {
            continue;
        }
        ASSERT(ERROR_CODE_UNDEFINED, indices.GetShape()[i] <= self.GetShape()[i]) << "The shape size of src and indices should be equal";
    }
}

Tensor Scatter(const Tensor &self, const Tensor &indices, const Tensor &src, int axis, ScatterMode reduce) {
    DECLARE_TRACER();
    ASSERT(ERROR_CODE_UNDEFINED, self.GetDataType() == src.GetDataType());
...
void TiledScatterUpdateFor2Dims(Function &function, const TileShape &tileShape, const LogicalTensorPtr &result,
    ...
    if (s == 0 || tileBS == 0) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, s == 0 || tileBS == 0);
    }
    if ((tileBS < s && s % tileBS != 0) || (tileBS > s && tileBS % s != 0)) {
        ALOG_ERROR_F("tileshape 0 is invalid, tileshape(%d, %d)", tileBS, tileD);
    }
    ASSERT(ERROR_CODE_UNDEFINED, (tileBS <= s && s % tileBS == 0) || (tileBS > s && tileBS % s == 0));
    ASSERT(ERROR_CODE_UNDEFINED, tileD == src->shape[NUM_VALUE_1]) << "The tileD and src shape[0] should be equal";
...
void TiledScatterUpdate(Function &function, const TileShape &tileShape, const LogicalTensorPtr &result,
    ...
    if (cacheMode == "PA_BSND") {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, src->shape.size() == NUM_VALUE_2 || src->shape.size() == NUM_VALUE_4);
        return;
    }
    // Check Operands Valid
    ASSERT(ERROR_CODE_UNDEFINED, result->shape.size() == result->offset.size()) << "The shape of result and offset should be equal";
    ASSERT(ERROR_CODE_UNDEFINED, src->shape.size() == src->offset.size()) << "The shape of src and offset should be equal";
    ASSERT(ERROR_CODE_UNDEFINED, index->shape.size() == index->offset.size()) << "The shape of index and offset should be equal";
...
static void CheckScatterUpdateInput(const Tensor &input) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, (input.GetShape().size() == NUM_VALUE_2 &&
               (input.GetShape(NUM_VALUE_0) != NUM_VALUE_0 && input.GetShape(NUM_VALUE_1) != NUM_VALUE_0)) ||
           (input.GetShape().size() == NUM_VALUE_4 &&
               (input.GetShape(NUM_VALUE_0) != NUM_VALUE_0 && input.GetShape(NUM_VALUE_1) != NUM_VALUE_0 &&
                   input.GetShape(NUM_VALUE_2) != NUM_VALUE_0 && input.GetShape(NUM_VALUE_3) != NUM_VALUE_0)))
        << "The shape of input is invaild";
    ASSERT(ERROR_CODE_UNDEFINED, input.GetShape().size() == NUM_VALUE_2 || input.GetShape().size() == NUM_VALUE_4)
...
static void CheckScatterUpdateIndex(const Tensor &index) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, index.GetDataType() == DT_INT64 || index.GetDataType() == DT_INT32 || index.GetDataType() == DT_INT16)
    ...
    ASSERT(ERROR_CODE_UNDEFINED, index.GetShape().size() == NUM_VALUE_2 && index.GetShape(NUM_VALUE_0) != NUM_VALUE_0 &&
           index.GetShape(NUM_VALUE_1) != NUM_VALUE_0)
...
static void CheckScatterUpdateInvalid(const Tensor &dst, const Tensor &index, const Tensor &src) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, src.GetShape().size() == dst.GetShape().size()) << "The shape size of src and dst should be equal";
...
Tensor ScatterUpdate(
    ...
    if (cacheMode == "PA_NZ") {
        axis = 1;
        ASSERT(ERROR_CODE_UNDEFINED, src.GetShape().size() == NUM_VALUE_2) << "Only support 2 dim"; // only support 2 dim
...
void TiledIndexPut(Function &function, const TileShape &tileShape, const LogicalTensorPtr &self, const LogicalTensorPtr &values,
    ...
    ASSERT(ERROR_CODE_UNDEFINED, self->GetShape().size() == self->GetOffset().size());
    ASSERT(ERROR_CODE_UNDEFINED, values->GetShape().size() == values->GetOffset().size());
    for (size_t i = 0; i < indices.size(); i++) {
        ASSERT(ERROR_CODE_UNDEFINED, indices[i]->GetShape().size() == indices[i]->GetOffset().size());
    }
...
void TensorIndexPut(Function &function, const LogicalTensorPtr &self, const LogicalTensors &indices, const LogicalTensorPtr &values,
    ...
    for (size_t i = 0; i < indicesSize; i++) {
        ASSERT(ERROR_CODE_UNDEFINED, indices[i]->GetShape().size() == 1) << "Tensors in indices should be 1D";
        ASSERT(ERROR_CODE_UNDEFINED, indices[i]->GetShape()[0] == indicesShape) << "Tensors in indices should have the same shape";
    }
    constexpr size_t num1 = 1;
    constexpr size_t num4 = 4;
    ASSERT(ERROR_CODE_UNDEFINED, indicesSize >= num1 && indicesSize <= num4) << "indicesSize is out of range [1, 4]";
    ASSERT(ERROR_CODE_UNDEFINED, dimSelf >= num1 && dimSelf <= num4) << "input dimSelf is out of range [2, 4]";
    ASSERT(ERROR_CODE_UNDEFINED, dimValues >= num1 && dimValues <= num4) << "input sizeIndices is out of range [1, 4]";
    ASSERT(ERROR_CODE_UNDEFINED, dimValues +  indicesSize == dimSelf + num1) << "unsupport the inputs shape combination: dimValues +  indicesSize != dimSelf + 1";
    ASSERT(ERROR_CODE_UNDEFINED, valuesFirstDim == indicesShape) << "valuesFirstDim should equal to indicesSize"; 
    for (size_t i = 1; i < dimValues; i++) {
        ASSERT(ERROR_CODE_UNDEFINED, selfShape[dimSelf - i] == valuesShape[dimValues - i]) << "valuesShape should match selfShape"; 
    }
...
int64_t GetRangeResSize(Element &start, Element &end, Element &step) {
    ...
    if (dataType == DT_INT32 || dataType == DT_INT64) {
        ...
        if (abs(stepValue) <= 0) {
            ASSERT(ERROR_CODE_UNDEFINED, false && "stepValue must not be 0");
        }
...
Tensor RealRange(Element &start, Element &end, Element &step) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, resultSize > 0 && "The positivity or negativity of the step should be aligned with the end-start");
...
DataType GetComputeDataType(const Element &start, const Element &end, const Element &step) {
    ...
    if (IsDataTypeUnsupport(startType)) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, false && errorMessage.c_str());
    }
...
Tensor GatherMask(const Tensor &self, const uint8_t patternMode) {
    ...
    if (patternMode == 1 || patternMode == 2) {
        ASSERT(ERROR_CODE_UNDEFINED, shape[shape.size() - 1] % 2 == 0) 
            << "The last axis of input shape should be divisible by 2 when ptternMode is 1 or 2";
        ASSERT(ERROR_CODE_UNDEFINED, vecTile.tile[vecTile.tile.size() - 1] % 2 == 0) 
