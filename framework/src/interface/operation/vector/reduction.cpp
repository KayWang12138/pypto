...
static Tensor ProcessResultShape(const Tensor &result, const Tensor &self, int axis, bool keepDim){
    ...
    if (keepDim || lastDim == 0) {
        ...
        auto reshapeVecTile = TileShape::Current().GetVecTile();
        std::vector<SymbolicScalar> outValidShape;
        ...
        reshapeVecTile.tile.erase(reshapeVecTile.tile.begin() + axis);
        const int alignNum = BLOCK_SIZE / BytesOf(self.GetStorage()->tensor->datatype);
        reshapeVecTile.tile[reshapeVecTile.tile.size() - 1] = (
            reshapeVecTile.tile[reshapeVecTile.tile.size() - 1] + alignNum - 1) / alignNum * alignNum;
        TileShape::Current().SetVecTile(reshapeVecTile);
