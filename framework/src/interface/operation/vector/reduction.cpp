...
Tensor RowSumExpand(const Tensor& operand)
{
    DECLARE_TRACER();
    std::unordered_set<DataType> supportedTypes = {DT_FP16, DT_BF16, DT_INT16, DT_INT32, DT_FP32};
    CheckTensorDataType(operand.GetStorage(), supportedTypes, "ROWSUMEXPAND");
    CheckTensorDimRange(operand.GetStorage(), 1, 4, "ROWSUMEXPAND");
    CheckTensorShapeSize(operand.GetStorage(), "ROWSUMEXPAND");
...
Tensor RowMaxExpand(const Tensor& operand)
{
    DECLARE_TRACER();
    std::unordered_set<DataType> supportedTypes = {DT_FP16, DT_BF16, DT_INT16, DT_INT32, DT_FP32};
    CheckTensorDataType(operand.GetStorage(), supportedTypes, "ROWMAXEXPAND");
    CheckTensorDimRange(operand.GetStorage(), 1, 4, "ROWMAXEXPAND");
    CheckTensorShapeSize(operand.GetStorage(), "ROWMAXEXPAND");
    Tensor result(operand.GetStorage()->Datatype(), operand.GetShape());
