...
Tensor Rsqrt(const Tensor& self, RsqrtAlgorithm precisionType)
{
    ...
    if (self.GetDataType() != DataType::DT_FP32) {
        RETURN_CALL(
            CastOperation<CastOpType::CAST>, *Program::GetInstance().GetCurrentFunction(), divResult, self.GetDataType(),
            CastMode::CAST_NONE);
    }
    return divResult;
}
...
Tensor Reciprocal(const Tensor& operand, ReciprocalAlgorithm precisionType)
{
    DECLARE_TRACER();
    ...
    const std::vector<LogicalTensorPtr>& oOperand, const Operation& op)
{
    ...
    (void)op.GetAttribute(OpAttributeKey::precisionType, precisionType);
    return TiledUnaryOperation<UnaryOpType::RECIPROCAL>(function, tileShape, iOperand[0], oOperand[0], static_cast<ReciprocalAlgorithm>(precisionType));
}
