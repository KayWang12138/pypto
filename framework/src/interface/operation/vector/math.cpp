...
LogicalTensorPtr TensorPowOperation(Function &function, const LogicalTensorPtr &self, const LogicalTensorPtr &other) {
    auto result = std::make_shared<LogicalTensor>(function, self->Datatype(), self->shape, self->dynValidShape_);
...
Tensor Pow(const Tensor &self, const Tensor &other) {
    ...
    auto result = CALL(PowOperation, *Program::GetInstance().GetCurrentFunction(), selfSt, otherSt);
    if (dataType != DT_FP32) {
