...
Tensor IsIntCondition(const LogicalTensorPtr &tensor) {
    ...
    return Compare(delta, Element(DataType::DT_FP32, NUM_VALUE_EPS), OpType::LT, OutType::BOOL);
}

Tensor IsOddCondition(const LogicalTensorPtr &tensor) {
    auto mod = Fmod(tensor, Element(tensor->Datatype(), NUM_VALUE_2));
    return Compare(mod, Element(DataType::DT_FP32, NUM_VALUE_EPS), OpType::LT, OutType::BOOL);
}
...
Tensor CalcPowByIntExponent(const LogicalTensorPtr &self, const LogicalTensorPtr &other) {
    ...
    auto selfIsOdd = IsOddCondition(self);
    ...
    auto sign = Where(selfLessThanZero, Where(selfIsOdd, allNegOne, allOne), allOne);
...
Tensor Pow(const Tensor &self, const Tensor &other) {
    ...
    auto result = Where(otherIsInt, intExponentResult, otherExponentResult).GetStorage();
    if (dataType != DT_FP32) {
