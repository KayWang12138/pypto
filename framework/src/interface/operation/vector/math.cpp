...
Tensor Clip(const Tensor& self, const Element& min, const Element& max)
{
    ...
    CheckTensorDimRange(self.GetStorage(), 2, 4, "CLIP");
...
Tensor Clip(const Tensor& self, const Tensor& min, const Tensor& max)
{
    ...
    if (min.GetStorage() != nullptr) {
        CheckTensorDataType(min.GetStorage(), supportedTypes, "CLIP");
        CheckTensorsDataTypeConsistency(self.GetStorage(), min.GetStorage(), "CLIP");
    ...
    if (max.GetStorage() != nullptr) {
        CheckTensorDataType(max.GetStorage(), supportedTypes, "CLIP");
        CheckTensorsDataTypeConsistency(self.GetStorage(), max.GetStorage(), "CLIP");
