...
void TiledLogicalNotOperation(
    ...
    ASSERT(ERROR_CODE_UNDEFINED, self->shape.size() == self->offset.size()) << "Shape size and offset size should be equal";
...
Tensor Log(const Tensor &self, LogBaseType base) {
    DECLARE_TRACER();
    ASSERT(ERROR_CODE_UNDEFINED, base == LogBaseType::LOG_E || base == LogBaseType::LOG_2 || base == LogBaseType::LOG_10)
        << "base is incorrect";
    ASSERT(ERROR_CODE_UNDEFINED, self.GetStorage()->tensor->datatype == DataType::DT_BF16 ||
...
void TiledOneHot(Function &function, const TileShape &tileShape, const LogicalTensorPtr &self,
    ...
    ASSERT(ERROR_CODE_UNDEFINED, self->shape.size() == self->offset.size()) << "Shape size and offset size should be equal";
    ASSERT(ERROR_CODE_UNDEFINED, numClasses == tileShape.GetVecTile()[result->shape.size() - 1])
...
void CheckCumSum(const Tensor &input, const int &axis) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, SHAPE_DIM1 <= shapeSize && shapeSize <= SHAPE_DIM5) << "The shape.size() only support 1~5";
    ...
    ASSERT(ERROR_CODE_UNDEFINED, std::find(CUMSUM_SUPPORT_DATATYPES.begin(), CUMSUM_SUPPORT_DATATYPES.end(), dataType) !=
    ...
    if (flag) {
        ASSERT(ERROR_CODE_UNDEFINED, tmpAxis0 == 0) << "when input.GetShape().size() is 1, axis must be 0";
    }
    ASSERT(ERROR_CODE_UNDEFINED, tmpAxis0 == 0 || static_cast<size_t>(tmpAxis0) < shapeSize)
...
void TensorTriUL(Function &function, const TriULPara &triULPara) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, SHAPE_DIM2 <= shapeSize && shapeSize <= SHAPE_DIM5) << "This operation's input only support 2-5 dims";
    std::unordered_set<DataType> TRIUL_SUPPORT_DATATYPES = {DataType::DT_FP32, DataType::DT_FP16, DataType::DT_INT32,
        DataType::DT_INT16, DataType::DT_INT8, DataType::DT_BF16};
    ASSERT(ERROR_CODE_UNDEFINED, TRIUL_SUPPORT_DATATYPES.count(dataType))<< "This datatype is not supported";
...
Tensor Clip(const Tensor &self, const Element &min, const Element &max) {
    ASSERT(ERROR_CODE_UNDEFINED, self.GetShape().size() >= SHAPE_DIM2 && self.GetShape().size() <= SHAPE_DIM4)
    ...
    ASSERT(ERROR_CODE_UNDEFINED, std::find(CLIP_SUPPORT_DATATYPES.begin(), CLIP_SUPPORT_DATATYPES.end(), self.GetDataType()) !=
    ...
    if (min_.GetDataType() != DT_BOTTOM) {
        ASSERT(ERROR_CODE_UNDEFINED, min_.GetDataType() == self.GetDataType()) << "The datatype of inputs should be same";
        result = Maximum(result, min_);
    }
    if (max_.GetDataType() != DT_BOTTOM) {
        ASSERT(ERROR_CODE_UNDEFINED, max_.GetDataType() == self.GetDataType()) << "The datatype of inputs should be same";
...
Tensor Clip(const Tensor &self, const Tensor &min, const Tensor &max) {
    ...
    if (min.GetStorage() != nullptr) {
        ASSERT(ERROR_CODE_UNDEFINED, min.GetDataType() == self.GetDataType()) << "The datatype of inputs should be same";
        std::vector minBroadcastAxes = GetBroadcastAxes(min.GetShape(), self.GetShape());
        ASSERT(ERROR_CODE_UNDEFINED, minBroadcastAxes.size() <= 1);
    ...
    if (max.GetStorage() != nullptr) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, maxBroadcastAxes.size() <= 1);
...
static void VarParamVaildCheck(const Tensor &input, std::vector<int> &dim)
{
    ...
    ASSERT(ERROR_CODE_UNDEFINED, shapeSize <= SHAPE_DIM4 && shapeSize >= SHAPE_DIM2) << "The shape.size() only support 2~4. Cur dimension"
        " is " << shapeSize;
    ASSERT(ERROR_CODE_UNDEFINED, dim.size() <= shapeSize) << "The dim.size() should <= input.size()";
    ASSERT(ERROR_CODE_UNDEFINED, (dtype == DT_FP32) || (dtype == DT_FP16) || (dtype == DT_BF16)) << "The datatype is only support float";
    for (uint64_t i = 0; i < shapeSize; i++) {
        ASSERT(ERROR_CODE_UNDEFINED, shape[i] > 0) << "The input shape should > 0";
    }
    ...
    for (size_t i = 0; i < dim.size(); i++) {
        ASSERT(ERROR_CODE_UNDEFINED, dim[i] < static_cast<int>(shapeSize) && dim[i] >= -(static_cast<int>(shapeSize))) <<
...
Tensor Exp2(const Tensor &self) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, SHAPE_DIM2 <= shapeSize && shapeSize <= SHAPE_DIM4) << "The shape.size() only support 2~4";
    ...
    ASSERT(ERROR_CODE_UNDEFINED, std::find(EXP2_SUPPORT_DATATYPES.begin(), EXP2_SUPPORT_DATATYPES.end(), dataType) !=
...
void TiledExp2(
    ...
    if (cur == input.tensor.GetShape().size()) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, SHAPE_DIM2 <= tileShapeLen && tileShapeLen <= SHAPE_DIM4) << "Length of tile shape only support 2~4";
...
Tensor Round(const Tensor &self, const int &decimals) {
    ...
    std::vector<DataType> ROUND_SUPPORT_DATATYPES = {
        DataType::DT_FP32, DataType::DT_FP16, DataType::DT_BF16, DataType::DT_INT32, DataType::DT_INT16};
    ASSERT(ERROR_CODE_UNDEFINED, std::find(ROUND_SUPPORT_DATATYPES.begin(), ROUND_SUPPORT_DATATYPES.end(), dataType) !=
...
Tensor Expm1(const Tensor &self) {
    ...
    std::vector<DataType> EXPM1_SUPPORT_DATATYPES = {
        DataType::DT_FP32, DataType::DT_FP16, DataType::DT_BF16, DataType::DT_INT32, DataType::DT_INT16};
    ASSERT(ERROR_CODE_UNDEFINED, std::find(EXPM1_SUPPORT_DATATYPES.begin(), EXPM1_SUPPORT_DATATYPES.end(), dataType) !=
