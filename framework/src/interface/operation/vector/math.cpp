...
Tensor Var(const Tensor &input, const std::vector<int> &dim, float correction, bool keepDim)
{
    ...
    if (!keepDim) {
        ...
        for (auto it = innerDim.rbegin(); it != innerDim.rend(); ++it) {
            vecTile.tile.erase(vecTile.tile.begin() + *it);
        }
        int64_t algnedSize = BLOCK_SIZE / BytesOf(dtype);
        if (vecTile.tile.empty()) {
            vecTile.tile.push_back(algnedSize);
        }
        int64_t lastDimSize = vecTile.tile.back();
        if (lastDimSize % algnedSize != 0) {
            vecTile.tile.back() = CeilDiv(lastDimSize, algnedSize) * algnedSize;
        }
        TileShape::Current().SetVecTile(vecTile.tile);
        res = Squeeze(res, innerDim);
    }
