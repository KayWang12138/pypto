...
void InnerTiledCumOperation(size_t cur, Function &function, const TileShape &tileShape, const CumOperationPara &cumOperationPara,
    ...
    const bool isSum = cumOperationPara.isSum;
    auto &vecTile = tileShape.GetVecTile();
    ...
    if (cur == dstTensor->shape.size()) {
        ...
        if (isSum) {
            ...
            op.SetAttribute(OP_ATTR_PREFIX + "flag", isSum);
        } else {
...
void TiledCumOperation(Function &function, const TileShape &tileShape, const CumOperationPara &cumOperationPara) {
    assert(cumOperationPara.input->GetShape().size() == cumOperationPara.input->GetOffset().size());
...
void TensorCumOperation(Function &function, const CumOperationPara &cumOperationPara) {
    if (cumOperationPara.input->Datatype() == DT_INT16) {
        ...
        op.SetAttribute(OP_ATTR_PREFIX + "flag", cumOperationPara.isSum);
...
void CheckCumOperation(const Tensor &input, const int &axis, const bool &isSum) {
    ...
    if (isSum) {
        ...
        ASSERT(std::find(CUMPROD_SUPPORT_DATATYPES.begin(), CUMPROD_SUPPORT_DATATYPES.end(), dataType) !=
           CUMPROD_SUPPORT_DATATYPES.end()) << "The datatype is not supported";
    }
    int tmpAxis0 = axis;
...
Tensor CumOperation(const Tensor &input, const int &axis, const bool &isSum) {
    DECLARE_TRACER();
    CheckCumOperation(input, axis, isSum);
    ...
    if ((resultDType != DataType::DT_INT64) && tmpAxis0 > 0 && tmpAxis0 == n_1) {
        ...
        CALL(CumOperation, *Program::GetInstance().GetCurrentFunction(),
            {tmpInput.GetStorage(), result.GetStorage(), transposedAxis, isSum});
        ...
        CALL(CumOperation, *Program::GetInstance().GetCurrentFunction(),
            {input.GetStorage(), result.GetStorage(), tmpAxis0, isSum});
...
Tensor CumSum(const Tensor &input, const int &axis) {
    bool isSum = true;
    Tensor result = CumOperation(input, axis, isSum);
    return result;
}
...
void CumSumOperationTileFunc(Function &function, const TileShape &tileShape,
    ...
    bool isSum = op.GetBoolAttribute(OP_ATTR_PREFIX + "flag");
    TiledCumOperation(function, tileShape, {iOperand[0], oOperand[0], axis, isSum});
}
