...
void TiledSignbitOperation(
    ...
    if (cur == input.tensor.GetShape().size()) {
        ...
        constexpr size_t ALIGN_SIZE = 32;
        int64_t tmpSize = ALIGN_SIZE / BytesOf(DT_FP16);
        if (input.tensor.GetDataType() == DT_FP32 || input.tensor.GetDataType() == DT_INT32 ||
            input.tensor.GetDataType() == DT_UINT32 || input.tensor.GetDataType() == DT_INT8 ||
            input.tensor.GetDataType() == DT_UINT8 || input.tensor.GetDataType() == DT_BOOL) {
            tmpSize = MultiplyLastTwoDims(input.tileInfo.shape);
        }
