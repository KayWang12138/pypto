...
Tensor TensorRound(Function &function, const LogicalTensorPtr &self, const int &decimals = 0) {
    auto result = std::make_shared<LogicalTensor>(function, self->Datatype(), self->GetShape(), self->GetDynValidShape());
    ...
    op.SetAttribute(OP_ATTR_PREFIX + "powDecimals", Element(DataType::DT_FP32, powDecimals));
    function.UpdateTensorDataUsage(op);
    return result;
}
...
void TiledRound(Function &function, const TileShape &tileShape, size_t cur, Input &input,
    ...
    if (cur == input.tensor.GetShape().size()) {
        auto tile = input.tensor.GetStorage()->View(function, input.tileInfo.shape, input.tileInfo.offset);
        auto resultTile = result->View(function, input.tileInfo.shape, input.tileInfo.offset);
        auto &newOp = function.AddOperation(Opcode::OP_ROUND, {tile}, {resultTile});
        float powDecimals = std::pow(static_cast<float>(10), static_cast<float>(decimals));
        if (decimals > 38) {
            powDecimals = INFINITY;
        }
        newOp.SetAttribute(OP_ATTR_PREFIX + "powDecimals", Element(DataType::DT_FP32, powDecimals));
        return;
    }
    auto &vecTile = tileShape.GetVecTile();
    for (int i = 0; i < input.tensor.GetShape()[cur]; i += vecTile[cur]) {
