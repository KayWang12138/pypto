...
DataType GetPowRealResultDataType(DataType selfType, DataType otherType) {
    if (selfType == DT_INT16) {
        return otherType;
    }
    if (otherType == DT_INT16) {
        return selfType;
    }
...
DataType GetPowCalcResultDataType(DataType selfType, DataType otherType) {
    if (selfType == DT_INT16 && otherType == DT_INT16) {
        return DT_INT16;
    }
    if ((selfType == DT_INT16 || selfType == DT_INT32) && (otherType == DT_INT16 || otherType == DT_INT32)) {
        return DT_INT32;
    }
    return DT_FP32;
}
