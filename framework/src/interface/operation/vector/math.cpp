...
std::int GetRealDiagonal(int diagonal, const std::vector<int64_t> &tileShape, int originXIdx, int originYIdx) {
    int realDiagonal = 0;
...
void InnerTiledTriUL(size_t cur, Function &function, const TileShape &tileShape, const TriULPara &triULPara,
    ...
    if (cur == dstTensor->shape.size()) {
        ...
        int realDiagonal = GetRealDiagonal(diagonal, triULTileInfo.inputTileInfo.shape, originXIdx, originYIdx);
