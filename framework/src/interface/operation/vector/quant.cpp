...
std::vector<int64_t> BuildQuantMXScaleShape(const std::vector<int64_t>& inputShape)
{
    ...
    return {mSize, CeilDiv(inputShape.back(), QUANT_MX_SCALE_GROUP_COLS), QUANT_MX_SCALE_PAIR_SIZE};
}

std::vector<SymbolicScalar> BuildQuantMXScaleValidShape(const std::vector<SymbolicScalar>& inputValidShape)
{
    SymbolicScalar mSize(1);
    for (size_t i = 0; i + 1 < inputValidShape.size(); ++i) {
        mSize = mSize * inputValidShape[i];
    }
    return {
        mSize,
        (inputValidShape.back() + QUANT_MX_SCALE_GROUP_COLS - 1) / QUANT_MX_SCALE_GROUP_COLS,
        SymbolicScalar(QUANT_MX_SCALE_PAIR_SIZE)};
}
...
void TiledQuantMXOperation(
    ...
    DequantScaleRoundingMode mode, int64_t axis)
{
    ...
    for (int64_t i = 0; i < input.tensor.GetShape()[cur]; i += step) {
        ...
        TiledQuantMXOperation(function, tileShape, cur + 1, input, dst, exp, maxScratch, scalingScratch, mode, axis);
    }
}
...
std::tuple<Tensor, Tensor> QuantMX(const Tensor& input, DataType quantDtype, DequantScaleRoundingMode mode, int64_t axis)
{
    DECLARE_TRACER();
