    ...
    const std::vector<LogicalTensorPtr>& oOperand, [[maybe_unused]] const Operation& op)
{
    ...
    ASSERT(VectorErrorCode::ERR_PARAM_INVALID, performanceMode == 0 ||
            performanceGroupedOffset == expectedPerformanceGroupedSize)
    ...
    const Tensor& input, DataType quantDtype, DequantScaleRoundingMode mode, int64_t axis, bool performanceMode)
{
    ...
    const std::vector<int64_t> groupedShape =
        performanceMode ? std::vector<int64_t>{BuildQuantMXPerformanceGroupedSize(inputShape)}
