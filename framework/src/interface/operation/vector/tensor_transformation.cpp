    ...
    if (isA5Architecture) {
        ...
        if (a5SupportedConversions.count(srcType) == 0 ||
            ...
            ASSERT(VectorErrorCode::ERR_PARAM_DTYPE_UNSUPPORTED, false)
                << "A5 architecture does not support cast from " << StringUtils::ToString(srcType) 
                << " to " << StringUtils::ToString(dstType) << " in " << opName;
        }
    } else {
        ...
        if (a2a3SupportedConversions.count(srcType) == 0 ||
            ...
            ASSERT(VectorErrorCode::ERR_PARAM_DTYPE_UNSUPPORTED, false)
                << "A2A3 architecture does not support cast from " << StringUtils::ToString(srcType) 
                << " to " << StringUtils::ToString(dstType) << " in " << opName;
        }
    }
}
