...
Tensor TensorTransData(Function& function, const Tensor& self, TransDataType transDataType)
{
    ...
    if (transDataType == TransDataType::NCHW2NC1HWC0) {
        ...
        if (padC != 0) {
            ...
            tmpShape1[-2] *= tmpShape1[-1];
            tmpShape1.pop_back();
            std::vector<SymbolicScalar> tmpValidShape1(self.GetValidShape());
            tmpValidShape1[-2] = tmpValidShape1[-2] * tmpValidShape1[-1];
            tmpValidShape1.pop_back();
            ...
            tmpShape2[1] = C1 * C0;
            std::vector<SymbolicScalar> tmpValidShape2(self.GetValidShape());
            tmpValidShape2[1] = SymbolicScalar(C1 * C0);
