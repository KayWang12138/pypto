    ...
    TransDataTileInfoPara& transDataTileInfoPara)
{
    ...
    auto tmpVecTile = tileShape.GetVecTile();
    tmpVecTile[1] = std::ceil(tmpVecTile[1] / C0);
    tmpVecTile.push_back(C0);
    ...
    if (cur == dstTensor->GetShape().size()) {
        ...
        auto tmpDstTile =
            std::make_shared<LogicalTensor>(function, self.GetStorage()->Datatype(), tmpVecTile, tmpVecTile);

        auto& op = function.AddOperation(Opcode::OP_NCHW2NC1HWC0, {inputTile}, {dstTensor, tmpDstTile});
        for (int i = 0; i < SHAPE_DIM4; i++) {
            tileParams[i] = SymbolicScalar(transDataTileInfoPara.inputTileInfo.offset[i]);
        }
        op.SetAttribute(OpAttributeKey::loopAxes, tileParams);
        return;
    }
...
TensorTransData(const Tensor& self, TransDataType transDataType)
{
    Function function = *Program::GetInstance().GetCurrentFunction();
    if (transDataType == TransDataType::NCHW2NC1HWC0) {
        Shape resultShape = self.GetShape();
        int64_t C0 = BLOCK_SIZE / BytesOf(self.GetDataType());
        int64_t padC = CeilAlign(resultShape[1], C0) - resultShape[1];
        int64_t C1 = static_cast<int64_t>(std::ceil(resultShape[1] / C0));
        resultShape[1] = C1;
        resultShape.push_back(C0);

        Tensor tmpInput;
        if (padC != 0) {
            tmpInput1 = Transpose(self, {1, 2});
            tmpInput2 = Pad(tmpInput1, {0, padC}, "constant");
            tmpInput = Transpose(tmpInput2, {1, 2});
        }

        auto result =
            std::make_shared<LogicalTensor>(function, self.GetStorage()->Datatype(), resultShape, resultShape);
        auto& op = function.AddOperation(Opcode::OP_NCHW2NC1HWC0, {tmpInput.GetStorage()}, {result});
...
Tensor TransData(const Tensor& self, TransDataType transDataType)
{
    ...
    RETURN_CALL(TransData, self, transDataType);
}
    ...
    const std::vector<LogicalTensorPtr>& oOperand, const Operation& op)
{
    if (op.HasAttr(OpAttributeKey::loopAxes)) {
        auto tileParams = op.GetSymbolicScalarAttribute(OpAttributeKey::loopAxes);
    }
