...
void CheckExpandTensorVaild(const LogicalTensorPtr &operand, const LogicalTensorPtr &result) {
    if (operand->shape.size() != result->shape.size()) {
        ASSERT(ERROR_CODE_UNDEFINED, false && "Dims not match");
    }

    for (size_t i = 0; i < result->shape.size(); ++i) {
        if (operand->shape[i] != result->shape[i] && operand->shape[i] != 1) {
            ASSERT(ERROR_CODE_UNDEFINED, 0 && "shape not match");
        }
    }
    ...
    ASSERT(ERROR_CODE_UNDEFINED, numExpandAxis <= 1) << "Only allow to expand one axis";
}
...
void Expand(Function &function, const TileShape &tileShape, const LogicalTensorPtr &operand,
    ...
    ASSERT(ERROR_CODE_UNDEFINED, function.GetGraphType() == GraphType::TILE_GRAPH) << "The GetGraphType of function is incorrect";
...
Tensor Expand(const Tensor &self, const std::vector<int64_t> &dstShape, std::vector<SymbolicScalar> validShape) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, self.GetShape().size() == dstShape.size()) << "The shape size of self and dst should be equal";
    int numExpandAxis = 0;
...
#define CASE(X) \
    ...
    switch (T) {
        ...
        default: ASSERT(ERROR_CODE_UNDEFINED, false && "unknown transpose op type");
    }
#undef CASE
}
...
void TensorInnerTranspose(
    ...
    ASSERT(ERROR_CODE_UNDEFINED, self->shape.size() == 3 || self->shape.size() == 4) // input should be 3 or 4 dims
...
Tensor Transpose(const Tensor &self, std::vector<int> perm) {
    DECLARE_TRACER();
    ASSERT(ERROR_CODE_UNDEFINED, perm.size() == 2) << "Transpose dim num should be 2."; // perm should be 2 dims
    ...
    ASSERT(ERROR_CODE_UNDEFINED, perm[0] < shapeSize && perm[0] >= 0) << "Transpose dim 0 is invalid.";
    ASSERT(ERROR_CODE_UNDEFINED, perm[1] < shapeSize && perm[1] >= 0) << "Transpose dim 1 is invalid.";
    ...
    auto oldVecTileShapes = TileShape::Current().GetVecTile();
    ASSERT(ERROR_CODE_UNDEFINED, (int)oldVecTileShapes.size() == shapeSize) << "TileShape dim num should same to input.";
    ...
    ASSERT(ERROR_CODE_UNDEFINED, (int)oldValidShapes.size() == shapeSize) << "ValidShape dim num should same to input.";
...
void TiledCastOperation(Function &function, const TileShape &tileShape, const LogicalTensorPtr &operand,
    const LogicalTensorPtr &result, const CastMode &mode) {
    ASSERT(ERROR_CODE_UNDEFINED, operand->shape.size() == operand->offset.size()) << "The shape size of operand and offset should be equal";
...
Tensor Cast(const Tensor &self, DataType dstDataType, CastMode mode) {
    DECLARE_TRACER();
    ASSERT(ERROR_CODE_UNDEFINED, self.GetShape().size() == self.GetStorage()->offset.size()) << "The shape size of self and offset should be equal";
...
void CheckCat(const std::vector<Tensor> &tensors, int axis) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, SHAPE_DIM2 <= shapeSize && shapeSize <= SHAPE_DIM4) << "The support dimension must be 2 to 4 dimensions";
    ...
    for (auto tensor : tensors) {
        ASSERT(ERROR_CODE_UNDEFINED, tensor.GetShape().size() == shapeSize) << "The shape size of all tensors should be equal";
        ASSERT(ERROR_CODE_UNDEFINED, tensor.Format() == format) << "The format of all tensors should be equal";
        ASSERT(ERROR_CODE_UNDEFINED, tensor.GetStorage() != nullptr) << "Each input must not be a null pointer";
        ASSERT(ERROR_CODE_UNDEFINED, tensor.GetDataType() == dataType) << "The dataType of all tensors should be equal";
    }
    ...
    for (auto tensor : tensors) {
        for (int i = 0; static_cast<size_t>(i) < shapeSize; ++i) {
            ...
            ASSERT(ERROR_CODE_UNDEFINED, shape[i] == tensor.GetShape()[i]) << "The shape of all tensors should be equal except at axis";
        }
    }
}
