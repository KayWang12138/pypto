...
enum class Opcode {
    ...
    // Distributed
    OP_MOE_DISPATCH_SEND_ROUTED,
    OP_MOE_DISPATCH_SEND_SHARED,
    OP_MOE_DISPATCH_LOCAL_COPY_OUT,
    OP_MOE_DISPATCH_SET_FLAG,
    OP_MOE_DISPATCH_WAIT_FLAG,
    OP_MOE_DISPATCH_ASSEMBLE_EXPANDX,
    OP_MOE_DISPATCH_ASSEMBLE_COMBINEINFO,
    OP_MOE_DISPATCH_EXPERT_TOKEN_NUM,
    OP_SHMEM_SET,
    OP_SHMEM_PUT,
    OP_SHMEM_PUT_UB2GM,
    OP_SHMEM_SIGNAL,
...
public:
    ...
    inline bool IsCopyOut(Opcode opCode) const {
        return opCode == Opcode::OP_COPY_OUT || opCode == Opcode::OP_UB_COPY_OUT || opCode == Opcode::OP_L0C_COPY_OUT ||
               ...
               opCode == Opcode::OP_MOE_DISPATCH_WAIT_FLAG || opCode == Opcode::OP_MOE_DISPATCH_ASSEMBLE_EXPANDX ||
               opCode == Opcode::OP_MOE_DISPATCH_ASSEMBLE_COMBINEINFO || opCode == Opcode::OP_MOE_DISPATCH_EXPERT_TOKEN_NUM ||
               opCode == Opcode::OP_MOE_DISPATCH_LOCAL_COPY_OUT || opCode == Opcode::OP_SHMEM_PUT ||
    ...
    inline bool IsSharedMemory(Opcode opCode) const {
        return opCode == Opcode::OP_SHMEM_WAIT_UNTIL || opCode == Opcode::OP_SHMEM_PUT ||
            ...
            opCode == Opcode::OP_MOE_DISTRIBUTED_COMBINE_SEND ||
            opCode == Opcode::OP_MOE_DISTRIBUTED_COMBINE_RECEIVE ||
            opCode == Opcode::OP_MOE_DISPATCH_ASSEMBLE_EXPANDX || opCode == Opcode::OP_MOE_DISPATCH_SEND_ROUTED ||
            opCode == Opcode::OP_MOE_DISPATCH_LOCAL_COPY_OUT || opCode == Opcode::OP_MOE_DISPATCH_SET_FLAG ||
            opCode == Opcode::OP_MOE_DISPATCH_WAIT_FLAG || opCode == Opcode::OP_MOE_DISPATCH_ASSEMBLE_COMBINEINFO ||
            opCode == Opcode::OP_MOE_DISPATCH_EXPERT_TOKEN_NUM;
    }
...
const std::unordered_set<Opcode> DISTRIBUTED_OPS{Opcode::OP_MOE_DISPATCH_SEND_ROUTED,
    Opcode::OP_MOE_DISPATCH_SEND_SHARED, Opcode::OP_MOE_DISPATCH_LOCAL_COPY_OUT, Opcode::OP_MOE_DISPATCH_SET_FLAG,
    Opcode::OP_MOE_DISPATCH_WAIT_FLAG, Opcode::OP_MOE_DISPATCH_ASSEMBLE_EXPANDX, Opcode::OP_MOE_DISPATCH_ASSEMBLE_COMBINEINFO, Opcode::OP_MOE_DISPATCH_EXPERT_TOKEN_NUM,
...
inline bool IsCopyOut(const Opcode &op) {
    return (op == Opcode::OP_COPY_OUT || op == Opcode::OP_L0C_COPY_OUT || op == Opcode::OP_TRANSPOSE_MOVEOUT ||
            op == Opcode::OP_INDEX_OUTCAST || op == Opcode::OP_MOE_DISPATCH_WAIT_FLAG || op == Opcode::OP_MOE_DISPATCH_ASSEMBLE_EXPANDX ||
            op == Opcode::OP_MOE_DISPATCH_ASSEMBLE_COMBINEINFO || op == Opcode::OP_MOE_DISPATCH_EXPERT_TOKEN_NUM || op == Opcode::OP_MOE_DISPATCH_LOCAL_COPY_OUT ||
