...
static inline void matmul_safe_snprintf(char* buf, size_t bufSize, const char* fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    vsnprintf_s(buf, bufSize, bufSize - 1, fmt, ap);
    va_end(ap);
}
...
#define MATMUL_ASSERT(error_code, cond, fmt, ...) \
    do { \
        if (!(cond)) { \
            MATMUL_LOGE("[ERR-FC%d] " fmt, static_cast<int>(error_code), ##__VA_ARGS__); \
            char err_msg[1024] = {0}; \
            matmul_safe_snprintf(err_msg, sizeof(err_msg), fmt, ##__VA_ARGS__); \
            ASSERT(false) << "[ERR-FC" << static_cast<int>(error_code) << "] " << err_msg; \
        } \
    } while (0)
