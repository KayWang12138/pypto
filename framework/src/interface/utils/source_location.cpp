...
std::map<std::pair<std::string, uint64_t>, std::vector<uint64_t>> BuildDlMap(const std::unordered_set<uint64_t>& pcSet)
{
    Dl_info dlinfo;
...
std::vector<std::string> BuildAddr2LineArgs(
...
bool ParseLineInfo(const std::string& lineStr, std::string& fname, int& lineno)
{
    if (lineStr.empty()) {
        return false;
    }
...
void SetFallbackLocation(std::string& fname, int& lineno, const std::string& elfName, uint64_t pc, uint64_t baseAddr)
{
    std::stringstream os;
...
void SourceLocation::Init() const
{
    ...
    for (auto& [info, pcs] : dlMap) {
        ...
        for (auto pc : pcs) {
            bool parsed = false;
            if (std::getline(iss, lineStr) && lineStr.find("inlined by") != std::string::npos &&
                !std::getline(iss, lineStr)) {
                lineStr.clear();
            }
            parsed = !lineStr.empty() && ParseLineInfo(lineStr, locMap[pc]->fname_, locMap[pc]->lineno_);
