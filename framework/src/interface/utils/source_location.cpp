...
void SetFallbackLocation(
    std::shared_ptr<SourceLocation>& loc, const std::string& elfName, uint64_t pc, uint64_t baseAddr)
{
    std::stringstream os;
    ...
    for (auto& [info, pcs] : dlMap) {
        ...
        for (auto pc : pcs) {
            ...
            if (std::getline(iss, lineStr)) {
                ...
                parsed = ParseLineInfo(lineStr, locMap[pc]->fname_, locMap[pc]->lineno_);
            }

            if (!parsed) {
                SetFallbackLocation(locMap[pc], info.first, pc, info.second);
            }
        }
    }
}
