...
void OperatorChecker::PostCheck()
{
    ...
    for (int i = preOpCount; i < postOpCount; i++) {
        ...
        ASSERT(FError::EINTERNAL, preOp <= lop.GetOpMagic() && lop.GetOpMagic() < postOp);
        for (auto& loperand : lop.GetOOperands()) {
            ASSERT(FError::EINTERNAL, preMagic <= loperand->GetMagic() && loperand->GetMagic() < postMagic);
            ASSERT(FError::EINTERNAL, preRawMagic <= loperand->tensor->GetRawMagic() && loperand->tensor->GetRawMagic() < postRawMagic);
            for (int j = i + 1; j < postOpCount; j++) {
                ...
                for (auto& roperand : rop.GetOOperands()) {
                    if (loperand->tensor->GetRawMagic() == roperand->tensor->GetRawMagic()) {
                        ASSERT(FError::EINTERNAL, !Overlap(loperand, roperand));
                    }
                }
            }
        }
