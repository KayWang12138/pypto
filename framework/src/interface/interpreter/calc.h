...
inline CalcTensorData Trans(LogicalTensorDataPtr data) {
    CalcTensorData calcData;
    if (data != nullptr) {
...
inline std::vector<CalcTensorData> TransVec(std::vector<LogicalTensorDataPtr> datas) {
    std::vector<CalcTensorData> result;
...
inline void MatMul(LogicalTensorDataPtr out, LogicalTensorDataPtr self, LogicalTensorDataPtr other, 
    ...
    GetCalcOps()->MatMul(Trans(out), Trans(self), Trans(other), nullptr, param);
}
...
inline void AccMatMul(LogicalTensorDataPtr out, LogicalTensorDataPtr self, LogicalTensorDataPtr other,
    ...
    if (acc == nullptr) {
        ...
        GetCalcOps()->MatMul(Trans(out), Trans(self), Trans(other), &accData, param);
    }
}
} // namespace npu::tile_fwk::calc
