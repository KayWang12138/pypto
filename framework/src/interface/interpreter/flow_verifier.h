...
public:
    ...
    static void CompareData(CompareResult &compareResult, size_t count, int64_t offset, const DataType *goldenValueList,
        ...
        for (size_t index = 0; index < count; index++) {
            auto goldenValue = (std::is_same_v<DataType, npu::tile_fwk::float16> || std::is_same_v<DataType, npu::tile_fwk::bfloat16>)
                                ? static_cast<float>(goldenValueList[index]) : static_cast<double>(goldenValueList[index]);       
            auto outputValue = (std::is_same_v<DataType, npu::tile_fwk::float16> || std::is_same_v<DataType, npu::tile_fwk::bfloat16>)
                                ? static_cast<float>(outputValueList[index]) : static_cast<double>(outputValueList[index]);
