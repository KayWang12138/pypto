...
private:
    ...
    static void CompareDataRecursiveWithLeaf(
        ...
        const std::shared_ptr<LogicalTensorData>& outputDataView, Leaf&& leaf, int64_t index)
    {
        auto& validShape = goldenDataView->GetValidShape();
        if (axis == validShape.size() - 1) {
            leaf(compareResult, validShape[axis], index, goldenOffset, goldenDataView, outputDataView);
        } else {
