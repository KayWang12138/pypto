...
void ExecuteOpAMulB(ExecuteOperationContext *ctx) {
    ...
    switch (ctx->op->GetOpcode()) {
        ...
        case Opcode::OP_A_MUL_BT: calc::MatMul(ret, lhs, rhs, param); break;
        case Opcode::OP_A_MULACC_BT: {
            ASSERT(ctx->ioperandDataViewList->size() == SIZE_THREE);
            auto acc = ctx->ioperandDataViewList->at(2);
            npu::tile_fwk::calc::AccMatMul<false, true>(ret, lhs, rhs, acc, kStep);
        } break;
        case Opcode::OP_AT_MUL_B: calc::MatMul(ret, lhs, rhs, param); break;
        case Opcode::OP_AT_MUL_BT: calc::MatMul(ret, lhs, rhs, param); break;
