...
FlowVerifier::CompareResult FlowVerifier::VerifyResult(const std::shared_ptr<LogicalTensorData> &goldenDataView,
    ...
    switch (goldenDataView->GetDataType()) {
        ...
        default: ASSERT(ExecuteOperationScene::INVALID_TENSOR_DTYPE, false); break;
    }
...
bool FlowVerifier::VerifyResult(const std::vector<std::shared_ptr<LogicalTensor>> &tensorDatalist,
    ...
    for (size_t k = 0; k < tensorDataViewList.size(); k++) {
        if (!goldenDataViewList[k]) {
            VERIFY_EVENT("%s Verify for %zu data view list index %zu result NO_COMPARE", key.c_str(),
                goldenDataViewList.size(), k);
            fprintf(functionInterpreter_->execDumpErrorFile,
        ...
        if (!tensorGraphResult.Check()) {
            VERIFY_LOGE_FULL_E(VerifyResultScene::VERIFY_RESULT_MISMATCH,
                "%s Verify for %zu data view list index %zu result FAILED", key.c_str(), goldenDataViewList.size(), k);
            fprintf(functionInterpreter_->execDumpErrorFile,
                "%s Verify for %zu data view list index %zu result FAILED\n", key.c_str(), goldenDataViewList.size(),
                k);
...
void FlowVerifier::VerifyPass(Function *func, int passIndex, const std::string &passIdentifier) {
    ...
    for (size_t captureIndex = 0; captureIndex < captureList.size(); captureIndex++) {
        ...
        auto res = VerifyResult(func->GetOutcast(), capture->func->GetOutcast(), key, tensorName, goldenDataViewList,
