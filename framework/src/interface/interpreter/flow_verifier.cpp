...
FlowVerifier::CompareResult FlowVerifier::VerifyResult(
    ...
    ASSERT(ERROR_CODE_UNDEFINED, goldenDataView->GetValidShape() == outputDataView->GetValidShape());
    ASSERT(ERROR_CODE_UNDEFINED, goldenDataView->GetDataType() == outputDataView->GetDataType());
...
void FlowVerifier::VerifyTensorGraph(Function *entry,
    ...
    ASSERT(ERROR_CODE_UNDEFINED, calc::IsVerifyEnabled()) << "Verify not supported";
    ...
    ASSERT(ERROR_CODE_UNDEFINED, inputSlotList.size() == attr->startArgsInputTensorList.size());
    ASSERT(ERROR_CODE_UNDEFINED, inputDataViewList.size() == inputSlotList.size());
    for (size_t i = 0; i < inputDataViewList.size(); i++) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, inputTensor->Datatype() == input->GetDataType());
    ...
    ASSERT(ERROR_CODE_UNDEFINED, outputDataViewList.size() == outputSlotList.size());
