...
FlowVerifier::CompareResult FlowVerifier::VerifyResult(
    ...
    switch (goldenDataView->GetDataType()) {
        case DT_INT8: return CompareData<int8_t>(goldenDataView, outputDataView, rtol, atol);
        case DT_INT16: return CompareData<int16_t>(goldenDataView, outputDataView, rtol, atol);
        case DT_INT32: return CompareData<int32_t>(goldenDataView, outputDataView, rtol, atol);
        case DT_INT64: return CompareData<int64_t>(goldenDataView, outputDataView, rtol, atol);
        case DT_FP16: return CompareData<npu::tile_fwk::float16>(goldenDataView, outputDataView, rtol, atol);
        case DT_FP32: return CompareData<float>(goldenDataView, outputDataView, rtol, atol);
        case DT_BF16: return CompareData<npu::tile_fwk::bfloat16>(goldenDataView, outputDataView, rtol, atol);
        case DT_UINT8: return CompareData<uint8_t>(goldenDataView, outputDataView, rtol, atol);
        case DT_UINT16: return CompareData<uint16_t>(goldenDataView, outputDataView, rtol, atol);
        case DT_UINT32: return CompareData<uint32_t>(goldenDataView, outputDataView, rtol, atol);
        case DT_UINT64: return CompareData<uint64_t>(goldenDataView, outputDataView, rtol, atol);
        case DT_DOUBLE: return CompareData<double>(goldenDataView, outputDataView, rtol, atol);
        case DT_BOOL: return CompareData<uint8_t>(goldenDataView, outputDataView, rtol, atol);
...
void FlowVerifier::VerifyTensorGraph(Function *entry,
    ...
    if (outputDataViewList.size() == 0){
        res = VerifyResult("tensor_graph", "tensor_graph", goldenDataViewList_, inputDataViewList_, static_cast<float>(1e-2), static_cast<float>(1e-2));
    } else {
        res = VerifyResult("tensor_graph", "tensor_graph", goldenDataViewList_, outputDataViewList_, static_cast<float>(1e-2), static_cast<float>(1e-2));
    }
    if (!res) {
        checkResult = false;
    }
...
void FlowVerifier::VerifyPass(Function *func, int passIndex, const std::string &passIdentifier) {
    ...
    float atol = 0;
    if (tolerance.size() < 2) {
        rtol = static_cast<float>(1e-3);
        atol = static_cast<float>(1e-3);
    } else {
        rtol = static_cast<float>(tolerance[0]);
        atol = static_cast<float>(tolerance[1]);
    }
