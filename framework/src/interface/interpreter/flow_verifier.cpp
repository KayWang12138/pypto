...
void FlowVerifier::VerifyPass(Function *func, int passIndex, const std::string &passIdentifier) {
    ...
    for (size_t captureIndex = 0; captureIndex < captureList.size(); captureIndex++) {
        const std::string key =  functionInterpreter_->execDumpFunPath + "_" + functionInterpreter_->execDumpPassFilter;
        ...
        try {
            captureExecution = functionInterpreter_->RunForPass(functionInterpreter_->execDumpPassFilter, func, capture);
