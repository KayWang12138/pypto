    ...
    const std::vector<std::shared_ptr<LogicalTensorData>>& tensorDataViewList, float rtol, float atol)
{
    ...
    for (size_t k = 0; k < tensorDataViewList.size(); k++) {
        ...
        ProgrameInfo[toIndex(ProgrameInfoCsvHeader::rtolAndAtol)] = functionInterpreter_->ToStrWithPrecesion(rtol)
                                                            + "/" + functionInterpreter_->ToStrWithPrecesion(atol);
        ...
        ProgrameInfo[toIndex(ProgrameInfoCsvHeader::mreTop1Permil)] =
                                                    functionInterpreter_->ToStrWithPrecesion(res.mreTop1Permil);
        ...
        ProgrameInfo[toIndex(ProgrameInfoCsvHeader::maeTop1Permil)] =
                                                    functionInterpreter_->ToStrWithPrecesion(res.maeTop1Permil);
