...
void ExecuteOpAssemble(ExecuteOperationContext *ctx) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, ctx->ioperandDataViewList->size() <= NUM_VALUE_2);
    ASSERT(ERROR_CODE_UNDEFINED, ctx->op != nullptr);
...
void ExecuteOpViewType(ExecuteOperationContext *ctx) {
    ASSERT(ERROR_CODE_UNDEFINED, ctx != nullptr && ctx->op != nullptr);
    ASSERT(ERROR_CODE_UNDEFINED, ctx->ooperandInplaceDataViewList->size() == 1);
    ASSERT(ERROR_CODE_UNDEFINED, ctx->ioperandDataViewList->size() == 1);

    auto &oop = ctx->ooperandInplaceDataViewList->at(0);
    auto &iop = ctx->ioperandDataViewList->at(0);
    ASSERT(ERROR_CODE_UNDEFINED, oop != nullptr && iop != nullptr);

    auto inData = iop->GetData();
    auto outData = oop->GetData();
    ASSERT(ERROR_CODE_UNDEFINED, inData != nullptr && outData != nullptr);
    ...
    ASSERT(ERROR_CODE_UNDEFINED, srcBytes == dstBytes);
...
void ExecuteOpCopyOut(ExecuteOperationContext *ctx) {
    ...
    auto &oop = ctx->ooperandInplaceDataViewList->at(0);
    auto iop = ctx->ioperandDataViewList->at(0);
    ASSERT(ERROR_CODE_UNDEFINED, iop != nullptr && oop != nullptr);
    ...
    ASSERT(ERROR_CODE_UNDEFINED, oopValid != nullptr);
