...
static void Range(const TensorData &out, const Element &start, const Element &end, const Element &step) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, tmp.numel() == expected_numel) << "Range numel mismatch: generated " << tmp.numel() << ", expected " << expected_numel;
    auto tout = From(out);
...
static void CompareImpl(const TensorData &out, const torch::Tensor& tself, const T& other_op,
    ...
    switch (operation) {
        ...
        case CmpOperationType::GE:
            ...
            break;
        default:
            ASSERT(ERROR_CODE_UNKNOWN, false) << "Unsupported compare type";
            break;
    }
    ...
    if (mode == CmpModeType::BIT) {
        if (tmp_result.dim() > 0) {
            ...
            ASSERT(ERROR_CODE_UNKNOWN, last_dim % NUM_VALUE_8 == 0) << "Last dimension must be divisible by 8 in BIT mode";
...
static void FormatND2NZ(const TensorData &out, const TensorData &self) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, shape.size() >= 0x2) << "Input tensor must have at least 2 dimensions";
...
static void QuantPreCompute(const TensorData &out, const TensorData &self, const TensorData *scalePtr, uint64_t scale, int relu) {
    ASSERT(ERROR_CODE_UNKNOWN, out.dataPtr != nullptr && self.dataPtr != nullptr);
    ASSERT(ERROR_CODE_UNKNOWN, out.dtype == DataType::DT_FP16 && self.dtype == DataType::DT_INT32);
    auto tself = From(self);
    auto tout = From(out);
...
void GatherMask(const TensorData &out, const TensorData &self, int patternMode) {
    ...
    if (patternMode == 7){
        ...
        switch(patternMode) {
            ...
            break;
            default:
            ASSERT(ERROR_CODE_UNKNOWN, patternMode >= 1 && patternMode <= 7) << "Invalid patternMode";
        }
...
static void Topk(const TensorData &out, const TensorData &self, int64_t axis, int64_t k, bool descending) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, axis >= 0 && axis < tselfHalf.dim()) <<
...
bool ScatterDateCopy(const std::vector<int64_t> &loopIdx, torch::Tensor &src, torch::Tensor &indices,
    ...
    ASSERT(ERROR_CODE_UNKNOWN, blockSize != 0);
    if (ret.dim() == 2) { // 2 dim
...
static void ScatterUpdate(const TensorData &out, const TensorData &self, const TensorData &index, const TensorData &dst, int axis,
   std::string cacheMode, int blockSize) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, indices.second.dim() == 2);                   // indices should be 2 dim
    ASSERT(ERROR_CODE_UNKNOWN, (src.second.dim() == 2) || (src.second.dim() == 4)); // only 2, 4 dim support
    ASSERT(ERROR_CODE_UNKNOWN, (ret.second.dim() == 2) || (ret.second.dim() == 4)); // only 2, 4 dim support
    ASSERT(ERROR_CODE_UNKNOWN, src.second.dim() == ret.second.dim());
