...
static void Range(const TensorData &out, const Element &start, const Element &end, const Element &step) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, tmp.numel() == expected_numel) << "Range numel mismatch: generated " << tmp.numel() << ", expected " << expected_numel;
    auto tout = From(out);
...
static void CompareImpl(const TensorData &out, const torch::Tensor& tself, const T& other_op,
    ...
    switch (operation) {
        ...
        case CmpOperationType::GE:
            ...
            break;
        default:
            ASSERT(ERROR_CODE_UNDEFINED, false) << "Unsupported compare type";
            break;
    }
    ...
    if (mode == CmpModeType::BIT) {
        if (tmp_result.dim() > 0) {
            ...
            ASSERT(ERROR_CODE_UNDEFINED, last_dim % NUM_VALUE_8 == 0) << "Last dimension must be divisible by 8 in BIT mode";
...
static void FormatND2NZ(const TensorData &out, const TensorData &self) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, shape.size() >= 0x2) << "Input tensor must have at least 2 dimensions";
...
static void QuantPreCompute(const TensorData &out, const TensorData &self, const TensorData *scalePtr, uint64_t scale, int relu) {
    ASSERT(ERROR_CODE_UNDEFINED, out.dataPtr != nullptr && self.dataPtr != nullptr);
    ASSERT(ERROR_CODE_UNDEFINED, out.dtype == DataType::DT_FP16 && self.dtype == DataType::DT_INT32);
    auto tself = From(self);
    auto tout = From(out);
...
void GatherMask(const TensorData &out, const TensorData &self, int patternMode) {
    ...
    if (patternMode == 7){
        ...
        switch(patternMode) {
            ...
            break;
            default:
            ASSERT(ERROR_CODE_UNDEFINED, patternMode >= 1 && patternMode <= 7) << "Invalid patternMode";
        }
...
static void Topk(const TensorData &out, const TensorData &self, int64_t axis, int64_t k, bool descending) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, axis >= 0 && axis < tselfHalf.dim()) <<
...
bool ScatterDateCopy(const std::vector<int64_t> &loopIdx, torch::Tensor &src, torch::Tensor &indices,
    ...
    ASSERT(ERROR_CODE_UNDEFINED, blockSize != 0);
    if (ret.dim() == 2) { // 2 dim
...
static void ScatterUpdate(const TensorData &out, const TensorData &self, const TensorData &index, const TensorData &dst, int axis,
   std::string cacheMode, int blockSize) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, indices.second.dim() == 2);                   // indices should be 2 dim
    ASSERT(ERROR_CODE_UNDEFINED, (src.second.dim() == 2) || (src.second.dim() == 4)); // only 2, 4 dim support
    ASSERT(ERROR_CODE_UNDEFINED, (ret.second.dim() == 2) || (ret.second.dim() == 4)); // only 2, 4 dim support
    ASSERT(ERROR_CODE_UNDEFINED, src.second.dim() == ret.second.dim());
