...
torch::Tensor &GatherINUBGolden(torch::Tensor &out, const torch::Tensor &params, const torch::Tensor &indices,
    ...
    const int64_t num_buffer_tokens = params.size(0);
    ...
    at::Tensor logical_block = logical.div(blockSize); // trunc div for int64
    ...
    TORCH_CHECK(
        physical.lt(num_buffer_tokens).all().item<bool>(), "physical_index out of range: must be < num_buffer_tokens");
...
void GatherINUB(LogicalTensorDataPtr out, LogicalTensorDataPtr params, LogicalTensorDataPtr indices,
    ...
    auto paramsRank = params->GetShape().size();
    auto indicesRank = indices->GetShape().size();
    auto paramsRank = params->GetShape().size();
    return GatherINUBGolden(tout, tparams, tindices, tpageTable, blockSize, axis);
}
