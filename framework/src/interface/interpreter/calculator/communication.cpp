...
void CommManager::InitControl(std::string name, int rank , int worldSize) {
    ...
    ctrlBase_ = (sem_t *) mmap(0, ctrlShmSize_, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    close(fd);
...
void CommManager::InitData(std::string name, size_t slotSize) {
    ...
    dataBase_ = (uint8_t *) mmap(0, dataShmSize_, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    close(fd);
}
...
CommManager::~CommManager() {
    if (ctrlBase_) {
        if (rank_ == 0) {
            for (int i = 0; i < worldSize_ * worldSize_; i++) {
