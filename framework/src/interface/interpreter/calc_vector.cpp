...
void ExecuteOpBinary(ExecuteOperationContext *ctx) {
    if (opcode == Opcode::OP_ADD_BRC || opcode == Opcode::OP_SUB_BRC || opcode == Opcode::OP_MUL_BRC ||
        ...
        ASSERT(ERROR_CODE_UNKNOWN, ctx->ooperandInplaceDataViewList->size() == SIZE_TWO);
    } else if (opcode == Opcode::OP_BITWISEXOR || opcode == Opcode::OP_COPYSIGN || opcode == Opcode::OP_POW) {
        ASSERT(ERROR_CODE_UNKNOWN, ctx->ooperandInplaceDataViewList->size() <= SIZE_TWO);
    } else {
        ASSERT(ERROR_CODE_UNKNOWN, ctx->ooperandInplaceDataViewList->size() == 1);
    }
    ASSERT(ERROR_CODE_UNKNOWN, ctx->ioperandDataViewList->size() == SIZE_TWO);
    ...
    switch (opcode) {
        ...
        default: ASSERT(ERROR_CODE_UNKNOWN, false);
    }
}
...
void ExecuteOpFmods(ExecuteOperationContext *ctx) {
    ASSERT(ERROR_CODE_UNKNOWN, ctx->ooperandInplaceDataViewList->size() <= SIZE_TWO);
    ASSERT(ERROR_CODE_UNKNOWN, ctx->ioperandDataViewList->size() == 1);
    auto &ret = ctx->ooperandInplaceDataViewList->at(0);
    auto &lhs = ctx->ioperandDataViewList->at(0);
...
void ExecuteOpVecDup(ExecuteOperationContext *ctx) {
    ASSERT(ERROR_CODE_UNKNOWN, ctx->ooperandInplaceDataViewList->size() == 1);
    ASSERT(ERROR_CODE_UNKNOWN, ctx->ioperandDataViewList->size() == 0);
    auto &ret = ctx->ooperandInplaceDataViewList->at(0);
    auto element = Element(DT_FP32, 0.0f);
    ctx->op->GetAttr(OpAttributeKey::scalar, element);
...
void ExecuteOpReduce(ExecuteOperationContext *ctx) {
    ...
    switch (opcode) {
        ...
        default: ASSERT(ERROR_CODE_UNKNOWN, false) << "opcode not support" << ctx->op->GetOpcodeStr();
    }
}
...
void ExecuteOpIndexOutcast(ExecuteOperationContext *ctx) {
    ASSERT(ERROR_CODE_UNKNOWN, ctx->ioperandDataViewList->size() == SIZE_THREE);
    auto oop = ctx->ooperandInplaceDataViewList->at(0);
...
void ExecuteOpIndexPut(ExecuteOperationContext *ctx) {
    ASSERT(ERROR_CODE_UNKNOWN, ctx->ooperandInplaceDataViewList->size() == 1);
    ASSERT(ERROR_CODE_UNKNOWN, ctx->ioperandDataViewList->size() <= SIZE_SIX);
...
void ExecuteOpTiledMrgSort(ExecuteOperationContext *ctx) {
    ASSERT(ERROR_CODE_UNKNOWN, ctx->ioperandDataViewList->size() == SIZE_FOUR);
    auto oop = ctx->ooperandInplaceDataViewList->at(0);
...
void ExecuteOpTopkSort(ExecuteOperationContext *ctx) {
    ASSERT(ERROR_CODE_UNKNOWN, ctx->ioperandDataViewList->size() == 1);
    ASSERT(ERROR_CODE_UNKNOWN, ctx->ooperandInplaceDataViewList->size() == 2);  // value + temp
