...
void ExecuteOpTransposeMoveOut(ExecuteOperationContext *ctx) {
    ...
    if (std::dynamic_pointer_cast<CopyOpAttribute>(ctx->op->GetOpAttribute())) {
        auto attr = std::dynamic_pointer_cast<CopyOpAttribute>(ctx->op->GetOpAttribute());
        std::vector<int64_t> shape = ctx->opInter->EvaluateOpImmediate(ctx->frame, attr->GetShape());
        if (ctx->op->GetOpcode() == Opcode::OP_TRANSPOSE_MOVEOUT) {
            std::vector<int64_t> toOffset = ctx->opInter->EvaluateOpImmediate(ctx->frame, attr->GetToOffset());
            auto oopCopy = oop->View(shape, toOffset);
            return calc::Transpose(oopCopy, iop, axises[0], axises[1]);
        } else {
            std::vector<int64_t> fromOffset = ctx->opInter->EvaluateOpImmediate(ctx->frame, attr->GetFromOffset());
