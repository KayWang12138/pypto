...
#undef CASE_DATA_TYPE_DIS
            ...
            default: ASSERT(ERROR_CODE_UNKNOWN, false); return Element();
        }
    }

    Element GetElement(int64_t *coords, size_t n) const {
        int64_t index = 0;
        ASSERT(ERROR_CODE_UNKNOWN, n == shape_.size());
    ...
    std::string DumpElement(int index) const {
        switch (GetDataType()) {
            ...
            default: ASSERT(ERROR_CODE_UNKNOWN, false); return "";
        }
    }
    ...
    static std::shared_ptr<RawTensorData> CreateConstantTensor(const Tensor &t, T value) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, sizeof(T) == tensorData->GetElementSize()) << "ConstantTensor's dtype and value's type don't match!";
    ...
    static std::shared_ptr<RawTensorData> CreateTensor(const Tensor &t, const std::vector<T> &values) {
        auto tensorData = std::make_shared<RawTensorData>(t.GetDataType(), t.GetShape());
        T *data = reinterpret_cast<T *>(tensorData->data());
        ASSERT(ERROR_CODE_UNKNOWN, sizeof(T) == tensorData->GetElementSize()) << "CreateTensor's dtype and value's type don't match!";
