...
public:
    ...
    void SetCompilerMonitorOptions(bool enable, int interval_sec);
    ...
    std::chrono::steady_clock::time_point GetLastPrintTime() const;
    void SetLastPrintTime(std::chrono::steady_clock::time_point t);
    int GetTotalFunctionCount() const;
    int GetCurrentFunctionIndex() const;
    std::map<std::string, double> GetStageElapsedTotals() const;
...
private:
    ...
    std::map<std::string, double> stage_elapsed_totals_;
