...
void MonitorManager::EndStage(const std::string& name) {
    ...
    // printf("=== Stage ==[%s]== end, sub stage cost %lfs.\n", name.c_str(), stage_elapsed_totals_[name]);

    double total_elapsed = std::chrono::duration<double>(now - total_start_).count();

    std::string stage_finish_msg = "[Compiler Monitor] Function: " + std::to_string(this->GetCurrentFunctionIndex()) +
        "/" + std::to_string(total_function_count_) + " | Stage: " + name + "(completed) | Stage elapsed: " +
        FormatElapsed(elapsed) + " | Total elapsed: " + FormatElapsed(total_elapsed) + " | Func:[" + current_function_ + "]";
    (void)fprintf(stdout, "%s\n", stage_finish_msg.c_str());
    (void)fflush(stdout);
    COMPILER_LOGI("%s", stage_finish_msg.c_str());

    // printf("[Compiler Monitor] Function: %d/%d | Stage: %s(completed) | Stage elapsed: %s | Total elapsed: %s | Func:[%s]\n", this->GetCurrentFunctionIndex(), total_function_count_, name.c_str(), FormatElapsed(elapsed).c_str(), FormatElapsed(total_elapsed).c_str(), current_function_.c_str());
}
