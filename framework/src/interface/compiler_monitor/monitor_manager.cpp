...
void MonitorManager::TryEndPrepareStage() {
    ...
    if (current_stage_ == "Prepare" && enable_) {
        ...
        std::string msg = "[Compiler Monitor] Stage: " + current_stage_ + " | Number of stashed function: " +
...
void MonitorManager::SetCompilerMonitorOptions(bool enable, int interval_sec, int timeout_sec, int total_timeout_sec) {
    std::lock_guard<std::mutex> lock(mutex_);
    enable_ = enable;
    interval_sec_ = (interval_sec > 0) ? interval_sec : 2;
    ...
    total_timeout_sec_ = (total_timeout_sec >= 0) ? total_timeout_sec : 60;
