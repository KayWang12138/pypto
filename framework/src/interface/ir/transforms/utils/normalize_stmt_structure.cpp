...
#include "ir/transforms/utils/normalize_stmt_structure.h"
...
StmtPtr NormalizeStmtStructureMutator::VisitStmt_(const IfStmtPtr& op) {
  ...
  auto new_then = NormalizeBody(op->thenBody_);

  // Normalize else branch if present
  std::optional<StmtPtr> new_else;
  bool else_changed = false;
  if (op->elseBody_.has_value()) {
    auto normalized_else = NormalizeBody(op->elseBody_.value());
    new_else = normalized_else;
    else_changed = (normalized_else.get() != op->elseBody_.value().get());
  }

  // Check if anything changed
  bool changed = (new_then.get() != op->thenBody_.get()) || else_changed;

  if (changed) {
    // Visit condition (shouldn't change for normalization, but call for consistency)
    auto new_condition = VisitExpr(op->condition_);
    return std::make_shared<IfStmt>(new_condition, new_then, new_else, op->returnVars_, op->span_);
  }
  return op;
}
...
StmtPtr NormalizeStmtStructureMutator::VisitStmt_(const ForStmtPtr& op) {
  ...
  if (new_body.get() != op->body_.get()) {
    ...
    return std::make_shared<ForStmt>(op->loopVar_, new_start, new_stop, new_step, op->iterArgs_, new_body,
                                     op->returnVars_, op->span_, op->kind_, op->chunkSize_,
...
StmtPtr NormalizeStmtStructureMutator::VisitStmt_(const WhileStmtPtr& op) {
  ...
  if (new_body.get() != op->body_.get()) {
    ...
    return std::make_shared<WhileStmt>(new_condition, op->iterArgs_, new_body, op->returnVars_, op->span_);
  }
  return op;
}
...
FunctionPtr NormalizeStmtStructure(const FunctionPtr& func) {
  ...
  return std::make_shared<Function>(func->name_, func->params_, func->paramDirections_, func->returnTypes_,
                                    normalized_body, func->span_, func->funcType_);
}
