...
#include "ir/transforms/utils/flatten_single_stmt.h"
...
StmtPtr FlattenSingleStmtMutator::VisitStmt_(const IfStmtPtr& op) {
  ...
  if (op->elseBody_.has_value()) {
    auto visited_else = VisitStmt(op->elseBody_.value());
    new_else = visited_else;
    else_changed = (visited_else.get() != op->elseBody_.value().get());
  }

  // Check if anything changed
  bool changed = (new_condition.get() != op->condition_.get()) || (new_then.get() != op->thenBody_.get()) ||
                 else_changed;
