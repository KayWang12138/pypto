    ...
    if (auto if_stmt = As<IfStmt>(stmt)) {
      ...
      if (then_ends_with_continue && !if_stmt->elseBody_.has_value()) {
        ...
        if (!pre_stmts.empty()) {
          ...
          if (lowered_remaining.empty()) {
            ...
            ExprPtr negated_cond = NegateCondition(if_stmt->condition_, if_stmt->span_);
            result.push_back(std::make_shared<IfStmt>(std::move(negated_cond), std::move(guard_body),
                                                      std::move(else_body), if_stmt->returnVars_,
                                                      if_stmt->span_));
          }
        }
      ...
      if (if_stmt->elseBody_.has_value()) {
        ...
        if (else_is_continue) {
          ...
          auto body_stmts = FlattenToStmtList(if_stmt->thenBody_);
          ...
          auto new_if = std::make_shared<IfStmt>(if_stmt->condition_, std::move(then_body),
                                                 std::move(else_body), if_stmt->returnVars_, if_stmt->span_);
...
ForStmtPtr LowerBreakInFor(const ForStmtPtr& op) {
  ...
  for (const auto& ia : op->iterArgs_) new_iter_args.push_back(ia);

  // new return_vars: [_can_continue_final, ...original]
  std::vector<VarPtr> new_return_vars = {brk_return_var};
  for (const auto& rv : op->returnVars_) new_return_vars.push_back(rv);
  ...
  for (const auto& ia : op->iterArgs_) skip_yield.push_back(ia);
  ...
  return std::make_shared<ForStmt>(op->loopVar_, op->start_, op->stop_, op->step_, std::move(new_iter_args),
                                   for_body, std::move(new_return_vars), span, op->kind_, op->chunkSize_,
                                   op->chunkPolicy_, op->loopOrigin_);
}
...
WhileStmtPtr LowerBreakInWhile(const WhileStmtPtr& op) {
  ...
  for (const auto& ia : op->iterArgs_) cond_false_yield.push_back(ia);
  ...
  StmtPtr VisitStmt_(const ForStmtPtr& op) override {
    ...
    if (!has_break && !has_continue) {
      ...
      return std::make_shared<ForStmt>(op->loopVar_, op->start_, op->stop_, op->step_, op->iterArgs_,
                                       std::move(new_body), op->returnVars_, op->span_, op->kind_,
                                       op->chunkSize_, op->chunkPolicy_, op->loopOrigin_);
    }

    // Create a working copy with recursed body (use ForStmtPtr = shared_ptr<const ForStmt>)
    ForStmtPtr working = std::make_shared<ForStmt>(op->loopVar_, op->start_, op->stop_, op->step_,
                                                   op->iterArgs_, std::move(new_body), op->returnVars_,
                                                   op->span_, op->kind_, op->chunkSize_, op->chunkPolicy_,
                                                   op->loopOrigin_);
    ...
    if (has_continue) {
      ...
      working = ForStmtPtr(std::make_shared<ForStmt>(
          ...
          std::move(lowered_body), working->returnVars_, working->span_, working->kind_,
          working->chunkSize_, working->chunkPolicy_, working->loopOrigin_));
    }
...
FunctionPtr LowerBreakContinueImpl(const FunctionPtr& func) {
  ...
  return std::make_shared<Function>(func->name_, func->params_, func->paramDirections_,
                                    func->returnTypes_, std::move(new_body), func->span_,
                                    func->funcType_);
}
