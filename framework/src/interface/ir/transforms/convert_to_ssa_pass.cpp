...
public:
  ...
  FunctionPtr Convert(const FunctionPtr& func) {
    ...
    for (size_t i = 0; i < func->params_.size(); ++i) {
      ...
      new_param_directions_.push_back(func->paramDirections_[i]);
    }
    ...
    return std::make_shared<Function>(func->name_, new_params_, new_param_directions_, func->returnTypes_,
                                      new_body, func->span_, func->funcType_);
  }
 ...
 protected:
  ...
  StmtPtr VisitStmt_(const IfStmtPtr& op) override {
    ...
    for (const auto& existing_rv : op->returnVars_) {
  ...
  StmtPtr VisitStmt_(const ScopeStmtPtr& op) override {
    ...
    auto new_body = VisitStmt(op->body_);
    ExitScope();
    return std::make_shared<ScopeStmt>(op->scopeKind_, new_body, op->span_);
  }
  ...
  StmtPtr VisitStmt_(const ForStmtPtr& op) override {
    ...
    auto new_loop_var = std::make_shared<Var>(loop_var_base + "_" + std::to_string(loop_var_version),
                                              loop_var_type, op->loopVar_->span_);
    ...
    return std::make_shared<ForStmt>(new_loop_var, new_start, new_stop, new_step, new_iter_args, final_body,
                                     return_vars, op->span_, op->kind_, op->chunkSize_, op->chunkPolicy_,
                                     op->loopOrigin_);
  }
