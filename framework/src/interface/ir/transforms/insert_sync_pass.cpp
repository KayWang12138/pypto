...
template <typename T>
void AppendVector(std::vector<T>& dst, const std::vector<T>& src) {
  if (src.empty()) return;
...
std::vector<PathElement> PathPrefix(const std::vector<PathElement>& path, size_t end) {
  std::vector<PathElement> prefix;
...
class SyncInserter {
 public:
  ...
  FunctionPtr Run(const FunctionPtr& func) {
    ...
    return std::make_shared<Function>(func->name_, func->params_, func->paramDirections_,
                                      func->returnTypes_, new_body, func->span_, func->funcType_);
  }
 ...
 private:
  ...
  void CollectFromIfStmt(const IfStmtPtr& if_stmt, MemRefSummary& state) {
    ...
    ctx_.EnterIfThen();
    CollectSyncPairsImpl(if_stmt->thenBody_, state);
    ctx_.Leave();
    ...
    if (if_stmt->elseBody_) {
      ctx_.EnterIfElse();
      state = state_before;
      CollectSyncPairsImpl(*if_stmt->elseBody_, state);
      ctx_.Leave();
  ...
  void CollectFromForStmt(const ForStmtPtr& for_stmt, MemRefSummary& state) {
    ...
    AppendVector(unrolled_stmts, seq->stmts_);
    AppendVector(unrolled_stmts, seq->stmts_);
  ...
  [[nodiscard]] Position EndOfOpScope(const Position& pos) const {
    if (pos.path.size() >= 2 && pos.path.back().kind == PathElement::Kind::OpIndex) {
      std::vector<PathElement> key = PathPrefix(pos.path, pos.path.size() - 1);
  ...
  [[nodiscard]] static Position BeginOfOpScope(const Position& pos) {
    if (pos.path.size() >= 2 && pos.path.back().kind == PathElement::Kind::OpIndex) {
      Position result;
      result.path = PathPrefix(pos.path, pos.path.size() - 1);
  ...
  static std::vector<PathElement> GetParentPath(const Position& pos) {
    ...
    parent = PathPrefix(pos.path, end);
    return parent;
  }
    ...
    auto flush_pending = [&]() {
      ...
      if (!result.empty()) {
        if (auto prev_ops = As<OpStmts>(result.back())) {
          ...
          merged.reserve(prev_ops->stmts_.size() + pending_ops.size());
          AppendVector(merged, prev_ops->stmts_);
          AppendVector(merged, pending_ops);
  ...
  StmtPtr ApplyToIfStmt(const IfStmtPtr& if_stmt, std::vector<PathElement>& path) {
    ...
    auto new_then = ApplyInsertions(if_stmt->thenBody_, path);
    path.pop_back();

    std::optional<StmtPtr> new_else = std::nullopt;
    if (if_stmt->elseBody_) {
      path.push_back({PathElement::Kind::IfElse, -1});
      new_else = ApplyInsertions(*if_stmt->elseBody_, path);
      path.pop_back();
    }
    return std::make_shared<const IfStmt>(if_stmt->condition_, new_then, new_else, if_stmt->returnVars_,
                                          if_stmt->span_);
  }
  ...
  StmtPtr ApplyToForStmt(const ForStmtPtr& for_stmt, std::vector<PathElement>& path) {
    ...
    return std::make_shared<const ForStmt>(
        for_stmt->loopVar_, for_stmt->start_, for_stmt->stop_, for_stmt->step_, for_stmt->iterArgs_,
        new_body, for_stmt->returnVars_, for_stmt->span_, for_stmt->kind_, for_stmt->chunkSize_,
        for_stmt->chunkPolicy_, for_stmt->loopOrigin_);
  }
};
