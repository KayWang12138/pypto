...
std::string BlockDslDataTypeToString(const DataType& dtype) {
...
Precedence GetPythonPrintPrecedence(const ExprPtr& expr) {
...
bool IsPythonPrintRightAssociative(const ExprPtr& expr) {
...
std::string IRPythonPrinter::Print(const TypePtr& type) {
  if (auto scalar_type = As<ScalarType>(type)) {
    ...
    return prefix_ + ".Scalar[" + prefix_ + "." + BlockDslDataTypeToString(scalar_type->dtype_) + "]";
  }

  if (auto ptr_type = As<PtrType>(type)) {
    // Print as pl.Ptr[pl.FP32]
    return prefix_ + ".Ptr[" + prefix_ + "." + BlockDslDataTypeToString(ptr_type->dtype_) + "]";
  }
  ...
  if (auto tensor_type = As<TensorType>(type)) {
    ...
    oss << "], " << prefix_ << "." << BlockDslDataTypeToString(tensor_type->dtype_);
  ...
  if (auto tile_type = As<TileType>(type)) {
    ...
    oss << "], " << prefix_ << "." << BlockDslDataTypeToString(tile_type->dtype_);
...
void IRPythonPrinter::VisitExpr_(const ConstIntPtr& op) {
  ...
  if (op->dtype() == DataType::INT64 || op->dtype() == DataType::INDEX) {
    ...
    stream_ << prefix_ << ".const(" << op->value_ << ", " << prefix_ << "." << BlockDslDataTypeToString(op->dtype())
            << ")";
  }
}
...
void IRPythonPrinter::VisitExpr_(const ConstFloatPtr& op) {
  if (op->dtype() != DataType::FP32) {
    stream_ << prefix_ << ".const(" << FormatFloatLiteral(op->value_) << ", " << prefix_ << "."
            << BlockDslDataTypeToString(op->dtype()) << ")";
  } else {
...
void IRPythonPrinter::VisitExpr_(const CallPtr& op) {
  ...
  for (const auto& [key, value] : op->kwargs_) {
    ...
    if (value.type() == typeid(int)) {
      ...
      stream_ << prefix_ << "." << BlockDslDataTypeToString(AnyCast<DataType>(value, "printing kwarg: " + key));
...
bool IRPythonPrinter::NeedsParens(const ExprPtr& parent, const ExprPtr& child, bool is_left) {
  Precedence parent_prec = GetPythonPrintPrecedence(parent);
  Precedence child_prec = GetPythonPrintPrecedence(child);
  ...
  if (child_prec == parent_prec) {
    if (IsPythonPrintRightAssociative(parent)) {
      return is_left;
    } else {
      return !is_left;
    }
...
void IRPythonPrinter::VisitExpr_(const NegPtr& op) {
  stream_ << "-";
  Precedence operand_prec = GetPythonPrintPrecedence(op->operand_);
...
void IRPythonPrinter::VisitExpr_(const CastPtr& op) {
  ...
  VisitExpr(op->operand_);
  stream_ << ", " << prefix_ << "." << BlockDslDataTypeToString(scalar_type->dtype_) << ")";
}
...
void IRPythonPrinter::VisitStmt_(const ForStmtPtr& op) {
  ...
  if (op->kind_ == ForKind::Unroll && !op->iterArgs_.empty()) {
  ...
  if (!op->iterArgs_.empty()) {
    ...
    if (op->iterArgs_.size() == 1) stream_ << ",";
    stream_ << ")";
  }

  // Add chunk kwargs
  if (op->chunkSize_.has_value()) {
    stream_ << ", chunk=";
    VisitExpr(*op->chunkSize_);
    if (op->chunkPolicy_ != ChunkPolicy::LeadingFull) {
      stream_ << ", chunk_policy=\"" << ChunkPolicyToString(op->chunkPolicy_) << "\"";
    }
  }
...
void IRPythonPrinter::VisitStmt_(const ScopeStmtPtr& op) {
  ...
  auto it = scope_kind_to_dsl.find(op->scopeKind_);
  INTERNAL_CHECK(it != scope_kind_to_dsl.end())
      << "Internal error: Unknown ScopeKind in python_printer: " << ScopeKindToString(op->scopeKind_);
...
void IRPythonPrinter::VisitStmtBody(const StmtPtr& body, const std::vector<VarPtr>& return_vars) {
  ...
  if (auto yield_stmt = As<YieldStmt>(body)) {
    ...
    for (size_t i = 0; i < seq_stmts->stmts_.size(); ++i) {
      ...
      if (auto inner_yield = As<YieldStmt>(stmt)) {
        if (is_last && !inner_yield->value_.empty() && !return_vars.empty()) {
          // Wrap as assignment
          stream_ << GetIndent();
          PrintYieldAssignmentVars(return_vars);
          stream_ << " = " << prefix_ << ".yield_(";
          for (size_t j = 0; j < inner_yield->value_.size(); ++j) {
            if (j > 0) stream_ << ", ";
            VisitExpr(inner_yield->value_[j]);
          }
          stream_ << ")";
        } else {
          stream_ << GetIndent();
...
void IRPythonPrinter::VisitFunction(const FunctionPtr& func) {
  ...
  for (size_t i = 0; i < func->params_.size(); ++i) {
    ...
    const auto& dir = func->paramDirections_[i];
    stream_ << var->name_ << ": ";
...
std::string IRPythonPrinter::PrintTileView(const TileView& tile_view) {
  ...
  switch (tile_view.blayout) {
    ...
    case TileLayout::col_major:
      ...
      break;
    default:
      INTERNAL_CHECK(false) << "Unknown TileLayout in PrintTileView blayout";
      break;
  }
  ...
  switch (tile_view.slayout) {
    ...
    case TileLayout::col_major:
      ...
      break;
    default:
      INTERNAL_CHECK(false) << "Unknown TileLayout in PrintTileView slayout";
      break;
  }
