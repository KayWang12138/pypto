...
class ConstFoldMutator : public IRMutator {
 public:
  ...
  StmtPtr VisitStmt_(const IfStmtPtr& op) override {
    ...
    if (op->elseBody_) else_body = VisitStmt(*op->elseBody_);
    ...
    if (else_body && !op->returnVars_.empty()) {
      ...
      if (then_vals.size() == op->returnVars_.size() && else_vals.size() == op->returnVars_.size()) {
        bool all_same = true;
        ...
        if (all_same) {
          ...
          for (size_t i = 0; i < op->returnVars_.size(); ++i) {
            assigns.push_back(std::make_shared<AssignStmt>(op->returnVars_[i], then_vals[i], op->span_));
          }
