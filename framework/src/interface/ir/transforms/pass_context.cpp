...
#include "ir/reporter/report_generator_registry.h"
#include "ir/transforms/ir_property.h"
#include "ir/transforms/passes.h"
#include "ir/verifier/property_verifier_registry.h"
...
void VerificationInstrument::RunBeforePass(const Pass& pass, const ProgramPtr& program) {
  ...
  if (has_errors) {
    ...
    throw ValueError("Pre-verification failed before pass '" + pass.GetName() + "':\n" + report);
  }
}
...
void VerificationInstrument::RunAfterPass(const Pass& pass, const ProgramPtr& program) {
  ...
  if (has_errors) {
    std::string report = IRVerifier::GenerateReport(diagnostics);
    throw ValueError("Post-verification failed after pass '" + pass.GetName() + "':\n" + report);
  }
}
