...
static void SerializeInitValue(SerializeContext &ctx, SourceCppASTNodePtr &stmtOpNode, const TileValuePtr &tile) {
    auto tileType = ctx.tileTypeNameDict[ObjectCast<TileType>(tile->GetType())];
    auto initNode = rtInitValueTile(tile->GetName(), tileType, tile->GetValidShape().size(), ctx.memNameDict[tile->GetMemory()], SerializeGetScalarArgument(tile));
...
static void SerializeInitValue(SerializeContext &ctx, SourceCppASTNodePtr &stmtOpNode, const TensorValuePtr &tensor) {
    auto tensorType = ctx.tensorTypeNameDict[ObjectCast<TensorType>(tensor->GetType())];
    auto initNode = rtInitValueTensor(tensor->GetName(), tensorType, tensor->GetShape().size(), SerializeGetScalarArgument(tensor));
    stmtOpNode->push_back(initNode);
}
