...
static std::string SerializeGetScalarArgument(const ScalarValuePtr &scalar) {
    std::string arg;
    if (SerializeIsSymbol(scalar)) {
        arg = scalar->GetName();
    } else {
        arg = std::to_string(scalar->GetImmediateValue());
    }
    return arg;
}
...
static std::vector<std::string> SerializeGetScalarArgument(const TileValuePtr &tile) {
    ...
    for (auto shape : tile->GetValidShape()) {
        argList.push_back(SerializeGetScalarArgument(shape));
    }
    return argList;
}
...
static std::vector<std::string> SerializeGetScalarArgument(const TensorValuePtr &tensor) {
    ...
    for (auto stride : tensor->GetStride()) {
        argList.push_back(SerializeGetScalarArgument(stride));
    }
    return argList;
}
    ...
    for (size_t k = 0; k < op->GetNumOutputOperand(); k++) {
        ...
        switch (oop->GetValueKind()) {
            ...
            case ValueKind::Tile: {
                ...
                auto declNode = rtDeclValueTile(oopTile->GetName());
            ...
            case ValueKind::Tensor: {
                ...
                auto declNode = rtDeclValueTensor(oopTensor->GetName());
                stmtOpNode->push_back(declNode);
        ...
        if (ctx.dependDict.count(oop)) {
            for (auto tile : ctx.dependDict[oop].tileValueList) {
                ctx.state.tilePredDict[tile]--;
                if (ctx.state.tilePredDict[tile] == 0) {
                    auto initNode = rtInitValueTile(tile->GetName(), tile->GetValidShape().size(), SerializeGetScalarArgument(tile));
                    stmtOpNode->push_back(initNode);
                }
            }
            for (auto tensor : ctx.dependDict[oop].tensorValueList) {
                ctx.state.tensorPredDict[tensor]--;
                if (ctx.state.tensorPredDict[tensor] == 0) {
                    auto initNode = rtInitValueTensor(tile->GetName(), tensor->GetShape().size(), SerializeGetScalarArgument(tensor));
                    stmtOpNode->push_back(initNode);
                }
            }
        }
...
static void SerializeStatement(SerializeContetx &ctx, SourceCppASTNodePtr &parentNode, const StatementPtr &stmt, const StatementPtr &yieldTarget) {
    switch (stmt->GetKind()) {
...
static void SerializeFunctionDeclTypeList(
        SerializeContext &ctx,
    ...
    for (auto tile : ctx.tileValueList) {
        auto tileType = tile->GetType();
        ctx.tileTypeNameDict[ObjectCast<TileType>(tileType)] = "TT" + std::to_string(tileTypeList.GetIndex(tileType));
    }
    for (auto tensor : ctx.tensorValueList) {
        auto tensorType = tensor->GetType();
        ctx.tensorTypeNameDict[ObjectCast<TensorType>(tensorType)] = "TG" + std::to_string(tensorTypeList.GetIndex(tensorType));
    }
...
static void SerializeFunctionDeclValueList(
