...
static SerializeCommand rtDeclTileType(IR_SOURCE_CPP_DECL_ADDR);
    ...
    for (size_t k = 0; k < op->GetNumOutputOperand(); k++) {
        ...
        switch (oop->GetValueKind()) {
            case ValueKind::Scalar: {
                ...
                auto declNode = rtDeclScalarValue(DTypeInfoOf(oopScalar->GetType()->GetDataType()).name, oopScalar->GetName());
