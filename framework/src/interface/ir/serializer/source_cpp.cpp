...
static void SerializeFunctionOutputList(
    ...
    for (auto tile : ctx.tileValueList) {
        funcNode->push_back(rtDeclTileValue(tile->GetName(), ctx.tileTypeNameDict.find(ObjectCast<TileType>(tile->GetType()))->second));
    }
    for (auto tensor : ctx.tensorValueList) {
        funcNode->push_back(rtDeclTensorValue(tensor->GetName(), ctx.tensorTypeNameDict.find(ObjectCast<TensorType>(tensor->GetType()))->second));
    }
}
...
static SourceCppASTNodePtr SerializeFunction(const FunctionPtr &func) {
    ...
    SerializeFunctionTypeList(ctx, funcNode);
    SerializeFunctionOutputList(ctx, funcNode);
