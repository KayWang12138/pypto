...
static void SerializeInitValue(SerializeContext &ctx, SourceCppASTNodePtr &stmtOpNode, const TensorValuePtr &tensor) {
    (void)ctx;
    auto initNode = rtInitValueTensor(SerializeValueName(tensor), SerializeTypeName(tensor), tensor->GetName() + "Addr", tensor->GetShape().size(), SerializeGetScalarArgument(tensor));
    stmtOpNode->push_back(initNode);
}

static bool IsSingleScalarOutput(const OperationPtr &op) {
...
static void SerializeOperationAssign(SerializeContext &ctx, SourceCppASTNodePtr &stmtOpNode, const std::vector<std::string> &argList) {
    std::ostringstream oss;
    oss << "#define " << argList[0] << " " << rtOperationMacro.GetRuntimeName() << "(";
    for (size_t k = 1; k < argList.size(); k++) {
        oss << (k == 1 ? "" : ", ") << argList[k];
    }
    oss << ")";
...
static void SerializeOperationMacro(SerializeContext &ctx, SourceCppASTNodePtr &stmtOpNode, const std::vector<std::string> &argList) {
...
static void SerializeOperation(SerializeContext &ctx, SourceCppASTNodePtr &stmtOpNode, const OperationPtr &op) {
    ...
    if (IsSingleScalarOutput(op)) {
        SerializeOperationAssign(ctx, stmtOpNode, argList);
    } else {
        SerializeOperationMacro(ctx, stmtOpNode, argList);
    }
...
static void SerializeFunctionDeclValueList(const SerializeContext &ctx, SourceCppASTNodePtr &funcNode) {
    for (auto scalar : ctx.scalarValueList) {
        auto nameName = SerializeValueName(scalar);
        if (!StringUtils::StartsWith(scalarName, IR_SOURCE_CPP_MACRO)) {
            funcNode->push_back(rtDeclValueScalar(scalarName, SerializePrimType(scalar)));
        }
    }
    for (auto tile : ctx.tileValueList) {
