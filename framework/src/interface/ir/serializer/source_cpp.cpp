...
static void SerializeFunctionOutputList(
    ...
    for (auto output: outputList) {
        switch (output->GetValueKind()) {
            case ValueKind::Scalar:
                funcNode->push_back(rtDeclScalarValue(output->GetName(), SerializeDataType(output->GetType())));
                break;
            case ValueKind::Tensor:
                funcNode->push_back(rtDeclTensorValue(output->GetName(), tensorTypeNameDict.find(ObjectCast<TensorType>(output->GetType()))->second));
                break;
            case ValueKind::Tile:
                funcNode->push_back(rtDeclTileValue(output->GetName(), tileTypeNameDict.find(ObjectCast<TileType>(output->GetType()))->second));
                break;
            default:
                break;
        }
...
static SourceCppASTNodePtr SerializeFunction(const FunctionPtr &func) {
    ...
    SerializeFunctionTypeList(funcNode, outputList, tensorTypeNameDict, tileTypeNameDict);
    SerializeFunctionOutputList(funcNode, outputList, tensorTypeNameDict, tileTypeNameDict);
