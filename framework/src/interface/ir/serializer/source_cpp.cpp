    ...
    for (size_t k = 0; k < op->GetNumOutputOperand(); k++) {
        ...
        switch (oop->GetValueKind()) {
            case ValueKind::Scalar: {
                ...
                auto declNode = rtDeclScalarValue(SerializeDataType(oopScalar->GetType()), oopScalar->GetName());
...
static OrderedSet<ValuePtr> SerializeFunctionFindAllOutput(const FunctionPtr &func) {
    struct Visit {
        static void VisitFunc(const FunctionPtr &func, OrderedSet<ValuePtr> &outputList) {
            VisitStmt(func->GetCompound(), outputList);
        }
        static void VisitStmt(const StatementPtr &stmt, OrderedSet<ValuePtr> &outputList) {
            switch (stmt->GetKind()) {
                case StatementKind::Compound: {
                    CompoundStatementPtr stmtCompound = ObjectCast<CompoundStatement>(stmt);
                    for (size_t k = 0; k < stmtCompound->GetStatementsNum(); k++) {
                        VisitStmt(stmtCompound->GetStatement(k), outputList);
                    }
                    break;
                }
                case StatementKind::Op: {
                    OpStatementPtr stmtOp = ObjectCast<OpStatement>(stmt);
                    for (auto &op : stmtOp->Operations()) {
                        VisitOp(op, outputList);
                    }
                    break;
                }
                case StatementKind::If: {
                    IfStatementPtr stmtIf = ObjectCast<IfStatement>(stmt);
                    VisitStmt(stmtIf->GetThenCompound(), outputList);
                    VisitStmt(stmtIf->GetElseCompound(), outputList);
                    break;
                }
                case StatementKind::For: {
                    ForStatementPtr stmtFor = ObjectCast<ForStatement>(stmt);
                    outputList.Insert(stmtFor->GetIterationVar());
                    for (size_t k = 0; k < stmtFor->Results().size(); k++) {
                        outputList.Insert(stmtFor->GetIterValue(k));
                    }
                    VisitStmt(stmtFor->GetCompound(), outputList);
                    for (size_t k = 0; k < stmtFor->Results().size(); k++) {
                        outputList.Insert(stmtFor->Results()[k]);
                    }
                    break;
                }
                case StatementKind::Yield: {
        ...
        static void VisitOp(const OperationPtr &op, OrderedSet<ValuePtr> &outputList) {
            for (size_t k = 0; k < op->GetNumOutputOperand(); k++) {
                outputList.Insert(op->GetOutputOperand(k));
            }
        }
    };
    OrderedSet<ValuePtr> outputList;
    Visit::VisitFunc(func, outputList);
    return outputList;
}

static void SerializeFunctionBuildTypeList(
    ...
    std::unordered_map<TypePtr, std::string> tensorTypeNameDict;
    std::unordered_map<TypePtr, std::string> tileTypeNameDict;
    for (auto tensorType : tensorTypeList) {
        tensorTypeNameDict[ObjectCast<TensorTypePtr>(tensorType)] = "TG" + std::to_string(tensorTypeList.GetIndex(tensorType));
    }
    for (auto tileType : tileTypeList) {
        tileTypeNameDict[ObjectCast<TileTypePtr>(tileType)] = "TT" + std::to_string(tileTypeList.GetIndex(tileType));
    }
}

static SourceCppASTNodePtr SerializeFunction(const FunctionPtr &func) {
    SourceCppASTNodePtr funcNode = rtFunction(func->GetName());
    OrderedSet<ValuePtr> outputList = SerializeFunctionFindAllOutput(func);
    std::unordered_map<TensorTypePtr, std::string> tensorTypeNameDict;
    std::unordered_map<TileTypePtr, std::string> tileTypeNameDict;
    SerializeFunctionBuildTypeList(outputList, tensorTypeNameDict, tileTypeNameDict);
    for (auto &[tensor, name] : tensorTypeNameDict) {
        funcNode->push_back(rtDeclTensorType(name, SerializeDataType(tensor->GetType()), tensor->GetDimNum()));
    }
    for (auto &[tile, name] : tileTypeNameDict) {
        funcNode->push_back(rtDeclTileType(name, SerializeDataType(tile->GetType()), tile->GetShape()));
    }
