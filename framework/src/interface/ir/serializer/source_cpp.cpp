    ...
    for (auto output: outputList) {
        switch (output->GetValueKind()) {
            case ValueKind::Scalar:
                ...
                break;
            case ValueKind::Tensor:
                funcNode->push_back(rtDeclTensorValue(output->GetName(), tensorTypeNameDict.find(output->GetType())->second));
                break;
            case ValueKind::Tile:
                funcNode->push_back(rtDeclTileValue(output->GetName(), tileTypeNameDict.find(output->GetType())->second));
                break;
            default:
                break;
        }
