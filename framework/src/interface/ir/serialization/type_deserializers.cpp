...
static std::optional<msgpack::object> GetOptionalFieldObj(
    const msgpack::object &fieldsObj, const std::string &fieldName, DeserializerContext & /*ctx*/) {
...
std::vector<std::pair<std::string, std::any>> DeserializeKwargs(
    const msgpack::object &kwargsObj, const std::string &fieldName) {
...
// Deserialize Var
static IRNodePtr DeserializeVar(const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
...
// Deserialize IterArg
static IRNodePtr DeserializeIterArg(const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
    auto type = ctx.DeserializeType(GET_FIELD_OBJ("type"), zone);
    std::string name = GET_FIELD(std::string, "name");
    auto initValue = std::static_pointer_cast<const Expr>(ctx.DeserializeNode(GET_FIELD_OBJ("initValue"), zone));
    return std::make_shared<IterArg>(name, type, initValue, span);
}

// Deserialize MemRef
static IRNodePtr DeserializeMemRef(const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
...
static IRNodePtr DeserializeConstInt(const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
    auto type = ctx.DeserializeType(GET_FIELD_OBJ("type"), zone);
...
static IRNodePtr DeserializeConstFloat(
    const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
    auto type = ctx.DeserializeType(GET_FIELD_OBJ("type"), zone);
    double value = GET_FIELD(double, "value");
    auto scalarType = As<ScalarType>(type);
    INTERNAL_CHECK(scalarType) << "ConstFloat is expected to have ScalarType type, but got " + type->TypeName();
    return std::make_shared<ConstFloat>(value, scalarType->dtype_, span);
}

// Deserialize ConstBool
static IRNodePtr DeserializeConstBool(
    const msgpack::object &fieldsObj, msgpack::zone & /*zone*/, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
...
// Deserialize Call
static IRNodePtr DeserializeCall(const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
    ...
    if (argsObj.type == msgpack::type::ARRAY) {
        for (uint32_t i = 0; i < argsObj.via.array.size; ++i) {
            args.push_back(std::static_pointer_cast<const Expr>(ctx.DeserializeNode(argsObj.via.array.ptr[i], zone)));
        }
    }
...
#define DESERIALIZE_BINARY_EXPR(ClassName)                                                                            \
    static IRNodePtr Deserialize##ClassName(                                                                          \
        const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {                            \
        auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));                                                       \
        auto type = ctx.DeserializeType(GET_FIELD_OBJ("type"), zone);                                                 \
        auto scalarType = As<ScalarType>(type);                                                                       \
        INTERNAL_CHECK(scalarType) << #ClassName " is expected to have ScalarType type, but got " + type->TypeName(); \
...
#define DESERIALIZE_UNARY_EXPR(ClassName)                                                                             \
    static IRNodePtr Deserialize##ClassName(                                                                          \
        ...
        auto scalarType = As<ScalarType>(type);                                                                       \
        INTERNAL_CHECK(scalarType) << #ClassName " is expected to have ScalarType type, but got " + type->TypeName(); \
        auto operand = std::static_pointer_cast<const Expr>(ctx.DeserializeNode(GET_FIELD_OBJ("operand"), zone));     \
...
static IRNodePtr DeserializeAssignStmt(
    const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
...
// Deserialize IfStmt
static IRNodePtr DeserializeIfStmt(const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
    auto condition = std::static_pointer_cast<const Expr>(ctx.DeserializeNode(GET_FIELD_OBJ("condition"), zone));

    // Deserialize then_body as single StmtPtr
    auto thenBody = std::static_pointer_cast<const Stmt>(ctx.DeserializeNode(GET_FIELD_OBJ("then_body"), zone));
...
static std::vector<ExprPtr> DeserializeExprArray(
    const msgpack::object &arrayObj, msgpack::zone &zone, DeserializerContext &ctx) {
...
static IRNodePtr DeserializeYieldStmt(const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
...
static IRNodePtr DeserializeReturnStmt(
    const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
    auto value = DeserializeExprArray(GET_FIELD_OBJ("value"), zone, ctx);
...
// Deserialize ForStmt
static IRNodePtr DeserializeForStmt(const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
...
static std::vector<StmtPtr> DeserializeStmtArray(
    const msgpack::object &arrayObj, msgpack::zone &zone, DeserializerContext &ctx) {
...
static IRNodePtr DeserializeSeqStmts(const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
...
// Deserialize OpStmts
static IRNodePtr DeserializeOpStmts(const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
    auto stmts = DeserializeStmtArray(GET_FIELD_OBJ("stmts"), zone, ctx);
...
static IRNodePtr DeserializeEvalStmt(const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
...
static IRNodePtr DeserializeFunction(const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
    std::string name = GET_FIELD(std::string, "name");
...
// Deserialize Program
static IRNodePtr DeserializeProgram(const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
    std::string name = GET_FIELD(std::string, "name");
...
static IRNodePtr DeserializeMakeTuple(const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
...
static IRNodePtr DeserializeTupleGetItemExpr(
    const msgpack::object &fieldsObj, msgpack::zone &zone, DeserializerContext &ctx) {
    auto span = ctx.DeserializeSpan(GET_FIELD_OBJ("span"));
