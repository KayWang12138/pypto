...
msgpack::object FieldSerializerVisitor::VisitLeafField(const std::vector<std::pair<std::string, std::any>> &kwargs) {
    ...
    for (const auto &[key, value] : kwargs) {
        ...
        if (value.type() == typeid(int)) {
            ...
            kwargsMsgs.push_back(makePair(key, VisitLeafField(AnyCast<bool>(value, "serializing kwarg: " + key))));
        } else if (value.type() == typeid(std::string)) {
            kwargsMsgs.push_back(
                makePair(key, VisitLeafField(AnyCast<std::string>(value, "serializing kwarg: " + key))));
        } else if (value.type() == typeid(double)) {
            kwargsMsgs.push_back(makePair(key, VisitLeafField(AnyCast<double>(value, "serializing kwarg: " + key))));
        } else if (value.type() == typeid(float)) {
            kwargsMsgs.push_back(makePair(key, VisitLeafField(AnyCast<float>(value, "serializing kwarg: " + key))));
        } else if (value.type() == typeid(DataType)) {
            kwargsMsgs.push_back(makePair(key, VisitLeafField(AnyCast<DataType>(value, "serializing kwarg: " + key))));
        } else {
