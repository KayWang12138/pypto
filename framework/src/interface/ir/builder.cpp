...
void IRBuilder::BeginForLoop(
    ...
    if (contextStack_.empty()) {
        throw RuntimeError(
            "Cannot begin for loop: not inside a function or another valid context at " + span.to_string());
    }
...
StmtPtr IRBuilder::EndForLoop(const Span &endSpan) {
    ...
    if (loopCtx->GetIterArgs().size() != loopCtx->GetReturnVars().size()) {
        ...
        oss << "For loop has " << iterArgsSize << " iteration arguments but " << returnVarsSize
    ...
    auto forStmt = std::make_shared<ForStmt>(loopCtx->GetLoopVar(), loopCtx->GetStart(), loopCtx->GetStop(),
        loopCtx->GetStep(), loopCtx->GetIterArgs(), body, loopCtx->GetReturnVars(), combinedSpan);
...
void IRBuilder::BeginElse(const Span &span) {
    ...
    INTERNAL_CHECK(!ifCtx->InElseBranch())
        << "Cannot begin else branch: already in else branch at " << span.to_string();
