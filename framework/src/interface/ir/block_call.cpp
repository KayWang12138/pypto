...
void CallBlock(BlockFunction blockFunction,
    ...
    for (const auto &tensorRef : inputTensorArgs) {
        ...
        inputLogicTensors.emplace_back(tensor.GetStorage(false));
    }
    ...
    printf("In block call Function name %s, type %d %d\n", function->GetMagicName().c_str(), (int)function->GetFunctionType(), (int)function->GetGraphType());
    // Here we put program module into TensorGraph and create a CallOp in TensorGraph
    if (function->programModule_ == nullptr) {
        function->programModule_ = std::make_shared<ProgramModule>(function->GetMagicName() + "_IR");
    }
    blockFunction(function->programModule_, inputArgs, outputArgs);
    auto &callOp = function->AddRawOperation(npu::tile_fwk::Opcode::OP_BLOCK_CALL, 
        inputLogicTensors, outputLogicTensors, false);

    // 2.1 Compute hash of program module
    FunctionHash hash = 1111;
    // programModule->ComputeHash(); // TODO IR block function hash
