...
TypePtr DeduceBlockRowExpandType(const std::vector<ExprPtr> &args,
    ...
    CHECK(rowColConst && rowColConst->value_ == 1)
        ...
        << (rowColConst ? std::to_string(rowColConst->value_) : "?") << " at " << span.ToString();
    ...
    if (tileRowsConst && rowRowsConst) {
        CHECK(tileRowsConst->value_ == rowRowsConst->value_)
            ...
            << " and row_vec rows=" << rowRowsConst->value_ << " at " << span.ToString();
    }
    ...
    CHECK(resultDtype) << "The operator " << opName << " requires compatible data types, but got "
                       << tileType->dtype_.ToString() << " and " << rowType->dtype_.ToString() << " at " << span.ToString();
...
REGISTER_OP("block.row_expand_sub")
    ...
    .SetDeduceType([](const std::vector<ExprPtr> &args, const std::vector<std::pair<std::string, std::any>> &kwargs,
        const Span &span) {
        return DeduceBlockRowExpandType(args, kwargs, "block.row_expand_sub", span);
    });
...
REGISTER_OP("block.row_expand_div")
    ...
    .SetDeduceType([](const std::vector<ExprPtr> &args, const std::vector<std::pair<std::string, std::any>> &kwargs,
        const Span &span) {
        return DeduceBlockRowExpandType(args, kwargs, "block.row_expand_div", span);
    });
...
REGISTER_OP("block.row_expand_mul")
    ...
    .SetDeduceType([](const std::vector<ExprPtr> &args, const std::vector<std::pair<std::string, std::any>> &kwargs,
        const Span &span) {
        return DeduceBlockRowExpandType(args, kwargs, "block.row_expand_mul", span);
    });
...
REGISTER_OP("block.row_expand_add")
    ...
    .SetDeduceType([](const std::vector<ExprPtr> &args, const std::vector<std::pair<std::string, std::any>> &kwargs,
        const Span &span) {
        return DeduceBlockRowExpandType(args, kwargs, "block.row_expand_add", span);
    });
