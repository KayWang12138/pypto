...
TypePtr DeduceBlockOpElementwiseBinaryType(const std::vector<ExprPtr>& args,
  ...
  INTERNAL_CHECK(tile_type1) << "The operator " << op_name << " requires first argument to be a TileType, but got "
                    << args[0]->GetType()->TypeName();
  INTERNAL_CHECK(tile_type2) << "The operator " << op_name << " requires second argument to be a TileType, but got "
  ...
  INTERNAL_CHECK(broadcast_result.success) << "The operator " << op_name << " requires compatible shapes, but got "
...
TypePtr DeduceBlockOpScalarBinaryType(const std::vector<ExprPtr>& args,
  ...
  INTERNAL_CHECK(scalar_type) << "The operator " << op_name << " requires second argument to be a ScalarType, but got "
                     << args[1]->GetType()->TypeName();
...
// Tile-scalar: use GetOrEmitFloatConstant for constant (emits to constants section), return instruction line
// only
