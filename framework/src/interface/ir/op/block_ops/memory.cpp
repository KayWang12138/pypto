...
CCECodegenFunc MakeBlockLoadCodegenCCE() {
  return [](const CallPtr& op, codegen::CCECodegen& codegen) -> std::string {
    INTERNAL_CHECK(op->args_.size() == 5)
    ...
    INTERNAL_CHECK(src_tensor_var_ptr != nullptr) << "block.load source tensor must be a Var";
...
CCECodegenFunc MakeBlockStoreCodegenCCE() {
  return [](const CallPtr& op, codegen::CCECodegen& codegen) -> std::string {
    INTERNAL_CHECK(op->args_.size() == 6)
    ...
    INTERNAL_CHECK(dst_tensor_var_ptr != nullptr) << "block.store destination tensor must be a Var";
...
CCECodegenFunc MakeBlockMoveCodegenCCE() {
  return [](const CallPtr& op, codegen::CCECodegen& codegen) -> std::string {
    INTERNAL_CHECK(op->args_.size() == 1) << "block.move requires 1 argument: source tile";
...
TypePtr DeduceBlockGetBlockIdxType(const std::vector<ExprPtr>& args,
  ...
  INTERNAL_CHECK(args.size() == 0) << "The operator " << op_name << " requires no arguments, but got " << args.size();
...
TypePtr DeduceBlockLoadType(const std::vector<ExprPtr>& args,
  ...
  INTERNAL_CHECK(args.size() >= 1) << "The operator " << op_name << " requires at least 1 argument, but got "
                          << args.size();
...
TypePtr DeduceBlockStoreType(const std::vector<ExprPtr>& args,
  ...
  INTERNAL_CHECK(args.size() >= 2) << "The operator " << op_name << " requires at least 2 arguments, but got "
                          << args.size();
  ...
  INTERNAL_CHECK(output_tensor_type) << "The operator " << op_name
...
TypePtr DeduceBlockAllocType(const std::vector<ExprPtr>& args,
  ...
  INTERNAL_CHECK(args.size() == 4) << "The operator " << op_name << " requires exactly 4 arguments, but got "
                          << args.size();
