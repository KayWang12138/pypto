...
CCECodegenFunc MakeBlockMatmulCodegenCCE() {
  return [](const CallPtr& op, codegen::CCECodegen& codegen) -> std::string {
    INTERNAL_CHECK(op->args_.size() == 2) << "block.matmul requires 2 arguments: lhs, rhs";
...
CCECodegenFunc MakeBlockMatmulAccCodegenCCE() {
  return [](const CallPtr& op, codegen::CCECodegen& codegen) -> std::string {
    INTERNAL_CHECK(op->args_.size() == 3) << "block.matmul_acc requires 3 arguments: acc, lhs, rhs";
...
TypePtr DeduceBlockMatMulType(const std::vector<ExprPtr>& args,
  ...
  INTERNAL_CHECK(lhs_shape.size() == 2) << "The operator " << op_name << " requires lhs to be 2D, but got "
                               << lhs_shape.size() << " dimensions";
  INTERNAL_CHECK(rhs_shape.size() == 2) << "The operator " << op_name << " requires rhs to be 2D, but got "
...
TypePtr DeduceBlockMatMulAccType(const std::vector<ExprPtr>& args,
                                 const std::vector<std::pair<std::string, std::any>>& kwargs,
                                 const std::string& op_name) {
  INTERNAL_CHECK(args.size() == 3) << "The operator " << op_name << " requires exactly 3 arguments, but got "
                          << args.size();
  ...
  INTERNAL_CHECK(acc_type) << "The operator " << op_name << " requires first argument (acc) to be a TileType, but got "
                  << args[0]->GetType()->TypeName();
  INTERNAL_CHECK(lhs_type) << "The operator " << op_name
                  << " requires second argument (lhs) to be a TileType, but got "
                  << args[1]->GetType()->TypeName();
  INTERNAL_CHECK(rhs_type) << "The operator " << op_name << " requires third argument (rhs) to be a TileType, but got "
  ...
  INTERNAL_CHECK(acc_shape.size() == 2) << "The operator " << op_name << " requires acc to be 2D, but got "
  ...
  if (n_acc_const && n_rhs_const) {
    INTERNAL_CHECK(n_acc_const->value_ == n_rhs_const->value_)
        << "The operator " << op_name
  ...
  INTERNAL_CHECK(lhs_rhs_dtype) << "The operator " << op_name
                       << " requires compatible lhs and rhs data types, but got "
                       << lhs_type->dtype_.ToString() << " and " << rhs_type->dtype_.ToString();

  auto result_dtype = PromoteDataTypes(acc_type->dtype_, *lhs_rhs_dtype);
  INTERNAL_CHECK(result_dtype) << "The operator " << op_name << " requires compatible accumulator data type, but got "
