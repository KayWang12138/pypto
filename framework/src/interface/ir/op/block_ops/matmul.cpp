...
TypePtr DeduceBlockMatMulType(const std::vector<ExprPtr> &args,
    ...
    CHECK(lhs_shape.size() == 2) << "The operator " << opName << " requires lhs to be 2D, but got " << lhs_shape.size()
                                 << " dimensions";
    CHECK(rhs_shape.size() == 2) << "The operator " << opName << " requires rhs to be 2D, but got " << rhs_shape.size()
                                 << " dimensions";
...
TypePtr DeduceBlockMatMulAccType(const std::vector<ExprPtr> &args,
    ...
    CHECK(acc_type) << "The operator " << opName << " requires first argument (acc) to be a TileType, but got "
                    << args[0]->GetType()->TypeName();
    CHECK(lhs_type) << "The operator " << opName << " requires second argument (lhs) to be a TileType, but got "
                    << args[1]->GetType()->TypeName();
    ...
    CHECK(acc_shape.size() == 2) << "The operator " << opName << " requires acc to be 2D, but got " << acc_shape.size()
                                 << " dimensions";
    CHECK(lhs_shape.size() == 2) << "The operator " << opName << " requires lhs to be 2D, but got " << lhs_shape.size()
                                 << " dimensions";
    CHECK(rhs_shape.size() == 2) << "The operator " << opName << " requires rhs to be 2D, but got " << rhs_shape.size()
    ...
    if (m_acc_const && m_lhs_const) {
        CHECK(m_acc_const->value_ == m_lhs_const->value_)
            << "The operator " << opName << " requires matching M dimensions, but got acc M=" << m_acc_const->value_
            << " and lhs M=" << m_lhs_const->value_;
    }

    if (n_acc_const && n_rhs_const) {
        CHECK(n_acc_const->value_ == n_rhs_const->value_)
            << "The operator " << opName << " requires matching N dimensions, but got acc N=" << n_acc_const->value_
            << " and rhs N=" << n_rhs_const->value_;
    }

    if (k_lhs_const && k_rhs_const) {
        CHECK(k_lhs_const->value_ == k_rhs_const->value_)
            << "The operator " << opName << " requires matching K dimensions, but got lhs K=" << k_lhs_const->value_
            << " and rhs K=" << k_rhs_const->value_;
    }

    // Promote data types
    auto lhs_rhs_dtype = PromoteDataTypes(lhs_type->dtype_, rhs_type->dtype_);
    CHECK(lhs_rhs_dtype) << "The operator " << opName << " requires compatible lhs and rhs data types, but got "
                         << lhs_type->dtype_.ToString() << " and " << rhs_type->dtype_.ToString();
