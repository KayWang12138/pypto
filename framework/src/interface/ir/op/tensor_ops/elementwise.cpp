...
TypePtr DeduceTensorOpElementwiseBinaryType(const std::vector<ExprPtr>& args,
  ...
  INTERNAL_CHECK(tensorType2) << "The operator " << opName
  ...
  INTERNAL_CHECK(broadcastResult.success) << "The operator " << opName << " requires compatible shapes, but got "
                                  << FormatShape(tensorType1->shape_) << " and "
                                  << FormatShape(tensorType2->shape_);
...
TypePtr DeduceTensorOpElementwiseScalarType(const std::vector<ExprPtr>& args,
  ...
  INTERNAL_CHECK(tensorType1) << "The operator " << opName << " requires first argument to be a TensorType, but got "
                      << args[0]->GetType()->TypeName();
  INTERNAL_CHECK(scalarType2) << "The operator " << opName
