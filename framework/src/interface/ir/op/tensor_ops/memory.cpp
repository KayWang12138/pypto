...
TypePtr DeduceTensorCreateType(
    ...
    INTERNAL_CHECK(found_dtype) << "tensor.create requires 'dtype' kwarg";

    // First argument must be TupleType (shape)
    auto shape_tuple_type = As<TupleType>(args[0]->GetType());
    INTERNAL_CHECK(shape_tuple_type) << "tensor.create requires shape to be TupleType, but got "
...
TypePtr DeduceTensorViewType(
    ...
    INTERNAL_CHECK(shape_tuple_type) << "tensor.view requires shape to be TupleType, but got "
    ...
    INTERNAL_CHECK(offset_tuple_type) << "tensor.view requires offset to be TupleType, but got "
    ...
    if (auto make_tuple = As<MakeTuple>(args[1])) {
        ...
        // Runtime tuple: use TupleGetItemExpr
        for (size_t i = 0; i < shape_tuple_type->types_.size(); ++i) {
            new_shape.emplace_back(std::make_shared<TupleGetItemExpr>(args[1], static_cast<int>(i), args[1]->span_));
        }
    }
...
TypePtr DeduceTensorAssembleType(
    ...
    INTERNAL_CHECK(target_type) << "tensor.assemble requires first argument to be a TensorType, but got "
                                << args[0]->GetType()->TypeName();

    // Second argument (source) must be TensorType
    auto source_type = As<TensorType>(args[1]->GetType());
    INTERNAL_CHECK(source_type) << "tensor.assemble requires second argument to be a TensorType, but got "
                                << args[1]->GetType()->TypeName();

    // Third argument must be TupleType (offset)
    auto offset_tuple_type = As<TupleType>(args[2]->GetType());
    INTERNAL_CHECK(offset_tuple_type) << "tensor.assemble requires offset to be TupleType, but got "
                                      << args[2]->GetType()->TypeName();
