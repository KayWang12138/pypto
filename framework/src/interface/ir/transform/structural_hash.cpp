  ...
  result_type VisitLeafField(const std::vector<std::pair<std::string, std::any>>& kwargs) {
    ...
    for (const auto& [key, value] : kwargs) {
      ...
      if (value.type() == typeid(int)) {
        ...
        // throw TypeError("Invalid kwarg type for key: " + key +  // Temporarily commented out
        throw std::logic_error("Invalid kwarg type for key: " + key +  // Using std::logic_error instead
...
StructuralHasher::result_type StructuralHasher::HashNode(const IRNodePtr& node) {
  ...
  if (!dispatched) {
    ...
    // throw pypto::TypeError("Unknown IR node type in StructuralHasher::HashNode");  // Temporarily commented out
    throw std::logic_error("Unknown IR node type in StructuralHasher::HashNode");  // Using std::logic_error instead
  } else {
