...
void IRVerifier::AddRule(VerifyRulePtr rule) {
    ...
    auto it = std::find_if(
        rules_.begin(), rules_.end(), [&rule](const VerifyRulePtr &r) { return r->GetName() == rule->GetName(); });
...
void IRVerifier::EnableRule(const std::string &name) {
    disabled_rules_.erase(name);
}

void IRVerifier::DisableRule(const std::string &name) {
    disabled_rules_.insert(name);
}

bool IRVerifier::IsRuleEnabled(const std::string &name) const {
    return disabled_rules_.count(name) == 0;
}
