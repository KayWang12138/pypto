...
DependencyGraph DependencyAnalyzer::Analyze(const FunctionPtr &func) {
    ...
    for (const auto &edge : allDependencies) {
        switch (edge.type) {
            case DependencyEdge::RAW: rawCount++; break;
            case DependencyEdge::WAR: warCount++; break;
            case DependencyEdge::WAW: wawCount++; break;
            default: break;
        }
    }
...
std::vector<BasicBlock> DependencyAnalyzer::IdentifyBasicBlocks(const StmtPtr &stmt) {
    ...
    public:
        explicit BasicBlockBuilder(std::vector<BasicBlock> &blocks, int &nextId) : blocks_(blocks), next_id_(nextId) {}
...
std::vector<DependencyEdge> DependencyAnalyzer::MergeDependencies(
    ...
    LOG_DEBUG << "Merged " << merged.size() << " unique dependencies from " << pathDependencies.size() << " paths";
...
std::string DependencyAnalyzer::GetPipeType(const CallPtr &callExpr) {
    ...
    if (pipeOpt.has_value()) {
        ...
        switch (pipe) {
            case PipeType::M: return "CUBE";
            case PipeType::V: return "VECTOR";
            case PipeType::MTE1: return "MTE1";
            case PipeType::MTE2: return "MTE2";
            case PipeType::MTE3: return "MTE3";
            case PipeType::S: return "SCALAR";
            case PipeType::FIX: return "FIX";
            case PipeType::ALL: return "ALL";
