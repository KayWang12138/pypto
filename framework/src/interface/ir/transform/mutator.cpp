...
#define DEFINE_BINARY_MUTATOR(OpType)                                                                       \
  ExprPtr IRMutator::VisitExpr_(const OpType##Ptr& op) {                                                    \
    ...
    auto newLeft = ExprFunctor<ExprPtr>::VisitExpr(op->left_);                                             \
    auto newRight = ExprFunctor<ExprPtr>::VisitExpr(op->right_);                                           \
    INTERNAL_CHECK(newLeft) << #OpType " left operand mutated to null";                                    \
    INTERNAL_CHECK(newRight) << #OpType " right operand mutated to null";                                  \
    auto scalarType = As<ScalarType>(op->GetType());                                                       \
    INTERNAL_CHECK(scalarType) << #OpType " has null type";                                                \
    if (newLeft.get() != op->left_.get() || newRight.get() != op->right_.get()) {                         \
      return std::make_shared<const OpType>(std::move(newLeft), std::move(newRight), scalarType->dtype_, \
                                            op->span_);                                                     \
    } else {                                                                                                \
      return op;                                                                                            \
    }                                                                                                       \
...
#define DEFINE_UNARY_MUTATOR(OpType)                                                                 \
  ExprPtr IRMutator::VisitExpr_(const OpType##Ptr& op) {                                             \
    ...
    auto newOperand = ExprFunctor<ExprPtr>::VisitExpr(op->operand_);                                \
    INTERNAL_CHECK(newOperand) << #OpType " operand mutated to null";                               \
    auto scalarType = As<ScalarType>(op->GetType());                                                \
    INTERNAL_CHECK(scalarType) << #OpType " has null type";                                         \
    if (newOperand.get() != op->operand_.get()) {                                                   \
      return std::make_shared<const OpType>(std::move(newOperand), scalarType->dtype_, op->span_); \
    } else {                                                                                         \
      return op;                                                                                     \
    }                                                                                                \
  }
...
StmtPtr IRMutator::VisitStmt_(const AssignStmtPtr& op) {
  ...
  // Cast newVar from ExprPtr to VarPtr (required by AssignStmt constructor)
...
StmtPtr IRMutator::VisitStmt_(const IfStmtPtr& op) {
  ...
  INTERNAL_CHECK(newThenBody) << "IfStmt thenBody mutated to null";
  ...
  if (op->elseBody_.has_value()) {
    ...
    INTERNAL_CHECK(newStmt) << "IfStmt elseBody mutated to null";
    newElseBody = newStmt;
  ...
  for (size_t i = 0; i < op->returnVars_.size(); ++i) {
    ...
    INTERNAL_CHECK(newVarExpr) << "IfStmt returnVars at index " << i << " mutated to null";
    // Cast newVar from ExprPtr to VarPtr (required by IfStmt constructor)
    auto newVar = As<Var>(newVarExpr);
    INTERNAL_CHECK(newVar) << "IfStmt returnVars at index " << i << " is not a Var after mutation";
  ...
  if (newCondition.get() != op->condition_.get() || thenChanged || elseChanged || returnVarsChanged) {
    if (newElseBody.has_value()) {
      return std::make_shared<const IfStmt>(std::move(newCondition), std::move(newThenBody),
                                            *newElseBody, std::move(newReturnVars), op->span_);
    } else {
      return std::make_shared<const IfStmt>(std::move(newCondition), std::move(newThenBody), std::nullopt,
                                            std::move(newReturnVars), op->span_);
    }
  } else {
    return op;
  }
...
StmtPtr IRMutator::VisitStmt_(const ForStmtPtr& op) {
  ...
  INTERNAL_CHECK(newLoopVarExpr) << "ForStmt loopVar mutated to null";
  auto newLoopVar = As<Var>(newLoopVarExpr);
  INTERNAL_CHECK(newLoopVar) << "ForStmt loopVar is not a Var after mutation";
  ...
  for (size_t i = 0; i < op->iterArgs_.size(); ++i) {
    ...
    INTERNAL_CHECK(newIterArgExpr) << "ForStmt iterArgs at index " << i << " mutated to null";
    auto newIterArg = As<IterArg>(std::static_pointer_cast<const IRNode>(newIterArgExpr));
    INTERNAL_CHECK(newIterArg) << "ForStmt iterArgs at index " << i << " is not an IterArg after mutation";
  ...
  for (size_t i = 0; i < op->returnVars_.size(); ++i) {
    ...
    auto newVarExpr = ExprFunctor<ExprPtr>::VisitExpr(op->returnVars_[i]);
    INTERNAL_CHECK(newVarExpr) << "ForStmt returnVars at index " << i << " mutated to null";
    // Cast newVar from ExprPtr to VarPtr (required by ForStmt constructor)
    auto newVar = As<Var>(newVarExpr);
    INTERNAL_CHECK(newVar) << "ForStmt returnVars at index " << i << " is not a Var after mutation";
    newReturnVars.push_back(newVar);
    if (newVar.get() != op->returnVars_[i].get()) {
      returnVarsChanged = true;
    }
  ...
  if (newLoopVar.get() != op->loopVar_.get() || newStart.get() != op->start_.get() ||
      newStop.get() != op->stop_.get() || newStep.get() != op->step_.get() || iterArgsChanged ||
      bodyChanged || returnVarsChanged) {
    return std::make_shared<const ForStmt>(std::move(newLoopVar), std::move(newStart), std::move(newStop),
                                           std::move(newStep), std::move(newIterArgs), std::move(newBody),
                                           std::move(newReturnVars), op->span_);
  } else {
    return op;
  }
