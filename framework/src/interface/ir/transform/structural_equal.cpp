...
public:
    ...
    template <typename IRNodePtrType>
    result_type VisitIRNodeField(const std::optional<IRNodePtrType> &lhs, const std::optional<IRNodePtrType> &rhs) {
    ...
    template <typename IRNodePtrType>
    result_type VisitIRNodeVectorField(const std::vector<IRNodePtrType> &lhs, const std::vector<IRNodePtrType> &rhs) {
    ...
    result_type VisitIRNodeMapField(
...
private:
     ...
     * \param lhsOp Left-hand side node
     * \param rhsOp Right-hand side node
...
bool StructuralEqualImpl<AssertMode>::EqualType(const TypePtr &lhs, const TypePtr &rhs) {
    ...
    if (auto lhsScalar = As<ScalarType>(lhs)) {
        ...
        for (size_t i = 0; i < lhsTensor->shape_.size(); ++i) {
            if (!Equal(lhsTensor->shape_[i], rhsTensor->shape_[i]))
                return false;
        }
        return true;
        ...
        if (lhsTile->dtype_ != rhsTile->dtype_) {
            if constexpr (AssertMode) {
                std::ostringstream msg;
                msg << "TileType dtype mismatch (" << lhsTile->dtype_.ToString() << " != " << rhsTile->dtype_.ToString()
                    << ")";
                ThrowMismatch(msg.str(), IRNodePtr(), IRNodePtr(), "", "");
            }
            return false;
        ...
        if (lhsTile->shape_.size() != rhsTile->shape_.size()) {
            if constexpr (AssertMode) {
                std::ostringstream msg;
                msg << "TileType shape rank mismatch (" << lhsTile->shape_.size() << " != " << rhsTile->shape_.size()
                    << ")";
                ThrowMismatch(msg.str(), IRNodePtr(), IRNodePtr(), "", "");
            }
            return false;
        ...
        for (size_t i = 0; i < lhsTile->shape_.size(); ++i) {
            if (!Equal(lhsTile->shape_[i], rhsTile->shape_[i]))
                return false;
        }
        // Compare tileView
        if (lhsTile->tileView_.has_value() != rhsTile->tileView_.has_value()) {
            if constexpr (AssertMode) {
                ThrowMismatch("TileType tileView presence mismatch", IRNodePtr(), IRNodePtr(), "", "");
            }
            return false;
        }
        if (lhsTile->tileView_.has_value()) {
            ...
            if (lhsTv.validShape.size() != rhsTv.validShape.size()) {
                if constexpr (AssertMode) {
                    std::ostringstream msg;
                    msg << "TileView validShape size mismatch (" << lhsTv.validShape.size()
            ...
            for (size_t i = 0; i < lhsTv.validShape.size(); ++i) {
                if (!Equal(lhsTv.validShape[i], rhsTv.validShape[i]))
                    return false;
            }
            // Compare stride
            ...
            for (size_t i = 0; i < lhsTv.stride.size(); ++i) {
                if (!Equal(lhsTv.stride[i], rhsTv.stride[i]))
                    return false;
            }
            // Compare startOffset
            if (!Equal(lhsTv.startOffset, rhsTv.startOffset))
                return false;
        }
        return true;
        ...
        for (size_t i = 0; i < lhsTuple->types_.size(); ++i) {
            if (!EqualType(lhsTuple->types_[i], rhsTuple->types_[i]))
                return false;
        }
        return true;
...
bool StructuralEqualImpl<AssertMode>::EqualVar(const VarPtr &lhs, const VarPtr &rhs) {
    if (!enable_auto_mapping_) {
        ...
        if (lhsIt != lhs_to_rhs_var_map_.end() && rhsIt != rhs_to_lhs_var_map_.end()) {
    ...
    auto rhsIt = rhs_to_lhs_var_map_.find(rhs);
    if (rhsIt != rhs_to_lhs_var_map_.end() && rhsIt->second != lhs) {
        if constexpr (AssertMode) {
            std::ostringstream msg;
