...
bool experimental::IsType(const std::string &key, const std::type_info &type) {
    std::string lookupKey = StringUtils::ToLower(key);
    {
        std::shared_lock lock(g_rwlock);
        auto iter = g_config.options.find(lookupKey);
    ...
    return ConfigManagerNg::GetInstance().Type(key) == type;
}
...
#define DEFINE_GET_OPTION(Type)                                               \
    bool experimental::GetOption(const std::string &key, Type &value) {       \
        ...
        if (!scope || !scope->HasConfig(key)) {                               \
            return false;                                                     \
        }                                                                     \
        try {                                                                 \
            value = scope->GetConfig<Type>(key);                              \
            return true;                                                      \
        } catch (...) {                                                       \
            return false;                                                     \
        }                                                                     \
...
template <typename T>
void SetOptionInternal(const std::string &key, const T &value) {
    if (ConfigManagerNg::GetInstance().Type(key) != typeid(void)) {
        ConfigManagerNg::CurrentScope()->UpdateValue(key, value);
    }
    std::unique_lock lock(g_rwlock);
...
void experimental::SetOption(const std::string &key, const char *value) {
    SetOptionInternal(key, std::string(value));
}
