...
void ConfigManager::PassConfigsDebugInfo(
    ...
    if (!node) {
        ALOG_INFO("[ConfigManager] Missing custom pass strategy <", strategy, "> configs. ",
                    "You may add your own custom strategy configs in 'tile_fwk_config_ng.json'.");
        return;
    }
    ...
    for (auto &&identifier : identifiers) {
        ...
        if (node->find(identifier) != node->end()) {
            ...
            strategy, "> is missing. You may add your own custom strategy configs in 'tile_fwk_config_ng.json'.");
        }
    }
}
