...
static std::string CreateLogTopFolder()
{
    ...
    ASSERT(FError::BAD_FD, ret) << "Failed to create dir: " << folderPath << ", ensure its parent dir exists.";
...
void CreateRunDataDir()
{
    ...
    ASSERT(FError::BAD_FD, res) << "Failed to create directory: " << g_config.rundataDir.montage();
}
