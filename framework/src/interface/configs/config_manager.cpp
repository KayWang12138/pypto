...
static std::string CreateLogTopFolder() {
    ...
    CHECK(ERROR_CODE_UNKNOWN, ret) << "Failed to create dir: " << folderPath << ", ensure its parent dir exists.";

    folderPath = folderPath + "/output_" + timestamp.str() + "_" + std::to_string(getpid());
    ret = CreateDir(folderPath);
    ASSERT(ERROR_CODE_UNKNOWN, ret) << "Failed to create dir: " << folderPath << ", ensure its parent dir exists.";
...
void CreateRunDataDir() {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, res) << "Failed to create directory: " << g_config.rundataDir.montage();
}
