...
public:
    ...
    CubeTile GetCubeTile() const {
        const Any& value = GetConfigInner("cube_tile_shapes");
    ...
    VecTile GetVecTile() const {
        const Any& value = GetConfigInner("vec_tile_shapes");
    ...
    std::vector<int64_t> GetMatrixSize() const {
        const Any& value = GetConfigInner("matrix_size");
    ...
    T GetConfigAllType(const std::string &key) const {
        if constexpr (std::is_same_v<T, bool>) {
            return AnyCast<bool>(GetConfigInner(key));
        } else if constexpr (std::is_integral_v<T>) {
            int64_t tmp = AnyCast<int64_t>(GetConfigInner(key));
            return static_cast<T>(tmp);
        } else {
            return AnyCast<T>(GetConfigInner(key));
        }
    }
...
private:
    ...
    const Any &GetConfigInner(const std::string &key) const;
};
