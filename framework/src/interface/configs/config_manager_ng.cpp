...
struct ConfigManagerImpl {
    ...
    void SetGlobalConfig(std::map<std::string, Any> &&values) {
        for (auto &it : values) {
...
void ConfigManagerNg::SetGlobalConfig(std::map<std::string, Any> &&values) {
    return impl_->SetGlobalConfig(std::move(values));
}
