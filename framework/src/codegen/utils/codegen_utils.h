...
inline std::string GetPipeId(PipeType queue) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, res != PIPE_ID.end()) << "can not find pipe id: " << ToUnderlying(queue);
    return res->second;
}
...
struct FloatSpecVal {
    ...
    std::string GetFsValueStr() const {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, false) << "FloatSpecVal not found, dtype: " << ToUnderlying(dtype) << ", value: " << value;
        return "";
    }
};
