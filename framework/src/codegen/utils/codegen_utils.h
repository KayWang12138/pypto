...
struct FloatSpecVal {
    ...
    std::string GetFsTypeStr() const {
        if (std::isinf(value)) {
            return std::signbit(value) ? "inf_neg" : "inf_pos";
        } else if (std::isnan(value)) {
            return "nan";
        }
        ASSERT(false) << "FloatSpecValType not found: " << value;
    }

    std::string GetFsValStr() const {
        static const std::unordered_map<std::pair<DataType, bool>, std::string> infMap = {
        ...
        static const std::unordered_map<DataType, std::string> nanMap = {
