...
enum class FrameworkErrorScene : uint32_t {
...
enum class OperationErrorScene : uint32_t {
...
enum class GenOpCodeErrorScene : uint32_t {
...
enum class CompileCCEErrorScene : uint32_t {
    COMPILE_CCE_FAILED = ToUnderlying(CodeGenErrorCategory::COMPILE_CCE) + 1U,
    INCLUDE_FILE_NOT_FOUND = ToUnderlying(CodeGenErrorCategory::COMPILE_CCE) + 2U,
    PTO_ISA_NOT_FOUND = ToUnderlying(CodeGenErrorCategory::COMPILE_CCE) + 3U,
    CMD_CHECK_FAILED = ToUnderlying(CodeGenErrorCategory::COMPILE_CCE) + 4U,
    FILE_IO_FAILED = ToUnderlying(CodeGenErrorCategory::COMPILE_CCE) + 5U,
}

template <typename T, typename = std::enable_if_t<std::is_enum_v<T>>>
std::string GenErrorCode(T errCode) {
    return "ErrCode: F" + std::to_string(ToUnderlying(errCode)) + " !! ";
}
