...
std::vector<int64_t> NormalizeShape(const std::vector<int64_t> &shapeVec, unsigned dim) {
    ...
    for (size_t i = 0; i < shapeVec.size(); i++) {
        ASSERT(i < dim) << GenErrorCode(OperationErrorScene::TENSOR_DIM_EXCEEDED) << "exceed dimension limit!";
...
std::string GetTypeForB16B32(const DataType &dtype) {
    ...
    ASSERT(false) << GenErrorCode(GenOpCodeErrorScene::DATA_TYPE_UNSUPPORTED)
                  << "can not support dtype: " << DataType2String(dtype);
    return {};
}
...
std::string GetAddrTypeByOperandType(OperandType type) {
    ...
    ASSERT(false) << GenErrorCode(OperationErrorScene::OPERAND_TYPE_UNSUPPORTED)
                  << "cannot support current OperandType " << type;
    return "";
}
...
int64_t CalcLinearOffset(const std::vector<int64_t> &shape, const std::vector<int64_t> &offset) {
    if (shape.empty() || offset.empty() || shape.size() != offset.size()) {
        CODEGEN_LOGE("%s Invalid Input! shape: %s, offset: %s",
            GenErrorCode(GenOpCodeErrorScene::TENSOR_SHAPE_INVALID).c_str(), IntVecToStr(shape).c_str(),
