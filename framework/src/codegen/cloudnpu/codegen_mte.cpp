...
std::string CodeGenOpCloudNPU::PrintL1CopyInTileTensor(const PrintMemCopyWithL1Param &param) const {
    ...
    outerValue = outerValue == 0 ? param.gmShape[ID0] : outerValue;
    innerValue = innerValue == 0 ? param.gmShape[ID1] : innerValue;

    std::vector<std::string> tileOpParamList = {dstTensor, srcTensor, coord, outerValue, innerValue};
    int64_t copyInMode = -1;
