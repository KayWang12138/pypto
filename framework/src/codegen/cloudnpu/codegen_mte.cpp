...
std::string CodeGenOpCloudNPU::GenMemL1SpillToGM(bool isLocalToGM, unsigned int uf) const {
    ...
    ASSERT(l1Shape.size() == SHAPE_DIM2) << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED)
    ...
    ASSERT(ret >= 0) << GenErrorCode(GenCodeErr::PRINT_FAILED)
...
std::string CodeGenOpCloudNPU::PrintMemL1ToL0TileTensor() const {
    ...
    if (!dynOffset.empty()) {
        ASSERT(dynOffset.size() == SHAPE_DIM2 || dynOffset.size() == SHAPE_DIM3)
            << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED) << "GenMemL1ToL0 only support 2-dim or 3-dim!";
...
std::string CodeGenOpCloudNPU::GenMemL1ToL0() const {
    ...
    if (!dynoffset.empty()) {
        ASSERT(dynoffset.size() == SHAPE_DIM2)
            << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED) << "GenMemL1ToL0 only support 2-dim!";
...
std::string CodeGenOpCloudNPU::GenMemL1ToBt() const {
    ...
    ASSERT(dynoffset.size() == SHAPE_DIM2)
        << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED) << "GenMemL1ToBt only support 2-dim!";
    ...
    ASSERT(dynoffset[ID1].IsValid()) << GenErrorCode(GenCodeErr::TENSOR_OFFSET_INVALID)
...
std::string CodeGenOpCloudNPU::GenIndexOutCastOp() const {
    ASSERT(opAttrs.count(OpAttributeKey::cacheMode))
        << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "cannot get cacheMode attr";
    ASSERT(opAttrs.count(OpAttributeKey::panzBlockSize))
        << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "cannot get panzBlockSize attr";
    auto cacheMode = AnyCast<std::string>(opAttrs.at(OpAttributeKey::cacheMode));
    auto blockSize = AnyCast<int64_t>(opAttrs.at(OpAttributeKey::panzBlockSize));
    unsigned gmIdx = 0;
    unsigned localIdx = 1;
...
std::string CodeGenOpCloudNPU::PrintMemCopyWithL0CStatic(const PrintMemCopyWithL0CParam &param) const {
    ...
    ASSERT(printRet >= 0) << GenErrorCode(GenCodeErr::PRINT_FAILED)
...
std::string CodeGenOpCloudNPU::GenLoadOp() const {
    ...
    ASSERT(dstDtype == srcDtype) << GenErrorCode(GenCodeErr::DATA_TYPE_MISMATCHED)
    ...
    ASSERT(dstRawShapes == offsetsRawShapes)
        << GenErrorCode(GenCodeErr::TENSOR_SHAPE_MISMATCHED) << "raw shape must be same!";
    ASSERT(dstOriShapes.size() == offsetsOriShapes.size())
        << GenErrorCode(GenCodeErr::TENSOR_SHAPE_MISMATCHED) << "ori shape must be same!";
    ...
    ASSERT(offsetsDtypeStr == "int64_t" || offsetsDtypeStr == "int32_t")
        << GenErrorCode(GenCodeErr::DATA_TYPE_UNSUPPORTED)
    ...
    if (dstRawShapes.size() == SHAPE_DIM2) {
        ...
        ASSERT(false) << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED) << "unsupport dim "
                      << dstRawShapes.size() << " , only support 2 or 3 now.";
    }

    ASSERT(ret >= 0) << GenErrorCode(GenCodeErr::PRINT_FAILED) << "GenLoadOp sprintf_s failed ";
...
std::string CodeGenOpCloudNPU::GenMemL1ToFB() const {
    ...
    auto dynoffset = offsetFromAttr[ID1];
    ASSERT(dynoffset.size() == SHAPE_DIM2)
        << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED) << "GenMemL1ToFB only support 2-dim!";
...
std::string CodeGenOpCloudNPU::PrintGatherInL1TileTensor() const {
    ...
    ASSERT(startOffset.HasValue() && (startOffset.Type() == typeid(int64_t)))
        << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "GenGatherInL1 startOffset must be int64_t!";
...
std::string CodeGenOpCloudNPU::GenGatherInL1() const {
    ...
    ASSERT(srcRawShapes.size() == SHAPE_DIM2)
        << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED) << "GenGatherInL1 only support 2-dim!";
    ...
    ASSERT(dstDtypeStr == srcDtypeStr) << GenErrorCode(GenCodeErr::DATA_TYPE_MISMATCHED)
                                       << "dstDtypeStr and srcDtypeStr must be same!";
    ASSERT(offsetsDtypeStr == "int64_t" || offsetsDtypeStr == "int32_t")
        << GenErrorCode(GenCodeErr::DATA_TYPE_UNSUPPORTED) << "offsetsDtypeStr must be int64_t or int32_t!";
    ASSERT(opAttrs.find("op_attr_blocksize") != opAttrs.end())
        << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "GenGatherOp: There is nop blocksize attribute here";
    ...
    ASSERT(ret >= 0) << GenErrorCode(GenCodeErr::PRINT_FAILED) << "GenGatherInL1 sprintf_s failed ";
    std::string ostring(buffer);
    return ostring;
}
/**
...
std::string CodeGenOpCloudNPU::PrintGatherDynamicUnaligned() const {
    ...
    ASSERT(resultDtypeStr == paramDtypeStr)
        << GenErrorCode(GenCodeErr::DATA_TYPE_MISMATCHED) << "resultDtypeStr: " << resultDtypeStr
...
std::string CodeGenOpCloudNPU::GenGatherOp() const {
    ...
    ASSERT(false) << GenErrorCode(GenCodeErr::PRINT_MODE_ERROR)
...
std::string CodeGenOpCloudNPU::PrintGatherInUBDynamicUnaligned() const {
    ...
    std::string blockTableDtypeStr = DataType2CCEStr(operandDtype[ID3]);
    ASSERT(resultDtypeStr == paramDtypeStr)
        << GenErrorCode(GenCodeErr::DATA_TYPE_MISMATCHED) << "resultDtypeStr and paramDtypeStr must be same!";
    ASSERT(opAttrs.find(OpAttributeKey::blockSize) != opAttrs.end())
        << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "GenGatherOp: There is nop blockSize attribute here";
    const int64_t blockSize = AnyCast<int64_t>(opAttrs.at(OpAttributeKey::blockSize));
    auto outputRawShapes = rawShape[ID0];
...
std::string CodeGenOpCloudNPU::GetConvCopyInMode() const {
    ...
    ASSERT(ret) << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "GenMemL1CopyInConv get CopyInMode failed";
    ...
    if (copyInMode == ToUnderlying(CopyInMode::COPY_MOD_ND2NZ)) {
        ...
        ASSERT(false) << GenErrorCode(OperErr::ATTRIBUTE_INVALID)
...
std::string CodeGenOpCloudNPU::GenMemL1CopyInConv() const {
    ...
    if (isConv3D) {
        ASSERT(dynOffset.size() == SHAPE_DIM5) << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED)
                                               << "GenMemL1CopyInConv offset should be 5-dim!";
        ASSERT(srcShape.size() == SHAPE_DIM5)
            << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED) << "GenMemL1CopyInConv shape should be 5-dim!";
        ...
        ASSERT(dynOffset.size() == SHAPE_DIM4) << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED)
                                               << "GenMemL1CopyInConv offset should be 4-dim!";
        ASSERT(srcShape.size() == SHAPE_DIM4)
            << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED) << "GenMemL1CopyInConv shape should be 4-dim!";
        offsetN = dynOffset[ID0].Concrete();
        offsetC = dynOffset[ID1].Concrete();
...
std::string CodeGenOpCloudNPU::GetConvCopyOutMode() const {
    ...
    ASSERT(ret) << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "Get CopyOutMode failed";
    if (copyOutMode == ToUnderlying(CopyOutMode::COPY_MOD_NZ2ND)) {
        ...
        ASSERT(false) << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "Check CopyOutMode failed";
    }
...
std::string CodeGenOpCloudNPU::GenMemL1CopyOutConv() const {
    ...
    ASSERT(realShape.size() == SHAPE_DIM2) << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED)
...
std::string CodeGenOpCloudNPU::GenMemL1ToL0Load3D() const {
    ...
    ASSERT(fmapL0Shape.size() == SHAPE_DIM2) << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED)
...
std::string CodeGenOpCloudNPU::GenMemL1ToL0Load2D() const {
    ...
    ASSERT(weightL0Shape.size() == SHAPE_DIM2) << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED)
