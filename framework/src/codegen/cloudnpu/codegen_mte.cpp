...
std::string CodeGenOpCloudNPU::GenMemL1SpillToGM(bool isLocalToGM, unsigned int uf) const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, l1Shape.size() == SHAPE_DIM2) << "L1 shape must be 2-dim!";
    ...
    ASSERT(ERROR_CODE_UNKNOWN, ret >= 0) << "sprintf_s failed in genMemOp_L1, return value:" << ret;
    return buffer;
}
...
std::string CodeGenOpCloudNPU::PrintMemL1ToL0TileTensor() const {
    ...
    if (!dynOffset.empty()) {
        ASSERT(ERROR_CODE_UNKNOWN, dynOffset.size() == SHAPE_DIM2 || dynOffset.size() == SHAPE_DIM3)
...
std::string CodeGenOpCloudNPU::GenMemL1ToL0() const {
    ...
    if (!dynoffset.empty()) {
        ASSERT(ERROR_CODE_UNKNOWN, dynoffset.size() == SHAPE_DIM2) << "GenMemL1ToL0 only support 2-dim!";
...
std::string CodeGenOpCloudNPU::GenMemL1ToBt() const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, dynoffset.size() == SHAPE_DIM2) << "GenMemL1ToBt only support 2-dim!";
    ...
    ASSERT(ERROR_CODE_UNKNOWN, dynoffset[ID1].IsValid()) << "GenMemL1ToBt offset is invalid";
...
std::string CodeGenOpCloudNPU::GenIndexOutCastOp() const {
    ASSERT(ERROR_CODE_UNKNOWN, opAttrs.count(OpAttributeKey::cacheMode)) << "cannot get cacheMode attr";
    ASSERT(ERROR_CODE_UNKNOWN, opAttrs.count(OpAttributeKey::panzBlockSize)) << "cannot get panzBlockSize attr";
    auto cacheMode = AnyCast<std::string>(opAttrs.at(OpAttributeKey::cacheMode));
    auto blockSize = AnyCast<int64_t>(opAttrs.at(OpAttributeKey::panzBlockSize));
    unsigned gmIdx = 0;
    unsigned localIdx = 1;
...
std::string CodeGenOpCloudNPU::GenMemCopyVar(bool isCopyLocalToGM, bool isSpillToGm, unsigned uf) const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, 0) << "GenMemCopyVar: cannot support current localType!!!" << localType;
    return {};
}
...
std::string CodeGenOpCloudNPU::PrintMemCopyWithL0CStatic(const PrintMemCopyWithL0CParam &param) const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, printRet >= 0) << "sprintf_s failed in genMemCopyVar(BUF_L0C), return value:" << printRet;
    return buffer;
}
...
std::string CodeGenOpCloudNPU::PrintMemCopyWithL0CDynamic(const PrintMemCopyWithL0CParam &param) const {
    ...
    if (isSupportDynamicAligned) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, printRet >= 0) << "sprintf_s failed in PrintMemCopyWithL0CDynamic, return value:" << printRet;
        return buffer;
    }
...
std::string CodeGenOpCloudNPU::PrintMemCopyWithL1Static(const PrintMemCopyWithL1Param &param) const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, printRet >= 0) << "sprintf_s failed in PrintMemCopyWithL1Static, return value:" << printRet;
    int64_t nzValue = 0;
    ...
    if (ret && nzValue == 1) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, printRet >= 0) << "sprintf_s failed in genMemCopyVar, return value:" << printRet;
    } else {
...
std::string CodeGenOpCloudNPU::PrintMemCopyWithUBStatic(const PrintMemCopyWithUBParam &param) const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, printRet >= 0) << "sprintf_s failed in PrintMemCopyWithUBStatic, return value:" << printRet;
    return buffer;
}
...
std::string CodeGenOpCloudNPU::GenLoadOp() const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, dstDtype == srcDtype) << "src and dst dtype must be same!";
    ...
    ASSERT(ERROR_CODE_UNKNOWN, dstRawShapes == offsetsRawShapes) << "raw shape must be same!";
    ASSERT(ERROR_CODE_UNKNOWN, dstOriShapes.size() == offsetsOriShapes.size()) << "ori shape must be same!";
    ...
    ASSERT(ERROR_CODE_UNKNOWN, offsetsDtypeStr == "int64_t" || offsetsDtypeStr == "int32_t") << "offsets dtype must be int32_t or int64_t";
    ...
    if (dstRawShapes.size() == SHAPE_DIM2) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, false) << "unsupport dim " << dstRawShapes.size() << " , only support 2 or 3 now.";
    }

    ASSERT(ERROR_CODE_UNKNOWN, ret >= 0) << "GenLoadOp sprintf_s failed ";
...
std::string CodeGenOpCloudNPU::GenMemL1ToFB() const {
    ...
    auto dynValidShape = dynamicValidShape[ID0];
    auto dynoffset = offsetFromAttr[ID1];
    ASSERT(ERROR_CODE_UNKNOWN, dynoffset.size() == SHAPE_DIM2) << "GenMemL1ToFB only support 2-dim!";

    std::ostringstream os;
    std::vector<std::string> paramList;
    paramList.emplace_back(srcDtypeStr);
    // only need the valid offset of tail axis
    ASSERT(ERROR_CODE_UNKNOWN, dynoffset[ID1].IsValid()) << "GenMemL1TFB offset is invalid";
    paramList.emplace_back(SymbolicExpressionTable::BuildExpression(dynoffset[ID1]));
    std::string templateParam = JoinString(paramList, CONN_COMMA);
    paramList.clear();
...
std::string CodeGenOpCloudNPU::PrintGatherInL1TileTensor() const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, startOffset.HasValue() && (startOffset.Type() == typeid(int64_t)))
...
std::string CodeGenOpCloudNPU::GenGatherInL1() const {
    ...
    const DataType srcDtype = operandDtype[ID1];
    const DataType offsetsDtype = operandDtype[ID2];
    ASSERT(ERROR_CODE_UNKNOWN, dstDtype == srcDtype) << "dstDtype and srcDtype must be same!";
    ...
    auto offsetsRawShapes = rawShape[ID2];
    auto dstOriShapes = dynamicValidShape[ID0];
    ASSERT(ERROR_CODE_UNKNOWN, srcRawShapes.size() == SHAPE_DIM2) << "GenGatherInL1 only support 2-dim!";
    ASSERT(ERROR_CODE_UNKNOWN, dstRawShapes.size() == SHAPE_DIM2) << "GenGatherInL1 only support 2-dim!";
    ASSERT(ERROR_CODE_UNKNOWN, offsetsRawShapes.size() == SHAPE_DIM2) << "GenGatherInL1 only support 2-dim!";
    ASSERT(ERROR_CODE_UNKNOWN, dstOriShapes.size() == SHAPE_DIM2) << "GenGatherInL1 only support 2-dim!";
    ...
    ASSERT(ERROR_CODE_UNKNOWN, dstDtypeStr == srcDtypeStr);
    ASSERT(ERROR_CODE_UNKNOWN, offsetsDtypeStr == "int64_t" || offsetsDtypeStr == "int32_t");
    ASSERT(ERROR_CODE_UNKNOWN, opAttrs.find("op_attr_blocksize") != opAttrs.end()) << "GenGatherOp: There is nop axis attribute here";
    ...
    ASSERT(ERROR_CODE_UNKNOWN, ret >= 0) << "GenGatherInL1 sprintf_s failed ";
    std::string ostring(buffer);
    return ostring;
}
/**
...
std::string CodeGenOpCloudNPU::PrintGatherDynamicUnaligned() const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, resultDtypeStr == paramDtypeStr);
...
std::string CodeGenOpCloudNPU::GenGatherOp() const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, false) << "Gather operator does not support static graph";
    return "";
}
...
std::string CodeGenOpCloudNPU::PrintGatherInUBLayout() const {
    ...
    std::vector<std::string> paramList;
    ASSERT(ERROR_CODE_UNKNOWN, opAttrs.find(OpAttributeKey::blockSize) != opAttrs.end()) << "GenGatherOp: There is nop axis attribute here";
...
std::string CodeGenOpCloudNPU::GetConvCopyInMode() const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, ret) << "GenMemL1CopyInConv get CopyInMode failed";
    ...
    if (copyInMode == ToUnderlying(CopyInMode::COPY_MOD_ND2NZ)) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, false) << "GenMemL1CopyInConv check CopyInMode failed";
    }
...
std::string CodeGenOpCloudNPU::GenMemL1CopyInConv() const {
    ...
    if (isConv3D) {
        ASSERT(ERROR_CODE_UNKNOWN, dynOffset.size() == SHAPE_DIM5) << "GenMemL1CopyInConv offset should be 5-dim!";
        ASSERT(ERROR_CODE_UNKNOWN, srcShape.size() == SHAPE_DIM5) << "GenMemL1CopyInConv shape should be 5-dim!";
        ...
        ASSERT(ERROR_CODE_UNKNOWN, dynOffset.size() == SHAPE_DIM4) << "GenMemL1CopyInConv offset should be 4-dim!";
        ASSERT(ERROR_CODE_UNKNOWN, srcShape.size() == SHAPE_DIM4) << "GenMemL1CopyInConv shape should be 4-dim!";
        offsetN = dynOffset[ID0].Concrete();
        offsetC = dynOffset[ID1].Concrete();
...
std::string CodeGenOpCloudNPU::GetConvCopyOutMode() const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, ret) << "Get CopyOutMode failed";
    if (copyOutMode == ToUnderlying(CopyOutMode::COPY_MOD_NZ2ND)) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, false) << "Check CopyOutMode failed";
    }
...
std::string CodeGenOpCloudNPU::GenMemL1CopyOutConv() const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, realShape.size() == SHAPE_DIM2) << "GenMemL1CopyOutConv valid shape should be 2-dim!";
    realM = realShape[ID0];
    realN = realShape[ID1];
    auto dynOffset = offsetFromAttr[ToUnderlying(MISOIdx::DST_IDX)];
    if (isConv3D) {
        ASSERT(ERROR_CODE_UNKNOWN, dynOffset.size() == SHAPE_DIM5) << "GenMemL1CopyOutConv offset should be 5-dim!";
        offsetN = dynOffset[ID0].Concrete();
        offsetC = dynOffset[ID1].Concrete();
        offsetD = dynOffset[ID2].Concrete();
        offsetH = dynOffset[ID3].Concrete();
        ...
        ASSERT(ERROR_CODE_UNKNOWN, dynOffset.size() == SHAPE_DIM4) << "GenMemL1CopyOutConv offset should be 4-dim!";
        offsetN = dynOffset[ID0].Concrete();
        offsetC = dynOffset[ID1].Concrete();
        offsetH = dynOffset[ID2].Concrete();
        offsetW = dynOffset[ID3].Concrete();
...
std::string CodeGenOpCloudNPU::GenMemL1ToL0Load3D() const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, fmapL0Shape.size() == SHAPE_DIM2) << "GenMemL1ToL0Load3D L0 fmap only support 2-dim!";
...
std::string CodeGenOpCloudNPU::GenMemL1ToL0Load2D() const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, weightL0Shape.size() == SHAPE_DIM2) << "GenMemL1ToL0Load2D L0 weight only support 2-dim!";
