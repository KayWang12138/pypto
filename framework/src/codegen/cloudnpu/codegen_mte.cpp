...
DynamicParamPackMTE CodeGenOpCloudNPU::PrepareDynamicShapeInfoForMTE(
    ...
    if (isGmSpill) {
        ...
        pack.gmShapeExpr = GenDynRawShapePacked(dynShapeIdx, dim, PREFIX_STR_RAW_SHAPE);
    }
...
std::string CodeGenOpCloudNPU::PrintMemCopyWithL0CDynamic(const PrintMemCopyWithL0CParam &param) const {
    ...
    std::vector<std::string> gmShapeExpr = GenDynRawShapePacked(param.gmIdx, SHAPE_DIM2, PREFIX_STR_RAW_SHAPE);
