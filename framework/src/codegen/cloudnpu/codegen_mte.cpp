...
std::string CodeGenOpCloudNPU::PrintTensorForCopyBetweenGM(
    unsigned oprandIdx, unsigned gmIdx, const std::string &gmVarName) const {
    std::string tensor = oprandIdx == gmIdx ? sm->QueryTileTensorByBufVarName(gmVarName) :
                                              sm->QueryTileTensorByMagic(operandWithMagic[oprandIdx]);
    return tensor;
}
