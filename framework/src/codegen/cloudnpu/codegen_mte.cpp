...
void NormalizeGatherShape(std::vector<T> &rawShape, const int paramDim, const int indicesDim, const int axis) {
    static_assert(
        (std::is_same_v<T, int64_t> || std::is_same_v<T, SymbolicScalar>), "T must be int64_t or SymbolicScalar");
...
std::string CodeGenOpCloudNPU::GenGatherOp() const {
    ...
    ASSERT(false) << GenErrorCode(GenCodeErr::PRINT_MODE_ERROR) << "Gather operator does not support static graph";
    return "";
}
...
std::string CodeGenOpCloudNPU::GetConvCopyInMode() const {
    ...
    if (copyInMode == ToUnderlying(CopyInMode::COPY_MOD_ND2NZ)) {
        ...
        ASSERT(false) << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "GenMemL1CopyInConv check CopyInMode failed";
    }
...
std::string CodeGenOpCloudNPU::GenMemL1CopyInConv() const {
    ...
    if (isConv3D) {
        ASSERT(dynOffset.size() == SHAPE_DIM5)
            << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED) << "GenMemL1CopyInConv offset should be 5-dim!";
        ...
        ASSERT(dynOffset.size() == SHAPE_DIM4)
            << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED) << "GenMemL1CopyInConv offset should be 4-dim!";
...
std::string CodeGenOpCloudNPU::GenMemL1CopyOutConv() const {
    ...
    ASSERT(realShape.size() == SHAPE_DIM2)
        << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED) << "GenMemL1CopyOutConv valid shape should be 2-dim!";
    realM = realShape[ID0];
    realN = realShape[ID1];
    auto dynOffset = offsetFromAttr[ToUnderlying(MISOIdx::DST_IDX)];
    if (isConv3D) {
        ASSERT(dynOffset.size() == SHAPE_DIM5)
            << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED) << "GenMemL1CopyOutConv offset should be 5-dim!";
        offsetN = dynOffset[ID0].Concrete();
        offsetC = dynOffset[ID1].Concrete();
        offsetD = dynOffset[ID2].Concrete();
        offsetH = dynOffset[ID3].Concrete();
        ...
        ASSERT(dynOffset.size() == SHAPE_DIM4)
            << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED) << "GenMemL1CopyOutConv offset should be 4-dim!";
        offsetN = dynOffset[ID0].Concrete();
        offsetC = dynOffset[ID1].Concrete();
        offsetH = dynOffset[ID2].Concrete();
        offsetW = dynOffset[ID3].Concrete();
...
std::string CodeGenOpCloudNPU::GenMemL1ToL0Load3D() const {
    ...
    ASSERT(fmapL0Shape.size() == SHAPE_DIM2)
        << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED) << "GenMemL1ToL0Load3D L0 fmap only support 2-dim!";
...
std::string CodeGenOpCloudNPU::GenMemL1ToL0Load2D() const {
    ...
    ASSERT(weightL0Shape.size() == SHAPE_DIM2)
        << GenErrorCode(GenCodeErr::TENSOR_DIM_UNSUPPORTED) << "GenMemL1ToL0Load2D L0 weight only support 2-dim!";
