...
std::string CodeGenOpCloudNPU::PrintRowMaxlineStatic(const PrintUnaryParam &param) const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, ((reduceAxis >= 0) && (reduceAxis < (int(rawShape[1].size()) - 1))))
...
std::string CodeGenOpCloudNPU::PrintRowMaxlineTileTensor() const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, ((reduceAxis >= 0) && (reduceAxis < (int(rawShape[1].size()) - 1)))) << "unsupported reduce axis";
...
std::string CodeGenOpCloudNPU::PrintExpand(const std::string &s0Var, const std::string &dVar,
    ...
    ASSERT(ERROR_CODE_UNKNOWN, (expandAxis >= 0) && (expandAxis <= (static_cast<int>(rawShape[1].size() - 1))))
    ...
    ASSERT(ERROR_CODE_UNKNOWN, ret >= 0) << "GenUnaryOp" << OpcodeManager::Inst().GetOpcodeStr(opCode) << " sprintf_s failed " << ret;
    return buffer;
}
