...
std::string CodeGenOpCloudNPU::PrintUnaryDynamicUnaligned(const PrintUnaryParam& param) const
{
    ...
    if (opCode == Opcode::OP_EXP || opCode == Opcode::OP_SQRT) {
        int64_t precisionType = 0;
        (void)GetAttr(OpAttributeKey::precisionType, precisionType);
        paramList.emplace_back(std::to_string(precisionType));
    }
...
std::string CodeGenOpCloudNPU::PrintUnaryTileTensor() const
{
    ...
    if (opCode == Opcode::OP_EXP || opCode == Opcode::OP_SQRT) {
        ...
        if (precisionType == 1) { // HIGH_PRECISION
            templateParamList.emplace_back("ExpAlgorithm::HIGH_PRECISION");
        } else {
            templateParamList.emplace_back("ExpAlgorithm::DEFAULT");
        }
    }
