...
std::string CodeGenOpCloudNPU::PrintReduceLastAxis(const PrintUnaryTmpBuffParam &param) const {
    ...
    ASSERT(ret >= 0) << GenErrorCode(GenOpCodeErrorScene::PRINT_FAILED) << "PrintReduceLastAxis"
                     << OpcodeManager::Inst().GetOpcodeStr(opCode) << " sprintf_s failed " << ret;
    return buffer;
}
...
std::string CodeGenOpCloudNPU::PrintExp2() const {
    ASSERT(isSupportLayout) << GenErrorCode(OperationErrorScene::PRINT_MODE_ERROR) << "Exp2 only support tile tensor";
...
std::string CodeGenOpCloudNPU::PrintRound() const {
    ASSERT(isSupportLayout) << GenErrorCode(GenOpCodeErrorScene::PRINT_MODE_ERROR) << "Round only support tile tensor";
...
std::string CodeGenOpCloudNPU::PrintExpm1() const {
    ASSERT(isSupportLayout) << GenErrorCode(GenOpCodeErrorScene::PRINT_MODE_ERROR) << "Expm1 only support tile tensor";
...
std::string CodeGenOpCloudNPU::PrintRowSumlineStatic(const PrintUnaryTmpBuffParam &param) const {
    ...
    ASSERT(((reduceAxis >= 0) && (reduceAxis < (int(rawShape[ID2].size()) - 1))))
        << GenErrorCode(OperationErrorScene::ATTRIBUTE_INVALID) << "unsupported reduce axis" << reduceAxis;
...
std::string CodeGenOpCloudNPU::PrintRowSumlineTileTensor() const {
    ...
    ASSERT(((reduceAxis >= 0) && (reduceAxis < (int(rawShape[ID2].size()) - 1))))
        << GenErrorCode(OperationErrorScene::ATTRIBUTE_INVALID) << "unsupported reduce axis";
