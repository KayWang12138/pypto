...
std::string CodeGenOpCloudNPU::PrintSortTileTensor() const {
    ...
    std::ostringstream oss;
    oss << tileOpName << WrapParamByAngleBrackets(GenOpAttr(false)) << WrapParamByParentheses({dstTensor, srcTensor})
        << STMT_END;
    return oss.str();
}
...
std::string CodeGenOpCloudNPU::PrintExtractTileTensor() const {
    ...
    std::ostringstream oss;
    oss << tileOpName << WrapParamByAngleBrackets(GenOpAttr(false)) << WrapParamByParentheses({dstTensor, src0Tensor})
        << STMT_END;
    return oss.str();
}
