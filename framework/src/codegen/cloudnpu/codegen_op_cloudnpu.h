...
private:
    ...
    std::vector<T> GetShapeInLoop(const std::vector<T> &input) {
        ASSERT(ERROR_CODE_UNDEFINED, input.size() > SHAPE_DIM2) << "input size " << input.size() << " is less than 2";
    ...
    bool GetAttr(const std::string &key, T &value) const {
        ...
        CODEGEN_LOGE(ERROR_CODE_UNDEFINED, "Type of attribute %s from PASS is mismatch: %s != %s", key.c_str(), it->second.Type().name(),
            typeid(T).name());
        return false;
    }
