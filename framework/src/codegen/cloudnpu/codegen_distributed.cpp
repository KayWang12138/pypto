...
RawSymbolicScalarPtr FormatSymbol(const RawSymbolicScalarPtr &rawScalar)
{
    if (rawScalar->IsSymbol()) {
        ...
        auto rawExpresssion = std::dynamic_pointer_cast<RawSymbolicScalarPtr>(rawScalar);
...
std::string CodeGenOpCloudNPU::GenDynOffset(int32_t operandIndex, int32_t dim) const
{
    ...
    for (int32_t index = 0; index < dim; ++index) {
        RawSymbolicScalarPtr rawScalar = offsetFromAttr[operandIndex][index].Raw();
...
std::string CodeGenOpCloudNPU::GenOffsetsAndRawShapesForShmemPut() const
{
    ...
    oss << ", " << GenOffsetsAndRawShapes(nonShmemDataIndex, nonShmemDataDim) << ", " << GenDynValidShape(validShapeIndex, nonShmemDataDim) <<
        ", " << GenDynOffset(shmemDataIndex, shmemDataDim) << ", " << GenRawShapes(shmemDataIndex, shmemDataDim);
    return oss.str();
}
...
std::string CodeGenOpCloudNPU::GenOffsetsAndRawShapesForShmemGet() const
{
    ...
    int32_t nonShmemDataDim = originShape[nonShmemDataIndex].size();
    int32_t shmemDataDim = 4;
    oss << ", " << GenDynOffset(nonShmemDataIndex, nonShmemDataDim) << ", " << GenRawShapes(nonShmemDataIndex, nonShmemDataDim) << ", " << GenOffsetsAndRawShapes(shmemDataIndex, shmemDataDim) <<
        ", " << GenDynValidShape(nonShmemDataIndex, nonShmemDataDim);
    return oss.str();
}
