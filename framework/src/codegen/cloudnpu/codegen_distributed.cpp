...
static bool IsDynOffsetValid(std::vector<SymbolicScalar> dynOffset, size_t dim)
{
    bool isDynOfffsetValid = true;
    for (size_t index = 0; index < dim; ++index) {
        isDynOfffsetValid = isDynOfffsetValid && dynOffset[index].IsValid();
    }
    return isDynOfffsetValid;
}
...
std::string CodeGenOpCloudNPU::GenDynOffset(int32_t operandIndex) const
{
    ...
    if (IsDynOffsetValid(offsetFromAttr[operandIndex], dim)) {
         for (size_t index = 0; index < dim; ++index) {
        ...
        oss << GenGetParamMacroPacked(operandIndex, dim, PREFIX_STR_OFFSET)[0];
    }
    return oss.str();
}
