...
std::string CodeGenOpCloudNPU::GenOffsetsAndRawShapesForShmemGet() const {
    ...
    int32_t nonShmemDataDim = originShape[nonShmemDataIndex].size();
    int32_t shmemDataDim = 4;
    oss << ", " << GenDynOffset(nonShmemDataIndex, nonShmemDataDim) << ", " << GenRawShapes(nonShmemDataIndex, nonShmemDataDim) <<
        ", " << GenOffsetsAndRawShapes(shmemDataIndex, shmemDataDim) << ", " << GenDynValidShape(nonShmemDataIndex, nonShmemDataDim);
    return oss.str();
}
