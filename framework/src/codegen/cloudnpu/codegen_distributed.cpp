...
static RawSymbolicScalarPtr FormatSymbol(const RawSymbolicScalarPtr& rawScalar)
{
...
std::string CodeGenOpCloudNPU::GenOffsetsAndRawShapesForShmemPut() const
{
    ...
    ASSERT(GenCodeErr::TENSOR_DIM_UNSUPPORTED, shmemTensorDim >= 2)
        << "shmem tensor dim = " << shmemTensorDim << ", should >= 2.";
    std::string viewOffsetStr = FormatSymbol(dynamicValidShape[shmemDataIndex][shmemTensorDim - 2].Raw())->Dump();
