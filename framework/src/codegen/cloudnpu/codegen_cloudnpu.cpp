        ...
        std::function task = [this, subFuncPair, &topFunc]() {
            ...
            if (config::GetRuntimeOption<int64_t>(CFG_RUN_MODE) != CFG_RUN_MODE_SIM) {
                ...
                if (config::GetRuntimeOption<int64_t>(CFG_RUN_MODE) == CFG_RUN_MODE_COMPILE_ONLY&&
                    config::GetHostOption<std::string>(COMPILE_STAGE) == COMPILE_STAGE_CODEGEN_INSTRUCTION) {
                    ALOG_INFO("Compilation stage terminates after codegen instruction.");
                    return;
                }
                DoCompileCCE(compileInfo, "");
                if (config::GetRuntimeOption<int64_t>(CFG_RUN_MODE) == CFG_RUN_MODE_COMPILE_ONLY&&
                    config::GetHostOption<std::string>(COMPILE_STAGE) == COMPILE_STAGE_CODEGEN_BINARY) {
