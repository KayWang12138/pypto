...
void CodeGenCloudNPU::GenerateMakefile(const std::string &makefilePath) const {
    ...
    for (const auto &task : compileTasks_) {
        ...
        makefile << "\t@/usr/bin/time -f \"%e\" -o " << task.outputPath << ".time " << task.compileCmd << " 2>&1\n\n";
    }
...
void CodeGenCloudNPU::ExecuteParallelCompile(const Function &topFunc) {
    ...
    // Log compilation time for each task
    for (const auto &task : compileTasks_) {
        std::string timeFilePath = task.outputPath + ".time";
        std::ifstream timeFile(timeFilePath);
        if (timeFile.is_open()) {
            double compileTime = 0.0;
            timeFile >> compileTime;
            timeFile.close();
            CODEGEN_LOGI("Compilation task: %s, time: %.3f seconds", task.outputPath.c_str(), compileTime);
        } else {
            CODEGEN_LOGW("Failed to read time file: %s", timeFilePath.c_str());
        }
    }
