...
void CodeGenCloudNPU::GenerateMakefile(const std::string &makefilePath) const {
    ...
    makefile << "# Generated at: " << __DATE__ << " " << __TIME__ << "\n\n";
...
void CodeGenCloudNPU::ExecuteParallelCompile() {
    if (compileTasks_.empty()) {
    ...
    std::string makefilePath = outputDir + "/Makefile.compile";
    
    GenerateMakefile(makefilePath);
    
    unsigned parallelJobs = ConfigManager::Instance().GetCodeGenConfig(KEY_PARALLEL_COMPILE, 128u);
    std::string makeCmd = "make -j" + std::to_string(parallelJobs) + " -f " + makefilePath;
    
    CODEGEN_LOGI("Starting parallel compilation: %u jobs, %zu tasks", parallelJobs, compileTasks_.size());
    CODEGEN_LOGI("Execute: %s", makeCmd.c_str());
    
    auto startTime = std::chrono::high_resolution_clock::now();
    int ret = DoCompileCmd(makeCmd);
    auto endTime = std::chrono::high_resolution_clock::now();
    
    auto duration = std::chrono::duration_cast<std::chrono::seconds>(endTime - startTime);
    CODEGEN_LOGI("Parallel compilation finished in %ld seconds", duration.count());
