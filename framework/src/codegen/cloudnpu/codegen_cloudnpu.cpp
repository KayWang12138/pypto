...
void CodeGenCloudNPU::GenFuncBody(Function &subFunc, Function &topFunc, std::ostringstream &oss) const {
    ...
    for (const auto &op : operationList) {
        ...
        ASSERT(tileOpSourceCode.find("CG_ERROR") == tileOpSourceCode.npos)
            << GenErrorCode(GenOpCodeErrorScene::GEN_OP_CODE_FAILED) << "Generate code of op failed, op is "
            << op.Dump();
...
void CodeGenCloudNPU::DumpCCE(const std::string &fileName, std::ostringstream &oss) const {
    ...
    try {
        ...
        CODEGEN_LOGE("%s CCE file operation failed: %s, error: %s, errno: %d",
            GenErrorCode(CompileCCEErrorScene::FILE_IO_FAILED).c_str(), fileName.c_str(), e.what(), errno);
        cceFile.close();
...
std::optional<std::string> CodeGenCloudNPU::GenExtraAlloc(
    ...
    if (OPERAND_TYPE_TO_MEMORY_TYPE.find(memType) == OPERAND_TYPE_TO_MEMORY_TYPE.end()) {
        CODEGEN_LOGE("%s memory type(%zu) of tensor from PASS is invalid, tensor is: %s",
            GenErrorCode(OperationErrorScene::OPERAND_TYPE_UNSUPPORTED).c_str(), static_cast<size_t>(memType),
            tensor->Dump().c_str());
        return std::nullopt;
    }
...
void CodeGenCloudNPU::DoCompileCCE(const CompileInfo &compileInfo, const std::string &compileOptions) const {
    ...
    ASSERT(ret == 0) << GenErrorCode(CompileCCEErrorScene::COMPILE_CCE_FAILED) << "CompileCCE failed. errCode = " << ret
...
std::string CodeGenCloudNPU::GetIncludePathForCompileCCE() const {
    ...
    ASSERT(false) << GenErrorCode(CompileCCEErrorScene::INCLUDE_FILE_NOT_FOUND)
...
std::string CodeGenCloudNPU::GetPtoTileLibPathByEnv() const {
    ...
    if (homePath != nullptr) {
        ...
        ASSERT(IsPathExist(envPath + "/pto")) << GenErrorCode(CompileCCEErrorScene::PTO_ISA_NOT_FOUND)
    ...
    if (homePath != nullptr) {
        ...
        ASSERT(IsPathExist(cannPath + "/pto")) << GenErrorCode(CompileCCEErrorScene::PTO_ISA_NOT_FOUND)
                                               << "Pto-isa path " << cannPath << "/pto not found! please check.";
        return cannPath;
    }

    ASSERT(false) << GenErrorCode(CompileCCEErrorScene::PTO_ISA_NOT_FOUND)
...
std::pair<int, std::string> CodeGenCloudNPU::CompileCCE(
    ...
    ASSERT(ret == 0) << GenErrorCode(CompileCCEErrorScene::CMD_CHECK_FAILED)
                     << "CheckInjectStr failed. errCode = " << ret;

    ret = std::system(bishengCmd.c_str());
    if (ret != 0) {
        CODEGEN_LOGE("%s: Compile cce kernel failed, ret = %d\ncompile cmd is:\n %s",
            GenErrorCode(CompileCCEErrorScene::COMPILE_CCE_FAILED).c_str(), ret, bishengCmd.c_str());
    }
