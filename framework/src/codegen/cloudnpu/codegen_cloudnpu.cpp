...
std::pair<int, std::string> CodeGenCloudNPU::CompileCCE(const CompileInfo &compileInfo, const std::string &funcName,
    ...
    CODEGEN_LOGE("Compile cce kernel funcName = %s, funcHash = %s, cost %f ms", funcName.c_str(), funcHash.c_str(),
        duration.count());
    if (ret != 0) {
        CODEGEN_LOGE("Compile cce kernel failed, ret = %d, duration = %ld ms, funcName = %s, funcHash = %s\ncompile "
                     "cmd is:\n %s",
            ret, duration.count(), funcName.c_str(), funcHash.c_str(), ccecCmd.c_str());
    }
