...
void CodeGenCloudNPU::GenFuncBody(Function &subFunc, Function &topFunc, std::ostringstream &oss) const {
    ...
    for (const auto &op : operationList) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, tileOpSourceCode.find("CG_ERROR") == tileOpSourceCode.npos)
...
std::string CodeGenCloudNPU::GenAlloc(
    ...
    if ((BUFFER_TYPE_TO_PREFIX.count(bufferType) == 0) || (OPERAND_TYPE_TO_ADDR_TYPE.count(bufferType) == 0)) {
        ASSERT(ERROR_CODE_UNKNOWN, false) << "invalid bufferType: " << static_cast<size_t>(bufferType);
        return "";
    }
...
void CodeGenCloudNPU::DoCompileCCE(const CompileInfo &compileInfo, const std::string &compileOptions) const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, ret == 0) << "CompileCCE failed. errCode = " << ret << ", cce file: " << compileInfo.GetCCEAbsPath()
...
std::string CodeGenCloudNPU::GetIncludePathForCompileCCE() const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, false) << "include path for compiling cce is unavailable";
    return "";
}
...
std::string CodeGenCloudNPU::GetPtoTileLibPathByEnv() const {
    ...
    if (homePath != nullptr) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, IsPathExist(envPath + "/pto")) << "Pto-isa path " << envPath << "/pto not found! please check.";
        return envPath;
    }
    ...
    if (homePath != nullptr) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, IsPathExist(cannPath + "/pto")) << "Pto-isa path " << cannPath << "/pto not found! please check.";
        return cannPath;
    }

    ASSERT(ERROR_CODE_UNKNOWN, false) << "Pto-isa path not found. please install pto-isa properly.";
    return "";
}
...
std::pair<int, std::string> CodeGenCloudNPU::CompileCCE(
    ...
    ASSERT(ERROR_CODE_UNKNOWN, ret == 0) << "CheckInjectStr failed. errCode = " << ret;

    ret = std::system(ccecCmd.c_str());
    if (ret != 0) {
        CODEGEN_LOGE(ERROR_CODE_UNDEFINED, "Compile cce kernel failed, ret = %d\ncompile cmd is:\n %s", ret, ccecCmd.c_str());
    }
