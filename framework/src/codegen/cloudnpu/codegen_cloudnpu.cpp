...
void CodeGenCloudNPU::GenCode(
    ...
    COMPILER_LOGE("hjhj start DumpCCE and DoCompileCCE to binary");
    std::deque<std::function<void(void)>> tasks;
    for (auto &subFuncPair : topFunc.rootFunc_->programs_) {
        std::function task = [this, subFuncPair, &topFunc]() {
            auto start = std::chrono::high_resolution_clock::now();
            COMPILER_LOGE("hjhj process subFunc start ----- subprogram id [%lu] -----", subFuncPair.first);
            ...
            if (std::getenv(ENV_ASCEND_HOME_PATH.c_str()) != nullptr) {
                ...
                CODEGEN_LOGE("hjhj process subFunc done %s, hash %s, Print cost %f ms, DumpCCE cost: %f ms, "
                             "DoCompileCCE cost %f ms",
    ...
    CODEGEN_LOGE("hjhj threadNum is %u", threadNum);
...
std::pair<int, std::string> CodeGenCloudNPU::CompileCCE(const CompileInfo &compileInfo, const std::string &funcName,
    ...
    CODEGEN_LOGE_FULL("Compile cce kernel funcName = %s, funcHash = %s, cost %f ms\ncompile cmd is:\n %s",
        funcName.c_str(), funcHash.c_str(), duration, ccecCmd.c_str());
    if (ret != 0) {
        CODEGEN_LOGE_FULL(
            "Compile cce kernel failed, ret = %d, duration = %f ms, funcName = %s, funcHash = %s\ncompile "
            "cmd is:\n %s",
