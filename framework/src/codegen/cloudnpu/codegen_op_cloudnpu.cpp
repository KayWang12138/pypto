...
TileTensor CodeGenOpCloudNPU::BuildTileTensor(
    ...
    if (shapeInLoop.loopDepth != 0) {
        std::string tensorName = sm->QueryTileTensorFullDimByMagic(tileTensor.magic);
        if (!tensorName.empty()) {
...
void CodeGenOpCloudNPU::UpdateTileTensorInfo() {
    ...
    if (GetAttr(OpAttributeKey::loopAxes, loopAxes) && !loopAxes.empty()) {
        ...
        int loopDepth = static_cast<int>(loopAxes.size());
        for (int i = 0; i < operandCnt; ++i) {
            ...
            TileTensorUsing tileTensorUsing{operandDtype[i], operandType[i],
...
ShapeInLoop CodeGenOpCloudNPU::BuildShapeInLoop(int paramIdx, int loopDepth) {
