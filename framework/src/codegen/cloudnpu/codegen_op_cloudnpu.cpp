...
void CodeGenOpCloudNPU::AppendLocalBufferVarOffset(
    ...
    for (auto &kv : vars) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, !varRawShape.empty()) << "varRawShape is empty!! operandIdx: " << operandIdx;
        ASSERT(ERROR_CODE_UNDEFINED, varOffset.size() == varRawShape.size())
        ...
        ASSERT(ERROR_CODE_UNDEFINED, !var.empty()) << "operandIdx: " << operandIdx << ", var is empty !!";
...
SymbolicScalar CodeGenOpCloudNPU::GetOperandStartOffset(int operandIdx) const {
    ...
    if (!dynOffset.empty()) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, !varRawShape.empty()) << "varRawShape is empty!!";
        ASSERT(ERROR_CODE_UNDEFINED, dynOffset.size() == varRawShape.size())
        ...
        ASSERT(ERROR_CODE_UNDEFINED, operandIdx < operandCnt) << "operandIdx: " << operandIdx << ", operandCnt: " << operandCnt;
...
std::string CodeGenOpCloudNPU::GenGmParamVar(unsigned gmParamIdx) const {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, iter != paramLocToParamListOffset.end())
...
void CodeGenOpCloudNPU::UpdateTileTensorInfo() {
    ...
    if (iter == SUPPORT_TILETENSOR_OPS.end()) {
        ASSERT(ERROR_CODE_UNDEFINED, iter != SUPPORT_TILETENSOR_OPS.end()) << "opCode: " << opCodeStr << " not support tile tensor!";
        return;
    }
...
TileTensor CodeGenOpCloudNPU::QueryTileTensorByIdx(int paramIdx) const {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, false) << "TileTensor: paramIdx " << paramIdx << ", tensor magic " << operandWithMagic[paramIdx]
...
std::string CodeGenOpCloudNPU::GetLastUse() const {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, valSize != 0) << "GetLastUse error!!!";
