...
void CodeGenOpCloudNPU::AppendLocalBufferVarOffset(
    ...
    for (auto &kv : vars) {
        ...
        ASSERT(!varRawShape.empty()) << GenErrorCode(GenCodeErr::TENSOR_SHAPE_INVALID)
                                     << "varRawShape is empty!! operandIdx: " << operandIdx;
        ASSERT(varOffset.size() == varRawShape.size())
            << GenErrorCode(GenCodeErr::TENSOR_SHAPE_MISMATCHED) << "varOffset " << IntVecToStr(varOffset)
        ...
        ASSERT(!var.empty()) << GenErrorCode(GenCodeErr::SYMBOL_NOT_FOUND) << "operandIdx: " << operandIdx
                             << ", var is empty !!";
...
SymbolicScalar CodeGenOpCloudNPU::GetOperandStartOffset(int operandIdx) const {
    ...
    if (!dynOffset.empty()) {
        ...
        ASSERT(dynOffset.size() == varRawShape.size())
            << GenErrorCode(GenCodeErr::TENSOR_SHAPE_MISMATCHED) << "dynOffset " << SymbolicVecToStr(dynOffset)
        ...
        ASSERT(operandIdx < operandCnt) << GenErrorCode(OperErr::OPERAND_COUNT_EXCEEDED)
...
std::string CodeGenOpCloudNPU::GenGmParamVar(unsigned gmParamIdx) const {
    ...
    ASSERT(iter != paramLocToParamListOffset.end())
        << GenErrorCode(GenCodeErr::PARAM_IDX_INVALID) << "paramLoc " << paramLoc
...
void CodeGenOpCloudNPU::UpdateTileTensorInfo() {
    ...
    if (iter == SUPPORT_TILETENSOR_OPS.end()) {
        ASSERT(iter != SUPPORT_TILETENSOR_OPS.end()) << GenErrorCode(GenCodeErr::OP_CODE_UNSUPPORTED)
...
TileTensor CodeGenOpCloudNPU::QueryTileTensorByIdx(int paramIdx) const {
    ...
    ASSERT(false) << GenErrorCode(GenCodeErr::TENSOR_NOT_FOUND) << "TileTensor: paramIdx " << paramIdx
                  << ", tensor magic " << operandWithMagic[paramIdx] << " is not found !!! res size is " << res.size();
...
std::string CodeGenOpCloudNPU::GetLastUse() const {
    ...
    ASSERT(valSize != 0) << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "GetLastUse error!!!";
