...
void CodeGenOpCloudNPU::AppendLocalBufferVarOffset(
    ...
    for (auto &kv : vars) {
        ...
        ASSERT(varOffset.size() == varRawShape.size())
            << GenErrorCode(GenCodeErr::TENSOR_SHAPE_MISMATCHED) << "varOffset " << IntVecToStr(varOffset) << ", size "
            << varOffset.size() << " vs varRawShape " << IntVecToStr(varRawShape) << ", size " << varRawShape.size()
...
SymbolicScalar CodeGenOpCloudNPU::GetOperandStartOffset(int operandIdx) const {
    ...
    if (!dynOffset.empty()) {
        ...
        ASSERT(!varRawShape.empty()) << GenErrorCode(GenCodeErr::TENSOR_SHAPE_INVALID) << "varRawShape is empty!!";
