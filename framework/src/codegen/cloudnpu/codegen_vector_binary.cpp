...
std::string CodeGenOpCloudNPU::GenBinaryWithBrc() const {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, ret >= 0) << "GenBinaryWithBrc sprintf_s failed ";
    return buffer;
}
...
std::string CodeGenOpCloudNPU::PrintVectorScalarOpDynamicUnalign(const PrintUnaryParam &param) const {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, ret >= 0) << "GenVectorScalarOpByMode sprintf_s failed ";
...
std::string CodeGenOpCloudNPU::GenVectorScalarOpByMode(VecScalMode mode) const {
    ...
    if (opAttrs.count(OP_EMUOP_PREFIX + "opc")) {
        ...
        if (emuopc == EMUOP_TENSOR_EXTRACT) {
            ...
            ASSERT(ERROR_CODE_UNDEFINED, ret >= 0) << "GenVectorScalarOpByMode " << OpcodeManager::Inst().GetOpcodeStr(opCode) << " failed "
                             << ret;
            return buffer;
        }
    }
    ...
    ASSERT(ERROR_CODE_UNDEFINED, ret >= 0) << "GenVectorScalarOpByMode" << OpcodeManager::Inst().GetOpcodeStr(opCode) << " sprintf_s failed "
                     << ret;
    return buffer;
}
