...
std::string CodeGenOpCloudNPU::GenVectorScalarOpByMode(VecScalMode mode) const {
    ...
    if (opAttrs.count(OP_EMUOP_PREFIX + "opc")) {
        ...
        if (emuopc == EMUOP_TENSOR_EXTRACT) {
            ...
            ASSERT(GenCodeErr::PRINT_FAILED, ret >= 0)
                << "GenVectorScalarOpByMode " << OpcodeManager::Inst().GetOpcodeStr(opCode) << " failed " << ret;
            return buffer;
        }
    }
    ...
    ASSERT(GenCodeErr::PRINT_FAILED, ret >= 0)
        << "GenVectorScalarOpByMode" << OpcodeManager::Inst().GetOpcodeStr(opCode) << " sprintf_s failed " << ret;
    return buffer;
}
