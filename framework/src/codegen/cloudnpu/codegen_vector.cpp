...
std::string CodeGenOpCloudNPU::PrintScatterElementSOpDynamicUnaligned(const PrintScatterElemParam &param) const {
    ...
    const std::vector<std::string> &dataTypeExpr = param.dataTypeExpr;
    const Element &scala = extOperandVal;
    std::string scalarDtypeBuffer;
    if (scala.GetDataType() == DT_FP32 || scala.GetDataType() == DT_BF16) {
        scalarDtypeBuffer = "float";
    } else if (scala.GetDataType() == DT_FP16) {
        scalarDtypeBuffer = "half";
    } else {
        scalarDtypeBuffer = dataTypeExpr[ToUnderlying(MISOIdx::DST_IDX)];
    }
