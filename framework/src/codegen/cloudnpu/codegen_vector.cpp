...
std::string CodeGenOpCloudNPU::GenCastOp() const {
    ...
    ret = sprintf_s(buffer, sizeof(buffer),
        ...
        s0Var.c_str());
    ASSERT(ERROR_CODE_UNKNOWN, ret >= 0) << "GenCastOp sprintf_s failed " << ret;
...
std::string CodeGenOpCloudNPU::GenDupOp() const {
    ...
    if (opAttrs.count(OpAttributeKey::dynScalar)) {
        ...
        ASSERT(ERROR_CODE_UNKNOWN, (scalar.HasValue()) && (scalar.Type() == typeid(SymbolicScalar)))
        ...
        ASSERT(ERROR_CODE_UNKNOWN, (scalar.HasValue()) && (scalar.Type() == typeid(Element)))
        ...
        ASSERT(ERROR_CODE_UNKNOWN, false) << "unsupported type";
    }
...
std::string CodeGenOpCloudNPU::GenGatherFromUBOp() const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, opAttrs.find("op_attr_axis") != opAttrs.end()) << "GenGatherOp: There is nop axis attribute here";
    const int64_t axis = AnyCast<int64_t>(opAttrs.at("op_attr_axis"));
    // shape: dst, src0, src1
    int src0Rank = shape[ID1].size();
    ASSERT(ERROR_CODE_UNKNOWN, src0Rank <= RANK4) << "GenGatherOp: src0 shape rank is not supported!";
...
std::string CodeGenOpCloudNPU::PrintIndexPut(const PrintIndexPutParam &param) const {
    ASSERT(ERROR_CODE_UNKNOWN, isDynamicFunction) << "Only Support the DynamicUnaligned tileOp";
...
std::string CodeGenOpCloudNPU::GenIndexPutOp() const {
    ASSERT(ERROR_CODE_UNKNOWN, opAttrs.count(OpAttributeKey::accumulate)) << "cannot get accumulate attr";
    ASSERT(ERROR_CODE_UNKNOWN, opAttrs.count(OpAttributeKey::indicesSize)) << "cannot get indicesSize attr";
...
std::string CodeGenOpCloudNPU::GenRangeOp() const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, start.HasValue() && step.HasValue()) << "GenRangeOp failed ";
    ...
    switch (operandDtype[ID0]) {
        ...
        case DataType::DT_INT64:
            ...
            break;
        default: CODEGEN_LOGE(ERROR_CODE_UNDEFINED, "RangeOp from PASS occured unsupport DataType: %d", operandDtype[ID0]); return "CG_ERROR";
    }
    if (opAttrs.count(OpAttributeKey::dynScalar)) {
        auto scalarAny = opAttrs.at(OpAttributeKey::dynScalar);
        ASSERT(ERROR_CODE_UNKNOWN, (scalarAny.HasValue()) && (scalarAny.Type() == typeid(SymbolicScalar)))
...
std::string CodeGenOpCloudNPU::GenIndexAddOp() const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, opAttrs.count(OP_ATTR_PREFIX + "axis")) << "cannot get axis attr";
...
std::string CodeGenOpCloudNPU::GenCumSumOp() const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, opAttrs.count(OP_ATTR_PREFIX + "flag")) << "cannot get flag attr";
...
std::string CodeGenOpCloudNPU::GenTriULOp() const {
    ASSERT(ERROR_CODE_UNKNOWN, opAttrs.count(OpAttributeKey::dynScalar)) << "cannot get diagonal attr";
    ASSERT(ERROR_CODE_UNKNOWN, opAttrs.count(OpAttributeKey::isUpper)) << "cannot get isUpper attr";
    auto scalarAny = opAttrs.at(OpAttributeKey::dynScalar);
    ASSERT(ERROR_CODE_UNKNOWN, (scalarAny.HasValue()) && (scalarAny.Type() == typeid(SymbolicScalar)))
    ...
    ASSERT(ERROR_CODE_UNKNOWN, isSupportLayout) << "TriU or TriL only support TileTensor mode";
...
std::string CodeGenOpCloudNPU::PrintScatterElementSOpStatic(const PrintScatterElemParam &param) const {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, src1Rank == RANK2) << "GenScatterElementSOp: src1 shape rank is not supported!";
    ASSERT(ERROR_CODE_UNKNOWN, dstRank == RANK2) << "GenScatterElementSOp: dst shape rank is not supported!";
...
std::string CodeGenOpCloudNPU::GenScatterElementSOp() const {
    ASSERT(ERROR_CODE_UNKNOWN, opAttrs.count(OP_ATTR_PREFIX + "scatter_mode")) << "cannot get scatter mode attr";
    ASSERT(ERROR_CODE_UNKNOWN, opAttrs.count(OP_ATTR_PREFIX + "axis")) << "cannot get axis attr";
    int axis = AnyCast<int64_t>(opAttrs.at(OP_ATTR_PREFIX + "axis"));
...
std::string CodeGenOpCloudNPU::GenHypotOp() const {
    ASSERT(ERROR_CODE_UNKNOWN, isSupportLayout) << "Hypot only support tile tensor";
...
std::string CodeGenOpCloudNPU::GenPreluOp() const {
    ASSERT(ERROR_CODE_UNKNOWN, isSupportLayout) << "PReLU only support tile tensor";
