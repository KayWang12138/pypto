...
std::string CodeGenOpCloudNPU::PrintCumOperationTileTensor(int axis, bool is_sum) const {
    ...
    std::vector<std::string> preParamList = {std::to_string(axis), std::to_string(is_sum)};
    std::ostringstream oss;
    oss << tileOpName << WrapParamByAngleBrackets(preParamList) << WrapParamByParentheses(paramList) << ";\n";
    return oss.str();
}
