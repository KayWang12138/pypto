...
std::string CodeGenOpCloudNPU::GenCastOp() const {
    ...
    ret = sprintf_s(buffer, sizeof(buffer),
        ...
        s0Var.c_str());
    ASSERT(ret >= 0) << GenErrorCode(GenCodeErr::PRINT_FAILED) << "GenCastOp sprintf_s failed " << ret;
...
std::string CodeGenOpCloudNPU::GenDupOp() const {
    ...
    if (opAttrs.count(OpAttributeKey::dynScalar)) {
        ...
        ASSERT((scalar.HasValue()) && (scalar.Type() == typeid(SymbolicScalar)))
            << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << AnyCast<SymbolicScalar>(scalar).IsValid()
        ...
        ASSERT((scalar.HasValue()) && (scalar.Type() == typeid(Element)))
            << GenErrorCode(OperErr::ATTRIBUTE_INVALID)
        ...
        auto scalar = opAttrs.at(OpAttributeKey::scalar);
        ASSERT((scalar.HasValue()) && (scalar.Type() == typeid(Element)))
            << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << AnyCast<Element>(scalar).IsSigned()
...
std::string CodeGenOpCloudNPU::GenGatherFromUBOp() const {
    ...
    ASSERT(opAttrs.find("op_attr_axis") != opAttrs.end())
        << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "GenGatherOp: There is nop axis attribute here";
    const int64_t axis = AnyCast<int64_t>(opAttrs.at("op_attr_axis"));
    // shape: dst, src0, src1
    int src0Rank = shape[ID1].size();
    ASSERT(src0Rank <= RANK4) << GenErrorCode(GenCodeErr::TENSOR_SHAPE_INVALID)
...
std::string CodeGenOpCloudNPU::PrintIndexPut(const PrintIndexPutParam &param) const {
    ASSERT(isDynamicFunction) << GenErrorCode(GenCodeErr::PRINT_MODE_ERROR)
...
std::string CodeGenOpCloudNPU::GenIndexPutOp() const {
    ASSERT(opAttrs.count(OpAttributeKey::accumulate))
        << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "cannot get accumulate attr";
    ASSERT(opAttrs.count(OpAttributeKey::indicesSize))
        << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "cannot get indicesSize attr";
...
std::string CodeGenOpCloudNPU::GenRangeOp() const {
    ...
    ASSERT(start.HasValue() && step.HasValue())
        << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "GenRangeOp failed ";
    ...
    switch (operandDtype[ID0]) {
        ...
        default:
            CODEGEN_LOGE("%s RangeOp from PASS occured unsupport DataType: %d",
                GenErrorCode(GenCodeErr::DATA_TYPE_UNSUPPORTED).c_str(), operandDtype[ID0]);
            return "CG_ERROR";
    }
    if (opAttrs.count(OpAttributeKey::dynScalar)) {
        auto scalarAny = opAttrs.at(OpAttributeKey::dynScalar);
        ASSERT((scalarAny.HasValue()) && (scalarAny.Type() == typeid(SymbolicScalar)))
            << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << AnyCast<SymbolicScalar>(scalarAny).IsValid()
            << "SCALAR attribute has to have symbolic value.";
...
std::string CodeGenOpCloudNPU::GenIndexAddOp() const {
    ...
    ASSERT(opAttrs.count(OP_ATTR_PREFIX + "axis"))
        << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "cannot get axis attr";
...
std::string CodeGenOpCloudNPU::GenCumSumOp() const {
    ...
    ASSERT(opAttrs.count(OP_ATTR_PREFIX + "flag"))
        << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "cannot get flag attr";
...
std::string CodeGenOpCloudNPU::GenTriULOp() const {
    ASSERT(opAttrs.count(OpAttributeKey::dynScalar))
        << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "cannot get diagonal attr";
    ASSERT(opAttrs.count(OpAttributeKey::isUpper))
        << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "cannot get isUpper attr";
    auto scalarAny = opAttrs.at(OpAttributeKey::dynScalar);
    ASSERT((scalarAny.HasValue()) && (scalarAny.Type() == typeid(SymbolicScalar)))
        << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << AnyCast<SymbolicScalar>(scalarAny).IsValid()
    ...
    ASSERT(isSupportLayout) << GenErrorCode(GenCodeErr::PRINT_MODE_ERROR)
...
std::string CodeGenOpCloudNPU::PrintScatterElementSOpStatic(const PrintScatterElemParam &param) const {
    ...
    ASSERT(src1Rank == RANK2) << GenErrorCode(GenCodeErr::TENSOR_SHAPE_INVALID)
                              << "GenScatterElementSOp: src1 shape rank is not supported!";
    ASSERT(dstRank == RANK2) << GenErrorCode(GenCodeErr::TENSOR_SHAPE_INVALID)
...
std::string CodeGenOpCloudNPU::GenScatterElementSOp() const {
    ASSERT(opAttrs.count(OP_ATTR_PREFIX + "scatter_mode"))
        << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "cannot get scatter mode attr";
    ASSERT(opAttrs.count(OP_ATTR_PREFIX + "axis"))
        << GenErrorCode(OperErr::ATTRIBUTE_INVALID) << "cannot get axis attr";
    int axis = AnyCast<int64_t>(opAttrs.at(OP_ATTR_PREFIX + "axis"));
...
std::string CodeGenOpCloudNPU::GenHypotOp() const {
    ASSERT(isSupportLayout) << GenErrorCode(GenCodeErr::PRINT_MODE_ERROR) << "Hypot only support tile tensor";
...
std::string CodeGenOpCloudNPU::GenPreluOp() const {
    ASSERT(isSupportLayout) << GenErrorCode(GenCodeErr::PRINT_MODE_ERROR) << "PReLU only support tile tensor";
