...
std::string CodeGenOpCloudNPU::PrintCumOperationTileTensor(int axis, bool isSum) const {
    axis = axis + 1;
    std::string dstTensor = QueryTileTensorNameByIdx(ToUnderlying(MISOIdx::DST_IDX));
    std::string srcTensor = QueryTileTensorNameByIdx(ToUnderlying(MISOIdx::SRC0_IDX));
    std::vector<std::string> paramList = {dstTensor, srcTensor, axis};
    std::ostringstream oss;
    oss << tileOpName << "<" << isSum << ">" << WrapParamByParentheses(paramList) << ";\n";
    return oss.str();
}
...
std::string CodeGenOpCloudNPU::GenCumOperationOp() const {
    ...
    bool isSum = AnyCast<bool>(opAttrs.at(OP_ATTR_PREFIX + "flag"));
