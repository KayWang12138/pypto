...
void ForBlockManager::UpdateAxesList(const std::vector<SymbolicScalar> &axesList) {
    ...
    for (size_t i = 0; i < axesList_.size(); ++i) {
        ...
        bool offsetVal = axesList_[i].ConcreteValid() && axesList_[i].Concrete() == 1 ? "0" : loopVar;
...
void ForBlockManager::UpdateTensorOffsetInLoop(const std::string &tensorFullDim, const std::string &tensorNameInLoop) {
    auto tileTensor = sm_->QueryTileTensorByBufVar(tensorFullDim);
    auto rawShape = tileTensor.rawShape;
    FillIntVecWithDummyInHead(rawShape, SHAPE_DIM5 - rawShape.size(), 1);
    auto newOffset = tensorOffsetInLoop_.at(DEFAULT_TENSOR_OFFSET);
...
void ForBlockManager::AddTensorInLoopBody(const std::string &tensorFullDim, const TileTensor &tileTensor) {
    ...
    UpdateTensorOffsetInLoop(tensorFullDim, tensorNameInLoop);
}
