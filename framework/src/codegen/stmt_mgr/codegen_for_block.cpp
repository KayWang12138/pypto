...
void ForBlockManager::UpdateAxesList(const std::vector<SymbolicScalar> &axesList) {
    ...
    for (size_t i = 0; i < axesList_.size(); ++i) {
        ...
        offsetInLoop.emplace_back(offsetVal);
    }
...
void ForBlockManager::UpdateTensorOffsetInLoop(Opcode opCode, size_t tensorFullDimHash, const std::string &tensorNameInLoop) {
    ...
    if (allOffsetsInLoop_.find(newOffset) == allOffsetsInLoop_.end()) {
        offsetName += "_" + std::to_string(offsetCnt_);
...
void ForBlockManager::AddTensorInLoopBody(size_t tensorFullDimHash, const TileTensor &tileTensor, Opcode opCode) {
    CODEGEN_LOGI("AddTensorInLoopBody : full tensor hash: %zu, loop tensor: %s, op code: %s", tensorFullDimHash,
        tileTensor.tensorName.c_str(), OpcodeManager::Inst().GetOpcodeStr(opCode).c_str());
    ...
    UpdateTensorOffsetInLoop(opCode, tensorFullDimHash, tensorNameInLoop);
}
