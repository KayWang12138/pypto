...
void ForBlockManager::UpdateAxesList(const std::vector<SymbolicScalar> &axesList) {
    ...
    allOffsetsInLoop_[offsetInLoop] = DEFAULT_TENSOR_OFFSET + "_" + std::to_string(offsetCnt_);
...
void ForBlockManager::UpdateTensorOffsetInLoop(size_t tensorFullDimHash, const std::string &tensorNameInLoop) {
    ...
    if (iter != allOffsetsInLoop_.end()) {
        tensorOffset_[tensorNameInLoop] = iter->second;
        return;
    }

    std::string offsetName = DEFAULT_TENSOR_OFFSET + "_" + std::to_string(offsetCnt_);
