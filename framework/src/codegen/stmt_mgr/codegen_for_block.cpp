...
std::string ForBlockManager::PrintForEnd(std::ostringstream &os) const {
    for (size_t i = 0; i < MAX_LOOP_DEPTH; ++i) {
...
std::string ForBlockManager::PrintOffsetDef(std::ostringstream &os) const {
    ...
    os << WrapParamByParentheses(loopVars) << SEMICOLON_ENDLINE;
}
...
std::string ForBlockManager::PrintTileOps(std::ostringstream &os) const {
    for (const auto &tileOp : opList_) {
        PrintIndent(os, MAX_LOOP_DEPTH + 1);
        os << tileOp << SEMICOLON_ENDLINE;
    }
}
