...
void SymbolManager::AddTileTensor(const TileTensor &tileTensor) {
    ...
    if (result.second) {
        ...
        tileTensorByMagic_.insert({tileTensor.magic, result.first});
    }
...
std::vector<TileTensor> SymbolManager::QueryTileTensorByMagic(int magic) {
    ...
    auto [start, end] = mm.equal_range(magic);
    for (auto it = start; it != end; ++it) {
        result.emplace_back(it->second); // 或 emplace_back，性能更优
    }

    ASSERT(!result.empty()) << "tensor magic " << magic << " is not found !!! ";
    return result;
}
