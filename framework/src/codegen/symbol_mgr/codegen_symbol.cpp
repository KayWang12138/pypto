...
AllocKey SymbolManager::CreateAllocKey(int tensorMagicNum) const {
    ...
    if (!tensor) {
        CODEGEN_LOGE("%s: can not query tensor object from tensor magicnum: %d",
            GenErrorCode(GenCodeErr::TENSOR_NOT_FOUND).c_str(), tensorMagicNum);
        return {};
    }
...
std::shared_ptr<LogicalTensor> SymbolManager::GetTensorByMagic(int magicNum) const {
    ...
    if (iter != tensorMap_.end()) {
        ...
        ASSERT(false) << GenErrorCode(GenCodeErr::TENSOR_NOT_FOUND)
...
std::string SymbolManager::QueryVariableName(const AllocKey &key) {
    ...
    ASSERT(iter != key2VariableName_.end())
        << GenErrorCode(GenCodeErr::SYMBOL_NOT_FOUND)
...
std::string SymbolManager::QueryVariableNameTileTensor(const AllocKey &key) {
    ...
    CODEGEN_LOGE("%s: failed to query by identifier: %s", GenErrorCode(GenCodeErr::SYMBOL_NOT_FOUND).c_str(),
        FormatAllocKey(key).c_str());
    ASSERT(false) << GenErrorCode(GenCodeErr::SYMBOL_NOT_FOUND)
...
std::vector<TileTensor> SymbolManager::QueryTileTensorByMagic(int magic) {
    ...
    ASSERT(!res.empty()) << GenErrorCode(GenCodeErr::TENSOR_NOT_FOUND) << "tensor magic " << magic
                         << " is not found !!! ";
    return res;
}
...
std::string SymbolManager::QueryTileTensorFullDimByTensorInLoop(const std::string &tensorName) {
    ...
    ASSERT(!fullDimTensorName.empty()) << GenErrorCode(GenCodeErr::SYMBOL_NOT_FOUND) << "tensor in loop "
...
const TileTensor &SymbolManager::QueryTileTensorByBufVar(const std::string &bufVarName) {
    ...
    ASSERT(false) << GenErrorCode(GenCodeErr::SYMBOL_NOT_FOUND) << "bufVarName " << bufVarName
                  << " is not found !!! ";
