...
bool INIParser::Initialize(const std::string& iniFilePath)
{
    ...
    if (!ReadINIFile(iniFilePath)) {
        PLATFORM_LOGE(FError::NOT_EXIST, "ReadINIFile failed.");
        return false;
    }
...
bool INIParser::ReadINIFile(const std::string& filepath)
{
    ...
    if (!file.is_open()) {
        PLATFORM_LOGE(FError::BAD_FD, "Failed to open ini file: %s.", filepath.c_str());
        return false;
    }
    std::string line;
    std::string section;
...
bool INIParser::GetStringVal(const std::string& column, const std::string& key, std::string& val) const
{
    ...
    if (iter == data_.end()) {
        PLATFORM_LOGE(FError::INVALID_TYPE, "Cannot find attr '%s' from the ini file.", column.c_str());
        return false;
    }
    auto value = iter->second;
    auto iter2 = value.find(key);
    if (iter2 == value.end()) {
        PLATFORM_LOGE(FError::INVALID_VAL, "Cannot find attr '%s' from the [%s] tab.", key.c_str(), column.c_str());
        return false;
    }
    val = iter2->second;
...
bool CmdParser::GetStringVal(const std::string& column, const std::string& key, std::string& val) const
{
    ...
    if (!CannHostRuntime::Instance().GetSocSpec(column, key, val)) {
        PLATFORM_LOGE(FError::UNKNOWN, "Cannot find soc spec '%s' from the [%s] column.", key.c_str(), column.c_str());
        return false;
    }
    return true;
}
