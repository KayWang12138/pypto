...
void Platform::ObtainPlatformInfo()
{
    ...
    if (!platformSocVersion.empty()) {
        PLATFORM_LOGD("User specified soc version %s, use specified version.", platformSocVersion.c_str());
        ...
        PLATFORM_LOGD("Cannot obtain platform through cann package, use default simulation info.");
