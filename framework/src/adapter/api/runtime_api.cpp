...
RtError RuntimeBinaryLoadFromFile(const char_t * const binPath, const RtLoadBinaryConfig * const optionalCfg,
    ...
    if (func != nullptr) {
        ...
        return runtimeFunc(binPath, reinterpret_cast<const rtLoadBinaryConfig_t* const>(optionalCfg), handle);
    }
#endif
    return StubBinaryLoadFromFile(binPath, optionalCfg, handle);
}

rtError_t RuntimeStreamCreate(RtStream *stm, int32_t priority) {
#ifdef BUILD_WITH_CANN
...
RtError RuntimeKernelLaunchWithHandleV2(void *hdl, const uint64_t tilingKey, uint32_t numBlocks,
    ...
    if (func != nullptr) {
        ...
        return runtimeFunc(hdl, tilingKey, numBlocks, reinterpret_cast<rtArgsEx_t*>(argsInfo), smDesc, stm, reinterpret_cast<const rtTaskCfgInfo_t*>(cfgInfo));
    }
#endif
...
RtError RuntimeAicpuKernelLaunchExWithArgs(const uint32_t kernelType, const char_t * const opName,
    ...
    if (func != nullptr) {
        ...
        return runtimeFunc(kernelType, opName, numBlocks, reinterpret_cast<rtAicpuArgsEx_t*>(argsInfo), smDesc, stm, flags);
    }
#endif
