...
#ifdef BUILD_WITH_CANN
    void *func = AdapterManager::Instance().GetHcclAdapter().GetFunction(AdumpFunc::GetDumpSwitch);
    if (func != nullptr) {
        uint64_t(*adumpFunc)(const Adx::DumpType) = reinterpret_cast<uint64_t(*)(const Adx::DumpType)>(func);
        return adumpFunc(static_cast<DumpType>(dumpType));
    }
#endif
...
#ifdef BUILD_WITH_CANN
    void *func = AdapterManager::Instance().GetHcclAdapter().GetFunction(AdumpFunc::DumpTensorV2);
    if (func != nullptr) {
