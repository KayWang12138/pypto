...
uint64_t get_sys_cnt() {
    auto cnt = AicoreEmulationManager::GetInstance()->GetEmulation()->GetSysCnt();
    return cnt;
}

int64_t get_coreid() {
    auto coreid = AicoreEmulationManager::GetInstance()->GetEmulation()->GetCoreId();
    return coreid;
}

int get_block_idx() {
    auto blockIdx = AicoreEmulationManager::GetInstance()->GetEmulation()->GetBlockIdx();
    return blockIdx;
}

void set_cond(uint64_t cond) {
    AicoreEmulationManager::GetInstance()->GetEmulation()->SetCond(cond);
}

uint64_t GetDataMainBase() {
    auto mainBase = AicoreEmulationManager::GetInstance()->GetEmulation()->GetDataMainBase();
    return mainBase;
}
