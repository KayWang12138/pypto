...
uint64_t get_sys_cnt() {
    auto cnt = npu::tile_fwk::machine::AicoreEmulationManager::GetInstance().GetEmulation()->GetSysCnt();
    return cnt;
}

int64_t get_coreid() {
    auto coreid = npu::tile_fwk::machine::AicoreEmulationManager::GetInstance().GetEmulation()->GetCoreId();
    return coreid;
}

int get_block_idx() {
    auto blockIdx = npu::tile_fwk::machine::AicoreEmulationManager::GetInstance().GetEmulation()->GetBlockIdx();
    return blockIdx;
}

void set_cond(uint64_t cond) {
    npu::tile_fwk::machine::AicoreEmulationManager::GetInstance()->GetEmulation().SetCond(cond);
}

uint64_t GetDataMainBase() {
    auto mainBase = npu::tile_fwk::machine::AicoreEmulationManager::GetInstance().GetEmulation()->GetDataMainBase();
    return mainBase;
}
