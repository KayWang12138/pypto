...
// ===================================================================
// Note on lines 77, 83, 155, 167:
// These MACHINE_LOGs are in static functions CompileCoreMachine (lines 77, 83)
// and LinkObject (lines 155, 167). They can only be reached through
// CompileAICoreKernel when GenAicoreSrcFile succeeds, which then
// calls CompileCoreMachine/LinkObject via parallel tasks.
//
// Line 77: snprintf_s returns < 0 (very unlikely with normal inputs)
// Line 83: system() returns != 0 (requires bisheng compiler invocation to fail)
// Line 155: snprintf_s returns < 0 (very unlikely with normal inputs)
// Line 167: system() returns != 0 (requires ld.lld linker invocation to fail)
//
// These lines require the bisheng toolchain to be partially available
// (GenAicoreSrcFile succeeds but compilation/linking fails), which is
// not practical in a standard UT environment. They should be covered
// by ST (system tests) in an environment with the toolchain installed.
// ===================================================================

// ===================================================================
// Note on line 137:
// Line 137: MACHINE_LOGE("Fail to open call.h.") in GenSubFuncCall
// requires a non-empty idxNameMap (i.e., leafDict with valid Function
// objects that have LeafFuncAttribute matching coreType), and then
// fopen to fail on the ccePath. Creating proper Function objects with
// LeafFuncAttribute requires the full compilation pipeline, making
// this impractical for unit testing. Should be covered by ST.
// ===================================================================
