...
// 供需要 workspace 的 DeviceTaskContext 用例共用，避免重复初始化 cache/devProg/workspace。
struct DeviceTaskContextWithWorkspace {
    alignas(64) uint8_t cacheBuf[4096] = {};
    alignas(64) uint8_t cacheMeta[sizeof(dynamic::DevControlFlowCache) + 256] = {};
    std::vector<uint8_t> progBuf;
    dynamic::DevControlFlowCache *cache = nullptr;
    dynamic::DevAscendProgram *devProg = nullptr;
    dynamic::DeviceWorkspaceAllocator workspace;
    dynamic::DeviceTaskContext ctx;

    DeviceTaskContextWithWorkspace() {
        progBuf.resize(sizeof(dynamic::DevAscendProgram) + 1024, 0);
        cache = reinterpret_cast<dynamic::DevControlFlowCache *>(cacheMeta);
        cache->isRecording = true;
        cache->cacheDataOffset = 0;
        cache->cacheData.size_ = 4096;
        cache->cacheData.data_ = cacheBuf;
        devProg = reinterpret_cast<dynamic::DevAscendProgram *>(progBuf.data());
        devProg->ctrlFlowCacheAnchor = cache;
        new (&workspace) dynamic::DeviceWorkspaceAllocator(devProg);
...
TEST_F(TestDevErrorLog, BuildDynFuncData_CceBinaryNotAligned_LogsError) {
    DeviceTaskContextWithWorkspace wrap;

    std::vector<uint8_t> taskBuf(sizeof(dynamic::DynDeviceTask) + 1024, 0);
    auto *dynTask = reinterpret_cast<dynamic::DynDeviceTask *>(taskBuf.data());
    dynTask->cceBinary = reinterpret_cast<const dynamic::DevCceBinary *>(0x1);

    int ret = wrap.ctx.BuildDynFuncData(dynTask, 0, nullptr, 0);
    EXPECT_EQ(ret, dynamic::DEVICE_MACHINE_ERROR);
}
...
TEST_F(TestDevErrorLog, ProcessSingleDynFuncData_AlignmentChecks_LogsError) {
    ...
    ret = ctx.ProcessSingleDynFuncData(&dyndata, dupFunc);
    EXPECT_EQ(ret, dynamic::DEVICE_MACHINE_ERROR);
