...
public:
    ...
    void CheckLogContent(const std::string &expectedSubStr, const char *fmt, ...) {
        va_list list;
        va_start(list, fmt);
        LogMsg logMsg{};
        LogManager::Instance().ConstructMessage(LogLevel::INFO, fmt, list, logMsg);
        va_end(list);
        std::string regexStr = expectedSubStr;
...
TEST_F(TestHostLog, test_log_construct_case0) {
    ...
    CheckLogContent("[-234][432][][]", "[%d][%+d][%+d][%u]", int32_val, int32_val, int32_val2, uint32_val);
    CheckLogContent("[][][][][][]", "[%o][%#o][%x][%X][%#x][%#X]", int32_val, uint32_val, int32_val, uint32_val, int32_val, uint32_val);
    ...
    CheckLogContent("[-789][987][fffffceb][3DB]", "[%ld][%lu][%lx][%lX]", int64_val, uint64_val, int64_val, uint64_val);

    float float_val = 123.456f;
    CheckLogContent("[123.456001][123.46][][][][]", "[%f][%.2f][%10.5][%e][%.3e][%E]", float_val, float_val, float_val, float_val, float_val, float_val);

    double double_val = -456.987321;
    CheckLogContent("[123.456001][123.46][][][][]", "[%f][%.2f][%10.5][%e][%.3e][%E]", double_val, double_val, double_val, double_val, double_val, double_val);
...
TEST_F(TestHostLog, test_log_construct_case1) {
    ...
    CheckLogContent("[4294967173][18446744073709551160]", "[%u][%lu]", -123, -456);
