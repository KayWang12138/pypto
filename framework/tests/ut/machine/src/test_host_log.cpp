...
TEST_F(TestHostLog, test_log_construct_case0) {
    ...
    CheckLogContent(log_manager, LogLevel::INFO, "[-234][432]", "[%d][%u][%x][%p]", int32_val, uint32_val, &int32_val, &uint32_val);

    int64_t int64_val = -789;
    uint64_t uint64_val = 987;
    CheckLogContent(log_manager, LogLevel::INFO, "[-789][987][fffffceb][3DB]", "[%ld][%lu][%x][%X]", int64_val, uint64_val, int64_val, uint64_val);

    float float_val = 123.456f;
    CheckLogContent(log_manager, LogLevel::INFO, "[123.456001][123.46]", "[%f][%.2f][%x][%p]", float_val, float_val, float_val, &float_val);
...
TEST_F(TestHostLog, test_log_construct_case1) {
    ...
    CheckLogContent(log_manager, LogLevel::INFO, "[ffffff85]", "[%x][%p]", -123, 3.14);
