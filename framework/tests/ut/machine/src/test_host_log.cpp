...
public:
    ...
    void CheckLogContent(LogManager &log_manager, const LogLevel logLevel, const std::string &expectedStr, const char *fmt, ...) {
        ...
        EXPECT_NE(retStr.find(expectedStr), std::string::npos);
    }
};
...
TEST_F(TestHostLog, test_log_construct_case0) {
    ...
    CheckLogContent(log_manager, LogLevel::INFO, "", "[%d][%u][%x][%p]", int32_val, uint32_val, &int32_val, &uint32_val);

    int64_t int64_val = -789;
    uint64_t uint64_val = 987;
    CheckLogContent(log_manager, LogLevel::INFO, "", "[%ld][%lu][%x][%X]", int64_val, uint64_val, int64_val, uint64_val);

    float float_val = 123.456f;
    CheckLogContent(log_manager, LogLevel::INFO, "", "[%f][%.2f][%x][%p]", float_val, float_val, float_val, &float_val);

    CheckLogContent(log_manager, LogLevel::INFO, "", "[%c%s]", 'H', "ello world");
}

TEST_F(TestHostLog, test_log_construct_case1) {
    LogManager log_manager;
    CheckLogContent(log_manager, LogLevel::INFO, "", "Hello world!", 123, "morgan");
    CheckLogContent(log_manager, LogLevel::INFO, "", "[%u][%lu]", -123, -456);
    CheckLogContent(log_manager, LogLevel::INFO, "", "[%f][%lu]", -123, 3.14f);
    CheckLogContent(log_manager, LogLevel::INFO, "", "[%x][%p]", -123, 3.14);
