/**
 * Unit tests for AiCore print helpers (DecodeF16/DecodeBf16, AiCorePrintTensor, etc.)
 */
...
struct MockLogger {
    ...
    static void PrintFloat(LogContext *c, __gm__ const char ** /*fmt*/, float val)
    {
        auto self = reinterpret_cast<MockLogger *>(c);
        self->buffer += std::to_string(val);
    }
    ...
    MockLogger()
    {
        ...
        ctx.PrintFloat = &MockLogger::PrintFloat;
...
TEST(AiCorePrintTest, DecodeF16BasicValues)
{
    // 0.0
...
TEST(AiCorePrintTest, DecodeBf16BasicValues)
{
    // 0.0
...
TEST(AiCorePrintTest, PrintFp16DecodedValue)
{
    MockLogger logger;
...
TEST(AiCorePrintTest, PrintBf16DecodedValue)
{
    MockLogger logger;
...
TEST(AiCorePrintTest, PrintIntAndFloatThroughLogContext)
{
    MockLogger logger;
    ...
    logger.ctx.PrintFloat(&logger.ctx, fmt, 3.5f);
...
TEST(AiCorePrintTest, AiCorePrintTensorFloat)
{
    MockLogger logger;

    float data[3] = {1.0f, 2.0f, 3.5f};
    AiCorePrintTensor<float>(&logger.ctx, data, 3);
...
TEST(AiCorePrintTest, AiCorePrintShape2D)
{
    MockLogger logger;

    // Use TileOp::Shape when TileOp utilities are available (AICore build).
...
TEST(AiCorePrintTest, AiCorePrintShape2D)
{
    GTEST_SKIP() << "AiCorePrintShape not available in this build configuration.";
}
#endif
