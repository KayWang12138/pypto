...
// 测试 IndexedTypeInfo<float> 返回正确的 DataType 和编码大小
TEST_F(AiCorePrintUTest, IndexedTypeInfo_FloatProperties)
{
    ...
    EXPECT_EQ(EncodeSizes::IndexedFp32WithEnd, IndexedTypeInfo<float>::Size);
}

// 测试 IndexedTypeInfo<int64_t> 返回正确的 DataType 和编码大小
TEST_F(AiCorePrintUTest, IndexedTypeInfo_Int64Properties)
{
    ...
    EXPECT_EQ(EncodeSizes::IndexedInt64WithEnd, IndexedTypeInfo<int64_t>::Size);
}

// 测试 IndexedTypeInfo 对未知类型(char/double)返回默认值 End 和 Size=0
TEST_F(AiCorePrintUTest, IndexedTypeInfo_UnknownTypeDefaults)
{
    EXPECT_EQ(AicorePrint::DataType::End, IndexedTypeInfo<char>::Type);
    EXPECT_EQ(0, IndexedTypeInfo<char>::Size);

    EXPECT_EQ(AicorePrint::DataType::End, IndexedTypeInfo<double>::Type);
    EXPECT_EQ(0, IndexedTypeInfo<double>::Size);
}
...
TEST_F(AiCorePrintUTest, IndexedTypeInfo_Bf16Properties)
{
    ...
    EXPECT_EQ(EncodeSizes::IndexedBf16WithEnd, IndexedTypeInfo<bfloat16_t>::Size);
}
...
TEST_F(AiCorePrintUTest, IndexedTypeInfo_Fp16Properties)
{
    ...
    EXPECT_EQ(EncodeSizes::IndexedFp16WithEnd, IndexedTypeInfo<half>::Size);
}
...
// 测试 IndexedTypeInfo 对 FP8 各类型的编码大小一致性
TEST_F(AiCorePrintUTest, IndexedTypeInfo_Fp8Sizes)
{
    EXPECT_EQ(EncodeSizes::IndexedFp8WithEnd, IndexedTypeInfo<float8_e4m3_t>::Size);
    EXPECT_EQ(EncodeSizes::IndexedFp8WithEnd, IndexedTypeInfo<float8_e5m2_t>::Size);
    EXPECT_EQ(EncodeSizes::IndexedFp8WithEnd, IndexedTypeInfo<float8_e8m0_t>::Size);
    EXPECT_EQ(EncodeSizes::IndexedFp8WithEnd, IndexedTypeInfo<hifloat8_t>::Size);
}
...
// ============================================================================
// Section 6: EncodeSizes 常量计算功能测试 - 编码记录大小计算验证
// ============================================================================

// 测试 EncodeSizes 各编码记录大小(type+index+value)的计算公式正确性
TEST_F(AiCorePrintUTest, EncodeSizes_RecordSizeCalculations)
{
    EXPECT_EQ(EncodeSizes::IndexedFp32, 1 + 8 + 4);
    EXPECT_EQ(EncodeSizes::IndexedInt64, 1 + 8 + 8);
    EXPECT_EQ(EncodeSizes::IndexedBf16, 1 + 8 + 2);
    EXPECT_EQ(EncodeSizes::IndexedFp16, 1 + 8 + 2);
    EXPECT_EQ(EncodeSizes::EndMarker, 1);
    EXPECT_EQ(EncodeSizes::OverflowWarningSize, 1 + 8 + 1);
}

// 测试 EncodeSizes WithEnd 版本等于基础大小加 EndMarker
TEST_F(AiCorePrintUTest, EncodeSizes_WithEndMarkerCorrect)
{
    EXPECT_EQ(EncodeSizes::IndexedFp32WithEnd, EncodeSizes::IndexedFp32 + EncodeSizes::EndMarker);
    EXPECT_EQ(EncodeSizes::IndexedInt64WithEnd, EncodeSizes::IndexedInt64 + EncodeSizes::EndMarker);
    EXPECT_EQ(EncodeSizes::IndexedBf16WithEnd, EncodeSizes::IndexedBf16 + EncodeSizes::EndMarker);
    EXPECT_EQ(EncodeSizes::IndexedFp16WithEnd, EncodeSizes::IndexedFp16 + EncodeSizes::EndMarker);
}

#if SUPPORT_FP8_HF8_PRINT
// 测试 EncodeSizes FP8 类型编码大小(type+index+1字节)的正确性
TEST_F(AiCorePrintUTest, EncodeSizes_Fp8Sizes)
{
    EXPECT_EQ(EncodeSizes::IndexedFp8, 1 + 8 + 1);
    EXPECT_EQ(EncodeSizes::IndexedFp8WithEnd, EncodeSizes::IndexedFp8 + EncodeSizes::EndMarker);
}
#endif

// ============================================================================
// Section 7: AicorePrintConst 常量功能测试 - 打印框架常量验证
// ============================================================================
...
// ============================================================================
// Section 8: DataType 枚举功能测试 - 数据类型标记值验证
// ============================================================================
...
// ============================================================================
// Section 9: 浮点格式常量功能测试 - FP32/FP16/BF16 位域常量验证
// ============================================================================
...
// ============================================================================
// Section 11: MockLogger 打印功能测试 - LogContext 函数指针调用
// ============================================================================
...
// ============================================================================
// Section 12: AicoreLogger Host侧完整功能测试 - 编码解码全流程
// ============================================================================
...
// 测试 AicoreLogger 使用足够大 buffer 初始化成功，返回有效 buffer 大小
TEST_F(AiCorePrintUTest, HostLogger_InitBufferSuccess)
{
    AicoreLoggerHostFixture fixture;
    fixture.InitBuffer();
    AicoreLogger logger;
    logger.Init(fixture.buffer, AicoreLoggerHostFixture::BUFFER_SIZE);
    EXPECT_GT(logger.GetBufferSize(), 0);
}

// 测试 AicoreLogger 使用最小 buffer(26字节)初始化成功
TEST_F(AiCorePrintUTest, HostLogger_InitMinBufferSuccess)
{
    alignas(64) uint8_t minBuffer[AicorePrintConst::MIN_BUFFER_TOTAL_SIZE];
    std::memset(minBuffer, 0, AicorePrintConst::MIN_BUFFER_TOTAL_SIZE);
    AicoreLogger logger;
    logger.Init(minBuffer, AicorePrintConst::MIN_BUFFER_TOTAL_SIZE);
    EXPECT_GT(logger.GetBufferSize(), 0);
}

// 测试 AicoreLogger 使用不足 buffer(10字节)初始化失败，返回 buffer 大小为 0
TEST_F(AiCorePrintUTest, HostLogger_InitInsufficientBufferFails)
{
    alignas(64) uint8_t smallBuffer[10];
    std::memset(smallBuffer, 0, 10);
    AicoreLogger logger;
...
TEST_F(AiCorePrintUTest, HostLogger_OverflowWarningWhenBufferFull)
{
    ...
    std::memset(smallBuffer, 0, smallBufferSize);
...
// 测试重新初始化 AicoreLogger 时状态正确变化(成功->失败->成功)
TEST_F(AiCorePrintUTest, HostLogger_ReinitChangesState)
{
    AicoreLoggerHostFixture fixture;
    alignas(64) uint8_t smallBuf[10];
    std::memset(smallBuf, 0, 10);

    AicoreLogger logger;

    logger.Init(smallBuf, 10);
    EXPECT_EQ(logger.GetBufferSize(), 0);
...
// ============================================================================
// Section 13: Shape 打印功能测试 - AiCore 设备侧 Shape 打印(条件编译)
// ============================================================================
