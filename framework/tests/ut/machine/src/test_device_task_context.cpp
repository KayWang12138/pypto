...
protected:
    ...
    WrapInfoQueue* SetupWrapQueueForTest(DynDeviceTask *dyntask, DeviceTaskContext &taskContext, CoreType coreType, 
        uint32_t wrapVecId, DevAscendFunction& devFunc, DevCceBinary* cceBinary, int* calleeList) {
...
TEST_F(TestDeviceTaskContext, test_process_wrap_queue_update_existing_wrap) {
    ...
    constexpr size_t kControlFlowCacheSize = 64 * 1024;
    auto controlFlowCacheBuf = std::make_unique<uint8_t[]>(kControlFlowCacheSize);
    SetupBasicTaskContext(taskContext, startArgs, &devProg, dyntask, workspace, controlFlowCacheBuf, kControlFlowCacheSize);

    DevAscendFunction devFunc;
    DevCceBinary cceBinary[1] = {};
    int calleeList[1] = {0};
    WrapInfoQueue *wrapQueue = SetupWrapQueueForTest(dyntask.get(), taskContext, CoreType::AIC, 0, devFunc, cceBinary, calleeList);
...
TEST_F(TestDeviceTaskContext, test_process_wrap_queue_aiv0) {
    ...
    DevAscendFunction devFunc;
    int calleeList[1] = {0};
    WrapInfoQueue *wrapQueue = SetupWrapQueueForTest(dyntask.get(), taskContext, CoreType::AIV, 0, devFunc, cceBinary, calleeList);
    ASSERT_NE(wrapQueue, nullptr);
...
TEST_F(TestDeviceTaskContext, test_process_wrap_queue_aiv1) {
    ...
    DevAscendFunction devFunc;
    DevCceBinary cceBinary[1] = {};
    WrapInfoQueue *wrapQueue = SetupWrapQueueForTest(dyntask.get(), taskContext, CoreType::AIV, 1, devFunc, cceBinary, calleeList);
    ASSERT_NE(wrapQueue, nullptr);
