...
TEST_F(TestDeviceTaskContext, test_init_die_ready_queues_mix_arch) {
    ...
    CreateMockDevAscendProgram(&devProg, ArchInfo::DAV_1001);
    ...
    for (size_t i = 0; i < DIE_NUM; i++) {
        EXPECT_EQ(dyntask->devTask.dieReadyFunctionQue.readyDieAivCoreFunctionQue[i], 0UL);
        EXPECT_EQ(dyntask->devTask.dieReadyFunctionQue.readyDieAicCoreFunctionQue[i], 0UL);
    }
}
...
TEST_F(TestDeviceTaskContext, test_build_ready_queue_for_func_mix_arch_no_die_id) {
    ...
    EXPECT_TRUE(taskContext.IsMixArch(&devProg));
}
...
TEST_F(TestDeviceTaskContext, test_build_ready_queue_for_func_func_index_out_of_range) {
    ...
    DevAscendProgram devProg;
    CreateMockDevAscendProgram(&devProg, ArchInfo::DAV_1001);
    EXPECT_FALSE(taskContext.IsMixArch(&devProg));
}

TEST_F(TestDeviceTaskContext, test_build_ready_queue_core_function_cnt_exceeds_stitch_function_size) {
    DeviceTaskContext taskContext;
    DevStartArgsBase startArgs;
    ...
    auto dyntask = std::make_unique<DynDeviceTask>(workspace);
    CreateMockDynDeviceTask(dyntask.get(), 11);
