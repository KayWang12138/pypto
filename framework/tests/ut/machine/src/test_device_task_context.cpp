...
#if GTEST_HAS_DEATH_TEST
// Covers device_execute_context.cpp SymbolHandlerIdToHandler default: DEV_ERROR + DEV_ASSERT (lines 83–84).
TEST(DeviceExecuteSymbolTest, SymbolHandlerIdToHandler_UnhandledEnum_AbortsAfterDevError) {
    ::testing::FLAGS_gtest_death_test_style = "threadsafe";
    ASSERT_DEATH(
        (void)DeviceExecuteContext::SymbolHandlerIdToHandler(SymbolHandlerId::GetTensorDataInt32Dim1),
        ".*");
}

TEST(DeviceExecuteSymbolTest, SymbolHandlerIdToHandler_OutOfRangeNumeric_AbortsAfterDevError) {
    ::testing::FLAGS_gtest_death_test_style = "threadsafe";
    ASSERT_DEATH((void)DeviceExecuteContext::SymbolHandlerIdToHandler(static_cast<SymbolHandlerId>(99999ULL)), ".*");
}
#endif