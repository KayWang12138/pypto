...
public:
    ...
    void TearDown() override { Platform::Instance().GetSoc().SetNPUArch(NPUArch::UNKNOWN); }
...
TEST_F(TestDeviceTaskContext, test_init_die_ready_queues_different_core_cnt) {
    DeviceTaskContext taskContext;
    DeviceWorkspaceAllocator workspace(1024 * 1024);
    DevStartArgsBase startArgs;
    ...
    std::vector<uint32_t> coreCounts = {10, 50, 100, 500};
    for (auto coreCnt : coreCounts) {
        DynDeviceTask dyntask;
        CreateMockDynDeviceTask(&dyntask, coreCnt);
        ...
        for (size_t i = 0; i < DIE_NUM; i++) {
            ...
            EXPECT_NE(aivQueue, nullptr);
            EXPECT_NE(aicQueue, nullptr);
            EXPECT_EQ(aivQueue->capacity, coreCnt);
            EXPECT_EQ(aicQueue->capacity, coreCnt);
        }
    }
}
...
TEST_F(TestDeviceTaskContext, test_build_ready_queue_for_func_mix_arch_no_die_id) {
    ...
    int wrapTaskNum = 0;
    taskContext.BuildReadyQueueForFunc(&dyntask, 0, false, nullptr, nullptr, nullptr, wrapTaskNum);

    EXPECT_EQ(wrapTaskNum, 0);
}

TEST_F(TestDeviceTaskContext, test_build_ready_queue_for_func_null_dyntask) {
    DeviceTaskContext taskContext;
    DeviceWorkspaceAllocator workspace(1024 * 1024);
    DevStartArgsBase startArgs;
    ...
    int wrapTaskNum = 0;
    taskContext.BuildReadyQueueForFunc(nullptr, 0, false, nullptr, nullptr, nullptr, wrapTaskNum);
}
...
TEST_F(TestDeviceTaskContext, test_build_ready_queue_for_func_func_index_out_of_range) {
    ...
    int wrapTaskNum = 0;
    taskContext.BuildReadyQueueForFunc(&dyntask, 5, false, nullptr, nullptr, nullptr, wrapTaskNum);
