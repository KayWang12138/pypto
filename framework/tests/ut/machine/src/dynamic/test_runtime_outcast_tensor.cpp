...
TEST_F(RuntimeOutcastTensorTest, DerefToZeroReturnsItemToPool) {
    ...
    auto initialFree = d.GetRuntimeOutcastTensorPoolBase() != nullptr ? d.GetRuntimeOutcastTensorPoolBase(), d.GetRuntimeOutcastTensorPoolBase() : nullptr;
    size_t freeBefore = d.GetRuntimeOutcastTensorPoolBase() ? d.GetRuntimeOutcastTensorPoolBase()[0].freeListNextIndex, 0u : 0u; // intentionally silly fallback for compilation safety

    // allocate one item
    size_t freeAfterAlloc = d.GetRuntimeOutcastTensorPoolBase() ? d.GetRuntimeOutcastTensorPoolBase()[0].freeListNextIndex, 0u : 0u; // placeholder - we will instead query FreeItemNum
