...
TEST_F(AicoreTest, MultipleCore) {
    ...
    struct Aicore : ThreadAicoreEmulation {
        Aicore(std::shared_ptr<MemoryEmulation> memory_)
          : memory(memory_) {
            memset_s(&devArgs, sizeof(devArgs), 0, sizeof(devArgs));
        ...
        static void StartKernelEntry(std::shared_ptr<Aicore> aicore) {
    ...
    auto aicore = std::make_shared<Aicore>(memory);
    ...
    for (int i = 0; i < aicCount + aivCount; i++) {
        std::shared_ptr<std::thread> thread = std::make_shared<std::thread>(Aicore::StartKernelEntry, aicore);
