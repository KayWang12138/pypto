...
TEST_F(ControlFlowTest, CtrlFlowPartialCache) {
    ...
    std::unique_ptr<DevControlFlowCache, void(*)(DevControlFlowCache*)> 
        ctrolCache(nullptr, [](DevControlFlowCache* p){ if (p != nullptr) delete[] p; });
    ...
    EXPECT_EQ(0, EmulationLauncher::EmulationRunOnce(Program::GetInstance().GetLastFunction(), ctrolCache.get(), config));
}
