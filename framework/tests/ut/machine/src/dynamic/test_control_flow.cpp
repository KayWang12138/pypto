...
TEST_F(ControlFlowTest, TestMainBlock) {
    ...
    FUNCTION("main", {tensor_a, tensor_b}, {output_tensor}) {
        LOOP("L0", FunctionType::DYNAMIC_LOOP, loop_idx, LoopRange(2)) {
            Tensor middle_tensor(DT_INT32, {tensor_dim, tensor_dim}, "O");
            LOOP("s0", FunctionType::DYNAMIC_LOOP, row_idx, LoopRange(4)) {
                LOOP("s1", FunctionType::DYNAMIC_LOOP, col_idx, LoopRange(4)) {
