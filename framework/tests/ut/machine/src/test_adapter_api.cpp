...
TEST_F(TestAdapterApi, test_hal_adapter) {
    ...
    EXPECT_EQ(AdapterManager::Instance().GetHalAdapter().GetFunction(HalFunc::ResMap) != nullptr, false);
...
TEST_F(TestAdapterApi, test_hccl_adapter) {
    bool hasCann = std::getenv("ASCEND_HOME_PATH") != nullptr;
    EXPECT_EQ(AdapterManager::Instance().GetHcclAdapter().GetFunction(HcclFunc::GetCommName) != nullptr, hasCann);
    EXPECT_EQ(AdapterManager::Instance().GetHcclAdapter().GetFunction(HcclFunc::GetL0TopoTypeEx) != nullptr, hasCann);
    EXPECT_EQ(AdapterManager::Instance().GetHcclAdapter().GetFunction(HcclFunc::GetCommHandleByGroup) != nullptr, hasCann);
    EXPECT_EQ(AdapterManager::Instance().GetHcclAdapter().GetFunction(HcclFunc::GetRootInfo) != nullptr, hasCann);
    EXPECT_EQ(AdapterManager::Instance().GetHcclAdapter().GetFunction(HcclFunc::CommInitRootInfo) != nullptr, hasCann);
    EXPECT_EQ(AdapterManager::Instance().GetHcclAdapter().GetFunction(HcclFunc::CommDestroy) != nullptr, hasCann);
    EXPECT_EQ(AdapterManager::Instance().GetHcclAdapter().GetFunction(HcclFunc::AllocComResourceByTiling) != nullptr, hasCann);
}
...
TEST_F(TestAdapterApi, test_runtime_adapter) {
    ...
    EXPECT_EQ(AdapterManager::Instance().GetRuntimeAdapter().GetFunction(RuntimeFunc::Memset) != nullptr, hasCann);
    EXPECT_EQ(AdapterManager::Instance().GetRuntimeAdapter().GetFunction(RuntimeFunc::Memcpy) != nullptr, hasCann);
