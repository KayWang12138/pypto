...
TEST_F(TestAdapterApi, test_acl_api) {
    EXPECT_EQ(AclInit(nullptr), ACL_RT_SUCCESS);
    EXPECT_EQ(AclFinalize(), ACL_RT_SUCCESS);
    EXPECT_EQ(AclRtMemcpy(nullptr, 0, nullptr, 0, AclRtMemcpyKind::HOST_TO_HOST), ACL_RT_SUCCESS);
    EXPECT_EQ(AclRtSetDevice(0), ACL_RT_SUCCESS);
    EXPECT_EQ(AclRtResetDevice(0), ACL_RT_SUCCESS);
    EXPECT_EQ(AclRtCreateEvent(nullptr), ACL_RT_SUCCESS);
    EXPECT_EQ(AclRtRecordEvent(nullptr, nullptr), ACL_RT_SUCCESS);
    EXPECT_EQ(AclRtCreateEventExWithFlag(nullptr, 0), ACL_RT_SUCCESS);
    EXPECT_EQ(AclRtStreamWaitEvent(nullptr, nullptr), ACL_RT_SUCCESS);
    EXPECT_EQ(AclRtGetStreamResLimit(nullptr, AclRtDevResLimitType::CUBE_CORE, nullptr), ACL_RT_SUCCESS);
    EXPECT_EQ(AclRtGetStreamAttribute(nullptr, AclRtStreamAttr::FAILURE_MODE, nullptr), ACL_RT_SUCCESS);
    EXPECT_EQ(AclRtCacheLastTaskOpInfo(nullptr, 0), ACL_RT_SUCCESS);
    EXPECT_EQ(AclRtSetExceptionInfoCallback(nullptr), ACL_RT_SUCCESS);
    EXPECT_EQ(AclMdlRICaptureGetInfo(nullptr, nullptr, nullptr), ACL_RT_SUCCESS);
    EXPECT_EQ(AclMdlRICaptureThreadExchangeMode(nullptr), ACL_RT_SUCCESS);
}
