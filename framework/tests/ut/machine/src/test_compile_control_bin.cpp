...
TEST_F(TestCompileControlBin, TieFwkAicpuPreCompile_CompileFails) {
    ...
    ofs << "int main() { return 0; }" << std::endl;
    ofs.close();
...
TEST_F(TestCompileControlBin, SharedAicpuCompile_CompileFails) {
    ...
    // The linking command will fail because the cross-compiler is not available
    bool result = SharedAicpuCompile("test_func", aicpuDir, "");
    EXPECT_FALSE(result);
}
