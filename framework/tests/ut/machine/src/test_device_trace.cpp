...
TEST_F(TestDeviceTrace, TraceInfo_FormatBasicMessage)
{
    std::string result = TraceInfo("test.cpp", 100, "testFunc", "Basic message %d", 42);
...
TEST_F(TestDeviceTrace, TraceInfo_FormatMultipleArgs)
{
    std::string result = TraceInfo("file.cpp", 200, "myFunc", "Args: %d, %s, %f", 123, "test", 3.14);
    EXPECT_NE(result.find("file.cpp"), std::string::npos);
    EXPECT_NE(result.find("200"), std::string::npos);
    EXPECT_NE(result.find("myFunc"), std::string::npos);
    EXPECT_NE(result.find("123"), std::string::npos);
    EXPECT_NE(result.find("test"), std::string::npos);
}

TEST_F(TestDeviceTrace, TraceInfo_WithNoFileNameSlash)
{
    std::string result = TraceInfo("nofile.cpp", 50, "func", "Simple message");
    EXPECT_NE(result.find("nofile.cpp"), std::string::npos);
