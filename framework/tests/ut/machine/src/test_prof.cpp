...
public:
    ...
    void SetUp() override
    {
        AiCoreProf::devProfSwitch_ = 0;
        AiCoreProf::devProfType_ = 0;
    }
...
protected:
    ...
    struct PmuTestEnv {
        ...
        PmuTestEnv(size_t regBufSize)
        {
            ...
            memset(regAddrsArr, 0, sizeof(regAddrsArr));
            regAddrsArr[0] = reinterpret_cast<int64_t>(addr);

            memset(pmuEventAddrsArr, 0, sizeof(pmuEventAddrsArr));
...
TEST_F(TestPro, test_check_level_l2_only)
{
    auto aicpuTaskPtr = CreateAicpuTaskManager();
    auto aicoreMng = CreateAiCoreManager(aicpuTaskPtr.get());
    AiCoreProf prof(*aicoreMng);

    auto data = CreateProfCommandHandle(65597, 1);
    auto size = sizeof(PyPtoMsprofCommandHandle);
...
TEST_F(TestPro, test_check_level_l2_and_l3)
{
    auto aicpuTaskPtr = CreateAicpuTaskManager();
    auto aicoreMng = CreateAiCoreManager(aicpuTaskPtr.get());
    AiCoreProf prof(*aicoreMng);

    auto data = CreateProfCommandHandle(65661, 1);
    auto size = sizeof(PyPtoMsprofCommandHandle);
