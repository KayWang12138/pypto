...
TEST_F(TestExprBatchGenerator, CheckExprDependCoreTest) {
    ...
    bool dependsCore = SymbolicExpressionTable::CheckExprDependCore(getInputDataExpr, tensorNameToDependCore);
    ASSERT_TRUE(dependsCore);

    tensorNameToDependCore[testTensorName] = false;
    bool dependsCoreNonExistent = SymbolicExpressionTable::CheckExprDependCore(getInputDataExpr, tensorNameToDependCore);
    ASSERT_FALSE(dependsCoreNonExistent);
}
