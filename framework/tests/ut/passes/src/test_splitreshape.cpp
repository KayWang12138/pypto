...
void BuildGraphForCollectCopyOut(std::shared_ptr<Function> &func, std::shared_ptr<LogicalTensor> &input1,
    ...
    std::shared_ptr<LogicalTensor> &output, std::vector<int64_t> &offset1, std::vector<int64_t> &offset2,
...
TEST_F(TestSplitReshapePass, TestBeCoveredSTest) {
    ...
    RunPassStra(*func, PassName::SPLIT_RESHAPE);
    CheckOpReshape(func, CheckReshapeStruct{tiledorigShape, kSizeOne, false, {}, tiledreshapeShape, kSizeTwo, true,
                             tiledviewShape, kNumTwo});
}
...
TEST_F(TestSplitReshapePass, TestDynPerfectlyMatchSTest) {
    ...
    std::vector<std::string> expectReshapeDynShape = {"RUNTIME_Max(((RUNTIME_GetViewValidShapeDim(a0,0,2)*RUNTIME_Ne("
                                                      "RUNTIME_GetViewValidShapeDim(a0,0,2), 0))-0), 0)",
...
TEST_F(TestSplitReshapePass, TestDynBeCoveredSTest) {
    ...
    std::vector<std::string> expectValidShape1 = {"4",
        "RUNTIME_Max(((RUNTIME_GetViewValidShapeDim(a,0,2)*RUNTIME_Ne(RUNTIME_GetViewValidShapeDim(a,0,2), 0))-0), 0)"};
    std::vector<std::string> expectValidShape2 = {"4", "RUNTIME_Max((((RUNTIME_GetViewValidShapeDim(a,2,2)+2)*RUNTIME_"
                                                       "Ne(RUNTIME_GetViewValidShapeDim(a,2,2), 0))-2), 0)"};
    std::unordered_map<LogicalTensorPtr, std::vector<int64_t>> expectAssembleOffset = {
        {inputs[0], assembleOffset1},
        {inputs[1], assembleOffset2}
    };
    std::unordered_map<LogicalTensorPtr, std::vector<std::string>> expectValidShapes = {
        {inputs[0], expectValidShape1},
...
TEST_F(TestSplitReshapePass, TestDynPerfectlyMatchWithAllSTest) {
    ...
    std::vector<std::string> expectValidShape = {"2", "2", "2",
        "RUNTIME_Max(((RUNTIME_GetViewValidShapeDim(a,0,2)*RUNTIME_Ne(RUNTIME_GetViewValidShapeDim(a,0,2), 0))-0), 0)"};
    std::unordered_map<LogicalTensorPtr, std::vector<std::string>> expectValidShapes = {
...
void BuildMultipleParallelCopyOutAssembleGraph(std::shared_ptr<Function> &func,
    const MultipleParallelCopyOutAssembleShapes &shapes, std::vector<std::shared_ptr<LogicalTensor>> &inputs,
...
void VerifyMultipleParallelCopyOutAssembleSplit(std::shared_ptr<Function> &func,
    const std::vector<int64_t> &expectedInputShape, const std::vector<int64_t> &expectedOutputShape,
    int expectedCount) {
