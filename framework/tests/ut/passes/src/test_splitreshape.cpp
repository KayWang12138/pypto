...
// Cover split_reshape.cpp lines 393, 420, 426, 430, 454, 492, 537, 541 (ShapeAlign/UpdateShapeOffset/ConstructShapeOffset/RawToAlign/AlignToRaw error paths)
TEST_F(TestSplitReshapePass, TestShapeAlignIndexExceeded) {
    auto currFunctionPtr = std::make_shared<Function>(Program::GetInstance(), "TestReshapeSplit", "TestReshapeSplit", nullptr);
    ASSERT_TRUE(currFunctionPtr != nullptr);
    SplitReshape pass;
    std::vector<int64_t> shape1 = {kNumTwo, kNumTwo, kNumTwo};  // product 8
    std::vector<int64_t> shape2 = {kNumEight};                  // one dim 8, so i2 reaches end first
    std::vector<int64_t> alignedShape;
