...
TEST_F(MixDependencyAnalyzerTest, UTest1) {
    ...
    analyzer.allIncasts = allIncasts;
    analyzer.allOutcasts = allOutcasts;
    analyzer.PropagateExternalDependenciesWithClosure(dependencies);
    ...
    EXPECT_EQ(analyzer.allIncasts[kNum0].size(), kNum1);
    EXPECT_EQ(analyzer.allOutcasts[kNum0].size(), kNum2);
    EXPECT_EQ(analyzer.allIncasts[kNum1].size(), kNum1);
    EXPECT_EQ(analyzer.allOutcasts[kNum1].size(), kNum2);
    EXPECT_EQ(analyzer.allIncasts[kNum2].size(), kNum2);
    EXPECT_EQ(analyzer.allOutcasts[kNum2].size(), kNum1);
    EXPECT_EQ(analyzer.allIncasts[kNum3].size(), kNum3);
    EXPECT_EQ(analyzer.allOutcasts[kNum3].size(), kNum1);
    EXPECT_EQ(analyzer.allIncasts[kNum4].size(), kNum3);
    EXPECT_EQ(analyzer.allOutcasts[kNum4].size(), kNum1);
}
...
TEST_F(MixDependencyAnalyzerTest, UTest2) {
    ...
    analyzer.allOutcasts = allOutcasts;
    analyzer.PropagateExternalDependenciesWithClosure(dependencies);
    analyzer.CollectInternalDependencies(dependencies, components);
    EXPECT_EQ(analyzer.internalDeps.size(), kNum4);
    EXPECT_TRUE(CheckPath(kNum0, kNum1, analyzer.internalDeps));
    EXPECT_TRUE(CheckPath(kNum0, kNum4, analyzer.internalDeps));
    EXPECT_TRUE(CheckPath(kNum1, kNum4, analyzer.internalDeps));
    ...
    EXPECT_EQ(analyzer.allIncasts[kNum3].size(), kNum1); // incast2
    EXPECT_EQ(analyzer.allOutcasts[kNum3].size(), kNum1); // outcast1
    EXPECT_EQ(analyzer.allIncasts[kNum4].size(), kNum2); // incast1, incast2
    EXPECT_EQ(analyzer.allOutcasts[kNum4].size(), kNum1); // outcast1
}
} // namespace tile_fwk
} // namespace npu
