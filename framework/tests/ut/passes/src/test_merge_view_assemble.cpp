...
TEST_F(MergeViewAssembleTest, ViewChainDifferentScopeIdsShouldNotMerge)
{
    Program program;
    std::string funcMagicName = "test_view_diff_scope";
    std::string funcRawName = "test_view_diff_scope_raw";
    ...
    // VIEW1(scopeId=1) -> VIEW2(scopeId=2), memory types compatible, should NOT merge
    auto midTensor = std::make_shared<LogicalTensor>(*function, DataType::DT_FP32, std::vector<int64_t>{8, 8});
    auto view1Attr = std::make_shared<ViewOpAttribute>(
    ...
    auto& view2Op = function->AddRawOperation(Opcode::OP_VIEW, {midTensor}, {outputTensor});
    view2Op.SetOpAttribute(view2Attr);
    view2Op.SetScopeId(2);

    MergeViewAssemble mergePass;
    ASSERT_EQ(mergePass.RunOnFunction(*function), SUCCESS);

    // Verify: 2 VIEWs remain (not merged due to different scopeIds)
    int viewCount = 0;
    for (auto& op : function->Operations()) {
        if (op.GetOpcode() == Opcode::OP_VIEW && !op.IsDeleted()) {
            viewCount++;
...
TEST_F(MergeViewAssembleTest, AssembleChainSameScopeIdsShouldMerge)
{
    ComputationalGraphBuilder G;
    ...
    // Verify: ASSEMBLEs merged into 1, with scopeId=1
    int assembleCount = 0;
    int mergedScopeId = -2;
    for (auto& op : function->Operations()) {
        if (op.GetOpcode() == Opcode::OP_ASSEMBLE && !op.IsDeleted()) {
