...
/*
 * TestAdjustReshapeCfg_NoIsCubeAttr
 * Test RESHAPE op without isCube attribute
 * Expected: coreType_ should not be changed to AIC
 */
TEST_F(InsertSyncTest, TestAdjustReshapeCfg_NoIsCubeAttr) {
    auto rootFuncPtr = std::make_shared<Function>(Program::GetInstance(), "TestNoIsCube", "TestNoIsCube", nullptr);
    rootFuncPtr->rootFunc_ = rootFuncPtr.get();
    auto currFunctionPtr = std::make_shared<Function>(Program::GetInstance(), "TestNoIsCubeLeaf", "TestNoIsCubeLeaf", rootFuncPtr.get());
    rootFuncPtr->rootFunc_->programs_.emplace(currFunctionPtr->GetFuncMagic(), currFunctionPtr.get());
    ...
    auto &reshapeOp = currFunctionPtr->AddRawOperation(Opcode::OP_RESHAPE, {tensor1}, {tensor2});

    PipeSync ps;
    auto opcfg = OpcodeManager::Inst().GetTileOpCfg(Opcode::OP_RESHAPE);
    CoreType originalCoreType = opcfg.coreType_;

    EXPECT_EQ(ps.AdjustReshapeCfg(opcfg, reshapeOp), SUCCESS);
    EXPECT_EQ(opcfg.coreType_, originalCoreType);
}

/*
 * TestAdjustReshapeCfg_IsCubeFalse
 * Test RESHAPE op with isCube attribute set to false
 * Expected: coreType_ should not be changed to AIC
 */
TEST_F(InsertSyncTest, TestAdjustReshapeCfg_IsCubeFalse) {
    auto rootFuncPtr = std::make_shared<Function>(Program::GetInstance(), "TestIsCubeFalse", "TestIsCubeFalse", nullptr);
    rootFuncPtr->rootFunc_ = rootFuncPtr.get();
    auto currFunctionPtr = std::make_shared<Function>(Program::GetInstance(), "TestIsCubeFalseLeaf", "TestIsCubeFalseLeaf", rootFuncPtr.get());
    rootFuncPtr->rootFunc_->programs_.emplace(currFunctionPtr->GetFuncMagic(), currFunctionPtr.get());
    ...
    auto &reshapeOp = currFunctionPtr->AddRawOperation(Opcode::OP_RESHAPE, {tensor1}, {tensor2});
    reshapeOp.SetAttr(OpAttributeKey::isCube, false);
...
/*
 * TestAdjustReshapeCfg_IsCubeTrue
 * Test RESHAPE op with isCube attribute set to true
 * Expected: coreType_ should be changed to AIC
 */
TEST_F(InsertSyncTest, TestAdjustReshapeCfg_IsCubeTrue) {
    auto rootFuncPtr = std::make_shared<Function>(Program::GetInstance(), "TestIsCubeTrue", "TestIsCubeTrue", nullptr);
    rootFuncPtr->rootFunc_ = rootFuncPtr.get();
    auto currFunctionPtr = std::make_shared<Function>(Program::GetInstance(), "TestIsCubeTrueLeaf", "TestIsCubeTrueLeaf", rootFuncPtr.get());
    rootFuncPtr->rootFunc_->programs_.emplace(currFunctionPtr->GetFuncMagic(), currFunctionPtr.get());
    ...
    auto &reshapeOp = currFunctionPtr->AddRawOperation(Opcode::OP_RESHAPE, {tensor1}, {tensor2});
    reshapeOp.SetAttr(OpAttributeKey::isCube, true);

    PipeSync ps;
    auto opcfg = OpcodeManager::Inst().GetTileOpCfg(Opcode::OP_RESHAPE);

    EXPECT_EQ(ps.AdjustReshapeCfg(opcfg, reshapeOp), SUCCESS);
    EXPECT_EQ(opcfg.coreType_, CoreType::AIC);
}
} // namespace tile_fwk
} // namespace npu
