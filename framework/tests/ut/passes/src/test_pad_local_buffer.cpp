...
TEST_F(TestPadLocalBuffer, L1toBt1) {
    ComputationalGraphBuilder graph;
    // bias
    EXPECT_EQ(graph.AddTensor(DataType::DT_FP16, {1, 16}, MemoryType::MEM_DEVICE_DDR, "t1"), true);
    EXPECT_EQ(graph.AddTensor(DataType::DT_FP16, {1, 16}, MemoryType::MEM_L1, "t2"), true);
    EXPECT_EQ(graph.AddOp(Opcode::OP_COPY_IN, {"t1"}, {"t2"}, "COPY1", true), true);
    EXPECT_EQ(graph.AddTensor(DataType::DT_FP32, {1, 16}, MemoryType::MEM_BT, "t3"), true);
    ...
    // b
    EXPECT_EQ(graph.AddTensor(DataType::DT_INT8, {32, 16}, MemoryType::MEM_DEVICE_DDR, "t1b"), true);
    EXPECT_EQ(graph.AddTensor(DataType::DT_INT8, {32, 16}, MemoryType::MEM_L1, "t2b"), true);
    EXPECT_EQ(graph.AddOp(Opcode::OP_COPY_IN, {"t1b"}, {"t2b"}, "COPYB", true), true);
    EXPECT_EQ(graph.AddTensor(DataType::DT_INT8, {1, 16}, MemoryType::MEM_L0B, "t3b"), true);
    EXPECT_EQ(graph.AddOp(Opcode::OP_L1_TO_L0_BT, {"t2b"}, {"t3b"}, "L1TOL0B", true), true);
    // fix
    EXPECT_EQ(graph.AddTensor(DataType::DT_UINT64, {1, 16}, MemoryType::MEM_DEVICE_DDR, "t1f"), true);
    EXPECT_EQ(graph.AddTensor(DataType::DT_UINT64, {1, 16}, MemoryType::MEM_L1, "t2f"), true);
    EXPECT_EQ(graph.AddOp(Opcode::OP_COPY_IN, {"t1f"}, {"t2f"}, "COPYFIX", true), true);
    EXPECT_EQ(graph.AddTensor(DataType::DT_UINT64, {1, 16}, MemoryType::MEM_FIX_QUANT_PRE, "t3f"), true);
    EXPECT_EQ(graph.AddOp(Opcode::OP_L1_TO_FIX_QUANT_PRE, {"t2f"}, {"t3f"}, "L1TOFIX", true), true);
    // amulb
    EXPECT_EQ(graph.AddTensor(DataType::DT_FP16, {32, 16}, MemoryType::MEM_L0C, "out"), true);
    ...
    padLocalBufferTest.RunOnFunction(*currFunctionPtr);
    auto t2 = graph.GetTensor("t2");
    EXPECT_EQ(t2->GetShape(), (std::vector<int64_t>{1, 32}));
    auto t3 = graph.GetTensor("t3");
    EXPECT_EQ(t3->GetShape(), (std::vector<int64_t>{1, 32}));
}

TEST_F(TestPadLocalBuffer, L1toBt2) {
    ComputationalGraphBuilder graph;
    // bias
    EXPECT_EQ(graph.AddTensor(DataType::DT_FP16, {1, 33}, MemoryType::MEM_DEVICE_DDR, "t1"), true);
    EXPECT_EQ(graph.AddTensor(DataType::DT_FP16, {1, 33}, MemoryType::MEM_L1, "t2"), true);
    EXPECT_EQ(graph.AddOp(Opcode::OP_COPY_IN, {"t1"}, {"t2"}, "COPY1", true), true);
    EXPECT_EQ(graph.AddTensor(DataType::DT_FP32, {1, 33}, MemoryType::MEM_BT, "t3"), true);
    EXPECT_EQ(graph.AddOp(Opcode::OP_L1_TO_BT, {"t2"}, {"t3"}, "L1TOBT", true), true);
    // amulb
    EXPECT_EQ(graph.AddTensor(DataType::DT_FP16, {32, 33}, MemoryType::MEM_L0C, "out"), true);
    EXPECT_EQ(graph.AddOp(Opcode::OP_A_MUL_B, {"t3"}, {"out"}, "AMULB", true), true);

    auto *currFunctionPtr = graph.GetFunction();
    PadLocalBuffer padLocalBufferTest;
    padLocalBufferTest.RunOnFunction(*currFunctionPtr);
    auto t2 = graph.GetTensor("t2");
    EXPECT_EQ(t2->GetShape(), (std::vector<int64_t>{1, 64}));
    auto t3 = graph.GetTensor("t3");
    EXPECT_EQ(t3->GetShape(), (std::vector<int64_t>{1, 64}));
}