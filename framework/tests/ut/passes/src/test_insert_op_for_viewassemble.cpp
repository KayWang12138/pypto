...
TEST_F(TestInsertCopyPass, TestOneTensorNAssembleDDR) {
    ...
    auto &assOp00 = currFunctionPtr->AddOperation(Opcode::OP_ASSEMBLE, {inTensor0}, {outTensor0});
    assOp00.SetOpAttribute(std::make_shared<AssembleOpAttribute>(MemoryType::MEM_DEVICE_DDR, offset0));
    auto &assOp10 = currFunctionPtr->AddOperation(Opcode::OP_ASSEMBLE, {inTensor1}, {outTensor0});
