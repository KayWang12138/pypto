...
// 公共：构建两子图 a->b->c，执行 DoPreCheck，设置 color 图并执行 DoPostCheck
static void RunColorOutGraphCheckTest(
    ...
    G.GetTensor("a")->subGraphID = 0;
    G.GetTensor("b")->subGraphID = 0;
    G.GetTensor("c")->subGraphID = 1;
    G.GetTensor("b")->isSubGraphBoundary = true;
    for (const auto& name : {"a", "b", "c"}) {
        G.GetTensor(name)->SetMemoryTypeBoth(MemoryType::MEM_UB);
    }
...
TEST_F(SubgraphToFunctionCheckTest, ColorOutGraphCheck_EdgeMissedInColorOutGraph_Fail) {
    RunColorOutGraphCheckTest("ColorTest_EdgeMissed",
        {{}, {0}}, {{}, {}},
        FAILED);
}
