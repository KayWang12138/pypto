...
//test preGraph.UpdateCopyOpIsCube 校验cube属性传递
...
// test cube_process.UpdateL0cDtype 校验mul的结果数据类型
TEST_F(PreGraphTest, TestUpdateL0Dtype_FP16) {
    ComputationalGraphBuilder G;
    // add tensor
...
// test setCopyAttr.ProcessSpecialMTEOperation 校验TransposeMoveout的opattr来自output
TEST_F(PreGraphTest, TestprocessSpecialMTEOperation) {
    ComputationalGraphBuilder G;
    // add tensor
...
//vec_in(UB) - CopyOut - vec_out 
//to
//CopyIn - vec_in(UB) - CopyOut - vec_out 
// test SetTensorBoundary.InsertTemporaryCopyIn 插入CopyIN
TEST_F(PreGraphTest, TestInsertTemporaryCopyIn) {
    ComputationalGraphBuilder G;
    // add tensor
    DataType inputAstDtype = DataType::DT_FP16;
    std::vector<int64_t> shape = {16, 16};
    G.AddTensor(inputAstDtype, shape, MemoryType::MEM_UB, "vec_in");
    auto vec_in = G.GetTensor("vec_in");
    G.AddTensor(inputAstDtype, shape, MemoryType::MEM_DEVICE_DDR, "vec_out");
    auto vec_out = G.GetTensor("vec_out");
    // add op
    G.AddOp(Opcode::OP_REMOTE_REDUCE, {"vec_in"}, {"vec_out"}, "copy_out");
    // set incast and outcast
    G.SetInCast({"vec_in"});
    G.SetOutCast({"vec_out"});
    //before
    EXPECT_EQ(vec_in->GetProducers().size(), 0);
    EXPECT_EQ(vec_in->GetMemoryTypeOriginal(), MemoryType::MEM_UB);
    Function *function = G.GetFunction();
    int beforeOpNum = function->Operations().size();
    //after
    PreGraphProcess preGraph;
    preGraph.Run(*function, "", "", 0);
    EXPECT_EQ(function->Operations().size(), beforeOpNum + 1);
}
...
//test SetBoundary.SetTensorBoundary 校验边界属性是否成功赋值                      
...
//wrong case -- shape[i]存在0
//                                      Assemble - vec_out1
//vec_in - Reshape - reshape_out{16, 0} 
//                                      Assemble - vec_out2
TEST_F(PreGraphTest, TestDivideByZero) {
    ComputationalGraphBuilder G;
    // add tensor
    DataType inputAstDtype = DataType::DT_FP16;
    std::vector<int64_t> shape = {16, 16};
    ...
    auto vec_in = G.GetTensor("vec_in");
    //wrong
    G.AddTensor(inputAstDtype, {16, 0}, MemoryType::MEM_DEVICE_DDR, "reshape_out");
    auto reshape_out = G.GetTensor("reshape_out");
    G.AddTensor(inputAstDtype, shape, MemoryType::MEM_DEVICE_DDR, "vec_out1");
    auto vec_out1 = G.GetTensor("vec_out1");
    // add op
    G.AddOp(Opcode::OP_RESHAPE, {"vec_in"}, {"reshape_out"}, "op_reshape");
    G.AddOp(Opcode::OP_ASSEMBLE, {"reshape_out"}, {"vec_out1"}, "op_assemble");
    std::vector<int64_t> assembleOffset = {0, 0};
    auto attrAssemble = std::make_shared<AssembleOpAttribute>(
        MemoryType::MEM_DEVICE_DDR, assembleOffset, SymbolicScalar::FromConcrete(assembleOffset));
    auto op_assemble = G.GetOp("op_assemble");
    op_assemble->SetOpAttribute(attrAssemble);
    // set incast and outcast
    G.SetInCast({"vec_in"});
    G.SetOutCast({"vec_out1"});
    G.SetOutCast({"vec_out2"});

    Function *function = G.GetFunction();
    RemoveRedundantAssemble removeRedundantAssemble;
    EXPECT_EQ(removeRedundantAssemble.DeleteRedundantAssemble(*function), FAILED);
}// namespace tile_fwk
