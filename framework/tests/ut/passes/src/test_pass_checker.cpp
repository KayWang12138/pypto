...
TEST_F(PassCheckTest, TestCheckOpIOValid_InputIsNull) {
    auto currFunctionPtr =
        std::make_shared<Function>(Program::GetInstance(), "TestCheckOpIOValid_InputIsNull", "TestCheckOpIOValid_InputIsNull", nullptr);
    EXPECT_TRUE(currFunctionPtr != nullptr);

    std::vector<int64_t> shape = {32, 32};
    auto mockTensor = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);

    bool inputIsNullFailed = false;
    std::vector<LogicalTensorPtr> mockInputTensors1 = {nullptr};
    for (const auto& input : mockInputTensors1) {
        if (input == nullptr) {
            inputIsNullFailed = true;
            break;
        }
    }
    EXPECT_TRUE(inputIsNullFailed);

    bool producerIsNullFailed = false;
    std::set<Operation*, LogicalTensor::CompareOp> mockProducers;
    mockProducers.insert(nullptr);
    for (const auto& producer : mockProducers) {
        if (producer == nullptr) {
            producerIsNullFailed = true;
            break;
        }
    }
    EXPECT_TRUE(producerIsNullFailed);

    bool consumerIsNullFailed = false;
    std::set<Operation*, LogicalTensor::CompareOp> mockConsumers;
    mockConsumers.insert(nullptr);
    for (const auto& consumer : mockConsumers) {
        if (consumer == nullptr) {
            consumerIsNullFailed = true;
            break;
        }
    }
    EXPECT_TRUE(consumerIsNullFailed);
}
