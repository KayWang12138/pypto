...
TEST_F(TestCodegenDynScatter, TestDynOpScatterElementFp16) {
    ...
    std::string res = cop.GenOpCode();
    std::string expect =
        R"!!!(TileOp::DynTscatterElementS<half, int64_t, half, 1, 1, 32, 1, 64, 64, 3, 0>((__ubuf__ half*)UB_S0_E0, (__ubuf__ half*)UB_S0_E0, (__ubuf__ int64_t*)UB_S0_E0, (half)1, 1, 1, 1, 32);
)!!!";
    EXPECT_EQ(res, expect);
}
