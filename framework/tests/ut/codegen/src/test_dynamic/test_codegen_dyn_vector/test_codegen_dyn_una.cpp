...
>>>>>>> e5597ce9d (fix(operation): Fix pad with tfillpad)
        }
    }
    auto function =
        Program::GetInstance().GetFunctionByRawName(FUNCTION_PREFIX + funcName + SUB_FUNC_SUFFIX + HIDDEN_FUNC_SUFFIX);
...
TEST_F(TestCodegenDynUna, TestPadDynamicFP16) {
    ...
    codeGen.GenCode(*function, {});
    const std::string res = GetResultFromCpp(*function);
    std::string expect = R"!!!(TPad<static_cast<pto::PadValue>(0x400000000001ULL)>(ubTensor_2, ubTensor_0);)!!!";
    CheckStringExist(expect, res);
}

TEST_F(TestCodegenDynUna, TestPadDynamicBF16) {
    config::SetCodeGenConfig(KEY_CODEGEN_SUPPORT_TILE_TENSOR, true);
    int S0 = 6;
    int S1 = 12;
    int D0 = 12;
    ...
    std::string funcName = "TestPadDynamicBF16";
    FUNCTION(funcName, {input, output}) {
        LOOP(funcName, FunctionType::DYNAMIC_LOOP, i, LoopRange(1)) {
            (void)i;
            output = Pad(input, {0, 6, 0, 8}, "constant", 2.0f);
