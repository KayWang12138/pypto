...
std::string generateCodeForOp(TestContext &ctx) {
    std::shared_ptr<SymbolManager> symbolManager = std::make_shared<SymbolManager>();
    CodeGenCtx ctx;
    CodeGenCloudNPU cga(ctx);
    cga.GenAllocForLocalBuffer(*ctx.op, symbolManager);
    CodeGenOpCloudNPUCtx opCtx(symbolManager, *ctx.function, *ctx.function->rootFunc_->program_[0], *ctx.op, {}, true);
