...
TEST_F(TestCodegenDynSort, TestDynTopkMerge) {
    ...
    std::string funcName = "TestDynTopkMerge";
    FUNCTION(funcName, {inputA, inputB, output}) {
        LOOP(funcName, FunctionType::DYNAMIC_LOOP, i, LoopRange(1)) {
            (void)i;
            output = Add(inputA, inputB);
    ...
    auto &op = function->AddOperation(Opcode::OP_TOPK_MERGE, {xVar}, {yVar});
    op.SetAttribute("GmTensorParamIdxInCallFunc", 0);
    op.SetAttribute(OP_ATTR_PREFIX + "axis", 0);
    SymbolicScalar startIdx(1);
    op.SetAttribute(OpAttributeKey::dynScalar, startIdx);
    ...
    std::string res = cop.GenOpCode();
    std::string expect =
        R"!!!(TileOp::DynTopKMerge<float, 64, 32, 0>((__ubuf__ float*)UB_S0_E0, (__ubuf__ float*)UB_S0_E0);
)!!!";
    EXPECT_EQ(res, expect);
}
