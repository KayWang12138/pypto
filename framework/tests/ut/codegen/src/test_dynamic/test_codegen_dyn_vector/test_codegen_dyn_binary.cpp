...
TEST_F(TestCodegenDynBinary, TestDivHighPrecisionFP16)
{
    ...
    const std::string res = GetResultFromCpp(*function);
    std::string expect = R"!!!(TDiv<DivAlgorithm::HIGH_PRECISION, LastUse2Dim<0, 0>>(ubTensor_2, ubTensor_0, ubTensor_1);
)!!!";
    CheckStringExist(expect, res);
}

TEST_F(TestCodegenDynBinary, TestDivHighPrecisionBF16)
{
    config::SetCodeGenConfig(KEY_CODEGEN_SUPPORT_TILE_TENSOR, true);
    ...
    Tensor input_b(DataType::DT_BF16, shape, "B");
    Tensor output(DataType::DT_BF16, shape, "C");

    std::string funcName = "TestDivHighPrecisionBF16";
    FUNCTION(funcName, {input_a, input_b, output})
    {
        LOOP(funcName, FunctionType::DYNAMIC_LOOP, i, LoopRange(1))
        {
...
TEST_F(TestCodegenDynBinary, TestDivDefaultPrecision)
{
    config::SetCodeGenConfig(KEY_CODEGEN_SUPPORT_TILE_TENSOR, true);
    ...
    const std::string res = GetResultFromCpp(*function);
    std::string expect = R"!!!(TDiv<DivAlgorithm::DEFAULT, LastUse2Dim<0, 0>>(ubTensor_2, ubTensor_0, ubTensor_1);
)!!!";
    CheckStringExist(expect, res);
}
