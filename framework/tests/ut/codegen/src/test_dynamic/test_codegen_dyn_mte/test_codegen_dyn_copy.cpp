...
TEST_F(TestCodegenDynCopy, TestGatherInL1TileTensor) {
    ...
    std::string res = cop.GenOpCode();
    std::string expect =
        R"!!!(TGatherInL1<0>(l1Tensor_1, gmTensor_2, gmTensor_2, gmTensor_2, Coord1Dim(0), Coord2Dim(GET_PARAM_OFFSET_BY_IDX(param, 0, -1, 2, 0), GET_PARAM_OFFSET_BY_IDX(param, 0, -1, 2, 1)), Coord2Dim(GET_PARAM_OFFSET_BY_IDX(param, 0, -1, 2, 0), GET_PARAM_OFFSET_BY_IDX(param, 0, -1, 2, 1)));
)!!!";
    EXPECT_EQ(res, expect);
}
...
TEST_F(TestCodegenDynCopy, L1CopyInTensor) {
    ...
    std::string expect =
        R"!!!(TLoad<CopyInMode::ND2NZ>(l1Tensor_1, gmTensor_2, Coord2Dim(GET_PARAM_OFFSET_2(param, 0, -1)), GET_PARAM_RAWSHAPE_BY_IDX(param, 0, -1, 2, 0), GET_PARAM_RAWSHAPE_BY_IDX(param, 0, -1, 2, 1));
)!!!";
    EXPECT_EQ(res, expect);
}
...
TEST_F(TestCodegenDynCopy, L0CopyOutTensor) {
    ...
    std::string expect =
        R"!!!(TStore<TileOp::TStoreConfig<CopyOutMode::NZ2ND, 0, 0>>(gmTensor_1, l0cTensor_2, l0cTensor_2, Coord2Dim(0, 0), GET_PARAM_RAWSHAPE_BY_IDX(param, 0, -1, 2, 0), GET_PARAM_RAWSHAPE_BY_IDX(param, 0, -1, 2, 1), 0);
)!!!";
    EXPECT_EQ(res, expect);
}
...
TEST_F(TestCodegenDynCopy, L0CToL1TileTensor) {
    ...
    std::string expect =
        R"!!!(TExtract<TileOp::TStoreConfig<CopyOutMode::NZ2NZ, 0, 0>>(l1Tensor_1, l0cTensor_2, l0cTensor_2, Coord2Dim(0, 0), Coord2Dim(0, 0), 0);
)!!!";
    EXPECT_EQ(res, expect);
}
