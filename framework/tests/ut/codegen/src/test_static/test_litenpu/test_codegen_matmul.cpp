...
TEST_F(LiteNPUCodeGenMatmul, test_matmul_001) {
    PROGRAM("MATMUL_001") {
        ...
        FUNCTION("MATMUL_001") {           
            TileShape::Current().SetCubeTile({16,16}, {16,16}, {16,16}, false, true);
...
TEST_F(LiteNPUCodeGenMatmul, tmp_test_pow) {
    PROGRAM("POW") {
    ...
    auto function = Program::GetInstance().GetFunctionByRawName(FUNCTION_PREFIX + "POW");
    npu::tile_fwk::CodeGenCtx ctx;
    npu::tile_fwk::CodeGenLiteNPU codeGen(ctx);
    codeGen.GenCode(*function, {});
}
