...
class LiteNPUCodeGenSum : public testing::Test {
public:
...
// sum test cases
TEST_F(LiteNPUCodeGenSum, test_sum_fp32_001) {
    PROGRAM("SUM_FP32_001") {
        TileShape::Current().SetVecTile({48});
        Tensor operand(DT_FP32, {112}, "operand");
        Tensor result;
        FUNCTION("SUM_FP32_001") {
            result = Sum(operand, -1, false);
        }
    }
...
// fp16 test cases
TEST_F(LiteNPUCodeGenSum, test_sum_fp16_001) {
    PROGRAM("SUM_FP16_001") {
        TileShape::Current().SetVecTile({48});
        Tensor operand(DT_FP16, {112}, "operand");
        Tensor result;
        FUNCTION("SUM_FP16_001") {
            result = Sum(operand, -1, false);
        }
    }
    auto function = Program::GetInstance().GetFunctionByRawName(FUNCTION_PREFIX + "SUM_FP16_001");
    npu::tile_fwk::CodeGenCtx ctx;
    npu::tile_fwk::CodeGenLiteNPU codeGen(ctx);
    codeGen.GenCode(*function, {});
}

TEST_F(LiteNPUCodeGenSum, test_sum_fp16_002) {
    PROGRAM("SUM_FP16_002") {
        TileShape::Current().SetVecTile({96});
        Tensor operand(DT_FP16, {100}, "operand");
        Tensor result;
        FUNCTION("SUM_FP16_002") {
            result = Sum(operand, -1, false);
        }
    }
    auto function = Program::GetInstance().GetFunctionByRawName(FUNCTION_PREFIX + "SUM_FP16_002");
    npu::tile_fwk::CodeGenCtx ctx;
    npu::tile_fwk::CodeGenLiteNPU codeGen(ctx);
    codeGen.GenCode(*function, {});
}

TEST_F(LiteNPUCodeGenSum, test_sum_fp16_003) {
    PROGRAM("SUM_FP16_003") {
        TileShape::Current().SetVecTile({2, 32});
        Tensor operand(DT_FP16, {4, 128}, "operand");
        Tensor result;
        FUNCTION("SUM_FP16_003") {
            result = Sum(operand, -1, false);
        }
    }
    auto function = Program::GetInstance().GetFunctionByRawName(FUNCTION_PREFIX + "SUM_FP16_003");
    npu::tile_fwk::CodeGenCtx ctx;
    npu::tile_fwk::CodeGenLiteNPU codeGen(ctx);
    codeGen.GenCode(*function, {});
}

TEST_F(LiteNPUCodeGenSum, test_sum_fp16_004) {
    PROGRAM("SUM_FP16_004") {
        TileShape::Current().SetVecTile({1, 128});
        Tensor operand(DT_FP16, {4, 130}, "operand");
        Tensor result;
        FUNCTION("SUM_FP16_004") {
            result = Sum(operand, -1, false);
        }
    }
    auto function = Program::GetInstance().GetFunctionByRawName(FUNCTION_PREFIX + "SUM_FP16_004");
    npu::tile_fwk::CodeGenCtx ctx;
    npu::tile_fwk::CodeGenLiteNPU codeGen(ctx);
    codeGen.GenCode(*function, {});
}

TEST_F(LiteNPUCodeGenSum, test_sum_fp16_005) {
    PROGRAM("SUM_FP16_005") {
        TileShape::Current().SetVecTile({1, 2, 32});
        Tensor operand(DT_FP16, {2, 4, 160}, "operand");
        Tensor result;
        FUNCTION("SUM_FP16_005") {
            result = Sum(operand, -1, false);
        }
    }
    auto function = Program::GetInstance().GetFunctionByRawName(FUNCTION_PREFIX + "SUM_FP16_005");
    npu::tile_fwk::CodeGenCtx ctx;
    npu::tile_fwk::CodeGenLiteNPU codeGen(ctx);
    codeGen.GenCode(*function, {});
}

TEST_F(LiteNPUCodeGenSum, test_sum_fp16_006) {
    PROGRAM("SUM_FP16_006") {
        TileShape::Current().SetVecTile({1, 2, 128});
        Tensor operand(DT_FP16, {2, 4, 140}, "operand");
        Tensor result;
        FUNCTION("SUM_FP16_006") {
            result = Sum(operand, -1, false);
        }
    }
    auto function = Program::GetInstance().GetFunctionByRawName(FUNCTION_PREFIX + "SUM_FP16_006");
    npu::tile_fwk::CodeGenCtx ctx;
    npu::tile_fwk::CodeGenLiteNPU codeGen(ctx);
    codeGen.GenCode(*function, {});
}

TEST_F(LiteNPUCodeGenSum, test_sum_fp16_007) {
    PROGRAM("SUM_FP16_007") {
        TileShape::Current().SetVecTile({1, 5, 32});
        Tensor operand(DT_FP16, {2, 5, 152}, "operand");
        Tensor result;
        FUNCTION("SUM_FP16_007") {
            result = Sum(operand, -1, false);
        }
    }
    auto function = Program::GetInstance().GetFunctionByRawName(FUNCTION_PREFIX + "SUM_FP16_007");
    npu::tile_fwk::CodeGenCtx ctx;
    npu::tile_fwk::CodeGenLiteNPU codeGen(ctx);
    codeGen.GenCode(*function, {});
}

TEST_F(LiteNPUCodeGenSum, test_sum_fp16_008) {
    PROGRAM("SUM_FP16_008") {
        TileShape::Current().SetVecTile({1, 3, 160});
        Tensor operand(DT_FP16, {2, 3, 170}, "operand");
        Tensor result;
        FUNCTION("SUM_FP16_008") {
            result = Sum(operand, -1, false);
        }
    }
    auto function = Program::GetInstance().GetFunctionByRawName(FUNCTION_PREFIX + "SUM_FP16_008");
    npu::tile_fwk::CodeGenCtx ctx;
    npu::tile_fwk::CodeGenLiteNPU codeGen(ctx);
    codeGen.GenCode(*function, {});
}

TEST_F(LiteNPUCodeGenSum, test_sum_fp16_009) {
    PROGRAM("SUM_FP16_009") {
        TileShape::Current().SetVecTile({2, 1, 2, 16});
        Tensor operand(DT_FP16, {5, 2, 4, 176}, "operand");
        Tensor result;
        FUNCTION("SUM_FP16_009") {
            result = Sum(operand, -1, false);
        }
    }
    auto function = Program::GetInstance().GetFunctionByRawName(FUNCTION_PREFIX + "SUM_FP16_009");
    npu::tile_fwk::CodeGenCtx ctx;
    npu::tile_fwk::CodeGenLiteNPU codeGen(ctx);
    codeGen.GenCode(*function, {});
}

TEST_F(LiteNPUCodeGenSum, test_sum_fp16_010) {
    PROGRAM("SUM_FP16_010") {
        TileShape::Current().SetVecTile({1, 1, 1, 128});
        Tensor operand(DT_FP16, {5, 2, 4, 130}, "operand");
        Tensor result;
        FUNCTION("SUM_FP16_010") {
            result = Sum(operand, -1, false);
        }
    }
    auto function = Program::GetInstance().GetFunctionByRawName(FUNCTION_PREFIX + "SUM_FP16_010");
    npu::tile_fwk::CodeGenCtx ctx;
    npu::tile_fwk::CodeGenLiteNPU codeGen(ctx);
    codeGen.GenCode(*function, {});
}

TEST_F(LiteNPUCodeGenSum, test_sum_fp16_011) {
    PROGRAM("SUM_FP16_011") {
        TileShape::Current().SetVecTile({1, 1, 5, 32});
        Tensor operand(DT_FP16, {2, 3, 5, 134}, "operand");
        Tensor result;
        FUNCTION("SUM_FP16_011") {
            result = Sum(operand, -1, false);
        }
    }
    auto function = Program::GetInstance().GetFunctionByRawName(FUNCTION_PREFIX + "SUM_FP16_011");
    npu::tile_fwk::CodeGenCtx ctx;
    npu::tile_fwk::CodeGenLiteNPU codeGen(ctx);
    codeGen.GenCode(*function, {});
}

TEST_F(LiteNPUCodeGenSum, test_sum_fp16_012) {
    PROGRAM("SUM_FP16_012") {
        TileShape::Current().SetVecTile({2, 2, 3, 32});
        Tensor operand(DT_FP16, {4, 2, 6, 135}, "operand");
        Tensor result;
        FUNCTION("SUM_FP16_012") {
            result = Sum(operand, -1, false);
        }
    }
    auto function = Program::GetInstance().GetFunctionByRawName(FUNCTION_PREFIX + "SUM_FP16_012");
    npu::tile_fwk::CodeGenCtx ctx;
    npu::tile_fwk::CodeGenLiteNPU codeGen(ctx);
    codeGen.GenCode(*function, {});
}

TEST_F(LiteNPUCodeGenSum, test_sum_fp16_013) {
    PROGRAM("SUM_FP16_013") {
        TileShape::Current().SetVecTile({1, 1, 4, 128});
        Tensor operand(DT_FP16, {6, 2, 4, 130}, "operand");
        Tensor result;
        FUNCTION("SUM_FP16_013") {
            result = Sum(operand, -1, false);
        }
    }
    auto function = Program::GetInstance().GetFunctionByRawName(FUNCTION_PREFIX + "SUM_FP16_013");
    npu::tile_fwk::CodeGenCtx ctx;
    npu::tile_fwk::CodeGenLiteNPU codeGen(ctx);
    codeGen.GenCode(*function, {});
}

TEST_F(LiteNPUCodeGenSum, test_sum_fp16_014) {
    PROGRAM("SUM_FP16_014") {
        TileShape::Current().SetVecTile({1, 2, 1, 128});
        Tensor operand(DT_FP16, {3, 2, 3, 139}, "operand");
        Tensor result;
        FUNCTION("SUM_FP16_014") {
            result = Sum(operand, -1, false);
        }
    }
    auto function = Program::GetInstance().GetFunctionByRawName(FUNCTION_PREFIX + "SUM_FP16_014");
    npu::tile_fwk::CodeGenCtx ctx;
    npu::tile_fwk::CodeGenLiteNPU codeGen(ctx);
    codeGen.GenCode(*function, {});
}

TEST_F(LiteNPUCodeGenSum, test_sum_fp16_015) {
    PROGRAM("SUM_FP16_015") {
        TileShape::Current().SetVecTile({3, 3, 5, 32});
        Tensor operand(DT_FP16, {6, 3, 5, 141}, "operand");
        Tensor result;
        FUNCTION("SUM_FP16_015") {
            result = Sum(operand, -1, false);
        }
    }
    auto function = Program::GetInstance().GetFunctionByRawName(FUNCTION_PREFIX + "SUM_FP16_015");
    npu::tile_fwk::CodeGenCtx ctx;
    npu::tile_fwk::CodeGenLiteNPU codeGen(ctx);
    codeGen.GenCode(*function, {});
}
...
// int32 test cases
TEST_F(LiteNPUCodeGenSum, test_sum_int32_001) {
    PROGRAM("SUM_INT32_001") {
        TileShape::Current().SetVecTile({48});
        Tensor operand(DT_INT32, {112}, "operand");
        Tensor result;
        FUNCTION("SUM_INT32_001") {
            result = Sum(operand, -1, false);
        }
    }
...
TEST_F(LiteNPUCodeGenSum, test_sum_int32_002) {
    PROGRAM("SUM_INT32_002") {
        TileShape::Current().SetVecTile({96});
        Tensor operand(DT_INT32, {100}, "operand");
        Tensor result;
        FUNCTION("SUM_INT32_002") {
            result = Sum(operand, -1, false);
        }
    }
...
TEST_F(LiteNPUCodeGenSum, test_sum_int32_003) {
    PROGRAM("SUM_INT32_003") {
        TileShape::Current().SetVecTile({2, 32});
        Tensor operand(DT_INT32, {4, 128}, "operand");
        Tensor result;
        FUNCTION("SUM_INT32_003") {
            result = Sum(operand, -1, false);
        }
    }
...
TEST_F(LiteNPUCodeGenSum, test_sum_int32_004) {
    PROGRAM("SUM_INT32_004") {
        TileShape::Current().SetVecTile({1, 128});
        Tensor operand(DT_INT32, {4, 130}, "operand");
        Tensor result;
        FUNCTION("SUM_INT32_004") {
            result = Sum(operand, -1, false);
        }
    }
...
TEST_F(LiteNPUCodeGenSum, test_sum_int32_005) {
    PROGRAM("SUM_INT32_005") {
        TileShape::Current().SetVecTile({1, 2, 32});
        Tensor operand(DT_INT32, {2, 4, 160}, "operand");
        Tensor result;
        FUNCTION("SUM_INT32_005") {
            result = Sum(operand, -1, false);
        }
    }
...
TEST_F(LiteNPUCodeGenSum, test_sum_int32_006) {
    PROGRAM("SUM_INT32_006") {
        TileShape::Current().SetVecTile({1, 2, 128});
        Tensor operand(DT_INT32, {2, 4, 140}, "operand");
        Tensor result;
        FUNCTION("SUM_INT32_006") {
            result = Sum(operand, -1, false);
        }
    }
...
TEST_F(LiteNPUCodeGenSum, test_sum_int32_007) {
    PROGRAM("SUM_INT32_007") {
        TileShape::Current().SetVecTile({1, 5, 32});
        Tensor operand(DT_INT32, {2, 5, 152}, "operand");
        Tensor result;
        FUNCTION("SUM_INT32_007") {
            result = Sum(operand, -1, false);
        }
    }
...
TEST_F(LiteNPUCodeGenSum, test_sum_int32_008) {
    PROGRAM("SUM_INT32_008") {
        TileShape::Current().SetVecTile({1, 3, 168});
        Tensor operand(DT_INT32, {2, 3, 170}, "operand");
        Tensor result;
        FUNCTION("SUM_INT32_008") {
            result = Sum(operand, -1, false);
        }
    }
...
TEST_F(LiteNPUCodeGenSum, test_sum_int32_009) {
    PROGRAM("SUM_INT32_009") {
        TileShape::Current().SetVecTile({2, 1, 2, 16});
        Tensor operand(DT_INT32, {5, 2, 4, 176}, "operand");
        Tensor result;
        FUNCTION("SUM_INT32_009") {
            result = Sum(operand, -1, false);
        }
    }
...
TEST_F(LiteNPUCodeGenSum, test_sum_int32_010) {
    PROGRAM("SUM_INT32_010") {
        TileShape::Current().SetVecTile({1, 1, 1, 128});
        Tensor operand(DT_INT32, {5, 2, 4, 130}, "operand");
        Tensor result;
        FUNCTION("SUM_INT32_010") {
            result = Sum(operand, -1, false);
        }
    }
...
TEST_F(LiteNPUCodeGenSum, test_sum_int32_011) {
    PROGRAM("SUM_INT32_011") {
        TileShape::Current().SetVecTile({1, 1, 5, 32});
        Tensor operand(DT_INT32, {2, 3, 5, 134}, "operand");
        Tensor result;
        FUNCTION("SUM_INT32_011") {
            result = Sum(operand, -1, false);
        }
    }
...
TEST_F(LiteNPUCodeGenSum, test_sum_int32_012) {
    PROGRAM("SUM_INT32_012") {
        TileShape::Current().SetVecTile({2, 2, 3, 32});
        Tensor operand(DT_INT32, {4, 2, 6, 135}, "operand");
        Tensor result;
        FUNCTION("SUM_INT32_012") {
            result = Sum(operand, -1, false);
        }
    }
...
TEST_F(LiteNPUCodeGenSum, test_sum_int32_013) {
    PROGRAM("SUM_INT32_013") {
        TileShape::Current().SetVecTile({1, 1, 4, 128});
        Tensor operand(DT_INT32, {6, 2, 4, 130}, "operand");
        Tensor result;
        FUNCTION("SUM_INT32_013") {
            result = Sum(operand, -1, false);
        }
    }
...
TEST_F(LiteNPUCodeGenSum, test_sum_int32_014) {
    PROGRAM("SUM_INT32_014") {
        TileShape::Current().SetVecTile({1, 2, 1, 136});
        Tensor operand(DT_INT32, {3, 2, 3, 139}, "operand");
        Tensor result;
        FUNCTION("SUM_INT32_014") {
            result = Sum(operand, -1, false);
        }
    }
...
TEST_F(LiteNPUCodeGenSum, test_sum_int32_015) {
    PROGRAM("SUM_INT32_015") {
        TileShape::Current().SetVecTile({3, 3, 5, 32});
        Tensor operand(DT_INT32, {6, 3, 5, 141}, "operand");
        Tensor result;
        FUNCTION("SUM_INT32_015") {
            result = Sum(operand, -1, false);
        }
    }
