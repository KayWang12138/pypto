...
rtError_t rtAicpuKernelLaunchExWithArgs(const uint32_t kernelType, const char_t *const opName, const uint32_t blockDim,
    const rtAicpuArgsEx_t *argsInfo, rtSmDesc_t *const smDesc, const rtStream_t stm, const uint32_t flags) {
    (void)kernelType;
    (void)opName;
    (void)blockDim;
    (void)argsInfo;
...
rtError_t rtsBinaryLoadFromFile([[maybe_unused]] const char *const binPath,
    [[maybe_unused]] const rtLoadBinaryConfig_t *const optionalCfg, [[maybe_unused]] rtBinHandle *binHandle) {
    return RT_ERROR_NONE;
}
