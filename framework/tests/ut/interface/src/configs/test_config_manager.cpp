...
TEST_F(TestConfigManager, Dump) {
    ...
    cm.BeginScope("scope2", {{"debug.print.edgeitems", 20L}});
    {
        ...
        auto linewidth = scope2->GetConfig<int>("debug.print.linewidth");
        EXPECT_EQ(linewidth, 120);
        auto edgeitems = scope2->GetConfig<int>("debug.print.edgeitems");
        EXPECT_EQ(edgeitems, 20);
        cm.EndScope();
    }

    auto scope = cm.CurrentScope();
    auto linewidth = scope->GetConfig<int>("debug.print.linewidth");
    EXPECT_EQ(linewidth, 80);
    auto edgeitems = scope->GetConfig<int>("debug.print.edgeitems");
    EXPECT_EQ(edgeitems, 20);
    cm.EndScope();
