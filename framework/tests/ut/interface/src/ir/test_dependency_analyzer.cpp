...
static FunctionPtr MakeSequentialFunction() {
    ...
    return std::make_shared<Function>("test", std::vector<VarPtr>{}, std::vector<TypePtr>{}, body, Span::Unknown());
}
...
TEST_F(DependencyAnalyzerTest, TestAnalyzeEmptyBody) {
    ...
    auto func =
        std::make_shared<Function>("empty", std::vector<VarPtr>{}, std::vector<TypePtr>{}, body, Span::Unknown());
...
TEST_F(DependencyAnalyzerTest, TestAnalyzeFunctionWithLoop) {
    ...
    auto func = std::make_shared<Function>(
        "loop_func", std::vector<VarPtr>{}, std::vector<TypePtr>{}, forStmt, Span::Unknown());
...
TEST_F(DependencyAnalyzerTest, TestAnalyzeWARDependency) {
    ...
    auto body = std::make_shared<SeqStmts>(stmts, Span::Unknown());
    auto func =
        std::make_shared<Function>("test", std::vector<VarPtr>{}, std::vector<TypePtr>{}, body, Span::Unknown());
...
TEST_F(DependencyAnalyzerTest, TestAnalyzeFunctionWithIf) {
    ...
    auto ifStmt = std::make_shared<IfStmt>(
        cond, thenBody, std::optional<StmtPtr>(elseBody), std::vector<VarPtr>{}, Span::Unknown());

    auto func =
        std::make_shared<Function>("if_func", std::vector<VarPtr>{}, std::vector<TypePtr>{}, ifStmt, Span::Unknown());
...
TEST_F(DependencyAnalyzerTest, TestAnalyzeNestedSeqStmts) {
    ...
    auto func =
        std::make_shared<Function>("nested", std::vector<VarPtr>{}, std::vector<TypePtr>{}, outerSeq, Span::Unknown());
...
TEST_F(DependencyAnalyzerTest, TestMergeDependenciesDeduplicate) {
    ...
    edge.variable = varX;
    edge.type = DependencyEdge::RAW;
    edge.producerPipe = "UNKNOWN";
    edge.consumerPipe = "UNKNOWN";
