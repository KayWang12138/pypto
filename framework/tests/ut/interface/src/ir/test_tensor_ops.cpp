...
TEST_F(TensorOpsTest, TestTensorMatMulWithTranspose) {
  ...
  auto call = registry_.Create(
      "tensor.matmul", {a, b}, kwargs, span_);
  ASSERT_NE(call, nullptr);
  auto rt = As<TensorType>(call->GetType());
  ASSERT_NE(rt, nullptr);
  ASSERT_EQ(rt->shape_.size(), 2);
...
TEST_F(TensorOpsTest, TestTensorCreateCreate) {
  ...
  auto call = registry_.Create(
      "tensor.create", {shape}, kwargs, span_);
  ASSERT_NE(call, nullptr);
  auto rt = As<TensorType>(call->GetType());
  ASSERT_NE(rt, nullptr);
  ASSERT_EQ(rt->shape_.size(), 2);
...
TEST_F(TensorOpsTest, TestTensorViewCreate) {
  ...
  auto call = registry_.Create(
      "tensor.view", {input, shape, offset}, span_);
  ASSERT_NE(call, nullptr);
  auto rt = As<TensorType>(call->GetType());
  ASSERT_NE(rt, nullptr);
  ASSERT_EQ(rt->shape_.size(), 2);
...
TEST_F(TensorOpsTest, TestTensorAssembleCreate) {
  ...
  auto offset = MakeShapeTuple({0, 0});
  auto call = registry_.Create(
      "tensor.assemble", {target, source, offset},
      span_);
  ASSERT_NE(call, nullptr);
  auto rt = As<TensorType>(call->GetType());
  ASSERT_NE(rt, nullptr);
...
TEST_F(TensorOpsTest, TestTensorReshapeCreate) {
  ...
  auto call = registry_.Create(
      "tensor.reshape", {input, shape}, span_);
  ASSERT_NE(call, nullptr);
  auto rt = As<TensorType>(call->GetType());
  ASSERT_NE(rt, nullptr);
  ASSERT_EQ(rt->shape_.size(), 2);
...
TEST_F(TensorOpsTest, TestTensorTransposeCreate) {
  ...
  auto call = registry_.Create(
      "tensor.transpose", {input, axis1, axis2}, span_);
  ASSERT_NE(call, nullptr);
  auto rt = As<TensorType>(call->GetType());
  ASSERT_NE(rt, nullptr);
  ASSERT_EQ(rt->shape_.size(), 2);
...
TEST_F(TensorOpsTest, TestIsRegistered) {
  ...
  ASSERT_TRUE(
      registry_.IsRegistered("tensor.assemble"));
  ASSERT_TRUE(
      registry_.IsRegistered("tensor.row_max"));
  ASSERT_TRUE(
      registry_.IsRegistered("tensor.row_sum"));
  ASSERT_TRUE(
      registry_.IsRegistered("tensor.reshape"));
  ASSERT_TRUE(
