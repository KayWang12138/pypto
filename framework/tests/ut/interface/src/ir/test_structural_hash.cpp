...
TEST_F(StructuralHashTest, TestIfStmtHash) {
    ...
    auto thenBody2 = std::make_shared<EvalStmt>(std::make_shared<ConstInt>(1, DataType::INT32, sp), sp);
    auto if2 = std::make_shared<IfStmt>(cond2, thenBody2, std::optional<StmtPtr>{}, std::vector<VarPtr>{}, sp);
