...
TEST_F(StructuralHashTest, TestTupleTypeHashConsistent) {
    ...
    auto tt2 = std::make_shared<TupleType>(std::vector<TypePtr>{
...
TEST_F(StructuralHashTest, TestIfStmtHash) {
    ...
    auto if1 = std::make_shared<IfStmt>(cond, thenBody, std::optional<StmtPtr>{}, std::vector<VarPtr>{}, sp);

    auto cond2 = std::make_shared<ConstBool>(true, sp);
    auto then_body2 = std::make_shared<EvalStmt>(std::make_shared<ConstInt>(1, DataType::INT32, sp), sp);
    auto if2 = std::make_shared<IfStmt>(cond2, then_body2, std::optional<StmtPtr>{}, std::vector<VarPtr>{}, sp);
...
TEST_F(StructuralHashTest, TestOpStmtsHash) {
    Span sp = Span::unknown();
    auto e1 = std::make_shared<ConstInt>(1, DataType::INT32, sp);
    auto s1 = std::make_shared<OpStmts>(std::vector<StmtPtr>{std::make_shared<EvalStmt>(e1, sp)}, sp);
    auto e2 = std::make_shared<ConstInt>(1, DataType::INT32, sp);
    auto s2 = std::make_shared<OpStmts>(std::vector<StmtPtr>{std::make_shared<EvalStmt>(e2, sp)}, sp);
    ASSERT_EQ(structural_hash(s1), structural_hash(s2));
}
...
TEST_F(StructuralHashTest, TestProgramHashConsistent) {
    ...
    auto prog1 = std::make_shared<Program>(std::vector<FunctionPtr>{func1}, "test", sp);
    ...
    auto prog2 = std::make_shared<Program>(std::vector<FunctionPtr>{func2}, "test", sp);
...
TEST_F(StructuralHashTest, TestCallWithStringKwarg) {
    ...
    auto call1 = std::make_shared<const Call>(
        baseCall->op_, std::vector<ExprPtr>{a}, kwargs1, std::make_shared<ScalarType>(DataType::INT32), sp);

    std::vector<std::pair<std::string, std::any>> kwargs2 = {
        {"label", std::any(std::string("test"))}
    };
    auto call2 = std::make_shared<const Call>(
        baseCall->op_, std::vector<ExprPtr>{a}, kwargs2, std::make_shared<ScalarType>(DataType::INT32), sp);
...
TEST_F(StructuralHashTest, TestCallWithDoubleKwarg) {
    ...
    auto call1 = std::make_shared<const Call>(
        baseCall->op_, std::vector<ExprPtr>{a}, kwargs1, std::make_shared<ScalarType>(DataType::FP32), sp);

    std::vector<std::pair<std::string, std::any>> kwargs2 = {
        {"scale", std::any(3.14)}
    };
    auto call2 = std::make_shared<const Call>(
        baseCall->op_, std::vector<ExprPtr>{a}, kwargs2, std::make_shared<ScalarType>(DataType::FP32), sp);
...
TEST_F(StructuralHashTest, TestTileTypeWithTileViewHash) {
    ...
    TileView tv1(std::vector<ExprPtr>{std::make_shared<ConstInt>(4, DataType::INT64, sp)},
    ...
    TileView tv2(std::vector<ExprPtr>{std::make_shared<ConstInt>(4, DataType::INT64, sp)},
        std::vector<ExprPtr>{std::make_shared<ConstInt>(1, DataType::INT64, sp)},
        std::make_shared<ConstInt>(0, DataType::INT64, sp));
...
TEST_F(StructuralHashTest, TestTileTypeWithVsWithoutTileViewHash) {
    Span sp = Span::unknown();
    auto shape1 = std::vector<ExprPtr>{std::make_shared<ConstInt>(4, DataType::INT64, sp)};
    TileView tv(std::vector<ExprPtr>{std::make_shared<ConstInt>(4, DataType::INT64, sp)},
        std::vector<ExprPtr>{std::make_shared<ConstInt>(1, DataType::INT64, sp)},
        std::make_shared<ConstInt>(0, DataType::INT64, sp));
...
TEST_F(StructuralHashTest, TestHashCachingWithSharedNode) {
    ...
    auto tuple = std::make_shared<MakeTuple>(std::vector<ExprPtr>{sharedVal, sharedVal}, sp);
