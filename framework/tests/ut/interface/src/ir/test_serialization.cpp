...
TEST_F(SerializationTest, TestFunctionRoundTrip) {
    ...
    auto original =
        std::make_shared<Function>("test_func", std::vector<VarPtr>{}, std::vector<TypePtr>{}, body, Span::Unknown());
...
TEST_F(SerializationTest, TestStructuralEqualAfterRoundTrip) {
    ...
    ASSERT_NE(result, nullptr);
    ASSERT_TRUE(StructuralEqual(original, result));
}
...
TEST_F(SerializationTest, TestVarRoundTrip) {
    ...
    auto original = std::make_shared<Var>("x", std::make_shared<ScalarType>(DataType::INT32), sp);
    auto bytes = serialization::Serialize(original);
    ASSERT_FALSE(bytes.empty());
    auto deserialized = serialization::Deserialize(bytes);
...
TEST_F(SerializationTest, TestNegRoundTrip) {
    ...
    auto deserialized = serialization::Deserialize(bytes);
    ASSERT_NE(deserialized, nullptr);
    ASSERT_TRUE(StructuralEqual(original, deserialized));
}
...
TEST_F(SerializationTest, TestMakeTupleRoundTrip) {
    ...
    auto original = std::make_shared<MakeTuple>(std::vector<ExprPtr>{e1, e2}, sp);
    auto bytes = serialization::Serialize(original);
    ASSERT_FALSE(bytes.empty());
    auto deserialized = serialization::Deserialize(bytes);
    ASSERT_NE(deserialized, nullptr);
...
TEST_F(SerializationTest, TestSeqStmtsRoundTrip) {
    ...
    auto original = std::make_shared<SeqStmts>(std::vector<StmtPtr>{s1, s2}, sp);
    auto bytes = serialization::Serialize(original);
    ASSERT_FALSE(bytes.empty());
    auto deserialized = serialization::Deserialize(bytes);
    ASSERT_NE(deserialized, nullptr);
...
TEST_F(SerializationTest, TestReturnStmtRoundTrip) {
    ...
    auto original = std::make_shared<ReturnStmt>(std::vector<ExprPtr>{v}, sp);
    auto bytes = serialization::Serialize(original);
    ASSERT_FALSE(bytes.empty());
    auto deserialized = serialization::Deserialize(bytes);
    ASSERT_NE(deserialized, nullptr);
...
TEST_F(SerializationTest, TestYieldStmtRoundTrip) {
    Span sp = Span::Unknown();
    auto v = std::make_shared<ConstInt>(1, DataType::INT32, sp);
    auto original = std::make_shared<YieldStmt>(std::vector<ExprPtr>{v}, sp);
    auto bytes = serialization::Serialize(original);
    ASSERT_FALSE(bytes.empty());
    auto deserialized = serialization::Deserialize(bytes);
    ASSERT_NE(deserialized, nullptr);
...
TEST_F(SerializationTest, TestForStmtRoundTrip) {
    Span sp = Span::Unknown();
    auto lv = std::make_shared<Var>("i", std::make_shared<ScalarType>(DataType::INT32), sp);
    ...
    auto original =
        std::make_shared<ForStmt>(lv, start, stop, step, std::vector<IterArgPtr>{}, body, std::vector<VarPtr>{}, sp);
    auto bytes = serialization::Serialize(original);
    ASSERT_FALSE(bytes.empty());
    auto deserialized = serialization::Deserialize(bytes);
    ASSERT_NE(deserialized, nullptr);
...
TEST_F(SerializationTest, TestOpStmtsRoundTrip) {
    ...
    auto original = std::make_shared<OpStmts>(std::vector<StmtPtr>{eval}, sp);
    auto bytes = serialization::Serialize(original);
    ASSERT_FALSE(bytes.empty());
    auto deserialized = serialization::Deserialize(bytes);
    ASSERT_NE(deserialized, nullptr);
...
TEST_F(SerializationTest, TestTensorTypeVarRoundTrip) {
    Span sp = Span::Unknown();
    auto var = std::make_shared<Var>("x",
        std::make_shared<TensorType>(std::vector<ExprPtr>{std::make_shared<ConstInt>(4, DataType::INT64, sp),
                                         std::make_shared<ConstInt>(8, DataType::INT64, sp)},
            DataType::FP32),
        sp);
...
TEST_F(SerializationTest, TestTileTypeVarRoundTrip) {
    Span sp = Span::Unknown();
    auto var = std::make_shared<Var>("t",
...
TEST_F(SerializationTest, TestFuncWithReturnTypes) {
    Span sp = Span::Unknown();
    auto param = std::make_shared<Var>("x", std::make_shared<ScalarType>(DataType::INT32), sp);
    auto body = std::make_shared<ReturnStmt>(std::vector<ExprPtr>{param}, sp);
    auto retType = std::make_shared<ScalarType>(DataType::INT32);
    auto original =
        std::make_shared<Function>("f", std::vector<VarPtr>{param}, std::vector<TypePtr>{retType}, body, sp);
    auto bytes = serialization::Serialize(original);
    ASSERT_FALSE(bytes.empty());
    auto deserialized = serialization::Deserialize(bytes);
    ASSERT_NE(deserialized, nullptr);
...
TEST_F(SerializationTest, TestSharedPointerDedup) {
    ...
    auto add = std::make_shared<Add>(sharedVal, sharedVal, DataType::INT32, sp);
...
TEST_F(SerializationTest, TestIterArgRoundTrip) {
    ...
    auto original = std::make_shared<IterArg>("acc", std::make_shared<ScalarType>(DataType::INT32), init, sp);
    auto bytes = serialization::Serialize(original);
    ASSERT_FALSE(bytes.empty());
    auto deserialized = serialization::Deserialize(bytes);
    ASSERT_NE(deserialized, nullptr);
