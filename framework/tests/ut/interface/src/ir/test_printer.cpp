...
TEST_F(IRPrinterTest, TestPrintConstantsAndBasicExprs)
{
    ...
    auto callResult = Print(std::make_shared<Call>(std::make_shared<Op>("op"), std::vector<ExprPtr>{arg1, arg2}, Sp()));
...
TEST_F(IRPrinterTest, TestPrintBasicStmts)
{
    ...
    // EvalStmt
    auto call = std::make_shared<Call>(std::make_shared<Op>("op"), std::vector<ExprPtr>{}, Sp());
