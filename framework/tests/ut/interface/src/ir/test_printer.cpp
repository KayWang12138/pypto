...
TEST_F(IRPrinterTest, TestPrintTileTypeWithMemRef) {
  Span sp = Span::unknown();
  std::vector<ExprPtr> shape = {
      std::make_shared<ConstInt>(16, DataType::INT64, sp),
      std::make_shared<ConstInt>(16, DataType::INT64, sp)};
...
TEST_F(IRPrinterTest, TestPrintTileTypeWithTileView) {
  ...
  auto addr = std::make_shared<ConstInt>(0, DataType::INT64, sp);
  auto memref = std::make_shared<MemRef>(MemorySpace::L0A, addr, 2048, 0);
  auto type = std::make_shared<TileType>(shape, DataType::FP16,
                                         std::optional<MemRefPtr>(memref),
                                         std::optional<TileView>(tv));
  std::string result = PythonPrint(type);
...
// ============================================================================
// IfStmt with return_vars and yield body
// ============================================================================
...
// ============================================================================
// SeqStmts with YieldStmt at end and return_vars
// ============================================================================
...
TEST_F(IRPrinterTest, TestPrintProgramCustomPrefix) {
  ...
  auto body = std::make_shared<EvalStmt>(
      std::make_shared<ConstInt>(0, DataType::INT32, sp), sp);
  auto func = std::make_shared<Function>(
      "main", std::vector<VarPtr>{}, std::vector<TypePtr>{}, body, sp);
  auto program = std::make_shared<Program>(
      std::vector<FunctionPtr>{func}, "my_prog", sp);
