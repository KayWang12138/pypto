...
// Force initialization of all config registrars before tests run
// This ensures static initialization order is deterministic
// The EnsureConfig_*_Initialized functions are generated by REGISTER_CONFIG macro
// and are placed in pto namespace for easy access
namespace {
    void EnsureAllConfigsInitialized() {
        // Ensure registry is initialized first
        ConfigRegistry::GetInstance();
        
        // Explicitly initialize all config registrars in a controlled order
        // This guarantees they are initialized before any test runs
        // These functions are inline functions in pto namespace defined by REGISTER_CONFIG macro
        pto::EnsureConfig_test_int32_Initialized();
        pto::EnsureConfig_test_bool_Initialized();
        pto::EnsureConfig_test_uint8_Initialized();
        pto::EnsureConfig_test_uint16_Initialized();
        pto::EnsureConfig_test_map_Initialized();
    }
    
    // Global initialization - runs before main() and any test
    // This lambda is executed during static initialization phase, ensuring
    // all configs are registered before any test code runs
    static const bool g_configInitialized = []() {
        EnsureAllConfigsInitialized();
        return true;
    }();
}
...
public:
    void SetUp() override {
        // Ensure configs are initialized (defensive check)
        // This provides an additional safety net in case static initialization didn't run
        EnsureAllConfigsInitialized();
    }
