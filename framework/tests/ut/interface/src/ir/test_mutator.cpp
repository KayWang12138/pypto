...
TEST_F(IRMutatorTest, TestIdentityUnaryOps) {
    ...
    auto absE = std::make_shared<Abs>(v, DataType::INT32, Span::Unknown());
    auto notE =
    ...
    auto castE = std::make_shared<Cast>(v, DataType::FP32, Span::Unknown());
    ASSERT_EQ(mutator.VisitExpr(absE).get(), absE.get());
    ASSERT_EQ(mutator.VisitExpr(notE).get(), notE.get());
    ASSERT_EQ(mutator.VisitExpr(bn).get(), bn.get());
    ASSERT_EQ(mutator.VisitExpr(castE).get(), castE.get());
}
