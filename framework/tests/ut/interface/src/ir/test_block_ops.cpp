...
protected:
  ...
  VarPtr MakeTileVar(const std::string &name,
    ...
    for (auto d : dims) {
      shape.push_back(
          std::make_shared<ConstInt>(d, DataType::INT64, span_));
    }
  ...
  ExprPtr MakeConstInt(int64_t val,
...
TEST_F(BlockOpsTest, TestBlockColExpandCreate) {
  ...
  auto call = registry_.Create(
      "block.col_expand", {target, col}, span_);
  ASSERT_NE(call, nullptr);
}
...
TEST_F(BlockOpsTest, TestBlockColExpandMulCreate) {
  ...
  auto col = MakeTileVar("col", {1, 8});
  auto call = registry_.Create(
      "block.col_expand_mul", {target, col}, span_);
  ASSERT_NE(call, nullptr);
}
...
TEST_F(BlockOpsTest, TestBlockColExpandDivCreate) {
  ...
  auto col = MakeTileVar("col", {1, 8});
  auto call = registry_.Create(
      "block.col_expand_div", {target, col}, span_);
  ASSERT_NE(call, nullptr);
}
...
TEST_F(BlockOpsTest, TestBlockColExpandSubCreate) {
  ...
  auto col = MakeTileVar("col", {1, 8});
  auto call = registry_.Create(
      "block.col_expand_sub", {target, col}, span_);
  ASSERT_NE(call, nullptr);
}
...
TEST_F(BlockOpsTest, TestBlockExpandsCreate) {
  ...
  auto call = registry_.Create(
      "block.expands", {target, scalar}, span_);
  ASSERT_NE(call, nullptr);
}
...
TEST_F(BlockOpsTest, TestBlockMatMulAccCreate) {
  ...
  auto b = MakeTileVar("b", {8, 16});
  auto call = registry_.Create(
      "block.matmul_acc", {acc, a, b}, span_);
  ASSERT_NE(call, nullptr);
  auto rt = As<TileType>(call->GetType());
  ASSERT_NE(rt, nullptr);
  ASSERT_EQ(rt->shape_.size(), 2);
...
TEST_F(BlockOpsTest, TestBlockRowExpandSubCreate) {
  auto tile = MakeTileVar("tile", {4, 8});
  auto rowVec = MakeTileVar("rowVec", {4, 1});
  auto call = registry_.Create(
...
TEST_F(BlockOpsTest, TestBlockTransposeCreate) {
  auto input = MakeTileVar("input", {4, 8});
  auto axis1 = std::make_shared<ConstInt>(
      0, DataType::INT32, span_);
  auto axis2 = std::make_shared<ConstInt>(
      1, DataType::INT32, span_);
  auto call = registry_.Create(
      "block.transpose", {input, axis1, axis2}, span_);
  ASSERT_NE(call, nullptr);
  auto rt = As<TileType>(call->GetType());
  ASSERT_NE(rt, nullptr);
  ASSERT_EQ(rt->shape_.size(), 2);
...
TEST_F(BlockOpsTest, TestAllBlockOpsRegistered) {
  ...
  ASSERT_TRUE(
      registry_.IsRegistered("block.col_expand"));
  ASSERT_TRUE(
      registry_.IsRegistered("block.expands"));
  ASSERT_TRUE(
      registry_.IsRegistered("block.get_block_idx"));
  ...
  ASSERT_TRUE(
      registry_.IsRegistered("block.row_max"));
  ASSERT_TRUE(
      registry_.IsRegistered("block.row_sum"));
  ASSERT_TRUE(
      registry_.IsRegistered("block.row_min"));
  ASSERT_TRUE(
      registry_.IsRegistered("block.batch_matmul"));
  // Unary ops
  ...
  ASSERT_TRUE(
      registry_.IsRegistered("block.matmul_acc"));
  // Broadcast ops
  ASSERT_TRUE(
      registry_.IsRegistered("block.row_expand_sub"));
  ASSERT_TRUE(
      registry_.IsRegistered("block.row_expand_div"));
  ASSERT_TRUE(
      registry_.IsRegistered("block.row_expand_mul"));
  ASSERT_TRUE(
      registry_.IsRegistered("block.row_expand_add"));
  // Transform ops
  ASSERT_TRUE(registry_.IsRegistered("block.view"));
  ASSERT_TRUE(
      registry_.IsRegistered("block.reshape"));
  ASSERT_TRUE(
      registry_.IsRegistered("block.transpose"));
  // Col expand ops
  ASSERT_TRUE(
      registry_.IsRegistered("block.col_expand_mul"));
  ASSERT_TRUE(
      registry_.IsRegistered("block.col_expand_div"));
  ASSERT_TRUE(
