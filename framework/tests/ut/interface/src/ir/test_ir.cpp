...
TEST_F(IRTypeStrTest, TestPtrTypeStr)
{
    ...
    ASSERT_EQ(PythonPrint(std::static_pointer_cast<const Type>(pt)), "ir.Ptr[ir.FP32]");
}
...
TEST_F(IRExprStrTest, TestCallStr)
{
    auto a = std::make_shared<ConstInt>(1, DataType::INT32, sp);
    auto b = std::make_shared<ConstInt>(2, DataType::INT32, sp);
    auto call = std::make_shared<Call>(std::make_shared<Op>("my_op"), std::vector<ExprPtr>{a, b}, sp);
...
{
    auto val = std::make_shared<ConstInt>(42, DataType::INT32, sp);
    auto call = std::make_shared<Call>(std::make_shared<Op>("some_op"), std::vector<ExprPtr>{val}, sp);
