...
TEST_F(IRVisitorTest, TestVisitCall)
{
    ...
    v.VisitExpr(std::make_shared<Call>(std::make_shared<Op>("op"), std::vector<ExprPtr>{arg}, Sp()));
    // Call visits args
...
TEST_F(IRVisitorTest, TestVisitEvalStmt)
{
    ...
    auto call = std::make_shared<Call>(std::make_shared<Op>("op"), std::vector<ExprPtr>{arg}, Sp());
