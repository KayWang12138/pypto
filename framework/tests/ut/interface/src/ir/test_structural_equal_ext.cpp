...
TEST_F(IRStructEqExprTest, TestCallAndTupleExprsEqual)
{
    ...
    // Call
    auto c1 = std::make_shared<Call>(std::make_shared<Op>("op"), std::vector<ExprPtr>{a1}, Sp());
    auto c2 = std::make_shared<Call>(std::make_shared<Op>("op"), std::vector<ExprPtr>{a2}, Sp());
    EXPECT_TRUE(structural_equal(Node(c1), Node(c2)));
    auto c3 = std::make_shared<Call>(std::make_shared<Op>("op_b"), std::vector<ExprPtr>{a1}, Sp());
...
TEST_F(IRStructEqExprTest, TestAllStmtsEqual)
{
    ...
    auto call = std::make_shared<Call>(std::make_shared<Op>("op"), std::vector<ExprPtr>{v42a}, Sp());
...
TEST_F(IRStructEqAssertTest, TestAssertExprMismatches)
{
    ...
    EXPECT_THROW(
        assert_structural_equal(
            Node(std::make_shared<Call>(std::make_shared<Op>("op_a"), std::vector<ExprPtr>{a}, Sp())),
            Node(std::make_shared<Call>(std::make_shared<Op>("op_b"), std::vector<ExprPtr>{a}, Sp()))),
        ValueError);

    // Call arg count mismatch
    EXPECT_THROW(
        assert_structural_equal(
            Node(std::make_shared<Call>(std::make_shared<Op>("op"), std::vector<ExprPtr>{a}, Sp())),
            Node(std::make_shared<Call>(std::make_shared<Op>("op"), std::vector<ExprPtr>{a, b}, Sp()))),
        ValueError);
}
