...
// FP4 E2M1 packed (DT_FP4_E2M1X2): 1 byte = 2 nibbles (low 4 bits first, high 4 bits second, ONNX packing).
// Cast to FP32 expands last dim x2.
TEST_F(InterpTypeConvertTest, Fp4E2M1PackedDecodeToFp32)
{
    // 0x10: low=0 -> 0; high=1 -> E2M1 subnormal mant=1 -> 0.5
    // 0x23: low=3 -> exp=1,m=1 -> 1.5; high=2 -> exp=1,m=0 -> 1.0
    const std::vector<uint8_t> packed = {0x10, 0x23};
    auto src = makeTensorData(DT_FP4_E2M1X2, {2}, packed);
    auto out = makeTensorData(DT_FP32, {4}, 0.0f);
    calc::Cast(out, src);
    const std::vector<float> goldenVals = {0.0f, 0.5f, 1.5f, 1.0f};
...
{
    // 0x14: low=4  -> E1M2 e=1,m=0 -> 1.0
    //       high=1 -> E1M2 e=0,m=1 -> 0.125
    // 0xFC: low=12 -> sign=1,e=1,m=0 -> -1.0
    //       high=15-> sign=1,e=1,m=3 -> -1.75
    const std::vector<uint8_t> packed = {0x14, 0xFC};
    auto src = makeTensorData(DT_FP4_E1M2X2, {2}, packed);
    auto out = makeTensorData(DT_FP32, {4}, 0.0f);
    calc::Cast(out, src);
    const std::vector<float> goldenVals = {1.0f, 0.125f, -1.0f, -1.75f};
    auto golden = makeTensorData(DT_FP32, {4}, goldenVals);
    ASSERT_ALLCLOSE_ATOL(out, golden, 1e-6f);
}
...
{
    // a (packed {0x12,0x9A}) -> [1.0, 0.5, -1.0, -0.5]
    // b (packed {0x21,0x92}) -> [0.5, 1.0,  1.0, -0.5]
    ...
    auto a = makeTensorData(DT_FP4_E2M1X2, {2}, aPacked);
    auto b = makeTensorData(DT_FP4_E2M1X2, {2}, bPacked);
    auto out = makeTensorData(DT_FP4_E2M1X2, {2}, std::vector<uint8_t>(2, 0));
    calc::Add(out, a, b);
