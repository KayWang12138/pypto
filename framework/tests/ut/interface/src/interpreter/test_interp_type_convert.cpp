...
#define ASSERT_ALLCLOSE(self, other) \
    ASSERT(ERROR_CODE_UNDEFINED, calc::AllClose(self, other)) << "lhs:\n" << self->ToString() << "\nrhs:\n" << other->ToString() << "\n"

#define ASSERT_ALLCLOSE_ATOL(lhs, rhs, atol) \
    ASSERT(ERROR_CODE_UNDEFINED, calc::AllClose(lhs, rhs, atol, 1e-5)) << "lhs:\n" << lhs->ToString() << "\nrhs:\n" << rhs->ToString() << "\n"
    ...
    for (const auto &c : cases) {
        ...
        if (c.is_nan) {
            for (int64_t i = 0; i < 4; ++i) {
                ASSERT(ERROR_CODE_UNDEFINED, std::isnan(out->Get<float>(i))) << "expected NaN at index " << i;
            }
        } else {
    ...
    for (const auto &c : cases) {
        ...
        if (c.is_nan) {
            ...
            for (int64_t i = 0; i < 4; ++i) {
                ...
                ASSERT(ERROR_CODE_UNDEFINED, std::isinf(v)) << "expected Inf at index " << i;
                if (c.expected > 0.0f) {
                    ASSERT(ERROR_CODE_UNDEFINED, v > 0.0f) << "expected +Inf at index " << i;
                } else {
                    ASSERT(ERROR_CODE_UNDEFINED, v < 0.0f) << "expected -Inf at index " << i;
                }
            }
