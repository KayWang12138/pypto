...
TEST_F(DynamicOpsTest, BitwiseRightShift) {
    ...
    ProgramData::GetInstance().AppendInputs({
        RawTensorData::CreateConstantTensor<float>(self, static_cast<int16_t>(4)),
        RawTensorData::CreateConstantTensor<float>(other, static_cast<int16_t>(1)),
    });
    ProgramData::GetInstance().AppendOutputs({
        RawTensorData::CreateConstantTensor<float>(out, static_cast<int16_t>(2)),
    });
    ProgramData::GetInstance().AppendGoldens({
        RawTensorData::CreateConstantTensor<float>(out, static_cast<int16_t>(2)),
    });
