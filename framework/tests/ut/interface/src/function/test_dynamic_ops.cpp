...
TEST_F(DynamicOpsTest, MatMulPertensor)
{
    std::string logOutput = CaptureLogFileAndEcho([]() {
        ...
        MatMulParam mmPertensor{};
        mmPertensor.bTrans = true;
        mmPertensor.scale = static_cast<uint64_t>(scaleValueTmp);
        mmPertensor.relu = 1;
        calc::MatMul(golden, logicTensor0, logicTensor1, mmPertensor);
...
TEST_F(DynamicOpsTest, MatMulPerchannel)
{
    std::string logOutput = CaptureLogFileAndEcho([]() {
        ...
        MatMulParam mmPerchannel{};
        mmPerchannel.bTrans = true;
        mmPerchannel.scalePtr = &logicScaleData;
        calc::MatMul(golden, logicTensor0, logicTensor1, mmPerchannel);
...
TEST_F(DynamicOpsTest, MatMulBias)
{
    std::string logOutput = CaptureLogFileAndEcho([]() {
        ...
        MatMulParam mmBias{};
        mmBias.biasPtr = &logicBiasData;
        calc::MatMul(golden, d0, d1, mmBias);
...
TEST_F(DynamicOpsTest, MatMulL0CToL1Fixpipe)
{
    std::string logOutput = CaptureLogFileAndEcho([]() {
        ...
        auto logicScaleData = Trans(logicScale);
        MatMulParam mmL0c{};
        mmL0c.scalePtr = &logicScaleData;
        calc::MatMul(golden, logicTensor0, logicTensor1, mmL0c);
