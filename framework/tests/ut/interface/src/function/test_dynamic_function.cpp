...
TEST_F(DynamicFunctionTest, TestOnlyExpression) {
    ...
    EXPECT_EQ(attr->DumpAttr(2), "attr[2][4611686018427387914,  0,(k*16), 16, 16, 16, 64, 16,RUNTIME_GetViewValidShapeDim(64,(k*16),16)]]");
}
...
TEST_F(DynamicFunctionTest, TestOnlySymbol) {
    ...
    auto attr = rootFunc->GetCallopAttrList().front();
    FUNCTION_LOGI("%s", attr->DumpAttr().c_str());
    EXPECT_EQ(attr->DumpAttr(2), "attr[2][4611686018427387914,  k,  0,  1, 64,  4, 64,RUNTIME_GetViewValidShapeDim(4,k,1), 64]]");
}
