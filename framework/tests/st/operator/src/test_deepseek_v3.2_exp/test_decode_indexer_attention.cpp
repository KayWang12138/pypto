...
void TestDecodeIndexerAttentionSTest(DSIASimpleParams &params) {
    ...
    auto dynamicTmpRowSumOut = CreateConstantDynamicOutputTensor<float>({b * s1 * n2, maxBlockNumPerBatch * blockSize},
        DT_FP32, "tmpRowSumOut", 0, {bSymbol * s1Symbol * n2, maxBlockNumSymbol * blockSize});
    ...
    std::vector<RawTensorDataPtr> inputDataList = {dynamicX.dataPtr, wDq.dataPtr, wUqQr.dataPtr, wUk.dataPtr,
        wDkvKr.dataPtr, gammaCq.dataPtr, gammaCkv.dataPtr, dynamicSin.dataPtr, dynamicCos.dataPtr,
        dynamicCacheIndex.dataPtr, kvCache.dataPtr, krCache.dataPtr, dynamicBlockTable.dataPtr, dynamicActSeqs.dataPtr,
        qW.dataPtr, kW.dataPtr, projW.dataPtr, lnW.dataPtr, lnBias.dataPtr, indexKCache.dataPtr};
    ...
    DecodeIndexerAttention(dynamicX.tensor, wDq.tensor, wUqQr.tensor, wUk.tensor, wDkvKr.tensor, gammaCq.tensor,
        gammaCkv.tensor, dynamicSin.tensor, dynamicCos.tensor, dynamicCacheIndex.tensor, kvCache.tensor, krCache.tensor,
        quantInputs, dynamicBlockTable.tensor, dynamicActSeqs.tensor, qW.tensor, kW.tensor, projW.tensor, lnW.tensor,
        lnBias.tensor, indexKCache.tensor, dynamicSaOut.tensor, dynamicGatherRes.tensor, dynamicTmpTopkInput.tensor,
