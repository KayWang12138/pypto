...
void TestDeepSeekIndexerAttentionQuantSTest(DSIASimpleParams &params) {
    ...
    auto indexerTopkTmpOut = CreateConstantDynamicOutputTensor<float>(indexerTopkTmpOutShape, DT_FP32,
        "indexerTopkTmpOut", 0, {bSymbol * s1Symbol * n2, maxBlockNumPerBatch * blockSize});
    ...
    std::vector<RawTensorDataPtr> inputDataList = {dynamicX.dataPtr, wDq.dataPtr, wUqQr.dataPtr, wUk.dataPtr,
        wDkvKr.dataPtr, gammaCq.dataPtr, gammaCkv.dataPtr, dynamicCos.dataPtr, dynamicSin.dataPtr, kvCache.dataPtr,
    ...
    DeepSeekIndexerAttentionQuant(dynamicX.tensor, wDq.tensor, wUqQr.tensor, wUk.tensor, wDkvKr.tensor, gammaCq.tensor,
        gammaCkv.tensor, dynamicCos.tensor, dynamicSin.tensor, dynamicCacheIndex.tensor, kvCache.tensor, krCache.tensor,
