...
namespace {
struct ColExpandExpDifOpFuncArgs : public OpFuncArgs {
    ColExpandExpDifOpFuncArgs(const std::vector<int64_t> &viewShape, const std::vector<int64_t> tileShape)
...
struct ColExpandExpDifOpMetaData {
    explicit ColExpandExpDifOpMetaData(const OpFunc &opFunc, const nlohmann::json &test_data)
...
static void ColExpandExpDifOperationExeFunc2Dims(
    const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
    FUNCTION("main", {inputs[0], inputs[1]}, {outputs[0]}) {
        SymbolicScalar firstDim = inputs[0].GetShape()[0];
        SymbolicScalar secondDim = inputs[0].GetShape()[1];
        auto args = static_cast<const ColExpandExpDifOpFuncArgs *>(opArgs);
        ...
        LOOP("LOOP_L0_bIdx", FunctionType::DYNAMIC_LOOP, bIdx, LoopRange(0, bloop, 1)) {
            LOOP("LOOP_L1_sIdx", FunctionType::DYNAMIC_LOOP, sIdx, LoopRange(0, sloop, 1)) {
                ...
                auto res = ColExpandExpDif(tileTensor0, tileTensor1);
...
static void ColExpandExpDifOperationExeFunc3Dims(
    const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
    FUNCTION("main", {inputs[0], inputs[1]}, {outputs[0]}) {
        SymbolicScalar firstDim = inputs[0].GetShape()[0];
        SymbolicScalar secondDim = inputs[0].GetShape()[1];
...
static void ColExpandExpDifOperationExeFunc4Dims(
    const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
    FUNCTION("main", {inputs[0], inputs[1]}, {outputs[0]}) {
        SymbolicScalar firstDim = inputs[0].GetShape()[0];
        SymbolicScalar secondDim = inputs[0].GetShape()[1];
...
class ColExpandExpDifOperationTest : public npu::tile_fwk::stest::TestSuite_STest_Ops_Aihac_param<ColExpandExpDifOpMetaData> {};

INSTANTIATE_TEST_SUITE_P(TestColExpandExpDif, ColExpandExpDifOperationTest,
    ::testing::ValuesIn(GetOpMetaData<ColExpandExpDifOpMetaData>(
        {ColExpandExpDifOperationExeFunc2Dims, ColExpandExpDifOperationExeFunc3Dims, ColExpandExpDifOperationExeFunc4Dims}, "ColExpandExpDif")));

TEST_P(ColExpandExpDifOperationTest, TestColExpandExpDif) {
    auto test_data = GetParam().test_data_;
    auto args = ColExpandExpDifOpFuncArgs(GetViewShape(test_data), GetTileShape(test_data));
    auto testCase = CreateTestCaseDesc<ColExpandExpDifOpMetaData>(GetParam(), &args);
