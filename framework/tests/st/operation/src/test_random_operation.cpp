...
static void RandomOperationExeFunc(
    ...
    FUNCTION("main", {}, {outputs[0]}) {
        ...
        const int64_t outputDim = outputs[0].GetShape()[0].Concrete();
        ...
        LOOP("LOOP_L0_idx", FunctionType::DYNAMIC_LOOP, idx, LoopRange(0, loop, 1)) {
            ...
            int64_t validShape = std::min(outputDim - offset, viewShape);
            
            uint64_t tileOffset = static_cast<uint64_t>(idx * viewShape);
