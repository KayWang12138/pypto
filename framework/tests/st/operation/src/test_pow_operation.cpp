...
static inline void DoPow(const std::vector<int64_t> &shapes, const std::vector<SymbolicScalar> &validShape,
    const std::vector<SymbolicScalar> &offset, const Tensor &input0, const Tensor &input1, Tensor &output) {
    Tensor tileTensor0 = View(input0, shapes, validShape, offset);
    Tensor tileTensor1 = View(input1, shapes, validShape, offset);
    auto res = Pow(tileTensor0, tileTensor1);
    Assemble(res, offset, output);
}
    ...
    FUNCTION("main", {inputs[0], inputs[1]}, {outputs[0]}) {
        ...
        LOOP("LOOP_L0_bIdx", FunctionType::DYNAMIC_LOOP, bIdx, LoopRange(0, bloop, 1)) {
            LOOP("LOOP_L1_sIdx", FunctionType::DYNAMIC_LOOP, sIdx, LoopRange(0, sloop, 1)) {
                ...
                DoPow(shapes, validShape, offset, inputs[0], inputs[1], outputs[0]);
            }
        }
    }
}
