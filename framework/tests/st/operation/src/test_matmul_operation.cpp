...
static void CheckBTransNZUnalignedImpl(bool transB, bool isCMatrixNz, const int &bN, const int &cN) {
...
static void CheckBTransNZUnaligned(auto &args, TestCaseDesc &testCase, Tensor &l0c2L1Tensor) {
    int cN = testCase.outputTensors[0].GetShape()[1];
    if(args.param_.is_as_left_matrix){
        int bN = args.param_.transB ? testCase.inputTensors[1].GetShape()[0] : testCase.inputTensors[1].GetShape()[1];
        CheckBTransNZUnalignedImpl(false, args.param_.isCMatrixNz, bN, cN);
    }else{
        int bN = args.param_.is_l0c2l1_trans ? l0c2L1Tensor.shape[1] : l0c2L1Tensor.shape[0];
        CheckBTransNZUnalignedImpl(args.param_.is_l0c2l1_trans, args.param_.isCMatrixNz, bN, cN);
    }
}
