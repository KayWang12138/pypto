...
namespace {
struct PReLUTensorDesc {
    std::vector<int64_t> inputShape_;
    std::vector<int64_t> weightShape_;
    DataType dtype_;
...
struct PReLUOpFuncArgs : public OpFuncArgs {
    PReLUOpFuncArgs(const PReLUTensorDesc &desc) : desc_(desc) {}

    PReLUTensorDesc desc_;
};
...
static void PReLUOperationExeFunc2Dims(
    ...
    FUNCTION("main", {inputs[0], inputs[1]}, {outputs[0]}) {
        ...
        const auto &desc = args->desc_;
        
        SymbolicScalar firstDim = inputs[0].GetShape()[0];
        SymbolicScalar secondDim = inputs[0].GetShape()[1];
        
        const int firstViewShape = desc.viewShape_[0];
        const int secondViewShape = desc.viewShape_[1];
        ...
        LOOP("LOOP_L0_bIdx", FunctionType::DYNAMIC_LOOP, bIdx, LoopRange(0, bloop, 1)) {
            LOOP("LOOP_L1_sIdx", FunctionType::DYNAMIC_LOOP, sIdx, LoopRange(0, sloop, 1)) {
                ...
                auto tileTensor1 = View(inputs[1], {weightViewShape},
                    {weightViewShape}, {sIdx * secondViewShape});
                
                TileShape::Current().SetVecTile(desc.tileShape_);
...
static void PReLUOperationExeFunc3Dims(
    ...
    FUNCTION("main", {inputs[0], inputs[1]}, {outputs[0]}) {
        ...
        const int firstViewShape = desc.viewShape_[0];
        const int secondViewShape = desc.viewShape_[1];
        const int thirdViewShape = desc.viewShape_[2];
...
static void PReLUOperationExeFunc4Dims(
    ...
    FUNCTION("main", {inputs[0], inputs[1]}, {outputs[0]}) {
        ...
        const int secondViewShape = desc.viewShape_[1];
        const int thirdViewShape = desc.viewShape_[2];
        const int fourthViewShape = desc.viewShape_[3];
...
TEST_P(PReLUOperationTest, TestPReLU) {
    ...
    PReLUTensorDesc desc;
    desc.inputShape_ = GetInputShape(test_data);
    desc.weightShape_ = {desc.inputShape_[1]};
    desc.dtype_ = GetDataType(test_data);
    desc.viewShape_ = GetViewShape(test_data);
    desc.tileShape_ = GetTileShape(test_data);
    
    auto args = PReLUOpFuncArgs(desc);
