...
constexpr int ALIGN_64 = 64;
constexpr int SHAPE_DIM_2 = 2;
...
static Tensor CallMXMatmulOp(const Tensor &tensorA, const Tensor &scaleA, const Tensor &tensorB, const Tensor &scaleB,
...
static void MatmulOperationExeFuncNoSplit(
    ...
    SymbolicScalar kScaleDim = (kDim + ALIGN_64 - 1)/ ALIGN_64;
    FUNCTION("testNoSplit",
        ...
        LOOP("mLoop", FunctionType::DYNAMIC_LOOP, mIdx, LoopRange(1)) {
            ...
            if (transA) {
                ...
                scaleA = View(
                    inputs[scaleAIndex], {kScaleDim, mDim, SHAPE_DIM_2}, {kScaleDim, mDim, SHAPE_DIM_2}, {0, 0, 0});
            } else {
                tensorA = View(inputs[0], {mDim, kDim}, {mDim, kDim}, {mIdx, 0});
                scaleA = View(
                    inputs[scaleAIndex], {mDim, kScaleDim, SHAPE_DIM_2}, {mDim, kScaleDim, SHAPE_DIM_2}, {mIdx, 0, 0});
            }
            Tensor tensorB;
            Tensor scaleB;
            if (transB) {
                ...
                scaleB = View(
                    inputs[scaleBIndex], {nDim, kScaleDim, SHAPE_DIM_2}, {nDim, kScaleDim, SHAPE_DIM_2}, {0, 0, 0});
            } else {
                tensorB = View(inputs[1], {kDim, nDim}, {kDim, nDim}, {0, 0});
                scaleB = View(
                    inputs[scaleBIndex], {kScaleDim, nDim, SHAPE_DIM_2}, {kScaleDim, nDim, SHAPE_DIM_2}, {0, 0, 0});
            }
            ...
            outputs[0] = CallMXMatmulOp(tensorA, scaleA, tensorB, scaleB, args->param_, param);
        }
    }
}
    ...
    FUNCTION("testMSplit",
        ...
        LOOP("mLoop", FunctionType::DYNAMIC_LOOP, mIdx, LoopRange(0, CeilDivSymbolicScalar(mDim, mView), 1)) {
            ...
            if (transA) {
                ...
                scaleA = View(inputs[scaleAIndex], {kScaleDim, mView, SHAPE_DIM_2},
                    {kScaleDim, std::min(mDim - mView * mIdx, mView), SHAPE_DIM_2}, {0, mIdx * mView, 0});
            } else {
                tensorA =
                    View(inputs[0], {mView, kDim}, {std::min(mDim - mView * mIdx, mView), kDim}, {mIdx * mView, 0});
                scaleA = View(inputs[scaleAIndex], {mView, kScaleDim, SHAPE_DIM_2},
                    {std::min(mDim - mView * mIdx, mView), kScaleDim, SHAPE_DIM_2}, {mIdx * mView, 0, 0});
            }
            Tensor tensorB;
            Tensor scaleB;
            if (transB) {
                tensorB = View(inputs[1], {nDim, kDim}, {nDim, kDim}, {0, 0});
                scaleB = View(inputs[scaleBIndex], {nDim, kScaleDim, SHAPE_DIM_2}, {nDim, kScaleDim, SHAPE_DIM_2}, {0, 0, 0});
            } else {
                tensorB = View(inputs[1], {kDim, nDim}, {kDim, nDim}, {0, 0});
                scaleB = View(inputs[scaleBIndex], {kScaleDim, nDim, SHAPE_DIM_2}, {kScaleDim, nDim, SHAPE_DIM_2}, {0, 0, 0});
            }
            Matrix::MatmulExtendParam param;
            if (args->param_.hasBias) {
                param.biasTensor = View(inputs[biasIndex], {1, nDim}, {1, nDim}, {0, 0});
            ...
            param.reluType = reluType;
            param.scaleValue = scaleValue;
            Tensor tensorC = CallMXMatmulOp(tensorA, scaleA, tensorB, scaleB, args->param_, param);
...
static void MatmulOperationExeFuncSplitN(
    ...
    FUNCTION("testNSplit",
        ...
        LOOP("nLoop", FunctionType::DYNAMIC_LOOP, nIdx, LoopRange(0, CeilDivSymbolicScalar(nDim, nView), 1)) {
            ...
            if (transA) {
                tensorA = View(inputs[0], {kDim, mDim}, {kDim, mDim}, {0, 0});
                scaleA = View(inputs[scaleAIndex], {kScaleDim, mDim, SHAPE_DIM_2}, {kScaleDim, mDim, SHAPE_DIM_2}, {0, 0, 0});
            } else {
                tensorA = View(inputs[0], {mDim, kDim}, {mDim, kDim}, {0, 0});
                scaleA = View(inputs[scaleAIndex], {mDim, kScaleDim, SHAPE_DIM_2}, {mDim, kScaleDim, SHAPE_DIM_2}, {0, 0, 0});
            }
            Tensor tensorB;
            Tensor scaleB;
            if (transB) {
                ...
                scaleB = View(inputs[scaleBIndex], {nView, kScaleDim, SHAPE_DIM_2},
                    {std::min(nDim - nIdx * nView, nView), kScaleDim, SHAPE_DIM_2}, {nIdx * nView, 0, 0});
            } else {
                tensorB =
                    View(inputs[1], {kDim, nView}, {kDim, std::min(nDim - nIdx * nView, nView)}, {0, nIdx * nView});
                scaleB = View(inputs[scaleBIndex], {kScaleDim, nView, SHAPE_DIM_2},
                    {kScaleDim, std::min(nDim - nIdx * nView, nView), SHAPE_DIM_2}, {0, nIdx * nView, 0});
            }
            Matrix::MatmulExtendParam param;
            param.reluType = reluType;
            param.scaleValue = scaleValue;
...
static void MXMatmulOperationExeFunc(
    ...
    if (args->param_.hasScale || args->param_.hasBias) {
        ...
        TileShape::Current().SetCubeTile({args->tileShape_[0][0], args->tileShape_[0][1]},
            {args->tileShape_[1][0], args->tileShape_[1][1]}, {nTile, nTile}, true, args->param_.enableKSplit);
    } else {
        TileShape::Current().SetCubeTile({args->tileShape_[0][0], args->tileShape_[0][1]},
