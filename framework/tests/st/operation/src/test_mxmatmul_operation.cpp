    ...
    FUNCTION("testMSplit",
        ...
        LOOP("mLoop", FunctionType::DYNAMIC_LOOP, mIdx, LoopRange(0, CeilDivSymbolicScalar(mDim, mView), 1)) {
            ...
            if (transA) {
                ...
                scaleA = View(inputs[scaleAIndex], {kScaleDim, mView},
                    {kScaleDim, std::min(mDim - mView * mIdx, mView)}, {0, mIdx * mView});
            } else {
                tensorA =
                    View(inputs[0], {mView, kDim}, {std::min(mDim - mView * mIdx, mView), kDim}, {mIdx * mView, 0});
                scaleA = View(inputs[scaleAIndex], {mView, kScaleDim},
                    {std::min(mDim - mView * mIdx, mView), kScaleDim}, {mIdx * mView, 0});
            }
            Tensor tensorB;
            Tensor scaleB;
            if (transB) {
    ...
    FUNCTION("testNSplit",
        ...
        LOOP("nLoop", FunctionType::DYNAMIC_LOOP, nIdx, LoopRange(0, CeilDivSymbolicScalar(nDim, nView), 1)) {
            ...
            if (transA) {
                ...
                scaleA = View(inputs[scaleAIndex], {mDim, kScaleDim}, {mDim, kScaleDim}, {0, 0});
            }
            Tensor tensorB;
            Tensor scaleB;
            if (transB) {
                ...
                scaleB = View(inputs[scaleBIndex], {nView, kScaleDim},
                    {std::min(nDim - nIdx * nView, nView), kScaleDim}, {nIdx * nView, 0});
            } else {
                tensorB =
                    View(inputs[1], {kDim, nView}, {kDim, std::min(nDim - nIdx * nView, nView)}, {0, nIdx * nView});
                scaleB = View(inputs[scaleBIndex], {kScaleDim, nView},
                    {kScaleDim, std::min(nDim - nIdx * nView, nView)}, {0, nIdx * nView});
            }
            Matrix::MatmulExtendParam param;
            param.reluType = reluType;
            param.scaleValue = scaleValue;
