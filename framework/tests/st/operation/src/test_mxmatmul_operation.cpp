...
{
    return Matrix::Matmul(param.outDtype, tensorA, tensorB, matmulExtendParam, param.transA, param.transA,
                          param.transB, param.transB, param.isCMatrixNz);
}
...
static void MatmulOperationExeFuncNoSplit(
    ...
    FUNCTION("testNoSplit",
             {inputs[0], inputs[1], inputs[scaleAIndex], intput[scaleBIndex], inputs[scaleIndex], inputs[biasIndex]},
             {outputs[0]})
    {
...
INSTANTIATE_TEST_SUITE_P(TestMXMatmul, MXMatmulOperationTest,
    ::testing::ValuesIn(GetOpMetaData<MXMatmulOpMetaData>({MatmulOperationExeFunc}, "MXMatmul")));
...
class MXMatmulVerifyOperationTest : public npu::tile_fwk::stest::TestSuite_STest_Ops_Aihac_param<MatmulOpMetaData> {};

INSTANTIATE_TEST_SUITE_P(TestMXMatmulVerify, MXMatmulVerifyOperationTest,
    ::testing::ValuesIn(GetOpMetaData<MatmulOpMetaData>({MXMatmulOperationExeFunc}, "MXMatmulVerify")));

TEST_P(MXMatmulVerifyOperationTest, TestMXMatmulVerify) {
    TestCaseDesc testCase;
    auto test_data = GetParam().test_data_;
    testCase.outputTensors = GetMatmulTensors(test_data, "output_tensors");
    testCase.inputTensors = GetMatmulTensors(test_data, "input_tensors");
    ...
    testCase.inputPaths = {
        ...
        GetGoldenDir() + "/" + testCase.inputTensors[1].GetStorage()->Symbol() + ".bin",
        GetGoldenDir() + "/" + testCase.inputTensors[scaleAIndex].GetStorage()->Symbol() + ".bin",
        GetGoldenDir() + "/" + testCase.inputTensors[scaleBIndex].GetStorage()->Symbol() + ".bin",
    };
    if (!args.param_.enableKSplit) {
