...
/*!
 * \file test_bitwise_and_operation.cpp
 * \brief
 */
...
namespace {
struct BitwiseAndOpFuncArgs : public OpFuncArgs {
    BitwiseAndOpFuncArgs(const std::vector<int64_t> &viewShape, const std::vector<int64_t> tileShape)
...
struct BitwiseAndOpMetaData {
    explicit BitwiseAndOpMetaData(const OpFunc &opFunc, const nlohmann::json &test_data)
...
static void BitwiseAndOperationExeFunc2Dims(
    const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {

    FUNCTION("main", {inputs[0], inputs[1]}, {outputs[0]}) {
        auto args = static_cast<const BitwiseAndOpFuncArgs *>(opArgs);
        ...
        LOOP("LOOP_L0_bIdx", FunctionType::DYNAMIC_LOOP, bIdx, LoopRange(0, bloop, 1)) {
            LOOP("LOOP_L1_sIdx", FunctionType::DYNAMIC_LOOP, sIdx, LoopRange(0, sloop, 1)) {
                ...
                auto res = BitwiseAnd(tileTensor0, tileTensor1);
...
static void BitwiseAndOperationExeFunc3Dims(
    const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
...
static void BitwiseAndOperationExeFunc4Dims(
    const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
    FUNCTION("main", {inputs[0], inputs[1]}, {outputs[0]}) {
        auto args = static_cast<const BitwiseAndOpFuncArgs *>(opArgs);
        const int firstViewShape = args->viewShape_[0];
...
class BitwiseAndOperationTest : public npu::tile_fwk::stest::TestSuite_STest_Ops_Aihac_param<BitwiseAndOpMetaData> {};

INSTANTIATE_TEST_SUITE_P(TestBitwiseAnd, BitwiseAndOperationTest,
    ::testing::ValuesIn(GetOpMetaData<BitwiseAndOpMetaData>(
        {BitwiseAndOperationExeFunc2Dims, BitwiseAndOperationExeFunc3Dims, BitwiseAndOperationExeFunc4Dims}, "BitwiseAnd")));

TEST_P(BitwiseAndOperationTest, TestBitwiseAnd) {
    auto test_data = GetParam().test_data_;
    auto args = BitwiseAndOpFuncArgs(GetViewShape(test_data), GetTileShape(test_data));
    auto testCase = CreateTestCaseDesc<BitwiseAndOpMetaData>(GetParam(), &args);
