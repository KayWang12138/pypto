...
namespace {
struct MinSOpFuncArgs : public OpFuncArgs {
    MinSOpFuncArgs(const Element &value, const std::vector<int64_t> &viewShape, const std::vector<int64_t> tileShape)
...
struct MinSOpMetaData {
    explicit MinSOpMetaData(const OpFunc &opFunc, const nlohmann::json &test_data)
...
static void MinSOperationExeFuncDoubleCut(
    const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
    config::SetCodeGenOption(SUPPORT_DYNAMIC_UNALIGNED, true);
    FUNCTION("main", {inputs[0]}, {outputs[0]}) {
        SymbolicScalar firstDim = inputs[0].GetShape()[0];
        SymbolicScalar secondDim = inputs[0].GetShape()[1];
        auto args = static_cast<const MinSOpFuncArgs *>(opArgs);
        ...
        LOOP("LOOP_L0_bIdx", FunctionType::DYNAMIC_LOOP, bIdx, LoopRange(0, bloop, 1)) {
            LOOP("LOOP_L1_sIdx", FunctionType::DYNAMIC_LOOP, sIdx, LoopRange(0, sloop, 1)) {
                ...
                auto res = MinS(tileTensor0, args->value_);
...
static void MinSOperationExeFuncTripleCut(
    const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
    config::SetCodeGenOption(SUPPORT_DYNAMIC_UNALIGNED, true);
    FUNCTION("main", {inputs[0]}, {outputs[0]}) {
        SymbolicScalar firstDim = inputs[0].GetShape()[0];
...
static void MinSOperationExeFuncQuadrupleCut(
    const std::vector<Tensor> &inputs, std::vector<Tensor> &outputs, const OpFuncArgs *opArgs) {
    config::SetCodeGenOption(SUPPORT_DYNAMIC_UNALIGNED, true);
    FUNCTION("main", {inputs[0]}, {outputs[0]}) {
        SymbolicScalar firstDim = inputs[0].GetShape()[0];
...
class MinSOperationTest : public npu::tile_fwk::stest::TestSuite_STest_Ops_Aihac_param<MinSOpMetaData> {};

INSTANTIATE_TEST_SUITE_P(TestMinS, MinSOperationTest,
    ::testing::ValuesIn(GetOpMetaData<MinSOpMetaData>(
        {MinSOperationExeFuncDoubleCut, MinSOperationExeFuncTripleCut, MinSOperationExeFuncQuadrupleCut}, "MinS")));
...
TEST_P(MinSOperationTest, TestMinS) {
    ...
    auto args = MinSOpFuncArgs(value, GetViewShape(test_data), GetTileShape(test_data));
    testCase.args = &args;
