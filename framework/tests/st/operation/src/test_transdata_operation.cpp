...
struct TransDataOpFuncArgs : public OpFuncArgs {
    TransDataOpFuncArgs(const std::vector<int64_t>& viewShape, const std::vector<int64_t> tileShape)
    ...
    const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs, const OpFuncArgs* opArgs)
{
    ...
    FUNCTION("main", {inputs[0]}, {outputs[0]})
    {
        ...
        auto res = TransData(tileTensor0, TransDataType::NCHW2NC1HWC0);
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::ValuesIn(GetOpMetaData<TransDataOpMetaData>({TransDataOperationExeFunc4Dims}, "TransData")));

TEST_P(TransDataOperationTest, TestTransData)
{
    auto test_data = GetParam().test_data_;

    auto args = TransDataOpFuncArgs(GetViewShape(test_data), GetTileShape(test_data));
