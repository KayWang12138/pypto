...
static void IndexAddOperationExeFunc2Dims(
    ...
    FUNCTION("main", {inputs[0], inputs[1], inputs[2]}, {outputs[0]}) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, idxDim == inputs[1].GetShape()[axis]);
        ASSERT(ERROR_CODE_UNDEFINED, viewShape[axis] >= std::max(inputs[0].GetShape()[axis], idxDim)); // 确保viewshape按最大的切
