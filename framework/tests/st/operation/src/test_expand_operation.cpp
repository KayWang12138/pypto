...
void UpdateInputViewShape(std::vector<int64_t> &inputViewShape, const int expandAxis, const std::vector<SymbolicScalar> inputsShape, std::vector<SymbolicScalar> outputsShape) {
    if (expandAxis != -1) {
        for (size_t i = 0; i < inputsShape.size(); i++) {
            if (inputsShape[i] != outputsShape[i]) {
...
static void ExpandOperationExeFunc2Dims(
    ...
    FUNCTION("main", {inputs[0]}, {outputs[0]}) {
        ...
        const int expandAxis = ExpandAxis(inputsShape, outputsShape, outputsShape);
        UpdateInputViewShape(inputViewShape, expandAxis, inputsShape);
