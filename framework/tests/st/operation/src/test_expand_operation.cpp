...
void UpdateInputViewShape(std::vector<SymbolicScalar> &inputViewShape, int expandAxis, const std::vector<SymbolicScalar> &inputsShape, const std::vector<SymbolicScalar> &outputsShape) {
    if (expandAxis != -1) {
        for (size_t i = 0; i < inputsShape.size(); i++) {
            if (inputsShape[i] != outputsShape[i]) {
...
static void ExpandOperationExeFunc2Dims(
    ...
    FUNCTION("main", {inputs[0]}, {outputs[0]}) {
        ...
        std::vector<SymbolicScalar> inputViewShape = viewShape;
