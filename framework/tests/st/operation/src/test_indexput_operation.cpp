...
void IndexPutTestCase(TestCaseDesc &testCase_, const nlohmann::json &test_data) {
    ...
    ASSERT(ERROR_CODE_UNKNOWN, sizeIndices >= sizeIndicesMin && sizeIndices <= sizeIndicesMax) << "unsupport the input indices dim";
    for (size_t i = 0; i < sizeIndices; i++) {
        ASSERT(ERROR_CODE_UNKNOWN, testCase_.inputTensors[i + NUM2].GetShape().size() == 1) << "indices must be a one-dimensional array";
    }
