...
def gen_quantmx_op_golden(case_name: str, output: Path, case_index: int = None) -> bool:
    def golden_func(inputs: list, _config: dict):
        ...
        if x.ndim != 2:
            raise ValueError("QuantMX golden only supports 2D input.")
        if x.shape[1] % 64 != 0:
            raise ValueError("QuantMX golden requires K aligned to 64.")

        rows, cols = x.shape
        group_cols = cols // 32
        quant = np.empty((rows, cols), dtype=np.uint8)
        exp = np.empty((rows, group_cols), dtype=np.uint8)

        for row in range(rows):
            for group in range(group_cols):
                start = group * 32
                end = start + 32
                group_data = x[row, start:end]
                ...
                for inner, value in enumerate(group_data):
                    quant[row, start + inner] = _encode_e4m3_fn(float(value) * group_scaling)
        return [quant, exp]
