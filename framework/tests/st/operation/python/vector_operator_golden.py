...
def generate_mx_matmul_scale_files(input_tensors: list, output_path: Path, config: dict):
    ...
    for input_tensor in config["input_tensors"]:
        ...
        if index == 0:
            if params.get("transA"):
                tensor_shape[0], tensor_shape[1] = tensor_shape[1], tensor_shape[0]
            if not params.get("scaleTransA"):
                ...
                scale_shape = (tensor_shape[1] // 32, tensor_shape[0])
                scale_mx_gm = np.random.uniform(127, 130, scale_shape).astype(np.uint8)
                scale_mx_gm_save = (scale_mx_gm.T.reshape(tensor_shape[0], tensor_shape[1] // 64, 2).transpose(1, 0, 2))
        if index == 1:
            if params.get("transB"):
                tensor_shape[0], tensor_shape[1] = tensor_shape[1], tensor_shape[0]
            if not params.get("scaleTransB"):
                ...
                scale_shape = (tensor_shape[1], tensor_shape[0] // 32)
                scale_mx_gm = np.random.uniform(127, 130, scale_shape).astype(np.uint8)
                scale_mx_gm_save = scale_mx_gm
...
def apply_mxmatmul_scaling(inputs: list, params: dict, tensor_a: np.ndarray, tensor_b: np.ndarray):
    ...
    scale_a = transpose_if_needed(inputs[2], params.get("scaleTransA"))
    scale_b = transpose_if_needed(inputs[3], params.get("scaleTransB"))
