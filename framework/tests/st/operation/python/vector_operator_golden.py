...
def scatter_tensor_golden_func(inputs, config: dict):
    ...
    min = input1_tensor["data_range"]["min"]
    max = input1_tensor["data_range"]["max"]
    ...
    if (axis == dims - 1 or axis + dims == dims - 1) and (len(reduceop) == 0 or reduceop == "None") and max >= shape[-1]:
        if dims == 2:
            for i in range(shape[0]):
                inputs[1][i] = np.random.choice(range(min, max), shape[-1], False).astype(dtype)
        elif dims == 3:
            for i in range(shape[0]):
                for j in range(shape[1]):
                    inputs[1][i, j] = np.random.choice(range(min, max), shape[-1], False).astype(dtype)
        elif dims == 4:
            for i in range(shape[0]):
                for j in range(shape[1]):
                    for k in range(shape[2]):
                        inputs[1][i, j, k] = np.random.choice(range(min, max), shape[-1], False).astype(dtype)
        else:
