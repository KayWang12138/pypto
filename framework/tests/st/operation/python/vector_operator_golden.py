...
def gen_fillpad_op_golden(case_name: str, output: Path, case_index: int = None) -> bool:
    def golden_func(inputs: list, config: dict):
        ...
        result = np.full(output_shape, pad_value, dtype=input_tensor.dtype) ##创建全填充 pad_value 的结果数组
        ...
        if ndim == 1:
            ...
            result[:valid_shape[0], :valid_shape[1], :valid_shape[2]] = input_tensor[:valid_shape[0], :valid_shape[1], :valid_shape[2]]
        elif ndim == 4:
            result[:valid_shape[0], :valid_shape[1], :valid_shape[2], :valid_shape[3]] = input_tensor[:valid_shape[0], :valid_shape[1], :valid_shape[2], :valid_shape[3]]
