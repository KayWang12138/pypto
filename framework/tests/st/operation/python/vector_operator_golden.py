...
def gen_GatherMask_op_golden(
    ...
    def golden_func(inputs: list, config: dict):
        ...
        params = config.get("params")
        patternMode = int(params.get("patternMode"))
        last_dim = input_tensor.shape[-1]
        if last_dim % 2 != 0 and patternMode in [1, 2]:
            raise ValueError("The last axis should be divisible by 2 when ptternMode is 1 or 2")
        if last_dim % 4 != 0 and patternMode in [3, 4, 5, 6]:
        ...
        if patternMode == 7:
            ...
            return [output.numpy()]
    logging.debug("Case(%s), Golden creating...", case_name)
    return gen_op_golden("CumSum", golden_func, output, case_index)
