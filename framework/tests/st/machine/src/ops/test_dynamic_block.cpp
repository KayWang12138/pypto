...
TEST_F(DynamicBlockTest, VectorCube) {
    ...
    auto codeGenUseIR = [&](const std::string &filepath, const std::string &source) -> std::string {
        if (StringUtils::EndsWith(filepath, "_aiv.cpp")) {
            size_t posVoid = source.find(" void ");
            size_t posLparen = source.find("(", posVoid);
            std::string name = source.substr(posVoid + 0x6, posLparen - posVoid - 0x6);

            printf("!!! %s !!! %s\n", name.c_str(), source.c_str());
            auto prog = pto::CreateAdd("TENSOR_L0_Unroll1_PATH0_hiddenfunc0_8_0_4503599627370496");
            SourceCppIRSerializer serializer;

            MemoryIRBuffer buf;
            serializer.Serialize(buf, prog);
            std::string text = buf.GetRawBuffer();
            return text;
        } else {
            return source;
        }
    ...
    FUNCTION("main", {inputA, inputB}, {output}) {
        ...
        LOOP("L0", FunctionType::DYNAMIC_LOOP, i, LoopRange(1)) {
            ...
            output = Matrix::Matmul(DT_FP32, a, inputB, false, false);
        }
    }
