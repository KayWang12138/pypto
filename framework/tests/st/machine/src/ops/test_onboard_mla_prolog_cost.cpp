...
/*!
 * \file test_onboard_mla_prolog_cost.cpp
 * \brief
 */
...
class MlaPrologOnBoardCostTest : public npu::tile_fwk::stest::TestSuite_STest_Ops_Aihac {};

template <typename T = npu::tile_fwk::float16>
void TestMlaProlog(std::vector<int> &params, string dataPath, bool isQuant = false, uint64_t opTimeThreshold = 0) {
    ...
    PROGRAM("MlaProlog") {
        ...
        Tensor wDq(dType, w_qa_shape, (uint8_t *)wDqPtr, "wDq");
        Tensor wUqQr(dTypeQuantIn, w_qb_shape, (uint8_t *)wUqQrPtr, "wUqQr");
        Tensor wDkvKr(dType, w_kv_a_shape, (uint8_t *)wDkvKrPtr, "wDkvKr");
        Tensor wUk(dType, w_kv_b_k_shape, (uint8_t *)wUkPtr, "wUk");
        ...
        if (isQuant) {
            ...
            TileFwkBeginFunction("MlaProlog_T", {x, wDq, wUqQr, w_qb_scale, wDkvKr, wUk, position_ids,
            ...
            TileFwkEndFunction();
            opArgsRun = {x_ptr, wDqPtr, wUqQrPtr, w_qb_scale_ptr, wDkvKrPtr, wUkPtr, position_ids_ptr, cos_ptr, sin_ptr, past_key_states_ptr, kv_len_ptr, q_out_ptr};
        } else {
            std::tuple<Tensor, Tensor> res;
            DeepseekAttention attention(g_deepseekConfig, aw, 1);
            ...
            TileFwkBeginFunction("MlaProlog_T", {x, wDq, wUqQr, wDkvKr, wUk, position_ids,
                    cos, sin, past_key_states, kv_len, output_q});
            {
                auto q_kv = attention.MlaPrologFoward(x, position_ids, cos, sin, kv_len, past_key_states, ropeTileConfig, isQuant);
                output_q = q_kv[0];
            ...
            TileFwkEndFunction();
            opArgsRun = {x_ptr, wDqPtr, wUqQrPtr, wDkvKrPtr, wUkPtr, position_ids_ptr, cos_ptr, sin_ptr, past_key_states_ptr, kv_len_ptr, q_out_ptr};
        }
        /* torch compile */
        ...
        TileFwkRunAsync(handle, workspaceAddr, machine::GetRA()->GetScheStream(), opArgsRun, opSizes);
    ...
    int ret = (taskTime > 0 && taskTime < opTimeThreshold * 1.10);
    std::cout << "allTimeThreshold:" << opTimeThreshold << ", cost:" << taskTime << std::endl;
    EXPECT_EQ(ret, true);
}

void readCostBlockTable(const std::string& filename, int rows, int cols, std::vector<std::vector<int>> & blockTable) {
...
void attention(std::vector<int> &params, string dataPath, bool isQuant = false, bool fuse = true, bool usePost = true,
    ...
    PROGRAM("MlaProlog") {
        ...
        Tensor sin(dType, cos_shape, (uint8_t *)sin_ptr, "sin");
        Tensor past_key_states(dType, past_key_states_shape, (uint8_t *)past_key_states_ptr, "past_key_states");
        Tensor kv_len(DT_INT32, kv_len_shape, (uint8_t *)kv_len_ptr, "kv_len");
        // output
        Tensor output_q(dType, q_shape, q_out_ptr, "output_q");
        ...
        readCostBlockTable(GetGoldenDir() + "/block_table.bin", b, maxBlockNumPerBatch, blockTable);
        ...
        if (isQuant) {
            ...
            FUNCTION("MlaProlog_T", {x, wDq, wUqQr, w_qb_scale, wDkvKr, wUk, position_ids,
                                                                           cos, sin, past_key_states, kv_len, output_q}) {
                auto q_kv = attention.MlaPrologFoward(x, position_ids, cos, sin, kv_len, past_key_states, ropeTileConfig, isQuant);
                output_q = q_kv[0];
                past_key_states = q_kv[1];
...
TEST_F(MlaPrologOnBoardCostTest, test_MlaProlog_float16_32_128_1_4096_7168_1536_quant) {  // b_n_s_s2_h_q_lora_rank
    ...
    std::vector<int> params = {b, s, s2, n, h, qLoraRank, qkNopeHeadDim, qkRopeHeadDim,
                               kvLoraRank, vHeadDim};
    TestMlaProlog<npu::tile_fwk::float16>(params, GetGoldenDir(), true, 16111);
}
