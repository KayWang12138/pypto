...
void TestFrameworkInit(OpTestParam &testParam, HcomTestParam &hcomTestParam, int &physicalDeviceId)
{
    ...
    CHECK(ERROR_CODE_UNDEFINED, mpiInit != nullptr) << "MpiInitFunc ptr not found";
    auto mpiCommSize = GetFunction<MpiCommSizeFunc>("MPI_Comm_size");
    CHECK(ERROR_CODE_UNDEFINED, mpiCommSize != nullptr) << "MpiCommSizeFunc ptr not found";
    auto mpiCommRank = GetFunction<MpiCommRankFunc>("MPI_Comm_rank");
    CHECK(ERROR_CODE_UNDEFINED, mpiCommRank != nullptr) << "MpiCommRankFunc ptr not found";
    auto mpiBcast = GetFunction<MpiBcastFunc>("MPI_Bcast");
    CHECK(ERROR_CODE_UNDEFINED, mpiBcast != nullptr) << "MpiBcastFunc ptr not found";
    auto mpiBarrier = GetFunction<MpiBarrierFunc>("MPI_Barrier");
    CHECK(ERROR_CODE_UNDEFINED, mpiBarrier != nullptr) << "MpiBarrierFunc ptr not found";
    ...
    if (!dev_list_str.empty()) {
        ...
        CHECK(ERROR_CODE_UNDEFINED, testParam.rankId < static_cast<int>(device_list.size())) << "RankID out of range";
    ...
    // ACL、NPU初始化与绑定
    CHECK(ERROR_CODE_UNDEFINED, aclInit(NULL) == 0) << "aclInit falied";   // 设备资源初始化
    if (testParam.rankId == 0) {
        CHECK(ERROR_CODE_UNDEFINED, rtSetDevice(physicalDeviceId) == 0) << "Set device falied";   // 将当前进程绑定到指定的物理NPU
    }
    CHECK(ERROR_CODE_UNDEFINED, aclrtSetDevice(physicalDeviceId) == 0) << "Set device falied";   // 指定集合通信操作使用的设备

    // 在 rootRank 获取 rootInfo
    hcomTestParam.rootRank = 0;
    if (testParam.rankId == hcomTestParam.rootRank) {
        CHECK(ERROR_CODE_UNDEFINED, HcclGetRootInfo(&hcomTestParam.rootInfo) == 0) << "HcclGetRootInfo failed";
    }
    ...
    CHECK(ERROR_CODE_UNDEFINED, HcclCommInitRootInfo(testParam.rankSize, &hcomTestParam.rootInfo, testParam.rankId,
        &hcomTestParam.hcclComm) == 0) << "HcclCommInitRootInfo failed";

    // 获取 group name
    CHECK(ERROR_CODE_UNDEFINED, HcclGetCommName(hcomTestParam.hcclComm, testParam.group) == 0) << "HcclGetCommName failed";
...
void TestFrameworkDestroy(int32_t timeout)
{
    ...
    CHECK(ERROR_CODE_UNDEFINED, mpiAbort != nullptr) << "MpiAbortFunc ptr not found";
    std::future<void> finalizeTask = std::async(
        [] {
            auto mpiFinalize = GetFunction<MpiFinalizeFunc>("MPI_Finalize");
            CHECK(ERROR_CODE_UNDEFINED, mpiFinalize != nullptr) << "MpiFinalizeFunc ptr not found";
            mpiFinalize();
        }
    );
