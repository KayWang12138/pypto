...
void* GetLibHandle()
{
    ...
    static auto handle = []() -> void* {
        ...
        DISTRIBUTED_LOGE(DistributedErrorCode::NULLPTR, "Failed to load MPI library from common candidate paths/names");
...
auto GetFunction(const std::string& funcName) -> FuncType
{
    ...
    if (!handle) {
        DISTRIBUTED_LOGE(DistributedErrorCode::NULLPTR, "Failed to load MPI library");
        return nullptr;
    }

    auto func = dlsym(handle, funcName.c_str());
    if (!func) {
        DISTRIBUTED_LOGE(DistributedErrorCode::NULLPTR, "Failed to find function %s: %s", funcName.c_str(), dlerror());
        return nullptr;
    }
...
void TestFrameworkDestroy(int32_t timeout)
{
    ...
    if (finalizeTask.wait_for(std::chrono::seconds(timeout)) == std::future_status::timeout) {
        DISTRIBUTED_LOGE(DistributedErrorCode::NULLPTR, "MPI_Finalize timeout, forcing exit");
