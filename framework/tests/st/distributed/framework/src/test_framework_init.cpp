...
void TestFrameworkInit(OpTestParam &testParam, HcomTestParam &hcomTestParam, int &physicalDeviceId)
{
    ...
    if (!dev_list_str.empty()) {
        ...
        CHECK(testParam.rankId < static_cast<int>(device_list.size()));
    ...
    // ACL、NPU初始化与绑定
    CHECK(aclInit(NULL) == 0);   // 设备资源初始化
    if (testParam.rankId == 0) {
        CHECK(rtSetDevice(physicalDeviceId) == 0);   // 将当前进程绑定到指定的物理NPU
    }
    CHECK(aclrtSetDevice(physicalDeviceId) == 0);   // 指定集合通信操作使用的设备

    // 在 rootRank 获取 rootInfo
    hcomTestParam.rootRank = 0;
    if (testParam.rankId == hcomTestParam.rootRank) {
        CHECK(HcclGetRootInfo(&hcomTestParam.rootInfo) == 0);
    }
    ...
    // 获取 group name
    CHECK(HcclGetCommName(hcomTestParam.hcclComm, testParam.group) == 0);
