...
void TestMoeDistributedCombine(OpTestParam& testParam, const nlohmann::json& testData)
{
    ...
    auto [batchSize, hiddenSize, moeExpertNum, topK, dtype_num] = GetParams<paramsSize>(goldenDir + "/params.bin");
    ...
    std::vector<T> expandXPtr = ReadToVector<T>(
        dispatchPath + goldenDir + "/y_rank_" + std::to_string(testParam.rankId) + ".bin", inShape);
    std::vector<int32_t> assistInfoForCombinePtr = ReadToVector<int32_t>(
        dispatchPath + goldenDir + "/combine_info_rank_" + std::to_string(testParam.rankId) + ".bin", combineInfoShape);
    std::vector<int32_t> recvCountsPtr = ReadToVector<int32_t>(
        dispatchPath + goldenDir + "/recv_counts_rank_" + std::to_string(testParam.rankId) + ".bin", recvCountsShape);
    std::vector<float> expertScalesPtr = ReadToVector<float>(
        dispatchPath + goldenDir + "/scale_rank_" + std::to_string(testParam.rankId) + ".bin", scaleShape);
    ...
    if (batchSize == 256) { // bs=256 暂时不支持零误差一致
        ...
        EXPECT_TRUE(CompareWithGolden<uint8_t*>(dType, goldenDir + "/out_rank_", outEleNum, outPtr, testParam, 0));
    }
}
