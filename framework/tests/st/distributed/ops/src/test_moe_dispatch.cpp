...
void TestShmemMoeDispatch(OpTestParam &testParam, std::string &goldenDir) {
    ...
    int32_t expandXRowShape =
        std::min(static_cast<int32_t>(batchSize) * static_cast<int32_t>(topK) * testParam.rankSize,
    ...
    FUNCTION("MoeDispatch", {tokenTensor, tokenExpertTable}, {expandX, expertTokenNums, combineInfo, recvCounts}) {
        Distributed::MoeDistributedDispatchV2(tokenTensor, tokenExpertTable, testParam.group,
            static_cast<uint32_t>(testParam.rankSize), routedNum, 0, 0, expandX, combineInfo, expertTokenNums,
            recvCounts);
    }
    ...
    EXPECT_TRUE(CompareWithGolden<uint8_t *>(DataType::DT_INT32, goldenDir + "/valid_count_rank_", expertNumPerRank,
        expertTokenNumsOutPut->GetDevPtr(), testParam));
    auto combineInfoOutPut = ProgramData::GetInstance().GetOutputData(2);
    EXPECT_TRUE(CompareWithGolden<uint8_t *>(DataType::DT_INT32, goldenDir + "/combine_info_rank_", combineInfoEleNum,
        combineInfoOutPut->GetDevPtr(), testParam));
