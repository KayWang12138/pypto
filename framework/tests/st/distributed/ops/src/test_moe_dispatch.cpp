...
void TestShmemMoeDispatch(OpTestParam& testParam, std::string& goldenDir)
{
    ...
    EXPECT_TRUE(CompareWithGolden<uint8_t *>(DataType::DT_INT32, goldenDir + "/valid_count_rank_", expertTokenNumsEleNum, expertTokenNumsOutPut->GetDevPtr(), testParam));
...
template void TestShmemMoeDispatch<int32_t>(OpTestParam& testParam);
template void TestShmemMoeDispatch<float>(OpTestParam& testParam);
template void TestShmemMoeDispatch<float16>(OpTestParam& testParam);
template void TestShmemMoeDispatch<bfloat16>(OpTestParam& testParam);
