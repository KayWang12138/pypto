...
void TestShmemMoeDispatch(OpTestParam& testParam, std::string& goldenDir)
{
    ...
    // EXPECT_TRUE(CompareWithGolden<uint8_t *>(dType, goldenDir + "/y_rank_", expandXEleNum,
    // expandXOutPut->GetDevPtr(), testParam)); auto expertTokenNumsOutPut =
    // ProgramData::GetInstance().GetOutputData(1); EXPECT_TRUE(CompareWithGolden<uint8_t *>(DataType::DT_INT32,
    // goldenDir + "/valid_count_rank_", expertNumPerRank, expertTokenNumsOutPut->GetDevPtr(), testParam));
    ...
    // EXPECT_TRUE(CompareWithGolden<uint8_t *>(DataType::DT_INT32, goldenDir + "/recv_counts_rank_", 1,
    // recvCountsOutPut->GetDevPtr(), testParam));
}
