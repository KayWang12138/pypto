...
std::tuple<Tensor, Tensor, Tensor, Tensor> InitializeTestData(OpTestParam &testParam, const nlohmann::json& testData) {
...
void TestAllGatherAttentionPostReducescatter(OpTestParam &testParam, const nlohmann::json& testData) {
    ...
    auto [b, s, n, kvLoraRank, vHeadDim, h, typeNum] = GetParams<paramsSize>(goldenDir + "/params.bin");
    DataType dtype = GetDataTypeNum(typeNum);
    auto [agIn, wLora, wOut, out] = InitializeTestData(testParam, testData);
    ...
    EXPECT_TRUE(CompareWithGolden<uint8_t*>(goldenDir + dtype, "/rs_out_rank_", outRow * h, output->GetDevPtr(), testParam, 0.1f));
}
