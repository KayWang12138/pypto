...
void TestAllGather(OpTestParam& testParam, std::string& goldenDir)
{
    ...
    auto contextSize = DistributedContext::GetHostCommContextSize(hcclContexts);
    ...
    ProgramData::GetInstance().AppendInputs({
        ...
        RawTensorData::CreateTensor(DT_INT8, Shape{1, contextSize[0]}, (uint8_t *)(hcclOpParam))
    });
    ...
    if(testParam.worldRankId%2 == 0) {
        ...
        EXPECT_TRUE(CompareWithGolden<uint8_t*>(dType, goldenDir + "/output__odd_rank_", outSize, outPtr, testParam));
    }
