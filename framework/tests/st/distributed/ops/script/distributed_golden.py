...
def generate_combine_info_tensor(rank_id: int, token_id: int, k_offset: int, generate_combine: bool) -> torch.Tensor:
    ...
    combine_info[dispatch_info_row - 3] = rank_id
    combine_info[dispatch_info_row - 2] = token_id
    combine_info[dispatch_info_row - 1] = k_offset
...
def generate_moe_dispatch_case(case: MoeCase, save_dir: Path, combine_generate: bool) -> None:
    ...
    y_list = y_list, combine_info_list = combine_info_list, generate_combine = generate_combine)
