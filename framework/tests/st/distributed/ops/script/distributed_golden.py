...
def get_optimal_block_size(dtype: torch.dtype) -> int:
    ...
    if dtype == torch.float32:
        ...
        return 16  # 可根据实际硬件特性调整为32
    elif dtype == torch.int8:
        return 32
    elif dtype == torch.int32:
        return 16
    else:
        # 对于未明确指定的数据类型，提供一个合理的默认值
        return 16

# def calculate_nz_block_params(row: int, col: int, dtype: torch.dtype, block_size: int = None) -> tuple:
#     """
#     计算2D张量转换为NZ分形格式后的关键分块参数。

#     Args:
#         row col: 输入的2D张量。
#         block_size: 手动指定的分块大小。如果为None, 则根据张量数据类型自动选择。

#     Returns:
#         一个元组 (H1, B0, W1, B1)，分别表示：
#         - H1: 行方向的分块数量。
#         - B0: 行方向每个块的大小。
#         - W1: 列方向的分块数量。
#         - B1: 列方向每个块的大小。
...
#         block_size = get_optimal_block_size(dtype)

#     B0 = block_size
#     B1 = block_size

#     # 计算需要填充到的尺寸，使其成为分块大小的整数倍
#     M_pad = ((row + B0 - 1) // B0) * B0
#     N_pad = ((col + B1 - 1) // B1) * B1
...
def convert_nd_to_nz(tensor_nd: torch.Tensor, block_size: int = 16) -> torch.Tensor:
    """
    ...
    M, N = tensor_nd.shape
    B0, B1 = block_size, block_size  # 分块大小通常为16（FP16）或32（INT8）[1](@ref)
    
    M_pad = ((M + B0 - 1) // B0) * B0
    N_pad = ((N + B1 - 1) // B1) * B1
    padded_tensor = torch.zeros((M_pad, N_pad), dtype=tensor_nd.dtype)
    padded_tensor[:M, :N] = tensor_nd
    
    H1, W1 = M_pad // B0, N_pad // B1
    blocked_tensor = padded_tensor.reshape(H1, B0, W1, B1)
    
    transposed_tensor = blocked_tensor.permute(0, 2, 1, 3)
...
def save_nz_tensors(tensor_lists, save_dir, prefix='input'):
    """
    根据指定的格式处理张量(进行ND到NZ的转换)并保存到文件
    Args:
        inputs: 输入张量列表
        save_dir: 保存目录路径
        prefix: 输入张量文件名前缀(默认为'input')
    """
