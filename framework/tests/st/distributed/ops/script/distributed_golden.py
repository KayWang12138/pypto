...
def generate_golden_case(case_name: str, output: Path, index: int = None) -> bool:
    ...
    if index is None:
        ...
        if index >= len(all_test_configs):
            raise IndexError(f'case_index {index} out of range')
        test_config = all_test_configs[index]
        output = generate_output_path(output, test_config, index)
