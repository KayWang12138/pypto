...
def generate_random_tensor_list(
    shape: Tuple[int, ...], dtype: torch.dtype, rank_size: int, value_range: ValueRange
) -> List[torch.Tensor]:
    return [generate_random_tensor(shape, dtype, value_range) for rank in range(rank_size)]


def generate_random_tensor_list_and_save(
    shape: Tuple[int, ...], dtype: torch.dtype, rank_size: int, value_range: ValueRange, 
    save_dir: Path, filename_prefix: str,) -> List[torch.Tensor]:
    tensor_list = generate_random_tensor_list(shape, dtype, rank_size, value_range)
...
def generate_moe_dispatch_input_data(case: MoeCase, save_dir: Path) \
    ...
    x_list = generate_random_tensor_list_and_save(
        (case.batch_size, case.hidden_size), case.dtype, case.rank_size, case.value_range, save_dir, 'x',
    )

    scores_list = generate_random_tensor_list(
        (case.batch_size, case.routed_expert_num), torch.float32, case.rank_size, case.value_range
    )
...
def generate_all_gather_golden(case_name: str, output: Path, case_index: int = None) -> bool:
    def golden_func(config: dict):
        ...
        inputs = generate_random_tensor_list_and_save(
            case.shape, case.dtype, case.rank_size, case.value_range, output, 'input'
        )
...
def gen_allgather_attnpost_reducescatter_case(case_name: str, output: Path, case_index: int = None) -> bool:
    def golden_func(config: dict):
        ...
        all_gather_inputs = generate_random_tensor_list_and_save(
            all_gather_input_shape, case.dtype, case.rank_size, case.value_range, output, 'ag_in'
        )
