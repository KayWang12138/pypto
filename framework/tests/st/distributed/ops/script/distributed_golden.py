...
def generate_all_gather_golden(config: dict, output: Path) -> bool:
    ...
    all_gather_and_save(DistributedOpAndSaveArgs(inputs=inputs, world_size=case.world_size, shape=case.shape, valid_shape=case.valid_shape, save_dir=output, filename_prefix='output'))
...
def generate_all_reduce_golden(config: dict, output: Path) -> bool:
    ...
    inputs = generate_random_tensor_list_and_save(gen_tensor_case, output, 'input')
    all_reduce_and_save(DistributedOpAndSaveArgs(inputs=inputs, world_size=case.world_size, shape=case.shape, valid_shape=case.valid_shape, save_dir=output, filename_prefix='output'))
...
def generate_allreduce_add_allreduce_golden(config: dict, output: Path) -> bool:
    ...
    inputs = generate_random_tensor_list_and_save(gen_tensor_case, output, 'input')
    all_reduce_outs = all_reduce_and_save(DistributedOpAndSaveArgs(inputs=inputs, world_size=case.world_size, shape=case.shape, valid_shape=case.valid_shape, save_dir=output, filename_prefix='all_reduce_out'))
    add_outs = [all_reduce_outs[0] + all_reduce_outs[0] for _ in range(case.world_size)]
    save_tensor_list(add_outs, output, 'add_out')
    all_reduce_and_save(DistributedOpAndSaveArgs(inputs=add_outs, world_size=case.world_size, shape=case.shape, valid_shape=case.valid_shape, save_dir=output, filename_prefix='out'))
