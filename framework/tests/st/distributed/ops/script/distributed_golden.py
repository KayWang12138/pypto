...
def generate_output_path(output: Path, test_config: dict, case_index: int = None) ->Path:
    output_path_parts = list(output.parts)
    if case_index is None:
        output_path_parts[-1] = test_config['operation']
        output_path_parts.append(test_config['case_name'])
    else:
        output_path_parts[-2] = test_config['operation']
        output_path_parts[-1] = test_config['case_name']
    return Path(*output_path_parts)
...
def generate_golden_case(case_name: str, output: Path, case_index: int = None) -> bool:
    ...
    if case_index is None:
        for test_config in all_test_configs:
            output = generate_output_path(output, test_config)
