...
def get_case_file() -> List[Path]:
    case_file = os.environ.get('JSON_PATH')
    if case_file and Path(case_file).exists():
        return [Path(case_file)]
    # 默认目录
    case_path = Path(Path(__file__).parent.parent, 'daily_test_case').resolve()
    logging.info('loading all Json files form default dir: %s', case_path)
    if not case_path.exists() or not case_path.is_dir():
        raise ValueError(f'No test case directory found at {case_path}')
    files = list(case_path.glob("*.json"))
    if not files:
        raise ValueError(f'No Json file found in {case_path}')
    return files
...
def load_all_test_configs(case_files: list[Path]) -> List[dict]:
    ...
    for json_file in case_files:
        logging.info('Loading test cases form: %s', json_file)
...
def generate_single_golden(config: dict, output: Path):
    ...
    logging.info('Generate golden for op: %s (case: %s)', op_name, config['name_case'])
...
def generate_golden_case(case_name: str, output: Path, case_index: int = None) -> bool:
    print('golden case_name ', case_name)
    print('golden case_index ', case_index)
    print('golden output ', output)
    case_files = get_case_file()
    all_test_configs = load_all_test_configs(case_files)
    if case_index is None:
        for index, test_config in enumerate(all_test_configs):
            output_path1 = Path(output, str(index))
            output_path1.mkdir(parents=True, exist_ok=True)
            generate_single_golden(case_name, test_config, output, case_index)
    else:
        if case_index >= len(all_test_configs):
            raise IndexError(f'case_index {case_index} out of range')   
        output.mkdir(parents=True, exist_ok=True)
        generate_single_golden(all_test_configs[case_index], output)
