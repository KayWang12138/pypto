...
struct OpMetaData {
    explicit OpMetaData(const nlohmann::json &testData, 
        std::string&v fileName) : testData_(testData), fileName_(fileName) {}
...
std::vector<T> GetOpMetaData()
{
    ...
    if (jsonPath != nullptr) {
        ...
        // casePath = std::filesystem::path(TEST_CASE_RELATIVE_PATH);
    ...
    ALOG_ERROR_F("JSON path exists but is neither a regular file not a directory:%s", casePath.string().c_str());
