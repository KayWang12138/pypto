...
struct DisOpRegister {
    ...
    void RegisterOp(const std::string& opName, TFunc func)
    {
        ...
        {
            if (dtype == "int32") func.template operator()<int32_t>(testParam, testData);
            else if (dtype == "float16") func.template operator()<float16>(testParam, testData);
            else if (dtype == "bfloat16") func.template operator()<bfloat16>(testParam, testData);
            else if (dtype == "float32") func.template operator()<float>(testParam, testData);
            else FAIL() << "Unsupported dtype: " << dtype;
        };
    }

    void Run(const std::string &opName, OpTestParam &testParam, const std::string &dtype, const nlohmann::json& testData)
    {
