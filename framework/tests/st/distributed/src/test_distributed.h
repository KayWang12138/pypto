...
struct DisTemplateOpRegister {
    using opFunc = std::function<void(OpTestParam&, const std::string&)>;
    std::unordered_map <std::string, opFunc> disRegisterMap;
    
    static DisTemplateOpRegister& GetRegister()
    {
        static DisTemplateOpRegister disOpRegister;
...
std::vector<T> GetOpMetaDataFromDir(const std::filesystem::path& disPath)
{
    ...
    if (!std::filesystem::exists(dirPath) || !std::filesystem::is_directory(disPath)) {
    ...
    for (const auto& file : std::filesystem::directory_iterator(dirPath)) {
        ...
        allTestCases.insert(allTestCases.end(), testCases.begin(), testCase.end());
    }
}
...
std::vector<T> GetOpMetaData()
{
    ...
    if (jsonPath == nullptr) {
        std::filesystem::path jsonPath = std::filesystem::path(TEST_CASE_DIR);
    }
    if (!std::filesystem::exists(jsonPath)) {
        std::cerr << "JSON path does not exist: " << jsonPath << std::endl;
    }
    if (std::filesystem::is_regular_file(jsonPath)) {
        return GetOpMetaDataFromFile<T>(jsonPath);
    }
    if (std::filesystem::is_directory(jsonPath)) {
        return GetOpMetaDataFromDir<T>(jsonPath);
    }
    std::cerr << "Invalid path type: " << jsonPath << std::endl;
    return {};
}
