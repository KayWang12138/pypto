...
public:
    ...
    void SetUp() override
    {
        ...
        CHECK(ERROR_CODE_UNDEFINED, res) << "Failed to create directory: " << outputDir;
...
protected:
    ...
    {
        // 销毁集合通信域
        CHECK(ERROR_CODE_UNDEFINED, HcclCommDestroy(hcomTestParam.hcclComm) == 0) << "HcclCommDestroy failed";
        // 重置设备
        CHECK(ERROR_CODE_UNDEFINED, aclrtResetDevice(physicalDeviceId) == 0) << "aclResetDevice failed";
        // 设备去初始化
        CHECK(ERROR_CODE_UNDEFINED, aclFinalize() == 0) << "aclFinalize failed";
    }
