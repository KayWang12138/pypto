...
public:
    ...
    void RunDistributedTestGeneric(const nlohmann::json& testData)
    {
        ...
        DisOpRegister::GetRegister().Run(opName, testParam, dtype, testData);
    }
...
void GegisterOps()
{
    ...
    // 模板算子
    reg.RegisterOp("AllGather", []<typename T>(OpTestParam &testParam, const nlohmann::json& testData) {
        Distributed::TestDynAllGather<T>(testParam, testData);
    });
    reg.RegisterOp("ReduceScatter", []<typename T>(OpTestParam &testParam, const nlohmann::json& testData) {
        Distributed::TestShmemReduceScatter<T>(testParam, testData);
    });
    reg.RegisterOp("AllReduce", []<typename T>(OpTestParam &testParam, const nlohmann::json& testData) {
        Distributed::TestShmemAllReduce<T>(testParam, testData);
    });
    reg.RegisterOp("AllReduceAddAllReduce", []<typename T>(OpTestParam &testParam, const nlohmann::json& testData) {
        Distributed::TestShmemAllReduceAddAllReduce<T>(testParam, testData);
    });
    reg.RegisterOp("MoeDistributedCombine", []<typename T>(OpTestParam &testParam, const nlohmann::json& testData) {
        Distributed::TestMoeDistributedCombine<T>(testParam, testData);
    });

    // 非模板算子
    reg.disRegisterMap["MoeDispatch"] = [](OpTestParam &testParam, const std::string&, const nlohmann::json& testData) {
 	    Distributed::TestShmemMoeDispatch(testParam, testData);
    };
    reg.disRegisterMap["AllGatherAttnPostReduceScatter"] = [](OpTestParam &testParam, const std::string&, const nlohmann::json& testData) {
        Distributed::TestAllGatherAttentionPostReducescatter(testParam, testData);
    };
    // 后续按照上面格式增加算子
}
