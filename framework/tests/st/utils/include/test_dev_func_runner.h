...
struct MemoryHelper {
    ...
    ~MemoryHelper() {
        if (isTest_) {
            for(auto ptr : testAllocatePtrs_) {
                if (ptr) {
                    free(ptr);
                }
            }
            testAllocatePtrs_.clear();
        }
    }
    ...
    uint8_t *AllocDev(size_t size, uint8_t **cachedDevAddrHolder) {
        ...
        if (isTest_) {
            size_t ailgnSize = 512;
            auto uniquePtr = std::make_unique<uint8_t[]>(size + ailgnSize);
            uint8_t *rawPtr = uniquePtr.get();
            if (rawPtr == nullptr) {
                return nullptr;
            }

            memset_s(rawPtr, size + ailgnSize, 0, size + ailgnSize);
            devPtr = (uint8_t *)((((uint64_t)rawPtr) + ailgnSize - 1) / ailgnSize * ailgnSize);
            testAllocatePtrs_.push_back(rawPtr);
        } else {
    ...
    bool isTest_{true};
    std::vector<uint8_t *> testAllocatePtrs_;
};
...
private:
    ...
    void RunModel(const std::vector<RawTensorDataPtr> &inputs, const std::vector<RawTensorDataPtr> &outputs) {
        ...
        DeviceInitTilingData(memoryHelper, kArgs, function_->GetDyndevAttribute()->devProgBinary, nullptr, config_, nullptr);
    ...
    void RunOnBoard(const std::vector<RawTensorDataPtr> &inputs, const std::vector<RawTensorDataPtr> &outputs) {
        ...
        for (int i = 0; i < config_.repeatNum; i++) {
            InitKernelInOuts(memoryHelper, kArgs, inputs, outputs, false, function_->GetDyndevAttribute()->disableL2List);
