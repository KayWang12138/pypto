...
struct MemoryHelper {
    MemoryHelper(bool isTest) : isTest_(isTest) {
        testAllocatePtrs_ = std::make_shared<std::vector<std::shared_ptr<uint8_t>>>();
    }
    ...
    bool isTest_{true};
    std::shared_ptr<:vector<std::shared_ptr<uint8_t>>> testAllocatePtrs_;
};
