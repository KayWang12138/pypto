...
struct MemoryHelper {
    ...
    uint8_t *AllocDev(size_t size, uint8_t **cachedDevAddrHolder) {
        ...
        if (isTest_) {
            ...
            if (rawPtr == nullptr) {
                MACHINE_LOGE_E(HostLauncherErr::TEST_ALLOC_DEV_MALLOC_FAILED,
...
private:
    ...
    void DumpTensorContents(const DeviceKernelArgs &kArgs,
        ...
        MACHINE_LOGE_E(HostLauncherErr::TEST_DUMP_TENSOR_DEBUG,
        ...
        auto printIODevAddrs = [&](const std::vector<RawTensorDataPtr> &ptrs) {
            ...
            for (auto &ptr : ptrs) {
                ...
                MACHINE_LOGE_E(HostLauncherErr::TEST_DUMP_TENSOR_DEBUG, "[DumpTensor] devPtr %d = %lu\n", idx++,
                               devPtr);
        ...
        MACHINE_LOGE_E(HostLauncherErr::TEST_DUMP_TENSOR_DEBUG, "[DumpTensor] #inputs=%zu\n", inputs.size());
        printIODevAddrs(inputs);
        MACHINE_LOGE_E(HostLauncherErr::TEST_DUMP_TENSOR_DEBUG, "[DumpTensor] #outputs=%zu\n", outputs.size());
    ...
    void RunOnBoard(const std::vector<RawTensorDataPtr> &inputs, const std::vector<RawTensorDataPtr> &outputs) {
        ...
        if (rc != 0 && rc != ACL_ERROR_REPEAT_INITIALIZE) {
            MACHINE_LOGE_E(HostLauncherErr::TEST_ACL_INIT_FAILED, "Acl init failed!!!");
            return;
        }
        CheckDeviceId();
