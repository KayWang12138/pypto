...
private:
        ...
        const std::vector<RawTensorDataPtr>& outputs)
    {
        ...
        RuntimeMemcpy(&dumpTensorWsUsed, sizeof(uint64_t), dumpTensorWsPtr, sizeof(uint64_t), RtMemcpyKind::DEVICE_TO_HOST);
        MACHINE_LOGE(
            RtErr::RtMemcpyKind::FAILED, "[DumpTensor] dumpTensorWsPtr=%p, memory used=%lu\n", dumpTensorWsPtr,
            dumpTensorWsUsed);
