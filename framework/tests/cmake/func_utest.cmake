...
function(PTO_Fwk_UTest_AddCaseLib)
    ...
    # 为编译提速, LINK_LIBRARIES 指定的 Target 延迟到可执行程序编译阶段 Link;
    # 出于易用性角度考虑, 此处获取 LINK_LIBRARIES 指定的 Target 的 PUBLIC 的编译属性并设置到 CaseLib 中
    set(_LinkLibraries_PublicIncludeDirectories)
    set(_LinkLibraries_PublicCompileDefinitions)
    foreach (_LinkLib ${ARG_LINK_LIBRARIES})
        if (TARGET ${_LinkLib})
            get_target_property(_LinkLibIntfIncDirs ${_LinkLib} INTERFACE_INCLUDE_DIRECTORIES)
            if (_LinkLibIntfIncDirs)
                list(APPEND _LinkLibraries_PublicIncludeDirectories ${_LinkLibIntfIncDirs})
            endif ()
            get_target_property(_LinkLibIntfCompDef ${_LinkLib} INTERFACE_COMPILE_DEFINITIONS)
            if (_LinkLibIntfCompDef)
                list(APPEND _LinkLibraries_PublicCompileDefinitions ${_LinkLibIntfCompDef})
            endif ()
        endif ()
    endforeach ()
    ...
    target_include_directories(${ARG_TARGET} PRIVATE ${ARG_PRIVATE_INCLUDE_DIRECTORIES} ${_LinkLibraries_PublicIncludeDirectories})
    target_compile_definitions(${ARG_TARGET} PRIVATE ${_LinkLibraries_PublicCompileDefinitions})
...
function(PTO_Fwk_UTest_AddExe_RunExe)
    ...
    list(FILTER PTO_Fwk_UTestCaseLkLibrariesExt    EXCLUDE REGEX "PARALLEL_SEPARATOR")
