...
class Executable:
    ...
    def run(self, gtest_filter: Optional[str] = None, params: Optional[List[str]] = None, check: bool = False,
            envs: Optional[Dict[str, str]] = None) -> Tuple[subprocess.CompletedProcess, str, timedelta]:
        """执行可执行文件

        :param gtest_filter: GTestFilter
        :param params: Optional[List[str]] 额外配置的命令参数
        :param check: bool 透传至 subprocess.run 的 check 参数
        ...
        ret = subprocess.run(shlex.split(cmd), env=act_env, cwd=cwd, timeout=self.timeout,
                             capture_output=True, check=check, text=True, encoding='utf-8')
