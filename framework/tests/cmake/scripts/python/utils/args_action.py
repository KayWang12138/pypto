...
class ArgsGTestFilterListAction(argparse.Action):
    ...
    def parse_all_cases(binary: str) -> List[str]:
        ...
        print(f"[INFO] Found {len(cost_tests)} tests with cost info, {len(no_cost_tests)} tests without.")
        if cost_tests:
            print(f"[INFO] First few cost-aware tests: {cost_tests}")

        return cost_tests + no_cost_tests
