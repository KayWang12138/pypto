...
class ArgsGTestFilterListAction(argparse.Action):
    ...
    def get_test_costs(binary: str) -> Dict[str, float]:
        ...
        try:
            ...
            logger.warning(f"Timeout when getting cost info from {binary}")
        except Exception as e:
            logger.warning(f"Failed to get test cost info from {binary}: {e}")
    ...
    def parse_all_cases(binary: str) -> List[str]:
        ...
        try:
            ...
            logger.warning(f"[WARNING] Failed to get test list from {binary}: {e}")
            return []
        ...
        # 4. 日志输出
        logger.info(f"Found {len(cost_tests_sorted)} tests with cost info, {len(no_cost_tests)} tests without.")
        if cost_tests_sorted:
            logger.info(f"First few cost-aware tests (desc order): {cost_tests_sorted}")
