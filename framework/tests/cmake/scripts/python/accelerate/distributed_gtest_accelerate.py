...
class DistriutedGTestAccelerate(ABC):
    ...
    def _create_4card_distributed_params(self, args) -> List[ExecParam]:
        ...
        if hasattr(args, 'device') and args.device is not None:
            total_gpus = len(args.device)
            logging.info(f"从 args.device 检测到 {total_gpus} 个可用 GPU: {args.device}")
        else:
            # 回退到默认值
            total_gpus = 8
            logging.warning(f"args.device 未提供，使用默认 GPU 数量: {total_gpus}")
        gpus_per_group = 4
        num_groups = total_gpus // gpus_per_group
        
        if num_groups == 0:
            logging.error("Not enough GPUs for 4-card distributed execution")
            return []
        
        # 创建设备组
        device_groups = []
        for group_id in range(num_groups):
            start_gpu = group_id * gpus_per_group
            device_list = list(range(start_gpu, start_gpu + gpus_per_group))
    ...
    def _start_cntr_process_group(self) -> List[Process]:
        ...
        for param in self.exe_params:
            device_info = f"GPUs{param.custom['devices']}"
            process = Process(
    ...
    def _case(self, cntr_id: int, param: ExecParam, gtest_filter: str):
        ...
        try:
            ...
            if return_code == 0:
                logging.info("用例%s执行成功", gtest_filter)
                self.case_execution_queue.put(ctx.brief)
            else:
                logging.error("用例%s执行失败，返回码: %s", gtest_filter, return_code)
                self._case_exception_exit(cntr_id, command, return_code, out = "输出在", err = f"return: {return_code}")
                
        except subprocess.TimeoutExpired as e:
            self._report_case_error(cntr_id, str(e), 1, "执行超时")
        except Exception as e:
