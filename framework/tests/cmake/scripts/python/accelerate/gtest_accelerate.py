...
from typing import List, Any, Optional, Tuple, Dict, Callable, Sequence
...
class GTestAccelerate(ABC):
    ...
    class ExecResult:
        ...
        def get_case_exec_duration_info(self, case_dict: Dict[str, CaseDesc]) -> str:
            """获取 Case 执行耗时统计信息.
    ...
    def __init__(self, args, scene_mark: str, cntr_name: str):
        ...
        self.case_dict, self.case_list = self._init_get_case_init_info(args=args)
    ...
    def _init_get_case_init_info(self, args) -> Tuple[Dict[str, CaseDesc], List[CaseDesc]]:
        ...
        return self._init_get_case_desc_info(case_name_list=case_name_list)
    ...
    def _init_get_case_desc_info(self, case_name_list: List[str]) -> Tuple[Dict[str, CaseDesc], List[CaseDesc]]:
        """
        获取经排序的用例描述列表
        
        :param case_name_list: 用例名列表
        :type case_name_list: List[str]
        :return: 用例描述信息字典, 经排序的用例描述列表
        :rtype: Tuple[Dict[str, CaseDesc], List[CaseDesc]]
        """
        # 补充刷新预估耗时信息
        desc_dict = {name: CaseDesc(name=name) for name in case_name_list}
        ...
        for test_name, cost_str in matches:
            case_desc = desc_dict.get(test_name, None)
            if case_desc:
        ...
        normal_cnt = len(desc_list) - estimated_cnt
        logging.info("Determine TestCase Order, OrderdCase(%s), NormalCase(%s)", estimated_cnt, normal_cnt)
    ...
    def _post_case_exec_info(self) -> Tuple[str, bool]:
        ...
        duration_brief = self.exe_result.get_case_exec_duration_info(self.case_dict)
