...
class GTestAccelerate(ABC):
    ...
    def brief(self) -> List[Any]:
        ...
        if self.cpu_rank_size:
            lst += ["CpuRankSize", self.cpu_rank_size]
            lst += ["CpuAffinityPolicy", self.cpu_affinity_policy]
    ...
    def _init_cpu_rank_size(args) -> Optional[int]:
        ...
        if args.cpu_rank_size:
            ...
            cpu_rank_size_str = os.environ.get("PYPTO_TESTS_CPU_RANK_SIZE", None)
    ...
    def _prepare_determine_cpu_affinity_policy(self):
        ...
        logging.info("Determine CpuAffinity, Policy=%s, CntrNum=%s, CpuNum=%s, CpuRankSize=%s",
                     self.cpu_affinity_policy, self.cntr_num, cpu_count(), self.cpu_rank_size)
