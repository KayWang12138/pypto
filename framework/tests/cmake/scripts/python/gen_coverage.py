...
class GenCoverage:
    ...
    def get_file_stats(cls, file_cov_info: Dict[str, Dict[int, int]],
        ...
        for start_line, end_line in line_ranges:
            for line_number in range(start_line, end_line + 1):
                ...
                # 检查行是否被覆盖
                if line_number in file_cov_info["lines"]:
                    hit_count = file_cov_info["lines"][line_number]
    ...
    def gen_full_cov_info_file(self):
        ...
        """
        # 生成覆盖率原始统计文件
        cmd = f"lcov -c -d {self.data_dir} -o {self.full_cov_info_file} --ignore-errors negative"
                    ...
                    for start_line, end_line in line_ranges:
                        for line_number in range(start_line, end_line + 1):
                            if line_number in file_coverage["lines"]:
                                hit_count = file_coverage["lines"][line_number]
    ...
    def compress_incr_root(self):
        ...
        try:
            with zipfile.ZipFile(zip_file, 'w', zipfile.ZIP_DEFLATED) as zipf:
                for root, _, files in os.walk(self.incr_root):
                    for file in files:
                        file_path = Path(root, file)
                        arcname = f"{self.incr_root.name}/{file_path.relative_to(self.incr_root)}"
