...
class GenCoverage:
    ...
    class LCovAblity:
        ...
        def _check_param_support(self, exe: str, param: str) -> bool:
            """检查指定可执行文件是否支持指定参数
    ...
    @classmethod
    def _merage_line_ranges(cls, lines: List[int]) -> List[Tuple[int, int]]:
        lines = sorted(lines)
        if not lines:
            return []
        ranges = []
    ...
    def gen_full_cov_info_file(self):
        ...
        # 滤掉某些文件/路径的覆盖率信息
        if not self.lcov_ability.lcov_supported_exclude:
    ...
    def gen_cov_html_report(self, cov_file: Path, dest: Path, scene: str = "full", hierarchical: bool = True):
        ...
        if self.lcov_ability.lcov_supported_hierarchical and hierarchical:
            cmd += f" --hierarchical"
    ...
    def gen_inc_cov_text_report(self):
        ...
        if self.incr_cov_rst['files']:
            ...
            for file_path, file_stats in self.incr_cov_rst['files'].items():
                ...
                uncov_ranges = self._merage_line_ranges(lines=uncov_lines)
    ...
    def _filter_changes_line_ranges_by_ori_cov_data(self, rel_path_str: str, line_ranges: List[Tuple[int, int]]):
        ...
        # 合并连续的行号范围
        return self._merage_line_ranges(lines=new_lines)
