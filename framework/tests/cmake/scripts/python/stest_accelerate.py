...
class STestAccelerate(GTestAccelerate):
    ...
    def __init__(self, args):
        ...
        self.device_list: List[int] = self._get_init_device_list(args=args)
    ...
    @staticmethod
    def get_device_id_envs(p: Any) -> Optional[Dict[str, str]]:
        self = p
        return {"TILE_FWK_DEVICE_ID": f"{self.cntr_id}"}

    @staticmethod
    def _get_init_device_list(args) -> List[int]:
        device_list = [0]
    ...
    def _prepare_get_params(self) -> List[GTestAccelerate.ExecParam]:
        ...
        for _id in self.device_list:
            p = GTestAccelerate.ExecParam(cntr_id=_id, envs_func=STestAccelerate.get_device_id_envs)
            params.append(p)
        return params
