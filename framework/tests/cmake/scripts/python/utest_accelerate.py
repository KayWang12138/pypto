...
class UTestAccelerate(GTestAccelerate):
    ...
    def __init__(self, args):
        ...
        self.job_num: int = self._get_init_job_num(args=args)
    ...
    @staticmethod
    def get_case_exe_envs(p: Any) -> Optional[Dict[str, str]]:
        # 参数提取
        self = p
        cntr_id = self.cntr_id
        cpu_affinity_env_val = self.custom
        if not cpu_affinity_env_val:
            return None
        logging.debug("Cntr(%s) CpuAffinity(%s)", cntr_id, cpu_affinity_env_val)
        return {"PYPTO_TESTS_PROCESS_AFFINITY_LIST": cpu_affinity_env_val}

    @staticmethod
    def _get_init_job_num(args) -> int:
        if args.job_num:
    ...
    def _prepare_get_params(self) -> List[GTestAccelerate.ExecParam]:
        ...
        for cntr_id in range(self.job_num):
            params.append(GTestAccelerate.ExecParam(cntr_id=cntr_id,
                                                    envs_func=UTestAccelerate.get_case_exe_envs,
                                                    custom=None)
        return params
