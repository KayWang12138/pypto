...
def gen_scatter_element_data(s0, s1, d0, d1, axis, dtype, indices_dtype, output_dir: Path):
    shape_src = [s0, s1]
    shape_indices = [d0, d1]
    axis = axis
    shape_res = shape_src

    src_path = Path(output_dir, 'src.bin')
    ...
    src = torch.randn(s0, s1, dtype=dtype)
    src_np = src.numpy()
    src_np.tofile(src_path)
    indices = torch.randint(0, shape_src[axis], (d0, d1), dtype=indices_dtype)
    indices_np = indices.numpy()
    indices_np.tofile(indices_path)

    indices_int64 = indices.int().to(torch.int64)

    res = src.scatter(axis, indices_int64, 1.0)
    res_np = res.numpy()
...
def gen_scatter_element_op_date(case_name: str, output: Path) -> bool:
    ...
    src_path = Path(output, 'src.bin')
    indices_path = Path(output, 'indices.bin')
    res_path = Path(output, 'res_golden.bin')
    complete = src_path.exists() and indices_path.exists() and res_path.exists()
...
def main() -> bool:
    ...
    for cs in case_name_list:
        ...
        ret = gen_scatter_element_op_date(case_name=cs, output=output)
    return ret
