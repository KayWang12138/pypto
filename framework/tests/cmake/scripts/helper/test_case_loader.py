...
class TestCaseLoader:
    ...
    def run(self) -> list:
        all_test_cases = []
        cur_index = 0
        def process_file_to_json(file_path, json_path, cur_index):
        ...
        if os.path.isdir(self._file):
            files = sorted([f for f in os.listdir(self._file) if f.endswith(
                (".csv", ".xlsx", ".xls"))], key=lambda x : x.lower())
            for file in files:
                file_path = os.path.join(self._file, file)
                json_path = os.path.join(self._json_path, f"{os.path.splitext(os.path.basename(file_path))[0]}.json")
                test_cases = process_file_to_json(file_path, json_path, cur_index)
                all_test_cases.extend(test_cases)
                cur_index += len(test_cases)
        else:
            test_cases = process_file_to_json(self._file, self._json_path, cur_index)
            all_test_cases.extend(test_cases)
