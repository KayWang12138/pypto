...
def get_dtype_by_name(name: str, is_torch: bool = False, check: bool = True):
    ...
    if check and name == "fp8_e8m0fnu" and float8_e8m0fnu is None:
        raise TypeError("No module named 'ml_dtypes'.")
    if check and name == "fp8_e5m2" and float8_e5m2 is None:
        raise TypeError("No module named 'ml_dtypes'.")
    if check and name == "fp8_e4m3fn" and float8_e4m3fn is None:
        raise TypeError("No module named 'ml_dtypes'.")
    ...
    str_to_dtype = {
        ...
        "fp8_e8m0fnu": [float8_e8m0fnu, torch.float8_e8m0fnu],
        "fp8_e5m2": [float8_e5m2, torch.float8_e5m2],
        "fp8_e4m3fn": [float8_e4m3fn, torch.float8_e4m3fn],
    }
