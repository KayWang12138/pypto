...
#include "simulation_ca/PipeSimulator.h"

#include <algorithm>
#include <fstream>
#include <unordered_set>

#include "simulation/config/EnvConfig.h"
...
#include "simulation/arch/A2A3/PostSimulatorA2A3.h"
...
namespace CostModel
{
    ...
    static std::vector<std::string> CompileAndRunCode(const std::string &source, const EnvConfig &config)
    {
        ...
        std::string cmd = config.cPlusPlus + " -w -std=c++17 " + source + " -o " + executable + " -I " + projSrcPath +
                          "/include/tileop/a2a3 -I " + projSrcPath +
                          "/tests/cost_model/simulation_ca/mock"; //  + ">/dev/null 2>&1"
