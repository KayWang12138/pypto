...
TEST_F(InsertSyncTest, TestFindDep) {
    ...
    tensor1->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = 0;
    tensor1->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM100;
    auto tensor2 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape1);
    tensor2->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor2->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = 0;
    tensor2->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM200;
    auto tensor3 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape2);
    tensor3->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor3->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM300;
    tensor3->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM499;
    auto tensor4 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape2);
    tensor4->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor4->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM500;
    tensor4->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM699;
    auto tensor5 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape2);
    tensor5->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor5->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM700;
    tensor5->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM900;
    auto tensor6 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape2);
    tensor6->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor6->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM150;
    tensor6->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM200;
    ...
    for (size_t i = 0; i < opLogPtr.size(); i++) {
        ...
        } else if (opLogPtr[i]->GetOpcode() == Opcode::OP_COPY_OUT) {
    ...
    tensor7->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM1000;
    tensor7->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM1100;
...
TEST_F(InsertSyncTest, TestViewAssembleProcess) {
    auto rootFuncPtr = std::make_shared<Function>(Program::GetInstance(), "TestViewAssembleProcess", "TestViewAssembleProcess", nullptr);
    rootFuncPtr->rootFunc_ = rootFuncPtr.get();
    auto currFunctionPtr = std::make_shared<Function>(Program::GetInstance(), "TestViewAssembleProcessLeaf", "TestViewAssembleProcessLeaf", rootFuncPtr.get());
    EXPECT_TRUE(currFunctionPtr != nullptr);
    rootFuncPtr->rootFunc_->programs_.emplace(currFunctionPtr->GetFuncMagic(), currFunctionPtr.get());
    ...
    auto tensor6 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape2);
    auto tensor7 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape2);
    auto tensor8 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape3);
    auto tensor9 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape3);
    auto tensor10 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape3);
    std::vector<Operation *> opLogPtr;
    auto &view1 = currFunctionPtr->AddRawOperation(Opcode::OP_VIEW, {tensor1}, {tensor2});
    auto &view2 = currFunctionPtr->AddRawOperation(Opcode::OP_VIEW, {tensor1}, {tensor3});
    auto &view3 = currFunctionPtr->AddRawOperation(Opcode::OP_VIEW, {tensor1}, {tensor4});
    auto &view4 = currFunctionPtr->AddRawOperation(Opcode::OP_VIEW, {tensor1}, {tensor5});
    auto &add1 = currFunctionPtr->AddRawOperation(Opcode::OP_ADD, {tensor2, tensor3}, {tensor6});
    auto &add2 = currFunctionPtr->AddRawOperation(Opcode::OP_ADD, {tensor4, tensor5}, {tensor7});
    auto &assemble1 = currFunctionPtr->AddRawOperation(Opcode::OP_ASSEMBLE, {tensor6, tensor7}, {tensor8});
    auto &view5 = currFunctionPtr->AddRawOperation(Opcode::OP_VIEW, {tensor8}, {tensor9});
    auto &assemble2 = currFunctionPtr->AddRawOperation(Opcode::OP_ASSEMBLE, {tensor9}, {tensor10});
    ...
    PipeSync ps;
    std::vector<Operation *> resLogPtr;
    ps.ProcessViewAssembleOrder(opLogPtr, resLogPtr);
    auto itView2 = std::find(resLogPtr.begin(), resLogPtr.end(), &view2);
    auto itView4 = std::find(resLogPtr.begin(), resLogPtr.end(), &view4);
    auto itAdd1 = std::find(resLogPtr.begin(), resLogPtr.end(), &add1);
    auto itAdd2 = std::find(resLogPtr.begin(), resLogPtr.end(), &add2);
    auto itAssemble1 = std::find(resLogPtr.begin(), resLogPtr.end(), &assemble1);
    auto itView5 = std::find(resLogPtr.begin(), resLogPtr.end(), &view5);
    auto itAssemble2 = std::find(resLogPtr.begin(), resLogPtr.end(), &assemble2);
    EXPECT_EQ(itView2 < itAdd1, true);
...
TEST_F(InsertSyncTest, TestUpdateDep) {
    ...
    auto tensor2 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor2->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor2->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM200;
    tensor2->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM300;
    auto tensor3 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor3->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor3->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM200;
    tensor3->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM300;
    auto tensor4 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor4->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor4->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM200;
    tensor4->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM300;
    auto tensor5 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor5->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor5->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM500;
    tensor5->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM600;
    auto tensor6 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor6->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor6->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM500;
    tensor6->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM600;
    auto tensor7 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor7->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor7->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = 0;
    tensor7->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM100;
    auto tensor8 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor8->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor8->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM101;
    tensor8->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM199;
    std::vector<Operation *> opLogPtr;
...
TEST_F(InsertSyncTest, TestHandleEventID) {
    ...
    tensor1->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor1->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = 0;
    tensor1->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM99;
    auto tensor2 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor2->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor2->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM100;
    tensor2->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM199;
    auto tensor3 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor3->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor3->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM200;
    tensor3->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM300;
    ...
    tensor4->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor4->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM500;
    tensor4->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM600;
    auto tensor5 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor5->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor5->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM700;
    tensor5->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM800;
    std::vector<Operation *> opLogPtr;
...
TEST_F(InsertSyncTest, TestRelaxFakeDataDep) {
    ...
    auto tensor1 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor1->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor1->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM100;
    tensor1->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM109;
    auto tensor2 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor2->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor2->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = 0;
    tensor2->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM9;
    auto tensor3 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor3->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor3->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM10;
    tensor3->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM19;
    auto tensor4 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor4->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor4->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM20;
    tensor4->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM29;
    auto tensor5 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor5->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor5->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM30;
    tensor5->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM39;
    auto tensor6 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor6->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor6->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM40;
    tensor6->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM49;
    auto tensor7 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor7->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor7->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM50;
    tensor7->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM59;
    auto tensor8 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor8->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor8->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM60;
    tensor8->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM69;
    auto tensor9 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor9->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor9->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM70;
    tensor9->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM79;
    auto tensor10 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor10->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor10->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM80;
    tensor10->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM89;
    auto tensor11 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor11->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor11->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM90;
    tensor11->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM99;
    auto tensor12 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor12->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor12->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM200;
    tensor12->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM209;
    auto tensor13 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor13->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor13->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM210;
    tensor13->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM219;
    auto tensor14 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor14->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor14->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM220;
    tensor14->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM229;
    auto tensor15 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor15->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor15->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM230;
    tensor15->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM239;
    auto tensor16 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor16->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor16->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM240;
    tensor16->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM249;
    auto tensor17 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor17->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor17->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM250;
    tensor17->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM259;
    auto tensor18 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor18->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor18->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM260;
    tensor18->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM269;
    auto tensor19 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor19->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor19->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM270;
    tensor19->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM279;
    auto tensor20 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor20->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor20->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM280;
    tensor20->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM289;
    auto tensor21 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor21->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor21->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM290;
    tensor21->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM299;
    auto tensor22 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor22->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor22->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM110;
    tensor22->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM119;
    auto tensor23 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor23->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor23->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM120;
    tensor23->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM129;
    auto tensor24 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor24->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor24->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM130;
    tensor24->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM139;
    auto tensor25 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor25->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor25->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM140;
    tensor25->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM149;
    auto tensor26 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor26->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor26->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM150;
    tensor26->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM159;
    auto tensor27 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor27->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor27->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM160;
    tensor27->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM169;
    auto tensor28 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor28->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor28->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM170;
    tensor28->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM179;
    auto tensor29 = std::make_shared<LogicalTensor>(*currFunctionPtr, DT_FP32, shape);
    tensor29->SetMemoryTypeBoth(MemoryType::MEM_UB);
    tensor29->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].start = IS_NUM180;
    tensor29->memorymap[BLOCK_GRAPH_DEFAULT_COLOR].end = IS_NUM189;
    std::vector<Operation *> opLogPtr;
    auto &copyin1 = currFunctionPtr->AddRawOperation(Opcode::OP_COPY_IN, {tensor1}, {tensor2});
    opLogPtr.emplace_back(&copyin1);
