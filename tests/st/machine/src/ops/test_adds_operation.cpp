...
struct AddsOpFuncArgs : public OpFuncArgs {
    AddsOpFuncArgs(const Element &value, const std::vector<int> &viewShape, const std::vector<int> tileShape)
...
struct AddsOpMetaData {
    explicit AddsOpMetaData(const std::vector<OpFunc> &funcs) : opFuncs_(funcs) {}
...
static const AddsOpMetaData metaData =
    AddsOpMetaData({AddsOperationExeFuncDoubleCut, AddsOperationExeFuncTripleCut, AddsOperationExeFuncQuadrupleCut});

class AddsOperationTest : public npu::tile_fwk::stest::TestSuite_STest_Ops_Aihac_param<AddsOpMetaData> {};

INSTANTIATE_TEST_SUITE_P(TestAdds, AddsOperationTest, ::testing::Values(metaData));
...
TEST_P(AddsOperationTest, TestAdds) {
    ...
    auto args = AddsOpFuncArgs(value, GetViewShape(config), GetTileShape(config));
    testCase.args = &args;
