...
struct MulOpMetaData {
    explicit MulOpMetaData(const std::vector<OpFunc> &funcs) : opFuncs_(funcs) {}
...
static const MulOpMetaData metaData =
    MulOpMetaData({MulOperationExeFunc2Dims, MulOperationExeFunc3Dims, MulOperationExeFunc4Dims});

class MulOperationTest : public npu::tile_fwk::stest::TestSuite_STest_Ops_Aihac_param<MulOpMetaData> {};

INSTANTIATE_TEST_SUITE_P(TestMul, MulOperationTest, ::testing::Values(metaData));
...
TEST_P(MulOperationTest, TestMul) {
    ...
    auto args = MulOpFuncArgs(GetViewShape(config), GetTileShape(config));
    testCase.args = &args;
