...
static void NonSplitFunc(const Tensor &tensor_a, const Tensor &tensor_b, Tensor &tensor_c) {
    ...
    FUNCTION("testNoSplit", FunctionType::DYNAMIC, {tensor_a, tensor_b}, {tensor_c}) {
        LOOP("mLoop", FunctionType::DYNAMIC_LOOP, mIdx, LoopRange(1)) {
            Tensor dyn_a = DViewPad(tensor_a, aShape, aValidShape, {0, mIdx, 0});
...
static void MSplitFunc(
    ...
    FUNCTION("testMSplit", FunctionType::DYNAMIC, {tensor_a, tensor_b}, {tensor_c}) {
        LOOP("mLoop", FunctionType::DYNAMIC_LOOP, mIdx,
            ...
            Tensor dyn_a;
            if (transA) {
                dyn_a = DViewPad(tensor_a, {aShape[0], aShape[1], viewShape[1]},
                    {aShape[0], aShape[1], std::min(aShape[2] - viewShape[1] * mIdx, viewShape[1])}, {0, 0, mIdx * viewShape[1]});
            } else {
                dyn_a = DViewPad(tensor_a, {aShape[0], viewShape[1], aShape[2]},
...
static void NSplitFunc(
    ...
    FUNCTION("testNSplit", FunctionType::DYNAMIC, {tensor_a, tensor_b}, {tensor_c}) {
        LOOP("nLoop", FunctionType::DYNAMIC_LOOP, nIdx,
            ...
            Tensor dyn_a = DViewPad(tensor_a, aShape, aValidShape, {0, 0, 0});
            Tensor dyn_b;
            if (!transB) {
                dyn_b = DViewPad(tensor_b, {bShape[0], bShape[1], viewShape[2]},
                    {bShape[0], bShape[1], std::min(bShape[2] - viewShape[2] * nIdx, viewShape[2])}, {0, 0, nIdx * viewShape[2]});
            } else {
                dyn_b = DViewPad(tensor_b, {bShape[0], viewShape[2], bShape[2]},
