    ...
    PROGRAM("MlaProlog") {
        ...
        if (isQuant) {
            ...
            FUNCTION("MlaProlog_T", FunctionType::STATIC,
                ...
                Program::GetInstance().GetTileShape().SetVecTileShapes({2, 16, 1, qk_rope_head_dim});
                auto q0_1 = Reshape(q0, {b * s1 * nq, kv_lora_rank});
                auto q1_1 = Reshape(q1, {b * s1 * nq, qk_rope_head_dim});
                Program::GetInstance().GetTileShape().SetVecTileShapes({2, 2, 128, qk_rope_head_dim});
                ...
                if (usePost) {
                    ...
                    int N = nq;
                    int H = h;
                    Program::GetInstance().GetTileShape().SetVecTileShapes({4, 16, 1, kv_lora_rank});
                    Tensor atten_res0 = Transpose(input_i, {1, 2});
                    Program::GetInstance().GetTileShape().SetVecTileShapes({4, 1, 32, kv_lora_rank});
                    Tensor atten_res1 = Reshape(atten_res0, {B * S, N, kv_lora_rank});
                    Program::GetInstance().GetTileShape().SetVecTileShapes({4, 16, kv_lora_rank});
                    Tensor t2_res = Transpose(atten_res1, {0, 1});

                    Program::GetInstance().GetTileShape().SetCubeTileShapes({32, 32}, {std::min(256, kv_lora_rank), std::min(256, kv_lora_rank)}, {std::min(128, v_head_dim), std::min(128, v_head_dim)});  // M 16对齐
                    // [n,bs,kv_lora_rank] * [n, kv_lora_rank, v_head_dim] = [n,bs,v_head_dim]
                    Tensor bmm4_res = Matrix::BatchMatmul(dType, t2_res, w_uv_i);

                    Program::GetInstance().GetTileShape().SetVecTileShapes(32, 4, v_head_dim); // 必须切，但是尾轴不能切
                    Tensor t3_res = Transpose(bmm4_res, {0, 1}); // [bs,n,v_head_dim]

                    Program::GetInstance().GetTileShape().SetVecTileShapes({4, 32, v_head_dim});
                    Tensor r2_res = Reshape(t3_res, {B * S, N*v_head_dim});

                    // [b,s, n*v_head_dim] @ [n*v_head_dim, h] = [b,s,h]
                    Program::GetInstance().GetTileShape().SetCubeTileShapes({32, 32}, {std::min(256, N*v_head_dim), std::min(256, N*v_head_dim)}, {std::min(128, H), std::min(128, H)});
                    Tensor bmm5_res = Matrix::Matmul<false, false>(dType, r2_res, w_o_i);
