...
class OperationTestCaseRunner:
    ...
    def run_test_case(self, test_case_info):
        ...
        run_cmd = f"python3 build.py -s='{test_case}' -d={self.device} {clean_str} 2>&1 | tee {log_file}"
        print(f"Start exec : {run_cmd}")
    ...
    def run(self):
        ...
        for test_case_info in test_case_info_list:
            # run test
            self.run_test_case(test_case_info)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="The end index of test case, it will be reset to the max index when it is less than 0 or greater than the max.",
    )
    parser.add_argument(
        "-c",
        "--clean",
...
def run_test_case(args):
    if args.input_file is None:
        args.input_file = (
            f"{os.getcwd()}/tests/st/operation/test_case/{args.op}_st_test_cases.csv"
        )
