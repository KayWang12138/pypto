...
void TestAllGatherEx(OpTestParam &testParam)
{
    ...
    PROGRAM("ALL_GATHER_EX") {
        ...
        void *xPtr = readToDev(GetGoldenDir() + "/input_rank_"+ std::to_string(testParam.rankId) + ".bin", outByteSize /sizeof(float));
        ...
        ConfigManager::Instance();
        FUNCTION("AllGather_Ex", FunctionType::STATIC, tensorParams) {
            Program::GetInstance().GetTileShape().SetDistTileShapes(
                {M / 2, 2, 0}, 
                {N / 2, 2, 0}, 
                {1, testParam.rankSize, 0});
    ...
    EXPECT_TRUE(CompareWithGolden<std::vector<uint8_t *>>(dType, "/output_rank_", size * testParam.rankSize, outPtrs, testParam));
}
