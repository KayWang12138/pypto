...
TEST_F(ScatterElementOnBoardTest, test_scatter_element_float_16_64_8_32_1) {
    ...
    TileFwkRunAsync(handle, workspaceAddr, runtime::GetRA()->GetStream(),  opArgsRun);

    std::vector<float> golden(capacity2);
    std::vector<float> dev_res(capacity2);
    runtime::GetRA()->CopyFromTensor((uint8_t *)dev_res.data(), (uint8_t *)src_ptr, capacity2 * sizeof(float));
