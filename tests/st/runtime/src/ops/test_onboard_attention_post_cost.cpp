...
TEST_F(OnBoardCostTest, test_attention_post_bf16_real_quant_batch4_onlymm5) {
    ...
    std::vector<int> wUvScaleWShape = {N,1,vHeadDim};
    std::vector<int> wOShape = {N*vHeadDim,H};
    std::vector<int> wOScaleWShape = {H};
    ...
    runtime::GetRA()->CopyFromTensor((uint8_t *)res.data(), (uint8_t *)outPtr, outputByteSize);
...
TEST_F(OnBoardCostTest, test_attention_post_bf16_real_quant_n128_onlymm5K) {
    ...
    std::vector<int> wOScaleWShape = {H};
    std::vector<int> outputShapeT = {B, S, H};
    std::vector<int> mm5Int32ShapeT = {B * S, H};
    std::vector<int> t1Shape = {B, S, N, kvLoraRank};
    void *inputPtr = readToDev<T>(GetGoldenDir() + "/input.bin", inputSize);
    void *wUvPtr = readToDev<T>(GetGoldenDir() + "/w_uv.bin", wUvSize);
    void *wUvScaleWptr = readToDev<T_FLOAT>(GetGoldenDir() + "/w_uv_scale_w.bin", wUvScaleWSize);
    ...
    runtime::GetRA()->CopyFromTensor((uint8_t *)res.data(), (uint8_t *)mm5int32_ptr, mm5Int32ByteSize);
    ...
    runtime::GetRA()->CopyFromTensor((uint8_t *)res.data(), (uint8_t *)mm5fp32_ptr, mm5fp32ByteSize);
