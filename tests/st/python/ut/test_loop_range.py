...
def test_init_loop_range_end():
    ...
    loop = pto.loop_range_(scalar)

    loop_end = loop.end()

    assert loop_end.concrete_valid() == True
...
def test_init_loop_range_begin_end():
    ...
    loop = pto.loop_range_(sym_begin, sym_end)

    loop_begin = loop.begin()
    loop_end = loop.end()

    assert loop_begin.concrete_valid() == True
...
def test_init_loop_range_begin_end_step():
    ...
    loop = pto.loop_range_(sym_begin, sym_end, sym_step)
    ...
    assert loop_step.concrete_valid() == True
...
def test_init_loop_range_end_implicit_conversion_from_int():
    expected_val = 123
    loop = pto.loop_range_(expected_val)
