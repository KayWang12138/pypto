...
def process_single(swimlane_path, topo_path, verbose=True, plot_path=None):
    ...
    result = {
        ...
        "wait_sche_time": actual_time - ideal_time_real,
    ...
    if verbose:
        ...
        print(f"  Wait sche time: {result['wait_sche_time']}")
...
def main():
    ...
    if args[0] == "--dir":
        ...
        print("\n" + "=" * 100)
        print("汇总结果:")
        print(f"{'文件名':<40} {'Ideal Time':>12} {'Actual Time':>12} {'Wait Sche':>12} {'Speedup':>10} {'Overhead':>10} {'瓶颈核心':<12}")
        print("-" * 100)
        for r in results:
            if "error" in r:
                print(f"{r['file']:<40} {'ERROR':>12}  {r['error']}")
            else:
                print(f"{r['file']:<40} {r['ideal_time']:>12.4f} {r['actual_time']:>12.4f} {r['wait_sche_time']:>12.4f} {r['speedup']:>9.4f}x {r['overhead_ratio']:>9.2f}% {r['bottleneck_core']:<12}")
        print("=" * 100)
        ...
        print(f"  Wait sche time: {r['wait_sche_time']}")
        if r['actual_time'] > 0:
