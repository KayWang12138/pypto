...
Usage:
  python tools/extract_hash_order_mapping.py <log_file_or_dir> [<log_file_or_dir> ...]

For each input log file, this script writes an output file with suffix
`_HASH_ORDER`.
For each input directory, this script scans:
...
def collect_logs_from_dir(root: Path) -> list[Path]:
    # 1) Nested pattern: Pass_*_NBufferMerge/*.log, Pass_*_L1CopyInReuseMerge/*.log
    nested_patterns = [
        "Pass_*_NBufferMerge/*.log",
        "Pass_*_L1CopyInReuseMerge/*.log",
    ]
    ...
    # 2) Flat pattern: pypto-log*.log directly under root (if no nested matches)
    if not files:
        files.extend(root.glob("pypto-log*.log"))
