...
#按照out_h\out_w遍历，每次处理一个kernel_size
...
def avg_pool_2d(shape, kernel_size, stride=None, padding_mode=None, run_mode="npu", dynamic=True):
    ...
    out_h = (in_h + tPad + bPad - kH) // sH + 1
    out_w = (in_w + lPad + rPad - kW) // sW + 1
    ...
    @pypto.frontend.jit(runtime_options={"run_mode": mode, "stitch_function_num_initial": 128,
                        "stitch_function_outcast_memory": 1024, "stitch_function_inner_memory": 1024}, debug_options=dict(runtime_debug_mode=1,compile_debug_mode=1))
    def avg_pool_2d_kernel(
        ...
        pypto.set_vec_tile_shapes(16, 16, 16, 32)
        ...
        for bc_idx, unroll_length in pypto.loop_unroll(0, bc_total, 1, name="LOOP_BC", idx_name="bc_idx", unroll_list=[256, 128, 64, 32, 4, 2, 1]):      
            ...
            for oh in range(out_h):
                ...
                cur_kH = h_end_clamped - h_start_clamped
                ...
                for ow in range(out_w):
                    ...
                    cur_kW = w_end_clamped - w_start_clamped
                    if cur_kH > 0 and cur_kW > 0:
                        window = input_single_row[:, :, w_start_clamped:w_end_clamped]

                        pypto.set_vec_tile_shapes(unroll_length, 8, 16)
                        window_1d = pypto.reshape(window, [unroll_length, -1], inplace=True)  #reshape 改成inplace=True
                        # for _ in pypto.loop(1, name="reshape_0",idx_name="tmp_loop0"):
                        #     window_1d = pypto.reshape(window, [unroll_length, -1], inplace=True)  #reshape 改成inplace=True

                        pypto.set_vec_tile_shapes(unroll_length, 8*16)
                        sum_val = pypto.sum(window_1d, dim=1, keepdim=True)
                        
                        avg_val = sum_val / (kH * kW)
                        print("avg_val.shape",avg_val.shape)
                        pypto.set_vec_tile_shapes(unroll_length,1)
                        avg_val = pypto.reshape(avg_val, [unroll_length, 1, 1], inplace=True)  #reshape 改成inplace=True
                        # for _ in pypto.loop(1, name="reshape_1",idx_name="tmp_loop1"):
                        #     avg_val = pypto.reshape(avg_val, [unroll_length, 1, 1], inplace=True)  #reshape 改成inplace=True
...
def avg_pool_2d_golden(x, kernel_size, stride, padding_mode):
    ...
    out_h = (H + tPad + bPad - kH) // sH + 1
    out_w = (W + lPad + rPad - kW) // sW + 1
    
    x_padded = np.pad(x, ((0, 0), (0, 0), (tPad, bPad), (lPad, rPad)), mode='constant', constant_values=0)
