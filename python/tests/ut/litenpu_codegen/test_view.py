...
class TestLiteNPUViewDtype(unittest.TestCase):
    def test_view_dtype_fp32_001(self):
        device = "cpu"
        dtype = torch.float32
        shape = (2, 2)
    ...
    def test_view_dtype_fp32_002(self):
        device = "cpu"
        dtype = torch.float32
        shape = (2, 2)

        input_tensor = torch.rand(shape, dtype=dtype, device=device)
        out_tensor = torch.rand(shape, dtype=torch.float16, device=device)

        view_kernel_dtype_fp32_fp16(input_tensor, out_tensor)

        golden_out = pypto_view_in_torch(input_tensor, dtype=torch.float16)
        cos_value = compare_cos(np.array(out_tensor.cpu()), np.array(golden_out.cpu()))
        self.assertGreaterEqual(cos_value, 0.9999)

    def test_view_dtype_fp32_003(self):
        device = "cpu"
        dtype = torch.float32
        shape = (4,)
    ...
    def test_view_dtype_fp32_004(self):
        device = "cpu"
        dtype = torch.float32
        shape = (2, 4)
    ...
    def test_view_dtype_fp32_005(self):
        device = "cpu"
        dtype = torch.float32
        shape = (3, 3)
