...
def _full_custom_executor_test_tu(n_inputs: int, *, op_type: str, n_outputs: int = 1) -> str:
    ...
    # For compile-only layout tests, use a trivial calc_workspace that always
    # returns a small positive workspace size.
    # Arity is one shape argument: generated workspace glue always reads GetInputTensor(0),
    # so the mock context must have n_inputs >= 1 (see test parametrization).
    def _ws(shape0: tuple[int, int] = (1, 1)) -> int:
        return 16
...
def _custom_executor_workspace_test_main_cpp(
    ...
    # exercises calc_workspace. *n_inputs* must match the calc_workspace arity.
    out_checks = "\n".join(
        f"  if (a[{k}] != static_cast<int64_t>(reinterpret_cast<std::uintptr_t>("
        f"ctx.GetOutputTensor({k})->GetAddr()))) return {10 + k};"
        for k in range(n_outputs)
