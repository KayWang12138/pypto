...
"""Simplified frontend runtime example for plm.store(..., fp_tile=...) on A5 PTO."""
...
def store_fp_kernel(
    ...
    with pl.section_cube():
        ...
        pl.system.bar_all()
        plm.printf("STORE_FP raw_out[0:8,0:8]\\n")
        plm.dump_tensor(raw_out, offsets=[0, 0], shapes=[8, 8])
        plm.printf("STORE_FP fp_out[0:8,0:8]\\n")
        plm.dump_tensor(fp_out, offsets=[0, 0], shapes=[8, 8])
...
@fe.jit()
def test_store_fp():
    compiled_lib = fe.compile(store_fp_kernel, arch="a5", codegen_mode="pto")
    if compiled_lib is None:
        raise RuntimeError("compile failed for store_fp_kernel")
    ...
    print(q[:8, :8])
    print("***********k input / identity (top-left 8x8)***********")
    print(k[:8, :8])
    ...
    print(raw_ref[:8, :8])
    print("***********expected store_fp int8 output = clip(round(raw * 2.0), -128, 127)***********")
    print(expected_fp[:8, :8])
    print("***********store_fp int8 output (top-left 8x8)***********")
    print(fp_out[:8, :8])
    print("***********store_fp summary***********")
    print(
        {
            "fp_params_dtype": str(fp_params.dtype),
            "fp_out_dtype": str(fp_out.dtype),
            "scale_value": FP_SCALE_VALUE,
            "min": int(fp_out.min().item()),
            "max": int(fp_out.max().item()),
        }
    )
    ...
    print("raw store and store_fp outputs both match expectations.")
