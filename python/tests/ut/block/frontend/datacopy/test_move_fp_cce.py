...
"""Frontend runtime example for plm.move(..., fp_tile=...) on A5 CCE."""
...
def test_move_fp_cce():
    compiled_lib = fe.compile(move_fp_cce_kernel, arch="a5", codegen_mode="cce")
    if compiled_lib is None:
        raise RuntimeError("compile failed for move_fp_cce_kernel")
    ...
    print(decoded_fp)
    print("***********cce expected move_fp int8 output***********")
    print(expected_out[:8, :8])
    print("***********cce move_fp int8 output (top-left 8x8)***********")
    print(move_fp_out[:8, :8])
    print("***********cce move_fp summary***********")
    print(
        {
    ...
    print("cce move_fp output matches expectation.")
