...
def compute_qk(ctx, state, q, k, q_mat_buf, k_mat_buf, left_buf, right_buf, acc_buf1, acc_buf2, qk_vec):
    ...
    if ctx.ki == 0:
        # BNSD layout: [B, N, S, D]. layout="dn" loads Q as [D, S]
        # for the K x Q^T DN path.
        plm.load_tile(q_mat_buf[ctx.q_count % 2], q, [ctx.b_idx, ctx.n_idx, ctx.qi, 0], layout="dn")
    plm.load_tile(k_mat_buf[buf_idx], k, [ctx.b_idx, ctx.n_idx, ctx.ki, 0])
...
def compute_pv(ctx, state, v, pv_vec, p_mat_buf1, p_mat_buf2, v_mat_buf, left_buf, right_buf, acc_buf1, acc_buf2):
    ...
    plm.load_tile(v_mat_buf[buf_idx], v, [ctx.b_idx, ctx.n_idx, ctx.ki, 0])
    pl.system.sync_src(set_pipe=pl.PipeType.MTE2, wait_pipe=pl.PipeType.MTE1, event_id=0)
    pl.system.sync_dst(set_pipe=pl.PipeType.MTE2, wait_pipe=pl.PipeType.MTE1, event_id=0)
...
def compute_gu(ctx, o, pv_vec, running_o, exp_corr, global_sum_buf, o_f16):
    ...
    if ctx.ki == last_ki:
        ...
        pl.system.sync_src(set_pipe=pl.PipeType.V, wait_pipe=pl.PipeType.MTE3, event_id=0)
        pl.system.sync_dst(set_pipe=pl.PipeType.V, wait_pipe=pl.PipeType.MTE3, event_id=0)
        plm.store_tile(o, o_f16, [ctx.b_idx, ctx.n_idx, ctx.qi * 2 + ctx.sub_id, 0])
...
def test_fa_causal_bn_a5():
    compiled = fe.compile(fa_causal_bnsd_dn_kernel_v6, arch="a5", codegen_mode="cce", timeout=120)
    if compiled is None:
        raise RuntimeError("compile failed; check error.txt for fa_causal_bnsd_dn_kernel_v6")
    ...
    for b, sq, n, skv, d, num_cores in [
        ...
        print(f"\nFA-BNSD-causal-DN-A5 (b={b}, sq={sq}, n={n}, skv={skv}, d={d}) cores={num_cores}")
...
if __name__ == "__main__":
    print("FA BNSD causal DN on A5 CCE")
    print("=" * 60)
