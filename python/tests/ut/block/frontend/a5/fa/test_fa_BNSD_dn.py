...
def softmax_body(ctx, sq_dim, skv_dim, qk_vec, qk_vec1, tmp_vec, p_f16, reduce_dst_rm, global_max_rm_buf, global_sum_rm_buf,
    ...
    plm.store_tile(p_buf, p_f16_store, [ctx.b_idx, ctx.n_idx, ctx.qi * 2 + ctx.sub_id, ctx.ki])
...
def test_fa_bn_a5():
    compiled = fe.compile(fa_bnsd_dn_kernel, arch="a5", codegen_mode="cce", timeout=120)
    if compiled is None:
        raise RuntimeError("compile failed; check error.txt for fa_bnsd_dn_kernel")
    ...
    for b, sq, n, skv, d, num_cores in [
        ...
        print(f"\nFA-BNSD-DN-A5 (b={b}, sq={sq}, n={n}, skv={skv}, d={d}) cores={num_cores}")
