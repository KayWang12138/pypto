...
def alloc_cube_buffer():
    ...
    acc_buf1 = plm.make_tile(plm.TileType(shape=[TS, TKV], dtype=pl.FP32, target_memory=pl.MemorySpace.Acc, blayout=2, slayout=1, fractal=1024), addr=CA0, size=QK_HALF_F32)
    acc_buf2 = plm.make_tile(plm.TileType(shape=[TS, TKV], dtype=pl.FP32, target_memory=pl.MemorySpace.Acc, blayout=2, slayout=1, fractal=1024), addr=CA1, size=PV_HALF_F32)
...
def compute_qk(ctx, state, const_info):
    """QK = Q * K^T → ACC→Vec via dual_split_m. All tile accesses use static independent vars."""
    ...
    if ctx.ki == 0:
        if ctx.q_count % 2 == 0:
            plm.load(q_mat_0, q, [ctx.sq_off, 0])
        else:
            plm.load(q_mat_1, q, [ctx.sq_off, 0])
    if buf_idx == 0:
...
def compute_pv(ctx, state, const_info):
    """PV = P * V → ACC→Vec via dual_split_m. All tile accesses use static independent vars."""
    ...
    if q_idx == 0:
        if p_fifo_slot == 0:
            if ctx.ki == 0:
                ...
                plm.muls(gmax_rm_0, reduce_dst_rm, 1.0)
                plm.muls(tmp_vec, tmp_vec, SCALE)
                plm.exp(qk_vec, tmp_vec)
                pl.system.bar_v()
                plm.row_sum(reduce_dst, qk_vec, tmp_vec)
                pl.system.bar_v()
                plm.muls(gsum_rm_0, reduce_dst_rm, 1.0)
                plm.cast(p_f16, qk_vec, target_type=pl.FP16, mode="round")
            if ctx.ki > 0:
                plm.row_max(reduce_dst, qk_vec, tmp_vec)
                pl.system.bar_v()
                plm.maximum(reduce_dst_rm, reduce_dst_rm, gmax_rm_0)
                pl.system.bar_v()
                plm.sub(exp_corr_rm_0, gmax_rm_0, reduce_dst_rm)
                pl.system.bar_v()
                plm.muls(gmax_rm_0, reduce_dst_rm, 1.0)
                pl.system.bar_v()
                plm.row_expand_sub(tmp_vec, qk_vec, reduce_dst)
                plm.muls(exp_corr_rm_0, exp_corr_rm_0, SCALE)
                plm.muls(tmp_vec, tmp_vec, SCALE)
                plm.exp(exp_corr_rm_0, exp_corr_rm_0)
                plm.exp(qk_vec, tmp_vec)
                plm.cast(p_f16, qk_vec, target_type=pl.FP16, mode="round")
                pl.system.bar_v()
                plm.mul(gsum_rm_0, gsum_rm_0, exp_corr_rm_0)
                plm.row_sum(reduce_dst, qk_vec, tmp_vec)
                pl.system.bar_v()
                plm.add(gsum_rm_0, gsum_rm_0, reduce_dst_rm)
        elif p_fifo_slot == 1:
            if ctx.ki == 0:
            ...
            if ctx.ki > 0:
                ...
                plm.maximum(reduce_dst_rm, reduce_dst_rm, gmax_rm_0)
                pl.system.bar_v()
                plm.sub(exp_corr_rm_1, gmax_rm_0, reduce_dst_rm)
                pl.system.bar_v()
                plm.muls(gmax_rm_0, reduce_dst_rm, 1.0)
                pl.system.bar_v()
                plm.row_expand_sub(tmp_vec, qk_vec1, reduce_dst)
                plm.muls(exp_corr_rm_1, exp_corr_rm_1, SCALE)
                plm.muls(tmp_vec, tmp_vec, SCALE)
                plm.exp(exp_corr_rm_1, exp_corr_rm_1)
                plm.exp(qk_vec1, tmp_vec)
                plm.cast(p_f16, qk_vec1, target_type=pl.FP16, mode="round")
                pl.system.bar_v()
                plm.mul(gsum_rm_0, gsum_rm_0, exp_corr_rm_1)
                plm.row_sum(reduce_dst, qk_vec1, tmp_vec)
                pl.system.bar_v()
                plm.add(gsum_rm_0, gsum_rm_0, reduce_dst_rm)
    else:
        if p_fifo_slot == 0:
            if ctx.ki == 0:
                ...
                pl.system.bar_v()
                plm.row_expand_sub(tmp_vec, qk_vec, reduce_dst)
                plm.muls(gmax_rm_1, reduce_dst_rm, 1.0)
                plm.muls(tmp_vec, tmp_vec, SCALE)
                plm.exp(qk_vec, tmp_vec)
                pl.system.bar_v()
                plm.row_sum(reduce_dst, qk_vec, tmp_vec)
                pl.system.bar_v()
                plm.muls(gsum_rm_1, reduce_dst_rm, 1.0)
                plm.cast(p_f16, qk_vec, target_type=pl.FP16, mode="round")
            if ctx.ki > 0:
                plm.row_max(reduce_dst, qk_vec, tmp_vec)
                pl.system.bar_v()
                plm.maximum(reduce_dst_rm, reduce_dst_rm, gmax_rm_1)
                pl.system.bar_v()
                plm.sub(exp_corr_rm_0, gmax_rm_1, reduce_dst_rm)
                pl.system.bar_v()
                plm.muls(gmax_rm_1, reduce_dst_rm, 1.0)
                pl.system.bar_v()
                plm.row_expand_sub(tmp_vec, qk_vec, reduce_dst)
                ...
                plm.cast(p_f16, qk_vec, target_type=pl.FP16, mode="round")
                pl.system.bar_v()
                plm.mul(gsum_rm_1, gsum_rm_1, exp_corr_rm_0)
                plm.row_sum(reduce_dst, qk_vec, tmp_vec)
                pl.system.bar_v()
                plm.add(gsum_rm_1, gsum_rm_1, reduce_dst_rm)
        elif p_fifo_slot == 1:
            if ctx.ki == 0:
                plm.row_max(reduce_dst, qk_vec1, tmp_vec)
                pl.system.bar_v()
            ...
            if ctx.ki > 0:
                ...
                plm.maximum(reduce_dst_rm, reduce_dst_rm, gmax_rm_1)
                pl.system.bar_v()
                plm.sub(exp_corr_rm_1, gmax_rm_1, reduce_dst_rm)
                pl.system.bar_v()
                plm.muls(gmax_rm_1, reduce_dst_rm, 1.0)
                pl.system.bar_v()
                plm.row_expand_sub(tmp_vec, qk_vec1, reduce_dst)
                ...
                plm.cast(p_f16, qk_vec1, target_type=pl.FP16, mode="round")
                pl.system.bar_v()
                plm.mul(gsum_rm_1, gsum_rm_1, exp_corr_rm_1)
                plm.row_sum(reduce_dst, qk_vec1, tmp_vec)
                pl.system.bar_v()
                plm.add(gsum_rm_1, gsum_rm_1, reduce_dst_rm)
...
def compute_gu(ctx, row_off):
    ...
    if pv_slot == 0:
        ...
        if ctx.ki > 0:
            plm.row_expand_mul(running_o, running_o, exp_corr_0)
            plm.add(running_o, running_o, pv_vec)
    else:
        if ctx.ki == 0:
            plm.move(running_o, pv_vec1)
        if ctx.ki > 0:
            plm.row_expand_mul(running_o, running_o, exp_corr_1)
            plm.add(running_o, running_o, pv_vec1)
    if ctx.ki == ctx.skv_tiles - 1:
        if ctx.q_count % 2 == 0:
            plm.row_expand_div(running_o, running_o, gsum_0)
        else:
            plm.row_expand_div(running_o, running_o, gsum_1)
...
def fa_perf_tkv_preload_kernel(
    ...
    with pl.section_vector():
        ...
        exp_corr_rm_0 = exp_corr_rm_fifo[0]
