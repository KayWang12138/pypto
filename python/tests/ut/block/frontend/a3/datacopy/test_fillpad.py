...
"""Frontend runtime validation for fillpad-family manual ops on PTO."""
...
def _run_fillpad_case(test_name: str, kernel, output_shape: tuple[int, int], output_label: str) -> None:
    compiled_lib = fe.compile(kernel, arch="a3")
...
@fe.kernel
def fillpad_inplace_dynamic_kernel(
    x: pl.Tensor[[8, 8], pl.INT32],
    z: pl.Tensor[[8, 8], pl.INT32],
) -> pl.Tensor[[8, 8], pl.INT32]:
    src_type = plm.TileType(
...
def test_fillpad_dynamic():
    _print_case_header("test_fillpad_dynamic")
    _run_fillpad_case("fillpad_dynamic", fillpad_dynamic_kernel, (8, 8), "output")


@fe.jit()
def test_fillpad_inplace_dynamic():
    _print_case_header("test_fillpad_inplace_dynamic")
    _run_fillpad_case("fillpad_inplace_dynamic", fillpad_inplace_dynamic_kernel, (8, 8), "output")


@fe.jit()
def test_fillpad_expand_dynamic():
    _print_case_header("test_fillpad_expand_dynamic")
    _run_fillpad_case("fillpad_expand_dynamic", fillpad_expand_dynamic_kernel, (8, 16), "output")


if __name__ == "__main__":
    cases = [test_fillpad_dynamic, test_fillpad_inplace_dynamic, test_fillpad_expand_dynamic]
    for case in cases:
        case()
    print("\nAll tests passed!")
