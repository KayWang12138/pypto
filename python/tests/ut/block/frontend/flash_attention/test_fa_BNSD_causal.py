...
Features:
  ...
  4. Mask format: [Sq, Skv] FP16 tensor (0=mask out, 1=attend)
     - Converted to bit-packed predicate using cmps (TCMPS instruction)
     - cmps with NE mode: mask != 0 -> bit=1 (attend)

Tensor shapes:
  - Q/K/V/O: [B, N, Sq, D], qk_buf: [B, N, Sq, Skv], p_buf: [B, N, Sq, Skv]
  - attn_mask: [Sq, Skv] FP16 - attention mask (0=mask out, 1=attend)

Mask processing flow (following AscendC pattern):
  1. Load FP16 mask (each element is 0 or 1)
  2. cmps(mask_bit, mask_fp16, 0, NE) -> bit-packed mask
  3. sel(dst, mask_bit, src0, src1, tmp)
...
def fa_causal_kernel_bn(
    ...
    attn_mask: pl.Tensor[[Sq2, Skv2], pl.FP16],  # Causal mask in FP16 format (0 or 1)
    ...
    with pl.section_vector():
        ...
        for work_id in pl.range(work_start, work_end):
            ...
            n_idx = work_id % n_dim
            for qi in pl.range(0, sq_tiles):
                pl.system.bar_all()  # Sync subblocks before reusing shared addresses
                sq_off = qi * TS
                q_mat_idx = q_count % 2
                causal_kv_tiles = pl.min(qi + 1, skv_tiles)
                ...
                if qi == 0:
                    plm.load_tile(mask_fp16, attn_mask, [qi * 2 + sub_id, 0])
                    pl.system.sync_src(set_pipe=pl.PipeType.MTE2, wait_pipe=pl.PipeType.V, event_id=0)
                    pl.system.sync_dst(set_pipe=pl.PipeType.MTE2, wait_pipe=pl.PipeType.V, event_id=0)
                ...
                for j in pl.range(1, causal_kv_tiles):
                    ...
                    if j == qi:
                        plm.load_tile(mask_fp16, attn_mask, [qi * 2 + sub_id, j])
                        pl.system.sync_src(set_pipe=pl.PipeType.MTE2, wait_pipe=pl.PipeType.V, event_id=0)
                        pl.system.sync_dst(set_pipe=pl.PipeType.MTE2, wait_pipe=pl.PipeType.V, event_id=0)
...
def test_fa_causal_bn():
    ...
    for b, n, sq, skv, d, num_cores in [
        ...
        # Create causal mask in FP16 format (0 or 1)
        attn_mask = torch.zeros((sq, skv), device=device, dtype=torch.float16)
        for i in range(sq):
            for j in range(skv):
                if j <= i:  # lower triangular (including diagonal)
                    attn_mask[i, j] = 1.0

        print(f"\n=== attn_mask verification (FP16 format) ===")
        print(f"Row 0 (first 8): {attn_mask[0, :8].cpu().numpy()}")
        print(f"Row 1 (first 8): {attn_mask[1, :8].cpu().numpy()}")
        print(f"Row 128 (cols 126-134): {attn_mask[128, 126:134].cpu().numpy()}")
        ...
        o_ref = flash_attention_causal_ref_bn(q, k, v, d, debug=(b == 1 and n == 1))
        ...
        print(f"[NPU] QK_buf[0,0,:8,:8]:\n{qk_buf[0, 0, :8, :8]}")
        print(f"[Golden] QK_raw[:8,:8]:\n{qk_raw_g[:8, :8]}")
        qk_diff = (qk_buf[0,0,:8,:8] - qk_raw_g[:8,:8]).abs().max().item()
        ...
        print(f"[Golden] row_max[:8]: {qk_max_g[:8, 0]}")
        print(f"[Golden] row_sum[:8]: {qk_sum_g[:8, 0]}")
        print(f"[Golden] Attention[:8,:8]:\n{attn_g[:8, :8]}")

        print(f"\n[NPU] P_buf[0,0,:8,:8]:\n{p_buf[0, 0, :8, :8]}")
        p_diff = (p_buf[0,0,:8,:8].float() - attn_g[:8,:8]).abs().max().item()
        ...
        print(f"\n--- Step 3: O = P @ V ---")
        print(f"[NPU] O[0,0,:2,:8]:\n{o[0, 0, :2, :8]}")
        print(f"[Golden] O_ref[0,0,:2,:8]:\n{o_ref[0, 0, :2, :8]}")
        ...
        print(f"\n{'='*60}")
        print(f"Final Result: max|O - O_ref| = {diff:.6f}")
        print(f"{'='*60}")
