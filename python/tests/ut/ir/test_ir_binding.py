...
def test_dtype():
    ...
    for (dtype, name, bit_cnt, byte_cnt, is_fp) in dtypes:
        ...
        assert dtype.bit_cnt() == bit_cnt
        assert dtype.byte_cnt() == byte_cnt
...
def test_control_flow():
    ...
    # ir: if i then outputX = mul(outputX, scale1) else outputY = mul(outputY, scale2)
