...
def block_function_add(args):
    ...
    with block.function_scope(func):
        ...
        # 插入同步 MTE2_TO_V
        block.call_3_ret_void(pipeMTE2, pipeV, eventID0, voidValue, "set_flag")
        block.call_3_ret_void(pipeMTE2, pipeV, eventID0, voidValue, "wait_flag")

        # 计算Add
        block.add(ubt0, ubt1, ubt0)

        # 插入同步 V_TO_MTE3
        block.call_3_ret_void(pipeV, pipeMTE3, eventID0, voidValue, "set_flag")
        block.call_3_ret_void(pipeV, pipeMTE3, eventID0, voidValue, "wait_flag")
