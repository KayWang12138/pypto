...
# block function计算
def test_block_function_add(args):
    ...
    with block.function_scope(func):
        ...
        constant3 = block.const(16384, "const_2")
        constant32 = block.const(32, "const_32")
        constant512 = block.const(512, "const_32")
        idx = block.scalar(ir.DataType.int32, "idx")
        block.call_1(constant0, idx, "GET_COA")
        offset0 = block.scalar(ir.DataType.int32, "offset0")
        block.muls(idx, constant3, offset0)

        i = block.scalar(ir.DataType.int32, "i")
        pipeV = block.scalar(ir.DataType.int32, "PIPE_V")
        pipeMTE2 = block.scalar(ir.DataType.int32, "PIPE_MTE2")
        pipeMTE3 = block.scalar(ir.DataType.int32, "PIPE_MTE3")
        eventID0 = block.scalar(ir.DataType.int32, "EVENT_ID0")
        voidValue = block.scalar(ir.DataType.int32, "_")
        # 定义 & 获取GM地址
        pipeMTE2 = block.scalar(ir.DataType.int32, "PIPE_MTE2")
        # 定义 UB地址
        tile_shape = [constant32, constant512]
        ubt0 = block.tile(tile_shape, ir.DataType::FP32, "ubt0")
        ubt0.set_valid_shape(tile_shape) # 后续需要支持validshape不等于tile的场景
        ubt0.set_memory_param(0x10000, ir.MemSpaceKind::UB, 0x0)

        ubt1 = block.tile(tile_shape, ir.DataType::FP32, "ubt1")
        ...
        # 插入同步 MTE2_TO_V
        block.call_3(ir.Opcode::OP_SCALAR_CALL_3_RETVOID, pipeMTE2, pipeV, eventID0, voidValue, "set_flag")
        block.call_3(ir.Opcode::OP_SCALAR_CALL_3_RETVOID, pipeMTE2, pipeV, eventID0, voidValue, "wait_flag")

        # 计算Add
        block.create_binary_op(ir.Opcode::OP_ADD, ubt0, ubt1, ubt0)

        # 插入同步 V_TO_MTE3
        block.call_3(ir.Opcode::OP_SCALAR_CALL_3_RETVOID, pipeV, pipeMTE3, eventID0, voidValue, "set_flag")
        block.call_3(ir.Opcode::OP_SCALAR_CALL_3_RETVOID, pipeV, pipeMTE3, eventID0, voidValue, "wait_flag")

        # Copy到Gm
        block.create_ub_copy_out_op(Opcode::OP_UB_COPY_OUT, ubt0, {offset0, constant0}, args[2])
        block.create_return([constant0])
    return func
