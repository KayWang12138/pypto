...
def compile_stage(compile_stage):
    @pypto.jit(
        host_options={"compile_stage": compile_stage}
...
def test_all_compile_stages():
    compile_stage(pypto.CompileStage.ALL_COMPLETE)
    compile_stage(pypto.CompileStage.TENSOR_GRAPH)
    compile_stage(pypto.CompileStage.TILE_GRAPH)
    compile_stage(pypto.CompileStage.EXECUTION_GRAPH)
    compile_stage(pypto.CompileStage.CODEGEN_INSTRUCTION)
    compile_stage(pypto.CompileStage.CODEGEN_BINARY)
    assert True
