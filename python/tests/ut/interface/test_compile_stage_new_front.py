...
def compile_add(a, b, comp_stage):
    @pypto.frontend.jit(
        host_options={"compile_stage": comp_stage},
        runtime_options={"run_mode": 1}
    )
    def add_kernel(a: pypto.Tensor((4, 4), pypto.DT_FP32),
                b: pypto.Tensor((4, 4), pypto.DT_FP32)) -> pypto.Tensor((4, 4), pypto.DT_FP32):
...
def compile_stage(comp_stage):
    ...
    output_c = compile_add(a, b, comp_stage)
