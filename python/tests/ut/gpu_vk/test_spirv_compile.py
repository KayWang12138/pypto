...
def test_compile_matmul_op_produces_real_spirv():
    compiled = compile_op("matmul", torch.randn(8, 16), torch.randn(16, 4))
    assert compiled.supports_real_vulkan is True
    assert compiled.artifact.is_real_spirv is True
    assert compiled.artifact.spirv[0] == 0x07230203


def test_compile_batched_matmul_op_produces_real_spirv():
    compiled = compile_op("matmul", torch.randn(2, 8, 16), torch.randn(2, 16, 4))
    assert compiled.supports_real_vulkan is True
    assert compiled.artifact.is_real_spirv is True
    assert compiled.artifact.spirv[0] == 0x07230203


def test_compile_broadcast_batched_matmul_op_produces_real_spirv():
    compiled = compile_op("matmul", torch.randn(8, 16), torch.randn(2, 16, 4))
    assert compiled.supports_real_vulkan is True
    assert compiled.artifact.is_real_spirv is True
    assert compiled.artifact.spirv[0] == 0x07230203


def test_compile_vector_matmul_op_produces_real_spirv():
    compiled = compile_op("matmul", torch.randn(16), torch.randn(16))
    assert compiled.supports_real_vulkan is True
    assert compiled.artifact.is_real_spirv is True
    assert compiled.artifact.spirv[0] == 0x07230203


def test_compile_batched_vector_matmul_op_produces_real_spirv():
    compiled = compile_op("matmul", torch.randn(16), torch.randn(2, 16, 4))
    assert compiled.supports_real_vulkan is True
    assert compiled.artifact.is_real_spirv is True
    assert compiled.artifact.spirv[0] == 0x07230203
