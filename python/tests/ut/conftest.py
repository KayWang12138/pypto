...
def _get_test_time_cost(item):
    ...
    """
    # 检查函数是否有_time_cost属性
    if hasattr(item.function, '_time_cost'):
        return item.function._time_cost

    # 检查类是否有_time_cost属性
    if hasattr(item, 'cls') and item.cls and hasattr(item.cls, '_time_cost'):
        return item.cls._time_cost
