...
def test_tanh_like_expression():
    """Test tanh-like expression using - operator: (exp(x) - exp(-x)) / (exp(x) + exp(-x))"""
    device_id = int(os.environ.get('TILE_FWK_DEVICE_ID', 0))
    torch.npu.set_device(device_id)
...
def test_combined_arithmetic():
    """Test combined arithmetic using operators: y = 2.0 * x - z + (x / 3.0)"""
    device_id = int(os.environ.get('TILE_FWK_DEVICE_ID', 0))
    torch.npu.set_device(device_id)
...
def test_complex_expression():
    """Test complex expression: y = 100.0 - x ** 2.0 % 10.0"""
    device_id = int(os.environ.get('TILE_FWK_DEVICE_ID', 0))
    torch.npu.set_device(device_id)
...
if __name__ == "__main__":
    print("=" * 80)
    print("Running Tensor Operator Overload Tests")
    print("=" * 80)
