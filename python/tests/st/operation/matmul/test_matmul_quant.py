...
from matmul_quant_test_case import PERTENSOR_TESTS, PERCHANNEL_TESTS, QUANT_RELU_TESTS, MatmulQuantConfig
...
def fixpipe_mask_scale(scale_input):
    ...
    if isinstance(scale_input, torch.Tensor):
        ...
        scale_input_uint64 = np.uint64(masked_int)
        
        return scale_input_uint64, np.float32(golden_scale)
    ...
    for m_idx in pypto.loop(0, m_loop, 1, name="LOOP_L0_mIdx", idx_name="m_idx"):
        for n_idx in pypto.loop(0, n_loop, 1, name="LOOP_L0_nIdx", idx_name="n_idx"):
            ...
            out_view = pypto.matmul(
                ...
                extend_params={"scale": scale, "relu_type": relu_mode},
            )
    ...
    for m_idx in pypto.loop(0, m_loop, 1, name="LOOP_L0_mIdx", idx_name="m_idx"):
        for n_idx in pypto.loop(0, n_loop, 1, name="LOOP_L0_nIdx", idx_name="n_idx"):
            ...
            out_view = pypto.matmul(
                ...
                a_trans=config.a_trans,
                b_trans=config.b_trans,
                extend_params={"scale_tensor": scale_view, "relu_type": relu_mode},
            )
...
def run_matmul_quant_test(case: dict):
    ...
    if config.quant_type == 1:
        ...
        matmul_quant_pertensor_kernel(a_tensor, b_tensor, c_tensor, config, scale_input)
        ...
        matmul_quant_perchannel_kernel(a_tensor, b_tensor, scale_tensor, c_tensor, config)
...
@pytest.mark.parametrize("case", [
    ...
    for case in PERTENSOR_TESTS + PERCHANNEL_TESTS + QUANT_RELU_TESTS
])
