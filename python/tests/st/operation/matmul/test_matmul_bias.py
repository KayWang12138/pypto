...
from matmul_bias_test_case import BIAS_TESTS, BIAS_RELU_TESTS, MatmulBiasConfig
...
def matmul_bias_kernel(
    ...
    for m_idx in pypto.loop(0, m_loop, 1, name="LOOP_L0_mIdx", idx_name="m_idx"):
        for n_idx in pypto.loop(0, n_loop, 1, name="LOOP_L0_nIdx", idx_name="n_idx"):
            ...
            bias_view = bias_tensor[:, n_idx * n_view: n_idx * n_view + n_view]
            ...
            out_view = pypto.matmul(
                ...
                extend_params={"bias_tensor": bias_view, "relu_type": relu_mode},
            )
...
def run_matmul_bias_test(case: dict):
    device_id = int(os.environ.get("TILE_FWK_DEVICE_ID", 0))
    torch.npu.set_device(device_id)
    
    config = MatmulBiasConfig.from_test_case(case)
    ...
    a_dtype = MatmulBiasConfig.get_torch_dtype(case["a_dtype"])
    b_dtype = MatmulBiasConfig.get_torch_dtype(case["b_dtype"])
    c_dtype = MatmulBiasConfig.get_torch_dtype(case["c_dtype"])
    bias_dtype = MatmulBiasConfig.get_torch_dtype(case["bias_info"]["dtype"])
    ...
    if bias_dtype == torch.int32:
        bias_tensor_cpu = torch.randint(-100, 101, config.bias_shape, dtype=bias_dtype)
    else:
        bias_tensor_cpu = torch.rand(config.bias_shape, dtype=bias_dtype)
    
    a_cpu = a_tensor_cpu.T if config.a_trans else a_tensor_cpu
    b_cpu = b_tensor_cpu.T if config.b_trans else b_tensor_cpu
    
    accum_dtype = torch.int32 if a_dtype == torch.int8 else torch.float64
    bias_cpu = bias_tensor_cpu.to(accum_dtype)
    
    golden = torch.matmul(a_cpu.to(accum_dtype), b_cpu.to(accum_dtype)) + bias_cpu
    
    if config.relu_type == 1:
        golden = torch.relu(golden)
    
    golden = golden.to(c_dtype)
    
    a_tensor = a_tensor_cpu.to(f"npu:{device_id}")
    b_tensor = b_tensor_cpu.to(f"npu:{device_id}")
    bias_tensor = bias_tensor_cpu.to(f"npu:{device_id}")
    c_tensor = torch.zeros(c_shape, dtype=c_dtype, device=f"npu:{device_id}")
    
    matmul_bias_kernel(a_tensor, b_tensor, bias_tensor, c_tensor, config)
    
    atol, rtol = MatmulBiasConfig.get_tolerance(case["c_dtype"])
...
@pytest.mark.parametrize("case", [
    ...
    for case in BIAS_TESTS + BIAS_RELU_TESTS
])
def test_matmul_bias(case: dict):
    run_matmul_bias_test(case)


def run_matmul_bias_demo():
    ...
    def matmul_bias_demo_kernel(
        ...
        for m_idx in pypto.loop(0, m_loop, 1, name="LOOP_L0_mIdx", idx_name="m_idx"):
            for n_idx in pypto.loop(0, n_loop, 1, name="LOOP_L0_nIdx", idx_name="n_idx"):
                ...
                bias_view = bias[:, n_idx * n_view_size: n_idx * n_view_size + n_view_size]
                out_view = pypto.matmul(
                    a_view, 
                    b_view, 
                    pypto.DT_FP16,
                    extend_params={"bias_tensor": bias_view, "relu_type": pypto.ReLuType.NO_RELU}
                )
                out[
    ...
    bias = torch.randn([1, n_size], dtype=torch.float16, device="npu:0")
    out = torch.empty(m_size, n_size, dtype=torch.float16, device="npu:0")
    matmul_bias_demo_kernel(a, b, bias, out)


if __name__ == "__main__":
    run_matmul_bias_demo()