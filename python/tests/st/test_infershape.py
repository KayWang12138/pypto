...
def kernel_wrapper(shape):
    ...
    def kernel(x: pypto.Tensor(shape, pypto.DT_BF16),
        ...
        result = pypto.Tensor((shape[0], num), pypto.DT_BF16)
        ...
        for t_idx in pypto.loop(loop_t_times, name="t_loop", idx_name="t_idx"):
            ...
            rms_res = x_fp32
            pre = rms_res[:, :num] * (scale[0: 1].reshape([1, 1]).expand_clone([tile_t, 1])) + base[:, :num]  # (tile_t, num)
            ones = pypto.full(pre.shape, 1.0, pre.dtype, valid_shape=pre.shape)
            pre = pypto.div(ones, pre + 1.0)
            result = pypto.cast(pre, pypto.DT_BF16)
...
def test_main(t=16):
    ...
    shape = (t, 24)  # x tensor shape

    # Move tensors to NPU device - new frontend automatically converts PyTorch tensors
    x_npu = x.to(device=f'npu:{device_id}')
    scale_npu = scale.to(device=f'npu:{device_id}')
    base_npu = base.to(device=f'npu:{device_id}')


    # Call kernel with automatic PyTorch conversion
