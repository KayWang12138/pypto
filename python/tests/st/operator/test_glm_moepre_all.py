...
def select_experts_glm(in_tensors, out_tensors, renormalize_flag, topk_group, num_expert_group, row_ids_flag, eps):
    ...
    # 5. 定义动态函数
    with pypto.function("MOEGATE", [hidden_states, residual, weight, bias_input, mm_weight, e_score_bias_input],
...
def select_experts(residual: torch.Tensor,
    ...
    select_experts_glm(inputs, outputs, renormalize, topk_group, num_expert_group, row_ids_flag, input_norm_eps)
