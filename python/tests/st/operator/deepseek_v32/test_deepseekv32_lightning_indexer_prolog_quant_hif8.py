...
def lighting_indexer_prolog_quant_dyn(inputs: IndexerPrologQuantInput, outputs: IndexerPrologQuantOutput,
    ...
    pto_inputs = [pypto.from_torch(tensor, dynamic_axis=axis, dtype=dtype) for tensor, (axis, dtype) in input_tensors.items()]
    pto_outputs = [pypto.from_torch(tensor, dynamic_axis=axis, dtype=dtype) for tensor, (axis, dtype) in output_tensors.items()]
...
def do_test_lighting_indexer_prolog_quant(case_name, configs):
    ...
    inputs = IndexerPrologQuantInput(
        ...
        w_qb=inputs_data["w_idx_qb"],  # TODO ND->NZ
        w_qb_scale=inputs_data["w_idx_qb_scale"],
        wk=inputs_data["w_idx_k"],  # TODO ND->NZ
        w_proj=inputs_data["w_idx_proj"],  # TODO ND->NZ
