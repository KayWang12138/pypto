...
def gen_inputs(dims, dtype=torch.bfloat16, qunat_dtype=torch.float8_e4m3fn):
    ...
    w_idx_qb_scale = torch.empty((n * d, q_lora_rank//32), dtype=torch.float32).uniform_(0, 1).to(torch.float8_e8m0fnu)
...
def indexer_prolog(inputs_initial: dict, dims: dict, platform: str = "cpu"):
    ...
    w_idx_qb_scale_expand = w_idx_qb_scale.cpu().transpose(0, 1).repeat_interleave(32, dim=0)
    # w_idx_qb_scale_expand = w_idx_qb_scale.cpu().repeat_interleave(32, dim=0)
    ...
    q_matmul_mx = torch.matmul(q_norm_fp32 * q_norm_scale_expand_fp32, w_idx_qb_fp32 * w_idx_qb_scale_expand_fp32) # (b, s, n * d)
...
def ascend_operator_accuracy_standard_version_2_1(pypto_out, npu_out, golden_out, out_name):
    ...
    assert result != "FAILED", f"{out_name} FAILED: result={result}, mare={mare:.6f}, mere={mere:.6f}, rmse={rmse:.6f}, small_errors={small_err}"
    print(f"{out_name} PASSED - mare={mare:.6f}, mere={mere:.6f}, rmse={rmse:.6f}, small_errors={small_err}")
...
def do_test_lighting_indexer_prolog_quant(case_name, configs):
    ...
    inputs = IndexerPrologQuantInput(
        ...
        w_qb=inputs_data["w_idx_qb"].npu(),  # TODO ND->NZ
        w_qb_scale=inputs_data["w_idx_qb_scale"].npu().reshape(head_num * idx_head_dim, q_lora_rank // 64, 2),  # TODO ND->DN
        wk=inputs_data["w_idx_k"].npu(),  # TODO ND->NZ
        w_proj=inputs_data["w_idx_proj"].npu(),  # TODO ND->NZ
