...
def rope_3d(x: pypto.Tensor, cos: pypto.Tensor, sin: pypto.Tensor) -> pypto.Tensor:
    ...
    pypto.set_vec_tile_shapes(16, 32, 64)
    x_view = pypto.cast(x, pypto.DT_FP32)
...
def lightning_indexer_prolog_quant_compute(x_in, q_norm_in, q_norm_scale_in, w_qb_in,
                                           ...
                                           cos_idx_rope_in, sin_idx_rope_in, hadamard_q_in,
                                           hadamard_k_in, k_quant_in, k_scale_in, k_cache_index_in,
                                           q_quant_out, q_scale_out, k_quant_out, k_scale_out, weights_out):
    ...
    for t_idx, unroll_length in pypto.loop_unroll(0, t, 1, name="IndexerPrologQuantQuantLoop", idx_name="tIdx",
        ...
        q_s32 = pypto.matmul(q_norm, w_qb_in, pypto.DT_BF16)

        pypto.set_semantic_label("Query-Dequant")
        pypto.set_vec_tile_shapes(8, 32 * 128)
        ...
        q_hadamard_3d = pypto.reshape(q_hadamard, [t_tile, head_num, head_dim])

        pypto.set_semantic_label("Query-Quant")
        pypto.set_vec_tile_shapes(8, 32, 128)
        q_res = prolog_quant(q_hadamard_3d)
        pypto.assemble(q_res[0], [t_idx, 0, 0], q_quant_out)
        pypto.assemble(q_res[1], [t_idx, 0, 0], q_scale_out)

        pypto.set_semantic_label("Key-Linear")
        pypto.set_cube_tile_shapes([t_tile_max, t_tile_max], [512, 512], [64, 64])
        x = pypto.view(x_in, [t_tile, h], [t_idx, 0])
        k = pypto.matmul(x, wk_in, pypto.DT_FP32) 

        pypto.set_semantic_label("Key-RmsNorm")
        pypto.set_vec_tile_shapes(t_tile_max, 128)
        ...
        pypto.set_cube_tile_shapes([t_tile_max, t_tile_max], [128, 128], [128, 128])
        ...
        pypto.set_vec_tile_shapes(t_tile_max, 1, 1, 128)
        ...
        pypto.set_vec_tile_shapes(t_tile_max, 32)
