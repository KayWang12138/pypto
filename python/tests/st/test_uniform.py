...
# -----------------------------------------------------------------------------------------------------------
"""
Test Uniform operation on board
"""
import os
import math
import torch
...
def uniform_golden(key, counter0, counter1, shape, rounds, dtype=np.float32):
    def uint32(x):
        return x & 0xFFFFFFFF
...
def test_uniform_onboard():
    ...
    golden = uniform_golden(key, counter0, counter1, output_shape, rounds, np.float32)
...
def test_uniform_onboard_large():
    device_id = int(os.environ.get('TILE_FWK_DEVICE_ID', 0))
    torch.npu.set_device(device_id)
    output_shape = (256,)
    view_shape = (128,)
...
def test_uniform_onboard_fp16():
    ...
    with pypto.function("MAIN", output):
        for idx in pypto.loop(loop_num, name="loop0", idx_name="idx"):
            ...
            pypto.set_vec_tile_shapes(tile_shape[0])
            res = pypto.uniform(key, counter0, counter1, view_shape, rounds, pypto.DataType.DT_FP16)
            pypto.assemble(res, [offset], output)
    ...
    golden = uniform_golden(key, counter0, counter1, output_shape, rounds, np.float16)
...
def test_uniform_onboard_bf16():
    ...
    with pypto.function("MAIN", output):
        for idx in pypto.loop(loop_num, name="loop0", idx_name="idx"):
            ...
            pypto.set_vec_tile_shapes(tile_shape[0])
            res = pypto.uniform(key, counter0, counter1, view_shape, rounds, pypto.DataType.DT_BF16)
            pypto.assemble(res, [offset], output)
    ...
    golden = uniform_golden(key, counter0, counter1, output_shape, rounds, bfloat16)
