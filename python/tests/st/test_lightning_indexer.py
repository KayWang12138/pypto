        ...
        with block.for_scope(fs_inner_s1):
            ...
            with block.for_scope(fs_inner_s2):
                ...
                with block.if_then_scope(ifs8):
                    ...
                    # g_red_cnt == 1
                    with blcok.if_then_scope(ifs11):
                    ...
                    with block.if_then_scope(ifs14):
                        ...
                        sort_ub_32 = blcok.tile([sort_ub_len], ir.DataType.int32)
                    ...
                    with block.if_else_scope(ifs14):
                        ...
                        cur_s2_idx = blcok.scalar(ir.DataType.int32, "cur_s2_idx")
...
def lightning_indexer(query, key, weights, act_seq_len_query, act_seq_len_query, sparse_count, sparse_mode,
    ...
    with block.function_scope(func):
        ...
        batch_size = ir.Scalar(ir.DataType.uint32, query.shape[SHAPE_DIM0], "kHeadNum")
        s1 = ir.Scalar(ir.DataType.uint32, query.shape[SHAPE_DIM1], "s1")
        s2 = ir.Scalar(ir.DataType.int64, key.shape[SHAPE_DIM1], "s2")
        q_head_num = ir.Scalar(ir.DataType.uint32, query.shape[SHAPE_DIM2], "qHeadNum")
        k_head_num = ir.Scalar(ir.DataType.uint32, 1, "kHeadNum")
        g_size = ir.Scalar(ir.DataType.uint32, query.shape[SHAPE_DIM2], "gSize")

        i = block.scalar(ir.Data)
        if sparse_count == 3:
            atten_mask_flag = ir.Scalar(ir.DataType.bool, True, "attenMaskFlag")
        else:
            atten_mask_flag = ir.Scalar(ir.DataType.bool, False, "attenMaskFlag")

        if sparse_count > 2048:
            is_sparse_count_over_2k = ir.Scalar(ir.DataType.bool, True, "isSparseCountOver2k")
        else:
            is_sparse_count_over_2k = ir.Scalar(ir.DataType.bool, False, "isSparseCountOver2k")
        if s_saprse_count_over_2k:
            s1_single_size = ir.Scalar(ir.DataType.uint32, SPARSE_COUNT_8K // sparse_count * 2, "s1SingleSize")
        else:
            s1_single_size = ir.Scalar(ir.DataType.uint32, 8, "s1SingleSize")
        s2_single_size = ir.Scalar(ir.DataType.uint32, 2048, "s2SingleSize")
        m_single_size = ir.Scalar(ir.DataType.uint32, s1_single_size * g_size, "mSingleSize")

        for bn2_idx in block.loop(0, batch_size * k_head_num, 1, unroll = 4):
            # s1、s2实际大小
            b_idx = ir.Scalar(ir.DataType.uint32, bn2_idx // k_head_num, "bIdx")
            n2_idx = ir.Scalar(ir.DataType.uint32, bn2_idx % k_head_num, "n2Idx")
            act_s1_size = ir.Scalar(ir.DataType.uint32, act_seq_len_query[b_idx], "actS1Size")
            act_s2_size = ir.Scalar(ir.DataType.uint32, act_seq_len_key[b_idx], "actS2Size")
            if act_s1_size == 0 or act_s2_size ==0:
                continue
            gs1_split_num = ir.Scalar(ir.DataType.uint32, ceil_div(act_s1_size * g_size, m_base_size), "gs1SplitNum")

            outpuit_s1 = ir.Tensor([act_s1_size * g_size, act_s2_size * VALUE_AND_INDEX_NUM], ir.DataType.int32, "outputS1", ir.Format.ND)

            for gs1_idx in block.loop(0, gs1_split_num, 1, unroll = 4):
                s2_block_num = ir.Scalar(ir.DataType.uint32, None, "s2BlockNum")
                if atten_mask_flag:   
                ...
                output_s2 = ir.Tensor([m_base_size, act_s2_size * VALUE_AND_INDEX_NUM], ir.DataType.int32, "outputS2", ir.Format.ND)

                for s2_idx in pypto.loop(0, s2_block_num, 1, unroll = 4):
                    # cube
                    # 512 × 2048
                    ...
                    # aic:aiv = 1:2
                    for aiv_idx in pypto.loop(0, 2, 1, name = "INDEX_LOOP_AIV", idx_name = "aivIdx"):
                        # vector
                        # 256 × 4096
