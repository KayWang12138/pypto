...
from pypto import Tensor as PTensor, loop, floordiv, SymInt
from pypto.frontend import jit, dynamic


def floor_div_2d(view_shape: List[SymInt], tile_shape: List[int]):
    b, s = dynamic("b"), dynamic("s")

    @jit
    def floor_div_2d_impl(
        ...
        for i in loop(floordiv(b, view_shape[0])):
            for j in loop(floordiv(s, view_shape[1])):
...
def test_floor_div():
    ...
    out = floor_div_2d(view_shape, tile_shape)(x_pt.npu(), y_pt.npu())
    assert out.shape == (32, 128)
    golden = torch.floor_divide(x_pt, y_pt).to(torch.int32)
