...
def kernel_min_dispatch(
    ...
    # in jit, m n will be convert to int
    _assert_min_result(min(m, n), expect_symbolic=False, expected_value=10)
    # in jit, pypto.symbolic_scalar(8) is SymbolicScalar
    _assert_min_result(min(pypto.symbolic_scalar(8), 16), expect_symbolic=True)
