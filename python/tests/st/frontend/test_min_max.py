...
def _check_builtin_min_dispatch_cases() -> None:
    """Plain Python operands must keep using the builtin min()."""
    _assert_min_result(
...
def _check_static_dim_min_dispatch_cases(a) -> None:
    """Static dims resolve to plain Python values and stay on builtin min()."""
    _assert_min_result(
        min(a.shape[1], 16),
        expect_symbolic=False,
        expected_value=8,
...
def _check_dynamic_dim_min_dispatch_cases(a) -> None:
    """Dynamic dims should be intercepted and produce SymbolicScalar results."""
    _assert_min_result(
        min(a.shape[0], 16),
...
def _check_symbolic_min_dispatch_cases() -> None:
    """Explicit SymbolicScalar operands follow the symbolic dispatch rules."""
...
def kernel_min_dispatch(
    ...
    """
    _check_builtin_min_dispatch_cases()
    _check_static_dim_min_dispatch_cases(a)
    _check_dynamic_dim_min_dispatch_cases(a)
    _check_symbolic_min_dispatch_cases()
