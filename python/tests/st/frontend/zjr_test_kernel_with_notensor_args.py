...
def test_add_with_kwargs_run():
    ...
    def sub_add_kernel(
        ...
        c: pypto.Tensor([32, ...], pypto.DT_INT32),
        d: pypto.Tensor([pypto.STATIC, ...], pypto.DT_INT32),
        scalar=0,
    ...
    def add_kernel(
        ...
        return sub_add_kernel(a, b, c, d, scalar)
    ...
    r = add_kernel(a, b, c, d, scalar=1)
    assert r.shape == (32, 32)
    assert torch.allclose(r.cpu().float(), torch.ones(32, 32) * 4 + 1)


# =============================================================================
# @fucntion检验场景验证
# =============================================================================
def test_add_with_kwargs_check():
    @pypto.frontend.function
    def sub_add_kernel(
        a: pypto.Tensor(dtype=pypto.DT_INT32),
        b: pypto.Tensor([31, ...], pypto.DT_INT32),
        scalar=0,
    ) -> pypto.Tensor((32, 32), pypto.DT_INT32):
        return a + b + scalar
    ...
    @pypto.frontend.jit(runtime_options={"run_mode": pypto.RunMode.NPU})
    def add_kernel(
        a: pypto.Tensor([pypto.STATIC, ...], pypto.DT_INT32),
        b: pypto.Tensor([...], pypto.DT_INT32),
        scalar=0,
    ) -> pypto.Tensor((32, 32), pypto.DT_INT32):
        pypto.set_vec_tile_shapes(16, 16)
        return sub_add_kernel(a, b, scalar)
