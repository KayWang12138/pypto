...
def test_correct_annotation():
    ...
    kernel(torch.rand(shape, dtype=torch.float32, device=f'npu:{device_id}'), torch.rand(shape, dtype=torch.float32, device=f'npu:{device_id}'))
...
def test_incorrect_number_of_dimensions_annotation():
    ...
    try:
        kernel(torch.rand((16, 16, 16), dtype=torch.float32, device=f'npu:{device_id}'), torch.rand(shape, dtype=torch.float32, device=f'npu:{device_id}'))
...
def test_incorrect_shape_annotation():
    ...
    kernel = create_compute_func(shape, dtype)
    try:
        kernel(torch.rand((16, 18), dtype=torch.float32, device=f'npu:{device_id}'), torch.rand((16, 16), dtype=torch.float32, device=f'npu:{device_id}'))
    except ValueError as e:
...
def test_incorrect_dtype_annotation():
    ...
    kernel = create_compute_func(shape, dtype)
    try:
        kernel(torch.rand(shape, dtype=torch.float16, device=f'npu:{device_id}'), torch.rand(shape, dtype=torch.float32, device=f'npu:{device_id}'))
    except ValueError as e:
...
if __name__ == "__main__":
    logging.info("=" * 70)
    logging.info("Testing @pypto.frontend.jit decorator dtype annotation")
    logging.info("=" * 70)
