...
def create_bmm_kernel_with_mn_split(tile_config):
...
def create_ub2l1_kernel_with_no_split(tile_config, vector_tile_shape):
    ...
    def matmul_kernel_ub2l1(
        ...
        pypto.set_cube_tile_shapes(tile_config.m_tile_shape, tile_config.k_tile_shape, tile_config.n_tile_shape,
                                    enable_multi_data_load=tile_config.mdl_flag, enable_split_k=tile_config.gm_acc)
        mm2_c = pypto.matmul(a2_tensor, mm1_c, a_trans=tile_config.a_trans, b_trans=tile_config.b_trans,
                                out_dtype=tile_config.out_dtype)
        return mm2_c
...
def create_l0c2ub_kernel_with_no_split(tile_config, vector_tile_shape):
    ...
    def matmul_kernel_l0c2ub(
        ...
        mm1_c = pypto.matmul(a1_tensor, b1_tensor, a_trans=tile_config.a_trans, b_trans=tile_config.b_trans,
                                out_dtype=tile_config.out_dtype)
        pypto.set_vec_tile_shapes(vector_tile_shape[0], vector_tile_shape[1])
...
def test_bmm_with_mn_split():
    ...
    c1 = create_bmm_kernel_with_mn_split(tile_config)(a1_tensor.npu(), b1_tensor.npu())
    assert torch.allclose(c1.cpu().to(torch.float32), golden, atol=1e-3, rtol=1e-3)
...
def test_l0c2l1_fixpipe_with_no_split():
    ...
    m2 = n1
    k2 = 127
    a1_tensor = torch.randint(low=-2, high=2, size=[k1, m1], dtype=torch.int8)
    b1_tensor = torch.randint(low=-2, high=2, size=[n1, k1], dtype=torch.int8)
    ...
    tensor_res_mm1_asc = torch.matmul(a1_tensor.to(torch.float32).T, b1_tensor.to(torch.float32).T).to(torch.float32)
...
def test_l0c2ub_with_no_split():
    ...
    mm2_c = create_l0c2ub_kernel_with_no_split(tile_config, vector_tile_shape)(a1_tensor.npu(), b1_tensor.npu(),
                                                a2_tensor.npu())
...
def test_mm_with_gm_acc_mn_split_int8():
    ...
    a1_tensor = torch.randint(low=-5, high=5, size=[k, m], dtype=torch.int8)
    b1_tensor = torch.randint(low=-5, high=5, size=[n, k], dtype=torch.int8)
