...
def test_l0c2l1_fixpipe_with_no_split():
    ...
    m2 = n1
    k2 = 127
    a1_tensor = torch.randint(low=-5, high=5, size=[k1, m1], dtype=torch.int8)
    b1_tensor = torch.randint(low=-5, high=5, size=[n1, k1], dtype=torch.int8)
    ...
    scale_tensor = np.random.uniform(0, 3, [1, n1]).astype(np.float32)
