...
def test_mm_with_mn_split(dynamic=True):
    ...
    m_view = 128
    n_view = 256
    tile_config = ShapeConfig([m, k, n], [tile_m, tile_m], [tile_k, tile_k], [tile_n, tile_n], [m_view, n_view], FP16, FP16,
...
def test_bmm_with_mn_split(dynamic=False):
    ...
    golden = torch.matmul(a1.to(torch.float32), b1.to(torch.float32))
    c1 = create_bmm_kernel_with_mn_split(tile_config, dynamic)(a1.npu(), b1.npu())
    assert torch.allclose(c1.cpu().to(torch.float32), golden, atol=1e-3, rtol=1e-3), "结果精度不匹配"
...
def test_ub2l1_with_no_split():
    ...
    tile_config = ShapeConfig([m1, k1, n1], [tile_m1, tile_m1], [tile_k1, tile_k1], [tile_n1, tile_n1], [-1, -1], FP16, FP16,
                               False, False, False, False, False)
