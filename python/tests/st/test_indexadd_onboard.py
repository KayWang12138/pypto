...
def indexadd_2dim_build(inputs: List[pypto.Tensor], outputs: List[pypto.Tensor], args: IndexaAddParamInfo):
    ...
    with pypto.function("INDEXADD", inputs[0], inputs[1], inputs[2], outputs[0]):
        for b_idx in pypto.loop(b_loop_num, name="LOOP_B0", idx_name="b_idx"):
            for s_idx in pypto.loop(s_loop_num, name="LOOP_S0", idx_name="s_idx"):
                ...
                view_index = pypto.view(inputs[2], [view_shape[axis]], [offsets[axis]], valid_shape=[src_valid_shape[axis]])
...
def run_indexadd(inputs: List[torch.Tensor], outputs: List[torch.Tensor], args: IndexaAddParamInfo) -> None:
    ...
    pto_x1_tensor = pypto.from_torch(inputs_tensors[0], "x1_tensor")
    pto_x2_tensor = pypto.from_torch(inputs_tensors[1], "x2_tensor")
    pto_x3_tensor = pypto.from_torch(inputs_tensors[2], "x3_tensor")
    pto_res_tensor = pypto.from_torch(outputs_tensors[0], "res_tensor")
...
def test_indexadd__onboard():
    ...
    view_shape = [8, 16]
    tile_shape = [8, 32]
    args = IndexaAddParamInfo(0, alpha, view_shape, tile_shape)
    ...
    golden = inputs[0].indexadd(axis, inputs[2], inputs[1], alpha=alpha)
    pypto_out = outputs[0]
