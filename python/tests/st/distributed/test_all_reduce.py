...
def shmem_all_reduce(pred_token, in_tensor, out_tensor, ranksize, group):
    in_shape = in_tensor.shape
    pypto.set_dist_tile_shapes([in_shape[0], 1, 0], [in_shape[1], 1, 0], [1, WORLD_SIZE, 0])
    pypto.one_shot_shmem_all_reduce(pred_token, in_tensor, group, ranksize, out_tensor)
...
def worker(in_tensor, out_tensor_golden, rank):
    ...
    pred_token = pto_outputs
    ranksize = WORLD_SIZE
    shmem_all_reduce(pred_token, pto_inputs, pto_outputs, ranksize, groups[0])
