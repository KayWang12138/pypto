...
def test_swim():
    ...
    aicpu_json_path = pref_path + "/aicpu_dev_pref.json"
    ...
    with open(aicpu_json_path, 'r', encoding='utf-8') as f:
        ...
        for core in core_list:
            ...
            core_count = sum(1 for task in tasks if str(task).startswith("BEGIN"))
            assert len(tasks) > 0, f"{core} Could not Get aicpu perf"
            assert core_count == 3, f"{core} Multiple Turn Get aicpu perf not success"
    swim_lane_json_path = aicpu_json_path + "/merged_swimlane.json"
