...
def matmul_add(pto_inputs, pto_outputs, mm, tiling):
    a = pto_inputs[0]
    b = pto_inputs[1]
    c = pto_inputs[2]
    d = pto_outputs[0]
    ...
    for _ in pypto.loop(1, name="s0", idx_name="i"):
        a0 = pypto.view(a, [mm[0], mm[1]], [0, 0])
        b0 = pypto.view(b, [mm[1], mm[2]], [0, 0])
...
def test_device_run_data_from_device_mix_nodep():
    ...
    for idx in range(count):
        ...
        matmul_add(pto_inputs, pto_outputs[0], [m, k, n], tiling=tiling)
