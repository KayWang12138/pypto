...
def test_vector_operation_1d_index_put():
    ...
    output = pypto.tensor(shape, dtype, "INDEX_PUT_1D_TENSOR_output")

    with pypto.function("INDEX_PUT_1D", a, indices, values, output):
        pypto.set_vec_tile_shapes(tile_shape[0])
        result = pypto.index_put(a, indices, values)
        result.move(output)
    ...
    pypto.runtime._device_run_once_data_from_host(pto_a_tensor, pto_indices_tensor, pto_values_tensor, pto_output_tensor)
