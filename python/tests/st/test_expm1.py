...
def test_vector_operation_expm1():
    ...
    expected = torch.tensor(np.expm1(a_tensor.numpy()), dtype=torch.float32)
    assert_allclose(b_tensor.flatten(), expected.flatten(), rtol=1e-3, atol=1e-3)
    pypto.runtime._device_fini()


def test_vector_operation_expm1_fp16():
    device_id = int(os.environ.get('TILE_FWK_DEVICE_ID', 0))
    torch.npu.set_device(device_id)
    dtype = pypto.DT_FP16
    tiling = 32
    ...
    tile_shape = (8, 8)
    pypto.runtime._device_init()
    a = pypto.tensor(shape, dtype, "EXPM1_TENSOR_a_fp16")
    b = pypto.tensor(shape, dtype, "EXPM1_TENSOR_b_fp16")

    with pypto.function("EXPM1_FP16", a, b):
        for b_idx in pypto.loop(int(np.ceil(n / view_shape[0])), name="LOOP_EXPM1_L0", idx_name="b_idx"):
            for s_idx in pypto.loop(int(np.ceil(m / view_shape[1])), name="LOOP_EXPM1_L1", idx_name="s_idx"):
                tile_a = pypto.view(a, view_shape,
                                    [b_idx * view_shape[0], s_idx * view_shape[1]],
    ...
    a_tensor = (torch.rand(n, m, dtype=torch.float16) * 4 - 2)  # Range: [-2, 2] for expm1
    b_tensor = torch.zeros(n, m, dtype=torch.float16)

    pto_a_tensor = pypto.from_torch(a_tensor, "a_tensor")
    pto_b_tensor = pypto.from_torch(b_tensor, "b_tensor")
    pypto.runtime._device_run_once_data_from_host(pto_a_tensor, pto_b_tensor)

    expected = torch.tensor(np.expm1(a_tensor.numpy()), dtype=torch.float16)
    assert_allclose(b_tensor.flatten(), expected.flatten(), rtol=1e-2, atol=1e-2)
    pypto.runtime._device_fini()