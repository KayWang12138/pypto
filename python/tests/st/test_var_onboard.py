...
def var_2dim_tensor_proc(input_shape, dst_shape, dim, correction, keepdim, is_tensor_func):
    ...
    with pypto.function("MAIN", input_tensor, dst_tensor):
        for b_idx in pypto.loop(b_loop_num, name="b0", idx_name="bidx"):
            for s_idx in pypto.loop(s_loop_num, name="s0", idx_name="sidx"):
                ...
                if is_tensor_func:
                    dst_tensor.move(view_tensor_input.var(dim, correction=correction, keepdim=keepdim))
                else:
                    dst_tensor.move(pypto.var(view_tensor_input, dim, correction=correction, keepdim=keepdim))
    ...
    result = torch.var(input0_tensor, dim, correction=correction, keepdim=keepdim)
...
def test_var0_onboard():
    ...
    correction = 1
    keepdim = False
    var_2dim_tensor_proc([b, s], [1], dim, correction, keepdim, False)
...
def test_var1_onboard():
    ...
    correction = 1
    keepdim = False
    var_2dim_tensor_proc([b, s], [b], dim, correction, keepdim, False)
...
def test_var5_onboard():
    ...
    correction = 1
    keepdim = False
    var_2dim_tensor_proc([b, s], [8], dim, correction, keepdim, False)
...
def test_var6_onboard():
    ...
    correction = 1
    keepdim = False
    var_2dim_tensor_proc([b, s], [8], dim, correction, keepdim, True)
...
def test_var8_onboard():
    ...
    dim = [-2]
    correction = 1
    var_2dim_tensor_proc([b, s], [8], dim, correction)