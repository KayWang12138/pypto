...
def trans_nd_to_fractal_nz(data: torch.Tensor, keep_m_dim=False):
    ...
    if not keep_m_dim:
        data = F.pad(data, ((0, padding_n, 0, padding_m) + batch_padding), "constant")
        ...
        data = F.pad(data, ((0, padding_n, 0, 0) + batch_padding), "constant")
...
def create_bmm3d_kernel_with_mn_split(tile_config):
    ...
    n_view = tile_config.view_shape[0]
...
def test_bmm3d_with_mn_split():
    ...
    shape_info = ShapeConfig([b, m, k, n], [tile_m, tile_m], [tile_k, 2 * tile_k], [tile_n, tile_n], [m_view, n_view],
                                FP16, FP32, False, False, True, True, False, False)
...
def test_bmm3d_with_mn_split_unaligned_nz():
    b = 3
    m = 129
    k = 257
    n = 513
    ...
    a_tensor = trans_nd_to_fractal_nz(a_tensor, True) if shape_info.a_format_nz else a_tensor
    b_tensor = trans_nd_to_fractal_nz(b_tensor, True) if shape_info.b_format_nz else b_tensor
...
def test_bmm4d_with_mn_split():
    ...
    m_view = 64
    n_view = 128
    shape_info = ShapeConfig([b1, b2, m, k, n], [tile_m, tile_m], [tile_k, 2 * tile_k], [tile_n, tile_n], [m_view, n_view],
                                FP16, FP32, False, False, False, False, False, False)
...
def test_bmm4d_with_mn_split_unaligned_nz():
    b1 = 3
    b2 = 2
    m = 127
    k = 255
