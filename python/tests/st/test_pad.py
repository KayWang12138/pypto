...
def op_pad(params, a, b):
    ...
    n_out = n_in + pad_t + pad_b
    m_out = m_in + pad_l + pad_r
    
    for b_idx in pypto.loop(int(np.ceil(n_out / view_shape[0])), name="LOOP_PAD_L0", idx_name="b_idx"):
        for s_idx in pypto.loop(int(np.ceil(m_out / view_shape[1])), name="LOOP_PAD_L1", idx_name="s_idx"):
