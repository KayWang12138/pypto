...
def pypto_fused_ce_forward_v1_original(
    ...
    for i in pypto.loop(0, M, T_M, name="row_loop_{}".format(task_id), unroll_list=[16]):
        # 初始状态
        def token_body(i):
...
def pypto_fused_ce_backward_v1_original(
    ...
    # with pypto.function("MAIN_BACKWARD", [logits_backward, lse_backward, labels_backward, labels_idexes_backward], [dlogits_backward]):
...
def run_separate_benchmarks(total_tokens, vocab_size, tiling_options, device="npu:5", warmup=5, total_iter=10, version=0):
