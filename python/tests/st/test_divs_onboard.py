#!/usr/bin/env python3
# coding: utf-8
# Copyright (c) 2025 Huawei Technologies Co., Ltd.
# This file is a part of the CANN Open Software.
# Licensed under CANN Open Software License Agreement Version 1.0 (the "License").
# Please refer to the License for details. You may not use this file except in compliance with the License.
# THIS SOFTWARE IS PROVIDED ON AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED,
# INCLUDING BUT NOT LIMITED TO NON-INFRINGEMENT, MERCHANTABILITY, OR FITNESS FOR A PARTICULAR PURPOSE.
# See LICENSE in the root of the software repository for the full text of the License.
# ======================================================================================================================

import os
import math
import torch
import pto
import pytest
from numpy.testing import assert_allclose
import torch_npu


def test_divs_onboard():
    device_id = int(os.environ.get('TILE_FWK_DEVICE_ID', 0))
    torch.npu.set_device(device_id)
    shape = (72, 71)
    view_shape = (32, 32)
    tile_shape = (32, 32)
    pto.runtime._device_init()

    input1 = pto.tensor(shape, pto.DataType.DT_FP32, "PTO_TENSOR_input1")
    input2 = 1.5
    output = pto.tensor(shape, pto.DataType.DT_FP32, "PTO_TENSOR_output")

    b_loop_num = math.ceil(shape[0] / view_shape[0])
    s_loop_num = math.ceil(shape[1] / view_shape[1])
    pto.set_codegen_options(support_dynamic_unaligned=True)
    with pto.function("MAIN", [input1], [output]):
        for b_idx in pto.loop(b_loop_num, name="b0", idx_name="bidx"):
            for s_idx in pto.loop(s_loop_num, name="s0", idx_name="sidx"):
                view_tensor_a = pto.view(input1, view_shape,
                                         [b_idx * view_shape[0],
                                             s_idx * view_shape[1]],
                                         valid_shape=[
                                             pto.min(pto.symbolic_scalar(shape[0]) - b_idx * view_shape[0],
                                                     pto.symbolic_scalar(view_shape[0])),
                                             pto.min(pto.symbolic_scalar(shape[1]) - s_idx * view_shape[1],
                                                     pto.symbolic_scalar(view_shape[1])),
                                         ],
                                         )
                pto.set_vec_tile_shapes(tile_shape[0], tile_shape[1])
                view_tensor_a.move(pto.div(view_tensor_a, input2))
                pto.assemble(view_tensor_a, [
                             b_idx * view_shape[0], s_idx * view_shape[1]], output)
                del view_tensor_a
    assert isinstance(output, pto.tensor)

    a_tensor = torch.rand(shape[0], shape[1], dtype=torch.float32)
    b_tensor = torch.zeros(shape[0], shape[1], dtype=torch.float32)

    pto.runtime._device_run_once_data_from_host([a_tensor], [b_tensor])
    golden = torch.div(a_tensor, input2)
    assert_allclose(b_tensor.flatten(), golden.flatten(), rtol=3e-3, atol=3e-3)
    pto.runtime._device_fini()
