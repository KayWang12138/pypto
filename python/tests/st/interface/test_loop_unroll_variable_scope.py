...
def test_loop_unroll_single_block():
    """Test loop_unroll with single unroll factor (edge case).
    
    Even with a single unroll block, the bug would still occur because
    liveness analysis is static and doesn't know about unroll_list size.
    """
    device_id = int(os.environ.get('TILE_FWK_DEVICE_ID', 0))
    torch.npu.set_device(device_id)
    torch.manual_seed(42)
    ...
    def loop_unroll_single_kernel(
        ...
        # Use loop_unroll with single unroll factor
        for bs_idx, tile_batch in pypto.loop_unroll(
            0, bs, 1,
            name="LOOP_UNROLL_SINGLE",
            idx_name="bs_idx",
            unroll_list=[1]  # Single block
        ):
            tile_input = input_tensor[bs_idx:bs_idx + tile_batch, :]
            tile_bias = pypto.tensor([tile_batch, ne], bias_2d.dtype, "tile_bias")
            ...
            # Use bias_2d.dtype after inner loop
            tile_result = pypto.add(tile_input, tile_bias)
            output[bs_idx:bs_idx + tile_batch, :] = tile_result
    ...
    # Run kernel - should not raise NameError
    output = loop_unroll_single_kernel(input_tensor, bias_input)
