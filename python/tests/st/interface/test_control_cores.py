...
def count_core_types(base_dir="./output"):
    ...
    if not base_path.exists() or not base_path.is_dir():
        print("错误: 目录 '{}' 不存在".format(base_dir))
        return 0, 0

    subdirs = [d for d in base_path.iterdir() if d.is_dir()]
    if not subdirs:
        print("错误: 目录 '{}' 中没有子文件夹".format(base_dir))
        return 0, 0
    latest_dir = max(subdirs, key=lambda d: d.stat().st_mtime)

    # 读取json文件统计AIC和AIV使用的个数
    file_path = str(latest_dir) + "/tilefwk_L1_prof_data.json"
...
def test_rts_device_control_cores():
    device_id = int(os.environ.get('TILE_FWK_DEVICE_ID', 0))
    torch.npu.set_device(device_id)
    torch.npu.set_device_limit(device_id, 14, 30)
