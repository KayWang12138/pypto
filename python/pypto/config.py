...
class CubeTile:
    ...
    def __init__(self, m: List[int], k: List[int], n: List[int], enable_split_k: bool = False):
        ...
        if len(m) != 2:
            raise FrontendError(ValueError(f"m must have exactly 2 elements, got {len(m)}"))
        if len(n) != 2:
            raise FrontendError(ValueError(f"n must have exactly 2 elements, got {len(n)}"))
        if len(k) not in [2, 3]:
            raise FrontendError(ValueError(f"k must have 2 or 3 elements, got {len(k)}"))
