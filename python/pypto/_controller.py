...
class Controller:
    ...
    def begin_function(cls):
        if cls.in_function:
            raise FrontendError(RuntimeError("function nested is not allowed"))
...
def is_loop_begin(scalar: SymInt) -> SymbolicScalar:
    ...
    if not hasattr(scalar, "_loop_begin"):
        raise FrontendError(ValueError("not loop index"))
    # implementation
...
def function(name: str, *args) -> Iterator:
    ...
    *args: tuple
    ...
    try:
        ...
        if func is None:
            raise FrontendError(RuntimeError(f"function {name} recording failed"))
        try:
            func.EndFunction()
        except Exception as e:
...
def cond(scalar: SymInt, file: Optional[str] = None, lineno: Optional[int] = None):
    ...
    if (file is None) ^ (lineno is None):
        raise FrontendError(ValueError("file and lineno must be provided together or omitted"))
