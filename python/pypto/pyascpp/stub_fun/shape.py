...
def dviewpad(a: Tensor, new_shape: Union[Shape, list[Union[int, Var]]], \
             new_offset: Union[Shape, list[Union[int, Var]]], block_size: Union[Shape, list[Union[int, Var]]]):
    if context.active_module is None:
        raise Exception()
    result = context.active_module.create_tensor()
    context.active_module.add_inst(Instruction('dviewpad', [a, new_shape, new_offset, block_size], result))
    return result


def shapesize(a: Shape) -> Var:
    if context.active_module is None:
        raise Exception()
