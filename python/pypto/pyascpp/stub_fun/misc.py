...
def datatype(a: Tensor) -> Var:
    if context.active_module is None:
        raise Exception()
    result = context.active_module.create_var('DataType')
    context.active_module.add_inst(Instruction('get_datatype', [a], result))
    return result


def tensor2aggregationvec(a: TensorMap) -> AggregationVec:
    if context.active_module is None:
        raise Exception()
    result = context.active_module.create_aggregationvec()
    context.active_module.add_inst(Instruction('tensor2aggregationvec', [a], result))
    return result


def is_not_nullptr(a: Tensor) -> Var:
    if context.active_module is None:
        raise Exception()
    result = context.active_module.create_var('bool')
    context.active_module.add_inst(Instruction('is_not_nullptr', [a], result))
    return result
