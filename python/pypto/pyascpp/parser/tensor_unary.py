...
def tsrabs(h: CodeHelper, inst: Instruction):
...
def tround(h: CodeHelper, inst: Instruction):
    if not isinstance(inst.src[0], Tensor):
        raise Exception()
...
def logicalnot(h: CodeHelper, inst: Instruction):
    if not isinstance(inst.src[0], Tensor):
        raise Exception()
    if not isinstance(inst.dst, Tensor):
        raise Exception()
...
def rotatehalf(h: CodeHelper, inst: Instruction):
    if not isinstance(inst.src[0], Tensor):
        raise Exception()
    if not isinstance(inst.dst, Tensor):
        raise Exception()
