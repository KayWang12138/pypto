...
class Tensor():
    ...
    def __init__(self, shape: Optional[Union[list[Union[Var, int]], 'Shape']] = None,
        ...
        if shape is not None and dtype is not None:
            ...
            if context.active_module is not None:
                self.idx = context.active_module.args_counter
                context.active_module.args_counter += 1
                context.active_module.add_inst(Instruction('tsr_declare', [], self))
    ...
    def __iadd__(self, other: Union['Tensor', Var, int, float]):
        from ..stub_fun import iadd, iadds
        if isinstance(other, Tensor):
            res = iadd(self, other)
            return res
        elif isinstance(other, (Var, int, float)):
            res = iadds(self, other)
            return res
        else:
            raise TypeError(f'Not supported type for tensor add {type(other)}')
    ...
    def __isub__(self, other: Union['Tensor', Var, int, float]):
        from ..stub_fun import isub, isubs
        if isinstance(other, Tensor):
            res = isub(self, other)
            return res
        elif isinstance(other, (Var, int, float)):
            res = isubs(self, other)
            return res
        else:
            raise TypeError(f'Not supported type for tensor sub {type(other)}')
    ...
    def __imul__(self, other: Union['Tensor', Var, int, float]):
        from ..stub_fun import imul, imuls
        if isinstance(other, Tensor):
            res = imul(self, other)
            return res
        elif isinstance(other, (Var, int, float)):
            res = imuls(self, other)
            return res
        else:
            raise TypeError(f'Not supported type for tensor mul {type(other)}')
    ...
    def __itruediv__(self, other: Union['Tensor', Var, int, float]):
        from ..stub_fun import idiv, idivs
        if isinstance(other, Tensor):
            res = idiv(self, other)
            return res
        elif isinstance(other, (Var, int, float)):
            res = idivs(self, other)
            return res
        else:
            raise TypeError(f'Not supported type for tensor div {type(other)}')
    ...
    def __getitem__(self, indexes: Sequence[Optional[Union[Var, int, slice, EllipsisType]]]):
        ...
        from ..stub_fun import add_comment, reshape, less_than0_inv
        ...
        def update_shape(idx: Optional[Union[Var, int, slice, EllipsisType]], \
            ...
            if idx is None:
                ...
                offset.emplace_back(less_than0_inv(idx, self.shape[front_dim2]))  # type: ignore
                front_dim2 += 1
            elif isinstance(idx, slice):
                ...
                start = less_than0_inv(start, self.shape[front_dim2])  # type: ignore
                stop = less_than0_inv(stop, self.shape[front_dim2])  # type: ignore
                length = stop - start
    ...
    def dviewpad(self, new_shape: Union[Shape, list[Union[int, Var]]], \
                 new_offset: Union[Shape, list[Union[int, Var]]], block_size: Union[Shape, list[Union[int, Var]]]):
        from ..stub_fun import dviewpad
        res = dviewpad(self, new_shape, new_offset, block_size)
        return res

    def datatype(self):
        from ..stub_fun import datatype
        res = datatype(self)
        return res
    ...
    def eq(self, expr: 'Tensor'):
        if context.active_module is None:
            raise Exception()
        context.active_module.add_inst(Instruction('tsr_eq', [self, expr], None))
        return expr

    def rowmaxsingle(self, axis=None):
        from ..stub_fun import rowmaxsingle
        res = rowmaxsingle(self, axis)
        return res

    def rowsumsingle(self, axis=None):
        from ..stub_fun import rowsumsingle
        res = rowsumsingle(self, axis)
        return res
