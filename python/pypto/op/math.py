...
def random(
    ...
    counter : List[int]
        A list of 4 int64 values that serve as the 128-bit counter, determining
    ...
    counter = [0, 0, 0, 0]
    shape = [1024]
    output = pypto.random(key, counter, shape, pypto.DT_UINT32, rounds=10)
    """
    if len(counter) != 4:
        raise ValueError(f"counter must have 4 elements, got {len(counter)}")
    if rounds not in [7, 10]:
        raise ValueError(f"rounds must be 7 or 10, got {rounds}")
    
    return pypto_impl.Random(key, counter, shape, dtype, rounds)
