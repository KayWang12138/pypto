...
def remainder(input: Union[Tensor, int, float], other: Union[Tensor, int, float]) -> Tensor:
    ...
    if isinstance(input, pypto_impl.Tensor):
        ...
        if isinstance(other, float):
            return pypto_impl.Remainder(input, pypto_impl.Element(pypto.DT_FP32, other))
        if isinstance(other, int):
            return pypto_impl.Remainder(input, pypto_impl.Element(pypto.DT_INT32, other))
    if isinstance(other, pypto_impl.Tensor):
        if isinstance(input, int):
            return pypto_impl.Remainder(pypto_impl.Element(pypto.DT_INT32, input), other)
        if isinstance(input, float):
            return pypto_impl.Remainder(pypto_impl.Element(pypto.DT_FP32, input), other)
