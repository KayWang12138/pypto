...
from pypto.error import PyptoError, _error_mapping
    ...
    if isinstance(other, pypto_impl.Tensor):
        ...
        if alpha == 1 or alpha == 1.0:
            ...
            if not isinstance(other, (int, float)):
                raise PyptoError(TypeError(f"alpha must be int or float, but got {type(other)}."), err_code=_error_mapping[TypeError])
...
def remainder(input: Union[Tensor, int, float], other: Union[Tensor, int, float]) -> Tensor:
    ...
    raise PyptoError(TypeError(f"Unsupported operand types for remainder: {type(input)} and {type(other)}"), err_code=_error_mapping[TypeError])
...
def bitwise_and(self: Tensor, other: Union[Tensor, int]) -> Tensor:
    ...
    if isinstance(other, pypto_impl.Tensor):
        ...
        if not isinstance(other, int):
            raise PyptoError(TypeError(f"Scalar operand for bitwise_and must be an integer, but got {type(other)}."), err_code=_error_mapping[TypeError])
...
def bitwise_or(input1: Tensor, input2: Union[Tensor, int]) -> Tensor:
    ...
    if isinstance(input2, pypto_impl.Tensor):
        ...
        if not isinstance(input2, int):
            raise PyptoError(TypeError(f"Scalar operand for bitwise_or must be an integer, but got {type(input2)}."), err_code=_error_mapping[TypeError])
...
def bitwise_xor(first: Tensor, second: Union[Tensor, int]) -> Tensor:
    ...
    if isinstance(second, pypto_impl.Tensor):
        ...
        if not isinstance(second, int):
            raise PyptoError(TypeError(f"Scalar operand for bitwise_xor must be an integer, but got {type(second)}."), err_code=_error_mapping[TypeError])
...
def pow(input: Tensor, other: Union[Tensor, int, float]) -> Tensor:
    ...
    if not isinstance(other, (pypto_impl.Tensor, int, float)):
        raise PyptoError(TypeError(f"other must be Tensor, int or float but got {type(other)}."), err_code=_error_mapping[TypeError])
    if isinstance(other, pypto_impl.Tensor):
...
def var(
    ...
    if isinstance(dim, int):
        ...
        raise PyptoError(TypeError(f"the type of dim is not supported. 'int' or 'Lise[int]' or 'Tuple[int]' is needed."), err_code=_error_mapping[TypeError])
...
def uniform(
    ...
    """
    if len(shape) != 1:
        raise PyptoError(ValueError(f"shape must be 1-dimensional, got {len(shape)} dimensions"), err_code=_error_mapping[ValueError])
    if rounds not in [7, 10]:
        raise PyptoError(ValueError(f"rounds must be 7 or 10, got {rounds}"), err_code=_error_mapping[ValueError])
    ...
    if dtype not in valid_dtypes:
        raise PyptoError(ValueError(f"dtype must be one of DT_FP32, DT_FP16, DT_BF16, got {dtype}"), err_code=_error_mapping[ValueError])
