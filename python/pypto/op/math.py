...
@op_wrapper
def bitwise_xor(inputTensor: Tensor, other: Union[Tensor, int]) -> Tensor:
    """Computes the element-wise bitwise XOR of `inputTensor` and `other`.

    This function calculates the formula: `out = inputTensor ^ other`.
    It supports broadcasting between the inputTensor tensors.
    ...
    """
    if isinstance(other, pypto_impl.Tensor):
        return pypto_impl.BitwiseXor(inputTensor, other)
    else:
        if not isinstance(other, int):
