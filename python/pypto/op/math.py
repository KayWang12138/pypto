...
from ..enum import DataType, DivAlgorithm, PowAlgorithm, ExpAlgorithm, SqrtAlgorithm, RsqrtAlgorithm, LogAlgorithm, RecipAlgorithm
...
@op_wrapper
def pow(input: Tensor, other: Union[Tensor, int, float], precision_type: PowAlgorithm = PowAlgorithm.HIGH_PRECISION) -> Tensor:
    ...
    if isinstance(other, pypto_impl.Tensor):
        return pypto_impl.Pow(input, other, precision_type)
    if isinstance(other, int):
        return pypto_impl.Pow(input, pypto_impl.Element(DataType.DT_INT32, other), PowAlgorithm.INTRINSIC)
    return pypto_impl.Pow(input, pypto_impl.Element(DataType.DT_DOUBLE, other), precision_type)
