...
def scatter_update(input: Tensor, dim: int, index: Tensor, src: Tensor) -> Tensor:
    ...
    """
    if dim != -2:
        raise PyptoError(ValueError("scatter currection only support the case where dim = -2."), err_code=_error_mapping[ValueError])
    dims = input.Dim()
    if dims == 4:
        ...
        raise PyptoError(ValueError("dim must be 2 or 4"), err_code=_error_mapping[ValueError])
...
def get_scatter_mode(reduce: str):
    if reduce is None:
        ...
        raise PyptoError(ValueError("scatter reduce only support 'add', 'multiply'"), err_code=_error_mapping[ValueError])
...
def scatter_(
    ...
    if index.dtype not in (pypto_impl.DT_INT32, pypto_impl.DT_INT64):
        raise PyptoError(TypeError(f"index tensor must be of int32 or int64, but got {index.dtype}"), err_code=_error_mapping[TypeError])
    ...
    if isinstance(src, (int, float)):
        ...
        raise PyptoError(TypeError(f"Expected src to be int, float, Element, or Tensor, but got {type(src).__name__}"), err_code=_error_mapping[TypeError])
...
def scatter(
    ...
    if isinstance(src, (int, float)):
        ...
        raise PyptoError(TypeError(
            f"Expected src to be int, float, Element, or Tensor, but got {type(src).__name__}"), err_code=_error_mapping[TypeError])
