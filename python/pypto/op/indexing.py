...
@op_wrapper
def gathermask(input: Tensor, pattern_mode: int) -> Tensor:
    """
    Based on the built-in Mask selected by PatternMode, 
    the positions in the input Tensor where the corresponding Bit is 1 form the output Tensor, 
    ...
    -------
    Tensor
        A new tensor, with the same dtype as `input`, and the Shape of the output Tensor is as follows: 
        - pattern_mode <= 2, the output shape's trailing axis is input.shape's trailing axis / 2, while other axes match the input shape;
        - When 2 < pattern_mode < 7, the output shape's trailing axis is input.shape's trailing axis divided by 4, while other axes remain consistent with the input shape;
    ...
    RuntimeError
        If 1 <= pattern_mode <= 2, input.shape[input.shape.size()-1] % 2 == 0.
        If 3 <= pattern_mode <= 6, input.shape[input.shape.size()-1] % 4 == 0.
    ...
    return pypto_impl.GatherMask(input, pattern_mode)