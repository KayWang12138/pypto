...
from pypto.enum import AtomicType, DataType
...
def shmem_put(
    ...
    *,
    ...
    """
    dummy = pred_tokens[0] if len(pred_tokens) == 1 else pypto_impl.Nop(pred_tokens)
...
def shmem_clear(
    ...
    *,
    ...
    dummy = pred_tokens[0] if len(pred_tokens) == 1 else pypto_impl.Nop(pred_tokens)
    if is_signal:
        src_tile = pypto_impl.View(src, [1, comm_config.world_size] + shape, [comm_config.my_pe, 0] + offset)
...
def my_symbolic_pe(group_name: str) -> SymbolicScalar:
    ...
    """
    return SymbolicScalar.from_base(pypto_impl.GetSymbolicScalarRankId(group_name))