...
def shmem_signal(
    ...
    *,
    ...
    if notify_pe is None:
        return pypto_impl.ShmemSignal(dst_tile, dst_pe, dst_pe, signal, sig_op, dummy)
    elif type(notify_pe) is int and notify_pe == -1:
        return pypto_impl.ShmemSignalAll(dst_tile, dst_pe, signal, sig_op, dummy)
    else:
        return pypto_impl.ShmemSignal(dst_tile, dst_pe, notify_pe, signal, sig_op, dummy)
...
def shmem_signal_all(
    ...
    *,
    ...
    """Writes a signal to all the pes, the shmem tensor belonging to dst_pe is ready.
    ...
    --------
    Load data from a remote device to local GM
    dummy = pypto.distributed.shmem_signal_all(
        shmem_signal,
        1,
        2,
        [1, 128, 256],
