...
def create_shmem_tensor(
    ...
    """
    shmem_data = Tensor([n_pes] + shape, dtype)
    shmem_signal = Tensor([n_pes, n_pes] + shape, DataType.DT_INT32)

    for _ in loop(1, name="CREATE_SHMEM_TENSOR", idx_name="_"):
        pypto_impl.CreateShmemData(group_name, n_pes, dtype, shape, shmem_data.base(), ShmemMemType.WIN_IN.value)
