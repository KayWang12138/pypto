...
def shmem_signal(
    ...
    参数
        dst: shmem signal
        dst_rank: 目标卡id
        shapes: shmem signal的形状列表, 目前只支持len(shapes)=1
        offsets: signal 在 shmem中的偏移量
        atomic_type: 原子操作类型
        pred_tokens: 控制边
...
def shmem_wait(
    ...
    """等待信号
    参数
        src: shmem signal, 仅能为本卡shmem signal
        shape: shmem signal 的形状列表, 目前只支持len(shapes)=1
        offsets: shmem_signal的偏移量
        cmp_value: 等待值
        clear_flag: reset signal
        pred_tokens: 控制边
...
def shmem_clear(
    ...
    参数
        src: 待清零的shmem tensor, 可能是data或者signal
        shape: 待清零的shmem tensor的形状, 可能是data或者signal
        offset: 待清零的shmem tensor的偏移量, 可能是data或者signal
        is_signal: 是否是signal
        pred_tokens: 控制边
