    ...
    *,
    ...
    if shape is not None and offset is not None:
        if valid_shape is not None:
            src = pypto_impl.ShmemView(src, shape, offset, valid_shape)
            return pypto_impl.ShmemGet(src, src_pe, dummy)
        else:
            src = pypto_impl.ShmemView(src, shape, offset)
            return pypto_impl.ShmemGet(src, src_pe, dummy)
    return pypto_impl.ShmemGet(src, src_pe, dummy)
...
def shmem_signal(
    ...
    offset: list[Union[int, SymbolicScalar]] = None,
    *,
    notify_pe: Union[int, SymbolicScalar] = None,
    ...
    dummy = __normalize_pred(pred)
    if shape is not None and offset is not None:
        dst = pypto_impl.ShmemView(dst, shape, offset)
    if notify_pe is None:
        return pypto_impl.ShmemSignal(dst, dst_pe, dst_pe, signal, sig_op, dummy)
    elif type(notify_pe) is int and notify_pe == -1:
