...
def create_shmem_tensor(
    ...
    """
    shmem_data_tensor = Tensor([world_size] + shape, dtype)
    shmem_signal_tensor = Tensor([world_size, world_size] + shape, DataType.DT_INT32)

    for _ in loop(1, name="CREATE_SHMEM_TENSOR", idx_name="_"):
        pypto_impl.CreateShmemData(group_name, world_size, dtype, shape, shmem_data_tensor.base(), ShmemMemType.WIN_IN.value)
        pypto_impl.CreateShmemSignal(group_name, shmem_data_tensor.base(), shmem_signal_tensor.base())
        
    global comm_config
    if comm_config is None:
        comm_config = CommConfig(group_name, world_size, pypto_impl.GetSymbolicScalarRankId(group_name))
    return shmem_data_tensor, shmem_signal_tensor


@op_wrapper
def create_shmem_barrier_signal(
    group_name: str, 
    world_size: int,
) -> tuple[Tensor, Signal, Tensor]:
    ...
    Examples
    --------
    shmem_barrier_signal = pypto.distributed.create_shmem_barrier_signal(
        "tp",
        8
    )
    """
...
def shmem_signal(
    ...
    for idx, shape in enumerate(shapes):
        dst_tile = pypto_impl.View(dst, [1, 1] + shape, [dst_rank, dst_rank] + offsets[idx])
...
def shmem_wait(
    ...
    for idx, shape in enumerate(shapes):
        src_tile = pypto_impl.View(src, [1, 1] + shape, [comm_config.my_pe, comm_config.my_pe] + offsets[idx])
