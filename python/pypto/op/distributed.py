...
def shmem_view(
    ...
    *,
    ...
    if valid_shape is None:
        return pypto_impl.ShmemView(tensor, shape, offsets)
    else:
        return pypto_impl.ShmemView(tensor, shape, to_syms(valid_shape), to_syms(offsets))
...
def shmem_signal(
    ...
    offsets: list[Union[int, SymbolicScalar]] = None,
    *,
    notify_pe: Union[int, SymbolicScalar],
    sig_op: AtomicType = AtomicType.SET,
    pred: list[Tensor] = None,
) -> Tensor:
    """Writes a signal to the notify_pe, the shmem tensor belonging to dst_pe is ready.

    Parameters
    ----------
    dst : ShmemTensor
        The shared memory tensor.
    dst_pe : int or SymbolicScalar
        The shmem tensor belong to.
    signal: int
    ...
    notify_pe: int or SymbolicScalar
        The target device to recieve this signal, if notify_pe is None, dst_pe will recieve this signal.
    sig_op : AtomicType
    ...
    dummy = __normalize_pred(pred)
    if shape is not None and offsets is not None:
        dst = pypto_impl.ShmemView(dst, shape, offsets)
    if type(notify_pe) is int and notify_pe == -1:
        return pypto_impl.ShmemSignalAll(dst, dst_pe, signal, sig_op, dummy)
    else:
        return pypto_impl.ShmemSignal(dst, dst_pe, notify_pe, signal, sig_op, dummy)
...
def shmem_clear_data(
    ...
    *,
    ...
    offsets : list of int
        The offsets of the shmem tensor to be cleared; can be for data or signal.
    pred : Tensor
        Predicate tokens used as control dependencies.
