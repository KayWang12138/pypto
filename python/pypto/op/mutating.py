...
def cast(input: Tensor, dtype: DataType, mode: CastMode = CastMode.CAST_NONE, 
         sat_mode: SaturationMode = SaturationMode.OFF) -> Tensor:
    ...
    sat_mode : SaturationMode, optional
    ...
    y = pypto.cast(x, pypto.DT_INT8, sat_mode=pypto.SaturationMode.ON)
    # Values will be clamped to [-128, 127] range

    Input  x: [2.0, 3.0] x.dtype: pypto.DT_FP32
    Output y: [2.0, 3.0] y.dtype: pypto.DT_FP16
    """
    if dtype == input.dtype and mode == CastMode.CAST_NONE and sat_mode == SaturationMode.OFF:
        return input
    else:
        return pypto_impl.Cast(input, dtype, mode, sat_mode)
