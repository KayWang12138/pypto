...
def scaled_mm(
    ...
    *,
    ...
    """
    __validate_inputs(input1, other, out_dtype, [a_trans, b_trans, c_matrix_nz, extend_params])
    __validate_scaled_inputs(input1, other, scale_a, scale_b)
    __validate_scaled_shape(input1, other, scale_a, scale_b, [a_trans, b_trans, scale_a_trans, scale_b_trans])
    if extend_params is not None:
        extend_params = pypto_impl.MatmulExtendParam(
            **__convert_matmul_extend_params(extend_params)
        )
        return pypto_impl.MatmulMX(
            out_dtype, input1, scale_a, other, scale_b, a_trans, scale_a_trans, b_trans, scale_b_trans,
            c_matrix_nz, extend_params
        )
    else:
        return pypto_impl.MatmulMX(
            out_dtype, input1, scale_a, other, scale_b, a_trans, scale_a_trans, b_trans, scale_b_trans, c_matrix_nz
        )
...
def __validate_scaled_shape(input_tensor1, input_tensor2, input_scale1, input_scale2, optional_param) -> None:
    ...
    m_scale_dim, kascale0_dim, kascale1_dim = (scale1_valid_shape[0], scale1_valid_shape[1],
    ...
    kbscale0_dim, n_scale_dim, kbscale1_dim = (scale2_valid_shape[0], scale2_valid_shape[1],
