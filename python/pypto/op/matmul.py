...
def __validate_inputs(input_tensor1, input_tensor2, out_dtype, optional_param) -> None:
    ...
    if not ((input1_dtype in (pypto_impl.DataType.DT_FP8E5M2, pypto_impl.DataType.DT_FP8E4M3) and
         input2_dtype in (pypto_impl.DataType.DT_FP8E5M2, pypto_impl.DataType.DT_FP8E4M3)) or
        (input1_dtype == input2_dtype)):
        raise ValueError(
            "Only DT_FP8E5M2 and DT_FP8E4M3 can be mixed. All other dtypes must be the same."
        )
