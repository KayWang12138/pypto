...
def scaled_mm(
    ...
    *,
    ...
    `input` and `other` can be 2-D.
    - If `input` is (n x k) and `other` is (k x m), output is (n x m).

    `scale_a` and `scale_b` can be 3-D.
    - If `input` is (n x k), `scale_a` is (n x kScale x 2)
    - If `other` is (k x m), `scale_b` is (kScale x m x 2)
    ...
    a_trans : bool, default=False
        If True, transpose the left matrix (`input`) before multiplication.
    b_trans : bool, default=False
        If True, transpose the right matrix (`other`) before multiplication.
    ...
    ------
    RuntimeError
        If input and other dimensions are invalid (!=2-D), or scale_a and scale_b dimensions are invalid (!=3-D),
        or if matrix dimensions are incompatible.

    Examples
    --------
    # Standard matrix multiplication
    a = pypto.tensor((16, 32), pypto.DT_FP8E4M3, "tensor_a")
    b = pypto.tensor((32, 64), pypto.DT_FP8E4M3, "tensor_b")
    a_scale = pypto.tensor((16, 32), pypto.DT_FP8E8M0, "scale_a")
    b_scale = pypto.tensor((32, 64), pypto.DT_FP8E8M0, "scale_b")
    ...
    """
    __validate_inputs(input, other, out_dtype, [a_trans, b_trans, c_matrix_nz, extend_params])
    __validate_scaled_inputs(input, other, scale_a, scale_b, [a_trans, b_trans, scale_a_trans, scale_b_trans])
    if extend_params is not None:
        extend_params = pypto_impl.MatmulExtendParam(
            **__convert_matmul_extend_params(extend_params)
        )
        return pypto_impl.MatmulMX(
            out_dtype, input, scale_a, other, scale_b, a_trans, scale_a_trans, b_trans, scale_b_trans,
            c_matrix_nz, extend_params
        )
    else:
        return pypto_impl.MatmulMX(
            out_dtype, input, scale_a, other, scale_b, a_trans, scale_a_trans, b_trans, scale_b_trans, c_matrix_nz
        )
...
def __validate_scaled_shape(input_tensor1, input_tensor2, input_scale1, input_scale2, optional_param) -> None:
    ...
    m_scale_dim, kascale0_dim, kascale1_dim = (scale1_valid_shape[1], scale1_valid_shape[0], scale1_valid_shape[2]) \
    if not a_scale_trans else (scale1_valid_shape[0], scale1_valid_shape[1], scale1_valid_shape[shape_dim_2])
    kbscale0_dim, n_scale_dim, kbscale1_dim = (scale2_valid_shape[1], scale2_valid_shape[0], scale2_valid_shape[2]) \
    if not b_scale_trans else (scale2_valid_shape[0], scale2_valid_shape[1], scale2_valid_shape[shape_dim_2])

    if kascale0_dim != kbscale0_dim:
        raise RuntimeError(
            "Scale Matrix K0 dimension mismatch. Expect scale_ka_size == scale_kb_size, "
            f"got scale_ka_size: {kascale0_dim}, scale_kb_size: {kbscale0_dim}."
        )

    if kascale1_dim != kbscale1_dim or kascale1_dim != shape_dim_2:
        raise RuntimeError(
            "Scale Matrix K1 dimension mismatch. Expect scale_a_shape[2] == scale_b_shape[2] and both equal to 2, "
            f"got scale_a_shape[2]: {kascale1_dim}, scale_b_shape[2]: {kbscale1_dim}."
        )
