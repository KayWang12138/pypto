...
class BuildOnlineManager:
    ...
    class _CMakeContext:
        ...
        def compile(self, src_dir: Path, tmp_dir: Path, cfg_cmd_ext: Optional[str] = None,
                  build_type: str = "Release", build_job_num: int = 4, capture_output: bool = True) -> Path:
            """执行编译流程(包含 CMake 的 Configure, Build 及 Install 阶段)

            本函数会在临时目录 tmp_dir 路径下以下临时目录:
            1. 创建 build 子目录作为 CMAKE_BINARY_DIR
            2. 创建 install 子目录作为 CMAKE_INSTALL_PREFIX;

            :param src_dir: 源码根目录, 对应 CMAKE_SOURCE_DIR
            :param tmp_dir: 临时目录
            :param cfg_cmd_ext: CMake Configure 阶段的扩展命令行
            :param build_type: 编译类型, 默认是 Release
            :param build_job_num: 编译并发度
            :param capture_output: 拦截调用 CMake 过程的输出
            ...
            # CMake Configure
            cfg_cmd_ext = cfg_cmd_ext if cfg_cmd_ext else ""
            cmd = f"{self.cmake} -S {src_dir} -B {build_dir} -DCMAKE_BUILD_TYPE={build_type}"
            cmd += f" -DCMAKE_INSTALL_PREFIX={install_dir} {cfg_cmd_ext}"
            subprocess.run(shlex.split(cmd), capture_output=capture_output, check=True, text=True, encoding='utf-8')
            # CMake Build
            cmd = f"{self.cmake} --build {build_dir}" + (f" -j {build_job_num}" if build_job_num else "")
            subprocess.run(shlex.split(cmd), capture_output=capture_output, check=True, text=True, encoding='utf-8')
            # CMake Install
    ...
    def build_and_load_calculator(self):
        ...
        with tempfile.TemporaryDirectory() as _tmp_dir:
            ...
            ext += f" -DPY3_MOD_TORCH_C_GLIBCXX_USE_CXX11_ABI={torch.torch_c_use_cxx11_abi}"
            install_prefix = cmake_ctx.compile(src_dir=Path(self.pkg_lib_dir, "calculator"), tmp_dir=Path(_tmp_dir),
                                               cfg_cmd_ext=ext)
            # 加载
