...
class AstMutator(ast.NodeTransformer):
    ...
    def _transform_for(self, node: ast.For) -> List[ast.AST]:
        ...
        if not (isinstance(node.iter, ast.Call) and 
                ...
                node.iter.func.value.id == 'block' and
                node.iter.func.attr == 'loop'):
        ...
        start = self.visit(loop_args[0]) if len(loop_args) > 0 else ast.Constant(value=0)
        end = self.visit(loop_args[1]) if len(loop_args) > 1 else (self.visit(loop_args[0]) if len(loop_args) > 0 else ast.Constant(value=0))
        ...
        fornode_assign = ast.Assign(
            ...
            value=ast.Call(
                ...
                args=[
                    ast.Name(id=loop_var_name, ctx=ast.Load()),
                    start,
                    end,
                    step
                ],
