...
def get_pass_default_config(key: PassConfigKey, default_value: bool) -> bool:
    ...
    if not isinstance(default_value, bool):
        raise PyptoError(TypeError(f"Expected boolean type, but received {type(default_value).__name__}"), _get_err_code(TypeError("")))
    if not isinstance(key, PassConfigKey):
        raise PyptoError(ValueError(f"key must be a member of PassConfigKey, got {key}. "), _get_err_code(ValueError("")))
...
def set_pass_default_config(key: PassConfigKey, value: bool):
    ...
    if not isinstance(value, bool):
        raise PyptoError(TypeError(f"Expected boolean type, but received {type(value).__name__}"), _get_err_code(TypeError("")))
    if not isinstance(key, PassConfigKey):
        raise PyptoError(ValueError(f"key must be a member of PassConfigKey, got {key}. "), _get_err_code(ValueError("")))
