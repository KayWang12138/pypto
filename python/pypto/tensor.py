...
class Tensor:
    ...
    @source_location
    def where(self, condition: 'Tensor', x: Union['Tensor', Element, float], y: Union['Tensor', Element, float]) -> 'Tensor':
        return pypto.where(self, condition, x, y)

    @source_location
    def lrelu(self, input: 'Tensor', negative_slope: Union[float, Element] = 0.01) -> 'Tensor':
        return pypto.lrelu(self, input, negative_slope)
