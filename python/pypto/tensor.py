...
class Tensor:
    ...
    def __init__(self, shape=None, dtype: Union[DataType, None] = None,
        ...
        if shape is None:
            ...
            if not isinstance(nshape, list):
                raise FrontendError(TypeError("shape must be a list of int or SymbolicScalar"))
    ...
    def __setitem__(self, key, value):
        ...
        if isinstance(key, slice) and isinstance(key.stop, Tensor):
            if not isinstance(key.start, int):
                raise FrontendError(TypeError(f"scatter key.start must be int, got {type(key.start).__name__}"))
        ...
        raise FrontendError(ValueError("tuple key must be int, SymbolicScalar or slice"))

    def __iter__(self):
        raise FrontendError(TypeError("Tensor is not iterable."))
    ...
    def __getitem__(self, key, *, valid_shape: Optional[List[Union[int, SymbolicScalar]]] = None):
        ...
        if isinstance(key, slice) and isinstance(key.stop, Tensor):
            if not isinstance(key.start, int):
                raise FrontendError(TypeError(f"gather key.start must be int, got {type(key.start).__name__}"))
    ...
    def __matmul__(self, other: 'Tensor') -> 'Tensor':
        if other.dtype in {pypto.DT_FP16, pypto.DT_BF16, pypto.DT_FP32}:
            ...
            raise FrontendError(RuntimeError("unsupported dtype"))
    ...
    def _get_assemble_offset(key, shape):
        ...
        for axis, k in enumerate(key):
            ...
            if step not in (1, None):
                raise FrontendError(ValueError("step must be 1 or None"))
    ...
    def move(self, other: 'Tensor') -> None:
        if isinstance(other, Tensor):
            ...
            raise FrontendError(TypeError(f"'{type(other).__name__}' type cannot be moved to Tensor"))
    ...
    def _normalize_key(self, key):
        ...
        if not isinstance(key, tuple):
            raise FrontendError(RuntimeError("Invalid key type"))

        if any(k is Ellipsis for k in key):
            ellipsis_count = sum(k is Ellipsis for k in key)
            if ellipsis_count > 1:
                raise FrontendError(ValueError("Only one ... is supported"))
