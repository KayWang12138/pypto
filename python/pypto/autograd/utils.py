...
This module provides helper functions used internally by autograd:
- zeros_like/ones_like: create tensors with same shape/dtype
- unbroadcast: reduce gradient to match original input shape
- is_differentiable_dtype: check if dtype supports differentiation
"""
...
# Differentiable dtypes (floating point types)
...
def is_differentiable_dtype(dtype: Any) -> bool:
    ...
    Returns:
        True if the dtype supports gradient computation
    """
    dtype_name = str(dtype) if not isinstance(dtype, str) else dtype
    # Handle both "DataType.DT_FP32" and "DT_FP32" formats
    if "." in dtype_name:
...
def zeros_like(tensor: "Tensor") -> "Tensor":
    ...
    This is an autograd-internal helper, not exposed as public API.

    Args:
        tensor: The reference tensor
...
def full_like(tensor: "Tensor", fill_value: Union[int, float]) -> "Tensor":
    ...
    Args:
        tensor: The reference tensor
        fill_value: The value to fill

    Returns:
        A new tensor filled with the value
    """
    import pypto
    shape = get_shape(tensor)
    dtype = get_dtype(tensor)
...
def _compute_broadcast_axes(
    ...
    Args:
        grad_shape: Shape of the gradient (result of broadcast)
        target_shape: Original shape before broadcast

    Returns:
        List of axis indices that were broadcast
    """
    ...
    # Handle case where target has fewer dimensions (implicit 1s on left)
    for i in range(grad_ndim - target_ndim):
        broadcast_axes.append(i)

    # Check each aligned dimension
...
def unbroadcast(
    ...
    Args:
        grad: The gradient tensor (shape matches broadcast result)
        target_shape: The original shape before broadcasting
        keepdim: Whether to keep reduced dimensions

    Returns:
        Gradient tensor with shape matching target_shape
    """
    import pypto

    grad_shape = get_shape(grad)

    # Convert symbolic shapes to concrete values for comparison
    ...
    if result_shape_concrete != target_shape_concrete:
        ...
        if len(result_shape_concrete) > len(target_shape_concrete):
            ...
            # Squeeze dimensions that should be 1
            result = pypto.reshape(result, target_shape_concrete)
...
def maybe_cast(tensor: "Tensor", target_dtype: Any) -> "Tensor":
    ...
    Returns:
        Cast tensor, or original if dtype matches
    """
    import pypto
