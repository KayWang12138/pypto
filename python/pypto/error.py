...
class ErrCode(enum.Enum):
    TYPE_ERROR = 0xF10001
    VALUE_ERROR = 0xF10002
    RUNTIME_ERROR = 0xF10003
    NOT_IMPLEMENTED_ERROR = 0xF10004
    PARSER_ERROR = 0xF10005
...
class ParserError(PTOError):
    """Error class for diagnostics with AST node association.

    Parser errors are associated with specific AST nodes for better
    source location reporting in error messages.
    """

    def __init__(self, node: ast.AST, msg: Union[str, Exception]):
        super().__init__(msg, ErrCode.PARSER_ERROR)
        self.node = node
...
class RenderedParserError(ParserError):
    ...
    This error type indicates that the error message has already been
    formatted and displayed, so it should not be re-rendered.
    """
