...
def _assign_target_roots(node: ast.AST) -> Set[str]:
    """Return root Name ids written by a single assignment-like node.

    Non-assignment nodes yield an empty set, which lets callers walk an AST and
    accumulate write targets without having to dispatch on node type themselves.
    """
    if isinstance(node, ast.Assign):
        result: Set[str] = set()
        for tgt in node.targets:
            result |= _target_root_names(tgt)
        return result
    if isinstance(node, (ast.AnnAssign, ast.AugAssign, ast.For)):
...
def _collect_writable_roots(stmts: list) -> Set[str]:
    ...
    for stmt in stmts:
        for sub in ast.walk(stmt):
            roots |= _assign_target_roots(sub)
    return roots
