...
class ContextFrame:
    ...
    def add(self, var: str, node: Optional[ast.AST] = None) -> None:
        ...
        if var in self.vars:
            if node is None:
                raise FrontendError(NameError(f"Variable '{var}' already exists in the current scope"))
            else:
                raise FrontendError(NameError(f"Variable '{var}' already exists in the current scope"))
        self.vars.add(var)
...
class Context:
    ...
    def delete(self, var: str) -> None:
        ...
        if var not in frame.vars:
            raise FrontendError(NameError(f"Variable '{var}' is not defined in the current scope"))
