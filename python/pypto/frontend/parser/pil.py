...
class PILBuilder(ast.NodeVisitor):
    ...
    def create_pil_assign_name_single(self, target_name:str, source_expr: PILExpr, node_attr: PILAttr = NOATTR) -> ast.Assign:
        """
        Parameter:
    ...
    def create_pil_assign_subscript(self, target_name:str, slices: list[tuple[PILExpr | None, PILExpr | None, PILExpr | None] | PILExpr], source_expr: PILExpr, node_attr: PILAttr = NOATTR) -> ast.Assign:
        """
        Parameter:
            target_name: identifier of the object
            slices:
    ...
    def create_pil_raise(self, exc: PILExpr | None, cause: PILExpr | None, node_attr: PILAttr = NOATTR) -> ast.Raise:
        """
        Parameter:
    ...
    def create_pil_call(self, func_expr: PILExpr, args: list[PILExpr], keywords: list[ast.keyword], node_attr: PILAttr = NOATTR) -> ast.Call:
        ...
        Parameter:
            ...
            keywords: list of keyword nodes (keyword arguments)
        Emit code example:
            func_expr(args[0], args[1], key=keywords[0])
        """
...
class PythonParser(PILBuilder, ast.NodeVisitor):
    ...
    def visit_slice(self, slice: ast.Slice | tuple[ast.Slice]) -> tuple[list[ast.stmt], list[tuple[str | None, str | None, str | None]]]:
        ...
        for s in slice_list:
            if isinstance(s, ast.Slice):
                ...
                subscript_stmt_list, subscript_name = self.visit(s)
                slice_stmt_list.extend(subscript_stmt_list)
                pil_slice_list.append(subscript_name)
        return slice_stmt_list, pil_slice_list

    def visit_lhs(self, target: ast.expr, source_expr: PILExpr) -> list[ast.stmt]:
        if isinstance(target, ast.Name):
            return [self.create_pil_assign_name_single(target.id, source_expr)]

        elif isinstance(target, ast.Attribute):
            obj_stmts, obj_name = self.visit(target.value)
            return obj_stmts + [self.create_pil_assign_attribute(obj_name, target.attr, source_expr)]

        elif isinstance(target, ast.Subscript):
            obj_stmts, obj_name = self.visit(target.value)
            slice_stmt_list, pil_slice_list = self.visit_slice(target.slice)
            return obj_stmts + slice_stmt_list + [self.create_pil_assign_subscript(obj_name, pil_slice_list, source_expr)]
            ...
            unpack_stmt = self.create_pil_assign_name(elt_temps, self.create_pil_expr(source_expr))
    ...
    def visit_FunctionDef(self, name: str, args: ast.arguments, body: list[ast.stmt], decorator_list: list[ast.expr], returns: ast.expr | None, type_comment: str | None, node_attr: PILAttr = NOATTR) -> tuple[list[ast.stmt], PILExpr | None]:
        """
    ...
    def visit_AugAssign(self, target: ast.expr, op: ast.operator, value: ast.expr, node_attr: PILAttr = NOATTR) -> tuple[list[ast.stmt], PILExpr | None]:
        ...
        Case 2 (attribute target):
            ...
            PIL:
                ...
                _tmp_1 = expr           # evaluate rhs
                _tmp_2 = _tmp_0.attr    # load
                _tmp_3 = _tmp_2 op _tmp_1
        ...
        Case 3 (subscript target):
            ...
            PIL:
                ...
                _tmp_3 = _tmp_0[_tmp_1] # load
                _tmp_4 = _tmp_3 op _tmp_2
        ...
        if isinstance(target, ast.Name):
            ...
            store_stmt = self.create_pil_assign_name_single(target.id, temp_name)
    ...
    def visit_AnnAssign(self, target: ast.expr, annotation: ast.expr, value: ast.expr | None, simple: int, node_attr: PILAttr = NOATTR) -> tuple[list[ast.stmt], PILExpr | None]:
        ...
        if value is None:
            return ann_stmts, None
        value_stmts, value_name = self.visit(value)
        return ann_stmts + value_stmts + self.visit_lhs(target, value_name), None
    ...
    def visit_While(self, test: ast.expr, body: list[ast.stmt], orelse: list[ast.stmt], node_attr: PILAttr = NOATTR) -> tuple[list[ast.stmt], PILExpr | None]:
        ...
        reeval_stmt_list = reeval_stmt_list + [self.create_pil_assign_name_single(test_name, reeval_name)]
    ...
    def visit_Try(self, body: list[ast.stmt], handlers: list[ast.excepthandler], orelse: list[ast.stmt], finalbody: list[ast.stmt], node_attr: PILAttr = NOATTR) -> tuple[list[ast.stmt], PILExpr | None]:
        ...
        for handler in reversed(handlers):
            ...
            if handler.type is not None:
                ...
                handler_pre_stmts.extend(handler_type_stmts)
            else:
                handler_type_name = None
            # Bind handler.name to exc_var if requested
            handler_body_stmts, _ = self.visit_stmts(handler.body)
            if handler.name is not None:
                handler_body_stmts = [self.create_pil_assign_name_single(handler.name, exc_var)] + handler_body_stmts
            if handler_type_name is not None:
                isinstance_temp = self.create_temp_identifier()
                isinstance_stmt = self.create_pil_assign_name(isinstance_temp, self.create_pil_call('isinstance', [exc_var, handler_type_name], []))
                dispatch_body = handler_pre_stmts + [isinstance_stmt, self.create_pil_if(isinstance_temp, handler_body_stmts, dispatch_body)]
            else:
                # bare except: catches everything
                dispatch_body = handler_pre_stmts + handler_body_stmts
    ...
    def visit_Continue(self, node_attr: PILAttr = NOATTR) -> tuple[list[ast.stmt], PILExpr | None]:
        if self.continue_stack[-1] is None:
            ...
            result_stmt_list = reeval_stmt_list + [self.create_pil_assign_name_single(test_name, reeval_name), self.create_pil_continue()]
        return result_stmt_list, None
    ...
    def visit_BoolOp(self, op: ast.boolop, values: list[ast.expr], node_attr: PILAttr = NOATTR) -> tuple[list[ast.stmt], PILExpr | None]:
        ...
        if isinstance(op, ast.And):
            ...
            rest_stmt = self.create_pil_if(
                first_name,
                rest_stmt_list + [self.create_pil_assign_name_single(temp_name, rest_name)],
                [self.create_pil_assign_name_single(temp_name, first_name)])
    ...
    def visit_NamedExpr(self, target: ast.expr, value: ast.expr, node_attr: PILAttr = NOATTR) -> tuple[list[ast.stmt], PILExpr | None]:
        ...
        value_stmt_list, value_name = self.visit(value)
        assign_stmt = self.create_pil_assign_name_single(target.id, value_name)
    ...
    def visit_IfExp(self, test: ast.expr, body: ast.expr, orelse: ast.expr, node_attr: PILAttr = NOATTR) -> tuple[list[ast.stmt], PILExpr | None]:
        ...
        temp_name = self.create_temp_identifier()
        result_body_stmt_list = body_stmt_list + [self.create_pil_assign_name_single(temp_name, body_name)]
        result_orelse_stmt_list = orelse_stmt_list + [self.create_pil_assign_name_single(temp_name, orelse_name)]
...
if __name__ == '__main__':
    def func():
        a, [b0()[b1() + b2()], *c0().c1], [*d, e], *[f, g, h] = foo()

        d = yield a + 20

        t = lambda a, b, *c: a + b + c

        t = [a * b for a, b, c in d*e if b + c and d + e for e, f, g in pp if zz + zz]

        z = f"{a:0.3f}+b+c+{d=}+e"

        with foo() as boo[4], goo() as g.a:
            pass
