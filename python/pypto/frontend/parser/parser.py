...
class Parser(ast.NodeVisitor):
    ...
    def get_signature(
        ...
        with self.context.with_frame():
            ...
            for _, (in_obj, def_obj) in enumerate(zip(tensor_input_args, tensor_input_args_def)):
                in_obj.name = def_obj.name

            # delete out_tensor from in_tensor
            all_count = len(tensor_input_args)
            need_count = len(tensor_input_args_def)
            if all_count > need_count:
                tensor_input_args = tensor_input_args[:need_count]
