...
from pypto.error import ParserError, RenderedParserError, FrontendError
...
class NestedFunctionMarker:
    ...
    def _check_input_defs_match(self, call_args: list, param_specs: list) -> None:
        ...
        if len(param_specs) != len(call_args):
            raise FrontendError(RuntimeError(f"There are {len(param_specs)} input param(s), \
...
class Parser(ast.NodeVisitor):
    ...
    def execute(self) -> Any:
        ...
        if self._parsed_node is None:
            raise FrontendError(RuntimeError("parse() must be called before execute()"))
