...
class Parser(doc.NodeVisitor):
    ...
    def _visit_for(self, node: doc.For) -> Any:
        ...
        if isinstance(iter_expr, range):
            iter_expr = self._convert_range_iterator(iter_expr)
        ...
        if isinstance(iter_expr, (list, tuple)):
            ...
            raise ParserError(
                node.iter,
                TypeError(
                    f"Loop iterator must be a range object, Iterator, or list/tuple, but got {type(iter_expr).__name__}."
                ),
            )
    ...
    def _convert_range_iterator(self, range_expr: range) -> list:
        """Convert range object to list for unified processing."""
        if isinstance(range_expr.start, SymbolicScalar) or isinstance(range_expr.stop, SymbolicScalar) or isinstance(range_expr.step, SymbolicScalar):
            raise ParserError(
                range_expr,
                TypeError(
