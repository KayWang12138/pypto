...
class Parser(ast.NodeVisitor):
    ...
    def _visit_return(self, node: ast.Return) -> Any:
        ...
        # Normalize return value(s) into list[Tensor]
        if isinstance(expr, pypto.Tensor):
            returned = [expr]
        ...
        final = self._finalize_return_outputs(node, returned, func_name)
    ...
    def _finalize_return_outputs(self, node: doc.Return, returned: list[pypto.Tensor], func_name: str) -> Optional[Any]:
        ...
        # Name signature outputs + writeback
        for i, tensor in enumerate(returned):
            if kind == "signature" and not getattr(outputs[i], "name", ""):
                outputs[i].name = f"output_{func_name}" if len(outputs) == 1 else f"output_{func_name}_{i}"
            outputs[i][:] = tensor
