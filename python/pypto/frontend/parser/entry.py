...
class JitCallableWrapper:
    ...
    def _check_input_defs_match_tensors(self, in_tensors: list, input_tensor_defs: list[pypto.Tensor]) -> None:
        ...
        if len(in_tensors) != len(input_tensor_defs):
            raise ValueError(f"The number of input tensors {len(in_tensors)} does not match the number of input tensor definitions {len(input_tensor_defs)}.")
        ...
        for in_tensor, input_tensor_def in zip(in_tensors, input_tensor_defs):
            ...
            if len(in_tensor.shape) != len(input_tensor_def.shape):
                raise ValueError(f"The number of dimensions of {ordinal(idx)} input tensor {in_tensor.shape} does not match the number of dimensions of input tensor definition {input_tensor_def.shape}.")
            for i, dim in enumerate(input_tensor_def.shape):
                if  isinstance(dim, int) and in_tensor.shape[i] != dim:
                    raise ValueError(f"The shape of {ordinal(idx)} input tensor {in_tensor.shape} does not match the shape of input tensor definition {input_tensor_def.shape}.")
            
            # Check the dtype of input tensors and input tensor definitions
            if self._dtype_dict[in_tensor.dtype] != input_tensor_def.dtype:
                raise ValueError(f"The dtype of {ordinal(idx)} input tensor {in_tensor.dtype} does not match the dtype of input tensor definition {input_tensor_def.dtype}.")
