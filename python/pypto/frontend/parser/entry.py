...
class RunMode(IntEnum):
    """Run mode enumeration: NPU(0) = hardware runtime, SIM(1) = simulation"""
    NPU = 0
    SIM = 1
...
class JitCallableWrapper:
    ...
    def __init__(
        ...
        captured_locals : Optional[dict[str, Any]], optional
            Local variables captured from the original function's scope (copied to a new dict to prevent external modification). Defaults to None.
        infer_controlflow_shape : Optional[type], optional
            Class type used for inferring control flow shape during compilation (None uses default inference logic). Defaults to None.
        use_cache : bool, optional
            Whether to use compilation caching. If True, reuse existing KernelModule from global cache; if False, force recompilation and update cache. Defaults to True.
        """
    ...
    def __call__(self, *args, **kwargs):
        ...
        **kwargs : Any
        ...
        if self._runtime_options.get("run_mode", None) == RunMode.NPU:
            print("LaunchKernel::RunMode.NPU")
