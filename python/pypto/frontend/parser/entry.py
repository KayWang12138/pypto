...
def _pto_to_tensor_data(
    ...
    for t in tensors:
        if t.ori_shape is None:
            raise FrontendError(RuntimeError("The ori_shape of the tensor is not specified."))
...
class JitCallableWrapper:
    ...
    def _get_func_nonlocals(func: Callable) -> dict[str, Any]:
        ...
        if not inspect.isfunction(func):
            raise FrontendError(TypeError(f"{func!r} is not a Python function"))
    ...
    def _parse_call_args(
        ...
        if len(args) < n_tensors:
            raise FrontendError(RuntimeError(
    ...
    def _check_input_defs_match_tensors(self, in_tensors: list, input_tensor_defs: list[pypto.Tensor]) -> None:
        ...
        if len(in_tensors) != len(input_tensor_defs):
            raise FrontendError(RuntimeError(f"There are {len(in_tensors)} input tensor(s), \
        ...
        for in_tensor, input_tensor_def in zip(in_tensors, input_tensor_defs):
            ...
            if len(input_tensor_def.shape) != 0 or input_tensor_def.status_shape is not None:
                ...
                if is_diff_shape:
                    raise FrontendError(ValueError(f"The number of dimensions of {ordinal(idx)} input tensor {in_tensor.shape} \
                ...
                for i, dim in enumerate(input_tensor_def.shape):
                    if isinstance(dim, int) and in_tensor.shape[i] != dim:
                        raise FrontendError(ValueError(f"The shape of {ordinal(idx)} input tensor {in_tensor.shape} \
            ...
            if input_tensor_def.explicit_dtype is not None:
                ...
                if (
                    ...
                    raise FrontendError(ValueError(f"The dtype of {ordinal(idx)} input tensor {in_tensor.dtype} \
            ...
            if in_tensor.device.type == "npu":
                if input_tensor_def.explicit_format is not None:
                    if self._format_dict[get_format(in_tensor)] != input_tensor_def.format:
                        raise FrontendError(ValueError(f"The format of {ordinal(idx)} input tensor {get_format(in_tensor)} \
    ...
    def _get_signature(
        ...
        for param_name in param_names:
            ...
            if ann is not None and hasattr(ann, 'to_tensor'):
                if seen_non_tensor:
                    raise FrontendError(ValueError(
    ...
    def _set_run_mode(self) -> None:
        ...
        if cann_is_configed:
            ...
            if torch.npu.is_available() is False:
                raise FrontendError(RuntimeError("NPU is not available."))
        else:
    ...
    def _run(
        ...
        if self._handler is None:
            raise FrontendError(RuntimeError("handler is not initialized"))
        ...
        if runtime_error_msg != "":
            raise FrontendError(RuntimeError(runtime_error_msg))
    ...
    def _run_with_npu(
        ...
        if device.type == "npu":
            ...
            raise FrontendError(RuntimeError(f"Unsupported device type: {device.type}"))
    ...
    *,
    ...
    def decorator_wrapper(f: Callable) -> JitCallableWrapper:
        if not inspect.isfunction(f):
            raise FrontendError(TypeError("jit decorator can only be used on functions"))
