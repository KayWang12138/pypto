...
def _count_calls(func):
    ...
    def wrapper(tensor, name: str = "", *, dynamic_axis: Optional[List[int]] = None,
                tensor_format: Optional[TileOpFormat] = None, dst_dtype: Optional[DataType] = None):
        nonlocal count
        count += 1
        if name == "":
            name = f"TENSOR_{count}"
        return func(tensor, name, dynamic_axis, tensor_format, dst_dtype)
...
def from_torch(tensor, name: str = "", dynamic_axis: Optional[List[int]] = None,
    ...
    >>> y = torch.randn(2, 3)
    >>> y_pto = pypto.from_torch(y, "input_tensor", dst_dtype=pypto.DataType.DT_HF8)
    >>> print(y_pto.dtype)
    DataType.DT_HF8
    """
    import torch
    ...
    dtype = _dtype_from(tensor.dtype) if dst_dtype is None else dst_dtype
    if tensor.dim() == 0:
        return Tensor(
            shape=tuple([1]),
            dtype=dtype,
