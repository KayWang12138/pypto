    ...
    m.def(
     ...
     m.def(
 	    ...
 	    [](const Tensor& predToken, Tensor &in, const char *group, Tensor &out) {
 	        return npu::tile_fwk::Distributed::OneShotAllReduce(predToken, in, group, out); },
 	    py::arg("pred_token"), py::arg("in"), py::arg("group"), py::arg("ranksie"), py::arg("out"), "Tensor all reduce");
 	m.def(
 	    "two_shot_all_reduce",
 	    [](const Tensor& predToken, Tensor &in, const char *group, Tensor &out) {
 	        return npu::tile_fwk::Distributed::TwoShotAllReduce(predToken, in, group, out); },
