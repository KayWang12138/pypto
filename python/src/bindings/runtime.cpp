...
public:
    KernelBinary(std::shared_ptr<Function> func) : dynFunc(func) {
        ...
        kernelBin = DeviceLauncher::RegisterKernelBin(dynAttr->devProgBinary);
...
public:
    ...
    void Launch(KernelBinary *kernel, bool isCaptureMode, aclrtStream aicoreStream, aclrtStream ctrlStream, aclrtStream schedtream,
        ...
        if (tripleStream) {
            ...
            int ret = DeviceLauncher::LaunchAicpuKernel(ctrlStream, rtAicpuArgs, 2);
            ...
            ret = DeviceLauncher::LaunchAicpuKernel(schedtream, rtAicpuArgs, 3);
            ...
            auto startTime = MsprofSysCycleTime();
            int ret = DeviceLauncher::LaunchAicpuKernel(schedtream, rtAicpuArgs, nrAicpu);
        ...
        auto startTime = MsprofSysCycleTime();
        auto ret = DeviceLauncher::LaunchAicoreKernel(aicoreStream, kernel->GetKernelBin(), blockDim, rtAicoreArgs, rtTaskCfg);
...
void LaunchKernel(py::object &module, int64_t stream, py::args &args) {
    ...
    bool isCaptureMode = DeviceLauncher::AddAicpuStream(
        aicoreStream, ctrlStream, schedtream, kmodule->IsTripleStream());
