...
public:
    ...
    std::pair<AiCpuArgs *, int64_t> BuildKernelArgs(const std::vector<DeviceTensorData> &tensors) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, (int64_t)tensors.size() == inputp[0]) << "mismatch tensor size";
...
private:
    void InitCachedArgs() {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, argSize % 8 == 0);
...
public:
    ...
    void Launch(KernelBinary *kernel, aclrtStream aicoreStream, std::vector<DeviceTensorData> &tensors,
        ...
        ASSERT(ERROR_CODE_UNDEFINED, ret == RT_ERROR_NONE) << "launch aicpu failed: " << ret;
        ...
        ASSERT(ERROR_CODE_UNDEFINED, ret == RT_ERROR_NONE) << "launch aicore failed: " << ret;
    }
    ...
    void EmulationLaunch(KernelBinary *kernel, std::vector<DeviceTensorData> &tensors) {
        ...
        ASSERT(ERROR_CODE_UNDEFINED, ret == RT_ERROR_NONE) << "emulation run failed: " << ret;
    }
...
static int GetInputTensors(py::args &args, std::vector<DeviceTensorData> &tensors) {
    ...
    ASSERT(ERROR_CODE_UNDEFINED, tensors.size()) << "No input tensors found";
