...
public:
    ...
    void EmulationLaunch(KernelBinary* kernel, std::vector<DeviceTensorData>& tensors, uint8_t* devCache)
    {
        ...
        if (devCache != nullptr) {
            auto devProg =
                reinterpret_cast<DevAscendProgram*>(functikernel->GetFunction()on_->GetDyndevAttribute()->devProgBinary.data());
            size_t ctrlCacheSize = devProg->ctrlCacheSize;
            ...
            if (RuntimeMemcpy(hostCacheVec.data(), ctrlCacheSize, devCache, ctrlCacheSize, RtMemcpyKind::DEVICE_TO_HOST) != RT_SUCCESS) {
                COMPILER_LOGE("RuntimeMemcpy cache failed: %d", ret);
                return;
            }
...
private:
    ...
    void DoLaunch(KernelBinary* kbinary)
    {
        ...
        kmodule->EmulationLaunch(kbinary, ctrlFlowCache, tensors);
