...
private:
    ...
    std::optional<DeviceGuard> devGuard;
...
public:
    KernelLauncher(py::object& m, int64_t stream, py::sequence& torch_tensors, py::sequence& tensor_defs,
        ...
        : module(m), torchTensors(torch_tensors), tensorDefs(tensor_defs),
          aicoreStream((aclrtStream)stream), tensors(tensors_ref)
    {
        devGuard.emplace(devId);
...
private:
    ...
    void DoLaunch(KernelBinary* kbinary)
    {
        ...
        int64_t* wsAddr = wsSize ? (int64_t*)py::getattr(module, "alloc")(wsSize).cast<int64_t>() : nullptr;
...
void LaunchKernelTorch(py::object& module, int64_t stream, 
                       py::sequence& torchTensors, py::sequence& tensorDefs)
{
