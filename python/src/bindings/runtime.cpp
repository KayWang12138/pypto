...
static bool TryParseDlpack(py::object torch_tensor, uintptr_t &data_ptr, std::vector<int64_t> &shape,
    ...
    if (!to_dlpack.is_none()) {
        try {
            ...
            if (ParseDlpackCapsule(cap, data_ptr, shape, device_id)) return true;
        } catch (...) {}
    }
    if (py::hasattr(torch_tensor, "__dlpack__")) {
        try {
...
static DeviceTensorData TorchTensorToDeviceTensorData(py::object torch_tensor, py::object tensor_def,
    ...
    DataType dtype = (!base.is_none() && py::isinstance<Tensor>(base))
        ? base.cast<Tensor &>().GetDataType()
        : tensor_def.attr("dtype").cast<DataType>();
...
void LaunchKernelTorch(py::object &module, py::object stream_obj, py::sequence tensors,
    ...
    for (size_t i = 0; i < n; i++) {
        int currentDevId = -1;
        tensors_data.push_back(TorchTensorToDeviceTensorData(
            tensors[py::int_(i)], tensor_defs[py::int_(i)], &currentDevId));
        
        if (currentDevId >= 0) {
            if (devId < 0) {
                devId = currentDevId;
            } else if (currentDevId != devId) {
                throw std::runtime_error("All input tensors must be on the same device");
            }
        }
    }
    ...
    if (devId < 0) {
        throw std::runtime_error("Could not get device ID from DLPack; ensure torch tensors support DLPack");
    }
