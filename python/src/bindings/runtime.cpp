...
public:
    ...
    DevControlFlowCache* GetHostCtrlFlowCache(KernelBinary* kernel, 
        std::vector<DeviceTensorData>& tensors, uint8_t* devCache)
    {
        DevControlFlowCache* ctrlCache = FindHostCtrlFlowCache(tensors);
        if (ctrlCache == nullptr && devCache != nullptr) {
            ...
            ctrlCache = FindHostCtrlFlowCache(tensors);
        }
        return ctrlCache;
    }
    ...
    void EmulationLaunch(KernelBinary* kernel, std::vector<DeviceTensorData>& tensors, uint8_t* devCache)
    {
        ...
        DeviceLauncherConfig config;
        DeviceLauncher::DeviceLauncherConfigFillDeviceInfo(config);
        DevControlFlowCache* ctrlCache = GetHostCtrlFlowCache(kernel, tensors, devCache);
...
private:
    ...
    DevControlFlowCache* FindHostCtrlFlowCache(std::vector<DeviceTensorData>& tensors)
    {
        int64_t hash = ControlFlowCache::Hash(tensors);
        for (auto& cache : hostCtrlFlowCaches) {
            if (cache.hash == hash) {
                return reinterpret_cast<DevControlFlowCache*>(cache.hostCache.data());
            }
        }
        return nullptr;
    }
