...
public:
    ...
    ~KernelBinary() { rtDevBinaryUnRegister(kernelBin); }
...
public:
    ...
    void Launch(KernelBinary *kernel, bool isCaptureMode, aclrtStream aicoreStream, aclrtStream ctrlStream, aclrtStream schedtream,
        ...
        if (tripleStream) {
            ...
            int ret = rtAicpuKernelLaunchExWithArgs(
                rtKernelType_t::KERNEL_TYPE_AICPU_KFC, "AST_DYN_AICPU", 2, &rtAicpuArgs, nullptr, ctrlStream, 0);
            ...
            ret = rtAicpuKernelLaunchExWithArgs(
                rtKernelType_t::KERNEL_TYPE_AICPU_KFC, "AST_DYN_AICPU", 3, &rtAicpuArgs, nullptr, schedtream, 0);
            ...
            auto startTime = MsprofSysCycleTime();
            int ret = rtAicpuKernelLaunchExWithArgs(
                rtKernelType_t::KERNEL_TYPE_AICPU_KFC, "AST_DYN_AICPU", nrAicpu, &rtAicpuArgs, nullptr, schedtream, 0);
        ...
        auto startTime = MsprofSysCycleTime();
        auto ret = rtKernelLaunchWithHandleV2(
            kernel->GetKernelBin(), tilingKey, blockDim, &rtAicoreArgs, nullptr, aicoreStream, &rtTaskCfg);
...
void LaunchKernel(py::object &module, int64_t stream, py::args &args) {
    ...
    bool isCaptureMode = AttachAicpuStream(aicoreStream, ctrlStream, schedtream, kmodule->IsTripleStream());

    uint8_t *ctrlFlowCache = kmodule->FindCtrlFlowCache(kbinary, module, args, tensors);

    kmodule->Launch(kbinary, isCaptureMode, aicoreStream, ctrlStream, schedtream, tensors, ctrlFlowCache, wsAddr);
}
