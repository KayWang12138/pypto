...
void bind_controller_scope(py::module &m) {
    m.def("BeginScope",
        [](const std::string &name, const py::dict &values = py::dict(),
        const std::string &filename = __builtin_FILE(), int lineno = __builtin_LINE()) {
        ...
        py::arg("filename") = __builtin_FILE(),
        py::arg("lineno") = __builtin_LINE()
    );
    ...
    m.def("SetScope",
        [](const py::dict &values, const std::string &filename = __builtin_FILE(), int lineno = __builtin_LINE()) {
            auto cpp_values = ConvertPyDictToCppMap(values);
