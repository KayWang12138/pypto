        ...
        .def(py::init([](DataType dtype, py::object val, std::string name = "") {
            if (val.is_none()) {
                ...
                if (DTypeInfoOf(dtype).isFloat) {
                    return std::make_shared<ScalarValue>(dtype, name, ScalarValueKind::Immediate, static_cast<double>(intVal));
                } else {
                ...
                if (DTypeInfoOf(dtype).isFloat) {
                    ...
                    return std::make_shared<ScalarValue>(dtype, name, ScalarValueKind::Immediate, static_cast<int64_t>(fval));
                }
            } else {
