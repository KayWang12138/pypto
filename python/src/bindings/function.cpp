...
void BindFunction(nb::module_ &m) {
    ...
    nb::class_<Function>(m, "Function")
        ...
        .def("IsFunctionType", nb::overload_cast<FunctionType>(&Function::IsFunctionType, nb::const_), nb::arg("type"),
            "Check if function is of given type")
        .def("IsGraphType", nb::overload_cast<GraphType>(&Function::IsGraphType, nb::const_), nb::arg("type"),
            "Check if function has given graph type")
        .def("IsFunctionTypeAndGraphType",
            nb::overload_cast<FunctionType, GraphType>(&Function::IsFunctionTypeAndGraphType, nb::const_),
            nb::arg("func_type"), nb::arg("graph_type"), "Check if function has given function type and graph type")
        .def("HasParent", &Function::HasParent, "Check if function has a parent")
        .def("GetRootFunction", &Function::GetRootFunction, nb::rv_policy::reference_internal, "Get the root function")
        .def(
            "GetIncast",
            [](const Function &self) -> std::vector<Tensor> {
                return std::vector<Tensor>(self.GetIncast().begin(), self.GetIncast().end());
            },
            nb::rv_policy::reference_internal , "Get input casts")
        .def(
            "GetOutcast",
            [](const Function &self) -> std::vector<Tensor> {
                return std::vector<Tensor>(self.GetOutcast().begin(), self.GetOutcast().end());
            },
            nb::rv_policy::reference_internal, "Get output casts")
        .def(
            "GetOriginIncast",
            [](const Function &self) -> std::vector<Tensor> {
                return std::vector<Tensor>(self.GetOriginIncast().begin(), self.GetOriginIncast().end());
            },
            nb::rv_policy::reference_internal, "Get original input casts")
        .def(
            "GetOriginOutcast",
            [](const Function &self) -> std::vector<Tensor> {
                return std::vector<Tensor>(self.GetOriginOutcast().begin(), self.GetOriginOutcast().end());
            },
            nb::rv_policy::reference_internal, "Get original output casts")
        .def("DumpFile", &Function::DumpFile, nb::arg("file_path"), "Dump the function to a file")
        .def(
            "DumpJsonFile", [](Function &self, const std::string &fileName) { self.DumpJsonFile(fileName); },
            nb::arg("file_name") = "", "Dump the function to a JSON file")
