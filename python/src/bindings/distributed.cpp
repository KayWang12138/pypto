...
void BindDistributed(py::module& m) {
    ...
    m.def(
        "CreateShmemData",
        [](const char* group, int64_t worldSize, DataType dataType, const Shape& shape) {
            return Distributed::CreateShmemData(group, worldSize, dataType, shape);
        },
        py::arg("group"), py::arg("worldSize"), py::arg("dataType"), py::arg("shape"), "Create shmem data.");

    m.def(
        "CreateShmemSignal",
        [](const char* group, int64_t worldSize) {
            return Distributed::CreateShmemSignal(group, worldSize);
        },
        py::arg("group"), py::arg("worldSize"), "Create shmem signal data.");
