...
namespace pypto {
void bind_distributed(py::module &m) {
    m.def(
        "CreateShmemData",
    ...
    m.def(
        "ShmemBarrier",
        [](const Tensor &predToken, Tensor &shmemSignal, const char *group, uint32_t worldSize, Tensor &out) {
            return Distributed::ShmemBarrier(predToken, shmemSignal, group, worldSize, out);
        },
        py::arg("predToken"), py::arg("shmemSignal"), py::arg("group"), py::arg("worldSize"), py::arg("out"),
    ...
    m.def(
        "GetSymbolicScalarRankId", [](std::string group) { return GetHcclRankId(group); }, py::arg("group"),
