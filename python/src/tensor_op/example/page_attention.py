...
if __name__ == '__main__':
    ...
    q_nope = pto.tensor([b * nq * sq, dn], pto.DT_BF16, "qNope")
    k_nope_cache = pto.tensor([int(block_num * block_size), nk * dn], pto.DT_BF16, "kNopeCache")
    v_nope_cache = pto.tensor([int(block_num * block_size), nk * dn], pto.DT_BF16, "vNopeCache")
    q_rope = pto.tensor([b * nq * sq, nk * dr], pto.DT_BF16, "qRope")
    k_rope_cache = pto.tensor([int(block_num * block_size), nk * dr], pto.DT_BF16, "kRope")
    block_table = pto.tensor([b, max_block_num_per_batch], pto.DT_INT32, "blockTable")
    act_seqs = pto.tensor([b], pto.DT_INT32, "actSeqs")
    pa_out = pto.tensor([b * nq * sq, dn], pto.DT_FP32, "paOut")
