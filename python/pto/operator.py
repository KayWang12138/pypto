...
def sin(input: Tensor) -> Tensor:
    ...
    n = pto.cast(x_over_pi, pto.DT_FP32, pto.CastMode.CAST_ROUND)
    n0 = pto.mul(x_over_pi, one_over_n)
    n0 = pto.cast(n0, pto.DT_FP32, pto.CastMode.CAST_ROUND)
    ...
    n2 = pto.cast(temp, pto.DT_FP32, pto.CastMode.CAST_ROUND)
    ...
    n_half2 = pto.cast(half_n2, pto.DT_FP32, pto.CastMode.CAST_FLOOR)
    n_half4 = pto.cast(half4_n2, pto.DT_FP32, pto.CastMode.CAST_FLOOR)
...
def sigmoid(input: Tensor) -> Tensor:
    ...
    ones = pto.full(res.shape, 1.0, pto.DT_FP32, valid_shape=res.shape)
