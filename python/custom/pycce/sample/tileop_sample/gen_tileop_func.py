    ...
    for m in Range(M1, M2, BASEM):
        for n in Range(0, N, BASEN):
            ...
            with auto_sync():
                ...
                # v
                sigmoid_tileop(outbuf[cnt], xbuf[cnt], rep)
                # mte3
                ub_to_gm(z[m,n], outbuf[cnt], BASEM, BASEN//16, 0, (N-BASEN)//16)


@tileop_func()
def sigmoid_tileop(outbuf: Tensor, xbuf: Tensor, shape1: Var, rep: Union[int, Var]):
    muls(outbuf, xbuf, -1.0, rep, 1, 1, 8, 8)
    exp(outbuf, outbuf, rep, 1, 1, 8, 8)
    adds(outbuf, outbuf, 1.0, rep, 1, 1, 8, 8)
    rec(outbuf, outbuf, rep, 1, 1, 8, 8)
...
def custom_add(outbuf: Tensor, abuf: Tensor, bbuf: Tensor, shape: Var):
    set_continuous_mask(shape)
    add(outbuf, abuf, bbuf, shape, 1, 1, 1, shape // 8, shape // 8, shape // 8)

if __name__=='__main__':
    length = 32
    a = Tensor(DT.half, length)
    b = Tensor(DT.half, length)
    out = Tensor(DT.half, length, is_output = True)
    ...
    opcode = 0
    core_type = OpCoreType.CORE_AIV
    op_code_name = "CustomAdd"
    input_memory_type = [MemoryType.MEM_UB, MemoryType.MEM_UB]
    output_memory_type = [MemoryType.MEM_UB]
    tile_op_cfg = TileOpCfg("custom_add_pytileopfn", CorePipeType.PIPE_V, CorePipeType.PIPE_V, CoreType.CORE_AIV)
    op_calc_type = CalcType.CALC_TYPE_ELTWISE
    attrs = ["op_attr_input_combine_axis"]
    op_code_info = CoreOpcodeInfo(opcode, core_type, op_code_name, \
        input_memory_type, output_memory_type, tile_op_cfg, op_calc_type, attrs)
    generate_custom_op(cust_add, op_code_info)
