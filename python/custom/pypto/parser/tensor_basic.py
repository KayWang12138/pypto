...
def declare(h: CodeHelper, inst: Instruction):
    if not isinstance(inst.dst, Tensor):
        raise Exception()
    h(f'Tensor tsr{inst.dst.idx};')


def assign(h: CodeHelper, inst: Instruction):
...
def dview_pad(h: CodeHelper, inst: Instruction):
    if not isinstance(inst.src[0], Tensor):
        raise Exception()
    if not isinstance(inst.src[1], (Shape, list)):
        raise Exception()
...
def get_data_type(h: CodeHelper, inst: Instruction):
    if not isinstance(inst.src[0], Tensor):
        raise Exception()
