...
from . import cheat_detector
from .config import load_config
from .kernel_verifier import KernelVerifier
from .manager import get_worker_manager, register_local_worker
...
def _cmd_cheat_check(args: argparse.Namespace) -> int:
    ...
    if not op_dir.is_dir():
        print(f"[verifier] op_dir 不存在或非目录: {op_dir}", file=sys.stderr)
        return 2
    ...
    if args.json_out:
        ...
        print(f"[verifier] cheat-check 报告已写入: {args.json_out}")
    print(payload)
...
async def _run_verify_async(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if cheat_report.verdict == "cheat" and not args.no_cheat_gate:
        output["correctness"] = {
            "status": "skipped",
            "log_excerpt": "skipped due to cheat verdict from cheat_detector.",
        }
        output["performance"] = {
            "status": "skipped",
            "log_excerpt": "skipped due to cheat verdict from cheat_detector.",
    ...
    cheat_verdict = (output["cheat_check"] or {}).get("verdict")
    if cheat_verdict == "cheat":
        output["verdict_machine"] = "failed_cheat"
...
def _cmd_verify(args: argparse.Namespace) -> int:
    if args.mode not in _MODE_CHOICES:
        print(f"[verifier] mode 必须是 {_MODE_CHOICES}, 实际: {args.mode!r}", file=sys.stderr)
        return 2
    ...
    if args.json_out:
        args.json_out.parent.mkdir(parents=True, exist_ok=True)
        args.json_out.write_text(payload + "\n", encoding="utf-8")
        print(f"[verifier] verify 报告已写入: {args.json_out}")
    print(payload)
