...
# 用法:
#   cd <pypto repo root>
#   bash integration/akg_bench/scripts/smoke_test.sh
#
...
# ------ 4.1 imports ------
python3 -c "
from integration.akg_bench import case_loader, pypto_runner, akg_verifier_runner, run_kernelbench, report
...
rm -rf /tmp/akg_bench_smoke
python3 -m integration.akg_bench.case_loader \
    "${KB_ROOT}/level1/19_ReLU.py" \
    --write /tmp/akg_bench_smoke
echo "  produced files:"
ls /tmp/akg_bench_smoke/ReLU/ | sed 's/^/    /'
echo "  SPEC.md head:"
head -8 /tmp/akg_bench_smoke/ReLU/SPEC.md | sed 's/^/    /'
...
import os
from integration.akg_bench.run_kernelbench import discover_cases
...
# ------ 4.5 CLI --help ------
python3 -m integration.akg_bench.run_kernelbench --help > /tmp/cli_help.txt 2>&1
