...
def _render_markdown(payload: Dict[str, Any]) -> str:
    ...
    if cases:
        ...
        for c in cases:
            ...
            lines.append(
                f"| `{c['op_name']}` | `{c['case_id']}` | {badge} | {cor_cell} "
                f"| {c.get('pypto_status', '')} | {c.get('verifier_status', '')} "
        ...
        if has_perf:
            ...
            for c in cases:
                ...
                wall_sec = _wall_from_record_dict(c)
                wall_cell = f"{wall_sec:.1f}" if wall_sec is not None else "—"
                fmt_num = lambda v, suffix="", spec=".2f": (
                    f"{v:{spec}}{suffix}" if isinstance(v, (int, float)) and v is not None else "—"
                )
                lines.append(
                    f"| `{c['op_name']}` "
                    f"| {fmt_num(gen)} "
                    f"| {fmt_num(base)} "
                    f"| {fmt_num(sp, suffix='x', spec='.3f')} "
                    f"| {fmt_num(rl_t)} "
                    f"| {fmt_num(rl_sp, suffix='x', spec='.3f')} "
                    f"| {wall_cell} | {pm} |"
                )
            lines.append("")
...
def _main_cli() -> int:
    ...
    for case_dir in sorted(args.report_dir.iterdir()):
        ...
        try:
            ...
            print(f"[skip] {result_file}: {e}", file=sys.stderr)
