...
- ``verifier_mode="opencode"`` (默认):
  spawn ``opencode run --agent pypto-kernel-validator``, cwd=pypto 仓根,
  让 opencode 自动 discover ``.opencode/skills/pypto-kernel-validate``.
...
async def _run_direct(
    *,
    ...
    from integration.benchmark.verifier import (
        KernelVerifier,
        get_worker_manager,
        load_config,
    ...
    return VerifierResult(
        op_name=op_name,
        status=VerifierStatus.PASSED if success else VerifierStatus.FAILED,
        correctness=bool(success),
        log_text=log_text,
        log_file=log_file,
        duration_sec=duration,
        message="" if success else "KernelVerifier.run 返回 False, 详见日志.",
        **perf_kw,
    )
...
_PYPTO_REPO_ROOT = Path(__file__).resolve().parents[2]   # …/pypto/
