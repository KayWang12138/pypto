...
本文档说明如何为 `integration/benchmark` 增加新的 KernelBench 风格 case，并让
...
```text
integration/benchmark/testdata/KernelBench/<level>/<N>_<Name>.py
```

示例：

```text
integration/benchmark/testdata/KernelBench/level1/101_DynamicAxisAdd.py
```
...
python3 -m integration.benchmark.run_kernelbench \
  --bench-dir integration/benchmark/testdata/KernelBench \
  --level level1 \
...
```bash
python3 -m pytest integration/benchmark/tests/test_case_loader_front_matter.py
```
