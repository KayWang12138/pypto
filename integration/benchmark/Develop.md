...
本文档记录 `integration/benchmark` 的内部设计、目录结构和调试信息。运行
...
python -m integration.benchmark.run_kernelbench --cases 19_ReLU --skip-stage7-perf-tune
```
...
- **`level dir 不存在`**: 先跑 `bash integration/benchmark/scripts/download_kernelbench.sh`,
