...
实现方式：
1. torch_npu: 使用 torch_npu.npu_silu
...
def silu_golden_numpy(x: np.ndarray) -> np.ndarray:
    """
    NumPy 实现的 silu
...
def silu_golden_torch(x: torch.Tensor) -> torch.Tensor:
    """
    PyTorch 实现的 silu
    """
...
def test_bfloat16(device_id: int = None, run_mode: str = "npu"):
    """BFloat16 精度测试"""
    print("\n" + "=" * 60)
    print("Test: npu_silu BFloat16 测试")
    print("=" * 60)
    ...
    golden_result = silu_golden_torch(x_float).cpu().numpy()
...
def test_large_shape(device_id: int = None, run_mode: str = "npu"):
    """大 Shape 测试"""
    print("\n" + "=" * 60)
    print("Test: npu_silu 大 Shape 测试")
    print("=" * 60)
