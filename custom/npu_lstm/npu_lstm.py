...
def lstm_numpy(x, h, c, weight_ih, weight_hh, bias_ih, bias_hh):
    """简化的LSTM numpy实现"""
    ...
    return output.astype(np.float16), h_t.astype(np.float16), c_t.astype(np.float16)
...
def test_basic(device_id=None, run_mode="npu"):
    ...
    c = np.random.randn(1, batch_size, hidden_size).astype(np.float16)
    
    weight_ih = np.random.randn(4 * hidden_size, input_size).astype(np.float16)
    weight_hh = np.random.randn(4 * hidden_size, hidden_size).astype(np.float16)
    bias_ih = np.zeros(4 * hidden_size, dtype=np.float16)
    bias_hh = np.zeros(4 * hidden_size, dtype=np.float16)
    
    golden_y, golden_h, golden_c = lstm_numpy(x, h, c, weight_ih, weight_hh, bias_ih, bias_hh)
    print(f"[golden] y shape={golden_y.shape}, mean={golden_y.mean():.4f}")
    
    torch_npu_result = None
    if run_mode == "npu":
        import torch_npu
        print(f"[torch_npu] npu_lstm requires FRACTAL_NZ format, skipping direct comparison")
