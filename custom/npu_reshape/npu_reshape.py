...
数学公式：
    reshape(x, shape) -> 改变形状，数据不变

实现方式：
1. torch_npu: 使用 torch_npu.npu_reshape
2. golden: 使用 numpy.reshape
3. pypto: 使用 pypto.reshape
"""
...
def reshape_golden_numpy(x: np.ndarray, shape: tuple) -> np.ndarray:
...
def test_2d_to_2d(device_id=None, run_mode="npu"):
    print("=" * 60)
    print("Test: npu_reshape 2D -> 2D")
    print("=" * 60)
    ...
    np.random.seed(42)
    x_np = np.random.uniform(0, 1, in_shape).astype(np.float32)
    ...
    golden_result = reshape_golden_numpy(x_np, out_shape)
    print("[golden 实现完成]")
    
    if run_mode == "npu":
        import torch_npu
        torch_npu_out = torch_npu.npu_reshape(x_torch, out_shape)
    ...
    reshape_kernel_2d(x_torch, pypto_out)
...
def test_1d_to_2d(device_id=None, run_mode="npu"):
    print("\n" + "=" * 60)
    print("Test: npu_reshape 1D -> 2D")
    print("=" * 60)
...
def test_3d_reshape(device_id=None, run_mode="npu"):
    print("\n" + "=" * 60)
    print("Test: npu_reshape 3D -> 3D")
    print("=" * 60)
    ...
    pypto_out = torch.empty(out_shape, dtype=torch.float32, device=device)
    reshape_kernel_3d(x_torch, pypto_out)
    pypto_result = pypto_out.cpu().numpy()
    print("[pypto 实现完成]")
...
def main():
    parser = argparse.ArgumentParser(description="npu_reshape 算子测试")
    parser.add_argument('test_case', nargs='?', default='all',
                        choices=['all', '2d', '1d', '3d'])
    parser.add_argument('--run_mode', default='npu', choices=['npu'])
    
    args = parser.parse_args()
    
    print("\n" + "=" * 60)
    print("npu_reshape 算子测试")
    print("=" * 60)
