#!/usr/bin/env python3
"""Test exact dv0 pattern: matmul result * exp result * mask → a_trans matmul."""
...
print(f"exact_dv0: max_diff={diff.max():.6e}, mean={diff.mean():.6e}")
print(f"  golden: [{dv0_golden.min():.6f}, {dv0_golden.max():.6f}]")
print(f"  impl:   [{dv0_out.cpu().min():.6f}, {dv0_out.cpu().max():.6f}]")

if diff.max() > 0.01:
    print("  BUG CONFIRMED!")
    # Check: is the issue specific to matmul(a_local, doc, a_trans=True)?
    # Compare with a_local from golden
    golden_matmul = (a_local.t() @ doc) * scale
    impl_dv0 = dv0_out.cpu()
    print(f"  golden dv0 row 0: {dv0_golden[0,:5].tolist()}")
    print(f"  impl dv0 row 0:   {impl_dv0[0,:5].tolist()}")
else:
    print("  PASS")
