#!/usr/bin/env python3
"""Test dv0 and dv_state separately in minimal kernels."""
...
def compare(name, g, i):
    ...
    print(f"  {name:20s}: max_diff={d.max():.6e}, mean={d.mean():.6e}, "
          f"golden=[{g.min():.4f},{g.max():.4f}], impl=[{i.min():.4f},{i.max():.4f}]")
...
test_dv0(to_npu(qc),to_npu(kc),to_npu(doc),to_npu(gc_raw),to_npu(c_cum),to_npu(m_le),
         to_npu(torch.tensor([scale])),dv0_out)
torch_npu.npu.synchronize()
print("=== Test A: dv0 ===")
compare("dv0", dv0_g, dv0_out.cpu())

# Test B: dv_state only
@pypto.frontend.jit(runtime_options={"stitch_function_max_num": 128})
def test_dvstate(
    kc_in: pypto.Tensor([BT,KD],pypto.DT_FP32),
    ds_in: pypto.Tensor([KD,VD],pypto.DT_FP32),
    gc_in: pypto.Tensor([BT],pypto.DT_FP32),
    c_cum_in: pypto.Tensor([BT,BT],pypto.DT_FP32),
    dvs_out: pypto.Tensor([BT,VD],pypto.DT_FP32),
):
    pypto.experimental.set_operation_options(combine_axis=True)
    pypto.set_cube_tile_shapes([128,128],[128,128],[128,128])
    g_cum=pypto.matmul(c_cum_in, gc_in.reshape([BT,1]), pypto.DT_FP32)
    pypto.set_vec_tile_shapes(128,128)
    gl=g_cum[BT-1:BT,:]
    s_tok=pypto.exp(pypto.sub(gl, g_cum))
    pypto.set_cube_tile_shapes([128,128],[128,128],[128,128])
    dv_state=pypto.mul(pypto.matmul(kc_in, ds_in, pypto.DT_FP32), s_tok)
    dvs_out[:]=dv_state

dvs_out=torch.zeros(BT,VD,device=npu,dtype=torch.float32)
test_dvstate(to_npu(kc),to_npu(d_s),to_npu(gc_raw),to_npu(c_cum),dvs_out)
torch_npu.npu.synchronize()
print("\n=== Test B: dv_state ===")
compare("dv_state", dv_state_g, dvs_out.cpu())
