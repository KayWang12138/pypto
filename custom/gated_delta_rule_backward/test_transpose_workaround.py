#!/usr/bin/env python3
"""Test workaround: use .transpose(0,1) instead of a_trans=True."""
...
# Workaround: explicit transpose then matmul
...
def kern_transpose(
    ...
    # WORKAROUND: explicit transpose instead of a_trans=True
    pypto.set_vec_tile_shapes(128, 128)
    a_local_T = a_local.transpose(0, 1)
    pypto.set_cube_tile_shapes([128, 128], [128, 128], [128, 128])
    dv0 = pypto.mul(pypto.matmul(a_local_T, doc_in, pypto.DT_FP32), scale_in)
    dv0_out[:] = dv0

dv0_out = torch.zeros(BT, VD, device=npu, dtype=torch.float32)
kern_transpose(to_npu(qc), to_npu(kc), to_npu(doc), to_npu(gc_raw),
               to_npu(c_cum), to_npu(m_le), to_npu(torch.tensor([scale])), dv0_out)
torch_npu.npu.synchronize()

diff = (dv0_golden - dv0_out.cpu()).abs()
print(f"Workaround (explicit transpose): max_diff={diff.max():.6e}, mean={diff.mean():.6e}")
print(f"  golden: [{dv0_golden.min():.6f}, {dv0_golden.max():.6f}]")
print(f"  impl:   [{dv0_out.cpu().min():.6f}, {dv0_out.cpu().max():.6f}]")

if diff.max() < 0.01:
    print("  WORKAROUND EFFECTIVE!")
else:
    # Check per-row
    ...
    for i in range(BT):
        if row_diffs[i] > 0.01:
            first_bad = i
            break
    print(f"  first_bad_row={first_bad}")
    print(f"  Row 8 diff: {row_diffs[8]:.6e}")
