...
def moe_gating_topk_core(
    x,
    bias,
    config: MoEGatingTopKConfig) -> (pypto.Tensor, pypto.Tensor, pypto.Tensor):
    """
...
def moe_gating_topk_cpu(
    ...
    if config.group_count > 1:
        ...
        mask = torch.ones((x.shape[0], config.group_count), dtype=torch.bool, device='cpu')
    ...
    y *= config.routed_scaling_factor 
    y2 = original_x
    return y, indices, y2
...
def test_moe_gating_topk():
    ...
    config = get_default_moe_gating_topk_config()
    ...
    y_npu, expert_idx_npu, out_npu = moe_gating_topk(
        ...
        config=config)(x_tensor, bias_tensor)
