...
def moe_gating_topk(
    ...
    Args:
        ...
        group_select_mode: 0=amax, 1=topk2 sum
        renorm: Renormalization flag
    ...
    if k <= 0 or k > num_experts:
        raise ValueError(f"k must be in range [1, {num_experts}], got {k}")
    if k_group <= 0 or k_group > group_count:
        raise ValueError(f"k_group must be in range [1, {group_count}], got {k_group}")
    if group_count > 1 and num_experts % group_count != 0:
        raise ValueError(f"num_experts ({num_experts}) must be divisible by group_count ({group_count})")
    if group_select_mode not in [0, 1]:
        raise ValueError(f"group_select_mode must be 0 or 1, got {group_select_mode}")
    if norm_type not in [0, 1]:
...
def test_moe_gating_topk():
    """
    Test function for MOE Gating TopK operator
    Compares CPU and NPU results with tolerance
    """
