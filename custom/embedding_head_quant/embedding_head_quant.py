...
def create_embedding_head_quant_kernel(m, eps=1e-4, min_v=-128.0, max_v=127.0, run_mode="npu", unroll_list=None):
    ...
    def embedding_head_quant_kernel(
        ...
        normalized_bf16: pypto.Tensor((N, m), pypto.DT_BF16),
    ):
        n = weight.shape[0]
        ...
        for n_offset, unroll_length in pypto.loop_unroll(
            ...
            normalized_tile = pypto.cast(normalized, pypto.DT_BF16)

            pypto.assemble(output_tile, [n_offset, 0], output_bf16)
            pypto.assemble(clamped_tile, [n_offset, 0], clamped_bf16)
            pypto.assemble(normalized_tile, [n_offset, 0], normalized_bf16)
...
def test_embedding_head_quant_3b(device_id=None, run_mode="npu"):
    ...
    expected_out, clamped_out, normalized_out = embedding_head_quant_golden(weight_torch.clone(), scale_torch.clone())
    ...
    normalized_torch = torch.zeros(weight_shape, dtype=torch.bfloat16, device=device)

    kernel = create_embedding_head_quant_kernel(m, run_mode=run_mode)
    kernel(weight_torch, scale_torch, output_torch, clamped_torch, normalized_torch)
