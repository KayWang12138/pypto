...
当前实现文件为 `custom/BNTrainingReduce.py`，算子实现和测试入口都在同一个脚本中。
...
```bash
python3 custom/BNTrainingReduce.py
```

当前脚本只支持 `npu` 运行模式，参数保留为：

```bash
python3 custom/BNTrainingReduce.py --run_mode npu
```
...
对应代码位于 `custom/BNTrainingReduce.py` 中的 `assert_allclose(...)`。
