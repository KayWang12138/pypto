...
def calculate_pool2d_params(shape, kernel_size, stride=None, padding_mode=None):
    """
    Calculate pooling parameters including output dimensions and padding values.
    
    Args:
        shape: Input tensor shape (batch_size, channels, in_h, in_w)
        kernel_size: Kernel size (k_h, k_w)
        stride: Stride (s_h, s_w), defaults to kernel_size if None
        padding_mode: 'VALID' or 'SAME'
    
    Returns:
        tuple: (batch_size, channels, in_h, in_w, k_h, k_w, s_h, s_w, 
                t_pad, b_pad, l_pad, r_pad, out_h, out_w)
    ...
    return (batch_size, channels, in_h, in_w, k_h, k_w, s_h, s_w, 
            t_pad, b_pad, l_pad, r_pad, out_h, out_w)


def avg_pool_2d(shape, kernel_size, stride=None, padding_mode=None, run_mode="npu", dynamic=True):
    batch_size, channels, in_h, in_w, k_h, k_w, s_h, s_w, t_pad, b_pad, l_pad, r_pad, out_h, out_w = \
        calculate_pool2d_params(shape, kernel_size, stride, padding_mode)
...
def test_avg_pool_2d_single(shape, kernel_size, stride, padding_mode, device_id=None, run_mode="npu", dynamic=True):
    """
    ...
    Args:
        ...
        padding_mode: 'VALID' or 'SAME'
        device_id: Device ID for NPU
        run_mode: 'npu' or 'sim'
        dynamic: Whether to use dynamic shape
    """
    ...
    print(f"✓ test_avg_pool_2d ({padding_mode}) passed: shape={shape}, kernel={kernel_size}, stride={stride}")
