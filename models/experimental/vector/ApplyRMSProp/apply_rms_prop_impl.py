...
def apply_rms_prop_kernel(
    ...
    var_out: pypto.Tensor([], pypto.DT_FP32),
    ms_out: pypto.Tensor([], pypto.DT_FP32),
    mom_out: pypto.Tensor([], pypto.DT_FP32),
    lr: float = 0.001,
    rho: float = 0.9,
    momentum: float = 0.9,
    epsilon: float = 1e-7,
    ...
    grad_sq = grad * grad
    ms_new = ms + (grad_sq - ms) * (1.0 - rho)
    mom_new = mom * momentum + (grad * lr) / pypto.sqrt(ms_new + epsilon)
    var_new = var - mom_new

    var_out.move(var_new)
    ms_out.move(ms_new)
    mom_out.move(mom_new)
