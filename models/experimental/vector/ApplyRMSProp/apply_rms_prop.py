...
def prepare_device_id(run_mode: str) -> int | None:
    ...
    if "TILE_FWK_DEVICE_ID" not in os.environ:
        LOGGER.error("ERROR: TILE_FWK_DEVICE_ID not set")
        LOGGER.error("Please set it before running:")
        LOGGER.error("  export TILE_FWK_DEVICE_ID=0")
        raise SystemExit(1)
    if importlib.util.find_spec("torch_npu") is None:
        raise RuntimeError("torch_npu is required when run_mode is npu.")
    importlib.import_module("torch_npu")
    try:
        device_id = int(os.environ["TILE_FWK_DEVICE_ID"])
    except ValueError as exc:
        raise RuntimeError("TILE_FWK_DEVICE_ID must be an integer.") from exc
