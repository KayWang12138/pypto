...
def check_min_max(param_name, param_value, min_max):
    ...
    if param_value < min_value or param_value > max_value:
        raise ValueError(
            f"{param_name} must be greater than or equal to {min_value} and less than or equal to {max_value}, but got {param_value}."
        )
...
def check_key_value_shape(inputs):
    ...
    if key_shape != value_shape:
        raise ValueError(F"key and value shape must be equal, but got key shape is {key_shape} and value shape is {value_shape}")
