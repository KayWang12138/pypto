...
def gen_mxfp8(inputs: GroupedMatmulInputs, tile_config: ShapeConfig):
    ...
    scaled_matmul_kernel(*pto_inputs, *pto_outputs, inputs.group_list, tile_config)
...
def test_gmm_mxfp8(params: TestParams):
    ...
    if params.transpose.b_trans:
        b = torch.randn((len(params.group_list), params.n, params.k), dtype=torch.float32).uniform_(0, 1).to(torch.float8_e4m3fn)
    else:
        b = torch.randn((len(params.group_list), params.k, params.n), dtype=torch.float32).uniform_(0, 1).to(torch.float8_e4m3fn)
