...
def flash_attention_varlen_backward_kernel(
    ...
    num_heads = q.shape[1]   # num_heads (静态)
    head_dim = q.shape[2]   # head_dim  (静态)
    hidden_dim = num_heads * head_dim       # hidden_dim
            ...
            for q_tile_idx in pypto.loop(num_q_tiles, name="q_tile_loop"):
                ...
                for kv_tile_idx in pypto.loop(num_kv_tiles, name="kv_tile_loop"):
                    ...
                    for h_s_idx in range(2):
                        ...
                        qi = pypto.view(q_2d, [S2_TILE, head_dim], [q_start + q_ofs, h_ofs], valid_shape=[sq, head_dim])
                        oi = pypto.view(o_2d, [S2_TILE, head_dim], [q_start + q_ofs, h_ofs], valid_shape=[sq, head_dim])
                        doi = pypto.view(do_2d, [S2_TILE, head_dim], [q_start + q_ofs, h_ofs], valid_shape=[sq, head_dim])
                        ...
                        ki_tile = pypto.view(k_2d, [S2_TILE, head_dim], [kv_start + s2_ofs, h_ofs], valid_shape=[s2, head_dim])
                        vi_tile = pypto.view(v_2d, [S2_TILE, head_dim], [kv_start + s2_ofs, h_ofs], valid_shape=[s2, head_dim])
