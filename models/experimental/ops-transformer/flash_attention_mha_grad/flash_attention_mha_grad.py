...
def flash_attention_varlen_backward_kernel(
    ...
    N = q.shape[1]   # num_heads (静态)
    D = q.shape[2]   # head_dim  (静态)
    ND = N * D       # hidden_dim
    bs = q.shape[0]  # batch_size (动态)
    scale = 1.0 / (D ** 0.5)

    # reshape inplace: q/k/v/o/do [bs, N, D] → [bs, N*D]
    q_2d = pypto.reshape(q, [bs, ND], inplace=True)
    k_2d = pypto.reshape(k, [bs, ND], inplace=True)
    v_2d = pypto.reshape(v, [bs, ND], inplace=True)
    o_2d = pypto.reshape(o, [bs, ND], inplace=True)
    do_2d = pypto.reshape(do, [bs, ND], inplace=True)
    # reshape inplace: l_input/m_input [bs, N, 1] → [bs, N]
    l_input_2d = pypto.reshape(l_input, [bs, N], inplace=True)
    m_input_2d = pypto.reshape(m_input, [bs, N], inplace=True)
    ...
    for b_idx in pypto.loop(bs, name="batch_loop", parallel=True):
        ...
        NUM_LOOP = N // 2
        for h_idx in pypto.loop(NUM_LOOP, name="head_loop"):
            ...
            for q_tile_idx in pypto.loop(num_q_tiles, name="q_tile_loop"):
                ...
                # dQ 累加器 (FP32), shape=[S2_TILE, D], 跨 kv_tile 累加
                dq_update = pypto.tensor([S2_TILE, D], pypto.DT_FP32, "dq_update")

                for kv_tile_idx in pypto.loop(num_kv_tiles, name="kv_tile_loop"):

                    for h_s_idx in range(2):
                        h_ofs = (h_idx * 2 + h_s_idx) * D
                        # L/M reshape 后为 [bs, N], head 索引不乘 D
                        h_idx_lm = h_idx * 2 + h_s_idx

                        # ---- Q 侧: 加载当前 Q tile 的 Q/O/dO/M/L ----
                        # q/o/do reshape 后为 [bs, N*D], 静态 shape=[S2_TILE, D]
                        qi = pypto.view(q_2d, [S2_TILE, D], [q_start + q_ofs, h_ofs], valid_shape=[sq, D])
                        oi = pypto.view(o_2d, [S2_TILE, D], [q_start + q_ofs, h_ofs], valid_shape=[sq, D])
                        doi = pypto.view(do_2d, [S2_TILE, D], [q_start + q_ofs, h_ofs], valid_shape=[sq, D])
                        # l_input/m_input reshape 后为 [bs, N], 每个 head 1 个值
                        m_i = pypto.view(m_input_2d, [S2_TILE, 1], [q_start + q_ofs, h_idx_lm], valid_shape=[sq, 1])
                        l_i = pypto.view(l_input_2d, [S2_TILE, 1], [q_start + q_ofs, h_idx_lm], valid_shape=[sq, 1])
                        ...
                        ki_tile = pypto.view(k_2d, [S2_TILE, D], [kv_start + s2_ofs, h_ofs], valid_shape=[s2, D])
                        vi_tile = pypto.view(v_2d, [S2_TILE, D], [kv_start + s2_ofs, h_ofs], valid_shape=[s2, D])

                        # D = sum(O * dO, dim=-1, keepdim=True) -> [sq, 1]
                        ...
                        # dP_tile = dO_tile @ V_tile^T -> [sq, s2]
                        # dtype: BF16 matmul → FP32
                        ...
                        # P_tile = exp(S * scale - M) / L  (softmax)
                        # dtype: FP32 全程
                        ...
                        # dtype: FP32 → cast → BF16 (用于后续 matmul 输入)
                        ds_half = pypto.cast(ds, pypto.DT_BF16)
                        p_half = pypto.cast(p, pypto.DT_BF16)
                        pypto.set_pass_options(sg_set_scope=-1)

                        # dK_tile += dS^T @ Q_tile * scale -> [s2, D]
                        # dtype: BF16 matmul → FP32, mul(scale), cast → BF16
                        ...
                        # dV_tile += P^T @ dO_tile -> [s2, D]
                        # dtype: BF16 matmul → BF16 (直接输出 BF16)
                        pypto.set_cube_tile_shapes([128, 512], [256, 512], [64, 64])
                        ...
                        # dQ_partial = dS @ K_tile * scale -> [sq, D]
                        # dtype: BF16 matmul → FP32, mul(scale)
                        pypto.set_cube_tile_shapes([128, 512], [256, 512], [64, 64])
...
def create_inputs(batch_size, s1_size, s2_size, num_heads, head_dim, device):
    ...
    # actual_q: [s1_size, s1_size, ...] — shape=[batch_size], Q 每个 batch 的 seqlen
    q_seqlens = [s1_size] * batch_size
    actual_q = torch.tensor(
        q_seqlens, dtype=torch.int32, device=device
    )
    # actual_kv: [s2_size, s2_size, ...] — shape=[batch_size], KV 每个 batch 的 seqlen
...
def attention_backward_golden(q, k, v, o_input, do_t, scale):
    ...
    """
    # scores = Q(BF16) @ K^T(BF16) → FP32 (kernel: matmul out_dtype=FP32)
    scores = torch.matmul(q.float(), k.float().T) * scale
    # P = softmax (kernel: exp(S*scale - M) / L, 全程 FP32)
    p = torch.softmax(scores, dim=-1)

    # ---- D = sum(O_fp32 * dO_fp32, dim=-1) ----
    ...
    # ---- dK = ds_half^T(BF16) @ Q(BF16) → FP32 * scale → BF16 ----
    # kernel: matmul(ds_half, qi, out_dtype=FP32, a_trans=True) 即 ds_half^T @ Q
    # ds_half: [s1, s2], Q: [s1, D] → ds_half^T: [s2, s1] @ Q: [s1, D] → [s2, D]
    dk_fp32 = torch.matmul(ds_half.float().T, q.float()) * scale
    dk = dk_fp32.to(torch.bfloat16)

    # ---- dV = p_half^T(BF16) @ dO(BF16) → BF16 ----
    # kernel: matmul(p_half, doi, out_dtype=BF16, a_trans=True) 即 p_half^T @ dO
    # p_half: [s1, s2], dO: [s1, D] → p_half^T: [s2, s1] @ dO: [s1, D] → [s2, D]
    # 注: 模拟 BF16 matmul — 先 FP32 计算再 cast BF16
    dv = torch.matmul(p_half.float().T, do_t.float()).to(torch.bfloat16)

    # ---- dQ = ds_half(BF16) @ K(BF16) → FP32 * scale → BF16 ----
    # kernel: matmul(ds_half, ki_tile, out_dtype=FP32) → mul(scale) → 累加(FP32) → cast(BF16)
...
def run_test(device, batch_size=None, num_heads=None, s1_size=None,
    ...
    print("=" * 60)
    print(f"Test Case: batch={batch_size}, heads={num_heads}, "
          f"Q:s1_size={s1_size}, KV:s2_size={s2_size}, dim={dim}")
    print(f"  hidden_dim={hidden_dim}, scale={scale:.6f}, "
    ...
    for name, npu_tensor, golden_tensor in [
        ...
        try:
            ...
            print(f"  {name}: PASSED (max_diff={max_diff:.6f}, rtol={rtol}, atol={atol})")
        except AssertionError as e:
            print(f"  {name}: FAILED (max_diff={max_diff:.6f}, rtol={rtol}, atol={atol})")
            print(f"    {e}")
            passed = False
...
def test_02(device):
    """batch=8, heads=8, s1=2432, s2=2432, dim=64"""
...
def main():
    ...
    """
    print("\n" + "=" * 60)
    print("Flash Attention Backward (3-loop, KV tiling)")
    print("=" * 60 + "\n")
    ...
    test_funcs = [
        ...
        # test_02,    # batch=8, heads=8, s1=2432, s2=2432, dim=64
    ...
    for i, fn in enumerate(test_funcs):
        print(f"[{i+1}/{len(test_funcs)}] {fn.__name__}: {fn.__doc__}")
        try:
            passed = fn(device)
