...
def fused_swiglu_fwd_kernel(
    ...
    b_g_reshape = pypto.reshape(b_g_fp32, [1, N], inplace=True)
    b_fc_reshape = pypto.reshape(b_fc_fp32, [1, N], inplace=True)
    ...
    for idx in pypto.loop(loop_count, name="LOOP_BWD_DW", idx_name="idx"):
        ...
        g_bias = pypto.matmul(x_tile, w_g, pypto.DT_FP32, extend_params={'bias_tensor': b_g_reshape})
        fc_bias = pypto.matmul(x_tile, w_fc, pypto.DT_FP32, extend_params={'bias_tensor': b_fc_reshape})
        pypto.set_vec_tile_shapes(128, 128)
