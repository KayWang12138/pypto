...
def run_test(device, batch_size=None, num_heads=None, s1_size=None,
    ...
    m_golden = torch.empty(total_q * num_heads, 1, dtype=torch.float32)
    l_golden = torch.empty(total_q * num_heads, 1, dtype=torch.float32)

    q_off, k_off = 0, 0
    print(f"zjr123 --- s1_size {s1_size}")
    for b in range(batch_size):
        sq, sk = q_seqlens[b], kv_seqlens[b]
        print(f"zjr123 --- sq {sq}")
        ...
        for h in range(num_heads):
            ...
            idx_base = cu_seqlens_q[b] * num_heads + h * s1_size
    
            print(f"zjr123 --- idx_base {idx_base}")

            m_golden[idx_base:idx_base + sq, :] = golden_m
            l_golden[idx_base:idx_base + sq, :] = golden_l
        q_off += sq
        k_off += sk

    # out_golden, m_golden, l_golden = attention_forward_golden(q, k, v, scale)

    # print(f"zjr123 ---- out_golden： {out_golden}")
    # ---- 调用 kernel ----
    ...
    for name, npu_tensor, golden_tensor, rtol, atol in [
        ("O", out, out_golden, 0.0078125, 0.0001),
        ("M", m_out, m_golden, 0.005, 0.000025),
        ("L", l_out, l_golden, 0.005, 0.000025),
    ]:
        ...
        # if name == "O":
        #     print(f"zjr123 --- golden_np: {golden_np}")

        # print(f"name： {name}")
        # print(f"")
        # print(f"zjr123 --- golden_np: {golden_np}")
        # print(f"")
        # print(f"zjr123 --- npu_tensor: {npu_tensor}")
...
def test_02(device):
    ...
    return run_test(device, batch_size=1, num_heads=2, s1_size=1024, s2_size=1024, dim=64)
