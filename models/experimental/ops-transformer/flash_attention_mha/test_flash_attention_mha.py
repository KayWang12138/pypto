...
def attention_forward_golden(q, k, v, scale):
    ...
    """
    scores = torch.matmul(q.cpu().float(), k.cpu().float().T).float() * scale
    ...
    p_norm = p_unnorm / l
    o = torch.matmul(p_norm, v.cpu().float()).bfloat16()
...
def run_test(device, batch_size=None, num_heads=None, s1_size=None,
    ...
    m_golden = torch.empty(total_q, 1, dtype=torch.float32)
    l_golden = torch.empty(total_q, 1, dtype=torch.float32)
