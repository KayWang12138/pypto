...
def flash_attention_varlen_forward_kernel(
    ...
    # Q侧: softmax中间量L, shape=[total_q * n, STATIC] (二维)
    l_output: pypto.Tensor([pypto.DYNAMIC, pypto.STATIC], pypto.DT_FP32),
    # Q侧: softmax中间量M, shape=[total_q * n, STATIC] (二维)
    ...
    for b_idx in pypto.loop(batch_size, name="batch_loop"):
        ...
        batch_base = cu_seqlens_q[b_idx] * num_heads

        for h_idx in pypto.loop(num_heads, name="head_loop"):
            h_offset = h_idx * head_dim

            row_base = batch_base + h_idx * seq_len_q

            # for q_tile_idx in pypto.loop(q_tile_count, name="q_tile_loop", submit_before_loop = True):
            for q_tile_idx in pypto.loop(q_tile_count, name="q_tile_loop"):
                ...
                for k_tile_idx in pypto.loop(k_tile_count, name="k_tile_loop"):
                    ...
                    k_tile_view = pypto.view(k_2d, [k_tile_size, head_dim],
                                        [k_start + k_tile_start, h_offset],
                                        valid_shape=[k_tile_len, head_dim])
                    v_tile_view = pypto.view(v_2d, [k_tile_size, head_dim],
                                        [k_start + k_tile_start, h_offset],
                                        valid_shape=[k_tile_len, head_dim])
                    ...
                    if pypto.is_loop_begin(k_tile_idx):
                        if pypto.is_loop_end(k_tile_idx):
                            ...
                            # row_base = b_idx * seq_len_q * num_heads + h_idx * seq_len_q
                            # row_base = cu_seqlens_q[b_idx] * num_heads + h_idx * seq_len_q
                            pypto.assemble(oij, [q_start + q_tile_start, h_offset], output)
                            pypto.assemble(lij, [row_base + q_tile_start, 0], l_output)
                            pypto.assemble(mij, [row_base + q_tile_start, 0], m_output)
                        else:
                            pypto.set_vec_tile_shapes(v1_tile[0], v1_tile[1])
                        ...
                        pypto.assemble(li_new, [row_base + q_tile_start, 0], l_output)
                        pypto.assemble(mi_new, [row_base + q_tile_start, 0], m_output)
                    else:
                        pypto.set_vec_tile_shapes(v1_tile[0], v1_tile[1])
