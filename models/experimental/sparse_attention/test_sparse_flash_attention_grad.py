...
def gen_test_data_tnd(actual_q_lens, actual_kv_lens, N1, N2, D, DR, K,
    ...
    # q_nope = gen_uniform_data([T1, N1, D], -1, 1, dtype)
    # q_pe = gen_uniform_data([T1, N1, DR], -1, 1, dtype)
    # k_nope = gen_uniform_data([T2, N2, D], -1, 1, dtype)
    # k_pe = gen_uniform_data([T2, N2, DR], -1, 1, dtype)
    # value = gen_uniform_data([T2, N2, D], -1, 1, dtype)
    # d_out = gen_uniform_data([T1, N1, D], -1, 1, dtype)
    ...
    print("sfa_forward_golden_tnd success!!")
...
def test_sfa_grad_npu_eager(case_name, actual_q_lens, actual_kv_lens,
    ...
    # print(dq_nope_out_npu.cpu())
    # print(dq_pe_out_npu.cpu())
    # print(dk_nope_out_npu.cpu())
    # print(dk_pe_out_npu.cpu())
    # print(dv_out_npu.cpu())
    ...
    compare(dk_pe_out_npu.cpu(), data['dk_pe_golden'], "dK_pe",
            atol=0.0001, rtol=0.0078125, max_error_count=100)
    # compare(dv_out_npu.cpu(), data['dv_golden'], "dV",
    #         atol=0.0001, rtol=0.0078125, max_error_count=100)
...
def test_sfa_grad_npu(case_name, actual_q_lens, actual_kv_lens,
    ...
    print("max min", dk_nope_out_slice.flatten().max(), dk_nope_out_slice.flatten().min())
    ...
    compare(dk_pe_out_slice.to(dtype), data['dk_pe_golden'].reshape(T2*N2, DR), "dK_pe",
            atol=0.0001, rtol=0.0078125, max_error_count=100)
    # compare(dv_out_npu.cpu().to(dtype), data['dv_golden'].reshape(T2*N2, D), "dV",
    #         atol=0.0001, rtol=0.0078125, max_error_count=100)
