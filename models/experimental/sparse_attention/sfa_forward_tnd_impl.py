...
def sfa_forward_tnd_compute(q_nope, compressed_kv_norm, topk_indices,
    ...
    # pypto.set_vec_tile_shapes(128, 576)
        ...
        # topk_2d = pypto.reshape(topk_indices, [T1_sym * n_kv, sparse_size], inplace=True)
    ...
    out_2d = pypto.tensor([T1_sym * Nq, dn], dtype=pypto.DT_BF16)
    ...
    for batch_idx in pypto.loop(0, B_sym, 1, name="LOOP_B", idx_name="bIdx"):
        ...
        for s_idx in pypto.loop(0, s_per_batch, 1, name="LOOP_S", idx_name="sIdx"):
            ...
            for kv_head_idx in range(n_kv):
                # if pypto.cond(eff_topk_cond <= 1024):
                #     kv_sel = pypto.view(kv_2d_dyn, [1024, dn], [prev_kv_prefix, 0],
                #                 valid_shape=[eff_topk, dn])
                #     # pypto.set_vec_tile_shapes(128, dr)
                #     k_pe_sel = pypto.view(k_pe_2d_dyn, [1024, dr], [prev_kv_prefix, 0],
                ...
                if pypto.cond(eff_topk_cond <= sparse_size):
                    ...
                    # cur_topk_view = pypto.view(topk_2d, [1, s2_tile], [topk_row, 0],
                    #                            valid_shape=[1, eff_topk])
                    cur_topk_1d = pypto.view(topk_1d, [s2_tile], [topk_row * sparse_size],
                                            valid_shape=[eff_topk])
                    # pypto.set_vec_tile_shapes(1, s2_tile)
                    # cur_topk_1d = pypto.reshape(cur_topk_view, [s2_tile], valid_shape=[eff_topk])
                ...
                # pypto.assemble(out, [t_idx * Nq + kv_head_idx * group, 0], out_2d)
                # core_attn_out[:] = pypto.reshape(out_2d, [core_attn_out.shape[0],
                #                                 core_attn_out.shape[1], 
                #                                 core_attn_out.shape[2]], inplace=True)
                pypto.set_vec_tile_shapes(group, dn)
                out_3d = pypto.reshape(out, [1, group, dn])
                # pypto.set_vec_tile_shapes(1, group, dn)
