...
@dataclass
class MatmulAllreduceKernelInputs:
    in_tensor: pypto.Tensor
    matmul_weight: pypto.Tensor
    residual: pypto.Tensor
    gamma: pypto.Tensor
    bias: pypto.Tensor
    out_tensor: pypto.Tensor
    residual_out: pypto.Tensor
    eps: float
    group_name: str
    world_size: int


def _run_matmul_allreduce_kernel(inputs: MatmulAllreduceKernelInputs):  # noqa: C901
    batch_size = inputs.in_tensor.shape[0]
    hidden_size = inputs.matmul_weight.shape[0]
    ...
    gamma_2d = pypto.reshape(inputs.gamma, [1, hidden_size], inplace=True)
    bias_2d = pypto.reshape(inputs.bias, [1, hidden_size], inplace=True)
    ...
    for bs_idx in pypto.loop(bs_loop, name="LOOP_MM_ALLREDUCE_ADD_RMSNORM", idx_name="bs_idx"):
        ...
        shmem_tensor = pypto.distributed.create_shmem_tensor(
            inputs.group_name, inputs.world_size, pypto.DT_FP32, shmem_shape)
        shmem_barrier_signal = pypto.distributed.create_shmem_signal(inputs.group_name, inputs.world_size)
        my_pe = pypto.distributed.my_symbolic_pe(inputs.group_name)
        for _ in pypto.loop(1, name="LOOP_MM_AR_ARMS_L0", idx_name="_"):
            in_tensor_tile = pypto.view(
                inputs.in_tensor, (view_row_shape, inputs.in_tensor.shape[1]), [bs_idx * view_row_shape, 0],
                valid_shape=[(batch_size - bs_idx * view_row_shape).min(view_row_shape), inputs.in_tensor.shape[1]])
            ...
            matmul_result = pypto.matmul(in_tensor_tile, inputs.matmul_weight, pypto.DT_FP32, b_trans=True)
            ...
            wait_until_out = pypto.distributed.shmem_wait_until(shmem_tensor, my_pe, inputs.world_size,
            ...
            residual_tile = pypto.view(
                inputs.residual, (view_row_shape, hidden_size), [bs_idx * view_row_shape, 0],
                valid_shape=[(batch_size - bs_idx * view_row_shape).min(view_row_shape), hidden_size])
            ...
            inputs.residual_out[bs_idx * pypto.symbolic_scalar(view_row_shape):] = residual_bf16_tmp
            inputs.out_tensor[bs_idx * pypto.symbolic_scalar(view_row_shape):] = hidden_bf16
...
def matmul_allreduce_add_rmsnorm_kernel(  # noqa: PLR0913, C901
    ...
    inputs = MatmulAllreduceKernelInputs(
        in_tensor=in_tensor,
        matmul_weight=matmul_weight,
        residual=residual,
        gamma=gamma,
        bias=bias,
    ...
    _run_matmul_allreduce_kernel(inputs)
...
def test_matmul_allreduce_add_rmsnorm_performance():
    logger.info("=" * 60)
    logger.info("开始运行matmul_allreduce性能测试")
    logger.info("=" * 60)
