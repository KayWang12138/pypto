...
def mla_prolog_quant_compute(
    ...
    for bs_offset, unroll_length in pypto.loop_unroll(0, t, 1, name="MLA_BS_LOOP", idx_name="bs_offset",
        ...
        vec_tile_bs = max(16, tile_bs)
        pypto.set_vec_tile_shapes(vec_tile_bs, 128)
        ...
        q_norm = q_kv[2]
        pypto.set_vec_tile_shapes(vec_tile_bs, q_lora_rank)
        pypto.assemble(q_norm, [bs_offset, 0], q_norm_out)
        q_norm_scale = q_kv[3]
        pypto.set_vec_tile_shapes(vec_tile_bs, 16)
        ...
        tile_shape = [max(16, min(16, tile_bs)), 32, qk_nope_head_dim]
        ...
        tile_shape = [16, max(16, min(32, tile_bs)), kv_lora_rank]
        pypto.set_vec_tile_shapes(*tile_shape)
        ...
        quant_tile0 = max(16, tile_bs)
        pypto.set_vec_tile_shapes(quant_tile0, kv_lora_rank)
        k_nope_split = pypto.reshape(k_nope, [tile_bs, 4, kv_lora_rank // 4])
        pypto.set_vec_tile_shapes(quant_tile0, 16, kv_lora_rank // 4)
