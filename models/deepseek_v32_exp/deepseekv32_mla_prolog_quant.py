...
def mla_prolog_quant_v32(params, input_tensors, golden_data, dtype, w_dtype, is_quant_a, \
    ...
    if is_p:
        ...
        if h <= 64:
            rope_tile_shape = RopeTileShapeConfig(two_dim=[16, 16], three_dim=[16, 16, 16], four_dim=[16, 16, 16, 16])
        else:
        ...
        from mla_prolog_quant_impl import RopeTileShapeConfig
        if h <= 64:
            rope_tile_shape = RopeTileShapeConfig(two_dim=[16, 16],
                three_dim=[16, 16, 16], four_dim=[16, 16, 16, 16])
        else:
...
def test_b4_s64k2_pa_nd_bf16_quantb_d():
    ...
    原始参数 -> 简化参数:
      ...
      - n1: 128 -> 2
      - h: 7168 -> 64
      - q_lora_rank: 1536 -> 64 (>=16, 且 /4 >=16)
      - kv_lora_rank: 512 -> 64 (>=16, 且 /4 >=16)
      - qk_nope_head_dim: 128 -> 16
      - qk_rope_head_dim: 64 -> 16
      - block_size: 128 -> 16
    
    注意: kv_lora_rank=64 以满足 quant split (kv_lora_rank // 4 = 16 >= 16)
    '''
    prep_env()
    params = {
        'b': 1,
...
dtype = pypto.DT_BF16
    ...
    tile_config.pre_quant_cube_tile = [16, 16, 64, 64, 64, 64]
    tile_config.mv_tile = mv_tile_value
    tile_config.q_vec_tile0 = 16
    tile_config.q_vec_tile1 = 16
    tile_config.k_vec_tile0 = 16
    tile_config.k_vec_tile1 = 64
    tile_config.unroll_list = [1]

    actual_seq = torch.tensor([params["s2"]] * params["b"], dtype=torch.int32).unsqueeze(-1)
    input_tensors, golden_data = gen_mla_prolog_quant_v32_data(params, (torch.bfloat16, torch.bfloat16), actual_seq, \
                    (is_quant_a, is_quant_b), False, 16, "PA_BSND")
    mla_prolog_quant_v32(params, input_tensors, golden_data, dtype, w_dtype, \
