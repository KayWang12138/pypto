...
def get_pfa_config_1(device="cpu"):
    """PFA (Prefill) 配置 - s1 >= 1"""
    b = 8
...
def pfa_func(q_shape, kv_shape, block_table_shape, causal_table_shape):
    ...
    def pfa_func_kernel(
        ...
        for b_idx in pypto.loop(b_scalar, name="LOOP_b", idx_name="b_idx", unroll_list=[32,16,4]):
            for s1_idx in pypto.loop(s1_scalar, name="LOOP_s1", idx_name="s1_idx", unroll_list=[64,16]):
                ...
                for n2_idx in pypto.loop(n2_sym, name="LOOP_n2", idx_name="n2_idx"):
                    for g_idx in pypto.loop(g_loop, name="LOOP_g", idx_name="g_idx"):
                        ...
                        for s2_idx in pypto.loop(s2_loop, name="LOOP_s2", idx_name="s2_idx", unroll_list=[128, 64]): # symbolicscalar
                            idx = s2_idx * block_num
                            actual_s2_tile = (cur_seq - s2_idx * s2_tile).min(s2_tile)
...
def run_pfa_test(atten_cfg):
    ...
    causal_table = create_causal_mask(s1, block_size)
