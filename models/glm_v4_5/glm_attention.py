...
def get_qwen_common_config(device="cpu", 950_flag=0):
    b = 8
    s1 = 1
    s2 = 16384
    q_d = 128
...
def ifa_func_kernel(
    ...
    atten_cfg, tile_cfg = get_qwen_common_config(device="cpu", 950_flag=0)
...
def ifa_func_kernel_950(
    ...
    atten_cfg, tile_cfg = get_qwen_common_config(device="cpu", 950_flag=1)
    softmax_scale = atten_cfg.softmax_scale
...
def test_ifa_950():
    ...
    device_id = os.environ.get('TILE_FWK_DEVICE_ID', 0)
    device = f'npu:{device_id}'
    atten_cfg, _ = get_qwen_common_config(device=device, 950_flag=1)
...
def test_ifa():
    ...
    device_id = os.environ.get('TILE_FWK_DEVICE_ID', 0)
    device = f'npu:{device_id}'
    atten_cfg, _ = get_qwen_common_config(device=device, 950_flag=0)
