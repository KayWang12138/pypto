                            ...
                            for s2_idx in pypto.loop(s2_loop, name="LOOP_s2", idx_name="s2_idx",
                                ...
                                if pypto.is_loop_begin(s2_idx):
                                    ...
                                    if pypto.is_loop_end(s2_idx):
                                        ...
                                        oi_update[:] = oi_tmp #vec2Res
                                        sum_update[:] = sum_local  # sum
                                        max_update[:] = amax_ij  # amax
                                    pypto.set_pass_options(sg_set_scope=-1)
                                else:
                                    pypto.set_pass_options(sg_set_scope=1)
                                    ...
                                    t2 = pypto.exp(t1)
                                    t6 = pypto.mul(t2, sum_update) # 更新全局sum
                                    t3 = pypto.sub(amax_ij, max_new)
                                    t4 = pypto.exp(t3)
                                    t5 = pypto.mul(t4, sum_local) # 更新局部sum
                                    sum_new = pypto.add(t6, t5)   # 计算新sum
                                    sum_update[:] = sum_new
                                    max_update[:] = max_new

                                    pypto.set_vec_tile_shapes(v2_tile[0], 128)
                                    oi_last = pypto.mul(oi_update, t2) # 更新oi_last
                                    oi_flash = pypto.mul(mm2_res, t4) # 更新本轮oi
                                    oi_tmp = pypto.add(oi_last, oi_flash) # 计算新oi
                                    if pypto.is_loop_end(s2_idx):
                                        pypto.set_vec_tile_shapes(16, v2_tile[0], v2_tile[1])
