...
def _get_shape_config():
    atten_cfg, _ = get_qwen_common_config()

    q_shape = (pypto.frontend.dynamic("q_shape"), atten_cfg.n1, atten_cfg.q_d)
    kv_shape = (pypto.frontend.dynamic("kv_shape"), atten_cfg.block_size,
                atten_cfg.n2, atten_cfg.kv_d)
    block_table_shape = (atten_cfg.block_table_batch,
                         atten_cfg.max_num_blocks_per_query)

    bs = pypto.frontend.dynamic("bs")

    return q_shape, kv_shape, block_table_shape, bs


Q_SHAPE, KV_SHAPE, BLOCK_TABLE_SHAPE, BS = _get_shape_config()
