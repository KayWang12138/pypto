...
def moe_distributed_dispatch_kernel(
    ...
    def kernel(
        ...
        expand_x: pypto.Tensor([expand_x_row, hidden_size], data_type),
...
def moe_distributed_dispatch(moe_case: MoeCase, operands: MoeDispatchOperands, logical_rank_id: int) -> None:
    ...
    expand_x_actual = torch.empty_like(expand_x_golden).to(f'npu:{physical_device_id}')
    assist_info_for_combine_actual = torch.empty_like(assist_info_for_combine_golden).to(f'npu:{physical_device_id}')
    expert_token_nums_actual = torch.empty_like(expert_token_nums_golden).to(f'npu:{physical_device_id}')
    recv_counts_actual = torch.empty_like(recv_counts_golden).to(f'npu:{physical_device_id}')
...
def moe_distributed_combine(
    ...
    out = torch.empty_like(out_golden).to(f'npu:{physical_device_id}')
