...
def moe_distributed_dispatch(
    ...
    logical_rank_id = logical_rank_id + PHYSICAL_START_DEVICE_ID
    x = x.to(f'npu:{logical_rank_id}')
    expert_ids = expert_ids.to(f'npu:{logical_rank_id}')
    expand_x = expand_x.to(f'npu:{logical_rank_id}')
    assist_info_for_combine = assist_info_for_combine.to(f'npu:{logical_rank_id}')
    expert_token_nums = expert_token_nums.to(f'npu:{logical_rank_id}')
    recv_counts = recv_counts.to(f'npu:{logical_rank_id}')
...
def test_moe_distributed_dispatch() -> None:
    ...
    for x, moe_expert_ids, expand_x, assist_info_for_combine, recv_counts, expert_token_nums, logical_rank_id in zip(
        ...
        p = mp.Process(
            ...
            args=(moe_case, operand, logical_rank_id),
        )
        p.start()
        processes.append(p)
    for p in processes:
