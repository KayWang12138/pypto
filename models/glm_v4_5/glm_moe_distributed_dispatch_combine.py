...
def generate_dispatch_golden(moe_case: MoeCase, torch_data_type: torch.dtype) -> MoeDispatchGolden:
    ...
    def kernel(
        ...
        for _ in pypto.loop(1, name='MOE_DISTRIBUTED_DISPATCH_CUM_SUM', idx_name='_'):
            ...
            for expert_id in range(expert_num_per_rank):
                expert_valid_cnt = cum_sum_input[expert_id * ep_world_size + 1:expert_id * ep_world_size + 1 + ep_world_size, :]
...
def moe_distributed_dispatch(moe_case: MoeCase, operands: MoeDispatchOperands, logical_rank_id: int) -> None:
    ...
    expert_token_nums = operands.expert_token_nums
    ...
    physical_device_id = logical_rank_id + PHYSICAL_START_DEVICE_ID
    ...
    expert_token_nums = expert_token_nums.to(f'npu:{physical_device_id}')
    ...
    for out, act in [
        (expand_x_kernel, expand_x),
        (assist_info_for_combine_kernel, assist_info_for_combine),
        (expert_token_nums_kernel, expert_token_nums),
        (recv_counts_kernel, recv_counts),
    ]:
...
def test_moe_distributed_dispatch() -> None:
    ...
    moe_case = MoeCase(
        ...
        moe_expert_num=160,
        topk=8,
        data_type=pypto.DT_BF16,
        ep_world_size=WORLD_SIZE,
    )
    dispatch_golden_data = generate_dispatch_golden(moe_case, torch.bfloat16)
    for x, moe_expert_ids, expand_x, assist_info_for_combine, recv_counts, expert_token_nums, logical_rank_id in zip(
        ...
        p = mp.Process(
            ...
            args=(moe_case, operands, logical_rank_id),
        )
        p.start()
        processes.append(p)
    for p in processes:
...
def test_moe_distributed_combine() -> None:
    ...
    (
        ...
        expert_scales_list,
        out_golden_list,
    ) = generate_combine_golden(moe_case, torch.bfloat16)
