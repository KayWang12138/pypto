...
def matmul_allreduce_add_rmsnorm_kernel(
    ...
    for bs_idx in pypto.loop(bs_loop, name="LOOP_MM_ALLREDUCE_ADD_RMSNORM", idx_name="bs_idx"):
        ...
        shmem_tensor = pypto.distributed.create_shmem_tensor_v1(
            group_name, world_size, pypto.DT_FP32, shmem_shape)
        shmem_barrier_signal = pypto.distributed.create_shmem_signal_v1_v1(group_name, world_size)
        ...
        for _ in pypto.loop(1, name="LOOP_MM_AR_ARMS_L0", idx_name="_"):
            ...
            # shmem_tensor_tile = pypto.distributed.shmem_view(shmem_tensor, shmem_shape, [0, 0, 0])

            # 2. clear data
            pypto.set_vec_tile_shapes(view_row_shape, hidden_size)
            data_clear_out = pypto.distributed.shmem_clear_data_v1(
                shmem_tensor, pred=[in_tensor_tile])
            signal_clear_out = pypto.distributed.shmem_clear_signal_v1(
                shmem_tensor, pred=[in_tensor_tile])
            pypto.set_vec_tile_shapes(1, 8)
            barrier_out = pypto.distributed.shmem_barrier_all_v1(
            ...
            for dyn_idx in range(world_size):
                put_out = pypto.distributed.shmem_put_v1(matmul_result, shmem_tensor, dyn_idx,
                    put_op=pypto.AtomicType.ADD, pred=[barrier_out])
                pypto.distributed.shmem_signal_v1(shmem_tensor, dyn_idx, 1, sig_op=pypto.AtomicType.ADD, pred=[put_out])
            wait_until_out = pypto.distributed.shmem_wait_until_v1(shmem_tensor, my_pe, pypto.OpType.EQ, world_size,
                clear_signal=True, pred=[in_tensor_tile])
            pypto.set_vec_tile_shapes(1, hidden_size)
            # shmem_tile = pypto.distributed.shmem_view(shmem_tensor, shmem_shape, [0, 0, 0], valid_shape=shmem_shape)
            # all_reduce_out = pypto.experimental.shmem_load_v1(
            #     shmem_tile, my_pe, pred=[wait_until_out]
            all_reduce_out = pypto.distributed.shmem_get_v1(
                shmem_tensor, my_pe, pred=[wait_until_out]
            )
