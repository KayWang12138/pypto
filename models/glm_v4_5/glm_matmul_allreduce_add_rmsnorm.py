        ...
        for batch_size_idx in pypto.loop(batch_size_loop, name="LOOP_MM_ALLREDUCE_ADD_RMSNORM", idx_name="batch_size_idx"):
            ...
            for _ in pypto.loop(1, name="LOOP_MM_AR_ARMS_L0", idx_name="_"):
                ...
                for dyn_idx in range(WORLD_SIZE):
                    ...
                    pypto.distributed.shmem_signal(shmem_signal, dyn_idx, 1, [1, 1] + shmem_shape, [dyn_idx, dyn_idx, 0, 0, 0],
                        sig_op=pypto.AtomicType.ADD, pred=[put_dummy])
                wait_dummy = pypto.distributed.shmem_wait_until(
                    shmem_signal, 1, WORLD_SIZE, [1, 1] + shmem_shape, [my_pe, my_pe, 0, 0, 0], clear_signal=True, pred=[tile_in_tensor])
