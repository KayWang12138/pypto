    ...
    def kernel(
        ...
        for batch_size_idx in pypto.loop(batch_size_loop, name="LOOP_MM_ALLREDUCE_ADD_RMSNORM", idx_name="batch_size_idx"):
            ...
            for _ in pypto.loop(1, name="LOOP_MM_AR_ARMS_L0", idx_name="_"):
                tile_in_tensor = pypto.view(in_tensor, (view_row_shape, in_tensor.shape[1]), [batch_size_idx * view_row_shape, 0],
                    valid_shape=[(batch_size - batch_size_idx * view_row_shape).min(view_row_shape), in_tensor.shape[1]])
                ...
                in_tensor_tile_fp32= pypto.experimental.shmem_load(
                    shmem_data, my_pe, shmem_shape, [0, 0, 0], pred_tokens=[wait_dummy]
                )

                # 5. Add RmsNorm
                residual_tile = pypto.view(residual, (view_row_shape, hidden_size), [batch_size_idx * view_row_shape, 0],
                ...
                out_tensor[batch_size_idx * pypto.symbolic_scalar(view_row_shape):, 0:] = hidden_bf16
    return kernel
...
def test_matmul_allreduce_add_rmsnorm(intput_data, output_data, rank):
    ...
    np.testing.assert_allclose(np.array(out_tensor.cpu().flatten().tolist()), 
                    np.array(golden_out_tensor.cpu().flatten().tolist()), rtol=8e-3, atol=7e-2)

    np.testing.assert_allclose(np.array(residual_out.cpu().flatten().tolist()), 
                    np.array(golden_residual.cpu().flatten().tolist()), rtol=8e-3, atol=7e-2)
