...
def test_matmul_allreduce_addrmsnorm(intput_data, output_data, rank):
    ...
    np.testing.assert_allclose(np.array(output_hidden_states.cpu().flatten().tolist()), 
                    np.array(golden_hidden_states.cpu().flatten().tolist()), rtol=8e-3, atol=7e-2)


@_dynamo.allow_in_graph
...
if __name__ == '__main__':
    ...
    for i in range(WORLD_SIZE):
        p = mp.Process(target=test_matmul_allreduce_addrmsnorm, args=(input_datas[i], output_datas[i], i))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()