...
def matmul_allreduce_add_rmsnorm_kernel(batch_size, attn_dim_per_tp, hidden_size, eps, group_name):
    ...
    def kernel(
        ...
        for batch_size_idx in pypto.loop(batch_size_loop, name="LOOP_MM_ALLREDUCE_ADD_RMSNORM", idx_name="batch_size_idx"):
            ...
            for _ in pypto.loop(1, name="LOOP_MM_AR_ARMS_L0", idx_name="_"):
                tile_in_tensor = pypto.view(x, (view_row_shape, x.shape[1]), [batch_size_idx * view_row_shape, 0],
                    valid_shape=[(batch_size - batch_size_idx * view_row_shape).min(view_row_shape), x.shape[1]])
                ...
                reduce_out = pypto.experimental.shmem_load(
                    shmem_data, my_pe, shmem_shape, [0, 0, 0], pred_tokens=[wait_dummy]
                )
                x_tile = pypto.cast(reduce_out, pypto.DT_BF16)

                # 5. Add RmsNorm
                residual_input_tile = pypto.view(residual_input, (view_row_shape, hidden_size), [batch_size_idx * view_row_shape, 0],
                                                valid_shape=[(batch_size - batch_size_idx * view_row_shape).min(view_row_shape), hidden_size])
                ...
                residual_out[batch_size_idx * pypto.symbolic_scalar(view_row_shape):, 0:] = residual_bf16_tmp
                hidden_states_out[batch_size_idx * pypto.symbolic_scalar(view_row_shape):, 0:] = hidden_bf16
    return kernel
...
def generate_golden_data():
    ...
    for _ in range(WORLD_SIZE):
        ...
        residual_input = torch.randn((batch_size, hidden_size), dtype=torch.bfloat16).share_memory_()
...
def test_matmul_allreduce_add_rmsnorm(intput_data, output_data, rank):
    ...
    np.testing.assert_allclose(np.array(output_hidden_states.cpu().flatten().tolist()), 
...
def matmul_allreduce_add_rmsnorm(
    ...
    hidden_size = hidden_states_res.shape[1]
...
if __name__ == '__main__':
    ...
    for i in range(WORLD_SIZE):
        p = mp.Process(target=test_matmul_allreduce_add_rmsnorm, args=(input_datas[i], output_datas[i], i))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()