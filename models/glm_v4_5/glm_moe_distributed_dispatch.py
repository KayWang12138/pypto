...
def dispatch_tokens(
    ...
    for rank_id in range(moe_case.world_size):
        ...
        fixed_shape_assist_info_for_combine = torch.zeros((row, 64), dtype=torch.int32)
...
def generate_golden_data(moe_case: MoeCase) -> tuple[TensorList, TensorList, TensorList, TensorList, TensorList]:
    ...
    expand_x_list, assist_info_for_combine_list, recv_counts_list, expert_token_nums_list = dispatch_tokens(
        moe_case, x_list, moe_expert_ids_list)
...
def moe_distributed_dispatch_kernel(
    ...
    @pypto.frontend.jit()
    def kernel(
        x: pypto.Tensor([batch_size, hidden_size], data_type),
        expert_ids: pypto.Tensor([batch_size, topk], pypto.DT_INT32),
    ) -> (
        ...
        shmem_info, shmem_info_signalt = pypto.distributed.create_shmem_tensor(group_name, world_size, pypto.DT_INT32, [moe_expert_num, batch_size, info_size])
