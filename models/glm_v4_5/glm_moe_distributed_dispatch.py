...
class MoeCase:
    ...
    def __post_init__(self):
        assert self.topk <= self.moe_expert_num, f'topk ({self.topk}) must be <= moe_expert_num ({self.moe_expert_num})'
...
def dispatch_tokens(
    ...
    return expand_x_per_rank, assist_info_for_combine_per_rank, recv_counts_per_rank, expert_token_nums_per_rank
...
def combine_tokens(
    ...
    for moe_expert_tokens, expert_scales in zip(moe_expert_tokens_list, expert_scales_list):
        out_golden = expert_scales.unsqueeze(1).matmul(moe_expert_tokens.to(dtype=torch.float32))
        out_golden_list.append(out_golden)
    
    return out_golden_list


def generate_dispatch_golden_data(moe_case: MoeCase) -> tuple[TensorList, TensorList, TensorList, TensorList, TensorList, TensorList]:
    x_list, moe_expert_ids_list, expert_scales_list = generate_inputs(moe_case)
    expand_x_list, assist_info_for_combine_list, recv_counts_list, expert_token_nums_list = dispatch_tokens(
        moe_case, x_list, moe_expert_ids_list, True)
    return expand_x_list, assist_info_for_combine_list, recv_counts_list, expert_token_nums_list, x_list, moe_expert_ids_list


def generate_combine_golden_data(moe_case: MoeCase) -> tuple[TensorList, TensorList, TensorList, TensorList, TensorList]:
    x_list, moe_expert_ids_list, expert_scales_list = generate_inputs(moe_case)
    expand_x_list, assist_info_for_combine_list, recv_counts_list, expert_token_nums_list = dispatch_tokens(
        moe_case, x_list, moe_expert_ids_list, False)
    out_golden_list = combine_tokens(
        moe_case, expand_x_list, assist_info_for_combine_list, recv_counts_list, expert_scales_list)
...
def moe_distributed_dispatch_kernel(
    ...
    assert isinstance(group_name, str), f"type of group_name must be str, but got {type(group_name)}"
    assert group_name.strip(), f"group_name can't be null string"
    assert batch_size == 8, f"batch_size must be 8, but got {batch_size}"
    assert hidden_size == 5120, f"hidden_size must be 5120, but got {hidden_size}"
    assert moe_expert_num == 160, f"moe_expert_num must be 160, but got {moe_expert_num}"
    assert topk == 8, f"topk must be 8, but got {topk}"
    assert data_type == pypto.DT_BF16, f"data_type must be pypto.DT_BF16, but got {data_type}"
    assert act_data_type == torch.bfloat16, f"torch_data_type must be torch.bfloat16, but got {act_data_type}"
    assert world_size == 4 or world_size == 8, f"world_size must be == 4 or 8, but got {world_size}"
    ...
    def kernel(
        ...
        """
        JIT compiled kernel for distributed MoE dispatch.
        
        Execution Mode:
            - Expert Parallel (EP)
            - Multiple experts per device
            - No shared experts across devices

        This kernel performs token expansion and expert routing preparation
        in a distributed Mixture-of-Experts (MoE) setting.

        Main Responsibilities:
            - Expand input tokens according to top-k expert assignments.
            - Recorder tokens so that tokens targeting the same expert are grouped.
            - Prepare auxiliary routing information for combine stage.
            - Compute per-rank expert token statistics for all-to-all communication.
        
        Args:
            x: Input hidden states tensor.
            expert_ids: Each row contains the indices of selected experts for the token.
        
        Returns:
            expand_x: Expanded and recordered hidden states from each local expert.
            assist_info_for_combine: Auxiliary metadata used in combine stage to restore token order.
            expert_token_nums: Number of tokens assigned to each expert on the current rank.
            recv_counts: Total number of tokens to be received in all-to-all stage.
        """
        ...
        shmem_data, shmem_data_signal = pypto.distributed.create_shmem_tensor(group_name, world_size, x.dtype, [moe_expert_num, batch_size, hidden_size])
        shmem_info, _ = pypto.distributed.create_shmem_tensor(group_name, world_size, pypto.DT_INT32, [moe_expert_num, batch_size, info_size])
        shmem_count, shmem_count_signal = pypto.distributed.create_shmem_tensor(group_name, world_size, pypto.DT_INT32, [1, cum_sum_row_size, count_size])
        ...
        for _ in pypto.loop(1, name='MOE_DISTRIBUTED_DISPATCH_SEND_DATA', idx_name = '_'):
            for index in range(batch_size * topk):
                ...
                row_index = index // topk
                col_index = index % topk
                tensor_tile = pypto.view(x, [1, hidden_size], [row_index, 0])
                pypto.set_vec_tile_shapes(1, info_size)
                ...
                shmem_data_out_put = pypto.distributed.shmem_put(tensor_tile, [remote_expert_offset*world_size+this_rank, token_offset, 0], shmem_data, remote_rank_id, pred=[offset_table])
                pypto.set_vec_tile_shapes(1, info_size)
                shmem_info_out_put = pypto.distributed.shmem_put(moe_info, [remote_expert_offset*world_size+this_rank, token_offset, 0], shmem_info, remote_rank_id, pred=[offset_table])
                pypto.set_vec_tile_shapes(1, hidden_size)
                pypto.distributed.shmem_signal(shmem_data_signal, world_size, 1, [world_size, 1, 1, 1, hidden_size], [0, 0, 0, 0, 0], sig_op=pypto.AtomicType.ADD, pred=[shmem_data_out_put, shmem_info_out_put])
        ...
        for expert_id in pypto.loop(moe_expert_num, name='MOE_DISTRIBUTED_DISPATCH_SEND_COUNT', idx_name = 'expert_id'):
            ...
            total_offset_tile = pypto.view(expert_offset, [1, 1], [0, batch_size * topk - 1])
            pypto.set_vec_tile_shapes(1, 1)
            shmem_put_out = pypto.distributed.shmem_put(total_offset_tile, [0, remote_expert_offset * world_size + this_rank + 1, 0], shmem_count, remote_rank_id, pred=[total_offset_tile])
            pypto.set_vec_tile_shapes(1, count_size)
            pypto.distributed.shmem_signal(shmem_count_signal, remote_rank_id, 1, [1, 1, 1, 1, count_size], [remote_rank_id, 0, 0, 0, 0], sig_op=pypto.AtomicType.ADD, pred=[shmem_put_out])
        ...
        for _ in pypto.loop(1, name='MOE_DISTRIBUTED_DISPATCH_CUM_SUM', idx_name = '_'):
            pypto.set_vec_tile_shapes(1, hidden_size)
            shmem_data_wait_out = pypto.distributed.shmem_wait_until(shmem_data_signal, pypto.OpType.EQ, batch_size * topk * world_size, [1, 1, 1, 1, hidden_size], [this_rank, 0, 0, 0, 0], clear_signal=True, pred=[cum_sum_result])
            pypto.set_vec_tile_shapes(1, count_size)
            shmem_count_wait_out = pypto.distributed.shmem_wait_until(shmem_count_signal, pypto.OpType.EQ, moe_expert_num, [1, 1, 1, 1, count_size], [this_rank, 0, 0, 0, 0], clear_signal=True, pred=[cum_sum_result])
            pypto.set_vec_tile_shapes(cum_sum_row_size, count_size)
            local_expert_recv_count = pypto.distributed.shmem_get(shmem_count, this_rank, [1, cum_sum_row_size, count_size], [0, 0, 0], pred=[shmem_data_wait_out, shmem_count_wait_out])
            pypto.set_vec_tile_shapes(cum_sum_row_size, count_size)
            cum_sum_input = pypto.distributed.shmem_get(shmem_count, this_rank, [1, cum_sum_row_size, count_size], [0, 0, 0], pred=[shmem_data_wait_out, shmem_count_wait_out])
            ...
            for expert_id in range(expert_num_per_rank):
                expert_valid_cnt = pypto.view(cum_sum_input, [world_size, count_size], [expert_id * world_size + 1, 0])
        ...
        for _ in pypto.loop(1, name='MOE_DISTRIBUTED_DISPATCH_RECEIVE', idx_name = '_'):
            for index in range(expert_num_per_rank * world_size):
                ...
                local_data_recv_count = pypto.experimental.shmem_load(shmem_data, this_rank, [1, batch_size, hidden_size], [index, 0, 0], pred=[cum_sum_result], valid_shape=[1, 1, cur_count, hidden_size])
                pypto.assemble(local_data_recv_count, [offset, 0], expand_x)
                pypto.set_vec_tile_shapes(batch_size, info_size)
                local_info_recv_count = pypto.experimental.shmem_load(shmem_info, this_rank, [1, batch_size, info_size], [index, 0, 0], pred=[cum_sum_result], valid_shape=[1, 1, cur_count, info_size])
                pypto.assemble(local_info_recv_count, [offset, 0], assist_info_for_combine)
...
def test_moe_distributed_dispatch(
    ...
    expand_x_kernel, assist_info_for_combine_kernel, expert_token_nums_kernel, recv_counts_kernel = kernel(x, expert_ids)
    ...
    try:
        ...
        print('pass combineinfo')
    except AssertionError as e:
        print('combineinfo mismatch')
        print(e)
    try:
        np.testing.assert_allclose(
        ...
        print('pass expert token nums')
    except AssertionError as e:
        print('expert token nums mismatch')
        print(e)
    try:
        np.testing.assert_allclose(
        ...
        print('pass recv counts')
    except AssertionError as e:
        print('recv counts mismatch')
        print(e)
...
def main() -> None:
    ...
    (
        ...
        x_list,
        moe_expert_ids_list,
    ) = generate_dispatch_golden_data(moe_case)
    for expand_x, assist_info_for_combine, recv_counts, expert_token_nums, x, moe_expert_ids, rank_id in zip(
        expand_x_list,
        assist_info_for_combine_list,
        recv_counts_list,
        expert_token_nums_list,
        ...
        p = mp.Process(
            ...
            args=(moe_case, x, moe_expert_ids, expand_x, assist_info_for_combine, expert_token_nums, recv_counts, rank_id),
        )
        p.start()
        processes.append(p)
    for p in processes:
