...
def init_hccl_comm(logical_rank: int) -> list[str, ...]:
    ...
    group_name = backend.get_hccl_comm_name(logical_rank)
    return [group_name]
...
class MoeCase:
    ...
    def __post_init__(self):
        check_cond(self.topk <= self.moe_expert_num, 'topk ({self.topk}) must be <= moe_expert_num ({self.moe_expert_num})')
...
def generate_dispatch_golden_data(moe_case: MoeCase
) -> MoeDispatchOperand:
    ...
    return MoeDispatchOperand(
        expand_x_list=expand_x_list,
        assist_info_for_combine_list=assist_info_for_combine_list,
        recv_counts_list=recv_counts_list,
        expert_token_nums_list=expert_token_nums_list,
        x_list=x_list,
        expert_ids_list=moe_expert_ids_list,)


def generate_combine_golden_data(moe_case: MoeCase
...
def align_up(value: int, alignment: int) -> int:
    check_cond(
        ...
        'alignment must be a power of two, but got {alignment}')
...
def moe_distributed_dispatch_kernel(
    ...
    check_cond(isinstance(group_name, str), 'Type of group_name must be str, but got {type(group_name)}')
    check_cond(group_name.strip(), "Group_name can't be empty string")
    check_cond(1 <= len(group_name) < 128, 'The length of group_name only supports [1, 128), but got {len(group_name)}')
    check_cond(batch_size == 8, 'Batch_size must be 8, but got {batch_size}')
    check_cond(hidden_size == 5120, 'Hidden_size must be 5120, but got {hidden_size}')
    check_cond(moe_expert_num == 160, 'Moe_expert_num must be 160, but got {moe_expert_num}')
    check_cond(topk == 8, 'Topk must be 8, but got {topk}')
    check_cond(data_type == pypto.DT_BF16, 'Data_type must be pypto.DT_BF16, but got {data_type}')
    check_cond(act_data_type == torch.bfloat16, 'Torch_data_type must be torch.bfloat16, but got {act_data_type}')
    check_cond(world_size == 4 or world_size == 8, 'World_size must be 4 or 8, but got {world_size}')
    ...
    def kernel(
        ...
        for _ in pypto.loop(1, name='MOE_DISTRIBUTED_DISPATCH_SEND_DATA', idx_name = '_'):
            for index in range(batch_size * topk):
                ...
                shmem_data_out_put = pypto.distributed.shmem_put(tensor_tile,
                    [remote_expert_offset*world_size+this_rank, token_offset, 0], shmem_data, remote_rank_id, pred=[offset_table])
                pypto.set_vec_tile_shapes(1, info_size)
                shmem_info_out_put = pypto.distributed.shmem_put(moe_info,
                    [remote_expert_offset*world_size+this_rank, token_offset, 0], shmem_info, remote_rank_id, pred=[offset_table])
                pypto.set_vec_tile_shapes(1, hidden_size)
                pypto.distributed.shmem_signal(shmem_data_signal, world_size, 1,
                    [world_size, 1, 1, 1, hidden_size], [0, 0, 0, 0, 0], sig_op=pypto.AtomicType.ADD, pred=[shmem_data_out_put, shmem_info_out_put])
        ...
        for expert_id in pypto.loop(moe_expert_num, name='MOE_DISTRIBUTED_DISPATCH_SEND_COUNT', idx_name = 'expert_id'):
            ...
            shmem_put_out = pypto.distributed.shmem_put(total_offset_tile,
                [0, remote_expert_offset * world_size + this_rank + 1, 0], shmem_count, remote_rank_id, pred=[total_offset_tile])
            pypto.set_vec_tile_shapes(1, count_size)
            pypto.distributed.shmem_signal(shmem_count_signal, remote_rank_id, 1,
                [1, 1, 1, 1, count_size], [remote_rank_id, 0, 0, 0, 0], sig_op=pypto.AtomicType.ADD, pred=[shmem_put_out])
        ...
        for _ in pypto.loop(1, name='MOE_DISTRIBUTED_DISPATCH_CUM_SUM', idx_name = '_'):
            pypto.set_vec_tile_shapes(1, hidden_size)
            shmem_data_wait_out = pypto.distributed.shmem_wait_until(shmem_data_signal,pypto.OpType.EQ,
                batch_size * topk * world_size, [1, 1, 1, 1, hidden_size], [this_rank, 0, 0, 0, 0], clear_signal=True, pred=[cum_sum_result])
            pypto.set_vec_tile_shapes(1, count_size)
            shmem_count_wait_out = pypto.distributed.shmem_wait_until(shmem_count_signal, pypto.OpType.EQ,
                moe_expert_num, [1, 1, 1, 1, count_size], [this_rank, 0, 0, 0, 0], clear_signal=True, pred=[cum_sum_result])
            pypto.set_vec_tile_shapes(cum_sum_row_size, count_size)
            local_expert_recv_count = pypto.distributed.shmem_get(shmem_count,
                this_rank, [1, cum_sum_row_size, count_size], [0, 0, 0], pred=[shmem_data_wait_out, shmem_count_wait_out])
            pypto.set_vec_tile_shapes(cum_sum_row_size, count_size)
            cum_sum_input = pypto.distributed.shmem_get(shmem_count,
                this_rank, [1, cum_sum_row_size, count_size], [0, 0, 0], pred=[shmem_data_wait_out, shmem_count_wait_out])
        ...
        for _ in pypto.loop(1, name='MOE_DISTRIBUTED_DISPATCH_RECEIVE', idx_name = '_'):
            for index in range(expert_num_per_rank * world_size):
                ...
                local_data_recv_count = pypto.experimental.shmem_load(shmem_data, this_rank,
                    [1, batch_size, hidden_size], [index, 0, 0], pred=[cum_sum_result], valid_shape=[1, 1, cur_count, hidden_size])
                expand_x[offset:offset+cur_count, 0:hidden_size] = local_data_recv_count
                pypto.set_vec_tile_shapes(batch_size, info_size)
                local_info_recv_count = pypto.experimental.shmem_load(shmem_info, this_rank,
                    [1, batch_size, info_size], [index, 0, 0], pred=[cum_sum_result], valid_shape=[1, 1, cur_count, info_size])
                assist_info_for_combine[offset:offset+cur_count, 0:info_size] = local_info_recv_count
...
def main() -> None:
    ...
    dispatch_golden_data = generate_dispatch_golden_data(moe_case)
    for expand_x, assist_info_for_combine, recv_counts, expert_token_nums, x, moe_expert_ids, rank_id in zip(
        ...
        dispatch_golden_data.expert_ids_list,
        LOGICAL_RANKS,
    ):
        p = mp.Process(
