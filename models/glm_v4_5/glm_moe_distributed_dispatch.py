...
def moe_distributed_dispatch_kernel(
    ...
    assert 1 <= len(group_name) < 128, f'The length of group_name only supports [1, 128), but got {len(group_name)}'
    ...
    assert data_type == pypto.DT_BF16, f"Fata_type must be pypto.DT_BF16, but got {data_type}"
