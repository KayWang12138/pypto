...
def moe_distributed_dispatch_kernel(
    ...
    assert world_size == 4 or world_size == 8, f"world_size must be > 0, but got {world_size}"
    ...
    def kernel(
        ...
        shmem_data, shmem_data_signal = pypto.distributed.create_shmem_tensor(group_name, world_size, [moe_expert_num, batch_size, hidden_size], x.dtype)
        shmem_info, shmem_info_signalt = pypto.distributed.create_shmem_tensor(group_name, world_size, [moe_expert_num, batch_size, info_size], pypto.DT_INT32)
        shmem_count, shmem_count_signal = pypto.distributed.create_shmem_tensor(group_name, world_size, [1, cum_sum_row_size, count_size], pypto.DT_INT32)
        ...
        for _ in pypto.loop(1, name='MOE_DISTRIBUTED_DISPATCH_SEND_DATA', idx_name = '_'):
            for index in range(batch_size * topk):
                ...
                shmem_data_out_put = pypto.distributed.shmem_put(tensor_tile, [remote_expert_offset*world_size+this_rank, token_offset, 0], shmem_data, remote_rank_id, pred_tokens=[offset_table])
                pypto.set_vec_tile_shapes(1, info_size)
                shmem_info_out_put = pypto.distributed.shmem_put(moe_info, [remote_expert_offset*world_size+this_rank, token_offset, 0], shmem_info, remote_rank_id, pred_tokens=[offset_table])
                pypto.set_vec_tile_shapes(1, hidden_size)
                pypto.distributed.shmem_signal(shmem_data_signal, world_size, [world_size, 1, 1, 1, hidden_size], [0, 0, 0, 0, 0], pred_tokens=[shmem_data_out_put, shmem_info_out_put], shmem_op=pypto.AtomicType.ADD)
        ...
        for expert_id in pypto.loop(moe_expert_num, name='MOE_DISTRIBUTED_DISPATCH_SEND_COUNT', idx_name = 'expert_id'):
            ...
            shmem_put_out = pypto.distributed.shmem_put(total_offset_tile, [0, remote_expert_offset * world_size + this_rank + 1, 0], shmem_count, remote_rank_id, pred_tokens=[total_offset_tile])
            pypto.set_vec_tile_shapes(1, count_size)
            pypto.distributed.shmem_signal(shmem_count_signal, remote_rank_id, [1, 1, 1, 1, count_size], [remote_rank_id, 0, 0, 0, 0], pred_tokens=[shmem_put_out], shmem_op=pypto.AtomicType.ADD)
        ...
        for _ in pypto.loop(1, name='MOE_DISTRIBUTED_DISPATCH_CUM_SUM', idx_name = '_'):
            pypto.set_vec_tile_shapes(1, hidden_size)
            shmem_data_wait_out = pypto.distributed.shmem_wait(shmem_data_signal, [1, 1, 1, 1, hidden_size], [this_rank, 0, 0, 0, 0], cmp_value=batch_size * topk * world_size, pred_tokens=[cum_sum_result], clear_flag=True)
            pypto.set_vec_tile_shapes(1, count_size)
            shmem_count_wait_out = pypto.distributed.shmem_wait(shmem_count_signal, [1, 1, 1, 1, count_size], [this_rank, 0, 0, 0, 0], cmp_value=moe_expert_num, pred_tokens=[cum_sum_result], clear_flag=True)
            pypto.set_vec_tile_shapes(1, count_size)
            local_expert_recv_count = pypto.distributed.shmem_get(shmem_count, this_rank, [1, cum_sum_row_size, count_size], [0, 0, 0], pred_tokens=[shmem_data_wait_out, shmem_count_wait_out])
            pypto.set_vec_tile_shapes(cum_sum_row_size, count_size)
            cum_sum_input = pypto.distributed.shmem_get(shmem_count, this_rank, [1, cum_sum_row_size, count_size], [0, 0, 0], pred_tokens=[shmem_data_wait_out, shmem_count_wait_out])
        ...
        for _ in pypto.loop(1, name='MOE_DISTRIBUTED_DISPATCH_RECEIVE', idx_name = '_'):
            for index in range(expert_num_per_rank * world_size):
                ...
                local_data_recv_count = pypto.experimental.shmem_load(shmem_data, [1, 1, batch_size, hidden_size], [this_rank, index, 0, 0], pred_tokens=[cum_sum_result], valid_shape=[1, 1, cur_count, hidden_size])
                pypto.assemble(local_data_recv_count, [offset, 0], expand_x)
                pypto.set_vec_tile_shapes(batch_size, info_size)
                local_info_recv_count = pypto.experimental.shmem_load(shmem_info, [1, 1, batch_size, info_size], [this_rank, index, 0, 0], pred_tokens=[cum_sum_result], valid_shape=[1, 1, cur_count, info_size])
