...
def get_pfa_config_v1(device="cpu"):
    """PFA (Prefill) 配置 - s1 >= 1"""
    b = 8
    s1 = 128       # PFA: query 长度可以大于 1
    s2 = 256        # PFA: KV 长度等于 query 长度
    q_d = 128
    nq = 12
    nkv = 1
    kv_layout = "PA_BSND"
    ...
    causal_table_step = 64 # 32
    s2_tile = 256 #128  小于等于s2
    ...
    atten_cfg = AttentionConfig(
        ...
        causal_table_step=causal_table_step, s2_tile=s2_tile
    )
...
def get_pfa_config(device="cpu"):
    ...
    block_size = 128
    kv_num_blocks = b * ((s2 + block_size - 1) // block_size)
    causal_table_step = min(s1//2, 32)  # 1 # 
    s1 = ((s1+causal_table_step-1)//causal_table_step)*causal_table_step  #确保S1是causal_table_step的整数倍
    s2_tile = 256  # 需要大于block_size，确保block_num>1
    ...
    tile_cfg = AttentionTileConfig(
        nq,
        s2_tile,
        # [[cube_tile, cube_tile], [64, 64], [256, 256]],    # cube--shape 分tile, vector tile, 尾轴不切，tile一致 [cube_tile, cube_tile], [cube_tile, cube_tile]
        [[64, 64], [64, 64], [256, 256]],    # cube--shape 分tile, vector tile, 尾轴不切，tile一致 [cube_tile, cube_tile], [cube_tile, cube_tile]
        [256, 256],
        [[64, 64], [64, 64], [256, 256]], # [[cube_tile, cube_tile], [64, 64], [256, 256]], 
        [256, 256],)
    return atten_cfg, tile_cfg
...
#     s2_padded: int,
#     step: int,
#     device: torch.device = None,  # 添加 device 参数
# ) -> torch.Tensor:
#     """
#     创建阶梯因果注意力掩码表
...
#     Args:
#         s1: Query 序列长度
#         s2_padded: KV 序列长度（对齐到 tile）
#         step: 阶梯步长
#         device: 计算设备（修复：添加此参数）
#     """
#     # 修复：使用传入的 device，默认为 cpu
#     if device is None:
#         device = torch.device('cpu')
    
#     causal_table = torch.zeros(s1, s2_padded, dtype=torch.float32, device=device)
    
#     for i in range(s1):
#         for j in range(s2_padded):
#             # 阶梯因果：query 位置 i 可以看到 block_idx <= i//step 的所有 KV
#             q_block_idx = i // step
#             kv_block_idx = j // step
            
#             if kv_block_idx > q_block_idx:
#                 causal_table[i, j] = float('-inf')
#             elif j > i:
#                 # 同一 block 内仍需满足因果
#                 causal_table[i, j] = float('-inf')
    
#     print(f"阶梯因果掩码 (s1_len={s1}, s2_len={s2_padded}, step={step}):")
#     print(causal_table)

#     return causal_table


def create_causal_mask(s1_len, s2_len, step=1, device: torch.device = None): #默认标准因果
    """
    ...
    # 1. 初始化全0掩码矩阵
    mask = torch.zeros((s1_len, s2_len), dtype=torch.float32, device=device)
    
    # 2. 生成Query/Key的块索引（每个位置属于哪个step块）
    q_block_idx = torch.arange(s1_len, device=device) // step  # shape: (s1_len,)
    k_block_idx = torch.arange(s2_len, device=device) // step  # shape: (s2_len,)
    print(f'q_block_idx:{q_block_idx},k_block_idx:{k_block_idx}')
...
def pfa_func(q_shape, kv_shape, block_table_shape, causal_table_shape):
    ...
    @pypto.frontend.jit(
        ...
        pass_options={
            "cube_l1_reuse_setting": {-1:16}, # {-1: 16},
            "cube_nbuffer_setting":{-1:16}, # {-1: 16},
            "vec_nbuffer_mode":2, 
    ...
    def pfa_func_kernel(
        ...
        s2_scalar = causal_table.shape[1]  # 修复: 从causal_table获取实际的s2长度
        nkv = shape_k[2]         # n2
        dn = shape_k[3]          # d
        ...
        step = atten_cfg.causal_table_step # 阶梯因果掩码步长
        ...
        for b_idx in pypto.loop(b_scalar, name="LOOP_b", idx_name="b_idx", unroll_list=[8]): #dynamic , unroll_list=[8]
            for s1_block_idx in pypto.loop(s1_block_num, name="LOOP_s1_block", idx_name="s1_block_idx"): #dynamic
                s1_start = s1_block_idx * step
                s1_end = (s1_block_idx + 1) * step
                s2_max_for_block = pypto.min((s1_block_idx + 1) * step, s2_scalar)  # 只遍历下三角, 使用pypto.min限制不超过实际s2长度
                s2_loop_for_block = (s2_max_for_block + s2_tile - 1) // s2_tile

                for s1_offset in pypto.loop(step, name="LOOP_s1_offset", idx_name="s1_offset"):  # , unroll_list=[step]
                    ...
                    if s1_idx < actual_seq_len:   # 只读取有效数据
                        for g_idx_merged in pypto.loop(g_loop_merged, name="LOOP_g_merged", idx_name="g_idx_merged"): # , unroll_list=[12]
                            ...
                            for s2_idx in pypto.loop(s2_loop_for_block, name="LOOP_s2", idx_name="s2_idx"):  # 优化：只遍历可见的KV块 # dynamic , unroll_list=[8,4,1]
                                pypto.set_pass_options(sg_set_scope=2)
                                idx = s2_idx * block_num
                                # s2_max_for_query = pypto.min((s1_idx//step + 1)*step, s2_scalar)
                                s2_max_for_query = pypto.min(s1_idx + 1, s2_scalar)  # 标准因果
                                actual_s2_tile = pypto.max(pypto.min(s2_max_for_query - s2_idx * s2_tile, s2_tile), 0) 

                                pypto.set_vec_tile_shapes(v1_tile[0], v1_tile[1])  # 256 256
                                ...
                                # QK^T 计算
                                pypto.set_cube_tile_shapes(c1_tile[0], c1_tile[1], c1_tile[2], enable_multi_data_load=True) #  [128, 128], [64, 64], [256, 256]
                                sij = pypto.matmul(qi, kj_assemble, pypto.DT_FP32, a_trans=False, b_trans=True)  # [12, 128] [128,256] [12,256]
                                ...
                                if pypto.is_loop_begin(s2_idx):
                                    ...
                                    sij_scale = pypto.add(pypto.mul(sij, softmax_scale), causal_mask_broadcast)
                                    tilda_mij = pypto.amax(sij_scale, dim=-1, keepdim=True)
                                    max_new = pypto.maximum(max_update, tilda_mij)  # shape检查
...
#     print(f'--------------block_table_batch:{atten_cfg.block_table_batch}, max_num_blocks_per_query:{max_num_blocks_per_query}')
...
#     step = atten_cfg.causal_table_step # 阶梯因果掩码步长
#     causal_table = create_causal_mask(s1, ((s2 + s2_tile -1)//s2_tile)* s2_tile, step, step) # 对齐到s2_tile防止访问越界
#     debug_print("PFA_TEST", f"Created causal_table: {causal_table}")
...
#     debug_print("PFA_TEST", f"DEBUG q[0,0,:8] = {q[0,0,:8]}")
#     debug_print("PFA_TEST", f"DEBUG q[1,0,:8] = {q[1,0,:8]}")
#     debug_print("PFA_TEST", f"DEBUG q[2,0,:8] = {q[2,0,:8]}")
#     debug_print("PFA_TEST", f"DEBUG q[3,0,:8] = {q[3,0,:8]}")
...
#         for j in range(min(2, s1)):
#             diff_j = torch.abs(out_torch[i * s1 + j] - attention_output[i * s1 + j]).max().item()
#             debug_print("PFA_TEST", f"DEBUG i={i}, j={j}: max_diff={diff_j:.4f}")
#     debug_print("PFA_TEST", f"DEBUG out_torch[0,:4] = {out_torch[0,:4]}")
#     debug_print("PFA_TEST", f"DEBUG attention_output[0,:4] = {attention_output[0,:4]}")
#     print('over')
...
def pfa_reference_pytorch(
    q: torch.Tensor,
    k_cache_bsnd: torch.Tensor,
    v_cache_bsnd: torch.Tensor,
    query_seq_len: torch.Tensor,
    atten_cfg,
    device: torch.device,
) -> torch.Tensor:
    """
    PFA Prefill Attention PyTorch 参考实现
    
    完全模拟 kernel 的逻辑：
    1. 阶梯因果掩码 (step causal)
    2. GQA 支持 (nq // nkv)
    3. 在线 Softmax 更新
    4. Block Table KV 组织
    ...
    Args:
        q: [b*s1, nq, d] Query
        k_cache_bsnd: [b, s2, nkv, d] Key cache (BSND 格式)
        v_cache_bsnd: [b, s2, nkv, d] Value cache (BSND 格式)
        query_seq_len: [b] 每个 batch 的实际序列长度
        atten_cfg: 配置对象
        device: 计算设备
    
    Returns:
        output: [b*s1, nq, d] 注意力输出
    """
    b = atten_cfg.b
    s1 = atten_cfg.s1
    s2 = atten_cfg.s2
    ...
    group_size = nq // nkv  # GQA group 大小
    
    # 初始化输出
    output = torch.zeros(b * s1, nq, d, dtype=torch.float32, device=device)
    
    # 主循环：按 batch 处理
    for batch_idx in range(b):
        actual_seq_len = query_seq_len[batch_idx].item()
        
        # 按 step 分块处理 query 序列
        s1_block_num = (actual_seq_len + step - 1) // step
        
        for s1_block_idx in range(s1_block_num):
            s1_start = s1_block_idx * step
            s1_end = min((s1_block_idx + 1) * step, actual_seq_len)
            
            # 当前 query block 可见的最大 KV 长度（阶梯因果）
            max_kv_len = (s1_block_idx + 1) * step
            max_kv_len = min(max_kv_len, s2)
            
            # 对当前 query block 内的每个位置
            for s1_idx in range(s1_start, s1_end):
                q_idx = batch_idx * s1 + s1_idx
                
                # 当前 query 位置实际可见的 KV 长度（因果：只能看 0 到 s1_idx）
                cur_kv_len = s1_idx + 1
                cur_kv_len = min(cur_kv_len, max_kv_len)
                
                if cur_kv_len <= 0:
                    continue
                
                # 初始化在线 softmax 状态
                O_acc = torch.zeros(nq, d, dtype=torch.float32, device=device)
                sum_acc = torch.zeros(nq, 1, dtype=torch.float32, device=device)
                max_acc = torch.full((nq, 1), float('-inf'), dtype=torch.float32, device=device)
                
                # 按 KV 序列分块处理（模拟 kernel 的 s2_tile 分块）
                s2_tile = atten_cfg.s2_tile
                kv_block_num = (cur_kv_len + s2_tile - 1) // s2_tile
                
                for kv_block_idx in range(kv_block_num):
                    kv_start = kv_block_idx * s2_tile
                    kv_end = min((kv_block_idx + 1) * s2_tile, cur_kv_len)
                    ...
                    # 获取当前 KV 块
                    # k_cache_bsnd: [b, s2, nkv, d]
                    k_block = k_cache_bsnd[batch_idx, kv_start:kv_end]  # [kv_len, nkv, d]
                    v_block = v_cache_bsnd[batch_idx, kv_start:kv_end]  # [kv_len, nkv, d]
                    
                    # 获取当前 Query
                    q_vec = q[q_idx]  # [nq, d]
                    
                    # GQA: 将 KV 扩展到与 Q 相同的头数
                    if nq != nkv:
                        # [kv_len, nkv, d] -> [kv_len, nq, d]
                        k_block = k_block.repeat_interleave(group_size, dim=1)
                        v_block = v_block.repeat_interleave(group_size, dim=1)
                    
                    # 计算 QK^T: [nq, d] @ [kv_len, d]^T = [nq, kv_len]
                    # q_vec: [nq, d], k_block: [kv_len, nq, d]
                    k_block_t = k_block.transpose(0, 1).transpose(1, 2)  # [nq, d, kv_len]
                    q_vec_exp = q_vec.unsqueeze(0)  # [1, nq, d]
                    scores = torch.matmul(q_vec_exp, k_block_t).squeeze(0)  # [nq, kv_len]
                    
                    # 应用 softmax 缩放
                    scores = scores * softmax_scale
                    
                    # 应用因果掩码（当前块内）
                    # 对于位置 s1_idx，只能看到 0 到 s1_idx 的 KV
                    # 在当前 kv_block 中，需要检查哪些位置是可见的
                    causal_mask = torch.zeros(nq, kv_len, dtype=torch.float32, device=device)
                    for head_idx in range(nq):
                        for kv_pos in range(kv_len):
                            global_kv_pos = kv_start + kv_pos
                            if global_kv_pos > s1_idx:
                                causal_mask[head_idx, kv_pos] = float('-inf')
                    
                    scores = scores + causal_mask
                    
                    # 在线 Softmax 更新
                    max_block = torch.amax(scores, dim=-1, keepdim=True)  # [nq, 1]
                    max_new = torch.maximum(max_acc, max_block)  # [nq, 1]
                    
                    # 计算校正因子
                    exp_diff_old = torch.exp(max_acc - max_new)  # [nq, 1]
                    exp_scores = torch.exp(scores - max_new)  # [nq, kv_len]
                    
                    # 更新指数和
                    sum_block = torch.sum(exp_scores, dim=-1, keepdim=True)  # [nq, 1]
                    sum_new = sum_acc * exp_diff_old + sum_block  # [nq, 1]
                    
                    # 更新输出
                    # exp_scores: [nq, kv_len], v_block: [kv_len, nq, d]
                    v_block_t = v_block.transpose(0, 1)  # [nq, kv_len, d]
                    O_block = torch.matmul(exp_scores.unsqueeze(1), v_block_t).squeeze(1)  # [nq, d]
                    O_new = O_acc * exp_diff_old + O_block  # [nq, d]
                ...
                # 最终归一化
                # print(f"O_final shape:{O_final.shape}, expected:[{nq}, {d}]")
                O_final = O_acc / (sum_acc + 1e-6)  # [nq, d]
                print(f"O_final shape:{O_final.shape}, expected:[{nq}, {d}]")
                O_final = O_final.reshape(nq,d).contiguous()
                print(f"O_final shape:{O_final.shape}, expected:[{nq}, {d}]")
                output[q_idx] = O_final.to(q.dtype)
    
    return output


def pfa_reference_pytorch_simple(
    q: torch.Tensor,
    k_cache_bsnd: torch.Tensor,
    v_cache_bsnd: torch.Tensor,
    query_seq_len: torch.Tensor,
    ...
    """
    PFA Prefill Attention PyTorch 参考实现（简洁版）
    ...
    for batch_idx in range(b):
        ...
        for s1_idx in range(min(s1, actual_seq_len)):
            q_idx = batch_idx * s1 + s1_idx
            ...
            # 获取可见的 KV [cur_kv_len, nkv, d]
            k_visible = k_cache_bsnd[batch_idx, :cur_kv_len]
            v_visible = v_cache_bsnd[batch_idx, :cur_kv_len]
            ...
            for q_head in range(nq):
                ...
                # q: [d], k: [cur_kv_len, d], v: [cur_kv_len, d]
                q_h = q_vec[q_head]  # [d]
                k_h = k_visible[:, kv_head]  # [cur_kv_len, d]
                v_h = v_visible[:, kv_head]  # [cur_kv_len, d]
                
                # QK^T: [d] @ [cur_kv_len, d]^T = [cur_kv_len]
                scores = torch.matmul(q_h, k_h.transpose(0, 1))  # [cur_kv_len]
                scores = scores * softmax_scale
                
                # 因果掩码（已经自然满足，因为只取了 :cur_kv_len）
                # 但为了阶梯因果，需要额外检查
                # 这里简化处理，假设阶梯因果已通过 cur_kv_len 控制
                
                # Softmax
                attn_weights = torch.softmax(scores, dim=-1)  # [cur_kv_len]
                
                # 输出
                out_h = torch.matmul(attn_weights.unsqueeze(0), v_h)  # [1, d]
                output[q_idx, q_head] = out_h.squeeze(0)
    
    return output


def pfa_reference_pytorch_optimized(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    """
    PFA Prefill Attention PyTorch 参考实现（优化版，修复维度问题）
    """
    b = atten_cfg.b
    s1 = atten_cfg.s1
    d = atten_cfg.q_d
    ...
    query_seq_len = query_seq_len.to(device)
    causal_table = causal_table.to(device) if causal_table is not None else None
    
    # 初始化输出 [b*s1, nq, d]
    output = torch.zeros(b * s1, nq, d, dtype=torch.float32, device=device)
    
    kv_num_blocks = k.shape[0]
    ...
    for batch_idx in range(b):
        ...
        for block_idx in range(block_table.shape[1]):
            table_idx = block_table[batch_idx, block_idx].item()
            if table_idx < 0 or table_idx >= kv_num_blocks:
                continue
            
            k_block = k[table_idx]  # [block_size, nkv, d]
            v_block = v[table_idx]  # [block_size, nkv, d]
        ...
        k_full = torch.cat(k_seq, dim=0)[:actual_seq_len]  # [actual_seq_len, nkv, d]
        v_full = torch.cat(v_seq, dim=0)[:actual_seq_len]  # [actual_seq_len, nkv, d]
        ...
        for s1_idx in range(min(s1, actual_seq_len)):
            ...
            # 因果限制
            cur_kv_len = min(s1_idx + 1, actual_seq_len) # 标准因果
            # cur_kv_len = min((s1_idx//step + 1)*step, actual_seq_len)
            ...
            # 获取可见的 KV [cur_kv_len, nkv, d]
            k_visible = k_full[:cur_kv_len]
            v_visible = v_full[:cur_kv_len]
            
            # ========== 修复：逐 head 计算注意力（避免维度广播问题）==========
            out_vec = torch.zeros(nq, d, dtype=torch.float32, device=device)
            ...
            for head_idx in range(nq):
                ...
                # q: [d], k: [cur_kv_len, d], v: [cur_kv_len, d]
                q_h = q_vec[head_idx]  # [d]
                k_h = k_visible[:, kv_head_idx]  # [cur_kv_len, d]
                v_h = v_visible[:, kv_head_idx]  # [cur_kv_len, d]
                ...
                # 应用因果掩码
                if causal_table is not None and s1_idx < causal_table.shape[0]:
                    causal_mask = causal_table[s1_idx, :cur_kv_len]  # [cur_kv_len]
                    scores = scores + causal_mask
                
                # Softmax
                attn_weights = torch.softmax(scores, dim=-1)  # [cur_kv_len]
                
                # 输出: [cur_kv_len] @ [cur_kv_len, d] = [d]
                out_h = torch.matmul(attn_weights, v_h)  # [d]
                out_vec[head_idx] = out_h
            
            output[q_idx] = out_vec.to(q.dtype)
...
def run_pfa_test(atten_cfg):
    ...
    step = 1 #标准因果
    debug_print("PFA_TEST", "create_causal_mask...")
    causal_table = create_causal_mask(
        s1, 
        ((s2 + s2_tile - 1) // s2_tile) * s2_tile, 
        step, 
        device=device_torch
    )
    debug_print("PFA_TEST", f"Created causal_table: {causal_table.shape}, device={causal_table.device}")
    ...
    debug_print("PFA_TEST", "Running PyTorch reference...")
    attention_output = pfa_reference_pytorch_optimized(
        q=q,
        k=k,
        v=v,
        block_table=block_table.to(device=device),  # 确保 block_table 在正确设备
        query_seq_len=query_seq_len.to(device=device),
        causal_table=causal_table,  # 已在 NPU 上
        atten_cfg=atten_cfg,
        device=device_torch,
    )
    ...
    # 逐位置检查差异
    for i in range(min(4, b)): # 4
        for j in range(min(10, s1)): # 4
            diff_j = torch.abs(out_torch[i * s1 + j] - attention_output[i * s1 + j]).max().item()
            debug_print("PFA_TEST", f"DEBUG i={i}, j={j}: max_diff={diff_j:.4f}")
...
def test_pfa():
    ...
    # assert atten_cfg.s2_tile <= atten_cfg.block_size, \
    #     f's2_tile={atten_cfg.s2_tile} must smaller than block_size={atten_cfg.block_size}'
