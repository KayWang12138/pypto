...
def generate_dispatch_golden(
    moe_case: MoeCase,
) -> tuple[TensorList, TensorList, TensorList, TensorList, TensorList, TensorList]:
...
def moe_distributed_combine_kernel(
    ...
    def kernel(
        ...
        for token_id in pypto.loop(batch_size, name='MOE_DISTRIBUTED_RECEIVE', idx_name='token_id'):
            ...
            out[token_id:, 0:] = matmul_out_fp16
...
def assert_allclose_with_eps(expected: torch.Tensor, actual: torch.Tensor, eps: float = 0.001) -> bool:
...
def test_moe_distributed_combine(
    moe_case: MoeCase,
    ...
    expert_scales = expert_scales.to(f'npu:{rank_id}')
    out_golden = out_golden.to(f'npu:{rank_id}')
...
def main() -> None:
    ...
    for expand_x, assist_info_for_combine, recv_counts, expert_scales, out_actual, rank_id in zip(
        expand_x_list,
        assist_info_for_combine_list,
        recv_counts_list,
        expert_scales_list,
        ...
        p = mp.Process(
            target=test_moe_distributed_combine,
            args=(moe_case, expand_x, assist_info_for_combine, recv_counts, expert_scales, out_actual, rank_id),
        )
        p.start()
        processes.append(p)
    for p in processes:
