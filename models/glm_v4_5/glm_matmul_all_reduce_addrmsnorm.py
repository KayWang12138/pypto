...
def matmul_allreduce_addrmsnorm_kernel(bs, ne, h_num, eps, group_name):
    ...
    def kernel(
        ...
        for bs_idx in pypto.loop(bs_loop, name="LOOP_MM_ALLREDUCE_ADDRMSNORM", idx_name="bs_idx"):
            ...
            shmem_data, shmem_signal, shmem_barrier_signal = pypto.distributed.create_shmem_tensor(group_name, WORLD_SIZE, 
                shmem_shape, pypto.DT_FP32)
            ...
            for _ in pypto.loop(1, name="LOOP_MM_AR_ARMS_L0", idx_name="_"):
                ...
                data_clear_dummy = pypto.distributed.shmem_clear(shmem_data, shmem_shape, [0, 0, 0], False, [tile_in_tensor])
                signal_clear_dummy = pypto.distributed.shmem_clear(shmem_signal, shmem_shape, [0, 0, 0], True, [tile_in_tensor])
                pypto.set_vec_tile_shapes(1, 8)
                barrier_dummy = pypto.distributed.shmem_barrier_all(shmem_barrier_signal, group_name, [data_clear_dummy, signal_clear_dummy])
                ...
                wait_dummy = pypto.distributed.shmem_wait(shmem_signal, [shmem_shape], [[0, 0, 0]], WORLD_SIZE, True, [tile_in_tensor])
...
def matmul_allreduce_addrmsnorm_result_golden(bs, num, input_datas):
    ...
    for idx in range(len(input_datas)):
        in_tensor, matmul_weight = input_datas[idx][:2]
    ...
    for idx in range(len(input_datas)):
        residual_input, gamma, bias, eps = input_datas[idx][-4:]
...
def matmul_allreduce_addrmsnorm(
    ...
    inputs = [hidden_states, matmul_weight, residual, input_norm_weight, input_norm_weight, bias, hidden_states_res, residual_res]

    bs, ne = hidden_states.shape
    h_num = hidden_states_res.shape[1]

    matmul_allreduce_addrmsnorm_kernel(bs, ne, h_num, eps, group_name)(*input)
