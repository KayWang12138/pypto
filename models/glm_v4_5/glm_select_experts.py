...
def process_main_loop_interation(
    ...
    # amax
    pypto.set_vec_tile_shapes(view_first, int((num_expert_group + 7) / 8) * 8, group_unit)
    max1 = pypto.amax(r1, -1, False)
    group_weight = max1

    # topk
    pypto.set_vec_tile_shapes(view_first, int((num_expert_group + 15) / 16) * 16)
    ...
    # sum & div
    pypto.set_vec_tile_shapes(view_first, int((topk + 31) / 32) * 32)
