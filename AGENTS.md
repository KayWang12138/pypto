...
#### 算子开发与编排
- `pypto-operator-develop-workflow`：统筹算子开发流程，串联相关技能与关键检查点
- `pypto-intent-understanding`：将自然语言算子需求整理为结构化规格
- `pypto-api-explorer`：探索 API 映射、约束条件与实现可行性
- `pypto-golden-generator`：生成用于精度对比的 golden 参考实现
- `pypto-op-design`：产出算子设计方案，明确数据切分、tiling 与 loop 结构
...
#### 性能分析
- `pypto-op-perf-analyzer`：解读性能数据、定位瓶颈并给出优化依据
- `pypto-op-perf-autotuner`：基于实测性能数据迭代调优，并回验精度与收益
...
- `pypto-pr-fixer`：修复 PR 的 CI 失败与 review 问题
...
- `pypto-pass-workflow-analyzer`：分析 Pass 业务流、执行顺序与数据流转
...
3. **区分事实、推断与建议**
   ...
   - 禁止编造不存在的文件、输出、报错、性能收益、通过状态或用户意图
4. **遵循最小必要改动原则**
...
4. **PyPTO 场景以官方资料和仓内样例为准**
   - 涉及 API 映射、约束、算子行为、编译、精度或性能判断时，优先依据 `docs/`、`examples/`、现有实现和官方文档
   - 当文档、样例与经验判断冲突时，应先指出冲突并回到可核实依据，不凭经验强行定论
5. **验证模式优先使用真实 NPU 环境**
   - 若 `npu-smi info` 检查到存在可用 NPU 环境，且用户未明确要求使用 sim 模式，则禁止使用 sim 模式进行验证
