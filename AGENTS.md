...
1. **统一 timeout 单位与下限（工具层）**
   - 通过工具（如 `bash` / `shell`）执行命令时，`timeout` 统一按 **毫秒（ms）** 理解与设置
   - 禁止使用过小超时（如 10/30 这类毫秒级值）执行秒级命令，避免被误判为执行失败
   - 推荐最小值：
     - 轻量命令（`ls` / `cat` / `grep`）：`5000`~`15000` ms
     - 含 `sleep 10` 的命令：不少于 `15000`~`30000` ms
     - Python 小脚本：不少于 `60000` ms
     - 构建/测试：不少于 `300000` ms

2. **禁止裸 sleep 轮询，改用可观测等待**
   - 禁止仅使用 `sleep N && cat log` 作为进度检查方式
   - 对后台任务（如 `nohup ... &`）必须使用“**进程存活检查 + 日志增量观察**”的可观测轮询
   - 推荐模式：
     - 启动后台任务后记录 PID
     - 使用 `kill -0 <PID>` 判断任务是否仍在运行
     - 周期性 `tail -n` 查看日志增量并输出进度
   - 若任务为重负载长时运行，必须在报告中明确：当前进度、资源占用风险、下一次检查点
