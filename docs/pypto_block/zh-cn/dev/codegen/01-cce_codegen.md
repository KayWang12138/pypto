...
| 组件 | 用途 | 位置 |
| ---- | ---- | ---- |
| `CCECodegen` | 主协调器, 扩展 IRVisitor | `framework/include/block/codegen/cce_codegen.h` |
| `CodeEmitter` | 带缩进的结构化输出 | `framework/include/block/codegen/code_emitter.h` |
| `CodeContext` | 变量名映射和指针跟踪 | `framework/include/block/codegen/code_context.h` |
| `TypeConverter` | IR 类型到 pto-isa C++ 类型 | `framework/include/block/codegen/type_converter.h` |
| `ISAMapper` | IR 操作到 pto-isa 指令 | `framework/include/block/codegen/isa_mapper.h` |
...
**位置:** `python/tests/ut/block/codegen/`
...
- 访问者模式: `framework/include/block/ir/transform/base/visitor.h`
