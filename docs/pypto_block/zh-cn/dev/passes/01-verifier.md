...
**头文件**：`framework/include/block/ir/transforms/property_verifier_registry.h`
...
**头文件**：`framework/include/block/ir/transforms/verifier.h`
...
添加到 `python/pypto_block/pypto_core/passes.pyi`：
...
| 位置 | 用途 |
| ---- | ---- |
| `framework/src/interface/block/transforms/your_rule.cpp` | 实现 |
| `framework/include/block/ir/transforms/passes.h` | 工厂声明（如需暴露） |
| `framework/src/interface/block/transforms/verifier.cpp` | 添加到 `CreateDefault()` |
| `python/bindings/modules/passes.cpp` | Python 绑定 |
| `python/tests/ut/block/ir/transforms/test_verifier.py` | 测试用例 |
...
- **错误处理**（`framework/include/block/core/error.h`）：Diagnostic 和 VerificationError 定义

## 测试

测试覆盖在 `python/tests/ut/block/ir/transforms/test_verifier.py` 中：有效/无效程序验证、规则启用/禁用、异常与诊断模式、Pass 集成、诊断字段访问、报告生成。
