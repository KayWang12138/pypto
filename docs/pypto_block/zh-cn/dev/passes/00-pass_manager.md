...
**头文件**：`framework/include/block/ir/transforms/ir_property.h`
...
**头文件**：`framework/include/block/ir/transforms/pass_context.h`
...
所有单元测试通过 `python/tests/ut/block/conftest.py` 自动在 BEFORE_AND_AFTER 验证模式下运行：
...
**文件**：`python/pypto_block/ir/pass_manager.py`
...
2. 在 `framework/src/interface/block/transforms/` 中**实现**，带有 `PassProperties`
3. 在 `python/bindings/modules/passes.cpp` 中添加 **Python 绑定**
4. **属性声明**：在工厂函数中设置 required/produced/invalidated
5. 在 `python/pypto_block/pypto_core/passes.pyi` 中添加**类型存根**
6. 如果是策略的一部分，在 PassManager 中**注册**
7. 在 `python/tests/ut/block/ir/transforms/` 中添加**测试**

## 测试

- `python/tests/ut/block/ir/transforms/test_ir_property.py` — IRProperty/IRPropertySet 测试
- `python/tests/ut/block/ir/transforms/test_pass_pipeline.py` — Pipeline、PassContext、插桩和自动验证测试
- `python/tests/ut/block/ir/transforms/test_pass_manager.py` — PassManager 向后兼容性测试
- `python/tests/ut/block/conftest.py` — 为所有测试启用 BEFORE_AND_AFTER 验证的 autouse fixture
