...
| 分类 | 类型 | 用途 | 文件位置 |
| ---- | ---- | ---- | -------- |
| **TensorOp** | TensorType | 支持广播的 N 维张量 (Tensor) 操作 | `framework/src/interface/block/op/tensor_ops/` |
| **BlockOp** | TileType | 硬件优化的块操作 | `framework/src/interface/block/op/block_ops/` |
| **SyncOp** | UnknownType/PipeType | 流水线屏障和同步 | `framework/src/interface/block/op/sync_ops/` |
...
**用途**：支持完整广播的通用 N 维张量
**类型**：`TensorType`（任意维度）
**位置**：`framework/src/interface/block/op/tensor_ops/`
...
**位置**：`framework/src/interface/block/op/block_ops/`
...
**位置**：`framework/src/interface/block/op/sync_ops/`
...
**C++ 注册 (`framework/src/interface/block/op/sync_ops/sync.cpp`)：**
...
| 目录/文件 | 内容 |
| --------- | ---- |
| `framework/src/interface/block/op/type_inference.cpp` | 共享的类型推断工具 |
...
1. **选择分类文件**：`framework/src/interface/block/op/tensor_ops/elementwise.cpp`、`matmul.cpp`、`reduction.cpp`，或 `framework/src/interface/block/op/block_ops/memory.cpp`、`unary.cpp`
...
4. **Python 封装** (`python/pypto_block/ir/op/tensor_ops.py`)：
...
5. **添加测试**，位于 `python/tests/ut/block/ir/`，如需要则更新 `CMakeLists.txt`

## 参考

- 常用常量：`framework/include/block/core/common.h`
- 类型定义：`framework/include/block/ir/type.h`
- 算子注册表：`framework/include/block/ir/op_registry.h`
- 类型推断工具：`framework/include/block/ir/type_inference.h`
- 类型推断实现：`framework/src/interface/block/op/type_inference.cpp`
- 算子注册表实现：`framework/src/interface/block/op_registry.cpp`
- 张量算子实现：`framework/src/interface/block/op/tensor_ops/`
- 块算子实现：`framework/src/interface/block/op/block_ops/`
- 同步算子实现：`framework/src/interface/block/op/sync_ops/`
