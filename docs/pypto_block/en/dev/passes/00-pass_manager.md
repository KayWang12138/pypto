...
**Header**: `framework/include/block/ir/transforms/ir_property.h`
...
**Header**: `framework/include/block/ir/transforms/pass_context.h`
...
All unit tests automatically run with BEFORE_AND_AFTER verification via `python/tests/ut/block/conftest.py`:
...
**File**: `python/pypto_block/ir/pass_manager.py`
...
2. **Implement** in `framework/src/interface/block/transforms/` with `PassProperties`
...
5. **Type stub** in `python/pypto_block/pypto_core/passes.pyi`
6. **Register** in PassManager if part of a strategy
7. **Test** in `python/tests/ut/block/ir/transforms/`

## Testing

- `python/tests/ut/block/ir/transforms/test_ir_property.py` — IRProperty/IRPropertySet tests
- `python/tests/ut/block/ir/transforms/test_pass_pipeline.py` — Pipeline, PassContext, instruments, and automatic verification tests
- `python/tests/ut/block/ir/transforms/test_pass_manager.py` — PassManager backward compatibility
- `python/tests/ut/block/conftest.py` — Autouse fixture enabling AFTER verification for all tests
