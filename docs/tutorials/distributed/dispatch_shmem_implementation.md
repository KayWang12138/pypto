...
| topk | int | 每个token选择的专家数量，固定为 8 |
| data_type | pypto.DataType | 数据类型，固定为 pypto.DT_BF16 |
| ep_world_size | int | 专家并行的rank数量，支持 4 或 8 |
...
| assist_info_for_combine | [expand_x_row, info_size] | DT_INT32 | 辅助信息，用于combine阶段恢复token顺序 |
...
**目的**: 计算每个token在目标专家共享内存区域中的写入偏移量，避免多个token写入同一位置。
...
3. 遍历所有token（batch_size * topk个），对每个 token：
...
- 通过比较expert_ids向量中与目标expert_id相等的元素数量
- 使用cumsum累计相等数量
- 返回当前token在该专家接收序列中的偏移量
...
**目的**: 将token数据和辅助信息发送到目标rank的共享内存区域。

**实现步骤**:
1. 遍历所有token（batch_size * topk个）
2. 对每个token：
   - **构造辅助信息**:
     - `moe_info[0, info_size-3] = this_rank`: 记录发送rank
     - `moe_info[0, info_size-2] = row_index`: 记录token_id
     - `moe_info[0, info_size-1] = col_index`: 记录k_offset
   ...
   - **计算目标位置**:
     ...
     - `token_offset = offset_table[row_index, col_index]`: 该expert内的token偏移
...
每个 expert 占用 ep_world_size 行，记录从各个 rank 接收的数量，有序用于做 CumSUM 操作计算每一条接收token在结果的行偏移
...
- expert 0的计数信息在`cum_sum_input[1:5, 0]`（从rank 0,1,2,3接收的 token 数量）
- expert 0的总接收数量为`sum(cum_sum_input[1:5, 0])`
- expert 0的0-3卡的接收数据在 expand_x 中的行偏移为`cum_sum_result[1:5, 0]`
...
```
expand_x布局:
[0:expert_0_count, :]          <- expert 0的 token 数据
[expert_0_count:expert_1_count, :]  <- expert 1的 token 数据
...
[sum(prev_counts):, :]        <- expert N的 token 数据
