...
本教程旨在为用户提供一套分析、定位通信算子 shmem_wait_until 超时，助力用户在使用 PyPTO 进行通信算子开发时能快速定位 shmem_wait_until 超时问题。

## 背景介绍

目前主流的模型训练/推理过程中都会通过集合通信库 (CCL) 来实现多卡环境下的数据交换以减少算子的训练/推理时间。下面通过该算子介绍超时报错的大致步骤，完整样例请参考：[glm_matmul_allreduce_add_rmsnorm.py](../../../models/glm_v4_5/glm_matmul_allreduce_add_rmsnorm.py)。
...
1. 打开 device 侧日志开关
    ...
    # 重定向日志落盘路径 默认目录位置$HOME/ascend/log
    export ASCEND_PROCESS_LOG_PATH={YOUR_PATH}
    ```
2. 确定 shmem_wait_until 超时

    在 [shmem_wait_until.cpp](../../../framework/src/machine/device/distributed/shmem_wait_until.cpp) 文件 SignalTileOp::PollCompleted() 函数第一行前增加打印日志:
    ```cpp
...
1. 增加定位日志，查看在 shmem_wait_until 任务完成几个
    ...
    ```
    其中：
    （1）SendTaskToAicore 表示task下发到aicore, taskId及前后依赖关系见 dyn_topo.txt (dyn_topo.txt 生成请参考：[machine.md](../../trouble_shooting/machine.md) leaf function 粒度的内存重叠检测章节)。
    （2）ReSolveByRegVal 表示下发到 aicore task已完成。
...
3. 检查写共享信号缓冲区的信号值是否符合预期。
    ...
    - tileop_shmem.h 文件 ShmemSignal() 增加日志，
    ```cpp
