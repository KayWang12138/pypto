...
- **put_dummy 切分**：`put_dummy` 同步切分为两个独立节点，分别作为对应 `tile_shmem_put` 的输出、`tile_shmem_signal` 的输入，构建 tileop 级别的依赖关系。
...
- **shmem_get 切分**：单个 `shmem_get` 算子被切分为 4 个 tileop：`tile_shmem_get0`、`tile_shmem_get1`、`tile_shmem_get2`、`tile_shmem_get3`，实现分块读取；
