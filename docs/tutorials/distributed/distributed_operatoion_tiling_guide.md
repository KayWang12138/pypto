...
切块大小**TileShape**通过`set_vec_tile_shapes`设置，它决定了数据如何被切分以及对应的操作如何展开。切块配置对通信算子的影响主要体现在：
...
将数据类型的tensor简称为**data tensor**，控制边类型的tensor简称为**dummy tensor**。二者在通信过程中的语义与作用各不相同。
...
1. **shmem_signal 写信号量时需确保对应的数据已经发送完成**
