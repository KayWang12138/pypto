# 编程范式<a name="ZH-CN_TOPIC_0000002370495134"></a>
...
## PTO编程范式概述<a name="section15666635174718"></a>
...
## 核心数据结构<a name="section14875744453"></a>
...
-   Tile：是Tensor的子区间（sub-Tensor），通过Tiling（切分）将大的Tensor切分为多个子块。Tile的设计目的是：
    ...
    在Tensor层次编程中，Tiling由框架自动完成，开发者只需通过配置接口指定Tile Shape，框架会自动进行切分。
...
## Tensor层次编程<a name="section46121919625"></a>
...
-   Function定义
    ...
    在PyPTO中，计算逻辑通过Function来组织，Function具有以下特点：

    -   SSA语义：支持单静态赋值（Single Static Assignment），确保数据流的清晰性
    -   函数嵌套：支持函数嵌套和调用

    Kernel入口函数通过@pypto.jit装饰器定义，在第一次调用时会进行JIT编译。
...
## 计算图<a name="section13214492213"></a>
...
-   计算图的组成
    ...
    -   Function：计算图的抽象，支持SSA语义与闭包语义。

-   计算图的构建

    计算图在Function中构建：

    -   图构建阶段：在Function内部，每个Tensor操作都会被添加到当前的计算图中
    -   Shape推断：框架自动推断每个Tensor的Shape
    -   依赖分析：分析操作之间的依赖关系
    -   图优化：应用各种优化Pass
...
-   计算图的转换流程
    ...
    ![](figures/CANN架构图-0.png)

    -   Tensor Graph Passes：Tensor级别的优化，如冗余操作消除、类型转换、内存冲突推断等
    -   Tile Graph Passes：Tile级别的优化，如内存类型分配、移动操作生成、子图切分等
    -   Block Graph Passes：Block级别的优化，如内存重用、同步点插入、乱序调度等
...
## MPMD执行模型<a name="section1243232114188"></a>
...
![](figures/CANN架构图-1.png)

## 编程示例<a name="section4188312162419"></a>
...
## 总结<a name="section827101192715"></a>
