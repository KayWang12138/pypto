...
综上考虑，在初始算子开发阶段可以先使用如下设置，后续再进一步调优。
...
### 调优步骤
我们应该按照优化项对性能影响的期望大小顺序进行算子的调优。根据已有算子的开发经验，优化步骤的排序应如下：
1. stitch配置项
2. Matmul与Vector的性能
3. 更改调度策略
...
当前stitch配置主要由3个参数决定，在jit装饰器中完成配置，可参考如下配置：
...
```
各参数含义与规格请参考对应的API介绍，后续该组参数将优化合并。
...
L1Reuse和CubeNBuffer的pass设置的具体介绍见。
两者都是用于合并Cube子图，前者用于合并具有冗余L1搬运的子图、后者用于合并同构的子图。L1Reuse可以减少L1搬运量，CubeNBuffer可以在不同分支的矩阵乘间隐藏搬运和计算耗时。它们都能减少子图调度开销。
考虑到实际的矩阵乘场景大多数是搬运bound，性能优化的重点是减少搬运量。因此用户应当优先使用L1Reuse。CubeNBuffer针对的是不能使能L1Reuse的场景，主要有以下两种：
...
1\. 通过`set_cache_policy`设置合理的L2 CacheMode，对于只访问一次的Global Memory数据设置其访问状态为不进入L2 Cache。

2\. 矩阵乘前后有transpose时，可以尝试更换左右矩阵并使用左右矩阵转置的配置。由于N轴在尾轴上，因此当M轴较大、N轴较小时，也可以尝试该方法，使得左右矩阵有更大的尾轴，提升搬运带宽。

3\. 特殊shape时可以尝试使用Vector操作提前处理输入矩阵，使其变成更标准的shape。以左右矩阵shape分别为(884736,16)和(16,16)的矩阵乘为例。如果只使用L1Reuse，只能优化到500us。而使用下面的写法，提前将四个重复的右矩阵在对角线拼成一个shape为(64,64)的新的右矩阵c，将它和reshape后的左矩阵做矩阵乘，则算子性能大幅提升到40us。
...
2\. 检查是否有不合理数据操作导致的冗余搬运，例如更换concat为assemble、尝试对reshape使用`inplace = True`。

3\. Vector的优化和矩阵乘的优化一样，需要避免处理小尾轴的Tensor。使用较大的Tilesize也无法避免OP的输入Tensor的尾轴较小时，我们可以使用concat、transpose或reshape等数据操作OP来增大尾轴。对矩阵求逆算子的优化用到了这一方法，具体介绍见。

#### 调整切图旋钮

切图配置旋钮的具体介绍见。需要注意的是，应先进行前面的优化步骤优化Tilesize，才能通过切图的旋钮得到较优的结果。因为不合适的Tilesize会导致复杂的依赖关系，从理论上无法得到既多核并行又融合多个OP的子图任务。
...
这一部分的具体介绍见。需要注意的是，应先进行前面的优化步骤将上下游子图的切分和合并调到较合适后，再尝试使用vecNBuffer来进行广度合并。当泳道图内有同构子图组具有大量的小子图（耗时在10u以下）时，应该使用该功能进行优化，以减少AICPU调度开销和kernel的头开销。
...
### 4. 更改调度策略
PyPTO算子的流水包括核间的流水与核内的流水。

核间的流水由AICPU对子图的调度确定，它基于子图间的依赖关系和核间任务的调度策略。我们可以尝试更改该调度策略以达到更优的算子性能。具体的策略列表见。当上下游子图之间依赖较为简单，或下游子图输入Tensor的L2命中率较为重要时，推荐使用L2亲和调度，配置方式如下：
