...
- 精度仿真：支持用户在CPU环境获取算子运算结果（精度仿真依赖CANN软件包，请参考[安装工具包](../../install/prepare_environment.md#安装工具包)）。
...
5. 精度仿真与NPU执行一致，执行完成后，会返回运行结果，用户可获取结果并进行处理（参考[样例运行](../../invocation/examples_invocation.md)）。