# 条件与分支<a name="ZH-CN_TOPIC_0000002527348233"></a>
...
## 静态条件分支<a name="section17790155919810"></a>
...
完整样例请参考：pypto/examples/01\_beginner/00\_introduction/add\_scalar\_loop\_static\_cond.py

## 动态条件分支<a name="section1399343718364"></a>

运行时判断条件及分支，执行对应的功能。核心接口包括：

-   pypto.cond\(condition\)：运行时判断条件。
-   pypto.is\_loop\_begin\(idx\)：判断是否为循环首个迭代。
-   pypto.is\_loop\_end\(idx\)：判断是否为循环最后一个迭代。
...
examples/01\_beginner/00\_introduction/add\_scalar\_loop\_dyn\_cond.py

examples/01\_beginner/00\_introduction/add\_scalar\_loop\_dyn\_loop\_cond.py
