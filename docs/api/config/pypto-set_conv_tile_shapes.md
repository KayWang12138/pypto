...
- buffer空间约束：
    ...
    - L1空间约束：
        ...
        ```
        CeilAlign(mL1,16)* CeilAlign(kL1, C0) * sizeof(aDtype) + CeilAlign(nL1,16) * CeilAlign(kL1, C0) * sizeof(bDtype) + CeilAlign(tileN,16)*sizeof(aDtype) <= L1_size
        ```
        ...
        - `aDtype`、`bDtype` 为输入数据类型，`cDtype` 固定为DT_FP32

        - `C0 = ALIGN_SIZE_32 / BytesOf(outType)`
