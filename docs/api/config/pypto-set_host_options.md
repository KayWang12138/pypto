...
| 参数名          | 输入/输出 | 说明                                                                 |
|-----------------|-----------|----------------------------------------------------------------------|
| compile_stage    | 输入      | 含义：控制编译执行的阶段 <br> 说明：<br> 0: 不控制编译执行阶段，完整执行；<br> 1: 生成TensorGraph后，终止后续执行；<br> 2: 仅完成Host编译，不执行Codegen编译，且终止后续执行; <br> 3: 执行完Codegen生成kernel代码后，忽略静态的上板流程，终止后续执行; <br> 类型: int <br> 取值范围: 0或1或2或3 <br> 默认值: 0 |
