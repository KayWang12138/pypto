...
| aicpu_advanced_time            | 输入      | 含义：设置AICPU提前启动时间 (单位: us) <br> 说明：AICPU 提前启动的时间，用于测准算子kernel耗时 <br> 类型：int <br> 取值范围：0~1000000 <br> 默认值：0 <br> 影响pass范围：NA |
| repeat_time                    | 输入      | 含义：设置单算子重复执行次数 <br> 说明：指定单算子在 Device 侧重复执行的轮数，稳定kernel耗时，同时仅采集最后一轮稳定的profiling数据 <br> 类型：int <br> 取值范围：5~1000000 <br> 默认值：5 <br> 影响pass范围：NA |
