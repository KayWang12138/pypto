# pypto.sigmoid<a name="ZH-CN_TOPIC_0000002503358001"></a>
...
对输入张量的每个元素应用 sigmoid 激活函数，计算公式为：

![](figures/zh-cn_formulaimage_0000002470577932.png)
...
返回一个与输入形状相同、数据类型一致的Tensor，其元素为输入元素经 sigmoid 函数映射到 \(0, 1\) 区间的结果。
