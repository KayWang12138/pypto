# pypto.logical\_and<a name="ZH-CN_TOPIC_0000002530282507"></a>
...
对两个输入的 Tensor 进行逐元素逻辑与（AND）运算。运算规则：

-   如果输入的 Tensor 为 bool 则 True and True -\> True，其余情况皆为 False。
-   如果输入的 Tensor 为数值，会自动转换成 True/False，0 为 False，非 0 为 True。
...
<p id="p1985220257305"><a name="p1985220257305"></a><a name="p1985220257305"></a>支持的数据类型为：DT_FP32，DT_FP16，INT8，UINT8，BOOL。</p>
<p id="p45217198302"><a name="p45217198302"></a><a name="p45217198302"></a>不支持空Tensor，且Shape仅支持2-4维，支持输入Tensor的数据类型不同，支持广播。</p>
</td>
</tr>
<tr id="row53361133143410"><td class="cellrowborder" valign="top" width="17.29%" headers="mcps1.1.4.1.1 "><p id="p123678376342"><a name="p123678376342"></a><a name="p123678376342"></a>other</p>
</td>
<td class="cellrowborder" valign="top" width="9.84%" headers="mcps1.1.4.1.2 "><p id="p2367163715347"><a name="p2367163715347"></a><a name="p2367163715347"></a>输入</p>
</td>
<td class="cellrowborder" valign="top" width="72.87%" headers="mcps1.1.4.1.3 "><p id="p1136713743418"><a name="p1136713743418"></a><a name="p1136713743418"></a>源操作数。</p>
<p id="p16801338193510"><a name="p16801338193510"></a><a name="p16801338193510"></a>支持的数据类型为：DT_FP32，DT_FP16，INT8，UINT8，BOOL。</p>
<p id="p1968023814352"><a name="p1968023814352"></a><a name="p1968023814352"></a>不支持空Tensor，且Shape仅支持2-4维，支持输入Tensor的数据类型不同，支持广播。</p>
</td>
</tr>
</tbody>
</table>
...
返回输出Tensor，Tensor的数据类型为BOOL，形状为广播后的形状。

## 约束说明<a name="section4426761937"></a>

1.  tileshape与self、other维度保持一致；
2.  由于存在临时内存使用，tileshape大小有额外约束，假设tileshape:\[a,b,c,d\]，那么a\*b\*c\*d\*sizeof\(self\) + a\*b\*c\*d\*sizeof\(other\) + a\*b\*c\*d\*sizeof\(BOOL\) + 1.1875KB<UB。
