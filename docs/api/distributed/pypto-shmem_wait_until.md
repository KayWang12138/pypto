...
| 参数名  | 输入/输出 | 说明                                                                 |
|---------|-----------|----------------------------------------------------------------------|
| src   | 输入      | 共享内存GM (win区) 中的信号张量 。 <br> Tensor支持的数据类型为：DT_INT32。 <br> 不支持空Tensor; Shape仅支持5维。 <br> 必须是通过create_shmem_tensor或create_shmem_siganl接口创建的用于进程间同步的信号张量。|
...
| offset   | 输入      | 信号张量在共享内存GM (win区)中的偏移量。 <br> 支持 int 或 SymbolicScalar 类型的列表。 <br> Shape仅支持5维。 |
