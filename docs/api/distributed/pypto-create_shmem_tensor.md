...
## 功能说明
创建一个 shared memory tensor 用于 pe 间进行数据传输。
...
| shape   | 输入      |创建的 shared memory tensor 的形状。 <br> 参数类型为 list[int] 类型。 <br> 运行时判断当前创建的 shared memory tensor 是否超出 win 区大小，进行报错提示。 |

## 返回值说明

生成一个 shared memory tensor 用于 pe 间进行数据传输。
