...
清除当前 pe 对应的 shared memory tensor 的部分视图
...
-   示例1：创建了一个 shape = [1, 128, 256] 的 shared memory tensor，清除当前 pe 对应的 shared memory tensor 的部分视图的数据。该部分视图的 shape 为 [1, 128, 128], offset 为 [0, 0, 0]。
...
-   示例2：创建了一个 shape = [1, 128, 256] 的 shared memory tensor，清除当前 pe 对应的 shared memory tensor 的全部视图的数据。
