...
| 参数名  | 输入/输出 | 说明                                                                 |
|---------|-----------|----------------------------------------------------------------------|
| src   | 输入      |  要清除的共享内存 GM (win区) 信号张量 。 <br> 必须是 create_shmem_siganl 接口创建的张量。|
| shape   | 输入      | 要清除的共享内存 GM (win区) 信号张量的形状。 <br> 参数类型为 list[int] 类型。 <br> 仅支持3维。 |
| offsets   | 输入      | 要清除的共享内存 GM (win区) 信号张量的偏移量。 <br> 支持 int 或 SymbolicScalar 类型的列表。 <br> 仅支持3维。 <br> 需要保证 offsets 小于 shape。 |
