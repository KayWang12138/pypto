...
| 参数名  | 输入/输出 | 说明                                                                 |
|---------|-----------|----------------------------------------------------------------------|
| dst   | 输入      | 共享内存GM (win区) 中的信号张量 。 <br> 必须是通过 create_shmem_tensor 或 create_shmem_siganl 接口创建。|
| dst_pe   | 输入      | 共享内存张量所属的设备编号, 0 <= pe < n_pes。 <br> 支持的数据类型为 int 或 SymbolicScalar 类型。 |
| signal   | 输入      | 写入到共享内存 GM (win区) 中的信号值。 <br> 支持的数据类型为：int类型。 |
| shape   | 输入      | 共享内存 GM (win区) 信号张量的形状。 <br> 参数类型为 list[int] 类型。 <br> Shape 仅支持3维。 |
| offset   | 输入      | 信号张量在共享内存 GM (win区) 中的偏移量。 <br> 支持 int 或 SymbolicScalar 类型的列表。 <br> 仅支持3维。 |
