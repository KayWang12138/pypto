...
| 参数名  | 输入/输出 | 说明                                                                 |
|---------|-----------|----------------------------------------------------------------------|
| src   | 输入      |  要提取局部视图的输入张量，必须是共享内存张量 。 <br> 必须是通过 create_shmem_tensor 接口创建的。 |
| shape   | 输入      | 获取共享内存视图的形状。 |
| offsets   | 输入      | 获取每个维度相对于输入张量的偏移量，用于构造共享内存视图。 <br> 需要保证 offsets 小于输入张量的 shape。 |
| valid_shape  | 输入      | 取出指定示意块中有效数据大小。 <br> 需要保证 valid_shape 小于输入张量的 shape。 |
