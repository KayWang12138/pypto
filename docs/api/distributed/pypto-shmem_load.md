...
| 参数名  | 输入/输出 | 说明                                                                 |
|---------|-----------|----------------------------------------------------------------------|
| src   | 输入      | 位于共享内存 GM (win区) 的源张量。 <br> Tensor支持的数据类型为：DT_INT32, DT_FP16, DT_FP32, DT_BF16。 <br> 不支持空Tensor; Shape仅支持4维; <br> 必须是通过create_shmem_tensor接口创建的用于传输数据tensor。 |
| src_pe   | 输入      | 源设备的 src_pe。 <br> 0 <= src_pe < n_pes。 <br> 支持的数据类型为 int 或 SymbolicScalar 类型。 |
| shape   | 输入      | 本地 ub 张量的形状。 <br> 参数类型为list[int]类型。 <br> Shape仅支持3维。 |
| offset   | 输入      | 目标张量在本地 ub 中的偏移量。 <br> 支持 int 或 SymbolicScalar 类型的列表。 <br> Shape仅支持3维。 |
