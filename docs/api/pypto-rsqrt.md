# pypto.rsqrt<a name="ZH-CN_TOPIC_0000002470438106"></a>
...
计算输入Tensor中每个元素的平方根倒数，逐元素运算，返回与输入形状相同的Tensor。当输入为负数时返回 NaN，输入为零时返回 Inf。
...
<p id="p1913163132011"><a name="p1913163132011"></a><a name="p1913163132011"></a>不支持空Tensor，Shape仅支持2-4维，且Shape Size不大于2147483647（即INT32_MAX）。</p>
</td>
</tr>
</tbody>
</table>
...
返回一个与输入形状相同、数据类型一致的Tensor，其元素为输入张量对应元素的平方根倒数。
