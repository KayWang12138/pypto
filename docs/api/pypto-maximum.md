# pypto.maximum<a name="ZH-CN_TOPIC_0000002503358009"></a>
...
计算输入与另一输入的逐元素最大值。支持二维、三维或四维的张量。
...
当两个源操作数均为张量时，两个张量必须满足广播关系。该接口返回一个与源操作数一和源操作数二广播后形状相同的张量，数据类型与源操作数相同，其元素为源操作数一和源操作数二的逐元素最大值。且源操作数为张量时，源操作数一和源操作数二均仅支持单轴广播。

当两个源操作数之中存在一个张量时，返回与输入张量相同形状的张量，其元素为源操作数一和源操作数二的逐元素最大值。
