...
from_torch(tensor: torch.Tensor, name: str="", *, dynamic_axis: Optional[List[int]] = None,
           tensor_format: Optional[TileOpFormat] = None, dst_dtype: Optional[DataType] = None) -> pypto.Tensor
```
...
| dst_dtype      | 输入      | 要指定的pypto.DataType类型。为None时根据torch.Tensor的dtype自动推导。 |
...
print(z_pto.format)
k = torch.randn(2, 3)
k_pto = pypto.from_torch(k, "k", dst_dtype=pypto.DataType.DT_HF8)
print(k_pto.dtype)
```
