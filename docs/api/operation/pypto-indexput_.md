...
|   参数�?  | 输入/输出 | 说明                                                                  |
|------------|-----------|----------------------------------------------------------------------|
|   input    |    输入   | 源操作数�?<br> 支持的类型为：Tensor�?<br> Tensor支持的数据类型为：DT_INT8，DT_UINT8，DT_INT16，DT_UINT16，DT_INT32，DT_UINT32，DT_INT64，DT_UINT64，DT_BF16，DT_FP16，DT_FP32�?<br> 不支持空Tensor，Shape仅支�?-4维，Shape Size不大�?147483647（即INT32_MAX）�?|
...
2. values不支持broadcast，其�?维的shape须和indices中一维Tensor的shape相同。若values的维度大于等�?，那么其除第0维外的后i个维度（i>0）和input后i个维度的shape完全相同�?
...
如输入input为[m, n, p]，输入indices�?[t])，输入values为[t, n, p], TileShape设置为[t1, n1, p1]，则t1用于切分t轴，n1用于切分n轴，p1用于切分p轴，m轴不切�?

如输入input为[m, n, p]，输入indices�?[t])，输入values为[t, n, p], TileShape设置为[t1, n1]，则TileShape会自动补全为[t1, n1, p]，t1用于切分t轴，n1用于切分n轴，m轴和p轴不切�?

如输入input为[m, n, p]，输入indices�?[t], [t])，输入values为[t, p], TileShape设置为[t1, p1], 则t1用于切分t轴，p1用于切分p轴，m轴和n轴不切�?
