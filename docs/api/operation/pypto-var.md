...
| 参数�?     | 输入/输出 | 说明                                                                 |
|------------|----------| |----------------------------------------------------------------------|
| input      | 输入      | 源操作数�?<br> 支持的类型为：Tensor�?<br> Tensor支持的数据类型为：DT_FP32, DT_FP16, DT_BF16�?<br> 不支持空Tensor；Shape仅支�?-4维；Shape Size不大�?147483647（即INT32_MAX）�?|
...
keepdim为True时，对应dim的shape规约�?，其他轴的shape不变；keepdim为False时，对应dim会被移除�?
## 约束说明
