...
| 参数�? | 输入/输出 | 说明                                                                 |
|---------|-----------|----------------------------------------------------------------------|
| input   | 输入      | 源操作数�?<br> 支持的类型为：Tensor�?<br> Tensor 支持的数据类型为：DT_FP32，DT_FP16，DT_BF16，DT_BOOL，DT_INT8，DT_UINT8�?<br> 不支持空Tensor；Shape仅支�?-4维；Shape Size不大�?147483647（即INT32_MAX）�?|

## 返回值说�?

返回输出Tensor，Tensor的数据类型为DT\_BOOL，Shape 与源操作�?input Shape相同 �?
