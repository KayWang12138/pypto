...
```python
pypto.rsqrt(input, precision_type=pypto.RsqrtAlgorithm.HIGH_PRECISION) -> Tensor
```
...
| precision_type | RsqrtAlgorithm, 可选 | 平方根倒数操作的精度模式。默认值为 `RsqrtAlgorithm.HIGH_PRECISION`。<br>**HIGH_PRECISION**：使用更高精度的计算方式，减少精度损失。<br>**INTRINSIC**：直接使用芯片指令进行计算。 |
