...
| 参数�?| 输入/输出 | 说明                                                                                                                                                                                                              |
| ------ | --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| input  | 输入      | 源操作数。`<br>` 支持的类型为：Tensor�?<br> Tensor支持的数据类型为：DT_FP16，DT_BF16，DT_INT16，DT_INT32，DT_FP32�?<br> 不支持空Tensor；Shape仅支�?-4维；Shape Size不大�?147483647（即INT32_MAX）�?|
...
1. dim：指定计算累积和的维度，必须在输入Tensor input的有效维度范围内，其值需满足-input.dim <= dim < input.dim�?2. input的dim轴ViewShape不可切分，其余维度不做限制；
