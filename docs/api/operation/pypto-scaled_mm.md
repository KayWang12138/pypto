...
| 参数名            | 说明                                                                 |
|-------------------|----------------------------------------------------------------------|
| bias_tensor       | 表示偏置矩阵。 <br> 输入为Tensor类型。 <br> Bias矩阵数据类型可选DT_FP16和DT_FP32。 <br> bias_tensor只支持ND格式。 <br> bias_tensor的第一维度应置1，且N维度需要与mat_b矩阵的N维度相等。 <br> Bias不支持多核切K功能。 <br> 仅支持矩阵维度为2维场景。 |
