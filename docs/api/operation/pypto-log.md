...
```python
pypto.log(input, precision_type=pypto.LogAlgorithm.HIGH_PRECISION) -> Tensor
```
...
| precision_type | LogAlgorithm, 可选 | 对数操作的精度模式。默认值为 `LogAlgorithm.HIGH_PRECISION`。<br>**HIGH_PRECISION**：使用更高精度的计算方式，减少精度损失。<br>**INTRINSIC**：直接使用芯片指令进行计算。 |
