...
| 参数名  | 输入/输出 | 说明                                                                 |
|---------|-----------|----------------------------------------------------------------------|
| input   | 输入      | 支持的类型为：Tensor。 <br> Tensor支持的数据类型为：DT_FP32, DT_FP16, DT_BF16, DT_DT_INT32, DT_INT16。 <br> 支持的维度：2维，4维 <br> 2维Shape[blockNum * blockSize, d]，4维Shape[blockNum, blockSize, 1, d] <br> 不支持空Tensor；Shape Size不大于2147483647（即INT32_MAX）。 |
| dim     | 输入      | 请保持默认值-2。 |
