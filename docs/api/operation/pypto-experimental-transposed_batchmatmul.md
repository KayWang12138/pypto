...
该算子执行转置批量矩阵乘法。具体操作为�?1. 将输入张�?`tensor_a` 从形�?(M, B, K) 转置�?(B, M, K)�?2. 执行批量矩阵乘法，将转置后的 `tensor_a` (B, M, K) �?`tensor_b` (B, K, N) 相乘，得到中间结�?(B, M, N)�?3. 将中间结果转置回形状 (M, B, N) 作为最终输出�?
## 函数原型
...
| 参数�?   | 输入/输出 | 说明                                                                 |
|-----------|-----------|----------------------------------------------------------------------|
| tensor_a  | 输入      | 左侧输入张量�?<br> 支持的数据类型为：DT_FP16, DT_BF16�?<br> 不支持空Tensor，支持三维�?<br> 形状必须�?(M, B, K)�?|
| tensor_b  | 输入      | 右侧输入张量�?<br> 支持的数据类型为：DT_FP16, DT_BF16�?<br> 不支持空Tensor，支持三维�?<br> 形状必须�?(B, K, N)�?|
