...
计算自然对数的底数e�?input - other)次幂，其中other的尾轴或次尾轴的值为1，返回与输入input数据类型及形状相同的Tensor�?
$$
e^{(input - other)}
$$
...
| 参数�? | 输入/输出 | 说明                                                                 |
|---------|-----------|----------------------------------------------------------------------|
| input   | 输入      | 源操作数�?<br> 支持的类型为：Tensor�?<br> Tensor支持的数据类型为：DT_FP16，DT_FP32, DT_BF16�?<br> 不支持空Tensor；Shape仅支�?-4维，并支持按照单个维度广播到相同形状；Shape Size不大�?147483647（即INT32_MAX）�?|
| other   | 输入      | 源操作数�?<br> 支持的类型为：Tensor�?<br> Tensor支持的数据类型为：DT_FP16，DT_FP32, DT_BF16�?<br> 不支持空Tensor；Shape仅支�?-4维，并支持按照单个维度广播到相同形状；尾轴或次尾轴的值必须为1；Shape Size不大�?147483647（即INT32_MAX）�?|

## 返回值说�?
返回输出Tensor，其数据类型及形状和input相同�?
## 约束说明

1. input �?other 类型应该相同�?2. other 的尾轴或次尾轴的值为1�?
## TileShape设置示例
