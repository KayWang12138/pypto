...
| pad    | 输入      | 填充大小序列。<br> 支持的类型为：tuple 或 list (包含int)。<br> 序列长度 $m$ 必须为偶数，且满足 $\frac{m}{2} \leq$ `input` 的维度数。<br> 格式为：`(pad_left, pad_right, pad_top, pad_bottom, ...)` <br> 所有填充值必须为非负整数，负值不支持。                           |
...
1. `pad` 参数的长度必须为2或者4，`pad` 参数中的所有填充值必须为非负整数。负值填充不支持。如果传入负值，将抛出`ValueError`。
