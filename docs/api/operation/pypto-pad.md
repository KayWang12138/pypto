...
| 参数名 | 输入/输出 | 说明                                                                                                                                                                                                                           |
| ------ | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| input  | 输入      | 需要进行填充的源操作数。<br> 支持的类型为：Tensor。<br> Tensor支持的数据类型为：DT_FP32、DT_FP16、DT_BF16。<br> 不支持空Tensor；Shape Size不大于2147483647（即INT32_MAX）。                                                    |
...
| value  | 输入      | 当填充模式为常量填充 (`'constant'`) 时的填充值。<br> 支持的类型为：float。<br> 默认值：`0.0`。                                                                                                                                 |
...
1. `pad` 参数的长度必须为偶数。
2. 当前算子实现**仅支持在右侧（Right）和底部（Bottom）进行填充**。即 `pad` 序列中向左和向上的填充量必须为 0（例如格式必须为 `(0, pad_right, 0, pad_bottom)`）。
3. 当前算子实现**仅支持 `'constant'`（常量填充）模式**，其他模式暂不支持。
4. 如果 `input` 不是 Tensor 类型，或 `pad` 不是整数序列，将抛出 `TypeError`。
...
out = pypto.pad(t4d, p1, mode="constant", value=9.0)
```
