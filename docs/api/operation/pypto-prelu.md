...
�?input 的每个元素进行带参数的整流线性单元运算，当元素值大于等�?时保持不变，小于0时乘以权重系数。计算公式如下：
...
| 参数�? | 输入/输出 | 说明                                                                 |
|---------|-----------|----------------------------------------------------------------------|
| input   | 输入      | 源操作数�?<br> 支持的类型为：Tensor�?<br> Tensor支持的数据类型为：DT_FP16、DT_FP32、DT_BF16�?<br> 不支持空Tensor；Shape仅支�?-4维；Shape Size不大�?147483647（即INT32_MAX）�?|
...
1.  input �?weight 类型应该相同�?2.  weight 的Shape必须为一维，且长度等�?input 的第二维大小�?3.  input �?weight 不支�?nan、inf 等特殊值�?4.  由于存在临时内存使用，输入维度为二维时，TileShape大小有额外约束，假设TileShape为\[a,b\]，那么a\*b\*sizeof\(self\) + b/8 + 8KB < UB�?
## 调用示例
...
计算过程说明�?- �?通道�?2.0 < 0，结�?= 0.25 × (-2.0) = -0.5�?.5 �?0，结�?= 0.5
- �?通道�?.0 �?0，结�?= 1.0�?1.0 < 0，结�?= 0.5 × (-1.0) = -0.5
- �?通道�?3.0 < 0，结�?= 0.1 × (-3.0) = -0.3�?.0 �?0，结�?= 2.0
