...
�?self 的每个元素除�?other 中对应位置的元素并向上取整，计算公式如下�?
$$
...
| 参数�?| 输入/输出 | 说明                                                                 |
|--------|-----------|----------------------------------------------------------------------|
| self  | 输入      | 源操作数�?<br> 支持的类型为：Tensor�?<br> Tensor支持的数据类型为：DT_INT32�?<br> 不支持空Tensor；Shape仅支�?-4维，并支持按照单个维度广播到相同形状；Shape Size不大�?147483647（即INT32_MAX）�?|
...
1.  input �?other 类型应该相同�?2. 只支持单轴广播�?
## 调用示例
