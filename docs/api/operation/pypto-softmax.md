...
| 参数�? | 输入/输出 | 说明                                                                 |
|---------|-----------|----------------------------------------------------------------------|
| input   | 输入      | 源操作数�?<br> 支持的数据类型为：DT_FP32 <br> 不支持空Tensor；Shape大小不大�?147483647（即INT32_MAX）�?|
| dim     | 输入      | 指定归一化的维度�?<br> 支持负索引（�?-1 表示最后一个维度）�?<br> 需�?[-input.dim, input.dim-1] 范围内�?|

## 返回值说�?
返回Tensor类型。其Shape、数据类型与输入Tensor一致，dim指定维度上的元素之和�?1�?
## 调用示例
