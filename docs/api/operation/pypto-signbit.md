...
检查输入Tensor中每个元素的符号位（sign bit）是否被设置（即是否为负），逐元素运算。
...
返回Tensor类型。其Shape与输入Tensor一致，数据类型为DT_BOOL，其元素为True或False，表示输入Tensor对应元素的符号位是否被设置。

## 约束说明

1. TileShape与input维度保持一致；
2.  由于存在临时内存使用，当输入数据类型为DT\_FP32或DT\_BF16时，TileShape大小有额外约束，假设TileShape为\[a,b,c,d\]，那么a\*b\*c\*d\*sizeof\(self\) + c\*d\*sizeof\(half\) <UB。
...
```python
输入数据x: [-5.0, -0.0, 0.0, 5.0]
输出数据y: [True, True, False, False]
```
