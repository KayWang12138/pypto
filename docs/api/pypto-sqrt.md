# pypto.sqrt<a name="ZH-CN_TOPIC_0000002503357993"></a>
...
计算输入Tensor中每个元素的平方根，逐元素运算，返回与输入形状相同的Tensor。

输入为负数时返回 NaN，输入为零时返回 Inf。
...
返回一个与输入形状相同、数据类型一致的Tensor，其元素为输入张量对应元素的平方根。
