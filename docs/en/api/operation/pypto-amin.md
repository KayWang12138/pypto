...
Computes the minimum value of a multi-dimensional tensor along a specified dimension.

The specified computation dimension (Reduce axis) is defined as the R axis, and the non-specified dimension (Normal axis) is defined as the A axis. As shown in the figures below, for a 2D matrix with shape \(2, 3\), computing the minimum along the first dimension yields \[1, 2, 3\]; computing the minimum along the second dimension yields \[1, 4\].

**Figure 1**  amin computation example along the first dimension
![](../figures/pypto.amin_1.png)

**Figure 2**  amin computation example along the last dimension
...
Example 1: If input shape is [m, n] and output is [m, 1], setting TileShape to [m1, n1] means m1 and n1 are used to tile the m and n axes respectively.
