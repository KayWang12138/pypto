...
Returns a tensor with the same shape as the input tensor and data type DT\_BOOL. If the element value at a given position in `input` is not equal to the corresponding element in `other`, the return value at that position is True; otherwise it is False.
