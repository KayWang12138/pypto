...
Creates a tensor of size `size` filled entirely with `0`. The data type is specified by `dtype`, with a default data type of `DT_FP32`.
...
Returns an output tensor whose data type is determined by `dtype`, shape is `size`, and all values are `0`.
...
# Example 2: Pass size as a tuple, explicitly specify dtype (DT_INT32)
