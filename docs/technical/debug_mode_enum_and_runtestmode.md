...
在 `DeviceLaunchOnceWithDeviceTensorData` 函数中，在 `RegisterKernelBin` 之前调用 RunTestMode：
...
int DeviceLauncher::DeviceLaunchOnceWithDeviceTensorData(...) {
    ...
    // RunTestMode should be called before RegisterKernelBin to avoid interference
...
**执行时机**: `RunTestMode` 在 Kernel 参数初始化完成后、`RegisterKernelBin` 之前执行，这样可以：
1. 避免与 NPU 设备资源注册产生冲突
2. 在 NPU 执行之前完成 AICPU 仿真
3. 确保仿真环境与 NPU 执行环境隔离
...
DeviceLaunchOnceWithDeviceTensorData()
  ├─> 初始化环境和 Kernel 参数
  ├─> DeviceInitTilingData() [初始化 Tiling 数据]
  ├─> DeviceInitKernelInOuts() [初始化输入输出]
  │
  ├─> RunTestMode() [AICPU 仿真 - 在 RegisterKernelBin 之前执行]
  ...
  │                   └─> 多线程 AICPU 仿真
  │
  ├─> DeviceRunner::RegisterKernelBin() [注册 Kernel Bin]
...
**注意**: `RunTestMode` 在 `RegisterKernelBin` 之前执行，以避免与 NPU 执行流程产生干扰。这样可以确保 AICPU 仿真在 NPU 设备资源注册之前完成。
...
flowchart TD
    ...
    SetConfig --> InitEnv[初始化环境和参数]
    InitEnv --> InitTiling[DeviceInitTilingData]
    InitTiling --> InitInOuts[DeviceInitKernelInOuts]
    InitInOuts --> CheckDebug1{runtime_debug_mode == 1?}
    
    CheckDebug1 -->|是| RunTest[RunTestMode<br/>AICPU 仿真]
    CheckDebug1 -->|否| RegisterBin[RegisterKernelBin]
    ...
    JoinThreads --> RegisterBin
    
    RegisterBin --> NPUExec[NPU 执行<br/>DynamicLaunch]
    NPUExec --> RunProfile[RunWithProfile]
    RunProfile --> CheckDebug2{runtime_debug_mode == 1?}
    CheckDebug2 -->|是| SyncProf[同步性能数据]
    SyncProf --> GenSwimlane[生成泳道图]
    CheckDebug2 -->|否| SkipProf[跳过性能分析]
    GenSwimlane --> End([结束])
    SkipProf --> End
...
6. **执行时机**: RunTestMode 在 `RegisterKernelBin` 之前执行，避免与 NPU 设备资源注册产生冲突，确保仿真环境与 NPU 执行环境隔离