...
| 环境类型 | 硬件要求                   | 运行模式                                     |
|:-----|:-----------------------|:-----------------------------------------|
| 真实环境 | 配备CPU及NPU硬件 | 支持在NPU上执行计算，也可以通过CPU仿真获取预估性能和计算精度对比 |
| 仿真环境 | 仅有CPU硬件      | 支持通过CPU仿真，获取预估性能和计算精度结果                    |
...
> PyPTO 支持两种仿真模式：
>
> - `性能仿真`：仅评估程序运行性能，无需 CANN 支持（系统中是否安装 CANN ，均可正常运行）。
> - `精度仿真`：模拟真实 NPU 的执行逻辑，获取运算结果，必须依赖 CANN 工具包。
>
> 因此：
> - 若仅编译和运行PyPTO`性能仿真`，可跳过本节。
> - 若需编译和运行`精度仿真`，或计划在**真实 NPU 环境**中编译运行PyPTO并使用其在NPU上执行计算的能力时，必须安装如下软件包。
