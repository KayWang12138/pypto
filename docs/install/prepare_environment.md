...
| 仿真环境 | 仅有CPU硬件              | 支持在CPU上执行计算，也可以通过CPU仿真获取预估性能                       |
...
若仅在**仿真环境**中编译和运行PyPTO并仅使用预估性能功能，可跳过本节。
在**真实环境**中编译运行PyPTO并使用其在NPU上执行计算的能力或在**仿真环境**中编译运行PyPTO并使用其在CPU上执行计算的能力，必须安装如下软件包：
