...
无论选择方式一、方式二还是方式三，都需要执行`pto-isa`获取流程。统一参考本节执行：
...
2. **获取pto-isa**
    ...
    方式二同样需要获取`pto-isa`。具体方法请参考方式一中的“获取pto-isa（方式1）”章节。
...
Docker安装相关内容已拆分为独立文档，请参考：

- [Docker环境部署](./docker_environment.md)

在阅读Docker文档前，请先确保已完成宿主机NPU驱动和固件安装。

方式三同样需要获取`pto-isa`。具体方法请参考方式一中的“获取pto-isa（方式1）”章节。
## 环境验证
