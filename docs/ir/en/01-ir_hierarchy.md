...
| **MakeTuple** | `fields_` | Create tuple from expression list |
...
| **ReturnStmt** | `values_` | Return values from function |
...
| **MemRefType** | `memory_space_`, `addr_`, `size_` | Memory reference type (standalone type node) |
...
### MemRef and MemRefType

**MemRefType** is a standalone type node used to represent memory reference types themselves.

**MemRef** is a struct that can be used as an optional field in TensorType/TileType to describe memory allocation for tensors/tiles.

#### MemRefType (Standalone Type Node)
...
#### MemRef (As TensorType/TileType Field)
...
| **Call/Access** | 3 | Call, MakeTuple, TupleGetItemExpr |
| **Operations** | 2 | Op, GlobalVar |
| **Statements** | 8 | AssignStmt, IfStmt, ForStmt, YieldStmt, ReturnStmt, EvalStmt, SeqStmts, OpStmts |
| **Types** | 7 | UnknownType, MemRefType, ScalarType, TensorType, TileType, TupleType, PipeType |
