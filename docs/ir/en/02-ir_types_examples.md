...
Standalone memory reference type node used to represent memory references as a type.

```python
# Create MemRefType
memref_type = ir.MemRefType()

# MemRefType is typically used in the type system to represent memory references
# Note: MemRef struct (not a type) is used as an optional field in TensorType/TileType
```

**Distinction**:
- **MemRefType**: Standalone type node, inherits from Type, used in the type system
- **MemRef**: Struct used as the `memref_` field in TensorType/TileType to describe concrete memory allocation
