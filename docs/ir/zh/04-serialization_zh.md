...
PyPTO IR 序列化提供了高效的基于 MessagePack 的序列化功能，具有以下特性：
- **指针共享保留**：同一对象仅序列化一次，引用关系正确恢复
- **往返相等性**：`deserialize(serialize(node))` 与原始节点结构相等
- **可扩展性**：字段访问者模式便于扩展
- **调试信息**：保留 Span（源代码位置）
...
| 组件 | 职责 |
|-----------|----------------|
| **IRSerializer** | 将 IR 序列化为 MessagePack，在 `ptr_to_id_` 映射中跟踪指针 |
| **IRDeserializer** | 从 MessagePack 反序列化，维护 `id_to_ptr_` 用于指针重建 |
...
```
序列化：   IR Node → IRSerializer → FieldVisitor → MessagePack bytes
反序列化： MessagePack bytes → IRDeserializer → TypeRegistry → IR Node
```
...
2. **在 `type_deserializers.cpp` 中添加反序列化器**：
...
| 操作 | IR 大小 | 时间 | 吞吐量 |
|-----------|---------|------|------------|
| 序列化小表达式 | 10 个节点 | ~10 μs | 1M 节点/秒 |
| 序列化函数 | 100 个节点 | ~50 μs | 2M 节点/秒 |
| 序列化程序 | 1000 个节点 | ~500 μs | 2M 节点/秒 |
| 反序列化小表达式 | 10 个节点 | ~15 μs | 650K 节点/秒 |
| 反序列化函数 | 100 个节点 | ~80 μs | 1.25M 节点/秒 |
| 反序列化程序 | 1000 个节点 | ~800 μs | 1.25M 节点/秒 |

**复杂度：** 对于 N 个唯一节点为 O(N)。内存开销：引用表约为节点数的 2-3 倍。

**优化：**
- 通过 MessagePack 的零拷贝设计最小化拷贝
- 使用哈希映射实现 O(1) 指针查找
- 紧凑的二进制格式比 JSON 更小
...
| 错误 | 异常 | 上下文 |
|-------|-----------|---------|
| 数据损坏 | `DeserializationError` | 带错误消息 |
| 未知节点类型 | `TypeError` | 带类型名称 |
| 无效引用 | `DeserializationError` | 缺失的 ID |
| 文件 I/O 错误 | `std::runtime_error` | 带文件路径 |
...
**问：序列化是否保留指针标识？**
答：在单次序列化中保留。在不同的序列化调用之间，指针是独立的。

**问：我可以序列化部分 IR 图吗？**
答：可以，序列化任何 IR 节点。所有引用的节点会自动包含。

**问：MemRef 和 TileView 总是被序列化吗？**
...
- [IR 概述](00-ir_overview_zh.md) - IR 节点结构和语义
- [结构比较](03-structural_comparison_zh.md) - 哈希和相等性语义
