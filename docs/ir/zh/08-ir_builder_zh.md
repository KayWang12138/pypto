...
IR Builder 提供了一个便捷的 API，用于使用上下文管理器（Python）或 Begin/End 模式（C++）增量构建 PyPTO IR。它管理上下文栈、验证构造并跟踪源位置。
...
- **显式 Span 参数**（C++）：所有方法都接受 span 参数
- **验证**：检查正确的上下文使用和结构
- **嵌套构造**：支持函数中的循环、循环中的 if 等

## Python API

使用上下文管理器（`with` 语句）提供简洁的接口。
...
| 方法 | 描述 | 示例 |
|--------|-------------|---------|
| `output(index=0)` | 通过索引获取单个返回变量 | `result = loop.output()` |
| `outputs()` | 以列表形式获取所有返回变量 | `sum_result, prod_result = loop.outputs()` |
...
with ib.function("add_and_return") as f:
    ...
    add_expr = ir.Add(x, y, DataType.INT64, ir.Span.unknown())
    ib.return_stmt(add_expr)  # 单个值
    # ib.return_stmt([x, y])  # 多个值
    # ib.return_stmt()        # 空返回
...
**与 Python API 的主要区别：**
- 使用 `BeginFunction`/`EndFunction` 而不是 `with` 语句
...
| 规则 | 描述 |
|------|-------------|
| **不允许嵌套函数** | 不能在另一个函数内部调用 `BeginFunction` |
| **上下文匹配** | 必须使用正确的 End 方法结束上下文 |
| **迭代参数匹配返回变量** | For 循环必须有相等数量的迭代参数和返回变量 |
| **正确的嵌套** | 循环/if 必须在函数或有效上下文内部 |

### 错误消息

```python
with ib.function("outer") as f:
    with ib.function("inner") as f2:  # 错误！
        pass
...
```python
ib.in_function()  # 如果在函数上下文内则返回 True
ib.in_loop()      # 如果在 for 循环上下文内则返回 True
ib.in_if()        # 如果在 if 语句上下文内则返回 True
```

```cpp
ib.InFunction()  // 如果在函数内则返回 true
ib.InLoop()      // 如果在循环内则返回 true
ib.InIf()        // 如果在 if 内则返回 true
```

## 类型创建辅助方法（Python）

用于创建带内存引用和 tile 视图的类型的便捷方法。
...
# 完整的 tile，包含 memref 和 tile_view
...
1. **显式 Span**：所有 IR 节点都需要源位置。Python 自动捕获；C++ 需要显式参数。
2. **不可变 IR**：Builder 创建不可变的 IR 节点。
3. **渐进式构造**：逐语句增量构建 IR。
4. **上下文安全**：Builder 验证正确的嵌套和闭合。
5. **SSA 风格**：For 循环使用迭代参数实现 SSA 风格的循环携带值。

## 测试

参见 `tests/ut/ir/test_builder.py` 和 `tests/ut/ir/test_flash_attention_builder.py` 获取全面的示例。
...
- `python/pypto/ir/builder.py` - Python 包装器
