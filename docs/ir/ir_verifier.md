...
`TileOpShapeVisitor` 是一个继承自 `IRVisitor` 的访问者类，用于遍历 IR 并检查操作的形状兼容性。
...
2. **二元操作（BinaryOp）**：
   - 输入和输出的形状必须一致，或者
   - 仅有一个输入的形状与输出不一致，且该输入中只有维度为 1 的维度与输出不一致（支持广播）

3. **二元标量混合操作（BinaryScalarMixOp）**：
   - 输入的 Tile 形状必须与输出形状完全一致
