...
报错或打 ERROR 日志时带上错误码，日志中显示为 **Fxxxxx**（如 F72004）。枚举值即错误码，可直接转成数值使用。例如原日志：

```cpp
MACHINE_LOGE("xxx failed.");
```

应改为带错误码的写法（使用 `MACHINE_LOGE_E`，枚举值经掩码打印为 **Fxxxxx**）：

```cpp
#include "tilefwk/pypto_fwk_log.h"  // 已内含 machine/utils/machine_error.h

MACHINE_LOGE_E(SchedErr::HANDSHAKE_TIMEOUT, "hand shake timeout.");
```

效果
[ERROR] PYPTO(411657):2026-03-13 14:25:42.400 [host_machine.cpp:135][MACHINE]:ErrCode: F72006! hand shake timeout.