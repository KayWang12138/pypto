...
1. **开启日志与计算图 dump**：
   ...
   （3）打开计算图 dump（便于后续定位问题 tensor 在图中的位置）：
...
6. **影响 workspace 大小的关键配置项**：
   ...
   | `stitch_function_inner_memory` | rootInner | 当未设置 stitch_function_max_num 时的 fallback 配置 |
   | `stitch_function_outcast_memory` | devTaskInnerOutCasts | 当未设置 stitch_function_max_num 时的 fallback 配置 |
