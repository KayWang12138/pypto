...
### 排查步骤
1. **检查入参**：查阅对应算子或 API 文档（如 `/docs/api/operation/pypto-matmul.md`、算子说明），确认输入及输出Tesnor是否符合Matmul约束规格。

2. **检查切分设置**：调用Matmul算子前，会执行`set_cube_tile_shapes()`设置切分大小，检查设置的TileShape大小是否符合Tiling的切分约束。
