...
ValuePtr IRBuilder::DuplicateValue(const ValuePtr old) {
    switch (old->GetValueKind()) {
        case ValueKind::Tensor: {
            const auto tensor = std::static_pointer_cast<Tensor>(old);
            return std::make_shared<Tensor>(tensor->GetShape(), tensor->GetDataType(), 
                                            tensor->GetName(), tensor->GetFormat());
        }
        case ValueKind::Tile: {
            const auto tile = std::static_pointer_cast<Tile>(old);
            return std::make_shared<Tile>(tile->GetName(), tile->GetValidShape(),
                                          tile->GetShape(), tile->GetStrides(),
                                          tile->GetStartOffset(), tile->GetDataType(),
                                          tile->GetMemory());
        }
        case ValueKind::Scalar: {
            const auto scalar = std::static_pointer_cast<Scalar>(old);
            return std::make_shared<Scalar>(scalar->GetDataType(), scalar->GetName(),
                                            scalar->GetScalarValueKind(), scalar->GetSymbolicExpr(), 
                                            scalar->GetConstantValue());
        }
    }
    return nullptr;
}
