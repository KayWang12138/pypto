    ...
    if input_dim == mat2_dim == 2:
        if (extend_params is None) or (not extend_params):
            return pypto_impl.Matmul(
                out_dtype, input, mat2, a_trans, b_trans, c_matrix_nz, builder
            )
        else:
            extend_params_obj = pypto_impl.MatmulExtendParam(
                **convert_matmul_extend_params(extend_params)
            )
            return pypto_impl.Matmul(
                out_dtype, input, mat2, a_trans, b_trans, c_matrix_nz, extend_params_obj, builder
            )
