// PTO-IR prototype: statement-level IR structures.
...
// A function call within the statement dialect.
class CallStatement : public Statement {
public:
    explicit CallStatement(const std::string& callee, const ValuePtrs& args,
        const ValuePtrs& res)
        : callee_(std::move(callee)), arguments_(std::move(args)), results_(std::move(res)) {}

    StatementKind GetKind() const override { return StatementKind::Call; }

    const std::string& GetCallee() const { return callee_; }

    ValuePtrs& Arguments() { return arguments_; }
    const ValuePtrs& Arguments() const { return arguments_; }

    ValuePtrs& Results() { return results_; }
    const ValuePtrs& Results() const { return results_; }

    void Print(std::ostream& os, int indent) const override;

private:
    std::string callee_;
    ValuePtrs arguments_;
    ValuePtrs results_;
};

using CallStatementPtr = std::shared_ptr<CallStatement>;
