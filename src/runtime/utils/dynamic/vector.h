...
#include "runtime/utils/dynamic/allocator/allocators.h"
...
public:
    ...
    value_type &operator[](size_type idx) { DEV_DEBUG_ASSERT(idx < size_); return data()[idx]; }
    const value_type &operator[](size_type idx) const { DEV_DEBUG_ASSERT(idx < size_); return data()[idx]; }
