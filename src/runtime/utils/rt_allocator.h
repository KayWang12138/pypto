...
public:
    ...
    void SetBlockSizeHint(size_t blockSize) { blockSize_ = blockSize; }

    RtAllocation Allocate(size_t size) {
    ...
    void Deallocate(RtAllocation allocation) {
        BlockNode *node = allocation.node;
        node->refCnt--;
        if (node->refCnt == 0) {
            if (active_ == node) {
                active_ = node->next;
...
private:
    ...
    void AllocateNewNode(size_t size) {
        BlockNode *newNode = (BlockNode *)malloc(sizeof(BlockNode) + size);
        if (newNode != nullptr) {
...
private:
    ...
    size_t blockSize_{DEFAULT_BLOCKSIZE};
};
