...
#ifndef AC_ENABLE_FRAMEWORK_WITHOUT_CANN
    runtime::GetRA()->AllocDevAddr(&workSpaceAddr, static_cast<unsigned>(workSpaceSize));
...
#ifndef AC_ENABLE_FRAMEWORK_WITHOUT_CANN
    runtime::GetRA()->AllocDevAddr(&invokeEntyDev, static_cast<unsigned>(invokeOffsetVecSize));
    if (invokeEntyDev == nullptr) {
        std::cerr << "[DEVICE AGENT] Error: Failed to allocate memory for invokeEntyDev!" << std::endl;
        return MACHINE_ERROR;
    }
    runtime::GetRA()->AllocDevAddr(&invokeEntyDevOri, static_cast<unsigned>(invokeOffsetOriVec.size() * sizeof(uint64_t)));
    if (invokeEntyDevOri == nullptr) {
        std::cerr << "[DEVICE AGENT] Error: Failed to allocate memory for invokeEntyDev!" << std::endl;
        return MACHINE_ERROR;
    }
    size_t invokeTensorsInfoSize = task->compileInfo.coreTensorInfoVec.size() * sizeof(TensorInfo);
    runtime::GetRA()->AllocDevAddr(&invokeTensorsInfoDev, static_cast<unsigned>(invokeTensorsInfoSize));
...
int MachineAgent::PrepareTopo(DeviceAgentTask *task) {
    ...
    runtime::GetRA()->AllocDevAddr(&topoGmAddr, static_cast<unsigned>(allocSize));
    if (topoGmAddr == nullptr) {
        std::cerr << "[DEVICE AGENT] Error: Failed to allocate topo memory!" << std::endl;
        return MACHINE_ERROR;
    }
    runtime::GetRA()->CopyToDev(topoGmAddr, reinterpret_cast<uint8_t *>(cacheTopo), allocSize);
#endif
...
int MachineAgent::PrepareCoreFunctionBin(DeviceAgentTask *task) {
    ...
    runtime::GetRA()->AllocDevAddr(&binGmAddr, static_cast<unsigned>(allocSize));
    ...
    runtime::GetRA()->CopyToDev(binGmAddr, reinterpret_cast<uint8_t *>(cacheBin), allocSize);
#endif
...
int MachineAgent::PrepareReadyCoreFunction(DeviceAgentTask *task) {
...
#ifndef AC_ENABLE_FRAMEWORK_WITHOUT_CANN
    runtime::GetRA()->AllocDevAddr(&task->deviceInfo.readyAicQueElmGmAddr,
        static_cast<unsigned>(cacheValue.header.coreFunctionNum * sizeof(uint64_t)));
    runtime::GetRA()->AllocDevAddr(&task->deviceInfo.readyAivQueElmGmAddr,
        static_cast<unsigned>(cacheValue.header.coreFunctionNum * sizeof(uint64_t)));
    runtime::GetRA()->AllocDevAddr(&task->deviceInfo.readyAicpuQueElmGmAddr,
        static_cast<unsigned>(cacheValue.header.coreFunctionNum * sizeof(uint64_t)));
    runtime::GetRA()->AllocDevAddr(
    ...
    auto funcSetQue = [](uint8_t *elm, uint8_t *que, uint64_t elmCnt) {
        ...
        rq.lock = 0;
        runtime::GetRA()->CopyToDev(que, reinterpret_cast<uint8_t *>(&rq), sizeof(rq));
...
int MachineAgent::PrepareHcclContext(DeviceAgentTask *task) {
    ...
    for (uint32_t groupIndex = 0; groupIndex < task->compileInfo.commGroups.size(); ++groupIndex) {
        ...
        ret = HcclAllocComResourceByTiling(commHandle, runtime::GetRA()->GetStream(), &commConfig,
...
int MachineAgent::PrepareReadyState(DeviceAgentTask *task) {
    ...
    runtime::GetRA()->AllocDevAddr(&readyState, static_cast<unsigned>(allocSize));
...
#ifndef AC_ENABLE_FRAMEWORK_WITHOUT_CANN
    runtime::GetRA()->CopyToDev(readyState,
...
int MachineAgent::ConstructDeviceTask(DeviceAgentTask *task) {
    ...
    runtime::GetRA()->AllocDevAddr(&coreFuncWsGmAddr, static_cast<unsigned>(allocSize));
...
#ifndef AC_ENABLE_FRAMEWORK_WITHOUT_CANN
    runtime::GetRA()->AllocDevAddr(&deviceTaskGmAddr, static_cast<unsigned>(sizeof(DeviceTask)));
...
#ifndef AC_ENABLE_FRAMEWORK_WITHOUT_CANN
    runtime::GetRA()->CopyToDev(deviceTaskGmAddr, reinterpret_cast<uint8_t *>(&devTask), sizeof(DeviceTask));
#endif
...
void MachinePipe::PipeProc(DeviceAgentTask *task) {
    ...
    aclrtStream aicpuStream = task->aicpuStream_ == nullptr ? runtime::GetRA()->GetStreamAICPU() : task->aicpuStream_;
    if (task->IsAsync()) {
