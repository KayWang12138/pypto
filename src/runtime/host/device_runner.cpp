...
void *DeviceRunner::DevAlloc(int size) {
    ...
    if (rc != 0) {
        runtime::GetRA()->FreeTensor(devPtr);
...
int DeviceRunner::InitDeviceArgs(DeviceArgs &args) {
    ...
    // aicore info
    if (runtime::GetRA()->GetAicoreRegInfo(aic, aiv, ADDR_MAP_TYPE_REG_AIC_CTRL) != 0) {
        return -1;
    }

    // pmu info
    if (runtime::GetRA()->GetAicoreRegInfo(aicPmu, aivPmu, ADDR_MAP_TYPE_REG_AIC_PMU_CTRL) != 0) {
        return -1;
    }
...
int DeviceRunner::Init(void) {
    ...
    coreStream_ = runtime::GetRA()->GetStream();
    return 0;
}
} // namespace npu::tile_fwk
