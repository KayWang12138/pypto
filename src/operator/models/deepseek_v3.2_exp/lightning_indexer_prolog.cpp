...
Tensor LayerNorm(const Tensor &x, const Tensor &weight, const Tensor &bias, const int dim) {
    ...
    ASSERT(x->Datatype() == DT_FP32);
    constexpr float epsilon = 1e-6f;
    int actualDim = dim < 0 ? dim + x.GetShape().size() : dim;

    // do division first to avoid overflow
    auto xScaled = MulS(x, Element(DataType::DT_FP32, 1.0f / x->shape[actualDim]));
    auto mean = RowSumSingle(xScaled, -1);

    auto diff = Sub(x, mean);
    auto squaredDiff = Mul(diff, diff);
    auto squaredDiffScaled = MulS(squaredDiff, Element(DataType::DT_FP32, 1.0f / x->shape[actualDim]));
...
Tensor RotateHalfValidShape(const Tensor &input) {
    ...
    std::vector<SymbolicScalar> validShape = input->GetDynValidShape();
...
Tensor Rope3D(const Tensor &x, const Tensor &cos, const Tensor &sin, const RopeTileShapeConfig &tileConfig) {
    (void)tileConfig;
    ASSERT(x->shape.size() == SHAPE_DIM3 && cos->shape.size() == SHAPE_DIM2 && sin->shape.size() == SHAPE_DIM2);
    ...
    castCos = Reshape(castCos, {x->shape[NUM_VALUE_0], 1, x->shape[NUM_VALUE_2]});
    castSin = Reshape(castSin, {x->shape[NUM_VALUE_0], 1, x->shape[NUM_VALUE_2]});

    std::vector<SymbolicScalar> xValidShape = x->GetDynValidShape();
    auto xView = Reshape(castX, {x->shape[NUM_VALUE_0], x->shape[NUM_VALUE_1], x->shape[NUM_VALUE_2] / NUM_VALUE_2, NUM_VALUE_2},
    ...
    auto res = Cast(xEmbed, x->Datatype());
    return res;
}

Tensor Rope(const Tensor &x, const Tensor &cos, const Tensor &sin, const RopeTileShapeConfig &tileConfig) {
    (void)tileConfig;
    ASSERT(x->shape.size() == SHAPE_DIM2 && cos->shape.size() == SHAPE_DIM2 && sin->shape.size() == SHAPE_DIM2);

    auto seqSize = x->shape[NUM_VALUE_0];
...
void LightningIndexerPrologCompute(
    ...
    Tensor x2D(inputs.x->Datatype(), {b * seq, dim}, "x2D");
    ...
    LOOP("IndexerPrologLoop", FunctionType::DYNAMIC_LOOP, bsIdx, LoopRange(b * seq), unrollList) {
        for (int unrollLength : unrollList) {
            UNROLL(unrollLength) {
                ...
                auto q = Cast(Reshape(q32, {tileBS, headNum, headDim}), qrBlock->Datatype());
                ...
                auto weights = Matrix::Matmul<false, false>(inputs.x->Datatype(), xBlock, inputs.projW);
                ...
                k = Cast(LayerNorm(k, lnW2D, lnBias2D, -1), xBlock->Datatype()); // {tileBS, headDim}
