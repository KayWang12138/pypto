...
std::vector<int> FordBellman(int start, int wholeGraphTopologySize, const std::vector<std::pair<int,int>> &edges) {
    std::vector<int> subgrDepthVector(wholeGraphTopologySize, INT_MAX);
    subgrDepthVector[start] = 0;
...
void FillGraphTopologyInfoAndFindDepth(std::map<int, std::set<int>> &subgrChild, std::map<int, int> &subgrChildNum, std::map<int, int> &subgrParentNum, 
                                       std::vector<int> &subgrDepthVector, int wholeGraphTopologySize, Function &function) {
    // Fill graph topology childs, parents & create edges
    ...
    for (auto& subg: wholeGraphTopology) {
        for (auto child: subg.outGraph) {
            subgrChild[subg.esgId].insert(child); // Fill graph topology childs
            subgrParent[child].insert(subg.esgId); // Fill graph topology parents
            edges.push_back({child, subg.esgId}); // Create edges
        }
    }
    ...
    for (size_t i = 0; i < lastVertices.size(); i++) {
        auto d = FordBellman(lastVertices[i], wholeGraphTopologySize, edges);
        for (size_t idx = 0; idx < subgrDepthVector.size(); idx++) {
            subgrDepthVector[idx] = std::max(subgrDepthVector[idx], -d[idx]);
        }
    }
    edges.clear();
    lastVertices.clear();
...
void DefineLevelsAndPriorities(std::map<int, std::set<int>> &subgrChild, std::map<int, int> &subgrChildNum, std::map<int, int> &subgrParentNum, std::vector<int> &subgrDepthVector,
                               std::map<int, std::vector<std::pair<int, std::pair<int, int>>>> &levels, std::map<int, std::vector<std::pair<int, int>>> &priorities, int &levelSize) {
    ...
    for (auto& [level, instance]: levels) {
        ...
        int prior = 0;
        for (auto& priorTmp: priorities[level - 1]) {
            for (auto &elem: instance) {
                if ((subgrChild[elem.first].find(priorTmp.first) != subgrChild[elem.first].end())) {
                    levelsTmp[level].push_back(std::make_pair(elem.first, std::make_pair(subgrChildNum[elem.first], subgrParentNum[elem.first])));
                    subgrChild.erase(elem.first);
                }
            }
            std::sort(levelsTmp[level].begin(), levelsTmp[level].end(), [](const std::pair<int, std::pair<int, int>> &x, const std::pair<int, std::pair<int, int>> &y) {return x.second.second > y.second.second;}); // Sort by parents
            std::sort(levelsTmp[level].begin(), levelsTmp[level].end(), [](const std::pair<int, std::pair<int, int>> &x, const std::pair<int, std::pair<int, int>> &y) {return x.second.first > y.second.first;}); // Sort by childs
            
            for (auto &elemPrior: levelsTmp[level]) {
                priorities[level].push_back(std::make_pair(elemPrior.first, prior));
                prior++;
            }
            levelsTmp.clear();
        }
        std::sort(priorities[level].begin(), priorities[level].end(), [](const std::pair<int, int> &x, const std::pair<int, int> &y) {return x.second < y.second;}); // Sort by priorities
    }
    subgrChild.clear();
    subgrChildNum.clear();
    levels.clear();
...
void PriorScheduling::PriorSchedulingFunc(Function &function) const {
    ...
    // Call functions
    FillGraphTopologyInfoAndFindDepth(subgrChild, subgrChildNum, subgrParentNum, subgrDepthVector, wholeGraphTopologySize, function);
    DefineLevelsAndPriorities(subgrChild, subgrChildNum, subgrParentNum, subgrDepthVector, levels, priorities, levelSize);
