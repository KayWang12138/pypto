    ...
    for (size_t i = 0; i < input.size(); ++i) {
        ...
        while (curVal > 1) {
            if (alignedShape[alignIdx] == 0) {
                APASS_LOG_ERROR_F("SplitReshape", "Tensor", "Incorrect alignedShape, alignedShape[%d] = 0; Please check whether the alignshape generated by the ShapeAlign function is corrent.", alignIdx);
                return FAILED;
            }
...
Status SplitReshape::UpdateDynShape(const std::shared_ptr<ReshapeOp> &reshapeOp, const std::vector<int64_t> &offset, const std::vector<SymbolicScalar> &dynShape) {
    ...
    if (curOffset.size() != dynShape.size()) {
        APASS_LOG_ERROR_F("SplitReshape", "Tensor", "dynShape = %s, curOffset = %s, the dim of curOffset does not equal to dynShape!", GetStr(dynShape).c_str(), GetStr(curOffset).c_str());
        return FAILED;
    }
...
Status SplitReshape::GroupReshapeOffset(const std::shared_ptr<ReshapeOp> &isAddReshapeop, const std::vector<int64_t> &offset) {
    ...
    if (curStartIdx.size() != upperleftIdx.size()) {
        APASS_LOG_ERROR_F("SplitReshape", "Tensor", "curStartIdx = %s, offset = %s, the dim of curStartIdx does not equal to upperleftIdx; Please check the dim of the curStartIdx and upperleftIdx.", GetStr(curStartIdx).c_str(), GetStr(offset).c_str());
        return FAILED;
    }
    ...
    for (auto &op : function.Operations()) {
        if (op.GetOpcode() == Opcode::OP_RESHAPE) {
            ...
            if (input == nullptr || output == nullptr || output->GetRawTensor() == nullptr) {
                APASS_LOG_ERROR_F("SplitReshape", "Operation", "input == nullptr || output == nullptr || output->GetRawTensor() == nullptr, reshape op is [%d]; Please check the input or output.", op.opmagic);
                return FAILED;
            }
        ...
        if (op.GetOpcode() == Opcode::OP_ASSEMBLE) { // output应该是reshape的input
            ...
            if (input == nullptr || output == nullptr || input->GetRawTensor() == nullptr || output->GetRawTensor() == nullptr) {
                APASS_LOG_ERROR_F("SplitReshape", "Operation", "input == nullptr || output == nullptr || input->GetRawTensor() == nullptr || output->GetRawTensor() == nullptr, assemble op is [%d]; Please check the input or output.", op.opmagic);
                return FAILED;
            }
...
Status SplitReshape::ShapeAlign(
    ...
    while (i1 < shape1.size() || i2 < shape2.size()) {
        ...
        if ((i1 >= shape1.size() || i2 >= shape2.size())) {
            APASS_LOG_ERROR_F("SplitReshape", "Tensor", "i1 >= shape1.size() || i2 >= shape2.size(); Please check the process of compute alignShape.");
            return FAILED;
        }
...
Status SplitReshape::UpdateShapeOffset(UpdatePara &para, bool &flag, int &currentShape, int &currentOffset) {
    ...
    if (flag && currentShape <= stride) {
        ...
        if (currentOffset + currentShape > stride) {
            APASS_LOG_WARN_F("SplitReshape", "Tensor", "Found tail cond, currentOffset = %d, currentShape = %d, stride = %d.", currentOffset, currentShape, stride);
            return WARNING;
        }
        return SUCCESS;
    }
    if (currentOffset % stride != 0) {
        APASS_LOG_WARN_F("SplitReshape", "Tensor", "Found tail cond, currentOffset = %d, stride = %d.", currentOffset, stride);
        return WARNING;
    }
    if (currentShape % stride != 0) {
        APASS_LOG_WARN_F("SplitReshape", "Tensor", "Found tail cond, currentShape = %d, stride = %d.", currentShape, stride);
        return WARNING;
    }
    para.OffsetVal = currentOffset / stride;
...
Status SplitReshape::ConstructShapeOffset(const ReshapeTilePara &shapePara, size_t &i, size_t j,
    ...
    while (stride > 1) {
        if (stride % alignedShape[i] != 0) {
            APASS_LOG_WARN_F("SplitReshape", "Tensor", "Cannot cut shape, Stride = %d, alignedShape[%zu] = %d.", stride, i, alignedShape[i]);
            return WARNING;
        }
...
Status SplitReshape::RawToAlign(
    ...
    for (size_t j = 0UL; j < shape.size(); j++) {
        if (shape[j] == 1) {
            if (tileOffset[j] != 0 || tileShape[j] != 1) {
                APASS_LOG_ERROR_F("SplitReshape", "Tensor", "shape[%zu] = 1, but tileOffset[%zu] = %d (should be 0), tileShape[%zu] = %d (should be 1); The tileshape is incorrect.", j, j, tileOffset[j], j, tileShape[j]);
                return FAILED;
            }
            continue;
        }
    ...
    if (i != alignedShape.size()) {
        APASS_LOG_WARN_F("SplitReshape", "Tensor", "i = %zu, alignedShape.size() = %zu, cannot apply splitting for current tiling case.", i, alignedShape.size());
        return WARNING;
    }
    return SUCCESS;
}
...
Status SplitReshape::AlignToRaw(
    ...
    for (size_t j = 0UL; j < newRawshape.size(); j++) {
        ...
        while (stride > 1) {
            if (rawShape[i] == 0) {
                APASS_LOG_ERROR_F("SplitReshape", "Tensor", "Incorrect rawShape, rawShape[%d] = 0; Please set the correct rawShape.", i);
                return FAILED;
            }
            if (stride % rawShape[i] != 0) {
                APASS_LOG_ERROR_F("SplitReshape", "Tensor", "Incorrect alignment, stride % rawShape[i] != 0; Please set the correct alignment.");
                return FAILED;
            }
            stride /= rawShape[i];
...
Status SplitReshape::ObtainCopyOutTile(Function &function, const copyOutTilePara &copyOutTile, LogicalTensors &overlaps, LogicalTensors &newOverlaps) {
    ...
    for (auto &copyOutSource : AssembleOutToInput[reshapeSource->GetRawTensor()->GetRawMagic()]) {
        ...
        if (ret == WARNING) {
            APASS_LOG_WARN_F("SplitReshape", "Operation", "Found RawToAlign warning case. %s", GetStr(CopyOutInfo).c_str());
            return WARNING;
        }
        if (ret == FAILED) {
            APASS_LOG_ERROR_F("SplitReshape", "Operation", "Run RawToAlign failed. %s", GetStr(CopyOutInfo).c_str());
            return FAILED;
        }
...
Status SplitReshape::ProcessOnetoOne(Function &function, Operation &op, const CalcOverlapPara &para) {
    ...
    if (AlignToRaw(reshapeInfo, reshapeTileOffset, reshapeTileShape) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "AlignToRaw failed. %s", GetStr(reshapeInfo).c_str());
        return FAILED;
    }
    if (reshapeTileShape != output->shape) {
        APASS_LOG_ERROR_F("SplitReshape", "Tensor", "reshapeTileShape[%s] != output->shape[%s]", GetStr(reshapeTileShape), GetStr(output->shape));
        return FAILED;
    }
...
Status SplitReshape::CalcTileInfo(const CalcOverlapPara &para, std::vector<int64_t> &newShape,
    ...
    if (AlignToRaw(newInfo, newOffset, newShape) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Failed to compute the tile of the raw input. %s", GetStr(newInfo).c_str());
        return FAILED;
    }
    ...
    if (AlignToRaw(reshapeTileInfo, reshapeTileOffset, reshapeTileShape) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Tensor", "Failed to compute the tile of the raw input of inputView. %s", GetStr(reshapeTileInfo).c_str());
        return FAILED;
    }
    return SUCCESS;
}
...
Status SplitReshape::ProcessOnetoMulti(Function &function, Operation &op, const CalcOverlapPara &para) {
    ...
    if (CalcTileInfo(para, newShape, newOffset, reshapeTileShape, reshapeTileOffset)) {
        APASS_LOG_WARN_F("SplitReshape", "Operation", "Process CalcTileInfo failed for view op[%d].", op.GetOpMagic());
        return FAILED;
    }
...
Status SplitReshape::ProcessMultitoOne(Function &function, Operation &op, const CalcOverlapPara &para) {
    ...
    if (AlignToRaw(newInfo, newReshapeSourceTileOffset, newReshapeSourceTileShape) != SUCCESS) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Failed to compute the tile of the raw input of reshapeSource. %s", GetStr(newInfo).c_str());
        return FAILED;
    }
...
Status SplitReshape::CheckValidOp(const CheckParam &para, CheckOutputParam &checkOutputParam) {
    ...
    if (dynStatus == FAILED) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Illegal dynamic info for the reshape op. input = %s, output = %s, dynOutput = %s", GetStr(checkOutputParam.reshapeSource->GetRawTensor()->GetRawShape()).c_str(), GetStr(input->GetRawTensor()->GetRawShape()).c_str(), GetStr(reshapeDynOutput[input->GetRawTensor()->GetRawMagic()]).c_str());
        return FAILED;
    }
    ...
    if (alignRet == WARNING) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Cannot process the cond. %s", GetStr(reshapeInfo).c_str());
        return WARNING;
    }
    if (alignRet == FAILED) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "Process RawToAlign failed. %s", GetStr(reshapeInfo).c_str());
        return FAILED;
    }
    return SUCCESS;
}
...
Status SplitReshape::CheckOp(Function &function, Operation &op) {
    ...
    if (input == nullptr || output == nullptr || input->GetRawTensor() == nullptr) {
        APASS_LOG_ERROR_F("SplitReshape", "Operation", "input is null or output is null or input->GetRawTensor() is null, op[%d].", op.GetOpMagic());
        return FAILED;
    }
    auto viewOpAttribute = dynamic_cast<ViewOpAttribute *>(op.GetOpAttribute().get());
    if (viewOpAttribute == nullptr) {
...
Status SplitReshape::CheckCopyIn(Function &function) {
    for (auto &op : function.Operations()) {
        ...
        if (CheckOp(function, op) == FAILED) {
            APASS_LOG_WARN_F("SplitReshape", "Operation", "Run CheckOp failed.");
            return FAILED;
        }
    }
    return SUCCESS;
...
Status SplitReshape::AddAssembleOp(const MemoryType &memoryType, const std::vector<int64_t> &outputOffset, const LogicalTensorPtr &input, const LogicalTensorPtr &output) {
    ...
    if (outputOffset.empty() || outputOffset.size() != curReshapeOffset.size()) {
        APASS_LOG_ERROR_F("SplitReshape", "Tensor", "incorrect axis, toOffset = %s, curReshapeOffset = %s", GetStr(outputOffset).c_str(), GetStr(curReshapeOffset).c_str());
        return FAILED;
    }
...
Status SplitReshape::GetReshapeDynShape(const std::shared_ptr<ReshapeOp> &op, std::vector<SymbolicScalar> &dynValidShape) {
    ...
    for (const auto &validShape : op->dynValidShapes) {
        ...
        if (dynValidShape.size() != validShape.size() || upperleftIdx.size() != validShape.size()) {
            APASS_LOG_ERROR_F("SplitReshape", "Tensor", "Incorrect size! dynValidShape = %s, validShape = %s, upperleftIdx = %s.", GetStr(dynValidShape).c_str(), GetStr(validShape).c_str(), GetStr(upperleftIdx).c_str());
            return FAILED;
        }
        for (size_t i = 0; i < validShape.size(); ++i) {
