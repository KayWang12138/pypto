...
Status IntraSubgraphAdapter::RunOnFunction(Function &function) {
    ...
    for (size_t i = 0; i < boundaryTensors.size(); i++) {
        ...
        if (CheckBoundaryTensor(tensor) != SUCCESS) {
            APASS_LOG_ERROR_F(GetName().c_str(), "Tensor", "Check boundary tensor failed; Please check the CheckBoundaryTensor method.");
            return FAILED;
        }
        ...
        if (commonColors.size() > 1) {
            APASS_LOG_ERROR_F(GetName().c_str(), "Tensor", "Process boundary tensor failed, tensor magic : %d; The producers and consumers cannot simultaneously appear in more than one subgraph.", tensor->GetMagic());
            return FAILED;
        }
        if (commonColors.size() == 0) {
            if (ProcessBoundaryTensor(function, tensor) == FAILED) {
                APASS_LOG_ERROR_F(GetName().c_str(), "Tensor", "Process boundary tensor failed, tensor magic : %d; Please check the ProcessBoundaryTensor method.", tensor->GetMagic());
                return FAILED;
            }
        }
        if (commonColors.size() == 1) {
            ...
            APASS_LOG_DEBUG_F(GetName().c_str(), "Tensor", "********** %s requires SplitBoundaryTensor. **********", function.GetMagicName().c_str());
            ...
            if (SplitBoundaryTensor(function, tensor, mainSubgraphID, newBoundaryTensors) == FAILED) {
                APASS_LOG_ERROR_F(GetName().c_str(), "Tensor", "Split boundary tensor failed, tensor magic : %d; Please check SplitBoundaryTensor method.", tensor->GetMagic());
                return FAILED;
            }
            if (ProcessBoundaryTensors(function, newBoundaryTensors) == FAILED) {
                APASS_LOG_ERROR_F(GetName().c_str(), "Tensor", "Process boundary tensors failed; Please check ProcessBoundaryTensors method.");
                return FAILED;
            }
        }
    }
...
Status IntraSubgraphAdapter::CheckBoundaryTensor(LogicalTensorPtr tensor) {
    ...
    if (tensor->GetMemoryTypeOriginal() != tensor->GetMemoryTypeToBe()) {
        APASS_LOG_ERROR_F(GetName().c_str(), "Tensor", "There is a conflict in the memory type, tensor magic : %d, tensor original memory type : %s, tensor tobe memory type : %s; The original memory type and tobe memory type should be consistent.",
    ...
    if (validBoundaryTensorMemType.find(tensor->GetMemoryTypeOriginal()) == validBoundaryTensorMemType.end()) {
        APASS_LOG_ERROR_F(GetName().c_str(), "Tensor", "Original memory type error, tensor magic : %d, tensor memory type : %s; The original type of boundary tensor should be in [UB, L1, DDR, L0C].",
            tensor->GetMagic(), MemoryTypeToString(tensor->GetMemoryTypeOriginal()).c_str());
        return FAILED;
    }
    return SUCCESS;
...
Status IntraSubgraphAdapter::SplitBoundaryTensor(Function &function, LogicalTensorPtr tensor,
    ...
    for (auto& producer : tensor->GetProducers()) {
        if (producer->GetSubgraphID() != mainSubgraphID) {
            ...
            if (OpcodeManager::Inst().GetOpCalcType(producerOpcode) != OpCalcType::MOVE_OUT &&
                ...
                APASS_LOG_ERROR_F(GetName().c_str(), "Tensor", "OpCalcType error, boundary tensor magic : %d, producer op magic : %d, producer op : %s; If the tensor has multiple producers, then the producers can only be OP_ASSEMBLE/OP_COPY_OUT.",
            ...
            if (producerInputs.size() != 1) {
                APASS_LOG_ERROR_F(GetName().c_str(), "Tensor", "Producer input error, boundary tensor magic : %d, producer op magic : %d, producer op : %s; The OP_ASSEMBLE should have one input operand.",
                    tensor->GetMagic(), producer->GetOpMagic(), producer->GetOpcodeStr().c_str());
                return FAILED;
            }
            ...
            if (tensor->GetConsumers().size() != 1) {
                ...
                APASS_LOG_DEBUG_F(GetName().c_str(), "Tensor", "SplitBoundaryTensor output of %s[%d]", producer->GetOpcodeStr().c_str(), producer->GetOpMagic());
                ...
                APASS_LOG_INFO_F(GetName().c_str(), "Tensor", "Adjust OP_ASSEMBLE(magic : %d) to subgraph %d.", producer->GetOpMagic(), mainSubgraphID);
                // The intermediate tensor become a new boundary tensor.
                newBoundaryTensors.push_back(newTensor);
                APASS_LOG_INFO_F(GetName().c_str(), "Tensor", "Add new tensor(magic : %d) to boundary tensors.", assembleInput->GetMagic());
            }
        }
    }
    ...
    for (auto& consumer : tensor->GetConsumers()) {
        if (consumer->GetSubgraphID() != mainSubgraphID) {
            ...
            APASS_LOG_DEBUG_F(GetName().c_str(), "Operation", "Insert %s[%d] to the subsidiaryConsumers.", consumer->GetOpcodeStr().c_str(), consumer->GetOpMagic());
        }
    }
    ...
    if (subsidiaryConsumers.size() != 0) {
        APASS_LOG_DEBUG_F(GetName().c_str(), "Operation", "=========== multi subsidiaryConsumers size: %d ==========", subsidiaryConsumers.size());
...
Status IntraSubgraphAdapter::ProcessBoundaryTensor(Function &function, LogicalTensorPtr tensor) {
    APASS_LOG_INFO_F(GetName().c_str(), "Tensor", "Process boundary tensor, tensor magic : %d, info: %s, mem: %s",
    ...
    if (AdapteTensorProducers(function, tensor) == FAILED) {
        APASS_LOG_ERROR_F(GetName().c_str(), "Tensor", "Adapter tensor producer failed, tensor magic : %d; Please check the AdapteTensorProducers method.", tensor->GetMagic());
        return FAILED;
    }
    ...
    if (AdapteTensorConsumers(function, tensor) == FAILED) {
        APASS_LOG_ERROR_F(GetName().c_str(), "Tensor", "Adapter tensor consumer failed, tensor magic : %d; Please check the AdapteTensorConsumers method.", tensor->GetMagic());
        return FAILED;
    }
...
Status IntraSubgraphAdapter::AdapteTensorProducers(Function &function, LogicalTensorPtr tensor) {
    if (tensor->GetProducers().size() > 1) {
        for (Operation* producer : tensor->GetProducers()) {
            ...
            if (OpcodeManager::Inst().GetOpCalcType(producer->GetOpcode()) != OpCalcType::MOVE_OUT &&
                ...
                APASS_LOG_ERROR_F(GetName().c_str(), "Operation", "OpCalcType error; If the tensor has multiple producers, then the producers can only be OP_ASSEMBLE/OP_COPY_OUT.");
                return FAILED;
            }
        }
        return SUCCESS;
    ...
    if (tensor->GetProducers().size() == 1) {
        ...
        if (crossCoreMoveOps.find(producer->GetOpcode()) != crossCoreMoveOps.end()) {
            ...
            APASS_LOG_DEBUG_F(GetName().c_str(), "Operation", "change %s[%d] opcode to OP_COPY_OUT.", producer->GetOpcodeStr().c_str(), producer->GetOpMagic());
        }
...
Status IntraSubgraphAdapter::AdapteTensorConsumers(Function &function, LogicalTensorPtr tensor) {
    ...
    for (auto& consumer : tensor->GetConsumers()) {
        ...
        if (crossCoreMoveOps.find(consumer->GetOpcode()) != crossCoreMoveOps.end()) {
            ...
            APASS_LOG_DEBUG_F(GetName().c_str(), "Operation", "change %s[%d] opcode to OP_COPY_IN.", consumer->GetOpcodeStr().c_str(), consumer->GetOpMagic());
            continue;
        }
...
LogicalTensorPtr IntraSubgraphAdapter::InsertOpBetween(Function &function, Opcode opcode,
    ...
    APASS_LOG_DEBUG_F(GetName().c_str(), "Tensor", "Compare Ori: %s", tensor->Dump().c_str());
    APASS_LOG_DEBUG_F(GetName().c_str(), "Tensor", "Compare New: %s", newTensor->Dump().c_str());
    ...
    APASS_LOG_DEBUG_F(GetName().c_str(), "Operation", "Insert New Op %s[%d], info: %s", newOp->GetOpcodeStr().c_str(), newOp->GetOpMagic(), newOp->Dump().c_str());
...
LogicalTensorPtr IntraSubgraphAdapter::InsertOpBetween(Function &function, Opcode opcode,
    ...
    */
    APASS_LOG_DEBUG_F(GetName().c_str(), "Operation", "IntraSubgraphAdapter::InsertOpBetween %s 2.", OpcodeManager::Inst().GetOpcodeStr(opcode).c_str());
    if (ops.size() == 0) {
        APASS_LOG_ERROR_F(GetName().c_str(), "Operation", "Insert op between tensor and ops failed; The ops to be inserted can't be empty.");
        return nullptr;
    }
