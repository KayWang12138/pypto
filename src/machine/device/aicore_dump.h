...
struct DumpTensorData {
    ...
    DumpTensorData(npu::tile_fwk::TensorInfo *info, uint64_t tensorAddr) {
        ...
        for (uint32_t i = 0; i < info->dims; i++) {
            DEV_INFO(
                "Tensor infor is not null shape[%u]: %d, stride[%u] : %d\n", i, info->shape[i], i, info->stride[i]);
        ...
        DEV_DEBUG("Start to MemCpy the tensor to host , pragramtype is %d\n", info->paramType);
        TraverseAllAhapeIndexCombinations(info->shape, info->stride, info->dims - 1, tensorData);
    }

    int GetDumpSize() const {
        DEV_DEBUG("Tensorinfo size is %zu, Tensor data size %d\n", sizeof(dumpTensorInfo), datasize);
        return datasize;
    }
...
public:
    ...
    inline bool DumpData(const IDE_SESSION &ideSession, std::string &fileName, unsigned char *dataBuf,
        ...
        DEV_DEBUG("Finish ideDump with ideState is %d \n", ideState);
        return ideState == 0;
    }
    ...
    void DoDump(int64_t tensorInfo, int64_t tensorNum, int64_t tensorAddr, std::string iOinfo) {
        ...
        DEV_DEBUG("Current host deviceid is %u, devicedeviceId is %u\n", info->deviceId, deviceid);
        ...
        DEV_DEBUG("Current pid is %d, privateInfo is %s \n", (int)info->hostpid, privateInfo.c_str());
        ...
        for (int i = 0; i < tensorNum; i++) {
            ...
            DEV_DEBUG("Current info: magic is %u, datatype is %d, paramType is %d, idx is %u\n", info->rawMagic,
            ...
            if (iOinfo == "input" && (info->paramType == LOCAL_INCAST || info->paramType == DUMP_INCAST)) {
                DEV_DEBUG("Input File_name is %s, with paramType is %d \n", fileName.c_str(), info->paramType);
            ...
            if (iOinfo == "output" && (info->paramType == LOCAL_OUTCAST || info->paramType == DUMP_OUTCAST)) {
                DEV_DEBUG("Output File_name is %s, with paramType is %d \n", fileName.c_str(), info->paramType);
                Dump(ideSession, info, tmpTensorAddr, fileName, isLast);
            }
        }
        ...
        if (m != 0) {
            DEV_WARN("Close ideSession failed with state %d\n", m);
        }
    }
