...
- Basic usage of conditional statements with pypto.cond
- Nested loops with conditional statements
- Conditional computation based on loop indices
"""
...
@pypto.frontend.jit()
def nested_loops_with_conditions_kernel_npu(
...
def nested_loops_with_conditions_kernel_sim(
    a: pypto.Tensor(SHAPE, pypto.DT_FP32),
    b: pypto.Tensor(SHAPE, pypto.DT_FP32),
) -> pypto.Tensor(SHAPE, pypto.DT_FP32):
    pypto.set_vec_tile_shapes(2, 8)
...
def test_nested_loops_with_conditions(device_id=None, run_mode: str = "npu") -> None:
    ...
    if run_mode == 'npu':
        y = nested_loops_with_conditions_kernel_npu(a, b)
        pypto.runtime._device_synchronize()
    else:
        y = nested_loops_with_conditions_kernel_sim(a, b)
