...
def sum_op(a: torch.Tensor, dim: int, run_mode: str = "npu", keepdim: bool = False) -> torch.Tensor:
    ...
    if run_mode == "npu":
        ...
        @pypto.frontend.jit()
        def sum_kernel_npu(a: pypto.Tensor(SHAPE, DTYPE)) -> pypto.Tensor(out_shape, DTYPE):
        ...
        out = sum_kernel_npu(a)
    else:
        @pypto.frontend.jit(runtime_options={"run_mode": pypto.RunMode.SIM})
        def sum_kernel_sim(a: pypto.Tensor(SHAPE, DTYPE)) -> pypto.Tensor(out_shape, DTYPE):
            print(out_shape)
            tile_shapes = [8 for _ in range(len(a.shape))]
            pypto.set_vec_tile_shapes(*tile_shapes)
            out = pypto.sum(a, dim=dim, keepdim=keepdim)
            return out
        out = sum_kernel_sim(a)
    return out
...
def amax_op(a: torch.Tensor, dim: int, run_mode: str = "npu", keepdim: bool = False) -> torch.Tensor:
    ...
    if run_mode == "npu":
        ...
        def amax_kernel_npu(a :pypto.Tensor(SHAPE, DTYPE)) -> pypto.Tensor(out_shape, DTYPE):
            print(SHAPE, out_shape)
            tile_shapes = [8 for _ in range(len(a.shape))]
            pypto.set_vec_tile_shapes(*tile_shapes)
        ...
        out = amax_kernel_npu(a)
    else:
        @pypto.frontend.jit(
            host_options={"only_codegen": True},
            runtime_options={"run_mode": pypto.RunMode.SIM}
            )
        def amax_kernel_sim(a :pypto.Tensor(SHAPE, DTYPE)) -> pypto.Tensor(out_shape, DTYPE):
            print(SHAPE, out_shape)
            tile_shapes = [8 for _ in range(len(a.shape))]
            pypto.set_vec_tile_shapes(*tile_shapes)
            out = pypto.amax(a, dim=dim, keepdim=keepdim)
            return out
        out = amax_kernel_sim(a)
    return out
...
def amin_op(a: torch.Tensor, dim: int, run_mode: str = "npu", keepdim: bool = False) -> torch.Tensor:
    ...
    if run_mode == "npu":
        ...
        def amin_kernel_npu(a :pypto.Tensor(SHAPE, DTYPE)) -> pypto.Tensor(out_shape, DTYPE):
            print(SHAPE, out_shape)
            tile_shapes = [8 for _ in range(len(a.shape))]
            pypto.set_vec_tile_shapes(*tile_shapes)
        ...
        out = amin_kernel_npu(a)
    else:
        @pypto.frontend.jit(
            host_options={"only_codegen": True},
            runtime_options={"run_mode": pypto.RunMode.SIM}
        )
        def amin_kernel_sim(a :pypto.Tensor(SHAPE, DTYPE)) -> pypto.Tensor(out_shape, DTYPE):
            tile_shapes = [8 for _ in range(len(a.shape))]
            pypto.set_vec_tile_shapes(*tile_shapes)
            out = pypto.amin(a, dim=dim, keepdim=keepdim)
            return out
        out = amin_kernel_sim(a)
...
def maximum_op(a: torch.Tensor, b: torch.Tensor, run_mode: str = "npu") -> torch.Tensor:
    ...
    if run_mode == "npu":
        @pypto.frontend.jit
        def maximum_kernel_npu(a: pypto.Tensor(SHAPE1, DTYPE), b: pypto.Tensor(SHAPE2, DTYPE)) -> pypto.Tensor(SHAPE1, DTYPE):
            print(SHAPE1, SHAPE2)
            tile_shapes = [8 for _ in range(len(a.shape))]
            pypto.set_vec_tile_shapes(*tile_shapes)
            out = pypto.maximum(a, b)
            return out
        out = maximum_kernel_npu(a, b)
    else:
        @pypto.frontend.jit(runtime_options={"run_mode": pypto.RunMode.SIM})
        def maximum_kernel_sim(a: pypto.Tensor(SHAPE1, DTYPE), b: pypto.Tensor(SHAPE2, DTYPE)) -> pypto.Tensor(SHAPE1, DTYPE):
            print(SHAPE1, SHAPE2)
            tile_shapes = [8 for _ in range(len(a.shape))]
            pypto.set_vec_tile_shapes(*tile_shapes)
            out = pypto.maximum(a, b)
            return out
        out = maximum_kernel_sim(a, b)
    return out
...
def minimum_op(a: torch.Tensor, b: torch.Tensor,  run_mode: str = "npu") -> torch.Tensor:
    ...
    if run_mode == "npu":
        @pypto.frontend.jit
        def minimum_kernel_npu(a: pypto.Tensor(SHAPE, DTYPE), b: pypto.Tensor(SHAPE, DTYPE)) -> pypto.Tensor(SHAPE, DTYPE):
            print(SHAPE)
            tile_shapes = [8 for _ in range(len(a.shape))]
            pypto.set_vec_tile_shapes(*tile_shapes)
            out = pypto.minimum(a, b)
            return out
        out = minimum_kernel_npu(a, b)
    else:
        @pypto.frontend.jit( runtime_options={"run_mode": pypto.RunMode.SIM})
        def minimum_kernel_sim(a: pypto.Tensor(SHAPE, DTYPE), b: pypto.Tensor(SHAPE, DTYPE)) -> pypto.Tensor(SHAPE, DTYPE):
            print(SHAPE)
            tile_shapes = [8 for _ in range(len(a.shape))]
            pypto.set_vec_tile_shapes(*tile_shapes)
            out = pypto.minimum(a, b)
            return out
        out = minimum_kernel_sim(a, b)
    return out
