...
#T_DIM = pypto.frontend.dynamic("K_DIM") # b * s1
...
"""
query_nope.shape=[384, 512] query_nope.dtype=DataType.DT_BF16
query_rope.shape=[384, 64] query_rope.dtype=DataType.DT_BF16
key_nope_2d.shape=[2048, 512] key_nope_2d.dtype=DataType.DT_INT8
key_rope_2d.shape=[2048, 64] key_rope_2d.dtype=DataType.DT_BF16
k_nope_scales.shape=[2048, 4] k_nope_scales.dtype=DataType.DT_FP32
offsets.shape=[3, 2048] offsets.dtype=DataType.DT_INT32
kv_act_seqs.shape=[1] kv_act_seqs.dtype=DataType.DT_INT32
fattention_out.shape=[3, 128, 512] attention_out.dtype=DataType.DT_BF16
"""


def get_sfa_quant_func(
    nq, n_kv, softmax_scale, topk, tile_config):
    ...
    @pypto.frontend.jit()
    def sparse_flash_attention_quant_p_compute(
        query_nope: pypto.Tensor((Q_DIM, DN), pypto.DT_BF16),
        query_rope: pypto.Tensor((Q_DIM, DR), pypto.DT_BF16),
        key_nope_2d: pypto.Tensor((K_DIM ,DN), pypto.DT_INT8),
        key_rope_2d: pypto.Tensor((K_DIM, DR), pypto.DT_BF16),
        k_nope_scales: pypto.Tensor((K_DIM, 4), pypto.DT_FP32),
        offsets: pypto.Tensor((3, 2048), pypto.DT_INT32),
        kv_act_seqs: pypto.Tensor((1,), pypto.DT_INT32)
    ) -> pypto.Tensor([3, 128, KV_DIM], pypto.DT_BF16):
        print("Q_DIM, K_DIM, KV_DIM, DN, DR by closure: ", Q_DIM, K_DIM, KV_DIM, DN, DR)
        ...
        attention_out = pypto.tensor((3, 128, KV_DIM), dtype=pypto.DT_BF16)
        ...
        for t_idx in pypto.loop(0, t_sym, 1, name="LOOP_L0_t_idx", idx_name="bIdx"):
            batch_idx = t_idx // s1_sym
            slc_idx = t_idx % s1_sym
            ...
            for n_kv_idx in pypto.loop(0, n_kv_sym, 1, name="LOOP_L1_n_kv_SA", idx_name="n_kvIdx"):
                for group_idx in pypto.loop(0, g_loop_sym, 1, name="LOOP_L2_g_SA", idx_name="gIdx"):
                    ...
                    oi_offset = [t_idx, n_kv_idx * group + group_idx * cur_group_tile, 0]
                    for s2_idx in pypto.loop(0, bn_per_batch, 1, name="LOOP_L3_s2_SA", idx_name="s2_idx", unroll_list={1}):
                        cur_s2_tile = s2_tile
                        ...
                        offset_view = pypto.view(offsets, [1, cur_s2_tile], [t_idx, s2_idx * cur_s2_tile],
    ...
    return sparse_flash_attention_quant_p_compute
